\bentry
\wordRemoveSpace{V-v}{V v}
\pron{viV} 
\gl{\nA}
\expl{(\bava\ \eng{Vs} yA \eng{V's}).}
\bmng
\bnum
\num{1} iMgilxSf vaNaRmAleya ipapxtetxraDaneya akaSxra. 
\num{2} mara \mo vugaLa paTiTxgaLa \eng{V} AkArada joVDaNe. 
\num{3} \eng{V} AkArada vasutx. 
\num{4} roVmanf saMKAyx saMkeVtadalilx aidu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{V}{V}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{(V.} saha) \eng{volt(s).} 
\emng
\eentry

\bentry
\wordnospeech{v.}{v.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{verse.} 
\num{2} \eng{verso.} 
\num{3} \eng{versus.} 
\num{4} \eng{very.} 
\num{5} \engit{vide.} 
\enum
\emng
\eentry

\bentry
\wordnospeech{V.}{V.}
\pron{?}
\gl{\saMkeV}
\expl{}
\bmng
 (\ravi) \eng{vanadium.} 
\emng
\eentry

\bentry
\wordnospeech{VA}{VA}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} (\ame) \eng{Veterans' Administration.} 
\num{2} \eng{Vicar Apostolic.} 
\num{3} \eng{Vice Admiral.} 
\num{4} (\ame) \eng{Virginia} (aMce baLakeyalilx). 
\num{5} \UK yalilx \eng{(Order of Victoria and Albert)}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{Va.}{Va.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Virginia.} 
\emng
\eentry

\bentry
\word{vac}
\pron{vAYxkf} 
\gl{\nA}
\expl{}
\bmng
 (\birx)(\AmA) (\kanmu\ vishavxvidAyxnilayagaLa) \eng{vacation} (\saMkiSx). 
\emng
\eentry

\bentry
\word{vacancy} 
\pron{veVkanisx} 
\gl{\nA}
\expl{(\bava\ \eng{vacancies}).}
\bmng
\bnum
\num{1} baridAgiruvike; KAliyAgiruvike. 
\num{2} shUnayxmanasakxte; jaDamanasakxte; manasisxna niSikxrXya sithxti. 
\num{3} KAli hudedx yA sAthxna: \eng{he has no vacancy on his staff} Atana sibabxMdi vagaRdalilx yAva KAli sAthxnavU ilalx. \eng{no vacancies at present in this office} sadayxdalilx I kaceVriyalilx yAva hudedxgaLU KAli ilalx. 
\num{4} (hoVTelu \mo vugaLalilx) laBayxviruva KAli koVNe, rUmu. 
\enum
\emng
\eentry

\bentry
\word{vacant} 
\pron{veVkaMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} baridAgiruva; KAli(yAgiruva); BatiRyAgilalxda: \eng{the house is still vacant} A mane inUnx KAli ide. 
\num{2} biDuvAgiruva; yAva kelasavU ilalxde virAmavAgiruva: \eng{this will amuse your vacant hours} ninanx biDuvAda, virAmada, hotitxnalilx idu ninage vinoVdavanunx koDutatxde. 
\num{3} terapAgiruva; KAli iruva: \eng{he applied for a vacant post} avanu terapAgidadx, KAli idadx, oMdu hudedxgAgi ajiR hAkikoMDa. 
\num{4} shUnayxmanasakxnAda; vicAra, BAva -- shUnayx Ada; niSikxrXya, jaDa -- manasakxnAda: \eng{his mind seems completely vacant} avanu saMpUNaRvAgi shUnayxmanasakxnaMtidAdxne. \eng{a vacant stare} shUnayxdaqSiTx; gamanakekx tegedukoLaLxde neTaTx daqSiTxyiMda elolxV noVDutitxruvudu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vacant position}{vacant position} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\birx) KAli savxtutx sAvxdhiVna; hiMde vAsavAgidadxvaru biTuTxhoVgidadxriMda KAliyAda mane \mo vugaLanunx AkarxmisikoLuLxvudu, sAvxdhiVnapaDisikoLuLxvudu. 
\emng
\eentry

\bentry
\word{vacatable} 
\pron{va(veV)keVTarxbflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (jAga, mane, hudedx, \mo vanunx) KAli mADalAguva; terapu mADalAguva. 
\num{2} (hiMdina tiVpuR \mo vugaLa \vi) radudx mADabahudAda. 
\enum
\emng
\eentry

\bentry
\word{vacate} 
\pron{va(veV)keVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (mane, koVNe, \mo vanunx) KAlimADu; biTuTxhoVgu. 
\num{2} (sAthxna, hudedx, \mo vugaLa) adhikAravanunx, anuBoVgavanunx -- tore, tayxjisu, biDu. 
\num{3} (\nAyxshA) (hiMdina tiVpuR, opapxMda, \mo vanunx) radudxmADu. 
\enum
\emng
\eentry

\bentry
\word[vacation(1)]{vacation} 
\pron{vakeVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (mane, hudedx, \mo vanunx) terapu, KAli -- mADuvike. 
\num{2} (vishavxvidAyxnilayagaLu, nAyxyAlayagaLu, \mo vakekx vaSeRVvaSeRV koDuva) diVGARvadhi yA vASiRka raja: kirxsfmasf, IsaTxrf, \mo (diVGARvadhi) raja. 
\num{3} (\ame) diVGARvadhi raja yA aMtha rajAkAlada vihAra. 
\enum
\emng
\eentry
 
\bentry
\word[vacation(2)]{vacation} 
\pron{vakeVSanf} 
\gl{\akirx}
\expl{}
\bmng
 (\ame) rajAkAladalilx vihAra mADu, viharisu. 
\emng
\eentry

\bentry
\word{vacationer} 
\pron{vakeVSanarf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{vacationist}{vacationist}. 
\emng
\eentry

\bentry
\word{vacationist} 
\pron{vakeVSanisfTx} 
\gl{\nA}
\expl{}
\bmng
 (\ame) rajAkAlada vihAri. 
\emng
\eentry

\bentry
\wordnospeech{vacation land}{vacation land}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\ame) (rajAkAlada vihArAthiRgaLigAgiyeV anukUla kalipxsiruva) vihAraparxdeVsha; vihAradANa. 
\emng
\eentry

\bentry
\word{vaccinal} 
\pron{vAYxkisxnalf} 
\gl{\gu}
\expl{}
\bmng
 vAYxkisxVnina yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vaccinate} 
\pron{vAYxkisxneVTf} 
\gl{\sakirx}
\expl{}
\bmng
 siDubau, deVvi, dApu -- hAku; yAvudeV roVgakekx vinAyiti uMTu mADalu saMbaMdhapaTaTx vAyxkisxVnanunx cucicx oLaseVrisu; cucucxmadudx hAku. 
\emng
\eentry

\bentry
\word{vaccination} 
\pron{vAYxkisxneVSanf} 
\gl{\nA}
\expl{}
\bmng
 vAyxkisxVnf, siDubau, deVvi -- cucicxke; cucucxmadudx hAkuvudu. 
\emng
\eentry

\bentry
\word{vaccinator} 
\pron{vAYxkisxneVTarf} 
\gl{\nA}
\expl{}
\bmng
 deVvi, siDubau -- hAkuvava; cucucxmadudxgAra. 
\emng
\eentry

\bentry
\word[vaccine(1)]{vaccine} 
\pron{vAYxkisxVnf} 
\gl{\gu}
\expl{}
\bmng
 danasiDubige yA cucucxmadidxge, vAyxkisxVnige saMbaMdhisida. 
\emng
\eentry

\bentry 
\word[vaccine(2)]{vaccine} 
\pron{vAYxkisxVnf} 
\gl{\nA}
\expl{}
\bmng
vAyxkisxVnu; lasike: 
\banum
\alnum{a} rakatxdalilx parxtikAyagaLu utapxtitxyAgalu parxcoVdane niVDabalalx padAthaR, \sA\ shakitxguMdisida roVgakAraka sUkaSx mxjiVvi. 
\alnum{b} (\ca) siDubige vinAyiti uMTumADalu vAyxkisxVnf Agi baLasutitxdadx, danasiDubau veYrasf. 
\eanum
\emng
\eentry

\bentry
\word{vaccinia} 
\pron{vAYxkisxnia} 
\gl{\nA}
\bmng
 (\veYshA) siDubige parihAravAgi baLasuva siDubau veYrasf. 
\emng
\eentry

\bentry
\word{vacillate} 
\pron{vAYxsileVTf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (atatxliMditatx yA pakakxdiMda pakakxkekx, A kaDeyiMda I kaDege) OlADu; UgalADu; tonedADu; hoyAdxDu. 
\num{2} (aBipArxya, saMkalapx, \mo vugaLalilx) hoyAdxDutitxru; tUgADutitxru; uyAyxleyADutitxru; DoVlAyamAnavAgiru; anishicxtavAgiru. 
\enum
\emng
\eentry

\bentry
\word{vacillation} 
\pron{vAYxsileVSanf} 
\gl{\nA}
\expl{}
\bmng
 (aBipArxya, saMkalapx, \mo vugaLa \vi) hoyAdxTa; DoVlAyamAna sithxti; hiMdumuMdu noVDutitxruvike; asithxrate; caMcalamanasakxte; anishicxtate. 
\emng
\eentry

\bentry
\word{vacillator} 
\pron{vAYxsileVTarf} 
\gl{\nA}
\expl{}
\bmng
 (nidhARra, saMkalapx, \mo vanunx tegedukoLaLxde) hiMdu muMdu noVDuvava; DoVlAyamAna manasakx. 
\emng
\eentry

\bentry
\word{vacua} 
\pron{vAYxkuyxa} 
\gl{\nA}
\expl{}
\bmng
 \eng{vacuum} padada bahuvacana. 
\emng
\eentry

\bentry
\word{vacuity} 
\pron{vakUyxiTi}
\gl{\nA}
\bmng
\bnum
\num{1} shUnayx sithxti; shUnayxte; KAliyAgiruvike; yAva vasutxvU ilalxde baridAgiruva sithxti. 
\num{2} baudidhxshUnayxte; BAvashUnayxte; vicArahiVnate. 
\num{3} shUnayxparxdeVsha; KAlijAga. 
\enum
\emng
\eentry

\bentry
\word{vacuolar} 
\pron{vAYxkuyxalarf} 
\gl{\gu}
\expl{}
\bmng
 kuharada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vacuolation} 
\pron{vAYxkuyxOleVSanf} 
\gl{\nA}
\expl{}
\bmng
 kuhariVkaraNa; kuharagaLuMTAguvike. 
\emng
\eentry

\bentry
\word{vacuole} 
\pron{vAYxkuyxOlf} 
\gl{\nA}
\expl{}
\bmng
 (\jiVvi) kuhara; jiVvakoVshada koVshadarxvadalilx vAyu, darxva yA AhArakaNa, \mo vugaLiruva ati cikakx guLeLx, poTare. 
\emng
\eentry

\bentry
\word{vacuous} 
\pron{vAYxkuyxasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (Gana, darxva, anila -- darxvayxvilalxde) baridAda; KAliyAda; shUnayxvAda. 
\num{2} (oLagaNa darxvayxvanenxlalx tegeduhAki) baridumADida; shUnayxgoLisida; KAlimADida; terapugoLisida. 
\num{3} baudidhxshUnayx yA BAvashUnayx: \eng{a vacuous remark} baudidhxshUnayxvAda yA athaRhiVnavAda -- mAtu, TiVke. \eng{a vacuous stare} shUnayxdaqSiTx. 
\enum
\emng
\eentry

\bentry
\word{vacuously} 
\pron{vAYxkuyxasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 shUnayxdaqSiTxyiMda; baudidhx yA BAva shUnayxvAda noVTadiMda: \eng{he stared at me vacuously} avanu nananx kaDe shUnayxdaqSiTxyiMda noVDutitxdadx. 
\emng
\eentry

\bentry
\word{vacuousness} 
\pron{vAYxkuyxasfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vacuity}{vacuity \((1 \& 2)\).} 
\emng
\eentry

\bentry
\word[vacuum(1)]{vacuum} 
\pron{vAYxkuyxamf} 
\gl{\nA}
\expl{(\bava\ \eng{vacua} \ucAcx\ vAYxkuyxa yA \eng{vacuums}.) }
\bmng
\bnum
\num{1} shUnayxparxdeVsha; yAvudeV vasutxvilalxda parxdeVsha. 
\num{2} nivARta -- pAterx, parxdeVsha; vAyuvanunx hecucxkaDime saMpUNaRvAgi tegaduhAkibiTiTxruva pAterx yA parxdeVsha: \eng{Torricellian vacuum} TAriceliyanf nivARtaparxdeVsha. 
\num{3} vAyuviraLate; vAyuvina otatxDa yA saMmadaR sAmAnayxvAgi irabeVkAda parxmANakikxMta kaDimeyAgiruvudu. 
\num{4} aBAva; rAhitayx; yAvudeV sathxLa, saninxveVsha, \mo vugaLalilx irabeVkAda vayxkitx, vasutx, caTuvaTike, \mo vu ilalxdiruvudu. 
\num{5} (\AmA) (\bava\ \eng{vacuums}) = \hyperlink{vacuum-cleaner}{vacuum-cleaner}. 
\enum
\emng
\eentry

\bentry
\word[vacuum(2)]{vacuum} 
\pron{vAYxkuyxamf} 
\gl{\sakirx}
\expl{}
\bmng
 vAYxkUyxmf kilxVnaranunx baLasu; nivARyu mAjaRkadiMda dhULu \mo vanunx kaLe (\akirx\ saha). 
\emng
\eentry

\bentry
\wordnospeech{vacuum brake}{vacuum brake}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 nivARyuroVdhaka; (reVcakadiMda vAyuvanunx tolagisi shUnayxparxdeVsha vanunxMTumADuvudara mUlaka taDetavanunxMTu mADuva) vAYxkUyxmf -- biri, berxVku. 
\emng
\eentry

\bentry
\word{vacuum-clean} 
\pron{vAYxkuyxamfkilxVnf} 
\gl{\sakirx}
\expl{}
\bmng
 nivARyu mAjaRkadiMda dhULu \mo vanunx kaLe hoVgalADisu, shucigoLisu. 
\emng
\eentry

\bentry
\word{vacuum-cleaner} 
\pron{vAYxkuyxamfkilxVnarf} 
\gl{\nA}
\expl{}
\bmng
 vAYxkUyxmf kilxVnaru; nivARyu mAjaRka; reVcakadiMda vAyuvanenxlalx tegedu shUnayxparxdeVshavanunxMTumADuvudara mUlaka, dhULu \mo vanunx kaLeyuva sAdhana. 
\emng
\eentry

\bentry
\wordnospeech{vacuum flask}{vacuum flask}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\birx) thamaRsf phAlxsukx; (eraDu BititxgaLa naDuve nivARyu parxdeVsha vanunxMTumADi, oLage tuMbiTaTx darxvada shAKavu hecucx kAla uLiyuvaMte nimiRsiruva) nivARyu pAterx, siVse. 
\emng
\eentry

\bentry
\word{vacuum-gauge} 
\pron{vAYxkuyxamfgeVjf} 
\gl{\nA}
\expl{}
\bmng
 (yAvudeV pAterxyiMda vAyuvanunx KAlimADida meVle uLida alapxsavxlapx vAyuvina otatxDada parxmANavanunx kaMDuhiDiyuva) nivARyu mApaka. 
\emng
\eentry

\bentry
\word{vacuum-packed} 
\pron{vAYxkuyxamfpAYxkfTx} 
\gl{\gu}
\expl{}
\bmng
 (yAvudeV pAterxya \vi) vAyuvanunx KAlimADida baLika BadarxvAgi moharu mADida. 
\emng
\eentry

\bentry
\wordnospeech{vacuum pump}{vacuum pump}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (nivARyu parxdeVshavanunxMTumADalu baLasuva) vAyxkUyxmfpaMpu; nivARyu paMpu. 
\emng
\eentry

\bentry
\wordnospeech{vacuum tube}{vacuum tube}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (viduyxtapxrXvAhavu sarAgavAgi hariyuvaMte mADalu, oLagina vAyuvanenxlalx hecucxkaDime pUtiRyAgi coVSisi, baLika moharumADida) nivARyu koLave; vAYxkUyxmf TUyxbau. 
\emng
\eentry

\bentry
\wordnospeech{VAD}{VAD}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Voluntary Aid Detachment.} 
\emng
\eentry

\bentry
\word{vade-mecum} 
\pron{vADimeVkamf, veVDimiVkamf} 
\gl{\nA}
\expl{}
\bmng
sadA joteyalelxV oyuyxva keYpiDi \mo\ yAvudeV vasutx. 
\emng
\eentry

\bentry
\word[vagabond(1)]{vagabond} 
\pron{vAYxgabAMDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} alemAriyAda; gotAtxda neleyilalxda; sithxravAda manemaThavilalxda; (sadA) aledADutitxruva. 
\num{2} poVkari; poVli(yAgi) tirugutitxruva. 
\enum
\emng
\eentry

\bentry
\word[vagabond(2)]{vagabond} 
\pron{vAYxgabAMDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (kelasa kAyaRvilalxde shudadhx niSapxrXyoVjakanAgi aMDaleyuva) alemAri; tiridADuvavanu. 
\num{2} (\AmA) apApoVli; poVkari; laphaMga. 
\enum
\emng
\eentry

\bentry
\word[vagabond(3)]{vagabond} 
\pron{vAYxgabAMDf} 
\gl{\akirx}
\expl{}
\bmng
 alemAriyaMte aleyutitxru; gotutxguriyilalxde tirugADutitxru. 
\emng
\eentry

\bentry
\word{vagabondage} 
\pron{vAYxgabAMDijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} alemAritana. 
\num{2} alemAri taMDa, vagaR. 
\enum
\emng
\eentry

\bentry
\word{vagabondish} 
\pron{vAYxgabAMDiSf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} alemAriyaMtha. 
\num{2} poVkariyaMtha; poVliyAda. 
\enum
\emng
\eentry

\bentry
\word{vagabondism} 
\pron{vAYxgabAMDisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{vagabondage}{vagabondage}. 
\num{2} apApoVlitana; laphaMgatana. 
\enum
\emng
\eentry

\bentry
\word{vagabondize} 
\pron{vAYxgabAMDeYsfZ} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} alemAriyAgi tirugu(titxru). 
\num{2} apApoVliyaMte vatiRsu yA sututxtitxru. 
\enum
\emng
\eentry

\bentry
\word{vagal} 
\pron{veVgalf} 
\gl{\gu}
\expl{}
\bmng
 (\aMrashA) veVgasf \eng{(vagus)} na yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vagarious} 
\pron{vageVriasf} 
\gl{\gu}
\expl{}
\bmng
 capalacitatxda; toVridaMte, manabaMdaMte naDeyuva. 
\emng
\eentry

\bentry
\word{vagary} 
\pron{veVgari} 
\gl{\nA}(\bava\ \eng{vagaries}). 
\bmng
\bnum
\num{1} vicitarx -- naDavaLike, vataRne. 
\num{2} kalapxne, BAvane, \mo vugaLa yadAvxtadAvx vataRne, tikakxlu; hucAcxTa: \eng{the vagaries of fortune} adaqSaTxda hucAcxTagaLu. 
\enum
\emng
\eentry

\bentry
\word{vagina} 
\pron{vajeYna} 
\gl{\nA}
\expl{(\bava\ \eng{vaginae} \ucAcx\ vajeYniV yA \eng{vaginas}). }
\bmng
\bnum
\num{1} yoVni(nALa); mAnavaralilx matutx itara sasatxnigaLalilx heNiNxna gaBaRkoVsha matutx jananAMgada horadAvxragaLa naDuve iruva nALa. 
\num{2} (\savi) vaqMtakavaca; eleya bauDadalilx kAMDada sutatx iruva kavaca, koVsha, hodike. 
\enum
\emng
\eentry

\bentry
\word{vaginal} 
\pron{vajeYnalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} yoVniya; yoVninALada. 
\num{2} (\savi) eleya vaqMtakavacada, vaqMtakoVshada. 
\enum
\emng
\eentry

\bentry
\word{vaginismus} 
\pron{vAYxjinisamxZsf} 
\gl{\nA}
\expl{}
\bmng
 (\veYshA) (idadxkikxdadxMte tuDiyutatx tuMba noVyuva) yoVni saMkoVcana; yoVnituDita. 
\emng
\eentry

\bentry
\word{vaginitis} 
\pron{vAYxjineYTisf} 
\gl{\nA}
\expl{}
\bmng
 (\veYshA) yoVninALada uriyUta. 
\emng
\eentry

\bentry
\word{vagrancy} 
\pron{veVgarxnisx} 
\gl{\nA}
\expl{}
\bmng
 alemAritana yA alemAri jiVvana. 
\emng
\eentry

\bentry
\word[vagrant(1)]{vagrant} 
\pron{veVgarxMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (gotutxguri ilalxde) aledADuva; sutAtxDutatxleV iruva; alemAriyAda: \eng{a vagrant tribe} oMdu alemAri bauDakaTuTx. 
\num{2} saMcAri; sadA saMcarisutatxleV iruva: \eng{a vagrant musician} obabx saMcAriV saMgiVtagAra. 
\num{3} dikAkxpAlAda; gotutxguri ilalxda; nilavu neleyilalxda: \eng{vagrant speculations} dikAkxpAlAda UhApoVhagaLu. 
\num{4} apApoVliyAda. 
\enum
\emng
\eentry

\bentry
\word[vagrant(2)]{vagrant} 
\pron{veVgarxMTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (manemaThavilalxda, kelasakAyaRvilalxda) alemAri; kelasa kAyaRvilalxde aleyutatxleV iruvavanu. 
\num{2} apApoVli; laphaMga. 
\enum
\emng
\eentry

\bentry
\word{vagrantly} 
\pron{veVgarxMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
 alemAriyAgi; alemAritanadiMda. 
\emng
\eentry

\bentry
\word{vague} 
\pron{veVgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asapxSaTx; sapxSaTxvAgi, nidiRSaTxvAgi -- heVLilalxda; asuPxTa: \eng{he returned only a vague answer} avanu EnoV oMdu asapxSaTxvAda utatxra koTaTx, aSeTx. \eng{I have not the vaguest notion of reasons} avana kAraNagaLeVnu eMba bagegx nanage tiVra asapxSaTxvAda BAvane kUDa ilalx. \eng{I heard a vague rumour to that effect} A athaR baruva yAvudoV oMdu asapxSaTx vadaMti nananx kivige bitutx. 
\num{2} (vayxkitxya yA manasisxna \vi) aniSakxqqSaTx; saMdigadhx; karAruvAkakxlalxda; Kacitavalalxda. 
\enum
\emng
\eentry

\bentry
\word{vaguely} 
\pron{veVgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} asapxSaTxvAgi; asuPxTavAgi. 
\num{2} aniSakxqqSaTxvAgi; saMdigadhxvAgi; karAruvAkAkxgilalxde. 
\enum
\emng
\eentry

\bentry
\word{vagueness} 
\pron{veVgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} asapxSaTxte; asuPxTate. 
\num{2} aniSakxqqSaTxte; saMdigadhxte; karAruvAkAkxgilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{vaguish} 
\pron{veVgiSf} 
\gl{\gu}
\expl{}
\bmng
 asapxSaTxvenisuva; aSeTxVnU sapxSaTxvalalxda. 
\emng
\eentry

\bentry
\word{vagus} 
\pron{veVgasf} 
\gl{\nA}
\expl{(\bava\ \eng{vagi} \ucAcx\ veVgeY). }
\bmng
 (\aMrashA) veVgasf; haqdaya, shAvxsakoVshagaLu, matitxtara oLAMgagaLige hoVguva shAKegaLuLaLx, hatatxneya kapAla nara joVDiyalilx oMdu. 
\emng
\eentry

\bentry
\word[vail(1)]{vail} 
\pron{veVlf} 
\gl{\sakirx}
\expl{}
\bmng
 (adhiVnateyanunx yA sharaNAgatiyanunx sUcisuva saluvAgi) ToVpi, hAYxTu, kiriVTa, turAyi, \mo\ shiroVBUSaNavanunx tege yA tagigxsu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 maNi; talebAgu; shiroVBUSaNavanunx tege. 
\emng
\eentry

\bentry
\word[vail(2)]{vail} 
\pron{veVlf} 
\gl{\nA}
\expl{(\pArxparx) (\sA\ \bava dalilx) }
\bmng
\bnum
\num{1} inAmu; bakiSxVsu. 
\num{2} meVlusaMpAdane; saMbaLakikxMta hecicxge baruva yAvudeV lABa, gaLike; (kelavomemx) laMca, ruSuvatutx. 
\enum
\emng
\eentry

\bentry
\word{vain} 
\pron{veVnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} poLuLx; ToLuLx; jaLuLx; oNa; shuSakx; huruLilalxda; tiruLilalxda; sAravilalxda: \eng{vain boasts} oNbaDAyigaLu. \eng{vain triumphs} huruLilalxda vijayagaLu. \eng{vain distinction} poLuLx parxtiSeThx. 
\num{2} vayxthaR; niSapxrXyoVjaka; niSaPxla; nirathaRka: \eng{vain hope} vayxthaRvAda Ashe. \eng{resistance is vain} parxtiBaTaneyu nirathaRka. \eng{it is vain to resist} parxtiBaTisuvudu niSaPxla. 
\num{3} (swMdayaR, sAmathaRyx, \mo vugaLa \vi) duraBimAnada; gavaRda; durahaMkArada: \eng{she is vain of her beauty} Akege tananx swMdayaRda bagegx duraBimAnavide. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{in vain} vayxthaRvAgi; nirathaRkavAgi; niSaPxlavAgi; niSapxrXyoVjakavAgi: \eng{it was in vain we protested} nAvu parxtiBaTisidudx vayxthaRvAyitu.\eng{we protested in vain} nAvu vayxthaRvAgi parxtiBaTisidevu. 
\num{2} \eng{take a person's name in vain} obabx vayxkitxya hesaranunx laGavAgi yA alakaSxyXdiMda baLasiko. 
\enum
\emng
\eentry

\bentry
\word{vainglorious} 
\pron{veVnfgolxVriasf} 
\gl{\gu}
\expl{}
\bmng
 oNajaMbada; baMDavALavilalxda baDAyiya; baDivArada. 
\emng
\eentry

\bentry
\word{vaingloriously} 
\pron{veVnfgolxVriasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 oNajaMbadiMda; baMDavALavilalxda baDAyiyiMda. 
\emng
\eentry

\bentry
\word{vaingloriousness} 
\pron{veVnfgolxVriasfnisf} 
\gl{\nA}
\expl{}
\bmng
 oNajaMba; baDAyi. 
\emng
\eentry

\bentry
\word{vainglory} 
\pron{veVnfgolxVri} 
\gl{\nA}
\expl{}
\bmng
 oNajaMba; baMDavALavilalxda baDAyi; baDivAra. 
\emng
\eentry

\bentry
\word{vainly} 
\pron{veVnfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vayxthaRvAgi; nirathaRkavAgi; niSaPxlavAgi; niSapxrXyoVjakavAgi. 
\num{2} gavaRdiMda; duraBimAnadiMda; durahaMkAradiMda; savxparxtiSeThxyiMda. 
\enum
\emng
\eentry

\bentry
\word{vainness} 
\pron{veVnfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vayxthaRte; nirathaRkate; niSaPxlate; niSapxrXyoVjakate. 
\num{2} gavaR; duraBimAna; durahaMkAra; savxparxtiSeThx. 
\enum
\emng
\eentry

\bentry
\word{vair} 
\pron{veVrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx\ yA \ca) (madhayx yugadalilx aMgi, niluvaMgi, \mo vakekx asatxriyAgiyoV, kutitxge, muMgeY, \mo vugaLa alaMkaraNakAkxgiyoV baLasutitxdadx) aLilina tupupxLu. 
\num{2} (\vaMlAM) tupu pxLina alaMkaraNavanunx sUcisalu, AkAshaniVli ilalxve beLiLxya baNaNxgaLu oMdAda meVloMdu baruvaMte, saNaNx gurANiya athavA gaMTeya AkArada citarxNa. 
\enum
\emng
\eentry

\bentry
\word{Vaishnava} 
\pron{veYSAnxvA} 
\gl{\nA}
\expl{}
\bmng
 (hiMdUdhamaR) veYSaNxva; viSuNx(vina) Bakatx. 
\emng
\eentry

\bentry
\word{Vaisya} 
\pron{veYsayx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (hiMdUdhamaRda) (nAlukx vaNaRgaLalilx mUraneyadAda) veYshayx (vaNaR, jAti). 
\num{2} veYshayx(vaNaRdavanu). 
\enum
\emng
\eentry

\bentry
\word{valance} 
\pron{vAYxlanfsx} 
\gl{\nA}
\expl{}
\bmng
 (maMcada capapxrada yA cwkaTiTxna sutatx, kiTakiya meVlABxgadalilx ilalxve kapATina keLaBAgadalilx hAkuva) moVTu parade; tuMDu parade. 
\emng
\eentry

\bentry
\word{valanced} 
\pron{vAYxlanfsxTf} 
\gl{\gu}
\expl{}
\bmng
moVTu parade hAkida. 
\emng
\eentry

\bentry
\word[vale(1)]{vale} 
\pron{veVlf} 
\gl{\nA}
\expl{}
\bmng
 (\pArxparx\ yA \kAparx) (sathxLanAmagaLanunx horatupaDisi) kaNive: \eng{over field and vale} meYdAna matutx kaNiveya meVle. \eng{Vale of Kashmir} kAshimxVrada kaNive. \nuga 
\bnum
\num{1} \eng{the vale of years} mupupx; vaqdAdhxpayx; vAdhaRkayx. 
\num{2} \eng{this} (\engit{or} \eng{the) vale of tears} (sAhitayxka) duHKada I kaNive; duHKamayavAda I loVka, I bALu, I baduku. 
\enum
\emng
\eentry

\bentry
\word[vale(2)]{vale} 
\pron{vAleV} 
\gl{\BAavayx}
\expl{}
\bmng
 (biVLokxDugeyalilx) deVvaru oLeLxyadanunx mADali! 
\emng
\eentry

\bentry
\word[vale(3)]{vale} 
\pron{vAleV} 
\gl{\nA}
\expl{}
\bmng
 (biVLokxDugeyalilx heVLuva) `deVvaru oLeLxyadanunx mADali' eMba shuBAshaya. 
\emng
\eentry

\bentry
\word{valediction} 
\pron{vAYxliDikfSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} biVLokxDuge; biVLokxLuLxvudu. 
\num{2} biVLokxDuvalilx heVLuva mAtu. 
\enum
\emng
\eentry

\bentry
\word{valedictorian} 
\pron{vAYxliDikoTxVrianf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) samAroVpa BASaNakAra; shAlAkAleVjugaLa atuyxnanxta taragatiya, atuyxnanxta sAthxna gaLisidadxriMda samAroVpada yA biVLokxDugeya BASaNa mADalu AyekxgoMDa hiriya vidAyxthiR. 
\emng
\eentry

\bentry 
\word[valedictory(1)]{valedictory} 
\pron{vAYxliDikaTxri}
\gl{\gu}
\expl{}
\bmng
 biVLokxDugeya; vidAya(da). 
\emng
\eentry

\bentry
\word[valedictory(2)]{valedictory} 
\pron{vAYxliDikaTxri} 
\gl{\nA}(\bava\ \eng{valedictories}). 
\bmng
 biVLokxDugeya yA samAroVpada BASaNa. 
\emng
\eentry

\bentry
\word[valence(1)]{valence} 
\pron{vAYxlanfsx} 
\gl{\nA}
\expl{}
\bmng
 \eng{valance} padada rUpAMtara. 
\emng
\eentry

\bentry
\word[valence(2)]{valence} 
\pron{veVlanfsx} 
\gl{\nA}
\expl{}
\bmng
 (\ravi) (\kanmu\ \ame) = \hyperlink{valency}{valency}. 
\emng
\eentry

\bentry 
\wordnospeech{valence electron}{valence electron}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\ravi) veVlenfsx ilekATxrXnf; rAsAyanika baMdhagaLanunx racisuvudaralilx BAgavahisuva, paramANuvina atayxMta horakavacadalilxna ilekATxrXnfgaLalilx oMdu. 
\emng
\eentry

\bentry
\word{Valenciennes} 
\pron{vAYxlAnisxenf} 
\gl{\nA}
\expl{}
\bmng
oMdu bageya utakxqqSaTx jaratAri. 
\emng
\eentry

\bentry
\word{valency} 
\pron{veVlanisx} 
\gl{\nA}
\expl{(\bava\ \eng{valencies}). }
\bmng
 (\birx) (\ravi) veVlenisx; (dhAtuvige saMbaMdhisidaMte) adara paramANu eSuTx bAri itara paramANugaLoMdige saMyoVgagoLaLxbalalxdeMbaudanunx sUcisuva saMKeyx. 
\emng
\eentry

\bentry
\word{valentine} 
\pron{vAYxlaneTxYnf} 
\gl{\nA}
\expl{}
\bmng
 vAyxlaMTeYnf: 
\banum
\alnum{a} saMta vAYxlaMTeYnana dina; pheZbarxvari \eng{14}raMdu roVmanf kAyxtholikaru Acarisuva, gaMDu heNuNx hakikxgaLu joVDigoLuLxtatxveyeMdu BAvisalAda, saMta vAYxlaMTeYnf hababx. 
\alnum{b} saMta vAYxlaMTeYnf hababxdaMdu varisida nalalx yA nalelx. 
\alnum{c} saMta vAYxlaMTeYnf hababxdaMdu puruSanige sitxrXV, sitxrXVge puruSa, kaLuhisuva anAmika yA parxNaya yA parxNaya viDaMbana patarx, citarx yA koDuge. 
\eanum
\emng
\eentry

\bentry
\word{valerian} 
\pron{valiarianf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kADu jaTAmAMsi; mAsalugeMpina yA biLiya hU biDuva, kaTugaMpina, bekukxgaLige pirxyavAda, oMdu giDa. 
\num{2} (shAmaka auSadhavAgi baLasuva) kADu jaTAmAMsiya beVru. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{common valerian} = \hyperlink{valerian}{valerian\((1)\)}. 
\emng
\eentry


\bentry
\wordRemoveSpace{valeric-acid}{valeric acid} 
\pron{vale(lia)rikf AYxsiDf} 
\gl{\nA}
\expl{}
\bmng
 (\ravi) valerikf Amalx; aidu kAbaRnf paramANugaLiruva payARpatx kAbARkisxlikf Amalx, sUtarx \eng{$\bg\rm C\eg_4\bg\rm H\eg_9\bg\rm COOH\eg$} (= \hyperref{kandict_p.pdf}{P}{pentanoic acid}{pentanoic acid}). 
\emng
\eentry

\bentry
\word[valet(1)]{valet} 
\pron{vAYxliTf, vAYxleV} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (variSaThx vagaRdavana) keYyALu; aMgaseVvaka; teYnAti; oDeyana uDigetoDigegaLanunx taMdiDuvudeV muMtAda heVLida kelasagaLanenxlalx mADalu avanoDaneyeV iruva paricAraka. 
\num{2} (hoVTelu \mo vugaLalilx ideV bageya kelasagaLigAgi neVmakavAgiruva) paricAraka; mANi. 
\enum
\emng
\eentry

\bentry
\word[valet(2)]{valet} 
\pron{vAYxliTf, vAYxleV} 
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{valeted}; \vakaq\ \eng{valeting}). }


\noindent
\gl{\sakirx}
\expl{}
\bmng
 (keYyALAgi, aMgaseVvakanAgi -- oDeyananunx) seVvisu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 (oDeyana) keYyALAgiru; aMgaseVvakanAgi vatiRsu. 
\emng
\eentry

\bentry
\word{valeta} 
\pron{valiVT} 
\gl{\nA}
\expl{}
\bmng
 \eng{veleta} padada rUpAMtara. 
\emng
\eentry

\bentry
\word[valetudinarian(1)]{valetudinarian} 
\pron{vAYxliTUyxDineVrianf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} daqDhAroVgayxvilalxda; roVgiSaThx; kAyileya; nitayxroVgiyAda. 
\num{2} AroVgayxhoMdalu bayasutitxruva; AroVgAyxkAMkiSxyAda. 
\num{3} tananx AroVgayxda viSayadalilx sadA kaLavaLapaDutitxruva; tanage oMdalalx oMdu roVgavideyeMdu AtaMkapaDutatxleV iruva; ilalxsalalxda roVgavideyeMdu shaMkisutatxleV iruva. 
\enum
\emng
\eentry

\bentry
\word[valetudinarian(2)]{valetudinarian} 
\pron{vAYxliTUyxDineVrianf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} roVgiSaThx; nitayxroVgi. 
\num{2} AroVgAyxkAMkiSx; AroVgAyxBilASi. 
\num{3} tananx AroVgayxda bagegx ati shaMkeyuLaLxvanu; oMdalalx oMdu roVgavideyeMdu kaLavaLapaDutatxleV iruvavanu. 
\enum
\emng
\eentry

\bentry
\word{valetudinarianism} 
\pron{vAYxliTUyxDineVrianisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} roVgiSaThxte; nitayxroVgitavx. 
\num{2} roVga -- shaMke, BiVti; tanage oMdalalx oMdu roVgavideyeMdu atiyAgi shaMkisuva, kaLavaLapaDuva parxvaqtitx iruvudu. 
\enum
\emng
\eentry

\bentry
\word[valetudinary(1)]{valetudinary} 
\pron{vAYxliTUyxDinari} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{valetudinarian(1)}{$^1$valetudinarian}. 
\emng
\eentry

\bentry
\word[valetudinary(2)]{valetudinary} 
\pron{vAYxliTUyxDinari} 
\gl{\nA}
\expl{}
(\bava\ \eng{valetudinaries}). \bmng
= \hyperlink{valetudinarian(2)}{$^2$valetudinarian}. 
\emng
\eentry

\bentry
\word{valgus} 
\pron{vAYxlagxsf} 
\gl{\nA}
\expl{}
\bmng
 horatirucu; keY yA pAdavu horagaDege tirucikoMDiruva virUpa. 
\emng
\eentry

\bentry
\word{Valhalla} 
\pron{vAYxlfhAYxl} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (nAveRya \pu) vAYxlAhxla; viVrasavxgaR; yudadhxdalilx kAdADi maDidavara BoVgakAkxgiyeV saqSiTxyAdadedxMdu BAvisidadx vishiSaTx savxgaR. 
\num{2} kiVtiRsheVSa -- Bavana, maMdira; divaMgatarAda viKAyxta puruSara sAmxraka sadana. 
\enum
\emng
\eentry

\bentry
\word{valiant} 
\pron{vAYxlayxMTf} 
\gl{\gu}
\expl{}
\bmng
 (vayxkitxya yA vataRneya \vi) dhiVra; shUra; viVra; kalitanada; gaMDugaliyAda. 
\emng
\eentry

\bentry
\word{valiantly} 
\pron{vAYxlayxMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
 kalitanadiMda; gaMDugaliyAgi; shwyaRdiMda. 
\emng
\eentry

\bentry
\word{valid} 
\pron{vAYxliDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (kAraNa, AkeSxVpa, vAda, \mo vugaLa \vi) sariyAda; samaMjasavAda; sakAraNavAda; saparxmANavAda; takaRbadadhxvAda; nAyxyasamamxtavAda. 
\numi{2} (\nAyxshA) (opapxMda, oDaMbaDike, \mo vugaLa \vi) nAyxyasamamxtavAda; kAnUnubadadhxvAda; nAyxyoVkatx riVtiyalilx yA karxmabadadhx vidhAnadalilx -- mADikoMDa: 
\banum
\alnum{a} \eng{a valid contract} karxmabadadhx karAru. 
\alnum{b} nAyxyAnuguNavAda; nAyxyAnusAravAda; kAnUnina daqSiTxyalilx aMgiVkArAhaRvAda: \eng{a valid passport} aMgiVkArAhaR pAsfpoVTfR. 
\alnum{c} vAyide tiVrilalxda; mukAtxyada yA koneya dina talupirada. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{validate} 
\pron{vAYxliDeVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} nAyxyasamamxtagoLisu; UjiRtagoLisu. 
\num{2} (karxmaloVpavidadxdadxnunx) karxmabadadhxgoLisu; vidhibadadhxrUpakekx taru. 
\num{3} kAyaMgoLisu; sithxriVkarisu. 
\enum
\emng
\eentry

\bentry
\word{validation} 
\pron{vAYxliDeVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nAyxyasamamxta goLisuvike; UjiRtagoLisuvike. 
\num{2} karxmabadadhx goLisuvike. 
\num{3} kAyaMgoLisuvike; sithxriVkarisuvike. 
\enum
\emng
\eentry

\bentry
\word{validity} 
\pron{vAYxliDiTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nAyxyasamamxtate; nAyxyAnuguNavAgiruvike. 
\num{2} kAnUnina vidhAnakekx anusAravAgiruvike; vidhibadadhxvAgiruvike. 
\num{3} takaRsamamxtavAgiruvike. 
\num{4} aMgiVkArAhaRte. 
\enum
\emng
\eentry

\bentry
\word{validly} 
\pron{vAYxliDfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (kAraNa, AkeSxVpa, vAda, \mo vugaLa \vi) samaMjasavAgi; sakAraNavAgi. 
\num{2} nAyxyasamamxtavAgi; kAnUnubadadhxvAgi. 
\num{3} nAyxyoVkatxvAda riVtiyalilx; karxmabadadhxvAgi. 
\num{4} nAyxyAnuguNavAgi; nAyxyAnusAravAgi. 
\enum
\emng
\eentry

\bentry
\word{valine} 
\pron{veVliVnf} 
\gl{\nA}
\expl{}
\bmng
 (\jiVra) veVlinf; kasheVrukagaLige muKayx poVSakAMshavAda matutx porxVTiVnugaLa sAmAnayx GaTakavAda ameYnoV Amalx. 
\emng
\eentry

\bentry
\word{valise} 
\pron{valiVsfZ} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} parxyANada keY -- ciVla, peTiTxge. 
\num{2} (\ame) saNaNx togalupeTiTxge; hasube peTiTxge. 
\enum
\emng
\eentry

\bentry
\word{Valium} 
\pron{vAYxliamf} 
\gl{\nA}
\expl{(\P)}
\bmng
vAYxliyamf; shAmakavAgi baLasuva DayAseZpAmf madudx. 
\emng
\eentry

\bentry
\word{Valkyrie} 
\pron{vAYxlikxari, vAYxlikxri} 
\gl{\nA}
\expl{}
\bmng
 (nAveRya \pu) vAYxlikxri; yudadhxdalilx kAdu viVrasavxgaR paDeyabeVkeMba vidhisaMkalapxkekx oLapaTaTxvaranunx Ayekx mADalu niyukatxrAda, parxdhAna deVvanAda ODinanxna \eng{12} maMdi paricArikeyaralilx obabxLu. 
\emng
\eentry

\bentry
\word{vallecula} 
\pron{valekuyxl} 
\gl{\nA}(\bava\ \eng{valleculae}
\expl{\ucAcx\ valekuyxliV). }
\bmng
(\aMrashA\ matutx \savi) valekuyxla; toVDu yA gADi. 
\emng
\eentry

\bentry
\word{vallecular} 
\pron{valekuyxlarf} 
\gl{\gu}
\expl{}
\bmng
valekuyxlada, adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{valleculate} 
\pron{valekuyxlaTf} 
\gl{\gu}
\expl{}
\bmng
valekuyxla uLaLx. 
\emng
\eentry

\bentry
\word{valley} 
\pron{vAYxli} 
\gl{\nA}
\expl{(\bava\ \eng{valleys}). }
\bmng
\bnum
\num{1} (beTaTxguDaDxgaLa naDuve, \sA\ tore, nadi hariyuva) kaNive; kaMdara. 
\num{2} (kaNiveyaMtaha yAvudeV) tagAgxda parxdeVsha. 
\num{3} (\vAshi) CAvaNiya horameYgaLu parasapxra saMdhisuvalilx EpaRDuva oLakoVna. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{valley of the shadow of death} 
\banum
\alnum{a} maraNasaMkaTa yA maraNasaMkaTada hotutx. 
\alnum{b} dAruNa duHKa yA dAruNa duHKada kAla. 
\eanum
\emng
\eentry

\bentry 
\word{vallum} 
\pron{vAYxlamf} 
\gl{\nA}
\expl{}
\bmng
 (\roV\ \pArxca) (rakaSxNA vayxvasethxyalilx nimiRsutitxdadx) koVTekotatxla; ALuveVri. 
\emng
\eentry

\bentry
\word{valonia} 
\pron{valoVnia} 
\gl{\nA}
\expl{}
\bmng
 valoVniya; togalu hadamADuvudu, raMgu hAkuvudu, shAyi mADuvudu, \mo vugaLalilx baLasuva, nitayxharidavxNaRda Okf marada hUvugaLa puSapxpuTa. 
\emng
\eentry

\bentry
\word{valor} 
\pron{vAYxlarf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{valour}{valour}. 
\emng
\eentry

\bentry
\word{valorise} 
\pron{vAYxlareYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{valorize}{valorize}. 
\emng
\eentry

\bentry
\word{valorization} 
\pron{vAYxlareYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ sakARrada niVti nivaRhaNa \mo\ kaqtaka vidhAnadiMda sarakugaLa) beleyanunx Erisuvike yA sithxriVkarisuvike; mwlayx -- vadhaRna yA sithxriVkaraNa. 
\emng
\eentry

\bentry
\word{valorize} 
\pron{vAYxlareYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 (\kanmu\ sakARrada niVti nivaRhaNa \mo\ kaqtaka vidhAnadiMda sarakugaLa) beleyanunx Erisu yA sithxriVkarisu. 
\emng
\eentry

\bentry
\word{valorous} 
\pron{vAYxlarasf} 
\gl{\gu}
\expl{}
\bmng
shwyaRda; parAkarxmada. 
\emng
\eentry

\bentry
\word{valorously} 
\pron{vAYxlarasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 shwyaRdiMda; parAkarxmadiMda. 
\emng
\eentry

\bentry
\word{valour} 
\pron{vAYxlarf} 
\gl{\nA}
\expl{}
\bmng
 (\birx) (\kAparx\ yA \hA\ yA \aupa) (\kanmu\ raNaRaMgadalilx parxdashiRsuva) shwyaR; parAkarxma. 
\emng
\eentry

\bentry
\word{valse} 
\pron{vAlfsx} 
\gl{\nA}
\expl{}
\bmng
= \hyperref{kandict_w.pdf}{W}{waltz(1)}{waltz}. 
\emng
\eentry

\bentry
\word[valuable(1)]{valuable} 
\pron{vAYxluyxabflf} 
\gl{\gu}
\expl{}
\bmng
 BAri beleya; bahaLa beleyuLaLx; apAra belebALuva; amUlayx(vAda): \eng{valuable property} bele bALuva Asitx. \eng{valuable information, help, etc.} amUlayx samAcAra, sahAya, \mo vu. 
\emng
\eentry

\bentry
\word[valuable(2)]{valuable} 
\pron{vAYxluyxabflf} 
\gl{\nA}
\expl{}
\bmng
 (\sA\ \bava dalilx) belebALuva vasutx(gaLu); amUlayx vasutx(gaLu); \kanmu\ oDave vasutx(gaLu), naganANayx(gaLu): \eng{she sent all her valuables to the bank} Ake tananx naganANayxgaLanenxlalx bAyxMkige kaLuhisidaLu. 
\emng
\eentry

\bentry
\word{valuably} 
\pron{vAYxluyxabilx} 
\gl{\kirxvi}
\expl{}
\bmng
 bele bALuvaMte. 
\emng
\eentry

\bentry
\word{valuate} 
\pron{vAYxluyxETf} 
\gl{\akirx}
\expl{}
\bmng
 (\kanmu\ \ame) belekaTuTx; mwlayxmApana mADu; bele niNaRyisu. 
\emng
\eentry

\bentry
\word{valuation} 
\pron{vAYxluyxESanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ adhikaqtanAda mwlayxniNaRyakAraniMda yA vaqtitxyiMda belekaTuTxva vayxkitxyiMda) belekaTuTxvike; mwlayx niNaRya. 
\num{2} (bele kaTuTxvavanu) kaTiTxda, niNaRyisida -- bele. 
\num{3} (vasutxvige iTaTx, hAkida) bele; mwlayx: \eng{valuation of land} jamiVnina mwlayxniNaRya: \eng{he disposed of it at a low valuation} avanu adanunx kaDime belege mAribiTaTx. \eng{he sets too high a valuation on his abilities} avanu tananx shakitxsAmathaRyxgaLige atiyAgi bele kaTuTxtAtxne. 
\enum
\emng
\eentry

\bentry
\word{valuator} 
\pron{vAYxluyxETarf} 
\gl{\nA}
\expl{}
\bmng
 belekaTuTxvavanu; mwlayxniNaRyakAra. 
\emng
\eentry

\bentry
\word[value(1)]{value} 
\pron{vAYxlUyx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} bele; mwlayx; kimamxtutx. 
\num{2} bele; parxyoVjana; upayukatxte: \eng{I now learnt the value of regular exercise} karxmabadadhxvAda vAyxyAmada parxyoVjanavanunx nAnu Iga aritukoMDe. 
\num{3} (vasutxvina, viSayada) bele; yoVgayxte; pArxmuKayx; mahatatxvX; guNa: \eng{I now learnt the value of fresh water} sihiniVrina bele EneMbaudu nanage Iga tiLiyitu. \eng{I now learnt the value of a friend} senxVhitana mahatatxvX nanage Iga tiLiyitu. 
\num{4} kaTiTxda bele; niNaRyisida mwlayx; mahatatxvX: \eng{he sets a high value on his time} avanu tananx kAlakekx BAri mahatatxvX koDutAtxne. 
\num{5} vinimaya mwlayx; badalubele; sarakige badalu paDeyabahudAda samabeleya haNa yA beVre saraku. 
\num{6} (sarakina yA vasutxvina) (sari)samabele; parxtimwlayx: \eng{government paid him the value of the acquired land} sakARravu vashapaDisikoMDa jamiVnina yA nelada parxtimwlayxvanunx avanige pAvati mADitu. 
\hypertarget{value(1)7}{} 
\num{7} sUkatxPala; vecacx mADida haNakekx sikukxva takakx parxtiPala. 
\num{8} mwlayx; bele; oMdu udedxVshavanunx keYgUDisuva yA oMdu pariNAmavanunx uMTumADuva sAmathaRyx: \eng{news value} vAtARmwlayx; sudidxbele. 
\num{9} (\bava dalilx) (vayxkitxya) mwlayxgaLu; tatatxvXgaLu; AdashaRgaLu; jiVvanada paramoVdedxVshaveMdu yA jiVvanadalilx atayxMta mahatatx vxpUNaRvAdadedxMdu obabxnu BAvisuva dheyxVya, lakaSxyX. 
\num{10} (pada \mo vugaLa) athaR. 
\num{11} (ADumAtinalilx) (savxra yA vayxMjanada) dhavxni; ucAcxraNe. 
\num{12} (\saM) savxrada avadhi; yAvudeV savxravanunx gAna yA vAdana mADalu hiDiyuva kAla. 
\num{13} (ATada niyamagaLa parxkAra isipxVTu ele, caduraMgada kAyi, \mo vugaLa) bele; pArxmuKayx; dajeR. 
\num{14} (vaNaRcitarxNa) vaNaRCAye; vaNaRsAmaMjasayx; baNaNxgaLa hoMdike; CAyAsAmarasayx; neLalubeLakugaLige saMbaMdhapaTaTxMte citarxda oMdu BAgakUkx inonxMdakUkx iruva parasapxra saMbaMdha. 
\num{15} (\ga) bele; mUlayx; biVjagaNitiVya pada yA ukitxyu parxtinidhisuva parimANa. 
\num{16} (\Bwvi\ matutx \ravi) parimANa saMKeyx; aMkigaLalilx heVLuva, yAvudeV parimANada aLate: \eng{the value of gravity at the equator} viSuvaderxVKeya baLi gurutavxda mwlayx. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
 \eng{value for money} = \hyperlink{value(1)7}{$^1$value\((7)\)}. 
\emng
\eentry

\bentry 
\word[value(2)]{value} 
\pron{vAYxlUyx} 
\gl{\sakirx}(vataRmAna \parxpu\ \Eva\ \eng{values}; \BU\ matutx \BUkaq\ \eng{valued}; \vakaq\ \eng{valuing}). 
\bmng
\bnum
\num{1} (\kanmu\ mwlayxniNaRyakAranaMte vasutxgaLige) belekaTuTx; bele aMdAju mADu; mwlayx niNaRyisu: \eng{I value the whole at Rs.} \eng{2000} iSaTxkUkx nAnu \eng{2000} rU. bele kaTuTxtetxVne. 
\num{2} belekoDu; mAnayxmADu; mananxNe, mAnayxte, mahatatx vx -- koDu: \eng{I value simplicity beyond all things} saraLatege nAnu elalxkikxMtalU miVri bele koDutetxVne, mahatatxvX koDutetxVne. \eng{a valued friend} sanAmxnayx mitarx. 
\num{3} udAtatx BAvane hoMdiru; mecicxke hoMdiru; hememxpaDu: \eng{he values himself highly on his conversational powers} tananx saMBASaNa sAmathaRyxkAkxgi avanu tananxnenxV bahaLavAgi mecicxkoLuLxtAtxne, tuMba hememx paDutAtxne. 
\enum
\emng
\eentry

\bentry
\wordnospeech{value added tax}{value added tax}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 bele kUDisida terige; mwlayx vadhiRta, mwlayxsaMkalita -- kara; utApxdaneya parxti haMtadalUlx sarakina bele eSuTx hecucxtatxdeyoV A mobalagina meVle vidhisida terige. 
\emng
\eentry

\bentry
\wordnospeech{value judgement}{value judgement}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 veYyakitxka tiVmARna; vayxkitxniSaThx niNaRya; yAvudakekxV Agali savxMta olavina AdhArada meVle koDuva bele, mADuva tiVmARna. 
\emng
\eentry

\bentry
\word{valueless} 
\pron{vAYxluyxlisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} beleyilalxda. 
\num{2} kelasakekx bArada; upayoVgavilalxda; niSapxrXyoVjaka. 
\enum
\emng
\eentry

\bentry
\word{valuelessness} 
\pron{vAYxluyxlisfnisf} 
\gl{\nA}
\expl{}
\bmng
 upayoVgavilalxdiruvike; niSapxrXyoVjakate; nirupayukatxte; kelasakekx bAradiruvike. 
\emng
\eentry

\bentry
\word{valuer} 
\pron{vAYxluyxarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (vasutxgaLige) bele kaTuTxvava; bele aMdAjugAra; mwlayx niNARyaka. 
\num{2} mwlayxmApaka; sAdhane, guNamaTaTx, \mo vanunx aLeyuvavanu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{value received}{value received}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 samamwlayx; huMDi \mo vakekx badalu paDeda haNa yA adakekx sarisamanAda motatx. 
\emng
\eentry

\bentry
\word{valuta} 
\pron{valUyxT} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (yAvudeV nANayxvayxvasethxya) twlanika -- bele, mwlayx. 
\num{2} (oMdu nANayxvayxvasethxyanunx inonxMdaroDane) samiVkarisida bele; samiVkaqta mwlayx. 
\enum
\emng
\eentry

\bentry
\word{valve} 
\pron{vAYxlfvx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} kavATa: 
\banum
\alnum{a} yAvudeV nALada muKAMtara hariyuva darxvavanunx oMdu dikikxnalilx mAtarx hariyuvaMte mADuva sAdhana. 
\alnum{b} (\aMrashA\ matutx \pArxvi) rakatx muMtAda darxvagaLu oMdu dikikxnalilx mAtarx hariyuvaMte noVDikoLuLxva poreyaMtha aMgaBAga. 
\alnum{c} (\saM) hitAtxLeya gALivAdayxdalilx koLaveya udadxvanunx hecicxsalu matutx tagigxsalu mADiruva avakAsha. 
\eanum
\numie
\num{2} (\birx) = \hyperref{kandict_t.pdf}{T}{thermionic valve}{thermionic valve}. 
\num{3} kiVlukada; maDicu kadada oMdu halage. 
\num{4} (\savi) (oNagi bAyibiTaTx) biVjakoVshada, parAgakoVshada -- adhaRBAga, hoVLu. 
\num{5} (shaMKashAsatxrX) kiVlucipipxna oMdadhaR; kapepxcipu px, mutitxnacipupx. 
\enum
\emng
\eentry

\bentry
\word{valved} 
\pron{vAYxlfvxDf} 
\gl{\gu}
\expl{}
\bmng
 kavAT(gaLu) uLaLx; kavATayukatx (\saMpa gaLalilx saha baLake). 
\emng
\eentry

\bentry
\word{valveless} 
\pron{vAYxlfvxlisf} 
\gl{\gu}
\expl{}
\bmng
 kavAT(gaLu) ilalxda; kavATarahita; kavATashUnayx. 
\emng
\eentry

\bentry
\word{valvular} 
\pron{vAYxlfvxvularf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{valved}{valved}. 
\num{2} kavATarUpada. 
\num{3} kavATada kelasavanunx mADuva; kavATadaMte vatiRsuva. 
\enum
\emng
\eentry

\bentry
\word{valvule} 
\pron{vAYxlfvxvUlf} 
\gl{\nA}
\expl{}
\bmng
 kirukavATa. 
\emng
\eentry

\bentry
\word{valvulitis} 
\pron{vAYxlfvxvuyxleYTisf} 
\gl{\nA}
\expl{}
\bmng
 kavAToVderxVka; \kanmu\ haqdayada kavATada yA kavATagaLa uriyUta. 
\emng
\eentry

\bentry
\word{vambrace} 
\pron{vAYxMberxVsf} 
\gl{\nA}
\expl{}
\bmng
 (\ca) (madhayxyugadalilx yoVdharu AtamxrakaSxNegAgi toDutitxdadx) muMdoVLina ukikxna kApu; parxkoVSaThx kavaca. 
\emng
\eentry

\bentry
\word{vamoose} 
\pron{vamUsf} 
\gl{\akirx}
\expl{}
\bmng
 (\ame) (\kanmu\ \BAavayx vAgi) (\ashi) birabirane tolagihoVgu. 
\emng
\eentry

\bentry
\word[vamp(1)]{vamp} 
\pron{vAYxMpf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (baUTina yA SUna) meVlABxgada muMtudi. 
\num{2} teVpehAkida (yAvudeV) vasutx. 
\num{3} (\saM) tatAkxlakekx odagisida yA hoMdisida pakakxvAdayx. 
\enum
\emng
\eentry

\bentry
\word[vamp(2)]{vamp} 
\pron{vAYxMpf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (baUTige yA SUge) meVlacxmaRda muMtudiyanunx holi. 
\num{2} (yAvudanenxV Agali) teVpe hAki ripeVri mADu. 
\num{3} agatayxvAda vasutxvanunx (cUrupAru seVrisi yA teVpe hAki) tayArisiko. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 tAtAkxlika pakakxvAdayxvanunx -- EpARDu mADu, odagisu (\sakirx\ saha). 
\emng
\eentry

\bentry
\word[vamp(3)]{vamp} 
\pron{vAYxMpf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} mAyagAti; mAyAvini; puruSaranunx maruLugoLisi badukuvavaLu. 
\num{2} yAvudakUkx heVsada, lajejxgeTaTx huDugi, bajAri. 
\enum
\emng
\eentry

\bentry
\word[vamp(4)]{vamp} 
\pron{vAYxMpf} 
\gl{\sakirx}
\expl{}
\bmng
 (heMgusina \vi) vAyxmoVhagoLisu; mAye biVru; maruLugoLisu; kAmaceVSeTxgaLiMda puruSananunx vashapaDisiko. 
\emng

\noindent
\gl{\akirx}
\bmng
mAyagAtiyAgi, mAyAviniyAgi -- vatiRsu. 
\emng
\eentry

\bentry
\word{vampire} 
\pron{vAYxMpeYarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} rakatxpishAci: 
\banum
\alnum{a} mATagAra, pASaMDa, aparAdhi, \mo vara, samAdhiyiMda rAtirxya veVLeyalilx horabaMdu nidirxsutitxruvavara rakatxvanunx hiVrutatxdeyeMdu naMbalAda pishAci. 
\alnum{b} (dayAdAkiSxNayxvilalxde parAvalaMbiyAgi AsharxyadAtara rakatxvanenxV hiVruva) parataMtarx jiVvi. 
\eanum
\numie
\num{2} = \hyperlink{vampire bat}{vampire bat}. 
\num{3} (raMgaBUmi) (pAtarxgaLu EkAEki adaqshayxvAgalu kalipxsiruva, sipxrXMganunx otutxvudariMda teredu mucicxkoLuLxva) kaLaLx -- kada, bAgilu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vampire bat}{vampire bat}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (danakarugaLu, kuduregaLu, nidirxsutitxruva manuSayxru, ivaranunx kacicx rakatx hiVruva yA hiVruvudenanxlAda, \da\ amerika \mo\ deVshagaLa) rakatxhiVruva bAvali. 
\emng
\eentry

\bentry
\word{vampiric} 
\pron{vAYxMpirikf} 
\gl{\gu}
\expl{}
\bmng
rakatxpishAciyaMte vatiRsuva; rakatx hiVruva. 
\emng
\eentry

\bentry
\word{vampirism} 
\pron{vAYxMpeYarisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} rakatxpishAcigaLanunx kurita naMbike. 
\num{2} rakatxpishAcitana; rakatxhiVruva savxBAva, vataRne. 
\enum
\emng
\eentry

\bentry
\word{vamplate} 
\pron{vAYxMpelxVTf} 
\gl{\nA}
\expl{}
\bmng
 (\ca) keYkApu; karakavaca; madhayxyugada yoVdharu BajiR ITigaLa tivita tapipxsikoLaLxlu baLasutitxdadx, ukikxna keYmusuku. 
\emng
\eentry

\bentry
\word[van(1)]{van} 
\pron{vAYxnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aduru pariVkeSx; pikAsiya eleya meVle yA yaMtarxda neraviniMda aduranunx toLedu, adara guNamaTaTxvanunx pariVkiSxsuvudu. 
\num{2} (\pArxparx) (dhAnayxvanunx, kALanunx) tUruva mora. 
\num{3} (\pArxparx\ yA \kAparx) rekekx. 
\enum
\emng
\eentry

\bentry
\word[van(2)]{van} 
\pron{vAYxnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{vanguard}{vanguard}. 
\num{2} muMBAga; agarx; muMcUNi: \eng{in the van of progress} parxgatiya muMcUNiyalilx. 
\enum
\emng
\eentry

\bentry
\word[van(3)]{van} 
\pron{vAYxnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (piVThoVpakaraNagaLu yA beVre sAmAnugaLu, kuduregaLu \mo\ pArxNigaLu, keYdigaLu, \mo varanunx sAgisuva) mucucxbaMDi; vAyxnu. 
\num{2} (\birx)(parxyANikara saraku, sAmAnugaLanunx sAgisuva) reYlu -- gADi, baMDi, Dabibx; lageVjf vAyxnu. 
\num{3} (\birx) reYlevx gADiRna gADi, baMDi, Dabibx. 
\num{4} jipisxgaLa saMcAri gADi \mo vu; yUroVpina deVshagaLalilx vAsisutitxdadx, jipisxgaLeMba hesarina alemArigaLu sathxLadiMda sathxLakekx saMcarisi, oMdeDe taMgalu baLasutitxdadx sAlugADigaLalolxMdu. 
\enum
\emng
\eentry

\bentry
\word[van(4)]{van} 
\pron{vAYxnf} 
\gl{\nA}
\expl{}
\bmng
 (\birx) (Tenisf \AmA) = \hyperref{kandict_a.pdf}{A}{advantage(1)3}{$^1$advantage(3)}(\saMkiSx). 
\emng
\eentry

\bentry
\word{vanadate} 
\pron{vAYxnaDeVTf} 
\gl{\nA}
\expl{}
\bmng
 (\ravi) venaDeVTf; veneVDikf Amalxda lavaNa. 
\emng
\eentry

\bentry
\word{vanadic} 
\pron{vanAYxDikf} 
\gl{\gu}
\expl{}
\bmng
 (\ravi) vanAYxDikf; (veneVDiya saMyukatxgaLa \vi) \eng{5} veVlenisxya veneVDiyamf anunx oLagoMDa. 
\emng
\eentry

\bentry
\word{vanadium} 
\pron{vaneVDiamf} 
\gl{\nA}
\expl{}
\bmng
 (\ravi) vaneVDiyamf; paramANu saMKeyx \eng{23} matutx paramANu tUka \eng{51}uLaLx, visheVSa riVtiya ukukxgaLa tayArikeyalilx baLasalAguva, baUdubaNaNxda gaTiTxyAda loVhadhAtu. 
\emng
\eentry

\bentry
\word{vanadous} 
\pron{vAYxnaDasf} 
\gl{\gu}
\expl{}
\bmng
 (\ravi) vAyxnaDasf; (veneVDiyamf saMyukatxgaLa \vi) \eng{3} veVlenisxya, kelavomemx \eng{2} veVlenisxya, veneVDiyamamxnunx oLagoMDa. 
\emng
\eentry

\bentry
\wordRemoveSpace{Van-Allen-belt}{Van Allen belt}
\pron{vAYxnf AYxlanf belfTx} 
\gl{\nA}
\expl{}
\bmng
 vAyxnf alenf -- paTiTx, valaya; BUmiyiMda meVlagxDe halavu sAvira kiloVmiVTarf etatxradalilxruva, BUmiyanunx BAgashaH sututxvarediruva, tiVkaSxNX vikiraNagaLiMda kUDida, eraDu valayagaLa peYki oMdu. 
\emng
\eentry

\bentry
\wordnospeech{Van Allen layer}{Van Allen layer}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{Van Allen belt}{Van Allen belt}. 
\emng
\eentry

\bentry
\wordnospeech{V. \& A.}{V. \& A.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 (\birx) (laMDaninxna) \eng{Victoria matutx Albert Museum.} 
\emng
\eentry

\bentry
\word[vandal(1)]{vandal} 
\pron{vAYxMDalf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} \eng{(Vandal)} vAYxMDalf; \eng{4 -- 5}neV shatamAnagaLalilx gAlf, sepxVnf, \kanu\ Aphirxka, matutx roVmf deVshagaLanunx koLeLxmADi, garxMtharAshiyanUnx kalAkaqtigaLanUnx nAsha mADida, jamARyxnikf bauDakaTiTxna vAyxMDalf eMba taMDadavanu. 
\num{2} vidhavxMsaka; vinAshaka; udedxVshapUvaRkavAgi yA durudedxVshadiMda AsitxpAsitxyanunx hALumADuvava. 
\enum
\emng
\eentry

\bentry
\word[vandal(2)]{vandal} 
\pron{vAYxMDalf} 
\gl{\gu}
\expl{}
\bmng
 vAYxMDalf taMDada. 
\emng
\eentry

\bentry
\word{vandalic} 
\pron{vAYxMDalikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vidhavxMsagoLisuva; vidhavxMsaka; vinAshaka. 
\num{2} (\eng{Vandalic}) vAYxMDalara, A taMDakekx saMbaMdhisida yA avaraMtha. 
\enum
\emng
\eentry

\bentry
\word{vandalism} 
\pron{vAYxMDalisaZmf} 
\gl{\nA}
\expl{}
\bmng
 vidhavxMsakate; vinAshakate. 
\emng
\eentry

\bentry
\word{vandalistic} 
\pron{vAYxMDalisiTxkf} 
\gl{\gu}
\expl{}
\bmng
 vidhavxMsakavAda; vinAshakateya. 
\emng
\eentry

\bentry
\word{vandalistically} 
\pron{vAYxMDalisiTxkali} 
\gl{\kirxvi}
\expl{}
\bmng
 vidhavxMsakavAgi; vinAshakateyiMda. 
\emng
\eentry

\bentry
\word{vandalize} 
\pron{vAYxMDaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 vidhavxMsagoLisu; nAshamADu. 
\emng
\eentry

\bentry
\wordRemoveSpace{van-de-Graaff-generator}{van de Graaff generator} 
\pron{vAyxnf Da gArxphf jenareVTarf} 
\gl{\nA}
\expl{}
\bmng
 (\viduyx) vAyxnf Da gArxphf utApxdaka; adhika voVlaTxteyalilx viduyxdivxsajaRneyanunx utApxdisuvudakAkxgi, tudiyilalxda shaqMgiVyapaTiTxyoMdara neraviniMda viduyxdAveVshavanunx niroVdhita \eng{(insulated)} loVhagumamxTa oMdakekx vagARyisuva yaMtarx. 
\emng
\eentry

\bentry
\wordRemoveSpace{van-der-Waals-forces}{van der Waals forces}
\pron{vAYxnf Da vAlfs' phoVsaRsf} 
\gl{\nA}
\expl{(\bava) (\ravi)}
\bmng
 vAyxnf Darf vAlfsxna balagaLu; anAviSaTx paramANugaLa yA aNugaLa naDuve kANbaruva, divxdhurxva mahatatxvX \eng{(moment)} gaLa aMtarakirxyeya pariNAmavAgi udaBxvisuva dubaRlavAda AkaSaRNa balagaLu. 
\emng
\eentry

\bentry
\word[vandyke(1)]{vandyke} 
\pron{vAYxnfDeYkf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (yAvudeV baTeTx, jaratAri vasatxrX, \mo vugaLa aMcinalilx heNediruva koVcugaLa sAlinalilx) oMdu koVcu. 
\num{2} (toVLilalxda kavaca, kupupxsa yA kutitxge paTiTxgaLige hAkiruva) koVcaMcu. 
\enum
\emng
\eentry

\bentry
\word[vandyke(2)]{vandyke} 
\pron{vAYxnfDeYkf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\eng{17}neya shatamAnada yUroVpina vaNaRcitarxkAra) vAyxnfDeYkana (BAvacitarxgaLalilx kaMDubaruva koVcaMcina uDupugaLa) sheYliya. 
\num{2} cUpAda; monacAda. 
\num{3} kaDu kaMdubaNaNxda. 
\enum
\emng
\eentry

\bentry
\word[vandyke(3)]{vandyke} 
\pron{vAYxnfDeYkf} 
\gl{\sakirx}
\expl{}
\bmng
 (baTeTx, hululxgAvalu, \mo vugaLanunx) koVcaMciruvaMte -- katatxrisu, mADu; koVcaMciDu. 
\emng
\eentry

\bentry
\wordnospeech{Vandyke beard}{Vandyke beard}
\pron{?}
\gl{\nA}
\bmng
monacu gaDaDx. \imglink{Vandyke beard:fig}{\raisebox{-0.25cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.6cm {V_Pictures/vandyke beard.jpg}}} 
\emng
\eentry

\bentry
\wordnospeech{Vandyke brown}{Vandyke brown}
\pron{?}
\gl{\nA}
\expl{}
\bmng
kaDu kaMdubaNaNx. 
\emng
\eentry

\bentry
\word{vane} 
\pron{veVnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (gALiya dikakxnunx sUcisalu etatxrada kaTaTxDagaLa meVle hAkuva, kelavomemx huMjadAkArada) gALi girugaTe; gALi huMja. 
\num{2} parxvAha dikUsxci; niVru girugaTe; niVrina harivina dikakxnunx sUcisalu adakekxdurAgi oDuDxva, gALigirugaTeyaMtha sAdhana. 
\num{3} (gALigiraNi, viduyxdAgAragaLa niVgARli, vimAnagaLa tirupu cAlaka, \mo vugaLa) toVLu; alagu; rekekx. 
\num{4} (saveRVkaSxNa yaMtarxgaLu, kAvxDarxMTf, \mo vugaLa) dashaRka; lakaSxyXka; viVkaSxNAsAdhana. 
\num{5} hakikxgariya, capapxTeyAgiruva taLaBAga. 
\enum
\emng
\eentry

\bentry 
\word{vaned} 
\pron{veVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gALigirugaTeyuLaLx. 
\num{2} (gALigiraNi \mo vugaLa \vi) toVLuLaLx; rekekxyuLaLx. 
\enum
\emng
\eentry

\bentry
\word{vaneless} 
\pron{veVnflisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gALigirugaTeyilalxda. 
\num{2} (gALigiraNi \mo vugaLa \vi) toVLugaLilalxda; rekekxyilalxda. 
\enum
\emng
\eentry

\bentry
\word{vanessa} 
\pron{vanesa} 
\gl{\nA}
\expl{}
\bmng
 vanesa; kaDugeMpu baNaNxda yA baNaNxbaNaNxda, ciTeTxgaLa kula yA A kulada ciTeTx. 
\emng
\eentry

\bentry
\word{vang} 
\pron{vAYxMgf} 
\gl{\nA}
\expl{}
\bmng
 (\nw) hAyiya tudiyiMda dakakxda tudiyavarege eraDu kaDegU hAyuva, hagagxgaLalolxMdu. 
\emng
\eentry

\bentry
\word{vanguard} 
\pron{vAYxnfgADfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (BUseYnayx yA nwkAseYnayxda) paDeya muMcUNi; agarxpaMkitx. 
\num{2} (caLavaLi, nidiRSaTx aBipArxya, \mo vanunx taLediruva paMtha \mo vugaLa) muMcUNi; nAyakavagaR; neVtAra vagaR. 
\enum
\emng
\eentry

\bentry
\word{vanilla} 
\pron{vanil} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} venila baLiLx; suvAsaneya hUvugaLanunx biDuva, mara, goVDe, \mo vugaLa meVleVruva, uSaNxvalayada hUbaLiLx, siVtALe yA ivugaLa kula. 
\hypertarget{vanilla(2)}{} 
\num{2} venila -- kAyi, koVDu. 
\num{3} (venila biVjada yA kaqtakavAgi tayArisida adaraMtha, aisfkirxVM \mo vugaLige hAkuva) venila rasa, esenfsx. 
\enum
\emng
\eentry

\bentry
\word{vanilla-pod} 
\pron{vanilapADf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vanilla(2)}{vanilla\((2)\)}. 
\emng
\eentry

\bentry
\word{vanillin} 
\pron{vanilinf} 
\gl{\nA}
\expl{}
\bmng
 venilinf; suvAsaneyuLaLx venila esenfsx yA sAra. 
\emng
\eentry

\bentry
\word{vanish} 
\pron{vAYxniSf} 
\gl{\sakirx}
\expl{}
\bmng
 kaNamxreyAgisu; adaqshayxgoLisu; kANadAgisu. 
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (idadxkikxdadxMte, EkAEki) kaNamxreyAgu; adaqshayxvAgu; agoVcaravAgu. 
\num{2} karxmakarxmavAgi kANadAgu; kiSxVNavAgutAtx hoVgi kaNamxreyAgu. 
\num{3} ilalxvAgu; nAshavAgu. 
\num{4} (\ga) sonenxyAgu; shUnayxvAgu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vanishing cream}{vanishing cream}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (camaRkekx tikikxdoDane iMgihoVgi) kANadAguva, adaqshayxvAguva -- kirxVmu, leVpa. 
\emng
\eentry

\bentry
\word{vanishing-point} 
\pron{vAYxniSiMgfpAiMTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (anaMta dUradavaregU eLeda samAnAMtara saraLareVKegaLa naDuvaNa aMtaraveV kANadaMtAgi eraDU seVrihoVdaMte toVruva) aMtadhARna biMdu; layabiMdu. 
\num{2} saMpUNaRvAgi kANadAguva sithxti; adashaRna, agoVcara -- sithxti. 
\enum
\emng
\eentry

\bentry
\word{Vanitory} 
\pron{vAYxniTari} 
\gl{\nA}
\expl{(\P) (\bava\ \eng{Vanitories}). }
\bmng
 saNaNx kapAToMdara meVle capapxTe halageyalilx kUrisida, keY \mo vanunx toLeyuva beVsinunx, boVguNi. 
\emng
\eentry

\bentry
\word{vanity} 
\pron{vAYxniTi} 
\gl{\nA}
\expl{(\bava\ \eng{vanities}). }
\bmng
\bnum
\num{1} poLuLxtana; ToLuLxtana; huruLilalxdiruvike. 
\num{2} nisAsxrate; vayxthaRte; niSaPxlate; niSapxrXyoVjakate; nirathaRkate: \eng{the vanity of worldly wealth} lwkika saMpatitxna nisAsxrate. \eng{the vanity of human achievement} mAnava(na) sAdhaneya vayxthaRte, niSaPxlate. \eng{the vanity of vanities} poLuLxgaLalUlx poLuLx. 
\num{3} mitheyx; avasutxtavx; diTavalalxdiruvike; 
\num{4} poLuLx jaMba; oNa jaMba; baMDavALavilalxda baDAyi. 
\num{5} ADaMbarada parxdashaRna; oNaDwlu: \eng{the pomps and vanities of this wicked world} I duSaTxparxpaMcada DwlugaLU poLuLxgaLU. 
\num{6} (\ame) alaMkaraNa meVju. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vanity bag}{vanity bag}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (kirugananxDi, alaMkaraNa sAmagirxgaLu, \mo vanunx oLagoMDa, heMgasara) keYciVla; jaMbada ciVla. 
\emng
\eentry

\bentry
\wordnospeech{vanity case}{vanity case}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vanity bag}{vanity bag}. 
\emng
\eentry

\bentry
\wordnospeech{Vanity Fair}{Vanity Fair}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (iMgelxMDina banayxnf eMba leVKaka tananx piligxrXmfsx porxVgerxsf eMba kAdaMbariyalilx vaNiRsiruva) ihaloVka eMba -- poLuLxsaMte, mithAyx parxpaMca. 
\emng
\eentry

\bentry
\wordnospeech{vanity unit}{vanity unit}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{Vanitory}{Vanitory}. 
\emng
\eentry

\bentry
\word{vanquish} 
\pron{vAYxMkivxSf} 
\gl{\akirx}
\expl{}
\bmng
 (sAhitayxka) soVlisu; parAjayagoLisu; parABavagoLisu (\rUpa\ saha). 
\emng
\eentry

\bentry
\word{vanquishable} 
\pron{vAYxMkivxSabflf} 
\gl{\gu}
\expl{}
\bmng
 soVlisabahudAda; parAjeVya. 
\emng
\eentry

\bentry
\word{vanquisher} 
\pron{vAYxMkivxSarf} 
\gl{\nA}
\expl{}
\bmng
 vijayi; vijeVta; gelAlxLu; soVlisuvava. 
\emng
\eentry

\bentry
\word{vantage} 
\pron{vAMTijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} anukUla sAthxna: 
\banum
\alnum{a} sutatxliruvudanunx noVDalu anukUlavAgiruva oLeLxya jAga, sathxLa. 
\alnum{b} AtamxrakaSxNege yA AkarxmaNakekx anukUlavAgiruva nele, jAga. 
\eanum
\numie
\num{2} (Tenisf) = \hyperref{kandict_a.pdf}{A}{advantage(1)3}{$^1$advantage(3)}. 
\num{3} (\pArxparx) lABa; parxyoVjana; anukUla; swlaBayx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vantage ground}{vantage ground}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vantage}{vantage\((1)\)}. 
\emng
\eentry

\bentry
\wordnospeech{vantage point}{vantage point}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vantage}{vantage\((1)\)}. 
\emng
\eentry

\bentry
\word{vapid} 
\pron{vAYxpiDf} 
\gl{\gu}
\expl{}
\bmng
 sapepxyAda; ruciyilalxda; niVrasa; oNa (\rUpa\ saha): \eng{vapid beer} ruciyilalxda biyaru. \eng{vapid conversation, remark, etc.} niVrasavAda saMBASaNe, TiVke, itAyxdi. \eng{vapid sports} niVrasavAda kirxVDegaLu. \eng{vapid moralising} oNa upadeVsha. 
\emng
\eentry

\bentry
\word{vapidity} 
\pron{vapiDiTi} 
\gl{\nA}
\expl{}
\bmng
 ruciyilalxdiruvike, sapepxtana; niVrasate. 
\emng
\eentry

\bentry
\word{vapidly} 
\pron{vAYxpiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
 sapepxsapepxyAgi; niVrasavAgi. 
\emng
\eentry

\bentry
\word{vapidness} 
\pron{vAYxpiDfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vapidity}{vapidity}. 
\emng
\eentry

\bentry
\word{vapor} 
\pron{veVparf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) = \hyperlink{vapour(1)}{vapour}. 
\emng
\eentry

\bentry
\word{vaporific} 
\pron{veVpariphikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AvisaMbaMdhi; Avige saMbaMdhisida. 
\num{2} Avijanaka; AviyanunxMTumADuva. 
\enum
\emng
\eentry

\bentry
\word{vaporiform} 
\pron{veVpariphAmfR} 
\gl{\gu}
\expl{}
\bmng
 AvirUpada; AvirUpi; AviyaMtiruva. 
\emng
\eentry

\bentry
\word{vaporimeter} 
\pron{veVparimiTarf} 
\gl{\nA}
\expl{}
\bmng
 AvimApaka; Aviya parxmANavananxLeyuva upakaraNa. 
\emng
\eentry

\bentry
\word{vaporise} 
\pron{veVpareYsfZ} 
\gl{\kirx}
\expl{}
\bmng
 = \hyperlink{vaporize}{vaporize}. 
\emng
\eentry

\bentry
\word{vaporizable} 
\pron{veVpareYsaZbflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AviVkarisabahudAda; aniliVkarisabahudAda; bASipxVkarisabahudAda. 
\num{2} anilarUpa taLeyabalalx; AviyAgabalalx. 
\enum
\emng
\eentry

\bentry
\word{vaporization} 
\pron{veVpareYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AviVkaraNa; aniliVkaraNa; bASipxVkaraNa; anilarUpakekx taruvudu. 
\num{2} AviVBavana; aniliVBavana; bASipxVBavana; anilarUpavAguvudu. 
\enum
\emng
\eentry

\bentry
\word{vaporize} 
\pron{veVpareYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 AviVkarisu; aniliVkarisu; bASipxVkarisu; darxvavanunx anilarUpakekx taru. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 AviyAgu; anilavAgu; bASapxvAgu; anilarUpakekx baru. 
\emng
\eentry

\bentry
\word{vaporizer} 
\pron{veVpareYsaZrf} 
\gl{\nA}
\expl{}
\bmng
 AviVkAraka; aniliVkAraka; bASipxVkAraka; anilarUpakekx taruva sAdhana. 
\emng
\eentry

\bentry
\word{vaporous} 
\pron{veVparasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AvirUpada; anilarUpada; bASapxrUpada. 
\num{2} (\rUpa) huruLilalxda; poLuLx; nijavalalxda. 
\enum
\emng
\eentry

\bentry
\word{vaporously} 
\pron{veVparasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} AvirUpadalilx. 
\num{2} (\rUpa) huruLilalxde. 
\enum
\emng
\eentry

\bentry
\word{vaporousness} 
\pron{veVparasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AviyAgiruvike. 
\num{2} (\rUpa) poLuLxtana; huruLilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{vapory} 
\pron{veVpari} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vaporous}{vaporous}. 
\emng
\eentry

\bentry 
\word[vapour(1)]{vapour} 
\pron{veVparf} 
\gl{\nA}
\bmng
\bnum
\num{1} habe; dhUma; (vAyumaMDaladalilx kaMDubaruva) maMju (rUpada teVva) yA hoge. 
\num{2} (\Bwvi) Avi; bASapx; sAmAnayxvAgi darxvasithxtiyalilxruva, viraLavAgi (kelavu padAthaRgaLa \vi) Ganasithxtiyalilxruva padAthaRda anilarUpa. 
\num{3} (\veYshA) habe; auSadhiyAgi usirADuvudakekx baLasuva yAvudeV padAthaR. 
\num{4} poLuLxkalapxne; huruLilalxda, nijavalalxda -- vasutx yA viSaya; \eng{it's all mere vapoury} adelalx bariya poLuLxkalapxne. 
\numi{5} (\bava dalilx) (\pArxparx) 
\banum
\alnum{a} (manasisxna) maMku; manoVmAlinayx; viSaNaNxte; gAlxni; vAyxkulate. 
\alnum{b} (unAmxda \mo\ yAvudeV bageya) manoVvikAra. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[vapour(2)]{vapour} 
\pron{veVparf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} AviyAgu; anilarUpa taLe; bASapxvAgu. 
\num{2} oNbaDAyi, oNajaMba, bauruDe -- kocucx; poLuLxharaTe hoDe. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vapour bath}{vapour bath}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\banum
\alnum{a} Avimajajxna; bASapxsAnxna; Aviyalilx kuLitu sAnxna mADuvudu. 
\alnum{b} Avi, bASapx -- sAnxnagaqha, sAnxnakoVNe; Aviyalilx sAnxna mADalu nimiRsiruva koThaDi yA mane. 
\eanum
\emng
\eentry

\bentry
\word{vapour-burner} 
\pron{veVparfbanaRrf} 
\gl{\nA}
\expl{}
\bmng
 (kAyisuvudakAkxgiyoV, parxkAsha biVruvudakAkxgiyoV baLasuva) anilajAvxlaka. 
\emng
\eentry

\bentry
\wordnospeech{vapour density}{vapour density}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 AvisAMdarxte; heYDorxjaninxnoDane hoVlisidaMte yAvudeV anilada yA Aviya sApeVkaSx sAMdarxte. 
\emng
\eentry

\bentry
\word{vapourer} 
\pron{veVpararf} 
\gl{\nA}
\expl{}
\bmng
 oNajaMbadavanu yA poLuLxharaTe malalx. 
\emng
\eentry

\bentry
\word{vapouring} 
\pron{veVpariMgf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\viparx) AviniSAkxsa; bASapx horabaruvudu. 
\num{2} oNbaDAyi kocucxvudu yA poLuLxharaTe hoDeyuvudu. 
\enum
\emng
\eentry

\bentry
\word{vapourish} 
\pron{veVpariSf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{vaporous}{vaporous}. 
\num{2} (manasisxna) viSaNaNxteya. 
\enum
\emng
\eentry

\bentry
\word{vapourous} 
\pron{veVparasf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vaporous}{vaporous}. 
\emng
\eentry

\bentry
\word{vapourously} 
\pron{veVparasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 = \hyperlink{vaporously}{vaporously}. 
\emng
\eentry

\bentry
\wordnospeech{vapour pressure}{vapour pressure}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 AviyotatxDa; AviBAra; nidiRSaTx tApadalilx darxvadoMdige samatoVladalilxruva Aviya otatxDa. 
\emng
\eentry

\bentry
\wordnospeech{vapour trail}{vapour trail}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 Avi jADu; ati etatxradalilx hArihoVgutitxruva rAkeTf yA jeTf vimAnada hiMbadiyalilx beLaLxge udadxvAgi rUpugoLuLxva, sAMdirxVkaqta niVruhanigaLa samUha. 
\emng
\eentry

\bentry
\word{vapoury} 
\pron{veVpari} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vapory}{vapory}. 
\emng
\eentry

\bentry 
\wordnospeech{var.}{var.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{variant.} 
\num{2} \eng{variety.} 
\enum
\emng
\eentry

\bentry
\word{varactor} 
\pron{varAYxkaTxrf} 
\gl{\nA}
\expl{}
\bmng
 parxyoVgisida voVleTxVjanunx avalaMbisida dhArakateyuLaLx arevAhaka DayoVDu. 
\emng
\eentry

\bentry
\word{varec} 
\pron{vAYxrekf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kaDala kaLe; kaDala joMDu. 
\num{2} = \hyperref{kandict_k.pdf}{K}{kelp}{kelp}. 
\enum
\emng
\eentry

\bentry
\word{variability} 
\pron{veVriabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (beVkAda riVti) vayxtayxsaniVyate; parivataRniVyate; badalAyisabahudAgiruvike; mApaRDisabahudAgiruvike. 
\num{2} asithxrate; caMcalate. 
\enum
\emng
\eentry

\bentry
\word[variable(1)]{variable} 
\pron{veVriabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (beVkAda riVti) badalAyisabahudAda; mApaRDisabahudAda; hoMdisikoLaLxbahudAda; vayxtAyxsasAdhayx: \eng{a rod of variable length} udadxvanunx mApaRDisikoLaLx bahudAda salAki. \eng{the pressure is variable} otatxDavu badalAyisuvaMthadu. 
\num{2} (kAru \mo vugaLa giyarina \vi) veVgavanunx badalAyisabahudAda. 
\num{3} badalAyisuva parxvaqtitxyuLaLx; asithxra; caMcala. \eng{variable mood} asithxra manoVvaqtitx. \eng{variable fortune} caMcalavAda adaqSaTx, BAgayx. 
\num{4} (gALiya \vi) dikukx badalAyisutatxleV iruva. 
\num{5} (\Kavi) (tAreya \vi) caMcala; kAMtiyu AvataRniVyavAgi badalAguva. 
\num{6} (\ga) cara; (\sA\ biVjagaNitiVya saMkeVtada \vi) vividha saMKAyxmwlayxviruva; anidiRSaTx mUlayxda. 
\numi{7} 
\banum
\alnum{a} (\jiVvi\ matutx \savi) (jiVvijAtiya \vi) jAtiya vishiSaTxte ilalxdiruva jiVvigaLanonxLagoMDa. 
\alnum{b} (\jiVvi) (jiVviya yA adara aMgada \vi) vayxtayxyasAdhayx; racane matutx kirxye badalAguva sAdhayxteyuLaLx. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[variable(2)]{variable} 
\pron{veVriabflf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\ga) cara; cara motatx; calaparimANa. 
\num{2} (\nw) caMcala, asithxra -- mAruta; dikukx badalAyisutatxleV iruva gALi. 
\num{3} (\bava dalilx) asithxra mAruta parxdeVsha; IshAnayx matutx AgenxVya vANijayx mArutagaLu biVsuva dikukxgaLige naDuvaNa parxdeVsha. 
\enum
\emng
\eentry

\bentry
\word{variableness} 
\pron{veVriabflfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{variability}{variability}. 
\emng
\eentry

\bentry
\word{variably} 
\pron{veVriabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (beVkAda riVti) badalAyisabahudAdaMte. 
\num{2} asithxravAgi; caMcalavAgi. 
\enum
\emng
\eentry

\bentry
\word{variance} 
\pron{veVrianfsx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aBipArxyaBeVda; vivAda; mataBeVda; BinAnxBipArxya: \eng{I have had a slight variance with him} avanigU nanagU savxlapx mataBeVda uMTAgide. \eng{on that point we are at variance (among ourselves)} A viSayavanunx kuritu namamxnamamxlilx BinAnxBipArxyavide. 
\num{2} vAyxjayx; virasa; veYmanasayx: \eng{he is at variance with the authorities} avanigU adhikArigaLigU virasavide, veYmanasayxvide. 
\num{3} vayxtAyxsa; BeVda; tAratamayx: \eng{this theory is at variance with all that is known on the subject} A shAsatxrXda viSayadalilx iduvaregU tiLidiruva sakalakUkx I sidAdhxMtakUkx vayxtAyxsavide. 
\num{4} (\nAyxshA) heVLike heVLikegU yA patarxpatarxkUkx naDuve kaMDubaruva -- vayxtAyxsa, BeVda, tAratamayx, asAmaMjasayx. 
\num{5} (\saMshA) vipalalxTa; parxmANaka vicalana yA dikucxyXtiya \eng{(standard deviation)} vagaR. 
\num{6} (\ame) mAmUlu niyamagaLanunx biTuTx kAyaRniNaRya yA kAyaR mADalu koDalAda anumati; (niyamadiMdAce hoVguva) viveVcanAdhikAra. 
\enum
\emng
\eentry

\bentry
\word[variant(1)]{variant} 
\pron{veVriaMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vi)Binanx; aMtara; savxrUpadalolxV vivaragaLalolxV parasapxra vayxtAyxsavuLaLx: \eng{a variant reading in some MSS} kelavu hasatxparxtigaLalilx kaMDubaruva BinanxpATha, pAThAMtara. 
\num{2} veYvidhayxvuLaLx; viBinanx, bagebageya -- rUpagaLiruva: \eng{he spoke of forty variant types of pigeon} nalavatutx viBinanxrUpada pArivALagaLanunx kuritu avanu mAtanADida. 
\num{3} badalAyisutitxruva; vayxtAyxsavAgutitxruva. 
\enum
\emng
\eentry

\bentry
\word[variant(2)]{variant} 
\pron{veVriaMTf} 
\gl{\nA}
\expl{}
\bmng
 (\rUpa, savxrUpa, kAguNita, jAti, pATha, \mo vugaLa) Binanxte; rUpAMtara; aMtara; BeVda: \eng{`enure' is a variant of `inure'. `enure'} eMbaudu \eng{`inure'} eMbaudara BinanxrUpa, rUpAMtara. 
\emng
\eentry

\bentry
\word{variate} 
\pron{veVriaTf} 
\gl{\nA}
\expl{}
\bmng
 (\saMshA) saMKAyxshAsitxrXVya parxyoVgadalilx saMBavaniVyate vayxtAyxsavAguva parimANa. 
\emng
\eentry

\bentry
\word{variation} 
\pron{veVriESanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} badalAvaNe; mApARDu; parivataRne: \eng{prices are subject to variation} belegaLu badalAvaNege oLapaTiTxve; belegaLu badalAvaNegoLuLxva saMBavavide. 
\num{2} (hiMdina yA sahaja sithxtiyiMda, kirxyeyalilx yA motatxdalilx, Aguva) vayxtAyxsa yA A vayxtAyxsada parxmANa: \eng{repeated variations of temperature} (havAmAna \mo vugaLalilx) matetxmatetx Aguva tApa(parxmANa)da badalAvaNegaLu, tApAMtaragaLu. \eng{it is subject to variation of several degrees} adara tApadalilx halavu DigirxgaLaSuTx vayxtAyxsa Aguva saMBavavide. 
\num{3} (\Kavi) cuyxti; KAgoVLika kAyavu tananx sarAsari kakeSx yA calaneyanunx biTuTxhoVguvudu. 
\num{4} (\ga) vayxtayxyana; Palana oMdaralilxna sithxragaLu \mo vugaLalAlxguva alapx badalAvaNeya PalavAgi PalanadalAlxguva badalAvaNe. 
\num{5} (mAdarigiMta, namUnegiMta, parxmANakikxMta, jAti yA vagaRkikxMta) BinanxvAda vasutx, viSaya. 
\num{6} (\saM) rAgaBeVda yA viSayaBeVda; kaqtiyanunx hADuvalilx yA vAdana mADuvalilx gAyakanu yA vAdanakAranu mADuva rAgada yA viSayada BeVda yA avugaLa visatxraNa. 
\num{7} (bAYxle naqtayx) Eka nataRka yA nataRki naqtayx. 
\enum
\emng
\eentry

\bentry
\word{variational} 
\pron{veVriESanalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} badalAvaNeya; mApARDina; parivataRnada. 
\num{2} vayxtAyxsada; BeVdada; aMtarada. 
\num{3} badalAvaNege, mApARDige -- oLapaTaTx. 
\enum
\emng
\eentry

\bentry
\word{varicella} 
\pron{vAYxrisel} 
\gl{\nA}
\expl{}
\bmng
 (\veYshA) siVtALe siDubau. 
\emng
\eentry

\bentry
\word{varices} 
\pron{vAYxrisiVsfZ} 
\gl{\nA}
\expl{}
\bmng
 \eng{varix} eMba padada \bava. 
\emng
\eentry

\bentry
\word{varicocele} 
\pron{vAYxrikasiVlf} 
\gl{\nA}
\expl{}
\bmng
 reVtarxtaMtuvinalilx vAyxrikoVsf siregaLiMdAgiruva gaDeDx. 
\emng
\eentry

\bentry
\word{varicolored} 
\pron{veVrikalaDfR} 
\gl{\gu}
\expl{}
\bmng
 (\ame) = \hyperlink{varicoloured}{varicoloured}. 
\emng
\eentry

\bentry
\word{varicoloured} 
\pron{veVrikalaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vividha vaNaRda; vaNaRveYvidhayxvuLaLx. 
\num{2} nAnA vaNaRgaLa; bagebageya baNaNxgaLa; vividha baNaNxgaLa. 
\enum
\emng
\eentry

\bentry
\word{varicose} 
\pron{vAYxrikoVsf} 
\gl{\gu}
\expl{}
\bmng
 vAYxrikoVsf; (\kanmu\ kAlugaLa siregaLa \vi) ubibxruva; UdikoMDiruva. 
\emng
\eentry

\bentry
\word{varicosity} 
\pron{vAYxrikAsiTi} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ kAlugaLa siregaLa \vi) Uta; ubibxkoMDiruvike. 
\emng
\eentry

\bentry
\word{varied} 
\pron{veVriDf} 
\gl{\gu}
\expl{}
\bmng
 veYvidhayxvuLaLx; veYvidhayxmayavAda : \eng{a varied scene} veYvidhayxmaya daqshayx. \eng{a varied life} veYvidhayxvuLaLx jiVvana. 
\emng
\eentry

\bentry
\word{variedly} 
\pron{veVriDfli} 
\gl{\kirxvi}
\expl{}
\bmng
 veYvidhayxmayavAgi. 
\emng
\eentry

\bentry
\word{variegate} 
\pron{veVrigeVTf, veVriageVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} veYvidhayxgoLisu; vividha rUpa koDu: 
\banum
\alnum{a} \kanmu\ vividhavaNaRdadxnAnxgi mADu; vaNaRveYvidhayxvanunx uMTumADu. 
\alnum{b} (A mUlaka) rUpaveYvidhayxvanunxMTumADu. 
\eanum
\numie
\num{2} bagebageya baNaNxda gurutanunx yA macecxyanunx hAku; baNaNxbaNaNxda paTeTxgaLanunx hAku; shabaliVkarisu. 
\enum
\emng
\eentry

\bentry
\word{variegated} 
\pron{veVrigeVTiDf} 
\gl{\gu}
\expl{}
\bmng
 (\savi) eraDu yA hecucx baNaNxgaLiruva elegaLuLaLx. 
\emng
\eentry

\bentry
\word{variegation} 
\pron{veVrigeVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} rUpaveYvidhayxvanunx yA vaNaRveYvidhayxvanunx uMTumADuvudu. 
\num{2} rUpaveYvidhayx yA vaNaRveYvidhayx; shabalate. 
\enum
\emng
\eentry

\bentry
\word{varietal} 
\pron{vareYaTalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} (\kanmu\ \savi\ matutx \pArxvi) upajAtiVya; upavagiRVya: 
\banum
\alnum{a} oMdu upajAtige yA vagaRkekx seVrida. 
\alnum{b} oMdu upajAti yA vagaR Aguva. 
\alnum{c} oMdu upajAtiyanunx yA vagaRvanunx sUcisuva. 
\eanum
\numie
\num{2} (madayxda \vi) oMdu nidiRSaTx jAtiya dArxkiSxyiMda mADida. 
\enum
\emng
\eentry

\bentry
\word{varietally} 
\pron{vareYaTali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\numi{1} (\kanmu\ \savi\ matutx \pArxvi) upajAtiVyavAgi; upavagiRVyavAgi: 
\banum
\alnum{a} oMdu upajAtige yA vagaRkekx seVridaMte. 
\alnum{b} avanunx rUpisuvaMte. 
\alnum{c} avanunx sUcisuvaMte. 
\eanum
\numie
\num{2} (veYnina \vi) oMdu nidiRSaTx jAtiya dArxkiSxyiMda mADidaMte. 
\enum
\emng
\eentry

\bentry
\word{varietist} 
\pron{vareYaTisfTx} 
\gl{\nA}
\expl{}
\bmng
 vilakaSxNa puruSa; vicitarx vayxkitx; aBAyxsagaLu \mo vugaLalilx itararigiMta beVre bageyavanu. 
\emng
\eentry

\bentry
\word{variety} 
\pron{vareYaTi} 
\gl{\nA}
\expl{(\bava\ \eng{varieties}). }
\bmng
\bnum
\num{1} bagebage(yAgiruvudu); vividhate; veYvidhayx: \eng{variety} \eng{of the scenery} A daqshAyxvaLiya veYvidhayx. \eng{Bangalore has the charm of variety} beMgaLUru nagarakekx veYvidhayxda moVhakate ide. 
\num{2} bahumuKate: \eng{the variety of his conversation} avana saMBASaNeyalilx kaMDubaruva bahumuKate. 
\num{3} bagebageya, vividha, nAnA bageya -- vasutxgaLu yA viSayagaLu yA avugaLa samudAya: \eng{a variety of books} bagebageya pusatxkagaLu. \eng{a variety of reasons} nAnA bageya kAraNagaLu. 
\hypertarget{variety(4)}{} 
\num{4} vividha vinoVdAvaLi; naqtayx, gAyana, hAsAyxBinaya, \mo vugaLa parxdashaRna kAyaRkarxma. 
\num{5} (\jiVvi) upajAti; sAmAnayx lakaSxNagaLalilx parxdhAna jAtivoMdariMda BinanxvAda oMdu upajAti yA adaralolxMdu pArxNi. 
\numi{6} (\jiVvi) 
\banum
\alnum{a} upajAti; oMdeV jAtige seVridudAgi, jAtiyoLage punarutapxtitx sAdhayxviruva, Adare AnuvaMshika lakaSxNagaLu savxlapxmaTiTxge BinanxvAgiruva vayxkitx yA guMpu. 
\alnum{b} = \hyperref{kandict_c.pdf}{C}{cultivar}{cultivar}. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\wordnospeech{variety show}{variety show}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{variety(4)}{variety\((4)\)}. 
\emng
\eentry

\bentry
\wordnospeech{variety store}{variety store}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\ame) vividhavasutxgaLa aMgaDi; nAnA bageya saNaNxpuTaTx vasutxgaLanunx mAruva aMgaDi. 
\emng
\eentry

\bentry
\wordnospeech{variety theatre}{variety theatre}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 vividha vinoVda parxdashaRna shAle; saMgiVta, nATaka, naqtayx, \mo\ halavu bageya vinoVdagaLanunx parxdashiRsuva maMdira. 
\emng
\eentry

\bentry
\word{varifocal} 
\pron{veVriphoVkalf} 
\gl{\gu}
\expl{}
\bmng
 (masUrada \vi) vayxtayxyanABiya; nABidUra vayxtAyxsavAgabalalx. 
\emng
\eentry

\bentry
\word{varifocals} 
\pron{veVriphoVkalfsx} 
\gl{\nA}
\expl{(\bava)}
\bmng
 vayxtayxyanABi masUragaLa kananxDaka. 
\emng
\eentry

\bentry
\word{variform} 
\pron{veVriphAmfR} 
\gl{\gu}
\expl{}
\bmng
 vividha rUpagaLuLaLx; nAnArUpagaLuLaLx; bahurUpi; nAnArUpi. 
\emng
\eentry

\bentry
\word{variola} 
\pron{vareYal} 
\gl{\nA}
\expl{}
\bmng
 (\veYshA) siDubau. 
\emng
\eentry

\bentry
\word{variolar} 
\pron{vareYalarf} 
\gl{\gu}
\expl{}
\bmng
 (\veYshA) siDubina. 
\emng
\eentry

\bentry
\word{variole} 
\pron{veVriOlf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} siDubina macecxyaMtha taTeTxyAda kuLi. 
\num{2} veVriyoleYTfnalilxruva cikakx goVlAkArada mudedxgaLu. 
\enum
\emng
\eentry

\bentry
\word{variolite} 
\pron{veVrialeYTf} 
\gl{\nA}
\expl{}
\bmng
 veVriyoleYTf; siDubaubaMDe; cikakxcikakx goVlAkArada mudedxgaLu meVlemxYyalilx hudugiruva, siDubina kalegaLiMda tuMbidaMte kANisuva meVlemxYuLaLx baMDe. 
\emng
\eentry

\bentry
\word{variolitic} 
\pron{veVrialiTikf} 
\gl{\gu}
\expl{}
\bmng
 siDubaubaMDeyaMtha; veVriyoleYTfnaMtha. 
\emng
\eentry

\bentry
\word{varioloid} 
\pron{vareYalAyfDx} 
\gl{\gu}
\expl{}
\bmng
 (\veYshA) siDubinaMtha. 
\emng
\eentry

\bentry
\word{variolous} 
\pron{vareYalasf} 
\gl{\gu}
\expl{}
\bmng
 (\veYshA) siDubauLaLx; siDubiniMda kUDida. 
\emng
\eentry

\bentry
\word{variometer} 
\pron{veVriAmiTarf} 
\gl{\nA}
\expl{}
\bmng
 veVriyAmiVTaru: 
\banum
\alnum{a} vayxtayxyakAraka; elekipxrXkf sakiRTf oMdara coVdakate \eng{(inductance)}yanunx badalAyisalu baLasuva oMdu sAdhana. 
\alnum{b} vayxtayxyasUcike; vimAna oMdara unanxti vayxtAyxsavAgutitxruva daravanunx sUcisuva sAdhana. 
\eanum
\emng
\eentry

\bentry
\word[variorum(1)]{variorum} 
\pron{veVriO(A)ramf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vividha TiVkAnivxta; beVrebeVre vAyxKAyxnakArara TiVkegaLanonxLagoMDa. 
\num{2} (obabx leVKakana kaqtigaLa Avaqtitxya bagegx) beVrebeVre pAThAMtaragaLanonxLagoMDa. 
\enum
\emng
\eentry

\bentry
\word[variorum(2)]{variorum} 
\pron{veVriOramf} 
\gl{\nA}
\expl{}
\bmng
 vividha pAThAnivxta yA TiVkAnivxta garxMtha. 
\emng
\eentry

\bentry
\word{various} 
\pron{veVriasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} beVrebeVreyAda; bagebageya; vividha: \eng{types so various as to defy classification} vagiRVkarisalAgadaSuTx vividha bagegaLu. 
\num{2} halavAru; halavoMdu; halavu bageya; bahu vidhavAda: \eng{I saw various kinds of people there} nAnA bageya janaranunx nAnalilx kaMDe. \eng{for various reasons} halavu bageya kAraNagaLigAgi. 
\enum
\emng
\eentry

\bentry
\word{variously} 
\pron{veVriasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 nAnA bageyAgi; bagebageyAgi; vidhavidhavAgi. 
\emng
\eentry

\bentry
\word{variousness} 
\pron{veVriasfnisf} 
\gl{\nA}
\expl{}
\bmng
 nAnA bageyAgiruvike; veYvidhayx. 
\emng
\eentry

\bentry
\word{varistor} 
\pron{varisaTxrf} 
\gl{\nA}
\expl{}
\bmng
 (\viduyx) parivataRna roVdhaka; parxyoVga mADida voVleTxVjanunx avalaMbisi roVdhavu badalAguva yA vayxtAyxsavAguva arevAhaka DayoVDu. 
\emng
\eentry

\bentry
\word{varix} 
\pron{veVrikfsx} 
\gl{\nA}(\bava\ \eng{varices}
\expl{\ucAcx\ vAYxrisiVsfZ). }
\bmng
\bnum
\num{1} (\veYshA) sireya yA dhamaniya Uta; sire yA dhamaniyu shAshavxtavAgi matutx apasAmAnayxvAgi higigxruvudu. 
\num{2} hAge ubibxruva sire yA dhamani. 
\num{3} Eka kavATada cipipxna suruLiya meVliruva diMDugaLalolxMdu. 
\enum
\emng
\eentry

\bentry
\word{varlet} 
\pron{vAliRTf} 
\gl{\nA}
\bmng
\bnum
\numi{1} (\pArxparx\ yA \hA) 
\banum
\alnum{a} (\hiV) seVvaka; kiVLumanuSayx; totutx; dAsa; gulAma. 
\alnum{b} kiVLumanuSayx; halAkxmanuSayx; shudadhx niVca. 
\alnum{c} tuMTa; puMDa; PaTiMga. 
\eanum
\numie
\num{2} (\ca) `neYTf' na birudAMkita BaTa, anucara; muMde tAnU obabx `neYTf' AgabeVkeMdu parxsidadhx `neYTf' obabxna baLi tarabeVti paDeyutitxdadx, eLehareyada taruNa. 
\enum
\emng
\eentry

\bentry
\word{varletry} 
\pron{vAliRTirx}
\gl{\nA}
\expl{}
\bmng
 totutxgaLa taMDa; dAsavagaR; seVvaka vagaR. 
\emng
\eentry

\bentry
\word{varmint} 
\pron{vAmiRMTf} 
\gl{\nA}
\expl{}
\bmng
 (\pArxM\ yA \ame). 
\bnum
\num{1} tuMT(manuSayx); niVca(manuSayx). 
\num{2} tuMTa, puMDa -- pArxNi, \kanmu\ nari. 
\enum
\emng
\eentry

\bentry
\word{varna} 
\pron{vAnaR} 
\gl{\nA}
\expl{}
\bmng
 vaNaR; hiMdUdhamaRda bArxhamxNa, kaSxtirxya, veYshayx, shUdarx eMba nAlukx vaNaRgaLalolxMdu. 
\emng
\eentry

\bentry
\word[varnish(1)]{varnish} 
\pron{vAniRSf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} merugeNeNx; vAniRVsu; mara, loVha, \mo vugaLige nuNupu hoLapu koDalu tikukxva aMTudarxvayx. 
\num{2} teVjaka; kAMtida; (bagebageya) merugu(darxvayx): \eng{nail varnish} uguru meragu; naKa -- teVjaka, kAMtada. 
\num{3}(sAvxBAvikavoV kaqtakavoV Ada) hoLapu; kAMti; thaLathaLike. 
\num{4} (naDenuDigaLalilx) bAhayxnaya; thaLuku. 
\num{5} kapaTavinaya; toVkeRnamarxte; soVgu mayARde; aMtaraMgada kApaTayxvanunx mucucxva, horagaNa vinaya mayARdegaLa soVgu. 
\enum
\emng
\eentry

\bentry
\word[varnish(2)]{varnish} 
\pron{vAniRSf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (mara, loVha, \mo vakekx) merugu -- hacucx, koDu; opapxviDu. 
\num{2} (vayxkitx, naDate, kAyaR, kathana yA heVLike \mo vakekx) baNaNx kaTuTx; merugu koDu; horagina naya koDu (\akirx\ saha). 
\num{3} apirxyavAda saMgatige (pirxyavAgi toVruvaMte) baNaNxkaTuTx; ahitavAda viSayavanunx maqduvAgi, upAyavAgi teVlisibiDu. 
\enum
\emng
\eentry

\bentry
\word{varnisher} 
\pron{vAniRSarf} 
\gl{\nA}
\expl{}
\bmng
 vAniRVsugAra; merugeNeNx -- mAruvava yA hAkuvava. 
\emng
\eentry

\bentry
\word{varnishing-day} 
\pron{vAniRSiMgfDeV} 
\gl{\nA}
\expl{}
\bmng
 merugu koDuva, opapxviDuva -- dina; (parxdashaRna samAraMBadalilx) parxdashaRnagoLisalu modaleV tUgahAkiruva citarxgaLige, parxdashaRnakekx muMce merugu koDalu gotutxmADida, parxdashaRnada hiMdina dina. 
\emng
\eentry

\bentry
\wordnospeech{varnish tree}{varnish tree}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 merugeNeNx mara; merugeNeNx tayArisalu upayukatxvAda mara. 
\emng
\eentry

\bentry
\word{varsity} 
\pron{vAsiRTi} 
\gl{\nA}
\expl{(\bava\ \eng{varsities}). }
\bmng
\bnum
\num{1} (\birx) (\AmA) (muKayxvAgi kirxVDegaLa \vi) = \hyperref{kandict_u.pdf}{U}{university}{university}. 
\num{2} (\ame) (vishavxvidAyxnilaya \mo vugaLa) parxthama kirxVDAdaLa. 
\enum
\emng
\eentry

\bentry
\word{varus} 
\pron{veVrasf} 
\gl{\nA}
\expl{}
\bmng
 oLatirucu; oLamaDicu; oLaDoMku; kAlina aDi yA aMgeY oLagaDege tirucikoMDiruva aMgaveYkalayx. 
\emng
\eentry

\bentry
\word{varve} 
\pron{vAvfR} 
\gl{\nA}
\expl{}
\bmng
 vASiRka -- padaru, nikeSxVpa, saMcaya; saroVvarada taladalilx vaSaRvelalx sheVKarisikoMDu gaTiTxyAgutAtx hoVguva, himanadigaLa saMcayagaLa kAlaniNaRyakekx baLasuva jeVDimaNuNx matutx maraLina rAshi. 
\emng
\eentry

\bentry
\word{varved} 
\pron{vAvfDxR} 
\gl{\gu}
\expl{}
\bmng
 (vaSaRvelalx sheVKaragoMDa jeVDimaNuNx maraLugaLa) vASiRka padaruLaLx; vASiRka nikeSxVpavuLaLx. 
\emng
\eentry

\bentry
\word{vary} 
\pron{veVri} 
\gl{\kirx}(vataRmAna \parxpu\ \Eva\ \eng{varies}; \BU\ matutx 
\BUkaq\ \eng{varied}). \bmng
\emng

\noindent
\gl{\sakirx}
\bmng
\bnum
\num{1} badalAyisu; mApaRDisu; vayxtAyxsagoLisu: \eng{we can vary the pressure at will} otatxDavanunx, saMmadaRvanunx, nAvu beVkAdaMte mApaRDisabahudu. \eng{he seldom varies the routine} dinacariyanunx avanu badalAyisuvudu tiVra viraLa, bahushaH badalAyisuvudilalx. \eng{the doctor varies the treatment according to circumstances} veYdayxnu cikitAsxvidhAnavanunx saMdaBoRVcitavAgi vayxtAyxsa mADutAtxne. 
\num{2} veYvidhayxgoLisu: \eng{his style is not sufficiently varied} avana sheYliyalilx sAkaSuTx veYvidhayxvilalx. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (guNa, rUpa, pariNAma, \mo vugaLalilx) badalAyisu; badalAvaNeyAgu; vayxtAyxsavAgu: \eng{his mood varies from day to day} avana manasusx, manaHsithxti dinadiMda dinakekx badalAyisutitxrutatxde. \eng{the temperature varies from} \eng{$30^\circ$} \eng{to} \eng{$70^\circ$} tApamAnavu \eng{$30^\circ$} yiMda \eng{$70^\circ$} ge vayxtAyxsavAgutatxde. 
\num{2} beVreyAgiru; BinanxvAgiru: \eng{this varies from the type} idu tananx jAtiyiMda BinanxvAgide, beVre bageyadAgide. 
\num{3} beVrebeVreyAgiru; bagebageyAgiru; vividhavAgiru; Binanx BinanxvAgiru: \eng{opinions vary on this point} I viSayadalilx aBipArxyagaLu bagebageyAgive, BinanxBinanxvAgive. 
\num{4} anuguNavAgiru; anusAravAgi, anuguNavAgi badalAvaNe hoMdu: \eng{the demand varies with the season} beVDike kAlakekx anuguNavAgirutatxde, anuguNavAgi vayxtAyxsavAgutatxde. 
\num{5} (\ga) (anuloVmavAgi yA viloVmavAgi) badalAyisu; Binanx mwlayxgaLanunx paDe: \eng{attraction of bodies varies directly as their masses and inversely as the square of their distance} kAyagaLa parasapxra AkaSaRNe avugaLa parimANakekx anuloVmavAgi matutx dUrakekx viloVmavAgi vayxtAyxsavAgutatxde, badalAyisutatxde. 
\enum
\emng
\eentry

\bentry
\word{varyingly} 
\pron{veV(ri)iMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
 badalAyisuvaMte; vayxtAyxsavAguvaMte. 
\emng
\eentry

\bentry
\word{vas} 
\pron{vAYxsf} 
\gl{\nA}(\bava\ \eng{vasa}
\expl{\ucAcx\ veVsa). }
\bmng
(\aMrashA) nALa. 
\emng
\eentry

\bentry
\word{vasal} 
\pron{veVsalf} 
\gl{\gu}
\expl{}
\bmng
nALada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vascular} 
\pron{vAYxsukxyXlarf} 
\gl{\gu}
\expl{}
\bmng
 nALiVya; (rakatx, sasayxrasa, \mo vanunx koMDoyuyxva) nALada yA nALagaLiMda racitavAda yA nALagaLanunx hoMdiruva. 
\emng
\eentry

\bentry
\word{vascularise} 
\pron{vAYxsukxyXlareYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vascularize}{vascularize}. 
\emng
\eentry

\bentry
\word{vascularity} 
\pron{vAYxsukxyXlAYxriTi} 
\gl{\nA}
\expl{}
\bmng
 nALiVyate; nALadaMtiruvike. 
\emng
\eentry

\bentry
\word{vascularize} 
\pron{vAYxsukxyXlareYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 nALarUpavAgisu; nALiVkarisu. 
\emng
\eentry

\bentry
\word{vascularly} 
\pron{vAYxsukxyXlarfli} 
\gl{\kirxvi}
\expl{}
\bmng
 nALiVyavAgi yA nALagaLiruvaMte. 
\emng
\eentry

\bentry
\wordnospeech{vascular plant}{vascular plant}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 nALasasayx; nALagaLaMtha Utaka uLaLx sasayx. 
\emng
\eentry

\bentry
\word{vasculum} 
\pron{vAYxsukxyXlamf} 
\gl{\nA}
\expl{(\bava\ \eng{vascula} \ucAcx\ vAYxsukxyXla). }
\bmng
 (sasayxshAsatxrXjacnxru sasayxmAdarigaLanunx saMgarxhisiTuTxkoLaLxlu baLasuva, \sA\ ududxdadxnAgi tereyuvaMtiruva) loVhada sasayxsaMpuTa. 
\emng
\eentry

\bentry
\wordRemoveSpace{vas-deferens}{vas deferens} 
\pron{vAYxsf Depharenfs'} 
\gl{\nA}
\expl{(\bava\ \eng{vasa deferentia} \ucAcx\ vAsf DaphareniSxya). }
\bmng
 (\aMrashA) reVtarxnALa; reVtarxvanunx vaqSaNadiMda mUtarxnALakekx koMDoyuyxva nALa. 
\emng
\eentry

\bentry
\word{vase} 
\pron{vAsfZ} 
\gl{\nA}
\expl{}
\bmng
 hUkuMDa; puSapxdAni; \kanmu\ hUvugaLanunx joVDisiTuTx alaMkAra goLisuvudakAkxgi baLasuva maNiNxna, gAjina, piMgANiya yA loVhada kalasha, kuMBa. 
\emng
\eentry

\bentry
\word{vasectomise} 
\pron{vasekaTxmeYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vasectomize}{vasectomize}. 
\emng
\eentry

\bentry
\word{vasectomize} 
\pron{vasekaTxmeYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 (vayxkitxge) reVtarxnALoVcePxVdane mADu. 
\emng
\eentry

\bentry
\word{vasectomy} 
\pron{vasekaTxmi} 
\gl{\nA}(\bava\ \eng{vasectomies}). 
\bmng
 (\shaveY) reVtarxnALoVcePxVdane; saMtAnaharaNakAkxgi reVtarxnALada BAgavanunx shasatxrXkirxyeyiMda katatxrisi tegedu hAkuvudu. 
\emng
\eentry

\bentry
\word{vaseful} 
\pron{vAYxsfPuZlf} 
\gl{\nA}(\bava\ \eng{vasefuls}). 
\bmng
 hUkalashada tuMba; puSapxdAni tuMbauvaSuTx parxmANa. 
\emng
\eentry

\bentry
\word[vaseline(1)]{Vaseline} 
\pron{vAYxsiliVnf} 
\gl{\nA}
\expl{(\P) }
\bmng
 vAYxsiliVnu; peTorxVliyaniMda tayArisida oMdu bageya mulAmu, leVpanadarxvayx. 
\emng
\eentry

\bentry
\word[vaseline(2)]{vaseline} 
\pron{vAYxsiliVnf} 
\gl{\sakirx}
\expl{}
\bmng
 vAYxsfliVnf -- hacucx, baLi, savaru, leVpisu. 
\emng
\eentry

\bentry
\wordnospeech{vase painting}{vase painting}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 \eng{(} pArxciVna girxVkaru, ciVniVyaru, \mo varu mADutitxdadx) kuMBacitarxNa; kalashada meVlaNa citarxNa. 
\emng
\eentry

\bentry
\word{vasiform} 
\pron{veVsiZphAmfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nALarUpi. 
\num{2} hUdAnirUpada; hUdAniya AkArada. 
\enum
\emng
\eentry

\bentry
\word{vaso-} 
\pron{veVsoV-}
\gl{\sapUpa}
\expl{}
\bmng
 rakatxnALada, rakatxnALakekx saMbaMdhisida eMbathaRda \sapUpa 
\emng
\eentry

\bentry
\word{vasoactive} 
\pron{veVsoZVAYxkiTxvf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vasomotor}{vasomotor}. 
\emng
\eentry

\bentry
\word{vasoconstrictive} 
\pron{veVsoZVkanfsiTxrXkiTxvf} 
\gl{\gu}
\expl{}
\bmng
 rakatxnALasaMkoVcaka; rakatxnALavanunx kiridAgisuva. 
\emng
\eentry

\bentry
\word{vasodilating} 
\pron{veVsoZVDeYleVTiMgf} 
\gl{\gu}
\expl{}
\bmng
 rakatxnALa vAyxkoVcaka; rakatxnALavanunx higagxlisuva. 
\emng
\eentry

\bentry
\word{vasodilation} 
\pron{veVsoZVDeYleVSanf} 
\gl{\nA}
\expl{}
\bmng
 rakatxnALavAyxkoVcana; rakatxnALavanunx higagxlisuvike. 
\emng
\eentry

\bentry
\word{vasomotor} 
\pron{veVsoZVmoVTarf} 
\gl{\gu}
\expl{}
\bmng
 nALa -- vaqdidhxhArxsaka; rakatxnALavanunx higagxlisuva yA saMkoVcisuva. 
\emng
\eentry

\bentry
\word{vasopressin} 
\pron{veVsoZVperxsinf} 
\gl{\nA}
\expl{}
\bmng
 (\jiVra) mUtarxvadhaRkavAgi matutx rakatxdotatxDa vadhaRkavAgi kelasa mADuva piTUyxTari hAmoRVnu. 
\emng
\eentry

\bentry
\word{vassal} 
\pron{vAYxsalf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\ca) (ULigamAnayx padadhxtiyalilx doreyiMdaleV neVravAgi yA parxdhAna hiDuvaLidAraniMda paroVkaSxvAgi hiDuvaLiyanunx yA uMbaLiyanunx paDeyutitxdadx) hiDuvaLidAra; uMbaLidAra; ULigadavanu. 
\num{2} sAmaMta; Ashirxta; dorege adhiVna nAda, pALeyapaTiTxna ADaLitagAra. 
\num{3} (\rUpa) ULigadALu; totutx; aDiyALu; dAsa. 
\enum
\emng
\eentry

\bentry
\word{vassalage} 
\pron{vAYxsalijf} 
\gl{\nA}
\expl{}
\bmng
 ULigatana; totutxtana; aDiyALutana; dAsayx. 
\emng
\eentry

\bentry
\word[vast(1)]{vast} 
\pron{vAsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} bahudoDaDx; BAri; baqhatAtxda; mahA: \eng{he shook his vast frame} avanu tananx BAriV deVhavanunx alAlxDisida. 
\num{2} tiVra vishAlavAda; bahu visAtxravAda: \eng{a vast expanse of water} tiVra vishAlavAda niVrina harahu. \eng{vast plains} bahaLa visAtxravAda bayalusiVmegaLu. 
\num{3} agAdha; apAra: \eng{a vast multitude} apAra janasotxVma. \eng{a vast scheme} agAdhavAda oMdu yoVjane. 
\num{4} (\AmA) tuMba; sAkaSuTx; bahaLa; BAri; apAra: \eng{this little success gave him vast satisfaction} I alapx vijayaveV avanige apAra taqpitxyanunx taMdukoTiTxtu. \eng{even this bit will make a vast difference} I alapxveV sAkaSuTx vayxtAyxsavanunxMTumADutatxde. 
\enum
\emng
\eentry

\bentry
\word[vast(2)]{vast} 
\pron{vAsfTx} 
\gl{\nA}
\expl{}
\bmng
 (\kAparx\ yA \rUpa) vishAlavAda, visAtxravAda -- parxdeVsha, jAga, haravu: \eng{the vast of heaven} vishAlavAda AkAsha parxdeVsha. 
\emng
\eentry


\bentry
\word{vastly} 
\pron{vAsfTxli} 
\gl{\kirxvi}
\expl{}
\bmng
 tuMba; bahaLa; apAravAgi. 
\emng
\eentry

\bentry
\word{vastness} 
\pron{vAsfTxnisf} 
\gl{\nA}
\expl{}
\bmng
 apArate; veYshAlayx 
\emng
. \eentry

\bentry
\word{VAT} 
\pron{viVETiV, vAYxTf} 
\gl{\saMkiSx}
\expl{}
\bmng
(\UK yalilx) \eng{value added tax.} 
\emng
\eentry

\bentry
\word[vat(1)]{vat} 
\pron{vAYxTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} BAri toTiTx; doDaDx piVpAyi; darxvagaLanunx tuMbiDalu yA darxvagaLalilx vasutx gaLanunx adadxlu baLasuva doDaDxgAtarxda pAterx yA toTiTx. 
\num{2} (baNaNxkaTaTxlu baTeTxgaLanunx adidx toVyisalu raMganunx berasida niVranunx tuMbida) piVpAyi yA toTiTx. 
\enum
\emng
\eentry

\bentry
\word[vat(2)]{vat} 
\pron{vAYxTf} 
\gl{\sakirx}(\BU\ matutx \BUkaq\ \eng{vatted}; \vakaq\ \eng{vatting}). 
\bmng
 (raMgina niVru tuMbida) toTiTxyalilx -- hAku, adudx, toVyisu, nenesu. 
\emng
\eentry

\bentry
\word{vatful} 
\pron{vAyxTfPuZlf} 
\gl{\nA}(\bava\ \eng{vatfuls}). 
\bmng
piVpAyi tuMba; toTiTx tuMbauvaSuTx parxmANa. 
\emng
\eentry

\bentry
\word{vatic} 
\pron{vAYxTikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BaviSayxvANiya; BaviSayxsUcaka(vAda); deYvAveVshadiMda muMde Aguva viSayagaLanunx modaleV heVLuva. 
\num{2} sUPxtiRyukatxvAda. 
\enum
\emng
\eentry

\bentry
\word{Vatican} 
\pron{vAYxTikanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vAYxTikanf; roVmf nagaradalilxruva poVpfgurugaLa aramane, nivAsa hAgU kAyARgAra. 
\num{2} poVpf parxButavx; poVpara ADaLita. 
\enum
\emng
\eentry

\bentry
\wordnospeech{Vatican City}{Vatican City}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 vAYxTikanf nagara; \eng{1929}ralilx roVmf nagaradalilx sAthxpitavAda poVpara parxteyxVka nagara rASaTxrX. 
\emng
\eentry

\bentry
\wordnospeech{Vatican Council}{Vatican Council}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 vAYxTikanf kwnisxlf: 
\banum
\alnum{a} \eng{1869-70}ralilx neredu, poVparu adhikaqtavAgi heVLida mAtu paramasatayxveMdU adaralilx doVSakekx eDeyeV ilalxveMdU GoVSisida, iDiV parxpaMcada savaRkerxYsatx saBe. 
\alnum{b} \eng{1962-65}ralilx matetx naDeda savaRkerxYsatxsaBe. 
\eanum
\emng
\eentry

\bentry
\word{Vaticanism} 
\pron{vAYxTikanisaZmf} 
\gl{\nA}
\expl{}
\bmng
 vAYxTikanf sidAdhxMta; poVpara adhikaqta heVLike paramasatayx, doVSAtiVtavAdadudx eMba sidAdhxMta. 
\emng
\eentry

\bentry
\word{Vaticanist} 
\pron{vAYxTikanisfTx} 
\gl{\nA}
\expl{}
\bmng
 vAYxTikanf sidAdhxMti; poVpara adhikaqta heVLike paramasatayxveMdU doVSAtiVtavAdadedxMdU parxtipAdisuvavanu. 
\emng
\eentry

\bentry
\word{vaticinal} 
\pron{vAYxTisinalf} 
\gl{\gu}
\expl{}
\bmng
 BaviSayx -- vANiya, kathanada. 
\emng
\eentry

\bentry
\word{vaticinate} 
\pron{vAYxTisineVTf} 
\gl{\sakirx}
\expl{}
\bmng
 (\aupa) BaviSayx heVLu (\akirx\ saha). 
\emng
\eentry

\bentry
\word{vaticination} 
\pron{vAYxTisineVSanf} 
\gl{\nA}
\expl{}
\bmng
 BaviSayx kathana; BaviSayx heVLuvike. 
\emng
\eentry

\bentry
\word{vaticinator} 
\pron{vAYxTisineVTarf} 
\gl{\nA}
\expl{}
\bmng
 BaviSayxvAdi; BaviSayx heVLuvavanu. 
\emng
\eentry

\bentry
\word{vaticinatory} 
\pron{vAYxTisinaTari} 
\gl{\gu}
\expl{}
\bmng
 BaviSayx -- vANiya, kathanada. 
\emng
\eentry

\bentry
\word{VATman} 
\pron{vAYxTfmAYxnf} 
\gl{\nA}(\bava\ \eng{VATmen}
\expl{\ucAcx\ vAYxTfmenf). }
\bmng
 (\AmA) mwlayxvadhiRta terige (hAkuva suMkada) adhikAri. 
\emng
\eentry

\bentry
\word{vaudeville} 
\pron{vA(voV)Davilf} 
\gl{\nA}
\expl{}
\bmng
 voVDavilf: 
\banum
\alnum{a} (\kanmu\ \ame) vividha vinoVdAvaLi. 
\alnum{b} tatAkxlakekx saMbaMdhisida viSayavanunx kurita yA viDaMbanAtamxkavAda, palalxviyanonxLagoMDa hADu. 
\alnum{c} naDunaDuve hADugaLiMda kUDida laGunATaka. 
\eanum
\emng
\eentry

\bentry
\word[vaudevillian(1)]{vaudevillian} 
\pron{vA(voV)Davilianf} 
\gl{\gu}
\expl{}
\bmng
voVDfvilf rUpada; vividha vinoVdAvaLiyaMtha yA laGu giVtanATakadaMtha. 
\emng
\eentry

\bentry
\word[vaudevillian(2)]{vaudevillian} 
\pron{vA(voV)Davilianf} 
\gl{\nA}
\expl{}
\bmng
 vividha vinoVdAvaLiyaMtha yA laGu giVtanATaka daMtha -- parxdashaRna. 
\emng
\eentry

\bentry
\word[Vaudois(1)]{Vaudois}
\pron{voVDAvx}
\gl{\nA}
\expl{(\bava\ adeV).}
\bmng
 voVDAvx: 
\banum
\alnum{a} pashicxma sivxTasxlaRMDina voVDf parxdeVshada nivAsi. 
\alnum{b} voVDf parxdeVshadalilx mAtanADuva pArxMtiVya pherxMcf BASe. 
\eanum
\emng
\eentry

\bentry
\word[Vaudois(2)]{Vaudois} 
\pron{voVDAvx} 
\gl{\gu}
\expl{}
\bmng
 voVDf janara yA BASeya yA avugaLige saMbaMdhisida. 
\emng
\eentry

\bentry
\word[Vaudois(3)]{Vaudois} 
\pron{voVDAvx} 
\gl{\nA}
\bmng
(\bava\ adeV). (\ca) voVDAvx: vAleDxniVsfZ dhAmiRka saMGada sadasayx. 
\emng
\eentry

\bentry
\word[Vaudois(4)]{Vaudois} 
\pron{voVDAvx} 
\gl{\gu}
\expl{}
\bmng
 vAleDxniVsfZ saMGada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word[vault(1)]{vault} 
\pron{vAlfTx} 
\gl{\nA}
\bmng
\bnum
\num{1} (\vAshi) kamAnucAvaNi; bilulxcAvaNi; cApavitAna. \imglink{\(^1\)vaults 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.6cm{V_Pictures/vaults.jpg}}} 
\num{2} kamAnu. 
\num{3} (oMdeV keVMdarxdiMda horaDuva) kamAnu sAlu yA taMDa. 
\num{4} gumamxTa; bililxnAkArada yA goVLAkArada (oLa)cAvaNi: \eng{the vault of heaven} AkAshagumamxTa. 
\num{5} gumamxTacAvaNiya nelamALige -- koVNe, koThaDi. 
\num{6} (veYnu, bAyxMkina TheVvaNigaLu, \mo vanunx surakiSxtavAgiDuva) gumamxTa cAvaNiya nelamALige: \eng{bank vault} bAyxMkina nelamALige. 
\num{7} gumamxTa samAdhi; shavasaMsAkxra mADuva, gumamxTacAvaNiyuLaLx -- nelamALige: \eng{family vault} vaMshadavara (samAdhigaLiruva) gumamxTa samAdhi. 
\num{8} (\aMrashA) (deVhada racaneyalilx) kuharada, gUDina -- gumamxTa cAvaNi. 
\num{9} negeta; laMGana. 
\enum
\emng
\eentry

\bentry
\word[vault(2)]{vault} 
\pron{vAlfTx} 
\gl{\sakirx}
\expl{}
\bmng
 gumamxTa cAvaNiyanunx, kamAnu cAvaNiyanunx -- nimiRsu, odagisu. 
\emng
\eentry

\bentry
\word[vault(3)]{vault} 
\pron{vAlfTx} 
\gl{\sakirx}
\expl{}
\bmng
 (keY yA gaNe UrikoMDu, goVDe \mo vanunx) hAru; negedu dATu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
(keYyUrikoMDu yA gaNeya neraviniMda) meVlakekx -- jigi, nege, hAru: \eng{vault over the gate} (keY yA gaNe UrikoMDu) geVTf Acege hAru. 
\emng
\eentry

\bentry
\word[vault(4)]{vault} 
\pron{vAlfTx} 
\gl{\nA}
\expl{}
\bmng
 (keY, gaNe, \mo vanunx UrikoMDu negeyuva) negeta; jigita; hAruvike; laMGana. 
\emng
\eentry

\bentry
\word{vaulter} 
\pron{vAlaTxrf} 
\gl{\nA}
\expl{}
\bmng
 (gaNe \mo vanunx hiDidu) hAruvavanu; laMGanakAra; hAruga. 
\emng
\eentry

\bentry
\word{vaulting} 
\pron{vAliTxMgf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kamAnu cAvaNi; gumamxTa cAvaNi. 
\num{2} (vAyxyAmashAleyalilx yA kirxVDeyalilx kasaratAtxgi mADuva) laMGana; negedu, jigidu hAruvike. 
\enum
\emng
\eentry

\bentry
\word{vaulting-horse} 
\pron{vAliTxMgfhAsfR} 
\gl{\nA}
\expl{}
\bmng
 hArugudure; jigigudure; laMGanAshavx; aMgasAdhane mADuvavaru UrikoMDu hAruvudakAkxgi baLasuva marada koraDu. 
\emng
\eentry

\bentry
\word[vaunt(1)]{vaunt} 
\pron{vAMTf} 
\gl{\sakirx}
\expl{}
\bmng
 (sAhitayxka) (yAvudanenxV kuritu) jaMba hoDe; baDAyi kocucx. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 jaMba hoDe; baDAyi kocicxko. 
\emng
\eentry

\bentry
\word[vaunt(2)]{vaunt} 
\pron{vAMTf} 
\gl{\nA}
\expl{}
\bmng
 (sAhitayxka) jaMba; baDAyi; baDivAra. 
\emng
\eentry

\bentry
\word{vaunt-courier} 
\pron{vAMTfkuriarf} 
\gl{\nA}
\expl{}
\bmng
(\pArxparx) (hiMde baruvavana sudidxyanunx modaleV tiLisalu muMcitavAgi ODibaruva) agarx sudidxvAhaka; munfsudidxgAra. 
\emng
\eentry

\bentry
\word{vaunter} 
\pron{vAMTarf} 
\gl{\nA}
\expl{}
\bmng
(\pArxparx) jaMbagAra; baDAyigAra. 
\emng
\eentry

\bentry
\word{vauntingly} 
\pron{vAMTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
 jaMbadiMda; baDAyi kocucxvaMte; jaMba hoDeyuvaMte. 
\emng
\eentry

\bentry
\word{vavasory} 
\pron{vAYxvasari} 
\gl{\nA}(\bava\ \eng{vavasories}). 
\bmng
 (\ca)upasAmaMtana -- hiDuvaLi, mAnayx, uMbaLi. 
\emng
\eentry

\bentry
\word{vavasour} 
\pron{vAYxvasuarf} 
\gl{\nA}
\expl{}
(\ca) \bmng
 (ULigamAnayx padadhxtiyalilx) upasAmaMta; doDaDx sAmaMtanobabxnige adhiVnanAgi, tananx keYkeLage pALeyagAraranunx hoMdiruva sAmaMta. 
\emng
\eentry

\bentry 
\wordnospeech{VC}{VC}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{Vice-Chairman.} 
\num{2} \eng{Vice-Chancellor.} 
\num{3} \eng{Vice-Consul.} 
\num{4} \eng{Victoria Cross.} 
\enum
\emng
\eentry

\bentry
\wordnospeech{VCR}{VCR}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{video cassette recorder.} 
\emng
\eentry

\bentry
\wordnospeech{VD}{VD}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{venereal disease.} 
\emng
\eentry

\bentry
\wordnospeech{VDU}{VDU}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{visual display unit.} 
\emng
\eentry

\bentry
\wordnospeech{VE}{VE}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
(\eng{1945}ralilx) \eng{Victory in Europe} 
\emng
\eentry

\bentry
\wordnospeech{VE day}{VE day}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(yUroVpina) vijayadina; meV \eng{8}neV tAriVKu. 
\emng
\eentry

\bentry
\wordnospeech{'ve}{'ve} 
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 (utatxma puruSa savaRnAmagaLa yA itara puruSagaLa bahuvacanada savaRnAmagaLa anaMtara baLake) = \hyperref{kandict_h.pdf}{H}{have(1)}{have} (\hyperref{kandict_i.pdf}{I}{I've}{I've}; \hyperref{kandict_w.pdf}{W}{we've}{we've}; \hyperref{kandict_y.pdf}{Y}{you've}{you've}; \hyperref{kandict_t.pdf}{T}{they've}{they've}). 
\emng
\eentry

\bentry
\word{veal} 
\pron{viVlf} 
\gl{\nA}
\expl{}
\bmng
 (AhAravAgi baLasuva) karuvina mAMsa. 
\emng
\eentry

\bentry
\word{vealy} 
\pron{viVli} 
\gl{}
\expl{}
\bmng
\bnum
\num{1} karuvina mAMsadaMtha. 
\num{2} (\rUpa) eLaseLasAda; ananuBaviyAda; sAkaSuTx viveVkavilalxda. 
\enum
\emng
\eentry

\bentry
\word[vector(1)]{vector} 
\pron{vekaTxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\Bwvi\ matutx \ga) sadisha; parxmANa matutx dikukx eraDU iruva parimANa: \eng{velocity and force are vectors} veVga matutx bala eraDU sadishagaLu. 
\num{2} (\veYshA) roVgavAhaka; aMTuroVgakekx kAraNavAda sUkaSx mxjiVvigaLanunx obabxriMdobabxrige haraDuva kiVTa itAyxdi. 
\num{3} vimAnapatha; vimAnavu anusarisabeVkAda mAgaR. 
\enum
\emng
\eentry

\bentry
\word[vector(2)]{vector} 
\pron{vekaTxrf} 
\gl{\sakirx}
\expl{}
\bmng
 nideRVshisu (hArutitxruva vimAnakekx) talupabeVkAda, udidxSaTxsAthxna sUcisu. 
\emng
\eentry

\bentry
\word{vectorial} 
\pron{vekoTxVrialf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sadishada yA adakekx saMbaMdhisida. 
\num{2} (hArATadalilxruva vimAnada \vi) nideRVshaka; talupabeVkAda sAthxnavanunx sUcisuva yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{vectorise} 
\pron{vekaTxreYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vectorize}{vectorize}. 
\emng
\eentry

\bentry
\word{vectorization} 
\pron{vekaTxreYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 sadishiVkaraNa; (parimANavanunx) sadishaveMdu parigaNisuvike. 
\emng
\eentry

\bentry
\word{vectorize} 
\pron{vekaTxreYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 sadishiVkarisu; (parimANavanunx) sadishaveMdu parigaNisu. 
\emng
\eentry

\bentry
\word{Veda} 
\pron{veV(viV)Da} 
\gl{\nA}
\expl{}
\bmng
 (\Eva\ yA \bava dalilx) veVda; QugevxVda, yujuveRVda, sAmaveVda matutx athavaRveVda -- eMba nAlukx veVdagaLu. 
\emng
\eentry

\bentry
\word{Vedanta} 
\pron{viDAYxMTa, veDAMT} 
\gl{\nA}
\expl{}
\bmng
 veVdAMta: 
\banum
\alnum{a} veVdagaLa koneya BAgagaLAda upaniSatutxgaLu. 
\alnum{b} (upaniSatutxgaLanunx avalaMbisida) hiMdU dashaRna; hiMdU tatatx vxshAsatxrX. 
\eanum
\emng
\eentry

\bentry
\word{vedantic} 
\pron{viDAYxMTikf, veDAMTikf} 
\gl{\gu}
\expl{}
\bmng
 veVdAMtada. 
\emng
\eentry

\bentry
\word{vedantist} 
\pron{viDAYxMTisfTx, veDAMTisfTx} 
\gl{\nA}
\expl{}
\bmng
 veVdAMti; hiMdU dAshaRnika; hiMdUdashaRnagaLalilx paMDita. 
\emng
\eentry

\bentry
\word{Vedda} 
\pron{veDa} 
\gl{\nA}
\expl{}
\bmng
 veDaDx; shirxVlaMkA deVshada (mUlanivAsigaLa bauDakaTuTxgaLa peYki) veDaDx bauDakaTiTxnavanu. 
\emng
\eentry

\bentry
\word{vedette} 
\pron{viDeTf} 
\gl{\nA}
\expl{}
\bmng
 (shaturxgaLa calanavalanagaLanunx viVkiSxsalu seYnayxda ukakxDadiMdAce nililxsiruva) kAvalu rAvuta. 
\emng
\eentry

\bentry
\word[Vedic(1)]{Vedic} 
\pron{veV(viV)Dikf} 
\gl{\gu}
\expl{}
\bmng
 veYdika; veVdada yA veVdagaLa. 
\emng
\eentry

\bentry
\word[Vedic(2)]{Vedic} 
\pron{veV(viV)Dikf} 
\gl{\nA}
\expl{}
\bmng
 veYdika saMsakxqqta; veVdada saMsakxqqta BASe. 
\emng
\eentry

\bentry
\word{vee} 
\pron{viV} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} \eng{V} eMba akaSxra, vaNaR. 
\num{2}\eng{V} AkArada (yAvudeV) vasutx. 
\enum
\emng
\eentry

\bentry
\word[veer(1)]{veer} 
\pron{viarf} 
\gl{\sakirx}
\expl{}
\bmng
(\nw) (hagagx \mo vanunx) saDila biDu; DiVla biDu. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
 \eng{veer and haul} 
\banum
\alnum{a} (\nw) (hagagx \mo vanunx) payARyavAgi bigimADu matutx saDilagoLisu. 
\alnum{b} (\rUpa) kwshaladiMda samaseyxyanunx pariharisu. 
\eanum
\emng
\eentry

\bentry
\word[veer(2)]{veer} 
\pron{viarf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (gALiya \vi) \kanmu\ eDadiMda balakekx tirugu. 
\num{2} (\nw) = \hyperref{kandict_w.pdf}{W}{wear(3)}{$^3$wear}. (\akirx) 
\num{3} (aBipArxya, BAva, BAvane, vataRne, mAgaR, \mo vugaLalilx) tirugu; badalAyisu. 
\enum
\emng
\eentry

\bentry
\word[veer(3)]{veer} 
\pron{viarf} 
\gl{\nA}
\expl{}
\bmng
 (dikukx, mAgaR, \mo vugaLa) badalAvaNe. 
\emng
\eentry

\bentry
\word{veg} 
\pron{vejf} 
\gl{\nA}
\expl{}
\bmng
 (\AmA) = \hyperlink{vegetable(1)}{vegetable(s)}. 
\emng
\eentry

\bentry
\word{Vega} 
\pron{viVga} 
\gl{\nA}
\expl{}
\bmng
 (\Kavi) aBijitf nakaSxtarxpuMjadalilxna ujajxvXla parxkAshada oMdu niVli nakaSxtarx. 
\emng
\eentry

\bentry
\word[vegan(1)]{vegan} 
\pron{viVganf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} shAkAhAri; sasAyxhAri. 
\num{2} shAkAhAra; sasAyxhAra. 
\enum
\emng
\eentry

\bentry
\word[vegan(2)]{vegan} 
\pron{viVganf} 
\gl{\gu}
\expl{}
\bmng
 (kaTuTxniTATxgi) shAkAhAri, sasAyxhAri -- Ada. 
\emng
\eentry

\bentry
\word[vegetable(1)]{vegetable} 
\pron{vejiTba(bf)lf, vejfTba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sasayxda; sasayxsavxrUpada; sasayxgaLiMda huTiTxda yA mADida; sasayxgaLige saMbaMdhisida; sasayxgaLanonxLagoMDa; (\kanmu) pArxNigaLiMdaloV, KanijagaLiMdaloV -- utapxnanxvAgada: \eng{vegetable kingdom} sasayx -- rAjayx, loVka, parxpaMca, jagatutx. 
\num{2} sasAyxhArada; shAkAhArada. 
\num{3} jaDa; parxcoVdanege parxtikirxye toVrisada: \eng{vegetable behaviour} jaDavataRne. 
\num{4} sAmAnayx; visheVSavilalxda; EkarUpada; EkatAnada: \eng{vegetable existence} EkatAnada jiVvana, baduku. 
\enum
\emng
\eentry

\bentry
\word[vegetable(2)]{vegetable} 
\pron{vejiTba(bf)lf, vejfTba(bf)lf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (AhAravAgi baLasuva giDamUlike gaLiMda huTuTxva) shAka; kAyipalayx, tarakAri, sopupx, \mo vu; \udA\, koVsugeDeDx, ele, AlUgeDeDx, badaneVkAyi, beMDekAyi, daMTina sopu px, itAyxdi. 
\num{2} (\AmA) sasayxsadaqsha vayxkitx; (\kanmu\ meduLina hAni \mo vugaLiMda yA utatxma guri \mo vu ilalxdadxriMda) caTuvaTikeyilalxde yA EkatAnavAgi UTaniderxgaLalelxV Ayususx savesuvavanu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vegetable butter}{vegetable butter}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 sasayx beNeNx; beNeNxyaMte kANuva sasayxmUlada meVdasusx, kobaubx. 
\emng
\eentry

\bentry
\wordnospeech{vegetable ivory}{vegetable ivory}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\hyperref{kandict_i.pdf}{I}{ivory pagu}{ivory} (\pagu) noVDi. 
\emng
\eentry

\bentry
\wordnospeech{vegetable marrow}{vegetable marrow}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_m.pdf}{M}{marrow(1)3}{$^1$marrow(3)}. 
\emng
\eentry

\bentry
\wordnospeech{vegetable oyster}{vegetable oyster}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_s.pdf}{S}{salsify}{salsify}.
\emng
\eentry

\bentry
\wordnospeech{vegetable parchment}{vegetable parchment}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_p.pdf}{P}{parchment(5)}{parchment(5)}. 
\emng
\eentry

\bentry
\wordnospeech{vegetable spaghetti}{vegetable spaghetti}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 sasayx seVvige: 
\banum
\alnum{a} seVvigeyaMtha tiruLiruva, kuMbaLakAyi baLagada, oMdu kAyi. 
\alnum{b} adara tiruLu. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{vegetable sponge}{vegetable sponge}
\pron{?}
\gl{\nA}
\bmng
= \hyperref{kandict_l.pdf}{L}{loofah}{loofah}. 
\emng
\eentry

\bentry
\wordnospeech{vegetable tallow}{vegetable tallow}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\hyperref{kandict_t.pdf}{T}{tallow(1)}{$^1$tallow} (\pagu\ noVDi). 
\emng
\eentry

\bentry
\wordnospeech{vegetable wax}{vegetable wax}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (sumAyxkf \mo\ kelavu giDagaLu sarxvisuva) sasayxrALa. 
\emng
\eentry

\bentry
\word{vegetal}
\pron{vejiTalf} 
\gl{\gu}
\expl{}
\bmng
 sasiyxVya: 
\banum
\alnum{a} sasayxgaLa yA sasayx savxBAvada. 
\alnum{b} sasayxgaLanunx kurita;~sasayxgaLige saMbaMdhisida. 
\eanum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{the vegetal functions} sasayxkirxyegaLu: 
\banum
\alnum{a} (sasayxgaLa) beLavaNige, rasaparicalane, \mo\ kirxyegaLu. 
\alnum{b} (pArxNivagaRdalilx) rakatxparicalane, AhAraseVvane, \mo\ kirxyegaLu. 
\eanum
\emng
\eentry

\bentry
\word[vegetarian(1)]{vegetarian} 
\pron{vejiTeVrianf} 
\gl{\nA}
\expl{}
\bmng
shAkAhAri; sasAyxhAri; geDeDxgeNasu, tarakAri, sopupx, dhAnayxgaLu, divxdaLa dhAnayxgaLu yA beVLegaLu, biVjagaLu, haNuNxhaMpalu, \mo vugaLanunx oLagoMDa sasayxjanayx AhAravanunx mAtarx tinunxva, pArxNigaLa mAMsavanunx tinanxda vayxkitx yA pArxNi. 
\emng
\eentry

\bentry
\word[vegetarian(2)]{vegetarian} 
\pron{vejiTeVrianf} 
\gl{\gu}
\expl{}
\bmng
 shAkAhArada; sasAyxhAri: \eng{a vegetarian diet} shAkAhAri AhAra. 
\emng
\eentry

\bentry
\word{vegetarianism} 
\pron{vejiTeVrianisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sasAyxhAra varxta, anuSAThxna; (mAMsAhAravanunx vajiRsi) bariya sasAyxhAravanenxV tinunxva tatatxvXvanunx yA AdashaRvanunx paripAlisuva AcaraNe, AcAra, padadhxti, saMparxdAya. 
\num{2} sasAyxhAra, shAkAhAra -- tatatx vx, dheyxVya, AdashaR. 
\enum
\emng
\eentry

\bentry
\word{vegetate} 
\pron{vejiTeVTf} 
\gl{\akirx}
\expl{}
\bmng
 sasayxjiVvana naDesu: 
\banum
\alnum{a} sasayxgaLaMte baduku, beLe; deYhikakirxyegaLanunx mAtarx mADutatx jiVvisu. 
\alnum{b} yAvoMdu visheVSa GaTanegaLU ilalxda, UTaniderx muMtAda deYhika kirxyegaLeV parxdhAnavAda, oMdeV riVtiya dinacari naDesu. 
\eanum
\emng
\eentry

\bentry
\word{vegetation} 
\pron{vejiTeVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sasayxgaLa bALanunx bALuvudu; sasayxjiVvana naDesuvudu (\rUpa\ saha). 
\num{2} sasayxgaLu; sasayxvagaR: \eng{luxuriant vegetation} soMpAgi beLeda sasayxvagaR. \eng{no sign of vegetation for miles around} sutatxmutatx meYlimeYligaLavaregU sasayxgaLa kuruheV ilalx. 
\enum
\emng
\eentry

\bentry
\word{vegetational} 
\pron{vejiTeVSanalf} 
\gl{\gu}
\expl{}
\bmng
 sasayxvagaRda; sasayxgaLa yA avakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vegetative} 
\pron{vejiTaTivf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sasayxka; sasiyxVya; leYMgika saMtAnoVtapxtitxgiMta BinanxvAgi deYhikapuSiTx matutx beLavaNigegaLige saMbaMdhisida. 
\num{2} = \hyperlink{vegetal}{vegetal}. 
\enum
\emng
\eentry

\bentry
\word{vegetatively} 
\pron{vejiTaTivfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sasayxkavAgi; sasiyxVyavAgi; leYMgika saMtAnoVtapxtitxgiMta BinanxvAgi, deYhika poVSaNe beLavaNigegaLige saMbaMdhisidaMte. 
\num{2} (keVvala) sasayxdaMte. 
\enum
\emng
\eentry

\bentry
\word{vegetativeness} 
\pron{vejiTaTivfnisf} 
\gl{\nA}
\expl{}
\bmng
 sasayxjiVvana; sasayxsadaqshavAgi badukuvike; yAvoMdu visheVSavU ilalxda EkatAnada dinacariyanunx anusarisuvike. 
\emng
\eentry

\bentry
\word{veggie} 
\pron{veji} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vegie}{vegie}. 
\emng
\eentry

\bentry
\word{vegie} 
\pron{veji} 
\gl{\nA}
\expl{}
\bmng
 (\AmA) shAkAhAri; sasAyxhAri. 
\emng
\eentry

\bentry
\word{vehemence} 
\pron{viVamanfsx} 
\gl{\nA}
\expl{}
\bmng
 BAvoVderxVka; BAvAveVsha; BAvAveVga; BAvoVdevxVga. 
\emng
\eentry

\bentry
\word{vehement} 
\pron{viVamaMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BAvoVdevxVgada; BAvAveVgadiMda kUDida; BAvAveVshapUNaR: \eng{a man of vehement character} BAvoVdevxVga savxBAvada vayxkitx. 
\num{2} utakxTa; parxcaMDa: \eng{a vehement desire} utakxTavAda, parxcaMDavAda Ashe. 
\num{3} tiVra birusAda; ati raBasada; parxcaMDa: \eng{a vehement wind} parxcaMDa mAruta. 
\enum
\emng
\eentry

\bentry
\word{vehemently} 
\pron{viVamaMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} BAvoVdevxVgadiMda; BAvAveVshadiMda. 
\num{2} utakxTavAgi. 
\num{3} ati raBasavAgi. 
\enum
\emng
\eentry

\bentry
\word{vehicle} 
\pron{viVika(kf)lf, viaka(kf)lf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ nelada meVle janaranonxV vasutxgaLanonxV sAgisalu baLasuva baMDi, gADi, kAru, basusx, lAri, \mo) vAhana. 
\num{2} (baNaNxgaLu, auSadhagaLu, \mo vanunx karagisalu baLasuva) darxva(mAdhayxma). 
\num{3} (aBipArxya, BAvane, saMkalapx, udedxVshisida kAyaR, \mo vugaLanunx horageDahalu sAdhanavAgi baLasuva) mAdhayxma; aBivayxkitx -- upakaraNa, sAdhana: \eng{he used the press as the vehicle for his opinions} Ata vaqtatxpatirxkegaLanunx tananx aBipArxyagaLanunx parxcArapaDisuva mAdhayxmavanAnxgi baLasikoMDa. \eng{I will not be used as the vehicle of your resentment} nananxnunx ninanx devxVSa tiVrisikoLuLxva sAdhanavanAnxgi baLasikoLaLxlu avakAsha koDalolelx. 
\num{4} (padagaLa) aBidhAthaR; akaSxrAthaR; lakaSxNAthaRdalilx baLasida padada yA padagaLa muKAyxthaR. 
\enum
\emng
\eentry

\bentry
\word{vehicular} 
\pron{vihikuyxlarf} 
\gl{\gu}
\expl{}
\bmng
 vAhanagaLa; vAhanagaLige saMbaMdhisida. 
\emng
\eentry

\bentry
\word[veil(1)]{veil} 
\pron{veVlf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\sA\ heMgasaru baLasuva) muKamusuku; muKaparade; baukiR; avakuMThana. 
\num{2} (roVmanf kAyxtholikf saMnAyxsiniyaru muKavanUnx, kutitxgeya hiMBAgavanUnx mucicxkoLaLxlu baLasuva) meVlumusuku; baukiR. 
\num{3} tere; parade; ya(ja)vanike; \kanmu\ yehUdayx deVvamaMdira \mo vugaLalilx gaBaRguDiyanUnx horaBAgavanUnx parxteyxVkisuva tere. 
\num{4} (\rUpa) veVSa; soVgu; nepa; vAyxja; musuku; mare; toVkeR: \eng{under the veil of religion} dhamaRda soVginalilx, veVSadalilx. 
\num{5} = \hyperlink{velum}{velum}. 
\num{6} (\CA) masuku; savxlapx mabaubxmabAbxgiruvudu. 
\num{7} (sahajavAgiyoV negaDiyiMdaloV Ada) gogagxra dani; dhavxniBAra; gaMTalu kaTiTxkoMDiruvike. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{beyond the veil} maraNAnaMtarada ajAcnxta -- dashe, sithxti. 
\num{2} \eng{draw a veil over} (viSayada meVle) tere eLe; viSayadatatx gamana seLeyade, caceRge eDegoDade -- maremADu. 
\num{3} \eng{take the veil} (\kanmu) roVmanf kAyxtholikf saMnAyxsiniyAgu, saMnAyxsini paMthakekx seVru. 
\enum
\emng
\eentry

\bentry 
\word[veil(2)]{veil} 
\pron{veVlf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} muKakekx tere, parade, baukiR -- hAkiko; avakuMThana dharisu (\akirx\ saha). 
\num{2} (\rUpa) (naya nuDi \mo vugaLa) musuku hAku; aremaresu; paroVkaSxvAgi sUcisu: \eng{veiled threat} nayanuDiya musuku hAkida bedarike. \eng{veiled resentment} aremareya asamAdhAna, asaMtoVSa. 
\enum
\emng
\eentry

\bentry
\word{veiling} 
\pron{veVliMgf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (tereyiMda, paradeyiMda) muKa maresikoLuLxvudu; muKakekx musuku, tere, parade hAkikoLuLxvudu. 
\num{2} (citarx, paTa, \mo vugaLa meVle) parade hAkuvudu; tere eLeyuvudu. 
\num{3} (aBipArxya, guTuTx, \mo vanunx) maremAcuvudu. 
\num{4} tereya baTeTx; baukiRya teLuvAda baTeTx. 
\enum
\emng
\eentry

\bentry
\word{veilless} 
\pron{veVlflisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (muKada meVle) paradeyilalxda; tereyilalxda; musukilalxda. 
\num{2} mucucxmareyilalxda; soVgilalxda. 
\enum
\emng
\eentry

\bentry
\word{vein} 
\pron{veVnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sire; haqdayakekx rakatxvanunx oyuyxva rakatxnALa. 
\num{2} sire; aBidhamani; (sAmAnayx baLakeyalilx) yAvudeV rakatxnALa, dhamani: \eng{he has a royal blood in his veins} avana rakatxnALagaLalilx rAj(manetanada)rakatx hariyutitxde, Ata rAjakulakekx seVridavanu. 
\num{3} sire; kiVTada rekekxyalilxna nALadaMtha racane. 
\num{4} paNaRnALa; eleya racaneyalilxruva huriyaMtha Utaka. 
\num{5} (\BUvi\ matutx \gaNi) sire; adiru muMtAdavugaLa nikeSxVpaviruva shilAbiruku. 
\num{6} (marada dAru, amaqta shile, hAlina giNuNx, \mo vugaLalilx naDuve kANuva beVre baNaNxda) reVKe; gere; eLe; paTeTx. 
\num{7} savxBAva; parxvaqtitx; manoVvaqtitx: \eng{he was of an imaginative vein} Ata obabx kalapxnAtamxka manoVvaqtitxyavanAgidadx. 
\num{8} dhATi; dhoVraNe; vishiSaTx -- guNa, riVti, sheYli, vidhAna: \eng{he said this in a humorous vein} Ata idanunx hAsayxsheYliyalilx heVLida. \eng{he made other remarks in the same vein} Ata adeV dhoVraNeyalelxV, dhATiyalelxV itara TiVkegaLanUnx mADida. 
\enum
\emng
\eentry

\bentry
\word{veining} 
\pron{veVniMgf} 
\gl{\nA}
\expl{}
\bmng
 (neyegx, baTeTx, \mo vugaLalilx) sireyaMtha alaMkaraNa, namUne. 
\emng
\eentry

\bentry
\word{veinless} 
\pron{veVnflisf} 
\gl{\gu}
\expl{}
\bmng
 (\savi) (\kanmu\ elegaLa \vi) sireyilalxda; eLegaLilalxda; kiru siregaLu, eLegaLu, paTeTxgaLu -- ilalxda. 
\emng
\eentry

\bentry
\word{veinlet} 
\pron{veVnfliTf} 
\gl{\nA}
\expl{}
\bmng
 kiru, cikakx -- sire. 
\emng
\eentry

\bentry
\word{veinlike} 
\pron{veVnfleYkf} 
\gl{\gu}
\expl{}
\bmng
 sireyaMtha; eLeyaMtha. 
\emng
\eentry

\bentry
\word{veinstone} 
\pron{veVnfsoTxVnf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_g.pdf}{G}{gangue}{gangue}. 
\emng
\eentry

\bentry
\word{veiny} 
\pron{veVni} 
\gl{\gu}
\expl{(\tara\ \eng{veinier}, \tama\ \eng{veiniest}).}
\bmng
\bnum
\num{1} rakatxsiregaLiMda tuMbida; muKayx siregaLuLaLx; rakatxsiregaLa, avugaLige saMbaMdhisida. 
\num{2} (\kanmu\ elegaLa \vi) aneVka eLegaLuLaLx. 
\num{3} baNaNxda paTeTxgaLiruva. 
\enum
\emng
\eentry

\bentry
\word{vela} 
\pron{viVl} 
\gl{\nA}
\expl{}
\bmng
 \eng{velum} eMba padada \bava 
\emng
\eentry

\bentry
\word{velamen} 
\pron{vileVmanf} 
\gl{\nA}(\bava\ \eng{velamina}
\expl{\ucAcx\ vileVmina). }
\bmng
 sAlAmisirx giDada beVrina (\kanmu\ biLalina) teLupore, aMguLu. 
\emng
\eentry

\bentry
\word{velar} 
\pron{viVlarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} horaporeya. 
\num{2} (\dhavxni) kaMThayx; maqdutAluvina hatitxrada kaMThasAthxnadalilx huTuTxva. 
\enum
\emng
\eentry

\bentry
\word{Velcro} 
\pron{velfkorxV} 
\gl{\nA}
\expl{(\P)}
\bmng
 velfkorxV; baTeTx \mo vanunx baMdhisalu baLasuva, eraDu paTiTxgaLanunx oMdara meVloMdu iTuTx amukidAga hiDidukoLuLxva baMdhani. 
\emng
\eentry

\bentry
\word{Velcroed} 
\pron{velfkorxVDf} 
\gl{\gu}
\expl{}
\bmng
 velfkorxV\eng{(Velcro)} hAki(baMdhisi)da. 
\emng
\eentry

\bentry
\word{veld} 
\pron{velfTx} 
\gl{\nA}
\expl{}
\bmng
 (\da\ Aphirxka) hululxgADu; hululx, kurucalugiDagaLu beLediruva meYdAna parxdeVsha. 
\emng
\eentry

\bentry
\word{veldschoen} 
\pron{velfsUkxnf} 
\gl{\nA}
\expl{}
\bmng
hadamADada cakakxLada meTuTx, pAdara. 
\emng
\eentry

\bentry
\word{veldt} 
\pron{velfTx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{veld}{veld}. 
\emng
\eentry

\bentry
\word{veleta} 
\pron{valiVT} 
\gl{\nA}
\expl{}
\bmng
 veliVTa; viLaMba, madhayxma matutx durxta mUrukAlagaLalUlx natiRsuva oMdu naqtayx. 
\emng
\eentry

\bentry
\word{velitation} 
\pron{veliTeVSanf} 
\gl{\nA}
\expl{}
\bmng
 (\pArxparx) 
\bnum
\num{1} laGuvAda kAdATa, kadana, kALaga. 
\num{2} vAdavivAda; vAguyxdadhx. 
\enum
\emng
\eentry

\bentry
\word{velleity} 
\pron{veliViTi} 
\gl{\nA}
\expl{}
\bmng
 (sAhitayxka) 
\bnum
\num{1} (kAyaRrUpa tALuvaSuTx balavalalxda) kiSxVNasaMkalapx. 
\num{2} nasubayake; kiSxVNeVcePx; dubaRla parxvaqtitx. 
\enum
\emng
\eentry

\bentry
\word{vellum} 
\pron{velmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (modalige karuvina camaRdiMda tayArisutitxdadx, teLuvU nayavU Ada) camaRda -- Ole, patarx. 
\num{2} camaRdoVleya hasatxparxti. 
\num{3} (camaRdoVleyanunx hoVluvaMte tayArisida) nuNupu kAgada. 
\enum
\emng
\eentry

\bentry
\word{velocimeter} 
\pron{velsimiTarf} 
\gl{\nA}
\expl{}
\bmng
 veVgamApaka; veVgavanunx aLeyuva upakaraNa. 
\emng
\eentry

\bentry
\word{velocipede} 
\pron{vilAsipiVDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\ca) savAranu tuLidu naDesutitxdadx laGu vAhana, \kanmu\ pArxciVna beYsikalf. 
\num{2} (\ame) makakxLa tirxcakarxvAhana, mugAgxli. 
\enum
\emng
\eentry

\bentry
\word{velocipedist} 
\pron{vilAsipiVDisfTx} 
\gl{\nA}
\expl{}
\bmng
 beYsikalf yA tirxcakarx seYkalf savAra. 
\emng
\eentry

\bentry
\word{velocity} 
\pron{vilAsiTi} 
\gl{\nA}(\bava\ \eng{velocities}). 
\bmng
\bnum
\num{1} (\Bwvi) veVga; (calisutitxruva kAyada \vi) nidiRSaTx kaSxNadalilx sathxLAMtaragoLuLxtitxruva dikukx matutx dara. 
\num{2} nidiRSaTx dikikxnalilx veVga. 
\num{3} (sAmAnayx baLakeyalilx) veVga. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{velocity of escape} = \hyperref{kandict_e.pdf}{E}{escape velocity}{escape velocity}. 
\emng
\eentry

\bentry 
\word{velodrome}
\pron{velaDorxVmf}
\gl{\nA}
\bmng
beYsikalf reVsina pathaviruva parxdeVsha yA kaTaTxDa. 
\emng
\eentry

\bentry
\word{velour} 
\pron{valuarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} velUrf baTeTx; maKamalfnaSuTx nuNupU hoLapU Ada phelfTx baTeTx. 
\num{2} (\pArxparx) phelfTx hAYxTu. 
\enum
\emng
\eentry

\bentry
\word{velours} 
\pron{valuasfR} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{velour}{velour}. 
\emng
\eentry

\bentry
\word{veloute} 
\pron{valUTeV} 
\gl{\nA}
\expl{}
\bmng
 tupapx, hiTuTx hAki mADida oMdu bageya biLibaNaNxda gojujx. 
\emng
\eentry

\bentry
\word{velum}
\pron{viVlamf}
\gl{\nA}
\expl{(\bava\ \eng{vela} \ucAcx\ viVla). }
\bmng
 pore; poreyaMtha kavaca, mucicxke, hodike. 
\emng
\eentry

\bentry
\word{velutinous} 
\pron{vilUTinasf} 
\gl{\gu}
\expl{}
\bmng
 (\savi) maKamalilxnaMtha; metuvAda sUkaSxmxroVmagaLiMda AvaqtavAgiruva. 
\emng
\eentry

\bentry
\word[velvet(1)]{velvet} 
\pron{velivxTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} maKamalfbaTeTx; (hiMde \kanmu\ reVSemxyiMda tayArisutitxdadx) oMdu pAshavxRdalilx moVTumoVTAda matutx daTaTxvAda tupupxLu juMguLaLx, otAtxgi heNeda, nuNupU hoLapU Ada baTeTxya jAti. 
\num{2} (gaMDu) jiMkemariya AgatAne mUDutitxruva koMbanunx mucicxkoMDiruva tupupxLinaMtha nuNupugUdala camaR. 
\num{3} maKamalilxnaMte nuNupU maqduvU Ada yAvudeV vasutx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{on velvet} anukUlavAda yA ELigeya sithxtiyalilx. 
\emng
\eentry

\bentry
\word[velvet(2)]{velvet} 
\pron{velivxTf} 
\gl{\gu}
\expl{}
\bmng
 maKamalilxna yA maKamalilxnaMtha; maKamalilxnaMte nuNapU hoLapU Ada. 
\emng
\eentry

\bentry
\wordnospeech{velvet bean}{velvet bean}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (amerikada dakiSxNa BAgadalilx beLeyuva, vaSaRkokxMdu phasalu koDuva) maKamalf huruLi. 
\emng
\eentry

\bentry
\word{velveted} 
\pron{velivxTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} maKamalf dharisida. 
\num{2} tupu pxLugUdalu mucicxda. 
\enum
\emng
\eentry

\bentry
\word{velveteen} 
\pron{velivxTiVnf} 
\gl{\nA}
\expl{}
\bmng
 velevxTiVnf: 
\banum
\alnum{a} maKamalilxnaMte juMguhodikeyuLaLx, hatitxya baTeTx. 
\alnum{b} (\bava dalilx) velevxTiVniniMda holida SarAyi \mo vu. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{velvet glove}{velvet glove}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (nijavAda haTha, kAThinayx, bedarike, \mo vanunx maremAcalu) horagaDe toVrisuva nayavinaya; bAhayxswmayxte; maKamalf musuku: \eng{with an iron hand in a velvet glove} maKamalf gavusina oLagina kabibxNada keYyiMda; swmayxteya musukina kAThinayxdiMda, niSuThxrateyiMda. 
\emng
\eentry

\bentry
\word{velvety} 
\pron{velivxTi} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} maKfmalfnaMtha; maKamalfnaMte nuNupU hoLapU Ada. 
\num{2} (nAlige, mAtu, dhavxni, \mo vugaLa \vi) tuMba nayavAda, maqduvAda. 
\enum
\emng
\eentry

\bentry
\wordnospeech{Ven.}{Ven.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 (AcfR DiVkananxna birudAgi) \eng{Venerable}. 
\emng
\eentry

\bentry
\wordRemoveSpace{vena-cava}{vena cava} 
\pron{viVna keVva} 
\gl{\nA}(\bava\ \eng{venae cavae}
\expl{\ucAcx\ viVniV keVviV). }
\bmng
 mahAsire; haqdayakekx rakatxvanunx koMDoyuyxva eraDu parxdhAna siregaLalilx oMdu. 
\emng
\eentry

\bentry
\word{venal} 
\pron{viVnalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} (vayxkitxya \vi) 
\banum
\alnum{a} laMcakoVra; tananx parxBAva, upakAra yA seVveyanunx haNakekx mArikoLuLxva. 
\alnum{b} BarxSaTxnAda; sAvxthaRsAdhakanAda. 
\eanum
\numie
\numi{2} (naDavaLike \mo vugaLa \vi) 
\banum
\alnum{a} laMcakoVratanada. 
\alnum{b} sAvxthaRsAdhaka; BarxSATxcArada. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{venality} 
\pron{viVnAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} laMcakoVratana. 
\num{2} BarxSATxcAra. 
\enum
\emng
\eentry

\bentry
\word{venally} 
\pron{viVnali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} laMcakoVranAgi. 
\num{2} sAvxthaRsAdhakanAgi; BarxSATxcAradiMda. 
\enum
\emng
\eentry

\bentry
\word{venation} 
\pron{viVneVSanf} 
\gl{\nA}
\bmng
 (\savi) sirAvinAyxsa; eleyalilx, kiVTagaLa rekekxgaLalilx yA pArxNideVhadalilx siregaLa haMcike. \imglink{venation:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.9cm height 0.6cm {V_Pictures/venation.jpg}}} 
\emng
\eentry

\bentry
\word{venational} 
\pron{viVneVSanalf} 
\gl{\gu}
\expl{}
\bmng
 sirAvinAyxsada; adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vend} 
\pron{veMDf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\nAyxshA) mAru; bikarimADu; vikarxyisu. 
\num{2} (cilalxre sAmAnanunx) mArATakikxDu; mAru; mArATamADu. 
\enum
\emng
\eentry

\bentry
\word{vendace} 
\pron{veMDeVsf} 
\gl{\nA}
\expl{}
\bmng
 (birxTanf deVshada kelavu saroVvaragaLalilx doreyuva, tuMba ruciyAdadedxnanxlAda, koVrigonasf AlubxyXla kulada) veMDeVsf eMba saNaNxmiVnu. 
\emng
\eentry

\bentry
\word{vendee} 
\pron{veMDiV} 
\gl{\nA}
\expl{}
\bmng
 (\nAyxshA) koLuLxga; karxyagAra; (\kanmu\ Asitxyanunx) koMDukoLuLxvava. 
\emng
\eentry

\bentry
\word{vender} 
\pron{veMDarf} 
\gl{\nA}
\expl{}
\bmng
 (\sA\ \saMpa gaLalilx \parx) = \hyperlink{vendor}{vendor}. 
\emng
\eentry

\bentry
\word{vendetta} 
\pron{veMDeT} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kulakalaha; kulaveYra; gAyagoLisalapxTaTxvana yA kole mADalapxTaTxvana kuTuMbadavaru A kaqtayx mADidavana yA avana kuTuMbadavara meVle hage tiVrisikoLaLxlu taLeda kaDuhagetana, badadhxdevxVSa. 
\num{2} (kAsiRka, sisiZli, \mo vugaLalilx baLakeyalilxdadx, baLakeyalilxruva) vAMshika seVDu; kulaveYra padadhxti. 
\numi{3} (diVGaR kAlada) 
\banum
\alnum{a} tiVvarx vAyxjayx; jagaLa. 
\alnum{b} kaDuhagetana; badadhxdevxVSa. 
\eanum
\numie
\enum
\emng
\eentry

\bentry 
\wordf{vendeuse} 
\pron{vAknfDasfZR} 
\gl{\nA}
\expl{\F\ }
\bmng
 (\kanmu\ phAYxSanf baTeTx aMgaDiyalilx mArATamADalu neVmitaLAgiruva) mArATagAti. 
\emng
\eentry

\bentry
\word{vendible} 
\pron{veMDibflf} 
\gl{\gu}
\expl{}
\bmng
 mAralu sAdhayxvAda; mArATakekx -- ahaRvAda, yoVgayxvAda; vikerxVya; bikarige lAyakAkxda. 
\emng
\eentry

\bentry
\word{vending-machine} 
\pron{veMDiMgfmaSiVnf} 
\gl{\nA}
\expl{}
\bmng
 (cilalxre sAmAnugaLanunx mArATa mADuva savxyaMcAlita) mAruveyaMtarx; mArATa yaMtarx. 
\emng
\eentry

\bentry
\word{vendor} 
\pron{veMDa(DA)rf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ \nAyxshA) mArATagAra; bikarigAra; vikarxyi. 
\num{2} = \hyperlink{vending-machine}{vending-machine}. 
\enum
\emng
\eentry

\bentry
\word{vendue} 
\pron{veMDUyx} 
\gl{\nA}
\expl{}
\bmng
(\ame) bahiraMga harAju; sAvaRjanika -- harAju, lilAvu. 
\emng
\eentry

\bentry
\word[veneer(1)]{veneer} 
\pron{viniarf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} (maramuTuTx, piVThoVpakaraNagaLu, \mo vugaLige) 
\banum
\alnum{a} oLeLxya marada teLuhalage hodisu. 
\alnum{b} merugu leVpa hacucx, baLi. (savxBAva, naDenuDi, \mo vugaLa doVSagaLanunx) nayavinayada toVkeRyiMda maremAcu; nayavinayada soVgu hAku. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[veneer(2)]{veneer} 
\pron{viniarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} (oraTAda maramuTuTxgaLige) 
\banum
\alnum{a} hodisuva oLeLxya marada teLuhodike, padara. 
\alnum{b} hacucxva horaleVpa. 
\alnum{c} koDuva hosa mirugu. 
\eanum
\numie
\num{2} pelxYvuDfna oMdu teLu padara, hALe. 
\num{3} bAhayx naya; hora nayagArike; nayavinayada soVgu, toVkeR. 
\enum
\emng
\eentry

\bentry
\word{veneering} 
\pron{viniariMgf} 
\gl{\nA}
\expl{}
\bmng
 oraToraTAda maramuTuTx \mo vakekx hodisuva yA leVpisuva sAmagirx. 
\emng
\eentry

\bentry
\word{veneer-moth} 
\pron{viniarfmAtf} 
\gl{\nA}
\expl{}
\bmng
 (kArxmibxDiV eMba upavaMshada, nayamusuku hAkidaMte toVruva baNaNxvuLaLx) mirugu pataMga. 
\emng
\eentry

\bentry
\word{venepuncture} 
\pron{viVnipaMkacxrf} 
\gl{\nA}
\expl{}
\bmng
 (\veYshA) sireraMdharxna; rakatxda mAdariyanunx paDeyalu yA sireyoLakekx cucucxmadudx cucacxlu sireyalilx tUtu mADuvudu. 
\emng
\eentry

\bentry
\word{venerability} 
\pron{venarabiliTi} 
\gl{\nA}
\expl{}
\bmng
 pUjayxte; mAnaniVyate; AdaraNiVyate. 
\emng
\eentry

\bentry
\word{venerable} 
\pron{venarabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (shiVla, vaqdAdhxpayx, satukxlate, \mo\ kAraNagaLiMda) pUjayx(nAda); mAnaniVya; AdaraNiVya: \eng{venerable priest} pUjayx guru. \eng{venerable relics} pUjayx avasheVSagaLu. 
\numi{2} pUjayx; mAnaniVya: 
\banum
\alnum{a} (cacfR Aphf iMgalxMDinalilx) AcfR DiVkananxna birudu. 
\alnum{b} (\roVkAyx\ cacfRnalilx) takakxSuTx pUjayxsAthxna paDediruva, Adare inUnx saMta padavige Erilalxda, guruvina birudu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{venerableness} 
\pron{venarabflfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{venerability}{venerability}. 
\emng
\eentry

\bentry
\word{venerably} 
\pron{venarabilx} 
\gl{\kirxvi}
\expl{}
\bmng
 pUjayxnA(vA)gi; mAnaniVyanA(vA)gi. 
\emng
\eentry

\bentry
\word{venerate} 
\pron{venareVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} pUjisu; pUjayxneM(veM)du BAvisu. 
\num{2} gwravisu; saMBAvisu. 
\enum
\emng
\eentry

\bentry
\word{veneration} 
\pron{venareVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} gADhavAda gwrava, Adara; pUjayxBAvane. 
\numi{2} 
\banum
\alnum{a} pUjayxteyiMda kANuvike; Adarisuvike; gwravisuvike. 
\alnum{b} pUjayxteyanunx paDediruvike. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{venerator} 
\pron{venareVTarf} 
\gl{\nA}
\expl{}
\bmng
 pUjayxteyiMda -- kANuvavanu, Adarisuvavanu, gwravisuvavanu. 
\emng
\eentry

\bentry
\word{venereal} 
\pron{viniarialf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} leYMgika bayakeya yA saMBoVgada. 
\num{2} ratiroVgakekx saMbaMdhisida; guhayxroVgada. 
\enum
\emng
\eentry

\bentry
\wordnospeech{venereal disease}{venereal disease}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 ratiroVga; guhayxroVga: \kanmu\ ratiroVga taguliruva vayxkitxyoDane saMBoVga mADuvudariMda taguluva roVga. 
\emng
\eentry

\bentry
\word{venereally} 
\pron{viniariali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} ratiroVgadiMda; guhayx roVgadiMda. 
\num{2} leYMgika saMBoVgakekx saMbaMdhisidaMte. 
\enum
\emng
\eentry

\bentry
\word{venereological} 
\pron{viniarialAjikalf} 
\gl{\gu}
\expl{}
\bmng
 ratiroVgashAsitxrXVya; ratiroVgashAsatxrXda yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{venereologist} 
\pron{viniariAlajisfTx} 
\gl{\nA}
\expl{}
\bmng
 ratiroVgashAsatxrXjacnx; ratiroVgashAsatxrXdalilx tajacnx. 
\emng
\eentry

\bentry
\word{venereology} 
\pron{viniariAlaji} 
\gl{\nA}
\expl{}
\bmng
 ratiroVgashAsatxrX; ratiroVgagaLa adhayxyana. 
\emng
\eentry

\bentry
\word[venery(1)]{venery} 
\pron{venari} 
\gl{\nA}
\expl{}
\bmng
 (\pArxparx) leYMgika laMpaTatana; savxcaPxMdavAda kAmukate. 
\emng
\eentry

\bentry
\word[venery(2)]{venery} 
\pron{venari} 
\gl{\nA}
\expl{}
\bmng
 (\pArxparx) SikAri; beVTe. 
\emng
\eentry

\bentry
\word{venesection} 
\pron{viVnisekfSanf} 
\gl{\nA}
\expl{}
\bmng
 rakatxsArxvaNa; cikitesxgAgi rakatxsArxva mADisuvudu. 
\emng
\eentry

\bentry
\word[Venetian(1)]{Venetian} 
\pron{viniVSanf} 
\gl{\gu}
\expl{}
\bmng
 (IshAnayx iTaliya) venisf nagarada. 
\emng
\eentry

\bentry
\word[Venetian(2)]{Venetian} 
\pron{viniVSanf} 
\gl{\nA}
\expl{}
\bmng
 veniVSanf: 
\banum
\alnum{a} venisf nagaradavanu; venisf nagaradalilx huTiTxdavanu. 
\alnum{b} venisf nagarada pArxMtaBASe; venisf nagaradavaru ADuva iTAYxliyanf BASeya pArxMtiVya rUpa. 
\alnum{c} \eng{(venetian)} = \hyperlink{venetian blind}{venetian blind}. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{venetian blind}{venetian blind}
\pron{?}
\gl{\nA}
\bmng
 veniVSanf kiTaki bAgilu; mara, loVha, pAlxsiTxkf, gAju, \mo vugaLa teLuvU kiriyagalavU Ada aDaDxpaTiTxgaLanunx joVDisuvudariMda yA sarisuvudariMda mucacxbahudAda yA tereyabahudAda kiTaki kada. \imglink{venetian blind:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.5cm{V_Pictures/venation blind.jpg}}} 
\emng
\eentry

\bentry
\wordnospeech{Venetian carpet}{Venetian carpet}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 uNeNxya hAsu matutx hatitxya eLe hokukx neyudx mADuva, \sA\ paTeTxpaTeTxyuLaLx, veniVSanf jamaKAna. 
\emng
\eentry

\bentry
\word{venetianed} 
\pron{viniVSanfDx} 
\gl{\gu}
\expl{}
\bmng
 veniVSanf kiTaki kada hAkida. 
\emng
\eentry

\bentry
\wordnospeech{Venetian glass}{Venetian glass}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 veniVSanf gAju; venisf nagarada baLi murAnoV eMbalilx tayArisuva, tuMba nAjUkAda, gAjina padAthaRgaLu. 
\emng
\eentry

\bentry
\wordnospeech{Venetian pearl}{Venetian pearl}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 gaTiTxgAjiniMda mADuva kaqtaka mututx; veniVSanf mututx. 
\emng
\eentry

\bentry
\wordnospeech{Venetian red}{Venetian red}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 veniVSanf keMpu; pherikf yA kabibxNada AkesxYDugaLiMda tayArisuva keMpu -- raMgu, vaNaRdarxvayx. 
\emng
\eentry

\bentry
\wordnospeech{Venetian sumac}{Venetian sumac}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 veniVSanf sumAYxkf; goMcalugoMcalAgi hUvugaLanunx biDuva yUroVpina oMdu baLiLx. 
\emng
\eentry

\bentry
\wordnospeech{Venetian window}{Venetian window}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 veniVSanf kiTaki; mUru bAgilugaLidudx, atayxMta etatxrada madhayxda bAgilina meVle kamAnuLaLx kiTaki. 
\emng
\eentry

\bentry
\word{vengeance} 
\pron{veMjanfsx} 
\gl{\nA}
\expl{}
\bmng
 seVDu; muyiyx; parxtiVkAra; tanagoV tananxvarigoV Ada apakAra, anAyxya, hiMse, \mo vakAkxgi aMtha kaqtayxvanunx mADidavana(ra) meVle tiVrisikoLuLxva hagetana, devxVSa. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{with a vengeance} atiyAda; vipariVtavAda; apeVkiSxsidadxkikxMta yA niriVkeSxgiMta hecicxna maTaTxdalilx: 
\banum
\alnum{a} \eng{this is punctuality with a vengeance} idu atiyAda yA vipariVtavAda kAlaniSeThx. 
\alnum{b} padada, padagaLa, saMpUNaR athaRdalilx. 
\eanum
\emng
\eentry

\bentry 
\word{vengeful} 
\pron{veMjfphulf} 
\gl{\gu}
\expl{}
\bmng
 seVDina manoVvaqtitxya; parxtiVkAra savxBAvada. 
\emng
\eentry

\bentry
\word{vengefully} 
\pron{veMjfPuZli} 
\gl{\kirxvi}
\expl{}
\bmng
 seVDiniMda; seVDina manoVvaqtitxyiMda; parxtiVkAra savxBAvadiMda. 
\emng
\eentry

\bentry
\word{vengefulness} 
\pron{veMjfPuZlfnisf} 
\gl{\nA}
\expl{}
\bmng
 seVDina manoVvaqtitx; parxtiVkAra savxBAva. 
\emng
\eentry

\bentry
\word{venial} 
\pron{viVnialf} 
\gl{\gu}
\expl{}
\bmng
 (doVSa yA pApada \vi) 
\banum
\alnum{a} kaSxmayx; kaSxmisabahudAda; kaSxmAhaRvAda. 
\alnum{b} gurutaravalalxda; laGuvAda; saNaNx; cikakx. 
\eanum
\emng
\eentry

\bentry
\word{veniality} 
\pron{viVniAYxliTi} 
\gl{\nA}
\expl{}
\bmng
 (doVSa, pApagaLa \vi) 
\banum
\alnum{a} kaSxmayxte; kaSxmAhaRte. 
\alnum{b} laGusavxrUpa; gurutaravAgilalxdiruvike. 
\eanum
\emng
\eentry

\bentry
\word{venially} 
\pron{viVniali} 
\gl{\kirxvi}
\expl{}
\bmng
 (doVSa, pApagaLa \vi) 
\banum
\alnum{a} kaSxmayxvAgi; kaSxmAhaRvAgi. 
\alnum{b} laGuvAgi; gurutaravAda riVtiyalalxlalxde. 
\eanum
\emng
\eentry

\bentry
\word{venialness} 
\pron{viVnialfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{veniality}{veniality}. 
\emng
\eentry

\bentry
\word{venipuncture} 
\pron{viVnipaMkacxrf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{venepuncture}{venepuncture}. 
\emng
\eentry

\bentry
\word{venire} 
\pron{vineYarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\nAyxshA) (\ca\ yA \ame) (koVTiRge hAjarAguvaMte nAyxyadashiRgaLige tiLisalu SeriVphanige koVTuR niVDida) kareyAjecnx. 
\num{2} (\ame) (koVTuR) kareyAjecnxyiMda karesida nAyxyamaMDali. 
\enum
\emng
\eentry

\bentry
\wordRemoveSpace{venire-facias}{venire facias} 
\pron{vineYarf pheVSiAYxsf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{venire}{venire\((1)\)}. 
\emng
\eentry

\bentry
\word{venireman} 
\pron{vineYarfmanf} 
\gl{\nA}
\expl{}
\bmng
 nAyxyadashiR; (koVTuR kareyAjecnxyiMda karesida) nAyxya maMDaliya sadasayx. 
\emng
\eentry

\bentry
\word{venisection} 
\pron{viVnisekaSxnf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{venesection}{venesection}. 
\emng
\eentry

\bentry
\word{venison} 
\pron{venisa(saZ)nf} 
\gl{\nA}
\expl{}
\bmng
 (AhAravAgi baLasuva) jiMke mAMsa. 
\emng
\eentry

\bentry
\word{Venite} 
\pron{vineYTi} 
\gl{\nA}
\expl{}
\bmng
 (\ca) vineYTi: 
\banum
\alnum{a} beYbflfna \eng{95}neya pArxthaRnA giVteyanonxLagoMDa sutxtigiVtAvaLi. 
\alnum{b} idakekx hoMdisida saMgiVta. 
\eanum
\emng
\eentry

\bentry
\wordRemoveSpace{Venn-diagram}{Venn diagram} 
\pron{venf DeYagArxyXmf} 
\gl{\nA}
\expl{}
\bmng
 (\ga) venfcitarx; gaNagaLanunx matutx avugaLa parasapxra saMbaMdhagaLanunx vaqtatxgaLa mUlaka nirUpisuva yA saMkeVtisalu vaqtatxgaLanunx baLasuva reVKAcitarx. 
\emng
\eentry

\bentry
\word{venom} 
\pron{venamf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (ceVLu, hAvu, \mo vugaLa) naMju; viSa. 
\num{2} kaDuhage(tana); ugarxdevxVSa. 
\num{3} naDenuDiyalilx -- ugarxte, kUrxrate. 
\enum
\emng
\eentry

\bentry
\word{venomed} 
\pron{venamfDx} 
\gl{\gu}
\expl{}
\bmng
 hagetanada; devxVSa kAruva. 
\emng
\eentry

\bentry
\word{venomous} 
\pron{venamasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} naMjiniMda kUDida yA tuMbida; viSayukatx; viSapUrita. 
\num{2} (kuTuki yA kacicx gAyagoLisi) viSa ereyuva. 
\num{3} hage, viSa, devxVSa -- kAruva. 
\enum
\emng
\eentry

\bentry
\word{venomously} 
\pron{venamasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 viSa, devxVSa -- kArutAtx. 
\emng
\eentry

\bentry
\word{venomousness} 
\pron{venamasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} viSa- yukatxte, pUritate. 
\num{2} (\rUpa) devxVSa- yukatxte, pUritate. 
\enum
\emng
\eentry

\bentry
\word{venose} 
\pron{viVnoVsfZ} 
\gl{\gu}
\expl{}
\bmng
 sirAyukatx; aneVka siregaLiruva. 
\emng
\eentry

\bentry
\word{venosity} 
\pron{vinAsiTi} 
\gl{\nA}
\expl{}
\bmng
 sirAyukatxte; sire hoMdiruvudu. 
\emng
\eentry

\bentry
\word{venous} 
\pron{viVnasf} 
\gl{\gu}
\expl{}
\bmng
 sireya yA sirege saMbaMdhisida; siregaLiMda tuMbida. 
\emng
\eentry

\bentry
\word{venously} 
\pron{viVnasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sireyaMte. 
\num{2} siregaLiMda tuMbida riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word[vent(1)]{vent} 
\pron{veMTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} (oLahogalu yA horahoraDalu eDegoDuva) terapu; kaMDi; bAyi; toLe; raMdharx; dAvxra: 
\banum
\alnum{a} PiraMgiya hiMBAgadalilx madudx hotitxsuva bAyi. 
\alnum{b} piVpAyiyiMda darxvavanunx horategeyuvalilx piVpAyiyoLakekx gALi nugagxlu adara meVlagxDe mADiruva raMdharx. 
\alnum{c} OlagadaMtha saMgiVtada vAdayxgaLalilx beraLugaLiMda mucacxlu tereyalu mADiruva raMdharxgaLalolxMdu. 
\alnum{d} (mane \mo vugaLa CAvaNiya meVlaNa) hogekaMDi. 
\alnum{e} jAvxlAmuKiya hoge uguLuva bAyi. 
\eanum
\numie
\num{2} (\kanmu\ keLavagaRgaLa pArxNigaLalilx malavisajaRna, saMtAnoVtapxtitxgaLeraDakUkx sAdhanavAda) guda; AsanadAvxra. 
\num{3} (\rUpa) (koVpatApa, asahane, \mo\ BAva, aBipArxya, BAvane, \mo vanunx horadoVralu yA vayxkatxpaDisalu baLasuva) vidhAna; upAya; dAri; mAgaR; riVti; avakAsha: \eng{he gave vent to his indignation at such injustice} aMtha anAyxyada viSayadalilx avaniguMTAda nAyxyavAda koVpavanunx avanu vayxkatxpaDisida. \eng{his impatience found a vent in his restless conduct} avana caDapaDikeyalilx avana asahane vayxkatxgoMDitu. 
\num{4} (niVrunAyi \mo\ jalacara pArxNigaLu usirADalu) niVriniMda horakekx baruvudu. 
\enum
\emng
\eentry

\bentry
\word[vent(2)]{vent} 
\pron{veMTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (PiraMgi, piVpAyi, \mo vugaLalilx) bAyi kore; dAvxra mADu. 
\numi{2} (tananx koVpatApa \mo\ BAvavanunx, aBipArxyavanunx, BAvaneyanunx) 
\banum
\alnum{a} horageDahu; vayxkatxpaDisu; parxkaTapaDisu: \eng{he vented his disgust in an epigram} tananx jugupesxyanunx avanu oMdu nATunuDiyalilx horageDahida. 
\alnum{b} kAru; kakukx: \eng{he vented his anger on the office boy} avanu tananx koVpavanunx kaceVriya ALuhuDugana meVle kArida. 
\eanum
\numie
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 (niVrunAyi \mo vugaLa \vi) usirADalu (niVriniMda) horabaru. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{vent one's spleen on} niSAkxraNavAgi bayuyx yA piVDisu. 
\emng
\eentry

\bentry
\word[vent(3)]{vent} 
\pron{veMTf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ aMgiya hiMBAgada keLage mADuva) udadxneya giVLu, siVLu. 
\emng
\eentry

\bentry
\word{vent-hole} 
\pron{veMTfhoVlf} 
\gl{\nA}
\expl{}
\bmng
 (oLahoguva yA horabaruva) terapu; kaMDi; bAyi. 
\emng
\eentry

\bentry
\word{ventiduct} 
\pron{veMTiDakfTx} 
\gl{\nA}
\expl{}
\bmng
 (\vAshi) gavAkaSx(kiSx); gALi beLakina kaMDi. 
\emng
\eentry

\bentry
\word{ventifact} 
\pron{veMTiphAZYxkfTx} 
\gl{\nA}
\expl{}
\bmng
 gALimaraLugalulx; vAyuseYkata shile; (gALi hoDedukoMDu baMdu hAkida maraLarAshiyiMda nimiRtavAda) maraLugalulx. 
\emng
\eentry

\bentry
\word{ventil} 
\pron{veMTilf} 
\gl{\nA}
\expl{}
\bmng
 (\saM) 
\bnum
\num{1} gALivAdayxda oMdu biraDe, kavATa. 
\num{2} AgaRnf vAdayxdalilx gALiya harivanunx niyaMtirxsuva mucacxLa. 
\enum
\emng
\eentry

\bentry
\word{ventilate} 
\pron{veMTileVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (koThaDi \mo vakekx) gALi beLakanunx ADisu; gALibeLaku baruvaMte mADu. 
\numi{2} (\veYshA) 
\banum
\alnum{a} gALiyanonxV AmalxjanakavanonxV hAyisi rakatxvanunx shudidhxVkarisu. 
\alnum{b} (shAvxsakoVshagaLige) gALiyanunx biDu yA taLuLx, nugigxsu. 
\eanum
\numie
\num{3} (oMdu viSaya, parxshenx, ajiR, dUru, \mo vanunx sAvaRjanika caceR, parAmasheR, \mo vakAkxgi) oDuDx; muMdiDu; salilxsu; parxkaTana mADu; shurxtapaDisu. 
\enum
\emng
\eentry

\bentry
\word{ventilation} 
\pron{veMTileVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} gALi beLakanunx hAyisuvudu, saMcAra mADisuvudu. 
\num{2} gALibeLakugaLa saMcAra. 
\num{3} (\rUpa) (yAvudeV viSaya \mo vanunx sAvaRjanika parAmasheR, TiVke, \mo vakAkxgi)muMdiDuvudu; salilxsuvudu; parxkaTisuvudu; shurxtapaDisuvudu. 
\enum
\emng
\eentry

\bentry
\word{ventilative} 
\pron{veMTileVTivf} 
\gl{\gu}
\expl{}
\bmng
 gALibeLakugaLa saMcArakekx anukUlavAda; gALibeLaku ADalu sahAyakavAda. 
\emng
\eentry

\bentry
\word{ventilator} 
\pron{veMTileVTarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} gavAkaSx(kiSx); vAtAyana; gALikaMDi; gALibeLakugaLa parxveVshakAkxgi mADiruva kaMDi. 
\num{2} gavAkaSxvanunx mucucxva, tereyuva -- sAdhana, upakaraNa. 
\num{3} (\rUpa) (yAvudeV viSayavanunx) sAvaRjanika parAmasheR \mo vakAkxgi salilxsuvavanu, horapaDisuvavanu, parxkaTapaDisuvavanu. 
\num{4} = \hyperref{kandict_r.pdf}{R}{respirator(b)1}{respirator(b)}. 
\enum
\emng
\eentry

\bentry
\word{ventral} 
\pron{veMTarxlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\pArxvi\ matutx \aMrashA) kukiSxya; udaraBAgada yA adara meVlina. 
\num{2} (\savi) muMBAgada yA keLaBAgada; puroVBAgada yA adhoVBAgada. 
\enum
\emng
\eentry

\bentry
\wordnospeech{ventral fin}{ventral fin}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 miVnina udaraBAgada meVliruva IjurekekxgaLalilx oMdu. 
\emng
\eentry

\bentry
\word{ventrally} 
\pron{veMTarxli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} udara(BAga)dalilx. 
\num{2} keLagaDe; adhoVBAgadalilx. 
\enum
\emng
\eentry

\bentry
\wordf{ventre a terre} 
\pron{vAMTfrx A TeVrf} 
\gl{\kirxvi}
\expl{\F}
\bmng
 tuMbau, pUNaR -- veVgadalilx; bahu veVgavAgi. 
\emng
\eentry

\bentry
\word{ventricle} 
\pron{veMTirxka(kf)lf} 
\gl{\nA}
\expl{}
\bmng
 (\aMrashA) 
\bnum
\num{1} deVhadoLagina -- kuhara, kuLi. 
\num{2} oMdu aMgada (\kanmu\ miduLina yA haqdayada) ToLuLx, gUDu, kuhara (\kanmu\ miduLugUDu, haqtukxhara). 
\enum
\emng
\eentry

\bentry
\word{ventricose} 
\pron{veMTirxkoVsfZ} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} DoLuLx hoTeTxya; ubibxda hoTeTxyuLaLx. 
\num{2} (\savi) piVna; ubibxruva. 
\enum
\emng
\eentry

\bentry
\word{ventricular} 
\pron{veMTirxkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
 kukiSxya yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{ventriloquial} 
\pron{veMTirxloVkivxalf} 
\gl{\gu}
\expl{}
\bmng
 dhavxnigAruDiya yA gAruDivANiyaMtha. 
\emng
\eentry

\bentry
\word{ventriloquise} 
\pron{veMTirxlakevxYsf} 
\gl{\akirx}
\expl{}
\bmng
 = \hyperlink{ventriloquize}{ventriloquize}. 
\emng
\eentry

\bentry
\word{ventriloquism} 
\pron{veMTirxlakivxsaZmf} 
\gl{\nA}
\expl{}
\bmng
 gAruDivANi; dhavxni -- gAruDi, yakiSxNi: 
\banum
\alnum{a} dhavxni yA mAtu tananx keYyalilxruva boMbeyiMdaloV, beVrelilxMdaloV horaDutitxruvaMte ucacxrisuvudu, mAtanADuvudu. 
\alnum{b} hiVge mADuva kale, kwshala. 
\eanum
\emng
\eentry

\bentry
\word{ventriloquist} 
\pron{veMTirxlakivxsfTx} 
\gl{\nA}
\expl{}
\bmng
 dhavxni -- gAruDiga, yakiSxNigAra; gAruDivANiyava. 
\emng
\eentry

\bentry
\word{ventriloquistic} 
\pron{veMTirxlakivxsiTxkf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{ventriloquial}{ventriloquial}. 
\emng
\eentry

\bentry
\word{ventriloquize} 
\pron{veMTirxlakevxYsfZ} 
\gl{\akirx}
\expl{}
\bmng
 gAruDi vANiyiMda -- mAtanADu; gAruDivANiyanunx parxdashiRsu. 
\emng
\eentry

\bentry
\word{ventriloquy} 
\pron{veMTirxlakivx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{ventriloquism}{ventriloquism}. 
\emng
\eentry

\bentry
\word{ventripotent} 
\pron{veMTirxpaTeMTf} 
\gl{\gu}
\expl{}
\bmng
 (sAhitayxka) 
\bnum
\num{1} DoLuLxhoTeTxya; ubibxda hoTeTxyuLaLx; guDANada hoTeTxya. 
\num{2} hoTeTxbAkatanada; kULubAkatanada. 
\enum
\emng
\eentry

\bentry
\word[venture(1)]{venture} 
\pron{venacxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sAhasa; sAhasoVdayxma; naSaTx \mo\ apAyagaLiMda kUDida parxyatanx yA adanunx keYgoLuLxvike: \eng{he declined the venture} avanu A sAhasoVdayxmavanunx keYgoLaLxlu olelxneMda. \eng{I am ready for any venture} yAvudeV sAhasakekx nAnu sidadhx. 
\num{2} saTATxvAyxpAra: \eng{one lucky venture made his fortune} adaqSaTxkaravAda oMdu saTATxvAyxpAra avanige aishavxyaRvanunx taMdukoTiTxtu. 
\num{3} (\pArxparx) paNa; (naSaTxvAdarU AgaleMdu dheYyaRmADi) oDiDxda -- haNa yA beVrAvudeV vasutx \mo vu. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{at a venture} (pUvARloVcaneyilalxde) diDhiVrane mADida; idadxkikxdadxMte keYgoMDa. 
\emng
\eentry

\bentry
\word[venture(2)]{venture} 
\pron{venacxrf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (mADuvudu, muMdiDuvudu, maMDisuvudu, paNa oDuDxvudu, \mo vakekx) dheYyaR, sAhasa -- mADu: \eng{he would not venture an opinion} tananx aBipArxya heVLalu dheYyaR mADalolalxde hoVda. \eng{he would not venture a guess} avanu Uhe saha mADalu dheYyaR mADalolalx. \eng{he would not venture a step} avanu oMdu hejejx muMdiDalU dheYyaR mADalolalx. 
\num{2} (paNa \mo vanunx oDuDxva, baMDavALa \mo vanunx hUDuva) sAhasa mADu: \eng{I will venture a bet on it} nAnu adara meVle oMdu paNa oDuDxtetxVne. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} dheYyaRmADu; sAhasamADu; aMjadiru: \eng{I did not venture to stop him} avananunx taDeyalu nAnu dheYyaR mADalilalx. \eng{I venture to differ with you} nimamx aBipArxyakekx BinanxvAda aBipArxyavanunx sUcisuva sAhasa mADutetxVne. 
\num{2} baralu yA hoVgalu muMtAda sAhasamADu, dheYyaRmADu: \eng{I shall not venture out} nAnu horakekx hoVgalu dheYyaR mADuvudilalx. 
\num{3} apAyagaLige mununxgugx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{nothing venture, nothing have} (\engit{or} \eng{gain} \engit{or} \eng{win)} leVshamAtarx sAhasa mADadidadxre leVshamAtarxvanUnx gelalxlAre(vu). 
\num{2} \eng{venture on} (\engit{or} \eng{upon)} (yAvudeV kelasa \mo vugaLalilx uduyxkatxnAguvudu, mADuvudu, \mo vakekx) dheYyaR mADu; sAhasa mADu; keYhAku; toDagu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{venture capital}{venture capital}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 sAhasoVdayxma baMDavALa; sAkaSuTx apAya, naSaTx iruva yoVjaneyalilx hUDida baMDavALa. 
\emng
\eentry

\bentry
\wordnospeech{venture capitalist}{venture capitalist}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 sAhasoVdayxma baMDavALagAra; sAkaSuTx apAya, naSaTx iruva yoVjaneyalilx baMDavALa hUDidava. 
\emng
\eentry

\bentry
\word{venturer} 
\pron{venacxrarf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ \ca) (vANijayxdalilx) sAhasoVdayxmi yA sAhasoVdayxmadalilx pAludAra. 
\emng
\eentry

\bentry
\wordnospeech{Venture Scout}{Venture Scout}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\birx) (\eng{16-20} vaSaRgaLoLagina) hiriya, anuBaviyAda, swkxTu(daLadava). 
\emng
\eentry

\bentry
\word{venturesome} 
\pron{venacxrfsamf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sAhasa parxvaqtitxya; naSaTxkekx, apAyakekx oDiDxkoLuLxva parxvaqtitxya. 
\num{2} apAyada; naSaTxda; hAnikaravAda. 
\enum
\emng
\eentry

\bentry
\word{venturesomely} 
\pron{venacxrfsamfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sAhasa parxvaqtitxyiMda; naSaTxkekx, apAyakekx oDiDxkoLuLxvaMte. 
\num{2} apAyakaravAgi; apAya saMBavisuva riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{venturesomeness} 
\pron{venacxrfsamfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sAhasaparxvaqtitx; naSaTxkekx, apAyakekx oDiDxkoLuLxva parxvaqtitx, savxBAva. 
\num{2} apAyaviruvike. 
\enum
\emng
\eentry

\bentry
\word{venturi} 
\pron{venfTuyxari} 
\gl{\nA}(\bava\ \eng{venturis}). 
\bmng
 veMTuyxri; (parxvAhavanunx aLeyalu yA hiVralu) agalavAda nALagaLa madheyx tUrisiruva ikakxTATxda cikakx nALa, koLave. 
\emng
\eentry

\bentry
\word{venue} 
\pron{venUyx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\nAyxshA)(\ca) vicAraNA sathxLa; nAyxyadashiRgaLanunx jamAyisi, mokadadxmeyanunx naDesabeVkAda, vicAraNe mADabeVkAda, (pUvaRdalilx aparAdha naDeda, sutatxmutatxla) parxdeVsha. 
\num{2} (paMdayx, samAgama, saMjekUTa, \mo vakAkxgi) nigadi mADida -- sathxLa, jAga, tANa, parxdeVsha. 
\num{3} seVruva sathxLa; jamAvaNe jAga. 
\enum
\emng
\eentry

\bentry
\word{venule} 
\pron{venUyxlf} 
\gl{\nA}
\expl{}
\bmng
 (\aMrashA) kirusire; roVmanALagaLa pakakxdalilxna cikakx sire. 
\emng
\eentry

\bentry
\word{Venus} 
\pron{viVnasf} 
\gl{\nA}(\bava\ \eng{Venuses}). 
\bmng
\bnum
\num{1} (roVmanf \pu) viVnasf; perxVmada adhideVvate. 
\num{2} shukarx (garxha); swravUyxhadalilx parxkAshadalilx sUrayxnige eraDaneyadAda garxha. 
\num{3} kAma; kAmAturate; leYMgika parxBAva yA bayake. 
\num{4} (\kAparx) celuve; suMdari. 
\enum
\emng
\eentry

\bentry
\word[Venusian(1)]{Venusian} 
\pron{vinUyxsiZanf} 
\gl{\gu}
\expl{}
\bmng
 shukarxna yA shukarxgarxhakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word[Venusian(2)]{Venusian} 
\pron{vinUyxsiZanf} 
\gl{\nA}
\expl{}
\bmng
 shukarxgarxhadalilx vAsisuva kAlapxnika vayxkitx. 
\emng
\eentry

\bentry
\wordnospeech{Venus's comb}{Venus's comb} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_s.pdf}{S}{shepherd's needle}{shepherd's needle}. 
\emng
\eentry

\bentry
\wordnospeech{Venus('s) fly-trap}{Venus('s) fly-trap}
\pron{?}
\gl{\nA}
\bmng
 (meVle kuLita kiVTagaLu \mo vugaLanunx elegaLiMda mucicxkoMDu avanunx tiMdu badukuva) viVnasf boVnu eMba giDa. \imglink{Venus('s) fly-trap:fig}{\raisebox{-0.20cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.7cm {V_Pictures/venus fly trap.jpg}}} 
\emng
\eentry

\bentry
\wordnospeech{Venus's girdle}{Venus's girdle}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (ribabxnfnaMtha IjurekekxyuLaLx) `viVnasaLa DAbau' (eMba kaDalahuLu). 
\emng
\eentry

\bentry
\wordnospeech{venus's looking-glass}{venus's looking-glass}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 viVnasf kananxDi giDa; legoVsiya kulada, cikakxcikakx niVli hUgaLanunx biDuva, halavAru giDagaLalolxMdu. 
\emng
\eentry

\bentry
\wordnospeech{Venus's slipper}{Venus's slipper}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_l.pdf}{L}{lady's slipper}{lady's slipper}. 
\emng
\eentry

\bentry
\word{veracious} 
\pron{vareVSasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} diTa nuDiyuva; nija heVLuva; satayxsaMdhanAda. 
\num{2} (heVLike \mo vugaLa \vi) diTavAda; nijavAda; satayxvAda; pArxmANikavAda. 
\enum
\emng
\eentry

\bentry
\word{veraciously} 
\pron{vareVSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 diTavAgi; nijavAgi; pArxmANikavAgi. 
\emng
\eentry

\bentry
\word{veraciousness} 
\pron{vareVSasfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{veracity}{veracity}. 
\emng
\eentry

\bentry
\word{veracity} 
\pron{varAYxsiTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} diTa; nija; satayx; pArxmANikate. 
\num{2} (heVLike \mo vugaLa) niKarate; niSakxqqSaTxte. 
\enum
\emng
\eentry

\bentry
\word{veranda} 
\pron{varAYxMDa} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} varAMDa; maneya pAshaRvxda yA pAshaRvxgaLa udadxkUkx cAvaNige UreyAgiruva kaMbasAlugaLuLaLx jaguli. 
\num{2} (\AseTxrXV\ matutx nUyxsiZVleMDf) aMgaDiya muMde nelagaTiTxna meVle hAkida sUru. 
\enum
\emng
\eentry

\bentry
\word{verandah} 
\pron{varAYxMDa} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{veranda}{veranda}. 
\emng
\eentry

\bentry
\word{veratrine} 
\pron{veraTirxVnf} 
\gl{\nA}
\expl{}
\bmng
 (\veYshA) veraTirxVnf; mekisxkoVdalilxna sabaDilAlx \eng{(sabadilla)} \mo\ giDadiMda paDeyuva, saMdhivAta matutx naraveVdanege cikitesx niVDalu baLasuva, oMdu saMyukatx. 
\emng
\eentry

\bentry
\word{verb} 
\pron{vabfR} 
\gl{\nA}
\expl{}
\bmng
 (\vAyx) 
\bnum
\num{1} dhAtu; kirxyApadada mUlAMsha. 
\num{2} kirxyApada; AKAyxta pada; sithxti, GaTane yA kAyaRvanunx heVLuva pada. 
\enum
\emng
\eentry

\bentry
\word[verbal(1)]{verbal} 
\pron{vabaRlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} padagaLa yA padagaLige saMbaMdhisida; shAbidxka yA shAbadx: \eng{verbal subtleties} shabadxsUkaSx mxtegaLu. \eng{verbal accuracy} pada niSakxqqSaTxte; niSakxqqSaTx padaparxyoVga. 
\num{2} shAbadx; shAbidxka; padagaLanunxLaLx yA padarUpada: \eng{verbal imagery} shAbadx, shAbidxka -- parxtime. 
\num{3} keVvala shabadxgaLige yA padagaLige saMbaMdhisida: \eng{a purely verbal distinction between two concepts} eraDu kalapxnegaLa naDuvaNa keVvala shAbidxka vayxtAyxsa. 
\num{4} vAcika; vAgUrxpada; bAyimAtina; mwKika: barahada mUlaka Agirade mAtina mUlaka tiLisida: \eng{verbal contract} bAyimAtina yA muKataH mADikoMDa -- karAru, opapxMda. \eng{verbal evidence} bAyimAtina, vAcA heVLida, mwKika -- sAkaSxyX. 
\num{5} (anuvAdada, tajuRmeya \vi) akaSxraniSaThx; padaniSaThx; akaSxrashaH, padashaH -- mADida. 
\num{6} (\vAyx) kirxyA padada yA rUpada: \eng{verbal inflexions} kirxyApadada kAla \eng{(tense)}, parxyoVga \eng{(voice)}, BAva \eng{(mood)}, vacana \eng{(number)} matutx puruSa \eng{(person)} eMba rUpAMtaragaLu yA rUpaBeVdagaLu. 
\num{7} vAcALiyAda. 
\enum
\emng
\eentry

\bentry
\word[verbal(2)]{verbal} 
\pron{vabaRlf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (\vAyx) = \hyperlink{verbal noun}{verbal noun}. 
\alnum{b} kirxyApadavAgi kelasa mADuva pada yA padagaLu. 
\eanum
\numie
\num{2} (\ashi) (\kanmu\ poliVsinavarige koTaTx) bAyimAtina, mwKika -- heVLike. 
\num{3} (\ashi) beYguLa; niMde: \eng{gave them the verbal} avarige beYguLa hAkida; avaranunx niMdisida, beYda. 
\enum
\emng
\eentry

\bentry
\word[verbal(3)]{verbal} 
\pron{vabaRlf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{verballed}; \vakaq}
\eng{verballing}). \bmng
 (\birx) (\ashi) (shaMkita vayxkitx avanigeV) toMdare, apAya, hAni, kaLaMka -- taruvaMtha heVLike koTuTxdAgi tiLisu, heVLu. 
\emng
\eentry

\bentry
\word{verbalisable} 
\pron{vabaRleYsaZbalf} 
\gl{\gu}
\expl{}
\bmng
 kirxyApadiVkaraNa sAdhayx; (nAmapada \mo vanunx) kirxyApadavanAnxgi mADabahudAda. 
\emng
\eentry

\bentry
\word{verbalisation} 
\pron{vabaRleYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{verbalization}{verbalization}. 
\emng
\eentry

\bentry
\word{verbalise} 
\pron{vabaRleYsfZ} 
\gl{\kirx}
\expl{}
\bmng
 = \hyperlink{verbalize}{verbalize}. 
\emng
\eentry

\bentry
\word{verbalism} 
\pron{vabaRlisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} pada; shabadx; shAbidxka aBivayxkitx. 
\num{2} shAbadx daqSiTx; shAbidxka vimasheR; shabadxshudidhxge, shabadxda rUpa parxyoVgagaLige sUkaSx mxgamana niVDuvike. 
\num{3} shabadx pArxdhAnayx; vasutxsithxti yA athaRkikxMtalU shabadxkekx, padakekx, BASege niVDuva pArxmuKayx. 
\num{4} shabAdxdhikayx; shabAdxDaMbara; AvashayxkavAdudakikxMtalU hecAcxgiruva matutx athaRvanunx masuku mADutatxdeMdu BAvisalAda shabadxparxyoVga. 
\enum
\emng
\eentry

\bentry
\word{verbalist} 
\pron{vabaRlisfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} shabadxkushali; shabadxparxyoVgadalilx niSANxta. 
\num{2} shabadxrasika; shabadxperxVmi; athaRkikxMtalU viSayakikxMtalU shabadxgaLalilx AsakatxnAdavanu. 
\enum
\emng
\eentry

\bentry
\word{verbalistic} 
\pron{vabaRlisiTxkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} shabadxjAlada. 
\num{2} shabadxmoVhada; shabadxrasikateya. 
\enum
\emng
\eentry

\bentry
\word{verbalization} 
\pron{vabaRleYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 (nAmapada \mo vanunx) kirxyApadavanAnxgi mADuvudu; kirxyApadarUpakekx taruvudu. 
\emng
\eentry

\bentry
\word{verbalize} 
\pron{vabaRleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (nAmapada \mo vanunx) kirxyApada mADu; kirxyApadarUpakekx taru. 
\num{2} padagaLalilx, shabadxgaLalilx, vAgUrxpadalilx -- heVLu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 shabadxjAla heNe; viSayakikxMta atiyAgi pada baLasu. 
\emng
\eentry

\bentry 
\word{verbalizer} 
\pron{vabaRleYsaZrf} 
\gl{\nA}
\expl{}
\bmng
 kirxyApadiVkAraka; nAmapada \mo vanunx kirxyApadavanAnxgi mADuvavanu. 
\emng
\eentry

\bentry
\word{verbally} 
\pron{vabaRli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} padataH; shabadxtaH; padagaLalilx; shabadxgaLa mUlaka. 
\num{2} mAtina mUlaka; vAcA. 
\enum
\emng
\eentry

\bentry
\wordnospeech{verbal noun}{verbal noun} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 kaqnAnxma; dhAtuviniMda huTiTx kirxyApadada kirxyegaLalilx kelavanunx (\kanmu\ apUNaRvAda kirxyeyanunx) sUcisuva nAmarUpa, \udA\ \eng{swimming is a thrilling exercise} eMbalilx \eng{swimming} shabadx. \eng{smoking is forbidden} eMbalilx \eng{smoking.} 
\emng
\eentry

\bentry
\wordnospeech{verbal senses}{verbal senses}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 kirxyAthaRgaLu; (yAvudeV dhAtuviniMda janayxvAda padagaLa \vi) A dhAtuvina kirxyApadada athaRgaLananxnusarisida athaRgaLu. 
\emng
\eentry

\bentry
\word[verbatim(1)]{verbatim} 
\pron{vabeRVTimf} 
\gl{\kirxvi}
\expl{}
\bmng
 padashaH; shabadxshaH; adeV padagaLalilx; padakekx pada hAki; mUlavanunx cAcU tapapxde; mUla idadxMteyeV: \eng{copied it verbatim} mUlAnusAravAgi nakalu mADida. 
\emng
\eentry

\bentry
\word[verbatim(2)]{verbatim} 
\pron{vabeRVTimf} 
\gl{\gu}
\expl{}
\bmng
 padakekx pada hAkida; mUladaMteyeV cAcU tapapxde iruva: \eng{verbatim copy} mUlAnusAriyAda nakalu. 
\emng
\eentry

\bentry
\word{verbena} 
\pron{vabiRVna} 
\gl{\nA}
\bmng
 vabiRVna; sugaMdhaBarita hUgaLa goMcalugaLanunx biDuva, giDagaLa kula yA A kulada giDa. \imglink{verbena:fig}{\raisebox{-0.25cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.7cm {V_Pictures/verbena.jpg}}} 
\emng
\eentry

\bentry
\word{verbiage} 
\pron{vabiRijf} 
\gl{\nA}
\expl{}
\bmng
 mAtina kagagx; shabadxjAla; shabAdxDaMbara; shabadxbAhuLayx. 
\emng
\eentry

\bentry
\word{verbicide} 
\pron{vabiRseYDf} 
\gl{\nA}
\expl{}
\bmng
 (\hA) pada, shabadx -- hateyx; padagaLa, shabadxgaLa -- kole; padagaLa, rUpa, athaR, BAva, parxyoVgagaLanunx hALugeDisuvudu. 
\emng
\eentry

\bentry
\word{verbose} 
\pron{vaboRVsf} 
\gl{\gu}
\expl{}
\bmng
 shabadxbAhuLayxda; atimAtina; shabAdxDaMbarada. 
\emng
\eentry

\bentry
\word{verbosely} 
\pron{vaboRVsfli} 
\gl{\kirxvi}
\expl{}
\bmng
 shabadxbAhuLayxdiMda; atiyAgi padagaLanunx baLasi. 
\emng
\eentry

\bentry
\word{verboseness} 
\pron{vaboRVsfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{verbosity}{verbosity}. 
\emng
\eentry

\bentry
\word{verbosity} 
\pron{vabARsiTi} 
\gl{\nA}
\expl{}
\bmng
 atimAtu; mAtina kaMte; shabadxbAhuLayx; shabAdxDaMbara. 
\emng
\eentry

\bentry
\wordf{verboten} 
\pron{vaboRVTanf} 
\gl{\gu}
\expl{\G\ }
\bmng
 (\kanmu\ adhikArigaLiMda, adhikaqtavAgi) niSeVdhisalapxTaTx; parxtiSeVdhisalapxTaTx. 
\emng
\eentry

\bentry
\word{verb.sap.} 
\pron{vabfR sAYxpf} 
\gl{\BAavayx}
\expl{}
\bmng
 (jANanAdavanige oMdeV mAtu sAku, hecucx vivarisa beVkAgilalx eMba athaRdalilx) (jANanige) oMdeV mAtu! 
\emng
\eentry

\bentry
\word{verdancy} 
\pron{vaDaRnisx} 
\gl{\nA}
\expl{}
\bmng
 hacacxtana; pacecxtana; hacacxhasurAgiruvike; haridavxNaRtavx. 
\emng
\eentry

\bentry
\word{verdant} 
\pron{vaDaRMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (hululx \mo vugaLa \vi) hacacxneya; hacacxhasurAda. 
\num{2} (meYdAna, hola, \mo vugaLa \vi) hacacxneya hululx beLediruva; pacecx beLediruva. 
\num{3} (vayxkitxya \vi) EnU ariyada; hasuLeya savxBAvada; mugadhx; ananuBaviyAda; loVkada kapaTa, vaMcane, \mo vanunx ariyada. 
\enum
\emng
\eentry

\bentry
\word{verd-antique} 
\pron{vaDARYxMTiVkf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} pacecx; oDavegaLalilx ratanxvAgi baLasuva, sapeRMTeYnf eMba hasuru baNaNxda parxshasatxshile. 
\num{2} (haLeya kAlada kaMcina pAterxgaLa meVle kaTiTxkoMDiruva) hasuru kiTaTx. 
\num{3} hasuru pAphiRri (\eng{porphyry}) shile. 
\enum
\emng
\eentry

\bentry
\word{verdantly} 
\pron{vaDaRMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
 hacacxhacacxge; hacacxhasurAgi. 
\emng
\eentry

\bentry
\word{verderer} 
\pron{vaDaRrarf} 
\gl{\nA}
\expl{}
\bmng
 (\birx) rAjAraNayxda nAyxyAdhiVsha; rAjana kADugaLa nAyxyAdhikAri. 
\emng
\eentry

\bentry
\word{verdict} 
\pron{vaDiRkfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (sivilf yA kirxminalf mokadadxmeya satAyxMshavanunx kuritu) nAyxyadashiR maMDali niVDida tiVpuR: \eng{the jury brought in a verdict for the plaintiff} nAyxyadashiR maMDaliyu vAdiya paravAgi tiVpuR niVDitu. \eng{the jury brought in a verdict of not guilty} ApAditanu(ru) nidoRVSi(gaLu) eMdu nAyxyadashiR maMDali tiVpuR niVDitu. 
\num{2} tiVmARna; niNaRya: \eng{the verdict of the people was in his favour} janara tiVmARna, niNaRya avana paravAgitutx. 
\enum
\emng
\eentry

\bentry
\word{verdigris} 
\pron{vaDiRgirx(V)sf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (tAmarxda meVle aseTikf Amalxda kirxyeyiMda uMTAguva, haraLugaTiTxda, auSadhacikitesxyalUlx raMgugaLalUlx baLasuva) hasuru kiTaTx. 
\num{2} (tAmarxda yA hitAtxLeya meVle uMTAguva) hasuru tukukx; kilubau. 
\enum
\emng
\eentry

\bentry
\word{verditer} 
\pron{vaDiRTarf} 
\gl{\nA}
\expl{}
\bmng
 (niVliya yA hasurina raMgugaLige mUladarxvayxvAgi baLasuva) tAmarxda kAboRneVTf. 
\emng
\eentry

\bentry
\word{verdure} 
\pron{vaDaRYxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sasayxvagaRda haridavxNaRte; hasurutana. 
\num{2} hasurubaNaNxda sasayxrAji. 
\num{3} (\kAparx) hosa kaLe; navayxteya kAMti. 
\enum
\emng
\eentry

\bentry
\word{verdured} 
\pron{vaDaRYxDfR} 
\gl{\gu}
\expl{}
\bmng
 hacacxneya sasayxrAjiyuLaLx. 
\emng
\eentry

\bentry
\word{verdurous} 
\pron{vaDaRYxrasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hacacxneya; sasayxhasurina. 
\num{2} hacacxneya; haridavxNaRda; sasayxmayavAda. 
\enum
\emng
\eentry

\bentry
\word[verge(1)]{verge} 
\pron{vajfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (tutatxtudiya) ENu; gaDi; aMcu: \eng{he stood at the verge of the cliff} avanu A kaDibaMDeya tudiyaMcinalilx niMtidadx. \eng{he drew near to the very verge of the stream} avanu A hoLeya aMcigeV baMdubiTaTx. \eng{on the verge of seventy} epapxtatxra aMcinalilx. \eng{on the verge of destruction} vinAshada aMcinalilx. \eng{on the verge of tears} kaNiNxVrina gaDiyalilx; kaNiNxVru surisuva sithxtiyalilx. \eng{on the verge of betraying his secret} avanu tananx guTaTxnunx raTuTxmADuvudaralilxdadx. 
\num{2} (\birx) (hUgiDada, rasetx, \mo vugaLa) hulilxna -- aMcu, arugu, diMDu, teMDe. 
\num{3} (\vAshi) caMdAyada, geVbalina heMcineVNina muMcAcu. 
\num{4} (biSapf, DiVnf, \mo vara muMde hiDidukoMDu hoVguva, avara padaviya lAMCanavAda) daMDa; doNeNx; kaTiTxge. 
\num{5} (\ca) (lADfR heY siTxVvaDfR eMba birxTaninxna rAjana yA rANiya kiriVTABiSeVkadalilx adhayxkaSxnAgi vatiRsuva adhikAriya) adhikAra vAyxpitxge oLapaTaTx parxdeVsha. 
\enum
\emng
\eentry

\bentry
\word[verge(2)]{verge} 
\pron{vajfR} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} keLakekx Olu, bAgu; iLimuKavAgu: \eng{the now verging sun} Iga iLimuKavAgutitxruva sUyaR. 
\num{2} samiVpisu; hatitxravAgu; aMcinalilxru: \eng{verge towards old age} mupipxnatatx sari; mupapxnunx samiVpisu. \eng{verge towards the close} samApitxya hatitxravAgu, aMcinalilxru. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\eng{verge on} aMcina hatitxra iru; samiVpisu; hatitxra iru, baMdiru: \eng{the path verges on the edge of a precipice} A dAri oMduparxpAtada aMcina baLi ide. \eentry

\bentry
\word{verger} 
\pron{vajaRrf} 
\gl{\nA}
\bmng
\bnum
\num{1} caciRna harikAra; caciRnalilx janaranunx avaravara Asanakokxyudx kUrisuva adhikAri. 
\num{2} (\birx) (biSapapxna yA vishavxvidAyxnilayada kulapati yA upakulapatiya muMde avaravara adhikAra lAMCanavAda daMDa hiDidukoMDu hoVguva) kaTiTxgeyavanu; daMDavAhaka. 
\enum
\emng
\eentry

\bentry
\word{vergership} 
\pron{vajaRrfSipf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} caciRna harikAra padavi. 
\num{2} daMDavAhaka padavi. 
\enum
\emng
\eentry

\bentry
\word{verglas} 
\pron{veVgARlxsf} 
\gl{\nA}
\expl{}
\bmng
 himada teLu padara; kiTakiya kada \mo vugaLa meVle bidadx himada teLuleVpa yA gaDeDxgaTiTxda maLeniVrina padara. 
\emng
\eentry

\bentry
\word{veridical} 
\pron{viriDikalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\aupa) satayxsaMdha(nAda); diTavanenxV, nijavanenxV -- nuDiyuva. 
\num{2} (\mashA) (kanasugaLu \mo vugaLa \vi) nijaGaTanegaLaMteyeV, vAsatxvAMshagaLaMteyeV iruva; vAsatxvikavAgi toVruva; yAthAtathayxvAgiruva. 
\enum
\emng
\eentry

\bentry
\word{veridicality} 
\pron{viriDikAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} satayxsaMdhate; pArxmANikate. 
\num{2} (\mashA) (kanasugaLa \vi) yAthAtathayxte; vAsatxvikavAgi, nijajiVvanadalilx iruvaMteyeV iruvike, toVruvike. 
\enum
\emng
\eentry

\bentry
\word{veridically} 
\pron{viriDikali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} satayxsaMdhanAgi; nijavanenxV nuDiyuvaMthavanAgi. 
\num{2} (\mashA) vAsatxvikavAgi toVrutAtx; jiVvanada GaTanegaLaMteyeV iruva; yathAtathayxvAgi. 
\enum
\emng
\eentry

\bentry
\word{veriest} 
\pron{ve(ri)isfTx} 
\gl{\gu}
\expl{}
\bmng
 (\eng{very} eMbaudara tamarUpa). (\pArxparx) 
\bnum
\num{1} nijavAda. 
\num{2} atayxMta: \eng{the veriest fool knows it} atayxMta daDaDxnigU adu gotutx. 
\enum
\emng
\eentry

\bentry
\word{verifiable} 
\pron{veripheYabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sari, tALe -- noVDabahudAda; tAkaRNe mADabahudAda. 
\num{2} parxyoVga, parxmANagaLiMda satayxveMdu samathiRsabahudAda, rujuvAtupaDisabahudAda. 
\enum
\emng
\eentry

\bentry
\word{verifiably} 
\pron{veripheYabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} tALe noVDabahudAdaMte. 
\num{2} parxmANagaLiMda satayxvanunx samathiRsuvaMte. 
\enum
\emng
\eentry

\bentry
\word{verification} 
\pron{veriphikeVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} tAkaRNe; sari, tALe -- noVDuvudu. 
\num{2} (\tashA) parxmANiVkaraNa; parxtijAcnxvAkayxda satayxteyanunx pArxyoVgikavAgi sAthxpisuvudu. 
\numi{3} 
\banum
\alnum{a} (\nAyxshA) dAveya vAda parxtivAdagaLige rujuvAtugaLiMda yA parxmANapatarxdiMda sAkaSxyX odagisuvudu. 
\alnum{b} daqDhiVkaraNa; dAveya vAda parxtivAdagaLanunx rujuvAtugaLu yA parxmANapatarxda mUlaka daqDhapaDisuvudu, satayxveMdu samathiRsuvudu. 
\eanum
\numie
\num{4} (shasAtxrXsatxrXgaLa opapxMdagaLalilx vidhisiruva) tAkaRNe; sarinoVDuva vidhAnagaLu. 
\enum
\emng
\eentry

\bentry
\word{verifier} 
\pron{veripheYarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} tAkaRNegAra; sari, tALe -- noVDuvavanu. 
\num{2} rujuvAtugAra; samathaRka; heVLike, aMkiaMshagaLu, \mo vanunx satayxveMdu sAdhAravAgi rujuvAtu mADuvavanu. 
\enum
\emng
\eentry

\bentry
\word{verify} 
\pron{veripheY} 
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{verifies}; \BU\ matutx}
\BUkaq\ \eng{verified}). \bmng
\bnum
\num{1} sari, tALe, tAkaRNe -- noVDu: \eng{always verify your references} ninanx ulelxVKagaLanunx tapapxde sarinoVDu, sariyAgideye eMbaudanunx Kacita mADiko. 
\num{2} parishiVlane, parxyoVga yA pariVkeSxgaLa mUlaka viSayavu satayx yA sari eMbaudanunx rujuvAtupaDisu: \eng{I must verify his statement} avana heVLike nijave eMbaudanunx nAnu pariVkiSxsi noVDabeVku. 
\num{3} (GaTane, kAyaR, \mo vugaLa \vi) niriVkaSxNege, vAgAdxna, \mo vakekx -- anuguNavAgiru; niriVkaSxNe, vAgAdxna, \mo vanunx nijapaDisu. 
\numi{4} (\nAyxshA) 
\banum
\alnum{a} dAveyalilx sUcisiruva vAdagaLige yA parxtivAdagaLige keYPiyatatxnunx yA dAKaleyanunx lagatitxsu. 
\alnum{b} heVLikeyanunx rujuvAtugaLiMda yA parxmANapatarxdiMda (satayxveMdu) samathiRsu, daqDhapaDisu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{verily} 
\pron{verili} 
\gl{\kirxvi}
\expl{}
\bmng
 (\pArxparx) nijavAgiyU; diTakUkx; nijakUkx; satayxvAgiyU. 
\emng
\eentry

\bentry
\word{verisimilar} 
\pron{verisimilarf} 
\gl{\gu}
\expl{}
\bmng
 nijada toVkeRya; diTaveMdu toVruva; ApAtasatayxda; satAyxBAsada. 
\emng
\eentry

\bentry
\word{verisimilitude} 
\pron{verisimiliTUyxDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nijaveMba, diTaveMba -- toVkeR; satayxveMdu toVruvike, kANuvike; ApAtasatayx; satAyxBAsa: \eng{the verisimilitude of the tale} A katheyalilx kaMDubaruva satayxteya toVkeR. \eng{verisimilitude is not proof} satayxteya toVkeRyeV rujuvAtalalx; satayxveMdu toVrida mAtarxkekx satayxvenanxlAguvudilalx. 
\num{2} nijaveMdu kANuva, nijavenisuva heVLike \mo vu. 
\enum
\emng
\eentry

\bentry
\word{verism} 
\pron{viarisaZmf} 
\gl{\nA}
\expl{}
\bmng
 (sAhitayxdalilx yA kaleyalilx) vAsatxvikatA vAda; badukina, vasutxsithxtiya niSuThxra nirUpaNe sAhitayxkalegaLige atAyxvashayxka enunxva vAda yA adara anusaraNe. 
\emng
\eentry

\bentry
\word{verismo} 
\pron{verisoZmxV} 
\gl{\nA}
\expl{(\bava\ \eng{verismos}).}
\bmng
 (\kanmu\ giVtanATakada yA AperAda \vi) = \hyperref{kandict_r.pdf}{R}{realism(4)}{realism(4)}. 
\emng
\eentry

\bentry
\word{verist} 
\pron{viarisfTx} 
\gl{\nA}
\expl{}
\bmng
 (sAhitayxdalilx yA kaleyalilx) vAsatxvikatA vAdi. 
\emng
\eentry

\bentry
\word{veristic} 
\pron{viarisiTxkf} 
\gl{\gu}
\expl{}
\bmng
 (\sA\ sAhitayx yA kaleyalilx) vAsatxvikatA vAdada. 
\emng
\eentry

\bentry
\word{veritable} 
\pron{veriTbflf} 
\gl{\gu}
\expl{}
\bmng
 nijavAda; diTa enanxbahudAda: \eng{a veritable boon} nijavAda vara. 
\emng
\eentry

\bentry
\word{veritably} 
\pron{veriTbilx} 
\gl{\kirxvi}
\expl{}
\bmng
 nijakUkx; nijavAgiyU; diTakUkx; diTavAgiyU. 
\emng
\eentry

\bentry
\word{verity} 
\pron{veriTi} 
\gl{\nA}
\expl{(\bava\ \eng{verities}). }
\bmng
\bnum
\num{1} (heVLike \mo vugaLa) nijatavx; satayxte. 
\num{2} (mUlaBUtavAda satayxvanonxLagoMDa) nijavAda heVLike; yathAthoRVkitx; satoyxVkitx. 
\num{3} (nijakUkx iruva) satayx vasutx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{of a verity} (\pArxparx) diTakUkx; nijakUkx; nijavAgiyU; satayxvAgiyU. 
\emng
\eentry

\bentry
\word{verjuice} 
\pron{vajURsf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (hiMde aDigeyalUlx auSadha cikitesxyalUlx baLasutitxdadx, kADuseVbau, huLidArxkiSx, \mo vugaLiMda tayArisutitxdadx) huLi madayx. 
\num{2} kahiBAvane, asamAdhAna, asaMtoVSa, \mo vu. 
\enum
\emng
\eentry

\bentry 
\wordf[verkrampte(1)]{verkrampte} 
\pron{pheVrfkArxYxmfpfTx} 
\gl{\gu}
\expl{\SAfr\ }
\bmng
(\kanmu\ kariyaranunx kurita vaNaRBeVda niVtiya \vi) saMkucita manoVvaqtitxya; anudAra savxBAvada; parxgatiparavalalxda. 
\emng
\eentry

\bentry 
\wordf[verkrampte(2)]{verkrampte} 
\pron{pheVrfkArxYxmfpfTx} 
\gl{\nA}
\expl{\SAfr}
\bmng
anudAri; saMkucita manoVvaqtitxyavanu. 
\emng
\eentry

\bentry
\wordf{verligte} 
\pron{pheVliRKfTx} 
\gl{\gu}
\expl{\SAfr}
\bmng
(\kanmu\ kapupx janAMgagaLanunx kurita vaNaRBeVda niVtiya \vi) vishAla manoVvaqtitxya; udAra savxBAvada; parxgatipara. 
\emng
\eentry

\bentry
\word{vermeil} 
\pron{vameRV(miR)lf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} beLiLxya mulAmu, giliVTu; rajataleVpana. 
\num{2} (kitatxLekeMpubaNaNxda) padamxrAga; rakatxmaNi. 
\num{3} (\kAparx) kuMkuma(baNaNx). 
\enum
\emng
\eentry

\bentry
\word{vermi-} 
\pron{vamiR\eng{-}} 
\gl{\sapUpa}
\expl{}
\bmng
huLu, kirxmi eMbathaRda \sapUpa 
\emng
\eentry

\bentry
\word{vermian} 
\pron{vamiRanf} 
\gl{\gu}
\expl{}
\bmng
huLugaLa; huLugaLanunx hoVluva; kirxmisadaqsha. 
\emng
\eentry

\bentry
\word{vermicelli} 
\pron{vamiRse(ce)li} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} seVvige; shAvige. 
\num{2} keVkfmeVle alaMkaraNakAkxgi baLasuva, cAkaleVTina turigaLu. 
\enum
\emng
\eentry

\bentry
\word{vermicide} 
\pron{vamiRseYDf} 
\gl{\nA}
\expl{}
\bmng
kirxminAshaka; huLugaLanunx kolulxva auSadha, vasutx. 
\emng
\eentry

\bentry
\word{vermicular} 
\pron{vamiRkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (rUpa, calanegaLalilx) kirxmiyaMtiruva; kirxmirUpada; kirxmisadaqsha. 
\num{2} (\veYshA) jaMtuhuLugaLa yA jaMtuhuLugaLiMda uMTAda. 
\num{3} otAtxda vaMkiyAkArada geregaLiMda tuMbida. 
\enum
\emng
\eentry

\bentry
\word{vermiculate} 
\pron{vamiRkuyxlaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{vermicular}{vermicular}. 
\num{2} huLu tiMda. 
\enum
\emng
\eentry

\bentry
\word{vermiculation} 
\pron{vamiRkuyxleVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} huLugaLiMda tiMduhoVgiruvike. 
\alnum{b} huLugaLa kATakokxLagAgiruvike. 
\alnum{c} huLugaLa rUpa taLediruvike; huLugaLa rUpakekx baMdiruvike. 
\eanum
\numie
\num{2} vaMki gurutu; vaMkiyAkArada reVKAcitarxNa. 
\num{3} huLu tiMda sithxti; kirxminaSaTx sithxti. 
\enum
\emng
\eentry

\bentry
\word{vermiculite} 
\pron{vamiRkuyxleYTf} 
\gl{\nA}
\expl{}
\bmng
 \sA\ aBarxkavu vayxtAyxsagoMDu, kAyisi sapxMjinaMte ubibxsida matutx shAKaroVdhakavAgi baLasuva silikeVTf Kanija. 
\emng
\eentry

\bentry
\word{vermiform} 
\pron{vamiRphAZmfR} 
\gl{\gu}
\expl{}
\bmng
 kirxmirUpada; kirxmirUpi. 
\emng
\eentry

\bentry
\wordnospeech{vermiform appendix}{vermiform appendix}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\aMrashA) = \hyperref{kandict_a.pdf}{A}{appendix(3)}{appendix(3)}. 
\emng
\eentry

\bentry
\word[vermifuge(1)]{vermifuge} 
\pron{vamiRphUyxjf} 
\gl{\gu}
\expl{}
\bmng
 (karuLugaLa) jaMtuhuLugaLanunx horadUDuva. 
\emng
\eentry

\bentry
\word[vermifuge(2)]{vermifuge} 
\pron{vamiRphUyxjf} 
\gl{\nA}
\expl{}
\bmng
 (karuLugaLalilx huTiTxkoMDiruva jaMtuhuLugaLanunx horadUDuva) jaMtuhuLu auSadha. 
\emng
\eentry

\bentry
\word[vermilion(1)]{vermilion} 
\pron{vamiRlayxnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} iMgaliVka; rasasiMdhUra. 
\num{2} (iMgaliVkavananxredu yA kaqtakavAgi tayArisida) kuMkuma yA kuMkuma vaNaRda raMgu. 
\enum
\emng
\eentry

\bentry
\word[vermilion(2)]{vermilion} 
\pron{vamiRlayxnf} 
\gl{\gu}
\expl{}
\bmng
 kuMkuma(vaNaRda). 
\emng
\eentry

\bentry
\word{vermin} 
\pron{vamiRnf} 
\gl{\nA}
\expl{}
\bmng
 (\sA\ \bava\ eMdu parigaNisalAgide). 
\bnum
\num{1} (pArxNi, pakiSx yA beLe \mo vakekx hAnikaravAda) nari, muMgusi, ili, gUbe, nusikiVTa, \mo\ piVDakapArxNigaLu yA upadarxvakAri jaMtugaLu. 
\num{2} (cigaTa, tigaNe, heVnu, \mo\ sasayx, pArxNi yA manuSayxvagaRda meVle paroVpajiVvigaLAgidudx hAniyanunxMTumADuva) kirxmikiVTagaLu. 
\num{3} (parataMtarxjiVvigaLAda) huLuhupapxTegaLu; kirxmikiVTagaLu. 
\num{4} piVDakaru; duSaTxru; niVca jana. 
\enum
\emng
\eentry

\bentry
\word{verminate} 
\pron{vamiRneVTf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (piVDaka -- pArxNigaLa yA kirxmikiVTagaLa \vi) huTiTxsu; saMtAnoVtapxtitx mADu. 
\num{2} paroVpajiVvi \mo vugaLa kATakekx oLagAgu; huLuhupapxTe bididxru, hiDidiru. 
\enum
\emng
\eentry

\bentry
\word{vermination} 
\pron{vamiRneVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (beVTeya pArxNi yA pakiSx vagaRkekx, beLege, manuSayxvagaRkekx hAniyanunx mADuva nari, muMgusi, ili, mola, \mo) piVDakapArxNigaLu vaqdidhxyAguvike. 
\num{2} (paroVpajiVvigaLU upadarxvakArigaLU Ada) kirxmikiVTagaLu hecucxvike. 
\enum
\emng
\eentry

\bentry
\word{verminous} 
\pron{vamiRnasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} piVDaka pArxNigaLiMda yA kirxmikiVTagaLiMda tuMbida; piVDakapArxNigaLa, kirxmikiVTagaLa kATakokxLapaTaTx. 
\num{2} piVDaka -- pArxNi yA kirxmikiVTagaLaMte upadarxvakAriyAda; niVcanAda; duSaTxnAda. 
\enum
\emng
\eentry

\bentry
\word{vermivorous} 
\pron{vamiRvarasf} 
\gl{\gu}
\expl{}
\bmng
 kirxmiBakaSxka; huLudiniyAda; kirxmigaLanunx, huLugaLanunx tiMdu badukuva. 
\emng
\eentry

\bentry
\word{vermouth} 
\pron{vamaR(mUR)tf} 
\gl{\nA}
\expl{}
\bmng
 vamaRtf; vamURtf; `vamfRvuDf' eMba kahimUlike yiMda yA beVre bageya parimaLa biVruva mUlikegaLiMda vAsanegaTiTxda madayx, veYnu. 
\emng
\eentry

\bentry
\word[vernacular(1)]{vernacular} 
\pron{vanARYxkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (BASe, pada, BASArUDhi -- ivugaLa \vi) deVshayx; (savx)deVshiVya; anayxBASeya yA pArxciVna BASeyadalalxda yA avugaLiMda janisirada. 
\num{2} (vAsutxshilapxda \vi) sAmAnayx kaTaTxDagaLa sheYliya yA avakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word[vernacular(2)]{vernacular} 
\pron{vanARYxkuyxlarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} deVshayx; deVshiVya BASe: \eng{Sanskrit gave place to the vernacular} saMsakxqqta BASe hoVgi adara sAthxnadalilx deVshiVya BASege eDegoTiTxtu. 
\num{2} oMdu parxteyxVka vagaRda yA guMpina BASe. 
\num{3} sAmAnayxjanara BASe; saraLaBASe. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vernacular architecture}{vernacular architecture}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 deVshiVya vAsutxshilapx; (BAri parxmANada, anayxdeVshiVya yA BavayxsheYliyadalalxda) sAmAnayx kaTaTxDagaLanunx kaTuTxva sheYli. 
\emng
\eentry

\bentry
\word{vernacularise} 
\pron{vanARYxkuyxlareYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vernacularize}{vernacularize}. 
\emng
\eentry

\bentry 
\word{vernacularism} 
\pron{vanARYxkuyxlarisaZmf} 
\gl{\nA}
\expl{}
\bmng
 (BASe \mo vugaLa \vi) deVshayxte; deVshiVyate. 
\emng
\eentry

\bentry
\word{vernacularity} 
\pron{vanARYxkuyxlAYxriTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} deVshayxda olavu, pirxVti; deVshayxBASeyalelxV nuDiyuva, bareyuva saMkalapx matutx AcaraNe. 
\num{2} = \hyperlink{vernacularism}{vernacularism}. 
\enum
\emng
\eentry

\bentry
\word{vernacularize} 
\pron{vanARYxkuyxlareYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 (BASe \mo vanunx) deVshayxgoLisu; deVshiVyagoLisu; anayxBASegaLa yA paMDita BASegaLa pada \mo vanunx tegeduhAki, deVshayxpadagaLanenxV baLasi, BASeyanunx sAmAniyxVkarisu, saraLagoLisu. 
\emng
\eentry

\bentry
\word{vernacularly} 
\pron{vanARYxkuyxlarfli} 
\gl{\kirxvi}
\expl{}
\bmng
 deVshayxvAgi; deVshiVya riVtiyalilx. 
\emng
\eentry

\bentry
\word{vernal} 
\pron{vanaRlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vasaMtada; vasaMtaQutuvina; vasaMtakAlada: \eng{vernal breezes} vasaMtada mArutagaLu. 
\num{2} vasaMtavanunx hoVluva; vasaMtasadaqsha: \eng{vernal flowers} vasaMta kusumagaLu; vasaMtada yA vasaMtakAlada hUvugaLanunx hoVluva hUvugaLu. 
\num{3} vasaMta(kAla)dalilx naDeyuva yA mADuva: \eng{vernal migration} vasaMtada valase. \eng{vernal fancies} vasaMtoVcita kalapxnegaLu; vasaMta Qutuvige takakx kalapxnegaLu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vernal equinox}{vernal equinox}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 meVSa saMkArxMti; vasaMta saMkArxMti (\su\ mAciR \eng{21}). 
\emng
\eentry

\bentry
\wordnospeech{vernal fever}{vernal fever}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 caLijavxra; maleVriya javxra. 
\emng
\eentry

\bentry
\wordnospeech{vernal grass}{vernal grass} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (meVvina hulAlxgi beLeyuva, yUroVpf KaMDada, suvAsaneyuLaLx) shuMThi hululx. 
\emng
\eentry

\bentry
\word{vernalisation} 
\pron{vanaRleYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vernalization}{vernalization}. 
\emng
\eentry

\bentry
\word{vernalise} 
\pron{vanaRleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vernalize}{vernalize}. 
\emng
\eentry

\bentry
\word{vernalization} 
\pron{vanaRleYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 sheYtiyxVkaraNa; taMpike; hUvugaLu beVga biDaleMdu, neDuva munanx biVjagaLanunx taMpugoLisuvike. 
\emng
\eentry

\bentry
\word{vernalize} 
\pron{vanaRleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 sheYtiyxVkarisu; taMpugoLisu; hUvugaLu beVga biDaleMdu, neDuva munanx biVjagaLanunx taMpugoLisuvudu. 
\emng
\eentry

\bentry
\word{vernally} 
\pron{vanaRli} 
\gl{\kirxvi}
\expl{}
\bmng
 vasaMtadalilx; vasaMta-Qutuvinalilx, kAladalilx. 
\emng
\eentry

\bentry
\word{vernation} 
\pron{vaneRVSanf} 
\gl{\nA}
\expl{}
\bmng
 (\savi) kisalaya vinAyxsa; ele haMcike; cigurelegaLu haMcikoMDiruva riVti, karxma. 
\emng
\eentry

\bentry
\word{vernicle} 
\pron{vaniRka(kf)lf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{veronica(b)}{veronica(b)}. 
\emng
\eentry

\bentry
\word{vernier} 
\pron{vaniRarf} 
\gl{\nA}
\expl{}
\bmng
 vaniRyarf; udadxdaLateya viBAgagaLa, upaviBAgagaLa lekakx sikukxvaMte parxdhAna aLatepaTiTxge hoMdikoMDu adara meVle saridADuvaMte baMdhisalAgiruva inonxMdu aLatepaTiTx. 
\emng
\eentry

\bentry 
\wordnospeech{vernier engine}{vernier engine}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 vaniRyarf eMjinunx; AkAshakiSxpaNiya yA voyxVmarAkeTiTxna calaneyalilx saNaNx puTaTx badalAvaNegaLanunx mADalu A rAkeTfge joVDisiruva cikakx eMjinunx. 
\emng
\eentry

\bentry
\word{veronal} 
\pron{veranalf} 
\gl{\nA}
\expl{}
\bmng
 (\ravi) veronalf; bAbiRTUyxrikf Amalxda janayxgaLalolxMdAda shAmaka madudx. 
\emng
\eentry

\bentry
\word{veronica} 
\pron{varAnika} 
\gl{\nA}
\expl{}
\bmng
 varAnika: 
\banum
\alnum{a} niVli, UdA, nasugeMpu, biLibaNaNxgaLa hUvugaLanunx biDuva verAnika kulada giDagaLa jAti. 
\hypertarget{veronica(b)}{} 
\hyperdef{V}{veronica(b)}{} 
\alnum{b} yeVsu kirxsatxnu shilubeyanunx hotutx kAYxlavxrige hoVgutitxdAdxga Atana muKada bevaranunx saMta verAnikaLu orasidAga adara meVle pavADarUpadalilx kirxsatxmuKamudirxtavAda baTeTx. 
\alnum{c} kirxsatxmuKada ideV bageya beVrAvudeV citarx. 
\alnum{d} (gULikALagadalilx) meVle hAyutitxruva gULiya eduriniMda gULikALagakAranu baTeTxyanunx Acege biVsuva biVsu. 
\eanum
\emng
\eentry

\bentry
\word{verruca} 
\pron{varUka} 
\gl{\nA}(\bava\ \eng{verrucae}
\expl{\ucAcx\ varUsiV yA \eng{verrucas}). }
\bmng
 gaMTu; gaMti yA aMthadeV beLavaNige. 
\emng
\eentry

\bentry
\word{verrucose} 
\pron{verukoVsfZ} 
\gl{\gu}
\expl{}
\bmng
 gaMtiyaMtha; gaMTinaMtha. 
\emng
\eentry

\bentry
\word{verrucous} 
\pron{verukasf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{verrucose}{verrucose}. 
\emng
\eentry

\bentry 
\word{versant} 
\pron{vasaRMTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} iLimeVDu; iLijAru nela. 
\num{2} nelada -- iLijAru, iLAlu, iLukalu. 
\enum
\emng
\eentry

\bentry
\word{versatile} 
\pron{vasaRTeYlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} bahumuKa, savaRtoVmuKa -- sAmathaRyxvuLaLx; yAvudeV viSaya yA kAyaRdiMda beVroMdakekx sulaBavAgi tiruguva shakitx sAmathaRyxvuLaLx yA bahukAyoRVpayoVgiyAda: \eng{versatile author} bahumuKa parxtiBeyuLaLx garxMthakataR. \eng{versatile mind} bahumuKa baudidhx. \eng{versatile material} bahu kAyoRVpayoVgiyAda vasutx. 
\num{2} (\pArxvi\ matutx \savi) cali; tonedADuva; OlADuva; sututx tiruguva; AdhAravoMdara meVle sutatxlU yA meVliMda keLakekx sarAgavAgi calisuva. 
\num{3} (\pArxparx) badalAguva; asithxra; parivataRnashiVla. 
\enum
\emng
\eentry

\bentry 
\word{versatilely} 
\pron{vasaRTeYlfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} bahumuKa, savaRtoVmuKa -- sAmathaRyxvuLaLxvanAgi. 
\num{2} (vasutxvina \vi) bahukAyoRVpayoVgiyAgi. 
\enum
\emng
\eentry

\bentry
\word{versatility} 
\pron{vasaRTiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} bahumuKa -- citatx, baudidhx, parxtiBe, kwshala, sAmathaRyx. 
\num{2} bahukAyoRVpayoVgitavx. 
\enum
\emng
\eentry

\bentry
\wordf{vers de societe} 
\pron{veVrf Da soVsiETeV} 
\gl{\nA}
\expl{\F}
\bmng
 sAmayika kavite; parxsakatx vidayxmAnagaLanunx kurita, laGuhAsayxdiMda kUDida kavite. 
\emng
\eentry

\bentry
\word[verse(1)]{verse} 
\pron{vasfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (CaMdoVbadadhxvAda) padayxda -- paMkitx, sAlu: \eng{he quoted some verses of the Iliad} avanu iliyaDf kAvayxda kelavu sAlugaLanunx udadhxrisida. 
\num{2} padayx, vaqtatx, sholxVka yA caraNa. 
\num{3} padayx(kAvayx) yA adara oMdu parxkAra: \eng{he wrote pages of verse} Ata puTagaTaTxle padayxvanunx bareda. \eng{what is not prose is verse} gadayxvalalxdudx padayx. \eng{he was awarded a prize for Sanskrit verse} saMsakxqqta BASeyalilx padayxkAvayx baredadadxkAkxgi avanige bahumAna niVDalAyitu. 
\num{4} beYbalf yA itara dhAmiRka garxMthada adhAyxyada aMgavAda saMKeyx koTuTx sUcisida cikakx KaMDagaLalilx oMdu. 
\num{5} kirugavite; saNaNx padayx. 
\num{6} obabxneV heVLabeVkAda, pArxthaRnA giVte \mo vugaLa biDi caraNa, tani nuDi. 
\enum
\emng
\eentry

\bentry
\word[verse(2)]{verse} 
\pron{vasfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} padayxdalilx heVLu; padayxracisu; padayxvAgisu. 
\num{2} (\sA\ \AtAmx) shikaSxNa koDu; boVdhisu; tajacnxnAgisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 padayxracane mADu. 
\emng
\eentry

\bentry
\word[versed(1)]{versed} 
\pron{vasfRTx} 
\gl{\gu}
\expl{}
\bmng
 (viSaya, kAyaR, \mo vugaLalilx) tajacnx; nurita; paLagida; nipuNa; niSANxtanAda. 
\emng
\eentry

\bentry
\word[versed(2)]{versed} 
\pron{vasfRTx} 
\gl{\gu}
\expl{}
\bmng
 (\ga) vipayaRsatx; vayxtirikatxvAgiruva. 
\emng
\eentry

\bentry
\wordnospeech{versed sine}{versed sine}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 oMdu meYnasf(koVnada) koVseYnf(\eng{$1-\cos x$}). 
\emng
\eentry

\bentry
\word{verselet} 
\pron{vasfRliTf} 
\gl{\nA}
\expl{}
\bmng
 kirugavite; saNaNx padayx. 
\emng
\eentry

\bentry
\word{verset} 
\pron{vasiRTf} 
\gl{\nA}
\expl{}
\bmng
 (\saM) (AgaRnf vAdanakAkxgi racisida) saMkeSxVpavAda, pArxsAtxvika giVte yA aMtara giVte. 
\emng
\eentry

\bentry 
\word{versicle} 
\pron{vasiRkalf} 
\gl{\nA}
\expl{}
\bmng
 saMvAda giVte; sAmUhika pArxthaRneyalilx guruvU BakatxvaqMdavU payARya karxmadalilx heVLuva saNaNx vAkayx yA nuDi. 
\emng
\eentry

\bentry
\word{versicoloured} 
\pron{vasiRkalaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} baNaNxbaNaNxda; nAnAvaNaRda; bagebageya baNaNxda; vividha vaNaRda. 
\num{2} vaNaRveYvidhayxda; vaNaRvayxtAyxsada; beVrebeVre beLakugaLalilx beVrebeVre baNaNxkekx tiruguva. 
\enum
\emng
\eentry

\bentry
\word{versicular} 
\pron{vasiRkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ beYbalf \vi) cuTaku padayxgaLa; kirugavitegaLa. 
\emng
\eentry

\bentry
\word{versification} 
\pron{vasiRphikeVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (gadayxvanunx) padayxkekx tirugisuvudu; padiyxVkaraNa. 
\num{2} padayxracane; padayx bareyuvudu. 
\enum
\emng
\eentry

\bentry
\word{versifier} 
\pron{vasiZRpheYarf} 
\gl{\nA}
\expl{}
\bmng
 padayxkAra; padayxvanunx bareyuvavanu yA gadayxvanunx padayxkekx tirugisuvavanu. 
\emng
\eentry

\bentry
\word{versify} 
\pron{vasiZRpheY} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (gadayxvanunx) padayxkekx tirugisu. 
\num{2} padayxvanunx bare, racisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 padayxracane mADu. 
\emng
\eentry

\bentry
\word{versin} 
\pron{vaseRYnf} 
\gl{\nA}
\expl{}
\bmng
 (\ga) = \hyperlink{versed sine}{versed sine}. 
\emng
\eentry

\bentry
\word{versine} 
\pron{vaseRYnf} 
\gl{\nA}
\expl{}
\bmng
 (\ga) = \hyperlink{versed sine}{versed sine}. 
\emng
\eentry

\bentry
\word{version} 
\pron{vaSaRnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} BASAMtara; anuvAda; tajuRme: 
\banum
\alnum{a} garxMtha \mo vanunx oMdu BASeyiMda beVroMdakekx tirugisida rUpa. 
\alnum{b} (pAThashAlegaLalilx vidAyxthiR vidAyxthiRniyarige aBAyxsakAkxgi koDuva) savxBASeyiMda videVshiVya BASegoV videVshiVya BASeyiMda savxBASegoV parivatiRsuva kelasa. 
\eanum
\numie
\num{2} kathana; nirUpaNe; obabxnu yAvudeV viSayavanunx tananx daqSiTxyiMda heVLuva heVLike: \eng{his version of the incident} GaTaneyanunx kurita avana heVLike, kathana. 
\num{3} rUpAMtara; yAvudeV vasutxvanunx parxdashaRna \mo vugaLigAgi sarihoMdisida rUpaBeVda. 
\num{4} (parxsUti vijAcnxna) vataRna; herigege anukUlavAguvaMte gaBARshayadalilxna piMDagUsanunx keYyiMda tirugisuvudu. 
\num{5} pAThAMtara; garxMthada vividha parxtigaLalilx baruva alapxsavxlapx vayxtAyxsa. 
\num{6} garxMthavoMdara nidiRSaTx -- mudarxNa yA anuvAda: \eng{Authorised Version} (beYbalina) adhikaqta anuvAda. 
\enum
\emng
\eentry

\bentry
\word{versional} 
\pron{vaSaRnalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BASAMtarada; anuvAdada; tajuRmeya. 
\num{2} pAThAMtarada. 
\enum
\emng
\eentry

\bentry
\wordf{vers libre} 
\pron{veV liVbarx} 
\gl{\nA}
\expl{\F\ }
\bmng
 mukatx CaMdasisxna kAvayx; CaMdoVniyamagaLanunx upeVkiSxsida yA pArxsarahitavAda padayx, kavite. 
\emng
\eentry

\bentry
\wordf{vers librist} 
\pron{veV liVbirxsfTx} 
\gl{\nA}
\expl{\F\ }
\bmng
 mukatx CaMdasisxna yA misharxlayada padayxkAra. 
\emng
\eentry

\bentry
\word{verso} 
\pron{vasoRV} 
\gl{\nA}
\expl{(\bava\ \eng{versos}). }
\bmng
\bnum
\num{1} terediruva pusatxkada eDagaDeya puTa. 
\num{2} mudirxta yA hasatxparxtiya -- hALeya himumxKa, hiMBAga. 
\num{3} nANayxda -- himumxKa, hiMBAga. 
\enum
\emng
\eentry

\bentry
\word{verst} 
\pron{vasfTxR} 
\gl{\nA}
\expl{}
\bmng
 vasfTxR; \eng{1.1} ki. miV. ge yA \eng{6.6} meYlige samavAda raSAyxdeVshada udadxda aLate. 
\emng
\eentry

\bentry
\word{versus} 
\pron{vasaRsf} 
\gl{\upa}
\expl{}
\bmng
 (\kanmu\ \nAyxshA\ matutx kirxVDegaLalilx) (\saMkiSx \eng{v} yA \eng{vs.}) naDuve; edurAgi; virudadhxvAgi; parxti: \eng{Palkhiwala vs. Govt. of India} BArata sakARrada virudadhx pAliKxvAl(vAyxjayx). \eng{India vs. England} BArata, iMgelxMDugaLa naDuve (paMdayx). 
\emng
\eentry

\bentry
\word{vert} 
\pron{vaTfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} (\nAyxshA\ matutx \ca) 
\banum
\alnum{a} (kADinalilxya) hasuruvANi; hasureleya sasayxgaLu. 
\alnum{b} (kADina) hasuruvANiyanunx kaDidukoLuLxva hakukx. 
\eanum
\numie
\num{2} (\vaMlAM) hasuru. 
\enum
\emng
\eentry

\bentry
\word{vertebra} 
\pron{vaTiRbarx} 
\gl{\nA}
\expl{(\bava\ \eng{vertebrae} \ucAcx\ vaTiRbirxV). }
\bmng
\bnum
\num{1} kasheVruKaMDa; benunxhuriya parxtiyoMdu saMdhiya mULe. 
\num{2} (\bava dalilx) kasheVru; benunxmULe; benenxlubau. 
\enum
\emng
\eentry

\bentry
\word{vertebral} 
\pron{vaTiRbarxlf} 
\gl{\gu}
\expl{}
\bmng
 benunxmULeya; benenxlubina; kasheVruvina. 
\emng
\eentry

\bentry
\word[vertebrate(1)]{vertebrate} 
\pron{vaTiRbarx(berxV)Tf} 
\gl{\nA}
\expl{}
\bmng
 (\pArxvi) kasheVruka; vaTiRbarxTa upaviBAgakekx seVrida, sasatxnigaLu, pakiSxgaLu, uBayacarigaLu matutx miVnugaLanunx oLagoMDa, benenxlubauLaLx yAvudeV jiVvi. 
\emng
\eentry

\bentry
\word[vertebrate(2)]{vertebrate} 
\pron{vaTiRbarx(berxV)Tf} 
\gl{\gu}
\expl{}
\bmng
 kasheVrukagaLa yA avakekx saMbaMdhisida. 
\emng
\eentry

\bentry 
\word{vertebration} 
\pron{vaTiRberxVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kasheVrukiVya viBajane; benunxmULeya BAgagaLaMte viBAgisuvudu. 
\num{2} (\rUpa) sethxYyaR; dADhaRyx. 
\enum
\emng
\eentry

\bentry
\word{vertex} 
\pron{vaTeRkfsx} 
\gl{\nA}
\expl{(\bava\ \eng{vertices} \ucAcx\ vaTiRsiVsfZ yA}
\eng{vertexes}). \bmng
\bnum
\num{1} atuyxcacx, atuyxnanxta biMdu; shiKara. 
\num{2} (\aMrashA) netitx; talebauruDeya atuyxnanxta BAga. 
\numi{3} (\jAyx) shaqMga: 
\banum
\alnum{a} bahuBuja, bahumuKi, modalAda AkaqtigaLalilx eraDu reVKegaLu seVri koVnavanunx uMTumADuva biMdu. 
\alnum{b} aMDAkaqtiya yA diVGaRvaqtatxdaMtha vakarxgaLalilx akaSxvu vakarxvanunx saMdhisuva biMdu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[vertical(1)]{vertical} 
\pron{vaTiRkalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} shaqMgiVya; (kiSxtijiVya reVKege) laMbavAgiruva. 
\num{2} (citarx \mo vugaLalilx) meVliniMda taLada kaDeya; UdhAvxRdhara dikikxna. 
\num{3} atuyxnanxta biMduvina. 
\num{4} KamadhayxbiMduvina kaDeya yA mUlakada. 
\numi{5} samagarx; AdayxMta; naKashiKAMta: 
\banum
\alnum{a} oMdu saMsethxya sherxVNi vayxvasethxyalilx elAlx maTaTxgaLanUnx oLagoLuLxva. 
\alnum{b} oMdu dajeRya sarakina utApxdaneya elAlx haMtagaLanUnx oLagoLuLxva: \eng{vertical integration} AdayxMta saMGaTane; bauDadiMda tudiyavaregina elAlx BAgagaLanUnx saMGaTisuvudu, samagarxgoLisuvudu. 
\eanum
\numie
\num{6} (\aMrashA) netitxya yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word[vertical(2)]{vertical} 
\pron{vaTiRkalf} 
\gl{\nA}
\expl{}
\bmng
laMbareVKe yA laMbatala. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{out of the vertical} laMbavAgilalxda. 
\emng
\eentry

\bentry
\wordnospeech{vertical angles}{vertical angles}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\ga) shaqMgiVya koVnagaLu; eraDu saraLareVKegaLu aDaDx hAyAdxga rUpugoLuLxva eraDu joVDi edurubadaru koVnagaLalilx oMdu joVDi. 
\emng
\eentry

\bentry
\wordnospeech{vertical fin}{vertical fin}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (miVnina) beninxna meVliruva Ijurekekx. 
\emng
\eentry

\bentry
\wordnospeech{verticality}{verticality}
\pron{?}
\pron{vaTiRkAYxliTi} 
\gl{\nA}
\expl{}
\bmng
 shaqMgiVyate; shaqMgiVyavAgiruvudu; laMbatavx. 
\emng
\eentry

\bentry
\word{verticalize} 
\pron{vaTiRkaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 shaqMgiVyavAgisu; laMbavAgi mADu. 
\emng
\eentry

\bentry
\word{vertically} 
\pron{vaTiRkali} 
\gl{\kirxvi}
\expl{}
\bmng
 shaqMgiVyavAgi; laMbavAgi. 
\emng
\eentry

\bentry
\wordnospeech{vertical plane}{vertical plane} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 shaqMgiVya tala; laMbatala; kiSxtijakekx laMbavAgiruva tala. 
\emng
\eentry

\bentry
\wordnospeech{vertical take-off}{vertical take-off}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 laMba -- jigita, uDaDxyana; vimAnavu hArADalu neVravAgi meVlakekx jigiyuvudu. 
\emng
\eentry

\bentry
\word{verticil} 
\pron{vaTiRsilf} 
\gl{\nA}
\bmng
 (\pArxvi\ matutx \savi) maMDala; cakarxka; elegaLu, kUdalu, \mo\ BAgagaLu oMdu akaSxda sutatx vatuRlAkAravAgi sutitxkoMDiruvudu. \imglink{verticil:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.7cm {V_Pictures/verticil.jpg}}} 
\emng
\eentry

\bentry
\word{verticillate} 
\pron{vaTiRsilaTf} 
\gl{\gu}
\expl{}
\bmng
 maMDaliVya; cakirxVya; maMDalavAgiruva; vatuRlavAgi sutitxkoMDiruva. 
\emng
\eentry

\bentry
\word{vertiginous} 
\pron{vaTiRjinasf} 
\gl{\gu}
\expl{}
\bmng
 tale -- tiruguva, sututxva; talesutitx biVLuvaMtAguva. 
\emng
\eentry

\bentry
\word{vertiginously} 
\pron{vaTiRjinasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 tale tiruguvaMte; bavaLiyaMte. 
\emng
\eentry

\bentry
\word{vertigo} 
\pron{vaTiRgoV} 
\gl{\nA}(\bava\ \eng{vertigos}). 
\bmng
 taletirugu; talesututx; giraki; bavaLi. 
\emng
\eentry

\bentry
\word{vertu} 
\pron{vaTUR} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{virtu}{virtu}. 
\emng
\eentry

\bentry
\word{vervain} 
\pron{vaveRVnf} 
\gl{\nA}
\expl{}
\bmng
 (\savi) vaveRVnf (mUlike); vabiRna kulada, \kanmu\ saNaNxsaNaNx niVli, biLi yA neVriLe baNaNxda hUvugaLanunx biDuva, oMdu giDa. 
\emng
\eentry

\bentry
\word{verve} 
\pron{vavfR} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu) sAhitayx yA kalAkaqtiyalilx kaMDubaruva kasuvu, Ojasusx. 
\emng
\eentry

\bentry
\word{vervet} 
\pron{vaviRTf} 
\gl{\nA}
\expl{}
\bmng
 vaveRTf; kaMdubaNaNxda, Aphirxkada saNaNx kapi. 
\emng
\eentry

\bentry
\word[very(1)]{very} 
\pron{veri} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx) nijavAda; vAsatxvavAda; yathAthaRvAda; sAkASxtAtxda; sAcA: \eng{very God} nijavAda deVvaru. 
\num{2} parama; acacx; apapxTa; keVvala; shudAdhxMgavAda: \eng{the veriest simpleton knows that} shudadhx muTAThxLanU adanunx balalx. \eng{the very truth} parama satayx. \eng{must consent from very shame} keVvala nAcikegAdarU, apakiVtiRya BayakAkxdarU opapxleV beVku. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\eng{the, this, that, his, etc. very} ivaneV, ideV, avaneV, adeV, avana savxMta, itAyxdi athaRgaLalilx avadhAraNAthaRdalilx, otitxheVLuva \parx : \eng{this is the very spot where I found it} nanageadu doreta sathxLa, ideV. \eng{the very same} tadavxtAtxgiadeV. \eng{these were his very words} ivu avana savxMta mAtugaLu; ImAtugaLu avanaveV. \eng{speaking in this very room} I koThaDiyalelxVmAtanADutAtx. \eng{he did it this very day} avanu adanunx ideV dina,iMdeV mADida. \eng{the very fact of his presence is enough} avanuhAjarAgidadx eMba saMgatiyeV sAku. \eng{you are the very man I am looking for} nAnu huDukutitxruva vayxkitx niVneV. \eng{a needle is the very thing for our purpose} namamx udedxVshakekx beVkAda vasutxeMdare sUjiyeV. \eng{come here this very minute} I kaSxN(dalelxV)ililx bA. \eng{it grieves me to the very heart} adu nananxhaqdayAMtarALavanenxV vayxthegoLisutatxde. \eng{the very stones cry out} A kalulxgaLeV kUgi heVLutatxve. \eng{at the very edge of the cliff} A koVDugalilxna tutatx tudiyalilx. \eng{a very little will do} tiVra, keVvala savxlapx sAku. \eng{give me only a very little}nanage bahaLa savxlapx, oMdu cuTaku mAtarx koDu. \eentry

\bentry
\word[very(2)]{very} 
\pron{veri} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\numi{1} (-tamarUpada \gu\ doDane). 
\banum
\alnum{a} atayxMta; kaTaTxkaDeya; atayxdhika; gariSaThx; paramAvadhi : \eng{drink it to the very last drop} adanunx kaTaTxkaDeya haniyavaregU kuDidubiDu. \eng{the very last thing I expected} (niSeVdhAthaRdalilx mAtarx) nAnu niriVkiSxsidadxralilx atayxMta kaDeyadu (eMdare adanunx nAnu niriVkiSxsiraleV ilalx). \eng{did the very best I could} nananx keYyalAlxda paramAvadhiyanunx mADide. \eng{did my very utmost} nananx atayxdhika parxyatanxvanUnx mADide. 
\alnum{b} \eng{own} padadoDane pUtiRyAgi; saMpUNARthaRdalilx: \eng{my very own room} nananxdeV koVNe. \eng{you may keep it for your very own} adanunx niVnu pUtiR ninanx savxMtakAkxgi iTuTxkoLaLxbahudu. 
\eanum
\numie
\num{2} (\gu gaLu, visheVSa kaqdaMtagaLu matutx \kirxvi gaLoDane) tuMba; bahaLa; bahu; balu; tiVra: \eng{that is very easy} adu bahaLa sulaBa. \eng{it is very annoying} adu tiVra kirikiriyanunxMTu mADuvaMthadu. \eng{it is very easily done} adanunx bahu sulaBavAgi mADabahudu. \eng{it is a very trying time} idu tiVra kaSaTxkaravAda kAla. \eng{it very often fails} adu bahuveVLe viPalavAgutatxde. \eng{we find very few instances} nidashaRnagaLu dorakuvudu tiVra viraLa. \eng{he gives very little trouble} avaniMdAguva sharxma atayxlapx. \eng{is very much better} bahumaTiTxge utatxmavAgidAdxne. \eng{I am very much annoyed} nanage tiVra kirikiriyuMTAgide. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\hyperdef{V}{very(2) pagu(1)}{} 
\bnum
\numi{1} \eng{not very} 
\banum
\alnum{a} tiVra kaDime, atayxlapx. 
\alnum{b} tiVra asaMBava. 
\eanum
\numie
\num{2} \eng{very good} (\engit{or} \eng{well)} (samamxti sUcisuvalilx) oLeLxyadu; Agali; sari; Ayitu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{very high freqency}{very high freqency}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\Bwvi) (reVDiyoV \vi) atuyxcacx AvataRna; \eng{30}riMda \eng{300} megAhaTfRsx vAyxpitxyalilx baruva AvataRna. 
\emng
\eentry

\bentry
\wordRemoveSpace{Very-light}{Very light} 
\pron{verileYTf, viarileYTf} 
\gl{\nA}
\expl{}
\bmng
 baNaNxda beLaku; saMjecnxgAgi yA tAtAkxlika beLakigAgi visheVSa pisUtxliniMda cimimxsuva jAvxle. 
\emng
\eentry

\bentry
\wordRemoveSpace{Very-pistol}{Very pistol} 
\pron{veripisaTxlf, viaripisaTxlf} 
\gl{\nA}
\expl{}
\bmng
 baNaNxda beLakanunxMTumADuva visheVSa pisUtxlu. 
\emng
\eentry

\bentry
\wordnospeech{Very Reverend}{Very Reverend}
\pron{?}
\gl{\gu}
\expl{}
\bmng
 (DiVnana birudu) parama pUjayx. 
\emng
\eentry

\bentry
\word{vesica} 
\pron{vesika} 
\gl{\nA}
\bmng
\bnum
\num{1} (\aMrashA\ matutx \pArxvi) koVsha; Ashaya; yAvudeV koVsha, \kanmu\ mUtarxkoVsha. 
\hyperdef{V}{vesica(2)}{} 
\hypertarget{vesica(2)}{} 
\num{2} (kale) madhayxyugada citarxNa yA shilapxgaLalilx baLasutitxdadx, aMDAkArada, diVGaR vaqtAtxkArada -- cUpu tudiya parxBAmaMDala yA parxBAvaLi. \imglink{vesica 2.:fig}{\raisebox{-0.25cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.7cm{V_Pictures/vesica.jpg}}} 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{vesica piscis} (\engit{or} \eng{piscium}) = \hyperlink{vesica(2)}{vesica(2)}. 
\emng
\eentry

\bentry 
\word{vesical}
\pron{vesikalf}
\gl{\gu}
\bmng
(\aMrashA) (\kanmu\ mUtarx) koVshada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word[vesicant(1)]{vesicant} 
\pron{vesikaMTf} 
\gl{\gu}
\expl{}
\bmng
bokekxgaLanenxbibxsuva. 
\emng
\eentry

\bentry
\word[vesicant(2)]{vesicant} 
\pron{vesikaMTf} 
\gl{\nA}
\expl{}
\bmng
 bokekx; hopapxLe. 
\emng
\eentry

\bentry
\word{vesicate} 
\pron{vesikeVTf} 
\gl{\sakirx}
\expl{}
\bmng
 bokekxgaLanenxbibxsu. 
\emng
\eentry

\bentry
\word{vesication} 
\pron{vesikeVSanf} 
\gl{\nA}
\expl{}
\bmng
 bokekxgaLanenxbibxsuvike. 
\emng
\eentry

\bentry
\word{vesicatory} 
\pron{vesikeVTari} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vesicant(1)}{$^1$vesicant}. 
\emng
\eentry

\bentry
\word{vesicle} 
\pron{ve(vi)sikalf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\aMrashA, \pArxvi\ matutx \savi) koVshaka; cikakx koVsha yA kuhara; poTare; gUDu; ToLuLx racane. 
\num{2} (\veYshA) bokekx; hopapxLe. 
\num{3} (\BUvi) shilAkuLi; kalulxpoTare; jAvxlAmuKi shileyalilx anilagaLa kAraNadiMda uMTAguva poTare. 
\enum
\emng
\eentry

\bentry
\word{vesicular} 
\pron{ve(vi)sikuyxlarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\aMrashA, \pArxvi\ matutx \savi) kuLiyuLaLx yA kuLiyaMtha. 
\num{2} (\veYshA) bokekxyaMtha yA bokekx tuMbida. 
\enum
\emng
\eentry

\bentry
\word{vesiculate} 
\pron{ve(vi)sikuyxlaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} koVshayukatx; kuLiyuLaLx; koVshagaLiMda tuMbida. 
\num{2} kuLiyaMtha. 
\num{3} (\veYshA) bokekxgaLiMda tuMbida. 
\enum
\emng
\eentry

\bentry
\word{vesiculation} 
\pron{ve(vi)sikuyxleVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} koVshayukatxte; kuLigaLiruvike. 
\num{2} (\veYshA) (\kanmu\ camaRda meVle) bokekxgaLu ELuvike yA tuMbiruvike. 
\enum
\emng
\eentry

\bentry
\word{vesper} 
\pron{vesapxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (sAyaMtAreyAda) shukarx. 
\num{2} (\kAparx) saMje; sAyaMkAla; saMdhAyxsamaya. 
\num{3} (\bava dalilx) caciRna sAyaMpArxthaRne; saMjeya pArxthaRne. 
\num{4} Araneya pArxthaRne kAla; pArxthaRnegAgi gotutxpaDisuva veVLegaLalilx Araneyadu. 
\num{5} sAyaMpArxthaRnegAgi kareyuva GaMTe, GaMTAnAda. 
\enum
\emng
\eentry

\bentry
\word{vespertine} 
\pron{vesapxTiR(TeRY)nf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sAyaMkAlada; sAyaMkAladalilx naDeyuva. 
\num{2} (\savi) (hUvina \vi) sAyaMkAla araLuva; sAyaM vikasanada. 
\num{3} (\jiVvi) (pakiSxgaLa \vi) sAyaMkAla hAruva. 
\num{4} (\Kavi) sAyamasatxda; sUyARsatxda hotitxge muLuguva. 
\num{5} (\pArxvi) saMjehotutx caTuvaTike naDesuva. 
\enum
\emng
\eentry

\bentry
\word{vespiary} 
\pron{vesipxari} 
\gl{\nA}
\expl{(\bava\ \eng{vespiaries}). }
\bmng
 kaNajagaLa yA kaDijiVrigegaLa gUDu. 
\emng
\eentry

\bentry
\word{vespine} 
\pron{vesepxYnf} 
\gl{\gu}
\expl{}
\bmng
 kaNajagaLa; kaDijiVrigegaLa. 
\emng
\eentry

\bentry
\word{vessel} 
\pron{vesalf} 
\gl{\nA}
\bmng
\bnum
\num{1} (\kanmu\ darxvavanunx tuMbiDalu baLasuva, piVpAyi, kaDAyi, biMdige, tapapxle, \mo) pAterx; BAMDa; BAjana. 
\num{2} (\kanmu\ doDaDxdAda) haDagu yA doVNi. 
\numi{3} 
\banum
\alnum{a} (\aMrashA) rakatx yA itara darxvavanunxLaLx yA darxvavanunx sAgisuva nALa (\kanmu\ rakatxnALa). 
\alnum{b} (\savi) jiVvarasa \mo vanunx uLaLx yA sAgisuva sasayxnALa. 
\eanum
\numie
\num{4} (\beY\ yA \hA) (yAvudeV guNa \mo vanunx paDediruva yA nidashiRsuva) vayxkitx; pAtarx: \eng{chosen vessels} mukitxpAtarxru; moVkaSxvanunx paDeyabalalxvaru. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{vessels of} \hyperref{kandict_w.pdf}{W}{wrath pagu(2)}{wrath}. 
\num{2} \eng{weak vessel} dubaRlacitatx; daqDhanishacxyavilalxda vayxkitx; necicxkege anahaRnAda vayxkitx. 
\num{3} \eng{weaker vessel} (\beY) heMgasu; abale. 
\enum
\emng
\eentry

\bentry 
\word{vesselful} 
\pron{vesalfphulf} 
\gl{\nA}
\expl{}
\bmng
 pAterx tuMba; pAterx tuMbauvaSuTx parxmANa. 
\emng
\eentry

\bentry
\word[vest(1)]{vest} 
\pron{vesfTx} 
\gl{\nA}
\bmng
\hyperdef{V}{vest(1)1}{} 
\bnum
\num{1} vesuTx; naDuvaMgi; naDugavaca; deVhada meVlABxgavanunx mucucxva, bigiyAda kiru aMgi. \imglink{\(^1\)vest 1.:fig}{\raisebox{-0.20cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.6cm {V_Pictures/vest1.jpg}}} 
\num{2} (\AseTxrXV\ matutx \ame) veVsfTx koVTu; toVLilalxda naDuvaMgi. 
\hypertarget{vest(1)3}{} 
\num{3} heMgasara uDupina katitxna baLiya terapanunx mucucxva, \sA\ \eng{V} AkAradalilxruva vasatxrX. 
\num{4} (\pArxparx) uDigetoDige. 
\enum
\emng
\eentry

\bentry
\word[vest(2)]{vest} 
\pron{vesfTx}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ \kaparx) (obabxnige adhikAra, adhikAravuLaLx padavi, Asitx, \mo vanunx) vahisu. 
\num{2} (Asitx, adhikAra, \mo vanunx) vayxkitxya vashadalilx irisu. 
\num{3} (kiSxparxdalilx yA BaviSayxdalilx vashavAguva gotAtxda hakakxnunx) vidhuyxkatxvAgi obabxnalilx irisu. 
\num{4} (\kAparx) (uDige toDigeyanunx) uDisu; toDisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (Asitx, hakukx, \mo vugaLa \vi) vayxkitxya vashakekx baru yA vayxkitxya vashadalilxru. 
\num{2} (pAdirx \mo vara \vi) vihita vasatxrXvanunx toTuTxko, dharisu. 
\enum
\emng
\eentry

\bentry
\word{vesta} 
\pron{vesa Tx} 
\gl{\nA}
\expl{}
\bmng
 (\ca) (marada kaDiDxyalilx yA aragina kaDiDxyalilx mADida) beMkikaDiDx. 
\emng
\eentry

\bentry
\word[vestal(1)]{vestal} 
\pron{vesaTxlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\roVpu) (roVmanara gaqhABimAni deVvateyAda) vesaTx deVvateya yA vesaTx deVvatege apiRtarAda kaneyxyara. 
\num{2} shudadhx; barxhamxcayaRda. 
\enum
\emng
\eentry

\bentry
\word[vestal(2)]{vestal} 
\pron{vesaTxlf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} barxhamxcAriNi, \kanmu\ saMnAyxsini. 
\num{2} (\roV\ \pArxkatx) = \hyperlink{vestal virgin}{vestal virgin}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vestal virgin}{vestal virgin}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\roV\ \pArxkatx) (roVmanara gaqhABimAni deVvateyAda) vesaTx (deVvatege apiRtaLAda) kaneyx; puruSasaMgavilalxde kaneyxyAgiyeV uLiyuva varxtavanunx keYgoMDu, vesaTx deVvateya balipiVThadalilx sadA uriyutitxruva naMdAginxyanunx saMrakiSxsuva hoNehotatx kaneyx. 
\num{2} (\rUpa) barxhamxcAri kaneyx, \kanmu\ saMnAyxsini. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vested interest}{vested interest} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\nAyxshA) kAnUnubadadhx hitAsakitx; vayxkitxyobabxnige seVridudeMdu kAnUniniMda parigaNitavAda (\sA\ jamiVnu yA nAyxsavAgiTiTxruva haNadalilxna) hakukx. 
\num{2} paTaTxBadarx hitAsakitx; \sA\ lABada niriVkeSxyiMda, oMdu viSayadalilx iruva veYyakitxka Asakitx. 
\enum
\emng
\eentry

\bentry
\word{vestee} 
\pron{vesiTxV} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vest(1)3}{$^1$vest\((3)\)}. 
\emng
\eentry

\bentry
\word[vestiary(1)]{vestiary} 
\pron{vesiTxari} 
\gl{\nA}(\bava\ \eng{vestiaries}). 
\bmng
\bnum
\num{1} = \hyperlink{vestry}{vestry\((1)\)}. 
\num{2} uDupu koVNe; baTeTxbare hAkikoLaLxlu iruva koVNe. 
\enum
\emng
\eentry

\bentry
\word[vestiary(2)]{vestiary} 
\pron{vesiTxari} 
\gl{\gu}
\expl{}
\bmng
 uDigetoDigege saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vestibular} 
\pron{vesiTxbauyxlarf} 
\gl{\gu}
\expl{}
\bmng
 mogasAleya, hajArada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vestibule} 
\pron{vesiTxbaUyxlf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oLa koVNe, koThaDi, hajAra, naDave, naDumane, \mo vu. 
\numi{2} 
\banum
\alnum{a} (cacfR \mo vugaLa) mogasAle; muKamaMTapa. 
\alnum{b} saMpakaR -- koVNe yA hAdi; inonxMdu koVNe yA hAdiyoMdige saMpakaR kalipxsuva koVNe yA hAdi. 
\eanum
\numie
\num{3} (\ame) reYlevxgADige parxveVshisalu, AvaraNa hAkida parxveVsha hAdi. 
\num{4} halulxgaLa horagina bAyiBAga. 
\num{5} (\aMrashA) saMpakaR -- kuhara yA nALa; itara kuhara yA nALadoMdige saMpakaR kalipxsuva kuhara yA nALa. 
\num{6} kivikuhara; oLagiviya keVMdarxdalilxya kuLi. 
\num{7} (\ame) (keYgArikegaLalilx hosadAgi seVrida nwkararige shikaSxNa koDuva) naDave shAle; naDumane shAle. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vestibule train}{vestibule train} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (tudiyiMda tudige hAduhoVguvaMte) kirudAriyiruva reYlugADi; ONireYlu. 
\emng
\eentry

\bentry
\word{vestige} 
\pron{vesiTxjf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} jADu; suLivu; kuruhu; gurutu; cihenx: \eng{vestiges of an earlier civilization} hiMdina oMdu nAgarikateya kuruhugaLu, cihenxgaLu. \eng{I found no vestige of his presence} avanu idudxdara yAva kuruhanUnx nAnu kANalilalx. 
\num{2} lavaleVsha; leVsha mAtarx; eLaLxSUTx; raveyaSUTx; tilamAtarxvU; alapxsavxlapxvU: \eng{he has not a vestige of clothing upon him} avana meYmeVle lavaleVshavU baTeTxbare ilalx. \eng{he has not a vestige of evidence for his assertion} tananx heVLikeyanunx samathiRsikoLuLxva leVshamAtarx sAkaSxyXvU avana baLi ilalx. 
\num{3} (\jiVvi) avasheVSa; kuruhu; pUvaRjara deVhadalilx yAvudeV agatayxvanunx pUreYsutitxdudx, Iga kiridAgiruva yA kirxyAsAmathaRyxvanunx kaLedukoMDiruva yAvudeV aMga yA deVhaBAga: \eng{vestige of an ear in earless breeds} kivigaLilalxda taLigaLalilx kiviya kuruhu. 
\enum
\emng
\eentry

\bentry
\word{vestigial} 
\pron{vesiTxjialf, vesiTxjalf} 
\gl{\gu}
\expl{}
\bmng
 avasheVSavAgiruva; kuruhina. 
\emng
\eentry

\bentry 
\word{vestigially} 
\pron{vesiTxjiali, vesiTxjali} 
\gl{\kirxvi}
\expl{}
\bmng
 kuruhAgi. 
\emng
\eentry

\bentry
\wordnospeech{vesting day}{vesting day}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (Asitx, adhikAra, hakukx, \mo vu) vashakekx baruva dina; sAvxmayxdina. 
\emng
\eentry

\bentry
\word{vestiture} 
\pron{vesiTxcarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} (\pArxparx) 
\banum
\alnum{a} (adhikAra, birudu, \mo vanunx) vidhivatAtxgi vahisuva samAraMBa. 
\alnum{b} baTeTxbare; uDigetoDige. 
\eanum
\numie
\num{2} (\pArxvi) AcAPxdana; hodike; meVlemxYyanunx mucicxruva kUdalu, shalekx, \mo vu. 
\enum
\emng
\eentry

\bentry
\word{vestment} 
\pron{vesfTxmaMTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ adhikAra sUcaka) uDupu; vasatxrXvasana; poVSAku. 
\num{2} (\kanmu\ pAdirx \mo varu pArxthaRnA samayadalilx dharisuva) vidhuyxkatx uDupu (\kanmu\ ArAdhane yA parxBuBoVjana saMsAkxradalilx toDuva toVLilalxda dagale). 
\enum
\emng
\eentry

\bentry
\word{vest-pocket} 
\pron{vesfTxpAkiTf} 
\gl{\nA}
\expl{}
\bmng
 (naDu aMgiya) saNaNx jeVbau. 
\emng
\eentry

\bentry
\word{vestral} 
\pron{vesaTxrXlf} 
\gl{\gu}
\expl{}
\bmng
 pAYxriSf saBege seVrida, saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vestry} 
\pron{vesiTxrX} 
\gl{\nA}
\expl{(\bava\ \eng{vestries}). }
\bmng
\bnum
\num{1} (pAdirx \mo varu adhikAravasatxrXgaLanunx iDuva, dharisuva) caciRge taguliruva mane yA koThaDi; caciRna vasAtxrXgAra. 
\numi{2} (\ca) (\birx) 
\banum
\alnum{a} (pAYxriSfge saMbaMdhisida viSayagaLanunx caciRsalu pAYxriSf sadasayxru, \kanmu\ caciRna vasAtxrXgAradalilx kUDuva) pAYxriSf saBe. 
\alnum{b} I riVti seVruva pAYxriSf saMGa. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{vestry-clerk} 
\pron{vesiTxrXkAlxkfR} 
\gl{\nA}
\expl{}
\bmng
 pAYxriSf karaNika; pAYxriSfna lekakxgaLanunx bareyalu pAYxriSf adhikArigaLu neVmisutitxdadx gumAsetx. 
\emng
\eentry

\bentry
\word{vestryman} 
\pron{vesiTxrXmanf} 
\gl{\nA}
\expl{(\bava\ \eng{vestrymen}). }
\bmng
 pAYxriSf saBeya sadasayx. 
\emng
\eentry

\bentry
\word[vesture(1)]{vesture} 
\pron{vesacxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\kAparx) vasatxrXvasana; uDigetoDige; poVSAku; dirisu. 
\num{2} hodike; AcAPxdana. 
\enum
\emng
\eentry

\bentry
\word[vesture(2)]{vesture} 
\pron{vesacxrf} 
\gl{\sakirx}
\expl{}
\bmng
 uDigetoDigeyanunx uDisu, toDisu. 
\emng
\eentry

\bentry
\word[vet(1)]{vet} 
\pron{veTf} 
\gl{\nA}
\expl{}
\bmng
 (\AmA) veTarinari veYdayx; pArxNiveYdayx; pArxNi shasatxrXcikitasxka. 
\emng
\eentry

\bentry
\word[vet(2)]{vet} 
\pron{veTf} 
\gl{\sakirx}(\BU\ matutx \BUkaq\ \eng{vetted}; \vakaq\
\eng{vetting}). \bmng
\bnum
\numi{1} 
\banum
\alnum{a} pArxNigaLanunx pariVkiSxsi, AroVgayxvAgiveyeMdu parxmANiVkarisu. 
\alnum{b} pArxNigaLige cikitesxmADu. 
\eanum
\numie
\num{2} (yoVjane, kAyaR, aBayxthiR, \mo vanunx) ecacxrikeyiMda, kUlaMkaSavAgi -- pariVkiSxsu. 
\enum
\emng
\eentry

\bentry
\word[vet(3)]{vet} 
\pron{veTf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) (\AmA) = \hyperlink{veteran(1)}{$^1$veteran} (eMbaudara \saMkiSx). 
\emng
\eentry

\bentry
\word{vetch} 
\pron{vecf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ visiya sAYxTeYva kulada, hecAcxgi danagaLa meVvAgi baLasuva baTANi, avare, huruLi, \mo\ jAtiya) vecf giDa. 
\emng
\eentry

\bentry
\word{vetchling} 
\pron{vecfliMgf} 
\gl{\nA}
\expl{}
\bmng
 vecfliMgf giDa; `lAYxtirisf' kulada, vecf giDakekx saMbaMdhisida, halavAru giDagaLalolxMdu. 
\emng
\eentry

\bentry
\word[veteran(1)]{veteran} 
\pron{veTaranf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (yAvudeV vaqtitxyalilx yA kasabinalilx, \kanmu\ seYnika seVveyalilx) nuritavanu; anuBavi; paLagidavanu; pariNata; diVGARnuBava paDedavanu yA adaralelxV AyuSayxvanunx kaLedavanu: \eng{a veteran of two world wars} eraDu vishavxyudadhxgaLa anuBavi seYnika, yoVdha. \eng{a veteran golfer} pariNata gAlfphx ATagAra. 
\num{2} (\ame) mAji -- seYnika, BaTa, yoVdha; \kanmu\ mahAyudadhxgaLalilx pAlugoMDu nivaqtatxnAdavanu(Lu). 
\enum
\emng
\eentry

\bentry
\word[veteran(2)]{veteran} 
\pron{veTaranf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anuBaviya; nuritavana; paLagidavana. 
\num{2} anuBavigaLigAgi iruva yA saMGaTitavAgiruva. 
\enum
\emng
\eentry

\bentry
\wordnospeech{veteran car}{veteran car}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\birx) (\eng{1905} yA \eng{1906}kekx muMce tayArAda) haLe(mAdariya) kAru. 
\emng
\eentry

\bentry
\wordnospeech{Veterans' Day}{Veterans' Day}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\ame) mAji BaTara, yoVdhara -- dina, dinAcaraNe (akoTxVbarf tiMgaLa \eng{4}neya soVmavAra). 
\emng
\eentry

\bentry
\word{veterinarian} 
\pron{veTarineVrianf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) pashuveYdayx; pArxNishasatxrXcikitasxka. 
\emng
\eentry

\bentry
\word[veterinary(1)]{veterinary} 
\pron{veTarinari} 
\gl{\gu}
\expl{}
\bmng
 pashuveYdayxda; pashuroVgagaLa cikitesxya. 
\emng
\eentry

\bentry
\word[veterinary(2)]{veterinary} 
\pron{veTarinari} 
\gl{\nA}
\expl{(\bava\ \eng{veterinaries}). }
\bmng
 = \hyperlink{veterinary surgeon}{veterinary surgeon}. 
\emng
\eentry

\bentry
\wordnospeech{veterinary surgeon}{veterinary surgeon}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\birx) pashuveYdayx; roVgakekx yA hAnigoLagAda pArxNigaLige cikitesx niVDuvavanu. 
\emng
\eentry

\bentry
\word{vetiver} 
\pron{veTivarf} 
\gl{\nA}
\bmng
 = \hyperref{kandict_c.pdf}{C}{cuscus(1)}{cuscus(1)}. 
\emng
\eentry

\bentry
\word[veto(1)]{veto} 
\pron{viVToV} 
\gl{\nA}
\expl{(\bava\ \eng{vetoes}). }
\bmng
\bnum
\numi{1} viVToV; nirAkaraNAdhikAra; anaMgiVkArAdhikAra : 
\banum
\alnum{a} rAja yA rANi, rASATxrXdhayxkaSx, gavanaRrf yA rAjayxpAla, shAsanasaBegaLa peYki meVlina saBe \mo vakekx iruva, keLashAsana saBeyu mADida niNaRyavanunx yA shAsanavanunx taLiLxhAkuva adhikAra. 
\alnum{b} vishavxsaMsethxya BadarxtA maMDaliya shAshavxtasadasayx rASaTxrXkekx maMDaliya bahumaMdi sadasayxru aMgiVkarisida niNaRyavanUnx saha nirAkarisuva adhikAra. 
\alnum{c} aMtha nirAkaraNe. 
\alnum{d} A nirAkaraNeyanonxLagoMDa saMdeVsha yA sudidx. 
\eanum
\numie
\num{2} taDe; niroVdha; niSeVdha; yAvudeV viSayakekx aDiDxmADuvudu: \eng{he put a} (\engit{or} \eng{his) veto on the proposal} avanu A salaheyanunx niSeVdhisida. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{suspensive veto} sathxgana viVToV; niNaRyavanunx yA shAsanavanunx tAtAkxlikavAgi taDehiDiyuva, sathxgitagoLisuva viVToV. 
\emng
\eentry

\bentry
\word[veto(2)]{veto} 
\pron{viVToV} 
\gl{\sakirx}(vataRmAna \parxpu\ \Eva\ \eng{vetoes}; \BU\ matutx
\BUkaq\ \eng{vetoed}) \bmng
 viVToV mADu: 
\banum
\alnum{a} (niNaRyavanunx yA shAsanavanunx) nirAkarisu; taLiLxhAku. 
\alnum{b} (yAvudanenxV) adhikAravANiyiMda niroVdhisu, niSeVdhisu. 
\eanum
\emng
\eentry

\bentry
\word{vex} 
\pron{vekfsx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (cilalxrepalalxre kirukuLa, kATa, \mo vugaLiMda) reVgisu; kirikirigoLisu; keraLisu: \eng{this would vex a saint} idu obabx saMtananUnx kUDa keraLisiVtu. \eng{how vexing}! eSoTxMdu kAT! 
\num{2} (\pArxparx) mananoVyisu; kelxVshapaDisu; vayxtheyanunxMTumADu. 
\num{3} (\pArxparx) (samudarx \mo vanunx) koSxVBegoLisu; parxkuSxbadhxgoLisu; kadaDu. 
\enum
\emng
\eentry

\bentry
\word{vexation} 
\pron{vekesxVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} reVgisuvudu; keraLisuvudu; kirukuLa koDuvudu. 
\alnum{b} kATa; piVDe; upadarxva; kirukuLa. 
\eanum
\numie
\num{2} (\pArxparx) kelxVsha; vayxthe; kirikiri: \eng{subjected to many vexations} halavAru kelxVshagaLige guripaDisidaru. 
\enum
\emng
\eentry

\bentry
\word{vexatious} 
\pron{vekesxVSasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} reVgisuva; keraLisuva. 
\alnum{b} kADuva; kATakoDuva; kirukuLa koDuva. 
\eanum
\numie
\num{2} kelxVshakara; vayxtheyanunxMTumADuva: \eng{a vexatious problem} kelxVshakaravAda samaseyx. 
\num{3} (\nAyxshA) (sAkaSuTx nAyxyasamamxtavAda kAraNagaLilalxde) parxtivAdige sumamxne kATa koDuvudakAkxgiyeV hUDida. 
\enum
\emng
\eentry

\bentry
\word{vexatiously} 
\pron{vekesxVSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 kATa koDuva riVtiyalilx; kelxVsha koDuvaMte. 
\emng
\eentry

\bentry
\word{vexatiousness} 
\pron{vekesxVSasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} reVgisuvike; keraLisuvike; kirukuLa. 
\num{2}kelxVshakaravAgiruvudu. 
\enum
\emng
\eentry

\bentry
\word{vexed} 
\pron{vekfsxTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} reVgisiruva; keraLisiruva. 
\num{2} (samaseyx, vivAdAMsha, \mo vugaLa \vi) kilxSaTx; samasAyxtamxka; toDakAda; jaTila; kaSaTxkaravAda matutx vivAdagarxsatxvAda. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{a vexed question} apAra caceRge silukiruva kaThina parxshenx; vivAdagarxsatx samaseyx. 
\emng
\eentry

\bentry
\word{vexedly} 
\pron{vekisxDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} reVgisuvaMte . 
\num{2} kilxSaTxvAgi; samasAyxtamxkavAgi. 
\enum
\emng
\eentry

\bentry
\word{vexer} 
\pron{vekfsxrf} 
\gl{\nA}
\expl{}
\bmng
 reVgisuvavanu; keraLisuvavanu; kATa koDuvavanu. 
\emng
\eentry

\bentry
\word{vexillological} 
\pron{vekisxlalAjika(kf)lf} 
\gl{\gu}
\expl{}
\bmng
 dhavxjashAsatxrXda yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vexillologist} 
\pron{vekisxlAlajisfTx} 
\gl{\nA}
\expl{}
\bmng
 dhavxjashAsatxrXjacnx; bAvuTagaLa bagegx adhayxyana mADiruvavanu. 
\emng
\eentry

\bentry
\word{vexillology} 
\pron{vekisxlAlaji} 
\gl{\nA}
\expl{}
\bmng
 dhavxjashAsatxrX; bAvuTagaLanunx kurita adhayxyana. 
\emng
\eentry

\bentry
\word{vexillum} 
\pron{vekisxlamf} 
\gl{\nA}(\bava\ \eng{vexilla}
\expl{\ucAcx\ vekisxla). }
\bmng
\bnum
\numi{1} (\roVca) 
\banum
\alnum{a} (\kanmu\ seYnika tukaDiya) bAvuTa. 
\alnum{b} iMtha bAvuTada aDiyalilx hoVrADuva seYnika tukaDi. 
\eanum
\numie
\num{2} (\savi) leguyxmineVsiV vaMshada sasayxgaLa ciTeTxyaMtha hUvinalilx meVlina doDaDx daLa. 
\num{3} (\pArxvi) hakikxya pukakxda gariya alagu. 
\numi{4} 
\banum
\alnum{a} (cacuR) biSapfna daMDada meVlaNa bAvuTa. 
\alnum{b} biSapapxna meravaNigeyalilx oyuyxva dhavxja yA shilube. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{vexing} 
\pron{vekisxMgf} 
\gl{\gu}
\expl{}
\bmng
 reVgisuva; keraLisuva; kATakoDuva; kirukuLakAri; kelxVshakara. 
\emng
\eentry

\bentry
\word{vexingly} 
\pron{vekisxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
 reVgisuvaMte; keraLisuvaMte; kelxVshakaravAgi. 
\emng
\eentry

\bentry
\wordnospeech{VG}{VG}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{very good.} 
\num{2} \eng{Vicar-General.} 
\enum
\emng
\eentry

\bentry
\wordnospeech{VHF}{VHF}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{very high frequency.} 
\emng
\eentry

\bentry
\wordnospeech{VI}{VI}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Virgin Islands.} 
\emng
\eentry

\bentry
\word{via} 
\pron{veYa} 
\gl{\upa}
\expl{}
\bmng
 mUlaka; dAvxrA; muKeVna: \eng{from Mysore to Bangalore via Mandya} meYsUriniMda beMgaLUrige maMDayxda mUlaka. \eng{send it via your secretary} ninanx kAyaRdashiRya muKeVna adanunx kaLuhisu. 
\emng
\eentry

\bentry
\word{viability} 
\pron{veYabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} badukuva shakitx; jiVvashakitx; kelavoMdu parisithxtiyalilx jiVvisabalalx, baduki uLiyabalalx shakitx. 
\num{2} (rasetx, mAgaR, \mo vugaLa \vi) naDeyalu, saMcarisalu -- sAdhayxvAgiruvike; saMcArayoVgayxvAgiruvike. 
\num{3} (yoVjane \mo vugaLa \vi) kArayxsAdhayxte. 
\enum
\emng
\eentry

\bentry
\word{viable} 
\pron{veYaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (BUrxNada yA AgatAne huTiTxda kUsina \vi) badukabalalx; jiVvashakitxyuLaLx. 
\num{2} (sasayx, pArxNigaLa \vi) (yAvudeV nidiRSaTx vAyuguNa \mo vugaLalilx) badukabalalx; jiVvisabalalx. 
\num{3} (biVjada, biVjakaNada \vi) moLeyabalalx; aMkuravAgabalalx. 
\num{4} (yoVjane \mo vugaLa \vi, \kanmu\ AthiRkavAgi) sAdhayx; kAyaRsAdhayxvAda; kAyaRrUpakekx barabalalx. 
\enum
\emng
\eentry

\bentry
\word{viably} 
\pron{veYabilx} 
\gl{\kirxvi}
\expl{}
\bmng
 (yoVjane \mo vugaLa \vi, \kanmu\ AthiRkavAgi) kAyaRsAdhayxvAguvaMte; kAyaRrUpakekx baralu sAdhayxvAguvaMte. 
\emng
\eentry

\bentry
\word{viaduct} 
\pron{veYaDakfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (kaNive, haLaLx, \mo vanunx hAyalu, rasetxyanonxV reYlu mAgaRvanonxV hAkalu kaTiTxda) kamAnusAlina seVtuve. 
\num{2} aMtha rasetx yA reYlumAgaR. 
\enum
\emng
\eentry

\bentry
\word{vial} 
\pron{veYalf} 
\gl{\nA}
\expl{}
\bmng
 (darxva auSadhada, \sA\ koLaveyAkArada gAjina) siVse; bauDiDx; kupipx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{pour vials of wrath} (\AmA) koVpa kAru. 
\emng
\eentry

\bentry
\word{vialful} 
\pron{veYalfphuZlf} 
\gl{\nA}
\expl{(\bava\ \eng{vialfuls}). }
\bmng
 (koLaveyAkArada gAjina) siVsetuMba. 
\emng
\eentry

\bentry 
\wordf{via media} 
\pron{veYa miVDia, viVa meDia} 
\gl{\nA}
\expl{\Latin}
\bmng
 (sAhitayxka) madhayxmAgaR; eraDu atireVka mAgaRgaLanunx biTuTx naDuvaNa yA hALatavAda dAriyanunx anusarisuvudu. 
\emng
\eentry

\bentry
\word{viand} 
\pron{veYaMDf} 
\gl{\nA}
\expl{}
\bmng
 (\aupa) 
\bnum
\num{1} AhArapadAthaR. 
\num{2} (\sA\ \bava dalilx) (aDige mADida) BakaSxyXBoVjayxgaLu; tinisugaLu. 
\enum
\emng
\eentry

\bentry
\word{viaticum} 
\pron{veYAYxTikamf} 
\gl{\nA}(\bava\ \eng{viatica}
\expl{\ucAcx\ veYAYxTika). }
\bmng
\bnum
\num{1} (sakARriV kelasada meVle parxyANa mADuva adhikArige koDuva) AhAra sarabarAyi yA parxyANa Batayx. 
\num{2} (cacuR) (sAvina haMtadalilxruvavanige koDuva) parxBuBoVjana saMsAkxra. 
\enum
\emng
\eentry

\bentry
\word{vibes} 
\pron{veYbfs'} 
\gl{\nA}
\expl{}
\bmng
(\bava) (\AmA) = 
\bnum
\num{1} \eng{vibraphone.} 
\num{2} kaMpanagaLu (\kanmu\ uMTumADida saMveVdanegaLu yA vAtAvaraNa eMbathaRdalilx): \eng{the house had bad vibes} maneyalilx keTaTx vAtAvaraNa yA pariNAmagaLidadxvu. 
\enum
\emng
\eentry

\bentry
\word{vibracular} 
\pron{veYbArxYxkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
 cAvaTiyaMtha. 
\emng
\eentry

\bentry
\word{vibraculum} 
\pron{veYbArxYxkuyxlamf} 
\gl{\nA}(\bava\ \eng{vibracula}
\expl{\ucAcx}
veYbArxYxkuyxla).\bmng
 (\pArxvi) kaMpana kasha; sheYvala pArxNigaLu AhAravanunx bAyiya oLage eLedukoLaLxlu baLasuva, cAvaTiyaMtha racane. \imglink{vibraculum:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.6cm {V_Pictures/vibraculum.jpg}}} 
\emng
\eentry

\bentry
\word{vibrancy} 
\pron{veYbarxnisx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sapxMdana; kaMpana; adirATa. 
\num{2} (shabadxda \vi) anuraNana. 
\num{3} shakutxyXtAsxhada tuDita. 
\enum
\emng
\eentry

\bentry
\word{vibrant} 
\pron{veYbarxMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sapxMdaka; sapxMdisuva; kaMpanashiVla; kaMpisuva. 
\num{2} (kirxye, vayxkitx, vasutx, \mo vugaLa \vi) roVmAMcaka; meYnavireVLisuva: \eng{vibrant with emotion} BAvAtireVkadiMda roVmAMcagoMDa. 
\num{3} shakitx utAsxhagaLiMda tuDiyutitxruva. 
\num{4} (shabadxda \vi) anuraNisuva. (\dhavxni) saGoVSada; dhavxnitaMtugaLu miDiyutitxruva. 
\enum
\emng
\eentry

\bentry
\word{vibrantly} 
\pron{veYbarxMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kaMpisutatx yA kaMpisuvaMte. 
\num{2} anuraNisutatx yA anuraNisuvaMte. 
\num{3} shakutxyXtAsxhadiMda tuDiyutatx. 
\enum
\emng
\eentry

\bentry
\word{vibraphone} 
\pron{veYbarxphoVnf} 
\gl{\nA}
\expl{}
\bmng
 kaMpanavAdayx; tAmarxda paTiTxgaLU moVTArf cAlita anuraNakagaLU iruva, seZYloVphoVninaMtha, oMdu tALavAdayx. 
\emng
\eentry

\bentry
\word{vibraphonist} 
\pron{veYbarxphoVnisfTx} 
\gl{\nA}
\expl{}
\bmng
veYbarxphoVnfvAdaka. 
\emng
\eentry

\bentry
\word{vibrate} 
\pron{veYberxVTf} 
\gl{\sakirx}
\expl{}
\bmng
 (loVlakadaMte, atitxMditatx) tUgADisu; OlADisu; AMdoVlagoLisu. 
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (loVlakada \vi, atitxMditatx) tUgADu; OlADu; AMdoVlavAgu. 
\num{2} (dhavxniya \vi, kiviyalilx) (nenapina \vi\ \rUpa\ saha). kaMpanavAgu; anuraNisu; more. 
\num{3} (\Bwvi) tUgADu; AMdoVlisu; nilalxde, satatavAgi atitxMditatx (\kanmu\ veVgavAgi) calisu. 
\num{4} (BAvoVdevxVgadiMda) adiru; naDugu; kaMpisu. 
\enum
\emng
\eentry

\bentry
\word{vibratile} 
\pron{veYbarxTeYlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sapxMdana, kaMpana, AMdoVla -- samathaR. 
\numi{2} (\jiVvi) 
\banum
\alnum{a} (kiVTagaLa loVmAMga \mo vugaLa \vi) kaMpisuva; adiruva. 
\alnum{b} kaMpanakekx saMbaMdhisida. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{vibration} 
\pron{veYberxVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} tUgADuvudu; tUgATa; OlADuvudu; OlATa; AMdoVlana. 
\num{2} (\Bwvi) kaMpana; sapxMdana; (taMtu, paTiTx, saraLu, tarala, viduyxtAkxMta keSxVtarxgaLu, sithxtisAthxpaka vasutx, \mo vugaLa \vi) kiSxparxvAgi oMdu kaDeyiMda inonxMdu kaDege punaHpunaH calisuvudu. 
\numi{3} (\bava dalilx) sapxMdana; kaMpana: 
\banum
\alnum{a} mAnasika parxBAva; manasisxna meVle Aguva (\kanmu\ atiVMdirxya) parxBAva. 
\alnum{b} alwkika parxBAva yA anuBava; alilx hAjariruva janarigelalx taTuTxvudeMdu naMbalAgiruva, oMdu sathxLadalilxruva vilakaSxNa vAtAvaraNa yA anuBava. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{vibrational} 
\pron{veYberxVSanalf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vibrative}{vibrative}. 
\emng
\eentry

\bentry
\word{vibrative} 
\pron{veYbarxTivf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaMpanada; kaMpisuva; kaMpanashiVla; sapxMdanada. 
\numi{2} (dhavxniya \vi) 
\banum
\alnum{a} kaMpisuva; aduruva. 
\alnum{b} anuraNisuva; moreyuva. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{vibrato} 
\pron{vibArxToV} 
\gl{\nA}(\bava\ \eng{vibratos}). 
\bmng
 (\saM) (hADuvAgaloV gALivAdayx yA taMtiVvAdayxvanunx nuDisuvAgaloV savxrasAthxyi yanunx alapxsavxlapx badalAyisidarU uMTAguva) savxrakaMpana. 
\emng
\eentry

\bentry
\word{vibrator} 
\pron{veYberxVTarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kaMpaka; sapxMdaka; kaMpana, sapxMdana -- kAri; kaMpanavanunx, sapxMdanavanunx uMTumADuva (\kanmu\ viduyxtf yA itara) upakaraNa. 
\num{2} (\saM) (piyAnoV, hAmoRVniyaM, \mo) (nAligeyiruva) vAdayxgaLa nAlige, piVpi. 
\enum
\emng
\eentry

\bentry
\word{vibratory} 
\pron{veYbarx(berxV)Tari} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vibrative}{vibrative}. 
\emng
\eentry

\bentry
\word{vibrissae} 
\pron{veYbirxsiV} 
\gl{\nA}
\expl{(\bava)}
\bmng
\bnum
\num{1} birugUdalugaLu; gaDusu kUdalugaLu; bekikxna miVse, mUgina hoLeLxyalilxruva gaDusAda oraTu kUdalugaLu muMtAduvu. 
\num{2} bAyimuLuLxgaLu; kiVTagaLanunx tinunxva pakiSxgaLa bAyiya hatitxraviruva muLiLxnaMtha garigaLu. 
\enum
\emng
\eentry

\bentry
\word{viburnum} 
\pron{veY(vi)banaRmf} 
\gl{\nA}
\expl{}
\bmng
 (\savi) (\sA\ biLihU biDuva) veYbarfna podegiDa, \udA\ kAdaMbau, heVlusuMDe, \mo vu yA avugaLa kula. 
\emng
\eentry

\bentry
\wordnospeech{Vic.}{Vic.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Victoria.} 
\emng
\eentry

\bentry
\word{vicar} 
\pron{vikarf} 
\gl{\nA}
\expl{}
\bmng
 vikarf: 
\banum
\alnum{a} (cacfR Aphf iMgelxMDf) maThAdAyagaLu cAYxpaTxrf eMba dhAmiRka samitigoV, oMdu dhAmiRka saMsethxgoV, ilalxve obabx lwkikanigoV seVrida jilelxya pAdirx. 
\alnum{b} (beVrobabxna parxtinidhiyAgi vatiRsuva) episokxVpalf cacfRna pAdirx. 
\alnum{c} (\roVkAyx cacuR) biSapfna parxtinidhi yA niyoVgi. 
\alnum{d} = \hyperlink{vicar choral}{vicar choral}. 
\eanum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1}\eng{cardinal vicar} kADiRnalf vikarf; roVmfna DayosiVsfge biSapf Agi vatiRsuva, poVpfguruvina niyoVgi yA parxtinidhi. 
\num{2} \eng{lay vicar} = \hyperlink{vicar choral}{vicar choral}. 
\num{3} \eng{Vicar of Bray} (tatatxvXgaLanunx balikoTuTx anukUlakekx takakxMte badalAyisuva) samayasAdhaka; samayAnuvatiR. 
\num{4} \eng{Vicar of Christ} poVpf(guru); yeVsuvina parxtinidhi. 
\enum
\emng
\eentry

\bentry
\word{vicarage} 
\pron{vikarijf} 
\gl{\nA}
\expl{}
\bmng
 vikarfna mAnayx yA uMbaLi yA nivAsa. 
\emng
\eentry

\bentry
\wordnospeech{vicar apostolic}{vicar apostolic}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\roVkAyx) roVmanf kAyxtholikf dhamaRparxcAraka yA biSapf birudAMkita vayxkitx. 
\emng
\eentry

\bentry
\wordnospeech{vicar choral}{vicar choral}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 sotxVtarxpAThaka; (kethiVDarxlfna pArxthaRneyalilx, kelavu sotxVtarxBAgagaLanunx hADuvudakAkxgi neVmakagoMDa) pAdirx yA sotxVtarxgAyana taMDada sadasayx. 
\emng
\eentry

\bentry
\wordRemoveSpace{vicar-forane}{vicar forane} 
\pron{vikarf phoVreVnf} 
\gl{\nA}
\expl{}
\bmng
 vikarf phoVreVnf; sathxLiVya adhikAravanunx nivaRhisalu roVmanf kAyxtholikf biSapfniMda neVmakagoMDa adhikAri. 
\emng
\eentry

\bentry
\word{vicar-general} 
\pron{vikarfjenaralf} 
\gl{\nA}(\bava\ \eng{vicars-general}). 
\bmng
 vikarfjenaralf: 
\banum
\alnum{a} (\roVkAyx biSapfge caciRna vayxvahAragaLalilx neravAguva) nAyxyAdhikAri. 
\alnum{b} (biSapf yA AcfR biSapfge cacfR Aphf iMgelxMDige seVrida ADaLita vayxvahAragaLalilx neravAguva yA avananunx parxtinidhisuva) AYxMgilxkanf adhikAri. 
\eanum
\emng
\eentry

\bentry
\word{vicarial} 
\pron{vikeVrialf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vikarfna. 
\num{2} vikarf Agi vatiRsuva. 
\enum
\emng
\eentry

\bentry
\word{vicariate} 
\pron{vikeVriaTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vikarfna padavi, adhikAra yA pArxtinidhayx. 
\num{2} vikarfna adhikArakokxLapaTaTx parxdeVsha. 
\enum
\emng
\eentry

\bentry
\word{vicarious} 
\pron{vikeVriasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vahisikoTaTx; parxtinidhi savxrUpada; pArxtinidhayxda; pArxtinidhika: \eng{vicarious authority} vahisikoTaTx adhikAra. 
\num{2} parAthaRvAda; beVrobabxnigAgi yA beVrobabxna parxtinidhiyAgi mADida: \eng{vicarious work} beVrobabxnigAgi yA beVrobabxna parxtinidhiyAgi mADida kelasa. 
\num{3} beVrobabxna anuBavavanunx kalipxsikoMDu anuBavisuva: \eng{vicarious pleasure} beVrobabxna saMtoVSavanunx kalipxsikoMDu anuBavisida saMtoVSa. 
\enum
\emng
\eentry

\bentry
\word{vicariously} 
\pron{vikeVriasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} beVrobabxnigAgi; parAthaRvAgi; parxtinidhiyAgi (\kanmu\ yeVsu kirxsatxna \vi\ pApigAgi). 
\num{2} pArxtinidhikavAgi; beVrobabxna anuBavavanunx kalipxsikoMDu tAnU anuBavisuva riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{vicariousness} 
\pron{vikeVriasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} pArxtinidhayx; beVrobabxna parxtinidhiyAgiruvike. 
\num{2} parAthaRte; beVrobabxrigAgi mADiruvike. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vicarious sacrifice}{vicarious sacrifice}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 parAthaR balidAna; (\kanmu\ yeVsu kirxsatxna \vi) pApigAgi anuBavisida kUrxshAroVhaNa, maqtuyx, balidAna. 
\emng
\eentry

\bentry
\wordnospeech{vicarious suffering}{vicarious suffering}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vicarious sacrifice}{vicarious sacrifice}. 
\emng
\eentry

\bentry
\word{vicarship} 
\pron{vikarfSipf} 
\gl{\nA}
\expl{}
\bmng
 vikarfgiri; vikarfna -- hudedx, adhikAra yA adhikArAvadhi. 
\emng
\eentry

\bentry
\word[vice(1)]{vice} 
\pron{veYsf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} keTaTx -- guNa, savxBAva; aniVti; dubauRdidhx; avaguNa; durAcAra; duguRNa; dushishxVla: \eng{drunkenness is not among his vices} avana durAcAragaLalilx kuDitavu oMdalalx. 
\alnum{b} (veVshAyxvaqtitx, mAdakavasutxgaLu, \mo vanonxLagoMDa) pApa; pAtaka; hiVnakAyaR: \eng{vice is duly punished and virtue rewarded in the fifth act} aidaneya aMkadalilx pApakekx takakx shikeSxyAgi puNayxkekx salulxva parxtiPalavu doreyutatxde. 
\eanum
\numie
\numi{2} 
\banum
\alnum{a} niVtiBarxSaTxte; duSaTxtana; keVDu; duSakxqqtayx. 
\alnum{b} keTaTx cALi, caTa; duraBAyxsa: \eng{has the vice of gluttony} avanalilx hoTeTxbAkatanada duraBAyxsavide. 
\eanum
\numie
\num{3} (naDate yA naDavaLikeyalilx) doVSa; kaLaMka; nUyxnate. 
\num{4} (sAhitayxsheYli \mo vugaLa \vi) kuMdu; doVSa; nUyxnate. 
\num{5} (kudure \mo vugaLalilx) doVSa(gaLu) yA duraBAyxsa(gaLu). 
\num{6} (\ca) (niVtinATakada pAtarxgaLalilx) yAvudeV durAcAravanunx viDaMbana mADuva, pAtarx vahisuva koVDaMgi, vidUSaka pAtarx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{redeeming vice} suguNAtireVkavanunx savxlapxmaTiTxge tagigxsuva duguRNa, doVSa: \eng{he has no redeeming vice} avana suguNAtireVkavanunx savxlapxmaTiTxge tagigxsuva yAvudeV duguRNavU avanalilxlalx. 
\emng
\eentry

\bentry 
\word[vice(2)]{vice} 
\pron{veYsf} 
\gl{\nA}
\expl{}
\bmng
 tiruDi; bigi hiDike; veYsf; \kanmu\ baDagiya ilalxve loVhagArana beMcige bigidiruva, tirupanunx, \kanmu\ sUkxrXsahAyadiMda tirugisuvudariMda adara davaDegaLu saridu madheyx iTaTx vasutxvanunx bahaLa bigiyAgi hiDidiDuva salakaraNe. 
\emng
\eentry

\bentry
\word[vice(3)]{vice} 
\pron{veYsf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} tiruDiyalilx, bigihiDikeyalilx -- bigi, baMdhisu. 
\num{2} (\rUpa) kapimuSiTxyalilx hiDi. 
\enum
\emng
\eentry

\bentry
\word[vice(4)]{vice} 
\pron{veYsf} 
\gl{\nA}
\expl{}
\bmng
 (\AmA) = \hyperlink{vice-president}{vice-president}, \hyperlink{vice admiral}{vice admiral}, itAyxdi. 
\emng
\eentry

\bentry
\word[vice(5)]{vice} 
\pron{veYsi} 
\gl{\upa}
\expl{}
\bmng
 -ra sathxLadalilx; parxtiyAgi; badalAgi: \eng{he was gazetted as captain vice Captain Jones promoted} baDati paDeda kAYxpaTxnf joVnfsx avara badalige Atananunx kAYxpaTxnanAnxgi neVmisi geseZTiTxnalilx dAKalAmxDalAyitu. 
\emng
\eentry

\bentry
\word{vice-} 
\pron{veYsf\eng{-}} 
\gl{\sapUpa}
\expl{}
\bmng
\bnum
\num{1} badaliyAgi, parxtinidhiyAgi vatiRsuva; badaliyAgi vatiRsalu ahaRteyuLaLx. 
\num{2} -ge utatxrAdhikAriyAda, dajeRyalilx muMdina eMba athaRgaLalilx nAmapadagaLa hiMde seVruva \sapUpa, \udA\ \eng{vice-president, vice-admiral,} \mo vu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vice admiral}{vice admiral}
\pron{?}
\gl{\nA}
\expl{}
\bmng
veYsf AYxDimxralf; mUraneya dajeRya AYxDimxralf, nwkAseVnAdhipati; AYxDimxralfgiMta keLagina riyarfAYxDimxralfgiMta meVlina nwkAdhikAri. 
\emng
\eentry

\bentry
\word{vice-admiralty} 
\pron{veYsfAYxDimxraliTx} 
\gl{\nA}
\expl{(\bava\ \eng{vice-admiralties}). }
\bmng
 veYsfAYxDimxralfna kaceVri: \eng{vice-admiralty courts} birxTiSf nelasunADugaLalilx nwkAseVnAdhipatiya adhikAravAyxpitxgoLapaTaTx nAyxyamaMDaligaLu. 
\emng
\eentry

\bentry
\word{vice-chairman} 
\pron{veYsfceVmaRnf} 
\gl{\nA}
\expl{}
\bmng
 upAdhayxkaSx; upasaBApati; adhayxkaSxnige sahAyakanAgiruva yA Atana geYruhAjariyalilx parxtinidhi yA badaliyAgi vatiRsuva adhikAri. 
\emng
\eentry

\bentry
\word{vice-chairmanship} 
\pron{veYsfceVmaRnfSipf} 
\gl{\nA}
\expl{}
\bmng
 upAdhayxkaSxte; upasaBApatitavx; upAdhayxkaSxna, upasaBApatiya -- padavi, adhikAra. 
\emng
\eentry

\bentry
\word{vice-chamberlain} 
\pron{veYsfceVMbaliRnf} 
\gl{\nA}
\expl{}
\bmng
 (\birx) (\kanmu\ lADfR ceVMbarflinf avara sahAyakAdhikAriyAda) upaceVMbarflinf; upagaqhAdhikAri. 
\emng
\eentry

\bentry
\word{vice-chancellor} 
\pron{veYsfcAnasxlarf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ birxTanf vishavxvidAyxnilayada) upakulapati. 
\emng
\eentry

\bentry
\word{vice-chancellorship} 
\pron{veYsfcAnasxlarfSipf} 
\gl{\nA}
\expl{}
\bmng
 upakulapatitavx; vishavxvidAyxnilayada upakulapatiya padavi, adhikAra. 
\emng
\eentry

\bentry
\word{vice-commodore} 
\pron{veYsfkAmaDoVrf} 
\gl{\nA}
\expl{}
\bmng
 upa kAmoDoVrf; riyarf AYxDimxralfgiMta keLagina, kAYxpaTxnfgiMta meVlina adhikAri. 
\emng
\eentry

\bentry
\word{vice-consul} 
\pron{veYsfkAnasxlf} 
\gl{\nA}
\expl{}
\bmng
 upa kAnasxlf; (rAyaBAri kaceVriyalilx savxdeVshada vANijayxvanunx aBivaqdidhxpaDisalu neVmakagoMDiruva) upavANijAyxdhikAri. 
\emng
\eentry

\bentry
\word{vice-consulate} 
\pron{veYsfkAnfsuyxlaTf} 
\gl{\nA}
\expl{}
\bmng
 upavANijAyxdhikAriya kaceVri. 
\emng
\eentry

\bentry
\word{vice-consulship} 
\pron{veYsfkAnasxlfSipf} 
\gl{\nA}
\expl{}
\bmng
 veYsfkAnasxlf yA upavANijAyxdhikAriya padavi, adhikAra. 
\emng
\eentry

\bentry
\word{vicegerency} 
\pron{veYsfjeranisx} 
\gl{\nA}(\bava\ \eng{vicegerencies}). 
\bmng
 (\deVva, rAja) parxtinidhitavx; deYvadatatxvAda yA rAjadatatxvAda - padavi, adhikAra. 
\emng
\eentry

\bentry
\word[vicegerent(1)]{vicegerent} 
\pron{veYsfjeraMTf} 
\gl{\nA}
\expl{}
\bmng
 (\deVva, rAja) parxtinidhi; BagavaMtaniMda yA rAjaniMda, datatxvAda adhikAravanunx naDesuva adhikAri. 
\emng
\eentry

\bentry
\word[vicegerent(2)]{vicegerent} 
\pron{veYsfjeraMTf} 
\gl{\gu}
\expl{}
\bmng
 (\deVva, rAja) parxtinidhiya; parxtinidhiyAgi datAtxdhikAravanunx calAyisuva. 
\emng
\eentry

\bentry
\word{vice-governor} 
\pron{veYsfgavanaRrf} 
\gl{\nA}
\expl{}
\bmng
 upa gavanaRrf; gavanaRrana sahAyakAdhikAri. 
\emng
\eentry

\bentry
\word{viceless} 
\pron{veYsflisf} 
\gl{\gu}
\expl{}
\bmng
 durAcArarahita; duguRNavilalxda. 
\emng
\eentry

\bentry
\word{vicelike} 
\pron{veYsfleYkf} 
\gl{\gu}
\expl{}
\bmng
 tiruDiyaMtha; bigihiDikeyaMtha; veYsfnaMtha. 
\emng
\eentry

\bentry
\word{vicennial} 
\pron{veYsenialf} 
\gl{\gu}
\expl{}
\bmng
 viMshativaSaRda yA viMshativASiRka; ipapxtutx vaSaRkAla iruva yA ipapxtutx vaSaRkokxmemx naDeyuva. 
\emng
\eentry

\bentry
\word{vice-presidency} 
\pron{veYsfperxsiDanisx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vice-presidentship}{vice-presidentship}. 
\emng
\eentry

\bentry
\word{vice-president} 
\pron{veYsfperxsiZDa(Df)MTf} 
\gl{\nA}
\expl{}
\bmng
 upAdhayxkaSx; adhayxkaSxnigiMta keLagina matutx adhayxkaSxna parxtinidhiyAgi kelasa mADuva adhikAri. 
\emng
\eentry

\bentry
\word{vice-presidential} 
\pron{veYsfperxsiZDenfSa(Sf)lf} 
\gl{\gu}
\expl{}
\bmng
 upAdhayxkaSxna, Atanige saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vice-presidentship} 
\pron{veYsfperxsiZDaMTfSipf} 
\gl{\nA}
\expl{}
\bmng
 upAdhayxkaSxna padavi yA adhikAra. 
\emng
\eentry

\bentry
\word{vice-principal} 
\pron{veYsfpirxnisxpalf} 
\gl{\nA}
\expl{}
\bmng
 upa pirxnisxpAla; upapArxMshupAla; pirxnisxpAlana sahAyakAdhikAri. 
\emng
\eentry

\bentry
\word{viceregal} 
\pron{veYsfriVga(gf)lf} 
\gl{\gu}
\expl{}
\bmng
 veYsfrAyfna, avanige saMbaMdhisida. 
\emng
\eentry

\bentry
\word{viceregally} 
\pron{veYsfriVgali} 
\gl{\kirxvi}
\expl{}
\bmng
 veYsfrAyfnaMte, avana riVtiyalilx. 
\emng
\eentry

\bentry
\word{vice-regent} 
\pron{veYsfriVjaMTf} 
\gl{\nA}
\expl{}
\bmng
 upa rAjaparxtinidhi. 
\emng
\eentry

\bentry
\word{vicereine} 
\pron{veYsfreVnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} veYsfrAyfna heMDati. 
\num{2} sitxrXV veYsfrAyf; veYsfrAyiNi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vice ring}{vice ring}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 duSaTxkUTa; kAnUnubAhira veVshAyxvaqtitx naDesuva duSaTxra guMpu. 
\emng
\eentry

\bentry
\word{viceroy} 
\pron{veYsfrAyf} 
\gl{\nA}
\expl{}
\bmng
 (parxdeVsha, vasAhatu, \mo vugaLalilx rAjana yA rANiya paravAgi adhikAra naDesuva) veYsfrAyf. 
\emng
\eentry

\bentry
\word{viceroyal} 
\pron{veYsfrAyalf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{viceregal}{viceregal}. 
\emng
\eentry

\bentry
\word{viceroyalty} 
\pron{veYsfrAyaliTx} 
\gl{\nA}
\expl{}
\bmng
 veYsfrAyfna -- padavi, adhikAra. 
\emng
\eentry

\bentry
\word{viceroyship} 
\pron{veYsfrAyfSipf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{viceroyalty}{viceroyalty}. 
\emng
\eentry

\bentry
\word{vicesimal} 
\pron{veYsesiZma(mf)lf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vigesimal}{vigesimal}. 
\emng
\eentry

\bentry
\wordnospeech{vice squad}{vice squad}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (veVshAyxvaqtitx, mAdakavasutx, \mo vugaLa virudadhxvAgi shAsanakarxmavanunx jarugisuva poliVsf ilAKeya shAKeyAda) durAcAra niroVdha daLa. 
\emng
\eentry

\bentry
\wordf{vice versa} 
\pron{veYsi vasaR} 
\gl{\kirxvi}
\expl{\Latin}
\bmng
 hiMtirugi; parxtiyAgi; parxtikarxmadalilx; vipayaRyavAgi: \eng{the man blames his wife, and vice versa} A vayxkitx tananx heMDatiyanunx dUrutAtxne, avaLU hiMtirugi avananunx dUrutAtxLe. \eng{the cat stole the dog's dinner and vice versa} bekukx nAyiya AhAravanunx kadidxtu, nAyiyU adakekx parxtiyAgi bekikxna AhAravanunx kadidxtu. \eng{he calls white black and vice versa} avanu biLupanunx kapepxMdU, kapapxnunx biLupeMdU kareyutAtxne. 
\emng
\eentry

\bentry
\word{vichyssoise} 
\pron{viVSiVsAvxsfZ} 
\gl{\nA}
\expl{}
\bmng
 viVSiVsAvxsfZ; liVkfsxgeDeDxyanUnx AlUgeDeDxyanUnx hAki mADida sAMbArina, taMpisi baDisida tiLi. 
\emng
\eentry

\bentry
\wordRemoveSpace{Vichy-water}{Vichy water} 
\pron{viVSiZV vATarf} 
\gl{\nA}
\expl{}
\bmng
 (siVsege hAkiTiTxruva) phArxnisxna viVSiV parxdeVshada Kanija jala. 
\emng
\eentry

\bentry
\word{vicinage} 
\pron{visinijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nereya parxdeVsha; sutatxmutatxla parxdeVsha. 
\num{2} nereyavara saMbaMdha. 
\enum
\emng
\eentry

\bentry
\word{vicinal} 
\pron{visi(seY)nalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nereya; magugxlina. 
\num{2} nereparxdeVshada; sathxLiVya. 
\enum
\emng
\eentry

\bentry
\word{vicinity} 
\pron{visiniTi} 
\gl{\nA}
\expl{(\bava\ \eng{vicinities}). }
\bmng
\bnum
\num{1} nerehore; sutatxmutatxla parxdeVsha. 
\num{2} hatitxra; sAmiVpayx; nikaTate; saniyate. 
\num{3} hatitxrada, nikaTa -- saMbaMdha. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{in the vicinity (of)} ( -kekx) hatitxra; nikaTavAgi; samiVpadalilx. 
\emng
\eentry

\bentry 
\word{vicious} 
\pron{viSasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} duraBAyxsada; durAcArada; pApada yA aneYtika: \eng{vicious tendencies} durAcAra parxvaqtitxgaLu. \eng{vicious life} pApiSaThx jiVvana. \eng{vicious habits} duraBAyxsagaLu. 
\num{2} keTaTx; keDukina; duSaTx; durAcAra, duraBAyxsagaLige bidadx: \eng{vicious companions} keTaTx, duSaTx -- oDanADigaLu. 
\num{3} (kudure \mo vugaLa \vi) tuMTa; duraBAyxsagaLuLaLx; doVSagaLuLaLx. 
\num{4} (BASe, dasAtxveVju, takaR, \mo vugaLa \vi) ashudadhx; doVSayukatx. 
\num{5} koVpiSaThx; duSaTx; kutisxta; kubaudidhxya: \eng{vicious dog} duSaTxnAyi. \eng{vicious mood} koVpiSaThx manoVvaqtitx. \eng{vicious remarks} kutisxta TiVkegaLu. 
\num{6} ugarxvAda; tiVvarxvAda; tiVkaSx NxvAda: \eng{vicious attack} ugarxvAda dALi. \eng{vicious weather} tiVkaSx NxvAda have. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vicious circle}{vicious circle}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_c.pdf}{C}{circle(1)15}{$^1$circle(15, 16, 17)}. 
\emng
\eentry

\bentry
\word{viciously} 
\pron{viSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aneYtikavAgi; duSaTxriVtiyalilx. 
\num{2} doVSayukatxvAgi. 
\num{3} ugarxvAgi; tiVkaSx NxvAgi. 
\numi{4} (pArxNigaLa \vi) 
\banum
\alnum{a} tuMTatanadiMda. 
\alnum{b} koVpadiMda. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{viciousness} 
\pron{viSasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aneYtikate; durAcAra; duSaTxtana. 
\num{2} (pArxNigaLa \vi) tuMTatana. 
\num{3} (BASe, takaR, \mo vugaLa \vi) doVSayukatxte; asAdhutavx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vicious spiral}{vicious spiral}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 keTaTxsuruLi; keTaTx Erike; cakarxka vaqdi dhx; kAyaRkAraNagaLa parasapxra keTaTx pariNAma, parxBAva (\kanmu\ belegaLu matutx veVtanagaLu parasapxra parxtikirxyeyiMda eraDU ErutAtx hoVguvudu). 
\emng
\eentry

\bentry
\word{vicissitude} 
\pron{vi(veY)sisiTUyxDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (sithxtigatigaLalilx, \kanmu\ adaqSaTxdalilx) badalAvaNe; vayxtAyxsa; hecucxkaDime; vayxtayxya. 
\num{2} (\pArxparx\ yA \kAparx) karxmabadadhxvAda -- badalAvaNe, vipayaRya: eng{the vicissitude of the seasons} QutugaLa vayxtAyxsa, vipayaRya. \eng{the vicissitude of night and day} hagalurAtirxgaLa badalAvaNe. 
\enum
\emng
\eentry

\bentry
\word{vicissitudinous} 
\pron{vi(veY)sisiTUyxDinasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (sithxtigatigaLalilx yA adaqSaTxdalilx padeVpadeV) badalAvaNegaLAguva; vaqdidhxhArxsagaLuLaLx; EriLitagaLAguva. 
\num{2} (vayxkitxya \vi) padeVpadeV EriLitagaLa hoDetakekx sikikxda. 
\enum
\emng
\eentry

\bentry
\word{victim} 
\pron{vikiTxmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} bali; yajacnxdalilx deVvatege AhutiyAgi apiRsida, sajiVva pashu,pArxNi. 
\num{2} (\rUpa) bali; yAvudeV udedxVsha, devxVSa, \mo vugaLa sAdhaneyalilx hAnigoLagAda vayxkitx yA vasutx: \eng{the victim of relentless ambition} kaThoVravAda mahatAvxkAMkeSxge baliyAdavanu. \eng{he fell a victim to his own avarice} avanu tananx durAshege tAneV tutAtxda. 
\num{3} GaTane, apaGAta yA parisithxtiya kAraNadiMdAgi satatx yA hAnigoLagAda vayxkitx: \eng{the victims of disease, (of a railway accident, of an earthquake, etc.)} roVgakekx (reYlevx apaGAta, BUkaMpa, \mo vugaLige) tutAtxdavaru, baliyAdavaru. 
\num{4} (moVsakekx) tutAtxdavanu; baliyAdavanu: \eng{the numerous victims of the confidence trick} naMbikedorxVhakekx guriyAda asaMKAyxta vayxkitxgaLu. 
\enum
\emng
\eentry

\bentry
\word{victimise} 
\pron{vikiTxmeYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{victimize}{victimize}. 
\emng
\eentry

\bentry
\word{victimization} 
\pron{vikiTxmeYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 (hiMse, anAyxya, \mo vakekx) baligoDuvike. 
\emng
\eentry

\bentry
\word{victimize} 
\pron{vikiTxmeYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vayxkitxyanunx) baliyanAnxgi mADu. 
\num{2} (vayxkitxyanunx) vaMcisu; (vayxkitxge) moVsamADu; dagAhAku. 
\num{3} (saMpugAra \mo varanunx vajAmADuvudeV \mo\ shikeSxgaLiMda) hiMsege, upadarxvakekx, anAyxyakekx -- guripaDisu, baligoDu, IDumADu. 
\enum
\emng
\eentry

\bentry
\word{victimizer} 
\pron{vikiTxmeYsaZrf} 
\gl{\nA}
\expl{}
\bmng
 (vayxkitxyanunx) balipashuvanAnxgi mADuvavanu; hiMsaka. 
\emng
\eentry

\bentry
\word{victor} 
\pron{vikaTxrf} 
\gl{\nA}
\expl{}
\bmng
 (kadanadalilx yA sapxdheRyalilx) vijayi; jayashAli; gedadxvanu. 
\emng
\eentry

\bentry
\word{victoria} 
\pron{vikoTxVria} 
\gl{\nA}
\expl{}
\bmng
 vikoTxVriya: 
\banum
\alnum{a} nAlukx cakarxgaLuLaLx, ibabxru kUruva, cAlakanige etatxravAda piVThavuLaLx, maDicu CAvaNiya, tagUgx haguravU Ada, kudure baMDi. \imglink{victoria a.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.5cm {V_Pictures/victoria.jpg}}} 
\alnum{b} vikoTxVriya amesAZnika kulada, dakiSxNa amerikada, BArigAtarxda neYdile hUvu. 
\alnum{c} taleya meVle shiKeyuLaLx bagebageya pArivALa. 
\alnum{d} sAku pArivALa; mane pArivALa; maneyalilx sAkuva oMdu vidhada pArivALa. 
\hypertarget{victoria(e)}{} 
\alnum{e} BArigAtarxda, keMpubaNaNxda, rasaBarita palxmfhaNuNx. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{Victoria Cross}{Victoria Cross}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 vikoTxVriya kArxsf; \eng{1856}ralilx vikoTxVriya rANi sAthxpisida, parAkarxma parxdashaRnakAkxgi niVDuva padaka. 
\emng
\eentry

\bentry
\word[Victorian(1)]{Victorian} 
\pron{vikoTxVrianf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vayxkitxya, \kanmu\ garxMthakataRna \vi) (birxTaninxna) \eng{19}neV shatamAnada vikoTxVriya rANiya kAlada yA A kAlada lakaSxNavuLaLx. 
\num{2} vikoTxVriya kAlada manoVBAvagaLige (\kanmu\ yukAtxyukatx parijAcnxna matutx neYtika niSuThxratege) saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word[Victorian(2)]{Victorian} 
\pron{vikoTxVrianf} 
\gl{\nA}
\expl{}
\bmng
 vikoTxVriya kAlada vayxkitx (\kanmu\ garxMthakataR). 
\emng
\eentry

\bentry
\word{Victoriana} 
\pron{vikoTxVriAna} 
\gl{\nA}
\expl{(\bava)}
\bmng
\bnum
\num{1} rANi vikoTxVriya kAlada sAmAnugaLu, vasutxgaLu. 
\num{2} vikoTxVriya kAlada vishiSaTx manoVBAvagaLu. 
\enum
\emng
\eentry

\bentry
\word{Victorianism} 
\pron{vikoTxVrianisfZ(saZ)mf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ aBiruci, naDate, yoVcanAriVti, \mo vugaLalilx) vikoTxVriya kAlada guNa yA sithxti. 
\emng
\eentry

\bentry
\wordnospeech{victoria plum}{victoria plum}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{victoria(e)}{victoria(e)}. 
\emng
\eentry

\bentry
\wordnospeech{Victoria sandwich}{Victoria sandwich}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 vikoTxVriya sAYxMDfvicf; sapxMjinaMtha eraDu padaragaLa madheyx jAYxmanunx seVrisi mADida sapxMjf keVkf yA sihiroTiTx. 
\emng
\eentry

\bentry
\wordnospeech{Victoria sponge}{Victoria sponge}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{Victoria sandwich}{Victoria sandwich}. 
\emng
\eentry

\bentry
\word{victorious} 
\pron{vikoTxVriasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} geluvina; jayashAliyAda; vijayiyAda. 
\num{2} vijayada; vijayoVtasxvada: \eng{victorious day} vijayoVtasxvada dina. 
\enum
\emng
\eentry

\bentry
\word{victoriously} 
\pron{vikoTxVriasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 gedudx; vijayiyAgi; jayashAliyAgi. 
\emng
\eentry

\bentry
\word{victoriousness} 
\pron{vikoTxVriasfnisf} 
\gl{\nA}
\expl{}
\bmng
 jayashAlitana; vijayitavx. 
\emng
\eentry

\bentry
\wordRemoveSpace{victor-ludorum}{victor ludorum} 
\pron{vikaTxrf lUDAramf} 
\gl{\nA}
\expl{}
\bmng
 (shAle \mo vugaLa kirxVDegaLalilx oTATxre atayxMta hecicxna sAdhanegaLanunx mADida) parama vijayi. 
\emng
\eentry

\bentry
\word{victory} 
\pron{vikaTxri} 
\gl{\nA}
\expl{(\bava\ \eng{victories}). }
\bmng
 (yudadhxdalilx, kALagadalilx yA sapxdheRyalilx edurALiyanunx soVlisi paDeda) gelavu; jaya; vijaya (\rUpa\ saha): \eng{the battle ended in a victory for him} A kadanavu avana vijayadalilx konegoMDitu. \eng{he fought hard for victory} avanu jayakAkxgi tiVvarxvAgi hoVrADida. \eng{he is a hero of many victories} avanu aneVka vijaya gaLanunx paDeda viVra. \eng{he gained a victory over his passions} avanu tananx rAgadevxVSagaLa meVle vijayavanunx sAdhisida. 
\emng
\eentry

\bentry
\word[victual(1)]{victual} 
\pron{viT(Tf)lf} 
\gl{\nA}
\expl{}
\bmng
 (\birx) (\aupa) (\sA\ \bava dalilx) (\kanmu\ baLakegAgi sidadhxpaDisida) AhAra yA AhAra sAmagirx. 
\emng
\eentry

\bentry
\word[victual(2)]{victual} 
\pron{viTalf} 
\gl{\kirx}
\expl{[\BU\ matutx \BUkaq\ \eng{victualled} }
(\ame\ \eng{victualed})\eng{;} \vakaq\ \eng{victualling} (\ame\\eng{victualing})]. 

\noindent
\gl{\sakirx}
\bmng
AhAravanunx yA AhArasAmagirxgaLanunx -- odagisu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} AhAra tinunx. 
\num{2}AhAra sAmagirx paDediru. 
\enum
\emng
\eentry

\bentry
\word{victualer} 
\pron{viTalxrf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) = \hyperlink{victualler}{victualler}. 
\emng
\eentry

\bentry
\word{victualler} 
\pron{viTalxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AhAra(sAmagirx) -odagisuvavanu, sarabarAju mADuvavanu; AhAra sarabarAjugAra. 
\num{2} (itara haDagugaLige AhArasAmagirxgaLanunx koMDoyayxlu neVmakavAda) AhAra sarabarAju haDagu. 
\hypertarget{victualler(3)}{} 
\num{3} (\birx) (madayxgaLu \mo vanunx sarabarAju mADalu) paravAnagi paDediruva AhAra sarabarAjugAra. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{licensed victualler} = \hyperlink{victualler(3)}{victualler\((3)\)}. 
\emng
\eentry

\bentry
\word{victualless}
\pron{viTalflisf} 
\gl{\gu}
\bmng
\bnum
\num{1} AhAravilalxda; AhArarahita. 
\num{2} AhArasAmagirxgaLilalxda; AhAra sAmagirxrahita. 
\enum
\emng
\eentry

\bentry
\word{vicuna} 
\pron{vikUyxna} 
\gl{\nA}
\bmng
\bnum
\num{1} vikUyxna; nayavAda reVSemxyaMtha uNeNxyuLaLx lAma eMba \da\ amerikada oMTeyaMtha pArxNi. \imglink{vicuna 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.6cm {V_Pictures/vicuna-1.jpg}}} 
\num{2} vikUyxna baTeTx; vikUyxnada tupapxTadiMda mADida yA aMtha kaqtaka tupapxTadiMda mADida baTeTx. 
\enum
\emng
\eentry

\bentry 
\wordf{vide} 
\pron{vi(V)DeV, veYDi} 
\gl{\sakirx}
\expl{\Latin }
\bmng
 (vidhayxthaRka) (pusatxka \mo vugaLa BAgada \vi) noVDu; avaloVkisu. 
\emng
\eentry

\bentry
\word{videlicet} 
\pron{viDeliseTf} 
\gl{\kirxvi}
\expl{}
\bmng
 = \hyperlink{viz}{viz}. 
\emng
\eentry

\bentry
\word[video(1)]{video} 
\pron{viDiO} 
\gl{\nA}(\bava\ \eng{videos}). 
\bmng
 viDiyoV; daqshayx : 
\banum
\alnum{a} kAMtiVya paTiTxya meVle kaNiNxge kANisuva biMbagaLanunx dAKalisuva, punarutapxtitx mADuva yA parxsAra mADuva parxkirxye. 
\alnum{b} dUradashaRna parxsArada daqshayxBAga. 
\alnum{c} viDiyo paTiTxya meVle dAKalisiruva calanacitarx \mo vu. 
\alnum{d} (\AmA) = \hyperlink{video cassette recorder}{video cassette recorder}. 
\eanum
\emng
\eentry

\bentry
\word[video(2)]{video} 
\pron{viDiO} 
\gl{\gu}
\expl{}
\bmng
viDiyoV; daqshayx: 
\banum
\alnum{a} kAMtiVya paTiTxya meVle kaNiNxge kANisuva biMbagaLanunx dAKalisuvudakekx yA punarutapxtitx mADuvudakekx yA parxsAra mADuvudakekx saMbaMdhisida. 
\alnum{b} dUradashaRna calanacitarxgaLanunx parxsAra mADuvudakekx saMbaMdhisida. 
\eanum
\emng
\eentry

\bentry
\word[video(3)]{video} 
\pron{viDiO} 
\gl{\sakirx}(vataRmAna \parxpu\ \Eva\ \eng{videoes}; \BU\ matutx
\BUkaq\ \eng{videoed}). \bmng
 viDiyoVkarisu; daqshiyxVkarisu. 
\emng
\eentry

\bentry
\wordnospeech{video cassette}{video cassette}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 viDiyo kAyxseTuTx; daqshayx citarxvanunx dAKalisiruva kAyxseTuTx. 
\emng
\eentry

\bentry
\wordnospeech{video cassette recorder}{video cassette recorder}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 viDiyoV kAyxseTf rekADaRru; viDiyoV paTiTxgaLa meVle biMbagaLanunx dAKalisuva matutx avanunx toVrisuva yA cAlanegoLisuva upakaraNa. 
\emng
\eentry

\bentry
\word{videodisc} 
\pron{viDiODisfkx} 
\gl{\nA}
\expl{}
\bmng
 daqshayxbilelx; viDiyoV Disukx; dUradashaRna tereya meVle mUDisabahudAda mAhitiyanunx dAKalisalu baLasuva loVhaleVpita bilelx. 
\emng
\eentry

\bentry
\wordnospeech{video frequency}{video frequency}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 daqshayxAvataRna; dUradashaRnadalilx viDiyo saMjecnxgaLigAgi baLasuva AvataRna vAyxpitx. 
\emng
\eentry

\bentry
\wordnospeech{video game}{video game}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 daqshayx ATa; dUradashaRna tereya meVle kaMpUyxTarf porxVgArxyXmf neraviniMda mUDisida biMbagaLanunx ilekATxrXnikf vidhAnadiMda nivaRhisi ADuva ATa. 
\emng
\eentry

\bentry
\wordnospeech{video nasty}{video nasty} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\AmA) asahayx daqshayx; BayAnakavAgiruva yA ashilxVlavAgiruva viDiyo calanacitarx. 
\emng
\eentry

\bentry
\word{videophone} 
\pron{viDiOphoVnf} 
\gl{\nA}
\expl{}
\bmng
 daqshayxvANi; daqshayxdUravANi; dhavxniyoMdige biMbavanUnx oyuyxva dUravANi. 
\emng
\eentry

\bentry
\wordnospeech{video recorder}{video recorder}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{video cassette recorder}{video cassette recorder}. 
\emng
\eentry

\bentry
\wordnospeech{video screen}{video screen}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 daqshayx tere; viDiyo citarxgaLanunx mUDisalu baLasuva tere. 
\emng
\eentry

\bentry
\wordnospeech{video signal}{video signal} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 viDiyo saMjecnx; dUradashaRna biMbagaLanunx utApxdisalu beVkAda mAhitiyanonxLagoMDa saMjecnx. 
\emng
\eentry

\bentry
 \word[videotape(1)]{videotape} \pron{viDiOTeVpf} 
\gl{\nA}
\expl{}
\bmng
 viDiyo, daqshayx -- paTiTx, TeVpu; dUradashaRna citarxgaLanUnx dhavxniyanUnx dAKalisalu baLasuva kAMtiVya paTiTx. 
\emng
\eentry

\bentry 
\word[videotape(2)]{videotape} 
\pron{viDiOTeVpf} 
\gl{\sakirx}
\expl{}
\bmng
 viDiyo TeVpf mADu; (dUradashaRna citarxgaLanunx yA dhavxniyanunx yA parxcAra sAmagirxyanunx) viDiyo paTiTxya meVle dAKalisu. 
\emng
\eentry

\bentry
\wordnospeech{videotape recorder}{videotape recorder}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{video recorder}{video recorder}. 
\emng
\eentry

\bentry
\word{videotex} 
\pron{viDiOTekfsx} 
\gl{\nA}
\expl{}
\bmng
 viDiyoTekfsx; yAvudeV ilekATxrXnikf mAhiti vayxvasethx (\kanmu\ \eng{teletext} yA \eng{viewdata}). 
\emng
\eentry

\bentry
\word{videotext} 
\pron{viDiOTekfsxTf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{videotex}{videotex}. 
\emng
\eentry

\bentry
\word{vidimus} 
\pron{veYDimasf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} lekakx \mo vugaLa -- taniKe, parishiVlana. 
\num{2} (lekakx \mo vanunx parishiVlisalAgideyeMdu parxmANiVkarisida) lekakxtaniKeya parxmANa patarx. 
\enum
\emng
\eentry

\bentry
\word{vie} 
\pron{veY} 
\gl{\akirx}
\expl{(\vakaq\ \eng{vying}). }
\bmng
 (meVlugeYyAgalu) sapxdhiRsu; peYpoVTi mADu; huruDisu; seNasu: \eng{vied with each other for recognition} mAnayxtegAgi parasapxra seNasidaru. 
\emng
\eentry

\bentry
\word{vielle} 
\pron{vielf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_h.pdf}{H}{hurdy-gurdy}{hurdy-gurdy}. 
\emng
\eentry

\bentry
\wordRemoveSpace{Vienna-schnitzel}{Vienna schnitzel} 
\pron{viyena SinxTfsaZlf} 
\gl{\nA}
\expl{}
\bmng
 berxDf tuMDugaLanunx hAki, bADisida, karuvina kAlina, mULeyilalxda mAMsada tuMDu. 
\emng
\eentry

\bentry
\wordnospeech{Vienna steak}{Vienna steak}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 hurida mAMsada cUru. 
\emng
\eentry

\bentry
\word[Viennese(1)]{Viennese} 
\pron{vianiVsfZ} 
\gl{\gu}
\expl{}
\bmng
 viyananxda, adakekx -- saMbaMdhisida, seVrida. 
\emng
\eentry

\bentry
\word[Viennese(2)]{Viennese} 
\pron{vianiVsfZ} 
\gl{\nA}
\expl{(\bava\ adeV). }
\bmng
 viyananxda vayxkitx; AsiTxrXya deVshada viyananx nagarada nivAsi yA parxje. 
\emng
\eentry

\bentry
\wordf{vi et armis} 
\pron{veY eTf AmiRsf} 
\gl{\kirxvi}
\expl{\Latin }
\bmng
 (\nAyxshA) sashasatxrX balAtAkxradiMda; (shasatxrX) balaparxyoVga diMda. 
\emng
\eentry

\bentry
\word[Vietnamese(1)]{Vietnamese} 
\pron{vieTanxmiVsfZ} 
\gl{\gu}
\expl{}
\bmng
 (ESAyxKaMDada AgenxVya BAgadalilxruva) viyaTAnxmf deVshada. 
\emng
\eentry

\bentry
\word[Vietnamese(2)]{Vietnamese} 
\pron{vieTanxmiVsfZ} 
\gl{\nA}
\expl{}
\bmng
(\bava\ adeV). viyaTAnxmf deVshada vayxkitx yA parxje yA BASe. 
\emng
\eentry

\bentry
\wordf{vieux jeu} 
\pron{viarf SaZrf} 
\gl{\gu}
\expl{\F}
\bmng
 ObiVrAyana kAlada; haLeya kaMdAcArada; haLasalAda; caviRtacavaRNavAda. 
\emng
\eentry

\bentry
\word[view(1)]{view} 
\pron{vUyx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} noVTa; viVkaSxNa; dashaRna; avaloVkana. 
\num{2} (\nAyxshA) (nAyxyadashiR maMDaliyu mokadadxmege saMbaMdhisida sathxLa, Asitx, \mo vanunx yA maqtadeVhavanunx) noVDuvudu; parishiVlane; pariVkaSxNa. 
\num{3} daqSiTx; noVTa; noVTada, kaNiNxna -- vAyxpitx: \eng{in full view of the crowd} guMpina kaNeNxdurinalelxV. \eng{the bird came into view} A pakiSx noVTada vAyxpitxge baMtu. \eng{the ship passed from our view} A haDagu namamx daqSiTxyiMda sariyitu, kaNamxreyAyitu. 
\num{4} noVTa; daqshayx; nidiRSaTx sAthxnadiMda noVDalapxTaTxdudx: \eng{the house has a view of the sea} A maneyiMda samudarxda noVTa sigutatxde, samudarx kANutatxde. \eng{a superb view} aduBxtavAda oMdu noVTa, Bavayxdaqshayx. 
\num{5} daqshayxcitarx; yAvudeV vasutx \mo vugaLa noVTavanunx oLagoMDiruva citarx. 
\num{6} daqSiTx; aBipArxya; BAvane; yAvudeV viSayavanunx -- noVDuva bage, kurita manoVBAva: \eng{he takes a different view} avanu beVre aBipArxyavanunx taLedidAdxne. \eng{his view is that we are aggressors} nAvu AkarxmaNakArareMbaudu avana BAvane. \eng{that is his view of her conduct} avaLa naDavaLikeya viSayadalilx avana daqSiTx adu. \eng{he holds extreme views (in politics etc. )} (rAjakiVya \mo vugaLalilx) avanu tiVvarxgAmi aBipArxyagaLanunx hoMdidAdxne. \eng{took a favourable view of the matter} viSayada bagegx anukUlakara aBipArxya taLeda. 
\num{7} udedxVsha; Ashaya; eNike: \eng{will this meet your views?} idu nimamx udedxVshagaLige sarihoMdutatxdeye? 
\num{8} viVkaSxNa; avaloVkana; pariVkaSxNa; samiVkaSxNa; cAkuSxSa yA mAnasika samiVkeSx; kaNiNxniMda yA mAnasikavAgi pariVkiSxsuvudu, taniKe mADuvudu. 
\num{9} viVkaSxNa; viVkaSxNAvakAsha: \eng{a view of the exhibition} parxdashaRnada viVkaSxNe, noVTa. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\numi{1} \eng{have in view} 
\banum
\alnum{a} (yAvudanenxV) guriyanAnxgi, udedxVshavanAnxgi hoMdiru. 
\alnum{b} (tiVmARnisuvAga, yAvudeV aMshavanunx) gamanadalilx, daqSiTxyalilx -- iTuTxkoMDiru. 
\eanum
\numie
\numi{2} \eng{in view of} 
\banum
\alnum{a} daqSiTxpathadalilx yA pathakekx; kaNiNxge biVLuva sAthxnadalilx: \eng{we came in view of the church} A cacuR kaNiNxge biVLuva sAthxnakekx baMdevu. \eng{he stood in view of the audience} avanu perxVkaSxkara daqSiTxpathadalilx niMta. 
\alnum{b} daqSiTxyiMda; noVDidare; noVDidalilx; gamanisidare; parigaNisidare; kAraNadiMda: \eng{in view of the recent developments, we do not think this step advisable} ititxVcina GaTanegaLanunx noVDidare, parigaNisidare, I hejejx (kAyaRkarxma) vihitavAdadedxMdu namage toVruvudilalx. 
\alnum{c} (\pArxparx) (muMde Aguvudanunx niriVkiSxsutAtx adakekx) pUvaRBAviyAgi. 
\eanum
\numie
\num{3} \eng{on view} noVDuvudakAkxgi, pariVkeSxgAgi, parishiVlanegAgi, avaloVkanakAkxgi -- parxdashiRsida. 
\num{4} \eng{private view} (citarx parxdashaRnada \vi) (senxVhitaru, vimashaRkaru, \mo) ApatxvagaRkekx mAtarx parxdashaRna; KAsagi parxdashaRna. 
\num{5} \eng{take a} \hyperref{kandict_d.pdf}{D}{dim(1) nuga}{$^1$dim} \eng{view of.} 
\num{6} \eng{take a} \hyperref{kandict_p.pdf}{P}{poor pagu(3)}{poor} \eng{view of.} 
\num{7} \eng{take the long-term view of} (yAvudeV \vi) dUrAloVcane mADu; BaviSayxvanunx ciMtisu. 
\num{8} \eng{to the view} bahiraMgavAgi; parxtayxkaSxvAgi; parxkaTavAgi; elalxrigU kANuvaMte. 
\numi{9} \eng{with a view to} 
\banum
\alnum{a} udedxVshadiMda yA niriVkeSxyiMda; guriyatatx oMdu hejejxyAgi. 
\alnum{b} (yAvudanenxV) paDeyuvudakAkxgi; (meVle) kaNiNxTuTx; paDeyuvudakokxVsakxra; paDeyuva saluvAgi: \eng{with a view to marriage} maduveya saluvAgi, daqSiTxyiMda. 
\eanum
\numie
\enum
\emng
\eentry

\bentry 
\word[view(2)]{view} 
\pron{vUyx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} kaNiNxMda noVDu; viVkiSxsu; dashiRsu; kANu. 
\num{2} kANu; noVDu; parigaNisu; aBipArxya taLe: \eng{the subject may be viewed in different ways} I viSayavanunx beVrebeVre riVtiyalilx kANbahudu, beVrebeVre daqSiTxyiMda parigaNisabahudu; I viSayadalilx beVrebeVre aBipArxyagaLanunx taLeyabahudu. \eng{he does not view the matter in the same light} avanu I viSayavanunx adeV daqSiTxyiMda noVDuvudilalx; I viSayadalilx avana aBipArxya beVre. 
\num{3} (mane \mo vanunx) noVDu; kaNiNxniMda pariVkiSxsu; noVDikoMDu baru: \eng{we are going to view the house} nAvu maneyanunx noVDalu, noVDikoMDu baralu hoVgutitxdedxVve. 
\num{4} (mAnasikavAgi) noVDu; avaloVkisu; parishiVlisu; samiVkeSx mADu: \eng{different ways of viewing a subject} viSayavoMdanunx parishiVlisuva viBinanx vidhAnagaLu, mAgaRgaLu. 
\num{5} (nariyu) marasanunx biTuTx horaTadadxnunx kANu. 
\enum
\emng

\noindent 	 
\gl{\akirx}
\expl{}
\bmng
 dUradashaRnavanunx noVDu. 
\emng
\eentry

\bentry
\word{viewable} 
\pron{vUyxabflf} 
\gl{\gu}
\expl{}
\bmng
 daqshayx; noVDalAguva; noVDalu sAdhayxvAda; dashaRnasAdhayx (rUpakavAgi mAnasikavAgi saha). 
\emng
\eentry

\bentry
\word{viewdata} 
\pron{vUyxDeVT} 
\gl{\nA}
\expl{}
\bmng
 daqshayxmAhiti; dUravANi koMDiyiMda dUradashaRnada terege seVrisida kaMpUyxTarf AkaradiMda siguva sudidx matutx mAhiti vayxvasethx. 
\emng
\eentry

\bentry
\word{viewer} 
\pron{vUyxarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} noVDuvavanu; viVkaSxka. 
\num{2} dUradashaRnavanunx noVDuvavanu; dUradashaRna viVkaSxka. 
\num{3} dashaRka; philaM meVle mUDisiruva citarx \mo vanunx noVDalu baLasuva sAdhana. 
\enum
\emng
\eentry

\bentry
\word{viewfinder} 
\pron{vUyxpheYMDarf} 
\gl{\nA}
\expl{}
\bmng
 (\CA) daqshayxsAdhaka; noVTasAdhaka; daqSiTxvAyxpitxdashaRka; kAyxmerAdiMda phoVTo tegeyuvAga masUravu oLagoLuLxva citarxda keSxVtarxvAyxpitxyanunx toVrisuva sAdhana. 
\emng
\eentry

\bentry
\wordnospeech{view halloo}{view halloo}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (beVTe) hoV! kaMDitu, kaMDitu! (marasiniMda nariyu horahoraTAga beVTegAranu kUguva saMtoVSa utAsxhagaLa kUgu). 
\emng
\eentry

\bentry
\word{viewing} 
\pron{vUyxiMgf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} parxdashaRna; noVDuva, viVkiSxsuva -- avakAsha yA saMdaBaR. 
\num{2} dUradashaRna viVkaSxNe; dUradashaRnavanunx noVDuvudu yA noVDuva aBAyxsa. 
\enum
\emng
\eentry

\bentry
\word{viewless} 
\pron{vUyxlisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aBipArxyarahita; aBipArxyagaLilalxda. 
\num{2} (\kAparx, \alaMshA) kANada; adaqshayx; agoVcara. 
\enum
\emng
\eentry

\bentry
\word{viewpoint} 
\pron{vUyxpAiMTf} 
\gl{\nA}
\expl{}
\bmng
 daqSiTxkoVna; nilavu; savxMta aBipArxya; veYyakitxka BAvane. 
\emng
\eentry

\bentry
\word{viewy} 
\pron{vUyxi} 
\gl{\gu}
\expl{}
\bmng
 (\AmA) citarxvicitarxvAda aBipArxyagaLanunx hoMdiruva. 
\emng
\eentry

\bentry
\word{vigesimal} 
\pron{vijesima(mf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ipapxtatxneV oMdara yA ipapxtatxra. 
\num{2} ipapxtitxpapxtAtxgi, ipapxtitxpapxtatxraMte -- eNike mADuva, gaNisida. 
\enum
\emng
\eentry

\bentry
\word{vigesimally} 
\pron{vijesimali} 
\gl{\kirxvi}
\expl{}
\bmng
 ipapxtatxneV oMdaraMte yA ipapxtatxraMte. 
\emng
\eentry

\bentry
\word{vigil} 
\pron{vijilf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} jAgaraNe; nidirxsabeVkAda veVLeyalilx kAyalu yA pArxthaRne mADalu ecacxravAgiruvudu: \eng{keep vigil} jAgaraNe mADu; jAgaraNe varxtavananxnusarisu. 
\alnum{b} jAgaraNe avadhi; ecacxravAgiruva kAla. 
\eanum
\numie
\num{2} (\sA\ \bava dalilx) (rAtirxya) jAgaraNe varxtAcaraNegaLu. 
\num{3} (cacuR) (dhAmiRka hababxda) hiMdina dina yA hiMdina iruLu; pUvaRdina yA muninxruLu; dhAmiRka varxtada hiMdina upavAsada divasa yA rAtirx. 
\enum
\emng
\eentry

\bentry
\word{vigilance} 
\pron{vijilanfsx} 
\gl{\nA}
\expl{}
\bmng
ecacxrike; jAgarUkate; kaNiNxTiTxruvudu; huSArAgiruvike; nigAvaNe. 
\emng
\eentry

\bentry
\wordnospeech{vigilance committee}{vigilance committee}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\ame) nigAsamiti; ecacxrike, jAgarxta, jAgarUkatA -- samiti; (shAMtipAlanA vayxvasethxyu utatxmavAgilalxda samudAyadalilx janaru tAveV nimiRsikoMDa savxyaMperxVrita) shAMtipAlanA samiti. 
\emng
\eentry

\bentry
\word{vigilant} 
\pron{vijilaMTf} 
\gl{\gu}
\expl{}
\bmng
 (apAya, kaSaTx, \mo vugaLa bagegx) ecacxrikeyiMdiruva; jAgarUkavAda. 
\emng
\eentry

\bentry
\word{vigilante} 
\pron{vijilAYxMTi} 
\gl{\nA}
\expl{}
\bmng
 shAMti -- pAlaka, rakaSxka; shAMtipAlanA samitiya yA adaraMtha yAvudeV samitiya, maMDaliya sadasayx. 
\emng
\eentry

\bentry
\word{vigilantly} 
\pron{vijilaMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
 ecacxrikeyiMda; jAgarUkanAgi; huSArAgi; kaNiNxTuTx. 
\emng
\eentry

\bentry
\word{vigneron} 
\pron{viVnayxrAnf} 
\gl{\nA}
\expl{}
\bmng
 dArxkiSx kaqSigAra; dArxkiSx baLiLxyanunx beLeyuvavanu. 
\emng
\eentry

\bentry
\word[vignette(1)]{vignette} 
\pron{viVneyxTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} viVneyxTf; doDaDxkaSxra \mo vugaLa sutatxlU yA KAli jAgadalilx citirxsuva alaMkAra. 
\num{2} muKapatarx citarx; pusatxkada, \kanmu\ muKapatarxda, meVle niSakxqqSaTxvAda aMcugere hAkada, citarx yA alaMkArada vinAyxsa. 
\num{3} meVlAyada citarx; tale matutx Buja mAtarx kANuva, hinenxleyu CAyeguMdutAtx hoVguva BAvacitarx yA CAyAcitarx. 
\num{4} vaNaRnAtamxka kiruparxbaMdha yA vayxkitxcitarx. 
\enum
\emng
\eentry

\bentry
\word[vignette(2)]{vignette} 
\pron{viVneyxTf} 
\gl{\sakirx}
\expl{}
\bmng
 meVlAyada citarx bare, tege; CAyAcitarxdalilx yA BAvacitarxdalilx hinenxleya CAyeguMdisi, vayxkitxya meVlAyavanunx, taleBujagaLanunx mAtarx toVrisu. 
\emng
\eentry

\bentry
\word{vignetter} 
\pron{viVneyxTarf} 
\gl{\nA}
\expl{}
\bmng
 meVlAyada citarxkAra; BAvacitarx, CAyAcitarxgaLalilx hinenxleyanunx CAyeguMdisi taleBujagaLanunx mAtarx toVrisuvavanu. 
\emng
\eentry

\bentry
\word{vignetting} 
\pron{viVneyxTiMgf} 
\gl{\nA}
\expl{}
\bmng
 meVlAyada citarxracane; BAvacitarx CAyAcitarxgaLalilx hinenxleyanunx CAyeguMdisi meVlAyavanunx, taleBujagaLanunx mAtarx toVrisuvudu. 
\emng
\eentry

\bentry
\word{vignettist} 
\pron{viVneyxTisfTx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vignetter}{vignetter}. 
\emng
\eentry

\bentry
\word{vigor} 
\pron{vigarf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) \eng{vigour} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{vigoro} 
\pron{vigaroV} 
\gl{\nA}
\expl{}
\bmng
 (\AseTxrXV) vigaroV; kirxkeTf matutx beVsfbAlf ATagaLa aMshagaLanunx seVrisi, taMDagaLalilx ADuva ceMDATa. 
\emng
\eentry

\bentry
\word{vigorous} 
\pron{vigarasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gaTiTxmuTATxda matutx caTuvaTikeyiMda kUDida; daqDhagAtarxvU kirxyAshiVlavU Ada. 
\num{2} (giDada \vi) balavAgi beLediruva. 
\num{3} joVrAda; hurupina; utAsxhada; shakitxyutavAda; deYhika yA mAnasika shakitxbalagaLiMda kUDida yA mADida. 
\num{4} kasuvu, joVru, satatxvX, viVyaR, ceYtanayx, hurupu, Ojasusx -- uLaLx, iruva; deYhika shakitx yA caTuvaTike parxdashiRsuva (ciMtane, mAtu yA sheYliya \vi, \rUpa\ saha). 
\enum
\emng
\eentry

\bentry
\word{vigorously} 
\pron{vigarasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 viVyaRvatAtxgi; hurupiniMda. 
\emng
\eentry

\bentry
\word{vigorousness} 
\pron{vigarasfnisf} 
\gl{\nA}
\expl{}
\bmng
 kasuvu; viVyaRvatatxte; hurupu; utAsxha. 
\emng
\eentry

\bentry
\word{vigour} 
\pron{vigarf} 
\gl{\nA}
\expl{}
\bmng
 (\birx) 
\bnum
\num{1} caTuvaTikeyiMda, kirxyAshiVlateyiMda kUDida deVhashakitx. 
\num{2} aBivaqdidhx sithxtiyalilxruva deVhasithxti. 
\num{3} AroVgayxkaravAda beLavaNige. 
\num{4} ceVtanashakitx; ceYtanayx. 
\num{5} (BASaNa, AloVcane, sAhitayxka sheYli, \mo vugaLalilx kaMDubaruva) kasuvu; satatx vx; viVyaR; ceYtanayx; hurupu; Ojasusx. 
\num{6} curuku; joVru; caTuvaTike. 
\enum
\emng
\eentry

\bentry
\word{vigourless} 
\pron{vigarflisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kasuvilalxda; joVrilalxda; satatx vxhiVna. 
\num{2} utAsxharahita; ceYtanayxhiVna; hurupilalxda. 
\num{3} curukilalxda; caTuvaTikeyilalxda. 
\enum
\emng
\eentry

\bentry
\word{vihara} 
\pron{vihAra} 
\gl{\nA}
\expl{}
\bmng
 bwdAdhxlaya; bwdadhxvihAra; bwdadhxBikuSxgaLa vAsagaqha. 
\emng
\eentry

\bentry
\word[Viking(1)]{Viking} 
\pron{veYkiMgf} 
\gl{\nA}
\expl{}
\bmng
 veYkiMgf; \eng{8-11}neV shatamAnagaLalilx vAyavayx yUroVpina BAgagaLalilx nelasidadx sAkxYxMDineVviyada vAyxpArigaLu matutx kaDalugaLaLxralilx obabx. 
\emng
\eentry

\bentry
\word[Viking(2)]{Viking} 
\pron{veYkiMgf} 
\gl{\gu}
\expl{}
\bmng
 veYkiMgf- janara yA avara kAlakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vilayet} 
\pron{vilAyeTf} 
\gl{\nA}
\expl{}
\bmng
 vilAyatf; TakiRya oMdu pArxMta. 
\emng
\eentry

\bentry
\word{vile} 
\pron{veYlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keTaTx; neYtikavAgi tucaPx; kutisxta; adhama; BarxSaTx; duSaTx; heVya. 
\num{2} (\pArxparx) kelasakekx bArada; nikaqSaTx; gaNayxvalalxda. 
\num{3} (\AmA) tiVra keTaTx, holasAda: \eng{vile weather} tiVra holasAda havA. 
\num{4} jugupAsxtamxka; asahayxkara; heVsigeya. 
\enum
\emng
\eentry

\bentry
\word{vilely} 
\pron{veYlfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} tucaPxvAda riVtiyalilx; kutisxtavAgi; heVyavAgi. 
\num{2} tiVra holasAgi. 
\enum
\emng
\eentry

\bentry
\word{vileness} 
\pron{veYlfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} tucaPxte; kutisxtate; niVcate. 
\num{2} heVyate; asahayxkarate. 
\enum
\emng
\eentry

\bentry
\word{vilification} 
\pron{viliphikeVSanf} 
\gl{\nA}
\expl{}
\bmng
 hesaru keDisuvike; mAna kaLeyuvike; haLiyuvike; dUSaNe; niMdane. 
\emng
\eentry

\bentry
\word{vilifier} 
\pron{vilipheYarf} 
\gl{\nA}
\expl{}
\bmng
 hesaru keDisuvavanu; mAna kaLeyuvavanu; dUSisuvavanu; niMdaka; dUSaka. 
\emng
\eentry

\bentry
\word{vilify} 
\pron{vilipheY} 
\gl{\sakirx}(vataRmAna \parxpu\ \Eva\ \eng{vilifies;}
\expl{\BU\ matutx}
\BUkaq\ \eng{vilified}). \bmng
 hesaru keDisu; mAna kaLe; apamAna -- mADu; haLi; dUSisu; tegaLu. 
\emng
\eentry

\bentry
\word{vill} 
\pron{vilf} 
\gl{\nA}
\expl{}
\bmng
 (\ca) ULigapura; ULigamAnayx padadhxtiyalilxdadx saNaNx Uru, pALeya. 
\emng
\eentry

\bentry
\word{villa} 
\pron{vil} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (paTaTxNavAsiyAda shirxVmaMtana) haLiLxya mane; toVTada mane; gArxmanivAsa. 
\num{2} (\birx) (upanagaradalilxya madhayxmavagaRdavana) biDimane; savxtaMtarxgaqha; pUtaR yA BAgashaH parxteyxVkavAgiruva mane. 
\num{3} (\roV\ \pArxkatx) udAyxnagaqha; toVTada mane; jamiVninoLage kaTiTxkoMDiruva doDaDx mane. 
\num{4} (\kanmu\ videVshadalilx) (bADigeya) parxvAsi -- swdha, maMdira; yAtirxnivAsa. 
\enum
\emng
\eentry

\bentry
\word{village} 
\pron{vilijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} haLiLx; gArxma. 
\num{2} (\ame) kaDime adhikAragaLuLaLx gArxmasaBe; gArxmADaLita noVDikoLuLxva saNaNx purasaBe. 
\num{3} (oMdu samudAyaveMdu parigaNitavAda) haLiLxgaru; gArxmasathxru; gArxma(ni)vAsigaLu. 
\num{4} (\birx) nagara baDAvaNe; haLiLx jiVvanada lakaSxNagaLanunx uLisikoMDiruva paTaTxNada yA nagarada savxyaMpUNaRvAda yA elAlx anukUlagaLanUnx oLagoMDa parxdeVsha yA janasamudAya. 
\num{5} (\AseTxrXV) upanagarada aMgaDi maLige. 
\enum
\emng
\eentry

\bentry
\word{villager} 
\pron{vilijarf} 
\gl{\nA}
\expl{}
\bmng
 haLiLxga; gArxmavAsi; gArxmiVNa; gArxmasathx. 
\emng
\eentry

\bentry
\word{villain} 
\pron{vilanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ati duruLa; kaDuniVca; dhUtaR; mahA duSaTx; KaLa; KULa; pAtaki; shaTha. 
\num{2} (\AmA, laGu hAsayxdalilx) dhUtaR; tuMTa; puMDa; taMTekoVra; lPaMga: \eng{you little villain}! loV tuMTamari! 
\num{3} (\pArxparx) haLiLxgamAra; haLiLxmukakx; oraTa; oDaDx. 
\num{4} (\birx) (\AmA) vaqtitxnirata -- aparAdhi, pAtaki; takisxVrudAra. 
\hypertarget{villain(5)}{} 
\num{5} (nATaka \mo vugaLalilx) KaLanAyaka; nAyakanige viroVdhiyAda duSaTx pAtarx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{villain of the piece} = \hyperlink{villain(5)}{villain\((5)\)}. 
\emng
\eentry

\bentry
\word{villainess} 
\pron{vilanisf} 
\gl{\nA}
\expl{}
\bmng
 tiVra duSeTx; atiniVce; duruLe. 
\emng
\eentry

\bentry
\word{villainous} 
\pron{vilanasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} dwrAtamxyXda; niVcatanada; duSaTx; dwSaTxyXda. 
\num{2} (\AmA) tuMbA keTaTx; tiVrA adhAvxnada; holasAda; daridarx: \eng{villainous weather} tuMbA keTaTx, daridarx -- havA. 
\enum
\emng
\eentry

\bentry
\word{villainously} 
\pron{vilanasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 niVcatanadiMda; dwSaTxyXdiMda. 
\emng
\eentry

\bentry
\word{villainousness} 
\pron{vilanasfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{villainy}{villainy}. 
\emng
\eentry

\bentry
\word{villainy} 
\pron{vilani} 
\gl{\nA}
\expl{(\bava\ \eng{villainies}).}
\bmng
 dwrAtamxyX; niVcatana; duSaTxtana; dwSaTxyX. 
\emng
\eentry

\bentry
\word{villanelle} 
\pron{vilanelf} 
\gl{\nA}
\expl{}
\bmng
 vilanelf; udadxkUkx eraDeV aMtayxpArxsagaLiMda nivaRhisida matutx kelavu sAlugaLu punarAvataRneyAguva, \eng{19} sAlugaLa, \sA\ gArxmiVNa kAvayxrUpada yA BAvagiVtAtamxka, kAvayx. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ville}{-ville} 
\pron{-vilf} 
\gl{\aupa}
\expl{}
\bmng
 -nagara, pura, Uru, paTaTxNa; yAvudAdarU nidiRSaTx guNa \mo vakekx saMbaMdhisidaMte kalipxtavAda Ura hesarugaLanunx racisalu baLasuva \saupa, \udA\ \eng{Dragsville, Squaresville.} 
\emng
\eentry

\bentry
\wordf{villeggiatura}
\pron{vilejaTuara} 
\gl{\nA}
\expl{\It\ }
\bmng
haLiLxvAsa; gArxmavAsa; haLiLxyalilx nelasuvudu. 
\emng
\eentry

\bentry
\word{villein} 
\pron{vilinf} 
\gl{\nA}
\expl{}
\bmng
 (\ca) (ULigamAnayx padadhxtiya) jiVtagAra; hiDuvaLidAranige yA ULigada manetanakekx saMpUNaRnAgi dAsanAdavanu. 
\emng
\eentry

\bentry
\word{villeinage} 
\pron{vilinijf} 
\gl{\nA}
\expl{}
\bmng
 (\ca) 
\bnum
\num{1} ULigamAnayx yA ULigamAnayxda hiDuvaLi. 
\num{2} (ULigamAnayxda) jiVtagArike; jiVtagAratana. 
\enum
\emng
\eentry

\bentry
\word{villiform} 
\pron{viliphAmfR} 
\gl{\gu}
\expl{}
\bmng
 vilasf\eng{(villus)} rUpada. 
\emng
\eentry

\bentry
\word{villose} 
\pron{viloV(lA)sf} 
\gl{\gu}
\expl{}
\bmng
 vilasf \eng{(villus)} yukatx. 
\emng
\eentry

\bentry
\word{villosity} 
\pron{vilAsiTi} 
\gl{\nA}
\expl{}
\bmng
 vilasfyukatxte; vilasf \eng{(villus)} gaLiMda tuMbiruvike. 
\emng
\eentry

\bentry
\word{villous} 
\pron{vilasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vilasf\eng{(villus)} yukatx. 
\num{2} vilasfgaLaMtha. 
\num{3} (\savi) (haNuNx, hU, \mo vugaLa meVle) niVLaroVmagaLuLaLx; udadxvAda maqdugUdalugaLuLaLx. 
\enum
\emng
\eentry

\bentry
\word{villus} 
\pron{vilasf} 
\gl{\nA}
\expl{(\bava\ \eng{villi} \ucAcx\ vileY)}
\bmng
 vilasf: 
\banum
\alnum{a} (\aMrashA) teLuvAda nALadaMtha cAcike, \udA\ cikakx karuLina loVLopxreya meVliruva, AhAradalilxna poVSakAMshagaLanunx hiVruva cAcikegaLalolxMdu. 
\alnum{b} (\savi) (\bava dalilx) haNuNx hUgaLa meVliruva udadxneya maqdu -- kUdalu, roVma. 
\eanum
\emng
\eentry

\bentry
\word{vim} 
\pron{vimf} 
\gl{\nA}
\expl{}
\bmng
 (\AmA) kasuvu; shakitx; ceYtanayx; satatx vx; hurupu. 
\emng
\eentry

\bentry
\word{vimineous} 
\pron{viminiasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1}(\savi). 
\banum
\alnum{a} kuDigaLa; cigurugaLa. 
\alnum{b} ciguroDeyuva; palalxvisuva; kuDiyoDeyuva; konaroDeyuva. 
\eanum
\numie
\numi{2} 
\banum
\alnum{a} eLereMbegaLa. 
\alnum{b} reMbeyoDeyuva; koMbegaLanunx utapxtitx mADuva. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{vina} 
\pron{viVna} 
\gl{\nA}
\bmng
 viVNe; BAratada oMdu taMtivAdayx. \imglink{vina:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.5cm{V_Pictures/vina.jpg}}} 
\emng
\eentry

\bentry
\word{vinaceous} 
\pron{veYneVSasf} 
\gl{\gu}
\expl{}
\bmng
 veYnina keMpubaNaNxda. 
\emng
\eentry

\bentry
\word{vinaigrette} 
\pron{vinigerxTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (mUsi noVDalu suvAsanAdarxvayxvanunx tuMbiTuTx koLuLxva, AlaMkArikavAda) gaMdha kUpi. 
\alnum{b} (mUsalu GATulavaNagaLaninxDuva) mUsusiVse; AGArxNaka siVse. 
\hypertarget{vinaigrette(2)}{} 
\eanum
\numie
\num{2} (koVsuMbari, pacacxDi, gojujx, \mo vakekx hAkuva) eNeNx matutx veYnina vinigarf ivugaLa ogagxraNe, vayxMjana. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vinaigrette sauce}{vinaigrette sauce}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vinaigrette(2)}{vinaigrette\((2)\)}. 
\emng
\eentry

\bentry
\word{vincibility} 
\pron{vinisxbiliTi} 
\gl{\nA}
\expl{}
\bmng
 (sAhitayxka) jeVyate; jayisalAguvike. 
\emng
\eentry

\bentry
\word{vincible} 
\pron{vinisxbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 (sAhitayxka) jeVya; jayisalAguva; jayisalu sAdhayxvAda. 
\emng
\eentry

\bentry
\word{vinculum} 
\pron{viMkuyxlamf} 
\gl{\nA}
\expl{(\bava\ \eng{vincula} \ucAcx\ viMkuyxla). }
\bmng
\bnum
\num{1} (\biVga) baMdhareVKe; samudAyareVKe; biVjagaNitadalilx padagaLa guMpanunx kaMsagaLalilx seVrisuva badalu guMpina meVle eLeyuva aDaDxgere; hiMdumuMdina padagaLoDane mADabeVkAda gaNitakirxyegaLige padagaLa oMdu guMpanunx, oMdu samudAyavAgi parigaNisabeVkeMdu toVrisuvudakAkxgi meVlugaDe eLeyuva aDaDxgere, \udA\ \eng{$\overline{a+b} \times c=ac+bc$}, Adare \eng{$a + b \times c = a+bc$} Agutatxde. 
\num{2} (\aMrashA) asithxrajujx; mULenAru. 
\enum
\emng
\eentry

\bentry
\word{vindicable} 
\pron{viMDikabflf} 
\gl{\gu}
\expl{}
\bmng
 sAthxpisalAguva; sAthxpisabahudAda; samathaRniVya. 
\emng
\eentry

\bentry
\word{vindicate} 
\pron{viMDikeVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx, dhamaR, \mo vanunx sAkaSxyX, vAda, \mo vugaLiMda) etitxhiDi; samathiRsu. 
\num{2} (obabxna meVle huTiTxruva doVSa, saMshaya, \mo vanunx tolagisi avananunx) nidoRVSiVkarisu; apavAda kaLe. 
\num{3} (obabxna satayxte, dheYyaR, naDavaLike, naDate, heVLike, \mo vanunx) sAthxpisu; samathiRsu; nijaveMdu yA avugaLalilx nAyxyavideyeMdu rujuvAtu paDisu. 
\enum
\emng
\eentry

\bentry
\word{vindication} 
\pron{viMDikeVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx, dhamaR, \mo vanunx sAkaSxyX, vAda, \mo vugaLiMda) etitx hiDiyuvike; samathaRne. 
\num{2} nidoRVSiVkaraNa; obabxna meVle huTiTxruva saMshaya \mo vanunx tolagisi avana meVlina apavAdada nivAraNe. 
\num{3} (obabxna satayxte, dheYyaR, naDavaLike, naDate, heVLike, \mo vugaLa) sAthxpane; samathaRne. 
\enum
\emng
\eentry

\bentry
\word{vindicative} 
\pron{viMDikeVTivf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} samathaRka; samathiRsuva; nijaveMdu yA nAyxyaveMdu rujuvAtupaDisuva. 
\num{2} apavAda nivAraka; nidoRVSasAdhaka. 
\enum
\emng
\eentry

\bentry
\word{vindicator} 
\pron{viMDikeVTarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} samathaRka; (vayxkitx, dhamaR, \mo vanunx) etitx hiDiyuvavanu; (satayxte \mo vanunx) samathiRsuvavanu. 
\num{2} apavAda nivAraka; nidoRVSasAdhaka. 
\enum
\emng
\eentry

\bentry
\word{vindicatory} 
\pron{viMDikeVTari} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{vindicative}{vindicative}. 
\num{2} (kAnUnugaLa \vi) shikASxrUpada; daMDanAtamxka. 
\enum
\emng
\eentry

\bentry
\word{vindictive} 
\pron{viMDikiTxvf} 
\gl{\gu}
\expl{}
\bmng
 seVDu tiVrisuva; parxtiVkAraka; parxtiVkAra savxrUpada; muyiyxge muyiyx tiVrisuva; Cala sAdhisuva. 
\emng
\eentry

\bentry
\wordnospeech{vindictive damages}{vindictive damages}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 parxtiVkAraka parihAra; sAmAnayx parihAravanunx miVri parxtivAdiyanunx shikiSxsuva rUpadalilx vidhisida matutx vAdige parihAravAgi mAtarxvalalxde itararige ecacxrikeyAgiruvaMte shikASxrUpavAgi parxtivAdige vidhisida hecucxvari daMDa; ecacxrike shikASxdaMDa. 
\emng
\eentry

\bentry
\word{vindictively} 
\pron{viMDikiTxvfli} 
\gl{\kirxvi}
\expl{}
\bmng
 seVDina manoVvaqtitxyiMda; hage kArutatx; parxtiVkArakavAgi; parxtiVkAra rUpadalilx. 
\emng
\eentry

\bentry
\word{vindictiveness} 
\pron{viMDikiTxvfnisf} 
\gl{\nA}
\expl{}
\bmng
 seVDina manoVvaqtitx; hage kAruvike; parxtiVkAra -- baudidhx, manoVBAva. 
\emng
\eentry

\bentry
\word{vine} 
\pron{veYnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (veYTasf kulada) dArxkiSx baLiLx, late. 
\num{2} (\ame) (teLuvAda kAMDavuLaLx, habaubxva) yAvudeV baLiLx, late. 
\enum
\emng
\eentry

\bentry
\word{vine-borer} 
\pron{veYnfboVrarf} 
\gl{\nA}
\expl{}
\bmng
 dArxkiSx kiVTa; dArxkiSx baLiLxyanunx koredu tinunxva huLu. 
\emng
\eentry

\bentry
\word{vine-dresser} 
\pron{veYnfDerxsarf} 
\gl{\nA}
\expl{}
\bmng
 dArxkiSx -- pAlaka; dArxkiSxbaLiLxyanunx katatxrisuvudu, beLasuvudu, poVSisuvudu, pAlisuvudu, \mo vanunx mADuvavanu. 
\emng
\eentry

\bentry
\word{vinegar} 
\pron{vinigarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vinigarf; (dArxkiSxya veYnu, seVbina veYnu, \mo vugaLiMda tegeda, rucikArakavAgiyoV upipxnakAyi hAkuvudaralolxV baLasuva) dArxkiSx, seVbaugaLa kaTu huLirasa. 
\num{2} (\rUpa) huLimoVre; siDuku savxBAva; siDimiDiguTuTxva naDavaLike yA guNa. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\hyperdef{V}{vinegarpagu}{} \eng{aromatic vinegar} (kapURra \mo vugaLa dArxvaNavanunx seVrisida) sugaMdhi vinigarf. 
\emng
\eentry

\bentry 
\word{vinegarish} 
\pron{vinigariSf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vinigarfnaMtha; huLihuLiyAda. 
\num{2} (\rUpa) huLimoVreyAda; siDuku savxBAvada. 
\enum
\emng
\eentry

\bentry
\word{vinegary} 
\pron{vinigari} 
\gl{\gu}
\expl{}
\bmng
 vinigarf anunx hoVluva; vinigarfnaMte huLiyAda. 
\emng
\eentry

\bentry
\word{vinery} 
\pron{veYnari} 
\gl{\nA}
\expl{(\bava\ \eng{vineries}). }
\bmng
\bnum
\num{1} dArxkASxgAra; dArxkiSx beLeyuva (gAjina) mane. 
\num{2} = \hyperlink{vineyard}{vineyard\((1)\)}. 
\enum
\emng
\eentry

\bentry
\word{vineyard} 
\pron{vinfyA(ya)DfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} dArxkiSxtoVTa; \kanmu\ veYnf tayArisuva saluvAgi beLeyuva dArxkiSxbaLiLxya toVTa. 
\num{2} (\beY) kAyaRkeSxVtarx; duDime mADuva sathxLa. 
\enum
\emng
\eentry

\bentry 
\wordf{vingt-et-un} 
\pron{vAYxknfTeVnfR} 
\gl{\nA}
\expl{\F\ }
\bmng
 (isipxVTu) = \hyperref{kandict_p.pdf}{P}{pontoon(3)}{$^3$pontoon(1)}. 
\emng
\eentry

\bentry
\word{vini-} 
\pron{vini-} 
\gl{\sapUpa}
\expl{}
\bmng
 veYnu eMbathaRda \sapUpa. 
\emng
\eentry

\bentry
\word{viniculture} 
\pron{vinikalacxrf} 
\gl{\nA}
\expl{}
\bmng
 dArxkiSxya beVsAya; dArxkASxkaqSi. 
\emng
\eentry

\bentry
\word{vinification} 
\pron{viniphikeVSanf} 
\gl{\nA}
\expl{}
\bmng
 veYniVkaraNa; dArxkASxrasa \mo vanunx veYnanAnxgi parivatiRsuvike. 
\emng
\eentry

\bentry 
\wordf{vin ordinaire} 
\pron{vAYxknf ADiRneVrf} 
\gl{\nA}
\expl{\F}
\bmng
 (phArxnfsxnalilx sAmAnayxvAgi niVrinoDane berasi kuDiyuva) agagxda (\sA\ keMpu) veYnu. 
\emng
\eentry

\bentry
\word{vinosity} 
\pron{veYnAsiTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} veYnftana; veYnina ruci. 
\num{2} veYnina -- caTa, duraBAyxsa, cALi. 
\enum
\emng
\eentry

\bentry
\word{vinous} 
\pron{veYnasf} 
\gl{\gu}
\expl{}
\bmng
 veYninaMtha; veYnina; veYniniMduMTAda; veYnina caTada: \eng{vinous flavour} veYnina vAsane yA ruci. \eng{vinous fermentation} veYnina kiNavxna. \eng{vinous eloquence} veYniniMduMTAda vAgimxte; veYnina amaliniMduMTAda haraTuva cALi. 
\emng
\eentry

\bentry 
\wordf{vin rose} 
\pron{vAYxknf roVseZV} 
\gl{\nA}
\expl{\F}
\bmng
= \hyperref{kandict_r.pdf}{R}{rose}{rose}. 
\emng
\eentry

\bentry
\word[vint(1)]{vint} 
\pron{viMTf} 
\gl{\sakirx}
\expl{}
\bmng
 veYnanunx mADu, tayArisu. 
\emng
\eentry

\bentry
\word[vint(2)]{vint} 
\pron{viMTf} 
\gl{\nA}
\expl{}
\bmng
 (isipxVTATa) viMTf; AkaSxnf birxjfnaMtha raSAyxdeVshada oMdu bageya isipxVTATa. 
\emng
\eentry

\bentry
\word[vintage(1)]{vintage} 
\pron{viMTijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} dArxkiSxsugigxya oTuTx Pasalu. 
\num{2} sugigxdArxkiSxyiMda tayArisida veYnu. 
\num{3} veYnf mADalu sugigxdArxkiSxya saMgarxhaNe. 
\num{4} dArxkiSx saMgarxhaNeya kAla. 
\num{5} nidiRSaTx vaSaRdalilx yA jilelxyalilx tayArisida utatxmadajeRya, sherxVSaThxguNada veYnu. 
\num{6} vasutx \mo vanunx tayArisida nidiRSaTx vaSaR, tayArike vaSaR, \mo vu. 
\num{7} nidiRSaTx vaSaR \mo vugaLalilx tayArisida padAthaR \mo vu. 
\num{8} (\kAparx\ yA \rUpa) veYnu. 
\enum
\emng
\eentry

\bentry
\word[vintage(2)]{vintage} 
\pron{viMTijf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ hiMdina kAlada yA vayxkitxyobabxna sAdhaneya atuyxcacx avadhiyalilx mADida) sherxVSaThx, unanxta, atuyxtatxma -- guNada. 
\num{2} hiMdina -- Qutuvina, avadhiya, kAlada. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vintage car}{vintage car}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\birx) \eng{1919-30}ra avadhiyalilx tayArisida hiMdinakAlada, haLeya mAdariya kAru. 
\emng
\eentry

\bentry
\wordnospeech{vintage festival}{vintage festival}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 dArxkiSxsugigx utasxva, hababx; dArxkiSx koyulx kAlada pArxraMBavanunx sUcisalu Acarisuva hababx. 
\emng
\eentry

\bentry
\word{vintager} 
\pron{viMTijarf} 
\gl{\nA}
\expl{}
\bmng
 dArxkiSx koyulxgAra; dArxkiSxyanunx koyuyxvavanu, kaTAvu mADuvavanu. 
\emng
\eentry

\bentry
\word{vintner} 
\pron{viMTanxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} dArxkiSx veYnina vAyxpAri, vataRka. 
\num{2} (yAvudeV) veYnina vAyxpAri yA tayAraka. 
\enum
\emng
\eentry

\bentry
\word{viny} 
\pron{veYni} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} dArxkiSxya; dArxkiSxya savxrUpada; dArxkiSxyaMtha. 
\num{2} (\rUpa) (AliMganada \vi) bigidapupxva; gADhAliMganada. 
\enum
\emng
\eentry

\bentry
\word[vinyl(1)]{vinyl} 
\pron{veYnilf} 
\gl{\gu}
\expl{}
\bmng
 (\ravi) veYnilf; \eng{$\bg\rm CH\eg_2= \bg\rm CH\eg$-} racaneya paramANugucaPx. 
\emng
\eentry

\bentry
\word[vinyl(2)]{vinyl} 
\pron{veYnilf} 
\gl{\nA}
\expl{}
\bmng
 veYnilf paramANu gucaPxviruva yAvudeV saMyukatxda, \kanmu\ veYnilf kolxVreYDfna pAlimariVkaraNadiMda dorakuva pAlxsiTxkf. 
\emng
\eentry

\bentry
\word{viol} 
\pron{veYalf} 
\gl{\nA}
\expl{}
\bmng
 vayalf vAdayx; \sA\ \eng{6} taMtigaLuLaLx, kAlugaLa yA moNakAlugaLa madheyx neTaTxge hiDidukoMDu nuDisuva piTiVlu \mo vanunx hoVluva, \hfill\break madhayxyugada, vAdayx. \imglink{viol:fig}{\raisebox{-0.25cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.6cm {V_Pictures/viol.jpg}}} 
\emng
\eentry

\bentry
\word[viola(1)]{viola} 
\pron{viOl} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} doDaDxpiTiVlu; viyoVla; piTiVligiMta doDaDxdAda, vayalinf jAtiya, maMdarxshurxtiya saMgiVta vAdayx. 
\num{2} viyoVla vAdaka. 
\num{3} = \hyperlink{viol}{viol}. 
\enum
\emng
\eentry

\bentry
\word[viola(2)]{viola} 
\pron{veYal} 
\gl{\nA}
\expl{}
\bmng
 veYyala: 
\banum
\alnum{a} puruSaratanx; pAyxnisx\eng{(pansy)}, vayoleTf, \mo vanonxLagoMDa vayala kulada yAvudeV giDa. 
\alnum{b} I kulada giDadoMdige kasimADi beLesida yAvudeV giDa. 
\eanum
\emng
\eentry

\bentry
\word{violable} 
\pron{veYalbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 muriyabahudAda; BaMga mADabahudAda; ulalxMGaniVya; atikarxmaNiVya. 
\emng
\eentry

\bentry
\word{violaceous} 
\pron{veYaleVSasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} neVriLe baNaNxda. 
\num{2} (\savi) vayoVleVsiV vaMshakekx seVrida vayoleTf vagaRda; puruSaratanx sasayxjAtiya. 
\enum
\emng
\eentry

\bentry
\wordRemoveSpace{viola-da-braccio}{viola da braccio} 
\pron{viOla Da bArxcoV} 
\gl{\nA}
\expl{}
\bmng
 Adhunika viyoVlavanunx hoVluva piTiVlu. 
\emng
\eentry

\bentry
\wordRemoveSpace{viola-da-gamba}{viola da gamba} 
\pron{viOla Da gAYxMba} 
\gl{\nA}
\expl{}
\bmng
 Adhunika vayolaMceloV \eng{(violoncello)} vanunx hoVluva, \kanmu\ vAdakana kAlugaLa naDuve hiDidukoMDu nuDisuva, viyoVlf vAdayx. 
\emng
\eentry

\bentry
\wordRemoveSpace{viola-d'amore}{viola d'amore} 
\pron{viOla DAYxmoVreV} 
\gl{\nA}
\expl{}
\bmng
 madhuranAdada tArashurxtiya viyoVla vAdayx. 
\emng
\eentry

\bentry
\word{violate} 
\pron{veYaleVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (parxmANa, karAru, shAsana, nibaMdhanegaLu, AtamxsAkiSx, \mo vanunx) muri; BaMgapaDisu; ulalxMGisu; miVrinaDe. 
\num{2} (pavitarx sathxLa \mo vanunx) keDisu; apavitarxgoLisu; kaluSitagoLisu; dUSita mADu. 
\num{3} (vayxkitxya EkAMtate \mo vanunx) BaMgagoLisu; atikarxmisu. 
\num{4} (\sA\ heMgasanunx) keDisu; (heMgusina) mAnaBaMga, shiVlaBaMga -- mADu. 
\enum
\emng
\eentry

\bentry
\word{violation} 
\pron{veYaleVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (parxmANa, karAru, shAsana, AtamxsAkiSx, vayxkitxya EkAMtate, \sA\ heMgasu shiVla, \mo vugaLanunx) muriyuvike; BaMga. 
\num{2} (pavitarx sathxLa \mo vugaLa \vi) keDisuvike; apavitarxgoLisuvike; kaluSitagoLisuvike. 
\enum
\emng
\eentry

\bentry
\word{violator} 
\pron{veYaleVTarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} BaMjaka; parxmANa, karAru, shAsana, AtamxsAkiSx, EkAMtate, \sA\ heMgusina shiVla, \mo vanunx BaMgamADuvavanu. 
\num{2} (pavitarx sathxLa \mo vanunx) keDisuvavanu; apavitarxgoLisuvavanu. 
\enum
\emng
\eentry

\bentry
\word{violence} 
\pron{veYalanfsx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hiMse; dwjaRnayx; hAni. 
\numi{2} 
\banum
\alnum{a} hiMsAcAra. 
\alnum{b} apacAra. 
\alnum{c} (heMgasina meVle) atAyxcAra; hAnimADuvike. 
\eanum
\numie
\num{3} joVru; raBasa; birusu. 
\numi{4} (\nAyxshA) 
\banum
\alnum{a} akarxma -- balAtAkxra, balaparxyoVga; juluM. 
\alnum{b} akarxma balaparxyoVgada bedarike. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
 \eng{do violence to} 
\banum
\alnum{a} virudadhxvAgi vatiRsu. 
\alnum{b} apacAra mADu. 
\alnum{c} atAyxcAra mADu. 
\alnum{d} (BASeya, padada, garxMthada \vi) apAthaR mADu; athaRtirucu. 
\alnum{e} (garxMtha, leVKanada \vi) AdhAravilalxde pATha -- tirucu, badalAyisu. 
\eanum
\emng
\eentry

\bentry
\word{violent} 
\pron{veYalaMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hiMseya; hiMsAtamxka. 
\num{2} raBasada; birusAda; joVrAda; parxcaMDa: \eng{a violent storm} parxcaMDa mAruta (matutx birumaLe). \eng{the scooter came into violent collision with the bus} A sUkxTaru basisxge birusAgi Dikikx hoDeyitu. 
\num{3} julumina; akarxmavAgi yA nAyxyabAhiravAgi balaparxyoVgavanunx, balAtAkxravanunx, oLagoMDa: \eng{he laid violent hands on the opponent} avanu edurALiya meVle akarxmavAgi keYmADida. 
\num{4} tiVvarx; BAri; ugarx; birusAda; parxcaMDa: \eng{violent pain} tiVvarxvAda yAtane. \eng{violent abuse} ugarxvAda beYguLa. \eng{violent controversy} birusAda vAdavivAda. \eng{violent discrepancy} BAri vayxtAyxsa. \eng{violent contrast} tiVvarxvAda asAdaqshayx. \eng{violent shock} BAri AGAta. 
\num{5} roVSAveVshada; BAvAveVshada: \eng{violent quarrel} roVSAveVshada jagaLa. 
\num{6} kaDu; tiVvarx; kaNiNxge rAcuva, baDiyuvaMtiruva : \eng{violent colours} kaDu baNaNxgaLu; kaNiNxge baDiyuvaMtha baNaNxgaLu. 
\num{7} (sAvina \vi) balavaMtada; bAhayx balaparxyoVga diMda yA viSadiMda saMBavisida. 
\enum
\emng
\eentry

\bentry
\wordnospeech{violent death}{violent death}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 dumaRraNa; GoVra maraNa; balaparxyoVgadiMdaloV viSaparxyoVga diMdaloV uMTAda sAvu. 
\emng
\eentry

\bentry
\word{violently} 
\pron{veYalaMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} hiMsAtamxkavAgi; hiMseyiMda. 
\num{2} birusAgi; raBasadiMda; parxcaMDavAgi. 
\num{3} (nAyxyabAhiravAda) balaparxyoVgadiMda. 
\num{4} tiVvarxvAgi; ugarxvAgi; parxcaMDavAgi. 
\num{5} korxVdhAveVshadiMda; roVSAveVshadiMda. 
\enum
\emng
\eentry

\bentry
\word[violet(1)]{violet} 
\pron{veYalaTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vayoleTf; puruSaratanx; (niVli, UdA, biLi, \mo\ baNaNxgaLa hUbiDuva) vayoVla kula yA A kulada hUgiDa yA hUvu. 
\num{2} (sapatxvaNaRpaTala yA roVhitadalilx) neVriLe baNaNx; niVlaloVhita. 
\numi{3} 
\banum
\alnum{a} kaDubaNaNxda, tiVkaSxNX vaNaRda raMgu. 
\alnum{b} vayoleTf baNaNxda baTeTx yA uDupu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[violet(2)]{violet} 
\pron{veYalaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (sapatxvaNaRpaTaladalilx) neVriLe baNaNxda; niVlaloVhita. 
\num{2} (baTeTx yA itara vasutxgaLa \vi) vayoleTf baNaNxda; neVriLe baNaNxda. 
\enum
\emng
\eentry

\bentry
\word{violin} 
\pron{veYalinf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} piTiVlu; kamAniniMda nuDisuva, nAlukx taMtigaLa, tirxsAthxyiya vAdayx. \imglink{violin:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.6cm {V_Pictures/violin.jpg}}} 
\hypertarget{violin(2)}{} 
\num{2} piTiVlu vAdaka. 
\enum
\emng
\eentry

\bentry
\word{violinist} 
\pron{veYalinisfTx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{violin(2)}{violin\((2)\)}. 
\emng
\eentry

\bentry 
\word{violist} 
\pron{veYalisfTx} 
\gl{\nA}
\expl{}
\bmng
 vayalisfTx; vayalf \eng{(viol)} yA viyoVl\eng{(viola) -} vAdaka, nuDisuvavanu. 
\emng
\eentry

\bentry
\word{violoncellist} 
\pron{veY(viV)alaMcelisfTx} 
\gl{\nA}
\expl{}
\bmng
 vayolanecxlo vAdaka. 
\emng
\eentry

\bentry
\word{violoncello} 
\pron{veY(viV)alaMceloV} 
\gl{\nA}(\bava\ \eng{violoncellos}). 
\bmng
 = \hyperref{kandict_c.pdf}{C}{cello}{cello}. 
\emng
\eentry

\bentry
\word{violone} 
\pron{vialoVni} 
\gl{\nA}
\expl{}
\bmng
 viyaloVni; atimaMdarx sAthxyiya, BArigAtarxda vayalfvAdayx. 
\emng
\eentry

\bentry
\wordnospeech{VIP}{VIP}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{very important person.} 
\emng
\eentry

\bentry
\word{viper} 
\pron{veYparf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (veYpariDiV vaMshada, GoVraviSada) veYparf hAvu; maMDalada hAvu, \kanmu\ \eng{common viper} (\engit{or} \eng{adder)} iMgelxMDina, GoVraviSada, AYxDarf hAvu. 
\num{2} GoVrasapaR (daMthavanu); viSakAruvavanu; keVDiga. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{viper in one's bosom} uDiya(lilxTuTxkoMDa) sapaR; upakArigaLigeV dorxVhamADuva kaDudorxVhi; kaqtaGanx. 
\emng
\eentry

\bentry
\word{viperiform} 
\pron{veYpariphAmfR} 
\gl{\gu}
\expl{}
\bmng
 maMDalada hAvinAkArada. 
\emng
\eentry

\bentry
\word{viperine} 
\pron{veYpareY(ri)nf} 
\gl{\gu}
\expl{}
\bmng
 veYparfnaMtha; (maMDalada) hAvinaMtha, viSamaya. 
\emng
\eentry

\bentry
\word{viperish} 
\pron{veYpariSf} 
\gl{\gu}
\expl{}
\bmng
 hAvinaMtha; GoVraviSada; haritavAda; tiVkaSx Nx: \eng{viperish tongue} viSada, haritavAda -- nAlige. 
\emng
\eentry

\bentry
\word{viperlike} 
\pron{veYparfleYkf} 
\gl{\gu}
\expl{}
\bmng
 maMDalada hAvinaMtha yA adara vataRneya. 
\emng
\eentry

\bentry
\word{viperous} 
\pron{veYparasf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{viperish}{viperish}. 
\emng
\eentry

\bentry
\wordRemoveSpace{viper's-bugloss}{viper's bugloss} 
\pron{veYpasfR baUyxgAlxsf} 
\gl{\nA}
\expl{}
\bmng
 niVlihU biDuva, muLuLx muLALxda, ekiya valAgxYxri kulada giDa. 
\emng
\eentry

\bentry
\wordnospeech{viper's grass}{viper's grass}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_s.pdf}{S}{scorzonera}{scorzonera}. 
\emng
\eentry

\bentry
\word{virago} 
\pron{virA(reV)goV} 
\gl{\nA}(\bava\ \eng{viragos}). 
\bmng
\bnum
\num{1} gaMDubiVri; bajAri; tATagititx; gayAyxLi. 
\num{2} (\pArxparx) (puruSabala, puruSasethxYyaRvuLaLx) diTeTx; pwruSada heMgusu; dheYyaRshAlini. 
\enum
\emng
\eentry

\bentry
\word{viral} 
\pron{veYara(rf)lf} 
\gl{\gu}
\expl{}
\bmng
veYrasfna; roVgakAraka sUkaSx mxjiVviya, adariMdAguva, adakekx saMbaMdhisida yA avugaLanunxLaLx. 
\emng
\eentry

\bentry 
\word{virally} 
\pron{veYarali} 
\gl{\kirxvi}
\expl{}
\bmng
 veYrasisxnaMte; roVgakAraka sUkaSx mxjiVviyaMte yA adariMda. 
\emng
\eentry

\bentry
\word{virelay} 
\pron{virileV} 
\gl{\nA}
\expl{}
\bmng
 virileV; kiru BAvagiVte; \kanmu\ haLe pherxMcina, parxtiyoMdu padayxdalUlx eraDeV eraDu pArxsagaLanunx vividhavAgi joVDisiruva, saNaNx BAvagiVte. 
\emng
\eentry

\bentry
\word{virement} 
\pron{veYarfmaMTf, viarfmAknf}
\gl{\nA}
\expl{}
\bmng
 lekakxKAte vagARvaNe; lekAkxcArada vivaragaLanunx oMdu KAteyiMda matotxMdakekx vagARyisuvudu. 
\emng
\eentry

\bentry
\word{vireo} 
\pron{viriO} 
\gl{\nA}(\bava\ \eng{vireos}).
\bmng
 viriyoV; viriyoVniDiV vaMshada, amerikada, hasuru baNaNxda, oMdu saNaNx hADuhakikx. \imglink{vireo:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.4cm {V_Pictures/vireo.jpg}}} 
\emng
\eentry

\bentry
\word{virescence} 
\pron{viresa(sf)nfsx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hasuru; pacecx; haritatxvX. 
\num{2} (\savi) (\sA\ savxlapx hoLapu baNaNxvirabeVkAda esaLu, ele, \mo vugaLalilx) atihasuru; haridatireVka. 
\enum
\emng
\eentry

\bentry
\word{virescent} 
\pron{viresaMTf} 
\gl{\gu}
\expl{}
\bmng
 hasuru (baNaNxda) CAyeya yA hasurige tiruguva. 
\emng
\eentry

\bentry
\word[virgate(1)]{virgate} 
\pron{vagaRTf} 
\gl{\gu}
\expl{}
\bmng
 (\savi\ matutx \pArxvi) teLaLxge -- neVravAgiruva, neTaTxgiruva; laMbAkArada saraLinaMtiruva. 
\emng
\eentry

\bentry
\word[virgate(2)]{virgate} 
\pron{vagaRTf} 
\gl{\nA}
\expl{}
\bmng
 (\birx) (\ca) BUmiya oMdu aniSakxqqSaTxvAda aLate, \kanmu\ \eng{30} ekaregaLu. 
\emng
\eentry

\bentry
\word{virger} 
\pron{vajaRrf} 
\gl{\nA}
\expl{}
\bmng
 \eng{verger} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{Virgillian} 
\pron{vajiRlianf} 
\gl{\gu}
\expl{}
\bmng
 (\kirxpU\ \eng{19}ralilx gatisida roVmanf kavi) vajiRlana yA A kaviya sheYliya, sheYliyalilxna. 
\emng
\eentry

\bentry
\word[virgin(1)]{virgin} 
\pron{vajiRnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} deYhika saMBoVgavananxriyada vayxkitx. 
\num{2} (heMgasina \vi) kaneyx; kaninxke; akaSxtayoVni; puruSasaMBoVgavAgada heMgasu. 
\num{3} kanAyxdiVkiSxte; kanAyxvarxta diVkeSxyanunx keYgoMDa yAvudeV saMsethxyavaLu. 
\hypertarget{virgin(4)}{} 
\num{4} pavitarx kaneyx; yeVsu kirxsatxna tAyiyAda meVri kaneyx. 
\num{5} pavitarx kaneyxya, yeVsuvina tAyiya, citarx yA parxtime. 
\num{6} kanAyxkiVTa; gaBARdhAnavilalxdeyeV moTeTxgaLaninxDuva heNuNxhuLu. 
\num{7} = \hyperlink{Virgo}{Virgo\((1)\)}. 
\num{8} ananuBavi; amAyaka; mugadhx: \eng{a political virgin} rAjakiVya ananuBavi. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{the (Blessed) Virgin (Mary)} = \hyperlink{virgin(4)}{virgin\((4)\)}. 
\emng
\eentry

\bentry
\word[virgin(2)]{virgin} 
\pron{vajiRnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kanAyxvasethxya; kanAyxsithxtiya; kanAyxdasheya. 
\num{2} kanoyxVcita; kanAyxyoVgayx; kanAyxhaR; kaneyxge takakx: \eng{virgin modesty} kanoyxVcitavAda saMkoVca, lajejx. 
\num{3} parishudadhx; niSakxlaMka; niduRSaTx. 
\num{4} baLasilalxda; kaqSi mADilalxda: \eng{virgin soil} kaqSi mADilalxda BUmi, jamiVnu. 
\num{5} (jeVDi maNiNxna \vi) hasi jeVDimaNuNx; suTiTxlalxda jeVDimaNuNx. 
\num{6} (loVhada \vi) aduranunx karagisi tayArisida. 
\num{7} (uNeNxya \vi) inUnx nUtilalxda yA neVyidxlalxda. 
\num{8} (kiVTada \vi) gaBARdhAnavilalxdeyeV moTeTxyiDuva. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{the Virgin Queen} kanAyxrANi; iMgelxMDina modalaneV elijZbetf rANi\eng{(1533-1603)}. 
\emng
\eentry

\bentry 
\word[virginal(1)]{virginal} 
\pron{vajiRnalf} 
\gl{\gu}
\expl{}
\bmng
 kaneyxya; kaneyxge seVrida; kaneyxge takakx; kanoyxVcita. 
\emng
\eentry

\bentry
\word[virginal(2)]{virginal} 
\pron{vajiRnalf} 
\gl{\nA}
\expl{}
\bmng
 (\sA\ \bava dalilx) vajiRnalf(vAdayx); 16, 17neV shatamAnagaLalilx baLasutitxdadx, peTiTxgeyalilxTiTxrutitxdadx, AdayxrUpada sipxneTf \eng{(spinet)} vAdayx. \imglink{\(^2\)virginal:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.5cm {V_Pictures/virginal(2).jpg}}} 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{pair of virginals} = \hyperlink{virginal(2)}{$^2$virginal}. 
\emng
\eentry

\bentry
\word{virginalist} 
\pron{vajiRnalisfTx} 
\gl{\nA}
\expl{}
\bmng
 vajiRnalf (vAdayx) vAdaka. 
\emng
\eentry

\bentry
\word{virginally} 
\pron{vajiRnali} 
\gl{\kirxvi}
\expl{}
\bmng
 kaneyxge takakxMte; kanoyxVcitavAgi. 
\emng
\eentry

\bentry
\wordnospeech{virgin birth}{virgin birth}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (yeVsu kirxsatxna jananada \vi) kanAyxjatavx; mAnavapitanilalxdeyeV mAnavamAteyAda meVriya udaradiMda janisidaneMba sidAdhxMta. 
\num{2} aniSeVka -- janana yA Palana. 
\enum
\emng
\eentry

\bentry
\wordnospeech{virgin comb}{virgin comb}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (jeVnutupapxkAkxgi mAtarx omemx baLasida, saMtAnoVtapxtitxgAgi eMdU baLasilalxda) jeVnuhuTuTx; kanenxhuTuTx. 
\emng
\eentry

\bentry
\wordnospeech{virgin forest}{virgin forest} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (mAnavanu keYhAkade iruva) sahajAraNayx. 
\emng
\eentry

\bentry 
\word{virginhood} 
\pron{vajiRnfhuDf} 
\gl{\nA}
\expl{}
\bmng
 kanAyxtavx; kanAyxvasethx; kanAyxdashe; kaneyxyAgiruva avadhi. 
\emng
\eentry

\bentry 
\wordnospeech{virgin honey}{virgin honey}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (kanAyxhuTiTxniMda tegeda yA jeVnuhuTiTxniMda kAvanonxV, otatxDavanonxV parxyoVgisade tegeda) kanenx jeVnutupapx. 
\emng
\eentry

\bentry 
\word{Virginia} 
\pron{vajiRnia} 
\gl{\nA}
\expl{}
\bmng
 vajiRniya: 
\banum
\alnum{a} (amerika deVshada) vajiRniyada taMbAku, hogesopu px. 
\alnum{b} vajiRniya hogesopipxniMda mADida sigareVTu. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{Virginia creeper}{Virginia creeper}
\pron{?}
\gl{\nA}
\expl{}
\bmng
vajiRniya baLiLx; alaMkArakAkxgi beLasuva, utatxra amerikada, pAtaRnoVsisasf kivxMkavxphoVlaya kulada oMdu haMbau. 
\emng
\eentry

\bentry 
\word[Virginian(1)]{Virginian} 
\pron{vajiRnianf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) vajiRniyada vayxkitx. 
\emng
\eentry

\bentry
\word[Virginian(2)]{Virginian} 
\pron{vajiRnianf} 
\gl{\gu}
\expl{}
\bmng
 (\ame) vajiRniyada. 
\emng
\eentry

\bentry
\wordnospeech{Virginian stock}{Virginian stock}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{Virginia stock}{Virginia stock}. 
\emng
\eentry

\bentry
\wordnospeech{Virginia reel}{Virginia reel}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\ame) vajiRniya kuNita, naqtayx; oMdu bageya haLiLxya kuNita. 
\emng
\eentry

\bentry 
\wordnospeech{Virginia stock}{Virginia stock}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 vajiRniya (sATxkf)giDa; biLiya matutx gulAbi baNaNxda hUvugaLanunx biDuva, koMbekoMbegaLAgi beLeyuva, mAYxlfkAlimxya mAYxriTima kulada, oMdu giDa. 
\emng
\eentry

\bentry 
\word{virginity} 
\pron{vajiRniTi} 
\gl{\nA}
\expl{}
\bmng
 kanAyxtavx; kanAyxsithxti; (deYhika)saMBoVgavananxriyada sithxti. 
\emng
\eentry

\bentry 
\wordnospeech{virgin queen}{virgin queen}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (gaBARdhAnavilalxda) kanenxjeVnu(huLu); rANijeVnu. 
\emng
\eentry

\bentry
\wordnospeech{virgin's bower}{virgin's bower}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 kilxmAYxTisf viTisela kulada baLiLx. 
\emng
\eentry

\bentry
\word{Virgo} 
\pron{vagoRV} 
\gl{\nA}
\expl{(\bava\ \eng{Virgos}). }
\bmng
\bnum
\numi{1} kanAyx rAshi: 
\banum
\alnum{a} kaneyxya AkAradalilxruvudeMdu sAMparxdAyikavAgi BAvisalAda, nakaSxtarxgaLa guMpu. 
\alnum{b} siMha matutx tulAgaLa naDuve iruva Araneya rAshi (= the \hyperlink{virgin(2)}{Virgin}). 
\eanum
\numie
\num{2} kanAyxrAshiya vayxkitx; sUyaRnu kanAyxrAshiyalilxdAdxga huTiTxda vayxkitx. 
\enum
\emng
\eentry

\bentry
\word[Virgoan(1)]{Virgoan} 
\pron{vagoRVanf} 
\gl{\gu}
\expl{}
\bmng
 kanAyxrAshiya yA adaralilx huTiTxda. 
\emng
\eentry

\bentry
\word[Virgoan(2)]{Virgoan} 
\pron{vagoRVanf} 
\gl{\nA}
\expl{}
\bmng
 kanAyxrAshiyalilx huTiTxda vayxkitx. 
\emng
\eentry

\bentry
\word{virgule} 
\pron{vagUyxRlf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} Oregere; padagaLa yA sAlugaLa viBAgavanunx gurutisalu eLeyuva Oregere. 
\num{2} = \hyperref{kandict_s.pdf}{S}{solidus(2)}{solidus(2)}. 
\enum
\emng
\eentry

\bentry
\word{viridescence} 
\pron{viriDisanfsx} 
\gl{\nA}
\expl{}
\bmng
 viriDisanfsx; hasuru CAye; haridavxNaR. 
\emng
\eentry

\bentry
\word{viridescent} 
\pron{viriDesa(sf)MTf} 
\gl{\gu}
\expl{}
\bmng
 haridavxNaR; hasuru CAyeya yA hasurige tirugutitxruva. 
\emng
\eentry

\bentry 
\word[viridian(1)]{viridian} 
\pron{viriDianf} 
\gl{\gu}
\expl{}
\bmng
 niVlimisharxvAda hasurina; niVlaharidavxNaRda. 
\emng
\eentry

\bentry
\word[viridian(2)]{viridian} 
\pron{viriDianf} 
\gl{\nA}
\expl{}
\bmng
 niVlaharidavxNaR darxvayx; korxVmiyamf AkesxYDfniMda tayArisida, niVlimisharxvAda hasuru raMgu. 
\emng
\eentry

\bentry 
\word{viridity} 
\pron{viriDiTi} 
\gl{\nA}
\expl{}
\bmng
 (sAhitayxka) hasurutana; haritatx vx; hasiru baNaNx. 
\emng
\eentry

\bentry
\word{virile} 
\pron{vireYlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gaMDina; puruSana. 
\num{2} puMsatx vxvuLaLx; saMtAnoVtapxtitx shakitxyuLaLx. 
\num{3} (manasusx, shiVla, sAhitayxsheYli, \mo vugaLa \vi) pwruSada; gaMDutanada; viVyaRvatAtxda; OjasivxyAda; kasuvuLaLx. 
\enum
\emng
\eentry

\bentry 
\wordnospeech{virile member}{virile member}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\pArxparx) tuNeNx; shishanx. 
\emng
\eentry

\bentry
\word{virilism} 
\pron{virilisaZmf} 
\gl{\nA}
\expl{}
\bmng
 (\veYshA) puruSatavx; puMsatx vx; heNiNxnalilx gwNapuruSa lakaSxNagaLu beLeyuvudu yA gaMDinalilx akAladalilxyeV pwrxDha puruSalakaSxNagaLu kANisikoLuLxvudu. 
\emng
\eentry

\bentry
\word{virility} 
\pron{viriliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} gaMDutana; puruSatavx. 
\num{2} puMsatxvX; saMtAnoVtApxdaka shakitx. 
\num{3} (manasusx, shiVla, sAhitayxsheYli, \mo vugaLa \vi) viVyaRvatatxte; Ojasusx; kasuvu. 
\enum
\emng
\eentry

\bentry 
\word{virological} 
\pron{veYralAjika(kf)lf} 
\gl{\gu}
\expl{}
\bmng
 veYrAlajiya; roVgasUkASxmXNushAsatxrXda; veYrasusxgaLa adhayxyanakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{virologically} 
\pron{veYralAjikali} 
\gl{\kirxvi}
\expl{}
\bmng
 veYrasusxgaLa adhayxyanakekx, sUkaSx mxroVgANushAsatxrXkekx -saMbaMdhisidaMte. 
\emng
\eentry

\bentry
\word{virologist} 
\pron{veYrAlajisfTx} 
\gl{\nA}
\expl{}
\bmng
 veYrasf shAsatxrXjacnx; sUkaSx mxroVgANushAsatxrXjacnx; veYjAcnxnikavAgi veYrasusxgaLa adhayxyana mADuvavanu. 
\emng
\eentry

\bentry
\word{virology} 
\pron{veYrAlaji} 
\gl{\nA}
\expl{}
\bmng
 veYrAlaji; roVgasUkASxmXNugaLa veYjAcnxnika adhayxyana. 
\emng
\eentry

\bentry
\word{virtu} 
\pron{vaTUR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (lalita) kalAperxVma; (lalita) kalABijacnxte; (kalA)rasikate; kalABiruci; lalitakalegaLa viSayadalilx visheVSajAcnxna, kwshala, aBirucigaLiruvike. 
\hypertarget{virtu(2)}{} 
\num{2} \kanmu\ gAyana yA vAdanadalilx visheVSa pariNAmagaLanunx uMTumADuva tAMtirxka aMshagaLige koDuva Adayxte, visheVSa gamana. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{article} (\engit{or} \eng{object) of virtu} kalAkwtuka vasutx; mATada aMda, pArxciVnate, apUvaRte, \mo\ daqSiTxgaLiMda -- sAvxrasayxkaravAda, kutUhalakAriyAda vasutx. 
\emng
\eentry

\bentry 
\word{virtual} 
\pron{vaTuRyxalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (hesarigalalxdidadxrU, niSakxqqSaTxvAda athaRda daqSiTxyiMdalalxdidadxrU) pariNAmasidadhxvAda; vasutxtaH; vAsatxvavAda; kAyaRtaH: \eng{he is the virtual manager of the business} Ata (hesarinalalxlalxdidadxrU) A udayxmada vAsatxva vayxvasAthxpaka. \eng{take this as a virtual promise} idanunx vasutxtaH vAgAdxnaveMdeV tiLiduko. \eng{this constitutes virtual exculpation of the wrongdoer} idariMda tapipxtasathxnanunx kAyaRtaH doVSavimukitxgoLisidaMteyeV Agutatxde. (\daqvi) vAsatxvapArxya; ABAsa; beLakina rashimxya pathagaLanunx himumxKavAgi laMbisidare avu saMdhisuva biMdugaLige saMbaMdhisida: \eng{virtual focus} ABAsanABi: \eng{virtual image} ABAsabiMba. 
\num{3} (\yaMshA) vayxvasethxyoMdaralilx biMduvoMdara atayxMta alapx sAthxnapalalxTakekx saMbaMdhisida. 
\num{4} (kaMpUyxTarf) vAsatxvapArxyavAda; vAsatxvABAsada; vAsatxvavAgi Bwtika asitxtavxdalilxlalxdiruva Adare sAphfTxveVrfniMda asitxtavxdalilxruvaMte toVruva: \eng{virtual memory} vAsatxvapArxyavAda samxraNe. 
\enum
\emng
\eentry

\bentry 
\word{virtuality} 
\pron{vaTuRyxAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (bAhayxrUpakikxMta BinanxvAda) sArasavxrUpa; mUlasatetx. 
\num{2} shakayxte; kAyaRrUpakekx baralu sAdhayxgoLisuva shakitx yA sAmathaRyx. 
\enum
\emng
\eentry

\bentry 
\word{virtually} 
\pron{vaTuRyxali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} mUlAMshagaLige saMbaMdhisidaMte. 
\num{2} vasutxtaH; vAsatxvikavAgi; kAyaRtaH. 
\enum
\emng
\eentry

\bentry
\word{virtue} 
\pron{vaTURyx, vacUR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (su)shiVla; Qujutavx; oLeLxyatana; suguNa; sadugxNa; dhamaR; puNayx: \eng{virtue is its own reward} suguNada parxtiPala suguNaveV; dhamaRkekx parxtiPala dhamaRveV. 
\num{2} yAvudeV nidiRSaTx rUpada sadugxNa: \eng{patience is a virtue} tALemx athavA sahane oMdu sadugxNa. 
\num{3} (\kanmu\ heMgasina \vi) saciPxVla; sacAcxritarxyx; pAtivarxtayx; sAdhivxVtavx; satiVtavx. 
\num{4} savxBAva; guNa; shakitx: \eng{he has the virtue of being adjustable} avanige hoMdikoLuLxva guNavide. 
\num{5} guNa; shakitx; sAmathaRyx; bala; satatx vx; parxBAva: \eng{no virtue in such drugs} aMtha madudxgaLalilx yAva satatx vxvU ilalx. 
\num{6} sapatxma vagaRda deVvadUtaralolxbabx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{by} (\engit{or} \eng{in) virtue of} baladiMda; parxBAvadiMda; kAraNadiMda; nimitatxdiMda; AdhArada meVle: \eng{he claims it by} (\engit{or} \eng{in) virtue of his long service} tananx diVGARvadhiya seVveya AdhArada meVle avanu adakekx hakukx hUDutAtxne. 
\num{2} \eng{make a virtue of} \hyperref{kandict_n.pdf}{N}{necessity pagu(3)}{necessity}. 
\enum
\emng
\eentry

\bentry 
\word{virtueless} 
\pron{vaTURyx(cUR)lisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} shiVlavilalxda; sadugxNaRahita. 
\num{2} satatx vxvilalxda; balavilalxda. 
\enum
\emng
\eentry

\bentry 
\word{virtuosic} 
\pron{vaTuRyx(cUR)Asikf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{virtuoso(2)}{$^2$virtuoso}. 
\emng
\eentry

\bentry 
\word{virtuosity} 
\pron{vaTuyxR(cuR)AsiTi} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{virtu(2)}{virtu\((2)\)}. 
\emng
\eentry
 
\bentry
\word[virtuoso(1)]{virtuoso} 
\pron{vaTuyxR(cuR)OsoV}
\gl{\nA}(\bava \eng{virtuosi}
\expl{\ucAcx\ vaTuRyxOsiV, \engit{or}
 virtuosos}).  
\bmng
\bnum
\num{1} kalABijacnx; kalArasika; kalegaLa lakaSxNagaLanunx balalxvanu, kalABiruciyuLaLxvanu. 
\num{2} kalAvida; kalAkushala; lalitakaleyalilx (\kanmu\ saMgiVtadalilx) pAraMgatanAda vayxkitx. 
\enum
\emng
\eentry

\bentry
\word[virtuoso(2)]{virtuoso} 
\pron{vaTuyxR(cuR)OsoV} 
\gl{\gu}
\expl{}
\bmng
 kwshala parxdashaRka; kalAvidana pArxviVNayxvanunx toVrisuva, parxdashiRsuva. 
\emng
\eentry

\bentry
\word{virtuosoship} 
\pron{vaTuRyxOsoVSipf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{virtuosity}{virtuosity}. 
\emng
\eentry

\bentry
\word{virtuous} 
\pron{vaTuRyx(cuR)asf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} guNavaMta (nAda); sadugxNi; dhAmiRka; puNayxkara. 
\num{2} (heMgasina \vi) sAdhivxyAda; sacAcxritarxyXda; pativarxteyAda. 
\enum
\emng
\eentry

\bentry
\wordnospeech{virtuous circle}{virtuous circle}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 puNayxcakarx; kAyaRkAraNa cakarxda anukUlakaravAda punarAvataRna, BarxmaNa. 
\emng
\eentry

\bentry
\word{virtuously} 
\pron{vaTuyx(cuR)asfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} guNavaMtanAgi; sadugxNiyAgi; shiVlavaMtanAgi; dhamaRdiMda; puNayxkaravAgi. 
\num{2} (heMgasina \vi) shiVlavaMteyAgi; pativarxteyAgi. 
\enum
\emng
\eentry

\bentry 
\word{virtuousness} 
\pron{vaTuyx(cuR)asfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sadugxNashiVlate; dhamaRparate. 
\num{2} (heMgasina \vi) sacAcxritarxyx; pAtivarxtayx. 
\enum
\emng
\eentry

\bentry 
\word{virulence} 
\pron{viru(ruyx)lanfsx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} viSapUritate; viSamayate. 
\num{2} (roVgada \vi) ugarxte; tiVvarxte; viSamate; apAyakarate. 
\num{3} kaDu hagetana; tiVvarx devxVSa; ugarx veYSamayx. 
\enum
\emng
\eentry

\bentry
\word{virulent} 
\pron{viru(ruyx)laMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} viSapUrita; viSamaya. 
\num{2} (roVgada \vi) ugarxvAda; tiVvarxvAda; viSamavAda; ulabxNavAda; apAyakaravAda. 
\num{3} (\rUpa) ugarxvAda; tiVvarx; tiVkaSxNX; kaTu; GoVra; kaDu; kUrxra; dAruNa: \eng{virulent animosity} kaDu hage; ugarxdevxVSa. \eng{virulent tone of the letter} A kAgadada tiVkaSxNX sheYli, riVti. \eng{virulent abuse} ugarxniMde; birubeYguLa. 
\enum
\emng
\eentry

\bentry 
\word{virulently} 
\pron{viru(ruyx)laMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
 tiVvarx veYSamayxdiMda; ugarxvAgi; kaTuvAgi. 
\emng
\eentry

\bentry 
\word{virus} 
\pron{veYarasf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} veYrasf: muKayxvAgi nUyxkelxyikf Amalx matutx porxVTiVnf hodike iruva, aneVkaveVLe roVgakArakavAgiruva, jiVvaMta koVshagaLalilx mAtarx saMKeyx vaqdidhx mADuva, sUkaSx mxjiVvi. 
\alnum{b} (kaMpUyxTarf) kaMpUyxTarfnalilx saMgarxhisiruva mAhitiyanunx keDisalu kaLaLxtanadiMda seVrisalAda saMkeVta. 
\eanum
\numie
\num{2} (\pArxparx) rasike; naMju; viSa; roVgada mUla. 
\num{3} duSapxrXBAva; viSa; hAnikaravAda yA hALugeDahuva parxBAva. 
\enum
\emng
\eentry

\bentry
\wordnospeech{Vis.}{Vis.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{Viscount.} 
\emng
\eentry

\bentry
\word[visa(1)]{visa} 
\pron{viVsaZ} 
\gl{\nA}
\expl{}
\bmng
 viVsaZ; pAsfpoVTiRna yA parxvAsAdhikArapatarxda meVle vivaragaLu sariyAgiruvuveMdU, \kanmu\ adanunx paDediruva vayxkitx deVshavanunx biDalu yA parxveVshisalu anumati koDabahudeMdU heVLuva parxmANa vAkayx, SarA. 
\emng
\eentry

\bentry 
\word[visa(2)]{visa} 
\pron{viVsaZ} 
\gl{\sakirx}(vataRmAna \parxpu\ \Eva\ \eng{visas}; \BU\ matutx 
\BUkaq\ \eng{visaed} \ucAcx\ viVsaZDf yA \eng{visa'd;} \vakaq\\eng{visaing}). \bmng
 viVsaZ SarA hAku; viVsaZ muderx otutx. 
\emng
\eentry

\bentry
\word{visage} 
\pron{visiZjf} 
\gl{\nA}
\expl{}
\bmng
 (sAhitayxka) muKa; muKa -- BAva, muderx, lakaSxNa. 
\emng
\eentry

\bentry 
\word{visaged} 
\pron{visiZjfDx} 
\gl{\gu}
\expl{}
\bmng
muKavuLaLx yA muKaBAvavuLaLx; muKamuderxyuLaLx (\saMpa gaLalUlx saha \parx). 
\emng
\eentry

\bentry 
\wordf[vis-a-vis(1)]{vis-\`a-vis} 
\pron{viVsAZviV} 
\gl{\kirxvi}
\expl{\F}
\bmng
edurubadurAgi; muKAmuKiyAgi; parasapxra aBimuKavAgi. 
\emng
\eentry

\bentry 
\wordf[vis-a-vis(2)]{vis-\`a-vis} 
\pron{viVsAZviV} 
\gl{\upa}
\expl{\F\ }
\bmng
\bnum
\num{1} edurAgi. 
\num{2} saMbaMdhisidaMte. 
\enum
\emng
\eentry

\bentry 
\wordf[vis-a-vis(3)]{vis-\`a-vis} 
\pron{viVsAZviV} 
\gl{\nA}
\expl{\F}
\bmng
(\bava\ adeV). 
\bnum
\num{1} (\kanmu\ kelavu bageya naqtayxgaLalilx) aBimuKa vayxkitx; eduredurAgi, muKAmuKiyAgi natiRsuva saMgAti. 
\num{2} saMvAdi; beVroMdu taMDadalilx tanage saMvAdiyAda vayxkitx. 
\num{3} (\ame) (sAmAjika kUTagaLalilx joteyalilxruva) saMgAti; oDanADi; sahaBAgi. 
\enum
\emng
\eentry

\bentry 
\word{Visc.} 
\pron{}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{Viscount.} 
\emng
\eentry

\bentry
\word{viscacha} 
\pron{visfkAYxca} 
\gl{\nA}
\expl{}
\bmng
 visAkxYxca; bilatoVDuva, bAcihalilxna, lajiDiyamf kulada, belebALuva tupu pxLucamaRda, \da\ \ame da oMdu daMshaka. 
\emng
\eentry

\bentry 
\word{viscera} 
\pron{visara} 
\gl{\nA}
\expl{}
\bmng
 (\bava) 
\bnum
\num{1} (deVhada oLakuharagaLalilx, \udA\ miduLu, haqdaya, yakaqtutx, \mo vugaLalilx iruva) oLAMgagaLu; oLAvayavagaLu. 
\num{2} (\kanmu\ kibobxTeTxyalilxruva) karuLu \mo vu. 
\enum
\emng
\eentry

\bentry 
\word{visceral} 
\pron{visaralf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oLAvayavagaLa; oLAMgagaLa. 
\num{2} (parxjAcnxpUvaRkavAda takaRvAgirade) AMtarika yA aMtaraMgada BAvagaLige, oLaBAvagaLige -- saMbaMdhisida. 
\enum
\emng
\eentry

\bentry 
\wordnospeech{visceral nerve}{visceral nerve}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_s.pdf}{S}{sympathetic nerve}{sympathetic nerve}. 
\emng
\eentry

\bentry 
\word{viscid} 
\pron{visiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aMTuva; aMTaMTAda; goVMdinaMtha. 
\num{2} aredarxvAvasethxya; aredarxvavAgiruva. 
\enum
\emng
\eentry

\bentry
\word{viscidity} 
\pron{visiDiTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aMTaMTAgiruvike; shAyxnatavx; goVMdinaMtiruvike. 
\num{2} aredarxva sithxti. 
\enum
\emng
\eentry

\bentry
\word{viscometer} 
\pron{visAkxmiTarf} 
\gl{\nA}
\expl{}
\bmng
 shAyxnatavx, sinxgadhxtA -- mApaka; darxvagaLa aMTusithxtiyanunx aLeyalu baLasuva upakaraNa. 
\emng
\eentry

\bentry 
\word{viscometric} 
\pron{visakxmeTirxkf} 
\gl{\gu}
\expl{}
\bmng
 sinxgadhxtA mApakada; darxvagaLa aMTusithxtiyanunx aLeyuva. 
\emng
\eentry

\bentry
\word{viscometrically} 
\pron{visakxmeTirxkali} 
\gl{\kirxvi}
\expl{}
\bmng
 shAyxnatavx, sinxgadhxtA -- mApakadiMda yA mApakada parxkAra. 
\emng
\eentry

\bentry
\word{viscometry} 
\pron{visAkxmiTirx} 
\gl{\nA}
\expl{}
\bmng
 sinxgadhxtAmApana; sinxgadhxtAmApakavanunx baLasi sinxgadhxteyanunx aLeyuvudu. 
\emng
\eentry

\bentry
\word{viscose} 
\pron{visokxVsf(sfZ)} 
\gl{\nA}
\expl{}
\bmng
 visokxVsf: 
\banum
\alnum{a} seluyxloVsina sari; kaqtaka reVSemx tayArisalu upayoVgisuva, dAravAgi eLeyalu anukUlisuvaMte seluyxloVsanunx kAsiTxkf Alakxli matutx kAbaRnf DeYsaZlePxYDfnalilx karagisi tayArisalAda sari, sinxgadhx aMTudArxvaNa. 
\alnum{b} adariMda tayArisida kaqtaka reVSemx yA reVyAnf baTeTx. 
\eanum
\emng
\eentry


\bentry 
\word{viscosimeter} 
\pron{visakxsimiTarf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{viscometer}{viscometer}. 
\emng
\eentry

\bentry
\word{viscosity} 
\pron{visAkxsiTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (darxvada \vi) aMTuguNa; jiguTu; shAyxnatavx; sinxgadhxte; sulaBavAgi hariyadiruvike. 
\num{2} (\Bwvi) darxvada yA anilada AMtarika GaSaRNe, adariMdAguva parxvAha niroVdhakate yA adara aLate. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{dynamic viscosity} balAtamxka sinxgadhxte; AMtarika GaSaRNeyanunx hatitxkakxlu agatayxvAda balada aLate. 
\num{2}\eng{kinematic viscosity} gativeYjAcnxnika sinxgadhxte; oMdu mAna sAMdarxtege eSuTx balAtamxka sinxgadhxte eMbaudara aLate yA parimANa. 
\enum
\emng
\eentry

\bentry 
\word{viscount} 
\pron{veYkwMTf} 
\gl{\nA}
\expl{}
\bmng
 veYkwMTf; alfRgU bAyxranfgU naDuvaNa aMtasitxna birxTiSf variSaThx, shirxVmaMta. 
\emng
\eentry

\bentry 
\word{viscountcy} 
\pron{veYkwMTfsi} 
\gl{\nA}
\expl{(\bava\ \eng{viscountcies}). }
\bmng
 veYkwMTfgiri; veYkwMTfna padavi, birudu. 
\emng
\eentry

\bentry
\word{viscountess} 
\pron{veYkwMTisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} veYkwMTisf; veYkwMTana heMDati yA vidhave. 
\num{2} veYkwMTf birudu paDeda heMgusu. 
\enum
\emng
\eentry

\bentry 
\word{viscountship} 
\pron{veYkwMTfSipf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{viscountcy}{viscountcy}. 
\emng
\eentry

\bentry
\word{viscounty} 
\pron{veYkwMTi}
\gl{\nA}
\expl{(\bava\ \eng{viscounties}).}
\bmng
 (\ca) veYkwMTanige oLapaTaTx BUmikANi, pArxMta, adhikAra keSxVtarx. 
\emng
\eentry

\bentry
\word{viscous} 
\pron{visakxsf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aMTaMTAda; aMTuva; shAyxna. 
\num{2} aredarxva sithxtiya. 
\num{3} (\Bwvi) sinxgadhxte hecAcxgiruva; sulaBavAgi hariyadiruva; parxvAhaniroVdha guNavuLaLx. 
\enum
\emng
\eentry

\bentry
\word{viscously} 
\pron{visakxsfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aMTuvaMte; aMTaMTAgi. 
\num{2} aredarxvasithxtiyalilx. 
\num{3} (\Bwvi) adhika sinxgadhxteyiMda. 
\enum
\emng
\eentry

\bentry
\word{viscousness} 
\pron{visakxsfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aMTaMTAgiruvike. 
\num{2} aredarxvasithxti. 
\num{3} (\Bwvi) adhika sinxgadhxte. 
\enum
\emng
\eentry

\bentry
\word{viscus} 
\pron{visakxsf} 
\gl{\nA}
\expl{(\bava\ \eng{viscera} \ucAcx\ visara).}
\bmng
 (\sA\ \bava dalilx) midu oLAMga; deVhadoLage iruva yAvudeV miduvAda aMga. 
\emng
\eentry

\bentry 
\word[vise(1)]{vise} 
\pron{veYsf} 
\gl{\nA}
\expl{}
\bmng
 (\ame) = \hyperlink{vice(2)}{$^2$vice}. 
\emng
\eentry

\bentry
\word[vise(2)]{vise} 
\pron{veYsf} 
\gl{\sakirx}
\expl{}
\bmng
 (\ame)= \hyperlink{vice(3)}{$^3$vice}. 
\emng
\eentry

\bentry
\word{Vishnu} 
\pron{viSUnx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} viSuNx; veYSaNxvariMda paradeYvaveMdu BAvisalapxTaTx hiMdU deVvate. 
\num{2} barxhamx, viSuNx, shiva I tirxmUtiRgaLalilx eraDaneV deVvate. 
\enum
\emng
\eentry

\bentry
\word{Vishnuism} 
\pron{viSUnxisaZmf} 
\gl{\nA}
\expl{}
\bmng
 veYSaNxva sidAdhxMta; viSuNxveV paradeYvaveMba parxtipAdane. 
\emng
\eentry

\bentry
\word[Vishnuite(1)]{Vishnuite} 
\pron{viSUnxiTf} 
\gl{\nA}
\expl{}
\bmng
 viSuNxBakatx; veYSaNxva; viSuNxveV paradeYvaveMba sharxdedhxyuLaLxvanu. 
\emng
\eentry

\bentry 
\word[Vishnuite(2)]{Vishnuite} 
\pron{viSUnxiTf} 
\gl{\gu}
\expl{}
\bmng
 veYSaNxva; viSuNxveV paradeYvaveMba sharxdedhxyuLaLx. 
\emng
\eentry

\bentry
\word{visibility} 
\pron{visiZbiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kANuvaMtiruvike; daqshayxte; daqgogxVcaratavx. 
\num{2} beLakina parxmANa; vAtAvaraNada parisithxtigaLu \mo vananxvalaMbisidaMte daqshayxsAdhayxte matutx adara vAyxpitx. 
\enum
\emng
\eentry

\bentry
\word{visible} 
\pron{visiZbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaNiNxge kANuva; daqshayx; daqgogxVcara; parxtayxkaSx; goVcaravAda. (viduyxtAkxMta taraMgagaLa, beLakina \vi) daqgAvxyXpitxya; cAkuSxSa vAyxpitxya; cakuSxgArxRhayx; kaNiNxge saMveVdaneyuMTumADuva taraMgadUragaLa vAyxpitxyalilxruva. 
\num{3} kaNiNxge kANuvaMtha; vicArisi tiLiyabahudAda; gotitxruva; parxkaTavAda; parxtayxkaSxvAda; bahiraMgavAda: \eng{he had no visible means of support} Atanige kaNiNxge kANuvaMtha, jiVvanoVpAya yAvudU ilalx. \eng{he spoke with visible impatience} avanu parxkaTavAda asahaneyiMda mAtanADida. 
\num{4} (raPuZtx \mo vugaLa \vi) vAsatxva sarakugaLa; kaNiNxge kANuvaMtha sarakugaLanonxLagoMDa. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{the Church visible} kerxYsatx samudAya; parxtayxkaSxvAgi kANuva kerxYsatxdhamiRgaLa iDiV samudAya. 
\emng
\eentry

\bentry
\wordnospeech{visible horizon}{visible horizon}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_h.pdf}{H}{horizon pagu(6)}{horizon} (\pagu \eng{6}). 
\emng
\eentry

\bentry
\word{visibleness} 
\pron{visiZbflfnisf} 
\gl{\nA}
\expl{}
\bmng
 kANuvaMtiruvike; daqshayxte; goVcarate. 
\emng
\eentry

\bentry 
\word{visibly} 
\pron{visiZbilx} 
\gl{\kirxvi}
\expl{}
\bmng
 kANuvaMte; parxtayxkaSxvAgi; goVcaravAgi. 
\emng
\eentry

\bentry 
\word{Visigoth} 
\pron{visiZgAtf} 
\gl{\nA}
\expl{}
\bmng
 visiZgAtf; pashicxmada gAtf; \eng{5}riMda \eng{8}neV shatamAnada avadhiyalilx phArxnfsx matutx sepxVnf deVshagaLalilx nelesida gAtf guMpinavanu. 
\emng
\eentry

\bentry 
\word[vision(1)]{vision} 
\pron{viSaZnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} noVTa; dashaRna; daqSiTx; noVDuva shakitx yA kirxye. 
\num{2} sAvxpanx yA vashayxsAvxpada -- vayxkitx, vasutx; (kanasinalilx yA saMmoVhana supitxyalilx kaMDa) vayxkitx yA vasutx. 
\num{3} (alwkika yA BaviSayxvAdi dashaRnada) atimAnuSa -- biMba, CAyAkaqti. 
\num{4} kalapxne; kalapxnAcitarx; (kalapxneyalilx mUDibaMda) sapxSaTx, vishada -- toVkeR: \eng{the romantic visions of youth} ywvanada romAYxMTikf kalapxnegaLu. 
\num{5} loVkoVtatxra, asAmAnayx swMdayaRvuLaLx vayxkitx \mo varu. 
\num{6} sUkaSx mx aMtadaRqSiTx; pArxtiBa daqSiTx. 
\num{7} rAjatAMtirxka daqSiTx; rAjataMtarx kwshaladiMda kUDida dUradaqSiTx yA yoVjanegaLanunx rUpisuvudaralilx toVruva anAyxdaqsha viveVka. 
\num{8} (dUradashaRna Bititxyalilx kANuva) noVTa; dashaRna; daqshayx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{field of vision} daqSiTx -- keSxVtarx, vAyxpitx; yAvudeV dikikxnalilx kaNuNxgaLanunx hAyisidAga kANuva samagarxnoVTa. 
\emng
\eentry

\bentry 
\word[vision(2)]{vision} 
\pron{viSaZnf} 
\gl{\sakirx}
\expl{}
\bmng
 (kanasinalolxV kalapxneyalolxV kaMDaMte) 
\banum
\alnum{a} kANisu; kANuvaMte mADu; kaNaNxmuMde taru; kaNiNxge biVLisu; parxkaTisu. 
\alnum{b} noVDu; kANu; dashiRsu. 
\eanum
\emng
\eentry

\bentry
\word{visional} 
\pron{viSaZnalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kANekxya; alwkika dashaRnada. 
\num{2} kalapxneya; kAlapxnika; satayxvalalxda. 
\enum
\emng
\eentry

\bentry 
\word{visionariness} 
\pron{viSaZnarinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kalapxnA vihAra; kanasugArike; kalapxnAloVkadalilxruvike. 
\num{2} avayxvahAyaRvAda, kAyaRsAdhayxvalalxda -- yoVjanegaLanunx hoMdiruvike. 
\enum
\emng
\eentry

\bentry 
\word[visionary(1)]{visionary} 
\pron{viSaZnari} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kalapxnAvihAri; BAvanAjiVvi; avayxvahArayxvAda BAvanegaLanonxV, kalapxnegaLanonxV hoMdiruva yA kAlapxnika tatatxvXgaLalilx, sidAdhxMtagaLalilx OlADuva. 
\num{2} sAvxpanx yA kAlapxnika; BAvanAmAtarxda; savxpanxdalolxV kalapxneyalolxV mAtarx iruva. 
\num{3} (yoVjanegaLa, BAvanegaLa \vi) kalipxta; nijavalalxda; avAsatxvika; kAyaRsAdhayxvalalxda; avayxvahAyaRvAda; kAyaRkAriyalalxda. 
\enum
\emng
\eentry

\bentry 
\word[visionary(2)]{visionary} 
\pron{viSaZnari} 
\gl{\nA}
\expl{(\bava\ \eng{visionaries}). }
\bmng
 kalapxnAjiVvi; BAvanAjiVvi; kanasiga; kanasugAra; vayxvahAraparxjecnx ilalxda, kalapxnA parxpaMcadalelxV viharisuva AdashaRvAdi. 
\emng
\eentry

\bentry 
\word{visionist} 
\pron{viSaZnisfTx} 
\gl{\nA}
\expl{}
\bmng
 kanasugAra; BavayxvAda kalapxnegaLanunx kANuvavanu. 
\emng
\eentry

\bentry 
\word{visionless} 
\pron{viSaZnflisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaNuNxgaLilalxda; daqSiTxhiVna; aMdha; kuruDanAda. 
\num{2} dUradaqSiTx yA oLanoVTa ilalxda: \eng{visionless leader} dUradaqSiTxyilalxda nAyaka. 
\num{3} aMtaHsUPxtiR yA aMtaHperxVraNe ilalxda. 
\enum
\emng
\eentry

\bentry
\word{vision-mixer} 
\pron{viSaZnfmikasxrf} 
\gl{\nA}
\expl{}
\bmng
 daqshayxmisharxka; dUradashaRna parxsAradalilx yA dUradashaRnada citarxgaLanunx rekADfR mADikoLuLxvudaralilx oMdu biMbadiMda inonxMdu biMbakekx tirugisuva, badalAyisuva udoyxVgi. 
\emng
\eentry

\bentry
\word[visit(1)]{visit} 
\pron{visiZTf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{visited;} \vakaq}
\eng{visiting}). \bmng
\bnum
\num{1} (senxVhadiMda, gwrava toVrisalu yA mayARdegAgi, kAyARthaRvAgi, yAvudeV udedxVshakAkxgi yA sAvxthaRkAkxgi) noVDalu, kANalu --baru yA hoVgu; BeVTi -- mADu, koDu. 
\num{2} (adhikArada vatiyiMda parishiVlanegAgi, meVlivxcAraNegAgi, samAloVcanegAgi yA parisithxtiyanunx saripaDisuvudakAkxgi) noVDalu baru yA hoVgu; BeVTi mADu, koDu. 
\num{3} (roVga, vipatutx, \mo vugaLa \vi) baDi; tagulu; eragu: \eng{the whole village was visited by} (\engit{or} \eng{with) the plague} A iDiV haLiLxge pelxVgf baDiyitu, tagulitu. 
\numi{4} (\beY) 
\banum
\alnum{a} (vayxkitxyanunx, pApavanunx) daMDisu. 
\alnum{b} (pApakAkxgi vayxkitxge) shikeSxyanunx koDu. 
\eanum
\numie
\num{5} (\pArxparx) (mukitx \mo vanunx niVDi) (vayxkitxyanunx) saMtayisu; samAdhAna mADu; anugarxhisu; kApADu. 
\num{6} (obabx vayxkitxya jote tAtAkxlikavAgi oMdu sathxLadalilx) iru; taMgu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
BeVTiyAgu; BeVTigAranAgu. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{visit with} 
\banum
\alnum{a} (\ame) (vayxkitxyanunx, \kanmu\ senxVhadiMda) BeVTimADu; kANalu hoVgu 
\alnum{b} (vayxkitxyoDane) mAtanADu; haraTe hoDe; saMBASisu. 
\eanum
\emng
\eentry

\bentry
\word[visit(2)]{visit} 
\pron{visiZTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} BeVTi; (saM)dashaRna; hoVgi yA baMdu noVDuvudu, kANuvudu: \eng{during his second visit to the East} pwrasatxyX deVshagaLige avana eraDaneya BeVTiya kAladalilx. 
\num{2} (oMdu sathxLadalilx yA obabx vayxkitxyoDane) BeVTi; taMguvike; haMgAmi vAsa: \eng{he was on a visit to some friends} avanu kelavu mitarxroDane savxlapxkAla iralu BeVTi koTiTxdadx. \eng{I paid him a long visit} nAnu avanige BeVTikoTuTx diVGaRkAla avanoDanidedx. 
\num{3} (\ame) haraTe; saMBASaNe. 
\num{4} veYdayx, daMtaveYdayx, \mo varalilxge mADida BeVTi. 
\num{5} (aupacArikavAda yA adhikArada vatiyiMda, parishiVlane, taniKe, \mo\ udedxVshakAkxgi niVDida) BeVTi; aupacArika, adhikaqta -- BeVTi. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{right of visit = right of} \hyperlink{visitation pagu(2)}{visitation}. 
\emng
\eentry

\bentry
\word{visitable} 
\pron{visiZTbflf}
 \gl{\gu} \bmng
 (adhikArada vatiyiMda) saMdashaRniVya; BeVTimADalu takakx, yoVgayxvAda. 
\emng
\eentry

\bentry
\word[visitant(1)]{visitant} 
\pron{visiZTaMTf} 
\gl{\gu}
\expl{}
\bmng
 (\pArxparx, \kAparx) saMdashaRka; BeVTikoDuva; (saM)dashaRna koDuva; noVDalu baruva. 
\emng
\eentry

\bentry 
\word[visitant(2)]{visitant} 
\pron{visiZTaMTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} BeVTi hakikx; (yAvudeV sathxLakekx haMgAmiyAgi baMdu nelesiruva) valasepakiSx. 
\num{2} (\kanmu\ atimAnuSaneMdu naMbalAda) BeVTigAra. 
\enum
\emng
\eentry

\bentry
\word{visitation} 
\pron{visiZTeVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} pariVkASx BeVTi; taniKA saMdashaRna; taniKegAgi yA pariVkeSxgAgi BeVTi koDuvudu (\kanmu\ DayosiVsfge biSapapxna pariVkASx BeVTi). 
\num{2} (\AmA) ati diVGaRvAda (baMdhumitarxra) BeVTi. 
\num{3} taniKe BeVTi; matotxMdu rASaTxrXda haDagina savxrUpa, udedxVshagaLananxriyalu adanunx hatutxva BeVTi. 
\num{4} deYvadaMDaneyAgi parigaNitavAda -- toMdare yA kaSaTxnaSaTx. 
\numi{5} (\eng{Visitation}) meVri mAte saMdashaRna: 
\banum
\alnum{a} yeVsuvina tAyi meVri tananx tAyi elisaZbataLige niVDida divayxBeVTi(lUyxkf \eng{I , 39}). 
\alnum{b} adara sAmxrakavAgi juleY \eng{2}riMda Acarisuva hababx. 
\eanum
\numie
\num{6} (\pArxvi) pArxNigaLa BAri valase. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{Nuns of the Visitation} (\eng{1610}ralilx sAthxpitavAda, Iga \kanmu\ shikaSxNakeSxVtarxda seVvege saMbaMdhisida \roVkAyx) saMnAyxsini saMGa. 
\hypertarget{visitation pagu(2)}{} 
\num{2} \eng{right of visitation} BeVTi hakukx; paranwkA parxveVshAdhikAra; beVroMdu rASaTxrXda haDagina savxrUpa, udedxVshagaLananxriyalu adanunx hatutxva (Adare shoVdhisuva adhikAravilalxda) hakukx. 
\enum
\emng
\eentry

\bentry 
\word{visitatorial} 
\pron{visiZTaToVrialf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{visitorial}{visitorial}. 
\emng
\eentry

\bentry
\word[visiting(1)]{visiting} 
\pron{visiZTiMgf} 
\gl{\nA}
\expl{}
\bmng
BeVTi koDuvudu. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{be on visitatorial terms with} = \hyperlink{visiting pagu2}{?pagu? \((2)\)}. 
\hypertarget{visiting pagu2}{} 
\num{2}\eng{have a visitatorial acquaintance with} (yAranenxV Agali) BeVTi mADuvaSuTx paricayavuLaLxvanAgiru. 
\enum
\emng
\eentry

\bentry
\word[visiting(2)]{visiting} 
\pron{visiZTiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BeVTi mADutitxruva. 
\num{2} (vidAvxMsana \vi) saMdashaRka; beVroMdu vidAyxsaMsethxge BeVTi niVDiruva yA saMsethxyalilx savxlapx kAlaviruva. 
\enum
\emng
\eentry

\bentry
\word{visiting-book} 
\pron{visiZTiMgfbaukf} 
\gl{\nA}
\expl{}
\bmng
 (BeVTi mADabeVkAdavara yA BeVTimADuvavara hesarugaLanunx barediTuTxkoMDiruva) BeVTipusatxka; saMdashaRna pusatxka. 
\emng
\eentry

\bentry 
\word{visiting-card} 
\pron{visiZTiMgfkADfR} 
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_c.pdf}{C}{card(3)12}{$^3$card(12)}.
\emng
\eentry

\bentry 
\wordnospeech{visiting professor}{visiting professor}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 saMdashaRka pArxdhAyxpaka; atithi pArxdhAyxpaka; beVroMdu vishavxvidAyxnilayakekx AgiMdAgegx BeVTikoTuTx, adhAyxpana kAyaRgaLanunx nivaRhisuva pArxdhAyxpaka. 
\emng
\eentry

\bentry 
\word{visitor} 
\pron{visiZTarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (vayxkitxyanunx, sathxLavanunx noVDalu baruva yA hoVguva) BeVTigAra. 
\num{2} (oMdu vaSaRdalilx oMdu jAgadalilx savxlapx samaya idudxhoVguva) valase hakikx: \eng{winter visitor} caLigAlada valase hakikx. 
\num{3} (\birx) (kAleVju \mo vugaLalilx) (AgiMdAgeyx BeVTi koTuTx, taniKe naDesi, varadi salilxsuva adhikAravuLaLx) BeVTi adhikAri. 
\enum
\emng
\eentry
 
\bentry
\word{visitorial} 
\pron{visiZToVrialf} 
\gl{\gu}
\expl{}
\bmng
 (kAleVju \mo vugaLa \vi, AgiMdAgeyx taniKe naDesalu) BeVTi koDuva adhikAravuLaLx yA A adhikAriya. 
\emng
\eentry

\bentry
\wordnospeech{visitors' book}{visitors' book}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (cacuR, rAyaBAri kaceVri, hoVTelu, \mo vakekx baruvavaru tamamx hesaru viLAsagaLanunx, kelavomemx tamamx TiVkeTipapxNigaLanunx bareyuva) BeVTigArara pusatxka. 
\emng
\eentry

\bentry 
\word{visor} 
\pron{veYsaZrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} muKakavaca; muKatarx; mogakApu; siVsakada sarisalAguva muKaBAga. 
\num{2} cAcuToVpiya muMBAga. 
\num{3} (\ca) muKavADa; muKa maresuva sAdhana. 
\num{4} vAhanada meVlaNa cAcuPalaka; beVkilalxda beLakiniMda kaNuNxgaLanunx rakiSxsikoLaLxlu, (\kanmu\ vAhanada gALitaDeya meVle) calisadaMte BadarxvAgi yA sarisuvaMte hAkiruva cAcumare. 
\enum
\emng
\eentry

\bentry 
\word{visored} 
\pron{veYsaZDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} muKakavaca hAkikoMDiruva. 
\num{2} muMcAcikoMDiruva. 
\num{3} muKavADa hAkikoMDiruva. 
\num{4} (vAhanada \vi) cAcuPalakavuLaLx; gALi, beLaku, \mo vanunx taDeyuva cAcuPalakavuLaLx. 
\enum
\emng
\eentry

\bentry
\word{visorless} 
\pron{veYsaZrflisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} muKakavaca ilalxda. 
\num{2} muKavADa ilalxda. 
\num{3} (vAhanada \vi) cAcuPalakavilalxda. 
\enum
\emng
\eentry

\bentry 
\word{vista} 
\pron{visaTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (marasAlugaLa naDuve kANuvaMtha kiriyagalada) niVLanoVTa; diVGaRdaqshayx. 
\num{2} mAnasika -- noVTa, daqshayx; hinonxVTa yA munonxVTa; daqSiTxpatha; nenapu mADikoMDa GaTanegaLa yA muninxriVkiSxsida GaTanegaLa sudiVGaR paraMpare, sAlu, sherxVNi: \eng{it opened up new vistas or a new vista to his ambition} adu avana hebabxyakege, mahatAtxvXkAMkeSxge, oMdu hosa daqshayxvanunx tereyitu. 
\enum
\emng
\eentry

\bentry 
\word{vistaed} 
\pron{visaTxDf} 
\gl{\gu}
\expl{}
\bmng
 niVLanoVTagaLuLaLx; diVGaRdaqshayxgaLuLaLx. 
\emng
\eentry

\bentry 
\word[visual(1)]{visual} 
\pron{visuZyx(SuZZyx)alf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} cAkuSxSa: 
\banum
\alnum{a} daqSiTxya; noVTada. 
\alnum{b} daqSiTxge, noVTakekx saMbaMdhisida. 
\eanum
\numie
\num{2} daqSiTxgoVcara; kaNiNxge kANisuva. 
\num{3} noVDalu yA viVkaSxNadalilx baLasuva. 
\enum
\emng
\eentry

\bentry 
\word[visual(2)]{visual} 
\pron{visuZyx(SuZZyx)alf} 
\gl{\nA}
\expl{}
\bmng
 (\sA\ \bava dalilx) 
\bnum
\num{1} cAkuSxSa parxtime; citarx. 
\num{2} cAkuSxSa parxdashaRna; kaNiNxna eduru yA kaNiNxge kANuvaMte parxdashiRsuvudu, parxkaTisuvudu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{visual aid}{visual aid}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (shikaSxNakAyaRdalilx neravAgi baLasuva mAdari, calanacitarx, \mo) viVkaSxNa sAdhana. 
\emng
\eentry

\bentry 
\wordnospeech{visual angle}{visual angle} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (noVTakokxLapaTaTx vasutxvina tudigaLiMda baruva kiraNagaLiMda kaNiNxna baLi yA biMduvinalilx rUpugoLuLxva) viVkaSxNa, daqSiTx -- koVna. 
\emng
\eentry

\bentry 
\wordnospeech{visual display unit}{visual display unit} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (kaMpUyxTarf) dashaRka GaTaka; tereya meVle akaSxragaLanonxLagoMDa mAhitiyanunx mUDisuva, \sA\ kiVlikeYPalakaviruva, sAdhana. 
\emng
\eentry

\bentry
\wordnospeech{visual field}{visual field} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 daqSiTx, viVkaSxNa -- keSxVtarx; daqSiTxvAyxpitx. 
\emng
\eentry

\bentry 
\word{visualisation} 
\pron{visuZyx(SuZyx)aleYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{visualization}{visualization}. 
\emng
\eentry

\bentry 
\word{visualise} 
\pron{visuZyx(SuZYx)aleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{visualize}{visualize}. 
\emng
\eentry

\bentry
\word{visuality} 
\pron{viSuZAYxliTi}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} daqshayxte; daqgogxVcarate; kaNiNxge kANisuvudu. 
\num{2} manoVgoVcarate; manoVdaqshayxte; manasisxge kANisuvudu; mAnasika citarx. 
\enum
\emng
\eentry

\bentry
\word{visualizable} 
\pron{visuZyx(SuZYx)aleYsaZba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
 kANuvaMte mADabahudAda; daqshayxvAgisabahudAda; cAkuSxSiVkarisabahudAda. 
\emng
\eentry

\bentry
\word{visualization} 
\pron{visuZyx(SuZYx)aleYseZSanf} 
\gl{\nA}
\expl{}
\bmng
 daqshiyxVkaraNa: 
\banum
\alnum{a} cAkuSxSiVkaraNa; kaNiNxge kANuvaMte mADuvike. 
\alnum{b} manoVgoVcariVkaraNa; manasisxge goVcarisuvaMte, kANuvaMte mADuvike. 
\eanum
\emng
\eentry

\bentry 
\word{visualize} 
\pron{visuZyx(SuZYx)aleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 daqshiyxVkarisu: 
\banum
\alnum{a} daqgogxVcaravAgisu; cAkuSxSiVkarisu; kaNiNxge -- kANisu, kANuvaMte mADu. 
\alnum{b} goVcaragoLisu; (\kanmu\ kaNiNxge kANisadadxnunx) manoVgoVcaragoLisu; manasisxge kANuvaMte mADu. 
\alnum{c} (manasisxna BAva, kalapxne, \mo vugaLige) rUpa, AkAra -- koDu; citirxVkarisu. 
\alnum{d} (BAvisikoMDadadxnunx, hiMde noVDidadxnunx) kaNaNxmuMde taMduko; daqshayxvAgisiko; goVcaravAguvaMte mADiko; cAkuSxSiVkarisiko. 
\eanum
\emng
\eentry

\bentry
\word{visually} 
\pron{visuZyx(SuZYx)ali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} cAkuSxSavAgi; noVTadiMda; noVTakekx saMbaMdhisidaMte. 
\num{2} viVkaSxNeyiMda; viVkaSxNa vidhAnadiMda. 
\enum
\emng
\eentry

\bentry 
\wordnospeech{visual point}{visual point}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 daqgf, daqSiTx -- biMdu; daqSiTxkiraNagaLu saMdhisuva, oMdugUDuva, kiSxtijareVKeyalilxya biMdu. 
\emng
\eentry

\bentry 
\wordnospeech{visual purple}{visual purple}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 daqSiTx -- dhUmarx, dhUmala; kaNiNxna pApeyalilxruva beLakige sUkaSxmXgArxhiyAda neVriLe baNaNx. 
\emng
\eentry

\bentry 
\wordnospeech{visual ray}{visual ray}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\daqvi) daqSiTx, daqshayx -- kiraNa, rashimx; kaNiNxgU kaNiNxge kANisuva vasutxvigU saMbaMdha kalipxsuva gere, reVKe. 
\emng
\eentry

\bentry 
\word[vital(1)]{vital} 
\pron{veYT(Tf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} jeYvika; jiVvadhAraka; jiVvanAdhAravAda; badukige, badukuva kirxyege, jiVvakirxyege yA jiVvavAyxpArakekx saMbaMdhisida yA atAyxvashayxkavAda: \eng{vital energies} jeYvikashakitxgaLu; ceYtanayxgaLu. \eng{vital functions} jeYvika -- kirxyegaLu, vAyxpAragaLu. \eng{wounded in a vital part} mamaRsAthxnadalilx, jiVvashakitxyu atisUkaSxmXvAgiruva sathxLadalilx -- gAyagoMDa. 
\num{2} (yAvudeV vasutxvina uLivige yA keYgoMDiruva viSayakekx) pArxNAdhAravAda; atAyxvashayxkavAda; atimuKayxvAda: \eng{a vital question or question of vital importance} atimuKayxvAda parxshenx, viSaya. \eng{secrecy is vital to the success of the scheme} I yoVjaneya yashasisxge goVpayxte atimuKayx, atayxgatayx. 
\num{3} pArxNada, jiVvada -- meVle pariNAmavuLaLx. 
\num{4} mAraka; pArxNAMtaka; jiVvakekx, pArxNakekx yA yAvudeV kAyaRda jayakekx -- mArakavAda: \eng{a vital wound} mArakavAda gAya. \eng{a vital error} (jayakekx, saPalatege) mArakavAda tapu px, doVSa. 
\num{5} jiVvaMta; sajiVva; jiVvaMtikeyiMda kUDida, tuMbida; ceYtanayx, caTuvaTike -- tuMbida. 
\num{6} (vivAda parxyoVga) muKayx; parxdhAna. 
\enum
\emng
\eentry

\bentry 
\word[vital(2)]{vital} 
\pron{veYT(Tf)lf} 
\gl{\nA}
\expl{}
\bmng
 (\bava dalilx) mamaRsAthxnagaLu; pArxNAMgagaLu (\kanmu\ haqdaya matutx miduLu). 
\emng
\eentry

\bentry
\wordnospeech{vital capacity}{vital capacity}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 niHshAvxsaparxmANa; horabiTaTx usirina motatx; atayxMta diVGaRvAgi usiranenxLedukoMDa baLika horakekx biTaTx usirina, gALiya motatx. 
\emng
\eentry

\bentry 
\wordnospeech{vital force}{vital force}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} = \hyperref{kandict_e.pdf}{E}{elan vital}{elan vital}. 
\num{2} jiVvALa; atimuKayxvAda yAvudeV gUDhatatatxvX. 
\enum
\emng
\eentry

\bentry
\word{vitalisation} 
\pron{veYTaleYseZVSa(Sf)nf}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vitalization}{vitalization}. 
\emng
\eentry

\bentry
\word{vitalise} 
\pron{veYTaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vitalize}{vitalize}. 
\emng
\eentry

\bentry
\word{vitalism} 
\pron{veYTalisaZ(sfZ)} 
\gl{\nA}
\expl{}
\bmng
 (\jiVvi) (rAsAyanika ilalxve beVrAva bageya Bwtika shakitxgaLigiMta BinanxvAda jiVvatatatxvXdiMda pArxNoVtapxtitxyAguvudenunxva) jiVvatatatxvX vAda; pArxNatatatxvX vAda. 
\emng
\eentry

\bentry 
\word{vitalist} 
\pron{veYTalisfTx} 
\gl{\nA}
\expl{}
\bmng
jiVvatatatxvXvAdi. 
\emng
\eentry

\bentry 
\word{vitalistic} 
\pron{veYTalisiTxkf} 
\gl{\gu}
\expl{}
\bmng
jiVvatatatxvXvAdada. 
\emng
\eentry

\bentry
\word{vitality} 
\pron{veYTAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hurupu; ulAlxsa; utAsxha; jiVvaMtike; caTuvaTike. 
\num{2} jeYvikate; pArxNadhAraNa shakitx; jiVvashakitx; (jiVva)ceYtanayx. 
\num{3} (\rUpa) (saMsethx, BASe, \mo vugaLa \vi. ) jiVvashakitx; satatxvX; asitxtavxdalilxdudxkoMDu tananx kelasavanunx nivaRhisutAtx hoVguva shakitx. 
\enum
\emng
\eentry

\bentry
\word{vitalization} 
\pron{veYTaleYseZVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} jiVvadhAraNe; jiVvAdhAna; pArxNAdhAna; pArxNatuMbauvike. 
\num{2} jiVvaMtagoLisuvike; saceVtanagoLisuvike; ujijxVvana. 
\enum
\emng
\eentry

\bentry 
\word{vitalize} 
\pron{veYTaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} jiVva, pArxNa -- tuMbau, hoyuyx. 
\num{2} jiVvaMtagoLisu; saceVtanagoLisu; ujijxVvisu. 
\enum
\emng
\eentry

\bentry 
\word{vitally} 
\pron{veYTali} 
\gl{\kirxvi}
\expl{}
\bmng
 atimuKayxvAgi; atayxgatayxvAgi; uLivige, jayakekx -- atAyxvashayxkavAgi. 
\emng
\eentry

\bentry 
\wordnospeech{vital power}{vital power}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 jiVvadhAraka, pArxNadhAraka -- shakitx; jiVvavanunx uLisuva shakitx. 
\emng
\eentry

\bentry 
\wordnospeech{vital statistics}{vital statistics}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} janana, maraNa, vivAha, roVga, \mo vugaLa saMKeyx; janana maraNagaLa lekakxgaLu; jiVvana gaNati. 
\num{2} (\AmA) heMgasina -- muKAyxMga parxmANa; ede, soMTa matutx nitaMba parxdeVshagaLa aLate. 
\enum
\emng
\eentry

\bentry
\word{vitamin} 
\pron{vi(veY)Taminf} 
\gl{\nA}
\expl{}
\bmng
 viTaminunx; jiVvasatatxvX; anAnxMga; pArxNigaLa AroVgayxkekx hAgU beLevaNigege atayxgatayxviruva, sAmAnayxvAgi AhArada mUlaka pArxNiya deVhakekx sarabarAjAguva halavAru kAbaRnika saMyukatxgaLalilx oMdu. 
\emng
\eentry

\bentry 
\wordnospeech{vitamin A}{vitamin A}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_r.pdf}{R}{retinol}{retinol}. 
\emng
\eentry

\bentry 
\wordnospeech{vitamin B}{vitamin B}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 bi viTaminf; deVhadalilx kelasa mADuva kelavu enfjeYmf kAyARcaraNege agatayxvAgiruva, niVrinalilx karaguva thayaminf, ribophelxvinf, piriDAkisxnf, sayanokobAlaminf muMtAda rAsAyanikavAgi parasapxra saMbaMdhisilalxdidadxrU halavomemx oMdeV AhAradalilx oTiTxge kANasiguva kAbaRnika saMyukatxgaLa guMpinalilx oMdu. (\eng{B} \eng{vitamins} eMdU \parx) 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1}\eng{vitamin B1} = \hyperref{kandict_t.pdf}{T}{thiamine}{thiamine}. 
\num{2}\eng{vitamin B2} = \hyperref{kandict_r.pdf}{R}{riboflavin}{riboflavin}. 
\num{3}\eng{vitamin B6} = \hyperref{kandict_p.pdf}{P}{pyridoxine}{pyridoxine}. 
\num{4}\eng{vitamin B12} pArxNijanayx AhAravAda pitatxjanakAMga, miVnu matutx moTeTxgaLalilx kANbaruva, bi kAMpelxkfsx veYTaminf guMpige seVrida oMdu jiVvasatatxvX (idara korateyiMda parxbalavAda rakatxhiVnate uMTAgutatxde). 
\enum
\emng
\eentry

\bentry
\wordnospeech{vitamin C}{vitamin C}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 si viTaminf; asAkxbiRkf Amalx eMba niVrinalilx karaguva, kAyisidare viBajisuva, deVhadalilx idara korateyAdare sakxviR roVgakekx kAraNavAguva viTaminf. 
\emng
\eentry

\bentry
\wordnospeech{vitamin D}{vitamin D}
\pron{?}
\gl{\nA}
\expl{}
\bmng
Di viTaminf; pArxNigaLa yakaqtitxnalilx matutx miVneNeNxgaLalilx sikukxva, namamx deVhavu tanage agatayxvAda kAyxlisxyamamxnunx meYgUDisikoLaLxlu, matutx makakxLalilx rikeTfsx roVgavanunx hAgU doDaDxvaralilx asithxmaqdutavx \eng{(osteomalacia)} vanunx taDeyalu nAvu AhArada mUlaka agatayxvAgi seVvisabeVkAda oMdu viTaminf. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1}\eng{vitamin D2} = \hyperref{kandict_c.pdf}{C}{calciferol}{calciferol}. 
\num{2}\eng{vitamin D3} (\jiVra) kAlikAYxlisxpharAlf; mULegaLalilx kAyxlisxyamf saMgarxhavAgalu beVkAda saMyukatx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vitamin E}{vitamin E}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_t.pdf}{T}{tocopherol}{tocopherol}. 
\emng
\eentry

\bentry
\wordnospeech{vitamin H}{vitamin H}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 bayoVTinf; moTeTxya haLadi baMDAra, yakaqtutx, matutx yiVsfTxgaLalilxruva bi kAMpelxkfsx viTaminf. 
\emng
\eentry

\bentry
\word{vitaminise} 
\pron{viTamineYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vitaminize}{vitaminize}. 
\emng
\eentry

\bentry
\word{vitaminize} 
\pron{viTamineYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 viTaminunxgaLanunx tuMbau, seVrisu. 
\emng
\eentry

\bentry
\wordnospeech{vitamin K}{vitamin K}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 ke viTaminf; hasirelegaLalilx sikukxva, namage rakatxsArxvavAdAga rakatxvu kUDaleV garaNegaTuTxvaMtAgalu nAvu AhArada mUlaka agatayxvAgi seVvisabeVkAda oMdu viTaminunx. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1}\eng{vitamin K1} = \hyperref{kandict_p.pdf}{P}{phylloquinone}{phylloquinone}. 
\num{2}\eng{vitamin K2} = \hyperref{kandict_m.pdf}{M}{menaquinone}{menaquinone}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vitamin M}{vitamin M}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ \ame) = \hyperref{kandict_f.pdf}{F}{folic acid}{folic acid}. 
\emng
\eentry

\bentry
\word{vitellary} 
\pron{vi(veY)Telari} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} moTeTxya oLagina janeyaMtha, haLadi loVLeyaMtha yA adakekx saMbaMdhisida. 
\num{2} aMDAshayada jiVvadarxvayxda yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry 
\word{vitelli} 
\pron{vi(veY)TeleY} 
\gl{\nA}
\expl{}
\bmng
 \eng{vitellus} padada \bava\ rUpaka. 
\emng
\eentry

\bentry
\word{vitellin} 
\pron{vi(veY)Talinf} 
\gl{\nA}
\expl{}
\bmng
 (\ravi) aMDasatatxvX; veYTalinf; moTeTxya haLadiloVLeyalilxya muKayx sasArajanaka vasutx. 
\emng
\eentry

\bentry
\word{vitelline} 
\pron{vi(veY)TaleY(li)nf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} moTeTxya oLagina janeyaMtha, haLadi loVLeya. 
\num{2} aMDAshayada jiVvadarxvayxda. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vitelline membrane}{vitelline membrane}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (moTeTxya oLagiruva janeya sutatxlU iruva) jane -- pore, ciVla, koVsha. 
\num{2} BUrxNAvaraNa; BUrxNakoVsha. 
\enum
\emng
\eentry

\bentry
\word{vitellus} 
\pron{vi(veY)Talasf} 
\gl{\nA}
\expl{(\bava\ \eng{vitelli} \ucAcx\ vi(veY)TaleY). }
\bmng
\bnum
\num{1} jane; moTeTxya oLagina haLadi loVLe. 
\num{2} aMDapiVtadarxvayx; aMDAshayada jiVvadarxvayx, aMkuradarxvayx. 
\enum
\emng
\eentry

\bentry 
\word{viti-} 
\pron{vi(veY)Ti\eng{-}} 
\gl{\sapUpa}
\expl{}
\bmng
 dArxkiSx, dArxkiSx baLiLx eMbathaRgaLa \sapUpa 
\emng
\eentry

\bentry 
\word{vitiate} 
\pron{viSiETf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (guNavanunx, dakaSxteyanunx) keDisu; kulageDisu; hALumADu; kaluSitagoLisu; viSamaya, malina, dUSita -- goLisu; doVSayukatx mADu: \eng{person's constitution vitiated by intemperance} BoVgaloVlupateyiMda vayxkitxya kulageTuTxhoVgiruva deVhasithxti. \eng{vitiated air} keTaTx gALi; malinagoLisalapxTaTx vAyu. \eng{vitiated blood} viSamayagoLisalapxTaTx rakatx. \eng{vitiated mind} kaluSita manasusx. \eng{vitiated judg(e)ment} doVSayukatx viveVcane. 
\num{2} UjiRtavAgadaMte mADu; nirathaRkagoLisu; vayxthaRgoLisu: \eng{a single word may vitiate a contract} oMdeV oMdu mAtu oMdu karAranenxV anUjiRtagoLisabahudu, nirathaRkagoLisabahudu. 
\enum
\emng
\eentry

\bentry 
\word{vitiation} 
\pron{viSiESanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kulageDisuvike; hALu mADuvike; kaluSitagoLisuvike; malinagoLisuvike. 
\num{2} anUjiRta, nirathaRka -- goLisuvike. 
\enum
\emng
\eentry

\bentry 
\word{vitiator} 
\pron{viSiETarf} 
\gl{\nA}
\expl{}
\bmng
 kulageVDi; kulageDisuva vayxkitx yA vasutx. 
\emng
\eentry

\bentry 
\word{viticulture} 
\pron{viTikalacxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} dArxkiSx -- beVsAya, kaqSi; dArxkiSxbaLiLxgaLa vayxvasAya mADuvudu. 
\num{2} dArxkiSxkaqSi vijAcnxna yA adara adhayxyana. 
\enum
\emng
\eentry

\bentry
\word{vitreous} 
\pron{viTirxasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gAjina (savxBAvada). 
\num{2} (kAThinayx, Bidurate, pAradashaRkate, racane, \mo vugaLalilx) gAjinaMtha; gAjanunx hoVluva: \eng{vitreous enamel} gAjinaMtha inAYxmalf. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vitreous body}{vitreous body}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\aMrashA) (kaNuNxguDeDxyalilx tuMbiruva pAradashaRkavAda) kAcadarxva; gAjinaMtha darxva; pAradashaRkavAda, loVLeyaMtha aMgAMsha. 
\emng
\eentry

\bentry 
\wordnospeech{vitreous humour}{vitreous humour}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vitreous body}{vitreous body}. 
\emng
\eentry

\bentry
\word{vitreousness} 
\pron{viTirxasfnisf} 
\gl{\nA}
\expl{}
\bmng
 kAcatavx; gAjinaMtiruvike. 
\emng
\eentry

\bentry
\word{vitrescence} 
\pron{viTirxsanfsx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kAciVBavana; gAjAguvike; gAjina rUpa tALuvudu. 
\num{2} kAcatavx, gAjutana -- hoMdiruvike. 
\enum
\emng
\eentry

\bentry 
\word{vitrescent} 
\pron{viTirxsa(sf)MTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kAcaparxvaqtitxya; gAjAguva. 
\num{2} gAjinaMtha. 
\num{3} kAciVBUta; gAjAda; gAjina rUpa tALida. 
\enum
\emng
\eentry

\bentry 
\word{vitrifaction} 
\pron{viTirxphAYxkfSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vitrification}{vitrification}. 
\emng
\eentry

\bentry
\word{vitrifiable} 
\pron{viTirxpheYabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 kAciVkarisalAguva; gAjanAnxgi mADabahudAda. 
\emng
\eentry

\bentry 
\word{vitrification} 
\pron{viTirxphikeVSanf} 
\gl{\nA}
\expl{}
\bmng
 kAciVkaraNa; gAjanAnxgi mADuvudu. 
\emng
\eentry

\bentry
\word{vitriform} 
\pron{viTirxphAmfR} 
\gl{\gu}
\expl{}
\bmng
 kAcarUpada; gAjinaMte kANuva. 
\emng
\eentry

\bentry 
\word{vitrify} 
\pron{viTirxpheY} 
\gl{\akirx}
\expl{}
\bmng
 (\kanmu\ shAKadiMda) gAjanAnxgi yA gAjinaMtha vasutxvanAnxgi mADu; kAciVkarisu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 (\kanmu\ shAKadiMda) gAjAgu yA gAjinaMtha vasutxvAgu; kAciVBavisu. 
\emng
\eentry

\bentry 
\word{vitriol} 
\pron{viTirxalf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} salUPxZyXrikf Amalx yA adara salePxZVTf; gaMdhakAmalx yA adara lavaNa. 
\num{2} kaTuTiVke; birunuDi; tiVkaSxNX nuDi; kaTuBAvane. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{blue} (\engit{or} \eng{copper) vitriol} tAmarxda salePxZVTf; meYlututatx. 
\num{2} \eng{green vitriol} kabibxNada (hasuru)saleZPxVTf. 
\num{3} \eng{oil of vitriol} parxbala yA sAMdirxVkarisida salUPxZyXrikf Amalx, gaMdhakAmalx. 
\num{4} \eng{white vitriol} satuvina (biLi) salePxZVTf. 
\enum
\emng
\eentry

\bentry
\word{vitriolic} 
\pron{viTirxAlikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxbala salUZPxZyxrikf yA gaMdhaka Amalxda yA adaraMtha. 
\num{2} (BASaNa yA TiVkeya \vi) kaTuvAda; birusAda; ugarxvAda; tiVkaSxNXvAda. 
\enum
\emng
\eentry

\bentry 
\word{vitriolize} 
\pron{viTirxaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} parxbala salUPxZyxrikf AmalxvanAnxgi mADu. 
\num{2} parxbala salUPxZyxrikf AmalxdiMda suDu, suTuTx gAyagoLisu. 
\enum
\emng
\eentry

\bentry 
\wordRemoveSpace{Vitruvian-scroll}{Vitruvian scroll} 
\pron{viTUrxvianf sokxrXVlf} 
\gl{\nA}
\expl{}
\bmng
 (kAniRVsf \mo vugaLa alaMkaraNadalilx baLasuva) suruLi namUne. 
\emng
\eentry

\bentry 
\word{vitta} 
\pron{viT} 
\gl{\nA}(\bava\ \eng{vittae}
\expl{\ucAcx\ viTiV) }
\bmng
\bnum
\num{1} (\savi) gejajxri baLagada kelavu sasayxgaLa haNiNxnalilxruva teYlanALa. 
\num{2} (\pArxvi) baNaNxda paTeTx. 
\enum
\emng
\eentry

\bentry 
\word{vittate} 
\pron{viTeVTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\savi). teYlanALagaLuLaLx. 
\num{2} (\pArxvi) baNaNxda paTeTxgaLuLaLx. 
\enum
\emng
\eentry

\bentry
\word{vituperate} 
\pron{viTUyxpareVTf} 
\gl{\sakirx}
\expl{}
\bmng
 (yAranenxV Agali) bayuyx; niMdisu; dUSisu; beYguLu suri (\akirx\ saha). 
\emng
\eentry

\bentry 
\word{vituperation} 
\pron{viTUyxpareVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
 beYguLa; bayuyxvike; niMdane; dUSaNe. 
\emng
\eentry

\bentry 
\word{vituperative} 
\pron{viTUyxparaTivf} 
\gl{\gu}
\expl{}
\bmng
 beYguLiMda kUDida; niMdAtamxka; dUSaNeya. 
\emng
\eentry

\bentry 
\word{vituperator} 
\pron{viTUyxpareVTarf} 
\gl{\nA}
\expl{}
\bmng
 bayuyxvavanu; niMdaka; dUSaka. 
\emng
\eentry

\bentry
\wordf[viva(1)]{viva} 
\pron{viVva} 
\gl{\BAavayx}
\expl{\It\ }
\bmng
niDugAla, cirakAla -- bALali!; bahukAla bALegx! 
\emng
\eentry

\bentry 
\wordf[viva(2)]{viva} 
\pron{viVva} 
\gl{\nA}
\expl{\It}
\bmng
 `niDugAla, cirakAla bALali'! eMba udAgxra, hAreYke. 
\emng
\eentry

\bentry 
\word[viva(3)]{viva} 
\pron{veYva} 
\gl{\nA}
\expl{}
\bmng
 (\birx) (\AmA) = \hyperlink{viva voce(3)}{$^3$viva voce}. 
\emng
\eentry

\bentry
\word[viva(4)]{viva} 
\pron{veYva} 
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{vivas;} \BU\ matutx}
\BUkaq\ \eng{vivaed} \ucAcx\ veYvaDf yA \eng{viva'd;} \vakaq\\eng{vivaing}). \bmng
(\birx, \AmA) = \hyperlink{viva-voce(1)}{$^4$viva-voce}. 
\emng
\eentry

\bentry 
\word{vivace} 
\pron{vivAci} 
\gl{\kirxvi}
\expl{}
\bmng
 (\saM) geluvu, utAsxhagaLa durxtagatiyiMda; KuSi hurupiniMda. 
\emng
\eentry

\bentry 
\word{vivacious} 
\pron{viveVSasf} 
\gl{\gu}
\expl{}
\bmng
 geluvAda; utAsxhadiMda kUDida; ulAlxsaBaritavAda; ceYtanayx cimumxtitxruva. 
\emng
\eentry

\bentry 
\word{vivaciously} 
\pron{viveVSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 gelaviniMda; KuSiyiMda; utAsxhadiMda; ulAlxsadiMda. 
\emng
\eentry

\bentry
\word{vivaciousness} 
\pron{viveVSasfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vivacity}{vivacity}. 
\emng
\eentry

\bentry
\word{vivacity} 
\pron{vivAYxsiTi} 
\gl{\nA}
\expl{}
\bmng
 gelavu; KuSi; utAsxhapUNaRte; ulAlxsaBaritate. 
\emng
\eentry

\bentry 
\word{vivarium} 
\pron{viveVriamf} 
\gl{\nA}
\expl{(\bava\ \eng{vivaria} \ucAcx\ viveVria).}
\bmng
 pArxNidhAma; pArxNigaLanunx avugaLa sahaja, sAvxBAvika -- sithxtiyalilx iDalikAkxgi kaqtakavAgi kalipxsida sathxLa. 
\emng
\eentry

\bentry 
\word[vivat(1)]{vivat} 
\pron{viV(veY)vAYxTf} 
\gl{\BAavayx}
\bmng
 = \hyperlink{viva(1)}{$^1$viva}. 
\emng
\eentry

\bentry
\word[vivat(2)]{vivat} 
\pron{viV(veY)vAYxTf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{viva(2)}{$^2$viva}. 
\emng
\eentry

\bentry
\wordspecial{viva voce}{1}{1}{\hyperlink{viva voce(2)}{\textcolor{superscript}{$^2$}\eng{viva voce}}\quad
\hyperlink{viva voce(3)}{\textcolor{superscript}{$^3$}\eng{viva voce}}\quad\hyperlink{viva-voce(1)}{\textcolor{superscript}{$^4$}\eng{viva-voce}}\quad}\pron{veYvavavoVsi(ci)} 
\gl{\kirxvi}
\expl{}
\bmng
 bAyimAtinalilx; vAcA; mwKikavAgi. 
\emng
\eentry

\bentry
\wordspecial{viva voce}{2}{2}{\hyperlink{viva voce(1)}{\textcolor{superscript}{$^1$}\eng{viva voce}}\quad
\hyperlink{viva voce(3)}{\textcolor{superscript}{$^3$}\eng{viva voce}}\quad\hyperlink{viva-voce(1)}{\textcolor{superscript}{$^4$}\eng{viva-voce}}\quad} \pron{veYvavoVsi(ci)} 
\gl{\gu}
\expl{}
\bmng
 bAyimAtina; vAcA; mwKika. 
\emng
\eentry

\bentry 
\wordspecial{viva voce}{3}{3}{\hyperlink{viva voce(1)}{\textcolor{superscript}{$^1$}\eng{viva voce}}\quad
\hyperlink{viva voce(2)}{\textcolor{superscript}{$^2$}\eng{viva voce}}\quad\hyperlink{viva-voce(1)}{\textcolor{superscript}{$^4$}\eng{viva-voce}}\quad}\pron{veYvavoVsi(ci)} 
\gl{\nA}
\expl{}
\bmng
 (vishavxvidAyxnilaya, kAleVju, \mo vugaLa) vAkapxriVkeSx; bAyimAtina, mwKika -- pariVkeSx. 
\emng
\eentry

\bentry 
\wordspecial{viva-voce}{1}{4}{\hyperlink{viva voce(1)}{\textcolor{superscript}{$^1$}\eng{viva voce}}\quad
\hyperlink{viva voce(2)}{\textcolor{superscript}{$^2$}\eng{viva voce}}\quad\hyperlink{viva voce(3)}{\textcolor{superscript}{$^3$}\eng{viva voce}}\quad} \pron{veYvavoVsi(ci)} 
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{viva-vocees}; \BU\ matutx \BUkaq\ \eng{viva-voceed;} \vakaq\ \eng{viva-voceing}). }
\bmng
bAyimAtiniMda, mwKika -- pariVkeSx mADu; vAkapxriVkeSx mADu. 
\emng
\eentry

\bentry 
\word{vivax} 
\pron{veYvAYxkfsx} 
\gl{\nA}
\expl{}
\bmng
 (dinabiTuTx dinada yA mUru dinakokxmemxya, javxra taruva) vAyide javxrada, pAlxYxsAmxDiyaM veYvAYxkfsx kulada, maleVriya parataMtarx jiVvi. 
\emng
\eentry

\bentry 
\word[viverrid(1)]{viverrid} 
\pron{vi(veY)veriDf} 
\gl{\gu}
\expl{}
\bmng
 punugu bekikxna vaMshada yA adara upavaMshada. 
\emng
\eentry

\bentry 
\word[viverrid(2)]{viverrid} 
\pron{vi(veY)veriDf} 
\gl{\nA}
\expl{}
\bmng
 vivariDeV vaMshakekx seVrida punugu bekukx, muMgusi, \mo\ pArxNi. 
\emng
\eentry

\bentry
\word{vivers} 
\pron{veYvasfZR} 
\gl{\nA}
\expl{}
\bmng
 (\bava) (sAkxTelxMDf) 
\bnum
\num{1} AhAra; ananx; tiMDi. 
\num{2} AhAra sAmagirx. 
\enum
\emng
\eentry

\bentry 
\word{vivid} 
\pron{viviDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (beLaku, baNaNx -- ivugaLa \vi) ujajxvXlavAda; parxkAshamAnavAda; edudxkANuva; kaDu: \eng{a vivid flash of lightning} miMcina ujajxvXlavAda hoLapu. \eng{a vivid green} kaDuhasuru. 
\num{2} (vayxkitxya \vi) lavalavikeya; ulAlxsaBaritanAda; shakitxshAliyAda. 
\num{3} (manashashxkitx, manasisxna meVlAda pariNAma, vaNaRne -- ivugaLa \vi) sapxSaTx(vAda); vishada; suPxTa; suvayxkatx; kaNiNxge kaTuTxvaMtha: \eng{he has a vivid imagination} avanige vishadavAda kalapxne ide. \eng{a vivid description} sapxSaTx vaNaRne. \eng{I have a vivid recollection of the scene} nanage A daqshayxda sapxSaTxvAda nenapide. 
\enum
\emng
\eentry

\bentry 
\word{vividly} 
\pron{viviDfli} 
\gl{\kirxvi}
\expl{}
\bmng
 sapxSaTxvAgi; suPxTavAgi; vishadavAgi. 
\emng
\eentry

\bentry 
\word{vividness} 
\pron{viviDfnisf} 
\gl{\nA}
\expl{}
\bmng
 sapxSaTxte; suPxTate; vishadate; veYshadayx. 
\emng
\eentry

\bentry
\word{vivification} 
\pron{viviphikeVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
 jiVvaMtavAgisuvike; ujijxVvana; jiVva, pArxNa, lavalavike, caTuvaTike -- tuMbauvike. 
\emng
\eentry

\bentry
\word{vivify} 
\pron{vivipheY} 
\gl{\akirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{vivifies;} \BU\ matutx}
\BUkaq\ \eng{vivified).} \bmng
 jiVva -- hoyuyx, tuMbau; jiVvaMtagoLisu; ceYtanayxgoLisu; ujijxVvisu; lavalavikegoLisu (\rUpa\ saha). 
\emng
\eentry

\bentry 
\word{viviparity} 
\pron{vivipAYxriTi} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{viviparousness}{viviparousness}. 
\emng
\eentry

\bentry 
\word{viviparous} 
\pron{veY(vi)viparasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\pArxvi) jarAyuja; marihAkuva; tatitxyaninxDade neVravAgi sasatxnigaLaMte mari hAkuva. 
\num{2} (\savi) kaMdu biDuva; aMkurajani; mUla sasayxkekx aMTikoMDiruvAgaleV moLake oDeyuvaMtha geDeDx yA biVjavanunx utapxtitx mADuva. 
\enum
\emng
\eentry

\bentry 
\word{viviparously} 
\pron{veY(vi)viparasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (\pArxvi) jarAyujavAgi. 
\num{2} (\savi) aMkurajaniyAgi. 
\enum
\emng
\eentry

\bentry
\word{viviparousness} 
\pron{veY(vi)viparasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\pArxvi) jarAyujate(tavx); tatitxyaninxDade neVravAgi mari hAkuvike. 
\num{2} (\savi) aMkurajanakate; kaMdubiDuvike. 
\enum
\emng
\eentry

\bentry
\word{vivisect} 
\pron{vivisekfTx} 
\gl{\akirx}
\expl{}
\bmng
 sajiVvacePxVda mADu; jiVvaMta sithxtiyalelxV pArxNiya deVhavanunx, aMgAMgagaLanunx CeVdisu, katatxrisu. 
\emng
\eentry

\bentry 
\word{vivisection} 
\pron{vivisekfSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sajiVvacePxVdana; veYjAcnxnika saMshoVdhanegAgi jiVvigaLa deVhavanunx jiVvaMtasithxtiyalelxV CeVdisuvudu, katatxrisuvudu. 
\num{2} kaTu TiVke; nidARkiSxNayx KaMDane; agatayxkikxMta hecucx vivaravAgi yA dayAdAkiSxNayxvilalxde CidarxCidarxvAgi KaMDisuvudu. 
\enum
\emng
\eentry

\bentry 
\word{vivisectional} 
\pron{vivisekfSanalf} 
\gl{\gu}
\expl{}
\bmng
 sajiVvaCeVdanada. 
\emng
\eentry

\bentry 
\word{vivisectionist} 
\pron{vivisekfSanisfTx} 
\gl{\nA}
\expl{}
\bmng
 sajiVvaCeVdana vAdi; veYjAcnxnika saMshoVdhanegAgi jiVvaMtasithxtiyalelxV pArxNigaLa deVhavanunx katatxrisabahudeMdu parxtipAdisuvavanu. 
\emng
\eentry

\bentry
\word{vivisector} 
\pron{vivisekaTxrf} 
\gl{\nA}
\expl{}
\bmng
 sajiVvaCeVdaka; saMshoVdhanegAgi jiVvaMtasithxtiyalelxV pArxNigaLa deVhavanunx katatxrisuvavanu. 
\emng
\eentry

\bentry 
\word{vivo} 
\pron{viVvoV} 
\gl{\kirxvi}
\expl{}
\bmng
 (\saM) gelavAgi; utAsxhadiMda; KuSiyiMda. 
\emng
\eentry

\bentry
\word{vixen} 
\pron{vikfsa(sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} heNuNx nari. 
\num{2} jagaLagaMTi (heMgasu); gaMDubiVri; bajAri; tATagititx; gayAyxLi. 
\enum
\emng
\eentry

\bentry
\word{vixenish} 
\pron{vikasxniSf} 
\gl{\gu}
\expl{}
\bmng
 (heMgusina \vi) gaMDubiVritanada; jagaLagaMTi savxBAvada. 
\emng
\eentry

\bentry 
\word{vixenly} 
\pron{vikasxnfli} 
\gl{\kirxvi}
\expl{}
\bmng
 (heMgusina \vi) jagaLagaMTiyaMte; gaMDubiVriyaMte; bajAriyaMte; gayAyxLiyaMte. 
\emng
\eentry

\bentry 
\word{viz} 
\pron{visfZ} 
\gl{\kirxvi}
\expl{}
\bmng
 eMdare; hAgeMdare; beVre mAtinalilx heVLuvudAdare: \eng{under the following conditions, viz.} muMde heVLuva SaratutxgaLa parxkAra, eMdare. . . \eng{a permanent board of three, viz.} mUru janara oMdu kAyaMmaMDali, eMdare,. . . 
\emng
\eentry

\bentry 
\word{vizard} 
\pron{visaZDfR} 
\gl{\nA}
\expl{}
\bmng
 (\pArxparx) 
\bnum
\num{1} muKavADa; muKamarasu. 
\num{2} veVSa. 
\enum
\emng
\eentry

\bentry
\word{vizcacha} 
\pron{visfZkAYxca} 
\gl{\nA}
\expl{}
\bmng
 \eng{viscacha} padada rUpAMtara. 
\emng
\eentry

\bentry 
\word{vizier} 
\pron{visiZarf} 
\gl{\nA}
\expl{}
\bmng
 (\ca) vajiVra; kelavu musalAmxna rASaTxrXgaLalilx, (\kanmu\ tukiRyalilx ATamanf rAjavaMshada keYkeLagina) unanxtAdhikAri. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{grand vizier} (hiMde TakiRyalilx matutx itara musilxMdeVshagaLalilx) parxdhAnamaMtirx. 
\emng
\eentry

\bentry 
\word{vizierate} 
\pron{visiZaraTf} 
\gl{\nA}
\expl{}
\bmng
 vajiVra padavi yA adhikArAvadhi yA adhikArakokxLapaTaTx parxdeVsha. 
\emng
\eentry

\bentry
\word{vizierial} 
\pron{visiZarialf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vajiVrana yA vajiVranige saMbaMdhisida. 
\num{2} (kAgadapatarx, hukuM, \mo vugaLa \vi) vajiVranu horaDisida. 
\enum
\emng
\eentry

\bentry
\word{viziership} 
\pron{visiZarfSipf} 
\gl{\nA}
\expl{}
\bmng
 vajiVragiri; vajiVra padavi yA adhikAra yA avadhi. 
\emng
\eentry

\bentry 
\word{vizor} 
\pron{veYsaZrf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{visor}{visor}. 
\emng
\eentry

\bentry
\word{vizsla} 
\pron{viSfZl} 
\gl{\nA}
\expl{}
\bmng
 viSalx; kaDugeMpu meYyuLaLx, haMgariya beVTenAyi. 
\emng
\eentry

\bentry 
\wordnospeech{v.l.}{v.l.} 
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 pAThAMtara; pAThaBeVda (\eng{varia lectio} eMba lAyxTinf padadiMda). 
\emng
\eentry

\bentry
\word[Vlach(1)]{Vlach} 
\pron{vAlxYxkf} 
\gl{\nA}
\expl{}
\bmng
 vAlxYxkf; AgenxVya yUroVpinalilx, eMdare romeVniya matutx soVviyatf okUkxTada kelavu BAgagaLalilx, vAsisuva romeVniyanf, eMdare lAYxTinf vagaRda, BASeyanAnxDuva janAMgadavanu. 
\emng
\eentry

\bentry 
\word[Vlach(2)]{Vlach} 
\pron{vAlxYxkf} 
\gl{\gu}
\expl{}
\bmng
 vAlxYxkf janara yA avarige saMbaMdhisida. 
\emng
\eentry

\bentry 
\word{vlei} 
\pron{velxV} 
\gl{\nA}
\expl{}
\bmng
 (\da\ Aphirxka) hoMDa; maLegAladalilx niVru kUDikoLuLxva haLaLx, tagugx. 
\emng
\eentry

\bentry
\word{V-neck}
\pron{viVnekf} 
\gl{\nA}
\expl{}
\bmng
(kelavomemx visheVSaNavAgi) viV -- nekf, katutx: 
\banum
\alnum{a} uDupinalilx \eng{V} AkArada katutx. 
\alnum{b} \eng{V} katitxruva uDupu. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{VO}{VO}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 (\UK yalilx)\eng{Royal Victorian Order.} 
\emng
\eentry

\bentry
\word{vocable} 
\pron{voVkabf(ba)lf} 
\gl{\nA}
\expl{}
\bmng
 (athaRkikxMta BinanxvAgi, rUpada daqSiTxyiMda) mAtu; pada; shabadx; vAcaka. 
\emng
\eentry

\bentry 
\word{vocabulary} 
\pron{vakAYxbauyxlari} 
\gl{\nA}
\expl{(\bava\ \eng{vocabularies}). }
\bmng
\bnum
\numi{1} shabadxkoVsha; padapaTiTx: 
\banum
\alnum{a} oMdu BASeya yA oMdu pusatxkadalilx yA oMdu vijAcnxna, kale, shAsatxrX, \mo vugaLalilx yA obabx garxMthakataRnu baLasiruva padagaLu: \eng{the ever-increasing scientific vocabulary} sadA hecucxtatxleV iruva vijAcnxna padakoVsha. 
\alnum{b} akArAdiyAgi joVDisi, athaRgaLanunx yA anuvAdagaLanunx niVDiruva A padagaLa paTiTx. 
\eanum
\numie
\num{2} obabxna shabadx -- saMpatutx, BaMDAra; vayxkitxyobabxnige tiLidiruva padagaLa vAyxpitx: \eng{his vocabulary is limited} avana shabadxsaMpatutx parimitavAdadudx. \eng{a word not found in the Chaucerian vocabulary} cAsarf kaviya shabadxBaMDAradalilx kaMDubarada oMdu pada. 
\num{3} kalArUpagaLa, sheYlirUpagaLa yA taMtarxgaLa (\kanmu\ bAYxle \mo\ naqtayxgaLa nAyxsagaLa) taMDa. 
\enum
\emng
\eentry

\bentry
\word[vocal(1)]{vocal} 
\pron{voVkalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ucAcxrada; vAkikxna; vAcika; nuDiya; shabadxda; dhavxniya; dhavxnige saMbaMdhisida; bAyimAtina; ucacxrisida: \eng{a vocal communication} bAyimAtina sudidx, varadi. 
\num{2} kaMThoVkatx(vAda); parxkaTavAda; manasisxnalilxdudxdanenxlalx, manasisxna BAvagaLanenxlalx, mucucxmareyilalxde heVLibiDuva: \eng{he was very vocal about his rights} avanu tananx hakukxgaLa viSayadalilx kaMThoVkatxvAgi mAtanADida. 
\num{3} (\kAparx) (maragaLu, niVru, \mo vugaLa \vi) nuDiyuva; dhavxni, shabadx -- mADuva; mAtADuvaMte toVruva. 
\num{4} (\dhavxni) (sa)GoVSada. 
\num{5} (saMgiVtada \vi) hADaleMdeV bareda; (vAdayxgaLilalxde yA vAdayxgaLa joteyalilx) hADuva; gAyanada. 
\enum
\emng
\eentry

\bentry
\word[vocal(2)]{vocal} 
\pron{voVkalf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hADugArikeya kaceVri; gAyanagoVSiThx. 
\num{2} (\Eva\ yA \bava) (saMgiVta kaqtiya \vi) hADuva, gAyana -- BAga. 
\enum
\emng
\eentry

\bentry 
\wordnospeech{vocal cords}{vocal cords} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 dhavxnitaMtugaLu; vAkatxMtugaLu; gaMTalu kuLiya terapina baLiya danipeTiTxge poreya meVle gALi hariyuvAga kaMpisi daniyanunx utapxtitx mADuva aMcuLaLx maDikegaLu. 
\emng
\eentry

\bentry 
\word{vocalic} 
\pron{vakAYxlikf} 
\gl{\gu}
\expl{}
\bmng
 (vayxMjanakekx virudadhxvAda) savxrada, savxragaLa yA savxragaLiMda kUDida. 
\emng
\eentry

\bentry
\word{vocalise} 
\pron{voVkaleYsfZ} 
\gl{\kirx}
\expl{}
\bmng
 = \hyperlink{vocalize}{vocalize}. 
\emng
\eentry

\bentry 
\word{vocalism} 
\pron{voVkalisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (mAtanADuvudaralilx) ucAcxra(Ne). 
\alnum{b} (\saM) gAyana; hADugArike. 
\eanum
\numie
\numi{2} 
\banum
\alnum{a} savxra (vayxMjanakekx virudadhxvAgi). 
\alnum{b} savxravayxvasethx. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{vocalist} 
\pron{voVkalisfTx} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ jAsfZ yA janapirxya hADugaLa) gAyaka; hADugAra. 
\emng
\eentry

\bentry 
\word{vocality} 
\pron{vakAYxliTi} 
\gl{\nA}
\expl{}
\bmng
 ucAcxraNa -- shakitx, guNa. 
\emng
\eentry

\bentry 
\word{vocalization} 
\pron{voVkaleYseZVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ucacxrisuvudu; ucAcxra(Ne). 
\num{2} (\saM) (hADina alaMkAramayavAda BAgavanunx) palukuvudu. 
\enum
\emng
\eentry

\bentry
\word{vocalize} 
\pron{voVkaleYsf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} dhavxni, shabadx -- mADu, ucacxrisu. 
\alnum{b} GoVSa(vaNaR)vAgisu, udA. \eng{k is vocalized into g} ka anunx ga Agi ucacxrisalAgide. 
\eanum
\numie
\num{2} (hiVbaUrx \mo\ BASegaLige) savxracihenx hAki bare. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (hAsayxdalilx) heVLu; oralu; odaru; aracu. 
\num{2} (\saM) (alaMkAramayavAda BAgavanunx) palukupalukAgi hADu. 
\num{3} ucacxrisu. 
\enum
\emng
\eentry

\bentry
\word{vocalizer} 
\pron{voVkaleYsaZrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ucAcxraka; ucacxrisuvavanu. 
\num{2} (\saM) (alaMkAramaya vAda BAgavanunx) palukuvavanu. 
\enum
\emng
\eentry

\bentry
\word{vocally} 
\pron{voVkali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} bAyimAtiniMda; vAcA; kaMThoVkatxvAgi. 
\num{2} (\saM) hADugArikeya, gAyanada -- rUpadalilx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vocal music}{vocal music}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\saM) 
\bnum
\num{1} (bAyi) hADugArike; gAyana. 
\num{2} gAyanakaqti; hADugArikegAgi racisida kaqti. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vocal score}{vocal score}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\saM) gAyanaparxsAtxra; kaqtiparxsAtxradalilx gAyanada BAgagaLanunx pUtiRyAgi toVrisuva parxsAtxra karxma. 
\emng
\eentry

\bentry
\word{vocation} 
\pron{vakeVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ahaRtABAvane; nidiRSaTx udoyxVga yA vaqtitxge tAnu takakxvaneMba, tanage ahaRteyideyeMba daqDhaBAvane. 
\num{2} deYvaperxVraNe; deVvAdiSaTxvAda vaqtitx; deYva saMkalipxta kAyaR yA sithxti; deYvaperxVritaveMdu BAvitavAda vaqtitx yA udoyxVga: \eng{he felt no vocation for the ministry} pAdirxvaqtitxge avanalilx deYvaperxVraNeyiralilalx. 
\num{3} (aMtaH)perxVraNe; parxvaqtitx; olavu: \eng{he has no vocation to literature} avanige sAhitayxracaneyatatx olavilalx, parxvaqtitxyilalx. 
\num{4} (\kanmu\ sharxdedhx toVrabeVkeMdu parigaNitavAda) vayxkitxya -- vaqtitx, kasubau, udoyxVga. 
\num{5} yAvudeV udoyxVga, vaqtitx, kasubau. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{mistook his vocation} tanage ogugxva vaqtitx hiDiyalilalx, tiLiyalilalx. 
\emng
\eentry

\bentry 
\word{vocational} 
\pron{vakeVSanalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} udoyxVgada; vaqtitxya; kasabina yA adakekx saMbaMdhisida. 
\num{2} (shikaSxNa yA tarabeVtiya \vi) vaqtitxpara; nidiRSaTx audoyxVgika; udoyxVga matutx adakekx beVkAda kwshalagaLanunx guriyAgiTuTxkoMDiruva: \eng{vocational instruction} audoyxVgika shikaSxNa. 
\enum
\emng
\eentry

\bentry
\word{vocationalise} 
\pron{vakeVSanaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vocationalize}{vocationalize}. 
\emng
\eentry

\bentry 
\word{vocationalism} 
\pron{vakeVSanalisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
 vaqtitx tarabeVtike; shikaSxNadalilx vaqtitxtarabeVtige pArxshasatxyx , otutx koDuvudu. 
\emng
\eentry

\bentry
\word{vocationalize} 
\pron{vakeVSanaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 (shikaSxNa \mo vanunx) vaqtitxparavAgisu; nidiRSaTx vaqtitxyanunx daqSiTxyalilxTuTxkoMDu tarabeVti koDuvaMte mADu. 
\emng
\eentry

\bentry
\word{vocationally} 
\pron{vakeVSanali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vaqtitxparavAgi; vaqtitxdaqSiTxyiMda; vaqtitx, kasabanunx daqSiTxyalilxTuTxkoMDu. 
\num{2} vaqtitxya riVtiyalilx; udoyxVgada vidhAnadalilx. 
\enum
\emng
\eentry

\bentry
\word[vocative(1)]{vocative} 
\pron{vAkaTivf} 
\gl{\gu}
\expl{}
\bmng
 (\vAyx) (\kanmu\ rUpaniSapxtitxgaLiruva BASegaLalilx) (nAmapada, savaRnAma matutx guNavAcakagaLa) saMboVdhanA viBakitxya; saMboVdhanA rUpada; vayxkitxyanonxV vasutxvanonxV kuritu mAtanADuva. 
\emng
\eentry

\bentry 
\word[vocative(2)]{vocative} 
\pron{vAkaTivf} 
\gl{\nA}
\expl{}
\bmng
 (\vAyx) saMboVdhanA viBakitx. 
\emng
\eentry

\bentry 
\wordnospeech{vocative case}{vocative case}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vocative(2)}{$^2$vocative}. 
\emng
\eentry

\bentry
\word{vociferance} 
\pron{vasipharansxf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vociferation}{vociferation}. 
\emng
\eentry

\bentry
\word[vociferant(1)]{vociferant} 
\pron{vasipharaMTf} 
\gl{\gu}
\expl{}
\bmng
 odaruva; kiricuva; bobebxyiDuva; ababxrisuva. 
\emng
\eentry

\bentry
\word[vociferant(2)]{vociferant} 
\pron{vasipharaMTf} 
\gl{\nA}
\expl{}
\bmng
 odaruvavanu; kiricuvavanu; ababxrisuvavanu; aracugAra. 
\emng
\eentry

\bentry
\word{vociferate} 
\pron{vasiphareVTf} 
\gl{\sakirx}
\expl{}
\bmng
 (shabadxgaLu \mo vanunx) odaru; kiricu; ababxrisu; aracu; kUgADu; gaTiTxyAgi heVLu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 odaru; kiricu; kUgADu; bobebxyiDu; aracu. 
\emng
\eentry

\bentry 
\word{vociferation} 
\pron{vasiphareVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
 (shabadxgaLanunx) odaruvike; kiricuvike; ababxrisuvike; aracuvike. 
\emng
\eentry

\bentry
\word{vociferator} 
\pron{vasiphareVTarf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vociferant(2)}{$^2$vociferant}. 
\emng
\eentry

\bentry
\word{vociferous} 
\pron{vasipharasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx, mAtu, \mo vugaLa \vi) odaruva; kUgATada; ababxrisuva; kiricuva; galATeyiMda kUDida. 
\num{2} AgarxhapUvaRkavAgi heVLuva; tananx aBipArxyagaLanunx otAtxyavAgi yA birusiniMda heVLuva. 
\enum
\emng
\eentry

\bentry
\word{vociferously} 
\pron{vasipharasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kiricutAtx; ababxrisutAtx; bobebxyiDutAtx. 
\num{2} (tananx aBipArxyagaLanunx heVLuva \vi) AgarxhapUvaRkavAgi; otAtxyadiMda; birusAgi. 
\enum
\emng
\eentry

\bentry 
\word{vociferousness} 
\pron{vasipharasfnisf} 
\gl{\nA}
\expl{}
\bmng
 kiricuvike; bobebx; ababxra. 
\emng
\eentry

\bentry 
\word{vodka} 
\pron{vADakx} 
\gl{\nA}
\expl{}
\bmng
 voVDakx; reY \mo\ dhAnayxvanunx Asavisi tegeyuva, AlakxhAlf parxmANa atiyAgiruva, raSAyxdeVshada tiVkaSxNX madayx. 
\emng
\eentry

\bentry
\word{voe} 
\pron{voV} 
\gl{\nA}
\expl{}
\bmng
 (AkfRniV matutx SeTflaMDf divxVpagaLa baLiya) saNaNx kolilx yA KAri. 
\emng
\eentry

\bentry 
\word{vogue} 
\pron{voVgf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (cAlitxyalilxruva) vADike; rUDhi: \eng{large hats are the vogue today} BAri hAyxTugaLu iMdina rUDhiyAgive. 
\num{2} cAlitx; janabaLake; janapirxyavAda rUDhi, calAvaNe: \eng{it has had a great vogue} adu BAri janapirxya rUDhi hoMdide. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{in vogue} baLakeyalilx; parxcAradalilx; vADikeyalilx; cAlitxyalilx. 
\emng
\eentry

\bentry 
\word{vogue-word} 
\pron{voVgfvaDfR} 
\gl{\nA}
\expl{}
\bmng
 cAlitxyalilxruva pada; rUDhiya pada. 
\emng
\eentry

\bentry 
\word{voguish} 
\pron{voVgiSf} 
\gl{\gu}
\expl{}
\bmng
 baLakeyalilxruva; cAlitxyalilxruva; vADikeyalilxruva; rUDhiyalilxruva; janapirxyavAgiruva. 
\emng
\eentry

\bentry 
\word[voice(1)]{voice} 
\pron{vAyfsx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (dhavxninALadalilx huTiTx bAyalilx horaDuva, \kanmu\ manuSayxra) dhavxni; dani; shabadx; kaMTha; gaMTalu: \eng{I heard a voice} nanage oMdu dhavxni keVLisitu. \eng{he spoke in a low voice} avanu meludaniyalilx mAtanADida. \eng{I did not recognize her voice} nanage Akeya dhavxniya guruteV sigalilalx. \eng{the voice of the cuckoo} koVkila dhavxni; koVkilakaMTha. \eng{he has such a loud voice} avanige aSoTxMdu gaTiTxyAda dhavxni ide. 
\numi{2} 
\banum
\alnum{a} (mAtina \vi) ucAcxraNe. 
\alnum{b} (hADina \vi) kaMTha; shAriVra. 
\eanum
\numie
\num{3} gaMTalu; kaMTha; dhavxni(shakitx); shAriVra(bala): \eng{has lost her voice} avaLa shAriVra kugigxhoVgide. 
\num{4} mAtu; dani; nAlige; vAkukx; vANi; \kanmu\ ADida yA bareda padagaLu: \eng{he gave voice to his indignation in a pamphlet} avanu oMdu kiruhotatxgeyalilx tananx nAyxyavAda koVpavanunx padagaLalilx horageDahida, koVpakekx nAlige koTaTx. \eng{the dog gave voice to its joy} A nAyi tananx AnaMdavanunx kUginalilx sUcisitu. 
\numi{5} 
\banum
\alnum{a} mAtu; dhavxni; vayxkatxpaDisida aBipArxya. 
\alnum{b} aBipArxya koDuva hakukx: \eng{I have no voice in this matter} I viSayadalilx nanage aBipArxya koDuva hakikxlalx. 
\alnum{c} vANi; aBipArxya vayxkatxpaDisalu baLasuva vayxkitx yA sAdhana. 
\eanum
\numie
\num{6} (\dhavxni) GoVSavaNaR; dhavxnitaMtugaLa anuraNanadiMda yA kaMpanadiMda huTuTxva dhavxni. 
\num{7} (\vAyx) (kirxyApadada) kataRri, kamaRNi yA BAveV -- parxyoVga rUpa; kataRqvigU kirxyegU iruva saMbaMdhavanunx sUcisuva kirxyArUpagaLalilx oMdu. 
\numi{8} 
\banum
\alnum{a} (\saM) saMgiVta kaqtiya hADuva yA gAyana BAga. 
\alnum{b} (\saM) PUZyxgf eMba saMgiVta kaqtiya gAyanAMsha. 
\eanum
\numie
\num{9} (\sA\ \bava dalilx) deYvavANi; ashariVra, agoVcara -- dhavxni; adaqshayx Atamxvu mAgaRdashaRna mADalu nuDidudeMdu naMbalAda mAtu, dhavxni, shabadx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{at the} \hyperref{kandict_t.pdf}{T}{top(1)16}{$^1$top} \eng{of one's voice.} 
\numi{2} \eng{in (good) voice} 
\banum
\alnum{a} (mAtanADuvudaralilx) oLeLxya ucAcxraNa shakitxyuLaLx. 
\alnum{b} (hADuvudaralilx) (utatxma) kaMThavuLaLx, shAriVravuLaLx. 
\eanum
\numie
\num{3} \eng{the voice of God} deYvavANi; deYva -- saMkalapx; korxVdha, shApa, \mo vugaLa aBivayxkitx. 
\num{4} \eng{with one voice} EkakaMThadiMda; EkABipArxyadiMda; okokxraliniMda; omamxtadiMda. 
\enum
\emng
\eentry

\bentry
\word[voice(2)]{voice} 
\pron{vAyfsx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} mAtinalilx -- vayxkatxpaDisu, heVLu, usiru, nuDi: \eng{I am voicing the general sentiment} savaRra BAvaneyanunx nAnu (mAtinalilx) vayxkatxpaDisutitxdedxVne, heVLutitxdedxVne. \eng{the letter voices our opinion} I kAgadavu namamx aBipArxyavanunx nuDiyutatxde, heVLutatxde, vayxkatxpaDisutatxde. 
\num{2} (\saM) (AgaRnf koLavigaLu \mo\ vAdayxgaLanunx niyaMtirxsi) dhavxni, shurxti -- saripaDisu. 
\num{3} (\kanmu\ \BUkaq\ rUpadalilx) GoVSavaNaRvAgi ucacxrisu. 
\enum
\emng
\eentry

\bentry 
\word{voice-box} 
\pron{vAyfsxbAkfsx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_l.pdf}{L}{larynx}{larynx}. 
\emng
\eentry

\bentry
\word{voiced} 
\pron{vAyfsxTf} 
\gl{\gu}
\expl{}
\bmng
 (\dhavxni) GoVSoVcAcxrada; saGoVSa, udA. \eng{b, d, g, v.} 
\emng
\eentry

\bentry 
\word{voiceful} 
\pron{vAyfsxPuZlf} 
\gl{\gu}
\expl{}
\bmng
 (\kAparx\ yA \alaMshA) 
\bnum
\num{1} tuMbaunAdada; nAdaBarita; nAdapUNaR. 
\num{2} anuraNisuva. 
\enum
\emng
\eentry

\bentry 
\word{voiceless} 
\pron{vAyfsxlisf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mAtilalxda; mUga; mUka. 
\num{2} (\dhavxni) aGoVSa; dhavxnitaMtugaLu kaMpisade ucacxrisuva, \udA\ \eng{f, k, p, t}. 
\enum
\emng
\eentry

\bentry
\word{voicelessly} 
\pron{vAyfsxlisfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} mUkanAgi; mUganAgi; mAtubArade. 
\num{2} aGoVSavAgi. 
\enum
\emng
\eentry

\bentry
\word{voicelessness} 
\pron{vAyfsxlisfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} mUkate; mAtilalxdiruvike. 
\num{2} (\dhavxni) aGoVSate. 
\enum
\emng
\eentry

\bentry
\word{voice-over} 
\pron{vAyfsxOvarf} 
\gl{\nA}
\expl{}
\bmng
 ashariVravANi; (calanacitarxdalilx) mAtanADutitxruva vayxkitxya citarxvanunx parxdashiRsade, mAtanunx mAtarx keVLisuvike. 
\emng
\eentry

\bentry 
\word{voice-print} 
\pron{vAyfsxpirxMTf} 
\gl{\nA}
\expl{}
\bmng
 dhavxnimudarxNa; dhavxnidAKale; AvataRna, avadhi matutx haravugaLige anusAravAgi vishelxVSisi tayArisida dhavxniya yA mAtina daqshayx(rUpada) dAKale. 
\emng
\eentry

\bentry
\word{voicer} 
\pron{vAyasxrf} 
\gl{\nA}
\expl{}
\bmng
 (\saM) shurxtigAra; vAdayxgaLa shurxti saripaDisuvavanu. 
\emng
\eentry

\bentry
\wordnospeech{voice vote}{voice vote}
\pron{?}
\gl{\nA}
\bmng
(\ame) dhavxnimata; pAliRmeMTinalilx yA shAsanasaBegaLalilx paravAda matutx virudadhxvAda matagaLanunx pakaSx parxtipakaSxgaLa `Agali', `beVDa' eMba shabadxgaLa saMKAyxbaladiMda ariyuva vidhAna. 
\emng
\eentry

\bentry 
\word[void(1)]{void} 
\pron{vAyfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} baridAda; KAliyAda; shUnayx(vAda). 
\num{2} (hudedxya \vi) terapAgiruva; KAli: \eng{the bishopric fell void} biSapapxna hudedxyu terapAyitu, KAliyAyitu. 
\num{3} (isipxVTADuvavana \vi) ele ilalxda; yAvudeV paTiTxnalilx yA variseyalilx agatayxvAda isipxVTele ilalxda. 
\num{4} (\kanmu\ \nAyxshA)(patarx, vAgAdxna, karAru, \mo vugaLa \vi) anUjiRta; nirathaRka; shAsana daqSiTxyiMda balavilalxda, kaDADxyavalalxda: \eng{the agreement is null and void} A karAru anUjiRtavAgide, adakekx shAsanabalavilalx. 
\num{5} (\kAparx, \alaMshA) pariNAmakAriyalalxda; parxBAva biVrada; niSaPxla; vayxthaRvAda; niSapxrXyoVjaka. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{void of} ilalxda; rahita; shUnayx: \eng{a proposal wholly void of sense} viveVkaveV, athaRveV -- ilalxda sUcane, salahe. \eng{his style is void of affectation} avana sheYliyalilx kaqtakate ilalx. 
\emng
\eentry

\bentry 
\word[void(2)]{void} 
\pron{vAyfDx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} shUnayx -- sathxLa, parxdeVsha; KAli jAga: \eng{he vanished into the void} avanu bayalAda, adaqshayxnAda, shUnayxdalilx mareyAda. 
\num{2} shUnayxte; aBAva: \eng{one cannot fill the void made by his death} Atana nidhanadiMdAgiruva shUnayxteyanunx tuMbalu sAdhayxveV ilalx. 
\num{3} (goVDeyalolxV kaTaTxDadalolxV) KAli jAga; terapu; EnanUnx kaTiTxlalxda yA iTiTxlalxda sathxLa. 
\num{4} (isipxVTu ATa) yAvudeV variseyalilx, paTiTxnalilx, agatayxvAda isipxVTeleya aBAva. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{the aching void of the heart} haqdayavanunx koreyutitxruva shUnayxte. 
\emng
\eentry

\bentry
\word[void(3)]{void} 
\pron{vAyfDx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (patarx, karAru, \mo vanunx) anUjiRtagoLisu; nirathaRkagoLisu; radudx mADu. 
\num{2} (malamUtarx) visajiRsu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 (malamUtarx) visajiRtavAgu. 
\emng
\eentry

\bentry 
\word{voidable} 
\pron{vAyaDxbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 (patarx, karAru, \mo vugaLa \vi) anUjiRta, nirathaRka -- goLisabahudAda; radudx mADabahudAda. 
\emng
\eentry

\bentry 
\word{voidance} 
\pron{vAyaDxnfsx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (cacuR) (dhamARdAya \mo vugaLa) anuBoVga tapipxsuvudu. 
\alnum{b} (maThavaqtitx \mo vugaLalilx) terapu; KAli. 
\eanum
\numie
\num{2} KAli, terapu -- mADuvike. 
\enum
\emng
\eentry

\bentry
\word{voided} 
\pron{vAyiDxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anUjiRtagoLisida; nirathaRka mADida. 
\num{2} terapu, KAli -mADida. 
\num{3} (\vaMlAM) oLaBAgavanunx koredu hAkida, terapu mADida. 
\enum
\emng
\eentry

\bentry
\word{voidness} 
\pron{vAyfDxnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} shUnayxte; terapu. 
\num{2} shUnayxparxdeVsha; KAlijAga. 
\enum
\emng
\eentry

\bentry
\word{voile} 
\pron{vAyalf, vAvxlf} 
\gl{\nA}
\expl{}
\bmng
 (teLuvU arepAradashaRkavU Ada, hatitxya, uNeNxya yA reVSemxya) vAyilf baTeTx. 
\emng
\eentry

\bentry 
\wordf{voir dire} 
\pron{vAvxrf Dearf} 
\gl{\nA}
\expl{\F\ }
\bmng
satayxvacana shapatha; (nAyxyadashiR itAyxdiyAgi vatiRsalu) tananx ahaRteya viSayadalilx keVLuva parxshenxgaLige satayxvAda utatxragaLanunx koDutetxVneMdu mADuva parxmANa, shapatha. 
\emng
\eentry

\bentry 
\wordnospeech{vol.}{vol.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{volume.} 
\emng
\eentry

\bentry 
\word{volant} 
\pron{voVlaMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\pArxvi) hAruva; hArabalalx. 
\num{2} (\vaMlAM) hArutitxruvaMte -- citirxsiruva, korediruva. 
\num{3} (sAhitayxka) curukAda; veVgavAda; durxtagatiya. 
\enum
\emng
\eentry

\bentry
\word{volar} 
\pron{voVlarf} 
\gl{\gu}
\expl{}
\bmng
 (\aMrashA) aMgeYya yA aMgAlina; hasatxtalada yA pAdatalada. 
\emng
\eentry

\bentry 
\word[volatile(1)]{volatile} 
\pron{vAlaTeYlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} bASapxshiVla; AviyAguva; (sAmAnayx shAKagaLalilxyeV) habe, bASapx -- Aguva: \eng{volatile salts} AvishiVla lavaNagaLu. 
\num{2} gelavina; ulAlxsa parxkaqtiya; lavalavikeya; laGu manoVvaqtitxya: \eng{volatile wit} laGuhAsayx. \eng{a volatile writer} ulAlxsa parxkaqtiya leVKaka. \eng{volatile disposition} lavalavikeya manoVvaqtitx. 
\num{3} caMcala; tarala; tavxritavAgi matutx UhisalasAdhayxvAgi badalAguva, \kanmu\ hecucx keDukAguva. 
\num{4} kaSxNika; asithxra. 
\num{5} BugileVLuva; hiMsAcArakekx tiruguva parxvaqtitxyuLaLx. 
\num{6} (kaMpUyxTarf samx qtiya \vi) viduyxtasxrabarAju iruvavarege mAtarx datAtxMshavanunx samxYtiyalilxTuTxkoLuLxva. 
\enum
\emng
\eentry

\bentry
\word[volatile(2)]{volatile} 
\pron{vAlaTeYlf} 
\gl{\nA}
\expl{}
\bmng
 bASapxshiVla darxvayx; (sAmAnayx shAKagaLalelxV) anilarUpa hoMduva darxvayx. 
\emng
\eentry

\bentry
\word{volatileness} 
\pron{vAlaTeYlfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ulAlxsa parxkaqti(yuLaLxvanAgiruvike); laGu manoVvaqtitx(yuLaLxvanAgiruvike). 
\num{2} caMcalate; taralate. 
\num{3} = \hyperlink{volatility}{volatility}. 
\enum
\emng
\eentry

\bentry 
\wordnospeech{volatile oil}{volatile oil} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_e.pdf}{E}{essential oil}{essential oil}. 
\emng
\eentry

\bentry
\word{volatilise} 
\pron{valAYxTileYsfZ} 
\gl{\kirx}
\expl{}
\bmng
 = \hyperlink{volatilize}{volatilize}. 
\emng
\eentry

\bentry 
\word{volatility} 
\pron{vAlaTiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} bASapxshiVlate; AviyAguvike; (sAmAnayx uSaNxtegaLalelxV) bASapxvAguva lakaSxNa. 
\num{2} caMcalate; tavxritavAgi matutx UhisalasAdhayxvAgi badalAguvaMtiruvudu. 
\enum
\emng
\eentry

\bentry
\word{volatilizable} 
\pron{valAYxTileYsaZbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 bASipxVkarisalAguva; bASapxgoLisalAguva; bASapxrUpakekx taralAguva. 
\emng
\eentry

\bentry 
\word{volatilization} 
\pron{valAYxTileYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} bASipxVkaraNa; AviVkaraNa; bASapxgoLisuvudu; AvigoLisuvike; AvirUpakekx taruvudu. 
\num{2} bASipxVBavana; AviVBavana; AviyAguvudu; bASapxrUpakekx baruvudu. 
\enum
\emng
\eentry

\bentry 
\word{volatilize} 
\pron{valAYxTileYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 bASipxVkarisu; AviVkarisu; bASapxrUpakekx taru; bASapxgoLisu; AviyAgisu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 bASipxVBavisu; bASapxrUpakekx baru; AviyAgu. 
\emng
\eentry

\bentry 
\word{vol-au-vent} 
\pron{vAloVvAknf} 
\gl{\nA}
\expl{}
\bmng
 vAloVvAnf; (mAMsa, miVnu, \mo vanUnx gojujx \mo\ vayxMjanavanUnx tuMbi mADida) hiTiTxna, \sA\ cikakx, paphf. 
\emng
\eentry

\bentry
\word{volcanic} 
\pron{vAlAkxYxnikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} jAvxlAmuKiya; aginxpavaRtada. 
\alnum{b} jAvxlAmuKiyaMtha; aginxpavaRtadaMtha. 
\alnum{c} jAvxlAmuKiVya; aginxpavaRtadiMda Ada. 
\eanum
\numie
\num{2} BugileVLuva; dhiDiVrane urideVLuva; idadxkikxdadxMte reVgibiVLuva: \eng{a volcanic temper} urideVLuva manoVvaqtitx, savxBAva. 
\enum
\emng
\eentry

\bentry 
\word{volcanically} 
\pron{vAlAkxYXnikali} 
\gl{\kirxvi}
\expl{}
\bmng
 jAvxlAmuKiyaMte. 
\emng
\eentry

\bentry 
\wordnospeech{volcanic bomb}{volcanic bomb}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 jAvxlAmuKi bAMbau; (\sA\ guMDAgiyU kelavomemx ToLALxgiyU iruva) jAvxlAmuKiya shilArasada guMDu. 
\emng
\eentry

\bentry 
\wordnospeech{volcanic glass}{volcanic glass} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_o.pdf}{O}{obsidian}{obsidian}. 
\emng
\eentry

\bentry
\word{volcanicity} 
\pron{vAlakxnisiTi} 
\gl{\nA}
\expl{}
\bmng
 jAvxlAmuKiVyate; jAvxlAmuKiya guNa, savxBAva. 
\emng
\eentry

\bentry 
\word{volcano} 
\pron{vAlekxVnoV} 
\gl{\nA}
\expl{(\bava\ \eng{volcanoes}).}
\bmng
\bnum
\num{1} aginxpavaRta; jAvxlAmuKi; beMkibeTaTx; shilArasa, keMDagaLu, niVrAvi, anilagaLu, \mo vugaLanunx uguLuva beTaTx: \eng{active volcano} jAgaqta jAvxlAmuKi. \eng{dormant volcano} supatx jAvxlAmuKi. \eng{extinct volcano} maqta yA satatx jAvxlAmuKi. 
\num{2} (\kanmu\ adumiTuTxkoMDa) niyaMtirxsalAgada, taDeyalAgada BAvoVderxVka. 
\num{3} hiMsAcoVdaka sithxti; hiMsAtamxka kaqtayxgaLanunx uMTumADabahudAda udirxkatx parisithxti. 
\enum
\emng
\eentry

\bentry
\word{volcanology} 
\pron{valakxnAlaji} 
\gl{\nA}
\expl{}
\bmng
 \eng{vulcanology} eMbaudara rUpAMtara. 
\emng
\eentry

\bentry 
\word[vole(1)]{vole} 
\pron{voVlf} 
\gl{\nA}
\expl{}
\bmng
 (\pArxparx) isipxVTATadalilx elalx paTuTxgaLanUnx, varisegaLanUnx hoDedukoLuLxvudu, gelulxvudu. 
\emng
\eentry

\bentry 
\word[vole(2)]{vole} 
\pron{voVlf} 
\gl{\nA}
\expl{}
\bmng
 voVlf; sasAyxhAriyAda, kirxseTiDiV vaMshada, iliyaMtha daMshaka. \imglink{\(^2\)vole:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.5cm {V_Pictures/vole(2).jpg}}} 
\emng
\eentry

\bentry
\word{volet} 
\pron{vAleV} 
\gl{\nA}
\expl{}
\bmng
 (citarxkale) pakaSxPalaka; pakASxMkaNa; mUru aMkaNada yA Palakada citarxdalilxya aMkaNa yA PalakagaLalolxMdu. 
\emng
\eentry

\bentry 
\word{volitant} 
\pron{vAliT(Tf)MTf} 
\gl{\gu}
\expl{}
\bmng
 (\pArxvi) = \hyperlink{volant}{volant}. 
\emng
\eentry

\bentry
\word{volition} 
\pron{valiSanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} icePx; saMkalapx; iciPxsuvudu yA saMkalipxsuvudu. 
\num{2} icAPxshakitx; saMkalapx parxvaqtitx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{of} (\engit{or} \eng{by) one's own volition} savxMta icePxyiMda; sevxVcePxyiMda; savxsaMkalapxdiMda. 
\emng
\eentry

\bentry
\word{volitional} 
\pron{valiSanalf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{volitive}{volitive}. 
\emng
\eentry

\bentry 
\word{volitive} 
\pron{vAliTivf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} icePxya; saMkalapxda. 
\num{2} icAPx, saMkalapx -- shakitxya. 
\enum
\emng
\eentry

\bentry 
\wordf{Volkerwanderung} 
\pron{phalakxvAnaDxruknf} 
\gl{\nA}
\expl{\G}
\bmng
(\kirxsha\ \eng{2}neV shatamAnadiMda \eng{11}neV shatamAnadavarege) \kanmu\ jamARYxnikf matutx sAlxYxvikf janaru, yUroVpige mADida valase. 
\emng
\eentry

\bentry
\word[volley(1)]{volley} 
\pron{vAli} 
\gl{\nA}(\bava\ \eng{volleys}). 
\bmng
\bnum
\num{1} oTATxgi, omemxleV, EkakAladalilx, aneVka AyudhagaLanunx hArisuvudu, biDuvudu. 
\num{2} kiSxpaNigaLa (\udA\ guMDu \mo vugaLa) surimaLe, vaqSiTx, vaSaR; omemxleV hArisida kiSxpaNigaLa samUha, surita. 
\num{3} (ANe, parxshenx, beYguLa, hAsayx, \mo vugaLa) surimaLe; oMdara hiMde oMdu yA puMKAnupuMKavAda surimaLe, vaSaR, vaqSiTx. 
\num{4} (lAnf Tenisf \mo) ceMDu nelavanunx muTuTxva munanxveV adanunx hiMdirugisuvudu. 
\num{5} (kAlecxMDATa) ceMDu nelamuTuTxva munanxveV adanunx odeyuvudu yA iMtha odeta. 
\numi{6} (kirxkeTf) 
\banum
\alnum{a} ceMDanunx nela tAkadeyeV bAYxTugAranalilxge yA saTxMpfsx baLige eseyuvudu. 
\alnum{b} iMtha boVliMgf, eseta. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[volley(2)]{volley} 
\pron{vAli} 
\gl{\kirx}(vataRmAna \parxpu\ \Eva\ \eng{volleys}; \BU\ matutx
\BUkaq\ \eng{volleyed}) \bmng
\emng

\noindent
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} ({\akirx} saha). 
\banum
\alnum{a} (Tenisf) edurALiya ceMDu nela tAguvudakekx muMce adanunx hoDedu hiMdirugisu. 
\alnum{b} (kAlecxMDATa) ceMDu nela tAguvudaroLage ode. 
\eanum
\numie
\num{2} (kiSxpaNigaLu, hAsoyxVkitxgaLu, beYguLagaLu, parxshenxgaLu, ANegaLu, \mo vanunx) surimaLegare; eDebiDade, puMKAnupuMKavAgi parxyoVgisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 (baMdUkagaLu, PiraMgigaLu) oTiTxge, omemxleV -- hAru yA hArida sadudxmADu. 
\emng
\eentry

\bentry 
\word{volleyball} 
\pron{vAlibAlf} 
\gl{\nA}
\expl{}
\bmng
 (doDaDx ceMDanunx keYyiMda keYge dUDutAtx, baleya meVlagxDeyiMda eduru aMkaNakekx hAkuva, 6-6 ATagAraranunx oLagoMDa eraDu taMDagaLu ADuva) vAlibAlf ATa. 
\emng
\eentry

\bentry 
\word{volleyer} 
\pron{vAliarf} 
\gl{\nA}
\expl{}
\bmng
 (Tenisf \mo valilx) edurALi hoDeda ceMDu nelatAguvudaroLage adanunx hoDedu hiMdirugisuvavanu. 
\emng
\eentry

\bentry 
\word[volplane(1)]{volplane} 
\pron{vAlfpelxVnf} 
\gl{\akirx}
\expl{}
\bmng
 (vimAnada \vi) jAriLita; eMjinanunx nililxsi nidhAnavAgi, OreyAgi iLiyuvudu. 
\emng
\eentry

\bentry 
\word[volplane(2)]{volplane} 
\pron{vAlfpelxVnf} 
\gl{\akirx}
\expl{}
\bmng
 (vimAnada \vi) eMjinanunx nililxsi, OreyAgi nidhAnavAgi iLi. 
\emng
\eentry

\bentry
\wordnospeech{vols.}{vols.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{volumes.} 
\emng
\eentry

\bentry 
\word[volt(1)]{volt} 
\pron{vA(voV)lfTx} 
\gl{\akirx}
\expl{}
\bmng
 (katitxvariseyalilx) edurALiya hoDetavanunx tapipxsikoLaLxlu -- taTakakxne sari, caMgane calisu. 
\emng
\eentry

\bentry
\word[volt(2)]{volt} 
\pron{vA(voV)lfTx} 
\gl{\nA}
\expl{}
\bmng
 (\viduyx) voVluTx; viduyxcAcxlaka balada mAna; oMdu Omf roVdhavanunx edurisi oMdu AYxMpiyarf viduyxtf parxvAhavanunxMTumADuvaSuTx viBavAMtara. 
\emng
\eentry

\bentry 
\word[volt(3)]{volt} 
\pron{vA(voV)lfTx} 
\gl{\nA}
\expl{}
\bmng
 \eng{volte} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{voltage} 
\pron{voVliTxjf} 
\gl{\nA}
\expl{}
\bmng
 voVleTxVjf; voVlfTxgaLalilx aLate mADida viduyxcAcxlaka balada yA viBavAMtarada parxmANa. 
\emng
\eentry

\bentry 
\word{voltaic} 
\pron{vAleTxVikf} 
\gl{\gu}
\expl{}
\bmng
 (\pArxparx) parxvAha viduyxtitxna; pArxthamika bAyxTariyoMdariMda baruva viduyxtitxna: \eng{voltaic battery} parxvAha viduyxtf koVsha. 
\emng
\eentry

\bentry 
\word{voltameter} 
\pron{vAlATxYxmiTarf} 
\gl{\nA}
\expl{}
\bmng
 voVlATxmApaka; viduyxdAveVshavanunx voVluTxgaLalilx aLeyuva upakaraNa. 
\emng
\eentry

\bentry 
\word{volte} 
\pron{voV(vA)lfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (katitxvarise) (edurALiya hoDetavanunx tapipxsikoLaLxlu mADuva) taTakakxneya sarita. 
\num{2} (kudureya \vi) pakakxpakakxkekx calisuvike yA maMDalAkArada, vaqtAtxkArada calana; pakakxpakakxda calana yA sututxsututxvike. 
\enum
\emng
\eentry

\bentry 
\word{volte-face} 
\pron{vAlfTxphAsf} 
\gl{\nA}
\expl{}
\bmng
tirugumurugu: 
\banum
\alnum{a} hiMdumuMdAgi tiruguvudu. 
\alnum{b} (\kanmu\ \rUpa) (vAda, rAjakiVya, \mo vugaLalilx) pUtiR tirugumurugu; tananx tatatxvXgaLu, aBipArxyagaLanunx biTuTxkoTuTx anayxtatatxvXgaLu, aBipArxyagaLanunx taLeyuvudu, parxtipAdisuvudu, etitxhiDiyuvudu. 
\eanum
\emng
\eentry

\bentry 
\word{voltmeter} 
\pron{voVlfTxmiVTarf} 
\gl{\nA}
\expl{}
\bmng
 voVlfTxmApaka; viBavAMtara yA viduyxcAcxlaka balavanunx aLeyuva upakaraNa. 
\emng
\eentry

\bentry
\word{volubility} 
\pron{vAluyxbiliTi} 
\gl{\nA}
\expl{}
\bmng
 vAcALitana; vAcAlate; vAgaJxri. 
\emng
\eentry

\bentry
\word{voluble} 
\pron{vAluyxbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (mAtina yA mAtugArana \vi) niragaRLavAgi, taDeyilalxde -- hariyuva; vAkapxrXvAhada; vAgaJxriya. 
\num{2} (\pArxparx) sututx tiruguva; giraki hoDeyuva. 
\num{3} (\savi) (Asareya sutatx) suruLi sutitxkoLuLxva. 
\enum
\emng
\eentry

\bentry 
\word{volubleness} 
\pron{vAluyxbf(ba)lfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{volubility}{volubility}. 
\emng
\eentry

\bentry
\word{volubly} 
\pron{vAluyxbilx} 
\gl{\kirxvi}
\expl{}
\bmng
 (mAtina \vi) niragaRLavAgi; parxvAharUpadalilx. 
\emng
\eentry

\bentry 
\word{volume} 
\pron{vAlUyxmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} saMpuTa: 
\banum
\alnum{a} mudirxsida, beraLacucx mADida yA keYbarahadalilxruva, garxMthada raTuTxgaTiTxda halavu BAgagaLalolxMdu BAga. 
\alnum{b} halavu kaqtigaLanonxLagoMDa garxMthada halavu BAgagaLalolxMdu saMpuTa: \eng{issued in 3 volumes} mUru saMpuTagaLAgi horaDisida, parxkaTisida. \eng{an old volume of `Punch'} `paMcf' niyatakAlikada oMdu haLeya saMpuTa. 
\eanum
\numie
\num{2} (\ca) Ole pusatxka; \sA\ bareda camaRpatarx, tALeyoVle, \mo vugaLa pusatxka suruLi. 
\num{3} (\bava dalilx) (hoge \mo vugaLa) (niDu)suruLi. 
\numi{4} 
\banum
\alnum{a} motatx; rAshi; Gana aLate; sale. 
\alnum{b} gAtarx; darxva, anila -- vasutxgaLu AkarxmisikoLuLxva parxdeVsha. 
\eanum
\numie
\num{5} (vAyxpAra, vANijayx, \mo vugaLa) motatx; parxmANa. 
\numi{6} (\saM) 
\banum
\alnum{a} nAdada -- parxmANa yA bala. 
\alnum{b} dhavxniya tuMbau; shAriVrada yA vAdayxda nAdada tuMbau. 
\eanum
\numie
\num{7} hariyutitxruva niVru \mo vugaLa parxvAhada parxmANa. 
\enum
\emng
\eentry

\bentry
\word{volumed} 
\pron{vAlUyxmfDx} 
\gl{\gu}
\expl{}
\bmng
 saMpuTagaLa; saMpuTagaLanonxLagoMDa (\saMpa gaLalUlx \parx). 
\emng
\eentry

\bentry 
\word{volumetric} 
\pron{vAluyxmeTirxkf} 
\gl{\gu}
\expl{}
\bmng
 gAtirxVya; gAtarxda; gAtarxkekx saMbaMdhisida: \eng{volumetric analysis} gAtirxVya vishelxVSaNe. 
\emng
\eentry

\bentry
\word{volumetrically} 
\pron{vAluyxmeTirxkali} 
\gl{\kirxvi}
\expl{}
\bmng
 gAtArxnusAravAgi; gAtarxdAdxgi; gAtarxkekx saMbaMdhisidaMte; gAtirxVyavAgi. 
\emng
\eentry

\bentry
\word{voluminosity} 
\pron{valUyx(lU)minAsiTi} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{voluminousness}{voluminousness}. 
\emng
\eentry

\bentry
\word{voluminous} 
\pron{valUyx(lU)minasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} halavu saMpuTagaLuLaLx. 
\num{2} (leVKakana \vi) halavAru garxMthagaLanunx racisiruva; vipulavAgi barediruva. 
\num{3} BAri gAtarxda; baqhadAgxtarxda. 
\num{4} (baTeTx \mo vugaLa \vi) dogaleyAgiruva; saDilavAgi, aLaLxkavAgi agalavAgiruva. 
\enum
\emng
\eentry

\bentry
\word{voluminously} 
\pron{valUyx(lU)minasfli} 
\gl{\kirxvi}
\expl{}
\bmng
vipulavAgi; samaqdidhxyAgi; doDaDxsaMKeyxyalilx yA BAri parxmANadalilx. 
\emng
\eentry

\bentry 
\word{voluminousness} 
\pron{valUyx(lU)minasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} baqhadAgxtarxte; BAri -- parxmANadAdxgiruvike, gAtarxvuLaLxdAdxgiruvike. 
\num{2} vipulate; BAri saMKeyxyalilxruvike. 
\enum
\emng
\eentry

\bentry
\word{voluntarily} 
\pron{vAlaMTarili} 
\gl{\kirxvi}
\expl{}
\bmng
 savxMta -- iSaTxdiMda, icePxyiMda; savxsaMtoVSadiMda; savxyaMperxVraNeyiMda. 
\emng
\eentry

\bentry
\word{voluntariness} 
\pron{vAlaMTarinisf} 
\gl{\nA}
\expl{}
\bmng
 savx, savxyaM -- perxVritate; savxMta icePxyiMda, savxsaMtoVSadiMda -- mADuvike, Aguvike, vatiRsuvike. 
\emng
\eentry

\bentry 
\word{voluntarism} 
\pron{vAlaMTarisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
\hypertarget{voluntarism(1)}{} 
\bnum
\num{1} (\ca) (cacuR \mo vugaLa \vi) savxtaMtarxtA sidAdhxMta; savxyaMsahAyatatatx vx; cacuR matutx shAlegaLu sakARradiMda savxtaMtarxvAgi, savxyaM koDugegaLu, caMdAhaNagaLiMdaleV naDeyabeVkeMba, savxtaMtarxteyanunx etitxhiDiyuva sidAdhxMta. 
\num{2} (\tashA) saMkalapx sidAdhxMta; vayxkitxyalilx yA vishavxdalilx saMkalapxshakitxyeV mUlaBUta tatatx vxveMdu parxtipAdisuva sidAdhxMta. 
\num{3} (otAtxya yA balAtAkxrakikxMta savxyaM perxVritakirxyeyanenxV avalaMbisabeVkeMdu parxtipAdisuva) savxyaMseVvA sidAdhxMta. 
\enum
\emng
\eentry

\bentry
\word{voluntarist} 
\pron{vAlaMTarisfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (cacfR \mo vugaLa \vi) savxtaMtarxtA sidAdhxMti; savxsahAyavAdi. 
\num{2} (\tashA) saMkalapxvAdi. 
\num{3} savxyaMseVvAvAdi. 
\enum
\emng
\eentry

\bentry 
\word[voluntary(1)]{voluntary} 
\pron{vAlaMTari} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} savx, savxyaM -- perxVrita; savxMta iSaTxdiMda, icePxyiMda -mADida yA mADuva yA mADabalalx; nibaRMdhitavAgi yA kaDADxyavAgi irada; savxsaMtoVSadiMda yA savxtaH mADida: \eng{voluntary contribution} savxsaMtoVSada kANike. 
\num{2} udedxVshapUvaRkavAda; icAPxpUvaRkavAda: \eng{voluntary waste} (hiDuvaLidAra yA bADigedAra mADida) udedxVshapUvaRkavAda nAsha, halAku. 
\num{3} veVtanarahita; saMbaLa ilalxda yA tegedukoLaLxda : \eng{voluntary work} veVtanarahita kelasa, udoyxVga. 
\num{4} (saMsethxya \vi) savxyaMperxVrita dAna, vaMtige, caMdA yA deVNigegaLiMda naDeyuva yA naDesuva. 
\num{5} (\birx) (shAleya \vi) savxyaMseVvA saMGavoMdu kaTiTxda Adare sathxLiVya shikaSxNa pArxdhikAravoMdu naDesuva, nivaRhisuva. 
\num{6} savxyaMseVveya; savxyaMperxVritavAgi mADida, utApxdisida, \mo. 
\num{7} (calane, mAMsaKaMDa yA aMgada \vi) saMkalapxpUvaRkavAda; icAPxniyaMtirxtavAda. 
\num{8} (aparAdhiya tapopxpipxgeya \vi) (bedarike, parxloVBana, \mo vugaLiMda Agirade) savxperxVraNeyiMdAda; KududxrAjiyiMda mADida: \eng{voluntary confession} savxyaMperxVrita tapopxpipxge. (\nAyxshA) (Asitx vagARvaNe yA iSaTxpatarxda parxkAra koDuvudara \vi) muPatAtxda; parxtiPalavAgi haNa yA beVrAvudeV parihAra bayasade koTaTx. 
\num{10} sahaja parxvaqtitxya; aperxVritavAda. 
\enum
\emng
\eentry

\bentry 
\word[voluntary(2)]{voluntary} 
\pron{vAlaMTari} 
\gl{\nA}(\bava\ \eng{voluntaries}). 
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (caciRna pArxthaRnege muMce, pArxthaRneya kAladalilx yA pArxthaRneya baLika naDeyuva) AgaRnf vAdayxda tanivAdana. 
\alnum{b} idara saMgiVta. 
\eanum
\numie
\num{2} (\pArxparx) (beVre bageya saMgiVtakekx \kanmu\ piVThikArUpadalilx naDesuva) AshusaMgiVta; pUvaRsidadhxteyilalxda saMgiVta. 
\num{3} (\ca) savxtaMtarxtA sidAdhxMti; cacuR yA shAlegaLu rASaTxrXda hatoVTige oLapaDade savxtaMtarxvAgi, janaru savxsaMtoVSadiMda niVDida vaMtigeyiMda naDeyabeVkeMdu vAdisuvavanu. 
\num{4} (sapxdheRgaLalilx) savxyaMperxVrita parxdashaRna; parxdashaRnakArana, sapxdhiRya AyekxgeV biTaTx visheVSa parxdashaRna. 
\enum
\emng
\eentry
 
\bentry
\wordnospeech{Voluntary Aid Detachment}{Voluntary Aid Detachment}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 savxyaMseVvA shushUrxSA saMsethx; savxyaMperxVraNeyiMda saMGaTisi naDesuva parxthama cikitesxya matutx dAdigaLa saMsethx. 
\emng
\eentry

\bentry 
\wordnospeech{voluntary conveyance}{voluntary conveyance}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{voluntary disposition}{voluntary disposition}. 
\emng
\eentry

\bentry
\wordnospeech{voluntary disposition}{voluntary disposition}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\nAyxshA) (Asitxya \vi) muPatutx vagARvaNe; parxtiPala, parihAra, tegedukoLaLxde Asitxyanunx vahisikoDuvudu. 
\emng
\eentry

\bentry
\word{voluntaryism} 
\pron{vAlaMT{ri}isaZmf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{voluntarism(1)}{voluntarism\((1, 3)\)}. 
\emng
\eentry

\bentry 
\word{voluntaryist} 
\pron{vAlaMT{ri}isfTx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{voluntarist}{voluntarist}. 
\emng
\eentry

\bentry 
\wordnospeech{voluntary school}{voluntary school}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\birx) savxsahAya pAThashAle; savxsaMtoVSadiMda caMdAhAki sAthxpisida, sathxLiVya vidAyxdhikAra saMsethxyiMda naDesalapxDuva shAle. 
\emng
\eentry

\bentry 
\word[volunteer(1)]{volunteer} 
\pron{vAlaMTiarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} savxyaM seVvaka: 
\banum
\alnum{a} yAvudeV kelasavanunx tAnAgiyeV keYgoLuLxvavanu. 
\alnum{b} seYnika seVvegoV beVrAvudeV seVvegoV savxMta icePxyiMda seVruvavanu; (\kanmu\ seYnika \ca) birxTaninxnalilx hiMde saMGaTisutitxdadx savxyaMseVvaka seYnikaranonxLagoMDa, Adare rASaTxrXvu shikaSxkaranunx, shasAtxrXsatxrXgaLanunx odagisutitxdadx savxyaM seVvaka daLadavanu. 
\eanum
\numie
\num{2} (yArU neDade) tAneVtAnAgi huTuTxva giDa; savxyaMruha. 
\enum
\emng
\eentry

\bentry
\word[volunteer(2)]{volunteer} 
\pron{vAlaMTiarf} 
\gl{\sakirx}
\expl{}
\bmng
 tAnAgi hegalu koDu; savxMta icePxyiMda, savxsaMtoVSadiMda seVve mADalu muMde baru; savxyaMseVve mADu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 savxyaMseVvakanAgu; seVve mADalu muMde bA. 
\emng
\eentry

\bentry 
\word[volunteer(3)]{volunteer} 
\pron{vAlaMTiarf} 
\gl{\gu}
\expl{}
\bmng
 (sasayxvagaRda \vi) savxyaMruha; tAneVtAnAgi huTuTxva. 
\emng
\eentry

\bentry 
\word[voluptuary(1)]{voluptuary} 
\pron{valapfTuyxari} 
\gl{\gu}
\expl{}
\bmng
 BoVgalAlaseya; viSayAsakatxnAda; iMdirxyasuKakAkxgi tahatahisuva; laMpaTatanada. 
\emng
\eentry

\bentry 
\word[voluptuary(2)]{voluptuary} 
\pron{valapfTuyxari} 
\gl{\nA}(\bava\ \eng{voluptuaries}). 
\bmng
 BoVgalAlasa; viSayAsakatx; viSayalaMpaTa. 
\emng
\eentry

\bentry 
\word{voluptuous} 
\pron{valapfTuyxasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} iMdirxyagaLanunx taNisuva; iMdirxyagaLige -- hitakaravAda, AhAlxdakaravAda: \eng{voluptuous music} sharxvaNAnaMdakaravAda saMgiVta. 
\num{2} BoVgalAlaseya; viSayasuKakekx saMbaMdhisida; viSayasuKadalilx AsakatxnAda; viSayAsakatx(nAda); laMpaTanAda: \eng{voluptuous woman} kAmAsakitxyuLaLx heMgasu. 
\enum
\emng
\eentry

\bentry 
\word{voluptuously} 
\pron{valapfTuyxasfli} 
\gl{\kirxvi}
\expl{}
\bmng
 BoVgalAlaseyiMda; viSayAsakatxnA(LA)gi. 
\emng
\eentry

\bentry 
\word{voluptuousness} 
\pron{valapfTuyxasfnisf} 
\gl{\nA}
\expl{}
\bmng
 BoVgalAlase; viSayAsakitx. 
\emng
\eentry

\bentry
\word[volute(1)]{volute} 
\pron{valUyxTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (ayoVnikf, koriMthiyanf matutx misharx, shilapxsheYligaLalilx satxMBagaLa meVle mADuva) suruLiyalaMkaraNa; AvataR; valaya. 
\num{2} suruLi, shaMKa -- siMpi; valUyxTa kulada, kaDalina udarapAdi, siMpi. 
\num{3} suruLi shaMKa; siMpiya suraLi cipupx. 
\enum
\emng
\eentry

\bentry
\word[volute(2)]{volute} 
\pron{valUyxTf} 
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ \savi) suruLisutitxda; AvataRgoMDa; valayita. 
\emng
\eentry

\bentry 
\word{voluted} 
\pron{valUyxTiDf} 
\gl{\gu}
\expl{}
\bmng
suruLiyalaMkaraNa mADida; AvataRgaLanunx kaDeda. 
\emng
\eentry

\bentry 
\word{volution} 
\pron{valU(lUyx)Sanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} suruLi (sutitxkoMDiruvudu); AvataR(na); valayana. 
\num{2} suruLisuruLiyAgiruvudu; tiricikoMDiruvudu. 
\num{3} (suruLi siMpiya) suruLi(gaLu). 
\num{4} (\aMrashA) valayana; suruLi; nulicu; tiricu. 
\enum
\emng
\eentry

\bentry 
\word{volva} 
\pron{vAlavx} 
\gl{\nA}
\expl{}
\bmng
 (\savi) (balitilalxda aNbeya meVlaNa) poreyaMtha teLu musuku. 
\emng
\eentry

\bentry 
\word{vomer} 
\pron{voVmarf} 
\gl{\nA}
\expl{}
\bmng
 (\aMrashA) siVrikAsithx; (manuSayxnalUlx halavu kasheVrukagaLalUlx) mUgina hoLeLxgaLanunx viBAgisuva teLumULe. 
\emng
\eentry

\bentry
\word[vomit(1)]{vomit} 
\pron{vAmiTf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{vomited;} \vakaq}
\eng{vomiting}). \bmng
\bnum
\num{1} vamana, vAMti -- mADu; kakukx; kAru. 
\num{2} (jAvxlAmuKi, hogekoLave, \mo vugaLa \vi) (raBasadiMda) kAru; horagese. 
\enum
\emng
\eentry

\bentry 
\word[vomit(2)]{vomit} 
\pron{vAmiTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vAMti; vAMtiyAdadudx; vamana. 
\num{2} (\pArxparx) vAmaka; vamanakAraka; vAMti mADisuva auSadha. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{black vomit} kapu pxvAMti; kAmAleroVgadalilx vAMtiyAguva kapu px padAthaR. 
\emng
\eentry

\bentry
\word{vomitorium} 
\pron{vAmiToVriamf} 
\gl{\nA}
\expl{(\bava\ \eng{vomitoria} \ucAcx\ vAmiToVria). }
\bmng
 = \hyperlink{vomitory(2)}{$^2$vomitory}. 
\emng
\eentry

\bentry
\word[vomitory(1)]{vomitory} 
\pron{vAmiTari} 
\gl{\gu}
\expl{}
\bmng
vAmaka; vamanakAraka; vAMti mADisuva. 
\emng
\eentry

\bentry 
\word[vomitory(2)]{vomitory} 
\pron{vAmiTari} 
\gl{\nA}
\expl{(\bava\ \eng{vomitories}).}
\bmng
 (\roV\ \pArxkatx) (perxVkaSxkara parxveVsha matutx nigaRmanagaLigAgi kaTiTxda, nATakashAleya yA vatuRla nATakashAleya) naDave(gaLalolxMdu); ONi(gaLalolxMdu). 
\emng
\eentry

\bentry 
\word[voodoo(1)]{voodoo} 
\pron{vUDU} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (vesfTxiMDiVsfna niVgorxV janaralilx parxcalitavAgiruva) mATa, yaMtarxmaMtarxgaLalilx naMbike. 
\num{2} aBicAra; mATa; shUnayx; mATada parxyoVga. 
\num{3} mATagAra; maMtarxvAdi. 
\num{4} mATada maMtarx; mATadalilx baLasuva maMtarx. 
\enum
\emng
\eentry

\bentry
\word[voodoo(2)]{voodoo} 
\pron{vUDU} 
\gl{\sakirx}(vataRmAna \parxpu\ \Eva\ \eng{voodoos}; \BU\ matutx
\BUkaq\ \eng{voodooed}). \bmng
 mATa mADu; mATadiMda vashapaDisiko. 
\emng
\eentry

\bentry 
\word{voodooism} 
\pron{vUDUisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
 aBicAra; mAT(gArike); mATada parxyoVga. 
\emng
\eentry

\bentry 
\word{voodooist} 
\pron{vUDUisfTx} 
\gl{\nA}
\expl{}
\bmng
 mATagAra; maMtarxvAdi. 
\emng
\eentry

\bentry 
\wordwithhyphen{hyp-vora}{-vora} 
\pron{-vara} 
\gl{\saupa}
\expl{}
\bmng
 (pArxNigaLanunx avu tinunxva AhAravananxnusarisi vagiRVkarisuva nAmapadagaLanunx racisuvalilx) -- AhArigaLu, -- BakaSxkagaLu eMbathaRda \saupa : \eng{carnivora} mAMsAhAri (pArxNi)vagaR. 
\emng
\eentry

\bentry
\word{voracious} 
\pron{vareVSasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hoTeTxbAkatanada; kULubAkatanada; bakAsuranaMtha: \eng{voracious eater} bakAsuranaMte tinunxvavanu. 
\num{2} Oduvudaralilx apAra Asakitxyiruva; savaRgArxhiyAda: \eng{a voracious reader} Oduvudaralilx apAra Asakatx. 
\enum
\emng
\eentry

\bentry
\word{voraciously} 
\pron{vareVSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
savaRgArxhiyAgi: 
\banum
\alnum{a} (tinunxvudaralilx) hoTeTxbAkanaMte; bakAsuranaMte. 
\alnum{b} (Oduvudaralilx) apAra AsakitxyiMda. 
\eanum
\emng
\eentry

\bentry 
\word{voraciousness} 
\pron{vareVSasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hoTeTxbAkatana; kULubAkatana; tinunxvudaralilx ati durAsheyiruvike. 
\num{2} Oduvudaralilx apAra Asakitx. 
\enum
\emng
\eentry

\bentry 
\word{voracity} 
\pron{varAYxsiTi} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{voraciousness}{voraciousness}. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-vore}{-vore} 
\pron{-voVrf} 
\gl{\saupa}
\expl{}
\bmng
 \eng{-} AhAri; \eng{-} BakaSxka; \eng{-} tinunxva; AhArAnusAra vagiRVkarisiruva pArxNivagaRgaLa peYki oMdu pArxNi, \udA\ \eng{herbivore} sasAyxhAri. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-vorous}{-vorous} 
\pron{-varasf} 
\gl{\saupa}
\expl{}
\bmng
 \sA\ tinunxva -- BakaSxka, -AhAri(yAda) eMbathaRda guNavAcakagaLanunx racisuva \saupa , \udA\ \eng{carnivorous} mAMsAhAri(yAda). 
\emng
\eentry

\bentry
\word{vortex} 
\pron{vATeRkfsx} 
\gl{\nA}
\expl{(\bava\ \eng{vortexes}, yA \eng{vortices} \ucAcx\ vATiRsiVsfZ). }
\bmng
\bnum
\num{1} AvataR; suLi; gataR; sututxtitxruva darxva yA anila, \kanmu\ niVrusuLi yA gALisuLi. 
\num{2} (\Bwvi) GUNaRBAga; AvataRcalanada kaNagaLuLaLx darxvada yA anilada BAga. (\rUpa) suLi; gataR; hatitxra baMdavaranunx, tananx parxBAvakekx sikikxdavaranunx suLiyaMte nuMgibiDuva vayxvasethx, vaqtitx, udayxma, \mo vu: \eng{the vortex of society} samAjada suLi. 
\enum
\emng
\eentry
 
\bentry
\word{vortex-ring} 
\pron{vATeRkfsxriMgf} 
\gl{\nA}
\expl{}
\bmng
uMgurasuLi; BarxmaNAkaSxvu oMdu saMvaqta vakarxreVKeyAgiruva suLi, \udA\ (cuMgANiyiMda yA taMbAku seVdutitxruvavana bAyiMda horaDuva) hoge suruLi. 
\emng
\eentry

\bentry 
\word{vortical} 
\pron{vATiRkalf} 
\gl{\gu}
\expl{}
\bmng
 suLiyaMtha; AvataRdaMtha; suLi sututxva. 
\emng
\eentry

\bentry 
\word{vorticella} 
\pron{vATiRsel} 
\gl{\nA}
\expl{}
\bmng
 GaMTANupArxNi; (taMguniVru \mo vugaLalilx kaMDubaruva, vATiRseliDiV vaMshada, sutatxlU kUdaluLaLx, gaMTeyAkArada tUtiruva, koLaveyaMtha daMDavuLaLx) GaMTANujiVvi vagaRdalolxMdu. 
\emng
\eentry

\bentry
\word{vorticism} 
\pron{vATiRsisaZ(sZf)mf} 
\gl{\nA}
\expl{}
\bmng
 (kale) (Adhunika nAgarikateya AvataRgaLanunx taLahadiyAgiTuTxkoMDu, suruLi yA suLivinAyxsada kalAkaqtigaLanunx nimiRsuva) AvataRpaMtha; suruLipaMtha. 
\emng
\eentry

\bentry 
\word{vorticist} 
\pron{vATiRsisfTx} 
\gl{\nA}
\expl{}
\bmng
 AvataRvAdi: 
\banum
\alnum{a} (\tashA) [deVkAtaR(\eng{Descartes}) naMte] vishavxvu Bwtadarxvayxda AvataRnarUpa eMdu parxtipAdisuva dAshaRnika. 
\alnum{b} (kale) (Adhunika nAgarikateya AvataRgaLanunx taLahadiyAgiTuTx koMDu, suruLigaLanunx yA suLigaLanunx baLasi kalAkaqtigaLanunx nimiRsuva citarxkAra, leVKaka, \mo) AvataRpaMthada, suraLipaMthada kalAvida. 
\eanum
\emng
\eentry

\bentry 
\word{vorticity} 
\pron{vATiRsiTi} 
\gl{\nA}
\expl{}
\bmng
 AvataRte; suLitana; suLi sututxtitxruvike. 
\emng
\eentry

\bentry 
\word{vorticose} 
\pron{vATiRkoVsf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vorticular}{vorticular}. 
\emng
\eentry

\bentry 
\word{vorticular} 
\pron{vATiRkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
 suLiyaMtha; AvataRvAgiruva; suLi sututxva. 
\emng
\eentry

\bentry
\word{vortiginous} 
\pron{vATiRjinasf} 
\gl{\gu}
\expl{}
\bmng
 (\pArxparx) suLisututxva; AvataR racisuva. 
\emng
\eentry

\bentry 
\word{votable} 
\pron{voVTbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 (\viparx) (aBi)mata niVDabalalx; matadAnada hakukxLaLx; voVTina hakukxLaLx. 
\emng
\eentry

\bentry
\word{votaress} 
\pron{voVTarisf} 
\gl{\nA}
\expl{}
\bmng
 (\sitxrXV) (vayxkitx, vaqtitx, vayxvasethx, paMtha, \mo vugaLa) anuyAyini; Baketx. 
\emng
\eentry

\bentry 
\word{votarist} 
\pron{voVTarisfTx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{votary(2)}{votary\((2)\)}. 
\emng
\eentry

\bentry 
\word{votary} 
\pron{voVTari} 
\gl{\nA}
\expl{(\bava\ \eng{votaries}, \sitxrXV\ \eng{votaress}).  }
\bmng
\bnum
\num{1} (BagavaMta \mo vara) upAsaka; Bakatx. 
\hypertarget{votary(2)}{} 
\num{2} (vayxkitx, vaqtitx, vayxvasethx, paMtha, \mo vugaLa) sharxdAdhxnivxta anuyAyi; aviciPxnanx Bakatx; kaTATx samathaRka. 
\enum
\emng
\eentry

\bentry
\word[vote(1)]{vote} 
\pron{voVTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} voVTu; (aBi)mata; aBayxthiRyanunx Ayekx mADuvudu, kAnUnanunx maMjUru mADuvudu, TharAvanunx opu pxvudu, \mo\ saMdaBaRgaLalilx matadAna patarxda meVle gurutisuvudu, keYetutxvudu, kUguvudu yA beVrAvudeV riVtiyalilx niVDuva aBipArxya: \eng{I shall give my vote to} (\engit{or} \eng{for) the Labour candidate} nAnu kAmiRka pakaSxda aBayxthiRge yA aBayxthiRya paravAgi nananx mata niVDutetxVne. \eng{the resolution was put to the vote and passed without a dissent vote} A TharAvanunx voVTige hAkalAgi Binanxmatavilalxde opapxlAyitu. 
\numi{2} (\birx) 
\banum
\alnum{a} (sakARravu koVrida haNada \vi) (pAliRmeMTina) bahumatada anumoVdane. 
\alnum{b} bahumatABipArxya; bahumatadiMda vayxkatxvAda aBipArxya. 
\eanum
\numie
\num{3} voVTugaLa, matagaLa -- motatx; (oMdu pakaSx \mo vu yA oMdu pakaSx \mo vakekx koTaTx yA koDabahudAda oTuTx voVTugaLu): \eng{he will lose the Labour vote} avanu kAmiRka pakaSxda matagaLanunx kaLedukoLuLxvanu. 
\num{4} (\kanmu\ rAjayxda cunAvaNeyalilx) matAdhikAra; matadAnada, voVTu mADuva -- hakukx: \eng{Swiss women now have the vote} sivxTZsxlaRMDina heMgusarige Iga voVTina, voVTumADuva hakikxde. 
\num{5} voVTu ciVTi; voVTanunx gurutisalu baLasuva kAgada \mo vu. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{put to a} (\engit{or} \eng{the) vote} matakekx hAki niNaRyisu, niNaRyakekx oDuDx yA opipxsu. 
\num{2} \eng{take a vote on} (yAvudeV viSayada bagegx) mata tegeduko; (yAvudanenxV) matadAnada mUlaka niNaRyisu. 
\num{3} \eng{vote of censure} = \hyperlink{vote1 pagu5}{?pagu? \((5)\)}. 
\num{4} \eng{vote of confidence} vishAvxsa mata; ADaLita pakaSxvanunx adhikasaMKAyxtaru beMbalisuvareMdu toVrisuva bahumata. 
\hypertarget{vote1 pagu5}{} 
\num{5} \eng{vote of no confidence} avishAvxsa mata; ADaLita pakaSxvanunx adhika saMKAyxtaru beMbalisuvudilalxveMdu sUcisuva bahumata. 
\enum
\emng
\eentry

\bentry
\word[vote(2)]{vote} 
\pron{voVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} voVTumADu; matahAku; bahumatadiMda 
\banum
\alnum{a} kAnUnu mADu. 
\alnum{b} TharAvanunx anumoVdisu. 
\alnum{c} haNa maMjUru mADu. 
\alnum{d} vayxkitxyanunx samiti \mo vugaLa sadasayxnanAnxgi cunAyisu yA samiti \mo vugaLa sadasayxtavxdiMda vajAmADu: \eng{was voted off} (\engit{or} \eng{voted out of) the committee} bahumata hAki avananunx samitiyiMda vajA mADalAyitu. 
\eanum
\numie
\num{2} (\AmA) savaRsamamxtiyiMda, savARnumatadiMda -- sAru, GoVSisu; sAmAnAyxBipArxyavAgi heVLu: \eng{the scheme was voted a failure} A yoVjane viPalavAyiteMdu (savaRsamamxtiyiMda) sAralAyitu. 
\num{3} (\AmA) tananx sUcaneyanunx horageDahu; salahe parxkaTapaDisu: \eng{I vote that we should go out on a picnic} nAvu oMdu vihArakAkxgi UriMdAce hoVgabeVkeMdu nAnu sUcisutetxVne, salahe mADutetxVne. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
mata -- hAku, calAyisu; voVTu mADu. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\hypertarget{vote2 pagu1}{} 
\bnum
\num{1} \eng{vote down} (sUcane \mo vanunx bahumatadiMda) taLiLxhAku; nirAkarisu; soVlisu; viPalagoLisu. 
\num{2} \eng{vote in} voVTugaLa mUlaka -- AyekxmADu, cunAyisu. 
\numi{3} \eng{vote out} 
\banum
\alnum{a} (vayxkitxyanunx samiti \mo vugaLa) sadasayxtavxdiMda vajAmADu, horadUDu. 
\alnum{b} = \hyperlink{vote2 pagu1}{?pagu? \((1)\)}. 
\eanum
\numie
\num{4} \eng{vote with one's feet} (\AmA) (obabxnu) tananx hAjarAtiyiMda yA geYruhAjariyiMda tananx aBipArxyavanunx sUcisu. 
\enum
\emng
\eentry

\bentry
\word{voteless} 
\pron{voVTflisf} 
\gl{\gu}
\expl{}
\bmng
 voVTilalxda; matAdhikAravilalxda; voVTina hakikxlalxda. 
\emng
\eentry

\bentry
\word{voter} 
\pron{voVTarf} 
\gl{\nA}
\expl{}
\bmng
 matadAra(Lu): 
\banum
\alnum{a} aBimata koDuva, matadAnada -- hakukxLaLxvanu(Lu). 
\alnum{b} mata calAyisuva vayxkitx. 
\eanum
\emng
\eentry

\bentry
\word{voting} 
\pron{voVTiMgf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} matadAna; bahumatadiMda 
\banum
\alnum{a} aBayxthiRyanunx Ayekx mADuvudu. 
\alnum{b} kAnUnu mADuvudu. 
\alnum{c} anumoVdisuvudu. 
\alnum{d} haNa maMjUru mADuvudu, samiti \mo vakekx cunAyisuvudu yA samiti \mo vugaLiMda vajA mADuvudu. 
\eanum
\numie
\num{2} (\AmA) savARnumatadiMda -- sAruvudu, GoVSisuvudu. 
\enum
\emng
\eentry

\bentry
\word{voting-machine} 
\pron{voVTiMgfmaSiVnf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ \ame dalilx) (matagaLanunx gurutisikoLuLxva savxyaMcAlita) matadAnayaMtarx. 
\emng
\eentry

\bentry
\word{voting-paper} 
\pron{voVTiMgfpeVparf} 
\gl{\nA}
\expl{}
\bmng
 matadAnada ciVTi; voVTu ciVTi; mata(dAna)patarx. 
\emng
\eentry

\bentry 
\wordnospeech{voting stock}{voting stock}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (baMDavALa patarxda mAliVkanige) matadAnada hakakxnunx, ahaRteyanunx, koDuva kaniSaThx parxmANada baMDavALa patarxda motatx. 
\emng
\eentry

\bentry 
\word{votive} 
\pron{voVTivf} 
\gl{\gu}
\bmng
 harake kaTiTxkoMDa yA salilxsida; harakeya; muDupina: \eng{votive offering} harakeya salilxke yA samapaRNa. \eng{votive tablet} harakeya Palaka, shAsana. 
\emng
\eentry

\bentry
\wordnospeech{votive mass}{votive mass}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (cacuR) harake salilxkegAgi naDesuva visheVSa pUje, pArxthaRne. 
\emng
\eentry

\bentry 
\word{vouch} 
\pron{vwcf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx) (heVLikeyanunx sAkaSxyX \mo vugaLa AdhAradiMda) rujuvAtu mADikoDu; daqDhiVkarisu; sithxrapaDisu; KAtirxpaDisu; sAthxpisu. 
\num{2} (\pArxparx) AdhAravAgi, parxmANavAgi -- koDu, ulelxVKisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
 hoNeyAgu; javAbAdxranAgu: \eng{I will vouch for the truth of this} idara satayxtege nAnu hoNegAranAguvenu. \eng{I will vouch for his honesty} avana pArxmANikatege nAnu javAbAdxranAguvenu. 
\emng
\eentry

\bentry 
\word{voucher} 
\pron{vwcarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx, heVLike, \mo vugaLa paravAgi) daqDhiVkaraNakAra; KAtirxgAra. 
\num{2} hoNegAra; javAbAdxra. 
\num{3} haNa saMdAyada rashiVti; salilxkeciVTi; (lekAkxcAra sariyAgide eMba) parxmANapatarx. 
\num{4} voVcaru; haNa saMdide eMbaudara gurutAgi, vasutxgaLigoV seVvegaLigoV badalAgi vinimaya mADabahudAda parxmANa patarx, ciVTi, huMDi. 
\enum
\emng
\eentry

\bentry 
\word{vouchsafe} 
\pron{vwcfseVphf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (koDuva, niVDuva) kaqpemADu; dayapAlisu; anugarxhisu: \eng{he vouchsafed me a visit} Ata nanage BeVTi niVDuva kaqpe mADida. 
\num{2} (yAvudanenxV mADuva) kaqpemADu. 
\enum
\emng
\eentry

\bentry 
\word{voussoir} 
\pron{vUsAvxrf} 
\gl{\nA}
\expl{}
\bmng
 kamAnu beNekalulx; kamAnu kaTaTxDada, beNeyAkArada kalulxgaLalolxMdu. 
\emng
\eentry

\bentry 
\word[vow(1)]{vow} 
\pron{vw}
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} (BagavaMtanige, yAvudeV deVvatege, saMtanige, saMtaLige) 
\banum
\alnum{a} (parxmANa mADikoTaTx) vacana; shapatha; parxtijecnx; ANe; BASe. 
\alnum{b} (kaTiTxkoMDa) harake; (kaTiTxTaTx) muDupu. 
\alnum{c} keYgoMDa varxta, noVMpi. 
\hypertarget{vow(1)2}{} 
\eanum
\numie
\num{2} jAcnxnasAnxnada -- parxtijecnxgaLu, parxmANagaLu; jAcnxnasAnxnada kAladalilx A saMsAkxra paDedavanAgali, Atana dhamiVRya pitaneV \mo\ baMdhugaLAgali, keYgoLuLxva parxtijecnx. 
\hypertarget{vow(1)3}{} 
\num{3} (\bava dalilx) parxNayigaLa yA satipatiyara parasapxra niSAThxparxtijecnxgaLu; avaru obabxrigobabxru parapuruSa, parasitxrXVyaranunx kAmisade, parasapxra dorxVha esagade, iruveveMdu mADuva parxmANagaLu. 
\hypertarget{vow(1)4}{} 
\num{4} (\bava dalilx) (saMnAyxsi yA saMnAyxsiniyu veYrAgayx, barxhamxcayaR yA sAdhivxVtavx matutx saMyamagaLige AjiVva badadhxrAgiruvudAgi diVkASxkAladalilx parxmANa mADuva) saMnAyxsa parxtijecnxgaLu yA diVkASxparxtijecnxgaLu. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{baptismal vows} = \hyperlink{vow(1)2}{$^1$vow\((2)\)}. 
\num{2} \eng{lovers'} (\engit{or} \eng{marriage) vows} = \hyperlink{vow(1)3}{$^1$vow\((3)\)}. 
\num{3} \eng{monastic vows} = \hyperlink{vow(1)4}{$^1$vow\((4)\)}. 
\num{4} \eng{under a vow} varxtabadadhxnA(LA)gi yA parxtijAcnxbadadhx nA(LA)gi. 
\enum
\emng
\eentry

\bentry
\word[vow(2)]{vow} 
\pron{}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vidhuyxkatxvAgi) harake kaTiTxko; harasiko; BagavaMtanige yA deVvatege yA saMtani(Li)ge muDipu salilxsuvudAgi shAsotxrXVkatx riVtiyalilx parxmANa mADu: \eng{he vowed a temple to Ganesha} avanu gaNeVshanige oMdu deVvasAthxna kaTiTxsuvudAgi harake kaTiTxkoMDa, harasikoMDa. 
\num{2} varxta toDu, hiDi; noVMpiyanunx mADuvudAgi parxmANamADi, niSeThxyiMda adanunx anuSAThxna mADu. 
\num{3} shapatha, parxtijecnx -- mADu, toDu: \eng{he vowed vengeance against the aggressor} avanu AkarxmaNakArana meVle hage tiVrisikoLuLxva, parxtiVkAra esaguva, shapatha mADida. 
\num{4} (\pArxparx) parxmANapUvaRka -- sAru, GoVSisu. 
\enum
\emng
\eentry

\bentry
\word{vowel} 
\pron{vwalf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} savxra; vayxMjanakikxMta hecAcxgi bAyi terediruva matutx ucAcxrAMshavanunx rUpisabalalx, dhavxnitaMtugaLa kaMpanadiMda uMTAguva, keVLisuvaMtha GaSaRNeyilalxda BASAdhavxni. 
\num{2} savxrAkaSxra, \udA\ iMgilxSinalilx \eng{a, e, i, o, u}. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{indeterminate} (\engit{or} \eng{neutral} \engit{or} \eng{obscure) vowel} madhayxsathx savxra; asapxSaTx yA aniSakxqqSaTx ucAcxraNeya savxra, udA. \eng{cous\underline{i}n, reas\underline{o}n, hadd\underline{o}ck,} \mo\ padagaLalilxya eraDaneya ucAcxraNAMshada \eng{i, o, o} eMba laGuvAgi ucAcxraNeyAguva savxra (= \hyperref{kandict_s.pdf}{S}{schwa}{schwa}). 
\emng
\eentry

\bentry
\wordnospeech{vowel gradation}{vowel gradation}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_a.pdf}{A}{ablaut}{ablaut}. 
\emng
\eentry

\bentry
\word{vowelise} 
\pron{vwaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vowelize}{vowelize}. 
\emng
\eentry

\bentry
\word{vowelize} 
\pron{vwaleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 (hiVbaUrx \mo\ BASegaLa saMkiSxpatx lipiyalilx) savxracihenx -- hAku, seVrisu. 
\emng
\eentry

\bentry
\word{vowelled} 
\pron{vwalfDx} 
\gl{\gu}
\expl{}
\bmng
 savxrayukatx; savxravuLaLx (\saMpa\ vAgi saha baLake). 
\emng
\eentry

\bentry
\word{vowelless} 
\pron{vwalflisf} 
\gl{\gu}
\expl{}
\bmng
 savxrarahita; savxravilalxda. 
\emng
\eentry

\bentry 
\word{vowelly} 
\pron{vwali} 
\gl{\kirxvi}
\expl{}
\bmng
 savxravAgi; savxrarUpadalilx: \eng{in `umbrella' `u' is used vowelly. `umbrella'} eMba padadalilx \eng{`u'} eMba akaSxravu savxravAgi baLasalapxTiTxde. 
\emng
\eentry

\bentry
\wordnospeech{vowel mutation}{vowel mutation}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_u.pdf}{U}{umlaut(2)}{umlaut(2)}. 
\emng
\eentry

\bentry
\wordRemoveSpace{vowel-point}{vowel point} 
\pron{}
\gl{\nA}
\expl{}
\bmng
 savxrabiMdu; hiVbaUrx \mo\ BASegaLalilxya savxragaLanunx sUcisuva cihenxgaLalilx oMdu. 
\emng
\eentry

\bentry 
\word{vox} 
\pron{vAkfsx} 
\gl{\nA}
\expl{}
\bmng
 vANi; mAtu. 
\emng
\eentry

\bentry 
\wordf{vox angelica} 
\pron{vAkfsx AYxMjelika} 
\gl{\nA}
\expl{\Latin}
\bmng
deVvavANi; iMpAda kaMpanayutavAda nAdavanunx huTiTxsuva, AgaRnf vAdayxda kiVlu. 
\emng
\eentry

\bentry 
\wordf{vox et praeterea nihil} 
\pron{vAkfsx eTf pirxTiVriya neYhilf} 
\gl{\nA}
\expl{\Latin }
\bmng
bariya shabadx, matetxVnU ilalx; athaRvilalxda bariya solulx; athaRhiVnavAda mAtu. 
\emng
\eentry

\bentry 
\wordf{vox humana} 
\pron{vAkfsx hUyxmAna} 
\gl{\nA}
\expl{\Latin}
\bmng
mAnava, manuSayx -- vANi; manuSayxdhavxniyanunx hoVluvudenanxlAda nAdavanunx huTiTxsuva, AgaRnf vAdayxda kiVlu, piVpi. 
\emng
\eentry

\bentry 
\wordRemoveSpace{vox-pop}{vox pop} 
\pron{vAkfsx pApf} 
\gl{\nA}
\expl{}
\bmng
 (reVDiyoV parxsAra) (\AmA) (sAvaRjanikara TiVkegaLu, heVLikegaLu yA I bageya saMdashaRnagaLu sUcisuva) janABipArxya; sAvaRjanikABipArxya. 
\emng
\eentry

\bentry 
\wordf{vox populi} 
\pron{vAkfsx pApuyxliV(leY)} 
\gl{\nA}
\expl{\Latin}
\bmng
janatAvANi; loVkavANi; janABipArxya; janasAmAnayxda naMbike, aBimata, tiVpuR, \mo vu. 
\emng
\eentry

\bentry
\word[voyage(1)]{voyage} 
\pron{vAijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} yAna; parxyANa; yAterx; \kanmu\ diVGaRdUrada, samudarx yA jalamAgaRda yA vAyumAgaRda yA bAhAyxMtarikaSxda yAterx. 
\num{2} parxyANada, yAterxya -- kathana, vaNaRne. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{bon voyage}! suKaparxyANa (Agali)! 
\emng
\eentry

\bentry
\word[voyage(2)]{voyage} 
\pron{vAijf} 
\gl{\sakirx}
\expl{}
\bmng
 yAterxmADu; \kanmu\ jala yA vAyu mAgaRvAgi -- saMcarisu, parxyANamADu. 
\emng
\eentry

\bentry 
\word{voyageable} 
\pron{vAijbflf} 
\gl{\gu}
\expl{}
\bmng
 saMcArAhaR; saMcArayoVgayx; saMcarisalu, parxyANa mADalu -- sAdhayxvAda. 
\emng
\eentry

\bentry 
\word{voyager} 
\pron{vAijarf} 
\gl{\nA}
\expl{}
\bmng
 parxyANika; yAtirxka; payaNiga. 
\emng
\eentry

\bentry 
\word{voyageur} 
\pron{vAyASaZrf, vAiaSaZrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ \ca) sAgaNegAra; parxyANikaranUnx sarakugaLanUnx oMdu vAyxpArakoVThiyiMda matotxMdakekx sAgisuvavanu. 
\num{2} (kenaDA) aMbiga; doVNi naDesuvavanu. 
\enum
\emng
\eentry

\bentry
\word{voyeur} 
\pron{vAyarf} 
\gl{\nA}
\expl{}
\bmng
 leYMgikadashaRna taqpatx; parara meYthunada yA jananAMgagaLa dashaRnadiMda, leYMgika taqpitx, suKa anuBavisuva vayxkitx. 
\emng
\eentry

\bentry 
\word{voyeurism} 
\pron{vAyarisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
 leYMgikadashaRna taqpitx; parara meYthunada yA jananAMgagaLa dashaRnadiMda leYMgika taqpitx, suKa paDeyuvike. 
\emng
\eentry

\bentry
\word{voyeuristic} 
\pron{vAyarisiTxkf} 
\gl{\gu}
\expl{}
\bmng
 leYMgikadashaRna taqpitxya; parara meYthunada yA parara jananAMgagaLa dashaRnadiMda leYMgika taqpitx, suKa paDeyuva. 
\emng
\eentry

\bentry
\word{voyeuristically} 
\pron{vAyarisiTxkali} 
\gl{\kirxvi}
\expl{}
\bmng
 leYMgikadashaRna taqpitxyaMte; parara meYthunada yA jananAMgagaLa yA mamARMga dashaRnadiMda -- leYMgika taqpitx, suKa paDeyuvaMte. 
\emng
\eentry

\bentry
\wordnospeech{VP}{VP}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Vice-President.} 
\emng
\eentry

\bentry
\wordnospeech{VR}{VR}
\pron{?}
\gl{\saMkiSx}
\expl{\Latin }
\bmng
\bnum
\num{1} \eng{Queen Victoria} \engit{(Victoria Regina)}. 
\num{2} \eng{variant reading.} 
\enum
\emng
\eentry

\bentry 
\wordnospeech{VS}{VS}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Veterinary Surgeon.} 
\emng
\eentry

\bentry
\wordnospeech{vs.}{vs.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{versus.} 
\emng
\eentry


\bentry
\word{V-sign} 
\pron{viVseYnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} \eng{V} -- sanenx; tirasAkxra, kiVLu riVtiya apahAsayx, \mo vugaLanunx sUcisalu modaleraDu beraLugaLanunx agalisi keYya hiMBAgavanunx horatoVrisuva \eng{V} AkArada sanenx. 
\num{2} \eng{V} -- sanenx, saMkeVta; vijayavanunx sUcisalu toVruberaLu matutx madhayxberaLugaLanunx agalisi, uLida beraLugaLanunx maDici, aMgeYyanunx horakANuvaMte toVrisuva \eng{V} AkArada saMkeVta. 
\enum
\emng
\eentry

\bentry
\wordnospeech{VSO}{VSO}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Voluntary Service Overseas.} 
\emng
\eentry

\bentry
\wordnospeech{VSOP}{VSOP} 
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Very Special Old Pale} (bArxMdi). 
\emng
\eentry

\bentry 
\wordnospeech{VT}{VT}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 (\ame) \eng{Vermont} (aMce baLake). 
\emng
\eentry

\bentry
\wordnospeech{Vt.}{Vt.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{Vermont.} 
\emng
\eentry

\bentry
\wordnospeech{VTO}{VTO}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{vertical take-off.} 
\emng
\eentry

\bentry
\word{VTOL} 
\pron{viVTAlf} 
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{vertical take-off and landing.} 
\emng
\eentry

\bentry
\word{V-2} 
\pron{viVTU} 
\gl{\nA}
\expl{}
\bmng
 viVTU; (\eng{1939-45}ra yudadhxdalilx baLasida) rAkeTfcAlita, oMdu vidhada jamaRnf kiSxpaNi. 
\emng
\eentry

\bentry
\word{vug} 
\pron{vagf} 
\gl{\nA}
\expl{}
\bmng
 saPxTika kuhara; haraLugaLu sAlugaTiTxruva kalulxkuLi, shilAkuhara. 
\emng
\eentry

\bentry 
\word{vuggy} 
\pron{vagi} 
\gl{\gu}
\expl{}
\bmng
 haraLugaLu sAlugaTiTxruva kalulxkuLiya, shilAkuharada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{vugular} 
\pron{vaguyxlarf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vuggy}{vuggy}. 
\emng
\eentry

\bentry 
\word{vulcanic} 
\pron{valAkxYxnikf} 
\gl{\gu}
\expl{}
\bmng
 \eng{volcanic} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{vulcanise} 
\pron{valakxneYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vulcanize}{vulcanize}. 
\emng
\eentry

\bentry
\word{vulcanite}
\pron{valakxneYTf}
\gl{\nA}
\expl{}
\bmng
 valakxneYTf; gaMdhaka (saMsAkxra paDeda) rababxrf, eboneYTf. 
\emng
\eentry

\bentry 
\word{vulcanizable} 
\pron{valakxneYsaZbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 valakxniVkarisabahudAda. 
\emng
\eentry

\bentry 
\word{vulcanization} 
\pron{valakxneYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
 valakxniVkaraNa; rababxrf yA rababxrfnaMtha sAmagirxya sithxtisAthxpakatavx, balagaLanunx hecicxsalu, adanunx \kanmu\ hecicxna tApadalilx, gaMdhaka \mo vugaLiMda saMsakxrisuvudu. 
\emng
\eentry

\bentry
\word{vulcanize} 
\pron{valakxneYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 valakxniVkarisu; gaMdhaka saMsAkxra koDu; rababxrf yA rababxrfnaMtha sAmagirxya sithxtisAthxpakatavxvanUnx balavanUnx hecicxsalu \kanmu\ hecicxna tApadalilx, gaMdhaka \mo vugaLiMda adanunx saMsakxrisu. 
\emng
\eentry

\bentry 
\word{vulcanizer} 
\pron{valakxneYsaZrf} 
\gl{\nA}
\expl{}
\bmng
 valakxniVkAri; rababxrf \mo\ sAmagirxyanunx gaMdhaka \mo vugaLiMda saMsakxrisuva vayxkitx yA sAdhana. 
\emng
\eentry

\bentry
\word{vulcanological} 
\pron{valakxnalAjikalf} 
\gl{\gu}
\expl{}
\bmng
 jAvxlAmuKi vijAcnxnada. 
\emng
\eentry

\bentry
\word{vulcanologist} 
\pron{valakxnAlajisfTx} 
\gl{\nA}
\expl{}
\bmng
 jAvxlAmuKi vijAcnxni; jAvxlAmuKiyanunx veYjAcnxnikavAgi adhayxyana mADidavanu. 
\emng
\eentry

\bentry
\word{vulcanology} 
\pron{valakxnAlaji} 
\gl{\nA}
\expl{}
\bmng
 jAvxlAmuKi vijAcnxna; jAvxlAmuKiya veYjAcnxnika adhayxyana. 
\emng
\eentry

\bentry
\word{vulgar} 
\pron{valagxrf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pAmara janateya; janasAmAnayxra. 
\num{2} nayanAjUkilalxda; oraToraTAda; asaMsakxqqta; gArxmayx; asaBayx; kiVLuriVtiya; pArxkaqta: \eng{vulgar expressions} gArxmayx padagaLu. \eng{vulgar mind} asaMsakxqqta manasusx. \eng{vulgar tastes} kiVLu, halAkx -- aBirucigaLu. \eng{vulgar finery} oraToraTAda alaMkAra. 
\num{3} sAmAnayx; baLakeya; rUDhiya; cAlitxyalilxruva; parxcalitavAda: \eng{vulgar errors} sAmAnayx doVSagaLu; rUDhiya tapu pxgaLu. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
 \eng{the vulgar tongue} (hiMde baLakeyalilxdadx paMDitaBASeyAda lAYxTinfge BinanxvAgi) deVsi; deVshiVya BASe; janapadara, janasAmAnayxra BASe. 
\emng
\eentry

\bentry
\wordnospeech{vulgar fraction}{vulgar fraction}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (aMsha, CeVdagaLanonxLagoMDa, dashamAMshagaLige BinanxvAda) sAmAnayx BinanxrAshi. 
\emng
\eentry

\bentry
\word{vulgarian} 
\pron{valegxVrianf} 
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ haNavaMtanAda) asaMsakxqqta vayxkitx; asaBayx; pAmara. 
\emng
\eentry

\bentry
\word{vulgarise} 
\pron{valagxreYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{vulcanize}{vulcanize}. 
\emng
\eentry

\bentry
\word{vulgarism} 
\pron{valagxrisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oraTutana; asaMsakxqqtate; asaBayxte. 
\num{2} gArxmayx, pAmara -- pada, parxyoVga. 
\enum
\emng
\eentry

\bentry 
\word{vulgarity} 
\pron{valAgxYxriTi} 
\gl{\nA}
\expl{(\bava\ \eng{vulgarities}). }
\bmng
 kiVLutana; halAkxtana; kiVLu tarada naDavaLike, mAtu, alaMkAra, \mo vu. 
\emng
\eentry

\bentry 
\word{vulgarization} 
\pron{valagxreYseZVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} pAmariVkaraNa; asaBayx, asaMsakxqqta, gArxmayx -goLisuvike. 
\num{2} sAmAniyxVkaraNa; sathxLa, BAvane, \mo vanunx janasAmAnayxkekx laBayxvAguvaMte, hiDisuvaMte mADuvike. 
\enum
\emng
\eentry

\bentry 
\word{vulgarize} 
\pron{valagxreYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx, naDavaLike, \mo vanunx) pAmariVkarisu; asaBayx, asaMsakxqqta, gArxmayx -- goLisu. 
\num{2} (sathxLa, BAvane, daqshayx, \mo vanunx) sAmAniyxVkarisu; janasAmAnayxkekx laBayxvAguvaMte, hiDisuvaMte mADu. 
\num{3} kiVLugoLisu; guNa \mo vanunx kaDime mADi agagxvAgisu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{vulgar Latin}{vulgar Latin}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_l.pdf}{L}{Latin pagu(9)}{Latin} (\pagu\ \eng{9}). 
\emng
\eentry

\bentry
\word{vulgarly} 
\pron{valagxrfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} gArxmayxvAgi; (susaMsakxqqta parxyoVgakekx BinanxvAgi) ashudadhxvAgi. 
\num{2} kiVLu, halAkx -- riVtiyalilx. 
\enum
\emng
\eentry

\bentry 
\word{Vulgate} 
\pron{valegxV(lagx)Tf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} valegxVTf; \kirxsha\ \eng{4}neya shatamAnada utatxrAdhaRdalilx \kanmu\ saMta jeroVmanu sidadhxgoLisida, beYbalf garxMthada lAYxTinf pATha. 
\num{2} \eng{1592}ralilx parxkaTagoMDa, beYbalfna adhikaqta roVmanf kAyxtholikf lAYxTinf pATha. 
\num{3} \eng{(vulgate).} (yAvaneV garxMthakataRna garxMthada) aMgiVkaqta pATha. 
\num{4} \eng{(vulgate).} janasAmAnayxra BASe; ADuBASe. 
\enum
\emng
\eentry

\bentry 
\word{vulnerability} 
\pron{valanxrabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} gAyagoLisalu sAdhayxvAguvike. 
\alnum{b} (\rUpa) mananoVyisalu sAdhayxvAguvike. 
\eanum
\numie
\num{2} shasatxrXBeVdayxte; shasatxrXgaLiMda GAsigoLisalAguvike; KaMDaniVyate. 
\num{3} TiVke, KaMDanegaLiMda noVyisalAguvike. 
\enum
\emng
\eentry

\bentry
\word{vulnerable} 
\pron{valanxrabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} gAyagoLisabahudAda. 
\alnum{b} (\rUpa) mananoVyisabahudAda. 
\eanum
\numie
\num{2} BeVdayx; shasatxrXdiMda iriyalAguva. 
\num{3} KaMDaniVya; TiVke, KaMDane, \mo vakekx -- oLapaDuva, pakAkxguva, guriyAguva. 
\num{4} (kAMTArxkfTx birxjf ATa) (mUraralilx) oMdu ATa mAtarx gedudx, hecicxna daMDakekx guriyAda. 
\enum
\emng
\eentry

\bentry 
\word{vulnerableness} 
\pron{valanxrabflfnisf} 
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{vulnerability}{vulnerability}. 
\emng
\eentry

\bentry
\word{vulnerably} 
\pron{valanxrabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} gAyagoLisalu sAdhayxvAguvaMte. 
\alnum{b} mananoVyisalAguvaMte. 
\eanum
\numie
\num{2} shasatxrX, TiVke, KaMDane, \mo vakekx -- oLapaDuvaMte, pakAkxguvaMte, IDAguvaMte. 
\enum
\emng
\eentry

\bentry 
\word[vulnerary(1)]{vulnerary} 
\pron{valanxrari} 
\gl{\nA}
\expl{(\bava\ \eng{vulneraries}).}
\bmng
 gAyavanunx mAyisuva mUlike yA auSadha. 
\emng
\eentry

\bentry
\word[vulnerary(2)]{vulnerary} 
\pron{valanxrari} 
\gl{\gu}
\expl{}
\bmng
 gAyavanunx -- mAyisuva, vAsi mADuva. 
\emng
\eentry

\bentry
\word{vulpine} 
\pron{valepxYnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nariya; nariyaMtha; nariya savxBAvada. 
\num{2} kutaMtarxda; kuyukitxya. 
\enum
\emng
\eentry 

\bentry
\word{vulvitis} 
\pron{valevxYTisf} 
\gl{\nA}
\expl{}
\bmng
 tulilxna, upasethxya -- uriyUta. 
\emng
\eentry

\bentry
\wordnospeech{vv.}{vv.}
\pron{?} 
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{verses}. 
\num{2} \eng{volumes.} 
\enum
\emng
\eentry

\bentry
\word{vying} 
\pron{veYiMgf} 
\gl{\kirx}
\expl{}
\bmng
 \eng{vie} padada \vakaq rUpa. 
\emng
\eentry

\bentry
\word{vulture} 
\pron{valacxrf} 
\gl{\nA}
\bmng
\bnum
\num{1} raNahadudx; gaqdharx; heNa biVLuvudakikxMta muMceyeV oTuTxgUDuvuvenanxlAda, taleyU katUtx tupupxLilalxde boVLAgiruva, satatx pArxNigaLa mAMsavanunx tinunxva, katATiRDiV yA AYxkisxpiTirxDiV vaMshakekx seVrida, oMdu doDaDx mAMsAhAri hiMsarx pakiSx. \imglink{vulture 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.4cm height 0.6cm {V_Pictures/vulture.jpg}}} 
\num{2} (\rUpa) raNahadudx; kUrxriyU hiMsakanU Ada vayxkitx. 
\enum
\emng
\eentry

\bentry
\word{vulturine} 
\pron{valacxreYnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} raNahadidxna. 
\hypertarget{vulturine(2)}{} 
\num{2} raNahadidxna savxBAvada; kUrxriyU hiMsakanU Ada. 
\enum
\emng
\eentry

\bentry
\word{vulturish} 
\pron{valacxriSf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vulturine(2)}{vulturine\((2)\)}. 
\emng
\eentry

\bentry
\word{vulturous} 
\pron{valacxrasf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{vulturish}{vulturish}. 
\emng
\eentry

\bentry
\word{vulva} 
\pron{valavx} 
\gl{\nA}
\expl{(\bava\ \eng{vulvas}). }
\bmng
 (\aMrashA, \pArxvi) tululx(\kanmu\ tulilxna bAyi); yoVni(dAvxra); upasethx. 
\emng
\eentry

\bentry
\word{vulvar} 
\pron{valavxrf} 
\gl{\gu}
\expl{}
\bmng
 tulilxna; yoVniya; upasethxya. 
\emng
\eentry

\newpage
\input figs_list
