\bentry
\wordRemoveSpace{N,-n}{N, n}
\pron{enf}
\gl{\nA}
\expl{(\bava\ \eng{Ns} yA \eng{N's}).}
\bmng
\bnum
\num{1} iMgilxSf vaNaRmAleya hadinAlakxneya akaSxra. 
\num{2} mudarxNadalilx agalada mAna (= \hyperref{kandict_e.pdf}{E}{en}{en}.) 
\num{3} (\biVga) enf: \eng{N;} anidiRSaTx sithxra saMKeyx. 
\num{4} (\Bwvi) avogADorx sithxra(da saMkeVta). 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{to the nth} (\engit{or} \eng{nth) degree} 
\banum
\alnum{a} yAvudAdarU beVkAda GAtakekx. 
\alnum{b} (\rUpa) eSaTxkekx beVkAdarU; paramAvadhige. 
\eanum
\emng
\eentry

\bentry
\word[N(1)]{N}
\pron{}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} (caduraMga) \eng{knight}. 
\num{2} \eng{New}. 
\num{3} \eng{newton(s)}. 
\num{4} \eng{North, Northern}. 
\num{5} \eng{nuclear}. 
\enum
\emng
\eentry

\bentry
\word[N(2)]{N}
\pron{}
\gl{\saMkeV}
\bmng
\banum
\alnum{a} (\biVga) anidiRSaTx sithxra (saMKeyx). 
\alnum{b} (\ravi) neYTorxVjanf paramANu(vina saMkeVta). 
\eanum
\emng
\eentry

\bentry
\wordnospeech{n}{n}
\pron{?}
\gl{\saMkiSx}
\bmng
 (\eng{n.} saha) 
\bnum
\num{1} \eng{name.} 
\num{2} \eng{nano-.} 
\num{3} \eng{neuter.} 
\num{4} \eng{noon.} 
\num{5} \eng{note.} 
\num{6} \eng{noun.} 
\enum
\emng
\eentry

\bentry
\wordnospeech{'n}{'n}
\pron{}
\gl{\saMavayx}
\bmng
 \eng{(`n'} saha) (\AmA) matutx; hAgU (\eng{and} eMba padada \saMkiSx). 
\emng
\eentry

\bentry
\word[hyp-n(1)]{n}
\pron{}
\gl{\uparx}
\bmng
= \hyperref{kandict_e.pdf}{E}{hyp-en(1)}{-$^1$en}.
\emng
\eentry

\bentry
\word[hyp-n(2)]{n}
\pron{}
\gl{\uparx}
\bmng
= \hyperref{kandict_e.pdf}{E}{hyp-en(5)}{-$^5$en}. 
\emng
\eentry

\bentry
\wordnospeech{Na}{Na}
\pron{?}
\gl{\saMkeV}
\bmng
(\ravi) \eng{sodium.} 
\emng
\eentry

\bentry
\word{na}
\pron{na}
\gl{\kirxvi}
\bmng
 (sAkxTelxMDf) (\sA\ sahAyaka kirxyApadadoDane) = \hyperlink{not}{not}, \udA\ \eng{I canna do it} nAnu mADalu sAdhayxvilalx. \eng{they didna go} avaru hoVgalilalx. 
\emng
\eentry

\bentry
\wordnospeech{n/a}{n/a}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{not applicable.} 
\num{2} \eng{not available.} 
\enum
\emng
\eentry

\bentry
\word{NAAFI}
\pron{nAYxphi}
\gl{\saMkiSx}
\bmng
(\birx) 
\bnum
\num{1} \eng{Navy, Army and Air Force Institutes.} 
\num{2} \eng{NAAFI} naDesuva seYnayxdavara kAyxMTiVnu. 
\enum
\emng
\eentry

\bentry
\word{nab}
\pron{nAYxbf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nabbed,} \vakaq\ \eng{nabbing}).}
\bmng
 (\ashi) hiDidu nililxsu; dasatxgiri mADu; tapipxta mADutitxruvAga hiDidu hAku. 
\emng
\eentry

\bentry
\word{nabob}
\pron{neVbAbf}
\gl{\nA}
\bmng
\bnum
\num{1} (\ca) navAba; moGalf sAmArxjayxdalilx musalAmxna adhikAri, pArxMtAyxdhikAri. 
\num{2} (\pArxparx) BoVgAsakatx aishavxyaRvaMta; (\kanmu) iMDiyadalilx aishavxyaRgaLisi iMgelxMDige hiMdirugidavanu. 
\enum
\emng
\eentry

\bentry
\word{nacarat}
\pron{nAyxkarAyxTf}
\gl{\nA}
\bmng
ujavxla kitatxLe keMpu (baNaNx). 
\emng
\eentry

\bentry
\word{nacelle}
\pron{naself}
\gl{\nA}
\bmng
naself: 
\banum
\alnum{a}(vimAnadalilx) eMjinf (irisiruva) saMpuTa. 
\alnum{b}vAyunwkeyalilx parxyANikarige nigadiyAda AvaraNa, aMkaNa. 
\eanum
\emng
\eentry

\bentry
\word{nacho}
\pron{nAYxcoV}
\gl{\nA}
\expl{(\bava\ \eng{nachos}).}
\bmng
 (\sA\ \bava dalilx) nAYxcoV; karagisida ciVsu, masAle padAthaRgaLanunx hAkida, musukina joVLada upepxVri. 
\emng
\eentry

\bentry
\word{NACODS}
\pron{neVkADfsx}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Association of Colliery Overmen, Deputies and Shotfirers.} 
\emng
\eentry

\bentry
\word{nacre}
\pron{neVkarf}
\gl{\nA}
\bmng
neVkarf: 
\banum
\alnum{a} mututx utApxdisuva cipupxjiVvi. 
\alnum{b} mutitxna cipipxna oLameY meVliruva vividha baNaNxgaLiMda hoLeyuva, (\kanmu\ kAyxlisxyaM kAboRneVTfniMdAda) padAthaR. 
\alnum{c} mutitxna cipupx. 
\eanum
\emng
\eentry

\bentry
\word{nacred}
\pron{neVkaDfR}
\gl{\gu}
\bmng
neVkarf hodisida, leVpisida. 
\emng
\eentry

\bentry
\word{nacreous}
\pron{neVkirxasf}
\gl{\gu}
\bmng
neVkarfnaMtha yA neVkarfniMdAda. 
\emng
\eentry

\bentry
\word{nacrous}
\pron{neVkarxsf}
\gl{\gu}
\bmng
= \hyperlink{nacreous}{nacreous}.
\emng
\eentry

\bentry
\word{nadir}
\pron{neV(nAYx)DiArf}
\gl{\nA}
\bmng
\bnum
\num{1} Kamadhayxkekx neVridirAgi perxVkaSxkana kAlaDi iruva biMdu. 
\num{2} (\rUpa) adhoVbiMdu; adhaHpAtALa; atayxMta keLagina biMdu yA parxdeVsha; atayxMta avanati \mo vugaLa sAthxna, kAla; \eng{the nadir of our hopes} namamx BaravasegaLa adhaHpAtALa. 
\enum
\emng
\eentry

\bentry
\word{naevus}
\pron{niVvasf}
\gl{\nA}
\expl{(\bava\ \eng{naevi} \ucAcx -- niVveY).}
\bmng
\bnum
\num{1} keMpu macecx; camaRda meVle sapxSaTxvAgi keMpage kANuva huTuTxmacecx. 
\num{2} = \hyperref{kandict_m.pdf}{M}{mole(1)}{$^1$mole}. 
\enum
\emng
\eentry

\bentry
\word[naff(1)]{naff}
\pron{nAYxphf}
\gl{\akirx}
\bmng
(\ashi) (AjAcnxthaRkavAgi) tolagu; tolagAce; nikalAyisu; horaTu hoVgu (\sA\ \eng{naff off)}. 
\emng
\eentry

\bentry
\word[naff(2)]{naff}
\pron{nAYxPf}
\gl{\gu}
\bmng
(\ashi) 
\bnum
\num{1} phAyxSanAnxgilalxda; (sAmAjikavAgi) nayanAjUkilalxda. 
\num{2} kelasakekx bArada; kacaDavAda; holasAda. 
\enum
\emng
\eentry

\bentry
\word{naffing}
\pron{nAYxphiMgf}
\gl{\gu}
\bmng
(AdhikayxvAcakavAgi) (hecicxna kirikiriyanunx vayxkatxpaDisalu) keTaTx; hALAda; daridarx. 
\emng
\eentry

\bentry
\word{Naffy}
\pron{nAYxphi}
\gl{\nA}
\bmng
(\ashi) = \hyperlink{NAAFI}{NAAFI}.
\emng
\eentry

\bentry
\word[nag(1)]{nag}
\pron{nAYxgf}
\gl{\kirx}
\bmng
(\BU\ matutx \BUkaq\ \eng{nagged,} \vakaq\ \eng{nagging}). 
\emng

\noindent
\gl{\sakirx}
\bmng
\bnum
\num{1} (vayxkitxyanunx) biDade, oMdeV samanAgi otAtxyisi yA tapupx hiDidu -- kADu: piVDisu, kirikiriyuMTumADu. 
\num{2} (vayxkitx, manasusx, \mo vanunx) kADu; kore: \eng{his mistake nagged him} avana tapupx avananunx kADitu, koreyitu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} \kanmu\ oMdeVsamane, biDade -- tapupx kaMDuhiDi, dUru yA otAtxyisu. 
\num{2} (noVvina \vi) maMdavAgi, Adare oMdeV samanAgi noVyu(titxru). 
\num{3} kADu; yAtane koDu. 
\enum
\emng
\eentry

\bentry
\word[nag(2)]{nag}
\pron{nAYxgf}
\gl{\nA}
\bmng
\bnum
\num{1} (\AmA) kudure. 
\num{2}taTuTx; saNaNx savAri (mADuva) kudure. 
\enum
\emng
\eentry

\bentry
\word{Naga}
\pron{nAga}
\gl{\nA}
\bmng
nAga: 
\banum
\alnum{a} (hiMdU \pu) adhaR mAnava adhaRsapaRvAgidudx maLe, nadi, \mo vugaLa adhideVvategaLa vaMshakekx seVridavanu. 
\alnum{b} BAratada IshAnayx parxdeVshada nAgA guDaDxgADina buDakaTiTxge seVridavanu. 
\eanum
\emng
\eentry

\bentry
\word{nagana}
\pron{nagAna}
\gl{\nA}
\bmng
nagAna; AphirXkada uSaNx valaya parxdeVshagaLalilx TisxTisxV noNada muKAMtara haraDuva, TirxpanasoVmf sUkaSxmXjiVviyiMda baruva, danagaLa mAraka roVga. 
\emng
\eentry

\bentry
\word{nagger}
\pron{nAYxgarf}
\gl{\nA}
\bmng
piVDaka; nakaSxtirxka; vayxkitxyanunx oMdeVsamanAgi otAtxyisuvavanu, piVDisuvavanu, kADuvavanu. 
\emng
\eentry

\bentry
\word{naggingly}
\pron{nAYxgiMgfli}
\gl{\kirxvi}
\bmng
piVDisutAtx; kADutAtx; nakaSxtirxkanaMte. 
\emng
\eentry

\bentry
\word{nagor}
\pron{neVgArf}
\gl{\nA}
\bmng
neVgArf; pashicxma AphirXkada keMgaMdu baNaNxda jiMke. 
\emng
\eentry

\bentry
\wordnospeech{Nah.}{Nah.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{Nahum}(\beY, haLeya oDaMbaDike). 
\emng
\eentry

\bentry
\word[Nahuatl(1)]{Nahuatl}
\pron{nAvATalf}
\gl{\nA}
\bmng
nAvATalf: 
\banum
\alnum{a} \da\ mekisxko matutx AsazfTekfsx seVridaMte madhayx amerikada janasamudAyakekx seVridavanu. 
\alnum{b} madhayx amerikada jana samudAyada BASe. 
\eanum
\emng
\eentry

\bentry
\word[Nahuatl(2)]{Nahuatl}
\pron{nAvATalf}
\gl{\gu}
\bmng
(\da\ mekisxko matutx AszfTekfsx seVridaMte) madhayx amerikada janasamudAyada. 
\emng
\eentry

\bentry
\word{naiad}
\pron{neYAYxDf}
\gl{\nA}
\expl{(\bava\ \eng{naiads} yA \eng{naiades,} \ucAcx-- neYaDiVszf).}
\bmng
\bnum
\num{1} (\pu) jalakaninxke; jalABimAni deVvi; jalApasxre. 
\num{2} DerxVganf miDate \mo vugaLa lAvaR, marihuLu. 
\enum
\emng
\eentry

\bentry
\word[nail(1)]{nail}
\pron{neVlf}
\gl{\nA}
\bmng
\bnum
\num{1} uguru; naKa. 
\num{2} (maqgagaLa, hakikxgaLa kAlina) moneyuguru. 
\num{3} (maqdu kokikxna hakikxgaLa) kokikxna meVlABxgadalilxna uguru padAthaRda gaTiTxgaMti. 
\num{4} moLe; ANi. 
\num{5} gajada \eng{1/16} BAga; \eng{$2\frac\bg 1\eg \bg 4\eg$} aMgula (hiMde baLasutitxdadx baTeTxya oMdu aLate). 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{bite one's nail} (aBAyxsabaladiMda yA tALemxgeTuTx) uguru kaDi. 
\num{2} \eng{drive nail in one's} \hyperref{kandict_c.pdf}{C}{coffin(1)nuga}{$^1$coffin}. 
\numi{3} \eng{hard as nails} 
\banum
\alnum{a} daqDhakAyanAgi; gaTiTxmuTATxgi. 
\alnum{b} kalelxdeya; nidaRya; kaThiNa. 
\hypertarget{nailpagu4}{} 
\eanum
\numie
\numi{4} \eng{hit the nail on the head} 
\banum
\alnum{a} sariyAda sapxSaTxne koDu. 
\alnum{b} yAvudu sariyoV A kelasa mADu. 
\alnum{c} gurimuTuTx. 
\eanum
\numie
\num{5} \eng{hit the right nail}= \hyperlink{nailpagu4}{?pagu?\((4)\)}. 
\num{6} \eng{nail -- biting} tiVvarx AtaMkavanunxMTumADuva. 
\num{7} \eng{on the nail} (\kanmu\ haNa pAvatiya \vi) takaSxNa; kUDaleV; taDavilalxde. 
\num{8} \eng{right as nails} sari; nija; Kare; KaMDita sari. 
\num{9} \eng{tooth and nail} shakitxyanenxlAlx biTuTx; balavAgi paTuTx hiDidu: \eng{fight tooth and nail} balavAgi paTuTx hiDidu hoVrADu. 
\enum
\emng
\eentry

\bentry
\word[nail(2)]{nail}
\pron{neVlf}
\gl{\sakirx}
\bmng
\bnum
\num{1} moLehoDe; moLe hoDedu -- BadarxpaDisu, baMdhisu. 
\num{2} (vayxkitx, gamana, \mo vanunx) baMdhisu; calisadaMtiDu; nATisu: \eng{surpise nailed him to the spot} AshacxyaRdiMda avanu nishacxlanAgi niMta. 
\num{3} (manuSayx, vasutx, \mo vanunx) BadarxpaDisu; hiDi; nililxsiko. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{nail (a lie etc.) to counter}(suLuLx \mo vanunx) bahiraMgapaDisu; suLeLxMdu toVrisu; suLaLxnunx etitx toVrisu. 
\num{2} \eng{nail} \hyperref{kandict_c.pdf}{C}{color(1) nuga(9)}{$^1$colour}\eng{s to the mast.} 
\numi{3} \eng{nail down} 
\banum
\alnum{a} sapxSaTxvAgi vivarisu. 
\alnum{b} (obabx vayxkitxyanunx avanu) koTaTx vacana \mo vugaLige kaTiTxhAku, baMdhisu. 
\alnum{c} kaTaTx kaDeyadAgi niSakxSeR mADibiDu. 
\alnum{d} (oMdu vasutxvanunx) moLe hoDedu bigi mADu, BadarxpaDisu. 
\eanum
\numie
\numi{4} \eng{nail up} 
\banum
\alnum{a} (bAgilu \mo vanunx) moLegaLiMda mucucx. 
\alnum{b} (vasutxvanunx) moLegaLa sahAyadiMda BadarxpaDisu, nililxsu, kaTuTx. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{nail-brush}
\pron{neVlfbarxSf}
\gl{\nA}
\bmng
ugurukuMca; uguru barxSuSx; uguranunx savxcaCxgoLisuva barxSuSx. 
\emng
\eentry

\bentry
\word{nailed}
\pron{neVlfDx}
\gl{\gu}
\bmng
\bnum
\num{1} uguriruva; uguruLaLx. 
\num{2} moLe hoDeda; moLe hoDedu BadarxpaDisida. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nail enamel}{nail enamel}
\pron{?}
\gl{\nA}
\bmng
(\ame) = \hyperlink{nail polish}{nail polish}.
\emng
\eentry

\bentry
\word{nailer}
\pron{neVlarf}
\gl{\nA}
\bmng
moLegAra; moLe mADuvavanu. 
\emng
\eentry

\bentry
\word{nailery}
\pron{neVlari}
\gl{\nA}
\bmng
moLe tayArike. 
\emng
\eentry

\bentry
\word{nail-file}
\pron{neVlfpheYlf}
\gl{\nA}
\bmng
= \hyperlink{nail-scissors}{nail-scissors}.
\emng
\eentry

\bentry
\word{nail-head}
\pron{neVlfheDf}
\gl{\nA}
\bmng
(\vAshi) moLe tale; kaTaTxDada oMdu riVtiya goVlu alaMkAra. 
\emng
\eentry

\bentry
\word{nailing}
\pron{neVliMgf}
\gl{\gu}
\bmng
(\pArxparx) sogasAda; majavAda; utakxqqSaTxvAda: \eng{a nailing good time} bahu sogasAda kAla. 
\emng
\eentry

\bentry
\word{nailless}
\pron{neVlflisf}
\gl{\gu}
\bmng
\bnum
\num{1} moLeyilalxda. 
\num{2} ugurilalxda; naKarahita. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nail polish}{nail polish}
\pron{?}
\gl{\nA}
\bmng
uguru meragu; uguru pAliSf. 
\emng
\eentry

\bentry
\word{nail-punch}
\pron{neVlfpaMcf}
\gl{\nA}
\bmng
moLe sutitxge; meVlugaDe kANisadaMte moLeyanunx oLakekx hoDedu iLisuva salakaraNe. 
\emng
\eentry

\bentry
\word{nail-scissors}
\pron{neVlfsisazrfsx}
\gl{\nA}
\bmng
uguru katatxri. 
\emng
\eentry

\bentry
\word{nail-set}
\pron{neVlfseTf}
\gl{\nA}
\bmng
= \hyperlink{nail-punch}{nail-punch}.
\emng
\eentry

\bentry
\wordnospeech{nail varnish}{nail varnish}
\pron{?}
\gl{\nA}
\bmng
(\birx) = \hyperlink{nail polish}{nail polish}.
\emng
\eentry

\bentry
\word{nainsook}
\pron{neVnfsukf}
\gl{\nA}
\bmng
(mUlataH) BAratada nAjUkAda hatitxya baTeTx. 
\emng
\eentry

\bentry
\word{naira}
\pron{neYra}
\gl{\nA}
\bmng
neYra; neYjiVriyada haNada GaTaka. 
\emng
\eentry

\bentry
\word{naive}
\pron{nAIvf, neYvf}
\gl{\gu}
\bmng
akaqtirxma; niSakxpaTa; QujuvAda; mugadhx; BoVLetanada; maguvinaMte saraLavAda. 
\emng
\eentry

\bentry
\word{naively}
\pron{nAIvfli, neYvfli}
\gl{\kirxvi}
\bmng
akaqtirxmavAgi; niSakxpaTavAgi. 
\emng
\eentry

\bentry
\word{naiveness}
\pron{nAIvfnisf, neYvfnisf}
\gl{\nA}
\bmng
akaqtirxmatana; niSAkxpaTayx. 
\emng
\eentry

\bentry
\wordf{naivete}
\pron{nAIvfTeV}
\gl{\nA}
\bmng
= \hyperlink{naivety}{naivety}.
\emng
\eentry

\bentry
\word{naivety}
\pron{nAIvfTi, neYvfTi}
\gl{\nA}
\expl{(\bava\ \eng{naiveties} yA \eng{naivetes}).}
\bmng
\bnum
\num{1} akaqtirxmate; niSAkxpaTayx; Qujute; mugadhxte; BoVLetana. 
\num{2} akaqtirxma kAyaR; niSakxpaTavAda kelasa. 
\enum
\emng
\eentry

\bentry
\word{naked}
\pron{neVkiDf}
\gl{\gu}
\bmng
\bnum
\num{1} naganx; betatxleya; nivARNa; baTeTx toTiTxrada; barimeYyina; nivaRsanada: \eng{as naked as my mother bore me} namamxmamx haDedAga heVgidedxnoV aMtha batatxle sithxtiyalilx. 
\num{2} arakiSxta; sulaBavAgi guriyAda, sikikxda; rakaSxNe ilalxda: \eng{left naked to temptations} sulaBavAgi parxloVBanegaLige sikikxda. 
\num{3} oreyilalxda; kApilalxda: \eng{a naked sword} oreyiMdeLeda katitx. 
\num{4} naganx; mucucxmareyilalxda; sapxSaTx; suPxTa; neVravAda: \eng{naked truth} naganxsatayx. 
\num{5} (pariVkeSxgAgi hodike) terediTaTx: \eng{his naked soul} avana terediTaTx Atamx. 
\num{6} baTaTxbayalAda; maragiDagaLilalxda; baMjaru: \eng{naked shores} baTaTx bayalAda karAvaLi. 
\num{7} rahita; ilalxda. 
\num{8} elegaLilalxda; boVLAda: \eng{naked trees} boVLu maragaLu. 
\num{9} (baMDe \mo vugaLa \vi) naganx; mucucxkaLeda; tereda. 
\num{10} (nelada \vi) maNuNx, giDagaMTegaLiMda mucicxrada. 
\num{11} (koThaDiya \vi) piVThoVpakaraNagaLilalxda; KAli. 
\num{12} alaMkAravilalxda; niralaMkaqta; nirABaraNavAda. 
\num{13} (hacicxda moVMbatitx \mo vugaLa \vi) gALigoDiDxda; sulaBadAhayx anilagaLigoDiDxda; mucacxde terediTaTx. 
\num{14} boVLuboVLAda; sipepx, ele, kUdalu, shalakxgaLu, cipupx, \mo vugaLilalxda. 
\num{15} bariya; keVvala; AdhArarahita: \eng{naked assertion} AdhArarahita heVLike. 
\num{16} yAvudeV sAdhanada neravilalxda; bariya: \eng{naked eye} (dUradashaRka, sUkaSxmXdashaRka, \mo vugaLa neravilalxde noVDuva) bariya kaNuNx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{naked boys}{naked boys}
\pron{?}
\gl{\nA}
\bmng
hululx meYdAnada kelavu eleyilalxda sasayxjAtigaLu; hululxkeVsari. 
\emng
\eentry

\bentry
\wordnospeech{naked ladies}{naked ladies}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{naked boys}{naked boys}.
\emng
\eentry

\bentry
\wordnospeech{naked lady}{naked lady}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{naked boys}{naked boys}.
\emng
\eentry

\bentry
\word{nakedly}
\pron{neVkiDfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} naganxvAgi; batatxleyAgi. 
\num{2} arakiSxtavAgi. 
\num{3} oreyilalxde. 
\num{4} mucucxmareyilalxde; sapxSaTxvAgi; suPxTavAgi. 
\num{5} boVLuboVLAgi. 
\num{6} alaMkAravilalxde; nirABaraNavAgi. 
\num{7} AdhAra, rujuvAtu -- ilalxde. 
\enum
\emng
\eentry

\bentry
\word{nakedness}
\pron{neVkiDfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} naganxte; bari batatxle sithxti. 
\num{2} bayalAgiruvike. 
\num{3} boVLu sithxti. 
\enum
\emng
\eentry

\bentry
\word{naker}
\pron{neVkarf}
\gl{\nA}
\bmng
(\ca) BeVri; saNaNx nagAri. 
\emng
\eentry

\bentry
\word{NALGO}
\pron{nAYxlogxV}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National and Local Government Officers' Association.} 
\emng
\eentry

\bentry
\word[namby-pamby(1)]{namby-pamby}
\pron{nAYxMbipAYxMbi}
\gl{\gu}
\bmng
\bnum
\num{1} nisAsxravAda aMdada; bariya binAnxNada; mududxmudAdxda. 
\num{2} sapepxya; kasuvu, joVru ilalxda. 
\enum
\emng
\eentry

\bentry
\word[namby-pamby(2)]{namby-pamby}
\pron{nAYxMbipAYxMbi}
\gl{\nA}
\expl{(\bava\ \eng{namby-pambies}).}
\bmng
\bnum
\num{1} nisAsxravAda aMdada mAtu; mududxmududx mAtu; binAnxNada mAtu. 
\num{2} sapepxya, binAnxNada -- vayxkitx. 
\enum
\emng
\eentry

\bentry
\word[name(1)]{name}
\pron{neVmf}
\gl{\nA}
\bmng
\bnum
\num{1} hesaru; nAma; nAmadheVya; aBidhAna: \eng{mention person by name} vayxkitxyanunx hesaru hiDidu heVLu. 
\num{2} hesaru; nAmadheVya; obabx vayxkitxya yA vayxkitxgaLa guMpina hesaru, \kanmu\ aneVka vayxkitxgaLige anavxyisuva hesaru. 
\num{3} parxKAyxta, parxsidadhx -- vayxkitx; KAyxtanAma: \eng{Gandhi himself and many great names were there} sAkASxtf gAMdhi mAtarxvalalxde, aneVka KAyxtanAmarU alilxdadxru. 
\num{4} oMdugotAtxda hesarinavaru; kula; baNa; manetana: \eng{all the clans hostile to the name of Nehru} neharU manetanavanunx devxVSisuva elalx baNadavaru. 
\num{5} (oLeLxya yA keTaTx) hesaru; kiVtiR; KAyxti: \eng{has an ill name} avanige keTaTx hesaru baMdide. 
\num{6} keVvala hesarige mAtarx iruvaMthadu; nAma mAtarxdudx; nijAMshavalalxdudx; nAmakAvAsetx; vAsatxvAMshavalalxdudx; keVvala kAlapxnika: \eng{honour had become a name} gwravaveMbudu hesarige mAtarx uLidididxtu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{a good name} oLeLxya hesaru. 
\hypertarget{name1pagu2}{} 
\num{2} \eng{by name} (eMba) hesarina; (eMdu) kareyalAda: \eng{Rama by name} rAma eMba hesarina. 
\num{3} \eng{by the name of} = \hyperlink{name1pagu2}{?pagu?\((2)\)}. 
\num{4} \eng{call person names} obabxna viSayavAgi avamAnada mAtugaLanAnxDu; obabxnanunx hiVnAmAnavAgi bayiyx. 
\num{5} \eng{give a} \hyperref{kandict_d.pdf}{D}{dog(1) nuga(10)}{$^1$dog} \eng{a bad name and hang him.} 
\hypertarget{name1pagu6}{} 
\num{6} \eng{has a name for honesty} pArxmANikaneMdu parxKAyxti hoMdidAdxne. 
\num{7} \eng{has the name of being honest} = \hyperlink{name1pagu6}{?pagu?\((6)\)}. 
\num{8} \eng{have to one's name} (haNa \mo vanunx) uLaLxvanAgiru; paDediru: \eng{does not have a penny to his name} avana hatitxra oMdu biDikAsU ilalx. 
\num{9} \eng{in all but name} hesaroMdu biTuTx uLidelalx; vAsatxvavAgi; kAyaRtaH. 
\hypertarget{namepagu10}{} 
\num{10} \eng{in a person's name}(obabxna) sAkiSxyAgi; (obabxna) hesarinalilx; deVvaru \mo vara meVle ANeyiTuTx: \eng{in the name of goodness} oLeLxyatanada meVle ANeyiTuTx; oLeLxyatanada hesarinalilx. 
\hyperdef{N}{name(1) pagu(11)}{} 
\hypertarget{namepagu11}{} 
\num{11} \eng{in God's name} deVvara sAkiSxyAgi; deVvarANegU. 
\num{12} \eng{in heaven's name} = \hyperlink{namepagu11}{?pagu?\((11)\)}. 
\num{13} \eng{in name} (\engit{or} \eng{name only)} nAmfkevAsetx; hesarige mAtarx: \eng{is the leader in name only} hesarige mAtarx nAyaka. 
\num{14} \eng{in one's own name} savxMta hesarinalilx; anadhikaqtavAgi; adhikArarahitavAgi; sariyAda adhikAra ilalxde, paDeyade; savxtaMtarxvAgi. 
\num{15} \eng{in the name of} = \hyperlink{namepagu10}{?pagu?\((10)\)}. 
\num{16} \eng{keep one's name on the books} kAleVju, saMGasaMsethx, \mo vugaLa sadasayxnAgiru. 
\numi{17} \eng{know by name} 
\banum
\alnum{a} hesaru mAtarx keVLiru: \eng{I know the man by name} nAnu avana hesaru mAtarx keVLidedxVne. 
\alnum{b} vayxkitxshaH tiLidiru; veYyakitxkavAgi paricaya paDediru: \eng{the teacher knows all the pupils by name} upAdhAyxyanige elAlx vidAyxthiRgaLa veYyakitxka paricayavide. 
\hypertarget{name1pagu18}{} 
\eanum
\numie
\num{18} \eng{make a name for oneself} oLeLxya hesaru mADu: KAyxtigaLisu. 
\num{19} \eng{name after} (obabx nidiRSaTx vayxkitxya) hesariDu: \eng{named him after his grandfather} avanige avana tAtana hesariTaTxru. 
\num{20} \eng{name of the game} (\AmA) kelasada -- muKayx udedxVsha, muKayxtatatx vx, \mo vu. 
\num{21} \eng{of the name of} = \hyperlink{name1pagu2}{?pagu?\((2)\)}. 
\num{22} \eng{one's good name} (obabxna) oLeLxya hesaru; oLeLxyavaneMba parxsididhx. 
\numi{23} \eng{put one's name down for} 
\banum
\alnum{a} umeVduvAranAgi ajiR \mo vanunx hAkiko; umeVduvAranAgi hesaru noVMdAyisu. 
\eanum
\numie
\numi{24} \eng{take person's (esp. God's) name in vain} 
\banum
\alnum{a} obabxna (\kanmu\ deVvara) hesaranunx -- laGuvAgi baLasu yA apavitarxgoLisu, kaLaMkagoLisu. 
\alnum{b} (\hA) obabxna hesaranunx ucacxrisu, heVLu. 
\eanum
\numie
\num{25} \eng{use (person's) name} obabxna hesaranunx baLasiko, AdhAravAgi, parxmANavAgi ulelxVKisu. 
\num{26} \eng{virtuous in name (only)} hesarige (mAtarx) shiVlavaMta; nAmamAtarx saMBAvita. 
\num{27} \eng{what's its} (\engit{or} \eng{his) name} (yAvudoV oMdara, obabxna hesaru heVLalu horaTu, adu nenapAgadiruvAga)  adara (avana) hesaru adeVnoV heVLutAtxralalxpApx! 
\num{28} \eng{what is in a name} hesarinalelxVnide? (hesareMbudu keVvala sAMkeVtika). 
\num{29} \eng{win oneself a name} = \hyperlink{name1pagu18}{?pagu?\((18)\)}. 
\enum
\emng
\eentry

\bentry
\word[name(2)]{name}
\pron{neVmf}
\gl{\sakirx}
\bmng
\bnum
\num{1} hesarukoDu; hesariDu; hesarisu; hesariTuTx kare; nAmakaraNa mADu: \eng{the child was named after its father} maguvige taMdeya hesaraninxDalAyitu. 
\num{2} (vayxkitx yA vasutxvanunx) nijavAda, sariyAda hesariniMda kare, hesarisu. 
\num{3} hesaranunx -- heVLu, gurutisu: \eng{I am sure that I have seen that bonny face, but I cannot name you} A celuvAda muKavanunx nAnu KaMDitavAgi noVDidedxVne, Adare A hesaranunx heVLalAre. 
\num{4} (sAthxna, hudedx, \mo vugaLige, obabx vayxkitxyanunx) nAmakaraNa mADu; hesaru sUcisu; neVmisu: \eng{Mr. X has been named for the directorship} nideRVshakana sAthxnakekx \eng{X}ravara hesaranunx sUcisalAgide. 
\num{5} heVLu; tiLisu; ucacxrisu. 
\num{6} sapxSaTxvAgi heVLu; namUdisu: \eng{name your price} ninanx bele heVLu; niVnu heVLuva bele eSeTxMbudanunx tiLisu. 
\num{7} daqSATxMtavAgi koDu; udAharaNeyAgi heVLu. 
\num{8} (shAsana saBeya adhayxkaSxna \vi) sadasayxnanunx avidheVyaneMdu hesarisi heVLu. 
\num{9} apeVkaSxNiVyavAdudeMdu tiLisu, sapxSaTxgoLisu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{name names} (\kanmu\ ApAdane mADuvAga vayxkitx \mo vanunx) hesariniMda nideRVshisu; hesaru heVLi (iMthavaneMdu) gurutisu: sapxSaTxpaDisu. 
\num{2} \eng{name the day} (heMgasu) maduve dinavanunx gotutxmADi heVLu. 
\num{3} \eng{you name it} (\AmA) EneV Agali; adeVneV irali; ninage iSaTx baMdadudx. 
\enum
\emng
\eentry

\bentry
\word[name(3)]{name}
\pron{neVmf}
\gl{\gu}
\bmng
\bnum
\num{1} hesaruLaLx; hesarina: \eng{name tag} hesaru(LaLx) paTiTx. 
\num{2} hesaruvAsiyAda; oLeLxya hesarina; parxsidadhx; parxKAyxta: \eng{a name brand} hesaruvAsiyAda (vAyxpAri) gurutu. \eng{name actor} parxsidadhx naTa. 
\enum
\emng
\eentry

\bentry
\word{nameable}
\pron{neVmabflf}
\gl{\gu}
\bmng
\bnum
\num{1} hesarisabahudAda; hesaru heVLalu yoVgayxvAda. 
\num{2} ulelxVKaniVya. 
\enum
\emng
\eentry

\bentry
\word{name-caller}
\pron{neVmfkAlarf}
\gl{\nA}
\bmng
bayuyxvavanu; (jagaLavADi) duBARSe baLasuvavanu. 
\emng
\eentry

\bentry
\word{name-calling}
\pron{neVmfkAliMgf}
\gl{\nA}
\bmng
(keVvala) duBARSe; keTaTx mAtu; jagaLadalilx, rAjakiVya parxcAradalilx baLasuva bari bayugxLada mAtu. 
\emng
\eentry

\bentry
\word{name-child}
\pron{neVmfceYlfDx}
\gl{\nA}
\bmng
(gwrava \mo vakAkxgi) matotxbabxna hesariTiTxruva magu. 
\emng
\eentry

\bentry
\word{name-day}
\pron{neVmfDeV}
\gl{\nA}
\bmng
\bnum
\num{1} saMta nAmadina; tAnu yAva saMtana hesariTuTxkoMDiruvanoV A saMtana dina. 
\num{2} (sATxkf ekfsxceVMjf) nAmaparxkaTana dina; SeVru \mo vugaLanunx iMthavanu koLuLxtAtxneMbudu niSakxSeRyAguva hiMdina dina; nijavAgi SeVrukoLuLxvavana hesaranunx SeVru dalAlxLige tiLisuva dina. 
\enum
\emng
\eentry

\bentry
\word{name-drop}
\pron{neVmfDArxpf}
\gl{\akirx}
\bmng
nAmoVlelxVKana mADu; mAtanADuvAga, kAgada bareyuvAga (doDaDx doDaDxvara) hesarugaLanunx ulelxVKisu, heVLu. 
\emng
\eentry

\bentry
\word{name-dropper}
\pron{neVmfDArxparf}
\gl{\nA}
\bmng
nAmoVlelxVKanakAra. 
\emng
\eentry

\bentry
\word{name-dropping}
\pron{neVmfDArxpiMgf}
\gl{\nA}
\bmng
nAmoVlelxVKa(na); (doDaDx doDaDxvara) hesarugaLanunx udurisuvudu; kAgada bareyuvAga, mAtanADuvAga doDaDx doDaDxvara paricaya tanagideyeMdu toVrisikoLaLxlu, jaMba kocacxlu doDaDx manuSayxra hesarugaLanunx ulelxVKisuvudu. 
\emng
\eentry

\bentry
\word{nameless}
\pron{neVmflisf}
\gl{\gu}
\bmng
\bnum
\num{1} hesarilalxda; hesaru ketitxlalxda; nAmarahita: \eng{nameless grave} hesarilalxda samAdhi. 
\num{2} ajAcnxta; aparxsidadhx. 
\num{3} (beVkeMdu) hesaru heVLadiruva; hesaru -- maresiTaTx, goVpayxvAgiTaTx, mucicxTaTx: \eng{a well-known person who shall be nameless} yAra hesaranunx beVkeMdeV mucicxDutitxruvaroV A parxsidadhx vayxkitx. 
\num{4} anAmika; anAmadheVya: \eng{a certain nameless politician} yAvanoV obabx anAmadheVya rAjakAraNi. 
\num{5} anivaRcaniVya; vivarisalAgada; vaNiRsalAgada: \eng{a nameless charm} oMdu anivaRcaniVya moVhakate. 
\num{6} avAcayx; hesaru heVLabAradaSuTx keTaTxdAda; asahayxvAda: \eng{nameless vices} hesaru heVLabArada duguRNagaLu. 
\num{7} kAnUnubAhira; akarxma. 
\enum
\emng
\eentry

\bentry
\word{namelessly}
\pron{neVmflisfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} hesarilalxda hAge. 
\num{2} ajAcnxtavAgi; aparxsidadhxvAgi. 
\num{3} anAmadheVyanAgi. 
\enum
\emng
\eentry

\bentry
\word{namelessness}
\pron{neVmflisfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} hesarilalxdiruvike. 
\num{2} ajAcnxtasithxti; aparxsidadhx sithxti. 
\num{3} anAmikavAgiruvike; anAmadheVya sithxti. 
\enum
\emng
\eentry

\bentry
\word{namely}
\pron{neVmfli}
\gl{\kirxvi}
\bmng
aMdare; EneMdare: \eng{only one boy was absent, namely Harish} obabxneV obabx huDuga, aMdare hariVshf, geYruhAjarAgidadx. 
\emng
\eentry

\bentry
\word{name-part}
\pron{neVmfpATfR}
\gl{\nA}
\bmng
nAmapAtarx; nATakakekx hesarukoTaTx pAtarx: \eng{who will play the name-part in Hamlet?} hAyxmfleTfnalilx nAmapAtarxvanunx (hAyxmfleTaTxna pAtarxvanunx) yAru mADutAtxre? 
\emng
\eentry

\bentry
\word{name-plate}
\pron{neVmfpelxVTf}
\gl{\nA}
\bmng
hesaruhalage; nAmaPalaka. 
\emng
\eentry

\bentry
\word{namesake}
\pron{neVmfseVkf}
\gl{\nA}
\bmng
samAnanAmaka; samAna nAmadhAri; adeV hesarinavanu yA hesarinadu: \eng{his namesake} avana hesarina inonxbabx. 
\emng
\eentry

\bentry
\word{name-tape}
\pron{neVmfTeVpf}
\gl{\nA}
\bmng
hesaruciVTi; (baTeTx \mo vugaLa meVle) hacicxda mAliVkana hesaruciVTi. 
\emng
\eentry

\bentry
\word{nan}
\pron{nAYxnf}
\gl{\nA}
\bmng
(\birx) (\AmA) ajijx. 
\emng
\eentry

\bentry
\word{nana}
\pron{nAna}
\gl{\nA}
\bmng
\bnum
\num{1} = \hyperlink{nan}{nan}.
\num{2} daDaDx; gugugx; gAMpa. 
\enum
\emng
\eentry

\bentry
\word[nance(1)]{nance}
\pron{nAYxnfsx}
\gl{\nA}
\bmng
\bnum
\num{1} heNiNxga; heNaNxpipx; heMgasinaMtha -- huDuga, gaMDasu. 
\num{2} saliMga kAmi(yAda gaMDasu). 
\enum
\emng
\eentry

\bentry
\word[nance(2)]{nance}
\pron{nAYxnfsx}
\gl{\gu}
\bmng
\bnum
\num{1} (gaMDasina \vi) heNiNxga(naMtha); heNaNxpipx; heMgasinaMtha. 
\num{2} (gaMDasina \vi) saliMgakAmada. 
\enum
\emng
\eentry

\bentry
\word[nancy(1)]{nancy}
\pron{nAYxnisx}
\gl{\nA}
\expl{(\bava\ \eng{nancies}).}
\bmng
= \hyperlink{nance(1)}{$^1$nance}. 
\emng
\eentry

\bentry
\word[nancy(2)]{nancy}
\pron{nAYxnisx}
\gl{\gu}
\bmng
= \hyperlink{nance(2)}{$^2$nance}.
\emng
\eentry

\bentry
\wordnospeech{nancy boy}{nancy boy}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{nance(1)}{$^1$nance}.
\emng
\eentry

\bentry
\word{nankeen}
\pron{nAYxnfkiVnf}
\gl{\nA}
\bmng
\bnum
\num{1} (haLadi baNaNxda) hatitxbaTeTx; hatitxyiMda mADida haLadi baTeTx. 
\num{2} (\bava dalilx) I baTeTxya SarAyi. 
\num{3} haLadi yA mAsalu haLadi baNaNx. 
\enum
\emng
\eentry

\bentry
\word{nanna}
\pron{nAYxna}
\gl{\nA}
\bmng
\eng{nan} eMbudara rUpAMtara. 
\emng
\eentry

\bentry
\word[nanny(1)]{nanny}
\pron{nAYxni}
\gl{\nA}
\expl{(\bava\ \eng{ nannies}).}
\bmng
\bnum
\num{1} heNuNx meVke. 
\num{2} maguvina dAdi. 
\num{3} (\rUpa) atiyAda joVpAnadiMda kApADuva vayxkitx, saMsethx. 
\num{4} = \hyperlink{nan}{nan}.
\enum
\emng
\eentry

\bentry
\word[nanny(2)]{nanny}
\pron{nAYxni}
\gl{\sakirx}
\bmng
(yAvudoV oMdanunx) atiyAda joVpAnadiMda kApADu. 
\emng
\eentry

\bentry
\word{nanny-goat}
\pron{nAnigoVTf}
\gl{\nA}
\bmng
= \hyperlink{nanny(1)}{$^1$nanny\((1)\)}. 
\emng
\eentry

\bentry
\word{nano-}
\pron{neV(nAYx)noV-}
\gl{\sapUpa.}
\bmng
biliyaninxna oMdu BAga; \eng{$10^{-9}$.} 
\emng
\eentry

\bentry
\word{nanometre}
\pron{nAYxnoVmiVTarf}
\gl{\nA}
\bmng
nAyxnoV miVTarf; oMdu miVTarina nUru koVTiyalilx oMdu BAga. 
\emng
\eentry

\bentry
\word{nanosecond}
\pron{nAYxnoVsekaMDf}
\gl{\nA}
\bmng
nAYxnoVsekeMDf; sekeMDina biliyaninxnalilx (nUru koVTiyalilx) oMdu BAga. 
\emng
\eentry

\bentry
\word{naos}
\pron{neVAsf}
\gl{\nA}
\expl{(\bava\ \eng{naoi} \ucAcx-- neVAyf).}
\bmng
 (\girxVca) deVvasAthxnada oLaBAga, gaBaRguDi. 
\emng
\eentry

\bentry
\word[nap(1)]{nap}
\pron{nAYxpf}
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{napped,} \vakaq\ \eng{napping}).}
\bmng
haguravAgi niderxmADu; nasuniderxmADu. 
\emng

\noindent
\gl{\pagu}
\bmng
\hyperdef{N}{nap(1) pagu}{} 
\eng{catch napping} 
\banum
\alnum{a} tUkaDisutitxruvAga hiDi; nidedx hoVgutitxruvAga hiDi. 
\alnum{b} (ecacxrikeyiMda ilalxdiruvAga yA suLivukoDade) haThAtAtxgi hiDiduhAku. 
\alnum{c} (kataRvayxvanunx alakiSxsutitxruvAga yA adaralilx tapupx mADutitxruvAga) Pakakxne hiDi; aniriVkiSxtavAgi hiDidu hAku. 
\eanum
\emng
\eentry

\bentry
\word[nap(2)]{nap}
\pron{nAyxpf}
\gl{\nA}
\bmng
(\kanmu\ hagalinalilxna) nasunidedx. 
\emng

\noindent
\gl{\pagu}
\bmng
 \eng{take a nap}(hagalinalilx) nasuniderx mADu; savxlapx hotutx nidedx mADu. 
\emng
\eentry

\bentry
\word[nap(3)]{nap}
\pron{nAYxpf}
\gl{\nA}
\bmng
\bnum
\num{1} juMju; juMgu; (baTeTxya) moVTu eLegaLanunx, juMjugaLanunx meVletitx takakxMte katatxrisi nayamADida baTeTxya meVluBAga, horameY BAga. 
\num{2} maqduvAda, metatxneya meVlemxY. 
\num{3} (\AseTxrXV) (\AmA) kaMbaLi, hAsige, sAmAnu, gaMTumUTe. 
\enum
\emng
\eentry

\bentry
\word[nap(4)]{nap}
\pron{nAYxpf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{napped,} \vakaq\ \eng{napping}).}
\bmng
(baTeTxya meYmeVle) juMganunx ebibxsu. 
\emng
\eentry

\bentry
\word[nap(5)]{nap}
\pron{nAYxpf}
\gl{\nA}
\bmng
\bnum
\num{1} nAyxpf ATa; oMdu (riVti) isipxVTATa; ATagAranu aidu paTiTxge miVradaMte, eSuTx paTaTxnunx gelalxbahudeMba Baravase ideyoV aSuTx saMKeyxyanunx kUgi heVLuva isipxVTATa. 
\num{2} I ATadalilx aidu heVLuvudu. 
\num{3} (jUju, bAji, \mo vugaLalilx) elalx haNavanUnx oMdeV ATadalilx oDiDx biDuvudu. 
\num{4} idakAkxgi jUju salahegAranu mADuva ATada Ayekx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{go nap} 
\banum
\alnum{a} nAyxpf ATadalilx aidu paTaTxnUnx gelalxlu parxyatinxsu. 
\alnum{b} (\rUpa) oMdeV ATadalilx elalxvanUnx oDiDxbiDu. 
\alnum{c} oMdu sherxVNiyalilx elAlx paMdayx \mo vanunx gelulx. 
\eanum
\numie
\num{2} \eng{nap hand} naSaTxvAdare AgabahudeMdu dheYyaRdiMda nugagxbahudAda, oLeLxya gelulxva sithxti. 
\num{3} \eng{not to go nap on}(\AseTxrXV)(\AmA) bahaLa utusxkanAgilalxdiru; hecAcxgi lakiSxsadiru, gamana koDadiru. 
\enum
\emng
\eentry

\bentry
\word[nap(6)]{nap}
\pron{nAYxpf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{napped,} \vakaq\ \eng{napping}).}
\bmng
gelalxbahudeMba niriVkeSxyiruva kudure \mo vanunx gurutisi heVLu; gelulxva kudureyanunx heVLu. 
\emng
\eentry

\bentry
\word{napa}
\pron{nAYxpa}
\gl{\nA}
\bmng
= \hyperlink{nappa}{nappa}. 
\emng
\eentry

\bentry
\word[napalm(1)]{napalm}
\pron{neVpAmf}
\gl{\nA}
\bmng
neVpAmf: 
\banum
\alnum{a} peTorxVlanunx maMdagoLisalu upayoVgisuva alUyxminiyamf, sAbUnugaLa misharxNa. 
\alnum{b} aMtha maMdakAriyanunxpayoVgisi maMdagoLisida, beMki haraDuva bAMbugaLa tayArikeyalilx baLasuva peTorxVlu. 
\eanum
\emng
\eentry

\bentry
\word[napalm(2)]{naplam}
\pron{neVpAmf}
\gl{\sakirx}
\bmng
neVpAmf bAMbu hAku; neVpAmf bAMbina dALi mADu. 
\emng
\eentry

\bentry
\word{nape}
\pron{neVpf}
\gl{\nA}
\bmng
heDaku; hekakxtutx; katitxna hiMBAga. 
\emng

\noindent
\gl{\pagu}
\bmng
 \eng{nape of the neck} = \hyperlink{nape}{nape}. 
\emng
\eentry

\bentry
\word{napery}
\pron{neVpari}
\gl{\nA}
\bmng
(sAkxTelxMDf \parx\ yA \pArxparx) maneya baLake baTeTx; (\kanmu) UTada meVjina baTeTx, hodike, hAsu. 
\emng
\eentry

\bentry
\word{naphtha}
\pron{nAYxphatx}
\gl{\nA}
\bmng
nAyxphatx: 
\banum
\alnum{a} peTorxVliyamf. 
\alnum{b} peTorxVliyamf saMsakxraNadiMdAgali kalilxdadxlu \mo vugaLa nAshaka AsavanadiMdAgali doreta, kAbaRnika padAthaRgaLanunx karagisalu baLasuva darxva, heYDorxkAbaRnfgaLa misharxNa. 
\eanum
\emng
\eentry

\bentry
\word{naphthalene}
\pron{nAYxphatxliVnf}
\gl{\nA}
\bmng
(\ravi) nAyxphatxliVnf; kalilxdadxlu DAmarina AsavanadiMda tayArisuva, beLaLxneya, saPxTikarUpada, eraDu benisxVnf cakarxgaLa saMkalanadiMdAda AYxroVmAyxTikf heYDorxkAbaRnf, \eng{$\bg\rm C\eg_1\bg_0 \rm H\eg_8$}. 
\emng
\eentry

\bentry
\word{naphthalic}
\pron{nAYxphAtxlikf}
\gl{\gu}
\bmng
nAyxphAtxlikf; nAYxphatxliVnina yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{naphthene}
\pron{nAYxphitxVnf}
\gl{\nA}
\bmng
(\ravi) nAYxphitxVnf; raSayxnf peTorxVliyamfnalilx adhika parxmANadalilx kANasiguva, \eng{$\bg\rm C\eg_\bg\rm n\eg \bg\rm H\eg_\bg\rm 2n\eg$} eMba sAmAnayx aNusUtarxviruva, payARpatx cakirxVya heYDorxkAbaRnfgaLalilx oMdu. 
\emng
\eentry

\bentry
\word{naphthenic}
\pron{nAYxphitxVnikf}
\gl{\gu}
\bmng
nAYxphitxVnikf; nAYxphitxVnina yA adara rAYxDikalfna. 
\emng
\eentry

\bentry
\wordnospeech{naphthenic acid}{naphthenic acid}
\pron{?}
\gl{\nA}
\bmng
nAYxphitxVnikf Amalx; yAvudeV nAYxphitxVnfnalilx heYDorxjanf paramANuvanunx kAbARkisxlf gucaCxdiMda AdeVshisidAga uMTAguva Amalx. 
\emng
\eentry

\bentry
\word{naphthol}
\pron{nAYxphAtxlf}
\gl{\nA}
\bmng
(\ravi) nAyxphAtxlf; nAyxphatxliVnf racaneyanonxLagoMDa yAvudeV aNuvinalilx heYDorxjanf paramANuvina sAthxnadalilx heYDArxkisxlf gucaCx AdeVshavAdAga doreyuva phinAlika saMyukatx. 
\emng
\eentry

\bentry
\wordRemoveSpace{Napierian-logarithm}{Napierian logarithm}
\pron{neVpiarianf lAgaridamf}
\gl{\nA\ (\gu)}
\bmng
neVpiVriyanf lAgaridamf; lAgaridamf vayxvasethxyalilx \eng{e}yanunx \eng{(2.71828...)} adhArasaMKeyxyAgiTuTxkoMDAga dorakuva lAgaridamf. 
\emng
\eentry

\bentry
\wordRemoveSpace{Napier's-bones}{Napier's bones}
\pron{neVpiarfsx boVnfsx}
\gl{\nA}
\bmng
neVpiyarfna saraLugaLu; aMkagaNitakekx saMbaMdhisida lekakxgaLanunx mADalu neravAguva, gurutu mADiruva, hanonxMdu (mULe, mara, \mo vugaLiMda tayArisida) saraLugaLa taMDa. 
\emng
\eentry

\bentry
\word{napkin}
\pron{nAYxpfkinf}
\gl{\nA}
\bmng
\bnum
\num{1} karavasatxrX; keYvasatxrX; UTa mADuvAga tuTigaLanunx yA beraLugaLanunx oresikoLuLxva yA miVnu \mo vanunx baDisikoLuLxvAga baLasuva (baTeTxya yA kAgadada) cwka. 
\hypertarget{napkin(2)}{} 
\num{2} saNaNx, cikakx -- cwka. 
\num{3} (\birx) = \hyperlink{nappy(1)}{nappy}. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{lay up in a napkin} baLasade (hAgeyeV) iTuTxbiDu. 
\num{2} \eng{table napkin} = \hyperlink{napkin(2)}{napkin \((2)\)}. 
\enum
\emng
\eentry

\bentry
\word{napkin-ring}
\pron{nAYxpfkinfriMgf}
\gl{\nA}
\bmng
karavasatxrXda baLe; meVjina karavasatxrXvanunx iDuva loVhada yA marada paTiTx yA baLe. 
\emng
\eentry

\bentry
\word{napless}
\pron{nAYxpflisf}
\gl{\gu}
\bmng
(baTeTxya \vi) juMjilalxda; juMgilalxda. 
\emng
\eentry

\bentry
\word{napoleon}
\pron{napoVlianf}
\gl{\nA}
\bmng
\bnum
\num{1} (modalaneya nepoVliyanf Thasesxyiruva) \eng{20} phArxMku beleya, phArxnisxna oMdu cinanxda nANayx. 
\num{2} \eng{(19}neya shatamAnada) oMdu riVtiya etatxrada `bUTu', moVja. 
\num{3} = \hyperlink{nap(5)}{$^5$nap\((1)\)}. 
\num{4} (\ame) = \hyperref{kandict_m.pdf}{M}{millefeuille}{millefeuille}. 
\enum
\emng
\eentry

\bentry
\word{Napoleonic}
\pron{napoVliAnikf}
\gl{\gu}
\bmng
\bnum
\num{1} (phArxnisxna modalaneya) nepoVliyanfna yA nepoVliyanfnaMtha. 
\num{2} nepoVliyanf manetanada yA nepoVliyanf manetanadaMtha. 
\enum
\emng
\eentry

\bentry
\word{Napoleonically}
\pron{napoVliAnikali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} nepoVliyananxna riVtiyalilx; nepoVliyananxna hAge. 
\num{2} nepoVliyananxnunx yA nepoVliyanf manetanavanunx hoVluvaMte. 
\enum
\emng
\eentry

\bentry
\word{Napoleonism}
\pron{napoVlianisaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} (\kanmu\ phArxnisxna modalaneya) nepoVliyanfna tatatxvX, rAjaniVti, \mo vu. 
\num{2} nepoVliyanfvaMshada mamate; nepoVliyanf pakaSxpAta. 
\enum
\emng
\eentry

\bentry
\word{Napoleonist}
\pron{napoVlianisfTx}
\gl{\nA}
\bmng
nepoVliyanfvAdi: 
\banum
\alnum{a} nepoVliyanf manetanada mamateyuLaLxvanu. 
\alnum{b} nepoVliyanfna tatatxvXgaLanunx samathiRsuvavanu. 
\eanum
\emng
\eentry

\bentry
\word{nappa}
\pron{nAYxpa}
\gl{\nA}
\bmng
(visheVSa riVtiyalilx saMsakxrisida) kuriya yA meVkeya maqdu camaR. 
\emng
\eentry

\bentry
\word{nappe}
\pron{nAYxpf}
\gl{\nA}
\bmng
(\BUvi) pakakx sarida padara; atiyAda otatxrisuvikege oLagAgi, akakxpakakxda satxragaLa meVle calisida baMDeya padara. 
\emng
\eentry

\bentry
\word{napper}
\pron{nAYxparf}
\gl{\nA}
\bmng
(\birx) (\ashi) tale. 
\emng
\eentry

\bentry
\word[nappy(1)]{nappy}
\pron{nAYxpi}
\gl{\gu}
\bmng
\bnum
\num{1} (\pArxparx) nore noreyAda; buruguLaLx. 
\num{2} matutx barisuva; tiVvarx vegaTuLaLx. 
\num{3} (sAkxTelxMDf) (\ashi) kuDida; matetxVrida; amaleVrida. 
\enum
\emng
\eentry

\bentry
\word[nappy(2)]{nappy}
\pron{nAYxpi}
\gl{\nA}
\bmng
(\pArxparx) norenoreyAda mAdaka pAniVya \mo vu. 
\emng
\eentry

\bentry
\word[nappy(3)]{nappy}
\pron{nAyxpi}
\gl{\nA}
\expl{(\bava\ \eng{nappies}).}
\bmng
(mala mUtArxdigaLu soVradaMte) makakxLige kaTuTxva baTeTx, cwka, \mo vu. 
\emng
\eentry

\bentry
\wordnospeech{nappy rash}{nappy rash}
\pron{?}
\gl{\nA}
\bmng
caDiDxdadudx; odedxyAda mala mUtarx cwkavu satatavAgi taguli taguli Aguva, maguvina camaRda uriyUta, gaMde, dadudx. 
\emng
\eentry

\bentry
\word{napu}
\pron{nApU}
\gl{\nA}
\bmng
(malayA divxVpada) kasUtxri jiMke. 
\emng
\eentry

\bentry
\word{narceine}
\pron{nAsiRInf}
\gl{\nA}
\bmng
(\ravi) nAsiRVnf; aPiVminiMda tegeda, nidedx barisuva AlakxlAyfDx, \eng{$\bg\rm C\eg_\bg 23\eg \bg\rm H\eg_\bg 27\eg \bg\rm NO\eg_8$.} 
\emng
\eentry

\bentry
\word{narcissism}
\pron{nAsiRsisaZmf}
\gl{\nA}
\bmng
(\mashA) 
\bnum
\num{1} Atamxrati; AtAmxsakitx; AtamxkAmane; sAvxnurakitx; tanonxlavu; tananx savxMta rUpAkaqtigaLalilx vipariVtavAda yA kAmAtamxkavAda pirxVti, Asakitx. 
\num{2} sAvxrAdhanA parxvaqtitx; AtamxshAlxGane; savxparxshaMse; tananxnunx tAneV mecicxkoLuLxvudu. 
\enum
\emng
\eentry

\bentry
\word{narcissist}
\pron{nAsiRsisfTx}
\gl{\nA}
\bmng
\bnum
\num{1} AtAmxsakatx; tananx savxMta rUpa, AkaqtigaLalilx vipariVtavAda yA kAmAtamxkavAda AsakitxyuLaLxva. 
\num{2} savxparxshaMsaka; tananxnunx tAnu ArAdhisuvavanu; tananxnunx tAnu mecicxkoLuLxvavanu. 
\enum
\emng
\eentry

\bentry
\word{narcissistic}
\pron{nAsiRsisiTxkf}
\gl{\gu}
\bmng
\bnum
\num{1} Atamxratiya. 
\num{2} sAvxrAdhanA parxvaqtitxya; AtamxshAlxGaneya; tananxnunx tAneV mecicxkoLuLxva. 
\enum
\emng
\eentry

\bentry
\word{narcissistically}
\pron{nAsiRsisiTxkali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} AtAmxsakitxyiMda; atamxratiyiMda. 
\num{2} sAvxrAdhanA parxvaqtitxyiMda; savxparxshaMseyiMda. 
\enum
\emng
\eentry

\bentry
\word{narcissus}
\pron{nAsiRsasf}
\gl{\nA}
\expl{(\bava\ \eng{narcissi} \ucAcx-- nAsiRseY, yA \eng{narcissuses}).}
\bmng
(\savi) nAsiRsasf; yUroVpinalilx visheVSavAgi kaMDubaruva DAyxphoDilfnaMtha tutUtxri AkArada hU biDuva, geDeDxgiDagaLa oMdu sasayxkula. 
\emng
\eentry

\bentry
\word{narcolepsy}
\pron{nAkaRlepisx}
\gl{\nA}
\bmng
(\veYshA) nAkoRlepisx; viciCxdarx niderx; AgAga alapxkAlika niderx baruva oMdu roVga. 
\emng
\eentry

\bentry
\word[narcoleptic(1)]{narcoleptic}
\pron{nAkaRlepiTxkf}
\gl{\nA}
\bmng
(\veYshA) nAkoRlepisx piVDita (roVgi). 
\emng
\eentry

\bentry
\word[narcoleptic(2)]{narcoleptic}
\pron{nAkaRlepiTxkf}
\gl{\gu}
\bmng
(\veYshA) nAkoRlepisx roVgada. 
\emng
\eentry

\bentry
\word{narcosis}
\pron{nAkoRVsisf}
\gl{\nA}
\bmng
\bnum
\num{1} (\veYshA) nAkoRVsisf; veVdanAshAmakagaLu pariNAmavAgi uMTAda maMparu athavA parxjecnx ilalxdiruvike. 
\num{2} saMveVdanAshUnayxte. 
\enum
\emng
\eentry

\bentry
\word[narcotic(1)]{narcotic}
\pron{nAkARTikf}
\gl{\gu}
\bmng
\bnum
\num{1} nAkoRTikf; veVdanAshAmaka; saMveVdanAhAri; saMveVdaneyanunx maMdagoLisi noVvu hoVgalADisabalalx. 
\num{2} (\rUpa) nidArxjanaka. 
\num{3} saMveVdanA rahita sithxtiya; veVdane tiLiyada sithxtiya. 
\num{4} (niderx barisuva madudx \mo vugaLa \vi) manasisxna veVle parxBAvavanunxMTumADuva, pariNAma biVruva; mAdaka. 
\enum
\emng
\eentry

\bentry
\word[narcotic(2)]{narcotic}
\pron{nAkARTikf}
\gl{\nA}
\bmng
(\veYshA) nAkoRTikf; mAdaka vasutx; saMveVdanA hAri; sAdhAraNa parxmANadalilx seVvisidAga saMveVdaneyanunx maMdagoLisi, noVvu hoVgalADisi, gADhaniderx barisabalalx matutx adhika parxmANadalilx seVvisidAga parxjecnx kaLeyabalalx, aPiVminaMtha yAvudeV madudx. 
\emng
\eentry

\bentry
\word{narcotically}
\pron{nAkARTikali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} niderx barisuvaMte; maMparugoLisuva riVtiyalilx. 
\num{2} veVdanAjaDateyanunxMTumADuvaMte. 
\enum
\emng
\eentry

\bentry
\word{narcotise}
\pron{nAkaRTeYsf}
\gl{\kirx}
\bmng
= \hyperlink{narcotize}{narcotize}. 
\emng
\eentry

\bentry
\word{narcotism}
\pron{nAkaRTisamf}
\gl{\nA}
\bmng
\bnum
\num{1} niderx, maMparu barisuva mAdaka vasutxgaLanunx seVvisuva duraBAyxsa. 
\num{2} nidArxjanaka vasutxgaLa kirxye yA parxBAva. 
\num{3} = \hyperlink{narcosis}{narcosis}.
\num{4} ati nidedx; atinidArxroVga. 
\enum
\emng
\eentry

\bentry
\word{narcotization}
\pron{nAkaRTeYseZVSanf}
\gl{\nA}
\bmng
\bnum
\numi{1} (nidArxjanaka baLasi) 
\banum
\alnum{a} niderx barisuvike. 
\alnum{b} parxjecnx tapipxsuvike. 
\eanum
\numie
\num{2} jaDatavxvanunxMTumADuvike. 
\enum
\emng
\eentry

\bentry
\word{narcotize}
\pron{nAkaRTeYsfZ}
\gl{\sakirx}
\bmng
\bnum
\numi{1} (nidArxjanaka vasutxvanunx baLasi) 
\banum
\alnum{a} maMparu barisu; niderx barisu. 
\alnum{b} parxjecnx tapipxsu; arivaLikeyanunxMTu mADu; veVdanAjaDateyanunxMTumADu. 
\eanum
\numie
\num{2} jaDatavxvanunxMTumADu; aceVtanagoLisu: \eng{to narcotize one's anxieties} AtaMka, kaLavaLagaLanunx aceVtanagoLisu, jaDuDxgaTiTxsu; talalxNateya arivaLisu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
niderx barisuva vasutxvinaMte kelasa mADu. 
\emng
\eentry

\bentry
\word{nard}
\pron{nADfR}
\gl{\nA}
\bmng
(\savi) jaTile; jaTAmAMsi: 
\banum
\alnum{a} himAlayadalilx sikukxva nADoRsATxkisf kulada mUlike. 
\alnum{b} adariMda tayArisida mulAmu. 
\eanum
\emng
\eentry

\bentry
\word{nardoo}
\pron{nADUR}
\gl{\nA}
\bmng
(\savi) nADUR; 
\banum
\alnum{a} AseTxrXVliyadalilx iruva kudure masAlegiDadaMtha oMdu giDa. 
\alnum{b} adara biVjagaLiMda tayArisida AdivAsigaLa AhAra adAthaR. 
\eanum
\emng
\eentry

\bentry
\word{nares}
\pron{neVriVszf}
\gl{\nA}
\expl{(\bava) (\aMrashA)}
\bmng
mUgina hoLeLxgaLu; nAsAraMdharxgaLu. 
\emng
\eentry

\bentry
\word{narghile}
\pron{nAgiRli}
\gl{\nA}
\bmng
hukakx; guDuguDi. \imglink{narghile:fig}{\raisebox{-0.20cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.6cm {N_Pictures/narghile.jpg}}} 
\emng
\eentry

\bentry
\word{narial}
\pron{neVrialf}
\gl{\gu}
\bmng
mUgina hoLeLxya yA mUgina hoLeLxgaLige saMbaMdhisida; nAsAraMdharxda; nAsAraMdhirxVya. 
\emng
\eentry

\bentry
\word[nark(1)]{nark}
\pron{nAkfR}
\gl{\nA}
\bmng
\bnum
\numi{1} (\birx) (poliVsinavariMda niyamitanAda) 
\banum
\alnum{a} beVhugAra; gUDhacAra. 
\alnum{b} (Ase toVrisi balege biVLisuva) vaMcaka. 
\eanum
\numie
\num{2} (\AseTxrXV) piVDaka; kADisuva, piVDisuva, reVgisuva -- vasutx, vayxkitx. 
\enum
\emng
\eentry

\bentry
\word[nark(2)]{nark}
\pron{nAkfR}
\gl{\sakirx}
\bmng
piVDisu; kADisu; reVgisu; kirikirigoLisu; uderxVkavanunxMTumADu: \eng{feel narked at unjust criticism} anAyxyavAda TiVkeyanunx keVLi kirikirigoMDu, uderxVkagoMDu. 
\emng

\noindent
\gl{\pagu}
\bmng
 \eng{nark it}! (vidhi rUpa) adu sAku! adanunx nililxsu! 
\emng
\eentry

\bentry
\word{narky}
\pron{nAkiR}
\gl{\gu}
\expl{(\tara\ \eng{narkier,} \tama\ \eng{narkiest}).}
\bmng
reVguva; uderxVkagoLuLxva; siTiTxgeVLuva; siDimiDiguTuTxva. 
\emng
\eentry

\bentry
\word{narratable}
\pron{nareVTabflf}
\gl{\gu}
\bmng
kathisabahudAda; kathana rUpadalilx -- heVLabahudAda, nirUpisabahudAda. 
\emng
\eentry

\bentry
\word{narrate}
\pron{nareVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (kathArUpavAgi) heVLu; nirUpisu; kathisu; kathana mADu; kathana bare; avicicxnanx vaqtAtxMta koDu. 
\num{2} (anuBava \mo vanunx) nirUpisu; heVLu; vivaravAgi -- heVLu, bare. 
\enum
\emng
\eentry

\bentry
\word{narration}
\pron{nareVSanf}
\gl{\nA}
\bmng
\bnum
\num{1} kathana; kathAnirUpaNe. 
\num{2} nirUpaNe; heVLuvike; vivaraNe (niVDike); vivaravAgi bareyuvudu, heVLuvudu. 
\enum
\emng
\eentry

\bentry
\word[narrative(1)]{narrative}
\pron{nAYxraTivf}
\gl{\nA}
\bmng
\bnum
\num{1} (\kanmu\ utatxma puruSadalilx heVLida) nirUpaNe; vaqtAtxMta nirUpaNe. 
\num{2} kathe; kathana; AKAyxna; AKAyxyike; kathA nirUpaNe. 
\num{3} kathana kale; kathana kirxye; kathA nirUpaNeya taMtarx, sheYli. 
\enum
\emng
\eentry

\bentry
\word[narrative(2)]{narrative}
\pron{nAYxraTivf}
\gl{\gu}
\bmng
kathanarUpada; kathArUpada; kathAnirUpaNege saMbaMdhisida. 
\emng
\eentry

\bentry
\word{narratively}
\pron{nAYxraTivfli}
\gl{\kirxvi}
\bmng
kathanarUpavAgi; kathArUpadalilx; kathAsaMbaMdhavAgi. 
\emng
\eentry

\bentry
\word{narrator}
\pron{nareVTarf}
\gl{\nA}
\bmng
nirUpaka: kathaka: vivaraNe niVDuvavanu, \udA\ nATakadalilx yA calanacitarx, reVDiyoV parxsAra, \mo vugaLalilx saMBASaNeya naDuvaNa GaTanegaLanunx nirUpisuvavanu. 
\emng
\eentry

\bentry
\word[narrow(1)]{narrow}
\pron{nAYxroV}
\gl{\gu}
\expl{(\tara\ \eng{narrower,} \tama\ \eng{narrowest}).}
\bmng
\bnum
\num{1} kaDameyagalada; agala kaDameyAda; (udadxkekx takakxMte) Agala sAlada. 
\num{2} (\vAyx) (savxrada \vi) nAlage kirunAlagegaLanunx bigumADi ucacxrisuva; saMkucita; saMvaqta. 
\num{3} kiridAda; ikakxTATxda; kiSikxMdada: \eng{within narrow bounds} kiridAda parimitiyoLage. 
\num{4} saMkucita; parimita: \eng{in the narrowest sense} tiVrA saMkucita athaRdalilx. 
\num{5} kaDimeyAgiruva; alapx parxmANada; savxlapx aMtarada: \eng{a narrow majority} atayxlapx aMtarada bahumata. 
\num{6} daqSiTxveYshAlayxvilalxda. 
\num{7} anukaMpavilalxda. 
\num{8} audAyaRvilalxda; saNaNx manasisxna; anudAra. 
\num{9} durAgarxhada. 
\num{10} parxteyxVkateya. 
\num{11} sAvxthaRda; savxvicAra maganx. 
\num{12} kaNaNxlilx kaNiNxTuTx noVDuva; sUkaSxmXvAda; kaTuTx niTATxda; karAruvAkAkxda: \eng{after a narrow examination} sUkaSxmX pariVkeSxya naMtara. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{a narrow escape} savxlapxdaralilx tapipxsikoMDadudx; kUdaleLeyaSaTxralilx pArAdadudx. 
\hypertarget{narrow1pagu2}{}. 
\num{2} \eng{narrow bed} samAdhi; goVri. 
\num{3} \eng{narrow cell} = \hyperlink{narrow1pagu2}{?pagu?\((2)\)}. 
\num{4} \eng{narrow house} = \hyperlink{narrow1pagu2}{?pagu?\((2)\)}. 
\num{5} \eng{narrow circumstances} baDatana; dAridarxyX. 
\numi{6} \eng{narrow squeak} 
\banum
\alnum{a} savxlapxdaralilx tapipxsikoMDadudx; kUdaleLeyalilx pArAdadudx. 
\alnum{b} sAkaSuTx anukUlagaLilalxdidadxrU paDeda gelavu, yashasusx; kUdaleLeya gelavu, jaya. 
\eanum
\numie
\num{7} \eng{narrow way} dhamaRmAgaR; sadAcAramAgaR. 
\enum
\emng
\eentry

\bentry
\word[narrow(2)]{narrow}
\pron{nAYxroV}
\gl{\nA}
\bmng
(\sA\ \bava dalilx) (jalasaMdhi, shabadx, nadi, kaNive, biVdi, \mo vugaLa) ikakxTuTx BAga. 
\emng
\eentry

\bentry
\word[narrow(3)]{narrow}
\pron{nAYxroV}
\gl{\sakirx}
\bmng
kiridu mADu; cikakxdAgisu; saMkucitagoLisu: \eng{she narrowed her eyelids} avaLu kaNuNxrepepxgaLanunx saMkucitagoLisikoMDaLu. 
\emng

\noindent
\gl{\akirx}
\bmng
kiridAgu; saNaNxdAgu. 
\emng
\eentry

\bentry
\wordnospeech{narrow boat}{narrow boat}
\pron{?}
\gl{\nA}
\bmng
(\kanmu\ ELu aDigiMta kaDime agalaviruvaMtha) kirudoVNi. 
\emng
\eentry

\bentry
\wordnospeech{narrow cloth}{narrow cloth}
\pron{?}
\gl{\nA}
\bmng
\eng{(52} aMgulakikxMta kaDame agala iruva) oMTi pananxda baTeTx. 
\emng
\eentry

\bentry
\wordnospeech{narrow gauge}{narrow gauge}
\pron{?}
\gl{\nA}
\bmng
nAYxroV geVju; \eng{52} aMgulagaLigiMta agala kiridAda reYludAri. 
\emng
\eentry

\bentry
\word{narrowish}
\pron{nAYxroViSf}
\gl{\gu}
\bmng
tusu kiriyagalada: tusu ikakxTiTxna; savxlapx saMkucitavAda. 
\emng
\eentry

\bentry
\word{narrowly}
\pron{nAYxroVli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} sUkaSxmXvAgi; kaNaNxlilx kaNiNxTuTx: \eng{narrowly watched him} avananunx kaNaNxlilx kaNiNxTuTx noVDida. 
\num{2} parimitavAgi; saMkucitavAgi. 
\num{3} savxlapxdaralilx; kUdaleLeyaSaTxralilx: \eng{one arrow very narrowly missed my left eye} oMdu bANa savxlapxdaralilx nananx eDagaNiNxge tAkade hoVyitu. 
\num{4} anudAravAgi; saMkucita manasisxniMda. 
\enum
\emng
\eentry

\bentry
\word{narrow-minded}
\pron{nAYxroVmeYMDiDf}
\gl{\gu}
\bmng
saNaNx manasisxna; saMkucita budidhxya; anudAra. 
\emng
\eentry

\bentry
\word{narrow-mindedly}
\pron{nAYxroVmeYMDiDfli}
\gl{\kirxvi}
\bmng
saNaNx manasisxniMda; saMkucita budidhxyiMda; anudArateyiMda. 
\emng
\eentry

\bentry
\word{narrow-mindedness}
\pron{nAYxroVmeYMDiDfnisf}
\gl{\nA}
\bmng
saNaNxmanasusx; saMkucita manoVBAva, budidhx; anudArate. 
\emng
\eentry

\bentry
\word{narrowness}
\pron{nAYxroVnisf}
\gl{\nA}
\bmng
\bnum
\num{1} kiridAgiruvike; ikakxTuTx; kiSikxMda. 
\num{2} saMkucitate. 
\num{3} anudArate; saNaNx manasusx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{narrow seas}{narrow seas}
\pron{?}
\gl{\nA}
\bmng
iMgilxSf kaDalAgxluve hAgU airiSf samudarx. 
\emng
\eentry

\bentry
\word{narthex}
\pron{nAteRkfsx}
\gl{\nA}
\bmng
\bnum
\num{1} (\sA\ heMgasaru, pApaniveVdane mADikoLuLxvavaru, matAMtara paDevavara saluvAgi kaTiTxda, kabibxNada salAkegaLanunx hAki beVpaRDisida) pArxciVna caciRna pashicxma BAgada muKamaMTapa, hajAra. 
\num{2} Adhunika caciRna aMthadeV muMgoVNe. 
\enum
\emng
\eentry

\bentry
\word{narwhal}
\pron{nAvaRlf}
\gl{\nA}
\bmng
nAvaRlf; utatxra shiVtavalayadalilx sikukxva, sumAru ipapxtatxDi udadxda, niVLavAda daMtaviruva, timiMgila baLagada pArxNi. \imglink{narwhal:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.5cm {N_Pictures/narwhal.jpg}}} 
\emng
\eentry

\bentry
\word{nary}
\pron{neVri}
\gl{\gu}
\bmng
(\AmA) oMdU ilalx; ilalx: \eng{nary a one} oMdU ilalx. 
\emng
\eentry

\bentry
\wordnospeech{NAS}{NAS}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) 
\bnum
\num{1} \eng{National Association of Schoolmasters.} 
\num{2} \eng{Noise Abatement Society.} 
\enum
\emng
\eentry

\bentry
\word{NASA}
\pron{nAYxsa}
\gl{\saMkiSx}
\bmng
(\ame) \eng{National Aeronautics and Space Administration.} 
\emng
\eentry

\bentry
\word[nasal(1)]{nasal}
\pron{neVsaZlf}
\gl{\gu}
\bmng
\bnum
\num{1} mUgina; nAsikada. 
\num{2} (akaSxra, dhavxni, \mo vugaLa \vi) anunAsika; mUginiMda ucacxrisabeVkAda (\udA\ \eng{m, n, ng).} 
\num{3} (dani, mAtugaLa \vi) mUginiMda ucacxrisuva. 
\enum
\emng
\eentry

\bentry
\word[nasal(2)]{nasal}
\pron{neVsaZlf}
\gl{\nA}
\bmng
\bnum
\num{1} (\dhavxni) anunAsikAkaSxra (\udA\ \eng{m,n}). 
\num{2} (\ca) mUgina kApu; shirasAtxrXNada mUganunx rakiSxsuva BAga. 
\enum
\emng
\eentry

\bentry
\word{nasalise}
\pron{neYsaZleYszf}
\gl{\kirx}
\bmng
= \hyperlink{nasalize}{nasalize}. 
\emng
\eentry

\bentry
\word{nasality}
\pron{neVsAZYXliTi}
\gl{\nA}
\bmng
anunAsikate. 
\emng
\eentry

\bentry
\word{nasalization}
\pron{neVsaleYseZVSanf}
\gl{\nA}
\bmng
anunAsikate; nAsikiyxVkaraNa; dhavxniyanunx mUginiMda ucacxrisuvike. 
\emng
\eentry

\bentry
\word{nasalize}
\pron{neVsaleYsfZ}
\gl{\sakirx}
\bmng
(dhavxniyanunx) mUginiMducacxrisu; anunAsikagoLisu. 
\emng

\noindent
\gl{\akirx}
\bmng
(bAyiyiMda ucacxrisuva dhavxni \mo vanunx) anunAsikavAgi horaDisu; mUginalilx mAtanADu. 
\emng
\eentry

\bentry
\word{nasally}
\pron{neVsaZli}
\gl{\kirxvi}
\bmng
anunAsikavAgi; mUginiMda; mUgina mUlaka. 
\emng
\eentry

\bentry
\wordnospeech{nasal organ}{nasal organ}
\pron{?}
\gl{\nA}
\bmng
(\hA) mUgu. 
\emng
\eentry

\bentry
\word{nascency}
\pron{neVsanisx}
\gl{\nA}
\bmng
\bnum
\num{1} huTuTx; utapxtitx; janamx. 
\num{2} huTuTxvike; janana. 
\enum
\emng
\eentry

\bentry
\word{nascent}
\pron{neVsaMTf}
\gl{\gu}
\bmng
\bnum
\num{1} huTuTxtatxliruva. 
\num{2} Iga tAne AraMBavAgutitxruva. 
\num{3} eLasAda; inUnx balitilalxda. 
\num{4} (\ravi) udaBxvAvasethxya; AgatAneV rUpugoLuLxtitxruva, AdudariMda bahupaTuvAda. 
\enum
\emng
\eentry

\bentry
\word{naseberry}
\pron{neVszfbari}
\gl{\nA}
\expl{(\bava\ \eng{naseberries}).}
\bmng
(\savi) sapoVTa (haNuNx); cikukx. 
\emng
\eentry

\bentry
\word{naso-}
\pron{neVsoZV-}
\gl{\sapUpa}
\bmng
nAsA--; mUgu--, mUgina eMbathaRda shabadxgaLa racaneyalilx baLasuva \sapUpa. 
\emng
\eentry

\bentry
\word{naso-frontal}
\pron{neVsoZVpharXMTalf}
\gl{\gu}
\bmng
nAsA PAlada; mUgina matutx haNeya. 
\emng
\eentry

\bentry
\word{nastic}
\pron{nAYxsiTxkf}
\gl{\gu}
\bmng
(\savi) oLotatxDada; aMtarotatxDada; (sasayxda calaneya \vi) bAhayx parxcoVdaneya pariNAmavAgirade oLagina otatxDa asamavAgi EpaRTuTxdariMda uMTAda. 
\emng
\eentry

\bentry
\word{nastily}
\pron{nAsiTxli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} asahayxvAguvaSuTx -- koLakAgi, galiVjAgi; balu -- holasAgi, heVsikeyiMda. 
\num{2} ashilxVlavAgi. 
\num{3} gabubx vAsaneyiMda kUDi; aroVcakavAgi. 
\num{4} piVDisuvaMte; kADuvaMte. 
\num{5} kaThiNavAgi; kaSaTxsAdhayx riVtiyalilx. 
\num{6} reVguva riVtiyalilx. 
\num{7} apAyakaravAgi. 
\enum
\emng
\eentry

\bentry
\word{nastiness}
\pron{nAsiTxnisf}
\gl{\nA}
\bmng
\bnum
\num{1} jugupAsxtamxkate; asahayxvAguvaSuTx -- koLakutana, galiVju, holasutana. 
\num{2} ashilxVlate. 
\num{3} gabubxvAsane; aroVcakate. 
\num{4} piVDisuvike; kADuvike. 
\num{5} kaThiNate; dusatxrate. 
\num{6} keTaTxsavxBAva. 
\enum
\emng
\eentry

\bentry
\word{nasturtium}
\pron{nasaTxSaRmf}
\gl{\nA}
\bmng
\bnum
\num{1} ujajxvXlavAda haLadi kitatxLe athavA keMpu hUbiDuva, duMDAda tinanxbalalx elegaLiruva, oMdu baLiLx. 
\num{2} catudaRLada hUgaLanunx biDuva nasaTxSiRyaM giDagaLa kula. 
\enum
\emng
\eentry

\bentry
\word{nasty}
\pron{nAsiTx}
\gl{\gu}
\expl{(\tara\ \eng{nastier,} \tama\ \eng{nastiest}).}
\bmng
\bnum
\num{1} asahayxvAguvaSuTx -- koLakAda, galiVjAda, holasAda, heVsigeyAda, jugupesxya. 
\num{2} ashilxVlavAda; asaBayxvAda: \eng{nasty stories} ashilxVla kathegaLu. 
\num{3} ashilxVla pirxya; ashilxVlavAdudaralilx AsakitxyuLaLx, olavuLaLx, pirxVtiyuLaLx. 
\num{4} keTaTx; ahitavAda; mUgige yA nAlagege hiDisada; aroVcaka; duvARsaneya; gabubxnAtada (\rUpa\ saha): \eng{nasty smell} dunARta. \eng{nasty experience} ahitavAda anuBava. 
\num{5} kirikirigoLisuva; reVgisuva. 
\num{6} AkeSxVpaNiVya: AkeSxVpAhaR. 
\numi{7} (vAyuguNa \mo vugaLa \vi) 
\banum
\alnum{a} keTaTx; ahita; asahayxvAda: \eng{nasty habit} keTaTx aBAyxsa; duraBAyxsa. 
\alnum{b} sheYtayxda; taMDiyAda. 
\alnum{c} (biru)gALi maLeya. 
\eanum
\numie
\num{8} nivaRhisalu -- kaSaTxvAda, dusatxravAda: \eng{a nasty fence} hAyalu kaSaTxvAda beVli. \eng{a nasty question} biDisalu kaSaTxvAda samaseyx. 
\num{9} apAyakaravAda; gaMBiVravAda: \eng{a nasty accident} apAyakaravAda apaGAta. \eng{nasty illness} gaMBiVravAda beVne. 
\num{10} duSaTx: keTaTx savxBAvada: \eng{a nasty dog} duSaTxnAyi. 
\num{11} koVpiSaThx; reVguva. 
\num{12} seVDina; devxVSa BAvada. 
\num{13} ugarx; AveVshada. 
\num{14} meVle biVLuva; AkarxmaNakAri. 
\num{15} (gAyada \vi) kiVtukoLuLxva; kiVvu tuMbikoLuLxva. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{a nasty bit}(\engit{or} \eng{piece) of work} (\AmA) tucaCx yA takarArina vayxkitx. 
\numi{2} \eng{a nasty one} 
\banum
\alnum{a} tirasAkxra; dhikAkxra. 
\alnum{b} CiVmAri; muKaBaMga. 
\alnum{c} mujugaragoLisuva, irasu murasina yA toDakAda -- parxshenx, samaseyx. 
\alnum{d} shakitxguMdisuva, dubaRlagoLisuva hoDeta \mo vu. 
\eanum
\numie
\num{3} \eng{cheap and nasty} agagxda bele mugagxlu mAlu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NAS/UWT}{NAS/UWT}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Association of Schoolmasters and Union of Women Teachers.} 
\emng
\eentry

\bentry
\wordnospeech{Nat.}{Nat.}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{National.} 
\num{2} \eng{Nationalist.} 
\num{3} \eng{Natural.} 
\enum
\emng
\eentry

\bentry
\word{natal}
\pron{neVTalf}
\gl{\gu}
\bmng
\bnum
\num{1} (obabxna) huTiTxna; jananada; huTiTxge saMbaMdhisida: \eng{natal hour} (obabxna) huTiTxda gaMTe; jananakAla. 
\num{2} Ajanamxda; huTiTxdAginiMda baMda; jAyamAnada. 
\enum
\emng
\eentry

\bentry
\word{natality}
\pron{naTAYxliTi}
\gl{\nA}
\expl{(\bava\ \eng{natalities}).}
\bmng
janasaMKAyx parxmANa; huTiTxna dara. 
\emng
\eentry

\bentry
\word{natation}
\pron{naTeVSanf}
\gl{\nA}
\bmng
(\aupa\ yA sAhitayxka) Ijuvudu yA Ijuva kale. 
\emng
\eentry

\bentry
\word{natatorial}
\pron{neV(nAYx)TaToVrialf}
\gl{\gu}
\bmng
\bnum
\num{1} Ijuva. 
\num{2} Ijina. 
\enum
\emng
\eentry

\bentry
\word{natatorium}
\pron{neVTaToVriamf}
\gl{\nA}
\bmng
(\ame) (\kanmu\ maneyoLagina) IjukoLa. 
\emng
\eentry

\bentry
\word{natatory}
\pron{neVTaTari, naTeVTari}
\gl{\gu}
\bmng
= \hyperlink{natatorial}{natatorial}. 
\emng
\eentry

\bentry
\word{natch}
\pron{nAYxcf}
\gl{\kirxvi}
\bmng
(\AmA) sahajavAgiyeV; sahajaPalavAgiyeV; niriVkiSxsidaMteyeV. 
\emng
\eentry

\bentry
\word{nates}
\pron{neVTiVsfZ}
\gl{\nA}
\bmng
(\bava) (\aMrashA) 
\bnum
\num{1} aMDugaLu; pirerxgaLu; kuMDegaLu; nitaMbagaLu. 
\num{2} pirerxgaLanunx hoVluva yAvudeV racane, \udA\ miduLinalilxruva daqSiTxge saMbaMdhisida hAlegaLalilx hiMdugaDeya joVDi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NATFHE}{NATFHE}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Association of Teachers in Further and Higher Education.} 
\emng
\eentry

\bentry
\word{nathless}
\pron{neVtflisf}
\gl{\kirxvi}
\bmng
(\pArxparx) = \hyperlink{nevertheless}{nevertheless}. 
\emng
\eentry

\bentry
\word{nation}
\pron{neVSanf}
\gl{\nA}
\bmng
\bnum
\num{1} janAMga; janate; deVsha; rASaTxrX; oMdeV kula, BASe, cariterx, rAjakiVya saMsethxgaLu, \mo vanunxLaLx parxteyxVka buDakaTiTxnavaru. 
\num{2} utatxra amerikada (reDf) iMDiyanf buDakaTuTx. 
\num{3} (madhayxyugada matutx sAkxTelxMDina kelavu vishavx vidAyxnilayagaLalilx) oMdu vishiSaTx deVsha yA pArxMtadiMda baMda vidAyxthiR taMDa. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{law of nations} aMtararASiTxrXVya nAyxyashAsatxrX. 
\num{2} \eng{most favoured nation} atAyxdaqta rASaTxrX; ati purasakxqqta rASaTxrX; oMdu sakARradiMda atayxMta kaDime Amadu suMka hAkalAda janAMga, rASaTxrX. 
\enum
\emng
\eentry

\bentry
\word[national(1)]{national}
\pron{nAYxSanalf}
\gl{\gu}
\bmng
janAMgiVya; janAMgada; janateya: 
\banum
\alnum{a} oTuTx janAMgakekx seVrida; rASiTxrXVya; deVshiVya. 
\alnum{b} oMdu janAMgakekx vishiSaTxvAda; oMdu rASaTxrXda lakaSxNagaLa parxtiVkavAda. 
\eanum
\emng
\eentry

\bentry
\word[national(2)]{national}
\pron{nAYxSanalf}
\gl{\nA}
\bmng
\bnum
\num{1} savxdeVshiVya (vayxkitx); savxrASiTxrXVya; adeV nADinavanu: \eng{consul's power over his own nationals} tananx savxdeVshiVyara meVle rAyaBArige iruva adhikAra. 
\num{2} deVshiVya; rASiTxrXVya; (oMdu vishiSaTx deVshada) parxje; nAgarika: \eng{French nationals} pherxMcf rASiTxrXVyaru. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\hypertarget{national2pagu1}{} 
\bnum
\numi{1} \eng{Grand National} (\birx) 
\banum
\alnum{a} mahA rASiTxrXVya paMdayx; parxtivaSaR livarfpUlfnalilx rasetxbiTuTx kADumeVDinalilx naDeyuva kudure jUju. 
\alnum{b} kADumeVDina hAdiyalilx naDeyuva kAlunaDige paMdayx. 
\eanum
\numie
\num{2} \eng{the National} = \hyperlink{national2pagu1}{?pagu?\((1)\)}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{national anthem}{national anthem}
\pron{?}
\gl{\nA}
\bmng
rASaTxrXgiVte; deVshaBakitx yA rASaTxrXniSeThxyanunx toVrisuvudeMdu rASaTxrXvu aMgiVkarisiruva giVte, hADu. 
\emng
\eentry

\bentry
\wordnospeech{National Assembly}{National Assembly}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} rASaTxrXda saMsatutx; vividha deVshagaLa cunAyita shAsanasaBegaLu. 
\num{2} (\kanmu\ cariterx) \eng{1789-91}ra avadhiya pherxMcf rASaTxrXsaBe. 
\enum
\emng
\eentry

\bentry
\wordnospeech{National Assistance}{National Assistance}
\pron{?}
\gl{\nA}
\bmng
(\ca) (\birx) 
\bnum
\num{1} nAyxSanalf infSUrenfsx vayxvasethxya parxkAra sikukxva pUraka parxyoVjana. 
\num{2} adara hiMdina hesaru. 
\enum
\emng
\eentry

\bentry
\wordnospeech{national bank}{national bank}
\pron{?}
\gl{\nA}
\bmng
(\ame) (pheDaralf sakARrada niyamAnusAra vayxvasethxgoMDa) rASiTxrXVya bAyxMku. 
\emng
\eentry

\bentry
\wordnospeech{national convention}{national convention}
\pron{?}
\gl{\nA}
\bmng
(\ame) (adhayxkaSx sAthxna \mo vakekx umeVduvAraranunx nAmakaraNa mADalu seVruva) parxmuKa pakaSxda rAjakiVya adhiveVshana. 
\emng
\eentry

\bentry
\wordnospeech{national debt}{national debt}
\pron{?}
\gl{\nA}
\bmng
rASiTxrXVya sAla. 
\emng
\eentry

\bentry
\wordnospeech{national football}{national football}
\pron{?}
\gl{\nA}
\bmng
(\AseTxrXV) AseTxrXVliyada niyamAvaLigaLa kAlecxMDATa. 
\emng
\eentry

\bentry
\wordnospeech{National Front}{National Front}
\pron{?}
\gl{\nA}
\bmng
rASiTxrXVya pakaSx, raMga; videVshiVyara valase \mo vugaLa \vi atiyAda parxtigAmi aBipArxyagaLanunx taLeda, iMgelxMDina oMdu saMsethx. 
\emng
\eentry

\bentry
\wordnospeech{National Government}{National Government}
\pron{?}
\gl{\nA}
\bmng
rASiTxrXVya sakARra; \eng{1931-35}ra avadhiyalilx iMgilxSf parxdhAni mAyxkoDxnAlfDx neVtaqtavxdalilx kelasa mADida elalx pakaSxgaLiMda kUDida okUkxTa sakARra. 
\emng
\eentry

\bentry
\wordnospeech{national grid}{national grid}
\pron{?}
\gl{\nA}
\bmng
(\birx) rASiTxrXVya jAla; rASiTxrXVya girxDf: 
\banum
\alnum{a} rASaTxrXdAdayxMta iruva parxdhAna viduyxcaCxkitx haMcike keVMdarxgaLanunx parasapxra baMdhisuva adhika voVleTxVjina viduyxtf taMtigaLa jAla. 
\alnum{b} birxTiSf divxVpagaLa nakAseyalilx baLasuva meTirxkf padadhxtiya BwgoVLika nideRVshAMkagaLu. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{National Guard}{National Guard}
\pron{?}
\gl{\nA}
\bmng
(\ame) rASiTxrXVya seVnAdaLa; BAgashaH saMsAthxnagaLa KaciRnalilx naDesikoMDu baruva, Adare agatayxbidAdxga pheDaralf sakARrada upayoVgakUkx sidadhxviruva pArxthamika miVsalu seYnayxdaLa. 
\emng
\eentry

\bentry
\wordnospeech{National Health Service}{National Health Service}
\pron{?}
\gl{\nA}
\bmng
(parxdhAnavAgi terigeya mUlaka nivaRhisuva) rASiTxrXVya -- AroVgayx seVve, veYdayxkiVya seVvAvayxvasethx. 
\emng
\eentry

\bentry
\wordnospeech{national holiday}{national holiday}
\pron{?}
\gl{\nA}
\bmng
(deVshAdayxMta Acarisuva) rASiTxrXVya rajAdina. 
\emng
\eentry

\bentry
\wordnospeech{national income}{national income}
\pron{?}
\gl{\nA}
\bmng
rASiTxrXVya -- AdAya, varamAna; iDiV deVshada oTuTx varamAna. 
\emng
\eentry

\bentry
\wordnospeech{National Insurance}{National Insurance}
\pron{?}
\gl{\nA}
\bmng
(\birx) rASiTxrXVya -- vime, vimAvayxvasethx; nivaqtatx vayasisxnavarigiMta keLagina elAlx vayasakxrU kAhile, nirudoyxVga, nivaqtitx, \mo vugaLigAgi sakARrada neravanunx paDeyalu kaDADxyavAgi koDabeVkAda, \eng{1948}ralilx jArige baMda, vaMtige vayxvasethx. 
\emng
\eentry

\bentry
\word{nationalise}
\pron{nAYxSanaleYsfZ}
\gl{\sakirx}
\bmng
= \hyperlink{nationalize}{nationalize}. 
\emng
\eentry

\bentry
\word{nationalism}
\pron{nAYxSanalisaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} rASiTxrXVyate; deVshiVyate; rASiTxrXVya BAvane, tatavxgaLu, \mo vu. 
\num{2} ugarx rASiTxrXVyate; atireVkada rASiTxrXVyate. 
\num{3} rASiTxrXVya sAvxtaMtarxyXda niVti. 
\enum
\emng
\eentry

\bentry
\word[nationalist(1)]{nationalist}
\pron{nAYxSanalisfTx}
\gl{\nA}
\bmng
\bnum
\num{1} rASiTxrXVyatAvAdi; deVshiVyatAvAdi. 
\num{2} rASaTxrXda sAvxtaMtarxyXvanunx samathiRsuvavanu. 
\num{3} ugarx rASiTxrXVyavAdi. 
\num{4} sAvxtaMtarxyX hoVrATagAra; deVshada sAvxtaMtarxyXkAkxgi hoVrADuvavanu. 
\enum
\emng
\eentry

\bentry
\word[nationalist(2)]{nationalist}
\pron{nAYxSanalisfTx}
\gl{\gu}
\bmng
= \hyperlink{nationalistic}{nationalistic}. 
\emng
\eentry

\bentry
\word{nationalistic}
\pron{nAYxSanalisiTxkf}
\gl{\gu}
\bmng
\bnum
\num{1} rASiTxrXVyatAvAdada; deVshiVyatAvAdakekx saMbaMdhisida. 
\num{2} rASaTxrXda sAvxtaMtarxyXvanunx samathiRsuva: \eng{nationalistic movements in Africa} Aphirxkadalilxna rASiTxrXVya sAvxtaMtarxyX caLuvaLigaLu. 
\num{3} ugarx rASiTxrXVyateya. 
\num{4} (deVshada) sAvxtaMtarxyX hoVrATakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{nationalistically}
\pron{nAYxSanalisiTxkali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} rASiTxrXVyatAvAdadaMte. 
\num{2} rASaTxrX sAvxtaMtarxyXvanunx samathiRsuvaMte. 
\num{3} ugarx rASiTxrXVyateyiMda. 
\num{4} (deVshada) sAvxtaMtarxyX hoVrATakekx saMbaMdhisidaMte. 
\enum
\emng
\eentry

\bentry
\word{nationality}
\pron{nAYxSanAYxliTi}
\gl{\nA}
\expl{(\bava\ \eng{nationalities}).}
\bmng
\bnum
\num{1} janAMgalakaSxNa; rASiTxrXVyate. 
\num{2} rASaTxrXperxVma; savxdeVshABimAna; deVsha perxVma. 
\num{3} (oMdu) janAMga; rASaTxrX; deVsha: \eng{men of all nationalities} elAlx janAMgagaLige seVridavaru. 
\num{4} janAMgiVyate; janAMgavAgiruvudu; (oMdu) rASaTxrXvAgiruvike. 
\num{5} (upa)janAMga; oMdu athavA hecicxna rAjakiVya rASaTxrXgaLa BAgavAgiruva oMdu vishiSaTx jAti, buDakaTuTx. 
\enum
\emng
\eentry

\bentry
\word{nationalization}
\pron{nAYxSanaleYseZVSanf}
\gl{\nA}
\bmng
rASiTxrXVkaraNa; (keYgArike \mo vanunx) rASaTxrXda savxtatxnAnxgi parivatiRsuvike. 
\emng
\eentry

\bentry
\word{nationalize}
\pron{nAYxSanaleYsfZ}
\gl{\sakirx}
\bmng
\bnum
\num{1} rASiTxrXVyavanAnxgi mADu; rASiTxrXVyavAgisu; deVshiVyagoLisu: \eng{he nationalized cosmopolitan traditions} vishavxsaMparxdAyagaLanunx avanu rASiTxrXVyagoLisida. 
\num{2} (oMdu) janAMgavanAnxgi mADu; (oMdu) rASaTxrXvanAnxgi mADu: \eng{the Poles were nationalized after the war} yudAdhxnaMtara poVlf janaru oMdu rASaTxrXvAgi saMGaTitarAdaru. 
\num{3} rASiTxrXVyananAnxgisu; (paradeVshadavananunx) I deVshada hakukx bAdhayxtegaLige ahaRnanAnxgi mADu; paradeVshiVyanige rASiTxrXVyate niVDu: \eng{nationalized Greeks} deVshada hakukx bAdhayxtegaLige ahaRte paDeda (videVshi) girxVkaru. 
\num{4} rASiTxrXVkarisu; rASiTxrXVkaraNa mADu; (jamiVnu, reYlu, keYgArike, \mo vanunx) rASaTxrXda savxtatxnAnxgi, rASiTxrXVya udayxmagaLanAnxgi parivatiRsu: \eng{nationalize the banks} bAyxMkugaLanunx rASiTxrXVkaraNa mADu. 
\enum
\emng
\eentry

\bentry
\word{nationalizer}
\pron{nAYxSanaleYsaZrf}
\gl{\nA}
\bmng
rASiTxrXVkaraNavAdi; rASiTxrXVkaraNa sidAdhxMtavanunx parxti pAdisuvavanu. 
\emng
\eentry

\bentry
\word{nationally}
\pron{nAYxSanali}
\gl{\kirxvi}
\bmng
rASiTxrXVyavAgi; deVshiVyavAgi. 
\emng
\eentry

\bentry
\wordnospeech{national newspaper}{national newspaper}
\pron{?}
\gl{\nA}
\bmng
rASiTxrXVya vAtAR patirxke; elelxDe parxcAravAguva sudidx patirxke. 
\emng
\eentry

\bentry
\wordnospeech{national park}{national park}
\pron{?}
\gl{\nA}
\bmng
rASiTxrXVya udAyxna(vana); rASoTxrXVdAyxna; sAvaRjanikara upayoVgakAkxgi kaTaTxDa \mo vanunx kaTuTxvudanunx niyaMtirxsuvudara mUlaka sakARrada neVtaqtavxdalilx saMrakiSxsalAda haLiLxgADu parxdeVsha. 
\emng
\eentry

\bentry
\wordnospeech{national service}{national service}
\pron{?}
\gl{\nA}
\bmng
(\birx) (\ca) rASiTxrXVya seVnA seVve; otAtxyavAgi seYnayxkekx seVri seVve salilxsabeVkAda vayxvasethx. 
\emng
\eentry

\bentry
\wordnospeech{National Socialism}{National Socialism}
\pron{?}
\gl{\nA}
\bmng
(\ca) rASiTxrXVya samAjavAda; jamaRniyalilx nAjigaLu purasakxrisida rASiTxrXVyate, parishudadhx janAMgiVyate, \mo\ tatatxvXgaLu. 
\emng
\eentry

\bentry
\wordnospeech{National Socialist}{National Socialist}
\pron{?}
\gl{\nA}
\bmng
(\ca) rASiTxrXVya samAjavAdi; \eng{1933-45}ra avadhiya jamaRniyalilx adhikAradalilxdadx, atuyxgarx rASiTxrXVyateyanunx samathiRsutitxdadx savARdhikAri pakaSxdavanu. 
\emng
\eentry

\bentry
\wordnospeech{National Trust}{National Trust}
\pron{?}
\gl{\nA}
\bmng
(birxTanf, AseTxrXVliya, \mo vugaLalilx) rASiTxrXVya TarxsfTx (saMsethx); perxVkaSxNiVya hAgU aitihAsika sathxLagaLanunx saMrakiSxsaloVsuga rUpugoMDa KAsagi saMsethx. 
\emng
\eentry

\bentry
\word{nationhood}
\pron{neVSanfhuDf}
\gl{\nA}
\bmng
rASaTxrXtavx; rASaTxrXte; rASaTxrXsAthxna. 
\emng
\eentry

\bentry
\word{nationwide}
\pron{neVSanfveYDf}
\gl{\gu}
\bmng
deVshAdayxMta; rASATxrXdayxMta. 
\emng
\eentry

\bentry
\word[native(1)]{native}
\pron{neVTivf}
\gl{\nA}
\bmng
\bnum
\num{1} deVshiVya; sathxLiVya; sathxLavaMdiga; (oMdu gotAtxda) sathxLadalilx huTiTxdavanu yA A sathxLadalilx nelasida taMde tAyigaLuLaLxvanu: \eng{native of Mysore} meYsUrinalilx huTiTxdava; meYsUriga. 
\num{2} nADiga; sathxLiVya; A Urinavanu. 
\num{3} (\AseTxrXV) AseTxrXVliyada biLiya; AseTxrXVliyadalilx huTiTxda biLiya janAMgadavanu, shevxVtavaNiRVya. 
\num{4} yUroVpiyaneVtara kuladavanu yA anAgarika janAMgadavanu. 
\num{5} (\da\ AphirXka) kariya; kariya janAMgadavanu. 
\num{6} sathxLiVya sasayx yA pArxNi; oMdu sathxLada yA deVshada pArxNi yA sasayx: \eng{the kangaroo is a native of Australia} kAMgarU AseTxrXVliyada pArxNi. 
\num{7} (\pArxvi) nADu siMpi; birxTiSf samudarxtiVradalilx kaqtaka maDigaLalilx beLesuva siMpi, AyisaTxrf miVnu. 
\enum
\emng
\eentry

\bentry
\word[native(2)]{native}
\pron{neVTivf}
\gl{\gu}
\bmng
\bnum
\num{1} (obabxnige, oMdu pArxNi \mo vugaLige) sAvxBAvikavAgi bMdiruva; savxBAvasidadhx; sahajavAda: \eng{native ability} sahaja sAmathayxR. 
\num{2} alaMkAravilalxda; saraLa; akaqtaka; neYjasavxBAvada: \eng{native charm} saraLa swMdayaR. 
\num{3} (obabxna) huTiTxna: \eng{native palce} huTiTxda sathxLa; janamx sathxLa. 
\num{4} huTiTxniMda baMda; janamxsidadhxvAda; Ajanamxsidadhx: jAyamAnada. 
\num{5} (loVha \mo vu) shudadhx rUpadalilx, asaMyukatxvAgi doreyuva: \eng{native gold} shudadhxrUpada baMgAra. 
\num{6} (\kanmu\ yUroVpiyaneVtarara \vi) deVshiVya; sathxLavaMdiga; horaginiMda baMdu nelasidavaradalalxda: \eng{native customs} deVshiVya saMparxdAyagaLu. 
\num{7} (oMdu) sathxLadalilx huTiTxdavara; deVshiVya; A sathxLadavara yA avarige saMbaMdhisida. 
\num{8} (\AseTxrXV\ matutx nUyxsiZVlaMDf) sadaqsha; inonxMdu kaDe suparicitavAda pArxNi yA sasayxvanunx hoVluva: \eng{native rabbit} mola sadaqsha. 
\num{9} sathxLiVya; oMdu nidiRSaTx sathxLakekx seVrida: \eng{ant-eater is native to South America} iruvebAkavu dakiSxNa amerikakekx seVridudx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
 \eng{go native} sathxLiVyanAgu; deVshiVyanAgu: 
\banum
\alnum{a} (valasigara \vi, \kanmu\ yUroVpeVtara deVshadalilx) deVshiVya jiVvanariVtiyanunx anusarisu; sathxLiVyanAgu; sathxLavaMdigara rUDhi, saMparxdAyagaLanunx sivxVkarisu. 
\alnum{b} (biLiyana \vi) tAnu vAsisuva sathxLada janara (anAgarika) jiVvana mAgaR anusarisu. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{native bear}{native bear}
\pron{?}
\gl{\nA}
\bmng
(\AseTxrXV\ matutx nUyxsiZVlaMDf) = \hyperref{kandict_k.pdf}{K}{koala}{koala}. 
\emng
\eentry

\bentry
\word{natively}
\pron{neVTivfli}
\gl{\kirxvi}
\bmng
sahajavAgi; sAvxBAvikavAgi; huTiTxniMda; janamxtaH. 
\emng
\eentry

\bentry
\word{nativeness}
\pron{neVTivfnisf}
\gl{\nA}
\bmng
sahajate; sAvxBAvikate. 
\emng
\eentry

\bentry
\wordnospeech{native rock}{native rock}
\pron{?}
\gl{\nA}
\bmng
sathxLiVya shile; (adara) mUlasAthxnadalilxruva shile. 
\emng
\eentry

\bentry
\word{nativism}
\pron{neVTivisaZmf}
\gl{\nA}
\bmng
(\tashA) sahaja jAcnxna sidAdhxMta; manasusx jAcnxnada mUlAMshagaLanunx bAhayx saMveVdaneyiMda paDedukoLaLxde savxtaH sahajavAgi paDediruvudeMba vAda, sidAdhxMta. 
\emng
\eentry

\bentry
\word{nativist}
\pron{neVTivisfTx}
\gl{\nA}
\bmng
sahaja jAcnxna sidAdhxMti. 
\emng
\eentry

\bentry
\word{nativity}
\pron{naTiviTi}
\gl{\nA}
\expl{(\bava\ \eng{nativities}).}
\bmng
\bnum
\num{1} yeVsukirxsatxna, vajiRnf meVriya yA saMta jAnfna huTuTx, janana. 
\num{2} yeVsukirxsatxna jananada citarx. 
\num{3} yeVsukirxsatxna huTiTxda hababx yA kirxsfmasf. 
\num{4} vajiRnf meVriya (sepeTxMbarf \eng{8}) yA saMta jAnfna (jUnf \eng{24}) huTiTxda hababx. 
\num{5} huTuTx; janana. 
\num{6} (vayxkitxya) jAtaka; janamxkuMDali. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{Nativity} = \hyperlink{nativity}{nativity} \((1 \& 3)\). 
\emng
\eentry


\bentry
\wordnospeech{nativity play}{nativity play}
\pron{?}
\gl{\nA}
\bmng
kirxsAtxvatAra nATaka; yeVsu -- janamxda, jananada nATaka; yeVsu kirxsatxna janamxkatheyanunx rUpisuva, makakxLa nATaka. 
\emng
\eentry

\bentry
\word{NATO}
\pron{neVToV}
\gl{\saMkiSx}
\bmng
(\eng{Nato} saha) \eng{North Atlantic Treaty Organization.} 
\emng
\eentry

\bentry
\word{natron}
\pron{neVTarxnf}
\gl{\nA}
\bmng
(\ravi) neVTarxnf; parxkaqtiyalilx doreyuva jalasaMyoVjita soVDiyamf kAboRneVTu, \eng{$\bg\rm Na\eg_2\bg\rm CO\eg_3 \bg 10\rm H\eg_2\bg\rm O\eg$.} 
\emng
\eentry

\bentry
\word{NATSOPA}
\pron{nAYxTosxVpa}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Society of Operative Printers, Graphical and Media Personnel.} 
\emng
\eentry

\bentry
\word[natter(1)]{natter}
\pron{nAyxTarf}
\gl{\akirx}
\bmng
(\AmA) 
\bnum
\num{1} vaTaguTuTx; vaTavaTane mAtanADu. 
\num{2} goNaguTuTx; (siDiguTuTxtatx) tananxSaTxkekx tAneV mAtanADiko: \eng{what is she nattering about now?} avaLu Iga Enanunx kuritu goNaguTuTxtitxdAdxLe? 
\enum
\emng
\eentry

\bentry
\word[natter(2)]{natter}
\pron{nAYxTarf}
\gl{\nA}
\bmng
\bnum
\num{1} athaRvilalxda haraTe; kADu haraTe; goDuDx haraTe. 
\num{2} goNagATa. 
\enum
\emng
\eentry

\bentry
\word{natterer}
\pron{nAYxTararf}
\gl{\nA}
\bmng
\bnum
\num{1} vaTaguTuTxvava. 
\num{2} goNaguTuTxvava. 
\enum
\emng
\eentry

\bentry
\word{natterjack}
\pron{nAYxTarfjAYxkf}
\gl{\nA}
\bmng
nAYxTarfjAYxkf; beninxna meVle haLadi paTeTxyuLaLx, kupapxLisade ODuva, pashicxma yUroVpina nelagapepx. 
\emng
\eentry

\bentry
\wordRemoveSpace{nattier-blue}{nattier blue}
\pron{nAYxTiarf bUlx}
\gl{\nA}
\bmng
mirugada, maMdavAda -- niVli CAye. 
\emng
\eentry

\bentry
\word{nattily}
\pron{nAYxTili}
\gl{\kirxvi}
\bmng
\bnum
\num{1} niVTAgi; ThiVkAgi; nAjUkiniMda. 
\num{2} keYcaLakadiMda. 
\num{3} caturateyiMda; kushalavAgi. 
\enum
\emng
\eentry

\bentry
\word{nattiness}
\pron{nAYxTinisf}
\gl{\nA}
\bmng
\bnum
\num{1} niVTu; nAjUkutana; ThiVkAgiruvike. 
\num{2} keYcaLaka. 
\num{3} cAtuyaR; kwshala. 
\enum
\emng
\eentry

\bentry
\word{natty}
\pron{nAYxTi}
\gl{\gu}
\expl{(\tara\ \eng{nattier,} \tama\ \eng{nattiest}).}
\bmng
\bnum
\num{1} (uDupina \vi) niVTAda; ThiVkAda; nAjUkAda: \eng{new and natty uniforms} hosa, niVTAda samavasatxrXgaLu. 
\num{2} keYcaLakavuLaLx. 
\num{3} catura; kushala. 
\enum
\emng
\eentry

\bentry
\word[natural(1)]{natural}
\pron{nAYxcaralf}
\gl{\gu}
\bmng
\bnum
\num{1} savxBAvasidadhx; neYja; neYsagiRka: \eng{natural law} neYsagiRka niyama. 
\num{2} parxkaqtisidadhx. 
\numi{3} (\saM) sahaja; sAvxBAvika: 
\banum
\alnum{a} (savxrada \vi) tAravAgali maMdarxvAgali Agirada. 
\alnum{b} (rAgaparxsAtxrada \vi) tAra savxragaLAgali maMdarx savxragaLAgali ilalxdiruva. 
\eanum
\numie
\num{4} rUDhiya; sAvxBAvikavAda; sAmAnayx vayxvahArada. 
\num{5} pArxkaqtika; neYsagiRka; sAvxBAvika; nAgarikateya parxBAvavilalxda yA dhamaR saMsAkxra \mo vugaLa soVMkilalxda. 
\num{6} BwtikavAgi asitxtavxdalilxruva; pAramAthiRkavalalxda yA bwdidhxkavalalxda yA kAlapxnikavalalxda. 
\num{7} Bwtika vasutxgaLige saMbaMdhisida: \eng{the natural world} Bwtika parxpaMca. 
\num{8} nisagaRdalilx beLediruva; sahaja rUpadalilx, sithxtiyalilx iruva; kaqtakavalalxda. 
\num{9} (sasayxgaLa \vi) tAneV tAnAgi huTiTxda; savxyaMjAta; vayxvasAyadiMda beLesirada. 
\num{10} (jamiVnina \vi) beVsAya mADirada; vayxvasAya mADilalxda. 
\num{11} savxBAvasidadhx; aMtagaRta; aMtaBURta. 
\num{12} sahaja; jiVvaMtavAgiruvaMte toVruva. 
\num{13} sahaja; saraLa; naTaneyalalxda; kaqtakavalalxda; kaqtirxmavalalxda: \eng{natural voice} sahaja dhavxni. \eng{a piece of natural acting} sahaja aBinaya. 
\num{14} sahaja; sAvxBAvika; AshacxyaRkaravalalxda; niriVkiSxsabahudAdaMtha: \eng{it is natural for a bird to fly} hakikx hAruvudu sahajaveV, AshacxyaRveVnalalx; hAruvudu hakikxya sahaja guNa. 
\num{15} virUpagoLisada; rUpa mArevADada. 
\num{16} sahaja; parxkaqtisidadhx; huTiTxniMda baruva; sulaBavAgi baruva; tAneV tAnAgi odaguva: \eng{he is a natural orator} avanobabx huTuTx vAgimx. \eng{comes natural to him} adu avanige sahajavAgi barutatxde. 
\num{17} sahaja; sAvxBAvika; savxBAvadiMda aMtiruva yA aMtAguva; hAgirabeVkeMdu huTuTxvAgaleV parxkaqti nidhaRrisidaMte toVruva: \eng{natural enemies} huTuTx hagegaLu. \eng{the natural antithesis} sahaja viroVdha. 
\num{18} keVvala pArxkaqta, deYhika saMbaMdhada; vivAhabAhayx; vivAheVtara saMbaMdhadalilx huTiTxda: \eng{natural son} vivAheVtara saMbaMdhada maga. 
\num{19} kAnUnu bAhayx; hAdarada: \eng{a natural child} jAraja shishu; hAdarada magu. 
\num{20} nisagARdhayxyanada; parxkaqtiyanunx kuritu adhayxyana mADuva. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{one's natural life} obabxna (aihika) Ayasusx; AyuHparxmANa. 
\num{2} \eng{the natural man} sahaja mAnava; nAgarikate, dhamaRsaMsAkxra, \mo vugaLa soVMkilalxda saraLa manuSayx. 
\enum
\emng
\eentry

\bentry
\word[natural(2)]{natural}
\pron{nAYxcaralf}
\gl{\nA}
\bmng
\bnum
\num{1} (huTiTxdaMdiniMda) aremaruLa. 
\num{2} takakxnAdavanu; yoVgayx; ahaR; (yAvudoV vishiSaTx pAtarxkekx yA viSayadalilx) sahaja sAmathayxRviruvavanu, kwshalaviruvavanu; \eng{he is a natural for the job} A kelasakekx takakxnAdavanu. 
\num{3} savxBAvataH (yAvudoV kelasa, udedxVsha, \mo vugaLige) takakxnAda vasutx; niNARyakavAda vasutx. 
\num{4} (\saM) (ucacx hAgU maMdarx savxragaLiMda hiMdakekx baMdAgina) sAbxBAvika savxrada cihenx. 
\num{5} (piyAnoV vAdayxdalilxna) biLiya kiVlikeY yA otutx biraDe. 
\num{6} (\saM) (tAravAgali maMdarxvAgali Agirada) sahaja savxra. 
\hyperdef{N}{natural(2)7}{} 
\numi{7} (isipxVTATadalilx) 
\banum
\alnum{a} pAMTUnf \eng{(pontoon)} eMba isipxVTATadalilx ele haMcidAga modalu \eng{21} aMka gaLisuva ATagAra. 
\alnum{b} (kArxyXpfsx ATadalilx dALagaLiMda) ELu yA hanonxMdara -- gara uruLisuvudu, hAkuvudu, eseyuvudu. 
\eanum
\numie
\num{8} maMkAda nasu haLadi mishirxta kaMdu(baNaNx). 
\enum
\emng
\eentry

\bentry
\word{natural-born}
\pron{nAYxcaralf bAnfR}
\gl{\gu}
\bmng
huTiTxniMda baMda(lakaSxNa, sAthxnagaLuLaLx); Ajanamx. 
\emng
\eentry

\bentry
\wordnospeech{natural childbirth}{natural childbirth}
\pron{?}
\gl{\nA}
\bmng
(\veYshA) sahaja janana; sahaja herige; kaniSaThx veYdayxkiVya yA tAMtirxka sahAyadiMda Aguva herige; shasatxrXkirxye yA arivaLikagaLa neravilalxde Aguva herige. 
\emng
\eentry

\bentry
\wordnospeech{natural classification}{natural classification}
\pron{?}
\gl{\nA}
\bmng
(\jiVvi) sAvxBAvika vagiRVkaraNa; sAvxBAvika lakaSxNagaLa hAgU hoVlikegaLa AdhArada meVle mADuva jiVvigaLa vagiRVkaraNa. 
\emng
\eentry

\bentry
\word{natural-coloured}
\pron{nAYxcaralf kalaDfR}
\gl{\gu}
\bmng
sahaja vaNaRda; kaqtakavAgi baNaNx \mo vanunx leVpisada. 
\emng
\eentry

\bentry
\wordnospeech{natural death}{natural death}
\pron{?}
\gl{\nA}
\bmng
sahajamaraNa; sAvxBAvika sAvu; apaGAta, viSa, hiMse, \mo vugaLu kAraNavAgirade mupipxniMda athavA roVgadiMda uMTAguva sAvu. 
\emng
\eentry

\bentry
\wordnospeech{natural food}{natural food}
\pron{?}
\gl{\nA}
\bmng
sAvxBAvika AhAra; rAsAyanikagaLanunx yA saMrakaSxkagaLanunx baLasi rakiSxsadiruva AhAra. 
\emng
\eentry

\bentry
\wordnospeech{natural gas}{natural gas}
\pron{?}
\gl{\nA}
\bmng
nisagARnila; BUmiyalilx, \sA\ peTorxVliyamf jatege doreyuva, iMdhanavAgi baLasuva, hagura heYDorxkAbaRnfgaLa misharxNa. 
\emng
\eentry

\bentry
\wordnospeech{natural habitat}{natural habitat}
\pron{?}
\gl{\nA}
\bmng
sAvxBAvika AvAsa, vAsasAthxna; yAvudeV sasayx yA pArxNi sAvxBAvikavAgi vAsisuva sathxLa, parxdeVsha yA deVsha. 
\emng
\eentry

\bentry
\wordnospeech{natural historian}{natural historian}
\pron{?}
\gl{\nA}
\bmng
parxkaqti caritekAra; parxkaqtiyalilx sikukxva, \kanmu\ sasayx matutx pArxNigaLa jiVvanavanunx kuritu bareyuvava yA avugaLa viSayadalilx tajacnx. 
\emng
\eentry

\bentry
\wordnospeech{natural history}{natural history}
\pron{?}
\gl{\nA}
\bmng
parxkaqti cariterx: 
\banum
\alnum{a} parxkaqtiyalilxruva \kanmu\ sasayx matutx pArxNi jiVvanada adhayxyana yA (\kanmu\ janasAmAnayxrigAgi) adara nirUpaNe. 
\alnum{b} oMdu parxdeVshadalilxna neYsagiRka vasutxgaLa, sasayxgaLa, pArxNigaLa athavA avugaLa lakaSxNagaLa vaqtAtxMta: \eng{natural history of Karnataka} kanARTakada parxkaqti cariterx. 
\eanum
\emng
\eentry

\bentry
\word{naturalise}
\pron{nAYxcaraleYszf}
\gl{\kirx}
\bmng
= \hyperlink{naturalize}{naturalize}.
\emng
\eentry

\bentry
\word{naturalism}
\pron{nAYxcaralisaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} sahaja kirxye; sAvxBAvika kirxye; sahaja parxvaqtitxgaLiMda parxcoVditavAda kirxye; sahaja parxvaqtitxgaLiMda parxcoVditavAda kirxye, kAyaR. 
\hypertarget{naturalism(2)}{} 
\num{2} pArxkaqtika dhamaR; neYsagiRka dhamaR; sahaja dhamaR; keVvala parxkaqtiyanunx, nisagaRvanunx Adharisiruva niVti shAsatxrX yA matadhamaR. 
\numi{3} (\tashA) parxkaqti vAda; nisagaR vAda; savxBAva vAda; BwtavAda; 
\banum
\alnum{a} jagatitxna saqSiTxge matutx sithxtige yAvudeV nisagARtiVta yA AdhAyxtimxka kAraNa matutx shakitx beVkAgilalx, adu savxtaMtarx savxyaMBu savxyaMpUNaR matutx savxyaM niyaMtarxka, adu EkameVvAdivxtiVya satayx, mAnava adara oMdu aMga -- eMba sidAdhxMta; jagatatxnunx kuritaMte yAvudeV atimAnuSa yA AdhAyxtimxka parigaNaneyanunx nirAkarisuva daqSiTx, vAda; keVvala neYsagiRka kAraNagaLu matutx niyamagaLu jagatitxnalilx elalxvanUnx vivarisabalalxveMba vAda. 
\alnum{b} elalx neYtika niNaRyagaLU vasutxniSaThx vaNaRnegaLu matutx avanunx pArxyoVgikavAgi pariVkiSxsabahudu eMba vAda. 
\alnum{c} (\deVva) matadhamaRda elalx tatatxvXgaLU nisagaRda kirxyegaLa adhayxyanadiMdaleV anugatavAgutatxve, avakekx deVva parxkAshana beVkAgilalx eMba vAda. 
\eanum
\numie
\num{4} (sAhitayx, kale) yathAthaR citarxNa; yathAthaRvAda; vAsatxvikate; vAsatxvika badukanunx AdashiRVkarisade, adu iruvaMteyeV pUNaR vivaragaLoMdige vasutxniSaThxvAgi, neYtika niNaRyagaLa soVMkilalxde, yathAvatAtxgi citirxsuva, vaNiRsuva vidhAna yA adanunx kurita sidAdhxMta. 
\num{5} sahaja vataRne; sAvxBAvika parxvaqtitx; saMparxdAyagaLa bagegx alakaSxyX, upeVkeSx. 
\enum
\emng
\eentry

\bentry
\word[naturalist(1)]{naturalist}
\pron{nAYxcaralisfTx}
\gl{\nA}
\bmng
\bnum
\num{1} parxkaqti tatatx vxvAdi; nisagaRvAdi; nisagaRvAdadalilx naMbikeyuLaLxvanu yA adara anuyAyi. 
\num{2} parxkaqtishAsatxrXjacnx; nisagARdhAyxyi; sasayx vijAcnxni yA pArxNi vijAcnxni; sasayx matutx pArxNigaLanunx kuritu adhayxyana mADuvavanu. 
\num{3} (sAhitayx, kale) yathAthaRvAdi; vAsatxvikatAvAdi. 
\enum
\emng
\eentry

\bentry
\word[naturalist(2)]{naturalist}
\pron{nAYxcaralisfTx}
\gl{\gu}
\bmng
\bnum
\num{1} sAvxBAvikatege, parxkaqtitatatx vxkekx -- anusAravAda. 
\num{2} parxkaqti cariterxya. 
\num{3} (sAhitayx, kale) yathAthaR -- sheYliya, vidhAnada; vAsatxvika riVtiya. 
\enum
\emng
\eentry

\bentry
\word{naturalistic}
\pron{nAYxcaralisiTxkf}
\gl{\gu}
\bmng
= \hyperlink{naturalist(2)}{$^2$naturalist}. 
\emng
\eentry

\bentry
\word{naturalistically}
\pron{nAYxcaralisiTxkali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} sAvxBAvikateganusAravAgi. 
\num{2} parxkaqti cariterxyananxnusarisi. 
\num{3} (sAhitayx, kale) vAsatxvika(tA) sheYliyalilx. 
\enum
\emng
\eentry

\bentry
\word{naturalization}
\pron{nAyxcaraleYsezVSanf}
\gl{\nA}
\bmng
\bnum
\num{1} deVshiVkaraNa; rASiTxrXVyatA parxdAna yA sivxVkAra; paradeVshadavanige parxjAhakukx koDuvudu yA paradeVshadalilx parxjAhakukx paDeyuvudu: \eng{naturalization papers} (paradeVshadalilx) rASiTxrXVyatA patarxgaLu; parxjAhakukx paDeyalu neravAguva kAgadapatarxgaLu. 
\num{2} (hosa parisarakekx) paLaguvike yA paLagisuvike. 
\enum
\emng
\eentry

\bentry
\word{naturalize}
\pron{nAYxcaraleYsazf}
\gl{\sakirx}
\bmng
\bnum
\num{1} deVshiVkarisu; deVshiVyananAnxgisu; rASiTxrXVyananAnxgisu; (paradeVshadavanige) parxjA hakakxnunx koDu: \eng{naturalize immigrants into the USA} valasegArarige amerika (saMyukatx saMsAthxna)dalilx parxjAhakukx koDu. 
\num{2} (paradeVshada pada, padadhxti, \mo vanunx) rUDhisiko; aLavaDisiko; sivxVkarisu; baLasiko; \eng{English sporting terms have been naturalized in many languages} kirxVDege saMbaMdhisida iMgilxSf padagaLanunx halavAru BASegaLu aLavaDisikoMDive. 
\num{3} (pArxNi, sasayx, \mo vanunx) deVshiVyavAgisu; matotxMdu deVshadalilx rUDhisu; paLagisu; hosa parisarakekx paLagisu. 
\num{4} saMparxdAya \mo vugaLiMda mukatxgoLisu; saMparxdAyabadadhxteyanunx tapipxsu; sAvxBAvikagoLisu. 
\num{5} pavADagaLiMda dUravirisu; parxkaqtitatatx vxdanusAra vivarisu. 
\num{6} sahajavAgi, sAvxBAvikavAgi toVruvaMte mADu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} paLagu; baLakege baru; deVshiVyavAgu. 
\num{2} parxkaqtishAsatxrXvanunx adhayxyana mADu; parxkaqtishAsatxrXdalilx saMshoVdhaneyanunx mADu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{natural key}{natural key}
\pron{?}
\gl{\nA}
\bmng
(\saM) sahaja parxsAtxra. 
\emng
\eentry

\bentry
\wordnospeech{natural language}{natural language}
\pron{?}
\gl{\nA}
\bmng
sahaja BASe; sAvxBAvikavAgi huTiTx beLedu baMda, kaqtakavAgi racisirada BASe (`kaqtaka BASe' yA `saMkeVta BASe' eMbudakekx virudadhx shabadx). 
\emng
\eentry

\bentry
\wordnospeech{natural law}{natural law}
\pron{?}
\gl{\nA}
\bmng
parxkaqti niyama; parxkaqti sUtarx; neYsagiRka niyama; nisagaR vidhi: 
\banum
\alnum{a} (\tashA) mAnavasavxBAva hoMdiruva kAraNadiMda elalxrigU samAnavAgiruva, shAshavxta neYtika vidhi yA sUtarx. 
\alnum{b} niyata karxma; nidiRSaTx saMgatigaLigU naDuve iruva pUvARpara anukarxmada yathAvatAtxda nirUpaNe, vaNaRne, varadi. 
\alnum{c} parxkaqti vayxvasethx; nisagaRdalilx kaMDubaruva niyatavAda karxma, vayxvasethx: \eng{where they saw chance, we see natural law} avaru Akasimxkavanunx kaMDidedxDe, nAvu parxkaqtiya niyamavanunx, vayxvasethxyanunx noVDutetxVve. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{natural life}{natural life}
\pron{?}
\gl{\nA}
\bmng
obabxna jiVvanada AyuSayx. 
\emng
\eentry

\bentry
\word{naturally}
\pron{nAYxcarali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} neYsagiRkavAgi; pArxkaqtikavAgi; parxkaqtisidadhxvAgi. 
\num{2} sahajavAgi; sAvxBAvikavAgi; akaqtirxmavAgi; akaqtakavAgi: \eng{speak and behave naturally} sahajavAda riVtiyalilx mAtanADu, naDeduko. 
\num{3} niriVkeSxyaMte; niriVkiSxsalAguvaMte; sahajavAgi; saMdeVhavilalxdaMte: \eng{did you answer her letter? naturally}! niVnu avaLa kAgadakekx javAbu koTeTxyA? (niriVkiSxsalAguvaMte) hwdu! sahajavAgiyeV! adaralelxVnu saMdeVha! 
\num{4} savxBAvataH; huTiTxniMdaleV: \eng{she is naturally musical} avaLu savxBAvata: saMgiVtapirxyaLu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{natural magic}{natural magic}
\pron{?}
\gl{\nA}
\bmng
pArxkaqtika mATa; neYsagiRka mATa; yAvudeV vayxkitxgaLa perxVtagaLanunx biTuTx amUtaR ceVtanagaLanunx ArAdhisi naDesuva mATa, aBicAra. 
\emng
\eentry

\bentry
\word{naturalness}
\pron{nAYxcaralfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} sahajate; sAvxBAvikate. 
\num{2} neYsagiRkate. 
\enum
\emng
\eentry

\bentry
\wordnospeech{natural numbers}{natural numbers}
\pron{?}
\gl{\nA}
\bmng
sAvxBAvika saMKeyxgaLu; eNisalu baLasuva \eng{1, 2, 3,} itAyxdi saMKeyxgaLu. 
\emng
\eentry

\bentry
\wordnospeech{natural philosopher}{natural philosopher}
\pron{?}
\gl{\nA}
\bmng
(\pArxparx) Bwta vijAcnxni; BwtashAsatxrXjacnx. 
\emng
\eentry

\bentry
\wordnospeech{natural philosophy}{natural philosophy}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} (\pArxparx) Bwta vijAcnxna. 
\num{2} \sA\ Bwta matutx rasAyana vijAcnxnagaLu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{natural religion}{natural religion}
\pron{?}
\gl{\nA}
\bmng
pArxkaqta dhamaR; neYsagiRka dhamaR; (alwkika divayxdashaRnavananxlalxde) manuSayxna veYcArikate hAgU anuBavagaLanunx Adharisida dhamaR. 
\emng
\eentry

\bentry
\wordnospeech{natural resources}{natural resources}
\pron{?}
\gl{\nA}
\bmng
neYsagiRka saMpanUmxlagaLu; keYgArikoVdayxmadalilx baLasalAguva, manuSayxna keYvADavilalxde dorakuva niVru, Kanija nikeSxVpagaLu, araNoyxVtapxnanxgaLu, itAyxdi. 
\emng
\eentry

\bentry
\wordnospeech{natural science}{natural science}
\pron{?}
\gl{\nA}
\bmng
nisagaR vijAcnxna; viSayaniSeThxyiMda avaloVkisi, aLedu aBAyxsa mADabahudAda vasutx, shakitx, avugaLa rUpAMtaragaLige saMbaMdhisida BwtavijAcnxna, rasAyana vijAcnxna, BUvijAcnxna, jiVvavijAcnxna yA sasayxvijAcnxna. 
\emng
\eentry

\bentry
\wordnospeech{natural selection}{natural selection}
\pron{?}
\gl{\nA}
\bmng
(\jiVvi) neYsagiRka Ayekx; nisagaRdalilx jiVvigaLa saMtAna beLeyutatx hoVdaMte parisarakekx hoMdikoLaLxbalalx lakaSxNagaLiruva jiVvigaLu cenAnxgi bALi, avugaLa saMtAna hulusAgi beLeyuva matutx parisarakekx hoMdikoLaLxlAgada jiVvigaLU avugaLa saMtAnavU karxmeVNa nashisi hoVguva neYsagiRka parxkirxye. 
\emng
\eentry

\bentry
\wordnospeech{natural theology}{natural theology}
\pron{?}
\gl{\nA}
\bmng
pArxkaqta deVvatA shAsatxrX; nisagaRdatatx takaRshakitxyiMda paDeda BagavaMtana viSayada jAcnxna. 
\emng
\eentry

\bentry
\wordnospeech{natural uranium}{natural uranium}
\pron{?}
\gl{\nA}
\bmng
neYsagiRka yureVniyamf; nisagaRdalilx doreyuva yureVniyamfnalilx adara beVre beVre aisoToVpugaLa parasapxra parxmANa heVgiruvudoV hAgeV iruva, kaqtakavAgi adanunx badalAyisadiruva yureVniyamf. 
\emng
\eentry

\bentry
\wordnospeech{natural virtues}{natural virtues}
\pron{?}
\gl{\nA}
\bmng
(\tashA) sahaja guNagaLu; nAyxya, viveVka, saMyama, dheYyaR. 
\emng
\eentry

\bentry
\wordnospeech{natural year}{natural year}
\pron{?}
\gl{\nA}
\bmng
sahaja vaSaR; pArxkaqtika vaSaR; neYsagiRkAyana; sUyaRna sutatx BUmiyu oMdu sututx sutatxlu tegedukoLuLxva kAla, \eng{365} divasagaLu, \eng{5} gaMTegaLu matutx \eng{48} nimiSagaLu. 
\emng
\eentry

\bentry
\word{nature}
\pron{neVcarf}
\gl{\nA}
\bmng
\bnum
\num{1} (asitxtavxkekx AvashayxkavAda) vasutxvina sahaja -- guNa, savxrUpa, lakaSxNa, dhamaR. 
\num{2} (aneVka veVLe \eng{Nature)}(manuSayxna, pArxNiya) parxkaqti; savxBAva: \eng{it is not in her nature to act like that} A riVti naDeyuvudu avaLa savxBAvavalalx. 
\num{3} manuSayx jAtiya savxBAva, lakaSxNa matutx BAvanegaLu. 
\num{4} manuSayx savxBAvada vishiSaTx aMsha: \eng{the rational nature} veYcArika savxBAva. \eng{animal nature} pashu (pArxyavAda) savxBAva. \eng{moral nature} neYtika savxBAva. 
\num{5} vishiSaTx savxBAvavuLaLx vayxkitx: \eng{sanguine natures do not feel this}... AshAvAdi savxBAvadavarige, AshAvAdigaLige idara arivu iruvudilalx. 
\num{6} tera; bage; riVti; vagaR: \eng{things of this nature of a request} idu koVrikeya riVtiyadu. 
\num{7} (manuSayxna shiVlavanunx, kirxyeyanunx nidhaRrisuva AMtarika) parxvaqtitxgaLu; perxVraNegaLu. 
\num{8} parxkaqti; nisagaR; jaDajagatitxna GaTanegaLige kAraNavAda Bwtashakitx. 
\num{9} parxkaqti vAyxpAra; (samaSiTxyAgi) nisagaRkirxye(gaLu). 
\num{10} parxkaqti (deVvi); mUtiVRkaqta nisagaR (shakitxgaLu): \eng{all nature looks gay} parxkaqtiyelalxvU naliyutitxde. \eng{Nature is the best physician} parxkaqti atuyxtatxma veYdayx. 
\num{11} sasayx, pArxNi, BUdaqshayx, \mo\ nisagaRda aMshagaLu: \eng{nature gives him comfort} nisagaR avanige nemamxdi koDutatxde. 
\num{12} kaqSi mADada yA kADu parxdeVsha. 
\num{13} asaMsakx qta sithxti, samAja, \mo vu. 
\num{14} (\kanmu\ kaNiNxge AkaSaRkavAda) gArxmiVNa, gArxmAMtara -- parxdeVsha; haLiLxgADu. 
\num{15} (vayxkitxtavxnunx parxBAvisuva yA nidhaRrisuva aMshavAgi) AnuvaMshiVyate \eng{(nurture}ge virudadhxvAgi). 
\num{16} jiVviyoMdara jeYvika caTuvaTike yA jiVvanAvashayxkategaLu: \eng{such a diet does not support nature} aMtha AhAra jiVvapoVSaNege sAladu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{against nature} savxBAvavirudadhxvAda; asAvxBAvika; aneYtika. 
\numi{2} \eng{against} (\engit{or} \eng{contrary to) nature} 
\banum
\alnum{a} pavADa sadaqsha. 
\alnum{b} pavADasadaqshavAgi. 
\alnum{c} \eng{back to nature} maraLi parxkaqtige; sahaja pArxkaqta enanxbahudAda jiVvanasithxtige hiMdiruguvike. 
\eanum
\numie
\num{4} \eng{by nature} savxBAvataH 
\num{5} \hyperref{kandict_e.pdf}{E}{ease(2) nuga(1)}{$^2$ease} \eng{nature.} 
\num{6} \eng{from nature} (kale \mo vugaLa \vi) parxkaqtiya, nisagaRda vasutxgaLanunx mAdariyAgi baLasikoLuLxvudu: \eng{paint from nature} parxkaqtiyanunx anusarisi citirxsu. 
\num{7} \eng{human nature} mAnava savxBAva; manuSayxra sAmAnayx lakaSxNa, BAvagaLu, \mo vu. 
\hyperdef{N}{nature pagu(8)}{} 
\numi{8} \eng{in a state of nature} 
\banum
\alnum{a} pArxkaqta sithxti \eng{(state of grace} eMbudakekx virudadhxvAda padagucacx). 
\alnum{b} (samAjavu vayxvasethxgoLuvudakikxMta pUvaRda) manuSayxna anAgarika sithxti, asaMsakx qta sithxti. 
\alnum{c} (pArxNigaLa, sasayxgaLa \vi) paLagisilalxda, rUDhisilalxda -- sithxti. 
\alnum{d} batatxleya sithxti. 
\eanum
\numie
\numi{9} \eng{in nature} 
\banum
\alnum{a} vAsatxvavAgi (asitxtavxdalilx) iruva. 
\alnum{b} elilx beVkAdarU; EnAdarU; yAvudeV riVtiyalilx. 
\hypertarget{naturepagu10}{} 
\eanum
\numie
\num{10} \eng{in} (\engit{or} \eng{by} \engit{or} \eng{from) the very nature of the case} tatavxshaH; vasutxtaH. 
\num{11} \eng{in} (\engit{or} \eng{by} \engit{or} \eng{from) the very nature of things} = \hyperlink{naturepagu10}{?pagu?\((10)\)}. 
\num{12} \eng{in} \engit{or} \eng{of the nature of} A vagaRdaMtha yA A vagaRkekx seVrida: \eng{in the nature of an excuse} nepada riVtiya. 
\num{13} \eng{in the} \hyperref{kandict_c.pdf}{C}{course(1) pagu(5)}{$^1$course} \eng{of nature}. 
\num{14} \eng{law of nature} = \hyperlink{natural law}{natural law}.
\num{15} \eng{one of Nature's} oMdu vagaR yA samudAyada atayxMta sahaja parxtinidhiyAda: \eng{one of Nature's gentlemen} sahaja sajajxna; huTuTx saBayx; parxkaqtiyalilx janamxtALida obabx sahaja sajajxna vayxkitx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nature cure}{nature cure}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{naturopathy}{naturopathy}.
\emng
\eentry

\bentry
\word{natured}
\pron{neVcaDfR}
\gl{\gu}
\bmng
savxBAvada; savxBAvavuLaLx: \eng{good nature} oLeLxya savxBAvada. \eng{ill natured} keTaTx savxBAvada. 
\emng
\eentry

\bentry
\word{nature-printing}
\pron{neVcarfpirxMTiMgf}
\gl{\nA}
\bmng
parxkaqti mudarxNa; takakxMte saMsakxrisida Palakada meVle ele \mo vanunx otitx citarx tegeyuva padadhxti. 
\emng
\eentry

\bentry
\wordnospeech{nature reserve}{nature reserve}
\pron{?}
\gl{\nA}
\bmng
miVsalu, rakiSxta -- BUmi; sasayx, pArxNi, meVlemxYlakaSxNa, \mo vugaLanunx uLisuvaMte noVDikoMDa BUparxdeVsha. 
\emng
\eentry

\bentry
\wordnospeech{nature study}{nature study}
\pron{?}
\gl{\nA}
\bmng
parxkaqtipATha; (pAThashAleya paThayx viSayavAgi) (parxtayxkaSx pariVkeSxyiMda) sasayx, pArxNi, \mo\ parxkaqti goVcara viSaya \mo vanunx kuritu vAyxsaMga mADuvudu. 
\emng
\eentry

\bentry
\wordnospeech{nature trail}{nature trail}
\pron{?}
\gl{\nA}
\bmng
parxkaqtijADu; kADu \mo\ parxdeVshagaLalilx parxkaqtiyalilxna sAvxrasayxkara vasutxgaLatatx gamana seLeyaleMdu nimiRsida mAgaRsUcaka kaMbagaLuLaLx hAdi, jADu, mAgaR. 
\emng
\eentry

\bentry
\wordnospeech{nature worship}{nature worship}
\pron{?}
\gl{\nA}
\bmng
parxkaqtipUje; nisagaRdalilxna giDa, mara, gALi, niVru, \mo vanunx deqvashakitxgaLeMdu BAvisi pUjisuva mata, dhamaR, \mo vu. 
\emng
\eentry

\bentry
\word{naturism}
\pron{neVcarisaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} = \hyperlink{naturalism(2)}{naturalism\((2)\)}. 
\num{2} parxkaqti pUje; neYsagiRka vasutxgaLa pUje. 
\num{3} = \hyperlink{nudism}{nudism}. 
\enum
\emng
\eentry

\bentry
\word{naturist}
\pron{neVcarisfTx}
\gl{\nA}
\bmng
= \hyperlink{naturalist(1)}{naturalist}. 
\emng
\eentry

\bentry
\word{naturopath}
\pron{neVcarapAYxtf}
\gl{\nA}
\bmng
parxkaqti cikitasxka; parxkaqti veqdayx; parxkaqti cikitesx niVDuvava(Lu). 
\emng
\eentry

\bentry
\word{naturopathic}
\pron{neVcarapAYxtikf}
\gl{\gu}
\bmng
parxkaqti cikitAsx vidhAnada. 
\emng
\eentry

\bentry
\word{naturopathy}
\pron{neVcarApati}
\gl{\nA}
\bmng
parxkaqti cikitesx; roVga tAnAgi sahajavAgi guNavAgalu neravu niVDuva, auSadhagaLanunx baLasada, \sA\ AhAra, vAyxyAma, \mo vugaLanunx oLagoMDa cikitAsx padadhxti. 
\emng
\eentry

\bentry
\word[naught(1)]{naught}
\pron{nATf}
\gl{\nA}
\bmng
(\pArxparx\ yA sAhitayxka) ilalxme; sonenx; shUnayx (=\hyperlink{nought}{nought}). 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{bring to naught} ilalxvAgisu; hALumADu; BaganxgoLisu. 
\num{2} \eng{come to naught} ilalxdAgu; hALAgu; vayxthaRvAgu; BaganxvAgu. 
\num{3} \eng{set at naught} kaDegaNisu; nirAkarisu; nilaRkiSxsu. 
\enum
\emng
\eentry

\bentry
\word[naught(2)]{naught}
\pron{nATf}
\gl{\gu}
\bmng
niSaTxrXyoVjaka; vayxthaR; niSaPxla; kelasakekx bArada. 
\emng
\eentry

\bentry
\word{naughtily}
\pron{nATili}
\gl{\kirxvi}
\bmng
\bnum
\num{1} tuMTatanadiMda haThamAritanadiMda; avidheVyavAgi. 
\num{2} (\pArxparx) keTaTxtanadiMda; duSaTxtanadiMda. 
\num{3} (\AmA) ayoVgayxvAgi; ashilxVlavAgi. 
\enum
\emng
\eentry

\bentry
\word{naughtiness}
\pron{nATinisf}
\gl{\nA}
\bmng
\bnum
\num{1} tuMTatana; haThamAritana; avidheVyate. 
\num{2} (\pArxparx) keTaTxtana; duSaTxtana. 
\num{3} ayoVgayxte; asaBayxte. 
\num{4} (\AmA) ashilxVlate; poVkaritana. 
\enum
\emng
\eentry

\bentry
\word{naughty}
\pron{nATi}
\gl{\gu}
\expl{(\tara\ \eng{naughtier,} \tama\ \eng{naughtiest}).}
\bmng
\bnum
\num{1} (makakxLa \vi) tuMTa; tuMTatanada; haThamAritanada; avidheVya; ashikiSxta; heVLidaMte keVLada: \eng{a naughty child} tuMTa magu. 
\num{2} \pArxparx\ keTaTx; duSaTax. 
\num{3} ayoVgayx; asaBayx. 
\num{4} \AmA\ \hA\ ashilxVla; poVkari; poVli: \eng{naughty stories} poVli kathegaLu. 
\enum
\emng
\eentry

\bentry
\word{nauplius}
\pron{nApilxasf}
\gl{\nA}
\expl{(\bava\ \eng{nauplii} \ucAcx-- nApilxai).}
\bmng
\pArxvi\ nApilxyasf; tatitxyoDedu horabaMdiruva cipupxjiVviya lAvaR. 
\emng
\eentry

\bentry
\word{nausea}
\pron{nAsi(siZ)a}
\gl{\nA}
\bmng
\bnum
\num{1} Okarike; pitotxVderxVka; ubabxLike; hoTeTx toLasuvike; hoTeTx toLasi vAkarike baruvaMtAguvudu; samudarx pitatx. 
\num{2} asahayxvAguvike; jugupesx; heVvarike: \eng{filled with nausea at the sight of cruelty to animals} pArxNihiMse mADuvudanunx noVDi asahayx huTiTx. 
\enum
\emng
\eentry

\bentry
\word{nauseate}
\pron{nAsiz(si)ETf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (ananx \mo vanunx) heVvarisu (\rUpa\ saha). 
\num{2} OkarisuvaMte mADu; pitotxVderxVkabarisu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} (ananx \mo vu seVrade) asahayxvAgu; OkarisuvaMtAgu. 
\num{2} (AhAra, kasubu, \mo vu) beVDavenisu; heVsikeyAgu. 
\enum
\emng
\eentry

\bentry
\word{nauseating}
\pron{nAsiz(si)ETiMgf}
\gl{\gu}
\bmng
\bnum
\num{1} heVvarike baruvaMtha; heVsikeyAguvaMtha: \eng{a nauseating sight} heVsikeyAguvaMtha daqshayx. 
\num{2} Okarike barisuva; hoTeTx toLasuva. 
\enum
\emng
\eentry

\bentry
\word{nauseatingly}
\pron{nAsiZ(si)ETiMgfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} heVvarike baruvaMte; heVsikeyAguva riVtiyalilx. 
\num{2} Okarike barisuvaMte; hoTeTx toLasuvaMte. 
\enum
\emng
\eentry

\bentry
\word{nauseous}
\pron{nAsiZ(si)asf}
\gl{\gu}
\bmng
\bnum
\num{1} jugupesx huTiTxsuva; asahayxvAda. 
\num{2} Okarike barisuva; hoTeTx toLasuvaMtha. 
\num{3} gabubxnAta hoDeyuva; asahayxvAda. 
\num{4} heVvarike barisuva; asahayx huTiTxsuva; heVsikeya; biVBatasxkara. 
\enum
\emng
\eentry

\bentry
\word{nauseously}
\pron{nAsiZ(si)asfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} jugupesx huTiTxsuvaMte; asahayxvAgi. 
\num{2} Okarike barisuvaMte; hoTeTx toLasuva hAge. 
\num{3} gabubxvAsaneyiMda kUDi. 
\num{4} heVvarike barisuva riVtiyalilx; biVBatasxriVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{nauseousness}
\pron{nAsiZ(si)asfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} biVBatasxte; jugupesx barisuvaMtiruvudu. 
\num{2} asahayxvAgiruvike; Okarike barisuvaMtiruvudu. 
\enum
\emng
\eentry

\bentry
\word{nautch}
\pron{nAcf}
\gl{\nA}
\bmng
sULeyara kuNita; tApe. 
\emng
\eentry

\bentry
\word{nautch-girl}
\pron{nAcfgalfR}
\gl{\nA}
\bmng
naTuvagititx; kuNitada sULe; nAyakasAni. 
\emng
\eentry

\bentry
\word{nautical}
\pron{nATikalf}
\gl{\gu}
\bmng
\bnum
\num{1} nAvikara. 
\num{2} samudarxyAnada; haDagu naDesuvudara. 
\num{3} nwkeya; nAveya; nAvege saMbaMdhisida. 
\num{4} kaDalasaMbaMdhavAda; sAmudarx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nautical almanac}{nautical almanac}
\pron{?}
\gl{\nA}
\bmng
nAvika paMcAMga; KagoVLa kAyagaLa sAthxnagaLu; samudarxda ubabxraviLitagaLu, \mo\ nAvikarige beVkAguva mAhitiyanonxLagoMDa vASiRka parxkaTaNe. 
\emng
\eentry

\bentry
\word{nautically}
\pron{nATikali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} nAvikana viSayavAgi. 
\num{2} samudarxyAnakekx saMbaMdhisidaMte. 
\num{3} nwkasaMbaMdhavAgi. 
\num{4} kaDalige saMbaMdhisida hAge. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nautical mile}{nautical mile}
\pron{?}
\gl{\nA}
\bmng
nAvika meYli; nAvikaru baLasuva dUrada mAna, \su\ \eng{2025} gajagaLu \eng{(1852} miV.). 
\emng
\eentry

\bentry
\word{nautilus}
\pron{nATilasf}
\gl{\nA}
\expl{(\bava\ \eng{nautiluses}, yA \eng{nautili} \ucAcx-- nATileY)}
\bmng
(\pArxvi) nATilasf: 
\hypertarget{nautilus(a)}{} 
\banum
\alnum{a} dakiSxNa pesiphikf matutx hiMdU mahAsAgaragaLalilx doreyuva, mutitxnaMtha oLameYya matutx aDaDx viBAgagaLiruva suruLiyAkArada cipipxnalilx vAsisuva shiVSaRpadi maqdavxMgigaLa oMdu kula. \imglink{nautilus a.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.5cm {N_Pictures/nautilus.jpg}}} 
\hyperdef{N}{nautilus(b)}{} 
\hypertarget{nautilus(b)}{} 
\alnum{b} AgoRnATa kulada, heNuNxgaLige navurAda kAgadadaMtha cipupx matutx haDagina paTadaMtha jAlabAhugaLiruva, oMdu shiVSaRpadi. 
\eanum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{paper nautilus} = \hyperlink{nautilus(b)}{nautilus (b)}. 
\num{2} \eng{pearly nautilus} = \hyperlink{nautilus(a)}{nautilus (a)}. 
\enum
\emng
\eentry

\bentry
\word{Navaho}
\pron{nAYxvahoV}
\gl{\nA}
\bmng
= \hyperlink{Navajo}{Navajo}.
\emng
\eentry

\bentry
\word{Navajo}
\pron{nAYxvahoV}
\gl{\nA}
\expl{(\bava\ \eng{Navajos).}}
\bmng
nAYxvahoV: 
\banum
\alnum{a} (nUyx mekisxko matutx AYxriseZ\char'261Vna parxdeVshadalilxna) reDf iMDiyanf buDakaTiTxnavanu. 
\alnum{b} A buDakaTiTxnavara BASe. 
\eanum
\emng
\eentry

\bentry
\word{naval}
\pron{neVvalf}
\gl{\gu}
\bmng
\bnum
\num{1} haDagina; nAveya; nwkeya. 
\num{2} nwkAdaLada; nAvika. 
\num{3} nwkegaLige, haDagugaLige saMbaMdhisida; \eng{ a naval battle} nwkAyudadhx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{naval academy}{naval academy}
\pron{?}
\gl{\nA}
\bmng
(nwkAdaLada adhikArigaLa) nwkA tarabeVti saMsethx; nwkA shikaSxNa keVMdarx. 
\emng
\eentry

\bentry
\wordnospeech{naval architect}{naval architect}
\pron{?}
\gl{\nA}
\bmng
nwkAshilipx; haDagu kaTuTxvavanu, racisuvavanu. 
\emng
\eentry

\bentry
\wordnospeech{naval architecture}{naval architecture}
\pron{?}
\gl{\nA}
\bmng
nwkAshilapx; haDagu vinAyxsa mADuvudu. 
\emng
\eentry

\bentry
\word{navally}
\pron{neVvali}
\gl{\kirxvi}
\bmng
nwkege saMbaMdhisidaMte; haDagina viSayavAgi. 
\emng
\eentry

\bentry
\wordnospeech{naval officer}{naval officer}
\pron{?}
\gl{\nA}
\bmng
nwkAdhikAri; nwkAdaLada adhikAri. 
\emng
\eentry

\bentry
\wordnospeech{naval stores}{naval stores}
\pron{?}
\gl{\nA}
\bmng
nwkAsAmagirx; haDagu kaTaTxlu baLasuva padAthaRgaLu, vasutxgaLu. 
\emng
\eentry

\bentry
\word{navarin}
\pron{nAYxvarAYxknf}
\gl{\nA}
\bmng
(shAKa niroVdhaka pAterxyalilx mucicx, beVyisida) tarakAri seVrisi mADida ADina yA kuriya mAMsada BakaSx yx. 
\emng
\eentry

\bentry
\word[nave(1)]{nave}
\pron{neVvf}
\gl{\nA}
\bmng
guMba; cakarxnABi; acicxna arekAlugaLanunx hiDidukoMDiruva cakarxda madhayxBAga. 
\emng
\eentry

\bentry
\word[nave(2)]{nave}
\pron{neVvf}
\gl{\nA}
\bmng
neVvf; kaMbasAliniMda parxteyxVkagoMDa, ikekxlada ONigaLanunx biTuTx, caciRna pashicxma bAgiliniMda pAdirxgaLu yA gAyakaru kUruva sathxLadavaregina madhAyxMgaNa. 
\emng
\eentry

\bentry
\word{navel}
\pron{neVvalf}
\gl{\nA}
\bmng
\bnum
\num{1} hokakxLu; nABi; BUrxNakekx poVSaNe odagisuva nALa uduri hoVda pariNAmavAgi, hoTeTxya muMBAgadalilxruva kuLi. 
\num{2} (yAvudAdarU vasutxvina) madhayxbiMdu; keVMdarx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{contemplate one's navel} shAMtacitatxdiMda dhAyxnadalilx toDagu. 
\emng
\eentry

\bentry
\wordnospeech{navel orange}{navel orange}
\pron{?}
\gl{\nA}
\bmng
hokakxLu kitatxLe; toTiTxdadx sathxLadalilx kuLi iruva oMdu bageya biVjarahita kitatxLe haNuNx. 
\emng
\eentry

\bentry
\wordnospeech{navel string}{navel string}
\pron{?}
\gl{\nA}
\bmng
hokakxLu baLiLx; nABinALa. 
\emng
\eentry

\bentry
\word{navelwort}
\pron{neVvalfvaTfR}
\gl{\nA}
\bmng
neVvalf vaTfR; heYDorxkoTeYlf athavA seMTelAkulada, duMDAda ele iruva sasayx. 
\emng
\eentry

\bentry
\word[navicular(1)]{navicular}
\pron{navikuyxlarf}
\gl{\gu}
\bmng
doVNiyAkArada. 
\emng
\eentry

\bentry
\word[navicular(2)]{navicular}
\pron{navikuyxlarf}
\gl{\nA}
\bmng
\bnum
\num{1} (aMgeYya yA \sA\ aMgAlina) dorxVNAsithx; doVNiyAkArada mULe. 
\hypertarget{navicular(2)2}{} 
\num{2} dorxVNAsithxroVga; kuduregaLanunx kuMTuvaMte mADuva, avugaLa kAlina doVNi mULeya uriyUta 
\enum
\emng
\eentry

\bentry
\wordnospeech{navicular bone}{navicular bone}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{navicular(2)}{$^2$navicular\((1)\)}. 
\emng
\eentry

\bentry
\wordnospeech{navicular disease}{navicular disease}
\pron{?}
\gl{\nA}
\bmng
\hyperlink{navicular(2)2}{$^2$navicular\((2)\)}. 
\emng
\eentry

\bentry
\word{navigability}
\pron{nAYxvigabiliTi}
\gl{\nA}
\bmng
\bnum
\num{1} (nadi, samudarx, \mo vu) nwkAyAnakekx yoVgayxvAgiruvudu; haDagina saMcArakekx takukxdAgiruva sithxti. 
\num{2} (haDagina \vi) samudarxyAna yoVgayxvAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{navigable}
\pron{nAYxvigabflf}
\gl{\gu}
\bmng
\bnum
\num{1} (nadi, samudarx, \mo vugaLa \vi) haDagina saMcArakekx yoVgayxvAda; nwkAsaMcArayoVgayx: \eng{in navigable condition} nwkAsaMcArayoVgayx sithxtiyalilx. 
\num{2} (AkAshabuTiTxya \vi) naDesalAguva; naDesalu sAdhayxvAda. 
\num{3} (haDagina \vi) samudarxyAnakekx yoVgayxvAda. 
\enum
\emng
\eentry

\bentry
\word{navigate}
\pron{nAYxvigeVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (samudarx, nadi, \mo vugaLa meVle) haDaginalilx ODADu, saMcarisu. 
\num{2} (nwkeya yA vimAnada) saMcAravanunx nideRVshisu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} samudarxyAna mADu; haDagu naDesu. 
\num{2} (vAhanadalilxya parxyANikana \vi) cAlakanige sariyAda dAri toVrisi sahAyamADu; cAlakanige mAgaRdashaRnada neravu niVDu. 
\enum
\emng
\eentry

\bentry
\word{navigation}
\pron{nAYxvigeVSanf}
\gl{\nA}
\bmng
\bnum
\num{1} samudarxyAna. 
\num{2} nwkAyAnashAsatxrX; nwkAgati shAsatxrX; reVKAgaNita hAgU nwkA KagoVLa shAsatxrXda sahAyadiMda haDagina sAthxna matutx mAgaRgaLanunx nidhaRrisuva karxmagaLu. 
\num{3} samudarxyAna; samudarx parxyANa. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{inland navigation} (nadi yA kAluvegaLa mUlaka naDesuva) oLanADa jalasaMcAra; aMtadeRVshiVya jalayAna. 
\emng
\eentry

\bentry
\word{navigational}
\pron{nAYxvigeVSanalf}
\gl{\gu}
\bmng
samudarxyAnada yA samudarxyAnadalilx baLasuva. 
\emng
\eentry

\bentry
\word{navigation-coal}
\pron{nAYxvigeVSanf koVlf}
\gl{\nA}
\bmng
Avi kalilxdadxlu; Aviya kudipAterxgaLa keLage urisalu yoVgayxvAda kalilxdadxlu. 
\emng
\eentry

\bentry
\wordnospeech{navigation light}{navigation light}
\pron{?}
\gl{\nA}
\bmng
yAna diVpa; rAtirxyalilx haDagu yA vimAna saMcarisuvAga adara sAthxna matutx gamanamAgaRvanunx sUcisuva adara meVliruva diVpa. 
\emng
\eentry

\bentry
\word{navigator}
\pron{nAYxvigeVTarf}
\gl{\nA}
\bmng
\bnum
\num{1} nAvika; samudarxyAna kushala. 
\num{2} samudarx saMcAra mADi hosa parxdeVsha \mo vugaLa saMshoVdhane naDesuvavanu. 
\num{3} (\birx) (\pArxparx) = \hyperlink{navvy(1)}{$^1$navvy\((1)\)}. 
\enum
\emng
\eentry

\bentry
\word[navvy(1)]{navvy}
\pron{nAYxvi}
\gl{\nA}
\expl{(\bava\ \eng{navvies)}.}
\bmng
\bnum
\num{1} (kAluve rasetx, reYlu rasetx, \mo vugaLa) toVDugelasada kUliyALu. 
\num{2} toVDu yaMtarx. 
\enum
\emng
\eentry

\bentry
\word[navvy(2)]{navvy}
\pron{nAYxvi}
\gl{\akirx}
\expl{{(\birx)} (vataRmAna \parxpu\ \Eva\ \eng{navvies}, \BU\ matutx}
\BUkaq\ \eng{navvied}.)\bmng
(kAluve, rasetx, \mo vugaLa) toVDugelasada kUliyALAgi kelasamADu. 
\emng
\eentry

\bentry
\word{navy}
\pron{neVvi}
\gl{\nA}
\expl{(\bava\ \eng{navies}). }
\bmng
\bnum
\num{1} haDagu paDe; nwkAtaMDa; nwkApaDe; kalAsigaLu matutx avara poVSaka sibabxMdi \mo varanenxlalx oLagoMDa, oMdu rASaTxrXda yudadhxnwkegaLa samagarx taMDa. 
\num{2} nwkAdaLa; nwkAdaLada adhikArigaLu matutx seYnikaru. 
\num{3} (\kAparx) haDagu sAlu; nwkApaMkitx. 
\num{4} = \hyperlink{navy blue}{navy blue}. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{the Navy} = \hyperlink{navvy(1)}{$^1$navvy} \((1 \& 2)\). 
\emng
\eentry

\bentry
\wordnospeech{navy blue}{navy blue}
\pron{?}
\gl{\nA}
\bmng
(nwkAbaladavara uDupina) gADha niVlivaNaR; nwkAniVli. 
\emng
\eentry

\bentry
\word{navy-cut}
\pron{neVvikaTf}
\gl{\nA}
\bmng
teLuvAgi siVLida bilelx hogesopupx. 
\emng
\eentry

\bentry
\wordnospeech{Navy Department}{Navy Department}
\pron{?}
\gl{\nA}
\bmng
(\ame) nwkA ilAKe; nwkApaDeya viSayavanunx noVDikoLuLxva sakARri viBAga. 
\emng
\eentry

\bentry
\wordnospeech{Navy List}{Navy List}
\pron{?}
\gl{\nA}
\bmng
(\birx) nwkA(viSaya)sUci; elalx nwkA adhikArigaLa hesaranUnx itara viSayagaLanUnx tiLisuva sakARri pusatxka. 
\emng
\eentry

\bentry
\wordnospeech{navy yard}{navy yard}
\pron{?}
\gl{\nA}
\bmng
(\ame) seqnikaralalxda kUli sibabxMdiyuLaLx sakARri haDaginaMgaLa, haDagu kaTuTxva sathxLa. 
\emng
\eentry

\bentry
\word{nawab}
\pron{navAbf}
\gl{\nA}
\bmng
navAba: 
\banum
\alnum{a} \ca\ (musilxM) deVshiVya pArxMtAdhikAri, shirxVmaMta. 
\alnum{b} A padaviyiMda baMda birudu. 
\alnum{c} pAkisAtxnadalilx gaNayx musilxM vayxkitx. 
\eanum
\emng
\eentry

\bentry
\word[nay(1)]{nay}
\pron{neV}
\gl{\kirxvi}
\bmng
\bnum
\num{1} (\pArxparx) = \hyperlink{no(4)}{$^4$no}. 
\num{2} (matotxbabxna heVLikege asamamxti sUcisuvalilx) (\pArxparx) Eke? oLeLxyadu! 
\num{3} alalxde; matutx; adakikxMta hecAcxgi: \eng{weighty, nay, unanswerable} tUkavAdadudx, aSeTxV alalx, parxti heVLalAgadaMthadu. 
\enum
\emng
\eentry

\bentry
\word[nay(2)]{nay}
\pron{neV}
\gl{\nA}
\bmng
\bnum
\num{1} `ilalx', eMba shabadx; (nakArAtamxka utatxra heVLuvalilx) `ilalx', `Agadu', `kUDadu', eMba shabadx itAyxdi: \eng{will not take nay} ``Agadu" enunxvudanunx avanu opupxvudilalx, sivxVkarisuvudilalx. 
\num{2} virudadhx mata, OTu: \eng{counted 16 nays} hadinAru virudadhx matagaLanunx eNisida. 
\enum
\emng

\noindent
\gl{\pagu}
{\eng{say nay} `Agadu', `ilalx' inunxvudu; `uhuM' eMdu heVLuvudu: \eng{cannot say him nay} avanige Agadu enanxlAgadu.}\eentry

\bentry
\word{naysay}
\pron{neVseV}
\gl{\kirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{naysays,} \BU\ matutx \BUkaq\ \eng{naysaid}).}
\bmng
 (\kanmu\ \ame) {\sakirx} nirAkarisu; alalxgaLe. 
\emng

\noindent
\gl{\akirx}
\bmng
ilalxvenunx; `ilalx' eMdu heVLu. 
\emng
\eentry

\bentry
\word[Nazarene(1)]{Nazarene}
\pron{nAYxsaZriVnf}
\gl{\nA}
\bmng
\bnum
\num{1} `nAyxsazratf' eMba Urinavanu (\kanmu\ Esukirxsatx). 
\num{2} (yehUdayx matutx musilxM parxyoVgadalilx) kerxYsatx; kirxshicxyanf dhamaRkekx seVridavanu 
\num{3} AdikAlada yehUdayx kerxYsatx paMgaDadavanu yA ipapxtatxneya shatamAnada pArxTeseTxMTf paMgaDadavanu. 
\enum
\emng
\eentry

\bentry
\word[Nazarene(2)]{Nazarene}
\pron{nAyxsaZriVnf}
\gl{\gu}
\bmng
\bnum
\num{1} `nAyxsaratf'na yA kerxYsatxna. 
\num{2} kirxshicxyanf; kerxYsatx dhamaRdavana. 
\num{3} AdikAlada yehUdayx kerxYsatx paMgaDada yA ipapxtatxneya shatamAnada pArxTeseTxMTf paMgaDada. 
\enum
\emng
\eentry

\bentry
\word[Nazarite(1)]{Nazarite}
\pron{nAYxsazreYTf}
\gl{\nA}
\bmng
`nAyxsaratf' eMba Urinavanu; nAyxsaratitxnavanu. 
\emng
\eentry

\bentry
\word[Nazarite(2)]{Nazarite}
\pron{nAYxsazreYTf}
\gl{\nA}
\bmng
(\ca) kelavu BoVga vajaRna varxtagaLanunx hiDida yehUdayx. 
\emng
\eentry

\bentry
\word{naze}
\pron{neVsaZf}
\gl{\nA}
\bmng
BUshira; BUkaMTha. 
\emng
\eentry

\bentry
\word[Nazi(1)]{Nazi}
\pron{nATisx, nAsiZ}
\gl{\nA}
\expl{(\bava\ \eng{Nazis}).}
\bmng
\bnum
\num{1} nAji; jamaRniya nAyxSanalf soVSiyalisfTx pATiRya sadasayx. 
\num{2} janAMgaBeVda niVtiyalilx naMbikeyuLaLx yAvudeV aMtha saMsethxya sadasayx. 
\enum
\emng
\eentry

\bentry
\word[Nazi(2)]{Nazi}
\pron{nATisx, nAsiZ}
\gl{\gu}
\bmng
\bnum
\num{1} nAji; nAji saMsethxya. 
\num{2} janAMgaBeVda niVtiyalilx naMbikeyiruva yA aMtha saMsethxya. 
\enum
\emng
\eentry

\bentry
\word{Nazidom}
\pron{nATisx(siZ)Damf}
\gl{\nA}
\bmng
nAjivagaR; nAjigaLu. 
\emng
\eentry

\bentry
\word{Nazify}
\pron{nATisx(siZ)pheY}
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{Nazifies,} \BU\ matutx \BUkaq\ \eng{Nazified}).}
\bmng
 nAjiVkarisu: nAjiyAgisu; nAji niVtige aLavaDisu. 
\emng
\eentry

\bentry
\word{Naziism}
\pron{nATisxV(siZV)isaZmf}
\gl{\nA}
\bmng
= \hyperlink{Nazism}{Nazism}. 
\emng
\eentry

\bentry
\word{Nazirite}
\pron{nAyxsaZreYTf}
\gl{\nA}
\bmng
= \hyperlink{Nazarite(2)}{$^2$Nazarite} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{Nazism}
\pron{nATisxsaZmf}
\gl{\nA}
\bmng
nAjitatatxvX; nAjiniVti; jamaRniya nASanalisfTx soVSiyalisfTx pATiRya janAMgaBeVdaniVti. 
\emng
\eentry

\bentry
\wordnospeech{NB}{NB}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} (\ame) \eng{Nebraska.} 
\num{2} \eng{New Brunswick.} 
\num{3} \eng{no ball.} 
\num{4} \eng{Scotland (North Britain).} 
\num{5} \engit{nota bene.} 
\enum
\emng
\eentry

\bentry
\wordnospeech{Nb}{Nb}
\pron{?}
\gl{\saMkeV}
\bmng
(\ravi) \eng{niobium}(dhAtu). 
\emng
\eentry

\bentry
\wordnospeech{NBC}{NBC}
\pron{?}
\gl{\saMkiSx}
\bmng
(\ame dalilx) \eng{National Broadcasting Company.} 
\emng
\eentry

\bentry
\wordnospeech{N. by E.}{N. by E.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{North by East} 
\emng
\eentry

\bentry
\wordnospeech{N. by W.}{N. by W.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{North by West} 
\emng
\eentry

\bentry
\wordnospeech{NC}{NC}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{North Carolina} 
\emng
\eentry

\bentry
\wordnospeech{NCB}{NCB}
\pron{?}
\gl{\saMkiSx}
\bmng
(\ca) (\birx) \eng{National Coal Board (1987}riMda \eng{British Coal} eMdu kareyalAgutitxde). 
\emng
\eentry

\bentry
\wordnospeech{NCO}{NCO}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{non-commissioned officer.} 
\emng
\eentry

\bentry
\wordnospeech{NCU}{NCU}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Communications Union.} 
\emng
\eentry

\bentry
\wordnospeech{ND}{ND}
\pron{?}
\gl{\saMkiSx}
\bmng
(\ame) \eng{North Dakota.} 
\emng
\eentry

\bentry
\wordnospeech{Nd}{Nd}
\pron{?}
\gl{\saMkeV}
\bmng
(\ravi) \eng{neodymium}(dhAtu). 
\emng
\eentry

\bentry
\wordnospeech{n.d.}{n.d.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{no date}. 
\emng
\eentry


\bentry
\wordwithhyphen{hyp-nd}{-nd}
\pron{}
\gl{\uparx}
\bmng
nAmapada matutx guNavAcakagaLa racaneyalilx \parx: \eng{dividend, reverend.} 
\emng
\eentry

\bentry
\wordnospeech{N. Dak.}{N. Dak.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{North Dakota.} 
\emng
\eentry

\bentry
\wordnospeech{NE}{NE}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{north-east} 
\num{2} \eng{north-eastern.} 
\enum
\emng
\eentry

\bentry
\wordnospeech{Ne}{Ne}
\pron{?}
\gl{\saMkeV}
\bmng
(\ravi) \eng{neon} (dhAtu). 
\emng
\eentry

\bentry
\word{ne}
\pron{neV}
\gl{\gu}
\bmng
huTiTxda; janamxtALida; (obabx vayxkitxya hiMdina hesaranunx Igina hesarina jate seVrisi heVLuvAga) \udA\ \eng{Lord Beaconsfield, ne Benjamin Disraeli} lADfRbiVkanfsx PiZVlfDx, huTiTxniMda beMjaminf DiserxVli. 
\emng
\eentry

\bentry
\word{Neanderthal}
\pron{niAYxMDaTARlf}
\gl{\gu}
\bmng
niyAMDaTARlf; pArxciVna shilAyugada yUroVpfnalilx jiVvisidadx, dapapxnAda hubibxna ENu matutx hiMdakekx Oluva haNe uLaLx, niyAMDaTARlf mAnavana yA avanige saMbaMdhisida. 
\emng
\eentry

\bentry
\word[neap(1)]{neap}
\pron{niVpf}
\gl{\gu}
\bmng
\bnum
\num{1} (Baratada \vi) atayxMta keLamaTaTxda ubabxrada. 
\num{2} cAMdarxmAnada modalaneya hAgU mUraneya pAdagaLAda kUDaleV samudarxda ubabxravu atayxMta keLamaTaTxdalilxruva. 
\enum
\emng
\eentry

\bentry
\word[neap(2)]{neap}
\pron{niVpf}
\gl{\nA}
\bmng
atayxMta keLamaTaTxda ubabxra (\da\ Barata). 
\emng
\eentry

\bentry
\word[neap(3)]{neap}
\pron{niVpf}
\gl{\sakirx}
\bmng
(\kaparx dalilx)(haDagina \vi) samudarxda iLitadiMdAgi, muMde hoVgalAgade taLahatutx yA baMdaru \mo\ kaDe taDehoMdu, niMtubiDu, sathxgitavAgu. 
\emng
\eentry

\bentry
\word[Neapolitan(1)]{Neapolitan}
\pron{niapAliTanf}
\gl{\gu}
\bmng
(iTaliyalilxna) neVpalisxna. 
\emng
\eentry

\bentry
\word[Neapolitan(2)]{Neapolitan}
\pron{niapAliTanf}
\gl{\nA}
\bmng
(iTaliya) neVpalfsx nagaradavanu; neVpalfsx parxje, nivAsi. 
\emng
\eentry

\bentry
\wordnospeech{Neapolitan ice-cream}{Neapolitan ice-cream}
\pron{?}
\gl{\nA}
\bmng
neVpalfsx aisfkirxVM; beVre beVre baNaNxda hAgU suvAsaneya padara yA reVkugaLiruva aisfkirxVM. 
\emng
\eentry

\bentry
\wordnospeech{Neapolitan violet}{Neapolitan violet}
\pron{?}
\gl{\nA}
\bmng
neVpalfsx vayoleTf suvAsaneyuLaLx `vayoleTf' jAtiya oMdu sutitxna hUvu. 
\emng
\eentry

\bentry
\wordnospeech{neap tide}{neap tide}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{neap(2)}{$^2$neap}. 
\emng
\eentry

\bentry
\word[near(1)]{near}
\pron{niarf}
\gl{\kirxvi}
\bmng
\bnum
\num{1} (sathxLadalAlxgali, kAladalAlxgali) samiVpadalilx; samiVpavAgi; hatitxravAgi; hatitxradalilx: \eng{the station is quite near} nilAdxNa bahaLa samiVpadalilxde. 
\num{2} (\pArxparx) bahumaTiTxge; inunx savxlapxdaralilx: \eng{he was as near as could be to being knocked down by a bus} savxlapxdaralilx basusx avana meVle haridu hoVguvudaralilxtutx. 
\num{3} hatitxra hatitxra: \eng{as near as one can guess, there were forty people} hatitxra hatitxra alilx nalavatutx janaridadxru. 
\num{4} (\pArxparx) bahaLa lekAkxcAravAgi; jipuNatanadiMda: \eng{he lives very near} avanu bahaLa jipuNatanadiMda jiVvana naDesutAtxne. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{far and near} elalx kaDe; elelxlUlx. 
\num{2} \eng{go near to do} (\engit{or} \eng{come} \engit{or} \eng{go near doing) (something)} inenxVnu mADuvudaralilxru. 
\numi{3} \eng{near at hand} 
\banum
\alnum{a} sulaBavAgi keYge sikukxvaMte; hatitxradalelxV: \eng{always have a dictionary near at hand} yAvAgalu shabadxkoVshavanunx hatitxraveV iTuTxkoMDiru. 
\alnum{b} (kAlada \vi) hatitxradalelxV; bahaLa dUravAgirade; anatikAladalelxV: \eng{the examinations are near at hand} pariVkeSxgaLu hatitxravAgalive. 
\eanum
\numie
\num{4} \eng{near by} hatitxradalelxV; anati dUradalelxV. 
\num{5} \eng{near upon} (\pArxparx) iSaTxralelxV; anatikAladalelxV: \eng{it was near upon 2 o'clock} inenxVnu eraDu gaMTeyAguvudaralilxtutx. 
\num{6} \eng{nowhere near} hatitxraveVnU Agirade; aSeTxVnU hatitxravalalxde; dUradalilx (Aguva saMBavavilalx eMbathaRdalilx): \eng{the cinema was nowhere near full} calanacitarx maMdira aSeTxVnU BatiRyAgiralilalx. 
\enum
\emng
\eentry

\bentry
\word[near(2)]{near}
\pron{niarf}
\gl{\upa}
\bmng
\bnum
\num{1} (deVsha, kAla, sithxti yA hoVlikeyalilx, yAvudoV oMdara yA oMdakekx) hatitxra; anatidUra; samiVpa: \eng{the sun is near setting} sUyaR muLuguvudaralilxdAdxne. \eng{the time draws near Christmas} kirxsfmasf(kAla) hatitxra barutitxde. 
\numi{2} (samAsa padagaLalilx): 
\banum
\alnum{a} hoVluva; sadaqsha; badali; parxtiyAda: \eng{near beer} biyarfnaMthadu. 
\alnum{b} hecucxkaDame; mukAkxlu mUru viVse pAlu; bahuteVka: \eng{near-hysterical} hecucx kaDame unAmxda sithxtiyalilx. \eng{a near-Communist} hecucxkaDame kamUyxnisfTx. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{comes no nearer to the end} kone muTuTxvaMteyeV ilalx. 
\num{2} \eng{near the} \hyperref{kandict_k.pdf}{K}{knuckle(1) nuga(2)}{$^1$knuckle}. 
\numi{3} \eng{(sail) near the wind} 
\banum
\alnum{a} (gALiya otatxDada neraviniMda) eSuTx sAdhayxvoV aSuTx edurugALige edurAgi yAna mADu. 
\alnum{b} (\AmA) ashilxVlatege yA apArxmANikatege hatitxravAgi. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[near(3)]{near}
\pron{niarf}
\gl{\gu}
\bmng
\bnum
\num{1} hatitxrada; nikaTa saMbaMdhada: \eng{near relation} hatitxrada saMbaMdhi. 
\num{2} Apatx; anoyxVnayx: \eng{a near friend} Apatx senxVhita. 
\num{3} eDagaDeya (\eng{off} padakekx vayxtirikatxvAgi): \eng{the near foreleg} eDagaDeya muMgAlu. 
\num{4} (sAthxna yA kAla) hatitxraviruva; samiVpada; nereya; pakakxda. 
\num{5} (rasetx yA hAdi) neVravAda. 
\num{6} bahu hatitxrada: \eng{a near guess} bahuteVka sariyAda Uhe. \eng{a near resemblance} hatitxrada hoVlike. 
\num{7} alapx aMtarada: \eng{near escape} alapx aMtaradalilx Ada pAru. 
\num{8} jugagx; jipuNatanada: \eng{he is very near with his money} avanu tananx haNada viSayadalilx bahaLa jugagx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{near and dear} namamxvaru tamamxvaru; iSaTxbaMdhugaLu: naMTariSaTxru; ApetxVSaTxru. 
\numi{2} \eng{near miss} (\rUpa\ saha) 
\banum
\alnum{a} (bAMbu \mo vugaLa \vi) gurige hatitxravAda; savxlapxdaralilx guri tapipxda. 
\alnum{b} savxlapxdaralilx GaSaRNe, Dikikx tapipxda parisithxti. 
\alnum{c} (parxyatanxda \vi) hecucxkaDime yashasivxyAda, Adare pUtiR udedxVsha sAdhisirada. 
\eanum
\numie
\num{3} \eng{near thing} savxlapx aMtaradalilx Ada pAru. 
\enum
\emng
\eentry

\bentry
\word[near(4)]{near}
\pron{niarf}
\gl{\sakirx}
\bmng
samiVpisu: \eng{the ship was nearing land} haDagu daDavanunx samiVpisutitxtutx. 
\emng

\noindent
\gl{\akirx}
\bmng
hatitxra baru: \eng{still it neared and neared} adu inUnx hatitxra hatitxrakekx baMtu. 
\emng
\eentry

\bentry
\word[nearby(1)]{nearby}
\pron{niarfbeY}
\gl{\gu}
\bmng
samiVpada; hatitxradalilxruva: \eng{a nearby hotel} samiVpada hoVTelu. 
\emng
\eentry

\bentry
\word[nearby(2)]{nearby}
\pron{niarfbeY}
\gl{\kirxvi}
\bmng
samiVpadalilx; dUradalilx alalxde. 
\emng
\eentry

\bentry
\word{Nearctic}
\pron{niakfRTikf}
\gl{\gu}
\bmng
(\pArxvi) utatxra amerikada shiVta hAgU samashiVtoVSaNx BAgagaLa. 
\emng
\eentry

\bentry
\word{nearish}
\pron{niariSf}
\gl{\gu}
\bmng
savxlapx hatitxraviruva; samiVpada. 
\emng
\eentry

\bentry
\word{nearishness}
\pron{niariSfnisf}
\gl{\nA}
\bmng
savxlapx hatitxraviruvike; sAmiVpayx. 
\emng
\eentry

\bentry
\word{nearly}
\pron{niarfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} hatitxradalilx; nikaTavAgi; bahuteVkavAgi; bahaLavAgi: \eng{nearly resemble} bahaLavAgi hoVlu. 
\num{2} hecucx kaDame; mukAkxlu mUru viVse pAlu: \eng{it is nearly one o'clock} hecucx kaDame oMdu gaMTeyAgide. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
 \eng{not nearly} sAladAgi; sAkAguvaSiTxlalxde: \eng{there isn't nearly enough time to learn all these words} I elalx padagaLanunx kaliyalu-kAla sAladu, kAla sAkaSiTxlalx. 
\emng
\eentry

\bentry
\word{nearside}
\pron{niarfseYDf}
\gl{\nA}
\bmng
(\sA\ guNavAcakavAgi) (\birx) vAhana, pArxNi, \mo vugaLa eDaBAga. 
\emng
\eentry

\bentry
\wordnospeech{near sight}{near sight}
\pron{?}
\gl{\nA}
\bmng
(\kanmu\ \ame) = \hyperref{kandict_s.pdf}{S}{short sight}{short sight}. 
\emng
\eentry

\bentry
\word{near-sighted}
\pron{niarfseYTiDf}
\gl{\gu}
\bmng
(\kanmu\ \ame) = \hyperref{kandict_s.pdf}{S}{short-sighted}{short-sighted}. 
\emng
\eentry

\bentry
\word{near-sightedly}
\pron{niarfseYTiDfli}
\gl{\kirxvi}
\bmng
= \hyperref{kandict_s.pdf}{S}{short-sightedly}{short-sightedly}.
\emng
\eentry

\bentry
\word{near-sightedness}
\pron{niarfseYTiDfnisf}
\gl{\nA}
\bmng
= \hyperref{kandict_s.pdf}{S}{short-sightedness}{short-sightedness}. 
\emng
\eentry

\bentry
\wordnospeech{near work}{near work}
\pron{?}
\gl{\nA}
\bmng
kaNuNx hatitxraviTuTxkoMDu mADabeVkAda kelasa. 
\emng
\eentry

\bentry
\word[neat(1)]{neat}
\pron{niVTf}
\gl{\nA}
\bmng
(\pArxparx) 
\bnum
\num{1} danada jAtiya (yAvudAdarU) pArxNi. 
\num{2} (\bava vAgi) dana; goVvu; pashu. 
\enum
\emng
\eentry

\bentry
\word[neat(2)]{neat}
\pron{niVTf}
\gl{\gu}
\bmng
\bnum
\num{1} (madayxda \vi) niVru beresilalxda. 
\num{2} aMdavAda; saraLateyuLaLx; nAjUkAda; acucxkaTATxda: \eng{a neat work} nAjUkAda kelasa. 
\numi{3} (BASe, sheYli, sUkitxgaLa \vi) 
\banum
\alnum{a} saMkeSxVpavU sapxSaTxvU monacU Ada. 
\alnum{b} jANa; caturoVkitxyiMda kUDida: \eng{a neat answer} jANatanada utatxra. 
\eanum
\numie
\num{4} kushala; nipuNa; keYcaLakavuLaLx; budidhxvaMtikeyiMda mADida: \eng{a neat conjuring trick} kushalavAda gAruDi videyxya caLaka. 
\num{5} opapxvAda; acucxkaTATxda; OraNada; karxmabadadhxvAda: \eng{he was neat and methodical in all small matters} avanu elAlx saNaNxpuTaTx viSayagaLalilxyU acucxkaTUTx karxmabadadhxvU Agidadxnu. 
\num{6} (\ame) BajaRri; atuyxtatxma; sherxVSaThx. 
\enum
\emng
\eentry

\bentry
\word{neaten}
\pron{niVTf(Ta)nf}
\gl{\sakirx}
\bmng
(\kanmu\ kasUtikelasada \vi) opapx OraNa mADu; ceMdagoLisu; nAjUkAgisu; aMdagoLisu. 
\emng
\eentry

\bentry
\word{neath}
\pron{niVtf}
\gl{\upa}
\bmng
(\kAparx) = \hyperref{kandict_b.pdf}{B}{beneath(1)}{beneath}. 
\emng
\eentry

\bentry
\word{neat-handed}
\pron{niVTfhAYxMDiDf}
\gl{\gu}
\bmng
catura; kushala; keYcaLakavuLaLx. 
\emng
\eentry

\bentry
\word{neat-herd}
\pron{niVTfhaDfR}
\gl{\nA}
\bmng
dana kAyuvavanu; danagAhi. 
\emng
\eentry

\bentry
\word{neat-house}
\pron{niVTfhwsf}
\gl{\nA}
\bmng
danada koTiTxge. 
\emng
\eentry

\bentry
\word{neatly}
\pron{niVTfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} aMdavAgi; nAjUkiniMda; acucxkaTATxgi. 
\num{2} saMkeSxVpavAgi; aDakanAgi; sapxSaTxvU monacU Ada riVtiyalilx. 
\num{3} jANatanadiMda. 
\num{4} kushalateyiMda; nipuNateyiMda; keYcaLakadiMda. 
\num{5} opapxOraNadiMda; karxmabadadhxvAgi. 
\enum
\emng
\eentry

\bentry
\word{neatness}
\pron{niVTfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} aMdaceMda; nAjUkAgiruvudu; acucxkaTATxgiruvike. 
\num{2} saMkiSxpatxte. 
\num{3} jANatanadiMda kUDiruvike. 
\num{4} kushalate; neYpuNayx; keYcaLaka. 
\num{5} opapxOraNa(riVti); karxmabadadhxvAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{neat's-foot}
\pron{niVTfsxphuTf}
\gl{\nA}
\bmng
(AhAravAgi baLasuva) danada kAlu. 
\emng
\eentry

\bentry
\wordnospeech{neat's-foot oil}{neat's-foot oil}
\pron{?}
\gl{\nA}
\bmng
danada kAlina eNeNx; danada kAlanunx kudisi tayArisida, togalanunx hadamADalu baLasuva, eNeNx. 
\emng
\eentry

\bentry
\wordnospeech{neat's leather}{neat's leather}
\pron{?}
\gl{\nA}
\bmng
danada togalu. 
\emng
\eentry

\bentry
\word{neat's-tongue}
\pron{niVTfsxTMgf}
\gl{\nA}
\bmng
(ahAravAgi baLasuva) danada nAlage. 
\emng
\eentry

\bentry
\wordnospeech{NEB}{NEB}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} (yuneYTeDf kiMgaDxmfnalilx) \eng{National Enterprise Board.} 
\num{2} \eng{New English Bible.} 
\enum
\emng
\eentry

\bentry
\wordnospeech{Neb}{Neb}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{Nebraska.} 
\emng
\eentry

\bentry
\word{neb}
\pron{nebf}
\gl{\nA}
\bmng
(sAkxTelxMDf matutx \kanu\ iMgelxMDf) 
\bnum
\num{1} hakikxya kokukx; caMcu. 
\num{2} mUgu. 
\num{3} mUti; musuDi (mUgu matutx bAyi). 
\num{4} mone; tudi; monacu. 
\enum
\emng
\eentry

\bentry
\word[nebbish(1)]{nebbish}
\pron{nebiSf}
\gl{\nA}
\bmng
(\AmA) 
\bnum
\num{1} namarx; viniVta; vidheVya; tagigx naDeyuva vayxkitx. 
\num{2} pukakxla manuSayx; hedarupukakxla; BiVru. 
\enum
\emng
\eentry

\bentry
\word[nebbish(2)]{nebbish}
\pron{nebiSf;}
\gl{\gu}
\bmng
(\AmA) 
\bnum
\num{1} namarx; vidheVya; viniVta; tagigx naDeyuva. 
\num{2} pukakxlanAda; hedarupukakxlanAda. 
\enum
\emng
\eentry

\bentry
\wordnospeech{Nebr.}{Nebr.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{Nebraska.} 
\emng
\eentry

\bentry
\word{nebula}
\pron{nebuyxla}
\gl{\nA}
\expl{(\bava\ \eng{nebulae} \ucAcx-- nebuyxliV yA \eng{nebulas}). }
\bmng
\bnum
\numi{1} niVhArike; nebuyxla: 
\banum
\alnum{a} (\Kavi) aMtara tArA parxdeVshadalilx moVDadaMte kANisuva, anila hAgU dhULina baqhadArxshigaLalolxMdu. 
\alnum{b} (\Kavi) namamx hAluhAdi yA kiSxVrapatha gelAyxkisxya horagaDe iruva, moVDadaMte kANisuva itara yAvudeV gelAyxkisx yA nakaSxtarxgaLa samUha. 
\eanum
\numie
\num{2} (\veYshA) nebuyxla; kaNiNxna kAniRyadalilx kelavu veVLe kaMDubaruva masaku, macecx. 
\enum
\emng
\eentry

\bentry
\word{nebular}
\pron{nebuyxlarf}
\gl{\gu}
\bmng
nebuyxlada yA niVhArikeya, adaraMtha yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\wordnospeech{nebular hypothesis}{nebular hypothesis}
\pron{?}
\gl{\nA}
\bmng
(\Kavi) niVhArikA sidAdhxMta; nebuyxla sidAdxMta; anila hAgU dhULina baqhadArxshiyoMdariMda swravUyxha matutx nakaSxtarxmaMDalagaLu rUpugoMDaveMba sidAdhxMta. 
\emng
\eentry

\bentry
\wordnospeech{nebular theory}{nebular theory}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{nebular hypothesis}{nebular hypothesis}. 
\emng
\eentry

\bentry
\word{nebulosity}
\pron{nebuyxlAsiTi}
\gl{\nA}
\bmng
niVhArikiVyate; niVhArikeyaMte, nebuyxladaMte, masuku moVDadaMte -- iruvike. 
\emng
\eentry

\bentry
\word{nebulous}
\pron{nebuyxlasf}
\gl{\gu}
\bmng
niVhArikiVya; niVhArikeyaMtha; nebuyxladaMtha: \eng{nebulous star} niVhArikA tAre; niVhArikeyaMte, nebuyxladaMte, masuku moVDadaMte kANuva -- tAre yA tAregaLa guMpu. 
\emng
\eentry

\bentry
\word{nebulously}
\pron{nebuyxlasfli }
\gl{\kirxvi}
\bmng
niVhArikeyaMte; niVhArikeya riVtiyalilx. 
\emng
\eentry

\bentry
\word{nebulousness}
\pron{nebuyxlasfnisf}
\gl{\nA}
\bmng
niVhArikeyaMtiruvike. 
\emng
\eentry

\bentry
\word{nebuly}
\pron{nebuyxli}
\gl{\gu}
\bmng
(\vaMlAM) 
\bnum
\num{1} aleyaleyAgiruva rUpada. 
\num{2} moVDadaMte citirxsida, rUpisida. 
\enum
\emng
\eentry

\bentry
\word[necessarian(1)]{necessarian}
\pron{nesiseVrianf}
\gl{\nA}
\bmng
= \hyperlink{necessitarian(1)}{$^1$necessitarian}. 
\emng
\eentry

\bentry
\word[necessarian(2)]{necessarian}
\pron{nesiseVrianf}
\gl{\gu}
\bmng
= \hyperlink{necessitarian(2)}{$^2$necessitarian}. 
\emng
\eentry

\bentry
\word{necessarianism}
\pron{nesiseVrianisaZmf}
\gl{\nA}
\bmng
= \hyperlink{necessitarianism}{necessitarianism}. 
\emng
\eentry

\bentry
\word{necessarily}
\pron{nesasa(se)rili}
\gl{\kirxvi}
\bmng
atayxvashayxvAgi; anivAyaRvAgi; agatayxvAgi. 
\emng
\eentry

\bentry
\word[necessary(1)]{necessary}
\pron{nesasari}
\gl{\gu}
\bmng
\bnum
\num{1} agatayxvAda; atayxvashayxkavAda; anivAyaR(vAda); beVkeV beVkAda: \eng{is it necessary that you should be so economical?} niVnu aSoTxMdu mitavayxya mADuvudu agatayxveV? 
\num{2} mADaleVbeVkAda; kaDADxyavAda; AgaleVbeVkAda. 
\num{3} vidhivihitavAda; niyatavAda; deYvasaMkalipxtavAda yA parxkaqti niyamagaLiMda niyaMtirxtavAda; sevxVceCxyiMdalalxda. 
\num{4} anivAyaRvAgi saMBavisuva yA iruva: \eng{necessary evil} anivAyaRvAda aniSaTx. 
\num{5} (kalapxneya yA manaHparxvaqtitxya \vi) (vasutxsithxtiyiMda) anivAyaRvAgi sididhxsaleVbeVkAda; (pUvaRsithxtiyiMda) PalisabeVkAda. 
\num{6} (kataqRvina \vi) niyativasha; niyatibadadhx; savxtaMtarx icACxshakitxyilalxda; saMkalapx sAvxtaMtarxyXvilalxda: \eng{man is not a free but necessary agent} manuSayxnu savxtaMtarxvAda icACxshakitxyilalxda kataqR. 
\enum
\emng
\eentry

\bentry
\word[necessary(2)]{necessary}
\pron{nesasari}
\gl{\nA}
\bmng
Avashayxka vasutx, jiVvanAdhArakekx agatayxvAdadudx: \eng{the necessaries of life} jiVvanAgatayxgaLu; jiVvanAvashayxkategaLu; badukige agatayxvAgi beVkAda (AhAra, vasati, baTeTx, \mo) vasutxgaLu. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{do the necessary} agatayxvAda kelasa pUreYsu. 
\num{2} \eng{the necessary} (\AmA) haNa yA kAyaR; oMdu udedxVshakAkxgi beVkAda vasutx, kAyaR, sAdhana saMpatutx: \eng{provide the necessary} sAdhana saMpatutx odagisu. 
\enum
\emng
\eentry

\bentry
\word[necessitarian(1)]{necessitarian}
\pron{nisesiTeVrianf}
\gl{\nA}
\bmng
niyativAdi; manuSayxnige icACxsAvxtaMtarxyXvilalxveMdU elalx kAyaRgaLU adara hiMdiruva kAraNagaLiMda nidhARravAguvuveMdU heVLuvavanu. 
\emng
\eentry

\bentry
\word[necessitarian(2)]{necessitarian}
\pron{nisesiTeVrianf}
\gl{\gu}
\bmng
niyativAdada; manuSayxnige icACxsAvxtaMtarxyXvilalxveMdU, elalx kAyaRgaLU avugaLa hiMdiruva kAraNagaLiMda nidhARravAguvuveMdU heVLuva tatatxvXda. 
\emng
\eentry

\bentry
\word{necessitarianism}
\pron{nisesiTeVrianisaZmf}
\gl{\nA}
\bmng
niyativAda; niyati sidAdhxMta. 
\emng
\eentry

\bentry
\word{necessitate}
\pron{nisesiTeVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} agatayxvAgisu; AvashayxkavenisuvaMte mADu; anivAyaRvenisu: \eng{the increase in population necessitates a greater food supply} janasaMKeyxyu hecAcxdAga AhArada utApxdaneyu hecucxvudu anivAyaR. 
\num{2} (oMdu kelasa mADalu, obabxnanunx) nibaRMdhisu; balAtakxrisu: \eng{each boy is necessitated to act for himself} parxtiyobabx huDuganU tanagAgi tAnu kelasa mADaleV beVkAgide. 
\enum
\emng
\eentry

\bentry
\word{necessitous}
\pron{nisesiTasf}
\gl{\gu}
\bmng
baDatanada; nigaRtika: \eng{in necessitous circumstances} baDatanadalilx. 
\emng
\eentry

\bentry
\word{necessity}
\pron{nisesiTi}
\gl{\nA}
\expl{(\bava\ \eng{necessities}).}
\bmng
\bnum
\num{1} (BwtaparxpaMcadalilx oMdu niyamavAgidudx, manuSayxna elAlx kAyaRgaLanUnx nidhaRrisuvudenanxlAda) otAtxya; nibaRMdha; niyati. 
\num{2} anivAyaRte; Avashayxkate; anivAyaRvAdadudx; AvashayxkavAdadudx. 
\num{3} ugarxvAda agatayx; tiVvarx Avashayxkate: \eng{necessity is the mother of invention} AvashayxkateyeV AviSAkxrada mUla. 
\num{4} mADaleVbeVkAda parisithxti; agatayx; anivAyaRte: \eng{the necessity of protecting life and property} jiVvavanUnx AsitxpAsitxgaLanUnx rakiSxsaleVbeVkAda atayxgatayx nibaRMdha. 
\num{5} anivAyaRvAdadudx; agatayxvAda vasutx; jiVvanAdhArakekx beVkAda vasutx: \eng{food and warmth are necessities} AhAra matutx shAKa atayxgatayx vasutxgaLu. 
\num{6} (\Eva\ yA \bava dalilx) rikatxte; baDatana; dAridarxyX; kaSaTxdeshe; tutARgi beVkAdadudx: \eng{a family in dire necessity} kaDubaDatanakekx sikikxda oMdu kuTuMba. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \hyperref{kandict_l.pdf}{L}{lay(1) nuga(23)}{$^1$lay} \eng{person under necessity.} 
\num{2} \eng{logical necessity} tAkiRka nibaRMdha; oMdu tatatxvXkekx virudadhxvAdadadxnunx Uhisalu sAdhayxvilalxdadxriMda adanenxV naMbabeVkAda nibaRMdha. 
\hyperdef{N}{necessity pagu(3)}{} 
\numi{3} \eng{make a virtue of necessity} 
\banum
\alnum{a} agatayxvanenxV dhamaRveMdu toVpaRDisu; yatanxvilalxde mADabeVkAgi baMdadadxnunx mADi adanunx satAkxrayxdaMte kataRvayxdaMte toVpaRDisu. 
\alnum{b} mADabeVkAdudanunx cenAnxgi, GanateyiMda -- mADu. 
\eanum
\numie
\num{4} \eng{necessity knows no law} agatayxkekx kaTuTxpADilalx; gatayxMtaravilalxde mADidudx aparAdhavalalx, shikASxhaRvalalx. 
\num{5} \eng{of necessity} anivAyaRvAgi; gatayxMtaravilalxde. 
\enum
\emng
\eentry

\bentry
\word[neck(1)]{neck}
\pron{nekf}
\gl{\nA}
\bmng
\bnum
\num{1} katutx; kutitxge; koraLu; goVNu; kaMTha; BujagaLigU talegU saMbaMdha kalipxsuva deVhaBAga. 
\num{2} (jUjina kudure) katitxnudadx; paMdayxdalilx eSuTx muMdide eMbudanunx aLeyuvAga lekakxkekx tegedukoLuLxva adara kutitxge hAgU taleya oTuTx udadxda aLate. 
\num{3} (AhArakAkxgi baLasuva pArxNiya) katitxna mAMsa: \eng{neck of lamb} kurimariya katutx, katitxna mAMsa. 
\num{4} (SaTuR \mo\ uDupina) katutx; koraLa BAga. 
\num{5} (\ashi) sokukx; udadhxTatana; dhASaTxyXR: \eng{you have got a neck asking that} hAge keVLuva udadhxTatana ninage ide. 
\num{6} (pAterxya, \kanmu\ siVseya) kaMTha; katutx. 
\num{7} (dAri, kaNive, kAluve, \mo vugaLa) kiridAda BAga; ikakxTuTx mAgaR. 
\num{8} kaNive. 
\num{9} BUkaMTha; BUshira. 
\num{10} kirugAluve. 
\num{11} (piTiVlu \mo vugaLalilx) kaMTha; beraLADisuva BAga. 
\num{12} katutx; oMdu vasutxvina eraDu BAga kUDisuva ikakxTuTx BAga. 
\num{13} (\BUvi) katutx; haLe jAvxlAmuKiya bAyiya boVguNiyalilx yA nALadalilx GaniVkarisida lAva yA aginxshile. 
\hypertarget{neck(1)14}{} 
\num{14} (\vAshi) kaMbada katutx; kaMbada boVdigeya keLaBAga. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{break one's neck} 
\banum
\alnum{a} goVNu muri; goVNumuridu sAyi. 
\alnum{b} (mADalu yA mADutatx) kaSaTxpaDu; sharxmapaDu: \eng{I'm not going to break my neck to finish the job today} A kelasavanunx iMdeV mugisalu nAnu sharxmapaDuvudilalx. 
\eanum
\numie
\num{2} \hyperref{kandict_d.pdf}{D}{dead(1) nuga(6)}{$^1$dead} \eng{from the neck up.} 
\numi{3} \eng{get it in the neck} (\AmA) 
\banum
\alnum{a} kutitxgege baru; pArxNApAyada, kaThiNavAda -- peTuTxtinunx, anuBavisu. 
\alnum{b} parxbalavAda AkeSxVpaNege, KaMDanege yA shikeSxge guriyAgu. 
\alnum{c} CiVmAri mADisiko. 
\eanum
\numie
\num{4} \eng{neck and crop} iDiyAgi; anAmatAtxgi; pUtAR. 
\num{5} \eng{neck and neck} (jUjinalilx) samasamanAgi ODu (\rUpa\ saha). 
\num{6} \eng{neck of the woods} (\AmA) dUraparxdeVsha; dUradalilxruva parxdeVsha, jAga. 
\num{7} \eng{neck or nothing} AdadAdxgaleMdu gelalxlu idudxdelalxvanUnx paNavoDuDx. 
\num{8} \eng{pain in the neck} (\AmA) piVDisuva, kADisuva, goVLu huyudxkoLuLxva -- vayxkitx; beVjAru huTiTxsuva, taruva vasutx \mo vu. 
\num{9} \eng{risk one's neck} pArxNApAyada saMBavakekx sikikx, bidudx, \mo vugaLiMda sAyuva sithxtiyalilxru. 
\num{10} \eng{save one's neck} taleyuLisiko; neVNu hAkuvudanunx tapipxsiko. 
\hyperdef{N}{neck(1) pagu(11)}{} 
\num{11} \eng{talk through} (\engit{or} \eng{through the back of) one's neck} (\ashi) aviveVkavAgi yA hucucxhucAcxgi mAtanADu. 
\num{12} \eng{up to one's neck} (\AmA) (yAvudeV \vi) kutitxgeyavaregU muLugi; saMpUNaRvAgi toDagisikoMDu. 
\enum
\emng
\eentry

\bentry
\word[neck(2)]{neck}
\pron{nekf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (\ashi) AliMgana mADu; tabibxko. 
\numi{2} (\ashi) 
\banum
\alnum{a} (kutitxge hisuki) usirukaTiTxsu. 
\alnum{b} tale katatxrisihAku; shiraceCxVdana mADu. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} (heNuNxgaMDu joVDiya \vi) katutx tabibxko; kaMThAliMgana mADu. 
\num{2} mudAdxDu: \eng{sitting on park benches petting and necking in the dark} pAkiRna beMcugaLa meVle kuLitu katatxlalilx mudAdxDutAtx, kaMThAliMgana mADutAtx. 
\num{3} (yAvudoV oMdara) kiridAda, ikakxTATxda BAgavAgu (\sakirx\ saha). 
\enum
\emng
\eentry

\bentry
\word{neckband}
\pron{nekfbAYxMDf}
\gl{\nA}
\bmng
koraLapaTiTx; katitxnapaTiTx; kutitxgepaTiTx. 
\emng
\eentry

\bentry
\word{neckcloth}
\pron{nekfkAlxtf}
\gl{\nA}
\bmng
(\ca) kaMThavasatxrX. 
\emng
\eentry

\bentry
\word{necked}
\pron{nekfTx}
\gl{\gu}
\bmng
\bnum
\num{1} (yAvudAdarU vishiSaTx riVtiya) kutitxgeyuLaLx; kutitxge iruva. 
\num{2} kaMThaviruva; kaMThavanunx hoMdiruva. 
\enum
\emng
\eentry

\bentry
\word{neckerchief}
\pron{nekaciRphf}
\gl{\nA}
\bmng
kaMThavasatxrX; kutitxgeya sutatx dharisuva cwka. 
\emng
\eentry

\bentry
\word{necking}
\pron{nekiMgf}
\gl{\nA}
\bmng
(\ashi) 
\bnum
\num{1} mudAdxDuvudu; tabibxkoLuLxvudu. 
\num{2} (\vAshi) = \hyperlink{neck(1)14}{$^1$neck\((14)\)}. 
\enum
\emng
\eentry

\bentry
\word[necklace(1)]{necklace}
\pron{nekfla(li)sf}
\gl{\nA}
\bmng
\bnum
\num{1} kaMThahAra; ratanxhAra; maNisara. 
\num{2} (\da\ Aphirxka) beMki hacacxlu obabx vayxkitxya kutitxgege kaTiTxda, peTorxVlanunx savarida TeYru. 
\enum
\emng
\eentry

\bentry
\word[necklace(2)]{necklace}
\pron{nekfla(li)sf}
\gl{\sakirx}
\bmng
(\da\ Aphirxka) kutitxgege peTorxVlf tuMbida TeYranunx kaTiTx obabxnanunx suDu, sAyisu. 
\emng
\eentry

\bentry
\word{neckless}
\pron{nekflisf}
\gl{\gu}
\bmng
kaMTharahita; kutitxgeyilalxda. 
\emng
\eentry

\bentry
\word{necklet}
\pron{nekfliTf}
\gl{\nA}
\bmng
\bnum
\num{1} kaMThABaraNa. 
\num{2} (tupupxLa camaRda) kaMThaveVSaTxna; koraLa kApu; koraLuDupu. 
\enum
\emng
\eentry

\bentry
\word{neckline}
\pron{nekfleYnf}
\gl{\nA}
\bmng
(uDupinalilx) kaMThareVKe; kutitxgeya terapina sutatxlina reVKe. 
\emng
\eentry

\bentry
\word{necktie}
\pron{nekfTeY}
\gl{\nA}
\bmng
kaMThabaMdha; kaMThakwpiVna. 
\emng
\eentry

\bentry
\wordnospeech{necktie party}{necktie party}
\pron{?}
\gl{\nA}
\bmng
(\ashi) neVNu hAkuvudu; galilxgeVrisuvudu. 
\emng
\eentry

\bentry
\word{neck-verse}
\pron{nekfvasfR}
\gl{\nA}
\bmng
(\kanmu\ \eng{Psalm 51}ra) beYbalina sotxVtarxda lAyxTinf shabadxgaLu (ivanunx Oduva mUlaka pAdirxya vinAyiti keVLuvavanu tananx katatxnunx neVNiniMda uLisikoLaLxbahudAgitutx). 
\emng
\eentry

\bentry
\word{neckwear}
\pron{nekfveVrf}
\gl{\nA}
\bmng
(aMgaDi mAtu) koraLapaTiTxgaLu hAgU kaMThabaMdhagaLu. 
\emng
\eentry

\bentry
\word{necro-}
\pron{nekorxV-}
\gl{\sapUpa}
\bmng
heNada, shavada, shavakekx saMbaMdhisida eMbathaRda shabadxgaLa racaneyalilx baLasuva \sapUpa. 
\emng
\eentry

\bentry
\word{necrobiosis}
\pron{nekorxVbeYOsisf}
\gl{\nA}
\bmng
Utaka kaSxya; aMgAMshagaLu nashisuvudu, kaSxyisuvudu. 
\emng
\eentry

\bentry
\word{necrobiotic}
\pron{nekorxVbeYATikf}
\gl{\gu}
\bmng
Utaka kaSxyada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{necrogenic}
\pron{nekorxVjenikf}
\gl{\gu}
\bmng
shavasaMpakaRdiMduMTAda. 
\emng
\eentry

\bentry
\word{necrolatry}
\pron{nekArxlaTirx}
\gl{\nA}
\bmng
maqtArAdhane; perxVtapUje. 
\emng
\eentry

\bentry
\word{necrological}
\pron{nekarxlAjikalf}
\gl{\gu}
\bmng
sAvina sudidxge saMbaMdhisida; nidhanavAteRya. 
\emng
\eentry

\bentry
\word{necrology}
\pron{nekArxlaji}
\gl{\nA}
\bmng
\bnum
\num{1} sAvina sudidx; nidhanavAteR. 
\num{2} (ititxVcege) satatxvara paTiTx; maqtasUci. 
\enum
\emng
\eentry

\bentry
\word{necromancer}
\pron{nekorxVmAYxnasxrf}
\gl{\nA}
\bmng
\bnum
\num{1} perxVta shAsatxrXjacnx. 
\num{2} jAdugAra; yakiSxNi mADuvavanu; aiMdarxjAlika; mATagAra; maMtarxvAdi. 
\enum
\emng
\eentry

\bentry
\word{necromancy}
\pron{nekorxVmAYxnisx}
\gl{\nA}
\bmng
\bnum
\num{1} perxVta videyx; satatxvaroMdige vayxvahAra naDesi BaviSayx heVLuva videyx. 
\num{2} kaNakxTuTx; yakiSxNi; iMdarxjAla; jAduvideyx. 
\enum
\emng
\eentry

\bentry
\word{necromantic}
\pron{nekorxVmAYxMTikf}
\gl{\gu}
\bmng
\bnum
\num{1} perxVta videyxge saMbaMdhisida. 
\num{2} aiMdarxjAlika; mATagArikeya; yakiSxNi videyxya. 
\enum
\emng
\eentry

\bentry
\word{necrophagous}
\pron{nekArxphagasf}
\gl{\gu}
\bmng
heNa tinunxva; shavaBakaSxka; koLeta mAMsa tinunxva. 
\emng
\eentry

\bentry
\word{necrophil}
\pron{nekarxphilf}
\gl{\nA}
\bmng
shavakAmi; heNa perxVmi; heNagaLanunx kurita vikaqtakAmadiMda kUDidavanu. 
\emng
\eentry

\bentry
\word{necrophile}
\pron{nekarxpheYlf}
\gl{\nA}
\bmng
= \hyperlink{necrophil}{necrophil}. 
\emng
\eentry

\bentry
\word{necrophilia}
\pron{nekarxphilia}
\gl{\nA}
\bmng
= \hyperlink{necrophily}{necrophily}. 
\emng
\eentry

\bentry
\word{necrophiliac}
\pron{nekarxphiliAYxkf}
\gl{\nA}
\bmng
= \hyperlink{necrophil}{necrophil}. 
\emng
\eentry

\bentry
\word{necrophilic}
\pron{nekarxphilikf}
\gl{\gu}
\bmng
shavakAmada; heNaperxVmada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{necrophilism}
\pron{nekArxphilisaZmf}
\gl{\nA}
\bmng
= \hyperlink{necrophily}{necrophily}. 
\emng
\eentry

\bentry
\word{necrophilist}
\pron{nekArxphilisfTx}
\gl{\nA}
\bmng
= \hyperlink{necrophil}{necrophil}. 
\emng
\eentry

\bentry
\word{necrophily}
\pron{nekArxphili}
\gl{\nA}
\bmng
shavakAma; heNa perxVma; heNagaLanunx kurita vikaqta kAma. 
\emng
\eentry

\bentry
\word{necrophobia}
\pron{nekarxphoVbia}
\gl{\nA}
\bmng
\bnum
\num{1} maraNaBaya; sAvaMjike; sAvina bagegx vipariVta Baya. 
\num{2} heNaBiVti; shavaBaya; heNagaLa bagegx vipariVta Baya. 
\enum
\emng
\eentry

\bentry
\word{necropolis}
\pron{nekArxpalisf}
\gl{\nA}
\bmng
(\kanmu\ purAtana kAlada) shamxshAna; goVrisAthxna. 
\emng
\eentry

\bentry
\word{necropsy}
\pron{nekArxpisx}
\gl{\nA}
\bmng
shavapariVkeSx. 
\emng
\eentry

\bentry
\word{necroscopy}
\pron{nekArxsakxpi}
\gl{\nA}
\bmng
= \hyperlink{necropsy}{necropsy}. 
\emng
\eentry

\bentry
\word{necrose}
\pron{nekorxVsf}
\gl{\akirx}
\bmng
nekorxVsisf Agu; Utaka sAyu, nAshavAgu. 
\emng
\eentry

\bentry
\word{necrosis}
\pron{nekorxVsisf}
\gl{\nA}
\bmng
(\veYshA\ matutx \shavi) nekorxVsisf; roVgadiMda yA gAyadiMda, \kanmu\ gAyxMgirxVnf athavA kaSxyaroVgada oMdu kuruhAgi, Utakada sAvu, kaSxya. 
\emng
\eentry

\bentry
\word{necrotic}
\pron{nekArxTikf}
\gl{\gu}
\bmng
(\veYshA) nekorxVTikf; (UtakagaLa \vi) (sathxLiVyavAgi) satitxruva; maqta; nAshavAgiruva. 
\emng
\eentry

\bentry
\word{necrotise}
\pron{nekarxTeYsfZ}
\gl{\sakirx}
\bmng
= \hyperlink{necrotize}{necrotize}. 
\emng
\eentry

\bentry
\word{necrotize}
\pron{nekarxTeYsfZ}
\gl{\sakirx}
\bmng
\bnum
\num{1} nekorxVsisf AguvaMte mADu; Utaka sAyuvaMte mADu. 
\num{2} (\veYshA) (UtakagaLa \vi) sathxLiVyavAgi sAyu, nAshavAgu. 
\enum
\emng
\eentry

\bentry
\word{nectar}
\pron{nekaTxrf}
\gl{\nA}
\bmng
\bnum
\num{1} (\girxVpu\ matutx \roVpu) deVvategaLa pAniVya; amaqta. 
\num{2} (yAvudAdarU) ruciyAda, inidAda, ApAyxyamAnavAda pAniVya. 
\num{3} makaraMda; baMDu; sasayxgaLu sarxvisuva, jeVnuhuLugaLu jeVnanAnxgi parivatiRsuva madhudarxva. 
\enum
\emng
\eentry

\bentry
\word{nectarean}
\pron{nekeTxVrianf}
\gl{\gu}
\bmng
\bnum
\num{1} makaraMdadaMtha. 
\num{2} amaqtadaMtha. 
\num{3} (makaraMdadaMte) sihiyAgiruva; ruciyAgiruva. 
\enum
\emng
\eentry

\bentry
\word{nactared}
\pron{nekaTxDfR}
\gl{\gu}
\bmng
\bnum
\num{1} (\rUpa\ saha) madhutuMbida. 
\num{2} makaraMdadiMda vAyxpatxvAda; baMDiniMda rucigaTiTxda. 
\enum
\emng
\eentry

\bentry
\word{nactareous}
\pron{nekeTxVriasf}
\gl{\gu}
\bmng
madhuvanunxLaLx; makaraMdadiMda tuMbida. 
\emng
\eentry

\bentry
\word{nactariferous}
\pron{nekaTxripharasf}
\gl{\gu}
\bmng
madhu sarxvisuva; baMDu surisuva; madhuvanunx utApxdisuva. 
\emng
\eentry

\bentry
\word{nectarine}
\pron{nekaTxri(V)nf}
\gl{\nA}
\bmng
\bnum
\num{1} nuNupAda teLusipepx hAgU gaTiTx tiruLu iruva, oMdu bageya piVcf haNuNx. 
\num{2} I haNuNx biDuva mara. 
\enum
\emng
\eentry

\bentry
\word{nectarous}
\pron{nekaTxrasf}
\gl{\gu}
\bmng
madhuvinaMtha; makaraMdada hAgiruva. 
\emng
\eentry

\bentry
\word{nectary}
\pron{nekaTxri}
\gl{\nA}
\expl{(\bava\ \eng{nectaries}).}
\bmng
(sasayxda yA hUvina) madhugarxMthi; makaraMda garxMthi; makaraMda sarxvisuva aMga. 
\emng
\eentry

\bentry
\wordnospeech{NEDC}{NEDC}
\pron{?}
\gl{\saMkiSx}
\bmng
(iMgelxMDinalilx) \eng{National Economic Development Council.} 
\emng
\eentry

\bentry
\word{neddy}
\pron{neDi}
\gl{\nA}
\expl{(\bava\ \eng{neddies}).}
\bmng
(\AmA) 
\bnum
\num{1} katetx. 
\num{2} \eng{(Neddy)} = \hyperlink{NEDC}{NEDC}. 
\enum
\emng
\eentry

\bentry
\word{nee}
\pron{neV}
\gl{\gu}
\bmng
huTiTxda; (maduveyAda) heMgasina hesarinoDane kanAyxnAma seVrisuvalilx baLasuva pada (\udA\ \eng{Mrs. Smith, nee Jones)} shirxVmati simxtf, modalu joVnfsx. 
\emng
\eentry

\bentry
\word[need(1)]{need}
\pron{niVDf}
\gl{\nA}
\bmng
\bnum
\num{1} beVkAdadudx; korate; Avashayxkate; agatayx; daduR: \eng{if need be} agatayxvuMTAdare; Avashayxkate idadxre; beVkAdare. 
\num{2} (yAvudoV oMdu vasutx \mo vu hAjarAgiruvudara yA adanunx paDediruvudara) agatayx; Avashayxkate: \eng{the need of further securities} inUnx hecucx baMDavALa patarxgaLanunx iTuTxkoLuLxvudara agatayx. 
\num{3} avasarada saMdaBaR; tutuR parisithxti; ApatAkxla; kaSaTxkAla: \eng{a friend in need is a friend indeed} ApatAkxlakekx AdavaneV nijavAda senxVhita. 
\num{4} baDatana; nigaRti; Avashayxka vasutxgaLa aBAva. 
\num{5} Avashayxkate; Avashayxka vasutx; beVkAdudu: \eng{my needs are few} nananx agatayxgaLu bahaLa savxlapxveV. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{at need} beVkAdAga; agatayx bidAdxga; kaSaTxkAladalilx. 
\num{2} \eng{had need} (\pArxparx) (yAvudanonxV mADuvudu) Avashayxka: agatayx: \eng{had need remember} nenapinalilxDabeVku. 
\num{3} \eng{have need of} beVkAgiru; AvashayxkavAgiru. 
\num{4} \eng{have need to} beVkAgiru; AvashayxkavAgiru: \eng{has need to be warned} ecacxrisabeVkAda Avashayxkateyide. 
\num{5} \eng{in need} neravu AvashayxkavAda, beVkAgiruva. 
\num{6} \eng{in need of} beVkAgiruva. 
\enum
\emng
\eentry

\bentry
\word[need(2)]{need}
\pron{niVDf}
\gl{\sakirx}
\bmng
\bnum
\num{1} avashayxvAgiru; agatayxvAgiru; beVkAgiru. 
\num{2} agatayxvAgu; AvashayxkavAgu: \eng{it needs to be done with care} joVpAnavAgi adanunx mADabeVku. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
(\pArxparx) agatayxvAgu. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{it needs not} adu agatayxvilalx. 
\num{2} \eng{more than needs} beVkAdadxkikxMta hecAcxgi. 
\num{3} \eng{need not have done} mADabeVkAgiralilalx; mADabeVkAda agatayxviralilalx. 
\num{4} \eng{need you ask} niVnu keVLuva agatayxvuMTeV? 
\enum
\emng
\eentry

\bentry
\word{needfire}
\pron{niVDfpheYarf}
\gl{\nA}
\bmng
\bnum
\num{1} araNiya aginx; (maMtarxshakitxyuLaLxdedxMdu BAvisalAda) oNa marada tuMDugaLanunx ujijx huTiTxsida beMki. 
\num{2} saMkeVta joyxVti; saMjecnx sUcisuva uri, beLaku, \mo vu. 
\num{3} utasxvAginx; meYdAnadalilx urisuva doDaDx beMki. 
\enum
\emng
\eentry

\bentry
\word{needful}
\pron{niVDfphulf}
\gl{\gu}
\bmng
avashayxkavAda; atayxgatayx; \eng{it is needful to} adu atayxgatayx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{the needful} 
\banum
\alnum{a} agatayxvAgi beVkAdudu; AvashayxkavAdudu. 
\alnum{b} (\ashi) oMdu udedxVshakekx agatayxvAda haNa, kAyaRkarxma, adakAkxgi mADabeVkAda kelasa, \mo vu: \eng{do what is needful} agatayxvAdadadxnunx mADu. \eng{do the needful} agatayxvAdudanunx mADu; Enu mADabeVkAgideyoV adanunx mADu. 
\eanum
\emng
\eentry

\bentry
\word{needfully}
\pron{niVDfphuli}
\gl{\kirxvi}
\bmng
AvashayxkavAgi; beVkAdaMte. 
\emng
\eentry

\bentry
\word{needfulness}
\pron{niVDfphulfnisf}
\gl{\nA}
\bmng
agatayxvAgiruvike; AvashayxkavAgiruvike. 
\emng
\eentry

\bentry
\word{neediness}
\pron{niVDinisf}
\gl{\nA}
\bmng
baDatana; nigaRtika sithxti; dAridarxyX. 
\emng
\eentry

\bentry
\word[needle(1)]{needle}
\pron{niVDflf}
\gl{\nA}
\bmng
\bnum
\num{1} (holigeyalilx upayoVgisuva, oMdu tudiyalilx kaNiNxruva) sUji. 
\num{2} kasUtisUji; pAlxsiTxkf, mara, \mo vugaLiMda mADida, (kaNuNx ilalxda) kasUti kelasakekx baLasuva kaDiDx. 
\num{3} korxVSa kaDiDx; kokekx iruva, korxVSa kelasakekx baLasuva kaDiDx. 
\num{4} (yaMtorxVpakaraNagaLa muKabilelxya meVlina) muLuLx. 
\num{5} (kAMta)sUci; dikUsxciyalilx, viduyxtf matutx kAMtiVya upakaraNagaLalilx baLasuva, kAMtiVkarisiruva ukikxna muLuLx. 
\num{6} (aMki, akaSxragaLanunx koreyuva) muLuLx. 
\num{7} (cucucxmadudx koDalu baLasuva) sUji. 
\num{8} (gArxmophoVnf rekADfR tayArisalu yA rekADfRniMda shabadx horaDisalu baLasuva) muLuLx. 
\num{9} (kelavu bageya baMdUkugaLalilx tupAki madudx hotitxsalu otutxva) kudure. 
\num{10} shiKara; beTaTxda koVDu; monacu tudiya baMDe. 
\num{11} (kaTaTxDada goVDe \mo vugaLa taLadalilx kelasa mADuvAga Asare koDalu, haMgAmiyAgi baLasuva) Asareya dUla, tole. 
\num{12} sUjiyAkArada -- saPxTika, haraLu. 
\num{13} pharf yA peYnf marada ele. 
\num{14} (\birx)(\ashi) taTaTxne reVguva, uderxVkagoLuLxva sithxti: \eng{got the needle while waiting} kAyutitxdAdxga taTaTxne reVgida. 
\num{15} = \hyperref{kandict_o.pdf}{O}{obelisk}{obelisk}. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{eye of a needle}= \hyperlink{needle1pagu2}{?pagu?\((2)\)}. 
\hypertarget{needle1pagu2}{} 
\num{2} \eng{needle's eye} sUjigaNuNx; ati saNaNx raMdharx. 
\num{3} \hyperref{kandict_p.pdf}{P}{pin(1) pagu(2)}{$^1$pin}\eng{s and needles.} 
\num{4} \eng{search for a needle in a haystack} hulilxna medeyalilx sUji huDuku; vaqthA sharxmapaDu. 
\num{5} \eng{sharp as a needle} (sAhitayxka, rUpaka saha) tiVkaSxNXvAda; sUjiyaMte monacAda. 
\enum
\emng
\eentry

\bentry
\word[needle(2)]{needle}
\pron{niVDflf}
\gl{\sakirx}
\num{1} {sUjiyiMda -- holi, cucucx, kelasa mADu.\num{2} (vasutx \mo vugaLa naDuve) nusuLikoMDu hoVgu; dArimADikoDu hoVgu.\num{3} (keLa Asare koDabeVkAda kaTaTxDakekx) Asare dUla hAku.\num{4} (\AmA) (oMdu kelasa \mo vanunx mADalu, obabxnanunx) perxVrisu; keraLisu;cucucx; parxcoVdisu; huriduMbisu; \eng{the silence needled him} mwna avananunx keraLisitu.}\eentry

\bentry
\word{needlecord}
\pron{niVDflfkADfR}
\gl{\nA}
\bmng
nayavAda, ububx eLegaLuLaLx baTeTx. 
\emng
\eentry

\bentry
\word{needlecraft}
\pron{niVDflfkArxphfTx}
\gl{\nA}
\bmng
kasUti kale; kasUti kelasa. 
\emng
\eentry

\bentry
\word{needlefish}
\pron{niVDflfphiSf}
\gl{\nA}
\bmng
sUjimiVnu; beloVniDeV, vaMshakekx seVrida, udadxvAda meq hAgU mUti iruva, hoTeTxbAka miVnu. 
\emng
\eentry

\bentry
\word{needleful}
\pron{niVDflfphulf}
\gl{\nA}
\expl{(\bava\ \eng{needlefuls}).}
\bmng
oMdu sala sUjige poVNisida dAra \mo vugaLa udadx. 
\emng
\eentry

\bentry
\wordnospeech{needle game}{needle game}
\pron{?}
\gl{\nA}
\bmng
(\birx) atayxMta peYpoVTiyiMda kUDida paMdayxdATa. 
\emng
\eentry

\bentry
\word{needle-lace}
\pron{niVDflfleVsf}
\gl{\nA}
\bmng
leVsu; kasUtiyiMda heNeda alaMkAra paTiTx. 
\emng
\eentry

\bentry
\wordnospeech{needle match}{needle match}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{needle game}{needle game}.
\emng
\eentry

\bentry
\word{needle-point}
\pron{niVDflfpAiMTf}
\gl{\nA}
\bmng
\bnum
\num{1} sUkaSx mx, cUpu-tudi. 
\num{2} = \hyperlink{needle-lace}{needle-lace}. 
\enum
\emng
\eentry

\bentry
\word{needless}
\pron{niVDflisf}
\gl{\gu}
\bmng
anAvashayxkavAda; beVkilalxda; anagatayx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{needless to say} (parxkeSxVpavAkayxvAgi) (eMbudanunx) nAnu heVLabeVkAgiyeV ilalx. 
\emng
\eentry

\bentry
\word{needlessly}
\pron{niVDflisfli}
\gl{\kirxvi}
\bmng
anAvashayxkavAgi; anagatayxvAgi; beVkilalxvAdarU. 
\emng
\eentry

\bentry
\word{needlessness}
\pron{niVDflisfnisf}
\gl{\nA}
\bmng
anAvashayxkate; anagatayxvAgiruvike. 
\emng
\eentry

\bentry
\wordnospeech{needle valve}{needle valve}
\pron{?}
\gl{\nA}
\bmng
sUji kavATa; teLuvAda matutx monacAda kavATa. 
\emng
\eentry

\bentry
\word{needlewoman}
\pron{niVDflfvumanf}
\gl{\nA}
\expl{(\bava\ \eng{needlewomen}).}
\bmng
kasUti yA holige kelasadavaLu. 
\emng
\eentry

\bentry
\word{needlework}
\pron{niVDflfvakfR}
\gl{\nA}
\bmng
\bnum
\num{1} kasUti kelasa. 
\num{2} holige kelasa. 
\enum
\emng
\eentry

\bentry
\word{needs}
\pron{niVDf\char144}
\gl{\kirxvi}
\bmng
(\pArxparx) atayxgatayxvAgi; avashayxvAgi. 
\emng

\noindent
\gl{\pagu}
\bmng
\hypertarget{needspagu1}{} 
\bnum
\numi{1} \eng{must needs do} 
\banum
\alnum{a} (\sA\ aviveVkadiMda) mADaleVbeVkeMdu haTha hiDiyutAtxne. 
\alnum{b} mADiyeV tiVrabeVku; mADade vidhiyilalx. 
\eanum
\numie
\num{2}\eng{needs must do} = \hyperlink{needspagu1}{?pagu?\((1)\)}. 
\enum
\emng
\eentry

\bentry
\word{needy}
\pron{niVDi}
\gl{\gu}
\bmng
\bnum
\num{1}(vayxkitxya \vi) baDavanAda; nigaRtikanAda; gatiyilalxda; daridarx. 
\num{2} (saninxveVsha \mo vugaLa \vi) nigaRtika; baDatanadiMda kUDida; dAridarxyXda. 
\enum
\emng
\eentry

\bentry
\word{neep}
\pron{niVpf}
\gl{\nA}
\bmng
(sAkxTelxMDf)= \hyperref{kandict_t.pdf}{T}{turnip}{turnip}. 
\emng
\eentry

\bentry
\word{ne'er}
\pron{nearf}
\gl{\kirxvi}
\bmng
(\kAparx)= \hyperlink{never}{never\((1)\)}. 
\emng
\eentry

\bentry
\word{nefarious}
\pron{nipheVriasf}
\gl{\gu}
\bmng
atiduSaTx; mahApAtakada. 
\emng
\eentry

\bentry
\word{nefariously}
\pron{nipheVriasfli}
\gl{\nA}
\bmng
atiduSaTxtanadiMda; mahApAtaka riVtiyalilx. 
\emng
\eentry

\bentry
\word{nefariousness}
\pron{nipheVriasfnisf}
\gl{\nA}
\bmng
ati duSaTxtana; mahApAtaka. 
\emng
\eentry

\bentry
\wordnospeech{neg.}{neg.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{negative.} 
\emng
\eentry

\bentry
\word{negate}
\pron{nigeVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} ilalxdaMte mADu; radudxpaDisu. 
\num{2} ilalx enunx; alalx enunx; nirAkarisu. 
\num{3} alalxvenisu; ilalxvenisu. 
\num{4} (yAvudaradeV) aBAva Agiru yA sUcisu. 
\enum
\emng
\eentry

\bentry
\word{negation}
\pron{nigeVSanf}
\gl{\nA}
\bmng
\bnum
\num{1} nakAra; nirAkaraNe. 
\num{2} niSeVdha; niSeVdhoVkitx yA niSeVdha tatatxvX. 
\num{3} nirAkaraNe; viroVdhoVkitx; parxtiSeVdha. 
\hypertarget{negation(4)}{} 
\num{4} (\takaR) BeVdavanAnxgali, vAyxvaqtitxyanAnxgali sAdhisuvudu. 
\num{5} (vAsatxvavAgirabeVkAdudara) anasitxtavx; aBAva; anavasAthxna; anavasithxti; ilalxme; ilalxdiruvike. 
\num{6} avAsatxvate; ilalxdiruvudu yA iruvudakekx virudadhxvAgiruvudu. 
\enum
\emng
\eentry

\bentry
\word{negationist}
\pron{nigeVSanisfTx}
\gl{\nA}
\bmng
neVtivAdi: \sA\ aMgiVkaqta naMbikegaLanunx nirAkarisi avugaLa badalu EnanUnx sUcisadavanu. 
\emng
\eentry

\bentry
\word[negative(1)]{negative}
\pron{negaTivf}
\gl{\gu}
\bmng
\bnum
\num{1} nirAkaraNAthaRka; niSeVdhAthaRka; neVtayxthaRka; neVtAyxtamxka; nakArAtamxka; niSeVdhasUcaka: \eng{a negative vote} niSeVdhasUcaka mata. 
\num{2} (sAkASxtAtxda, sapxSaTxvAda guNagaLa) aBAvada; aBAvavuLaLx. 
\num{3} niSeVdhAtamxka: \eng{negative reaction} niSeVdhAtamxka parxtikirxye. 
\num{4} (\biVga) QuNa; QuNAtamxka; sonenxgiMta kaDameyAda. 
\num{5} (\biVga) kaLeyabeVkAdudanunx sUcisuva. 
\num{6} (\viduyx) QuNa; ilekATxrXnfgaLalilx yAva bageya viduyxdAveVshavideyoV aMtha viduyxdAveVshaviruva yA aMthadanunx utapxtitx mADuva. 
\num{7} niSeVdha savxrUpada; virudadhx rUpada: \eng{debt is negative capital} sAlavu niSeVdharUpada baMDavALa. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{in the negative} 
\banum
\alnum{a} nakArAtamxkavAgi; neVtAyxtamxkavAgi. 
\alnum{b} tirasakxrisuvaMte. 
\eanum
\numie
\num{2} \eng{negative evidence} niSeVdhAtamxka sAkaSxyX. 
\num{3} \eng{negative} \hyperref{kandict_g.pdf}{G}{geotropism pagu}{geotropism}. 
\num{4} \eng{negative instance} aGaTaneya nidashaRna; GaTaneya aBAvada, (yAvudeV) naDedilalxdiruvudara, saMBavisilalxdiruvudara -- nidashaRna. 
\num{5} \eng{negative proposition} (\takaR) niSeVdhavAkayx; neVtayxthaRka parxtijecnx. 
\num{6} \eng{negative quantity} (\hA) sonenx; shUnayx. 
\num{7} \eng{negative sign} QuNa cihenx. 
\num{8} \eng{negative virtue} duguRNABAva; tapupx mADadiruva, pApa mADadiruva guNa. 
\enum
\emng
\eentry

\bentry
\word[negative(2)]{negative}
\pron{negaTivf}
\gl{\nA}
\bmng
\bnum
\num{1} aBAvAtamxka heVLike; niSeVdha -- pada, utatxra: \eng{it is hard to prove a negative} oMdu niSeVdhAtamxka heVLikeyanunx rujuvAtu mADi toVrisuvudu kaSaTx. \eng{two negatives make an afirmative} eraDu nAsatx yxthaRkagaLu oMdu asatx yxthaRkavanunx sUcisutatxve. 
\num{2} aBAvAtamxkate; yAvudeV guNada aBAva, ilalxdiruvike: \eng{his character is made up of negatives} avana savxBAvavelalx aBAvamayavAgide. 
\num{3} (CAyAcitarxda \vi) negeTivf; QuNabiMba; neraLu, beLakugaLu nijavAgiyU iruvudakekx virudadhxvAgiruva biMba. 
\num{4} = \hyperlink{negation(4)}{negation\((4)\)}. 
\enum
\emng
\eentry

\bentry
\word[negative(3)]{negative}
\pron{negaTivf}
\gl{\sakirx}
\bmng
\bnum
\num{1} nirAkarisu; taLiLxhAku; niSeVdhisu; aMgiVkarisadiru; opapxdiru; samamxtisadiru. 
\num{2} (anumAna, UhAparxtijecnx -- ivanunx) parXmANavalalxveMdu sAthxpisu. 
\num{3} (heVLikeyanunx) viroVdhisu; niSeVdhisu; KaMDisu. 
\num{4} (parxBAva \mo vanunx) niSapxriNAmavAgisu; taTasathxgoLisu. 
\enum
\emng
\eentry

\bentry
\word{negatively}
\pron{negaTivfli}
\gl{\kirxvi}
\bmng
\bnum
\numi{1} nakArAtamxkavAgi: 
\banum
\alnum{a} ilalx eMdu; Aguvudilalx enunxva mUlaka: \eng{answered negatively} ilalx eMdu utatxrisida. 
\alnum{b} nirAkarisuvaMte; tirasakxrisuvaMte. 
\eanum
\numie
\num{2} QuNa viduyxtutx iruvaMte: \eng{negatively charged} QuNa viduyxdAviSaTxvAgi. 
\enum
\emng
\eentry

\bentry
\word{negativeness}
\pron{negaTivfnisf}
\gl{\nA}
\bmng
= \hyperlink{negativity}{negativity}. 
\emng
\eentry

\bentry
\wordnospeech{negative pole}{negative pole}
\pron{?}
\gl{\nA}
\bmng
(ayasAkxMtada) QuNa dhurxva. 
\emng
\eentry

\bentry
\word{negativism}
\pron{negaTivisaZmf}
\gl{\nA}
\bmng
niSeVdhavAda: 
\banum
\alnum{a} atiyAgi saMshayapaDuvudu. TiVkisuvudu. \mo vu. 
\alnum{b} saMparxdAya viroVdha; paraMparAgatavAda naMbikegaLanunx alalxgaLeyuvudu. 
\eanum
\emng
\eentry

\bentry
\word{negativist}
\pron{negaTivisfTx}
\gl{\nA}
\bmng
niSeVdhavAdi: 
\banum
\alnum{a} ati saMshaya paDuvava, TiVkisuvava. 
\alnum{b} saMparxdAya viroVdhi; paraMparAgatavAda naMbikegaLanunx alalxgaLeyuvava. 
\eanum
\emng
\eentry

\bentry
\word{negativistic}
\pron{negaTivisiTxkf}
\gl{\gu}
\bmng
niSeVdhAtamxka; nakArAtamxka; nirAkaraNAtamxka. 
\emng
\eentry

\bentry
\word{negativity}
\pron{negaTiviTi}
\gl{\nA}
\bmng
nakArAtamxkate; ilalx yA Aguvudilalx enunxvike. 
\emng
\eentry

\bentry
\word{negator}
\pron{nigeVTarf}
\gl{\nA}
\bmng
\bnum
\num{1} nirAkarisuvavanu; ilalx enunxvava. 
\num{2} tirasakxrisuvava. 
\enum
\emng
\eentry

\bentry
\word{negatory}
\pron{negaTari}
\gl{\gu}
\bmng
\bnum
\num{1} nakArada; nirAkaraNeya; alalxvenunxva. 
\num{2} BeVda yA vAyxvaqtitxyanunx sAdhisuva. 
\num{3} aBAvada; anasitxtavxda; anavasithxtiya. 
\num{4} avAsatxvavAda; ilalxda. 
\num{5} virudadhxvAda. 
\enum
\emng
\eentry

\bentry
\word[neglect(1)]{neglect}
\pron{nigelxkfTx}
\gl{\sakirx}
\bmng
\bnum
\num{1} AlakaSxyX toVrisu; gamana harisadiru; asaDeDxyiMdiru: \eng{the noble discourse neglected by the generation to which it was addressed} yAva piVLigegoVsakxra A udAtatx parxvacana niVDalAyitoV avariMdaleV adu alakaSxyXkekx oLagAgi. 
\num{2} upeVkiSxsu; udAsiVna toVrisu; lakaSxyX mADadiru; Asakitx vahisadiru: \eng{neglect one's studies} tananx vAyxsaMgavanunx upeVkiSxsu. 
\num{3} mADade nililxsu; pUNaRgoLisadiru: \eng{don't neglect writing to your mother} nimamx tAyige kAgada bareyadirabeVDa. 
\num{4} mADade biDu; mADadiru; mADalu tapupx: \eng{if they neglect to punish crime} aparAdhavanunx avaru shikiSxsade biTaTxre. 
\enum
\emng
\eentry

\bentry
\word[neglect(2)]{neglect}
\pron{nigelxkfTx}
\gl{\nA}
\bmng
\bnum
\num{1} alakaSxyX; asaDeDx: \eng{the garden was in a state of neglect} toVTa asaDeDxya sithxtiyalilxtutx. 
\num{2} kaDegaNike; nilaRkaSxyX: \eng{the state recovering from the effects of neglect} nilaRkaSx yxkokxLagAda rAjayxvu adara pariNAmagaLiMda ceVtarisikoLuLxtAtx. 
\num{3} (kataRvayx \mo vugaLa \vi) avajacnxte; udAsiVnate; asaDeDx; upeVkeSx: \eng{he lost his job because of neglect of duty} tananx kataRvayxda bagegx upeVkeSx toVrisidadxriMda avanu kelasa kaLedukoMDa. 
\enum
\emng
\eentry

\bentry
\word{neglectful}
\pron{nigelxkfTxphulf}
\gl{\gu}
\bmng
udAsiVnada; nilaRkaSx yxda; tAtAsxrada; gamana koDadiruva: \eng{boys who are neglectful of their appearance} tamamx (baTeTxbare \mo vugaLa) veVSaBUSaNagaLa bagegx aSATxgi gamanaviVyada huDugaru. 
\emng
\eentry

\bentry
\word{neglectfully}
\pron{nigelxkfTxphuli}
\gl{\kirxvi}
\bmng
nilaRkaSx yxdiMda; tAtAsxradiMda; udAsiVnavAgi. 
\emng
\eentry

\bentry
\word{neglectfulness}
\pron{nigelxkfTxphulfnisf}
\gl{\nA}
\bmng
nilaRkaSx yx; tAtAsxra; audAsiVnayx. 
\emng
\eentry

\bentry
\word{neglige}
\pron{negilxSeV}
\gl{\nA}
\bmng
= \hyperlink{negligee}{negligee}. 
\emng
\eentry

\bentry
\word{negligee}
\pron{negilxSezV}
\gl{\nA}
\bmng
\bnum
\num{1} (heMgasina) saDila, anwpacArika uDupu (\kanmu\ teLuvAda DerxsiMgf gwnu). 
\num{2} saraLavAda vidhuyxkatxvalalxda uDupu. 
\enum
\emng
\eentry

\bentry
\word{negligence}
\pron{negilxjanfsx}
\gl{\nA}
\bmng
\bnum
\num{1} udAsiVnate; upeVkeSx; alakaSx yxte; asaDeDx; sariyAda joVke, gamana-ilalxdiruvike. 
\num{2} = \hyperref{kandict_c.pdf}{C}{contributory negligence}{contributory negligence}. 
\num{3} (sAhitayx yA kale \mo vugaLalilx) sahajate; anibaRMdhate; nisasxMkoVca manoVBAva; kaqtakate, nibaRMdhagaLilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{negligent}
\pron{negilxjaMTf}
\gl{\gu}
\bmng
nilaRkaSx yxteya; asaDeDxya; kataRvayxloVpada: \eng{he was negligent of his duties} avanu tananx kataRvayxda bagegx nilaRkaSxyXdiMdidadx. 
\emng
\eentry

\bentry
\word{negligently}
\pron{negilxjaMTfli}
\gl{\kirxvi}
\bmng
nilaRkaSxyXdiMda; kataRvayxloVpa mADi. 
\emng
\eentry

\bentry
\word{negligibility}
\pron{negilxjibiliTi}
\gl{\nA}
\bmng
nilaRkiSxsabahudAgiruvike; gaNanege bAradiruvike. 
\emng
\eentry

\bentry
\word{negligible}
\pron{negilxjiba(bf)lf}
\gl{\gu}
\bmng
nilaRkiSxsabahudAda; gaNanege bArada; nagaNayx; lekakxkekx bArada: \eng{negligible quantity} nagaNayx vayxkitx \mo varu; lekakxkekx tegedukoLaLxbeVkAgilalxda vayxkitx \mo varu. 
\emng
\eentry

\bentry
\word{negligibly}
\pron{negilxjibilx}
\gl{\kirxvi}
\bmng
nilaRkiSxsabahudAdaMte; gaNanege bArada riVtiyalilx. 
\emng
\eentry

\bentry
\word{negotiability}
\pron{nigoVSabiliTi}
\gl{\nA}
\bmng
\bnum
\num{1} parihAyaRte; nivAraNiVyate. 
\num{2} (huMDi, cekukx, \mo vugaLa \vi) vayxvahAyaRte. 
\num{3} saMdhAna sAdhayxte; saMdhAna naDesalu, caciRsalu sAdhayxviruvike. 
\num{4} badalisalu avakAshaviruvike. 
\enum
\emng
\eentry

\bentry
\word{negotiable}
\pron{nigoVSabflf}
\gl{\gu}
\bmng
\bnum
\num{1} (beVli \mo vugaLa \vi) dATalu sAdhayxvAda. 
\num{2} (aDacaNe \mo vugaLa \vi) nivAraNiVya; parihAyaR. 
\num{3} (huMDi \mo vugaLa \vi) vinimeVya; parxtiPala paDedu matotxbabxnige vagARyisabahudAda. 
\num{4} (huMDi, cekukx, \mo vugaLa \vi) vayxvahAyaR; bele paDeyalu sAdhayxvAda; bele koDalu sAdhayxviruva. 
\num{5} saMdhAna naDesalu, caciRsalu sAdhayxvAda. 
\num{6} badalAyisalu avakAshaviruva. 
\enum
\emng
\eentry

\bentry
\word{negotiant}
\pron{nigoVSiaMTf}
\gl{\nA}
\bmng
= \hyperlink{negotiator}{negotiator}. 
\emng
\eentry

\bentry
\word{negotiate}
\pron{nigoVSiETf}
\gl{\sakirx}
\bmng
\bnum
\num{1} saMdhAnada mUlaka (vayxvahAravoMdanunx) tiVmARnisu; \eng{negotiated a settlement} saMdhAnada mUlaka oMdu vayxvahAravanunx tiVmARnisida. 
\num{2} (saMdhAnada mUlaka) udidxSaTx pariNAmavanunx taru, uMTumADu; (udidxSaTx guriyanunx) sAdhisu, neraveVrisu: \eng{negotiate loan} sAlakAkxgi saMdhAnaveVpaRDisu. 
\num{3} parxtiPala paDedu (huMDiyanunx) matotxbabxnige vagARyisu. 
\num{4} (huMDi, cekukx -- ivakekx) bele -- paDe, koDu. 
\num{5} nagadigAgali noVTugaLigAgali huMDiyanunx murisu. 
\num{6} (beVli \mo vanunx) dATu: \eng{my horse negotiated the fence very well} nananx kudure beVliyanunx bahaLa cenAnxgi dATitu. 
\num{7} (aDacaNe, toMdare, \mo vanunx) nivArisu; pariharisu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
(rAji yA opapxMda mADikoLaLxlu matotxbabxnoDane) parxsAtxpisu; samAloVcisu; saMdhAna naDesu: \eng{we have decided to negotiate with the employers abour our wage claims} (namage barabeVkAda) kUli bagegx mAliVkaroDane saMdhAna naDesalu nishacxyisidedxVve. 
\emng
\eentry

\bentry
\word{negotiation}
\pron{nigoVSi(si)ESanf}
\gl{\nA}
\bmng
\bnum
\num{1} saMdhAna; samAloVcane; mAtukate. 
\num{2} saMdhAna saBe; opapxMdada SaratutxgaLa caceR. 
\enum
\emng
\eentry

\bentry
\word{negotiator}
\pron{nigoVSiETarf}
\gl{\nA}
\bmng
saMdhAnakAra; samAloVcaka. 
\emng
\eentry

\bentry
\word{Negress}
\pron{niVgirxsf}
\gl{\nA}
\bmng
heNuNx niVgorxV; niVgorxV sitxrXV. 
\emng
\eentry

\bentry
\word{Negrillo}
\pron{nigirxloV}
\gl{\nA}
\expl{(\bava\ \eng{Negrillos}).}
\bmng
gujAjxri niVgorxV; kubajx niVgorxV; giDadx niVgorxV; (madhayx matutx dakiSxNa AphirXkada nivAsiyAda) moVTu niVgorxV jAtiyavanu. 
\emng
\eentry

\bentry
\word{Negrito}
\pron{nigirxVToV}
\gl{\nA}
\bmng
(malayoV -- pAlineVSiyA parxdeVshada) giDaDx niVgorxV mAdariya jAtiyavanu. 
\emng
\eentry

\bentry
\word{Negritude}
\pron{niVgirxTUyxDf}
\gl{\nA}
\bmng
\bnum
\num{1} niVgorxVtana; niVgorxV Agiruvike yA niVgorxV Agiruva sithxti. 
\num{2} niVgorxV tatatx vx; niVgorxVvAdi; niVgorxV saMsakx qtiya veYshiSaTxyXdalilx niSeThxyuLaLxvanu. 
\enum
\emng
\eentry

\bentry
\word[Negro(1)]{Negro}
\pron{}
\gl{\nA}
\expl{(\bava\ \eng{Negroes}.)}
\bmng
niVgorxV (jAtiyavanu); (mUlataH AphirXkada) kariya vaMshasathx, buDakaTiTxnavanu. 
\emng
\eentry

\bentry
\word[Negro(2)]{Negro}
\pron{niVgorxV}
\gl{\gu}
\bmng
\bnum
\num{1} niVgorxV; niVgorxV jAtiya; kariya buDakaTiTxna. 
\num{2} niVgorxV janara; niVgorxV janariMda AkarxmitavAda; niVgorxV janarige saMbaMdhisida. 
\num{3} (pArxNi, kiVTa, \mo vugaLa \vi) kariya; karibaNaNxda; kapupx camaRda: \eng{negro ant} kari iruve. 
\enum
\emng
\eentry

\bentry
\word{Negro-head}
\pron{niVgorxV heDf}
\gl{\nA}
\bmng
\bnum
\num{1} (jagiyalu baLasuva) kapupx, kaTu hogesopipxna uMDe, kaDiDx, \mo vu. 
\num{2} kiVLuterada, keLamaTaTxda iMDiyA rababxru. 
\enum
\emng
\eentry

\bentry
\word{Negrohood}
\pron{niVgorxVhuDf}
\gl{\nA}
\bmng
niVgorxVtana; niVgorxV Agiruvike. 
\emng
\eentry

\bentry
\word[Negroid(1)]{Negroid}
\pron{niVgArxyfDx}
\gl{\gu}
\bmng
niVgorxV mAdariya; \kanmu\ kapupx camaR, oraTu kUdalu, capapxTeTx mUgu, \mo vu iruva. 
\emng
\eentry

\bentry
\word[Negroid(2)]{Negroid}
\pron{niVgArxyfDx}
\gl{\nA}
\bmng
niVgorxV buDakaTiTxnavanu; niVgorxV mAdariyavanu. 
\emng
\eentry

\bentry
\word{Negroidal}
\pron{niVgArxyaDxlf}
\gl{\gu}
\bmng
= \hyperlink{Negroid(1)}{$^1$Negroid}. 
\emng
\eentry

\bentry
\word{Negroism}
\pron{niVgorxVisaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} niVgorxVvAda; niVgorxV janara hitAsakitxgaLanunx, hakukxgaLanunx -- samathiRsuvudu, etitx hiDiyuvudu. 
\num{2} niVgorxV nuDigaTuTx; (\kanmu\ niVgorxV janaru baLasuva) nuDigaTuTx \mo vu. 
\enum
\emng
\eentry

\bentry
\word[Negrophil(1)]{Negrophil}
\pron{niVgorxVphilf}
\gl{\gu}
\bmng
niVgorxV perxVmavuLaLx; niVgorxV perxVmiyAda. 
\emng
\eentry

\bentry
\word[Negrophil(2)]{Negrophil}
\pron{niVgorxVphilf}
\gl{\nA}
\bmng
niVgorxV perxVmi; niVgorxVgaLanunx pirxVtiyiMda kANuvavanu. 
\emng
\eentry

\bentry
\word[Negrophile(1)]{Negrophile}
\pron{niVgorxVPeYlf}
\gl{\gu}
\bmng
= \hyperlink{Negrophil(1)}{$^1$Negrophil}. 
\emng
\eentry

\bentry
\word[Negrophile(2)]{Negrophile}
\pron{niVgorxVpheYlf}
\gl{\nA}
\bmng
= \hyperlink{Negrophil(2)}{$^2$Negrophil}. 
\emng
\eentry

\bentry
\word{Negrophobia}
\pron{niVgorxVphoVbia}
\gl{\nA}
\bmng
niVgorxV devxVSa; niVgorxVgaLa meVle badadhx devxVSa. 
\emng
\eentry

\bentry
\word{Negus}
\pron{niVgasf}
\gl{\nA}
\bmng
(\ca) ithiyoVpiyA (yA abisiVniya)da rAja. 
\emng
\eentry

\bentry
\word{negus}
\pron{niVgasf}
\gl{\nA}
\bmng
sakakxre, niMbehaNuNx, jAyikAyi matutx bisiniVru seVrisida dArxkASx madayx. 
\emng
\eentry

\bentry
\wordnospeech{Neh.}{Neh.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{Nehemiah} (haLeya oDaMbaDike). 
\emng
\eentry

\bentry
\word[neigh(1)]{neigh}
\pron{neV}
\gl{\akirx}
\bmng
(kudureyaMte) kene; heVSArava mADu. 
\emng
\eentry

\bentry
\word[neigh(2)]{neigh}
\pron{}
\gl{\nA}
\bmng
(kudureya) keneta; heVSArava. 
\emng
\eentry

\bentry
\word[neighbor(1)]{neighbor}
\pron{}
\gl{\nA}
\bmng
 (\ame) = \hyperlink{neighbour(1)}{$^1$neighbour}. 
\emng
\eentry

\bentry
\word[neighbor(2)]{neighbor}
\pron{neVbarf}
\gl{\gu}
\bmng
(\ame)= \hyperlink{neighbour(2)}{$^2$neighbour}. 
\emng
\eentry

\bentry
\word[neighbor(3)]{neighbor}
\pron{neVbarf}
\gl{\kirx}
\bmng
(\ame)= \hyperlink{neighbour(3)}{$^3$neighbour}. 
\emng
\eentry

\bentry
\word{neighborhood}
\pron{neVbarfhuDf}
\gl{\nA}
\bmng
(\ame) = \hyperlink{neighbourhood}{neighbourhood}. 
\emng
\eentry

\bentry
\word[neighbour(1)]{neighbour}
\pron{neVbarf}
\gl{\nA}
\bmng
\bnum
\num{1} nereyavanu; pakakxda maneyavanu, adeV biVdiyavanu, adeV haLiLxyavanu, adeV jilelxyavanu, pakakxda nADinavanu: \eng{our neighbours across the channel} kaDalAgxluveyAceya namamx nere deVshadavaru. 
\num{2} nerehoreyavanu; senxVhaBAvadiMda vishAvxsadiMda irabeVkAdavanu: \eng{good neighbour} oLeLx nereyavanu. 
\num{3} pakakxdavanu; hatitxradavanu; pakakxdudx; hatitxradalilxruvudu: \eng{my neighbour at dinner} nananx pakakxdalilx UTa mADutitxruvavanu; UTa mADutitxruva nananx pakakxdavanu. \eng{the falling tree brought down its neighbour} maravu tAnu biVLuvAga tananx pakakxda maravanUnx keDavitu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{duty to one's neighbour} manuSayxnu manuSayxnige mADabeVkAda kataRvayx, upakAra. 
\emng
\eentry

\bentry
\word[neighbour(2)]{neighbour}
\pron{neVbarf}
\gl{\gu}
\bmng
pakakxda; baLiya; nereya: \eng{one of our neighbour nations} namamx nere rASaTxrXgaLalolxMdu. 
\emng
\eentry

\bentry
\word[neighbour(3)]{neighbour}
\pron{neVbarf}
\gl{\sakirx}
\bmng
\bnum
\num{1} hatitxradalilxru; pakakxdalilxru; samiVpadalilxru. 
\num{2} hatitxrakekx taru; pakakxdalilxDu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} hatitxradalilx vAsavAgiru. 
\num{2} senxVhadiMdiru. 
\enum
\emng
\eentry

\bentry
\word{neighbourhood}
\pron{neVbarfhuDf}
\gl{\nA}
\bmng
\bnum
\num{1} nerehore; akakxpakakxdavaru; nere parxdeVshadavaru. 
\num{2} (oMdu paTaTxNa yA nagaradoLagina) samudAyadavanu; hatitxra parxdeVshadavaru: \eng{the whole neighbourhood was there} hatitxra parxdeVshadavarelalxrU alilx seVridadxru. 
\num{3} hatitxra hatitxra: \eng{in the neighbourhood of Rs. 1,000} hatitxra hatitxra sAvira rUpAyigaLu. 
\num{4} senxVhaBAva; senxVhada naDate. 
\num{5} hatitxradalilxruvike; sAmiVpayx; \eng{the neighbourhood of the airport} vimAna nilAdxNada sAmiVpayx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{neighbourhood watch}{neighbourhood watch}
\pron{?}
\gl{\nA}
\bmng
nerehore kAvalu; baDAvaNe \mo vugalilx Asitx pAsitx naSaTxvAgadaMte sathxLiVya janaru mADuva pahare, kAvalu. 
\emng
\eentry

\bentry
\word{neighbouring}
\pron{neVbariMgf}
\gl{\gu}
\bmng
hatitxrada; pakakxda; nerehoreya: \eng{in the neighbouring village} pakakxda haLiLxyalilx. 
\emng
\eentry

\bentry
\word{neighbourless}
\pron{neVbarflisf}
\gl{\gu}
\bmng
nerehoreyilalxda; akakxpakakxdalilx yArU ilalxdiruvaMtha. 
\emng
\eentry

\bentry
\word{neighbourliness}
\pron{neVbarflinisf}
\gl{\nA}
\bmng
\bnum
\num{1} nerehorake; nerehoreyavarige takukxdAgiruvike. 
\num{2}senxVhadiMdiruvike. 
\enum
\emng
\eentry


\bentry
\word{neighbourly}
\pron{neVbarfli}
\gl{\gu}
\bmng
\bnum
\num{1} nerehoreya; nerehoreyavarige yoVgayxvAda, takukxdAda. 
\num{2} senxVhada; vishAvxsapUvaRkavAda. 
\enum
\emng
\eentry

\bentry
\word{neighbourship}
\pron{neVbarfSipf}
\gl{\nA}
\bmng
\bnum
\num{1} nerehorake; nerehoreyAgiruvudu. 
\num{2} sAmiVpayx; hatitxravAgiruvike. 
\num{3} nerehore saMbaMdha; akakxpakakxda senxVha. 
\enum
\emng
\eentry

\bentry
\word[neither(1)]{neither}
\pron{neY(niV)darf}
\gl{\kirxvi}
\bmng
\bnum
\num{1} \eng{(nor} jateyalilxruvAga \eng{either} eMbudara niSeVdha pada) eraDaralilx oMdU alalx: \eng{would neither come in nor go out} oLakekx baralU olalx, horakekx hoVgalU olalx. 
\num{2} (\ashi) (hiMdina niSeVdhAthaRvanunx otitx heVLuvAga) \eng{I don't know that neither} nanage adU gotitxlalx. 
\num{3} eraDaralilx oMdU alalx: \eng{he did not go, neither did I} avanu hoVgalilalx, nAnU saha hoVgalilalx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{neither fish nor flesh nor fowl} (yAva jAtigU seVrada eDabiDaMgi eMbathaRdalilx) miVnU alalx, maqgavU alalx, pakiSxyU alalx. 
\num{2} \eng{neither} \hyperref{kandict_h.pdf}{H}{here(1) pagu(12)}{$^1$here} \eng{nor there.} 
\enum
\emng
\eentry

\bentry
\word[neither(2)]{neither}
\pron{neY(niV)darf}
\gl{\saMavayx}
\bmng
(\pArxparx) jotege alalx yA ilalx: \eng{I know not, neither can I guess} nanage gotitxlalx. jotege UhisalU sAdhayxvilalx. 
\emng
\eentry

\bentry
\word[neither(3)]{neither}
\pron{neY(niV)darf}
\gl{\gu}
\bmng
(eraDu yA hecucx iruvudaralilx) yAvudU alalx; idU alalx adU alalx: \eng{neither of them knows} ibabxralilx obabxnigU gotitxlalx. 
\emng
\eentry

\bentry
\word[neither(4)]{neither}
\pron{neY(niV)darf}
\gl{\sanA}
\bmng
(shithilavAgi) (nideRVshisida eSoTxV vasutxgaLalilx) yAvudU alalx; \eng{neither of his visitors saw him} BeVTikAraralilx yArobabxnU avananunx noVDalilalx. 
\emng
\eentry

\bentry
\word{nek}
\pron{nekf}
\gl{\nA}
\bmng
(\da\ APirxka) = \hyperref{kandict_c.pdf}{C}{col(1)}{col(1)}. 
\emng
\eentry

\bentry
\word{nekton}
\pron{nekaTxnf}
\gl{\nA}
\bmng
(\pArxvi) nekaTxnf; sAgara, saroVvaragaLa beVre beVre ALagaLalilx savxtaMtarxvAgi Ijabalalx pArxNirAshi. 
\emng
\eentry

\bentry
\word{nelly}
\pron{neli}
\gl{\nA}
\expl{(\bava\ \eng{nellies}).}
\bmng
\bnum
\num{1} nelilx; peTirxlf eMba kaDala hakikxgaLalilx atayxdhika gAtarxda parxBeVda. 
\num{2} heNiNxga; heNaNxpipx (manuSayx). 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{not on your nelly} (\birx) (\ashi) KaMDita ilalx. 
\emng
\eentry

\bentry
\word{nelson}
\pron{nelasxnf}
\gl{\nA}
\bmng
(kusitxyalilx) `nelasxnf' paTuTx; hiMdiniMda edurALiya toVLa keLage tananx toVLanunx tUrisi keYyiMda avana taleyanunx amukuvudu. 
\emng
\eentry

\bentry
\word{nelumbo}
\pron{nilaMboV}
\gl{\nA}
\expl{(\bava\ \eng{nelumbos}).}
\bmng
(\savi) nilaMboV; tAvareyanonxLagoMDa sasayxkula. 
\emng
\eentry

\bentry
\word{nematocyst}
\pron{nimAYxTasisfTx, nemaTasisfTx}
\gl{\nA}
\bmng
(\savi) nemaTosisfTx; loVLe miVnu muMtAda silaMTareVTf jiVvigaLalilx kaMDu baruva, kuTukalu baLasuva dArada suruLi iruva koVsha. 
\emng
\eentry

\bentry
\word[nematode(1)]{nematode}
\pron{nemaToVDf}
\gl{\nA}
\bmng
(\jiVvi) nemaToVDf; valayagaLilalxda uruLeyAkArada udadxvAda huLugaLanonxLagoMDa oMdu jiVvivagaR. 
\emng
\eentry

\bentry
\word[nematode(2)]{nematode}
\pron{nemaToVDf}
\gl{\gu}
\bmng
nemaToVDf vagaRda. 
\emng
\eentry

\bentry
\word{Nembutal}
\pron{neMbuyxTAlf}
\gl{\nA}
\bmng
(\ravi) neMbuyxTAlf; upashamanakAriyAgi baLasuva peMToV bAbiRToVnfna soVDiyamf lavaNada vANijayx hesaru. 
\emng
\eentry

\bentry
\wordf{nem.con.}
\pron{}
\gl{\saMkiSx}
\expl{\Latin} 
\bmng
\engit{nemine contradicente.} 
\emng
\eentry

\bentry
\word[nemertean(1)]{nemertean}
\pron{nimaTiRanf}
\gl{\nA}
\bmng
nimaTiRyanf; nemaTiRya eMba samudarxvAsi huLugaLa vagaRkekx seVrida yAvudeV jiVvi. 
\emng
\eentry

\bentry
\word[nemertean(2)]{nemertean}
\pron{nimaTiRanf}
\gl{\gu}
\bmng
nemaTiRya vagaRda. 
\emng
\eentry

\bentry
\word[nemertine(1)]{nemertine}
\pron{nimaTeYRnf}
\gl{\nA}
\bmng
= \hyperlink{nemertean(1)}{$^1$nemertean}. 
\emng
\eentry

\bentry
\word[nemertine(2)]{nemertine}
\pron{nimaTeYRnf}
\gl{\gu}
\bmng
= \hyperlink{nemertean(2)}{$^2$nemertean}. 
\emng
\eentry

\bentry
\word{nemesia}
\pron{nimiVSaZ}
\gl{\nA}
\bmng
(\savi) nimiVSa: nidiRSaTx AkAravilalxda vividha baNaNxda hU biDuva, \da\ APirxkada sasayx kula. 
\emng
\eentry

\bentry
\wordnospeech{nemisis}{nemisis}
\pron{?}
\gl{\nA}
\expl{(\bava\ \eng{nemises} \ucAcx-- nemisiVsf).}
\bmng
\bnum
\num{1} takakx shAsitx; duSakxmaR Pala; nAyxyavAda shikeSx. 
\num{2} (parxtiVkAra deVvateyu niVDuva) nAyxyavAda shikeSx. parxtiVkAra nAyxya, 
\num{3} parxtiVkAra deVvate. 
\enum
\emng
\eentry

\bentry
\wordf{nemine contradicente}
\pron{ne(V)mini kAMTarxDiseMTi}
\gl{\kirxvi}
\expl{\Latin}
\bmng
aviroVdhavAgi; yAra AkeSxVpaNeyU ilalxde; savARnumatadiMda. 
\emng
\eentry

\bentry
\word{neo-}
\pron{niVO-}
\gl{\sapUpa}
\bmng
\bnum
\num{1} hosadAda; Adhunika; navayx; nava. 
\num{2} punarujijxVvitavAda. 
\enum
\emng
\eentry

\bentry
\word{neo-Cambrian}
\pron{niOkAYxMbirxanf}
\gl{\gu}
\bmng
(\BUvi) nava keVMbirxyanf; keVMbirxyanf avadhiya yA keVMbirxyanf satxrada utatxrAdhaRda, adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neoclassic}
\pron{niVOkAlx Yxsikf }
\gl{\gu}
\bmng
= \hyperlink{neoclassical}{neoclassical}. 
\emng
\eentry

\bentry
\word{neoclassical}
\pron{niVOkAlx Yxsikalf}
\gl{\gu}
\bmng
\bnum
\num{1} nava kAlx yxsikalf yugada yA sheqliya. 
\num{2} (sAhitayx, kale, saMgiVta, \mo\ keSxVtarxgaLalilx) kAlx yxsikalf sheqli yA nirUpaNA vidhAnada punarutAthxnada, adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{neoclassicism}
\pron{niVOkAlx YxsisisaZmf}
\gl{\nA}
\bmng
navakAlxsikalf -- vAda, tatatxvX, sidAdhxMta yA saMparxdAya; (\eng{17}neV shatamAnadiMda \eng{19}ra madhayxBAgadavaregina sAhitayx, kale, vAsutx shilapx, \mo vugaLalilx) pArxciVna girxVkf matutx lATinf parxkAragaLa matutx saMparxdAyagaLa 
\banum
\alnum{a} anukaraNa, anusaraNe. 
\alnum{b} punarudaya: punarutAthxna. 
\eanum
\emng
\eentry

\bentry
\word{neoclassicist}
\pron{niVOkAlxYxsisisfTx}
\gl{\nA}
\bmng
nava kAlxsikalfvAdada anuyAyi, paMthi, samathaRka. 
\emng
\eentry

\bentry
\word{neocolonialism}
\pron{niVOkaloVnialisaZmf}
\gl{\nA}
\bmng
navavasAhatushAhi niVti, tatatx vx; AthiRka, rAjakiVya yA beVre shakitx parxBAvagaLanunx baLasikoMDu beVre deVshagaLa meVle (\kanmu\ hiMdina vasAhatugaLa meVle) parxBAva biVruva niVti, tatatx vx. 
\emng
\eentry

\bentry
\word[neocolonialist(1)]{neocolonialist}
\pron{niVOkaloVnialisfTx}
\gl{\nA}
\bmng
navavasAhatushAhi vAdi, tatatx vxsamathaRka; AthiRka, rAjakiVya yA beVre shakitx parxBAvagaLanunx baLasikoMDu beVre deVshagaLa meVle (\kanmu\ hiMdina vasAhatugaLa meVle) parxBAva biVruva niVtiyanunx samathiRsuvava. 
\emng
\eentry

\bentry
\word[neocolonialist(2)]{neocolonialist}
\pron{niVOkaloVnialisfTx}
\gl{\gu}
\bmng
nava vasAhatushAhi niVtige, tatatx vxkekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neodymium}
\pron{niVaDimiamf}
\gl{\nA}
\bmng
(\ravi) niyoVDimiyamf; paramANu saMKeyx \eng{60}, paramANu tUka \eng{144.24} uLaLx, lAyxMtaneqDf sherxVNige seVrida viraLa BasamxdhAtu (\saMkeV\ \eng{Nd.}) 
\emng
\eentry

\bentry
\word{neolithic}
\pron{niValitikf}
\gl{\gu}
\bmng
(ujijx mADida yA sANe hiDidu merugu koTaTx shilAyudhagaLanunx baLasutitxdadx) navashilAyugada. 
\emng
\eentry

\bentry
\word[neologian(1)]{neologian}
\pron{niValoVjianf}
\gl{\nA}
\bmng
(\deVva) navasidAdhxMti; hosa yA tAkiRka sidAdhxMta parxtipAdaka. 
\emng
\eentry

\bentry
\word[neologian(2)]{neologian}
\pron{niValoVjianf}
\gl{\gu}
\bmng
(\deVva) naviVna sidAdhxMtada; hosa sidAdhxMtada yA adara kaDe olaviruva. 
\emng
\eentry

\bentry
\word{neologise}
\pron{niVAlajeYsfZ}
\gl{\akirx}
\bmng
= \hyperlink{neologize}{neologize}. 
\emng
\eentry

\bentry
\word{neologism}
\pron{niVAlajisaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} navapada parxyoVga; hosa padagaLa baLake. 
\num{2} navapada; hosa shabadx; hosadAgi saqSiTxsida, racisida pada. 
\num{3} naviVna sidAdhxMta; naviVna mata; matadhamaRda bagegx hosa yA tAkiRka parxtipAdane yA adara kaDe olavu. 
\enum
\emng
\eentry

\bentry
\word{neologist}
\pron{niVAlajisfTx}
\gl{\nA}
\bmng
\bnum
\num{1} navapada parxyoVgisuvavanu. 
\num{2} shabadxbarxhamx; padabarxhamx; padagaLanunx saqSiTxsuvavanu. 
\num{3} naviVna sidAdhxMti; matadhamaRdalilx hosa sidAdhxMtagaLanunx parxtipAdisuvavanu yA avugaLa kaDe olaviruvavanu. 
\enum
\emng
\eentry

\bentry
\word{neologize}
\pron{niVAlajeYsfZ}
\gl{\akirx}
\bmng
\bnum
\num{1} hosa pada saqSiTxsu. 
\num{2} hosa pada baLasu. 
\num{3} mata dhamaRdalilx hosa sidAdhxMtagaLanunx parxtipAdisu yA avugaLa kaDe olavu hoMdu. 
\enum
\emng
\eentry

\bentry
\word{neomycin}
\pron{niVOmeYsinf}
\gl{\nA}
\bmng
(\veYshA) niyomeYsinf; seTxrXpoTxVmeYsinf baLagada oMdu vishAla roVhita parxtijiVvaka. 
\emng
\eentry

\bentry
\word{neon}
\pron{niVAnf}
\gl{\nA}
\bmng
(\ravi) niyAnf; paramANu saMKeyx \eng{10} paramANu tUka \eng{20} uLaLx, jaDAnilagaLa vagaRkekx seVrida dhAtu. 
\emng
\eentry

\bentry
\word{neonatal}
\pron{niVAneVTalf}
\gl{\gu}
\bmng
Aga tAne janisida maguvina; hosadAgi huTiTxda maguvige saMbaMdhisida; navajAta shishuvina. 
\emng
\eentry

\bentry
\word{neonate}
\pron{neVaneVTf}
\gl{\nA}
\bmng
hosadAgi huTiTxda, Aga tAne huTiTxda -- magu: navajAta shishu. 
\emng
\eentry

\bentry
\word{neontologist}
\pron{niVAMTAlajisfTx}
\gl{\nA}
\bmng
avinaSaTx jiVvishAsatxrXjacnx; vaMshanaSaTxvAgadiruva, inUnx asitxtavxdalilxruva jiVvigaLa adhayxyana naDesuva vijAcnxni. 
\emng
\eentry

\bentry
\word{neontology}
\pron{niVAMTAlaji}
\gl{\nA}
\bmng
avinaSaTx jiVvishAsatxrX; vaMshanaSaTxvAgadiruva, inUnx asitxtavxdalilxruva jiVvigaLa adhayxyana naDesuva vijAcnxna shAKe. 
\emng
\eentry

\bentry
\word{neopentane}
\pron{niVOpeMTeVnf}
\gl{nA}
\bmng
(\ravi) niyoVpeMTeVnf; aidu kAbaRnf paramANugaLuLaLx AlekxVnf vagaRda heYDorxVkAbaRninxna samAMgigaLalolxMdu. 
\emng
\eentry

\bentry
\word{neophron}
\pron{niVaphArxnf}
\gl{\nA}
\bmng
IjipiTxna biLi raNahadudx, shevxVtagaqdharx yA adanonxLagoMDa pakiSxkula. 
\emng
\eentry

\bentry
\word{neophyte}
\pron{niVapheYTf}
\gl{\nA}
\bmng
\bnum
\num{1} (\kanmu\ Adima kerxYsatxralilx yA \roVkAyx ralilx) navaparxveVshi; hosadAgi matakekx seVridavanu. 
\num{2} navadiVkiSxta; hosadAgi diVkeSx paDeda \roVkAyx\ pAdirx. 
\num{3} navashiSayx; (oMdu mata paMgaDakekx) hosadAgi seVridavanu. 
\num{4} hosab; ananuBavi; AraMBa desheyalilxruvavanu. 
\enum
\emng
\eentry

\bentry
\word{neoplasia}
\pron{niVapelxVsiZa}
\gl{\nA}
\bmng
navoVtakate; deVhada yAvudeV BAgadalilx hosadAgi Utaka, \kanmu\ geDeDx, rUpugoLuLxvudu. 
\emng
\eentry

\bentry
\word{neoplasm}
\pron{niVOpAlx YxsaZmf}
\gl{\nA}
\bmng
navoVtaka; deVhada yAvudeV BAgadalilx hosadAgi rUpugoLuLxva Utaka, \kanmu\ geDeDx. 
\emng
\eentry

\bentry
\word{neoplastic}
\pron{niVOpAlx YxsiTxkf}
\gl{\gu}
\bmng
navoVtakada; hosadAgi rUpugoMDa Utakada, \kanmu\ geDeDxya, yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{Neoplatonic}
\pron{niVOpalxTAnikf}
\gl{\gu}
\bmng
\bnum
\num{1} navapelxVToVvAdada yA adakekx saMbaMdhisida. 
\num{2} navapelxVToVvAdiya yA avanige saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{Neoplatonism}
\pron{niVOpelxVTanisaZmf}
\gl{\nA}
\bmng
nava pelxVToVvAda; kirx. sha. \eng{3}neV shatamAnadalilx polxVTeYnasana anuyAyigaLu pelxVToVna ciMtaneyanunx pwrasatxyX anuBAvadoMdige saMyoVjisi beLesida dAshaRnika matutx dhAmiRka sidAdhxMta. 
\emng
\eentry

\bentry
\word{Neoplatonist}
\pron{niVOpelxVTanisfTx}
\gl{\nA}
\bmng
navapelxVToVvAdi; navapelxVToV sidAdhxMtada anuyAyi. 
\emng
\eentry

\bentry
\word{neoprene}
\pron{niVOpirxVnf}
\gl{\nA}
\bmng
(\ravi) niyopirxVnf; kolxVroVpirxVnina pAlimariVkaraNadiMda tayArisuva kaqtaka rababxru. 
\emng
\eentry

\bentry
\word{neo-scholasticism}
\pron{niVOsakxlAYxsiTxsisaZmf}
\gl{\nA}
\bmng
nava sokxlAYxsiTxkf tatatx vx; madhayxyugada takaR shAsatxrXjacnxra vicAra paraMpareyanunx punashecxYtanayxgoLisuvike, matetx parxtipAdisuvike. 
\emng
\eentry

\bentry
\word{neotenic}
\pron{niATenikf}
\gl{\gu}
\bmng
(\jiVvi) 
\banum
\alnum{a} vayasakx sithxtiyalilxyU sheYshavada lakaSxNagaLu uLidiruva. 
\alnum{b} lAvaR haMtadalilxyeV leYMgika pakavxte paDediruva. 
\eanum
\emng
\eentry

\bentry
\word{neotenous}
\pron{niATinasf}
\gl{\gu}
\bmng
= \hyperlink{neotenic}{$^1$neotenic}. 
\emng
\eentry

\bentry
\word{neoteny}
\pron{niATini}
\gl{\nA}
\bmng
(\jiVvi) niyATini: 
\banum
\alnum{a} lAvaR haMtadalilxyeV leYMgika pakavxte paDeyuvudu. 
\alnum{b} vayasakx sithxtiyalilxyU sheYshavada lakaSxNagaLu uLidiruvudu. 
\eanum
\emng
\eentry

\bentry
\word{neoteric}
\pron{niVaTerikf}
\gl{\gu}
\bmng
(sAhitayx) Icina; Adhunika; naviVna; navayxriVtiya; navakalipxta. 
\emng
\eentry

\bentry
\word{neotropical}
\pron{niVOTArxpikalf}
\gl{\gu}
\bmng
nava uSaNxvalayada; dakiSxNa hAgU uSaNxvalayada amerikakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{Neozoic}
\pron{niVaseZ\char'261 Vikf}
\gl{\gu}
\bmng
(\BUvi) nava jiVviyugada; BUmiya itihAsadalilx pArxciVna jiVviyuga athavA madhayxjiVvi yugada taruvAyada. 
\emng
\eentry

\bentry
\word[Nepalese(1)]{Nepalese}
\pron{nepaliVsfZ}
\gl{\gu}
\bmng
(\bava\ adeV) = \hyperlink{Nepali(1)}{$^1$Nepali}. 
\emng
\eentry

\bentry
\word[Nepalese(2)]{Nepalese}
\pron{nepaliVsfZ}
\gl{\nA}
\bmng
(\bava\ adeV) = \hyperlink{Nepali(2)}{$^2$Nepali}. 
\emng
\eentry

\bentry
\word[Nepali(1)]{Nepali}
\pron{nipAli}
\gl{\gu}
\bmng
neVpALada; neVpALakekx, neVpALada janakekx yA BASege saMbaMdhisida. 
\emng
\eentry

\bentry
\word[Nepali(2)]{Nepali}
\pron{nipAli}
\gl{\nA}
\expl{(\bava\ adeV yA \eng{Nepalis}).}
\bmng
\bnum
\num{1} neVpALi; neVpALadavanu; neVpALadalilx huTiTxdavanu yA neVpALada deVshiVya. 
\num{2} neVpALi(BASe). 
\enum
\emng
\eentry

\bentry
\word{nepenthe}
\pron{nipenitx}
\gl{\nA}
\bmng
= \hyperlink{nepenthes}{nepenthes\((1)\)}. 
\emng
\eentry

\bentry
\word{nepenthes}
\pron{nipenitxVsfZ}
\ga{\nA}\bmng
\bnum
\num{1} (\kAparx) duHKa visAmxraka; duHKavanunx mareyuvaMte mADuva auSadha. 
\num{2} (\savi) nepeMtiVsf; hUji giDavanonxLagoMDa sasayxkula. 
\enum
\emng
\eentry

\bentry
\word{nephelometer}
\pron{nephalAmiTarf}
\gl{\nA}
\bmng
aBarxmApaka; aBarxmApi; darxvadalilx vilaMbitavAgiruva atisUkaSxmX kaNagaLa pariNAmavAgi darxva bagagxDavAgidAdxga A bagagxDateya maTaTxvanunx aLeyuva, A mUlaka yAvudeV rAsAyanikada sArateyanunx niNaRyisuva upakaraNa. 
\emng
\eentry

\bentry
\word{nephelometric}
\pron{nephalAmeTirXkf}
\gl{\gu}
\bmng
aBarxmApakada; darxvada bagagxDateyanunx aLeyuva yA A mUlaka rAsAyanikagaLa sArateyanunx niNaRyisuva. 
\emng
\eentry

\bentry
\word{nephelometry}
\pron{nephalAmiTirx}
\gl{\nA}
\bmng
aBarxmApana: darxvada bagagxDateyanunx aLeyuvudu. 
\emng
\eentry

\bentry
\word{nephew}
\pron{nevUZyx, nephUyx}
\gl{\nA}
\bmng
\bnum
\num{1} aNaNxna yA tamamxna maga. 
\num{2} soVdaraLiya; akakxna yA taMgiya maga. 
\enum
\emng
\eentry

\bentry
\word{nephology}
\pron{niphAlaji}
\gl{\nA}
\bmng
meVGashAsatxrX: moVDagaLanunx kurita shAsitxrXVya adhayxyana. 
\emng
\eentry

\bentry
\word{nephrite}
\pron{nepherx YTf}
\gl{\nA}
\bmng
 = \hyperref{kandict_j.pdf}{J}{jade(3)1}{$^3$jade(l)}. 
\emng
\eentry

\bentry
\word{nephritic}
\pron{niphirxTikf}
\gl{\gu}
\bmng
\bnum
\num{1} mUtarxpiMDada yA mUtarxpiMDadalilxruva. 
\num{2} mUtarxpiMDada uriyUtada yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{nephritis}
\pron{nipherx YTisf}
\gl{\nA}
\bmng
mUtarxpiMDada uriyUta. 
\emng
\eentry

\bentry
\word{nephr(o)-}
\pron{nephorxV-}
\gl{\sapUpa}
\bmng
mUtarxpiMDada yA adakekx saMbaMdhisida eMbathaRdalilx baLasuva \sapUpa. 
\emng
\eentry

\bentry
\word{nephrectomy}
\pron{nepherxkaTxmi}
\gl{\nA}
\bmng
(\veYshA) mUtarxpiMDoVceCxVdana; mUtarxpiMDakaqMtana; mUtarxpiMDavanunx katatxrisi tegeduhAkuva shasatxrXkirxye. 
\emng
\eentry

\bentry
\word{nephrologist}
\pron{nephArxlajisfTx}
\gl{\nA}
\bmng
(\veYshA) mUtarxpiMDashAsatxrXjacnx; mUtarxpiMDashAsatxrXdalilx tajacnx. 
\emng
\eentry

\bentry
\word{nephrology}
\pron{nephArxlaji}
\gl{\nA}
\bmng
(\veYshA) mUtarxpiMDashAsatxrX; mUtarxpiMDada racane, kirxye, \mo vugaLa shAsitxrXVya adhayxyana. 
\emng
\eentry

\bentry
\word{nephrotomy}
\pron{nephArxTami}
\gl{\nA}
\bmng
(\veYshA) mUtarxpiMDakaqMtana; mUtarxpiMDaCeVdana; (kalulx \mo vanunx tegeyalu) mUtarxpiMDavanunx koyuyxva shasatxrXkirxye. 
\emng
\eentry

\bentry
\wordf{ne plus ultra}
\pron{neV pulxsf ulATxrX}
\gl{\nA}
\expl{\Latin}
\bmng
\bnum
\num{1} muMde hoVgabeVDaveMba Ajecnx. 
\num{2} dATalAgada aDacaNe. 
\num{3} mucicxda yA mucacxlAguva tutatxtudi. 
\num{4} paramAvadhi; parAkASeThx; paramoVnanxti. 
\enum
\emng
\eentry

\bentry
\word{nepotism}
\pron{nepaTisaZmf}
\gl{\nA}
\bmng
savxjanapakaSxpAta; meVladhikAradalilxruvavaru hatitxrada baMdhu janarige (yoVgayxte gaNisade) toVruva pakaSxpAta. 
\emng
\eentry

\bentry
\word{nepotist}
\pron{nepaTisfTx}
\gl{\nA}
\bmng
savxjanapakaSxpAti. 
\emng
\eentry

\bentry
\word{nepotistic}
\pron{nepaTisiTxkf}
\gl{\gu}
\bmng
savxjanapakaSxpAtada yA savxjanapakaSxpAtigaLige saMbaMdhisida. 
\emng
\eentry

\bentry
\word{Neptune}
\pron{nepfTUyxnf}
\gl{\nA}
\bmng
\bnum
\num{1} (pArxciVna roVmanara) samudarx deVvate. 
\num{2} nepUcxnf; swravUyxhadalilx yureVnasf matutx pUlxToVgaLa naDuve iruva garxha. 
\enum
\emng
\eentry

\bentry
\word[Neptunian(1)]{Neptunian}
\pron{nepfTUyxnianf}
\gl{\gu}
\bmng
\bnum
\num{1} (\BUvi) jalaja; niVrina kirxyeyiMda udaBxvisida. 
\num{2} (vayxkitxya \vi) jalaja vAdada yA vAdiya; nidiRSaTx shilegaLu jalamUlaveMdu vAdisuva. 
\enum
\emng
\eentry

\bentry
\word[Neptunian(2)]{Neptunian}
\pron{nepfTUyxnianf}
\gl{\nA}
\bmng
jalajavAdi; nidiRSaTx shilegaLu jalamUladaveMdu vAdisuvava(Lu). 
\emng
\eentry

\bentry
\wordnospeech{neptunium}{neptunium}
\pron{nepfTUyxniamf}
\gl{\nA}
\bmng
(\ravi) nepfTUyxniyamf; \eng{238} tUkada yureVniyamf paramANu biVjavu nUyxTArxnf tADanege guriyAdAga utapxtitx mADuva, \eng{93} paramANu saMKeyxya rAsAyanika dhAtu, \saMkeV \eng{Np.} 
\emng
\eentry

\bentry
\wordnospeech{NERC}{NERC}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{Natural Environmental Research Council.} 
\emng
\eentry

\bentry
\word{nerd}
\pron{naDfR}
\gl{\nA}
\bmng
(\kanmu\ \ame) (\ashi) daDaDx yA niVrasa vayxkitx. 
\emng
\eentry

\bentry
\word{nereid}
\pron{niari{i}Df}
\gl{\nA}
\bmng
\bnum
\num{1} samudarx(sitxrXV) deVvate. 
\num{2} (\pArxvi\ matutx \jiVvi) niriyiDf; niriVyasf kulakekx seVrida, udadxvAda samudarxhuLu. 
\enum
\emng
\eentry

\bentry
\word{nerine}
\pron{nireYni}
\gl{\nA}
\bmng
(\savi) nireYni; suMdaravAda keMpu yA pATala vaNaRda hU biDuva, \da APirxkada sasayxkula. 
\emng
\eentry

\bentry
\wordf{nero antico}
\pron{neVroV AMTiVkoV}
\gl{\nA}
\expl{\It}
\bmng
(hALubidadx roVmanf avasheVSagaLa parxdeVshadalilx sikukxva) oMdu riVtiya kariya amaqta shile, kapupxhAlugalulx. 
\emng
\eentry

\bentry
\word{neroli}
\pron{niarali}
\gl{\nA}
\bmng
= \hyperlink{neroli oil}{neroli oil}. 
\emng
\eentry

\bentry
\wordRemoveSpace{neroli-oil}{neroli oil}
\pron{?}
\gl{\nA}
\bmng
niVroVli kitatxLe hUgaLiMda tegeda suvAsanA teYla. 
\emng
\eentry

\bentry
\word{Neronian}
\pron{niroVnianf}
\gl{\gu}
\bmng
\bnum
\num{1} (roVmanf cakarxvatiR) niVroVvina yA niVroVvinaMtha. 
\num{2} kUrxra; nidaRya; hiMsaka. 
\num{3} sevxVcACxcAri. 
\enum
\emng
\eentry

\bentry
\word{nervate}
\pron{naveRVTf}
\gl{\gu}
\bmng
(eleya \vi) narayukatx; naragaLiruva. 
\emng
\eentry

\bentry
\word{nervation}
\pron{naveRVSanf}
\gl{\nA}
\bmng
(eleya \vi) narayukatxte; naragaLiruvike. 
\emng
\eentry

\bentry
\word[nerve(1)]{nerve}
\pron{navfR}
\gl{\nA}
\bmng
\bnum
\num{1} nara; deVhada yAvudeV BAgadiMda miduLigAgali benunxhurigAgali saMveVdaneyanunx sAgisabalalx taMtu yA taMtugaLa kaMte yA avugaLanunx rUpisiruva padAthaR. 
\numi{2} (\bava dalilx) 
\banum
\alnum{a} deVhada sUkaSxmXveVdane viSayadalUlx miduLina matutx deVhada itara BAgagaLa parasapxra kirxyA pariNAmada viSayadalUlx iruva deVhada sithxti. 
\alnum{b} naroVderxVka; tiVkaSxNXvAda, aneVka veVLe unAmxda keraLisuva naragaLa sithxti. 
\alnum{c} I bageya asavxsathx sithxti. 
\alnum{d} vipariVta sUkaSxmX saMveVdane. 
\eanum
\numie
\num{3} (\kAparx) (\pArxparx) sAnxyu rajujx. 
\num{4} viVyaR; shakitx; kasuvu. 
\num{5} (\savi) nara; eleyalilx edudxkANuva nALa, \kanmu\ toTiTxniMda tudige hoVguva parxdhAna nALa. 
\num{6} apAyadalilx shAMtacitatxte; akuSxbadhxte; dheYyaR; sethx YyaR. 
\num{7} (\AmA) udadhxTatana; dhASaTx yxR; audadxtayx: \eng{he had the nerve to ask for more} inanxSuTx beVkenunxva dhASaTxyXR avanalilxtutx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{a fit of nerves} naragaLa taLamaLa; naragaLa koSxVBe. 
\num{2} \eng{bundle of nerves} uderxVkada mudedx; yAvAgalU atayxMta udirxkatx sithxtiyalilxruva vayxkitx. 
\num{3} \eng{get on one's nerves}(obabxnanunx) kADu; kirikiriyuMTu mADu; manasisxge samAdhAnakoDadaMtiru; AtaMkapaDisuvaMtiru. 
\hypertarget{nerves pagu4}{} 
\num{4} \eng{has iron nerves} gaTiTxmanasisxnavanu; sAmAnayxvAgi adhiVranAguvudilalx; avanige eMtha eDaranUnx tALikoLuLxva sAmathayxRvide. 
\num{5} \eng{has nerves of steel} = \hyperlink{nerves pagu4}{?pagu?\((4)\)}. 
\num{6} \eng{he does not know what nerves are} naroVderxVkavanunx ariyanu (yAvAgalU samAdhAnasithxtiyalilxruvanu). 
\num{7} \eng{lose one's nerves} dheYyaR tapipxhoVgu; edeguMdu. 
\num{8} \eng{nerve war} = \hyperref{kandict_w.pdf}{W}{war(1) pagu(19)}{$^1$war} \eng{of nerves.} 
\num{9} \eng{strain every nerve} sAdhayxvAda savaRparxyatanxvanUnx mADu. 
\enum
\emng
\eentry

\bentry
\word[nerve(2)]{nerve}
\pron{navfR}
\gl{\sakirx}
\bmng
\bnum
\num{1} shakitx, ceYtanayx, dheYyaR -- koDu: \eng{nerve oneself for a task} kaSaTxda kelasavoMdakekx tanage tAneV shakitx taMduko. 
\num{2} (apAyavanAnxgali veVdaneyanAnxgali edurisalu) dheYyaR taMduko: \eng{nerve oneself ready to face trouble} kaSaTxgaLanenxdurisalu dheYyaR taMduko. 
\enum
\emng
\eentry

\bentry
\word{nerve-cell}
\pron{navfRself}
\gl{\nA}
\bmng
(\pArxvi) narakoVsha; naragaLa Utakadalilxruva, saMveVdanegaLanunx sAgisuva jiVvakoVsha. 
\emng
\eentry

\bentry
\word{nerve-centre}
\pron{navfRseMTarf}
\gl{\nA}
\bmng
\bnum
\num{1} (\pArxvi) narakeVMdarx; parasapxra nikaTa saMbaMdhaviruva nidiRSaTx kataRvayxgaLanunx pAlisuva narakoVshagaLa guMpu. 
\num{2} (saMsethx \mo vugaLa) adhikArakeVMdarx; niyaMtarxNakeVMdarx. 
\enum
\emng
\eentry

\bentry
\word{nerved}
\pron{navfDxR}
\gl{\gu}
\bmng
\bnum
\num{1} (\savi) (ele \mo vugaLa \vi) naragaLiruva; diMDugaLiruva; naDureVKegaLiruva. 
\num{2} (\saupa vAgi) nidiRSaTx lakaSxNada yA saMKeyxya naragaLiruva; naravishiSaTx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nerve gas}{nerve gas}
\pron{?}
\gl{\nA}
\bmng
narAnila naravUyxhakekx hAniyuMTumADuva oMdu viSAnila. 
\emng
\eentry

\bentry
\word{nerve-knot}
\pron{navfRnATf}
\bmng
(\pArxparx) naragarxMthi; narakoVshagaLa samUha. 
\emng
\eentry

\bentry
\word{nerveless}
\pron{navfRlisf}
\gl{\gu}
\bmng
\bnum
\num{1} jaDa; ceYtanayxvilalxda; nirutAsxhada; niviRVyaR; dubaRla. 
\num{2} (sheYliya \vi) bigiyilalxda. saDilavAda; aLaLxkavAda; shithila; satatx vxvilalxda. 
\num{3} (\savi) (ele) nararahita; nADigaLilalxda; AdhAra taMtugaLilalxda. 
\num{4} (kiVTa) rekekxya upanaragaLilalxda. 
\num{5} (\aMrashA matutx \pArxvi) naragaLilalxda; nararahita. 
\enum
\emng
\eentry

\bentry
\word{nervelessly}
\pron{navfRlisfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} niviRVyaRvAgi; dubaRlavAgi; ceYtanayxvilalxde. 
\num{2} (\aMrashA\ matutx \pArxvi) naragaLilalxdaMte; nararahitavAgi. 
\enum
\emng
\eentry

\bentry
\word{nervelessness}
\pron{navfRlisfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} niviVRyaRte; dwbaRlayx; ceYtanayxvilalxdiruvike. 
\num{2} (\aMrashA\ matutx \pArxvi) nararAhitayx. 
\enum
\emng
\eentry

\bentry
\word{nerve-racking}
\pron{navfRrAYxkiMgf}
\gl{\gu}
\bmng
 narapiVDaka; narahiMsaka; naragaLige parxbala parxyAsavanunx, atayxMta kaSaTxkara veVdaneyanunx uMTumADuva. 
\emng
\eentry

\bentry
\word{nervily}
\pron{naviRli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} uderxVkashiVlavAgi; thaTaTxne keraLuvaMte; sulaBavAgi uderxVkagoLuLxvaMte. 
\num{2} (\ame) dhASaTx yxRdiMda; udadhxTatanadiMda. 
\enum
\emng
\eentry

\bentry
\word[nervine(1)]{nervine}
\pron{naveYRnf}
\gl{\gu}
\bmng
(\veYshA) narashAmaka; naragaLa asapxsathxteyanunx shamana mADuva, pariharisuva. 
\emng
\eentry

\bentry
\word[nervine(2)]{nervine}
\pron{naveYRnf}
\gl{\nA}
\bmng
(\veYshA) narashAmaka; naragaLa asavxsathxteyanunx shamana mADuva auSadha. 
\emng
\eentry

\bentry
\word{nerviness}
\pron{naviRnisf}
\gl{\nA}
\bmng
\bnum
\num{1} thaTaTxne keraLuvike; reVguvike. 
\num{2} dhASaTx yxR; udadhxTatana. 
\enum
\emng
\eentry

\bentry
\word{nervo-}
\pron{navoRV-}
\gl{\sapUpa}
\bmng
naragaLu matutx itara aMgagaLige saMbaMdhisida eMbathaRda shabadxgaLa niSapxtitxyalilx baLasuva \pUpa, \udA\ \eng{ nervomuscular} naragaLu matutx mAMsaKaMDagaLige saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nervous}
\pron{navaRsf}
\gl{\gu}
\bmng
\bnum
\num{1} naragaLa yA naragaLiMdAda: \eng{nervous system} naramaMDala; naravUyxha; naragaLu, narakeVMdarxgaLu, muMtAdavugaLa samucacxya. 
\num{2} naragaLa meVle kelasa mADuva. 
\num{3} sUkaSx mxnaragaLuLaLx yA asavxsathx, dubaRla naragaLuLaLx. 
\num{4} reVguva savxBAvada. 
\num{5} uderxVkada; keraLuvikeyiMdAda: \eng{nervous headache} uderxVkada talenoVvu. 
\num{6} (sAhitayx sheYli) satavxvuLaLx; bigiyAda; aDakavAda. 
\num{7} (\pArxparx) sAnxyupUNaR; baladiMda kUDida; shakitxyuta. 
\num{8} asithxravAda; dheYyaRgeTaTx: \eng{nervous moment} asithxragoLisuva samaya. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{central nervous system} keVMdarx naramaMDala, vUyxha; miduLu matutx kasheVruka. 
\num{2} \eng{nervous breakdown} narAGAta; ugarxvAda naradwbaRlayx yA naragaLa asatxvayxsatx sithxti; \sA\ kaDuKinanxte matutx kaLavaLagaLiMda Aguva mAnasika asavxsathxte. 
\num{3} \eng{nervous of doing (something)}(EnanAnxdarU mADuvaSuTx)dheYyaR athavA AtamxvishAvxsavilalxdiru: \eng{I am nervous to drive a racing car} reVsf kAranunx naDesuvaSuTx dheYyaR nanagilalx. 
\num{4} \eng{nervous wreck}(\AmA) mAnasika otatxDa, baLalike, \mo vugaLiMda naraLutitxruvava; mAnasika Baganx. 
\enum
\emng
\eentry

\bentry
\word{nervously}
\pron{navaRsfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} naragaLige saMbaMdhisidaMte. 
\num{2} reVguva yA thaTaTxne keraLuva riVtiyalilx 
\num{3} pukakxluhatitx; adheYyaRdiMda: \eng{smiled nervously} adheYyaRdiMda nakakx. 
\enum
\emng
\eentry

\bentry
\word{nervousness}
\pron{navaRsfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} naroVderxVka; sUkaSx mxnaragaLuLaLx yA asavxsathx. dubaRla naragaLuLaLx sithxti. 
\num{2} hedarike savxBAva. 
\num{3} asithxrate; adheYyaRda sithxti. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nervous system}{nervous system}
\pron{?}
\gl{\nA}
\bmng
nara-vUyxha, maMDala. 
\emng
\eentry

\bentry
\word{nervure}
\pron{navuyxRarf}
\gl{\nA}
\bmng
nara; pataMgada rekekxya cwkaTaTxnunx nimiRsuva yA sasayxda elegaLalilx kANuva yAvudeV nALa, \kanmu\ eleyalilx toTiTxniMda eleya tudige hoVguva nALa. 
\emng
\eentry


\bentry
\word{nervy}
\pron{naviR}
\gl{\gu}
\expl{(\tara\ \eng{nervier}, \tama\ \eng{nerviest}).}
\bmng
\bnum
\num{1} uderxVkada yA thaTaTxne keraLuva. 
\num{2} dhASaTxyXRtanada; udadhxTa. 
\enum
\emng
\eentry

\bentry
\word{nescience}
\pron{nesianfsx}
\gl{\nA}
\bmng
(sAhitayx) (yAvudoV oMdara) arivilalxdiruvudu; ajAcnxna. 
\emng
\eentry

\bentry
\word[nescient(1)]{nescient}
\pron{nesiaMTf}
\gl{\gu}
\bmng
\bnum
\num{1} (oMdara) jAcnxnavilalxda; arivilalxda. 
\num{2} ajecnxVyateya. 
\enum
\emng
\eentry

\bentry
\word[nescient(2)]{nescient}
\pron{nesiaMTf}
\gl{\nA}
\bmng
ajecnxVyatAvAdi. 
\emng
\eentry

\bentry
\word{ness}
\pron{nesf}
\gl{\nA}
\bmng
BUshira; nelacAcu; samudarxdoLakekx cAcikoMDiruva BUmiya BAga. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ness}{-ness}
\pron{-nisf}
\gl{\uparx}
\bmng
`iruva sithxti' modalAda athaRda shabadxgaLa racaneyalilx baLasuva \uparx: \eng{bitterness} kahi. 
\emng
\eentry

\bentry
\word[nest(1)]{nest}
\pron{nesfTx}
\gl{\nA}
\bmng
\bnum
\num{1} (hakikxgaLa) gUDu; niVDa. 
\numi{2} (pArxNiya, kiVTada) 
\banum
\alnum{a} ikekx; hakekx; moTeTx hAkuva, mari mADuva sathxLa. 
\alnum{b} AvAsa; nivAsa; AsharxyasAthxna; malaguva sathxLa. 
\eanum
\numie
\num{3} suKakaravAda, anukUlavAda -- EkAMtasathxLa, vivikatx sAthxna. 
\num{4} kaLaLxra keVMdarx; kUTasAthxna; kaLaLxkAkaru visheVSavAgi seVruva sathxLa. 
\num{5} pApa \mo vu beLeyuva sathxLa, avugaLa AvAsasAthxna: \eng{a nest of vice} pApakUpa. 
\num{6} (oMdeV gUDu, nivAsa, \mo vugaLalilxruva) hiMDu; guMpu. 
\num{7} meSinfganfgaLa taMDa, guMpu. 
\num{8} (oMdeV tarahada vasutxgaLa) saMgarxha; taMDa; sAlu; paMkitx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \hyperref{kandict_f.pdf}{F}{feather(2) nuga(1)}{$^2$feather} \eng{one's nest.} 
\num{2} \eng{it is an ill bird that fouls its own nest} tananx manege tAneV kaLaMka tarabAradu. 
\num{3} \eng{nest of tables} meVjutaMDa; (baLakege beVDavAdAga) oMdaraDiyalolxMdaraMte joVDisida meVjugaLu. 
\enum
\emng
\eentry

\bentry
\word[nest(2)]{nest}
\pron{nesfTx}
\gl{\akirx}
\bmng
\bnum
\num{1} (oMdu gotAtxda sathxLadalilx) gUDukaTuTx; gUDanunx mADiko; gUDukaTuTxva kelasadalilx toDagu. 
\num{2} (kADu hakikxgaLa) gUDu lUTi mADu; gUDanunx yA moTeTxgaLanunx beVTeyADu. 
\num{3} (peTiTxge, meVju, \mo vanunx) oMdaroLagoMdaninxrisi aDaka mADu. 
\enum
\emng
\eentry

\bentry
\word{nested}
\pron{nesiTxDf}
\gl{\gu}
\bmng
\bnum
\num{1} gUDinalilx yA gUDinalilxruvaMte nelasida. 
\num{2} (peTiTxge, pAterx, \mo vanunx) oMdaroLagoMdaninxTuTx aDakisida. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nest egg}{nest egg}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} husimoTeTx; kaqtakamoTeTx; gUDumoTeTx; moTeTxgaLanunx iDutatx hoVgalu koVLiyanunx perxVrisalu gUDinalilxTaTx nijavAda yA kaqtakavAda moTeTx. 
\num{2} (keYgAvaligAgali, muMde aBivaqdidhxyAguvaMte mADuvudakAkxgali) tegediTaTx haNa; miVsalu haNa. 
\enum
\emng
\eentry

\bentry
\word{nestful}
\pron{nesfTxphulf}
\gl{\nA}
\expl{(\bava\ \eng{nestfuls}).}
\bmng
gUDu tuMba; gUDuBatiR; oMdu gUDu tuMbuvaSuTx saMKeyxyalilxruva mari, moTeTx, \mo vugaLu. 
\emng
\eentry

\bentry
\word{nesting}
\pron{nesiTxMgf}
\gl{\nA}
\bmng
(kADuhakikxgaLa) gUDulUTi mADuvike; gUDanunx yA moTeTxgaLanunx beVTeyADuvike. 
\emng
\eentry

\bentry
\word{nestle}
\pron{nesalf}
\gl{\sakirx}
\bmng
(tale, Buja, \mo vanunx) pirxVtiyiMda, becacxgiruvaMte oLadUrisu: \eng{she nested the baby in her arms} avaLu maguvanunx toVLugaLalilx tabibx hiDididadxLu. 
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} (ele, hodike, kuciR, \mo vugaLalilx) suKavAgi, AyakaTATxgi -- nelasu, kuLituko. 
\num{2} pirxVtiyiMda apipxko: \eng{the child was nestling closely to its mother} magu pirxVtiyiMda tAyiyanunx apipxkoMDitutx. 
\num{3} aremareyAgiru yA oLage seVrikoMDiru. 
\enum
\emng
\eentry

\bentry
\word{nestlike}
\pron{nesfTxleYkf}
\gl{\gu}
\bmng
gUDinaMtha; gUDina AkArada. 
\emng
\eentry

\bentry
\word{nestling}
\pron{nesilxMgf, nesfTxliMgf}
\gl{\nA}
\bmng
gUDina mari; gUDu biTuTx baralAgadaSuTx eLeya hakikx. 
\emng
\eentry

\bentry
\word{Nestor}
\pron{nesaTxrf}
\gl{\nA}
\bmng
 tiLivaLikasathxnAda hiriya; jAcnxnavaqdadhxnAda muduka. 
\emng
\eentry

\bentry
\word[Nestorian(1)]{Nestorian}
\pron{nesoTxVrianf}
\gl{\nA}
\bmng
nesoTxVriyasfna anuyAyi; (yeVsukirxsatxnige parxteyxVka divayx hAgU mAnava rUpagaLidudxveMdu parxtipAdisida) nesoTxVriyasf eMbuvana sidAdhxMtada anuyAyi. 
\emng
\eentry

\bentry
\word[Nestorian(2)]{Nestorian}
\pron{nesoTxVrianf}
\gl{\gu}
\bmng
nesoTxVriyasfna tatatxvXda. 
\emng
\eentry

\bentry
\word{Nestorianism}
\pron{nesoTxVrianisaZmf}
\gl{\nA}
\bmng
nesoTxVriyasfna tatatxvX, sidAdhxMta. 
\emng
\eentry

\bentry
\word[net(1)]{net}
\pron{neTf}
\gl{\nA}
\bmng
\bnum
\num{1} (huri, hagagx, kUdalu, \mo vugaLiMda mADida) bale; jAla; kalilx. 
\num{2} (miVVnu \mo vanunx hiDiyuvudakekx baLasuva) bale. 
\num{3} (mucucxva, aDakavAgisuva, rakaSxNeya, tuMbi hiDidukoMDu hoVguva, \mo\ upayoVgagaLuLaLx) bale. 
\num{4} (PuTfbAlf \mo\ ATagaLalilx goVlf parxdeVshavanunx mucacxlu, Tenisf \mo\ ATagaLalilx ATada parxdeVshavanunx viBAgisalu hAkuva) bale. 
\num{5} (neYtika yA mAnasikavAgi) inonxbabxnanunx, oMdanunx hiDidu hAkuva sAdhana; bale. 
\num{6} (obabx vayxkitxyanunx moVsadiMda hiDiyalu baLasuva) baleyaMtha karxma yA sAdhana. 
\num{7} jeVDana bale. 
\num{8} jAlabaMdha; heNeda baleya kelasa. 
\num{9} (\bava dalilx) (balegaLiMda) sututxvareda kirxkeTfna aBAyxsa meYdAna. 
\num{10} = \hyperlink{network(1)}{network}. 
\enum
\emng
\eentry

\bentry
\word[net(2)]{net}
\pron{neTf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{netted}; \vakaq\ \eng{netting}).}
\bmng
\bnum
\num{1} (baleyanunxpayoVgisi) keDavu; hiDi; hiDidiDu. 
\num{2} (balegaLoLage) seVrisiru. 
\num{3} (balegaLiMda) mucucx; sututxgaTuTx; maremADu. 
\num{4} baleyiMdaloV eMbaMte hiDi. 
\num{5} (ceMDanunx) baleyoLakekx yA goVlinoLakekx -- tUrisu, hAku, keDavu, hoDe. 
\num{6} (nadi \mo vugaLalilx) bale haraDi, biVsi -- miVnu hiDi. 
\num{7} (miVnu \mo vanunx hiDiyalu nadiyalilx) bale biVsu; bale oDuDx; bale kaTuTx; baleharaDu. 
\num{8} balegelasa mADu; bale heNe; baleheNigeyiMda ciVla, joVli, \mo vanunx mADu. 
\num{9} (\sA\ \BUkaq dalilx) baleya, jAlariya mAdariya gurutumADu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
balegelasa mADu. 
\emng
\eentry

\bentry
\word[net(3)]{net}
\pron{neTf}
\gl{\gu}
\bmng
\bnum
\num{1} nivavxLa; ainu; kaLetavilalxda; vajA mADabeVkAdadadxlalxda; kaLeyabeVkAdadadxnenxlalx kaLeduLida: \eng{net profit} nivavxLa lABa. 
\num{2} (beleya \vi) soVDi ilalxda; soVDi kaLeyalu avakAshavilalxdiruvaMtha. 
\num{3} (tUka) (vasutxvaninxrisida Dabibx, siVse, \mo vugaLa tUkavanunx kaLeda) nivavxLa tUkada; ainutUkada. 
\num{4} (pariNAma, PalitAMsha, \mo vugaLa \vi) aMtayxda; koneya; motatxda. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{net} \hyperref{kandict_t.pdf}{T}{ton(1) pagu(4)}{$^1$ton}. 
\emng
\eentry

\bentry
\word[net(4)]{net}
\pron{neTf}
\gl{\sakirx}
\bmng
(motatx) nivavxLa lABavAgi 
\banum
\alnum{a} paDe, gaLisu. 
\alnum{b} baru; taru. 
\eanum
\emng
\eentry

\bentry
\word{netball}
\pron{neTfbAlf}
\gl{\nA}
\bmng
neTfbAlf; bale ceMDu; baleyanunx neVtuhAkiruva aDaDxlAda baLeyoLakekx ceMDanunx esedu goVlugaLanunx mADuva oMdu ATa. 
\emng
\eentry

\bentry
\word{netful}
\pron{neTfphulf}
\gl{\nA}
\bmng
baletuMba; baletuMbuvaSuTx parxmANa: \eng{a netful of fish} baletuMba miVnu. 
\emng
\eentry

\bentry
\word{nether}
\pron{nedarf}
\gl{\gu}
\bmng
(\pArxparx\ matutx \hA) keLagiruva; (oMdu BAga \mo vugaLa) keLage iruva: \eng{nether lip} keLatuTi. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{nether man} kAlugaLu \mo vu. 
\hypertarget{nether pagu2}{} 
\num{2} \eng{nether regions} naraka; AdhoVloVka. 
\num{3} \eng{nether world} = \hyperlink{nether pagu2}{?pagu?\((2)\)}. 
\enum
\emng
\eentry

\bentry
\word{Netherlander}
\pron{nedarflaMDarf}
\gl{\nA}
\bmng
nedarflaMDfsx parxdeVshadavanu; hAleMDfnavanu. 
\emng
\eentry

\bentry
\word{Netherlandish}
\pron{nedarflaMDiSf}
\gl{\gu}
\bmng
nedarflaMDfsx parxdeVshada; hAleMDina. 
\emng
\eentry

\bentry
\word{Netherlands}
\pron{nedarflaMDf\char144}
\gl{\nA}
\bmng
\bnum
\num{1}(utatxra yUroVpina) hAleMDf deVsha. 
\num{2}(\ca) nedarfleMDfsx; hAleMDf, belijxyamf matutx lakasxMbagfR. 
\enum
\emng
\eentry

\bentry
\word{nethermost}
\pron{nedarfmoVsfTx}
\gl{\gu}
\bmng
tiVra keLagina. 
\emng
\eentry

\bentry
\word{netsuke}
\pron{neTfsuki}
\gl{\nA}
\expl{(\bava\ adeV yA \eng{netsukes}).}
\bmng
(joVtADuva alaMkAra vasutxgaLanunx dharisalu japAniyaru soMTadalilx dharisutitxdadx) ketatxneya kelasadalilx koreda (\kanmu\ daMtada yA marada) guMDi. 
\emng
\eentry

\bentry
\word[nett(1)]{nett}
\pron{neTf}
\gl{\gu}
\bmng
\hyperlink{net(3)}{$^3$net} enunxvudara rUpAMtara. 
\emng
\eentry

\bentry
\word[nett(2)]{nett}
\pron{neTf}
\gl{\sakirx}
\bmng
\hyperlink{net(4)}{$^4$net} enunxvudara rUpAMtara. 
\emng
\eentry

\bentry
\word{netted}
\pron{neTiDf}
\gl{\gu}
\bmng
balebale mADida; bale mAdariya; baleya gurutu hAkida. 
\emng
\eentry

\bentry
\word{netting}
\pron{neTiMgf}
\gl{\nA}
\bmng
\bnum
\num{1}(\kanmu) bale(ya hAge heNeda baTeTx \mo vu); taMti, dAra, \mo vugaLiMda heNeda baleyaMtha racane. 
\num{2} (halavu kelasagaLige baLasuva) bale heNige \mo vu. 
\enum
\emng
\eentry

\bentry
\word[nettle(1)]{nettle}
\pron{neTflf}
\gl{\nA}
\bmng
(\savi) 
\bnum
\num{1} curucurike giDa; yUTiRka kulakekx seVrida, cucucxva roVmagaLiMda tuMbida giDa. 
\num{2} (idanunx hoVluva) itara jAtiya kuTuku giDagaLu. 
\enum
\emng
\eentry

\bentry
\word[nettle(2)]{nettle}
\pron{neTflf}
\gl{\sakirx}
\bmng
\bnum
\num{1} turuciyiMda cucucx, kaDita uMTumADu. 
\num{2} (\rUpa) reVgisu; keraLisu; kirikirigoLisu. 
\enum
\emng
\eentry


\bentry
\word{nettle-rash}
\pron{neTflfrAYxSf}
\gl{\nA}
\bmng
turuci dadudx; turuci muLiLxniMda AguvaMtha camaRda meVlina dadudxgaLu. 
\emng
\eentry

\bentry
\word[network(1)]{network}
\pron{neTfvakfR}
\gl{\nA}
\bmng
jAla; bale; jAlabaMdha: 
\banum
\alnum{a} baleya racaneyaMte aDaDxhAyuva, aDaDxlAda matutx laMbavAda geregaLa vayxvasethx. 
\alnum{b} reYludArigaLu, rasetxgaLu, kAluvegaLu, \mo vugaLa saMkiVNaR vayxvasethx. 
\alnum{c} vaqtitxsaMbaMdhavAda yA sAmAjika udedxVshagaLigAgi mAhiti, saMpakaRgaLu matutx anuBavagaLanunx vinimaya mADikoLuLxva janara taMDa. 
\alnum{d} parasapxra saMbaMdha kalipxsida kaMpUyxTarugaLu, yaMtarxgaLu yA kAyARcaraNegaLa saraNi. 
\alnum{e} saMpakaR kalipxsida viduyxcaCxkitxya vAhakagaLa vayxvasethx. 
\alnum{f} oMdu (reVDiyoV yA TeliviSanf) kAyaRkarxmavanunx EkakAladalilx parxsAra mADuva saluvAgi oTiTxge saMpakaR kalipxsida parxsAra keVMdarxgaLa taMDa. 
\eanum
\emng
\eentry

\bentry
\word[network(2)]{network}
\pron{neTfvakfR}
\gl{\sakirx}
\bmng
\bnum
\num{1} (yaMtarxgaLa, \kanmu\ kaMpUyxTarugaLa) jAlakalipxsu; (avugaLanunx) sahayoVgadiMda kAyARcaraNe mADuvaMte saMpakaR kalipxsu, joVDisu. 
\num{2} parxsAra keVMdarxgaLa jAlada mUlaka parxsAra mADu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} jAlavanunx sAthxpisu. 
\num{2} vaqtitx saMbaMdhavAda yA sAmAjika udedxVshagaLigAgi mAhiti, saMpakaRgaLu matutx anuBavagaLanunx vinimaya mADikoLuLxva janara taMDada sadasayxnAgu. 
\enum
\emng
\eentry

\bentry
\word{networker}
\pron{neTfvakaRrf}
\gl{\nA}
\bmng
neTfvakfRrf; jAlasadasayx: 
\banum
\alnum{a} maneyiMda yA horagina kaCeVriyiMda kAyARcaraNe naDesuva, kaMpUyxTarf jAlada saMsethxya sadasayx. 
\alnum{b} vaqtitxsaMbaMdhavAda yA sAmAjika udedxVshada mAhiti anuBavagaLanunx haMcikoLuLxva taMDada sadasayx. 
\eanum
\emng
\eentry

\bentry
\word{networkful}
\pron{neTfvakfRphulf}
\gl{\nA}
\bmng
jAlatuMba; halavu vayxkitx, saMsethx, \mo vanunx seVrisida jAlagaLanunx tuMbidudx. 
\emng
\eentry

\bentry
\word{neum}
\pron{nUyxmf}
\gl{\nA}
\bmng
= \hyperlink{neume}{neume}. 
\emng
\eentry

\bentry
\word{neume}
\pron{nUyxmf}
\gl{\nA}
\bmng
(\saM) (madhayxyugada) sAMparxdAyika cacuRgiVtegaLalilx oMdu ucAcxrAMshadoLage hADabeVkAda savxra yA savxrapuMjavanunx sUcisuva cihenx. 
\emng
\eentry

\bentry
\word{neural}
\pron{nuyxaralf}
\gl{\gu}
\bmng
(\aMrashA) 
\bnum
\num{1} narada yA narakekx saMbaMdhisida. 
\num{2} naravUyxhada; keVMdarx naravUyxhada yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{neuralgia}
\pron{nuyxarAYxlajx}
\gl{\nA}
\bmng
narashUle; naragaLige GAsiyAdudara pariNAmavAgi punaH punaH (\kanmu\ tale yA muKadalilx) marukaLisuva tiVvarx noVvu. 
\emng
\eentry

\bentry
\word{neuralgic}
\pron{nuyxarAYxlijxkf}
\gl{\gu}
\bmng
narashUleya; naragaLa veVdanege saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neurally}
\pron{nuyxarali}
\gl{\kirxvi}
\bmng
narakekx yA naravUyxhakekx saMbaMdhisidaMte. 
\emng
\eentry

\bentry
\word{neurasthenia}
\pron{nuyxarasitxVniA}
\gl{\nA}
\bmng
naradwbaRlayx; naragaLa dwbaRlayxdiMda kANabaruva AyAsa, kaLavaLa, lavalavike ilalxdiruvike, \mo vu. 
\emng
\eentry

\bentry
\word[neurasthenic(1)]{neurasthenic}
\pron{nuyxarasetxnikf}
\gl{\nA}
\bmng
naradubaRla; naradwbaRlayxdiMda naraLuvavanu. 
\emng
\eentry

\bentry
\word[neurasthenic(2)]{neurasthenic}
\pron{nuyxarasetxnikf}
\gl{\gu}
\bmng
naradwbaRlayxda; naragaLa dwbaRlayxkekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neuritic}
\pron{nuyxariTikf}
\gl{\gu}
\bmng
naragaLa uriyUtada; naragaLa uriyUta iruva. 
\emng
\eentry

\bentry
\word{neuritis}
\pron{nuyxareYTisf}
\gl{\nA}
\bmng
naragaLa uriyUta. 
\emng
\eentry

\bentry
\word{neuro-}
\pron{nuyxaroV-}
\gl{\sapUpa}
\bmng
narada yA narakekx saMbaMdhisida eMbathaRda \sapUpa. 
\emng
\eentry

\bentry
\word{neurogenesis}
\pron{nuyxaroVjenisisf}
\gl{\nA}
\bmng
narajanana; narada Utakada beLavaNige. 
\emng
\eentry

\bentry
\word{neurogenic}
\pron{nuyxaroVjenikf}
\gl{\gu}
\bmng
naragaLa UtakadiMda Ada. 
\emng
\eentry

\bentry
\word{neuroglia}
\pron{nuyxarAgilxa}
\gl{\nA}
\bmng
narAdhAra Utaka; keVMdarx naravUyxhakekx AdhAravAda, naragaLiMda racitavAgirada Utaka. 
\emng
\eentry

\bentry
\word{neurohormone}
\pron{nuyxaroVhAmoRVnf}
\gl{\nA}
\bmng
narahAmoRVnu; narakoVshagaLu utApxdisi rakatxsaMcAradoLakekx sarxvisuva hAmoRVnu. 
\emng
\eentry

\bentry
\word{neurological}
\pron{nuyxaralAjikalf}
\gl{\gu}
\bmng
naraveYjAcnxnika; narashAsitxrXVya; naragaLa shAsitxrXVya adhayxyanada, adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neurologically}
\pron{nuyxaralAjikali}
\gl{\kirxvi}
\bmng
naraveqjAcnxnikavAgi; narashAsitxrXVyavAgi; nara vijAcnxnariVtAyx. 
\emng
\eentry

\bentry
\word{neurologist}
\pron{nuyxarAlajisfTx}
\gl{\nA}
\bmng
naravijAcnxni; narashAsatxrXjacnx; naragaLa shAsitxrXVya adhayxyana naDesuvava(Lu) yA adaralilx pariNata(te). 
\emng
\eentry

\bentry
\word{neurology}
\pron{nuyxarAlaji}
\gl{\nA}
\bmng
naravijAcnxna; narashAsatxrX; naragaLa shAsitxrXVya adhayxyana. 
\emng
\eentry

\bentry
\word{neuroma}
\pron{nuyxaroVma}
\gl{\nA}
\expl{(\bava\ \eng{neuromas} yA \eng{neuromata} \ucAcx-- nuyxaroVmaTa).}
\bmng
narageDeDx; naradiMda mUDi beLeda, \sA\ narataMtugaLiMda kUDida geDeDx. 
\emng
\eentry

\bentry
\word{neuromuscular}
\pron{nuyxaroVmasukxyXlarf}
\gl{\gu}
\bmng
narasAnxyuka; naragaLigU sAnxyugaLigU saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neuron}
\pron{nuyxarAnf}
\gl{\nA}
\bmng
= \hyperlink{nerve-cell}{nerve-cell}. 
\emng
\eentry

\bentry
\word{neuronal}
\pron{nuyxaroVnalf}
\gl{\gu}
\bmng
narakoVshada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neurone}
\pron{nuyxaroVnf}
\gl{\nA}
\bmng
= \hyperlink{neuron}{neuron}. 
\emng
\eentry

\bentry
\word{neuronic}
\pron{nuyxarAnikf}
\gl{\gu}
\bmng
\bnum
\num{1} narakoVshaviruva. 
\num{2} narakoVsha savxrUpada yA savxBAvada. 
\enum
\emng
\eentry

\bentry
\word{neuropath}
\pron{nuyxaroVpAYxtf}
\gl{\nA}
\bmng
nararoVgi; vilakaSxNa narasaMveVdaneyuLaLxva(Lu) yA naragaLige saMbaMdhisida vAyxdhiyiMda naraLuvava(Lu). 
\emng
\eentry

\bentry
\word{neuropathic}
\pron{nuyxaroVpAYxtikf}
\gl{\gu}
\bmng
nararoVgada, adakekx saMbaMdhisida yA nararoVgadiMda naraLuva. 
\emng
\eentry

\bentry
\word{neuropathologist}
\pron{nuyxaroVpatAlajisfTx}
\gl{\nA}
\bmng
nararoVgashAsatxrXjacnx; nararoVgatajacnx. 
\emng
\eentry

\bentry
\word{neuropathology}
\pron{nuyxaroVpatAlaji}
\gl{\nA}
\bmng
nararoVgashAsatxrX; nararoVgagaLa shAsitxrXVya adhayxyana. 
\emng
\eentry

\bentry
\word{neuropathy}
\pron{nuyxarApati}
\gl{\nA}
\bmng
nararoVga; naravAyxdhi; \sA\ naragaLu shithilavAgiruva kAraNa vilakaSxNa riVtiyalilx naramaMDala kelasa mADuvike. 
\emng
\eentry

\bentry
\word{neurophysiological}
\pron{nuyxaroVphisiZalAjikalf}
\gl{\gu}
\bmng
narashAriVraka; naradeYhika; narashariVra vijAcnxnada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neurophysiologically}
\pron{nuyxaroVphisiZalAjikali}
\gl{\kirxvi}
\bmng
narashAriVrakavAgi; naradeYhikavAgi; narashariVravijAcnxnariVtAyx yA narashariVra vijAcnxnakekx saMbaMdhisidaMte. 
\emng
\eentry

\bentry
\word{neurophysiologist}
\pron{nuyxaroVphisiZAlajisfTx}
\gl{\nA}
\bmng
narashariVra shAsatxrXjacnx. 
\emng
\eentry

\bentry
\word{neurophysiology}
\pron{nuyxaroVphisiZAlaji}
\gl{\nA}
\bmng
narashariVravijAcnxna; jiVvaMta vayxvasethxgaLalilx jaraguva, jiVvakekx vishiSaTxvAda vAyxpAragaLa adhayxyana naDesuva shariVravijAcnxnadalilx nidiRSaTxvAgi naragaLige saMbaMdhisida shAKe. 
\emng
\eentry

\bentry
\word{neuroptera}
\pron{nuyxarApaTxra}
\gl{\nA}
\bmng
(\jiVvi) nUyxrApeTxrA; edudxkANuva naragaLiMda kUDida, nAlukx pAradashaRka rekekxgaLuLaLx kiVTagaLa oMdu gaNa. 
\emng
\eentry

\bentry
\word{neuropteran}
\pron{nuyxarApaTxranf}
\gl{\nA}
\bmng
(\jiVvi) nUyxrApeTxrA (gaNakekx seVrida) kiVTa. 
\emng
\eentry

\bentry
\word{neuropterous}
\pron{nuyxarApaTxrasf}
\gl{\gu}
\bmng
nuyxrApeTxrA gaNada, adakekx seVrida, yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{neuroses}
\pron{nuyxaroVsiVsfZ}
\gl{\nA}
\bmng
\eng{neurosis} eMba padada \bava. 
\emng
\eentry

\bentry
\word{neurosis}
\pron{nuyxaroVsisf}
\gl{\nA}
\expl{(\bava\ \eng{neuroses} \ucAcx-- nuyxaroVsiVsfZ)}
\bmng
 naravikAra; naravAyxdhi; naravikaqti; deYhikavAgi GAsiyAgadeyeV naragaLu hadageTuTx vilakaSxNavAgi vatiRsuva yA jiVvanada bagege sakAraNavAda viSayaniSaThx daqSiTxyiMda noVDuva sAmathayxR ilalxde KinanxteyiMda kUDida mAnasika vAyxdhi, roVga. 
\emng
\eentry

\bentry
\word{neurosurgeon}
\pron{nuyxaroVsajaRnf}
\gl{\nA}
\bmng
nara shasatxrXcikitAsx tajacnx. 
\emng
\eentry

\bentry
\word{neurosurgery}
\pron{nuyxaroVsajaRri}
\gl{\nA}
\bmng
nara shasatxrXkirxye; naramaMDalada meVle naDesuva shasatxrXkirxye. 
\emng
\eentry

\bentry
\word{neurosurgical}
\pron{nuyxaroVsajiRkalf}
\gl{\gu}
\bmng
nara shasatxrXcikitesxya yA adakekx saMbaMdhisida yA narashasatxrXcikitesxyiMda mADida. 
\emng
\eentry

\bentry
\word[neurotic(1)]{neurotic}
\pron{nuyxarATikf}
\gl{\nA}
\bmng
naravikaqta; naravAyxdhigarxsatx; nararoVgi: 
\banum
\alnum{a} deYhikavAgi GAsiyuMTAgadeyeV naragaLu hadageTuTx vilakaSxNavAgi vatiRsuvava(Lu). 
\alnum{b} jiVvanada bagege sakAraNavAda vasutxniSaThx daqSiTxyiMda noVDuva sAmathayxRvilalxdava(Lu). 
\eanum
\emng
\eentry

\bentry
\word[neurotic(2)]{neurotic}
\pron{nuyxarATikf}
\gl{\gu}
\bmng
naravikaqta; naravikArada; naravAyxdhiya: 
\banum
\alnum{a} naragaLu hadageTuTx vilakaSxNavAgi vatiRsuva. 
\alnum{b} jiVvanavanunx vasutxniSaThx daqSiTxyiMda noVDuva sAmathayxRvilalxda. 
\eanum
\emng
\eentry

\bentry
\word{neurotically}
\pron{nuyxarATikali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} naravAyxdhiyiMda naraLutAtx. 
\num{2} naravAyxdhige saMbaMdhisidaMte. 
\num{3} (\AmA) ati vAsatxvika daqSiTxyiMda noVDutAtx; atisUkaSxmXveVdiyAgi. 
\enum
\emng
\eentry

\bentry
\word{neuroticism}
\pron{nuyxarATisisaZmf}
\gl{\nA}
\bmng
naravikaqti; naravAyxdhigarxsatx sithxti. 
\emng
\eentry

\bentry
\word{neurotomy}
\pron{nuyxarATami}
\gl{\nA}
\expl{(\bava\ \eng{neurotomies}).}
\bmng
naraviceCxVdana; saMveVdane sAdayxvilalxdaMte mADuvudakAkxgi naragaLanunx katatxrisuvudu. 
\emng
\eentry

\bentry
\word{neurotransmitter}
\pron{nUyxaroVTArx YxnfsimxTarf}
\gl{\nA}
\bmng
(\jiVra) naraperxVkaSxka; oMdu naradiMda inonxMdu narakekx yA sAnxyuvige coVdaneyanunx ravAnisalu neravige baruva, narataMtuviniMda osaruva rAsAyanika vasutx. 
\emng
\eentry

\bentry
\word[neuter(1)]{neuter}
\pron{nUyxTarf}
\gl{\gu}
\bmng
\bnum
\num{1} (\vAyx) (\nA\ \mo vugaLa \vi) napuMsaka (liMga); pulilxMgavAgali sitxrXVliMgavAgali alalxda. 
\num{2} (yudadhx, vAda, aBipArxya, \mo vugaLalilx) taTasathx; yAva pakaSxkUkx -- seVrada, sahAya mADada. 
\num{3} (\savi) niliRMga; shalAkegaLu, puMkeVsaragaLu eraDU ilalxdiruva. 
\num{4} (\kiVvi) leYMgikavAgi aBivaqdidhx hoMdirada. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{stand neuter} taTasathxvAgiru; taTasathx dhoVraNe GoVSisu. 
\emng
\eentry

\bentry
\word[neuter(2)]{neuter}
\pron{nUyxTarf}
\gl{\nA}
\bmng
\bnum
\num{1} napuMsaka pada yA shabadx; napuMsaka -- nAmapada, guNavAcaka, savaRnAma; akamaRka kirxyApada. 
\num{2} avayxkatx liMga; jananeVMdirxya vayxkatxpaTiTxlalxda heNuNx kiVTa (\kanmu\ jeVnuhuLu yA iruve). 
\num{3} biVja oDeda, hiDa mADida -- pArxNi. 
\num{4} taTasathx; yAva pakaSxkUkx seVradavanu. 
\enum
\emng
\eentry

\bentry
\word[neuter(3)]{neuter}
\pron{nUyxTarf}
\gl{\sakirx}
\bmng
(pArxNigaLalilx) biVja oDe; hiDa mADu. 
\emng
\eentry

\bentry
\word[neutral(1)]{neutral}
\pron{nUyxTarxlf}
\gl{\gu}
\bmng
\bnum
\numi{1} taTasathx: 
\banum
\alnum{a} taTasathx rASaTxrXkekx seVrida; yudadhx rASaTxrXgaLalilx yAva pakaSxkUkx sahAya mADadiruva. 
\alnum{b} vAdadalilx. aBipArxyaBeVdadalilx yAva pakaSxvanUnx vahisada. 
\eanum
\numie
\num{2} udAsiVna BAvada; nirAsakatx; niSapxkaSxpAtada. 
\num{3} anidiRSaTx; asapxSaTx; sapxSaTxvAda gurutu, baNaNx ilalxda. 
\numi{4} taTasathx: 
\banum
\alnum{a} (\ravi) AmilxVyavU alalxda, parxtAyxmilxVyavU alalxda. 
\alnum{b} (\viduyx) dhana, QuNa eraDU alalxda. 
\eanum
\numie
\num{5} (\savi) niliRMgi. 
\num{6} (\kiVvi) leYMgikavAgi aBivaqdidhx hoMdada, beLeyada. 
\num{7} nUyxTarxlf; (giyarina \vi) cAlita BAgagaLiMda eMjininxna saMpakaR tapipxsida. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{neutral} \hyperref{kandict_e.pdf}{E}{equilibrium pagu(1)}{equilibrium}. 
\num{2} \eng{neutral tint} bUdu baNaNx yA selxVTu baNaNx. 
\enum
\emng
\eentry

\bentry
\word[neutral(2)]{neutral}
\pron{nUyxTarxlf}
\gl{\nA}
\bmng
\bnum
\num{1}taTasathx vayxkitx yA rASaTxrX. 
\num{2} taTasathx rASaTxrXda parxje. 
\num{3} nUyxTarxlf; nUyxTarxlf giyaru; cAlita BAgagaLige eMjininxna saMpakaR kalipxsada giyaru. 
\enum
\emng
\eentry

\bentry
\word{neutralise}
\pron{nUyxTarxleYsfZ}
\gl{\kirx}
\bmng
= \hyperlink{neutralize}{neutralize}. 
\emng
\eentry

\bentry
\word{neutralism}
\pron{nUyxTarxlisaZmf}
\gl{\nA}
\bmng
(rAjakiVyadalilx) tATasathxyX niVti. 
\emng
\eentry

\bentry
\word{neutralist}
\pron{nUyxTarxlisfTx}
\gl{\nA}
\bmng
(rAjakiVyadalilx) taTasathx(nAgiruvava). 
\emng
\eentry

\bentry
\word{neutrality}
\pron{nUyxTArx YxliTi}
\gl{\nA}
\bmng
\bnum
\numi{1} tATasathxyX: 
\banum
\alnum{a} taTasathx niVti, dhoVraNe; GaSaRNeyalilxruva yA yudadhx mADutitxruva rASaTxrXgaLa naDuve yAra pakaSxkUkx seVradiruvudu. 
\alnum{b} taTasathx manoVBAva. 
\alnum{c} udAsiVna manoVBAva. 
\alnum{d} (yAvudeV vivAdadalilx) eraDu pakaSxgaLigU seVradiruvudu. 
\eanum
\numie
\num{2} (\ravi) taTasathxte; AmilxVyavU alalxda, kASxriVyavU alalxda sithxti. 
\enum
\emng
\eentry

\bentry
\word{neutralization}
\pron{nUyxTarxleYseZVSanf}
\gl{\nA}
\bmng
\bnum
\num{1} taTasithxVkaraNa; taTasathxgoLisuvike. 
\num{2} (parxti parxBAvadiMda) samagoLisuvike; niSapxriNAmagoLisuvike. 
\num{3} (yudAdhxcaraNegaLiMda) vinAyiti (koDuvike). 
\enum
\emng
\eentry

\bentry
\word{neutralize}
\pron{nUyxTarxleYsfZ}
\gl{\sakirx}
\bmng
\bnum
\num{1} taTasathxgoLisu. 
\num{2} (parxtibaladiMda, parxtiparxBAvadiMda) samarUpagoLisu; niSapxriNAmagoLisu. 
\num{3} (oMdu sathxLakekx) yudadhxvAyxpaneyiMda, yudadhx kAyARcaraNeyiMda -- vinAyiti koDu, horagu mADu. 
\enum
\emng
\eentry

\bentry
\word{neutralizer}
\pron{nUyxTarxleYsaZrf}
\gl{\nA}
\bmng
 taTasathxkAri; yAvudanenxV taTasathxgoLisuva, niSapxriNAmagoLisuva vasutx yA vayxkitx. 
\emng
\eentry

\bentry
\word{neutrally}
\pron{nUyxTarxli}
\gl{\kirxvi}
\bmng
taTasathxvAgi yA taTasathxnAgi. 
\emng
\eentry

\bentry
\word{neutrino}
\pron{nUyxTirxVnoV}
\gl{\nA}
\expl{(\bava\ \eng{neutrinos}).}
\bmng
 (\Bwvi) nUyxTirxVnoV; viduyxdAveVshavilalxda, darxvayxrAshiyu hecucx kaDame sonenxyAgiruva, vasutxvinoDane atayxlapxvAgi vatiRsuva, beLakina veVgadalilx calisuva kaNa. 
\emng
\eentry

\bentry
\word{neutron}
\pron{nUyxTArxnf}
\gl{\nA}
\bmng
(\Bwvi) nUyxTArxnf; hecucxkaDame porxVTAninaSeTxV darxvayxrAshi iruva, Adare viduyxdAveVshavilalxda, sAmAnayx heYDorxjanf vinA uLidelalx paramANu biVjagaLalilxyU iruva mUla kaNa. 
\emng
\eentry

\bentry
\wordRemoveSpace{neutron-bomb}{neutron bomb}
\pron{?}
\gl{\nA}
\bmng
nUyxTArxnf bAMbu; BAri soPxVTadiMda kaTaTxDa muMtAdavugaLige hAniyuMTumADade, heVraLavAgi nUyxTArxnugaLanunx utApxdisi. pArxNahAniyuMTumADabalalx oMdu bageya bAMbu. 
\emng
\eentry

\bentry
\wordRemoveSpace{neutron-star}{neutron star}
\pron{?}
\gl{\nA}
\bmng
nUyxTArxnf tAre; tAregaLa jiVvanada koneya haMtadalilx jeYvika iMdhanavelalx mugidu hoVdAga rUpugoLuLxvudeMdu heVLalAgiruva, hecucxkaDime bariV nUyxTArxnugaLiMdaleV kUDidudx, matutx miliyanfnaSuTx sAMdarxte iruvudeMdu naMbalAgiruva shevxVtakubajx \eng{(white dwarf)}veMdu kareyuva tAre. 
\emng
\eentry

\bentry
\wordnospeech{Nev.}{Nev.}
\pron{?}
\gl{\saMkiSx}
\bmng
 \eng{Nevada.} 
\emng
\eentry

\bentry
\word{neve}
\pron{neveV}
\gl{\nA}
\bmng
himakaNarAshi; otAtxgi kUDirade himanadiya shiroVBAgadalilx inUnx oTATxgade kaNakaNavAgiruva himada rAshi. 
\emng
\eentry

\bentry
\word{never}
\pron{nevarf}
\gl{\kirxvi}
\bmng
\bnum
\num{1}yAvAgalU, yAva saMdaBaRdalUlx, eMdeMdigU -- ilalx: \eng{have never been there} alilxge eMdeMdigU hoVgiyeV ilalx. 
\num{2}eMdeMdigU ilalx; yAvatUtx AgalAradu: \eng{it is never likely to succeed} adu eMdigU yashasivxyAgalAradu. 
\num{3} beVDa; bAradu: \eng{never fear} hedarabeVDa. 
\num{4} (\AmA) (AshacxyaR yA saMdeVsha sUcisuvalilx) Ce, elAlxdarU uMTe! eMdigU ilalx! \eng{`He ate the whole turkey}'! \eng{`Never'}! `iDiV (TakiR) koVLiyanunx tiMdaneV!' `KaMDita ilalx!' 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1}\eng{I never did}! iMtha viSaya nAnu keVLilalx! 
\num{2} \eng{it is never too late to mend} tididxkoLaLxlu eMdU kAla miVri hoVgadu; tididxkoLuLxvudAdare eMdAdarU tididxkoLaLxbahudu. 
\num{3} \eng{never is a long day} = \hyperlink{never pagu4}{?pagu?\((4)\)}. 
\hypertarget{never pagu4}{} 
\num{4} \eng{never is a long time}(duDukina tAyxga yA nirAse yA AshAdAyakavalalxda BaviSayx vANi -- ivanunx kuritu heVLuvAga) `eMdigU sAdhayxvilalx' eMdu duDuki heVLabAradu. 
\num{5} \eng{never is a long word} = \hyperlink{never pagu4}{?pagu?\((4)\)}. 
\num{6} \eng{never a one} obabxnU ilalx; oMdU ilalx. 
\num{7} \eng{never ending} eMdeMdigU mugiyada. 
\num{8} \eng{never} \hyperref{kandict_m.pdf}{M}{mind(2) pagu(6)}{$^2$mind}. 
\num{9} \eng{never-never}(otitx heVLuvalilx) eMdigU ilalx; ilalxveV ilalx. 
\num{10} \eng{never say die} eMdU nirAsharAgabAradu. 
\num{11} \eng{never so}(hiMde) aSoTxMdu mitimiVri eMdU mADalilalx. \eng{the task was beyond him, though he never worked `so hard}A kelasa avana shakitx miVridAdxgitutx -- hiMdeMdU avanu aSuTx mitimiVri parxyAsapaTiTxralilalxvAdarU saha. 
\num{12} \eng{never-to-be-forgotten} eMdeMdigU mareyabArada, mareyakUDada; avisamxraNiVya. 
\num{13} \hyperlink{now(1) pagu(7)}{$^1$now or never.} 
\num{14} \eng{(the) never-never}(\birx) (\AmA) kaMtu pAvatiya (meVle koLuLxva) vAyxpAra. 
\num{15} \eng{well I never}! iMtha mAtu nAnelUlx keVLilalx. 
\enum
\emng
\eentry

\bentry
\word[never-do-well(1)]{never-do-well}
\pron{nevarfDuvelf}
\gl{\gu}
\bmng
kelasakekx bArada. 
\emng
\eentry

\bentry
\word[never-do-well(2)]{never-do-well}
\pron{nevarfDuvelf}
\gl{\nA}
\bmng
kelasakekx bAradavanu; aparxyoVjaka. 
\emng
\eentry

\bentry
\word{nevermore}
\pron{nevarfmoVrf}
\gl{\kirxvi}
\bmng
muMdeMdigU ilalx; `inunx muMde ...... ilalx!' 
\emng
\eentry

\bentry
\wordnospeech{Never-Never Land}{Never-Never Land}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1}\eng{(never-never land)}(AdashaR) kAlapxnika suKiV rAjayx; kAlapxnika AdashaR rAjayx; rAmarAjayx. 
\num{2} (\AseTxrXV) utatxra kivxVnfsxleMDf matutx utatxra BAgada dUra parxdeVshagaLu. 
\enum
\emng
\eentry

\bentry
\word{nevertheless}
\pron{nevadaRlesf}
\gl{\kirxvi}
\bmng
AdAgUyx; EneV irali; adeVneV idadxrU; adeVneV heVLidarU. 
\emng
\eentry

\bentry
\word{nevus}
\pron{niVvasf}
\gl{\nA}
\bmng
(\ame) \eng{naevus} eMbudara rUpAMtara. 
\emng
\eentry

\bentry
\word[new(1)]{new}
\pron{nUyx}
\gl{\gu}
\bmng
\bnum
\num{1} hosa(dAda); nava; naviVna; nUtana; Aga tAne nimiRsida; hosadAgi baLakege taMda; Aga tAne kaMDuhiDida; hosadAgi -- keVLida, tiLida, anuBavakekx baMda. 
\num{2} paricitavalalxda; gotitxlalxda \eng{ideas new to us} namage gotitxlalxda vicAragaLu. 
\num{3} naviVkarisida. 
\num{4} tAjA. 
\num{5} hecicxna. 
\num{6} seVrisida. 
\num{7} beVreyAda. 
\num{8} mApARDAda; \eng{the new chairman} hosa(dAgi badalAyisida) adhayxkaSxru. \eng{my new tailor} nananx hosa(dAgi Icege paricayavAda) dajiR. 
\num{9} (sathxLada hesarugaLa \vi, `mUlada hesarigiMtalU Icina'. `Icege sAthxpisida' eMbathaRdalilx) hosa; naviVna: \eng{new Orleans}hosa AliRyanfsx. 
\num{10} tAjA; hosadAgi -- tayArisida, huTiTxda, beLeda, baMda; Iga tAneV moTaTxmodalu baLasutitxruva; inUnx haLatAgi hoVgada. \eng{new bread} tAjA berxDuDx. 
\num{11} (BASeya \vi) Adhunika; madhayxyugadiMdiVcege baLakege baMda. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{a new life} hosa -- bALu, baduku, jiVvana; hiMdina parisithxtigaLigiMta bahaLa BinanxvAda parisithxtiyalilx -- jiVvana naDesuvudu. 
\num{2} \eng{a new one on}(\AmA) hosadAda, hiMdeMdU kANada, tiLiyada -- kathe, saMdaBaR, \mo vu. 
\num{3} \eng{as good as new} bahuteVka hosa; inUnx baLakeyiMda mAsilalxda. 
\num{4} \eng{new} \hyperref{kandict_b.pdf}{B}{birth nuga(3)}{birth}. 
\num{5} \eng{new} \hyperref{kandict_b.pdf}{B}{broom(1) nuga}{$^1$broom}. 
\num{6} \eng{New Christian} nava kerxYsatx; balavaMtadiMda kerxYsatx dhamaRkekx matAMtaragoMDa, madhayxyugada sepxVnina yehUdayx yA morAkodavanu. 
\num{7} \eng{new} \hyperref{kandict_c.pdf}{C}{chum(2) pagu}{$^2$chum}. 
\num{8} \eng{new} \hyperref{kandict_d.pdf}{D}{deal(1) pagu(4)}{$^1$deal}. 
\num{9} \eng{new fashioned} hosa phAyxSaninxna; ititxVcina (atAyxdhunika) phAyxSaninxganusAravAgi mADida, tayArAda. 
\num{10} \eng{new foundation} hosa saMsAthxpane; rephameRVSanfniMdiVcege sAthxpitavAda ketheDarxlf \mo vu. 
\num{11} \eng{new light}(matadhamaRgaLa \vi) Adhunika yA vishAla BAvanegaLu yA avanunx opipxkoLuLxva vayxkitx. 
\hyperdef{N}{new pagu(12)}{} 
\num{12} \eng{new man}(\deVva) kerxYsatx dhamaRkekx matAMtaragoMDavanu. 
\num{13} \eng{put on the new man} guNa badalAvaNeyiMda, tananxnunx tAneV sudhArisikoMDu matAMtaraveMdareVneMbudanunx toVrisu. 
\numi{14} \eng{the new} 
\banum
\alnum{a} (guNaBeVdavanunx sUcisuva vishiSaTx shabadhxvAgi) ititxVcina; Adhunika. 
\alnum{b} (niMdAthaRdalilx) navayxtA aBimAnavuLaLx; ati naviVnavAda, 
\eanum
\numie
\num{15} \eng{the new mathematics} naviVna, navayx -- gaNita; pArxthamika haMtadalilxyeV gaNasidAdhxMta \eng{(set theory)} muMtAdavanunx upayoVgisi kalisuva gaNita. 
\num{16} \eng{the new poor} hosa baDavaru; navadaridarxru; Icina sAmAjika badalAvaNegaLiMda baDavarAdavaru. 
\num{17} \eng{the new rich} nava shirxVmaMtaru; Icina sAmAjika badalAvaNeyiMda shirxVmaMtarAdavaru. 
\num{18} \eng{the new woman} (niMdAthaRka) nava mahiLe; (saMparxdAyavanunx nirAkarisi sAvxtaMtarxyXvanunx bayasuva) naviVna sitxrXV(yaru). 
\num{19} \eng{turn over a new leaf}(\rUpa) hosa adhAyxya pArxraMBisu. 
\enum
\emng
\eentry

\bentry
\word[new(2)]{new}
\pron{nUyx}
\gl{\kirxvi}
\bmng
(visheVSaNa padakekx Iga \sA\ kUDugereyiMda seVrisida \parx). 
\bnum
\num{1} hosadAgi; Icege; Iga tAne: \eng{new-blown} Iga tAne araLida; sadoyxVvikasita (\rUpa\ saha). 
\num{2} Iga tAne hosa jiVva taLeda; tAjA; hocacx hosa. 
\enum
\emng
\eentry

\bentry
\word[newborn(1)]{newborn}
\pron{nUyxbAnfR}
\gl{\gu}
\bmng
(magu \mo vugaLa \vi) navajAta; ititxVcege huTiTxda; Iga tAneV jiVva taLeda. 
\bnum
\num{2} punajARta; AdhAyxtimxkavAgi hosa huTuTx paDeda, hosa daqSiTx paDeda. 
\enum
\emng
\eentry

\bentry
\word[newborn(2)]{newborn}
\pron{nUyxbAnfR}
\gl{\nA}
\bmng
navajAta shishu; Iga tAne huTiTxda magu. 
\emng
\eentry

\bentry
\word{Newburg}
\pron{nUyxbagfR}
\gl{\gu}
\bmng
(kaDala AhArada \vi) kene, moTeTxya haLadi loVLe, beNeNx, matutx bArxMdi athavA veYnfgaLa sAsfnalilx beVyisida. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{lobster new} gaTiTxyAda kirxVmf matutx Seri athavA bArxMdiyalilx beVyisida samudarx naLiLx. 
\emng
\eentry

\bentry
\wordRemoveSpace{Newcastle-disease}{Newcastle disease}
\pron{?}
\gl{\nA}
\bmng
nUyxkAyxsalf vAyxdhi; koVLi matitxtara pakiSxgaLige tagaluva, shAvxsasaMbaMdhavAda veYrasf roVga. 
\emng
\eentry

\bentry
\word{newcome}
\pron{nUyxkamf}
\gl{\gu}
\bmng
IgatAne baMda. 
\emng
\eentry

\bentry
\word{newcomer}
\pron{nUyxkamarf}
\gl{\nA}
\bmng
\bnum
\num{1} AgaMtuka; Iga tAne baMdavanu. 
\num{2} (yAvudeV kelasadalilx) hosaba; hosa kalikeyava; AraMBiga. 
\enum
\emng
\eentry

\bentry
\word{newel}
\pron{nUyxalf}
\gl{\nA}
\bmng
\bnum
\num{1} sututxmeTaTxlu haMtada naDugaMba. 
\num{2} meTiTxlu kaMba; haMtada meVlAgali keLagAgali keYkaMbige AsareyAda kaMba. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{open} (\engit{or} \eng{hollow) newel} sututx meTaTxlu haMtada naDuteravu. 
\emng
\eentry

\bentry
\wordRemoveSpace{New-Englander}{New Englander}
\pron{?}
\gl{\nA}
\bmng
`nUyx iMgelxMDf'na nivAsi; amerika saMyukatx saMsAthxnagaLa IshAnayxda Aru rAjayxgaLa yA AseTxrXVliyada utatxra nUyx swtf veVlfsx BAgada nivAsi. 
\emng
\eentry

\bentry
\word{newfangled}
\pron{nUyxphAYxMgflfDx}
\gl{\gu}
\bmng
(niMdAthaR) AkeSxVpAhaRvAgi navayx; haLeya satasxMparxdAyakekx BinanxvAda hosa kalapxneya; naviVnateya. 
\emng
\eentry

\bentry
\word{newfound}
\pron{nUyxphwMDf}
\gl{\gu}
\bmng
Iga tAne kaMDuhiDida yA beLakige taMda. 
\emng
\eentry

\bentry
\word{Newfoundland}
\pron{nUyxphwMDflaMDf}
\gl{\nA}
\bmng
oLeLxya Ijuva shakitxyuLaLx doDaDx nAyi (jAti). \imglink{Newfoundland:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.5cm {N_Pictures/newfoundland.jpg}}} 
\emng
\eentry

\bentry
\wordnospeech{Newfoundland dog}{Newfoundland dog}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{Newfoundland}{Newfoundland}. 
\emng
\eentry

\bentry
\word{newish}
\pron{nUyxiSf}
\gl{\gu}
\bmng
\bnum
\num{1} hecucx kaDame hosatanada; bahuteVka naviVnateya. 
\num{2} bahaLaSuTx AdhunikateyuLaLx. 
\num{3} sumAru tAjA Ada. 
\enum
\emng
\eentry

\bentry
\word{newishness}
\pron{nUyxiSfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} hecucx kaDame hosadAgiruvudu. 
\num{2} bahuteVka AdhunikavAgiruvike. 
\num{3} bahaLaSuTx tAjA Agiruvudu. 
\enum
\emng
\eentry

\bentry
\word{new-laid}
\pron{nUyxleVDf}
\gl{\gu}
\bmng
(moTeTx) IgatAne iTaTx; IgaSeTxV hAkida. 
\emng
\eentry

\bentry
\wordnospeech{new look}{new look}
\pron{?}
\gl{\nA}
\bmng
(\AmA) hosarUpa; badalAyisida, naviVkarisida yA atayxMta Adhunika riVtiyalilx tididxda rUpa, AkAra. 
\emng
\eentry

\bentry
\word{newly}
\pron{nUyxli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} (ititxVcege \sA\ \BUkaq\ kirxyApadada jate): \eng{the newly discovered country} hosadAgi (Icege) kaMDuhiDida deVsha. 
\num{2} hosa riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word[newly-wed(1)]{newly-wed}
\pron{nUyxliveDf}
\gl{\gu}
\bmng
navavivAhita, hosadAgi maduveyAda. 
\emng
\eentry

\bentry
\word[newly-wed(2)]{newly-wed}
\pron{nUyxliveDf}
\gl{\nA}
\bmng
navavivAhita yA navavivAhite; hosadAgi maduveyAdavanu(Lu). 
\emng
\eentry

\bentry
\word{Newmarket}
\pron{nUyxmAkiRTf}
\gl{\nA}
\bmng
nUyx mAkeRTf; oMdu bageya isipxVTina jUjATa. 
\emng
\eentry

\bentry
\word{new-model}
\pron{nUyx mADalf}
\gl{\sakirx}
\bmng
hosadAgi rUpisu; hosadAgi vayxvasethxgoLisu. 
\emng
\eentry

\bentry
\wordnospeech{new moon}{new moon}
\pron{?}
\gl{\nA}
\bmng
amAvAseyx(ya dina yA kAla). 
\emng
\eentry

\bentry
\word{new-mown}
\pron{nUyxmoVnf}
\gl{\gu}
\bmng
(oNahululx \mo vugaLa \vi) Iga tAne koyudx. 
\emng
\eentry

\bentry
\word{news}
\pron{nUyxsfZ}
\gl{\nA}
\bmng
\bnum
\num{1} (\sA\ \Eva vAgi \parx) sudidx; vAteR; samAcAra; (hosa) vataRmAna; vaqtAtxMta; hosa viSaya; hosa GaTane: \eng{have you heard this news}I sudidx keVLididxVyeVnu? 
\num{2} sudidx parxsAra mAdhayxma. 
\num{3} (kulxpatx, hosa) sudidx samAcAragaLa bAnuli parxsAra. 
\num{4} \eng{News}(vaqtatxpatirxkegaLa hesarina BAgavAgi baruva) samAcAra patirxke, \udA\ \eng{Evening News}. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{bad news} 
\banum
\alnum{a} keTaTx sudidx; ashuBa vAteR. 
\alnum{b} (\AmA) ahitakara vayxkitx yA vasutx. 
\eanum
\numie
\num{2} \eng{be news} (Iga tAneV keVLida) hosa viSaya: \eng{that is news to me} A sudidx nAnu I tanaka keVLiyeV iralilalx. 
\num{3} \eng{good news} shuBavAteR; oLeLxya sudidx. 
\hyperdef{N}{news pagu(4)}{} 
\num{4} \eng{no news is good news} sudidxyilalxdiruvudeV oMdu oLeLx sudidx; sudidxyilalxdiruvudu oLeLxya sithxtiyanunx sUcisutatxde. 
\num{5} \eng{that is no news} adu elalxrigU gotitxruva samAcAra; adeVnU hosa sudidxyalalx. 
\num{6} \eng{be in the news} (sudidxyanunxMTumADuvaSuTx) parxsidadhxvAgu: \eng{she was in the news last year} hoVda vaSaR avaLadedxV sudidx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{news agency}{news agency}
\pron{?}
\gl{\nA}
\bmng
vAtAR saMsethx; sudidx saMsethx; sudidxya aMshagaLanunx saMgarxhisi vitaraNe mADuva saMsethx. 
\emng
\eentry

\bentry
\word{newsagent}
\pron{nUyxsfZEjaMTf}
\gl{\nA}
\bmng
(\birx) nUyxsf EjeMTu; vaqtatxpatirxkegaLu matutx adakekx saMbaMdhisida leVKana sAmagirx \mo vugaLanunx mAruva vayxkitx yA aMgaDi. 
\emng
\eentry

\bentry
\word{newsboy}
\pron{nUyxsfZbAyf}
\gl{\nA}
\bmng
vaqtatxpatirxke (mAruva athavA haMcuva) huDuga. 
\emng
\eentry

\bentry
\word{newsbrief}
\pron{nUyxsfZbirxVphf}
\gl{\nA}
\bmng
harxsavx sudidx; miMcu sudidx; TeliviSaZnfnalilx sudidxya oMdu saNaNx tuNuku. 
\emng
\eentry

\bentry
\wordnospeech{news bulletin}{news bulletin}
\pron{?}
\gl{\nA}
\bmng
sudidx saMgarxha: (\kanmu\ bAnuli parxsArakAkxgi) saMgarxhisida visheVSa sudidx aMshagaLu. 
\emng
\eentry

\bentry
\word{newscast}
\pron{nUyxsfZkAsfTx}
\gl{\nA}
\bmng
sudidx parxsAra; (\kanmu\ bAnuli yA dUradashaRnadalilx) sudidxya varadigaLanunx parxsAra mADuvudu. 
\emng
\eentry

\bentry
\word{newscaster}
\pron{nUyxsfZkAsaTxrf}
\gl{\nA}
\bmng
sudidx parxsAraka; (\kanmu\ bAnuli yA dUradashaRnadalilx) sudidx varadigaLanunx parxsAra mADuvavanu(Lu). 
\emng
\eentry

\bentry
\wordnospeech{news conference}{news conference}
\pron{?}
\gl{\nA}
\bmng
vAtARgoVSiThx; patirxkAgoVSiThx. 
\emng
\eentry

\bentry
\word{newsdealer}
\pron{nUyxsfZDiVlarf}
\gl{\nA}
\bmng
(\ame) = \hyperlink{newsagent}{newsagent}. 
\emng
\eentry

\bentry
\word{newsflash}
\pron{nUyxsfZphAlxYxSf}
\gl{\nA}
\bmng
miMcusudidx; parxteyxVkavAgi matutx halavomemx beVre yAvudAdarU kAyaRkarxmagaLa madheyx parxsAra mADuva muKayxvAda oMdu sudidx. 
\emng
\eentry

\bentry
\word{news-gatherer}
\pron{nUyxsfZgAYxdararf}
\gl{\nA}
\bmng
sudidx saMgArxhaka; \kanmu\ bAnuli parxsArakAkxgi yA parxkaTaNegAgi sudidxgaLanunx saMgarxhisuvava. 
\emng
\eentry

\bentry
\word{news-gathering}
\pron{nUyxsfZgAYxdariMgf}
\gl{\nA}
\bmng
sudidxsaMgarxhaNe; \kanmu\ bAnuli parxsArakAkxgi yA parxkaTaNegAgi sudidxgaLanunx saMgarxhisuvudu. 
\emng
\eentry

\bentry
\word{newsgirl}
\pron{nUyxsfZgalfR}
\gl{\nA}
\bmng
vaqtatxpatirxke (mAruva yA haMcuva) huDugi. 
\emng
\eentry

\bentry
\word{newshawk}
\pron{nUyxsfZhAkf}
\gl{\nA}
\bmng
patirxkoVdayxmi; patirxkA varadigAra; sudidxhadudx. 
\emng
\eentry

\bentry
\word{newshound}
\pron{nUyxsfZhwMDf}
\gl{\nA}
\bmng
(\ame) = \hyperlink{newshawk}{newshawk}. 
\emng
\eentry

\bentry
\word{newsless}
\pron{nUyxsflisf}
\gl{\gu}
\bmng
sudidxrahita; sudidx samAcAragaLilalxda. 
\emng
\eentry

\bentry
\word{newsletter}
\pron{nUyxsfZleTarf}
\gl{\nA}
\bmng
sudidxpatarx; vAtARpatarx; kalxbf \mo\ saMsethxgaLu parxkAshapaDisuva, vidhuyxkatxvalalxda laGu parxkaTaNe. 
\emng
\eentry

\bentry
\word{newsman}
\pron{nUyxsfZmAYxnf}
\gl{\nA}
\expl{(\bava\ \eng{newsmen}).}
\bmng
patirxkA varadigAra; patirxkoVdoyxVgi. 
\emng
\eentry

\bentry
\word{newsmonger}
\pron{nUyxsfZmaMgarf}
\gl{\nA}
\bmng
sudidx malalx; sudidx haraDuvavanu. 
\emng
\eentry

\bentry
\word{newspaper}
\pron{nUyxsfpeVparf}
\gl{\nA}
\bmng
\bnum
\num{1} vaqtatx patirxke; samAcAra patirxke. 
\num{2} radidx (vaqtatx patirxke); haLe peVparu. 
\enum
\emng
\eentry

\bentry
\word{newspaperman}
\pron{nUyxsfpeVparfmanf}
\gl{\nA}
\expl{(\bava\ \eng{newspapermen}).}
\bmng
patirxkoVdayxmi. 
\emng
\eentry

\bentry
\word{Newspeak}
\pron{nUyxsipxVkf}
\gl{\nA}
\bmng
rAjakiVya parxcAradalilx baLasuva asapxSaTxvAda swmayx BASe. 
\emng
\eentry

\bentry
\word{newsprint}
\pron{nUyxsfZpirxMTf}
\gl{\nA}
\bmng
vaqtatxpatirxke kAgada; vaqtatxpatirxkegaLanunx mudirxsuva, keLadajeRya kAgada. 
\emng
\eentry

\bentry
\word{newsreader}
\pron{nUyxsfZriVDarf}
\gl{\nA}
\bmng
= \hyperlink{newscaster}{newscaster}. 
\emng
\eentry

\bentry
\word{newsreel}
\pron{nUyxsfZriVlf}
\gl{\nA}
\bmng
vAtARcitarx; ititxVcina sudidx aMshagaLanunx niVDuva calanacitarx. 
\emng
\eentry

\bentry
\wordnospeech{news room}{news room}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} (vaqtatxpatirxke) vAcanAlaya. 
\num{2} sudidx koThaDi; vAtARkoVNe; (parxsAra \mo vugaLigAgi) vAteRgaLanunx sidadhxpaDisuva koVNe. 
\enum
\emng
\eentry

\bentry
\word{news-sheet}
\pron{nUyxsfZSiVTf}
\gl{\nA}
\bmng
\bnum
\num{1} sudidxhALe; vaqtatxpatirxkeya saraLa saMcike. 
\num{2} = \hyperlink{newsletter}{newsletter}. 
\enum
\emng
\eentry

\bentry
\word{news-stand}
\pron{nUyxsfsATxYxMDf}
\gl{\nA}
\bmng
vaqtatx patirxkegaLa vAyxpArakaTeTx; sudidx patirxkegaLa mArATada aMgaDi. 
\emng
\eentry

\bentry
\wordnospeech{new star}{new star}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{nova}{nova}. 
\emng
\eentry

\bentry
\wordnospeech{news theatre}{news theatre}
\pron{?}
\gl{\nA}
\bmng
vAtAR (calana)citarx maMdira; \kanmu\ vAtARcitarxgaLanenxV parxdashiRsuva calanacitarxmaMdira. 
\emng
\eentry

\bentry
\word{news-vendor}
\pron{nUyxsfZveMDarf}
\gl{\nA}
\bmng
vaqtatx patirxke vAyxpAri, mAruvavanu. 
\emng
\eentry

\bentry
\word{newsworthiness}
\pron{nUyxsfZvadiRnisf}
\gl{\nA}
\bmng
sudidxyoVgayxte; sudidxyeMdu parxkaTisuvaSuTx muKaYxvU sAvxrasayxvU Agiruvike. 
\emng
\eentry

\bentry
\word{newsworthy}
\pron{nUyxsfZvadiR}
\gl{\gu}
\bmng
sudidxyoVgayxvAda; sudidxyeMdu parxkaTisuvaSuTx muKayxvU sAvxrasayxvU Ada. 
\emng
\eentry

\bentry
\word{newsy}
\pron{nUyxsiZ}
\gl{\gu}
\expl{(\tara\ \eng{newsier,} \tama\ \eng{newsiest}).}
\bmng
(\AmA) sudidx tuMbida; sudidx haraDuva: \eng{a newsy letter} sudidx tuMbida kAgada. 
\emng
\eentry

\bentry
\word{newt}
\pron{nUyxTf}
\gl{\nA}
\bmng
nUyxTf; TerxYTarasf kulakekx seVrida, udadxvAda bAlada, cikakx uBaya cara pArxNi. \imglink{newt:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.6cm {N_Pictures/newt.jpg}}} 
\emng
\eentry

\bentry
\word{newton}
\pron{nUyxTanf}
\gl{\nA}
\bmng
(\Bwvi) nUyxTanf; balada EkamAna; oMdu kiloVgArxmf kAyada meVle vatiRsi oMdu sekeMDige oMdu miVTarf veVgoVtakxSaR uMTu mADuvaSuTx bala. 
\emng
\eentry

\bentry
\word{Newtonian}
\pron{nUyxToVnianf}
\gl{\gu}
\bmng
nUyxToVniyanf; vishavxdalilxruva kAyagaLigiruva parasapxra saMbaMdha, avugaLa calane, \mo vugaLa bagegx nUyxTanf parxtipAdisida tatatx vxgaLa yA avakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\wordnospeech{Newtonian mechanics}{Newtonian mechanics}
\pron{?}
\gl{\nA}
\bmng
nUyxTananxna yaMtarxvijAcnxna; parxyoVgavAguva balagaLu matutx avugaLa pariNAmavAgi uMTAguva calanegaLu, muMtAdavanunx kuritaMte nUyxTanf parxtipAdisiruva calaneya niyamagaLa AdhArada meVle nimiRsalAda yaMtarx vijAcnxna. 
\emng
\eentry

\bentry
\wordnospeech{Newtonian telescope}{Newtonian telescope}
\pron{?}
\gl{\nA}
\bmng
nUyxTananxna dUradashaRka; parxdhAna duyxtidaMDakekx \eng{45} Digirx koVnaviruvaMte aLavaDisalAgiruva oMdu divxtiVyaka dapaRNavu duyxtidaMDavanunx vadhaRka neVtarxkakekx parxtibiMbisuva EpARTiruva oMdu parxtiPalaka dUradashaRka. 
\emng
\eentry

\bentry
\wordnospeech{Newton's Laws of motion}{Newton's Laws of motion}
\pron{?}
\gl{\nA}
\bmng
(\Bwvi) nUyxTananxna calaneya niyamagaLu; calisuva kAyada darxvayxrAshiya bala, veVga, veVgoVtakxSaRgaLige saMbaMdhisidaMte nUyxTanf parxtipAdisiruva mUru parxsidadhx niyamagaLu. 
\emng
\eentry

\bentry
\wordnospeech{new town}{new town}
\pron{?}
\gl{\nA}
\bmng
(\birx) hosa paTaTxNa; sakARrada vatiyiMda, valase baMdavarigAgi kaTaTxlAda hosa paTaTxNa. 
\emng
\eentry

\bentry
\wordnospeech{new wave}{new wave}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} = \hyperlink{nouvelle vague}{nouvelle vague}. 
\num{2} hosa ale; nUyxveVvf; \eng{1970}ra dashakadalilx janapirxyavAgidadx oMdu bageya `rAkf' saMgiVta. 
\enum
\emng
\eentry

\bentry
\wordRemoveSpace{New-World}{New World}
\pron{nUyx valfDxR}
\gl{\nA}
\bmng
yUroVpige saMbaMdhisidaMte utatxra matutx dakiSxNa amerikagaLanunx oTiTxge heVLuva hesaru. 
\emng
\eentry

\bentry
\wordnospeech{new year}{new year}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} hosa vaSaR. 
\num{2} kAyxleMDarfnalilxna hosa vaSaRda modala kelavu dinagaLu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{New Year's Day} janavari \eng{$1$}neya tAriVKu. 
\num{2} \eng{New Year's Eve} DiseMbarf \eng{$31$}neya tAriVKu. 
\enum
\emng
\eentry
 
\bentry
\wordRemoveSpace{New-Zealander}{New Zealander}
\pron{nUyx siZVlaMDarf}
\gl{\nA}
\bmng
nUyxsiZVleMDinavanu: 
\banum
\alnum{a} nUyxsiZVleMDf deVshadalilx huTiTxdava yA nUyxsiZVleMDf rASiTxrXVya. 
\alnum{b} nUyxsiZVleMDf mUladavanu. 
\eanum
\emng
\eentry

\bentry
\word[next(1)]{next}
\pron{nekfsfTx}
\gl{\gu}
\bmng
\bnum
\num{1} pakakxda; nereya; samiVpada; atayxMta hatitxrada; magugxlalilxruva; pakakxdalilx vAsisuva: \eng{my next neighbour} nananx pakakxda nereyavanu. 
\num{2} oDaneyeV anusarisi baruva, hiMdeyeV baruva; mAraneya; AmeVlina; muMdina: \eng{will ask the next man} muMde sikukxvavananunx keVLutetxVne. \eng{next Friday} muMdina shukarxvAra. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{next but one} pakakxdadxriMdAceya; pakakxda oMdanunx biTuTx Aceya. 
\num{2} \eng{next} \hyperref{kandict_d.pdf}{D}{door(1) nuga(10)}{$^1$door} \eng{to.} 
\numi{3} \eng{next to} 
\banum
\alnum{a} (\rUpa) hecucx kaDame; bahumaTiTxge; bahuteVka: \eng{next to impossible} hecucx kaDame asAdhayx. 
\alnum{b} (obabxna jate) bahaLa geLetanadiMda; saligeyiMda. 
\eanum
\numie
\num{4} \eng{the next man}(yAvaneV) sAmAnayx manuSayx; shirxV sAmAnayx. 
\num{5} \eng{the next world} paraloVka. 
\enum
\emng
\eentry

\bentry
\word[next(2)]{next}
\pron{nekfsfTx}
\gl{\kirxvi}
\bmng
\bnum
\num{1} pakakxdalilx: hatitxra; samiVpadalilx: \eng{placed his chair next to hers} avaLa kuciRya pakakxdalilx tananx kuciR iTaTx. 
\num{2} AmeVle; muMde; taruvAya; oMdara naMtara: \eng{next came a strange figure} AmeVle obabx vicitarx vayxkitx baMda. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{what next?}(\rUpa) inenxVnu aviveVkavoV? AmeVle inenxVnu adhAvxna? 
\emng
\eentry

\bentry
\word[next(3)]{next}
\pron{nekfsfTx}
\gl{\upa}
\bmng
(\pArxparx) 
\bnum
\num{1} hatitxradalilx; pakakxdalilx: \eng{I was standing next him} nAnu avana pakakxdalilx niMtidedx. 
\num{2} AmeVle; taruvAya; anaMtara: \eng{loves him next her own child} tananx maguvina taruvAya avananunx pirxVtisutAtxLe. 
\enum
\emng
\eentry

\bentry
\word[next(4)]{next}
\pron{nekfsfTx}
\gl{\nA}
\bmng
\bnum
\num{1} pakakxda, samiVpada -- vayxkitx, vasutx, \mo vu. 
\num{2} AmeVle baMda, taruvAyada -- vayxkitx, vasutx, \mo vu: \eng{she was the next to arrive} AmeVle baMdavaLu avaLu; avaLu AmeVle baMdaLu. 
\num{3} muMdina kAgada, saMcike, \mo vu: \eng{will tell you in my next} nananx muMdina kAgadadalilx heVLutetxVne. 
\num{4} muMdina vAra \mo vu: \eng{the Friday after next} muMdina shukarxvArada anaMtarada shukarxvAra. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\hyperdef{N}{next(4) pagu(1)}{} 
\bnum
\num{1} \eng{next of kin}(badukiruvavaralilx) hatitxrada naMTaru; samiVpa baMdhugaLu. 
\numi{2} \eng{next please} 
\banum
\alnum{a} idu sAku, muMdina parxshenx keVLi. 
\alnum{b} nimamxdu Ayitu, muMdinavanu barali. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{next-best}
\pron{nekfsfTx besfTx}
\gl{\nA}
\bmng
(adanunx biTuTx) uLidudaralilx atuyxtatxmavAdadudx. 
\emng
\eentry

\bentry
\word{nexus}
\pron{nekasxsf}
\gl{\nA}
\bmng
(\bava\ adeV). 
\bnum
\num{1} seVrike; joVDaNe; naMTu; saMbaMdha (\sA\ \rUpa): \eng{the cash nexus}(bariya) haNada naMTu. 
\num{2} (parasapxra saMbaMdhisida) taMDa yA sherxVNi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NF}{NF}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Front.} 
\emng
\eentry

\bentry
\wordnospeech{Nfld.}{Nfld.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{(NF} saha) \eng{Newfoundland.} 
\emng
\eentry

\bentry
\wordnospeech{NFU}{NFU}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Farmer's Union.} 
\emng
\eentry

\bentry
\wordnospeech{n.g.}{n.g.}
\pron{?}
\gl{\saMkiSx}
\bmng
 \eng{no good.} 
\emng
\eentry

\bentry
\wordnospeech{NGA}{NGA}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Graphical Association.} 
\emng
\eentry

\bentry
\word{ngaio}
\pron{neYO}
\gl{\nA}
\expl{(\bava\ \eng{ngaios}.)}
\bmng
(tinanxlu ruciyAda haNuNx hAgU hagura biLiya cwbiVnege takukxdAda) nUyxsiZVleMDina oMdu saNaNx mara. 
\emng
\eentry

\bentry
\wordnospeech{NGO}{NGO}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{non-governmental organization}. 
\emng
\eentry

\bentry
\wordnospeech{NH}{NH}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{New Hampshire.} 
\emng
\eentry

\bentry
\wordnospeech{NHI}{NHI}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Health Insurance.} 
\emng
\eentry

\bentry
\wordnospeech{NHS}{NHS}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Health Service.} 
\emng
\eentry

\bentry
\wordnospeech{NI}{NI}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} (\birx) \eng{National Insurance.} 
\num{2} \eng{Northern Ireland.} 
\enum
\emng
\eentry

\bentry
\word{niacin}
\pron{neYasinf}
\gl{\nA}
\bmng
= \hyperlink{nicotinic acid}{nicotinic acid}. 
\emng
\eentry

\bentry
\word[nib(1)]{nib}
\pron{nibf}
\gl{\nA}
\bmng
\bnum
\num{1} garileVKaniya mone, muLuLx; (leVKani kaDiDxyalilx sikikxsuva loVhada yA gariya) bareyuva muLuLx. 
\num{2} (Ayudha \mo vugaLa) mone; cUpu tudi. 
\num{3} (\bava dalilx) sipepxyiMda biDisida koVko yA kAphi biVjagaLu. 
\enum
\emng
\eentry

\bentry
\word[nib(2)]{nib}
\pron{nibf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nibbed}, \vakaq\ \eng{nibbing}).}
\bmng
\bnum
\num{1} (leVKanige) bareyuva muLuLx sikikxsu. 
\num{2} gari leVKaniyanunx heredu sarimADu, monacu mADu. 
\enum
\emng
\eentry

\bentry
\word[nibble(1)]{nibble}
\pron{nibflf}
\gl{\sakirx}
\bmng
\bnum
\num{1} koMca koMcavAgi kacicx tinunx; cUru cUru tinunx. 
\num{2} (\kanmu\ gALada ereyanunx miVnu kacucxva \vi\ yA molagaLa \vi) melalxge, ecacxrikeyiMda, ATavADuvaMte -- kacucx(\akirx\ saha). 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
alapx viSayagaLalilx TiVkisu, AkeSxVpisu. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{nibble at}(\rUpa\ saha) 
\banum
\alnum{a} savxlapx savxlapxvAgi kacucx. 
\alnum{b} parxloVBanegaLoDane sarasavADu. 
\alnum{c} (vAyxpAra \mo vugaLalilx) cwkAsi mADu; jugAgxDu. 
\eanum
\numie
\num{2} \eng{nibble off} koMca koMca kacicx tiMdu mugisu. 
\enum
\emng
\eentry

\bentry
\word[nibble(2)]{nibble}
\pron{nibflf}
\gl{\nA}
\bmng
\bnum
\num{1} (\kanmu\ miVnugALada ereyanunx) kacucxvudu. 
\num{2} (oMdu sala) kacicx tinunxvaSuTx (hululx \mo vugaLa) parxmANa, cUru. 
\num{3} (AhAra \mo vugaLa) tiVrA cikakx cUru; saNaNx tuMDu. 
\num{4} (kaMpUyxTarf) adhaR beYTu, aMdare \eng{4} biTfgaLu. 
\enum
\emng
\eentry

\bentry
\word{nibbler}
\pron{nibalxrf}
\gl{\nA}
\bmng
koMcakoMcavAgi kacicx tinunxvava. 
\emng
\eentry

\bentry
\word{niblick}
\pron{nibflikf}
\gl{\nA}
\bmng
(gAlfphx ATadalilx upayoVgisuva) BAravAda kabibxNada duMDu taleyuLaLx doDaDx dAMDu. 
\emng
\eentry

\bentry
\word{nibs}
\pron{nibf\char144}
\gl{\nA}
\bmng
(\ashi) adhikAradalilxruva vayxkitx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{his nibs}(\AmA) (\hA) durahaMkArada, savxparxtiSeThxya vayxkitx (\eng{His Majesty} enunxvaMte vayxMgAyxthaRdalilx). 
\emng
\eentry

\bentry
\word{nice}
\pron{neYsf}
\gl{\gu}
\bmng
\bnum
\num{1} sogasina. 
\num{2} sUkASx mxBiruciya. 
\num{3} kusubiSeTxya; mecicxsalu kaSaTxvAda. 
\num{4} nAjUkina. 
\num{5} niyamaniSaThx. 
\num{6} niSakx qSaTx; karAruvAkAkxda. 
\num{7} kaTuTxniTATxda: \eng{must not be too nice about the means} sAdhanoVpAyagaLa bagegx bahaLa kaTuTxniTATxgirabAradu. 
\num{8} niSakx qSaTxte; gamana, EkAgarxte, ecacxrike, kwshala, viveVcanegaLu -- avashayxvAda: \eng{a nice experiment} kwshaladiMda kUDida parxyoVga. 
\num{9} sUkaSx mxvAda: \eng{a nice shade of meaning} sUkaSx mxvAgi noVDabeVkAda athaRda CAye. 
\num{10} ati sUkaSx mxviveVcaneya; sUkaSxmXgArxhiyAda: \eng{a nice ear} sUkaSxmXgArxhiyAda kivi. 
\numi{11} (\AmA) 
\banum
\alnum{a} hitavAda; AkaSaRka; ramayx; manoVhara; koVmala. 
\alnum{b} rucikara; suvAsanABarita. 
\alnum{c} taqpitxdAyaka. 
\alnum{d} dayApara. 
\alnum{e} senxVhapara. 
\alnum{f} (inonxbabxra bagegx) kALajiyiruva. 
\alnum{g} sAmAnayxvAgi shAlxGaniVya. 
\alnum{h} (vayxMgayx) avalakaSxNavAda; keTaTxdAda; avayxvasethxya; asahayxkara: \eng{made a nice mess of it} oLeLx avayxvasethx mADibiTaTx. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{nice and} (matutx beVroMdu guNavAcakada jate baruvalilx) (\AmA) takakxSuTx aMdada; oLeLxya: \eng{the house stands nice and high} mane takakxSuTx aMdavAgiyU etatxravAgiyU ide. 
\num{2} \eng{nice looking} (\AmA) oLeLxya; suMdara; citAtxkaSaRka rUpada; noVDalu aMdavAda. 
\num{3} \eng{nice work} cenAnxgi mADida kelasa; pariSAkxravAda kelasa. 
\num{4} \eng{not very nice} (\AmA) hitavalalxda; cenAnxgilalxda. 
\enum
\emng
\eentry

\bentry
\word{niceish}
\pron{neYsiSf}
\gl{\gu}
\bmng
\bnum
\num{1} niSakxqqSaTxte, ecacxrike, kwshala, viveVcanegaLu -- agatayxvAda. 
\num{2} sUkASxmXtisUkaSx mx. 
\enum
\emng
\eentry

\bentry
\word{nicely}
\pron{neYsfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} sogasAgi. 
\num{2} sUkASx mxBiruciyiMda. 
\num{3} kusubiSeTxyiMda. 
\num{4} nAjUkAgi. 
\num{5} niSakx qSaTxvAgi. 
\num{6} viveVcaneyiMda; kwshaladiMda. 
\num{7} sUkaSxmX daqSiTxyiMda. 
\num{8} ati sUkaSxmXvAgi. 
\num{9} hitavAgi. 
\num{10} AkaSaRkavAgi; manoVharavAgi; ramaNiVyavAgi. 
\enum
\emng
\eentry

\bentry
\word{Nicene}
\pron{neYsiVnf}
\gl{\gu}
\bmng
(ESAyx meYnarfna vAyavayxkikxruva pArxciVna paTaTxNavAda) neYsiyAkekx saMbaMdhisida. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{first and second Nicene Councils} deVvatA vigarxhagaLa bagegx edadx Eriyanf vivAdavanunx pariharisalu \kirxpU\ \eng{325} matutx \kirxpU\ \eng{787}ralilx seVrida samAloVcana saBegaLu. 
\emng
\eentry

\bentry
\wordnospeech{Nicene Creed}{Nicene Creed}
\pron{?}
\gl{\nA}
\bmng
neYsiVnf mata; neYsiVnf saBeyalilx aMgiVkarisida niNaRyada AdhArada meVle niVDalAda, kerxYsatxdhamaRkekx saMbaMdhisida vidhivatAtxda heVLike. 
\emng
\eentry

\bentry
\word{niceness}
\pron{neYsfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} sogasu. 
\num{2} sUkASx mxBiruci. 
\num{3} kusubiSeTx; sulaBavAgi mecacxdiruvike. 
\num{4} nAjUkutana. 
\num{5} niSakxqqSaTxte. 
\num{6} AkaSaRka riVti; manoVharate; ramaNiVyavAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{nicety}
\pron{neYsiTi}
\gl{\nA}
\expl{(\bava\ \eng{niceties}).}
\bmng
\bnum
\num{1} sUkaSx mxneVmaniSeThx. 
\num{2} sUkaSx mxte; sUkASxmXMsha. 
\num{3} niSakxqqSaTxte; karAruvAkAkxgiruvike. 
\num{4} jaTilate yA sUkaSx mxBAva: \eng{a point of great nicety} atayxMta jaTilavAda vicAra. 
\num{5} sUkaSxmX vayxtAyxsa; atisUkaSxmX yA amuKayx vivara. 
\num{6} (\bava dalilx) sUkaSxmXtegaLu; sUkaSxmX vivaragaLu; atayxMta laGu vivaragaLu. 
\enum
\emng
\eentry

\bentry
\word[niche(1)]{niche}
\pron{nicf, niVSf}
\gl{\nA}
\bmng
\bnum
\num{1} (parxtime, alaMkAra pAterx, kuMDa, \mo vanunx iDalu goVDeyalilx mADida, hecucx ALavilalxda) gUDu; mADa. \imglink{\(^1\)niche 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.6cm {N_Pictures/niche-1.jpg}}} 
\num{2} (\rUpa) (obabxnige takukxdAda yA obabxna sAdhanege gurutAgi kiVtiR maMdiradalilx gotutxpaDisiruva gaNayx) sAthxna; jAga; AvaraNa. 
\num{3} (parisara vijAcnxna) nele; dANa; yAvudeV jiVvijAtiya jiVvana matutx aBivaqdidhxge atayxnukUlavAda parisithxtigaLiruva sAthxna, jAga, parxdeVsha. 
\enum
\emng
\eentry

\bentry
\word[niche(2)]{niche}
\pron{nicf, niVSf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (\sA\ \BUkaq dalilx) gUDu, gUDina piVTha, \mo vugaLalilx parxtime \mo vanunx iDu; sAthxpisu; nelegoLisu; parxtiSiThxsu. 
\num{2} (\kanmu\ AtAmxthaRka) gUDinalilx, mUleyalilx -- kuLituko, sAthxpisiko, sAthxpitavAgu. 
\enum
\emng
\eentry

\bentry
\word{niched}
\pron{nicfTx, niVSfDx}
\gl{\gu}
\bmng
\bnum
\num{1} gUDinalilx sAthxpisiruva. 
\num{2} (\kanmu\ \AtAmx) gUDinalilx, mUleyalilx -- sAthxpitavAgiruva. 
\enum
\emng
\eentry

\bentry
\word{Nichrome}
\pron{neYkorxVmf}
\gl{\nA}
\bmng
\eng{P} neYkorxVmf; viduyxtAtxpakagaLa tayArikege yoVgayxvenisiruva nikalf matutx korxVmiyaMgaLa misharxloVha. 
\emng
\eentry

\bentry
\word{nicish}
\pron{neYsiSf}
\gl{\gu}
\bmng
= \hyperlink{niceish}{niceish}. 
\emng
\eentry

\bentry
\word{Nick}
\pron{nikf}
\gl{\nA}
\bmng
\eng{old Nick}(eMba \pagu dalilx mAtarx parxyoVga) seYtAna. 
\emng
\eentry

\bentry
\word[nick(1)]{nick}
\pron{nikf}
\gl{\nA}
\bmng
\bnum
\num{1} (hiDikegAgi, gurutigAgi, mAgaRdashaRkavAgi upayoVgisuva) kacucx; gADi; siVLike. 
\num{2} (mudarxNa) sariyAgi joVDisalu (sAdhayxvAguvaMte) moLeya oMdu pakakxdalilx mADida kacucx, siVLu gurutu. 
\numi{3} (\birx) (\ashi) 
\banum
\alnum{a} seremane. 
\alnum{b} poliVsf ThANe. 
\alnum{c} (\AmA) sithxti; parisithxti: \eng{in reasonable nick} takakxSuTx oLeLxya sithxtiyalilx. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{in the nick of time} A kaSxNakekx; gotAtxda kAlakekx sariyAgi; sariyAda hotitxge. 
\emng
\eentry

\bentry
\word[nick(2)]{nick}
\pron{nikf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (yAvudAdarU oMdara meVle) kacucx, gADi, siVLike -- mADu. 
\num{2} (kudure bAlada) keLagaDe kacucxmADu. 
\num{3} (kudure tananx bAlavanunx etatxrisiTuTxkoLaLxlu bAlada buDadalilx) adanunx koyuyx. 
\num{4} (muhUtaR, reYlu, \mo vanunx) hotitxge sariyAgi sAdhisu, hiDi. 
\num{5} (\birx) (\ashi) (aparAdhi \mo varanunx) (aparAdhadalilx toDagiruvAgaleV) suLivu koDade hiDi, dasatxgiri mADu. 
\num{6} (\birx) (\ashi) kadi; kaLaLxtana mADu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} (beVTe, OTada paMdayx, \mo vugaLa \vi) mUle, tiruvu, \mo\ kaDegaLalilx sararxne tirugu, cakakxne tirugu. 
\num{2} (taLiyebibxsuvudaralilx) itara taLigaLoDane sariyagi saMgamavAgu, bere (\sakirx\ saha). 
\enum
\emng
\eentry

\bentry
\word[nick(3)]{nick}
\pron{nikf}
\gl{\akirx}
\bmng
(\AseTxrXV) (\ashi) (\eng{off} yA \eng{in}) beVgane yA kadudx -- ODu. 
\emng
\eentry

\bentry
\word[nickel(1)]{nickel}
\pron{nikalf}
\gl{\nA}
\bmng
nikalulx; paramANu saMKeyx \eng{28}, paramANu tUka \eng{58.71} iruva, beLiLxyaMte beLaLxgiruva loVhadhAtu. 
\emng
\eentry

\bentry
\word[nickel(2)]{nickel}
\pron{nikalf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nickelled}, \vakaq\ \eng{nickelling}. \ame\ \BU\ matutx \BUkaq\ \eng{nickeled}, \vakaq\ \eng{nickeling}).}
\bmng
nikalf leVpakoDu; nikalf mulAmu mADu. 
\emng
\eentry

\bentry
\wordnospeech{nickel brass}{nickel brass}
\pron{?}
\gl{\nA}
\bmng
nikalf hitAtxLe; tAmarx matutx satugaLa jatege nikalf seVriruva oMdu misharx loVha. 
\emng
\eentry

\bentry
\word{nickelic}
\pron{nikalikf}
\gl{\gu}
\bmng
(\ravi) nikalikf; (nikalf saMyukatxgaLa \vi) \eng{2}kikxMta hecicxna veVlenisxya nikalf iruva. 
\emng
\eentry

\bentry
\word{nickelodeon}
\pron{nikaloVDianf}
\gl{\nA}
\bmng
(\ame) (\AmA)= \hyperref{kandict_j.pdf}{J}{juke-box}{juke-box}. 
\emng
\eentry

\bentry
\word{nickelous}
\pron{nikalasf}
\gl{\gu}
\bmng
(\ravi) nikalfyukatx; (nikalf saMyukatxgaLa \vi) eraDakikxMta hecicxna veVlenisxya nikalf iruva. 
\emng
\eentry

\bentry
\word{nickel-plate}
\pron{nikalfpelxVTf}
\gl{\sakirx}
\bmng
nikalf leVpisu; (yAvudeV vidhAnadiMda) nikalf leVpa koDu. 
\emng
\eentry

\bentry
\word{nickel-plated}
\pron{nikalfpelxVTiDf}
\gl{\gu}
\bmng
nikalf leVpita; nikalfleVpa koTaTx. 
\emng
\eentry

\bentry
\wordnospeech{nickel silver}{nickel silver}
\pron{?}
\gl{\nA}
\bmng
= \hyperref{kandict_g.pdf}{G}{German silver}{German silver}. 
\emng
\eentry

\bentry
\wordnospeech{nickel steel}{nickel steel}
\pron{?}
\gl{\nA}
\bmng
nikalf ukukx; nikalf beresiruva ukukx. 
\emng
\eentry

\bentry
\word{nicker}
\pron{nikarf}
\gl{\nA}
\bmng
(\birx) (\ashi) (\bava\ adeV) oMdu pwMDu saTxliRMgf (nANayx). 
\emng
\eentry

\bentry
\word{nick-nack}
\pron{nikfnAYxkf}
\gl{\nA}
\bmng
= \hyperref{kandict_k.pdf}{K}{knick-knack}{knick-knack}. 
\emng
\eentry

\bentry
\word[nickname(1)]{nickname}
\pron{nikfneVmf}
\gl{\nA}
\bmng
\bnum
\num{1} (manuSayxna, sathxLada, vasutxvina, nijavAda hesarina badalu yA nijavAda hesarina jate, hAsayxkAkxgi yA hogaLikegAgi upayoVgisiva) aDaDxhesaru; upanAma. 
\num{2} (obabxna) kerxYsatxnAmada saMkiSxpatx yA rUDhiya rUpa. 
\enum
\emng
\eentry

\bentry
\word[nickname(2)]{nickname}
\pron{nikfneVmf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (obabxnanunx, oMdu vasutxvanunx) aDaDx hesariniMda kare. 
\num{2} (obabxnige, vasutxvige) aDaDx hesariDu. 
\enum
\emng
\eentry

\bentry
\word{nicol}
\pron{nikalf}
\gl{\nA}
\bmng
(\Bwvi) nikalf; dhurxviVkaqta beLakanunx paDeyuvudakAkxgi sUkatxvAgi katatxrisida kAyxlesxYTf haraLugaLeraDanunx joVDisi tayArisida sAdhana. 
\emng
\eentry

\bentry
\wordnospeech{nicol prism}{nicol prism}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{nicol}{nicol}. 
\emng
\eentry

\bentry
\word{nicotine}
\pron{nikaTiVnf}
\gl{\nA}
\bmng
(\ravi) nikoTinf; taMbAkina vishiSaTx guNagaLige kAraNavAda adara muKayx satavxvAgiruva, \eng{$\bg\rm C\eg_\bg 10\eg \bg\rm H\eg_\bg 14\eg \bg\rm N\eg_2$} aNusUtarxda AlakxlAyfDx. 
\emng
\eentry

\bentry
\wordRemoveSpace{nicotinic-acid}{nicotinic acid}
\pron{nikaTinikf AYxsiDf}
\gl{\nA}
\bmng
(\ravi) nikoTinikf Amalx; nikoTinfna utakxSaRNadiMda doreyuva, bi-\eng{2} guMpina viTaminfgaLalilx oMdAgiruva, \eng{$\bg\rm C\eg_6\bg\rm H\eg_5\bg\rm NO\eg_2$} aNu sUtarxviruva, airiTiVnfjanayx kAbARkisxlikf Amalx. 
\emng
\eentry

\bentry
\word{nicotinise}
\pron{nikaTiVneYsf}
\bmng
= \hyperlink{nicotinize}{nicotinize}. 
\emng
\eentry

\bentry
\word{nicotinism}
\pron{nikaTiVnisaZmf}
\gl{\nA}
\bmng
taMbAkanunx, nikoTinanunx atiyAgi upayoVgisidadxra pariNAma. 
\emng
\eentry

\bentry
\word{nicotinize}
\pron{nikaTiVneYsfZ}
\gl{\sakirx}
\bmng
nashegAgi nikoTinanunx, taMbAkanunx baLasu. 
\emng
\eentry

\bentry
\word{nictitate}
\pron{nikiTxTeVTf}
\gl{\akirx}
\bmng
kaNuNxmiTukisu; repepx baDi. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nictitating membrane} miTuku pore; kelavu pArxNigaLalilx kaMDubaruva, teredu mucucxtitxruva mUraneya oLarepepx. 
\emng
\eentry

\bentry
\word{nictitation}
\pron{nikiTxTeVSanf}
\gl{\nA}
\bmng
repepxbaDita; nimeVSa; kaNuNx miTukisuvudu. 
\emng
\eentry

\bentry
\word[niddle-noddle(1)]{niddle-noddle}
\pron{niDflfnADflf}
\gl{\gu}
\bmng
\bnum
\num{1} tale tUgADisuva. 
\num{2} naDuguva; kaMpisuva; adaqDha. 
\enum
\emng
\eentry

\bentry
\word[niddle-noddle(2)]{niddle-noddle}
\pron{niDflfnADflf}
\gl{\sakirx}
\bmng
(\akirx\ saha) 
\bnum
\num{1} tale tUgADisu 
\num{2} alAlxDisu; naDugisu; kaMpisuvaMte mADu. 
\enum
\emng
\eentry

\bentry
\word{nide}
\pron{neYDf}
\gl{\nA}
\bmng
(\birx\ \eng{nye} \ucAcx-- neY) phesaMTf hakikxgaLa marigaLa -- guMpu, hiMDu. 
\emng
\eentry

\bentry
\word{nidificate}
\pron{niDiphikeVTf}
\gl{\akirx}
\bmng
= \hyperlink{nidify}{nidify}. 
\emng
\eentry

\bentry
\word{nidification}
\pron{niDiphikeVSanf}
\gl{\nA}
\bmng
gUDu kaTuTxvike. 
\emng
\eentry

\bentry
\word{nidify}
\pron{niDipheY}
\gl{\akirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{nidifies}, \BU\ matutx \BUkaq\ \eng{nidified}).}
\bmng
gUDukaTuTx. 
\emng
\eentry

\bentry
\word{nidnod}
\pron{niDfnADf}
\gl{\akirx}
\bmng
taletUgADisutitxru (\sakirx\ saha). 
\emng
\eentry

\bentry
\word{nidus}
\pron{neYDasf}
\gl{\nA}
\expl{(\bava\ \eng{niduses} yA \eng{nidi} \ucAcx-- neYDeY).}
\bmng
\bnum
\num{1} gUDu; (\kanmu\ huLu, kiVTa, \mo vu) moTeTxyiDuva sathxLa; saMtAnABivaqdidhxya sathxLa. 
\num{2} biVjakaNa yA biVjagaLu aMkurisuva jAga. 
\num{3} (roVga, guNa, lakaSxNa, tatatx vx, \mo vugaLa) mUlasAthxna; huTuTxdANa. 
\num{4} vaqdidhxsAthxna; beLeyuva jAga; yAvudAdarU guNa, lakaSxNa, tatatxvX aBivaqdidhx hoMduva sathxLa. 
\num{5} sAvxBAvikavAda, sahajavAda -- pAterx, koVsha. 
\num{6} (moTeTx, saNaNxgaMti, \mo vugaLa) rAshi; oTiTxlu. 
\enum
\emng
\eentry

\bentry
\word{niece}
\pron{niVsf}
\gl{\nA}
\bmng
soVdarana yA soVdariya magaLu; soVdara magaLu; soVdara sose. 
\emng
\eentry

\bentry
\word{niello}
\pron{nieloV}
\gl{\nA}
\expl{(\bava\ \eng{niellos} yA \eng{nielli} \ucAcx-- nieliV).}
\bmng
niyelo: 
\banum
\alnum{a} beLiLx yA itara loVhagaLa meVle mADuva citarxNada geregaLanunx tuMbalu baLasuva, gaMdhaka beresida beLiLx, tAmarx, siVsagaLa misharxloVha. 
\alnum{b} A riVti tayArisida AlaMkArika vasutx. 
\eanum
\emng
\eentry

\bentry
\word{nielloed}
\pron{nieloVDf}
\gl{\gu}
\bmng
niyeloV iTuTx alaMkarisida. 
\emng
\eentry

\bentry
\word{Nietzschean}
\pron{niVcianf}
\gl{\nA}
\bmng
\bnum
\num{1} niVcfvAdi; niVce (eMba \eng{19}neV shatamAnada jamaRnf tAtitxvXkana) anuyAyi. 
\num{2} (niVcfvAdavanunx) samathiRsuvavanu; (\kanmu) shakitx, adhikAragaLanunx gaLisaloVsuga loVkoVtatxra puruSanAdavanu kuSxdarx kAmanegaLelalxvanUnx jayisi, uLidelalxranUnx meTiTx ALalu shakatxnAgutAtxneMba tatatx vxvanunx samathiRsuvavanu. 
\enum
\emng
\eentry

\bentry
\word{Nietzscheanism}
\pron{niVcianisaZmf}
\gl{\nA}
\bmng
niVcfvAda; niVcf (eMba tAtitx vxkana) sidAdhxMta. 
\emng
\eentry

\bentry
\word[niff(1)]{niff}
\pron{niphf}
\gl{\nA}
\bmng
(\birx) (\AmA) (\kanmu\ keTaTx) vAsane; nAta. 
\emng
\eentry

\bentry
\word[niff(2)]{niff}
\pron{niphf}
\gl{\nA}
\bmng
(\birx) (\AmA) vAsane hoDeyutitxru; dunARta hoDeyutitxru. 
\emng
\eentry

\bentry
\word{niffy}
\pron{niphi}
\gl{\gu}
\bmng
dunARtada; duvARsaneyuLaLx. 
\emng
\eentry

\bentry
\word{niftily}
\pron{niphiTxli}
\gl{\kirxvi}
\bmng
(\AmA) 
\bnum
\num{1} niVTAgi; acucxkaTATxgi; sogasugArikeyiMda. 
\num{2} atuyxtakxqqSaTxvAgi. 
\num{3} cAlAkiniMda. 
\enum
\emng
\eentry

\bentry
\word{niftiness}
\pron{niphiTxnisf}
\gl{\nA}
\bmng
(\AmA) 
\bnum
\num{1} niVTAgiruvike; acucxkaTATxgiruvike. 
\num{2} atuyxtakxqqSaTxvAgiruvike. 
\num{3} cAlAkutana. 
\enum
\emng
\eentry

\bentry
\word{nifty}
\pron{niphiTx}
\gl{\gu}
\expl{(\tara\ \eng{niftier}, \tama\ \eng{niftiest}).}
\bmng
(\AmA) 
\bnum
\num{1} niVTAda; ThiVkAda; acucxkaTATxda; beDagina; sogasugArikeya. 
\num{2} atuyxtakx qSaTx; atuyxtatxma. 
\num{3} jANatanada; cAlAkina. 
\enum
\emng
\eentry

\bentry
\word[niggard(1)]{niggard}
\pron{nigaDfR}
\gl{\nA}
\bmng
jipuNa; jiVna; kaqpaNa; loVBi; keqhiDitadavanu; jugagx. 
\emng
\eentry

\bentry
\word[niggard(2)]{niggard}
\pron{nigaDfR}
\gl{\gu}
\bmng
(\pArxparx) = \hyperlink{niggardly(1)}{$^1$niggardly}. 
\emng
\eentry

\bentry
\word{niggardliness}
\pron{nigaDfRlinisf}
\gl{\nA}
\bmng
jipuNatana; jiVnatana; kaqpaNate; loVBitana; jugagxtana. 
\emng
\eentry

\bentry
\word[niggardly(1)]{niggardly}
\pron{nigaDfRli}
\gl{\gu}
\bmng
\bnum
\num{1} jipuNa; jiVna; kaqpaNa; loVBitanada; jugagxnAda. 
\num{2} tiVrA kaDameya; sAkUsAkAgada. 
\num{3} keYhiDitada; koDalArade koDuva yA koTaTx. 
\enum
\emng
\eentry

\bentry
\word[niggardly(2)]{niggardly}
\pron{nigaDfRli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} jipuNatanadiMda; jiVnatanadiMda; loVBiya hAge; jugagxnaMte. 
\num{2} tiVrA kaDameyAgi; sAkUsAkAgadaMte. 
\num{3} atiyAgi keYhiDita mADutAtx. 
\enum
\emng
\eentry

\bentry
\word{nigger}
\pron{nigarf}
\gl{\nA}
\bmng
\bnum
\num{1} (\sA\ niMdAthaR) niVgorxV. 
\num{2} kariya; kari camaRdavanu; kapupx baNaNxdavanu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\hyperdef{N}{nigger pagu(1)}{} 
\bnum
\num{1} \eng{a nigger in the wood-pile} (\engit{or} (\ame) \eng{fence}) (\ashi) saMshayakekxDekoDuva saMdaBaR; oMdu oLeLxyadanunx keDisuvaMtaha inonxMdu aMsha. 
\num{2} \eng{work like a nigger} bahaLa parxyAsapaDu; bahaLa kaSaTxpaTuTx duDi. 
\enum
\emng
\eentry

\bentry
\word{niggle}
\pron{nigflf}
\gl{\sakirx}
\bmng
saNaNxpuTaTx kATakoDu; upadarxvavanunxMTu mADu. 
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} amuKayx viSayagaLanunx kuritu anAvashayxkavAgi kAlakaLe; kAlaharaNa mADu. 
\num{2} ati visAtxravAda, (anagatayxvAda) vivaraNe koDu. 
\num{3} alapxtanadiMda TiVkisu, tapupx kaMDuhiDi; kuSxdarxriVtiyalilx AkeSxVpisu. 
\enum
\emng
\eentry

\bentry
\word{niggling}
\pron{nigilxMgf}
\gl{\gu}
\bmng
\bnum
\num{1} alapxvAda; kuSxlalxkavAda; keVvala saNaNx viSayagaLige saMbaMdhisida. 
\num{2} udAravAgirada; saMkucita; halAkxtanadalilx vatiRsuva. 
\num{3} (keY baravaNige) turuki bareda; irukisi bareda; Odalu kaSaTxvAda. 
\enum
\emng
\eentry

\bentry
\word{nigglingly}
\pron{nigilxMgfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} alapxvAgi; kuSxlalxkavAgi. 
\num{2} (vataRneya \vi) saNaNxteradalilx; halAkxbudidhxyiMda. 
\enum
\emng
\eentry

\bentry
\word{niggly}
\pron{nigilx}
\gl{\gu}
\bmng
\bnum
\num{1} = \hyperlink{niggling}{niggling}. 
\num{2} siDukuva; uderxVkagoLuLxva; reVguva. 
\enum
\emng
\eentry

\bentry
\word[nigh(1)]{nigh}
\pron{neY}
\gl{\kirxvi}
\bmng
(\pArxparx\ yA \pArxM) = \hyperlink{near(1)}{$^1$near}. 
\emng
\eentry

\bentry
\word[nigh(2)]{nigh}
\pron{neY}
\gl{\upa}
\bmng
(\pArxparx\ yA \pArxM) = \hyperlink{near(2)}{$^2$near}.
\emng
\eentry

\bentry
\word[nigh(3)]{nigh}
\pron{neY}
\gl{\gu}
\bmng
(\pArxparx\ yA \pArxM) = \hyperlink{near(3)}{$^3$near}.
\emng
\eentry

\bentry
\word{night}
\pron{neYTf}
\gl{\nA}
\bmng
\bnum
\num{1} iruLu; rAtirx; nishe; sUyARsatxdiMda sUyoRVdayadavaregina kAla. 
\num{2} iruLa katatxle: \eng{went forth into the night} katatxloLakekx hoVda. 
\num{3} (\alaMshA) aMdhakAra sithxti; tamasusx; rAtirxya katatxlinaMtha sithxti. 
\num{4} mucacxMje; mabubx; saMdhAyxkAla; muMgatatxle: \eng{shall not reach home before night} mucacxMjeya oLage (katatxlu kaviyuva munanx) mane talupalAre. 
\num{5} rAtirxya -- have, anuBavagaLu yA kelasa kAyaRgaLu. 
\num{6} (nATaka \mo) parxdashaRnagaLu naDeyuva saMje: \eng{first night} nATakada modalaneya (rAtirx) parxdashaRna. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\hypertarget{night pagu1}{} 
\bnum
\num{1} \eng{all night} iruLelalx; rAtirxyiDiV. 
\num{2} \eng{all night long} = \hyperlink{night pagu1}{?pagu?\((1)\)}. 
\num{3} \eng{at night} saMjeyalilx; katatxlAda meVle; rAtirxyalilx; saMje \eng{6}riMda madhayxrAtirxyavaregina kAladalilx: \eng{ten o'clock at night}rAtirx hatutx gaMTe. 
\num{4} \eng{black} (\engit{or} \eng{dark) as night} rAtirxyaMtha kapupx; iruLugapupx; iruLinaMtha katatxlu. 
\num{5} \eng{by night} rAtirxyalilx; iruLa mareyalilx. 
\num{6} \eng{have a bad night} rAtirx niderxyilalxde bAdhepaDu. 
\num{7} \eng{have a good night} rAtirx suKa niderx mADu. 
\num{8} \eng{last night} hiMdina, kaLeduhoVda, atayxMta Icina -- rAtirx. 
\num{9} \eng{make a night of it} rAtirxyelAlx utasxva samAraMBadalilx kaLe. 
\num{10} \eng{night and day} hagaliruLU; eDebiDade; yAvAgalU; sadA. 
\num{11} \eng{spend the night with} (\sA\ leYMgika saMbaMdhavanunx sUcisuva athaRdalilx) (\kanmu) rAtirx malagalu avakAsha paDe. 
\num{12} \eng{stay the night} rAtirx taMgu; (maneyiMdAce) beLagAguvavarege uLiduko. 
\num{13} \eng{turn night into day} dinada hAge rAtirx iDiV (agatayxvAda ujavxla diVpagaLa beLakinalilx) kelasa mADu. 
\num{14} \eng{work nights} rAtirxya saradiyalilxru. 
\enum
\emng
\eentry

\bentry
\wordnospeech{night attire}{night attire}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{nightclothes}{nightclothes}. 
\emng
\eentry

\bentry
\word{night-bell}
\pron{neYTfbelf}
\gl{\nA}
\bmng
rAtirx gaMTe; (\kanmu\ veYdayxra maneya biVdi bAgilinalilx) rAtirxya hotutx ebibxsalu hAkiruva karegaMTe. 
\emng
\eentry

\bentry
\word{nightbird}
\pron{neYTfbaDfR}
\gl{\nA}
\bmng
\banum
\alnum{a} rAtirxhakikx (\kanmu\ gUbe yA neYTiMgeVlf). 
\alnum{b} rAtirx saMcAri (vayxkitx); nishAcara. 
\eanum
\emng
\eentry

\bentry
\word{night-blindness}
\pron{neYTfbelx YMDfnisf}
\gl{\nA}
\bmng
iruLugaNuNx; rAtirxguruDu; musuku beLakinalilx kaNiNxnoLakekx hecucx beLakanunx biDuvaMte pApe tanage tAneV higugxva sAmathayxRvanunx kaLedukoMDa pariNAmavAgi mabubx beLakinalilx matutx tAtirxyalilx daqSiTx maMdavAgiruvudu. 
\emng
\eentry

\bentry
\word{night-boat}
\pron{neYTf boVTf}
\gl{\nA}
\bmng
rAtirx (saMcArada parxyANika) haDagu yA doVNi. 
\emng
\eentry

\bentry
\word{nightcap}
\pron{neYTfkAYxpf}
\gl{\nA}
\bmng
\bnum
\num{1} (\ca) rAtirx malaguvAga hAkikoLuLxva ToVpi; rAtirx ToVpi. 
\num{2} rAtirx (malaguva muMce kuDiyuva) pAniVya, \kanmu\ madayx. 
\enum
\emng
\eentry

\bentry
\word{night-chair}
\pron{neYTfceVrf}
\gl{\nA}
\bmng
rAtirxkuciR; rAtirx upayoVgakikxruva malapAterxya nAgAlu. 
\emng
\eentry

\bentry
\word{nightclothes}
\pron{neYTfkolxVdfsx}
\gl{\nA}
\bmng
rAtirxyuDupu; rAtirx malaguvAga hAkikoLuLxva uDupu. 
\emng
\eentry

\bentry
\word{nightclub}
\pron{neYTfkalxbf}
\gl{\nA}
\bmng
rAtirxkalxbubx; rAtirxUTa, saMgiVta, nataRna, \mo\ manaraMjane odagisuva vihAra maMdira. 
\emng
\eentry

\bentry
\word{nightdress}
\pron{neYTfDerxsf}
\gl{\nA}
\bmng
(heMgasina yA maguvina) rAtirx (dharisuva, aLaLxka) uDupu. 
\emng
\eentry

\bentry
\word{nightfall}
\pron{neYTfPAzlf}
\gl{\nA}
\bmng
muMgatatxle; mucacxMje; saMjeya mabubx. 
\emng
\eentry

\bentry
\wordnospeech{night fighter}{night fighter}
\pron{?}
\gl{\nA}
\bmng
(katatxlinalilx vimAna dALi edurisalu hoVrADuva) rAtirx hoVrATa vimAna. 
\emng
\eentry

\bentry
\word{nightflying}
\pron{neYTfphezlxYiMgf}
\gl{\gu}
\bmng
(hakikx, vimAna, \mo vugaLa \vi) rAtirxyalilx hAruva. 
\emng
\eentry

\bentry
\word{night-glass}
\pron{neYTfgAlxsf}
\gl{\nA}
\bmng
rAtirxya (hotutx baLasuva cikakx) dUradashaRka. 
\emng
\eentry

\bentry
\word{nightgown}
\pron{neYTfgwnf}
\gl{\nA}
\bmng
\bnum
\num{1} = \hyperlink{nightdress}{nightdress}. 
\num{2} (\ca) = \hyperref{kandict_d.pdf}{D}{dressing-gown}{dressing-gown}. 
\enum
\emng
\eentry

\bentry
\word{night-hag}
\pron{neYTf hAYxgf}
\gl{\nA}
\bmng
rAtirxya (hotutx gALiyanenxVri baruvudeMdu parxtiVtiyiruva oMdu) heNuNxdevavx. 
\emng
\eentry

\bentry
\word{night-hawk}
\pron{neYTfhAkf}
\gl{\nA}
\bmng
\bnum
\num{1} oMdu jAtiya iruLu hakikx. 
\num{2} = \hyperlink{nightjar}{nightjar}. 
\num{3} (rAtirxgaLaLx \mo) rAtirx saMcArigaLu. 
\enum
\emng
\eentry

\bentry
\word{nightie}
\pron{neYTi}
\gl{\nA}
\bmng
(\AmA) = \hyperlink{nightdress}{nightdress}. 
\emng
\eentry

\bentry
\word{nightingale}
\pron{neYTiMgeVlf}
\gl{\nA}
\bmng
neYTiMgeVlf pakiSx; bulf-bulf hakikx; \kanmu\ rAtirx hotutx iMpAgi ucacxsavxradiMda hADuva, keMpu CAyeya kaMdubaNaNxda,  oMdu deVshAMtara saMcAri pakiSx. \imglink{nightingale:fig}{\raisebox{-0.20cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.5cm {N_Pictures/nightingale.jpg}}} 
\emng
\eentry

\bentry
\word{nightjar}
\pron{neYTfjArf}
\gl{\nA}
\bmng
kakaRsha dhavxniyuLaLx oMdu jAtiya iruLu hakikx. 
\emng
\eentry

\bentry
\word{nightlatch}
\pron{neYTflAYxcf}
\gl{\nA}
\bmng
rAtirxbiVga; horaginiMda biVgada keY hAgU oLaginiMda hiDikeya mUlaka tereyabalalx oMdu riVtiya bAgilu biVga. 
\emng
\eentry

\bentry
\word{nightless}
\pron{neYTflisf}
\gl{\gu}
\bmng
rAtirxyilalxda; iruLilalxda. 
\emng
\eentry

\bentry
\wordnospeech{night letter}{night letter}
\pron{?}
\gl{\nA}
\bmng
(\ame) rAtirxaMce; agagxda daradalilx rAtirx hotutx kaLuhisuva tAru (TeligArxM). 
\emng
\eentry

\bentry
\word{night-life}
\pron{neYTfleYphf}
\gl{\nA}
\bmng
(nagaragaLalilx) rAtirx jiVvana; rAtirx baduku; rAtirx hotutx doreyuva manaraMjane. 
\emng
\eentry

\bentry
\word{night-light}
\pron{neYTfleYTf}
\gl{\nA}
\bmng
rAtirxdiVpa: 
\banum
\alnum{a} moVTu, dapapxneya -- moVMbatitx yA viduyxtf diVpa. 
\alnum{b} (rAtirx hotutx) makakxLa koVNe yA roVgigaLa koThaDiyalilx uriyuva maMda beLakina diVpa. 
\eanum
\emng
\eentry

\bentry
\word{night-line}
\pron{neYTfleYnf}
\gl{\nA}
\bmng
rAtirxgALa; rAtirx hotutx miVnu hiDiyalu niVrinalilx koMDige ere cucicx biTaTx gALa. 
\emng
\eentry

\bentry
\word[night-long(1)]{night-long}
\pron{neYTflAMgf}
\gl{\gu}
\bmng
rAtirxpUtiRya; iruLelalx naDeyuva, naDeda. 
\emng
\eentry

\bentry
\word[night-long(2)]{night-long}
\pron{neYTflAMgf}
\gl{\kirxvi}
\bmng
rAtirxyelalx. 
\emng
\eentry

\bentry
\word[nightly(1)]{nightly}
\pron{neYTfli}
\gl{\gu}
\bmng
\bnum
\num{1} rAtirxya; rAtirxyalilx naDeyuva, naDeda, saMBavisuva, iruva. 
\num{2} parxti rAtirxyU naDeyuva. 
\num{3} (\kAparx) iruLina; rAtirxge takakx; nishoVcita. 
\enum
\emng
\eentry

\bentry
\word[nightly(2)]{nightly}
\pron{neYTfli}
\gl{\kirxvi}
\bmng
parxtirAtirxyU. 
\emng
\eentry

\bentry
\word{nightmare}
\pron{neYTfmeVrf}
\gl{\nA}
\bmng
\bnum
\num{1} savxpanx pishAci; nidedxyalilx edeya meVle kuLitu usiru kaTiTxsuvaMte manasisxge toVruva, oMdu heNuNx pishAci. 
\num{2} duHsavxpanx; GoVra savxpanx. 
\num{3} (\AmA) du:savxpanxdaMtha -- GoVra anuBava, yoVcane, parisithxti. 
\num{4} eDebiDade kADuva Baya. 
\num{5} asapxSaTx BiVtiya vasutx. 
\enum
\emng
\eentry

\bentry
\word{nightmarish}
\pron{neYTfmeVriSf}
\gl{\gu}
\bmng
duHsavxpanxdaMtha. 
\emng
\eentry

\bentry
\word{nightmarishly}
\pron{neYTfmeVriSfli}
\gl{\kirxvi}
\bmng
duHsavxpanxdaMtha GoVra anuBava, yoVcane, parisithxtiya riVtiyalilx; duHsavxpanxdaMte. 
\emng
\eentry

\bentry
\word{night-night}
\pron{neYTfneYTf}
\gl{\nA}
\bmng
(\AmA) shuBarAtirx; rAtirx shuBavAgali eMba hAreYke. 
\emng
\eentry

\bentry
\wordnospeech{night nurse}{night nurse}
\pron{?}
\gl{\nA}
\bmng
rAtirx dAdi; rAtirx hotutx roVgigaLanunx noVDikoLaLxlu neVmisuva dAdi. 
\emng
\eentry

\bentry
\word{night-owl}
\pron{neYTf aulf}
\gl{\nA}
\bmng
\bnum
\num{1} rAtirx (hotutx hAruva) gUbe. 
\num{2} (\AmA) rAtirx bahaLa hotutx ecacxravAgidudx caTuvaTikeyiMdiruva vayxkitx. 
\enum
\emng
\eentry

\bentry
\word{night-piece}
\pron{neYTfpiVsf}
\gl{\nA}
\bmng
rAtirx citarx yA rAtirxya BU daqshayxda vaNaRcitarx. 
\emng
\eentry

\bentry
\word{night-rail}
\pron{neYTfreVlf}
\gl{\nA}
\bmng
(\ca) heMgasara aLaLxkavAda gwnu, hora uDupu. 
\emng
\eentry

\bentry
\word{night-robe}
\pron{neYTfroVbf}
\gl{\nA}
\bmng
= \hyperlink{nightdress}{nightdress}. 
\emng
\eentry

\bentry
\word{night-safe}
\pron{neYTfseVphf}
\gl{\nA}
\bmng
rAtirx tijoVri; rAtirx hotutx bAyxMku mucicxruvAga haNavanunx TheVvaNiyiDalu bAyxMkina horagoVDeyalilx bAgiliruva tijoVri. 
\emng
\eentry

\bentry
\word{night-school}
\pron{neYTfsUkxlf}
\gl{\nA}
\bmng
rAtirx shAle; hagalu duDidAda meVle kelasagArarige shikaSxNa koDuva rAtirx pAThashAle. 
\emng
\eentry

\bentry
\word{nightshade}
\pron{neYTfSeVDf}
\gl{\nA}
\bmng
`soleVnamf' kulada oMdu viSakara sasayxvisheVSa. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{black nightshade} biLiya hUvU kapapxne viSada kAyigaLU biDuvaMtha sasayx. 
\num{2} \eng{deadly nightshade} = \hyperref{kandict_b.pdf}{B}{belladonna}{belladonna}. 
\hyperdef{N}{nightshade pagu(3)}{} 
\num{3} \eng{woody nightshade} dhUmarx vaNaRda hUvU hoLapina keMbaNaNxda kAyigaLU biDuva sasayxjAti. 
\enum
\emng
\eentry

\bentry
\wordnospeech{night shift}{night shift}
\pron{?}
\gl{\nA}
\bmng
rAtirxsaradi; rAtirxpALi; rAtirxsULu; rAtirx kelasa mADuva kAmiRkara saradi. 
\emng
\eentry

\bentry
\word{night-shirt}
\pron{neYTfSaTfR}
\gl{\nA}
\bmng
rAtirxSaTuR; huDuga yA gaMDasu rAtirx malaguvAga dharisuva, udadxneya SaTuR. 
\emng
\eentry

\bentry
\word{night-soil}
\pron{neYTf sAilf}
\gl{\nA}
\bmng
(\kanmu\ gobabxrakAkxgi rAtirx hotutx etitx hAkuva paripAThavidadx) pAyiKAne, mala, \mo\ kashamxla padAthaR. 
\emng
\eentry

\bentry
\word{nightspot}
\pron{neYTfsATxTf}
\gl{\nA}
\bmng
(\AmA) = \hyperlink{nightclub}{nightclub}. 
\emng
\eentry

\bentry
\word{nightstick}
\pron{neYTfsiTxkf}
\gl{\nA}
\bmng
(Ame) poliVsinavana doNeNx, soVTa, adhikAra daMDa. 
\emng
\eentry

\bentry
\word{night-stool}
\pron{neYTfsUTxlf}
\gl{\nA}
\bmng
= \hyperlink{night-chair}{night-chair}. 
\emng
\eentry

\bentry
\word{night-time}
\pron{neYTfTeYmf}
\gl{\nA}
\bmng
rAtirx (hotutx). 
\emng
\eentry

\bentry
\word{night-watch}
\pron{neYTfvAcf}
\gl{\nA}
\bmng
\bnum
\num{1} rAtirxgAvalu. 
\num{2} rAtirxgAvalinavanu yA rAtirxgAvalinavaru. 
\num{3} jAva; yAma; (hiVbUrx athavA roVmanf padadhxtiyalilx) iruLanunx viBAgisiruva mUru yA nAlukx BAgagaLalilx oMdu. 
\enum
\emng
\eentry

\bentry
\word{night-watchman}
\pron{neYTfvAcfmanf}
\gl{\nA}
\bmng
\bnum
\num{1} rAtirxgAvalinavanu. 
\num{2} (kirxkeTf) dinada ATada koneya (kaSaTxda avadhiyalilx utatxma dAMDugAraranunx kaLedukoLaLxbAradeMba udedxVshadiMda kaLuhisalAguva, aSeTxVnU utatxmavalalxda) dAMDugAra, bAYxTfsxmanf. 
\enum
\emng
\eentry

\bentry
\word{night-work}
\pron{neYTfvakfR}
\gl{\nA}
\bmng
rAtirxgelasa; rAtirx mADuva athavA mADaleVbeVkAda kelasa. 
\emng
\eentry

\bentry
\word{nigrescence}
\pron{nigerxsanfsx}
\gl{\nA}
\bmng
nasu kapApxguvike. 
\emng
\eentry

\bentry
\word{nigrescent}
\pron{nigerxsaMTf}
\gl{\gu}
\bmng
nasu kapApxda. 
\emng
\eentry

\bentry
\word{nigritude}
\pron{nigirxTUyxDf}
\gl{\nA}
\bmng
kapupx; kaqSaNxte (\rUpa\ saha). 
\emng
\eentry

\bentry
\word{nigromancy}
\pron{nigarxmAYxnisx}
\gl{\nA}
\bmng
= \hyperlink{necromancy}{necromancy}. 
\emng
\eentry

\bentry
\wordf{nihil ad rem}
\pron{neYhilf AYxDf remf}
\gl{\gu}
\expl{\Latin}
\bmng
asaMbadadhx; aparxsutxta; aparxsakatx. 
\emng
\eentry

\bentry
\word{nihilism}
\pron{neYhi(i)lisfZmf}
\gl{\nA}
\bmng
\bnum
\num{1} nirAkaraNavAda; niSeVdhatatatxvX; dhAmiRka athavA neYtika viSayagaLalilx rUDhiya naMbikegaLanenxlAlx saMpUNaRvAgi tirasakxrisuvudu, alalxgaLeyuvudu. 
\num{2} (\tashA) shUnayxvAda; elalxdara asitxtavxvanunx nirAkarisuva, yAvudU ilalxveMdu vAdisuva saMdeVhavAda. 
\num{3} parxsutxtadalilxruva vayxvasethxyalilx yAvudanUnx opapxda, \eng{19}neya shatamAnada raSAyxdalilxdadx, atuyxgarx kArxMtikAri pakaSxdavara tatatx vxgaLu; savaRnirAkaraNavAda. 
\enum
\emng
\eentry

\bentry
\word{nihilist}
\pron{neYhi(i)lisfTx}
\gl{\nA}
\bmng
shUnayxvAdi. 
\emng
\eentry

\bentry
\word{nihilistic}
\pron{neYhi(i)lisiTxkf}
\gl{\gu}
\bmng
shUnayx seYdAdhxMtika. 
\emng
\eentry

\bentry
\word{nihility}
\pron{neYhiliTi}
\gl{\nA}
\bmng
\bnum
\num{1} nAsitxtavx; shUnayxte. 
\num{2} gaNanege bAradudx; keVvala shUnayx; sonenx. 
\enum
\emng
\eentry

\bentry
\wordf{nihil obstat}
\pron{neYhilf AbfsATx YxTf}
\gl{\nA}
\expl{\Latin}
\bmng
\bnum
\num{1} (\roVkAyx) yAvudeV oMdu pusatxkadalilx dhAmiRka daqSiTxyiMda AkeSxVpaNiVya aMshagaLAvuvU ilalxveMdu niVDuva yoVgayxtA patarx. 
\num{2} (\rUpa) adhikAra patarx; apapxNe patarx; adhikaqta parxmANa patarx. 
\enum
\emng
\eentry

\bentry
\wordwithhyphen{hyp-nik}{-nik}
\pron{-nikf}
\gl{\uparx}
\bmng
yAvudoV vishiSaTx vasutx athavA guNagaLige saMbaMdhisida vayxkitxyanunx sUcisuva \nA gaLanunx racisuvalilx baLasuva \uparx: \eng{beatnik, refusenik.} 
\emng
\eentry

\bentry
\word{nil}
\pron{nilf}
\gl{\nA}
\bmng
(\kanmu\ ATagaLa gelalxMkagaLalilx) sonenx; shUnayx: \eng{three goals to nil} sonenx virudadhx mUru; oMdu kaDeya ATagAraru mUru goVlanUnx inonxMdu kaDeyavaru sonenx goVlanUnx paDedaru. 
\emng
\eentry

\bentry
\word[Nile(1)]{Nile}
\pron{neYlf}
\gl{\nA}
\bmng
maMkAda, masukAda hasiru niVli yA hasiru. 
\emng
\eentry

\bentry
\word[Nile(2)]{Nile}
\pron{neYlf}
\gl{\gu}
\bmng
maMkAda hasiru niVliya yA hasirina. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{Nile-blue} maMkAda hasiru niVli. 
\num{2} \eng{Nile-green} maMkAda hasiru. 
\enum
\emng
\eentry

\bentry
\word{nilgai}
\pron{niVlegxY}
\gl{\nA}
\bmng
niVli jiMke; saNaNx koMbuLaLx, doDaDx AkArada BAratiVya jiMke, cigare. \imglink{nilgai:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.6cm {N_Pictures/nilgai.jpg}}} 
\emng
\eentry

\bentry
\word{Nilometer}
\pron{neYlAmiTarf}
\gl{\nA}
\bmng
neYlf mApaka; neYlf nadi nereyeVruva maTaTxvanunx toVrisuva aLateya reVKegaLuLaLx kaMba \mo vu. 
\emng
\eentry

\bentry
\word{Nilotic}
\pron{neYlATikf}
\gl{\gu}
\bmng
neYlf (nadiya) pArxMtada; A pArxMtada nivAsigaLa yA A pArxMtada BASeya. 
\emng
\eentry

\bentry
\word{nim}
\pron{nimf}
\gl{\nA}
\bmng
nimf (ATa); ibabxru ATagAraru obabxrAda meVle obabxraMte aneVka guDeDxgaLalilx oMdu guDeDxyiMda oMdu athavA hecucx vasutxgaLanunx tegedukoLuLxtAtx koneyalilx uLiyuva vasutxvanunx tegedukoLuLxvaMtAgadaMte ADuva ATa. 
\emng
\eentry

\bentry
\word{nimble}
\pron{niMbflf}
\gl{\gu}
\expl{(\tara\ \eng{nimbler,} \tama\ \eng{nimblest}).}
\bmng
\bnum
\num{1} curukugatiyuLaLx; caTuvaTikeyuLaLx; veVgavuLaLx. 
\num{2} (manoVdhamaR \mo vugaLa \vi) savaRtoVmuKa shakitxyuLaLx; bahumuKa sAmathayxRda; catura; sUkaSxmXgArxhiyAda. 
\num{3} keYcaLakavuLaLx; nipuNa. 
\enum
\emng
\eentry

\bentry
\word{nimbleness}
\pron{niMbflfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} curuku gati; caTuvaTike; tiVvarxgati. 
\num{2} savaRtoVmuKa shakitx; bahumuKa sAmathayxR. 
\enum
\emng
\eentry

\bentry
\word{nimbly}
\pron{niMbilx}
\gl{\kirxvi}
\bmng
\bnum
\num{1} curukugatiyalilx; caTuvaTikeyiMda. 
\num{2} savaRtoVmuKa shakitxyiMda; caturateyiMda; sUkaSx mxgArxhiyAgi. 
\num{3} keYcaLakadiMda; neYpuNayxdiMda. 
\enum
\emng
\eentry

\bentry
\word{nimbostratus}
\pron{niMboVseTxrXV(sATxrX)Tasf}
\gl{\nA}
\expl{[\bava\ \eng{nimbostrati}\ucAcx-- niMboVseTxrXV(sATxrX)TeY].}
\bmng
(\pashA) niMbosATxrXTasf; keLamaTaTxdalilx visAtxravAgi haraDikoMDiruva, kapapxgiruva maLeya moVDa. 
\emng
\eentry

\bentry
\word{nimbus}
\pron{niMbasf}
\gl{\nA}
\expl{(\bava\ \eng{nimbuses} yA \eng{nimbi} \ucAcx-- niMbeY).}
\bmng
\bnum
\num{1} (deVvara, manuSayxna ya vasutxvina sutatx AvarisikoMDiruva) parxbAmaMDala; parxkAsha meVGa; beLaku moVDa. 
\num{2} (citarxdalilx saMta \mo vara taleya sutatxlU athavA meVle iDuva) parxBAmaMDala; parxkAsha biMba; parxBAvalaya. 
\num{3} (\pashA) (EkariVtiyAgi bUdubaNaNxviruva) maLeya moVDa. 
\enum
\emng
\eentry

\bentry
\word{nimbused}
\pron{niMbasfTx}
\gl{\gu}
\bmng
parxBAmaMDaladiMda kUDida; parxkAshabiMbasahitavAda. 
\emng
\eentry

\bentry
\word{nimiety}
\pron{nimeYiTi}
\gl{\nA}
\bmng
(sAhitayxka) miVrike; hecAcxgiruvike; atireVka. 
\emng
\eentry

\bentry
\word{niminy-piminy}
\pron{nimini pimini}
\gl{\gu}
\bmng
\bnum
\num{1} asahaja naDateya; kaqtaka riVtiya. 
\num{2} binAnxNada; vayAyxrada. 
\num{3} dubaRla; satavxhiVna. 
\enum
\emng
\eentry

\bentry
\word{Nimrod}
\pron{nimfrADf}
\gl{\nA}
\bmng
doDaDx yA viVra beVTegAra. 
\emng
\eentry

\bentry
\word{nincompoop}
\pron{ninfkaMpUpf}
\gl{\nA}
\bmng
heDaDx; gugugx; bepapx. 
\emng
\eentry

\bentry
\word[nine(1)]{nine}
\pron{neYnf}
\gl{\nA}
\bmng
\bnum
\num{1} oMbatutx. 
\num{2} oMbatatxra saMkeVta: \eng{9; IX; ix.} 
\num{3} oMbatatxneya saMKeyx; oMbatutx. 
\num{4} (isipxVTele) oMbatutx; oMbatutx cihenxgaLiruva ele. 
\num{5} oMbatutx gaMTe samaya. 
\num{6} (aLate itAyxdi) oMbatatxneya saMKeyx. 
\num{7} navaka; oMbatutx vasutx itAyxdigaLiruva taMDa, guMpu. 
\num{8} (\bava dalilx) \eng{9}neya aLateya keYgavasu, pAdarakeSx, itAyxdi. 
\num{9} (\ame) beVsfceMDATada taMDa, daLa. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{cat has nine lives} bekukx sulaBavAgi sAyuvudilalx. 
\num{2} \eng{dressed up to the nines} atayxMta ThiVkAgi. pariSAkxravAgi-uDupu dharisida. 
\hyperdef{N}{nine(1) pagu(3)}{} 
\num{3} \eng{nine days' wonder} oMbatutx dinada AshacxyaR; savxlapxkAla bahu AkaSaRkavAgidudx beVga maretuhoVguvaMthadudx. 
\num{4} \eng{nine tenths} hatatxralilx oMbatutx BAga; hecucx kaDame pUtiRyAgi. 
\num{5} \eng{nine times out of ten} (\birx) sAmAnayxvAgi; bahuteVka; mukAkxlu mUru viVse pAlu. 
\num{6} \eng{nine to five} oMbatatxriMda aidaravarege: sAmAnayxvAgi kaceVriya kelasada veVLe. 
\num{7} \eng{possession is nine points of the law} oMdu vasutxvina sAvxdhiVnAnuBavavidadxre adara meVle hatatxralilx oMbatutx pAlu nAyxyAnukUlavAda hakukx idadxMteyeV. 
\num{8} \eng{the Nines} oMbatutx kAvayx deVviyaru. 
\num{9} (\birx\ \eng{999}) tutuR seVvege miVsalAda dUravANi saMKeyx. 
\enum
\emng
\eentry

\bentry
\word[nine(2)]{nine}
\pron{neYnf}
\gl{\gu}
\bmng
oMbatutx; oMbatAtxguva. 
\emng
\eentry

\bentry
\word[nine-fold(1)]{nine-fold}
\pron{neYnfphoVlfDx}
\gl{\gu}
\bmng
oMbatutx maDi; oMbatutx paTuTx; navaguNa. 
\emng
\eentry

\bentry
\word[nine-fold(2)]{nine-fold}
\pron{neYnfphoVlfDx}
\gl{\kirxvi}
\bmng
oMbatatxraSuTx. 
\emng
\eentry

\bentry
\word{ninepin}
\pron{neYnfpinf}
\gl{\nA}
\bmng
(\sA\ \bava dalilx, \Eva vAgi \parx). 
\bnum
\num{1} oMbatutx gUTada ATa; capapxTe taLada oMbatutx gUTagaLanunx nililxsi avakekx ceMDesedu, kaDame esetagaLiMda avanunx uruLisabeVkAda oMdu bageya ATa. 
\num{2} I ATadalilx baLasuva oMdu gUTa. 
\enum
\emng
\eentry

\bentry
\word[nineteen(1)]{nineteen}
\pron{neYnfTiVnf}
\gl{\gu}
\bmng
\bnum
\num{1} hatotxMbatutx. 
\num{2} hatotxMbatatxra saMkeVta: \eng{19; xix; XIX.} 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{talk nineteen to the} \hyperref{kandict_d.pdf}{D}{dozen nuga(6)}{dozen}. 
\emng
\eentry

\bentry
\word[nineteen(2)]{nineteen}
\pron{neYnfTiVnf}
\gl{\gu}
\bmng
hatotxMbatutx; hatotxMbatAtxguva. 
\emng
\eentry

\bentry
\word[nineteenth(1)]{nineteenth}
\pron{neYnfTiVnftx}
\gl{\gu}
\bmng
hatotxMbatatxneya. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nineteenth hole}(\hA) gAlfphx ATada pAna shAle: `bArf'. 
\emng
\eentry

\bentry
\word[nineteenth(2)]{nineteenth}
\pron{neYnfTiVnftx}
\gl{\nA}
\bmng
\bnum
\num{1} hatotxMbatatxneyadu. 
\num{2} hatotxMbatatxneV oMdu BAga \eng{$(1/19)$}. 
\enum
\emng
\eentry

\bentry
\word[ninetieth(1)]{ninetieth}
\pron{neYnfTiatf}
\gl{\gu}
\bmng
toMbatatxneya. 
\emng
\eentry

\bentry
\word[ninetieth(2)]{ninetieth}
\pron{neYnfTiatf}
\gl{\nA}
\bmng
toMbatatxneyadu. 
\emng
\eentry

\bentry
\word[ninety(1)]{ninety}
\pron{neYnfTi}
\gl{\nA}
\expl{(\bava\ \eng{nineties}.)}
\bmng
\bnum
\num{1} toMbatutx. 
\num{2} toMbatatxra saMkeVta: \eng{90; xc; XC.} 
\num{3} toMbatatxneya saMKeyx: toMbatutx. 
\num{4} (\bava dalilx) (\kanmu\ shatamAnadalilx yA Ayusisxnalilx) toMbatatxriMda toMbatotxMbatatxravaregina vaSaRgaLu; toMbatatxra dashaka. 
\enum
\emng
\eentry

\bentry
\word[ninety(2)]{ninety}
\pron{neYnfTi}
\gl{\gu}
\bmng
toMbatutx; toMbatAtxguva. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{ninety-first, ninety-second, etc.} 
\banum
\alnum{a} toMbatotxMdaneya, toMbatetxraDaneya, \mo. 
\alnum{b} toMbatotxMdaneya, toMbatetxraDaneya \mo\ vasutx yA vayxkitx. 
\eanum
\numie
\num{2} \eng{ninety-nine out of a hundred}(\rUpa) nUrakekx toMbatotxMbatutx BAga; hecucx kaDame elAlx; bahuteVka elAlx. 
\num{3} \eng{ninety-one} toMbatotxMdu. 
\enum
\emng
\eentry

\bentry
\word{Ninevite}
\pron{niniveYTf}
\gl{\nA}
\bmng
(purAtana asisxVriyAda) nineve paTaTxNadavanu. 
\emng
\eentry

\bentry
\word{ninja}
\pron{niMja}
\gl{\nA}
\bmng
`niMjuTUsx' eMba kadana kaleyalilx parxviVNanAdavanu. 
\emng
\eentry

\bentry
\word{ninjutsu}
\pron{niMjuTUsx}
\gl{\nA}
\bmng
niMjuTUsx; japAnina oMdu bageya kadanakale. 
\emng
\eentry

\bentry
\word{ninny}
\pron{nini}
\gl{\nA}
\expl{(\bava\ \eng{ninnies}).}
\bmng
gugugx; heDaDx; budidhxyilalxdavanu. 
\emng
\eentry

\bentry
\word{ninon}
\pron{ninAnf}
\gl{\nA}
\bmng
hagara reVSemx uDupina baTeTx. 
\emng
\eentry

\bentry
\word[ninth(1)]{ninth}
\pron{neYnftx}
\gl{\gu}
\bmng
oMbatatxneya; oMbatatxneya sAthxnada; navama. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{ninth part} oMbatatxneya BAga; samanAda oMbatutx BAgagaLalilx oMdu BAga. 
\emng
\eentry

\bentry
\word[ninth(2)]{ninth}
\pron{neYnftx}
\gl{\nA}
\bmng
\bnum
\num{1} oMbatatxneya BAga; oMbatatxralilx oMdu BAga. 
\num{2} (\saM) navaka; madhayxma sapatxkada SaDajxdiMda tArasapatxkada QuSaBadavaregina oMbatutx savxragaLa meVLa. 
\enum
\emng
\eentry

\bentry
\word{ninthly}
\pron{neYnftxli}
\gl{\kirxvi}
\bmng
oMbatatxneyadAgi; oMbatatxneya sAthxnadalilx. 
\emng
\eentry

\bentry
\word{Niobe}
\pron{neYabi}
\gl{\nA}
\bmng
shoVkamUtiR; duHKAteR; pirxya bAMdhavara viyoVgadiMda, saMtayisalAgada duHKavuLaLx sitxrXV. 
\emng
\eentry

\bentry
\word{Niobean}
\pron{neYabiVanf}
\gl{\gu}
\bmng
duHKAtaRLAda; shoVkatapatxLAda. 
\emng
\eentry

\bentry
\word{niobic}
\pron{neYObikf}
\gl{\gu}
\bmng
(\ravi) nayoVbikf; (saMyukatxda \vi) paMcaveVlenisxVya nayoVbiyamimxna. 
\emng
\eentry

\bentry
\word{niobium}
\pron{neYObiamf}
\gl{\nA}
\bmng
(\ravi) nayoVbiyamf: paramANu saMKeyx \eng{41,} paramANu tUka \eng{92.9} uLaLx, bUdubaNaNxda loVhadhAtu. 
\emng
\eentry

\bentry
\word{niobous}
\pron{neYObasf}
\gl{\gu}
\bmng
(\ravi) nayoVbasf; (saMyukatxda \vi) aidakikxMta kaDame veVlenisxya nayoVbiyamimxna. 
\emng
\eentry

\bentry
\word{Nip}
\pron{nipf}
\gl{\nA}
\bmng
(\ashi) japAni; japAnina vayxkitx \eng{(Nipponese} enunxvudara \saMkiSx). 
\emng
\eentry

\bentry
\word[nip(1)]{nip}
\pron{nipf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nipped,} \vakaq\ \eng{nipping}).}
\bmng
\bnum
\num{1} civuTu; birusAgi, balavAgi -- hisuku, amuku, hiMDu; kaThiNavAgi kacucx, kaDi. 
\num{2} (mogugx \mo vanunx) civuTi hAku; (yAvudAdarU oMdara) beLavaNigeyanunx taDe (\rUpa\ saha). 
\num{3} (shiVta, caLi, \mo vu) keDaku mADu; noVvu koDu. 
\num{4} (\ame) (\ashi) kadi; apaharisu. 
\num{5} (Pakakxne, matotxbabxrige kANadaMte) etitxko; hArisibiDu; egarisu; lapaTAyisu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
(\birx) (\ame) Pakakxne jAriko, dATiko, nusiduko, nusuLiko: \eng{`nip in, sir' said the driver} `oLakekx nusidukoLiLx sAvxmi' eMda cAlaka. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nip in the bud}(\rUpa) moLakeyalilxyeV civuTi hAku; beLeyuvudakekx muMceyeV nimURla mADu. 
\emng
\eentry

\bentry
\word[nip(2)]{nip}
\pron{nipf}
\gl{\nA}
\bmng
\bnum
\num{1} cucucx nuDi; kaTaki nuDi; kuTukuva vayxMgoyxVkitx. 
\num{2} (sasayxda beLavaNigege taDe mADuva) gALiya sheYtayx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nip and tuck} (paMdayx, sapxdheR, \mo vugaLa \vi) sumAru samanAda; bahuteVka hatitxra hatitxravAda: \eng{it was nip and tuck until the final round} koneya sutitxnavaregU (paMdayxvu) bahuteVka samasamavAgitutx. 
\emng
\eentry

\bentry
\word[nip(3)]{nip}
\pron{nipf}
\gl{\nA}
\bmng
(curukugoLisalu seVvisuva madayx \mo vugaLa) oMdu guTuku. 
\emng
\eentry

\bentry
\word[nip(4)]{nip}
\pron{nipf}
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nipped,} \vakaq\ \eng{nipping}). }
\bmng
oMdu guTuku kuDi, guTukisu. 
\emng
\eentry

\bentry
\word{nipa}
\pron{niVpa}
\gl{\nA}
\bmng
\bnum
\num{1} (IsfTx iMDiVsfna) oMdu jAtiya tALeya mara. 
\num{2} I tALeya maradiMda tegeda madayx. 
\enum
\emng
\eentry

\bentry
\word{nipper}
\pron{niparf}
\gl{\nA}
\bmng
\bnum
\num{1} gALavanunx yA itara miVnugaLanunx kacucxva miVnu. 
\num{2} (\birx) (\asaM) cikakx huDuga yA huDugi; poVra yA poVri. 
\num{3} (\bava dalilx) cimuTa; cimuTa katatxri; balavAgi kacicx hiDidukoLuLxvudakAkxgali taMti \mo vanunx katatxrisuvudakAkxgali baLasuva, davaDeyaMtha BAgagaLuLaLx Ayudha. 
\num{4} (\bava dalilx) (\pArxparx) kacucxkananxDaka; mUgukananxDaka; mUgige kacicxkoLuLxva kananxDaka. 
\num{5} kudureya bAcihalulx. 
\num{6} (EDi, naLiLx, \mo) valakxvaMtavagaRda pArxNigaLa cimuTAMga, paMja. 
\enum
\emng
\eentry

\bentry
\word{nippily}
\pron{nipili}
\gl{\kirxvi}
\bmng
\bnum
\num{1} caTuvaTikeyiMda; suTiyAgi; cAlUkAgi. 
\num{2} koreyuvaSuTx taNaNxge. 
\enum
\emng
\eentry

\bentry
\word{nipping}
\pron{nipiMgf}
\gl{\gu}
\bmng
noVyisuva; hiMse koDuva; toMdareyanunxMTumADuva. 
\emng
\eentry

\bentry
\word{nipple}
\pron{nipflf}
\gl{\nA}
\bmng
\bnum
\num{1} (sasatxnigaLa, \kanmu\ heNiNxna) moletoTuTx; cUcuka. 
\num{2} (maguvina) hAlu shiVseya, buDiDxya (rababxrina) moletoTuTx. 
\num{4} (camaR, gAju, loVha, \mo vugaLa meVlina) gubuTu; mole toTiTxnAkArada ububx. 
\num{4} beTaTxda meVlina saNaNx ububx, duMDu unanxti. 
\num{5} (\ca) keVpu toTuTx; tupAkiya cApinalilx keVpu iDalu mADiruva, tUtugaLuLaLx cAcuBAga. 
\num{6} (\ame) (inonxMdaroDane seVrisalu) eraDu tudigaLalUlx sututxgaLiruva naLikeya, koLaveya BAga. 
\enum
\emng
\eentry

\bentry
\word{nipplewort}
\pron{nipflfvaTfR}
\gl{\nA}
\bmng
haLadi hU biDuva oMdu jAtiya joMDu. 
\emng
\eentry

\bentry
\word{Nippon}
\pron{nipanf}
\gl{\nA}
\bmng
japAnf deVsha. 
\emng
\eentry

\bentry
\word[Nipponese(1)]{Nipponese}
\pron{nipaniVsf}
\gl{\nA}
\bmng
(\bava\ adeV). japAninavanu. 
\emng
\eentry

\bentry
\word[Nipponese(2)]{Nipponese}
\pron{nipaniVsf}
\gl{\gu}
\bmng
japAnina; japAnige seVrida. 
\emng
\eentry

\bentry
\word[Nipponian(1)]{Nipponian}
\pron{nipoVnianf}
\gl{\nA}
\bmng
= \hyperlink{Nipponese(1)}{$^1$Nipponese}. 
\emng
\eentry

\bentry
\word[Nipponian(2)]{Nipponian}
\pron{nipoVnianf}
\gl{\gu}
\bmng
= \hyperlink{Nipponese(2)}{$^2$Nipponese}. 
\emng
\eentry

\bentry
\word{nippy}
\pron{nipi}
\gl{\gu}
\expl{(\tara\ \eng{nippier}, \tama\ \eng{nippiest}.)}
\bmng
(\AmA) 
\bnum
\num{1} caTuvaTikeya; suTiyAda; cUTiyAda; cAlUkAda. 
\num{2} koreyuvaSuTx taNaNxneya. 
\enum
\emng
\eentry

\bentry
\word{NIREX}
\pron{neYrekfsx}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{Nuclear Industry Radioactive Waste Executive.} 
\emng
\eentry

\bentry
\word{nirvana}
\pron{navARna, niavARna}
\gl{\nA}
\bmng
(bwdadhx mata) nivARNa: mukitx; moVkaSx. 
\emng
\eentry

\bentry
\word{nisei}
\pron{niVseV}
\gl{\nA}
\bmng
(\ame) japAni saMjAta; japAniniMda valase baMda taMde tAyigaLa maga, magaLu; japAniniMda valase baMdavara vaMshada, amerikadalilx huTiTx alelxV shikaSxNa paDeda vayxkitx. 
\emng
\eentry

\bentry
\word{nisi}
\pron{neYseY}
\gl{\gu}
\bmng
(\nAyxshA) horatu; vinA: \eng{decree nisi} soVpAdhika niNaRya; jArige baralu kelavu niyamagaLanunx pAlisabeVkAgiruvudeMdu, kelavu SaratutxgaLige oLapaTiTxrabeVkAgiruvudeMdu niVDuva tiVpuR, ajecnx, vidhi, itAyxdi. 
\emng
\eentry

\bentry
\wordf{nisi prius}
\pron{neYseY perxYasf}
\gl{\nA}
\expl{\Latin}
\bmng
(\nAyxshA) (iMgelxMDf) kirxminalf koVTuRgaLa yA (\ca) nAyxyadashiR koVTuRgaLa adhikAravAyxpitxge seVrida nAyxyAdhiVsharu sivilf mokadadxmegaLa vicAraNe naDesuvudu. 
\emng
\eentry

\bentry
\wordRemoveSpace{Nissen-hut}{Nissen hut}
\pron{nisanf haTf}
\gl{\nA}
\bmng
nisanf guDisalu; ububxtagigxna kabibxNada tagaDugaLiMda mADida, simeMTf nelavuLaLx, suraMgadAkArada guDisalu, joVpaDi. 
\emng
\eentry

\bentry
\word[nit(1)]{nit}
\pron{niTf}
\gl{\nA}
\bmng
\bnum
\num{1} siVru; heVnina yA itara paroVpajiVvi kiVTada moTeTx yA mari. 
\num{2} (\birx) (\ashi) daDaDx; aviveVki. 
\enum
\emng
\eentry

\bentry
\word[nit(2)]{nit}
\pron{niTf}
\bmng
\BAavayx\ (\AseTxrXV) (\ashi) huSArf! yAroV obabxru barutitxdAdxreMbudanunx tiLisalu balasuva \BAavayx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{keep nit} huSArAgiru! huSAru! 
\emng
\eentry

\bentry
\word{nit-pick}
\pron{niTfpikf}
\gl{\akirx}
\bmng
(\AmA) heVnu hekukx; kuSxlalxka tapupxgaLanunx huDukuvudaralilx toDagiru. 
\emng
\eentry

\bentry
\word{nit-picker}
\pron{niTfpikarf}
\gl{\nA}
\bmng
(\AmA) heVnu hekukxvava; kuSxlalxka tapupxgaLanunx huDukuvava. 
\emng
\eentry

\bentry
\word[nit-picking(1)]{nit-picking}
\pron{niTfpikiMgf}
\gl{\gu}
\bmng
(\AmA) kuSxlalxka tapupxgaLanunx huDukuva; `heVnu hekukxva'. 
\emng
\eentry

\bentry
\word[nit-picking(2)]{nit-picking}
\pron{niTfpikiMgf}
\gl{\nA}
\bmng
(\AmA) kuSxlalxka tapupxgaLa huDukuvike. 
\emng
\eentry

\bentry
\word{niter}
\pron{neYTarf}
\gl{\nA}
\bmng
(\ame) \hyperlink{nitre}{nitre} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{nitery}
\pron{neYTari}
\gl{\nA}
\bmng
(\ame) (\AmA) = \hyperlink{nightclub}{nightclub}. 
\emng
\eentry

\bentry
\word{nitid}
\pron{niTiDf}
\gl{\gu}
\bmng
(\kAparx) parxkAshamAnavAda; hoLeyuva (\rUpa\ saha). 
\emng
\eentry

\bentry
\word{nitinol}
\pron{niTinAlf}
\gl{\nA}
\bmng
niTinAlf; nikalf matutx TeYTeVniyaMgaLa oMdu misharxloVha. 
\emng
\eentry

\bentry
\word[nitrate(1)]{nitrate}
\pron{neYTerxVTf}
\gl{\nA}
\bmng
(\ravi) 
\bnum
\num{1} neYTerxVTf; neYTirxkf Amalxda lavaNa yA esaTxru. 
\num{2} PalavatAkxrakavAgi upayoVgisuva soVDiyamf yA poTAYxsiyamf neYTerxVTf. 
\enum
\emng
\eentry

\bentry
\word[nitrate(2)]{nitrate}
\pron{neYTerxVTf}
\gl{\sakirx}
\bmng
(\ravi) neYTirxVkarisu; sArayuta neYTirxkf Amalx yA sArayuta salUPxyXrikf Amalx matutx sArayuta neYTerxVTf AmalxgaLa misharxNavanunxpayoVgisi kAbaRnika saMyukatx oMdara heYDorxjanf paramANugaLanunx neYTorx \eng{$(\bg\rm NO\eg_2)$}paramANu gucaCxgaLiMda AdeVshisu. 
\emng
\eentry

\bentry
\word{nitration}
\pron{neYTerxVSanf}
\gl{\nA}
\bmng
(\ravi) neYTirxVkaraNa; neYTirxVkarisuva kirxye. 
\emng
\eentry

\bentry
\word{nitre}
\pron{neYTarf}
\gl{\nA}
\bmng
(\birx) (\ravi) neYTarf; poTAYxsiyamf neYTerxVTf. 
\emng
\eentry

\bentry
\word{nitric}
\pron{neYTirxkf}
\gl{\gu}
\bmng
(\ravi) neYTirxkf; (\kanmu\ paMcaveVlenisxVya) neYTorxjanf uLaLx. 
\emng
\eentry

\bentry
\wordnospeech{nitric acid}{nitric acid}
\pron{?}
\gl{\nA}
\bmng
(\ravi) neYTirxkf Amalx. neYTirxVkarisalu, soPxVTakagaLanUnx PalavatAkxrakagaLanUnx tayArisalu upayoVgisuva oMdu kAbARnika Amalx, \eng{$\bg\rm HNO\eg_3$.} 
\emng
\eentry

\bentry
\wordnospeech{nitric oxide}{nitric oxide}
\pron{?}
\gl{\nA}
\bmng
(\ravi) neYTirxkf AkesxYDf; neYTorxjanf yA amoVniyada utakxSaRNadiMda tayArisuva, vAyuvinalilxruva AkisxjaninxnoMdige sulaBavAgi saMyoVjisi kaMdubaNaNxda neYTorxjanf DeYAkesxYDf koDuva vaNaRrahita anila, \eng{NO.} 
\emng
\eentry

\bentry
\word{nitride}
\pron{neYTerxYDf}
\gl{\nA}
\bmng
(\ravi) neYTerxYDf; yAvudeV rAsAyanika dhAtu neYTorxjaninxnoDane saMyoVgagoMDu utapxtitx mADuva saMyukatx. 
\emng
\eentry

\bentry
\word{nitrifiable}
\pron{neYTirxpheYabflf}
\gl{\gu}
\bmng
neYTerxVTiVkarisabahudAda. 
\emng
\eentry

\bentry
\word{nitrification}
\pron{neYTirxphikeVSanf}
\gl{\nA}
\bmng
(\ravi) neYTerxVTiVkaraNa; neYTerxVTf Agi yA neYTirxkf AmalxvAgi parivatiRsuvudu, \kanmu\ bAyxkiTxVriyAgaLu amoVniyamf saMyukatxgaLanunx utakxSiRsi neYTerxVTf Agi parivatiRsuvudu. 
\emng
\eentry

\bentry
\word{nitrify}
\pron{neYTirxpheY}
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{nitrifies,} \BU\ matutx \BUkaq\ \eng{nitrified).}}
\bmng
(\ravi) neYTerxVTiVkarisu; (\kanmu\ bAyxkiTxVriyAgaLa \vi) amoVniyamf saMyukatxgaLanunx neYTerxVTfgaLAgi parivatiRsu. 
\emng
\eentry

\bentry
\word{nitrile}
\pron{neYTerxYlf}
\gl{\nA}
\bmng
(\ravi) neYTerxYlf; kAbaRnika sayaneYDf; yAvudeV kAbaRnika saMyukatxdalilx heYDorxjanf paramANuvu sayaneYDf \eng{(CN)} rAyxDikalfniMda AdeVshagoMDu uMTAgiruva saMyukatx. 
\emng
\eentry

\bentry
\word{nitrite}
\pron{neYTerxYTf}
\gl{\nA}
\bmng
(\ravi) neYTerxYTf; neYTarxsf Amalxda lavaNa yA esaTxru. 
\emng
\eentry

\bentry
\word{nitro-}
\pron{neYTorxV-}
\gl{\sapUpa}
\bmng
(\ravi) 
\bnum
\num{1} neYTirxkf AYxsiDf, neYTfrf yA neYTorxjanf uLaLx yA adakekx saMbaMdhisida. 
\num{2} ivugaLiMda yA ivugaLalolxMdariMda mADida. 
\num{3} EkaveVlanisxVya -- \eng{$\bg\rm NO\eg_2$} gucaCxvuLaLx. 
\enum
\emng
\eentry

\bentry
\word{nitrobenzene}
\pron{neYTorxVbeni\char144Vnf}
\gl{\nA}
\bmng
(\ravi) neYTorxVbeni\char144Vnf; neYTirxVkaraNa kirxyeyiMda beni\char144Vnina oMdu heYDorxjanf paramANuvanunx neYTorxVgucaCxdiMda AdeVshisidAga baruva utapxnanx, \eng{$\bg\rm C\eg_6\bg\rm H\eg_5\bg\rm NO\eg_2$.} 
\emng
\eentry

\bentry
\word{nitrocellulose}
\pron{neYTorxVseluyxloVsf(sfZ)}
\gl{\nA}
\bmng
(\ravi) neYTorxseluyxloVsf; meragu koDuva teYlagaLu, seluyxlAyfDx muMtAdavugaLa tayArikeyalilx baLasuva seluyxloVsf neYTerxVTf. 
\emng
\eentry

\bentry
\word{Nitro-chalk}
\pron{neYTorxVcAkf}
\gl{\nA}
\bmng
\eng{P}(\ravi) neYTorxVcAkf; kAyxlisxyaM kAboRneVTf matutx amoVniyamf neYTerxVTfgaLanonxLagoMDa PalavatAkxraka. 
\emng
\eentry

\bentry
\word{nitrocompound}
\pron{neYTorxVkAMpwMDf}
\gl{\nA}
\bmng
(\ravi) neYTorx saMyukatx; heYDorxVjanf paramANugaLa sAthxnadalilx neYTorx gucaCxviruva yAvudeV kAbaRnika saMyukatx. 
\emng
\eentry

\bentry
\word{nitrogen}
\pron{neYTarxjanf}
\gl{\nA}
\bmng
(\ravi) neYTorxjanf; sArajanaka; paramANu saMKeyx \eng{7}, paramANu tUka \eng{14} iruva, vAyumaMDaladalilx sumAru sheVkaDa \eng{80}raSiTxruva, rAsAyanika paTutavx kaDame iruva aniladhAtu, \saMkeV\ \eng{N.} 
\emng
\eentry

\bentry
\wordnospeech{nitrogen cycle}{nitrogen cycle}
\pron{?}
\gl{\nA}
\bmng
(\ravi) neYTorxjanf cakarx; parxkaqtiyalilx maNiNxniMda sasayxgaLige, sasayxgaLiMda pArxNigaLige, jeYvika vasutxgaLa koLeyuvikeyiMda punaH maNiNxge I riVti vividha riVtiyalilx cakArxkAradalilx neYTorxjanf vagARvaNegoLuLxtitxruvudu. 
\emng
\eentry

\bentry
\wordnospeech{nitrogen fixation}{nitrogen fixation}
\pron{?}
\gl{\nA}
\bmng
neYTorxjanf sithxriVkaraNa; 
\banum
\alnum{a} vAyumaMDaladalilxruva neYTorxjananxnunx rAsAyanika kirxyege oLapaDisi, Gana yA darxva saMyukatxvAgi parivatiRsuvudu. 
\alnum{b} maNiNxnalilxruva bAyxkiTxVriyagaLu vAyumaMDaladalilxruva neYTorxjananxnunx meYgUDisikoMDu sasayxgaLige odagisalu anukUlavAguvaMte neYTerxVTugaLAgi parivatiRsuvudu. 
\eanum
\emng
\eentry

\bentry
\word{nitrogenous}
\pron{neYTArxjinasf}
\gl{\gu}
\bmng
(\ravi) neYTorxjanfyukatx; neYTorxjananxnonxLagoMDiruva. 
\emng
\eentry

\bentry
\word{nitroglycerin}
\pron{neYTorxVgilxsarinf}
\gl{\nA}
\bmng
= \hyperlink{nitroglycerine}{nitroglycerine}. 
\emng
\eentry

\bentry
\word{nitroglycerine}
\pron{neYTorxVgilxsarinf}
\gl{\nA}
\bmng
(\ravi) neYTorxgilxsarinf; sArayuta salUphxyXrikf matutx neYTirxkf AmalxgaLa misharxNavu gilxsarAlf meVle vatiRsi utapxtitx mADuva soPxVTaka saMyukatx, gilxsarAlf neYTerxVTf. 
\emng
\eentry

\bentry
\wordnospeech{nitro group}{nitro group}
\pron{?}
\gl{\nA}
\bmng
(\ravi) neYTorx (paramANu) gucaCx, \eng{$\bg\rm NO\eg_2$.} 
\emng
\eentry

\bentry
\word{nitro-lime}
\pron{neYTorxVleYmf}
\gl{\nA}
\bmng
(\ravi) neYTorxleYmf; PalavatAkxrakavAgi upayoVgisuva kAyxlisxyamf seYyanameYDf. 
\emng
\eentry

\bentry
\word{nitro-powder}
\pron{neYTorxVpwDarf}
\gl{\nA}
\bmng
neYTorxpuDi; neYTirxkf AmalxdiMda tayArisida tupAki madudx. 
\emng
\eentry

\bentry
\word{nitrous}
\pron{neYTarxsf}
\gl{\gu}
\bmng
(\ravi) neYTarxsf; (\kanmu\ kaDame veVlenisxya) neYTorxjanf uLaLx. 
\emng
\eentry

\bentry
\wordnospeech{nitrous acid}{nitrous acid}
\pron{?}
\gl{\nA}
\bmng
(\ravi) neYTarxsf Amalx; dArxvaNadalilx hAgU lavaNa yA esaTxrf rUpadalilx mAtarx paricitavAgiruva asithxra saMyukatx, \eng{$\bg\rm HNO\eg_2$.} 
\emng
\eentry

\bentry
\wordnospeech{nitrous oxide}{nitrous oxide}
\pron{?}
\gl{\nA}
\bmng
(\ravi) neYTarxsf AkesxYDf; saMveVdanAhAriyAgi baLasuva, naguva anila eMba hesariniMda kareyalAgiruva, vaNaRrahita anilarUpadalilxruva neYTorxjanf AkesxYDf, \eng{$\bg\rm N\eg_2\bg\rm O\eg$.} 
\emng
\eentry

\bentry
\word{nitty-gritty}
\pron{niTigirxTi}
\gl{\nA}
\bmng
(\ashi) vasutxsithxti; vAsatxvAMshagaLu yA oMdu viSayakekx saMbaMdhisida mUlaBUta vicAragaLu. 
\emng
\eentry

\bentry
\word{nitwit}
\pron{niTfviTf}
\gl{\nA}
\bmng
(\AmA) daDaDx; gAMpa; gugugx. 
\emng
\eentry

\bentry
\word{nitwitted}
\pron{niTfviTiDf}
\gl{\gu}
\bmng
daDaDx; gAMpatanada; gugugx; mUKaRtanada. 
\emng
\eentry

\bentry
\word{nitwittedness}
\pron{niTfviTiDfnisf}
\gl{\nA}
\bmng
daDaDxtana; gAMpatana; mUKaRtana. 
\emng
\eentry

\bentry
\word[nix(1)]{nix}
\pron{nikfsx}
\gl{\BAavayx}
\bmng
(\ashi) yajamAna, adhikAri, \mo varu barutitxdAdxreMdu sahoVdoyxVgigaLige koDuva munenxcacxrike. 
\emng
\eentry

\bentry
\word[nix(2)]{nix}
\pron{nikfsx}
\gl{\nA}
\bmng
(\sitxrXV\ \eng{nixie} \ucAcx-- nikisx). jaladeVvate; jalakaninxke. 
\emng
\eentry

\bentry
\word[nix(3)]{nix}
\pron{nikfsx}
\gl{\nA}
\bmng
\bnum
\num{1} (\ashi) EnU ilalx; shUnayx. 
\num{2} nirAkaraNe; ilalxvenunxvudu. 
\enum
\emng
\eentry

\bentry
\word[nix(4)]{nix}
\pron{nikfsx}
\gl{\sakirx}
\bmng
(\ashi). 
\bnum
\num{1} radudxmADu. 
\num{2} nirAkarisu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NJ}{NJ}
\pron{?}
\gl{\saMkiSx}
\bmng
(\ame) \eng{New Jersey}. 
\emng
\eentry

\bentry
\wordnospeech{NM}{NM}
\pron{?}
\gl{\saMkiSx}
\bmng
(\ame) \eng{New Mexico.} 
\emng
\eentry

\bentry
\wordnospeech{n.m.}{n.m.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{nautical mile.} 
\emng
\eentry

\bentry
\wordnospeech{N.Mex}{N.Mex}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{New Mexico.} 
\emng
\eentry

\bentry
\wordnospeech{NMR}{NMR}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{(nmr} saha) \eng{nuclear magnetic resonance.} 
\emng
\eentry

\bentry
\wordnospeech{NNE}{NNE}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{north-north-east.} 
\emng
\eentry

\bentry
\wordnospeech{NNW}{NNW}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{north-north-west.} 
\emng
\eentry

\bentry
\wordspecial{No}{1}{1}{\hyperlink{No(2)}{\textcolor{superscript}{$^2$}\eng{No}}}
\pron{}
\gl{\saMkeV}
\bmng
(\ravi) \eng{nobelium} (dhAtu). 
\emng
\eentry

\bentry
\wordspecial{No}{2}{2}{\hyperlink{No(1)}{\textcolor{superscript}{$^1$}\eng{No}}}
\pron{noV}
\gl{\nA}
\bmng
\eng{Noh} enunxvudara rUpAMtara. 
\emng
\eentry

\bentry
\wordnospeech{No.}{No.}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{number.} 
\num{2} (\ame) \eng{North.} 
\enum
\emng
\eentry

\bentry
\word[no(1)]{no}
\pron{noV}
\gl{\gu}
\bmng
\bnum
\num{1} (yAvudU) alalxda; ilalxda: \eng{no circumstances could justify it} yAvudeV saninxveVshavU adanunx samathiRsalAguvudilalx. 
\num{2} miVrida; seVrada; oLapaDada; horatAda: \eng{it is no part of my plan} adu nananx yoVjanege seVrida saMgatiyalalx. \eng{he is no genius} avaneVnU parxtiBA saMpananxnalalx (avanu parxtiBAvaMtara guMpige seVridavaneVnU alalx). 
\num{3} tiVra; tusu; ilalxveV ilalx enunxvaSuTx savxlapx: \eng{he did it in no time} avanu adanunx atayxlapx kAladalilx mADida; avanu adanunx kaSxNAdhaRdalilx mADi mugisida. \eng{it is no distance} adu dUraveV alalx (adu tiVra hatitxra eMbathaRdalilx). 
\num{4} yAvudU sAdhayxvilalx, salalxdu: \eng{there is no accounting for taste} ruciya viSayadalilx hiVgeyeV eMdu vivaraNe niVDalu sAdhayxvilalx. \eng{there was no mistaking what he meant} avana iMgitaveVneMbudanunx tiLiyadiralu sAdhayxveV iralilalx. 
\num{5} `kUDadu', `sAdhayxvilalx', itAyxdi niSeVdhAthaR sUcaka GoVSaNe parxkaTaNegaLalilx adhAyxhAravAgi: \eng{no parking} (ililx) vAhanagaLanunx nililxsakUDadu. \eng{no Popery} poVpf adhikAra ililx salalxdu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{by no} \hyperref{kandict_m.pdf}{M}{means pagu(5)}{means}. 
\num{2} \eng{king or no king} rAja (athavA avana parxButarx) irali yA biDali. 
\num{3} \eng{no account} lekakxkikxlalxda; beleyilalxda; niSapxrXyoVjaka; nirupayukatx: \eng{a man of no account} niSapxrXyoVjaka manuSayx. 
\num{4} \eng{no bishop no king} biSapf ilalxdidadxre rAjanU ilalx. 
\num{5} \eng{no claim(s) bonus} kaMtu riyAyiti; hiMdina avadhiya jiVvavimeya lekakxdalilx haNa paDeyadidadxvarige jiVvavime kaMtu pAvatiyalilx sikukxva riyAyiti. 
\num{6} \eng{no date} (\saMkiSx\ \eng{n.d.)}(pusatxkagaLa \vi) tAriVKinalalxda; parxkaTaNeya tAriVKu namUdisilalxda. 
\num{7} \eng{no dice}(koVrike \mo vugaLa) nirAkaraNe; tirasAkxra; ilalxveMdu heVLuvike: \eng{no asked for a raise, but it was no dice} avanu (tanage) baDitx beVkeMdu keVLida, Adare adu tirasakxqqtavAyitu. 
\num{8} \eng{no doubt} nisasxMdeVhavAgi; KaMDitavAgi; saMdeVhavilalxde. 
\num{9} \eng{no end} koneyilalxdaSuTx; aneVka; bahaLa; apAra. 
\num{10} \eng{no entry}(pAdacArigaLige yA vAhagaLige yAvudoV parxdeVsha yA rasetxyalilx parxveVshavilalxveMdu tiLisuva parxkaTaNe) `parxveVshavilalx', `ililx parxveVshisakUDadu'. 
\num{11} \eng{no-fault}(jiVvavime \vi) Akasimxka maraNa \mo vu AdAga, tapupx yAradedxV AdarU, vimedArana hakukx siMdhuvAguva vayxvasethx. 
\num{12} \eng{no} \hyperref{kandict_f.pdf}{F}{fear(1) nuga(1)}{$^1$fear}. 
\num{13} \eng{No} \hyperref{kandict_f.pdf}{F}{flower(1) pagu(3)}{$^1$flower}s. 
\num{14} \eng{no frills} alaMkAragaLilalxda; saraLa. 
\num{15} \eng{no} -\hyperref{kandict_g.pdf}{G}{go(2) pagu(7)}{$^2$go}. 
\num{16} \eng{no-go area} anadhikaqtarige niSidadhxvAda parxdeVsha. 
\num{17} \eng{no} \hyperref{kandict_g.pdf}{G}{good(3) nuga(11)}{$^3$good}. 
\num{18} \eng{no-hitter}(\ame) taMDada yAva ATagAranU modalaneya `nele'yanunx talupade hoVda `beVsf bAlf' ATa. 
\num{19} \eng{no-hoper}(\AseTxrXV) (\ashi) niSaTxrXyoVjaka (vayxkitx); kelasakekx bAradavanu(Lu). 
\num{20} \eng{no} \hyperref{kandict_j.pdf}{J}{joke(1) pagu(2)}{$^1$joke}. 
\num{21} \eng{no man} yAvanU alalx; obabxnU alalx. 
\numi{22} \eng{no man's land} 
\banum
\alnum{a} pALu nela; yArigU seVrilalxda parxdeVsha. 
\alnum{b} (miliTari) pakaSx parxtipakaSxgaLa eduru baduru kaMdakagaLa naDuvaNa parxdeVsha. 
\alnum{c} (\rUpa) saMdigadhx keSxVtarx; yAvudeV oMdu vishiSaTx viSayada vAyxpitxge oLapaDada keSxVtarx. 
\eanum
\numie
\num{23} \eng{no} \hyperref{kandict_m.pdf}{M}{mistake(1) pagu(1)}{$^1$mistake}. 
\num{24} \eng{no news is good} \hyperref{kandict_n.pdf}{N}{news pagu(4)}{news}. 
\num{25} \eng{no-no}(\AmA) adu sAdhayxvilalx; adanunx opapxlu sAdhayxvilalx (eMbathaRdalilx). 
\num{26} \eng{no nonsense} gaMBiVravAda; huDugATadadxlalxda. 
\numi{27} \eng{no one} 
\banum
\alnum{a} (\sanA) yAvaneV vayxkitx (niSeVdhAthaR). 
\alnum{b} (\gu) oMdeV vayxkitx (niSeVdhAthaR): \eng{no one man could lift it} oMdeV vayxkitxge adanunx etutxvudu sAdhayxviralilalx. 
\eanum
\numie
\num{28} \eng{no place} (\ame) elUlx ilalx; (beVre) yAva sathxLavU ilalx. 
\num{29} \eng{no-show}(parxdashaRna itAyxdigaLalilx) sathxLavanunx kAdirisi, adanunx baLasade yA radudxpaDisade iruvavanu. 
\numi{30} \eng{no side} 
\banum
\alnum{a} (ragibx phuTfbAlf) ATada mukAtxya. 
\alnum{b} ATavu mugiyiteMdu rephariya GoVSaNe. 
\eanum
\numie
\num{31} \eng{no small} gaNaniVyavAda; sAkaSuTx doDaDx. 
\num{32} \eng{no sweat} toMdareyilalx; bAdhakavilalx. 
\num{33} \eng{no thoroughfare} dAriyilalx yA parxveVshavilalx; hAdi, biVdi, \mo vugaLa matotxMdu tudi mucacxlAgideyeMdAgali hAduhoVgalu anumatiyilalxveMdAgali sUcane, tiLivaLike. 
\num{34} \eng{no-trumper} (birxDfDx) turuPilalxdATa heVLabahudAda yA GoVSisida ATagAra. 
\num{35} \eng{no trump(s)}(birxDfjx isipxVTATadalilx) turuPilalx eMdu GoVSisuvudu. 
\numi{36} \eng{no way}(\AmA): 
\banum
\alnum{a} yAva vidhadalUlx ilalx; ilalxveV ilalx. 
\alnum{b} nAnu opupxvudilalx \mo. 
\eanum
\numie
\num{37} \eng{no whence}... elilxMdalU ilalx. 
\num{38} \eng{no} \hyperref{kandict_w.pdf}{W}{whit pagu(2)}{whit}. 
\num{39} \eng{no whither} elilxgU ilalx. 
\num{40} \eng{no-win} gelalxlu, yashasivxyAgalu sAdhayxvilalxda parisithxti. 
\num{41} \eng{no} \hyperref{kandict_w.pdf}{W}{wonder(1) pagu(6)}{$^1$wonder}. 
\num{42} \eng{or no} idadxrU; AdarU; irali biDali: \eng{rain or no rain, I shall go out} maLeyirali biDali, nAnu horage hoVgutetxVne. 
\num{43} \eng{there is no}.....\eng{ing}... vudu sAdhayxvilalx. \eng{there was no mistaking what he meant} avana iMgitavanunx tapApxgi tiLidukoLuLxvudu sAdhayxveV ilalx. 
\enum
\emng
\eentry

\bentry
\word[no(2)]{no}
\pron{noV}
\gl{\kirxvi}
\bmng
\bnum
\num{1} irali biDali; ilalxdirali (yAvAgalU \eng{or} padada muMde; pakASxMtaravAgi): \eng{pleasant or no, it is true} adeSeTxV hitada saMgatiyAgirali biDali, adaMtU nija. 
\num{2} (sAkxTelxMDf) ilalx: \eng{will ye no come back again?} niVnu matetx hiMtirugi baruvudilalxveV? 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} (\AmA) \eng{no can do} nananx keYli sAdhayxvilalx, nAnu mADalAre. 
\hyperdef{N}{no(2) pagu(2)}{} 
\num{2} \eng{whether or no} irali, iradirali -- eraDu saMdaBaRgaLalUlx; AdarU, AgadidadxrU: \eng{tell me whether or no} uMToV ilalxvoV heVLibiDu. 
\enum
\emng
\eentry

\bentry
\word[no(3)]{no}
\pron{noV}
\gl{\kirxvi}
\bmng
(yAvAgalU `tara' rUpagaLoDane) savxlapxvU ilalx; koMcavU ilalx: \eng{she is no better than before} avaLu modaligiMta utatxmaveVnU Agilalx. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{ is no better than she should be} (shiVlada \vi) avaLu aSeTxVnU yoVgayxLalalx. 
\numi{2} \eng{is no less}( --giMta) 
\banum
\alnum{a} kaDameyilalxdaSuTx \eng{[is no less than} saha]: \eng{he gave me Rs. 50, no less (he gave me no less than Rs. 50)} avanu nanage \eng{50} rU.gaLige kaDame ilalxdaSuTx haNa koTaTx. 
\alnum{b} ( --giMta) kaDame parxmuKanalalxda yA pArxmuKayxvalalxda: \eng{no less a person than Gandhi} gAMdhigiMta kaDame parxmuKanalalxdavaneV (aMdare gAMdhiyaMthavaneV). 
\eanum
\numie
\num{3} \eng{is no more} ininxlalx; maqta. 
\num{4} \eng{no longer} = \hyperref{kandict_l.pdf}{L}{long(3)3}{$^3$long(3)}. 
\numi{5} \eng{no more} 
\banum
\alnum{a} (\nA) hecicxge ilalx: \eng{no more to say} idakikxMta hecicxge heVLuvudu EnU ilalx. 
\alnum{b} (\gu) inanxSiTxlalx: \eng{no more wine?} inanxSuTx madayxvilalxvoV? 
\alnum{c} (\kirxvi) jiVvaMtavAgilalx; jiVvadiMdilalx; \eng{is no more} avanu jiVvaMtavAgilalx; avanu tiVrikoMDa. 
\alnum{d} (\kirxvi) hecicxgeyAgi ilalx; visheVSavAgi ilalx. \eng{he is no more a lord than I am} nanagiMta avaneVnU visheVSa riVtiya shirxVmaMtanalalx (aMdare, nAneSaTxra maTiTxge shirxVmaMtanoV avanU aSeTxV). 
\alnum{e} ibabxrU ilalx: \eng{you did not come,and no more did he}... niVnu baralilalx; avanU baralilalx. 
\alnum{f} inunx muMde ilalx. 
\alnum{g} matetxMdU, inenxMdigU ilalx. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[no(4)]{no}
\pron{noV}
\gl{\kirxvi}
\bmng
\bnum
\num{1} ilalx; Agadu; Aguvudilalx; sariyalalx; taqpitxkaravalalx. 
\num{2} (otutx hAki heVLuvAga) ilalx; eMdeMdigU ilalx. 
\num{3} mAtarxvalalx (\mo\ mAtugaLanunx baLasuvAga): \eng{a man could not lift it, no nor half a dozen} obabxniMda adanunx etatxlAgalilalx -- mAtarxvalalx, Aru janarigU adu sAdhayxvAgalilalx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{oh no}! 
\banum
\alnum{a} (BayasUcaka udAgxravAgi) ayoyxV, ilalx! 
\alnum{b} KaMDita ilalx, savaRthA ilalx. 
\eanum
\emng
\eentry

\bentry
\word[no(5)]{no}
\pron{noV}
\gl{\nA}
\expl{(\bava\ \eng{noes}).}
\bmng
\bnum
\num{1} \eng{`no'} eMba mAtu; ilalx (eMba mAtu). 
\num{2} nirAkaraNe; ilalxvenunxvudu. 
\num{3} nakArAtamxka OTu, mata. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{say no} 
\banum
\alnum{a} (beVDikeyanunx nirAkarisuvalilx) Aguvudilalx; sAdhayxvilalx. 
\alnum{b} (heVLikeyanunx nirAkarisuvAga) hAge heVLalilalx. 
\eanum
\numie
\num{2} \eng{the noes have it} ilalx enunxvavara mAteV naDeyutatxde; ilalx enunxvavareV hecucx saMKeyxyalilxdAdxre. 
\num{3} \eng{will not take no for an answer}(ilalxveMdu heVLutitxdadxrU balavaMta mADuva saMdaBaRdalilx) niVnu `ilalx' (sAdhayxvilalx) eMdarU nAnu keVLalolelx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{n.o.}{n.o.}
\pron{?}
\gl{\saMkiSx}
\bmng
(kirxkeTf) \eng{not out}. 
\emng
\eentry

\bentry
\word{Noachian}
\pron{noVEkianf}
\gl{\gu}
\bmng
(\beY) noVvana; noVvana kAlada. 
\emng
\eentry

\bentry
\word{Noachic}
\pron{noVEkikf}
\gl{\gu}
\bmng
= \hyperlink{Noachian}{Noachian}. 
\emng
\eentry

\bentry
\wordRemoveSpace{Noah's-ark}{Noah's ark}
\pron{noVasfZ AkfR}
\gl{\nA}
\bmng
(\beY) noVvana nwke: 
\banum
\alnum{a} mahAparxLayadalilx noVvananUnx avana kuTuMbavanUnx itara pArxNigaLanUnx uLisida haDagu. 
\alnum{b} (idara mAdariya) makakxLa ATada haDagu. 
\alnum{c} doDaDx yA aDakavAgilalxda yA purAtana kAlada peTiTxge, peThAri yA vAhana. 
\alnum{d} (doVNiyAkArada cipupxLaLx) icicxpipxna oMdu saNaNx pArxNi. 
\eanum
\emng
\eentry

\bentry
\word[nob(1)]{nob}
\pron{nAbf}
\gl{\nA}
\bmng
(\ashi) tale. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{his nob}(kirxbeVjf isipxVTATa) elegaLanunx kalasuvavanu maguci etitxda eleya raMgina gulAmananunx paDediruvavanige sikukxva oMdu `pAyiMTf' yA gelalxMka. 
\emng
\eentry

\bentry
\word[nob(2)]{nob}
\pron{nAbf}
\gl{\nA}
\bmng
(\birx) (\ashi) (samAjadalilx) meVludajeRyavanu; aishavxyaRvaMta; shirxVmaMta. 
\emng
\eentry

\bentry
\word[no-ball(1)]{no-ball}
\pron{noVbAlf}
\gl{\nA}
\bmng
\bnum
\num{1} (kirxkeTf) lekakxkikxlalxda ceMDu; niyamavirudadhxvAgi eseda ceMDu. 
\num{2} ceMDu lekakxkikxlalxveMdu aMpeYrf mADuva GoVSaNe. 
\enum
\emng
\eentry

\bentry
\word[no-ball(2)]{no-ball}
\pron{noVbAlf}
\gl{\sakirx}
\bmng
bwlaranu niyamavirudadhxvAgi ceMDanenxsedaneMdu GoVSisu, tiVpuR koDu. 
\emng
\eentry

\bentry
\word{nobble}
\pron{nAbflf}
\gl{\sakirx}
\bmng
(\birx) (\ashi) 
\bnum
\num{1} (jUju kudurege mAdaka vasutx \mo vanunx tininxsi) gelalxdaMte (mADalu) kaqtirxma naDesu. 
\num{2} (kaqtirxma mAgaRdiMda obabx vayxkitxyanunx) tananx kaDege olisiko yA dubaRlagoLisu. 
\num{3} (haNa \mo vanunx) moVsadiMda, apArxmANikateyiMda paDe. 
\num{4} (aparAdhiyanunx) hiDi. 
\enum
\emng
\eentry

\bentry
\word{nobbler}
\pron{nAbalxrf}
\gl{\nA}
\bmng
(\AseTxrXV) (\ashi) oMdu baTaTxlu madayx. 
\emng
\eentry

\bentry
\word{nobbut}
\pron{nAbaTf}
\gl{\kirxvi}
\bmng
(\pArxM) keVvala; mAtarx; miVrirade. 
\emng
\eentry

\bentry
\word{nobby}
\pron{nAbi}
\gl{\gu}
\bmng
(\ashi) 
\bnum
\num{1} aishavxyaRvaMtanige takakx, yoVgayxvAda. 
\num{2} ThiVkAda; sogasAda. 
\enum
\emng
\eentry

\bentry
\word{Nobelist}
\pron{noVbelisfTx}
\gl{\nA}
\bmng
(\ame) noVbelf parxshasitx vijeVta. 
\emng
\eentry

\bentry
\word{nobelium}
\pron{noVbiVliamf}
\gl{\nA}
\bmng
(\ravi) noVbiVliyamf; kaqtakavAgi tayArisida \eng{102} paramANu saMKeyxya, yureVniyamf Aceya rAsAyanika dhAtu, \saMkeV\ \eng{No.} 
\emng
\eentry

\bentry
\wordRemoveSpace{Nobel-prize}{Nobel prize}
\pron{noVbelf perxYsfZ}
\gl{\nA}
\bmng
nobelf parxshasitx, bahumAna; parxtivaSaRvU BwtashAsatxrX, rasAyanashAsatxrX, shariVra vijAcnxna yA auSadhashAsatxrX, sAhitayx, athaRshAsatxrX, shAMti sAthxpane, \mo\ viSayagaLalilx vishiSaTx sAdhane mADidavarige (sivxVDaninxnalilx, DeYnameYTf kaMDuhiDida AlfPerxDf nobelfna AdeVshadaMte) niVDalAguva bahumAna, parxshasitx. 
\emng
\eentry

\bentry
\word{nobiliary}
\pron{nabilayxri}
\gl{\gu}
\bmng
shirxVmaMtikeya; utatxma vagaRda. 
\emng

\noindent
\gl{\pagu}
\bmng
 \eng{nobiliary particle}(okakxNeyalilx shirxVmaMta vagaRdavarige baLasuva) gwravasUcaka \upa: \udA\ pherxMcf \eng{de}, jamaRnf \eng{von}. 
\emng
\eentry

\bentry
\word{nobility}
\pron{nobiliTi}
\gl{\nA}
\expl{(\bava\ \eng{nobilities}).}
\bmng
\bnum
\num{1} udAtatx guNa; udAtatxte; doDaDx manasusx; Ganate. 
\num{2} shirxVmaMta padavi; gwravada aMtasutx; kuliVnate. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{a nobility} kuliVna vagaR; shirxVmaMta vagaR. 
\num{2} \eng{the nobility} shirxVmaMta vagaRdavaru. 
\enum
\emng
\eentry

\bentry
\word[noble(1)]{noble}
\pron{noVbflf}
\gl{\gu}
\expl{(\tara\ \eng{nobler}, \tama\ \eng{noblest}).}
\bmng
\bnum
\num{1} (aMtasitxniMda, birudiniMda yA huTiTxniMda) sherxVSaThx; parxsidadhx; kiVtiRvaMta. 
\num{2} kuliVna; shirxVmaMta vagaRkekx seVrida. 
\num{3} udAtatx guNa yA AdashaRgaLuLaLx. 
\num{4} mahoVnanxta guNavuLaLx; udAra hqdayada; udAtatx niVtiya. 
\num{5} GanavAda; ujavxlavAda; bahugaMBiVravAda; viBavavuLaLx; parxBAvayutavAda. 
\num{6} atuyxtatxmavAda; shAlxGayxvAda; sogasAda: \eng{a noble horse} atuyxtatxmavAda kudure. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{the noble science} muSiTx kALaga. 
\emng
\eentry

\bentry
\word[noble(2)]{noble}
\pron{noVbflf}
\gl{\nA}
\bmng
\bnum
\num{1} = \hyperlink{nobleman}{nobleman}. 
\num{2} = \hyperlink{noblewoman}{noblewoman}. 
\num{3} \eng{6} SiliMgf, \eng{8} peninx beleyuLaLx, (\eng{$1351$}ralilx horaDisidadx) oMdu iMgilxSf cinanxda nANayx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{noble gas}{noble gas}
\pron{?}
\gl{\nA}
\bmng
(\ravi) sherxVSAThxnila; AvataR koVSaTxkadalilx sonenxya guMpige seVruva, hecucxkaDame rAsAyanika paTutavxveV ilalxdiruva jaDAnilagaLAda hiVliyamf, niyAnf, modalAda anila dhAtugaLalilx oMdu. 
\emng
\eentry

\bentry
\word{nobleman}
\pron{noVbalfmanf}
\gl{\nA}
\expl{(\bava\ \eng{noblemen}).}
\bmng
kuliVna; doDaDx manetanadavanu; shirxVmaMta vagaRdavanu; unanxta padaviyuLaLxvanu. 
\emng
\eentry

\bentry
\wordnospeech{noble metal}{noble metal}
\pron{?}
\gl{\nA}
\bmng
sherxVSaThx loVha; Amalx yA kASxragaLa kirxyege sulaBavAgi oLagAgadiruva beLiLx, cinanx, pAlxyXTinamf, \mo\ yAvudeV loVha. 
\emng
\eentry

\bentry
\word{noble-minded}
\pron{noVbflfmeYMDiDf}
\gl{\gu}
\bmng
udAra manasisxna; udAtatx BAvada. 
\emng
\eentry

\bentry
\word{nobleness}
\pron{noVbflfnisf}
\gl{\nA}
\bmng
sherxVSaThxte; udAtatxte; Ganate; udArate; shirxVmaMtike, haqdayaveYshAlayx. 
\emng
\eentry

\bentry
\wordnospeech{noble savage}{noble savage}
\pron{?}
\gl{\nA}
\bmng
udAtatx anAgarika; AdashaR anAgarika; romAyxMTikf sAhitayxdalilx AdashiRVkarisuva pArxciVna mAnava. 
\emng
\eentry

\bentry
\wordf{noblesse}
\pron{noVbelxsf}
\gl{\nA}
\expl{\F}
\bmng
(\kanmu\ hora deVshada) shirxVmaMta vagaR; shirxVmaMta vagaRdavaru. 
\emng
\eentry

\bentry
\wordf{noblesse oblige}
\pron{nAbilxVsfZ AbilxVSfZ}
\gl{\nA}
\expl{\F}
\bmng
(doDaDx padaviyalilxruvavaru doDaDxtanadiMdalU vatiRsabeVku, shirxVmaMtikeya hakukx eMdare shirxVmaMta hoNegArikeyU adaralilxrabeVku -- eMba athaRgaLalilx) savalatutx hoNegArikeyanunx oLagoLuLxtatxde. 
\emng
\eentry

\bentry
\word{noblewoman}
\pron{noVbalfvumanf}
\gl{\nA}
\expl{(\bava\ \eng{noblewomen}).}
\bmng
kuliVne; shirxVmaMta sitxrXV; kuliVna manetanadavaLu. 
\emng
\eentry

\bentry
\word{nobly}
\pron{noVbilx}
\gl{\kirxvi}
\bmng
shirxVmaMtikeyiMda; audAyaRdiMda; haqdayavishAlateyiMda; sherxVSaThx riVtiyalilx; udAtatx mAgaRdalilx. 
\emng
\eentry

\bentry
\word[nobody(1)]{nobody}
\pron{nobaDi}
\gl{\sanA}
\expl{(\bava\ \eng{nobodies}).}
\bmng
\bnum
\num{1} obabxnU alalx; yAvanU alalx. 
\num{2} nagaNayx; gaNanege bAradavanu; anAmadheVya; adhikAra, padavi, sAthxna, \mo vugaLilalxdavanu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{like nobody's} \hyperref{kandict_b.pdf}{B}{business nuga(7)}{business}. 
\num{2} \eng{nobody's} \hyperref{kandict_f.pdf}{F}{fool(1) nuga(9)}{$^1$fool}. 
\enum
\emng
\eentry

\bentry
\word[nobody(2)]{nobody}
\pron{noVbaDi}
\gl{\nA}
\bmng
agaNayx; mahatavx, adhikAra yA sAthxnamAna -- ilalxdava. 
\emng
\eentry

\bentry
\word[nock(1)]{nock}
\pron{nAkf}
\gl{\nA}
\bmng
\bnum
\num{1} bililxna kopupx; bililxna tudigaLalilx (jAradaMte) hedeyanunx hiDidiralu mADiruva kacucx. 
\num{2} bililxna hede kUDuvaMte mADiruva aMcina (koMbu tudiya) kacucx. 
\enum
\emng
\eentry

\bentry
\word[nock(2)]{nock}
\pron{nAkf}
\gl{\sakirx}
\bmng
aMbu hUDu; bANa saMdhAna mADu. 
\emng
\eentry

\bentry
\word{noctambulant}
\pron{nAkATxYxMbuyxlaMTf}
\gl{\gu}
\bmng
nishAcara; rAtirxcara; rAtirx saMcarisuva. 
\emng
\eentry

\bentry
\word{noctambulism}
\pron{nAkATxYxMbuyxlisaZmf}
\gl{\nA}
\bmng
nidArxsaMcAra; niderxyalilx ODATa mADuvudu. 
\emng
\eentry

\bentry
\word{noctambulist}
\pron{nAkATx YxMbuyxlisfTx}
\gl{\nA}
\bmng
nidArxsaMcAri; niderxyalilx naDedADuvavanu. 
\emng
\eentry

\bentry
\word{noctambulous}
\pron{nAkATx YxMbuyxlasf}
\gl{\gu}
\bmng
= \hyperlink{noctambulant}{noctambulant}. 
\emng
\eentry

\bentry
\word{noctivagant}
\pron{nAkiTxvagaMTf}
\gl{\gu}
\bmng
nishAcara; rAtirx aleyuva. 
\emng
\eentry

\bentry
\word{noctivagous}
\pron{nAkiTxvagasf}
\gl{\gu}
\bmng
= \hyperlink{noctivagant}{noctivagant}. 
\emng
\eentry

\bentry
\word{noctule}
\pron{nAkUTx yxlf}
\gl{\nA}
\bmng
(birxTaninxna atayxMta doDaDx) nikaTxlasf nAkuTx yxla kulada bAvali. 
\emng
\eentry

\bentry
\word{nocturn}
\pron{nAkaTxnfR}
\gl{\nA}
\bmng
(\roVkAyx, cacfR) rAtirxgiVta; hiMde rAtirx hADutitxdadx Bakitx giVteya yA pArxthaRneya hADina BAga. 
\emng
\eentry

\bentry
\word{nocturnal}
\pron{nAkaTxnaRlf}
\gl{\gu}
\bmng
\bnum
\num{1} rAtirxya; rAtirxyalilxya. 
\num{2} rAtirx mADida; rAtirx naDeyuva; rAtirx caTuvaTikeya. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nocturnal emission}{nocturnal emission}
\pron{?}
\gl{\nA}
\bmng
savxpanxsaKxlana; tAneVtAnAgi niderxyalilx viVyaRda hora sUsuvike, visajaRne. 
\emng
\eentry

\bentry
\word{nocturnally}
\pron{nAkaTxnaRli}
\gl{\kirxvi}
\bmng
rAtirxyalilx; rAtirxyalilx naDeyuva riVtiyalilx. 
\emng
\eentry

\bentry
\word{nocturne}
\pron{nAkaTxnfR}
\gl{\nA}
\bmng
\bnum
\num{1} savxpanxmayavAda giVta, hADu, saMgiVta kaqti. 
\num{2} rAtirx daqshayxda citarx. 
\enum
\emng
\eentry

\bentry
\word{nocuous}
\pron{nAkuyxasf}
\gl{\gu}
\bmng
(sAhitayxka) holasina; keDukAda; anAroVgayxkara yA toMdareyanunxMTumADuva. 
\emng
\eentry

\bentry
\word[nod(1)]{nod}
\pron{nADf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nodded}, \vakaq\ \eng{nodding}).}
\bmng
\bnum
\num{1} (tale) bagigxsu; bAgisu; Ore mADu. 
\num{2} (opipxge \mo vanunx) taletUgi sUcisu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} namisu; talebAgisu. 
\num{2} (vaMdane, opipxge, apapxNe ivanunx sUcisalu) tale Olisu, bAgisu, tUgu; goVNu hAku. 
\num{3} tUkaDisu; maMparadiMdiru. 
\num{4} (yAvudoV kaSxNadalilx) ecacxrikeyilalxde savxlapx tapupx mADu. 
\num{5} (kaTaTxDa \mo vugaLa \vi\ \rUpa\ saha) Olu; oMdu kaDege bAgu: \eng{nodding to its fall} biVLuvaSuTx Olutatxde. 
\num{6} (hakikxya gari, pukakx, \mo vugaLa \vi) ODADuvAga meVlakUkx keLakUkx -- tUgADu, tone, alAlxDu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{Homer sometimes nods} hoVmaranU omomxmemx tUkaDisutAtxne; eMtha patxtiBAshAliyU omomxmemx eDavabahudu. tapupx mADabahudu, niVrasavAgabahudu. 
\num{2} \eng{nodding acquaintance}(vayxkitx yA viSayada bagege) alapx paricaya; laGu paricaya. 
\num{3} \eng{nod off}(\AmA) nidedxhoVgu; nidirxsibiDu. 
\numi{4} \eng{nod through}(\AmA) 
\banum
\alnum{a} taleyADisu; opupx. 
\alnum{b} (\birx\ pAliRmeMTu) voVTf mADalu sAdhayxvAgadidAdxgalU voVTf mADidaMte pAliRmeMTf sadasayxnanunx eNisu, lekakxkekx tegeduko. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[nod(2)]{nod}
\pron{nADf}
\gl{\nA}
\bmng
\bnum
\num{1} taledUguvike. 
\num{2} (niraMkusha adhikAra yA opipxgeya sUcaneyAgi) talebAguvike; namana. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{get the nod} (\ame) AyekxyAgu yA anumoVdane paDe. 
\num{2} \eng{land of Nod} nidArxloVka; nidedx. 
\numi{3} \eng{on the nod} (\AmA) 
\banum
\alnum{a} sAlavAgi. 
\alnum{b} (caceRyilalxde) keVvala vidhuyxkatxvAda opipxgeyaMte. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{nodal}
\pron{noVDalf}
\gl{\gu}
\bmng
geNuNx, gaMTu, garxMthi, saMdhisAthxna, \mo vugaLa yA avakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{noddingly}
\pron{nADiMgfli}
\gl{\kirxvi}
\bmng
(taleyADisi) opupxvaMte. 
\emng
\eentry

\bentry
\word[noddle(1)]{noddle}
\pron{nADflf}
\gl{\nA}
\bmng
(\AmA) tale. 
\emng
\eentry

\bentry
\word[noddle(2)]{noddle}
\pron{nADflf}
\gl{\sakirx}
\bmng
(tale) bAgisu; alAlxDisu. 
\emng
\eentry

\bentry
\word{noddy}
\pron{nADi}
\gl{\nA}
\expl{(\bava\ \eng{noddies}).}
\bmng
\bnum
\num{1} heDaDx; daDaDx; gugugx. 
\num{2} uSaNxvalayada oMdu kaDala hakikxjAti, samudarxpakiSx jAti. 
\enum
\emng
\eentry

\bentry
\word{node}
\pron{noVDf}
\gl{\nA}
\bmng
\bnum
\numi{1} (\savi) 
\banum
\alnum{a} geNuNx; beVru yA reMbeyalilx kANabaruva gaMTu. 
\alnum{b} ele horaDuva sathxLa; paNaRmUla. 
\eanum
\numie
\num{2} (\aMrashA) gaMTu; deVhada yAvudeV BAgadalilx yA aMgadalilx Aguva ububx. 
\num{3} (\Kavi) pAta; saMpAta; garxhada kakeSx hAgU kArxMtivaqtatxgaLu yA KagoVLada yAvudeV eraDu mahAvaqtatxgaLu saMdhisuva biMdu. 
\numi{4} (\Bwvi) nisapxMda: 
\banum
\alnum{a} (kaMpisutitxruva kAyada \vi) nishacxla biMdu. 
\alnum{b} viduyxtapxrXvAha yA voVleTxVju sonenxyAguva biMdu. 
\eanum
\numie
\num{5} (\ga) garxMthi; yAvudeV vakarx reVKeyu tananxnenxV saMdhisuva biMdu. 
\num{6} gArxyXphfnalilx shaqMga. 
\num{7} noVDf; kaMpUyxTarf jAladalilx oMdu GaTaka. 
\enum
\emng
\eentry

\bentry
\word{nodi}
\pron{noVDeY}
\gl{\nA}
\bmng
\hyperlink{nodus}{nodus} eMbudara \bava. 
\emng
\eentry

\bentry
\word{nodical}
\pron{noVDikalf}
\gl{\gu}
\bmng
(\Kavi) saMpAtada; saMpAtakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nodose}
\pron{naDoVsf}
\gl{\gu}
\bmng
garxMthila; garxMthigaLiruva; gaMTugaMTAda; gubuTugaLuLaLx. 
\emng
\eentry

\bentry
\word{nodosity}
\pron{naDAsiTi}
\gl{\nA}
\bmng
gaMTugaMTAgiruvike; garxMthilate. 
\emng
\eentry

\bentry
\word{nodular}
\pron{nADuyxlarf}
\gl{\gu}
\bmng
gaMTugaMTAda; gubuTAda; gaMtiyaMtha. 
\emng
\eentry

\bentry
\word{nodulated}
\pron{nADuyxleVTiDf}
\gl{\gu}
\bmng
gaMTugaMTAda; gubuTAda; gaMtirUpada. 
\emng
\eentry

\bentry
\word{nodulation}
\pron{nADuyxleVSanf}
\gl{\nA}
\bmng
gaMTugaMTAgiruvike; gubuTAgiruvike. 
\emng
\eentry

\bentry
\word{nodule}
\pron{nADUyxlf}
\gl{\nA}
\bmng
\bnum
\num{1} (yAvudAdarU padAthaRda, \udA\ tAMDavALadalilxna iMgAla, samudarxdaMDeyalilxna Kanija, \mo vugaLa) saNaNx guMDu, goVli, gaMTu. 
\num{2} gaMTu; gaDeDx; divxdaLa dhAnayxgaLaMtha leguyxminiVsiV vaMshada sasayxgaLa beVrugaLa meVle kaMDubaruva, bAyxkiTxVriyAgaLanonxLagoMDa gaMTu. 
\enum
\emng
\eentry

\bentry
\word{nodulose}
\pron{nADuyxloVsf}
\gl{\gu}
\bmng
= \hyperlink{nodulous}{nodulous}. 
\emng
\eentry

\bentry
\word{nodulous}
\pron{nADuyxlasf}
\gl{\gu}
\bmng
saNaNx saNaNx gaMTugaLuLaLx. 
\emng
\eentry

\bentry
\word{nodus}
\pron{noVDasf}
\gl{\nA}
\expl{(\bava\ \eng{nodi} \ucAcx -- noVDeY).}
\bmng
(kathAvasutx \mo vugaLalilx) kilxSaTxte; saMkiVNaRvAda aMsha; gaMTu(kaTiTxkoMDiruva) aMsha; toDaku; samaseyx; saMkiVNaRte. 
\emng
\eentry

\bentry
\word{Noel}
\pron{noVelf}
\gl{\nA}
\bmng
(\kanmu) kirxsfmasf (hababxda AnaMdagiVtadalilx, saMtoVSasUcaka \BAavayx vAgi). 
\emng
\eentry

\bentry
\word[noetic(1)]{noetic}
\pron{noVe(I)Tikf}
\gl{\gu}
\bmng
\bnum
\num{1} bwdidhxka; budidhxge saMbaMdhisida. 
\num{2} keVvala bwdidhxkavAda yA amUtaRvAda. 
\num{3} bwdidhxka ciMtaneya; tAkiRka parxvaqtitxya. 
\enum
\emng
\eentry

\bentry
\word[noetic(2)]{noetic}
\pron{noVe(I)Tikf}
\gl{\nA}
\expl{(\bava dalilx saha)}
\bmng
budidhxshAsatxrX. 
\emng
\eentry

\bentry
\word[nog(1)]{nog}
\pron{nAgf}
\gl{\nA}
\bmng
\bnum
\num{1} saNaNx gUTa; marada gUTa; (iTiTxge \mo vugaLoLakekx moLe hoDeyalu anukUlavAguvaMte irisuva) marada gaTuTx, tuMDu. 
\num{2} oMdu marada meVliruva modudx, moVTu. 
\num{3} marada cwkaTuTx, jaMti, \mo vugaLa madheyx iTiTxgeyiMda kaTiTxda kaTaTxDa. 
\enum
\emng
\eentry

\bentry
\word[nog(2)]{nog}
\pron{nAgf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nogged}, \vakaq\ \eng{nogging}).}
\bmng
\bnum
\num{1} moLe, gUTa, \mo vugaLiMda BadarxpaDisu. 
\num{2} marada cwkaTuTx, jaMti, \mo vugaLa madheyx iTiTxge \mo vugaLiMda kaTuTx. 
\enum
\emng
\eentry

\bentry
\word[nog(3)]{nog}
\pron{nAgf}
\gl{\nA}
\bmng
\bnum
\num{1} (\birx) pUvaR AyxMgilxyAdalilx tayArisida tiVkaSxNXvAda biyarf. 
\num{2} = \hyperref{kandict_e.pdf}{E}{egg-flip}{egg-flip}. 
\enum
\emng
\eentry

\bentry
\word{noggin}
\pron{nAginf}
\gl{\nA}
\bmng
\bnum
\num{1} saNaNx koLaga pAterx; kuDiyuva pAterx; magugx. 
\num{2} (\sA) \eng{$\frac\bg 1\eg /\bg 4\eg$} peYMTf \eng{(pint)}(aLateya) pAterx. 
\num{3} (\ame) (\ashi) tale; maMDe. 
\enum
\emng
\eentry

\bentry
\word{nogging}
\pron{nAgiMgf}
\gl{\nA}
\bmng
marada cwkaTuTx, jaMti \mo vugaLa naDuve iTiTxgeyiMda kaTiTxda kaTaTxDa; marada cwkaTiTxna naDuvaNa iTiTxgeya racane. 
\emng
\eentry

\bentry
\word{Noh}
\pron{noV}
\gl{\nA}
\bmng
noV; saMgiVta, naqtayxgaLiMda kUDida, dhAmiRka vidhigaLige saMbaMdhisida japAni (sAMparxdAyika) nATaka parxBeVda. 
\emng
\eentry

\bentry
\word{nohow}
\pron{noVhw}
\gl{\kirxvi}
\bmng
\bnum
\num{1} (\ame) yAvudeV riVtiyalilxyU ilalx; yAva parxkAravAgiyU ilalx: \eng{I can't learn this nohow}... yAva riVtiyalilxyU nAnu idanunx kalitukoLaLxlAre. 
\numi{2} (\pArxM) 
\banum
\alnum{a} vayxvasethxgeTuTx; sariyAgilalxde; karxmatapipx. 
\alnum{b} asavxsathxteyiMda; anAroVgayxdalilx. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{noil}
\pron{nAilf}
\gl{\nA}
\bmng
(\Eva\ yA \bava dalilx) (neVyalu sidadhxpaDisida) uNeNxya giDeDxLegaLu, cikakx eLegaLu. 
\emng
\eentry

\bentry
\word[noise(1)]{noise}
\pron{nAisfZ}
\gl{\nA}
\bmng
\bnum
\num{1} sadudx; gadadxla; bobebx; gaTiTxyAda kUgATa; kalakala; koVlAhala. 
\num{2} (\kanmu\ gaTiTxyAda yA asahaniVyavAda yA kakaRshavAda) sapapxLa; shabadx. 
\num{3} (\Bwvi) gadadxla; reVDiyoV \mo vugaLalilx parxsAravAgabeVkAgiruva saMkeVtagaLige saMbaMdhisadiruva, Adare avugaLoDane baruva aniyata kaMpanagaLu. 
\num{4} (\bava dalilx) keVvala aupacArikavAda mAtugaLu yA nijavAgi mAtugaLAgirada todalu shabadxgaLu: \eng{he made sympathetic noises} avanu sahAnuBUti sUcisuva shabadxgaLanunx todalida. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{make a noise} (\rUpa\ saha) 
\banum
\alnum{a} sadudxmADu; goMdalavebibxsu; yAvudoV oMdara viSayavAgi bahaLa mAtanADu, raMpamADu, galATe mADu, AkeSxVpa mADu. 
\alnum{b} (obabxna viSayavAgi) horage hecucx mAtanADu, AkeSxVpisu. 
\alnum{c} hecucx janara kivige bididxru; parxpaMcadalilx apaKAyxti paDe, kuparxsidadhxnAgu. 
\eanum
\numie
\num{2} \eng{ noises off} raMga sathxLada hinenxleyiMda keVLibaruva shabadxgaLu, mAtugaLu, itAyxdi. 
\enum
\emng
\eentry

\bentry
\word[noise(2)]{noise}
\pron{nAisfZ}
\gl{\sakirx}
\bmng
(obabxna KAyxtiyanunx, saMgatiyanunx) parxkaTamADu; bahiraMgapaDisu; horage haraDu: \eng{it was noised abroad that} hAgeMdu bahiraMgapaDisalAyitu. 
\emng

\noindent
\gl{\akirx}
\bmng
(\pArxparx) bahaLa gadadxla mADu; dAMdale mADu; raMpavebibxsu. 
\emng
\eentry

\bentry
\word{noiseless}
\pron{nAisfZlisf}
\gl{\gu}
\bmng
\bnum
\num{1} nishayxbadxvAda; niVravavAda; mwnavAda. 
\num{2} shabadxmADada; sapapxLavilalxda: \eng{the noiseless foot of time} sapapxLavilalxda kAlada hejejx. 
\enum
\emng
\eentry

\bentry
\word{noiselessly}
\pron{nAisflisfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} nishayxbadxvAgi; niVrasavAgi; mwnadiMda. 
\num{2} shabadxmADade; sapapxLavilalxde. 
\enum
\emng
\eentry

\bentry
\word{noiselessness}
\pron{nAisfZlisfnisf}
\gl{\nA}
\bmng
nishayxbadxte; niVravate; mwna. 
\emng
\eentry

\bentry
\word{noise-maker}
\pron{nAisfmeVkarf}
\gl{\nA}
\bmng
shabadxka; nAdaka; (utasxva, saMtoVSa samAraMBa, \mo vugaLalilx) gaTiTxyAgi shabadxmADalu baLasuva sAdhanoVpakaraNa. 
\emng
\eentry

\bentry
\wordnospeech{noise pollution}{noise pollution}
\pron{?}
\gl{\nA}
\bmng
shabadxmAlinayx; kaNaRkaThoVravAda, kivige toMdareyuMTu mADuva, kirikiriyuMTumADuva -- shabadx, sadudx. 
\emng
\eentry

\bentry
\word{noisette}
\pron{nAvxseZTf}
\gl{\nA}
\bmng
saNaNx, duMDaneya, mAMsada cUrina tiMDi. 
\emng
\eentry

\bentry
\word{noisily}
\pron{nAisiZli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} gadadxladiMda kUDi; galATeyiMda kUDi; galibiliyiMda kUDi. 
\num{2} (baNaNx \mo vugaLa \vi) kaNiNxge rAcuvaMte; ramAramiyAgi. 
\num{3} ADaMbaradiMda; ABaRTadiMda. 
\enum
\emng
\eentry

\bentry
\word{noisiness}
\pron{nAisiZnisf}
\gl{\nA}
\bmng
\bnum
\num{1} gadadxladiMda kUDiruvike; galATeyiMda tuMbiruvudu; galibili; huyilu. 
\num{2} ADaMbarate. 
\num{3} kaNiNxge rAcuvaMtiruvike. 
\enum
\emng
\eentry

\bentry
\word{noisome}
\pron{nAisamf}
\gl{\gu}
\bmng
(sAhitayxka) 
\bnum
\num{1} keVDu mADuva; keDukAda; apAyakara. 
\num{2} keTaTxnAtada; duvARsaneya. 
\num{3} AkeSxVpAhaR; ahitakara; asahayxkara; asahaniVya. 
\enum
\emng
\eentry

\bentry
\word{noisomeness}
\pron{nAisamfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} keVDanunxMTumADuvike; apAyavanunxMTumADuvudu. 
\num{2} AkeSxVpAhaRvAgiruvudu; ahitakaravAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{noisy}
\pron{nAisiZ}
\gl{\gu}
\expl{(\tara\ \eng{noisier}, \tama\ \eng{noisiest}).}
\bmng
\bnum
\num{1} gadadxla mADuva; galATe mADuva; kUgADuva; bobebx hAkuva: \eng{a noisy crowd} gadadxla mADuva guMpu. 
\num{2} gadadxla tuMbida; galATeyiMda kUDida; galibiliya; huyilina: \eng{a noisy market} gadadxladiMda tuMbida mArukaTeTx, peVTe. 
\num{3} (baNaNx, uDige, sAhitayx, sheqli, \mo vugaLa \vi) edudx kANuva; kaNiNxge rAcuva; ramArami. 
\num{4} ADaMbarada; ABaRTada. 
\enum
\emng
\eentry

\bentry
\wordf{nolens volens}
\pron{noVlenf\char144 voVlenf\char144}
\gl{\kirxvi}
\expl{\Latin}
\bmng
(sAhitayxka) beVkAgirali, beVDadirali; manasisxrali, ilalxdirali; balavaMtadiMda; kaDADxyavAgi; otAtxyadiMda. 
\emng
\eentry

\bentry
\wordf{nolle prosequi}
\pron{nAli pArxsikevxY}
\gl{\nA}
\expl{\Latin}
\bmng
(\nAyxshA) 
\banum
\alnum{a} vAdiyAgali, pArxsikUyxTarf Agali moKadadxmeyanunx `inunx tAnu muMduvarisaliciCxsuvudilalx' enunxvudu. 
\alnum{b} hiVge heVLuvudanunx nAyxyAlayada dasAtxveVjinalilx dAKalu mADuvudu. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{nom.}{nom.}
\pron{?}
\gl{\saMkiSx}
\bmng
\hyperlink{nominal}{nominal}. 
\emng
\eentry

\bentry
\word[nomad(1)]{nomad}
\pron{noVmAYxDf}
\gl{\nA}
\bmng
\bnum
\num{1} saMcArigoVpAlaka; danada meVvigAgi hululx meYdAnagaLanunx huDukikoMDu sathxLadiMda sathxLakekx tirugutitxruva jAtiyavanu; cara pashupAla(kuladavanu). 
\num{2} alemAri(jAtiyavanu). 
\enum
\emng
\eentry

\bentry
\word[nomad(2)]{nomad}
\pron{noVmAYxDf}
\gl{\gu}
\bmng
alemAri; aledADuva; alemAri jAtiya. 
\emng
\eentry

\bentry
\word{nomadic}
\pron{noVmAYxDikf}
\gl{\gu}
\bmng
\bnum
\num{1} cara pashupAla kulada; alemAri vaMshada. 
\num{2} alemAri; saMcAri; BarxmaNashiVla. 
\enum
\emng
\eentry

\bentry
\word{nomadically}
\pron{noVmAYxDikali}
\gl{\kirxvi}
\bmng
alemAriyAgi; aledADutatx; sathxLadiMda sathxLakekx sututxtAtx. 
\emng
\eentry

\bentry
\word{nomadise}
\pron{noVmAYxDeYsfZ}
\gl{\akirx}
\bmng
= \hyperlink{nomadize}{nomadize}. 
\emng
\eentry

\bentry
\word{nomadism}
\pron{noVmAYxDisaZmf}
\gl{\nA}
\bmng
alemAritana; aledADuva parxvaqtitx; pariBarxmaNashiVlate; aleyutitxruva aBAyxsa, jiVvana padadhxti, sithxti. 
\emng
\eentry

\bentry
\word{nomadize}
\pron{noVmAYxDeYsfZ}
\gl{\akirx}
\bmng
alemAriyAgu; (cara pashupAla kuladavanaMte) aledADu; alemAriyAgi jiVvisu. 
\emng
\eentry

\bentry
\word{nombril}
\pron{nAMbirxlf}
\gl{\nA}
\bmng
(\vaMlAM) gurANiya nABisAthxna; gurANiya madhayxdiMda hiDidu aDiyavaregina parxdeVshada madhayxda sAthxna. 
\emng
\eentry

\bentry
\wordf{nom de guerre}
\pron{nAmf Da geVrf}
\gl{\nA}
\expl{\F\ (\bava\ \engit{noms de guerre} \ucAcx\ adeV).}
\bmng
(yudadhx, aBinaya, leVKana, \mo vugaLalilx obabxnu iTuTxkoMDa) gupatxnAma; saMkeVtanAma. 
\emng
\eentry

\bentry
\wordf{nom de plume}
\pron{nAmf Da pUlxmf}
\gl{\nA}
\expl{\F\ (\bava\ \engit{noms de plume} \ucAcx\ adeV).}
\bmng
\bnum
\num{1} (garxMthakataRna) kalipxtanAma; saMkeVtanAma; gupatxnAma. 
\num{2} (baravaNigeyalilx savxMta hesarige badalAgi hAkuva) leVKakana birudu, hesarina modalakaSxragaLu yA iTuTxkoMDa hesaru. 
\enum
\emng
\eentry

\bentry
\word{nomen}
\pron{noVmenf}
\gl{\nA}
\bmng
(\roVpu) obabxna saMpUNaR hesarinalilx avana deVsha yA vaMsha sUcisuva (eraDaneya) BAga. (\udA\ \eng{Marcus Tullius Cicero} eMbudaralilx \eng{Tullius}). 
\emng
\eentry

\bentry
\word{nomenclative}
\pron{noVmanfkalxTivf}
\gl{\gu}
\bmng
nAmakaraNa parxvaqtitxyuLaLx. 
\emng
\eentry

\bentry
\word{nomenclator}
\pron{noVmanfkelxVTarf}
\gl{\nA}
\bmng
\bnum
\numi{1} nAma niveVdaka: 
\banum
\alnum{a} BeVTiyAdavara hesarugaLanunx tananx yajamAnanige, AsharxyadAtanige tiLisalu niyamitanAgidadx dAsa, seVvaka, Ashirxta. 
\alnum{b} BoVjana samAraMBagaLalilx atithigaLige sathxLanideRVsha mADutitxdadx seVvaka. 
\eanum
\numie
\num{2} (\kanmu\ veYjAcnxnika viBajane yA jAti viMgaDaNe mADuvAga) hesaru koDuvavanu; nAmadAta. 
\enum
\emng
\eentry

\bentry
\word{nomenclatural}
\pron{noVmenfkAlxYxcaralf}
\gl{\gu}
\bmng
nAmakaraNa padadhxtiya yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nomenclature}
\pron{noVmenfkalxcarf, noVmanf kelxVcarf}
\gl{\nA}
\bmng
\bnum
\num{1} (obabxna yA oMdu janAMgavu vasutxgaLige koDuva) hesaru padadhxti; nAmakaraNa vayxvasethx. 
\num{2} (oMdu shAsatxrX \mo vugaLa) pariBASe. 
\num{3} (oMdu padadhxtiyananxnusarisi) nAmakaraNa mADuvudu. 
\num{4} hesaru paTiTx; nAmasUci; nAmAvaLi dAKale pusatxka, rijisaTxru. 
\enum
\emng
\eentry

\bentry
\word{nominal}
\pron{nAminalf}
\gl{\gu}
\bmng
\bnum
\num{1} nAmavAcakada yA nAmavAcakadaMtha: \eng{nominal roots} nAma parxkaqtigaLu. 
\num{2} hesarina; hesarinalilxya: \eng{nominal and essential distinctions} hesarina matutx vAsatxvika vayxtAyxsagaLu. 
\num{3} hesarugaLuLaLx; hesarugaLanunx tiLisuva, namUdisiruva: \eng{nominal list of officers} adhikArigaLa hesarupaTiTx. 
\num{4} hesarige mAtarxviruva; nAmamAtarxda; vAsatxvavalalxda: \eng{nominal and real rulers} hesarige mAtarx ALuvavaru (`nAmf ke vAsetx' rAjaru) matutx nijavAgi ALuvavaru. 
\num{5} (bADige \mo vugaLa \vi) hesarige mAtarx hAkiruva; atayxlapx motatxda; nijavAda belegiMta tiVra kaDame iruva: \eng{nominal rent} hesarige mAtarx hAkiruva (tiVrA kaDame) bADige. 
\num{6} hesarugaLuLaLx, hesarugaLanunx tiLisuva, namUdisiruva: \eng{nominal list of officers} adhikArigaLa hesarupaTiTx. 
\enum
\emng
\eentry

\bentry
\word{nominalise}
\pron{nAminaleYsfZ}
\gl{\sakirx}
\bmng
= \hyperlink{nominalize}{nominalize}. 
\emng
\eentry

\bentry
\word{nominalism}
\pron{nAminalisaZmf}
\gl{\nA}
\bmng
(\tashA) nAmavAda; nAmamAtarx sidAdxMta; asitxtavxviruvudu keVvala vayxSiTxvAcakagaLige yA biDi vasutxvAcakagaLige mAtarx matutx BAvavAcakagaLu, jAtivAcakagaLu yA vishavxsAmAnayx vAcakagaLu vasutxshUnayxvAda keVvala shabadxgaLu mAtarx enunxva yUroVpina madhayxyugada oMdu sidAdhxMta. 
\emng
\eentry

\bentry
\word{nominalist}
\pron{nAminalisfTx}
\gl{\nA}
\bmng
nAmamAtarxvAdi; nAmamAtarx sidAdhxMtavanunx opipxkoLuLxvavanu. 
\emng
\eentry

\bentry
\word{nominalistic}
\pron{nAminalisiTxkf}
\gl{\gu}
\bmng
nAmamAtarx sidAdhxMtada; nAma(mAtarx)vAdakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nominalization}
\pron{nAminaleYseZVSanf}
\gl{\nA}
\bmng
(kirxyApada, guNavAcaka, \mo vugaLiMda) nAmavAcakavanunx rUpisuvudu. 
\emng
\eentry

\bentry
\word{nominalize}
\pron{nAminaleYsfZ}
\gl{\sakirx}
\bmng
(kirxyApada, guNavAcaka, \mo vugaLiMda) nAmavAcakavanunx rUpisu. \udA\ \eng{put out} niMda \eng{out-put, true} eMba padadiMda \eng{truth}. 
\emng
\eentry

\bentry
\word{nominally}
\pron{nAminali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} hesariniMda; hesarige saMbaMdhisidaMte. 
\num{2} nAmf ke vAsetx; nAmamAtarxvAgi; vAsatxvavAgalalxde. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nominal value}{nominal value}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} (nANayx, SeVru, \mo vugaLa \vi) muKabele. 
\num{2} (SeVru \vi mAkeRTf belegiMta viBinanxvAda eMbathaRdalilx) kaMpeniya rijisaTxrinalilx dAKalAgiruva bele. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nominal wages}{nominal wages}
\pron{?}
\gl{\nA}
\bmng
(\athaRshA) nAma mAtarx(vAda) kUli daragaLu; kUlihaNavanunx haNavu koMDukoLaLxbalalx vasutxgaLa yA seVvegaLa beleya AdhArada meVle aLeyuvudara badalu keVvala haNada mwlayxda parxkAra aLedu niVDuva kUli daragaLu. 
\emng
\eentry

\bentry
\word{nominate}
\pron{nAmineVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (yAvudoV) hesariTuTx kare; nAmakaraNa mADu; hesaru heVLu; gotAtxgi heVLu. 
\num{2} (tAriVKu, sathxLa) heVLu; nidhaRrisu; gotutxmADu. 
\num{3} (oMdu adhikAra sAthxnakekx, cunAvaNege obabxna) hesaru sUcisu; nAmakaraNa mADu: \eng{the candidates were nominated today} aBayxthiRgaLanunx ivatutx nAmakaraNa mADalAyitu. 
\enum
\emng
\eentry

\bentry
\word{nomination}
\pron{nAmineVSanf}
\gl{\nA}
\bmng
\bnum
\num{1} nAmakaraNa; nAmanideRVshana; hesaru koDuvudu. 
\num{2} (iMtha tAriVKu, sathxLa eMdu) nidhaRrisuvudu; gotutxpaDisuvudu. 
\num{3} (adhikAra, sAthxna, \mo vugaLige) nAmakaraNa (mADuva) hakukx: \eng{have a nomination at your disposal} oMdu nAmakaraNada hakakxnunx ninanx vashadalilxTuTxko. 
\enum
\emng
\eentry

\bentry
\word{nominatival}
\pron{nAminaTeYvalf}
\gl{\gu}
\bmng
parxthamA viBakitxya; parxthamA viBakitxge saMbaMdhisida. 
\emng
\eentry

\bentry
\word[nominative(1)]{nominative}
\pron{nAminaTivf}
\gl{\gu}
\bmng
(\vAyx) parxthamA viBakitxya. 
\emng
\eentry

\bentry
\word[nominative(2)]{nominative}
\pron{nAminaTivf}
\gl{\nA}
\bmng
(\vAyx) parxthamA viBakitx(ya pada). 
\emng
\eentry

\bentry
\word[nominative(3)]{nominative}
\pron{nAmineVTivf}
\gl{\gu}
\bmng
\bnum
\num{1} nAmakaraNada; nAmakaraNa mADuva; nAmakaraNakekx saMbaMdhisida: \eng{the nominative and elective principles} nAmakaraNa mADuva hAgU Ayekx mADuva sUtarxgaLu. 
\num{2} nAmakaraNadiMda neVmisida. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nominative absolute}{nominative absolute}
\pron{?}
\gl{\nA}
\bmng
ananivxta kataqR; oMdu bageya parxthamA viBakitx parxyoVga. \udA\ \eng{this being so, I did nothing} idu hAgidudxdariMda, nAneVnU mADalilalx. 
\emng
\eentry

\bentry
\word{nominator}
\pron{nAmineVTarf}
\gl{\nA}
\bmng
nAmakaraNadAra; nAmakaraNa mADuvava. 
\emng
\eentry

\bentry
\word{nominee}
\pron{nAminiV}
\gl{\nA}
\bmng
nAmini; nidiRSaTx nAmi: 
\banum
\alnum{a} oMdu hudedxge yA datitxyanunx paDeyalu nAmakaraNagoMDiruva vayxkitx. 
\alnum{b} (vANijayx) sATxku \mo vu yAra hesarinalilx noVMdAyisalapxTiTxdeyoV (mAliVkaneV AgabeVkAgilalx) avanu. 
\eanum
\emng
\eentry

\bentry
\word{nomogram}
\pron{nA(noV)magArx Yxmf}
\gl{\nA}
\bmng
noVmagArx Yxmf; niyamareVKe; eraDoV mUroV savxtaMtarx caragaLa bele tiLididadxre, neVra aMciruva oMdu paTiTxyanunx upayoVgisi, avalaMbita carada beleyanunx tiLiyabahudAda oMdu nakeSx. 
\emng
\eentry

\bentry
\word{nomograph}
\pron{nA(noV)magArxPf}
\gl{\nA}
\bmng
= \hyperlink{nomogram}{nomogram}. 
\emng
\eentry

\bentry
\word{nomographic}
\pron{nA(noV)magArxYxphikf}
\gl{\gu}
\bmng
noVmogArxYxmfna yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nomographically}
\pron{nA(noV)magArxYxphikali}
\gl{\kirxvi}
\bmng
noVmoVgArxYxmfge saMbaMdhisidaMte. 
\emng
\eentry

\bentry
\word{nomography}
\pron{namAgarxphi}
\gl{\nA}
\bmng
noVmoVgArxYxmf racisuvudu, racisuva vidhAna. 
\emng
\eentry

\bentry
\word{nomothetic}
\pron{nA(noV)mateTikf}
\gl{\gu}
\bmng
\bnum
\num{1} nAyxyavidhAyaka; kAnUnu racisuva; kaTaTxLe vidhisuva. 
\num{2} kAnUnanunx -- tiLiyaheVLuva, tiLisuva, namUdisuva, nirUpisuva, vayxkatxpaDisuva. 
\num{3} (\kanmu\ veYjAcnxnika) sUtarx, niyama -- nirUpaka. 
\enum
\emng
\eentry

\bentry
\wordwithhyphen{hyp-nomy}{-nomy}
\pron{-nami}
\gl{\saupa}
\bmng
oMdu jAcnxnashAKe yA adara niyamagaLige saMbaMdhisidudx eMba athaRdalilx baLasuva \saupa: \eng{economy} athaRshAsatxrX. 
\emng
\eentry

\bentry
\word{non-}
\pron{nAnf-}
\gl{\pUparx}
\bmng
niSeVdhAthaRka \pUparx, \kanmu\ 
\bnum
\num{1} mADada, ilalxda yA saMbaMdhapaDada: \eng{non-attendance} hAjarAgadiruvike; geYru hAjari. \eng{non-payment} pAvati mADadiruvudu. 
\num{2} vivarisida vagaRkekx, bagege -- seVrada; \eng{non-alcoholic} AlakxhAlf vagaRkekx seVrada. \eng{non-member} sadasayxra vagaRkekx seVrada; sadasayxnalalxda. 
\num{3} \gu vAgi baLasuva padagaLanunx racisuvalilx: \eng{non-party} pakaSxkekx seVrirada; pakASxtiVtavAda. 
\num{4} ilalxda; aBAvada: \eng{non-access} parxveVsha (mAgaR) ilalxda. 
\num{5} (\kirxvi gaLoDane) heVLiruva riVtiyalilx ilalxda: \eng{non-aggressively} AkarxmaNakAriyAgilalxde. 
\num{6} kirxyApadagaLiMda Agada, mADada, udedxVshakakxlalxda eMba athaRda \gu gaLanunx racisuvalilx: \eng{non-skid} jArada. 
\num{7} \eng{in-} yA \eng{un-} \pUparx gaLiMda racitavAda niSeVdhAthaRka shabadxgaLu vishiSATxthaRvoMdanonxV virudAdhxthaRvoMdanonxV koDuva yA visheVSavalalxda sAmAnayx niSeVdhAthaRvanunx koDuva padagaLanunx racisuvalilx: \eng{non-effective} pariNAmakAriyalalxda. \eng{non-human} mAnavanalalxda (Adare amAnaviVyavAgirada). 
\enum
\emng
\eentry

\bentry
\word{nona-}
\pron{nAna-}
\gl{\sapUpa}
\bmng
oMbatutx; nava(`oMbatutx koVna \mo vugaLanunx hoMdiruva' eMbathaRda shabadxgaLa racaneyalilx \parx): \eng{nonagon}. 
\emng
\eentry

\bentry
\word{non-abstainer}
\pron{nAnabfseTxVnarf}
\gl{\nA}
\bmng
(\kanmu\ madayxvanunx) biDadiruvavanu; tayxjisadavanu. 
\emng
\eentry

\bentry
\word{non-acceptance}
\pron{nAnakesxpaTxnfsx}
\gl{\nA}
\bmng
anaMgiVkAra; asamamxti; opipxge ilalxdiruvudu. 
\emng
\eentry

\bentry
\word{non-access}
\pron{nAnfAYxkesxsf}
\gl{\nA}
\bmng
\bnum
\num{1} aparxveVsha; parxveVsharAhitayx; talupalu mAgaR, avakAsha ilalxdiruvike. 
\num{2} (\nAyxshA) anavakAsha; avakAsharAhitayx; taMde yAreMdu nidhaRrisuvalilx, gaMDanu deVshAMtaradalilxdudxdu yA haDaginalilx parxyANa mADutitxdudxdu. \mo\ kAraNagaLiMda heMDatiyoDane saMBoVga sAdhayxviralilalx eMba saMgati. 
\enum
\emng
\eentry

\bentry
\word{non-addictive}
\pron{nAnaDikiTxvf}
\gl{\gu}
\bmng
(mAdaka vasutx, aBAyxsa, \mo) caTavuMTumADada; giVLu hiDisada. 
\emng
\eentry

\bentry
\word{nonage}
\pron{noV(nA)nijf}
\gl{\nA}
\bmng
\bnum
\num{1} (\ca) cikakx vayasusx; bAlAyxvasethx; apwrxDhAvasethx; kAnUnina parxkAra apArxpatxvayasakxte. 
\num{2} apakavxte; pArxraMBa dashe. 
\enum
\emng
\eentry

\bentry
\word[nonagenarian(1)]{nonagenarian}
\pron{noV(nA)najineVriAnf}
\gl{\nA}
\bmng
navativaSiRVya: \eng{90}kUkx \eng{99}kUkx madhayxda, toMbatatxra vayasisxna. 
\emng
\eentry

\bentry
\word[nonagenarian(2)]{nonagenarian}
\pron{noV(nA)najineVrianf}
\gl{\nA}
\bmng
navativaSiRVya: \eng{90}kUkx \eng{99}kUkx naDuvaNa toMbatatxra vayasisxnavanu. 
\emng
\eentry

\bentry
\word{non-aggression}
\pron{nAnfAgerxSanf}
\gl{\nA}
\bmng
anAkarxmaNa; AkarxmaNa mADadiruvike. 
\emng
\eentry

\bentry
\word{nonagon}
\pron{nAnaganf}
\gl{\nA}
\bmng
(\ga) navaBuja; navakoVna; oMbatutx Buja, oMbatutx koVnagaLiruva samatalAkaqti. \imglink{nanagon:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.5cm {N_Pictures/nanagon-1.jpg}}} 
\emng
\eentry

\bentry
\word[non-alcoholic(1)]{non-alcoholic}
\pron{nAnAYxlakxhAlikf}
\gl{\gu}
\bmng
(peVya \mo vugaLa \vi) anAlakxhAlika; AlakxhAlf ilalxda. 
\emng
\eentry

\bentry
\word[non-alcoholic(2)]{non-alcoholic}
\pron{nAnAYxlakxhAlikf}
\gl{\nA}
\bmng
anAlakxhAlika peVya. 
\emng
\eentry

\bentry
\word{non-aligned}
\pron{nAnaleYnfDx}
\gl{\gu}
\bmng
taTasathx; Alipatx. 
\emng
\eentry

\bentry
\word{non-alignment}
\pron{nAnaleYnfmaMTf}
\gl{\nA}
\bmng
(\kanmu\ rAjaniVtiyalilx, rASaTxrXgaLa \vi) alipatxniVti; tATasathxyX niVti. 
\emng
\eentry 

\bentry
\word{non-allergic}
\pron{nAnalajiRkf}
\gl{\gu}
\bmng
alajiR uMTumADada; analajiRya. 
\emng
\eentry

\bentry
\word{non-ambiguous}
\pron{nAnfAYxMbiguyXasf}
\gl{\gu}
\bmng
asapxSaTxvalalxda; asaMdigadhx. 
\emng
\eentry

\bentry
\word{non-appearance}
\pron{nAnapiaranfsx}
\gl{\nA}
\bmng
(\kanmu\ nAyxyasAthxnakekx kakiSxgAranAgali sAkiSxyAgali) bAradiruvudu; hAjarAgadiruvudu; geYruhAjari. 
\emng
\eentry

\bentry
\word{non-art}
\pron{nAnfATfR}
\gl{\nA}
\bmng
asAMparxdAyika kale; kaleya rUDhiya niyamagaLanunx anusarisada kale, kalAvidhAna. 
\emng
\eentry

\bentry
\word[nonary(1)]{nonary}
\pron{noVnari}
\gl{\gu}
\bmng
(gaNita matutx aMkana parxmANada \vi) navamAna; oMbatatxnunx AdhAravAguLaLx. 
\emng
\eentry

\bentry
\word[nonary(2)]{nonary}
\pron{noVnari}
\gl{\nA}
\bmng
navaka; oMbatatxra taMDa. 
\emng
\eentry

\bentry
\word[Non-Aryan(1)]{Non-Aryan}
\pron{nAnfErianf}
\gl{\gu}
\bmng
(vayxkitx yA BASeya \vi) AyeRVtara; AyaRvalalxda; AyaR mUladadxlalxda. 
\emng
\eentry

\bentry
\word[Non-Aryan(2)]{Non-Aryan}
\pron{nAnfErianf}
\gl{\nA}
\bmng
AyeRVtara; AyaRkulakekx, buDakaTiTxge seVradava. 
\emng
\eentry

\bentry
\word{non-attached}
\pron{nAnaTAYxcfTx}
\gl{\gu}
\bmng
\bnum
\num{1} seVrisada; joVDisada; saMyoVjisada. 
\num{2} seVrirada; taTasathx. 
\enum
\emng
\eentry

\bentry
\word{non-attendance}
\pron{nAnaTeMDanfsx}
\gl{\nA}
\bmng
geYru hAjari; hAjarAgadiruvike. 
\emng
\eentry

\bentry
\word{non-attributable}
\pron{nAnaTirxbuyxTabflf}
\gl{\gu}
\bmng
AroVpisalAgada; oMdu nidiRSaTxvAda mUla \mo vakekx seVridedxMdu heVLalAgada. 
\emng
\eentry

\bentry
\word{non-attributably}
\pron{nAnaTirxbuyxTabilx}
\gl{\kirxvi}
\bmng
AroVpisalAgadaMte; oMdu nidiRSaTx mUla \mo vakekx seVridedxMdu heVLalAgadaMte. 
\emng
\eentry

\bentry
\word{non-availability}
\pron{nAnaveVlabiliTi}
\gl{\nA}
\bmng
alaBayxte; sigada sithxti. 
\emng
\eentry

\bentry
\word{non-believer}
\pron{nAnfbiliVvarf}
\gl{\nA}
\bmng
nAsitxka; asharxdAdhxLu; (\kanmu\ dhAmiRka) naMbikeyilalxdava. 
\emng
\eentry

\bentry
\word{non-belligerency}
\pron{nAnfbalijaranisx}
\gl{\nA}
\bmng
yudadhxdalilx toDagadiruvike, BAgavahisadiruvike. 
\emng
\eentry

\bentry
\word[non-belligerent(1)]{non-belligerent}
\pron{nAnfbalijaraMTf}
\gl{\gu}
\bmng
(\kanmu\ rASaTxrXda \vi) yudadhxkekx seVrada; yudadhxdalilx BAgavahisada. 
\emng
\eentry

\bentry
\word[non-belligerent(2)]{non-belligerent}
\pron{nAnfbalijaraMTf}
\gl{\nA}
\bmng
yudadhxkekx seVrada rASaTxrX; alipatx rASaTxrX; taTasathxdeVsha. 
\emng
\eentry

\bentry
\word{non-biological}
\pron{nAnfbeYalAjikalf}
\gl{\gu}
\bmng
ajeYvika; jiVvavijAcnxnakekx yA jiVvigaLige saMbaMdhisilalxda. 
\emng
\eentry

\bentry
\word[non-Black(1)]{non-Black}
\pron{nAnfbAlxYxkf}
\gl{\gu}
\bmng
\bnum
\num{1} (vayxkitxya \vi) kariyanalalxda; niVgorxV alalxda. 
\num{2} kariyaralalxdavara; niVgorxV alalxda janara yA avarige saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word[non-Black(2)]{non-Black}
\pron{nAnfbAlxYxkf}
\gl{\nA}
\bmng
kariyanalalxdava; niVgorxV alalxda vayxkitx. 
\emng
\eentry

\bentry
\word{non-breakable}
\pron{nAnfberxVkabflf}
\gl{\gu}
\bmng
aBidura; oDeyalAgada yA oDeduhoVgada. 
\emng
\eentry

\bentry
\word{non-capital}
\pron{nAnfkAYxpiTalf}
\gl{\gu}
\bmng
(aparAdhada \vi) maraNadaMDanAhaRvalalxda; maraNadaMDanege yoVgayxvalalxda. 
\emng
\eentry

\bentry
\word[non-Catholic(1)]{non-Catholic}
\pron{nAnfkAYxta(tf)likf}
\gl{\gu}
\bmng
akAyxtholika; roVmanf kAyxtholikf alalxda. 
\emng
\eentry

\bentry
\word[non-Catholic(2)]{non-Catholic}
\pron{nAnfkAYxta(tf)likf}
\gl{\nA}
\bmng
akAyxtholikf; roVmanf kAyxtholikf paMgaDakekx seVrada vayxkitx. 
\emng
\eentry

\bentry
\word{nonce}
\pron{nAnfsx}
\gl{\nA}
\bmng
tAtAkxlika; parxkaqta saMdaBaR; sadayx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{for the nonce} sadayxkekx; tAtAkxlikavAgi; I saMdaBaRdalilx. 
\emng
\eentry

\bentry
\word{nonce-word}
\pron{nAnfsxvaDfR}
\gl{\nA}
\bmng
sAMdaBiRka pada; saMdaBaRkAkxgi kalipxsida pada. 
\emng
\eentry

\bentry
\word{non-chalance}
\pron{nAnfSalanfsx}
\gl{\nA}
\bmng
\bnum
\num{1} anudirxkatxte; nirudivxganxte; nirudevxVga; shAMta nirAsakitx; BAvAveVshavilalxdiruvike. 
\num{2} audAsiVnayx; tAtAsxra manoVBAvadiMdiruvike. 
\enum
\emng
\eentry



\bentry
\word{non-chalant}
\pron{nAnfSalaMTf}
\gl{\gu}
\bmng
\bnum
\num{1} anudirxkatx; nirudivxganx; BAvAveVshavilalxda; kadalada manasisxna; shAMta matutx nirAsakatx. 
\num{2} udAsiVna; tAtAsxra manoVBAvada. 
\enum
\emng
\eentry

\bentry
\word{non-chalantly}
\pron{nAnfSalaMTfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} anudirxkatxvAgi; BAvAveVshavilalxdaMte; udevxVgavilalxde; nirAsakitxyiMda. 
\num{2} udAsiVnavAgi; tAtAsxra manoVBAvadiMda. 
\enum
\emng
\eentry

\bentry
\word[non-Christian(1)]{non-Christian}
\pron{nAnfkirxsaTxyXnf, nAnfkirxsacxnf}
\gl{\gu}
\bmng
akerxYsatx; kerxYsatx matAnuyAyiyalalxda. 
\emng
\eentry

\bentry
\word[non-Christian(2)]{non-Christian}
\pron{nAnfkirxsaTx yxnf, nAnfkirxsacxnf}
\gl{\nA}
\bmng
akerxYsatx; kerxYsatx matakekx seVrada vayxkitx. 
\emng
\eentry

\bentry
\word{non-citizen}
\pron{nAnfsiTisaZnf}
\gl{\nA}
\bmng
(oMdu deVsha, nagara, \mo vugaLa) parxjeyalalxdavanu; nAgarikeVtara; parxjAtavxrahita; aparxje. 
\emng
\eentry

\bentry
\word{non-classified}
\pron{nAnfkAlxYxsipheYDf}
\gl{\gu}
\bmng
(\kanmu\ mAhiti) avagiRVkaqta; vagiRVkarisada; vagiRVkaraNa mADirada. 
\emng
\eentry

\bentry
\word{non-clerical}
\pron{nAnfkelxrikalf}
\gl{\gu}
\bmng
\bnum
\num{1} gumAsetx kelasavalalxda yA kelasa mADada. 
\num{2} apAdirx; pAdirx -- kelasavalalxda, kelasa mADada. 
\enum
\emng
\eentry

\bentry
\word{non-collegiate}
\pron{nAnfkaliVjaTf}
\gl{\gu}
\bmng
\bnum
\num{1} kAleVjige saMbaMdhisirada; kAleVjina saMbaMdhavilalxda. 
\num{2} kAleVjugaLilalxda; kAleVjugaLanunx hoMdirada. 
\enum
\emng
\eentry

\bentry
\word{non-com}
\pron{nAnfkAmf}
\gl{\nA}
\bmng
(\AmA) \eng{non-commissioned officer}(eMbudara \saMkiSx). 
\emng
\eentry

\bentry
\word[non-combatant(1)]{non-combatant}
\pron{nAnfkAMbaTaMTf}
\gl{\gu}
\bmng
(\kanmu\ seYnayxdalilx kelasa mADuvavara \vi) yudodhxVdoyxVgiyalalxda. 
\emng
\eentry

\bentry
\word[non-combatant(2)]{non-combatant}
\pron{nAnfkAMbaTaMTf}
\gl{\nA}
\bmng
ayoVdha; aseYnika; nAgarika; seYnayxdalilx shasatxrXveYdayx, pAdirx, itAyxdi kelasa mADuvavanu. 
\emng
\eentry

\bentry
\word{non-commissioned}
\pron{nAnfkamiSanfDx}
\gl{\gu}
\bmng
(\kanmu\ seYnAyxdhikArigaLa \vi) sananxdu paDedilalxda; sananxdurahita; beVsananxdu. 
\emng
\eentry

\bentry
\word{noncommittal}
\pron{nAnfkamiTalf}
\gl{\gu}
\bmng
anibadadhx; yAvudAdarU oMdu kAyaRkarxmakAkxgali, aBipArxyakAkxgali -- kaTuTxbiVLadiruva. 
\emng
\eentry

\bentry
\word{noncommittally}
\pron{nAnfkamiTali}
\gl{\kirxvi}
\bmng
anibadadhxvAgi; yAvudoMdu kAyaRkarxmakAkxgali, aBipArxyakAkxgali kaTuTxbiVLadaMte. 
\emng
\eentry

\bentry
\word{non-communicant}
\pron{nAnfkamUyxnikaMTf}
\gl{\nA}
\bmng
parxBuBoVjana saMsAkxravanunx paDeyadavanu. 
\emng
\eentry

\bentry
\word{non-communicating}
\pron{nAnfkamUyxnikeVTiMgf}
\gl{\gu}
\bmng
(sudidx, samAcAra, \mo vanunx) tiLisada; aruhada; tiLiyapaDisada; asaMvAhaka. 
\emng
\eentry

\bentry
\word[non-communist(1)]{non-communist}
\pron{nAnfkAmuyxnisfTx}
\gl{\gu}
\bmng
kamUyxnisaTxlalxda; kamUyxnisfTx tatatx vxvanunx anusarisada yA parxtipAdisada. 
\emng
\eentry

\bentry
\word[non-communist(2)]{non-communist}
\pron{nAnfkAmuyxnisfTx}
\gl{\nA}
\bmng
kamUyxnisaTxlalxdava; kamUnisfTx tatatxvXvanunx anusarisadava yA parxtipAdisadava. 
\emng
\eentry

\bentry
\word{non-compliance}
\pron{nAnfkaMpelxYanfsx}
\gl{\nA}
\bmng
an anupAlane; ananuvataRne; koVrike, Ajecnx, \mo vugaLa parxkAra naDedukoLaLxdiruvike. 
\emng
\eentry

\bentry
\wordf{non compos mentis}
\pron{nAnf kAMpAsf meMTisf}
\gl{\gu}
\expl{\Latin}
\bmng
budidhx neTaTxgilalxda; sariyAda manaHsithxti ilalxda; mAnasika sithxti sariyAgilalxda. 
\emng
\eentry

\bentry
\word{non-conducting}
\pron{nAnfkaMDakiTxMgf}
\gl{\gu}
\bmng
avAhaka; uSaNxvanAnxgali, shabadxvanAnxgali, viduyxtatxnAnxgali tananx mUlaka sAgisalArada. 
\emng
\eentry

\bentry
\word{non-conductor}
\pron{nAnfkaMDakaTxrf}
\gl{\nA}
\bmng
avAhaka; uSaNxvanAnxgali, shabadxvanAnxgali, viduyxtatxnAnxgali tananx mUlaka sAgisalArada vasutx. 
\emng
\eentry

\bentry
\word{non-confidential}
\pron{nAnfkAnfphiDenaSxlf}
\gl{\gu}
\bmng
guTaTxlalxda; rahasayxvalalxda; goVpayxvalalxda. 
\emng
\eentry

\bentry
\word{non-confidentially}
\pron{nAnfkAnfphiDenaSxli}
\gl{\kirxvi}
\bmng
rahasayxvalalxdaMte; goVpayxvalalxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{nonconformism}
\pron{nAnfkanfphAzmiRsaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} (pArxTeseTxMTf matakekx seVridavara \vi) anaMgiVkAra; adhikaqta caciRna tatatx vxgaLanunx yA niyamapAlaneyanunx opapxdiruvike (\kanmu\ iMgilxSf cacaRnunx pArxTesaTxMTaru aMgiVkarisadiruvudu). 
\num{2} rUDhiviroVdha; cAlitxyalilxna yAvudeV niyamavanunx opapxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{Nonconformism}
\pron{nAnfkanfphAzmiRsaZmf}
\gl{\nA}
\bmng
iMgelxMDina adhikaqta cacaRnunx opapxdiruvike. 
\emng
\eentry

\bentry
\word{nonconformist}
\pron{nAnfkanfphAmiRsfTx}
\gl{\nA}
\bmng
\bnum
\num{1} (pArxTeseTxMTf matakekx seVrida) anaMgiVkAri; anaMgiVkataR; adhikaqta caciRna tatatx vxgaLanunx aMgiVkarisada pArxTeseTxMTf paMgaDadavanu. 
\num{2} rUDhi viroVdhi; cAlitxyalilxruva yAvudeV niyamavanunx anusarisadavanu. 
\enum
\emng
\eentry

\bentry
\word{Nonconformist}
\pron{nAnfkanfphAmiRsfTx}
\gl{\nA}
\bmng
iMgelxMDina adhikaqta AyxMgilxkanf cacaRnunx opapxda (\sA\ pArxTeseTxMTf) paMthadavanu. 
\emng
\eentry

\bentry
\word{nonconformity}
\pron{nAnfkanfphAzmiRTi}
\gl{\nA}
\bmng
\bnum
\numi{1} \eng{(Nonconformity)} 
\banum
\alnum{a} anaMgiVkArigaLa -- mata tatatx vxgaLu, AcAravayxvahAragaLu yA paMgaDa. 
\alnum{b} (pArxTeseTxMTf paMgaDada) anaMgiVkAra; asamamxti. 
\eanum
\numie
\num{2} ananusaraNe; (niyama \mo vanunx) anusarisadiruvudu. 
\num{3} asAMgatayx; asAmaMjasayx; vasutxgaLalilx parasapxra hoMdikeyilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{Nonconformity}
\pron{nAnfkanfphAmiRTi}
\gl{\nA}
\bmng
iMgelxMDina adhikaqta cacaRnunx opapxdiruvike; \kanmu\ pArxTeseTxMTf asamamxti. 
\emng
\eentry

\bentry
\word{non-contagious}
\pron{nAnfkaMTeVjasf}
\gl{\gu}
\bmng
soVMkalalxda; asAMkArxmika. 
\emng
\eentry

\bentry
\word{non-content}
\pron{nAnfkaMTeMTf}
\gl{\nA}
\bmng
(\birx) hwsf Aphf lADfsxRnalilx niSeVdha sUcisuva matadAra, viroVdhimata koDuvava. 
\emng
\eentry

\bentry
\word{non-contentious}
\pron{nAnfkaMTenaSxsf}
\gl{\gu}
\bmng
\bnum
\num{1} kalahashiVlanalalxda; jagaLagaMTanalalxda. 
\num{2} vivAdAtamxkavalalxda; avivAdAtamxka. 
\enum
\emng
\eentry

\bentry
\word{non-contributory}
\pron{nAnfkaMTirxbuyxTari}
\gl{\gu}
\bmng
\bnum
\num{1} (\kanmu\ piMcaNi yoVjaneya \vi) caMdAkoDabeVkAgilalxda; vaMtige koDabeVkAgilalxda. 
\num{2} sahAyakavalalxda; sahAyakavAgirada. 
\enum
\emng
\eentry

\bentry
\word{non-controversial}
\pron{nAnfkAMTarxvaSaRlf}
\gl{\gu}
\bmng
avivAdAtamxka; vivAdAtamxkavalalxda. 
\emng
\eentry

\bentry
\word{non-cooperation}
\pron{nAnfkoVApareVSanf}
\gl{\nA}
\bmng
sahakarisadiruvudu; asahakAra. 
\emng
\eentry

\bentry
\word{nonda}
\pron{nAMDa}
\gl{\nA}
\bmng
gulAbi vagaRda, kivxVnfsxleMDina oMdu vaqkaSxjAti yA adara haNuNx. 
\emng
\eentry

\bentry
\word{non-delivery}
\pron{nAnf Delivari}
\gl{\nA}
\bmng
beVbaTavADe; haMcike, vitaraNe, baTavADe-mADadiruvudu. 
\emng
\eentry

\bentry
\word{non-denominational}
\pron{nAnfDinAmineVSanalf}
\gl{\gu}
\bmng
apaMthiVya; yAvudeV dhAmiRka paMgaDa, shAKe, saMparxdAyagaLige siVmitagoMDirada. 
\emng
\eentry

\bentry
\word[nondescript(1)]{nondescript}
\pron{nAnfDisikxrXpfTx}
\gl{\gu}
\bmng
\bnum
\num{1} vishiSaTx lakaSxNagaLilalxda; anidiRSaTx guNada. 
\num{2} avagayxR; sulaBavAgi vagiRVkarisalAgada. 
\num{3} ibabxMdi yA saMdigadhx jAtiya; idU alalx adU alalx enunxvaMtha. 
\enum
\emng
\eentry

\bentry
\word[nondescript(2)]{nondescript}
\pron{nAnfDesikxrXpfTx}
\gl{\nA}
\bmng
(guNada yA lakaSxNada) vayxkitx, vasutx. 
\bnum
\num{2} avagayxR; vagiRVkarisalAgada vayxkitx, vasutx. 
\num{3} ibabxMdi; saMdigadhx rUpada -- vayxkitx, vasutx. 
\enum
\emng
\eentry

\bentry
\word{nondescriptly}
\pron{nAnfDisikxrXpfTxli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} vishiSaTx lakaSxNagaLilalxdiruvaMte; anidiRSaTx guNadaMte. 
\num{2} avagayxRvAgi; sulaBavAgi vagiRVkarisalAgadaMte. 
\enum
\emng
\eentry

\bentry
\word{nondescriptness}
\pron{nAnfDesikxrXpfTxnisf}
\gl{\nA}
\bmng
\bnum
\num{1} vishiSaTx lakaSxNagaLilalxdiruvike; anidiRSaTx guNaviruvike. 
\num{2} avagayxRte; sulaBavAgi vagiRVkarisalAgadiruvike. 
\enum
\emng
\eentry

\bentry
\word{non-destructive}
\pron{nAnfDisaTxrXkiTxvf}
\gl{\gu}
\bmng
avinAshaka; hAni yA nAsha mADada. 
\emng
\eentry

\bentry
\word{non-drinker}
\pron{nAnfDirxMkarf}
\gl{\nA}
\bmng
amadayxpAyi; madayxpAna mADadava. 
\emng
\eentry

\bentry
\word{non-driver}
\pron{nAnfDerxYvarf}
\gl{\nA}
\bmng
moVTArf cAlakanalalxdava. 
\emng
\eentry

\bentry
\word[none(1)]{none}
\pron{nanf}
\gl{\sanA}
\bmng
oMdU alalx; obabxnU alalx; yAvudU alalx: \eng{none of them came} avaralilx obabxnU baralilalx. \eng{none of this concerns me} idaralilx yAvudU nanage (savxlapxvU) saMbaMdhisilalx. \eng{none but fools have ever believed it} mUKaRralalxde (mUKaRranunx biTuTx) beVre yArU idanunx eMdU naMbilalx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{none other} beVre yArU alalx, ilalx. 
\emng
\eentry

\bentry
\word[none(2)]{none}
\pron{nanf}
\gl{\gu}
\bmng
\bnum
\num{1} (pUvoRVkatx \nA kekx saMbaMdhisidaMte) ilalx; EnU ilalx: \eng{poetry, we have almost none} kAvayxveMbudu hecucx kaDime namamxlilx ilalxveV ilalx. 
\num{2} oMdu nidiRSaTx jAtige, vagaRkekx seVrisalAgada: \eng{if a linguist is wanted, I am none} BASA vijAcnxni beVkAgidadxre, nAnaMtU alalx. 
\enum
\emng
\eentry

\bentry
\word[none(3)]{none}
\pron{nanf}
\gl{\kirxvi}
\bmng
hececxVnU alalx; savxlapxvU ilalx; ilalxveV ilalx: \eng{I am none the wiser for it} adariMda nAneVnU hecucx budidhxvaMtanAgalilalx. \eng{the pay is none too high} saMbaLa bahaLa jAsitxyeVnU alalx. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{none the less} = \hyperlink{nevertheless}{nevertheless}. 
\emng
\eentry

\bentry
\word[none(4)]{none}
\pron{noVnf}
\gl{\nA}
\expl{(\bava dalilx saha \parx)}
\bmng
kerxYsatxra aidaneya pArxthaRnA samayada (madhAyxhanx \eng{3} gaMTe) ArAdhane. 
\emng
\eentry

\bentry
\word{non-earning}
\pron{nAnfaniRMgf}
\gl{\gu}
\bmng
(\kanmu\ niyatavAgi kUli yA saMbaLa) saMpAdisutitxlalxda. 
\emng
\eentry

\bentry
\word{non-effective}
\pron{nAnf iphekiTxvf}
\gl{\gu}
\bmng
pariNAmarahita; pariNAmakAriyalalxda; pariNAma, parxBAva -- ilalxda. 
\emng
\eentry

\bentry
\word{non-ego}
\pron{nAnfIgoV}
\gl{\nA}
\bmng
(\tashA) anAtamx; jaDa; aceVtana; jAcnxtaqvalalxda elalxvU. 
\emng
\eentry

\bentry
\word{nonentity}
\pron{nAnf eMTiTi}
\gl{\nA}
\expl{(\bava\ \eng{non-entities}).}
\bmng
\bnum
\num{1} aBAva; ilalxdiruvudu; ilalxme; anasitxtavx; shUnayxte. 
\num{2} ilalxdudx; ilalxda vasutx; (manaH) kalipxta vasutx. 
\num{3} anAmadheVya; agaNayx; yaHkashicxtf vayxkitx, vasutx; gaNanege bArada vayxkitx, vasutx. 
\enum
\emng
\eentry

\bentry
\word{nones}
\pron{noVnf\char144}
\gl{\nA}
\bmng
(\bava) 
\bnum
\num{1} (\roVpu) mAcfR, meV, juleY matutx akoTxVbarf tiMgaLugaLa \eng{7}neya tAriVKu, uLida tiMgaLugaLa \eng{5}neya tAriVKu. 
\numi{2} (\Eva\ saha) (\kerxY) 
\banum
\alnum{a} hiMde \eng{$9$}neya gaMTeyalilx (eMdare madhAyxhanx \eng{$3$} gaMTege) mADutitxdadx pArxthaRne. 
\alnum{b} A gaMTe; hotutx; veVLe. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{none-so-pretty}
\pron{nanfsoVperxTi}
\gl{\nA}
\bmng
= \hyperref{kandict_l.pdf}{L}{London pride}{London pride}. 
\emng
\eentry

\bentry
\word{non-essential}
\pron{nAnisenaSxlf}
\gl{\gu}
\bmng
Avashayxkavalalxda; agatayxvalalxda; anAvashayxkavAda. 
\emng
\eentry

\bentry
\word{nonesuch}
\pron{nanfsacf}
\gl{\nA}
\bmng
\hyperlink{nonsuch}{nonsuch} eMbudara rUpAMtara. 
\emng
\eentry

\bentry
\word{nonet}
\pron{noVneTf}
\gl{\nA}
\bmng
(\saM) 
\bnum
\num{1} navakagiVta; oMbatutx vAdayxgaLige yA dhavxnigaLige hoMduvaMte racisida giVta. 
\num{2} aMtha giVtavanunx hADuvavaru yA nuDisuvavaru; navakagiVta gAyakaru yA vAdakaru. 
\num{3} navaka; oMbatatxra guMpu taMDa. 
\enum
\emng
\eentry

\bentry
\word{nonetheless}
\pron{nanfdalesf}
\gl{\kirxvi}
\bmng
= \hyperlink{nevertheless}{nevertheless}. 
\emng
\eentry

\bentry
\word{non-Euclidean}
\pron{nAnfyUkilxDiAnf}
\gl{\gu}
\bmng
(jAyxmitiVya padadxtiya \vi) yUkilxDeVtara, yUkilxDana jAyxmitiyalilx aMgiVkarisuva AdhAra, BAvanegaLanunx tirasakxrisi muMduvariyuva yA yUkilxDana tatatx vxgaLanunx miVri hoVguva. 
\emng
\eentry

\bentry
\word[non-European(1)]{non-European}
\pron{nAnfyuarapianf}
\gl{\gu}
\bmng
airoVpeyxVtara; yUroVpinadalalxda. 
\emng
\eentry

\bentry
\word[non-European(2)]{non-European}
\pron{nAnfyuarapianf}
\gl{\nA}
\bmng
airoVpeyxVtara (vayxkitx). 
\emng
\eentry

\bentry
\word{non-event}
\pron{nAnfiveMTf}
\gl{\nA}
\bmng
amuKayx GaTane; avishiSaTx GaTane; (beVkeMdeV visheVSa mahatavxvanunx koDalu parxyatinxsidarU) vishiSaTxvAgada GaTane, saMdaBaR. 
\emng
\eentry

\bentry
\word{non-existence}
\pron{nAnigi\char144saTxnfsx}
\gl{\nA}
\bmng
anasitxtavx; asitxtavxdalilxlalxdiruvudu. 
\emng
\eentry

\bentry
\word{non-existent}
\pron{nAnigi\char144saTxMTf}
\gl{\gu}
\bmng
anasitxtavxda; asitxtavxdalilxlalxda. 
\emng
\eentry

\bentry
\word{non-explosive}
\pron{nAnikfsxpolxVsivf}
\gl{\gu}
\bmng
(oMdu vasutxvina \vi) sopxVTisada; asoPxVTaka. 
\emng
\eentry

\bentry
\word{non-fattening}
\pron{nAnfphAYxTaniMgf}
\gl{\gu}
\bmng
(AhArada \vi) kobibxsada; kobubx uMTumADada, beLesada. 
\emng
\eentry

\bentry
\word{non-feasance}
\pron{nAnfphiVsaZnfsx}
\gl{\nA}
\bmng
(\nAyxshA) kataRvayxloVpa; mADabeVkAgidudxdanunx mADade biDuvudu. 
\emng
\eentry

\bentry
\word{non-ferrous}
\pron{nAnfpherasf}
\gl{\gu}
\bmng
(\ravi) (loVhada vi) kabibxNeVtara; ayaseVtara; kabibxNa matutx ukukxgaLige saMbaMdhisirada. 
\emng
\eentry

\bentry
\word{non-fiction}
\pron{nAnfphikfSanf}
\gl{\nA}
\bmng
akalipxta vasutx kaqti; vasutxBUtavAda viSaya, saMgatigaLanunx kuritu racisida sAhitayx kaqti (\kanmu\ kathe kAdaMbarigaLalalxda, jiVvana cariterx yA parAmashaRna garxMtha \mo\ kaqti yA kaqtigaLu). 
\emng
\eentry


\bentry
\word{non-fictional}
\pron{nAnfphikfSanalf}
\gl{\gu}
\bmng
(\kanmu\ kathe, kAdaMbarigaLalalxda jiVvanacariterx, parAmashaRna garxMtha, \mo) akalipxta vasutx kaqtiya. 
\emng
\eentry

\bentry
\word{non-flam}
\pron{nAnfphAlxYxmf}
\gl{\gu}
\bmng
= \hyperlink{non-flammable}{non-flammable}. 
\emng
\eentry

\bentry
\word{non-flammable}
\pron{nAnfphAlxYxmabflf}
\gl{\gu}
\bmng
uri hotitxkoLaLxda; adAhayx. 
\emng
\eentry

\bentry
\word{non-fulfilment}
\pron{nAnfphulfphilfmaMTf}
\gl{\nA}
\bmng
(oMdu kataRvayxvanunx) IDeVrisade iruvudu; neraveVrisade iruvudu. 
\emng
\eentry

\bentry
\word{non-functional}
\pron{nAnfphaMkfSanalf}
\gl{\gu}
\bmng
kAyaRkAriyalalxda; kAyaRrahita; yAvudeV kelasa mADada yA ilalxda; akirxyAkAri. 
\emng
\eentry

\bentry
\word{nong}
\pron{nAMgf}
\gl{\nA}
\bmng
(\AseTxrXV) (\ashi) gAMpa; gugugx; daDaDx. 
\emng
\eentry

\bentry
\word{non-governmental}
\pron{nAnfgavanfRmeMTalf}
\gl{\gu}
\bmng
asakARri; sakARreVtara; sakARrakekx seVrada yA saMbaMdhisirada. 
\emng
\eentry

\bentry
\word[non-human(1)]{non-human}
\pron{nAnfhUYxmanf}
\gl{\gu}
\bmng
manuSeyxVtara; mAnavaBinanx; manuSayxjAtige seVrada. 
\emng
\eentry

\bentry
\word[non-human(2)]{non-human}
\pron{nAnfhUYxmanf}
\gl{\nA}
\bmng
manuSeyxVtara; mAnavaBinanx; manuSayxjAtige seVrada jiVvi. 
\emng
\eentry

\bentry
\word{non-infectious}
\pron{nAnfinfphekfSasf}
\gl{\gu}
\bmng
(roVgada \vi) asAMkArxmika; soVMkalalxda; aMTalalxda. 
\emng
\eentry

\bentry
\word{non-inflected}
\pron{nAnfinfphelxkiTxDf}
\gl{\gu}
\bmng
(BASeya \vi) (viBakitx, AKAyxta) parxtayxyagaLilalxda; parxtayxya rahita; pada niSapxtitxgAgi viBakitx yA AKAyxta parxtayxyagaLanunx hacacxda, baLasada. 
\emng
\eentry

\bentry
\word{non-interference}
\pron{nAnfiMTarfphiaranfsx}
\gl{\nA}
\bmng
(\kanmu\ rAjaniVti hAgU aMtararASiTxrXVya vayxvahAradalilx) tATasathxyX (niVti); itarara vivAdagaLalilx talehAkadiruvudu, taTasathxvAgiruvudu, madhayx parxveVshisadiruvudu. 
\emng
\eentry

\bentry
\word{non-intervention}
\pron{nAnfiMTaveRnaSxnf}
\gl{\nA}
\bmng
= \hyperlink{non-interference}{non-interference}. 
\emng
\eentry

\bentry
\word{non-intoxicating}
\pron{nAniMTAkisxkeVTiMgf}
\gl{\gu}
\bmng
(pAniVyada \vi) nashe barisada; matutxbarisada. 
\emng
\eentry

\bentry
\word{non-iron}
\pron{nAnfaianfR}
\gl{\gu}
\bmng
(baTeTxya \vi) isitxrX mADabeVkAgilalxda. 
\emng
\eentry

\bentry
\word{non-joinder}
\pron{nAnfjAiMDarf}
\gl{\nA}
\bmng
(\nAyxshA) pAlugArike loVpa; sahaBAgitavx loVpa; pAludAra \mo varu vayxvahAradalilx matotxbabx pAludAra \mo varoDane sahaBAgiyAgi seVrade, sahakarisade tapupxva loVpa. 
\emng
\eentry

\bentry
\word{non-juring}
\pron{nAnfjuariMgf}
\gl{\gu}
\bmng
\bnum
\num{1} parxmANa vacana sivxVkarisada yA nirAkarisuva. 
\num{2} (\ca) (pAdirxgaLa \vi) \eng{1689}ralilx iMgelxMDina viliyaM matutx meVriyarige niSeThxyiMdiruva parxmANavacanavanunx nirAkarisidadx. 
\enum
\emng
\eentry

\bentry
\word{non-juror}
\pron{nAnfjuararf}
\gl{\nA}
\bmng
\bnum
\num{1} parxmANa vacana sivxVkarisadavanu. 
\num{2} (\ca) (\kanmu) \eng{1689}ralilx iMgelxMDina rAja viliyaM vatutx rANi meVriyarige niSeThxyiMdiruva parxmANa vacanavanunx sivxVkarisalu nirAkarisida pAdirx vagaRdavanu. 
\enum
\emng
\eentry

\bentry
\word{non-jury}
\pron{nAnfjuari}
\gl{\gu}
\bmng
(vicAraNe \vi) nAyxyadashiRgaLilalxde mADida. 
\emng
\eentry

\bentry
\word{non-linear}
\pron{nAnfliniarf}
\gl{\gu}
\bmng
(\ga) areVKiVya; reVKiVya saMbaMdha yA samiVkaraNagaLige -- horatAda. 
\emng
\eentry

\bentry
\word{non-literary}
\pron{nAnfliTarari}
\gl{\gu}
\bmng
asAhitayxka; (bareha \mo vugaLa \vi) guNadalilx sAhitAyxtamxkavalalxda. 
\emng
\eentry

\bentry
\word{non-logical}
\pron{nAnflAjikalf}
\gl{\gu}
\bmng
takaRBinanx; takeRVtara; takaRvanunx oLagoLaLxda; takaR mAgaRvanunx biTuTx beVre mAgaRdalilx parxvatiRsuva; atAkiRka. 
\emng
\eentry

\bentry
\word{non-logically}
\pron{nAnflAjikali}
\gl{\kirxvi}
\bmng
atAkiRkavAgi; takaRmAgaRvanunx biTuTx beVre mAgaRdalilx. 
\emng
\eentry

\bentry
\word{non-magnetic}
\pron{nAnfmAYxgenxTikf}
\gl{\gu}
\bmng
(oMdu vasutxvina \vi) akAMtiVya; kAMtiVyavalalxda. 
\emng
\eentry

\bentry
\word{non-member}
\pron{nAnfmeMbarf}
\gl{\nA}
\bmng
sadaseyxVtara; (oMdu saMGa, kalxbubx, \mo vugaLa) sadasayxnalalxdava; asadasayx. 
\emng
\eentry

\bentry
\word{non-membership}
\pron{nAnfmeMbarfSipf}
\gl{\nA}
\bmng
(oMdu saMGa, kalxbubx, \mo vugaLa) sadasayxnalalxdiruvike; asadasayxtavx. 
\emng
\eentry

\bentry
\word{non-metal}
\pron{nAnfmeTalf}
\gl{\nA}
\bmng
(\ravi) aloVha; loVhavalalxdiruva rAsAyanika dhAtu. 
\emng
\eentry

\bentry
\word{non-metallic}
\pron{nAnfmiTAYxlikf}
\gl{\gu}
\bmng
(\ravi) aloVhiVya; loVhavalalxda; loVhaguNagaLilalxda. 
\emng
\eentry

\bentry
\word{non-militant}
\pron{nAnfmiliTaMTf}
\gl{\gu}
\bmng
\bnum
\num{1} kadanashiVlavalalxda; kadanashiVlanalalxda; AkarxmaNashiVlava(na)lalxda. 
\num{2} yudadxniratanalalxda; yudadhxdalilx toDagirada. 
\enum
\emng
\eentry

\bentry
\word{non-military}
\pron{nAnfmiliTari}
\gl{\gu}
\bmng
aseYnika: 
\banum
\alnum{a} seVnAcaraNeyanonxLagoLaLxda. 
\alnum{b} seVnege seVrirada; nAgarika. 
\eanum
\emng
\eentry

\bentry
\word{non-ministerial}
\pron{nAnfminisiTxarialf}
\gl{\gu}
\bmng
\bnum
\num{1} sacivanige saMbaMdhapaDada. 
\num{2} maMtirx maMDalakekx saMbaMdhapaDada. 
\num{3} sakARrada ilAKege seVrirada, saMbaMdhapaDada. 
\enum
\emng
\eentry

\bentry
\word{non-moral}
\pron{nAnfmAralf}
\gl{\gu}
\bmng
niVtishAsatxrXkekx, neYtikatege -- saMbaMdhisirada (\eng{amoral, immoral} hoVlisi). 
\emng
\eentry

\bentry
\word{non-morally}
\pron{nAnfmArali}
\gl{\kirxvi}
\bmng
niVti shAsatxrXkekx, neYtikatege -- saMbaMdhisiradaMte. 
\emng
\eentry

\bentry
\word{non-natural}
\pron{nAnfnAYxcaralf}
\gl{\gu}
\bmng
parxkaqtiBinanx; parxkaqti mAgaRkekx yA parxkirxyege seVrirada, adakekx anusAravAgi vatiRsada yA irada aneYsagiRka. 
\emng
\eentry

\bentry
\word{non-negotiable}
\pron{nAnfnigoVSabflf}
\gl{\gu}
\bmng
\bnum
\num{1} (huMDi \mo vugaLa \vi) inonxbabxnige vagARvaNe mADalAgada. 
\num{2} (huMDi \mo vanunx) nagadisalAgada; nagadAgi parivatiRsalAgada. 
\enum
\emng
\eentry

\bentry
\word{non-net}
\pron{nAnfneTf}
\gl{\gu}
\bmng
(pusatxka \mo vu) kaniSaThx mArATa bele(sUcane)ge oLapaDada; mArATada kaniSaThx beleyanunx nigadipaDisada. 
\emng
\eentry

\bentry
\word{non-nuclear}
\pron{nAnfnUyxkilxarf}
\gl{\gu}
\bmng
\bnum
\num{1} abeYjika; (paramANu) biVjakekx yA beYjika shakitxge saMbaMdhisada. 
\num{2} aNavxsatxrXgaLilalxda; aNavxsatxrX baLasada; aNavxsatxrX rahita. 
\enum
\emng
\eentry

\bentry
\word{non-observance}
\pron{nAnfabasxvaRnfsx}
\gl{\nA}
\bmng
(\kanmu\ opapxMda, Avashayxkate, \mo vanunx) pAlisadiruvudu; ananupAlane. 
\emng
\eentry

\bentry
\word{non-operational}
\pron{nAnfApareVSanalf}
\gl{\gu}
\bmng
\bnum
\num{1} kAyaR mADada; kAyaRkAriyalalxda. 
\num{2} keTiTxruva; kelasa mADada; keTuTx hoVgiruva. 
\enum
\emng
\eentry

\bentry
\word{non-organic}
\pron{nAnfAgAYxRnikf}
\gl{\gu}
\bmng
= \hyperref{kandict_i.pdf}{I}{inorganic}{in-organic}. 
\emng
\eentry

\bentry
\word[non-pareil(1)]{non-pareil}
\pron{nAnfpara(reV)lf}
\gl{\nA}
\bmng
eNeyilalxda vasutx yA vayxkitx; aparxtimavAdudu; adivxtiVyavAda vayxkitx yA vasutx. 
\emng
\eentry

\bentry
\word[non-pareil(2)]{non-pareil}
\pron{nAnfpara(reV)lf}
\gl{\gu}
\bmng
eNeyilalxda; aparxtima; adivxtiVya. 
\emng
\eentry

\bentry
\word{non-participating}
\pron{nAnfpATiRsipeVTiMgf}
\gl{\gu}
\bmng
BAgavahisada; aBAgi. 
\emng
\eentry

\bentry
\word{non-partisan}
\pron{nAnfpATiRsAZYxnf}
\gl{\gu}
\bmng
niSapxkaSxpAta; pakaSxpAtarahitavAda; yAvudeV pakaSxda paravAgi irada. 
\emng
\eentry

\bentry
\word{non-party}
\pron{nAnfpATiR}
\gl{\gu}
\bmng
pakASxtiVta; rAjakiVya pakaSxgaLalalxda. 
\emng
\eentry

\bentry
\word{non-payment}
\pron{nAnfpeVmaMTf}
\gl{\nA}
\bmng
koDade iruvudu, beVpAvati; pAvati mADade iruvudu. 
\emng
\eentry

\bentry
\word{non-person}
\pron{nAnfpasaRnf}
\gl{\nA}
\bmng
agaNayx vayxkitx; asitxtavxvilalxda yA asitxtavxvanunx gaNanege tegedukoLaLxda vayxkitx. 
\emng
\eentry

\bentry
\word{non-personal}
\pron{nAnfpasaRnalf}
\gl{\gu}
\bmng
aveYyakitxka; veYyakitxka saMbaMdha, Asakitx, \mo vu irada. 
\emng
\eentry

\bentry
\word{non-physical}
\pron{nAnfphisiZkalf}
\gl{\gu}
\bmng
\bnum
\num{1} aBwtika. 
\num{2} adeYhika; ashAriVraka. 
\enum
\emng
\eentry

\bentry
\word{non-physically}
\pron{nAnfphisiZkali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} aBwtikavAgi. 
\num{2} deVhakekx saMbaMdhisirade; ashAriVrakavAgi. 
\enum
\emng
\eentry

\bentry
\wordf{non placet}
\pron{nAnf pelxVseTf}
\gl{\nA}
\expl{\Latin}
\bmng
(caciRna yA vishavxvidAyxnilayada saBegaLalilx niVDalAguva) nakArAtamxka mata yA voVTu. 
\emng
\eentry

\bentry
\word{non-playing}
\pron{nAnfpelxViMgf}
\gl{\gu}
\bmng
(oMdu ATa \mo vugaLalilx) ADada; BAgavahisada. 
\emng
\eentry

\bentry
\word[nonplus(1)]{nonplus}
\pron{nAnfpalxsf}
\gl{\nA}
\bmng
digaBxrXme; tabibxbubx; dikukx toVrada sithxti; satxbadhxte. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{at a nonplus} dikukx toVrade; galibiliyalilx. 
\num{2} \eng{reduce one to a nonplus}(obabxnanunx) galibiligoLisu; tabibxbAbxgisu. 
\enum
\emng
\eentry

\bentry
\word[nonplus(2)]{nonplus}
\pron{nAnfpalxsf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nonplussed}, \vakaq\ \eng{nonplussing}).}
\bmng
digaBxrXmegoLisu; tabibxbAbxgisu, dikukx toVradaMte mADu; galibiligoLisu. 
\emng
\eentry

\bentry
\word{non-poisonous}
\pron{nAnfpAisaZnasf}
\gl{\gu}
\bmng
(oMdu vasutxvina \vi) viSavalalxda; naMjalalxda. 
\emng
\eentry

\bentry
\word{non-political}
\pron{nAnfpaliTikalf}
\gl{\gu}
\bmng
rAjakiVyavalalxda; rAjakiVyadalilx ilalxda, seVrilalxda; rAjakiVyakekx saMbaMdhisada. 
\emng
\eentry

\bentry
\word{non-porous}
\pron{nAnfpoVrasf}
\gl{\gu}
\bmng
(oMdu vasutxvina \vi) tUtugaLilalxda; raMdharxgaLilalxda; araMdharx; niVraMdharx. 
\emng
\eentry

\bentry
\wordf{non possumus}
\pron{nAnf pAsuyxmasf}
\gl{\nA}
\expl{\Latin}
\bmng
(nidiRSaTx \vi) `yAvudeV kAyaRkarxma keYgoLaLxlu sAdhayxvilalx' eMdu tiLisuva heVLike. 
\emng
\eentry

\bentry
\word{non-productive}
\pron{nAnfparxDakiTxvf}
\gl{\gu}
\bmng
anutApxdaka; utApxdane ilalxda yA mADada. 
\emng
\eentry

\bentry
\word{non-productively}
\pron{nAnfparxDakiTxvfli}
\gl{\kirxvi}
\bmng
anutApxdakavAgi; utApxdane mADade yA ilalxdaMte. 
\emng
\eentry

\bentry
\word{non-professional}
\pron{nAnfparxpheSanalf}
\gl{\gu}
\bmng
(\kanmu\ dajeRyalilx) vaqtitxniratanalalxda. 
\emng
\eentry

\bentry
\word{non-profit}
\pron{nAnfpArxphiTf}
\gl{\gu}
\bmng
(vAyxpAra, udayxma, \mo vugaLa \vi) 
\banum
\alnum{a} lABoVdedxVshavilalxda; lABApeVkeSxyilalxda; lABa nirapeVkaSx; keVvala lABa gaLisuva udedxVshavilalxde naDesuva. 
\alnum{b} lABa gaLisada. 
\eanum
\emng
\eentry

\bentry
\word{non-profit-making}
\pron{nAnfpArxphiTfmeVkiMgf}
\gl{\gu}
\bmng
(oMdu udayxmada \vi) lABanirapeVkaSx; lABoVdedxVshavilalxda; keVvala lABa gaLisuva udedxVshadiMda naDesada; lABApeVkeSxyilalxda. 
\emng
\eentry

\bentry
\word{non-proliferation}
\pron{nAnfparxliphareVSanf}
\gl{\nA}
\bmng
(\kanmu\ aNavxsatxrXgaLa \vi) Adhikayx niroVdha; parxsaraNa niSeVdha; hecacxLavanunx yA hecicxna utApxdaneyanunx taDegaTuTxvudu. 
\emng
\eentry

\bentry
\word{non-racial}
\pron{nAnfreVSalf}
\gl{\gu}
\bmng
vaNaRBeVdavalalxda; vaNaRda yA vaNaRkekx saMbaMdhisida aMshagaLilalxda. 
\emng
\eentry

\bentry
\word{non-reader}
\pron{nAnfriVDarf}
\gl{\nA}
\bmng
Odadava yA Odalu baradava. 
\emng
\eentry

\bentry
\word{non-residence}
\pron{nAnfresiZDanfsx}
\gl{\nA}
\bmng
\bnum
\num{1} oMdu parxdeVshadalilx tAtAkxlikavAgi taMgiruvudu. 
\num{2} (pAdirxya \vi) vaqtitx naDesutitxruva sathxLadalilx irade beVre kaDe vAsisuvudu. 
\num{3} hudedxdAranu hudedx iruva sathxLadalilx vAsisabeVkeMba kaDADxyavilalxdiruvudu. 
\enum
\emng
\eentry

\bentry
\word[non-resident(1)]{non-resident}
\pron{nAnfresiZDaMTf}
\gl{\gu}
\bmng
anivAsi: 
\banum
\alnum{a} oMdu parxdeVshadalilx tAtAkxlikavAgi taMgiruva yA beVroMdu kaDe vAsisuva. 
\alnum{b} (pAdirx \mo vara \vi) vaqtitx naDesutitxruva sathxLadalilx vAsisade beVroMdu kaDe vAsisuva. 
\alnum{c} (hudedxya \vi) hudedxdAranu adeV sathxLadalilx vAsisabeVkeMba kaDADxyavilalxda. 
\eanum
\emng
\eentry

\bentry
\word[non-resident(2)]{non-resident}
\pron{nAnfresiZDaMTf}
\gl{\nA}
\bmng
anivAsi: 
\banum
\alnum{a} sathxLavaMdiganalalxdavanu; oMdu parxdeVshadalilx tAtAkxlikavAgi taMgiruvavanu yA beVroMdu kaDe vAsisutitxruvavanu. 
\alnum{b} sathxLavaMdiganalalxda pAdirx; vaqtitx naDesutitxruva sathxLadalilx vAsisuva badalu beVre kaDe vAsisuva pAdirx. 
\alnum{c} hudedxdAranu adeV sathxLadalilx vAsisabeVkeMba kaDADxyavilalxda hudedx. 
\alnum{d} oMdu hoVTelinalilx vAsamADade alilxna swkayaRgaLanunx baLasikoLuLxvavanu. 
\eanum
\emng
\eentry

\bentry
\word{non-residential}
\pron{nAnfresiDenaSxlf}
\gl{\gu}
\bmng
anivAsiVya; anivAsi. 
\emng
\eentry

\bentry
\word{non-resistance}
\pron{nAnfrisiZsaTxnfsx}
\gl{\nA}
\bmng
parxtiroVdhABAva; parxtiroVdha mADadiruvudu yA ilalxdiruvudu. 
\emng
\eentry

\bentry
\word{non-returnable}
\pron{nAnfriTanaRbflf}
\gl{\gu}
\bmng
hiMdirugisabeVkAgilalxda yA hiMdirugisalAgada. 
\emng
\eentry

\bentry
\word{non-rigid}
\pron{nAnfrijiDf}
\gl{\gu}
\bmng
(\kanmu\ vasutxgaLa \vi) namayx; bAguva. 
\emng
\eentry

\bentry
\word{non-scientific}
\pron{nAnfseYaniTxphikf}
\gl{\gu}
\bmng
vijAcnxneVtara; vijAcnxna athavA veYjAcnxnika vidhAnagaLanunx oLagoLaLxda. 
\emng
\eentry

\bentry
\word{non-scientist}
\pron{nAnfseYaniTxsfTx}
\gl{\nA}
\bmng
vijAcnxniyalalxdava. 
\emng
\eentry

\bentry
\word{non-sectarian}
\pron{nAnfsekeTxVrianf}
\gl{\gu}
\bmng
apaMthiVya; yAvudeV paMthakekx seVrada. 
\emng
\eentry

\bentry
\word{nonsense}
\pron{nAnfse(sa)nfsx}
\gl{\nA}
\bmng
\bnum
\num{1} asaMbadadhxte; tiVra aviveVkada, asaMbadadhxda, athaRvilalxda mAtu yA kalapxne; hucucx mAtu. 
\num{2} atireVkada yA aviveVkada naDavaLike: \eng{I stand no nonsense} nAnu aviveVkada naDavaLikeyanunx sahisalAre. 
\num{3} asamamxtavAda EpARDu, vayxvasethx, itAyxdi. 
\num{4} tiVra asaMbadadhx (KaMDita nijaviralAradu): \eng{stuff and nonsense} shudAdhxMga suLuLx, asaMbadadhx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{no nonsense} gaMBiVravAda; lAGavavalalxda; celulxcelAlxgirada. 
\emng
\eentry

\bentry
\word{nonsense-verses}
\pron{nAnfsanfsx vasaRsf}
\gl{\nA}
\bmng
anathaR kavanagaLu; asaMbadadhx kavanagaLu; athaRvilalxda yA asaMbadAdhxthaRda padayxgaLu. 
\emng
\eentry

\bentry
\word{nonsensical}
\pron{nAnfsenisxkalf}
\gl{\gu}
\bmng
asaMbadadhxvAda; aviveVkada; hucucxhucAcxda, athaRvilalxda. 
\emng
\eentry

\bentry
\word{nonsensicality}
\pron{nAnfsenisxkAYxliTi}
\gl{\nA}
\expl{(\bava\ \eng{nonsensicalities}).}
\bmng
asaMbadadhxte; hucucx hucAcxda, aviveVkada naDavaLike. 
\emng
\eentry

\bentry
\word{nonsensically}
\pron{nAnfsenisxkali}
\gl{\kirxvi}
\bmng
asaMbadadhxvAgi; aviveVkadiMda; hucucxhucAcxgi; aviveVkavAgi; athaRvilalxda riVtiyalilx. 
\emng
\eentry

\bentry
\wordf{non sequitur}
\pron{nAnfsekiTxTarf}
\gl{\nA}
\expl{\Latin}
\bmng
(\takaR) takaRrahita -- anumAna, niNaRya; parxtijAcnx vAkayxgaLiMda yA AdhAravacanagaLiMda, tAkiRka saMbaMdhavilalxde horaDuva anumAna, anumiti, niNaRya, tiVmARna. 
\emng
\eentry

\bentry
\word{non-sexual}
\pron{nAnfsekusx yxalf, nAnfsekfSualf}
\gl{\gu}
\bmng
aleYMgika; leYMgikateyanunx avalaMbisada yA leYMgikateyanunx oLagoLaLxda. 
\emng
\eentry

\bentry
\word{non-sexually}
\pron{nAnfsekusx yxali, nAnfsekfSuali}
\gl{\kirxvi}
\bmng
aleYMgikavAgi; leYMgikateyanunx oLagoLaLxde. 
\emng
\eentry

\bentry
\word{non-skid}
\pron{nAnfsikxDf}
\gl{\gu}
\bmng
(vAhanagaLa rababxru paTiTx \mo vugaLa \vi) jAradaMtha; jAruvudeMba Bayavilalxda; horaLadaMte nimiRsida. 
\emng
\eentry

\bentry
\word{non-slip}
\pron{nAnfsilxpf}
\gl{\gu}
\bmng
jAri hoVgada hAge saMyoVjisida; nuNucikoLaLxda hAge racisida. 
\emng
\eentry

\bentry
\word{non-smoker}
\pron{nAnfsomxVkarf}
\gl{\nA}
\bmng
\bnum
\num{1} adhUmapAni; sigareVTu vageYregaLanunx seVdadavanu. 
\num{2} (reYlu \mo vugaLalilx) sigareVTu seVdabAradeMba niyamakokxLagAgiruva aMkaNa, are; dhUmapAna niSidadhx viBAga, Dabibx. 
\enum
\emng
\eentry

\bentry
\word[non-smoking(1)]{non-smoking}
\pron{nAnfsomxVkiMgf}
\gl{\gu}
\bmng
dhUmapAna mADada. 
\emng
\eentry

\bentry
\word[non-smoking(2)]{non-smoking}
\pron{nAnfsomxVkiMgf}
\gl{\nA}
\bmng
dhUmapAna mADadiruvudu. 
\emng
\eentry

\bentry
\word{non-soluble}
\pron{nAnfsAluYxbflf}
\gl{\gu}
\bmng
(\kanmu\ oMdu vasutxvina \vi) adArxvayx; karagada. 
\emng
\eentry

\bentry
\word{non-specialist}
\pron{nAnfsepxSalisfTx}
\gl{\nA}
\bmng
(oMdu nidiRSaTx \vi) atajacnx; apariNata; tajacnxnalalxdava; pariNatanalalxdava. 
\emng
\eentry

\bentry
\word{non-specific}
\pron{nAnfsipxsiphikf}
\gl{\gu}
\bmng
anidiRSaTx; nidiRSaTxgoLisirada. 
\emng
\eentry

\bentry
\word{non-standard}
\pron{nAnfsATxYxMDaDfR}
\gl{\gu}
\bmng
sATxyXMDaDfR alalxda; parxmANaka alalxda. 
\emng
\eentry

\bentry
\word{non-starter}
\pron{nAnfsATxTaRrf}
\gl{\nA}
\bmng
\bnum
\num{1} (paMdayxdalilx) shurumADada, AraMBisada yA upakarxmisada -- pArxNi yA vayxkitx. 
\num{2} (\AmA) yashasivx yA pariNAmakAri yA parxBAvashAli Aguva saMBavavilalxda -- vayxkitx yA vasutx. 
\enum
\emng
\eentry

\bentry
\word{non-stick}
\pron{nAnfsiTxkf}
\gl{\gu}
\bmng
aMTada: 
\banum
\alnum{a} aMTikoLaLxda. 
\alnum{b} tanage beVroMdu vasutx aMTalu biDada. 
\eanum
\emng
\eentry

\bentry
\word[non-stop(1)]{non-stop}
\pron{nAnfsATxpf}
\gl{\gu}
\bmng
\bnum
\num{1} (reYlu, basusx, \mo vu) taDerahita; madheyx nilalxda; nilugaDeyilalxda; naDuvaNa nilAdxNagaLalilx nilalxda. 
\num{2} (parxyANa) madheyx elUlx taMgade naDesida; madheyx elUlx nilalxde mADida. 
\enum
\emng
\eentry

\bentry
\word[non-stop(2)]{non-stop}
\pron{nAnfsATxpf}
\gl{\nA}
\bmng
\bnum
\num{1} taDerahita, madheyx nilalxda-reYlu, basusx, \mo vu. 
\num{2} taDerahita parxyANa; madheyx nilalxda reYlu, basusxgaLalilx parxyANa. 
\enum
\emng
\eentry

\bentry
\word[non-stop(3)]{non-stop}
\pron{nAnfsATxpf}
\gl{\kirxvi}
\bmng
taDerahitavAgi; madheyx elUlx nilalxde, taMgade. 
\emng
\eentry

\bentry
\word{non-subscriber}
\pron{nAnfsabfsekxrXYbarf}
\gl{\nA}
\bmng
caMdAdAranalalxdava. 
\emng
\eentry

\bentry
\word{nonsuch}
\pron{nanfsacf}
\gl{\nA}
\bmng
\bnum
\num{1} eNeyilalxda, aparxtima -- vayxkitx yA vasutx; anupamavAdadudx yA anupamavAdavanu. 
\num{2} oMdu taraha kudure masAle sopupx. 
\enum
\emng
\eentry

\bentry
\word[nonsuit(1)]{nonsuit}
\pron{nAnfsUyx(sU)Tf}
\gl{\nA}
\bmng
(\nAyxshA) dAvA radudx; dAvA radidxyAti; (vAdi vayxvahAra kAraNavanunx kAnUnina parxkAra sAthxpisalAgadidadxlilx yA takakxSuTx sAkaSxyX taradidadxre) nAyxyAdhiVshanu dAveyanunx radudxpaDisuvudu. 
\emng
\eentry

\bentry
\word[nonsuit(2)]{nonsuit}
\pron{nAnfsUyx(sU)Tf}
\gl{\sakirx}
\bmng
(\nAyxshA) dAvA radudxpaDisu; vayxvahAra radudxgoLisu. 
\emng
\eentry

\bentry
\word{non-swimmer}
\pron{nAnfsivxmarf}
\gl{\nA}
\bmng
Ijadava yA Iju bAradava. 
\emng
\eentry

\bentry
\word{non-technical}
\pron{nAnfTekinxkalf}
\gl{\gu}
\bmng
\bnum
\num{1} atAMtirxka; tAMtirxkavalalxda. 
\num{2} taMtarxjAcnxnarahita; taMtarxjAcnxnavilalxda. 
\num{3} tAMtirxka yA shAsitxrXVya pariBASeyadalalxda. 
\enum
\emng
\eentry

\bentry
\word{non-toxic}
\pron{nAnfTAkisxkf}
\gl{\gu}
\bmng
viSakaravalalxda; naMjina savxBAvadadxlalxda. 
\emng
\eentry

\bentry
\word{non-transferable}
\pron{nAnfTArxYxnfsxpharabflf}
\gl{\gu}
\bmng
hasAtxMtaraNa sAdhayxvilalxda; vagARvaNe sAdhayxvAgada. 
\emng
\eentry

\bentry
\word{non-U}
\pron{nAnf yU}
\gl{\gu}
\bmng
(\AmA) meVluvagaRkekx sahajavalalxda; ucacx vagaRdavara lakaSxNavAgirada. 
\emng
\eentry

\bentry
\word{non-uniform}
\pron{nAnfyUniphAmfR}
\gl{\gu}
\bmng
EkarUpavAgilalxda; oMdeV riVtiyAgirada. 
\emng
\eentry

\bentry
\word{non-union}
\pron{nAnfyUnianf}
\gl{\gu}
\bmng
\bnum
\num{1} kAmiRka saMGakekx seVrada. 
\num{2} kAmiRka saMGada sadasayxriMda mADirada yA utApxdisirada. 
\enum
\emng
\eentry

\bentry
\word{non-usage}
\pron{nAnfyUsiZjf}
\gl{\nA}
\bmng
anupayoVga; beVbaLake; upayoVgisadiruvudu; baLasadiruvudu. 
\emng
\eentry

\bentry
\word{non-use}
\pron{nAnfyUsfZ}
\gl{\nA}
\bmng
= \hyperlink{non-usage}{non-usage}. 
\emng
\eentry

\bentry
\word{non-user}
\pron{nAnfyUsaZrf}
\gl{\nA}
\bmng
(\nAyxshA) anupayoVga; iruva hakakxnunx calAyisade alakaSx yxdiMdiruvudu matutx A mUlaka hakakxnunx kaLedukoLuLxvudu. 
\emng
\eentry

\bentry
\word{non-verbal}
\pron{nAnfvabaRlf}
\gl{\gu}
\bmng
padarahita; mAtilalxda; padagaLu, mAtu -- ilalxda. 
\emng
\eentry

\bentry
\word{non-verbally}
\pron{nAnfvabaRli}
\gl{\kirxvi}
\bmng
padarahitavAgi; mAtilalxde; padagaLiMda, mAtiniMda heVLade. 
\emng
\eentry


\bentry
\word{non-vintage}
\pron{nAnfviMTijf}
\gl{\gu}
\bmng
(veYnu \mo vugaLa \vi) sugigxya dArxkiSxyiMda mADada. 
\emng
\eentry

\bentry
\word{non-violence}
\pron{nAnfveYalanfsx}
\gl{\nA}
\bmng
ahiMse. 
\emng
\eentry

\bentry
\word{non-violent}
\pron{nAnfveYalaMTf}
\gl{\gu}
\bmng
ahiMseya; hiMse mADadiruvudakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{non-volatile}
\pron{nAnfvAlaTeYlf}
\gl{\gu}
\bmng
(\kanmu\ oMdu vasutxvina \vi) anAvishiVla; avishiVlavalalxda. 
\emng
\eentry

\bentry
\word{non-voter}
\pron{nAnfvoVTarf}
\gl{\nA}
\bmng
mata calAyisadava yA mata calAyisuva hakukx ilalxdava. 
\emng
\eentry

\bentry
\word{non-voting}
\pron{nAnfvoVTiMgf}
\gl{\gu}
\bmng
voVTu -- ilalxda yA mADada. 
\emng
\eentry

\bentry
\word[non-White(1)]{non-White}
\pron{nAnfveYTf}
\gl{\gu}
\bmng
\bnum
\num{1} (vayxkitxya \vi) biLiyanalalxda. 
\num{2} biLiyaralalxdavara yA avarige saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word[non-White(2)]{non-White}
\pron{nAnfveYTf}
\gl{\nA}
\bmng
biLiyanalalxdava. 
\emng
\eentry

\bentry
\word{non-word}
\pron{nAnfvaDfR}
\gl{\nA}
\bmng
dAKaleyalilx ilalxda yA baLakeyeV mADirada-pada. 
\emng
\eentry

\bentry
\word[noodle(1)]{noodle}
\pron{nUDflf}
\gl{\nA}
\bmng
\bnum
\num{1} daDaDx; heDaDx; gugugx; gAMpa. 
\num{2} (\ashi) tale. 
\enum
\emng
\eentry

\bentry
\word[noodle(2)]{noodle}
\pron{nUDflf}
\gl{\nA}
\bmng
oNagisida hiTiTxniMda paTiTxgaLaMte yA baLegaLaMte mADida oMdu bageya tinisu (\udA\ saMDige, shAvige). 
\emng
\eentry

\bentry
\word{noodledom}
\pron{nUDflfDamf}
\gl{\nA}
\bmng
daDaDxtana; heDaDxtana. 
\emng
\eentry

\bentry
\word{nook}
\pron{nUkf}
\gl{\nA}
\bmng
\bnum
\num{1} goMdi; mUle tAku. 
\num{2} vivikatxvAgiruva jAga; mUle 
\enum
\emng
\eentry

\bentry
\word{nookie}
\pron{nUki}
\gl{\nA}
\bmng
= \hyperlink{nooky}{nooky}. 
\emng
\eentry

\bentry
\word{nooky}
\pron{nUki}
\gl{\nA}
\bmng
(\ashi) saMBoVga; meYthuna. 
\emng
\eentry

\bentry
\word{noon}
\pron{nUnf}
\gl{\nA}
\bmng
madhAyxhanx; madhAyxhanx hanenxraDu gaMTe. 
\emng
\eentry

\bentry
\word{noonday}
\pron{nUnfDeV}
\gl{\nA}
\bmng
naDuhagalu. 
\emng
\eentry

\bentry
\wordnospeech{no one}{no one}
\pron{?}
\gl{\nA}
\bmng
obabxnU ilalx; yAvanU ilalx. 
\emng
\eentry

\bentry
\word{noontide}
\pron{nUnfTeYDf}
\gl{\nA}
\bmng
= \hyperlink{noonday}{noonday}. 
\emng
\eentry

\bentry
\word{noontime}
\pron{nUnfTeYmf}
\gl{\nA}
\bmng
= \hyperlink{noonday}{noonday}. 
\emng
\eentry

\bentry
\word[noose(1)]{noose}
\pron{nUsf}
\gl{\nA}
\bmng
\bnum
\num{1} (\kanmu\ bale, jiVruguNikeya bAru, neVNu hAkuvavana hagagx, \mo vugaLalilx eLedare bigiyuvaMtha sarigaMTuLaLx)  kuNike; sariguNike; urulu. \imglink{\(^1\) noose 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.55cm {N_Pictures/noose.jpg}}} 
\num{2} (\hA) maduveya gaMTu; vivAha baMdhana. 
\num{3} bale yA pAsha; baMdhana. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{put one's head in a noose} tananx nAshakekx tAneV kAraNanAgu; tanage tAneV maqtuyxvAgu; tananx patanakekx tAneV kAraNanAgu. 
\emng
\eentry

\bentry
\word[noose(2)]{noose}
\pron{nUsf}
\gl{\sakirx}
\bmng
\bnum
\num{1} jiVruguNike hagagx hAki hiDi; balege keDavu. 
\num{2} (hagagxdalilx) jiVruguNike mADu. 
\num{3} (katutx \mo vugaLa sutatxlU hagagxvanunx) jiVruguNike mADi hAku; urulu hAku. 
\enum
\emng
\eentry

\bentry
\word{nopal}
\pron{noVpalf}
\gl{\nA}
\bmng
(kirumaMji huLuvanunx beLesalu neDuvanagaLalilx beLeyuva) amerikada pApAsukaLiLx jAti. 
\emng
\eentry

\bentry
\word{nopalry}
\pron{noVpalirx}
\gl{\nA}
\bmng
(amerikada) pApAsu kaLiLx kADu, `pAlxMTeVSanf', 
\emng
\eentry

\bentry
\word{nope}
\pron{noVpf}
\gl{\kirxvi}
\bmng
(\AmA) `ilalx'; `beVDa', 
\emng
\eentry

\bentry
\word{nor}
\pron{nA(na)rf}
\gl{\saMavayx}
\bmng
\bnum
\num{1} matUtx alalx; matetxVnU ilalx; matutx bAradu; eraDaralilx yAvudU alalx: \eng{not a man nor a child was to be seen} yAvaneV manuSayxnAgali maguvAgali kANisalilalx. \eng{all that is true, nor must we forget} adelAlx nija, matutx nAvu (adanenxlAlx) mareyabAradu. 
\num{2} kUDa alalx: \eng{`I cannot go', `Nor can I'} ``nAnu hoVgalAre", ``nAnu kUDa hoVgalAre''. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nor}....\eng{nor}..... (\kAparx\ yA \pArxparx)= \hyperlink{neither(1)}{neither}....\eng{nor}. 
\emng
\eentry

\bentry
\word[nor'(1)]{nor'}
\pron{nArf}
\gl{\nA}
\bmng
 = \hyperlink{north(1)}{north} (\kanmu\ saMyukatx padagaLalilx) utatxra (dikukx): \eng{nor'wester} = \hyperlink{northwester}{northwester}. 
\emng
\eentry

\bentry
\word[nor'(2)]{nor'}
\pron{nArf}
\gl{\gu}
\bmng
(\kanmu\ saMyukatx padagaLalilx) utatxrada: utatxra dikikxna. 
\emng
\eentry

\bentry
\word[nor'(3)]{nor'}
\pron{nArf}
\gl{\kirxvi}
\bmng
(\kanmu\ saMyukatx padagaLalilx) utatxradalilx: \eng{nor'ward} = \hyperlink{northward(1)}{northward}. 
\emng
\eentry

\bentry
\word{noradrenalin}
\pron{nAraDarxnalinf}
\gl{\nA}
\bmng
(\jiVra) nAraDarxnalinf; aDirxnalf garxMthiya keVMdarx BAgadiMda matutx anuveVdanA narAgarxgaLiMda osaruva, naraperxVSakavAgi kelasa mADuva hAmoRVnf. 
\emng
\eentry

\bentry
\word{noradrenaline}
\pron{nAraDarxnalinf}
\gl{\nA}
\bmng
= \hyperlink{noradrenalin}{noradrenalin}. 
\emng
\eentry

\bentry
\word[Nordic(1)]{Nordic}
\pron{nADiRkf}
\gl{\gu}
\bmng
\bnum
\num{1} nADiRkf kulada: (\kanmu\ sAkxyXMDineVviyAdalilx hAgU \kanu\ yUroVpinalilx iruva etatxravAda, nasu hoMbaNaNxda deVha kUdalugaLuLaLx, niDudaleya) oMdu jamAYxRnikf mAnavakulada. 
\num{2} sAkxyXMDineVviya yA phinfleMDfge seVrida. 
\num{3} ('sikxVyiMgf' eMba niVru hAgU himada meVle jAruva ATada \vi) rasetxyanunx biTuTx bayalinalilx hoVguva hAgU kupapxLisuva vAyxyAmavanunxLaLx. 
\enum
\emng
\eentry

\bentry
\word[Nordic(2)]{Nordic}
\pron{nADiRkf}
\gl{\nA}
\bmng
sAkxyXMDineVviya yA phinfleMDinavanu. 
\emng
\eentry

\bentry
\wordRemoveSpace{Norfolk-jacket}{Norfolk jacket}
\pron{nAphaRkf jAYxkiTf}
\gl{\nA}
\bmng
nAphaRkf jAkiVTu; soMTapaTiTx matutx eraDU kaDe edeyanunx mucucxva maDikegaLuLaLx aMgi. \imglink{Norfolk jacket:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.6cm {N_Pictures/norfolk jacket.jpg}}} 
\emng
\eentry

\bentry
\word{nork}
\pron{nAkfR}
\gl{\nA}
\bmng
(\AseTxrXV) (\ashi) (sAmAnayxvAgi \bava dalilx) mole; satxna. 
\emng
\eentry

\bentry
\word{norland}
\pron{nAlaRMDf}
\gl{\nA}
\bmng
(\birx) utatxrada parxdeVsha, BAga. 
\emng
\eentry

\bentry
\word{norm}
\pron{nAmfR}
\gl{\nA}
\bmng
\bnum
\num{1} rUDhiya maTaTx yA kulxpatx parxmANa yA mAdari. 
\num{2} kulxpatx parxmANadalilx mADabeVkAda utApxdane yA kelasa. 
\num{3} rUDhiya, saMparxdAyada naDavaLike itAyxdi. 
\enum
\emng
\eentry

\bentry
\word[normal(1)]{normal}
\pron{nAmaRlf}
\gl{\gu}
\bmng
\bnum
\num{1}(gotAtxda) parxmANakakxnusAravAda. 
\num{2} karxmabadadhxvAda. 
\num{3} rUDhiya. 
\num{4} sAmAnayx maTaTxda. 
\num{5} (\jAyx) laMbavAgiruva: \eng{90} Digirx koVnavanunx uMTumADuva. 
\num{6} sAmAnayx sithxtiya; manoVvikAravAgali, BAvoVderxVkavAgali ilalxda. 
\numi{7} (\ravi) 
\banum
\alnum{a} (anilagaLa tApa, otatxDagaLa \vi) parxmANaka: \eng{$\bg 0^\circ\eg\bg\rm C\eg$} matutx \eng{$760$} mimiV. otatxDa uLaLx. 
\alnum{b} (dArxvaNada \vi) nAmaRlf; oMdu liVTarinalilx oMdu gArxmf samAna tUka saMyukatx karagiruva. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[normal(2)]{normal}
\pron{nAmaRlf}
\gl{\nA}
\bmng
\bnum
\num{1} (\jAyx) laMba reVKe. 
\num{2} (\kanmu\ manuSayx deVhada) sahaja (sithxtiya) shAKa. 
\num{3} mAmUlu parisithxti; yathAsithxti; eMdina sithxti, maTaTx, itAyxdi. 
\enum
\emng
\eentry

\bentry
\word{normalcy}
\pron{nAmaRlisx}
\gl{\nA}
\bmng
(\kanmu\ oMdu deVshada sAmAnayx AthiRka, rAjakiVya hAgU sAmAjika parisithxtiya \vi) sAmAnayx maTaTx, parisithxti; rUDhiya sithxti; yathAsithxti; eMdina hAgeyeV iruva sithxti, riVti, maTaTx. 
\emng
\eentry

\bentry
\wordnospeech{normal distribution}{normal distribution}
\pron{?}
\gl{\nA}
\bmng
(\saMshA) sAmAnayx vitaraNe; aneVka yAdaqciCxka caragaLa vitaraNeyanunx samimxta gaMTeyAkArada nakeSxyAgi nirUpisuva gaNita Palana. 
\emng
\eentry

\bentry
\word{normalise}
\pron{nAmaRleYsfZ}
\gl{\kirx}
\bmng
= \hyperlink{normalize}{normalize}. 
\emng
\eentry

\bentry
\word{normality}
\pron{nAmAYxRliTi}
\gl{\nA}
\bmng
(\kanmu\ \ame) = \hyperlink{normalcy}{normalcy}. 
\emng
\eentry

\bentry
\word{normalization}
\pron{nAmaRleYseZVSanf}
\gl{\nA}
\bmng
yathAsithxtige taruvike; eMdina sAmAnayx -- maTaTxkekx, sithxtige taruvike. 
\emng
\eentry

\bentry
\word{normalize}
\pron{nAmaRleYsfZ}
\gl{\sakirx}
\bmng
\bnum
\num{1} yathAsithxtige taru; eMdina sAmAnayx -- maTaTxkekx taru, sithxtige parivatiRsu. 
\num{2} anuguNavAgi yA anurUpavAgi mADu. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
yathAsithxtiyalilxru; eMdina sithxtiyalilxru. 
\emng
\eentry

\bentry
\word{normally}
\pron{nAmaRli}
\gl{\kirxvi}
\bmng
sAmAnayxvAgi; sAmAnayx padadhxtiyaMte; niyamAnusAravAgi; karxmaparxkAravAgi; eMdinaMte. 
\emng
\eentry

\bentry
\wordnospeech{normal school}{normal school}
\pron{?}
\gl{\nA}
\bmng
(amerika, phArxnfsx, \mo\ deVshagaLalilx) shikaSxkara tarabeVti shAle yA kAleVju; shikaSxkarige tarabeVti koDuva shAle, kAleVju. 
\emng
\eentry

\bentry
\word[Norman(1)]{Norman}
\pron{nAmaRnf}
\gl{\nA}
\bmng
\bnum
\numi{1} nAmaRnf: 
\banum
\alnum{a} nAmaRMDi pArxMtadavanu. 
\alnum{b} \eng{$1066$}ralilx iMgelxMDanunx gedudx nAmaRMDiyalilx nelasida sAkxyXMDineVviyanf matutx phArxMkiSf buDakaTuTxgaLa misharx saMtatiyavanu. 
\eanum
\numie
\num{2} nAmaRnf rAja(vaMshiVya); oMdaneV viliyamfniMda siTxVphanf varegina yAvaneV iMgilxSf rAja. 
\num{3} nAmaRnfru ADutitxdadx pherxMcf BASe. 
\num{4} (vAsutxshilapxdalilx) nAmaRnf sheYli; nAmaRnara ALivxkeyalilx iMgelxMDinalilxdadx vAsutxsheYli. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{the Norman} \hyperref{kandict_c.pdf}{C}{conquest pagu}{conquest}. 
\emng
\eentry

\bentry
\word[Norman(2)]{Norman}
\pron{nAmaRnf}
\gl{\gu}
\bmng
\bnum
\num{1} nAmaRnara yA avarige saMbaMdhisida. 
\num{2} nAmaRnf sheYliya vAsutxshilapxda yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\wordRemoveSpace{Norman-English}{Norman English}
\pron{nAmaRnf iMgilxSf}
\gl{\nA}
\bmng
nAmaRnaru ADutitxdadx, nAmaRnara parxBAvadiMda mApaRTaTx, iMgilxSf BASe. 
\emng
\eentry

\bentry
\word{Normanesque}
\pron{nAmaRnesfkx}
\gl{\gu}
\bmng
(vAsutxshilapxdalilx) nAmaRnf (vAsutx)sheYliya; nAmaRnf (vAsutx) sheYliyanunx hoVluva. 
\emng
\eentry

\bentry
\wordRemoveSpace{Norman-French}{Norman French}
\pron{nAmaRnf pherXMcf}
\gl{\nA}
\bmng
nAmaRnaru ADutitxdadx yA \eng{1066}ra taruvAya iMgelxMDina nAyxyAlayagaLalilx baLasutitxdadx pherxMcf BASe. 
\emng
\eentry

\bentry
\word{Normanise}
\pron{nAmaRneYsf}
\gl{\kirx}
\bmng
= \hyperlink{Normanize}{Normanize}. 
\emng
\eentry

\bentry
\word{Normanism}
\pron{nAmaRnisfZmf}
\gl{\nA}
\bmng
\bnum
\num{1} nAmaRnf parxButavxda yA veYshiSaTx yxgaLa pArxbalayx. 
\num{2} nAmaRnfra riVtiniVtigaLanunx samathiRsuva yA anukaraNe mADuva parxvaqtitx. 
\enum
\emng
\eentry

\bentry
\word{Normanize}
\pron{nAmaRneYsfZ}
\gl{\sakirx}
\bmng
nAmaRnf riVtige parivatiRsu, aLavaDisu. 
\emng

\noindent
\gl{\akirx}
\bmng
nAmaRnf BASe yA naDavaLikeyanunx rUDhisiko, aLavaDisiko. 
\emng
\eentry

\bentry
\wordnospeech{Norman Style}{Norman Style}
\pron{?}
\gl{\nA}
\bmng
nAmaRnf vAsutxsheYli; duMDaneya kamAnugaLu hAgU BAri kaMbagaLiMda kUDirutitxdadx vAsutxshilapx sheYli. 
\emng
\eentry

\bentry
\word{normative}
\pron{nAmaRTivf}
\gl{\gu}
\bmng
parxmANaka; kulxpatx parxmANakekx saMbaMdhisida yA adanunx sAthxpisuva. 
\emng
\eentry

\bentry
\word{normatively}
\pron{nAmaRTivfli}
\gl{\kirxvi}
\bmng
parxmANakavAgi; kulxpatx parxmANadalilx yA adanunx sAthxpisuvaMte. 
\emng
\eentry

\bentry
\word{normativeness}
\pron{nAmaRTivfnisf}
\gl{\nA}
\bmng
parxmANakate; kulxpatx parxmANadalilxruvike yA adanunx sAthxpisuvike. 
\emng
\eentry

\bentry
\word{Norn}
\pron{nAnfR}
\gl{\nA}
\bmng
(sAkx YxMDineVviyAda purANadalilx) adaqSaTxdeVvategaLalolxbabxLu. 
\emng
\eentry

\bentry
\word{Norroy}
\pron{nArAyf}
\gl{\nA}
\bmng
(\birx) TerxMTfna utatxrada (hAgU utatxra aileRMDina) parxdeVshadalilx adhikAravAyxpitxyuLaLx mUraneya parxdhAna vaMdimAgadha. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{Norroy and Ulster} = \hyperlink{Norroy}{Norroy}. 
\emng
\eentry

\bentry
\word[Norse(1)]{Norse}
\pron{nAsfR}
\gl{\nA}
\bmng
\bnum
\num{1} nAveR deVshada BASe. 
\num{2} sAkx YxMDineVviyanf BASegaLa vagaR. 
\numi{3} \eng{(the Norse)}(\bava vAgi \parx) 
\banum
\alnum{a} nAveR deVshadavaru. 
\alnum{b} \eng{$8-10$}ne shatamAnagaLalilxdadx, vAyavayx yUroVpina BAgagaLalilx nelasidadx kaDalugaLaLxru, veYkiMgfgaLu. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{Old Norse} 
\banum
\alnum{a} sAkx yxMDineVviyada BASegaLa mUlavAda jamAYxRnikf BASe. 
\alnum{b} \eng{$14$}neya shatamAnadavaregina nAveR matutx adara vasAhatugaLa BASe. 
\eanum
\emng
\eentry

\bentry
\word[Norse(2)]{Norse}
\pron{nAsfR}
\gl{\gu}
\bmng
purAtana sAkx yxMDineVviyada, (\kanmu) nAveR deVshada. 
\emng
\eentry

\bentry
\word{Norseman}
\pron{nAsfRmanf}
\gl{\nA}
\expl{(\bava\ \eng{Norsemen}).}
\bmng
nAveRyava; sivxVDanf deVshadavanu. 
\emng
\eentry

\bentry
\word[north(1)]{north}
\pron{nAtfR}
\gl{\kirxvi}
\bmng
utatxrakekx; utatxradalilx. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{north and south}(udadxdalilx) dakiSxNoVtatxravAgi. 
\num{2} \eng{north by east} utatxrakUkx utatxreVshAnayxkUkx naDuvaNa. 
\num{3} \eng{north by west} utatxrakUkx utatxra vAyuvayxkUkx naDuvaNa. 
\num{4} \eng{the North West} vAyavayx parxdeVsha (\kanmu\ kenaDa). 
\num{5} \eng{to the north of}... diMda utatxrakekx. 
\enum
\emng
\eentry

\bentry
\word[north(2)]{north}
\pron{nAtfR}
\gl{\nA}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} baDaga; utatxra (dikukx). 
\alnum{b} dikUsxciyalilx idakekx sariyAgiruva biMdu. 
\alnum{c} I biMduviruva dikukx. 
\eanum
\numie
\numi{2} (\sA \eng{the North}) 
\banum
\alnum{a} parxpaMcada yA deVshada yA nagarada utatxra dikikxnalilxruva parxdeVsha, utatxraBAga. 
\alnum{b} AkfRTikf parxdeVsha; BUmiya utatxradhurxvada parxdeVsha, utatxra valaya. 
\alnum{c} keYgArikA rASaTxrXgaLu; keYgArikoVdayxmagaLalilx aBivaqdidhx sAdhisida rASaTxrXgaLu. 
\alnum{d} (\kanmu) iMgelxMDina utatxraBAga yA amerikasaMsAthxnagaLa utatxrarAjayxgaLu. 
\eanum
\numie
\num{3} \eng{(North)}(birxDfjx isipxVTATa) ``utatxra" eMdu hesarisiruva kaDe kuLitu ADuva ATagAra. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{North Briton} (\pArxparx) sAkxTelxMDf (parxdeVsha). 
\emng
\eentry

\bentry
\word[north(3)]{north}
\pron{nAtfR}
\gl{\gu}
\bmng
\bnum
\num{1} utatxrada -- kaDeya, samiVpada yA utatxrABimuKavAda. 
\num{2} utatxradiMda baruva; utatxrada: \eng{north wind} utatxrada mAruta. 
\enum
\emng
\eentry

\bentry
\word(North American[1]){North American}
\pron{nAtfR amerikanf}
\gl{\gu}
\bmng
utatxra amerikada. 
\emng
\eentry

\bentry
\word(North American[2]){North American}
\pron{nAtfR amerikanf}
\gl{\nA}
\bmng
utatxra amerikadavanu (\kanmu\ saMyukatx saMsAthxnagaLu matutx kenaDagaLa deVshiVya yA nivAsi). 
\emng
\eentry

\bentry
\word{Northants}
\pron{nAtAYxRMTfsx}
\gl{\saMkiSx}
\bmng
\eng{Northamptonshire}. 
\emng
\eentry

\bentry
\word{northbound}
\pron{nAtfRbwMDf}
\gl{\gu}
\bmng
utatxrABimuKavAgi parxyANa mADuva yA oyuyxva. 
\emng
\eentry

\bentry
\wordnospeech{north country}{north country}
\pron{?}
\gl{\nA}
\bmng
iMgelxMDina utatxra parxdeVsha. 
\emng
\eentry

\bentry
\wordnospeech{North countryman}{North countryman}
\pron{?}
\gl{\nA}
\bmng
iMgelxMDina utatxra BAgada nivAsi; autatxreVya; utatxradavanu. 
\emng
\eentry

\bentry
\word[north-east(1)]{north-east}
\pron{nAtfRIsfTx}
\gl{\nA}
\bmng
\bnum
\num{1} IshAnayx; utatxrakUkx pUvaRkUkx naDuve iruva dikukx. 
\num{2} IshAnayx dikakxnunx sUcisuva dikUsxci biMdu. 
\num{3} I biMduviruva dikukx. 
\enum
\emng
\eentry

\bentry
\word[north-east(2)]{north-east}
\pron{nAtfRIsfTx}
\gl{\gu}
\bmng
IshAnayxda yA IshAnayxda kaDeya yA IshAnayxdiMda baruva, 
\emng
\eentry

\bentry
\word[north-east(3)]{north-east}
\pron{nAtfRIsfTx}
\gl{\kirxvi}
\bmng
IshAnayxda kaDe yA IshAnayxdalilx yA IshAnayxda hatitxra. 
\emng
\eentry

\bentry
\word{North-East}
\pron{nAtfRIsfTx}
\gl{\nA}
\bmng
deVshada IshAnayxkikxruva BAga, parxdeVsha. 
\emng
\eentry

\bentry
\word{northeaster}
\pron{nAtiRVsaTxrf}
\gl{\nA}
\bmng
IshAnayx mAruta; IshAnayxdiMda baruva mAruta. 
\emng
\eentry

\bentry
\word[north-easterly(1)]{north-easterly}
\pron{nAtaRIsaTxrfli}
\gl{\gu}
\bmng
= \hyperlink{north-east(2)}{$^2$north-east}. 
\emng
\eentry

\bentry
\word[north-easterly(2)]{north-easterly}
\pron{nAtfRIsaTxrfli}
\gl{\kirxvi}
\bmng
= \hyperlink{north-east(3)}{$^3$north-east}. 
\emng
\eentry

\bentry
\word{north-eastern}
\pron{nAtfRIsaTxnfR}
\gl{\gu}
\bmng
IshAnayx dikikxgiruva. 
\emng
\eentry

\bentry
\wordnospeech{north-east passage}{north-east passage}
\pron{?}
\gl{\nA}
\bmng
IshAnayx mAgaR; yUroVpina hAgU ESAyxda utatxra karAvaLi mUlaka pUvaR deVshagaLige hAgU aTAlxMTikfniMda pesiphikf sAgara parxdeVshakekx haDagugaLa mUlaka parxyANa mADabahudeMdu hiMdina kAladalilx UhisalAgidadx dAri. 
\emng
\eentry

\bentry
\word{norther}
\pron{nAtaRrf}
\gl{\nA}
\bmng
(\ame) mAgi hAgU caLigAlagaLalilx (amerikada) TekAsxsf, phAlxriDa matutx mekisxko kolilx parxdeVshagaLa meVle utatxradiMda biVsuva, parxbala shiVtada gALi; utatxra mAruta. 
\emng
\eentry

\bentry
\word[northerly(1)]{northerly}
\pron{nAdaRliR}
\gl{\gu}
\bmng
\bnum
\num{1} utatxra BAgadalilx yA utatxra dikikxnalilx iruva. 
\num{2} (gALiya \vi) utatxradiMda biVsuva. 
\enum
\emng
\eentry

\bentry
\word[northerly(2)]{northerly}
\pron{nAdaRliR}
\gl{\kirxvi}
\bmng
utatxra BAgadalilx yA dikikxnalilx. 
\emng
\eentry

\bentry
\word[northerly(3)]{northerly}
\pron{nAdaRliR}
\gl{\nA}
\bmng
utatxra BAgadiMda yA dikikxniMda biVsuva gALi; utatxra mAruta. 
\emng
\eentry

\bentry
\word{northern}
\pron{nAdaRnfR}
\gl{\gu}
\bmng
\bnum
\num{1} utatxrada yA utatxradalilxruva. 
\num{2} autatxreVya; utatxra parxdeVshadalilx vAsisuva. 
\num{3} utatxrABimuKavAda yA utatxrakikxruva. 
\enum
\emng
\eentry

\bentry
\word{northerner}
\pron{nAdaRnarf}
\gl{\nA}
\bmng
autatxreVya; utatxra vAsi; utatxra BAgadalilx vAsisuvavanu. 
\emng
\eentry

\bentry
\wordnospeech{Northern hemisphere}{Northern hemisphere}
\pron{?}
\gl{\nA}
\bmng
utatxrAdhaRgoVLa. 
\emng
\eentry

\bentry
\wordnospeech{northern lights}{northern lights}
\pron{?}
\gl{\nA}
\bmng
= \hyperref{kandict_a.pdf}{A}{aurora borealis}{aurora borealis}. 
\emng
\eentry

\bentry
\word{northernmost}
\pron{nAdaRnfRmoVsfTx}
\gl{\gu}
\bmng
atayxMta utatxradalilxruva; atayxMta utatxrada dikikxnalilxruva. 
\emng
\eentry

\bentry
\wordnospeech{Northern States}{Northern States}
\pron{?}
\gl{\nA}
\bmng
saMyukatx amerika saMsAthxnada utatxra BAgada saMsAthxnagaLu, rAjayxgaLu. 
\emng
\eentry

\bentry
\word{northing}
\pron{nAdiRMgf}
\gl{\nA}
\bmng
(\nw) 
\bnum
\num{1} utatxra dikikxnalilx parxyANa mADida yA aLate mADida dUra. 
\num{2} utatxra dikukx. 
\enum
\emng
\eentry

\bentry
\word{Northland}
\pron{nAtfRlaM(lAYxM)Df}
\gl{\nA}
\bmng
(\kAparx) utatxra nADu(gaLu); deVshada utatxra BAga(gaLu); baDaganADu(gaLu). 
\emng
\eentry

\bentry
\wordnospeech{north light}{north light}
\pron{?}
\gl{\nA}
\bmng
utatxrada beLaku; (\kanmu\ citarxkAraru hAgU kAKARnegaLanunx kaTuTxvavaru bayasuva) utatxradiMda beLaku baruvaMtha vayxvasethx. 
\emng
\eentry

\bentry
\word{Northman}
\pron{nAtfRmanf}
\gl{\nA}
\expl{(\bava\ \eng{Northmen).}}
\bmng
(\kanmu\ nAveR deVshada) sAkxYxMDineVviyada nivAsi. 
\emng
\eentry

\bentry
\word[north-north-east(1)]{north-north-east}
\pron{nAtfR nAtfR IsfTx}
\gl{\nA}
\bmng
utatxreVshAnayx; utatxrakUkx IshAnayxkUkx madheyx iruva dikukx. 
\emng
\eentry

\bentry
\word[north-north-east(2)]{north-north-east}
\pron{nAtfR nAtfR IsfTx}
\gl{\gu}
\bmng
utatxreVshAnayxda. 
\emng
\eentry

\bentry
\word[north-north-east(3)]{north-north-east}
\pron{nAtfR nAtfR IsfTx}
\gl{\kirxvi}
\bmng
utatxreVshAnayxkekx; utatxreVshAnayxdalilx. 
\emng
\eentry

\bentry
\word[north-north-west(1)]{north-north-west}
\pron{nAtfR nAtfR vesfTx}
\gl{\nA}
\bmng
utatxra vAyavayx; utatxra dikikxgU vAyavayx dikikxgU naDuvaNa dikukx. 
\emng
\eentry

\bentry
\word[north-north-west(2)]{north-north-west}
\pron{nAtfR nAtfR vesfTx}
\gl{\gu}
\bmng
utatxra vAyavayxda. 
\emng
\eentry

\bentry
\word[north-north-west(3)]{north-north-west}
\pron{nAtfR nAtfR vesfTx}
\gl{\kirxvi}
\bmng
utatxra vAyavayxdalilx. 
\emng
\eentry

\bentry
\wordnospeech{North Pole}{North Pole}
\pron{?}
\gl{\nA}
\bmng
utatxra dhurxva. 
\emng
\eentry

\bentry
\wordnospeech{North Sea}{North Sea}
\pron{?}
\gl{\nA}
\bmng
(birxTanf, nedarfleMDfsx, jamaRni hAgU sAkx yxMDineVviyAgaLa naDuvaNa) utatxra samudarx. 
\emng
\eentry

\bentry
\wordnospeech{North Star}{North Star}
\pron{?}
\gl{\nA}
\bmng
dhurxva nakaSxtarx. 
\emng
\eentry

\bentry
\wordnospeech{Northumb.}{Northumb.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{Northumberland}. 
\emng
\eentry

\bentry
\word[Northumbrian(1)]{Northumbrian}
\pron{nAtaRMbirxanf}
\gl{\gu}
\bmng
(iMgelxMDina haMbarf nadiya utatxra BAgada) pArxciVna nAtaRMbiyada yA Adhunika nAtaRMbarfleMDina. 
\emng
\eentry

\bentry
\word[Northumbrian(2)]{Northumbrian}
\pron{nAtaRMbirxanf}
\gl{\nA}
\bmng
\bnum
\num{1} purAtana nAtaRMbirxya parxdeVshadavanu. 
\num{2} purAtana nAtaRMbirxyada pArxMtaBASe yA deVsi nuDi. 
\enum
\emng
\eentry

\bentry
\word[northward(1)]{northward}
\pron{nAtfRvaDfR}
\gl{\gu}
\bmng
autatxreVya; utatxraBAgada; utatxrABimuKavAgiruva; utatxrada kaDeya. 
\emng
\eentry

\bentry
\word[northward(2)]{northward}
\pron{nAtfRvaDfR}
\gl{\nA}
\bmng
utatxra dikukx yA utatxra dikikxna parxdeVsha. 
\emng
\eentry

\bentry
\word[northward(3)]{northward}
\pron{nAtfRvaDfR}
\gl{\kirxvi}
\bmng
utatxra BAgadalilx; utatxrakekx. 
\emng
\eentry

\bentry
\word{northwards}
\pron{nAtfRvaDf\char144R}
\gl{\kirxvi}
\bmng
= \hyperlink{northward(3)}{$^3$northward}. 
\emng
\eentry

\bentry
\word[north-west(1)]{north-west}
\pron{nAtfRvesfTx}
\gl{\nA}
\bmng
\bnum
\num{1} vAyuvayx. 
\num{2} vAyavayx dikakxnunx sUcisuva dikUsxciya biMdu. 
\enum
\emng
\eentry

\bentry
\word[north-west(2)]{north-west}
\pron{nAtfRvesfTx}
\gl{\gu}
\bmng
vAyavayxda, vAyavayxda kaDeyiMda baruva. 
\emng
\eentry

\bentry
\word[north-west(3)]{north-west}
\pron{nAtfRvesfTx}
\gl{\kirxvi}
\bmng
= \hyperlink{north-westerly}{north-westerly}. 
\emng
\eentry

\bentry
\word{North-West}
\pron{nAtfRvesfTx}
\gl{\nA}
\bmng
(deVshada yA nagarada) vAyavayx parxdeVsha. 
\emng
\eentry

\bentry
\word{northwester}
\pron{nAtfRvesaTxrf}
\gl{\nA}
\bmng
vAyavayx mAruta. 
\emng
\eentry

\bentry
\word{north-westerly}
\pron{nAtfRvesaTxrfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} vAyavayxdiMda: \eng{winds blowing north-westerly} vAyavayxdiMda biVsuva mArutagaLu. 
\num{2} vAyavayxda kaDege: \eng{ journeyed north-westerly all day} iDiV dina vAyavayxda kaDe parxyANa mADida. 
\enum
\emng
\eentry

\bentry
\word{north-western}
\pron{nAtfRvesaTxnfR}
\gl{\gu}
\bmng
vAyavayxda kaDe iruva. 
\emng
\eentry

\bentry
\wordnospeech{north-west passage}{north-west passage}
\pron{?}
\gl{\nA}
\bmng
vAyavayxmAgaR; amerikada utatxra karAvaLiya mUlaka aTAlxMTikf samudarxdiMda pesiphikf samudarxkekx haDagugaLu hoVgabahudeMdu hiMde BAvisalAgidadx mAgaR. 
\emng
\eentry

\bentry
\word{north-wind}
\pron{nAtfRviMDf}
\gl{\nA}
\bmng
utatxra mAruta; utatxradiMda biVsuva gALi. 
\emng
\eentry

\bentry
\wordRemoveSpace{Norway-lobster}{Norway lobster}
\pron{nAveRV lAbfsaTxrf}
\gl{\nA}
\bmng
yUroVpina cikakx samudarxda naLiLx yA EDi. 
\emng
\eentry

\bentry
\wordnospeech{Norway rat}{Norway rat}
\pron{?}
\gl{\nA}
\bmng
kaMdili; sAmAnayx jAtiya kaMdu(baNaNxda) ili. 
\emng
\eentry

\bentry
\word[Norwegian(1)]{Norwegian}
\pron{nAviRVjanf}
\gl{\gu}
\bmng
nAveR deVshada. 
\emng
\eentry

\bentry
\word[Norwegian(2)]{Norwegian}
\pron{nAviRVjanf}
\gl{\nA}
\bmng
\bnum
\num{1} nAveR deVshadalilx huTiTxdavanu; nAveR nivAsi. 
\num{2} nAveR(deVshada) BASe. 
\enum
\emng
\eentry

\bentry
\word{nor'-wester}
\pron{nAveRsaTxrf}
\gl{\nA}
\bmng
\bnum
\num{1} vAyavayx mAruta. 
\num{2} oMdu gAlxsu tiVkaSx NxvAda madayx. 
\num{3} meVNagapaTada ToVpi, hAyxTu; agalavAda aMciruva, niVriLiyadaMtha baTeTxyiMda mADida ToVpi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{Nos.}{Nos.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{numbers}. 
\emng
\eentry

\bentry
\word[nose(1)]{nose}
\pron{noVsf}
\gl{\nA}
\bmng
\bnum
\num{1} mUgu; nAsika; GArxNeVMdirxya. 
\num{2} GArxNashakitx; vAsane hiDiyuva shakitx. 
\num{3} (oNahululx, TiV, madayx, hogesopupx, \mo vugaLa) vAsane; kaMpu; gaMdha. 
\num{4} (nALa, koLave, tidi, bakapAterx, \mo vugaLa) mUti; tereda tudi. 
\num{5} haDagina, doVNiya -- mUti, cAcuBAga. 
\num{6} = \hyperlink{nosing}{nosing}. 
\num{7} (moVTArf kAru, vimAna, \mo vugaLa) muMBAga; mUti. 
\num{8} (\ashi) rahasayx varadigAra; aparAdhigaLanunx patetx mADi, poliVsarige avara bagegx mAhiti koDuvavanu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{as plain as the nose on your face} sapxSaTxvAgide; sapxSaTxvAgi kANisuvaMtide. 
\num{2} \eng{by a nose} atayxlapx aMtaradiMda; savxlapxdaralilx; \eng{won the race by a nose} atayxlapx aMtaradiMda paMdayxvanunx gedadx. 
\numi{3} \eng{count noses} 
\banum
\alnum{a} hAjariruvavaranunx, beMbala koDuvavaranunx, anumoVdisuvavaranunx lekakx mADu; mUgeNisu. 
\alnum{b} (oMdu viSayavanunx) keVvala saMKAyxbaladiMda nidhaRrisu. 
\eanum
\numie
\num{4} \eng{ get up a (person's) nose} (\ashi) (obabxnanunx) kADu; piVDisu; reVgisu. 
\numi{5} \eng{has a good nose} curuku mUgide: 
\banum
\alnum{a} (nAyi \mo vugaLa \vi) vAsane hiDiyuva oLeLx shakitx ide. 
\alnum{b} (\rUpa) (gupatx poliVsaru \mo vara \vi) (aparAdha \mo vugaLa) suLivu hiDiyuva, patetx hacucxva -- kwshala, oLeLxya shakitx ide. 
\eanum
\numie
\num{6} \eng{hold one's nose} dunARta tapipxsikoLaLxlu mUgu mucicxko, hoLeLxgaLanunx beraLugaLiMda otitxhiDiduko. 
\num{7} \eng{keep one's nose clean} (\ashi) sariyAgi naDeduko; toMdarege sikikxbiVLada hAgiru. 
\num{8} \eng{keep one's nose to the grindstone} kaSaTxpaTuTx, vishArxMtiyilalxde, niraMtaravAgi duDi, kelasa mADu. 
\num{9} \hyperref{kandict_l.pdf}{L}{look(1) nuga(8)}{$^1$look} \eng{down one's nose at}. 
\num{10} \eng{nose of wax} meVNadaMthavanu yA meVNadaMtha vasutx; sulaBavAgi tidadxbahudAda -- manuSayx, vasutx. 
\numi{11} \eng{on the nose} 
\banum
\alnum{a} (\ame) (\ashi) sariyAgi; karAruvAkAkxgi. 
\alnum{b} (\AseTxrXV) (\ashi) reVgisuvaMtha; piVDisuvaMtha. 
\hypertarget{nose pagu12}{} 
\eanum
\numie
\num{12} \eng{parson's nose} beVyisida (hakikx \mo vugaLa) paqSaThxda BAga. 
\hyperdef{N}{nose(1) pagu(13)}{} 
\num{13} \eng{pay through the nose} salalxbeVkAdudakikxMta vipariVta hecAcxgi koDabeVkAgu; dubAri bele kakakxbeVkAgu; hecucx bele terabeVkAgu. 
\num{14} \eng{pope's nose} = \hyperlink{nose pagu12}{?pagu?\((12)\)}. 
\hypertarget{nose1 pagu15}{} 
\num{15} \eng{to bite one's nose off} reVgi, muKa muriyavaMte mAtanADu. 
\num{16} \eng{to cut off one's nose to spite one's face} muKada meVlina munisige mUgu koyudxko; tananx munisiniMda tananx kelasavanenxV keDisiko; 
\numi{17} \eng{to follow one's nose} 
\banum
\alnum{a} edurige neVravAda dAri hiDidu naDe; mUgina neVrakekx hoVgu. 
\alnum{b} (tananx) sahaja parxvaqtitxge takakxMte naDe. 
\eanum
\numie
\num{18} \eng{to} \hyperref{kandict_l.pdf}{L}{lead(1) nuga(3)}{$^1$lead} \eng{by the nose}. 
\hypertarget{nose1pagu19}{} 
\num{19} \eng{ to make a long nose} (obabxna kuritu hiVyALisalu) hebebxTaTxnunx mUgina meVliTuTx keYyagalisu; tucaCxvAgi kANu. 
\hypertarget{nose1 pagu20}{} 
\num{20} \eng{ to poke one's nose}(tanage saMbaMdhavilalxdadxralilx) mUgutUrisu; madheyx tale hAku. 
\hyperdef{N}{nose(1) pagu(21)}{} 
\numi{21} \eng{to put one's nose out of joint} 
\banum
\alnum{a} obabxna udedxVsha IDeVradaMte mADu; udedxVsha BaMga mADu; kAyaRBaMga mADu. 
\alnum{b} (obabxnige) mujugara uMTumADu. kirikiri uMTumADu. 
\alnum{c} (obabxna) citatxsAvxsathxyX keDisu; manasisxge galibili uMTu mADu. 
\alnum{d} obabxna anugarxha saMpAdisalu Atana anugarxhakekx pAtarxnAda matotxbabxnanunx tolagisibiDu. 
\hyperdef{N}{nose(1) pagu(22)}{} 
\eanum
\numie
\num{22} \eng{to rub person's nose in it} (obabxnige) avamAnavAguva riVtiyalilx -- avana tapapxnunx etitxtoVrisu, matetx nenapu mADu. 
\num{23} \eng{to see no further than one's nose} (tananx kAyaRda pariNAma \mo vugaLa bagegx) dUradaqSiTxyilalxdiru. 
\num{24} \eng{to snap one's nose off} = \hyperlink{nose1 pagu15}{?pagu?\((15)\)}. 
\num{25} \eng{to speak through one's nose} mUginalilx mAtanADu. 
\num{26} \eng{to thrust one's nose} = \hyperlink{nose1 pagu20}{?pagu?\((20)\)}. 
\num{27} \eng{to thumb one's nose (at)} = \hyperlink{nose1pagu19}{?pagu?\((19)\)}. 
\hyperdef{N}{nose(1) pagu(28)}{} 
\num{28} \eng{to turn up one's nose (at)} tirasAkxra toVrisu; mUgu muri; dhikakxrisu. 
\hyperdef{N}{nose(1) pagu(29)}{} 
\num{29} \eng{under person's nose} (parxtiBaTane yA kaNiNxge biVLada vataRnegaLa \vi) (obabxna) kaNeNxdurinalelxV; edurigeV; samakaSxmaveV; mUgina keLageV. 
\num{30} \eng{with one's nose in the air} ahaMkAradiMda; sokikxniMda. 
\enum
\emng
\eentry

\bentry
\word[nose(2)]{nose}
\pron{noVsfZ}
\gl{\sakirx}
\bmng
\bnum
\num{1} mUsu; (yAvudAdarU vasutx \mo vugaLa) vAsane noVDu; AGArxNisu. 
\num{2} vAsane mUlaka patetx mADu. 
\num{3} (\rUpa) patetx hacucx; kaMDuhiDi. 
\num{4} mUginiMda ujujx. 
\num{5} mUganunx otutx yA oLakekx tUrisu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} mUgu hAku; bedaku; talehAku: \eng{to nose about in someone else's business} inonxbabxra viSayadalilx mUgu tUrisu, parxveVshisu. 
\num{2} (dAri mADikoMDu) oLakekx nugugx; parxveVshisu; jAgarUkateyiMda muMdakekx hoVgu (\sakirx\ saha) \eng{to nose into the wind} gALiyoLakekx nugugx. 
\num{3} huDuku. 
\num{4} mUsu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nose ape}{nose ape}
\pron{?}
\gl{\nA}
\bmng
(\pArxvi) mUgu koVti; bAyigiMta muMdakekx cAcida mUgiruva oMdu bageya koVti. 
\emng
\eentry

\bentry
\word{nosebag}
\pron{noVsfZbAYxgf}
\gl{\nA}
\bmng
toVbari; dhAnayx tuMbi kudureya bAyige kaTuTxva dArada ciVla. \imglink{nosebag:fig}{\raisebox{-0.15cm}[0pt][0pt] {\pdfimage width 0.6cm height 0.6cm {N_Pictures/nosebag.jpg}}} 
\emng
\eentry

\bentry
\word{noseband}
\pron{noVsfZbAyxMDf}
\gl{\nA}
\bmng
lagAmina mUgupaTiTx. 
\emng
\eentry

\bentry
\word{nosebleed}
\pron{noVsfZbilxVDf}
\gl{\nA}
\bmng
mUginiMda rakatxsArxva(vAguvudu). 
\emng
\eentry

\bentry
\word{nose-cone}
\pron{noVsfZkoVnf}
\gl{\nA}
\bmng
shaMkutudi; (rAkeTf \mo vugaLa) shaMkuvinAkaqtiya tudi. 
\emng
\eentry

\bentry
\word{nosed}
\pron{noVsfDx}
\gl{\gu}
\bmng
\bnum
\num{1} (vishiSaTx AkArada) mUgiruva. 
\num{2} oLeLxya vAsaneyuLaLx; suvAsita; tuMbA parimaLavuLaLx. 
\num{3} (sapxSaTxvAgi edudx kANuvaMtha) mUguLaLx; doDaDx mUgina: \eng{nose monkey} doDaDx mUgina koVti. 
\enum
\emng
\eentry

\bentry
\word[nosedive(1)]{nosedive}
\pron{noVsfZDeYvf}
\gl{\nA}
\bmng
\bnum
\num{1} mUgu muMdAgi (BUmige) Bararxne eraguva vimAnada muLuku. 
\num{2} diDhiVrane biVLuvudu, iLiyuvudu. 
\enum
\emng
\eentry

\bentry
\word[nosedive(2)]{nosedive}
\pron{noVsfZDeYvf}
\gl{\akirx}
\bmng
(vimAna) mUgu muMdAgi BUmige Bararxne eragu. 
\emng
\eentry

\bentry
\word{nose-flute}
\pron{noVsfZphUlxTf}
\gl{\nA}
\bmng
(phiji \mo\ deVshagaLalilx baLakeyalilxruva) mUgu koLalu; mUginiMda Uduva oMdu vAdayx. 
\emng
\eentry

\bentry
\word{nosegay}
\pron{noVsfZgeV}
\gl{\nA}
\bmng
(\sA\ suvAsaneya) hUgoMcalu; puSapxgucaCx. 
\emng
\eentry

\bentry
\wordnospeech{nose leaf}{nose leaf}
\pron{?}
\gl{\nA}
\bmng
mUgu hALe; parxtidhavxniya sAthxna kaMDu hiDiyalu kelavu bageya bAvaligaLa mUgina hoLeLxgaLa meVliruva mAMsada BAga. 
\emng
\eentry

\bentry
\word{noseless}
\pron{noVsfZlisf}
\gl{\gu}
\bmng
mUgilalxda; nAsArahita; mUgu katatxrisida. 
\emng
\eentry

\bentry
\word{nose-monkey}
\pron{noVsfZmaMki}
\gl{\nA}
\bmng
= \hyperlink{nose ape}{nose ape}. 
\emng
\eentry

\bentry
\word{nose-piece}
\pron{noVsfZpiVsf}
\gl{\nA}
\bmng
\bnum
\num{1} = \hyperlink{noseband}{noseband}. 
\num{2} (shirasAtxrXNa \mo vugaLalilx) mUgukApu; nAsArakeSx; mUganunx rakiSxsuva BAga. 
\num{3} (sUkaSx mxdashaRkada \vi) mUti; vasutxkavanunx \eng{(object-glass)} joVDisiruva BAga. 
\enum
\emng
\eentry

\bentry
\word{nosepipe}
\pron{noVsfZpeYpf}
\gl{\nA}
\bmng
mUtinaLike; yAvudeV upakaraNadalilx mUtiyAgi baLasuva naLike. 
\emng
\eentry

\bentry
\word{noser}
\pron{noVsaZrf}
\gl{\nA}
\bmng
(muKakekx baDiyuva) balavAda edurugALi. 
\emng
\eentry

\bentry
\word{nose-rag}
\pron{noVsfZrAYxgf}
\gl{\nA}
\bmng
(\ashi) kise karavasatxrX. 
\emng
\eentry

\bentry
\word{nosering}
\pron{noVsfZriMgf}
\gl{\nA}
\bmng
\bnum
\num{1} (hagagxkaTiTx eLedukoMDu hoVgalu gULi \mo vugaLa mUgige toDisuva) mUgubaLe; mUguMgura. 
\num{2} (alaMkArakAkxgi hAkikoLuLxva) mUguMgura; mukura. 
\enum
\emng
\eentry

\bentry
\word{nose-to-tail}
\pron{noVsfZ Tu TeVlf}
\gl{\gu}
\bmng
(vAhanagaLa \vi) (\kanmu\ atiyAda vAhanagaLa saMcAradalilx) oMdara hiMde oMdu sAlAgi calisutitxruva yA niMtiruva. 
\emng
\eentry

\bentry
\word{nose-wheel}
\pron{noVsfZviVlf}
\gl{\nA}
\bmng
(vimAnada) mUticakarx; muMBAgada mUtiya keLagiruva, nelamuTaTxlu baLasuva cakarx. 
\emng
\eentry

\bentry
\word{nosey}
\pron{noVsiZ}
\gl{\gu\ matutx \nA}
\bmng
 \hyperlink{nosy(1)}{nosy} eMba padada rUpAMtara. 
\emng
\eentry

\bentry
\word[nosh(1)]{nosh}
\pron{nASf}
\gl{\sakirx}
\bmng
(\akirx\ saha) (\kanmu\ UTavAda naMtara, UTakekx muMce) EnanAnxdarU kuruku, tinunx yA kuDi. 
\emng
\eentry

\bentry
\word[nosh(2)]{nosh}
\pron{nASf}
\gl{\nA}
\bmng
\bnum
\num{1} AhAra yA pAniVya. 
\num{2} (\ame) upAhAra. 
\enum
\emng
\eentry

\bentry
\word{noshery}
\pron{nASari}
\gl{\nA}
\expl{(\bava\ \eng{nosheries}).}
\bmng
(\ashi) hoVTelu; resoTxVreMTu; upAhAra gaqha. 
\emng
\eentry

\bentry
\word{nosh-up}
\pron{nASf apf}
\gl{\nA}
\bmng
(\birx) BAri UTa. 
\emng
\eentry

\bentry
\word{nosily}
\pron{noVsiZli}
\gl{\kirxvi}
\bmng
adhikaparxsaMgiyaMte; adhikaparxsaMgitanadiMda; tAnAgi oLa viSayagaLanunx kedaki tiLiyaletinxsutAtx. 
\emng
\eentry

\bentry
\word{nosiness}
\pron{noVsiZnisf}
\gl{\nA}
\bmng
adhikaparxsaMgitana; saMbaMdhavilalxda viSayadalilx tale hAkuvudu. 
\emng
\eentry

\bentry
\word{nosing}
\pron{noVsiZMgf}
\gl{\nA}
\bmng
(meTaTxlu, erakada acucx, \mo vugaLa) duMDumADida ENu yA idakekx toDisuva loVhada kApu. 
\emng
\eentry

\bentry
\word{nosography}
\pron{nasAgarxphi}
\gl{\nA}
\bmng
roVgavagiRVkaraNa; roVgagaLanunx vayxvasithxtavAgi vagiRVkarisuvudu. 
\emng
\eentry

\bentry
\word{nosological}
\pron{nAsalAjikalf}
\gl{\gu}
\bmng
roVga vagiRVkaraNa shAsatxrXda yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nosology}
\pron{nasAlaji}
\gl{\nA}
\bmng
roVgavagiRVkaraNa shAsatxrX; roVgagaLanunx vagiRVkarisuvudakekx saMbaMdhisida veYdayxshAsatxrXda shAKe. 
\emng
\eentry

\bentry
\word{nostalgia}
\pron{nAsATxYxlijxa, nAsATxYxlajx}
\gl{\nA}
\bmng
\bnum
\num{1} gaqhaviraha; maneya haMbala; manegiVLu; mane yA savxdeVsha biTuTx horagiruvudariMdAguva manoVvayxtheya roVga. 
\num{2} haLeya kAlada viSaNaNx nenapu yA haMbalada samxraNe. 
\num{3} (haLeya kAlada yAvudoV avadhiya kurita) BAvuka haMbala; utakxMThate. 
\enum
\emng
\eentry

\bentry
\word{nostalgic}
\pron{nAsATxYxlijxkf}
\gl{\gu}
\bmng
\bnum
\num{1} gaqhavirahada; manegiVLina. 
\num{2} (haLeya kAlada) viSaNaNx nenapina; haMbalada nenapina. 
\num{3} BAvukavAda haMbaladiMda tuMbida; utakxMThateya. 
\enum
\emng
\eentry

\bentry
\word{nostalgically}
\pron{nAsATxYxlijxkali}
\gl{\kirxvi}
\bmng
\bnum
\num{1} maneya haMbaladiMda kUDi. 
\num{2} (haLeya kAlada) viSaNaNx nenapiniMda tuMbi; nenapina guMginalilx. 
\num{3} BAvukavAgi; utakxMThateyiMda. 
\enum
\emng
\eentry

\bentry
\word{nostoc}
\pron{nAsATxkf}
\gl{\nA}
\bmng
nAsATxkf; loVLeyaMtiruva niVli hasiru baNaNxda EkakoVshada AYxlagx(pAci) kula. 
\emng
\eentry

\bentry
\word{Nostradamus}
\pron{nAsaTxrXDAmasf}
\gl{\nA}
\bmng
BaviSayxvAdi; kaNi heVLuvavanu; BaviSayxda GaTanegaLanunx heVLuvudAgi heVLikoLuLxvavanu. 
\emng
\eentry

\bentry
\word{nostril}
\pron{nAsiTxrXlf}
\gl{\nA}
\bmng
hoLeLx; mUgina soLeLx; nAsA puTa. 
\emng

\noindent
\gl{\pagu}
\bmng
\hyperdef{N}{nostril pagu}{} 
\eng{stink in the nostrils of} bahaLa asahayxvAgiru; heVsigeyanunxMTumADu: \eng{our judgements stink in the nostrils of the people} namamx tiVpuRgaLu janarige asahayxvanunxMTumADutatxve. 
\emng
\eentry

\bentry
\word{nostrilled}
\pron{nAsiTxrXlfDx}
\gl{\gu}
\bmng
(mUgina) hoLeLxgaLiruva. 
\emng
\eentry

\bentry
\word{nostrum}
\pron{nAsaTxrXM}
\gl{\nA}
\bmng
\bnum
\num{1} savaRroVgahara; rAmabANa; AvaTada madudx; bUTATikeya auSadha; tayArisidavaneV savoRVtatxmaveMdu hogaLikoLuLxva, EkasAvxmayx paDeda auSadha. 
\num{2} (\rUpa) (rAjakiVya keSxVtarx, sAmAjika keSxVtarx, \mo vugaLalilx) acucxmecicxna yoVjane; sudhAraNe taralu savoRVtatxmaveMdu BAvitavAgiruva. hiDisiruva yoVjane. 
\enum
\emng
\eentry

\bentry
\word[nosy(1)]{nosy}
\pron{noVsiZ}
\gl{\gu}
\expl{(\tara\ \eng{nosier}, \tama\ \eng{nosiest}).}
\bmng
\bnum
\num{1} udadxmUgina; niDumUgina. 
\num{2} vishiSaTx vAsaneya; vishiSaTxvAda (keTaTx yA oLeLxya) vAsaneyuLaLx dunARtada yA suvAsaneya parimaLavuLaLx. 
\num{3} (\ashi) (niMdAthaRka) mUgu tUrisuva; mUgu hAkuva; adhika parxsaMgiyAda; adhikaparxsaMgitanadiMda tale hAkuva; tAnAgi oLa viSayagaLanunx kedaki tiLiyaletinxsuva. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{Nosy Parker}(\birx, \AmA) adhika parxsaMgi. 
\emng
\eentry

\bentry
\word[nosy(2)]{nosy}
\pron{noVsiz}
\gl{\nA}
\expl{(\bava\ \eng{nosies})}
\bmng
udadx mUginava(Lu). 
\emng
\eentry

\bentry
\word{not}
\pron{nATf}
\gl{\kirxvi}
\bmng
\bnum
\num{1} (\pArxparx) alalx; ilalx: \eng{I know not} nanage gotitxlalx. \eng{I doubt not} nanage saMshayavilalx. 
\numi{2} ilalx: 
\banum
\alnum{a} (sahAyaka \kirx da jate \eng{n't} seVrisidAga) ilalx: \eng{I can't say} nAnu heVLalAre. \eng{he'll not come} avanu baruvudilalx. (\ashi) \eng{he ain't here} avanililxlalx. 
\alnum{b} (vidhuyxkatx heVLikeyalilx) \eng{did you not tell me} niVnu nanage heVLalilalxveV? 
\eanum
\numie
\num{3} kaqdaMtagaLanunx dhAtavxthaRvAcigaLa hiMde niSeVdhAthaR sUcakavAgi baLasuvAga: \eng{not knowing} tiLiyade. \eng{asked him not to come} barabeVDaveMdu avanige heVLide. 
\num{4} (niSeVdhAthaRka vAkayx, kirxyApada yA padagucaCxgaLige adhAyxhAravAgi baruvAga): \eng{``Are you ill?" ``Not at all"} ninage kAhileyAgideyeV? KaMDita ilalx. \eng{popular or not, it is right} janapirxyavAgirali biDali, adu sariyAda mAtu hwdu. 
\num{5} (payARyavAgi kelavu mAtugaLanunx seVrisutAtx, avugaLalilx kelavanunx niSeVdhisutAtx mAtanADuvAga) `adu adalalx -- Adare adu beVroMdu' enunxva hAge: \eng{he is not my son, but yours} avanu nananx maganalalx, ninanx maga. 
\num{6} (oMdu niSeVdhAthaRsUcaka vAkayxda naMtara baruva savaRnAmavanunx otitx heVLuvAga) \eng{they will not be fooled, not they} avaru moVsa hoVgalAraru -- avare? eMdigU ilalx. 
\num{7} (\eng{`a'} eMba anideRVshaka guNavAciya hiMde baMdAga, `oMdu saha ilalx' eMbathaRdalilx) \eng{not a hair of your head shall be touched} ninanx taleya oMdu kUdalanunx saha muTuTxvudilalx. 
\num{8} (inonxMdu niSeVdha rUpada shabadxda jate, swmAyxthaR sUcaka saMdaBaRdalilx baMdAga) \eng{not a few} alapxsavxlapxvalalx (eMdare `bahaLa' eMbathaRdalilx). \eng{not seldom} aparUpavAgalalx; omomxmemxyalalx (eMdare `aneVka veVLe' eMbathaRdalilx). 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{as likely as not} eSuTx saMBavavoV aSeTxV asaMBavavU hwdu. 
\num{2} \eng{not at all} (yArAdarU kaqtajacnxte sUcisidAga shiSATxcArakAkxgi utatxra koDutAtx) aMthadeVnU ilalx; hAge niVvu heVLuva agatayxvilalx. 
\num{3} \eng{not a thing} yAvudU ilalx oMdU ilalx. 
\num{4} \eng{not but that} = \hyperlink{notpagu16}{?pagu?\((16)\)}. 
\num{5} \eng{not but}(\pArxparx) = \hyperlink{notpagu16}{?pagu?\((16)\)}. 
\num{6} \eng{no} \hyperref{kandict_h.pdf}{H}{half(3) nuga}{$^3$half}. 
\num{7} \eng{not least} kaDame pArxmuKayxvalalx (eMdare sAkaSuTx parxmuKavAda). 
\num{8} \eng{not} \hyperref{kandict_m.pdf}{M}{much(3) pagu(6)}{$^3$much}. 
\num{9} \eng{not once nor twice} = \hyperlink{notpagu10}{?pagu?\((10)\)}. 
\hypertarget{notpagu10}{} 
\num{10} \eng{not once or twice} oMderaDu salavalalx; aneVka bAri. 
\numi{11} \eng{not quite}: 
\banum
\alnum{a} hecucx kaDame. 
\alnum{b} aSeTxVnU alalx; bahuteVka: \eng{it is not quite proper} adu aSeTxVnU ucitavalalx. 
\hypertarget{notpagu12}{} 
\eanum
\numie
\num{12} \eng{not so well} aSeTxVnU oLeLxyadalalxda (riVtiyalilx). 
\hyperdef{N}{not pagu(13)}{} 
\num{13} \eng{not that} hAgeMdu athaRvalalx, Adare: \eng{if he said so-not that he ever did-he lied} avanu hAge heVLidadxre -- hAge eMdigU heVLiralAra -- adu suLuLx. 
\num{14} \eng{not too well} = \hyperlink{notpagu12}{?pagu?\((12)\)}. 
\num{15} \eng{not} \hyperref{kandict_v.pdf}{V}{very(2) pagu(1)}{$^2$very}. 
\hypertarget{notpagu16}{} 
\numi{16} \eng{not but what}(\pArxparx) 
\banum
\alnum{a} adeVneV irali; AdarU; hAgidadxrU: \eng{ not what I can do it, not but that a stronger man might} nAnu adanunx mADalAre; adeVneV irali nanagiMta shakatxnAdavanu adanunx mADirutitxdadx. 
\alnum{b} aMthadadxlalx; aMthavanalalx: \eng{not such a fool but what he can see it}adanunx kANadiruvaSuTx avanu daDaDxnalalx. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\wordf{nota bene}
\pron{noVTa beneV}
\gl{\sakirx}
\expl{\Latin}
\bmng
(vidhayxthaR) (\sA\ hiMde baMdudara visheVSa guNavanunx otitx heVLuvAga) cenAnxgi gamanisu, gamanaviDu; muMde baruvudanunx gamanisu. 
\emng
\eentry

\bentry
\word{notability}
\pron{noVTabiliTi}
\gl{\nA}
\bmng
\bnum
\num{1} gamanAhaRte; pArxdhAnayx; gamanAhaRvAgiruvike: \eng{names of no historical notability} aitihAsikavAgi gamanAhaRte ilalxda hesarugaLu. 
\num{2} parxKAyxta vayxkitx; gaNayx, parxsidadhx -- puruSa; KAyxtanAma. 
\enum
\emng
\eentry

\bentry
\word[notable(1)]{notable}
\pron{noVTaba(bf)lf}
\gl{\gu}
\bmng
\bnum
\num{1} gaNayx; gamanAhaRvAda. 
\num{2} gamana seLeyuva; edudx kANisuvaMtha. 
\num{3} asAdhAraNavAda; asAmAnayxvAda. 
\num{4} parxKAyxta; parxsidadhx; parxmuKa; parxdhAna; sherxVSaThx; utatxma. 
\enum
\emng
\eentry

\bentry
\word[notable(2)]{notable}
\pron{noVTaba(bf)lf}
\gl{\nA}
\bmng
sherxVSaThx, utatxma -- vayxkitx. 
\emng
\eentry

\bentry
\word{notableness}
\pron{noVTabalfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} gamanAhaRte. 
\num{2} edudx kANisuvaMtiruvike. 
\num{3} asAdhAraNate. 
\num{4} pArxmuKayx; pArxdhAnayx. 
\enum
\emng
\eentry

\bentry
\word{notably}
\pron{noVTabilx}
\gl{\kirxvi}
\bmng
visheVSavAgi; gamanAhaRvAda riVtiyalilx. 
\emng
\eentry

\bentry
\word{notarial}
\pron{noVTeVrialf}
\gl{\gu}
\bmng
\bnum
\num{1} noVTariya; leVKayxparxmANakanige saMbaMdhisida; parxmANiVkaraNa adhikAriya. 
\num{2} noVTari yA parxmANiVkaraNa adhikAriyu bareda yA sidadhxpaDisida yA rujumADida. 
\enum
\emng
\eentry

\bentry
\word{notarially}
\pron{noVTeVriali}
\gl{\kirxvi}
\bmng
leVKayxparxmANakanige, noVTarige -- saMbaMdhisiruvaMte; parxmANiVkaraNa adhikAri sidadhxpaDisi baredu rujumADiruvaMte. 
\emng
\eentry

\bentry
\word{notarise}
\pron{noVTareYsfZ}
\gl{\sakirx}
\bmng
= \hyperlink{notarize}{notarize}. 
\emng
\eentry

\bentry
\word{notarize}
\pron{noVTareYsfZ}
\gl{\sakirx}
\bmng
(\ame) (leVKayx parxmANaka adhikAriya, noVTariya \vi) parxmANiVkarisu; daqDhiVkarisu; nijaveMdu sAkaSx yxkoDu, sAkiSxhAku. 
\emng
\eentry

\bentry
\word{notary}
\pron{noVTari}
\gl{\nA}
\expl{(\bava\ \eng{notaries}).}
\bmng
noVTari; leVKayxparxmANaka; parxmANiVkaraNa adhikAri; karAru \mo vugaLanunx bareyuvudu yA baredudanunx parxmANiVkarisuvudu, salilxkegAgi koTaTx huMDi \mo vanunx tirasakxrisalAyiteMdu parxmANapatarx koDuvudu -- iveV \mo\ adhikAragaLuLaLx obabx sAvaRjanika adhikAri. 
\emng
\eentry

\bentry
\wordnospeech{notary public}{notary public}
\pron{?}
\gl{\nA}
\bmng
= \hyperlink{notary}{notary}. 
\emng
\eentry

\bentry
\word{notate}
\pron{noVTeVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (saMKeyx, motatx, \mo vanunx) saMkeVtagaLiMda toVrisu; saMkeVta lipiyiMda sUcisu. 
\num{2} (\saM) (savxra, tALa, avadhi, \mo vanunx) savxralipiyiMda sUcisu; savxralipiyalilx bare. 
\enum
\emng
\eentry

\bentry
\word{notation}
\pron{noVTeVSa(Sf)nf}
\gl{\nA}
\bmng
\bnum
\num{1} aMkana (padadhxti); saMkeVtana; saMKeyx, motatx \mo vanunx saMkeVtagaLiMda, cihenxgaLiMda, toVrisuvike. 
\num{2} (hiVge toVrisuva) saMkeVtagaLu; cihenxgaLu. 
\num{3} (\saM) savxralipi; savxrasaMkeVta; savxragati, savxrasAthxyi, kAla, \mo vugaLanunx cihenxgaLa mUlaka sUcisuvike yA hAge sUcisida cihenxgaLu. 
\num{4} (caduraMgadATada naDegaLu, naqtayxda hejejxgaLu, \mo vanunx sUcisuva) cihenxgaLu; saMkeVtagaLu. 
\num{5} (\ame) TipapxNi yA dAKale. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{scale of notation}(aMkagaNitadalilx) saMKAyxkarxma; saMKAyxmAna; saMKAyxpadadhxtiya AdhAra: \eng{decimal scale of notation} dashamAna saMKAyxpadadhxti. 
\emng
\eentry

\bentry
\word{notational}
\pron{noVTeVSanalf}
\gl{\gu}
\bmng
aMkanakekx, saMkeVtagaLige, cihenxgaLige -- saMbaMdhisida. 
\emng
\eentry

\bentry
\word[notch(1)]{notch}
\pron{nAcf}
\gl{\nA}
\bmng
\bnum
\num{1} (aMcinalilx yA horabAguLaLx meVlemx Yyalilx mADida) \eng{v-}AkArada kacucx. 
\num{2} (eNikegAgi koVlu \mo vugaLa meVle mADiruva) koyatx; kacucx. 
\num{3} (\AmA) (sherxVNiyalilx, munanxDeyalilx) haMta; aMtasutx; sAthxna; baDati. 
\num{4} (\ame) kaNive; ALavAda kamari. 
\enum
\emng
\eentry

\bentry
\word[notch(2)]{notch}
\pron{nAcf}
\gl{\sakirx}
\bmng
\bnum
\num{1} kacucxmADu; koyudx geremADu. 
\num{2} halulxkoyudx garagasa \mo vugaLanAnxgi mADu. 
\num{3} kacucx mADi gelulxsaMKeyx gurutisu (\rUpa\ saha). 
\num{4} (soVpAnada cwkaTiTxnalilx gADigaLanunx, ugilugaLanunx mADi meTiTxlu \mo vanunx) BadarxvAgi -- seVrisu, kUrisu, joVDisu. 
\enum
\emng
\eentry

\bentry
\word{notched}
\pron{nAcfTx}
\gl{\gu}
\bmng
(\savi\ matutx \pArxvi) oraTu oraTAgi kacucxgaLanunx mADidaMtiruva; garagasada ENinAkArada. 
\emng
\eentry

\bentry
\word{notcher}
\pron{nAcarf}
\gl{\nA}
\bmng
koyudx geremADuva, kacucxmADuva -- vayxkitx yA sAdhana. 
\emng
\eentry

\bentry
\word{notchy}
\pron{nAci}
\gl{\gu}
\expl{(\tara\ \eng{notchier}, \tama\ \eng{notchiest}).}
\bmng
kacucxkacAcxda. 
\emng
\eentry

\bentry
\word[note(1)]{note}
\pron{noVTf}
\gl{\nA}
\bmng
\bnum
\num{1} (\sA\ \bava dalilx) (BASaNakAkxgali, leVKanakAkxgali mADikoMDa, nenapiTuTxkoLaLxlu sahAykavAguva) vAsatxvAMshagaLa, viSayagaLa, aBipArxyagaLa -- TipapxNi, muKAyxMshagaLa paTiTx: \eng{make} (\engit{or} \eng{take) notes of} TipapxNi mADiko; muKAyxMshagaLanunx gurutuhAku. \eng{speak without notes} TipapxNi ilalxde mAtanADu. 
\num{2} (anuBavagaLu \mo vugaLa, \sA\ aliKita) kathana. 
\num{3} ciVTi; anwpacArika yA saNaNx patarx. 
\num{4} (pusatxka \mo vugaLalilx garxMthaBAgagaLanunx, viSayagaLanunx vishadapaDisalu seVrisiruva) TipapxNi; vivaraNe; aDiTipapxNi. 
\num{5} (\birx) = \hyperref{kandict_b.pdf}{B}{banknote}{banknote}. 
\num{6} (oMdu gotAtxda avadhiylilx, namUdAda haNa koDuvudAgi baredukoTaTx halavAru riVtiya) keYpatarx (gaLalolxMdu). 
\numi{7} 
\banum
\alnum{a} gamana; lakaSxyX: \eng{worthy of note} gamanAhaRvAda. 
\alnum{b} Ganate; parxsididhx; (parx)KAyxti: \eng{a critic of note} parxsidadhx vimashaRka. \eng{a person of note} parxsidadhx vayxkitx. 
\eanum
\numie
\numi{8} 
\banum
\alnum{a} \saM\ savxracihenx; savxralipi; saMgiVta dhavxniya tiVvarxteya maTaTx matutx avadhiyanunx sUcisuva lipi saMkeVta. 
\alnum{b} (saMgiVtavAdayx, dhavxni, eMjinf, \mo vu mADuva) nidiRSaTx shurxtiy savxra. 
\alnum{c} (piyAnoV \mo\ vAdayxgaLalilx, dhavxni huTiTxsuva) kiVlikeY. 
\eanum
\numie
\numi{9} 
\banum
\alnum{a} hakikxya hADu yA kare yA kUgu. 
\alnum{b} A hADina oMdu savxra. 
\eanum
\numie
\num{10} (obabxnu mAtanADuva yA tananx manoVBAvavanunx yA aBipArxyavanunx sUcisuva) oMdu vishiSaTx dhavxni, riVti: \eng{there is a note of optimism in his report} avana varadiyalilx oMdu AshABAvada dhavxniyide. \eng{must sound a note of warning} ecacxrikeya sUcane koDabeVku. 
\num{11} cihenx; gurutu; lakaSxNa; veYshiSaTxyX: \eng{catholicity is one note of the true church} manoVveYshAlayx nijavAda caciRna oMdu lakaSxNa. 
\num{12} (\pArxparx) kaLaMka; doVSa; KaMDaney cihenx: \eng{on which the law has set a note of infamy} kAnUnu adara meVle apaKAyxtiy kaLaMka horiside. 
\num{13} (\pArxparx) udAgxrada, mecicxkey yA parxshAnxthaRka cihenx, gurutu: \eng{a note of interrogation} parxshAnxthaRka cihenx. 
\num{14} (vidhuyxkatxvAda rAjakiVy) niyoVgapatarx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\hyperdef{N}{note(1) pagu(1)}{} 
\bnum
\num{1} \eng{hit} (\engit{or} \eng{strike) the right note} sariyAda riVtiylilx mAtanADu yA kelasamADu; yukatxvAdudanunx ADu yA mADu. 
\num{2} \eng{strike a false note} anucitavAgi nuDi yA naDeduko. 
\num{3} \eng{take note (of)} noVDu; gamanisu; lakaSxyXkoDu. 
\num{4} \eng{of note} parxmuKa; parxsidadhx. 
\enum
\emng
\eentry

\bentry
\word[note(2)]{note}
\pron{noVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} noVDu; daqSiTxyiDu; gamanisu; gamanakoDu; lakaSxyXkoDu. 
\num{2} (nenapigAgi, gamanisuvudakAkxgi) gurutu hAkiko; barediTuTxko. 
\num{3} (\kaparx dalilx) (oMdu guNa, caTuvaTike, \mo vugaLigAgi) parxsidadhxnAgiru; KAyxti hoMdiru: \eng{were noted for their generosity} avaru tamamx audAyaRkekx parxsidadhxrAgidadxru. 
\num{4} (garxMtha \mo vakekx) TipapxNi, TiVku -- bare. 
\enum
\emng
\eentry

\bentry
\word{notebook}
\pron{noVTfbukf}
\gl{\nA}
\bmng
noVTfbukukx; TipapxNi pusatxka; TipapxNi mADikoLaLxlu iTuTxkoLuLxva kirupusatxka. 
\emng
\eentry

\bentry
\word{notecase}
\pron{noVTfkeVsf}
\gl{\nA}
\bmng
(haNada noVTugaLanunx iTuTxkoLaLxlu baLasuva) pasuR; capapxTeciVla. 
\emng
\eentry

\bentry
\word{noted}
\pron{noVTiDf}
\gl{\gu}
\bmng
parxKAyxtavAda; parxsidadhxvAda; hesarAMta; (yAvudoV oMdakekx) hesaruvAsiyAda. 
\emng
\eentry

\bentry
\word{noteless}
\pron{noVTflisf}
\gl{\gu}
\bmng
\bnum
\num{1} gamanAhaRvalalxda; lakaSxyXkekx bAradiruva. 
\num{2} nAdamayavalalxda; saMgiVtavalalxda. 
\enum
\emng
\eentry

\bentry 
\word{notelet}
\pron{noVTfliTf}
\gl{\nA}
\bmng
(\kanmu) (modalaneya hALeylilx citarxvinAyxsagaLiruva, maDisida anwpacArika kAgadapatarx bareyalu baLasuva) saNaNx kAgada, ciVTi yA kADuR. 
\emng
\eentry

\bentry 
\word{notepaper}
\pron{noVTfpeVparf}
\gl{\nA}
\bmng
(\kanmu\ KAsagi) patarxvayxvahArakekx baLasuva kAgadada hALe. 
\emng
\eentry

\bentry 
\word{noteworthiness}
\pron{noVTfvadiRnisf}
\gl{\nA}
\bmng
gamaniVyate; gamanAhaRte; gamanadalilx iDabeVkAgiruvike yA gamanisalu yoVgayxvAgiruvike. 
\emng
\eentry

\bentry 
\word{noteworthy}
\pron{noVTfvadiR}
\gl{\gu}
\bmng
gamanAhaR; gamaniVya; gamanadalilxDabeVkAda; gamanisalu yoVgayxvAda. 
\emng
\eentry

\bentry
\word[nothing(1)]{nothing}
\pron{natiMgf}
\gl{\nA}
\bmng
\bnum
\num{1} (guNavAcakada hiMde baMdAga) yAvudeV vasutx alalx, ilalx(dudx): \eng{nothing great is easy} mahatavxvAdadudx yAvudU sulaBavalalx. \eng{nothing has been done} EnanUnx mADalAgilalx; yAvudU Agilalx. \eng{have nothing to do} mADalu (kelasa) EnU ilalx. 
\num{2} (kelamomemx pUrakavAkayxda joteylilx) yAvudU, EnU ilalx: \eng{I see nothing that I want} nanage beVkAda yAvudU nanage kANutitxlalx. \eng{can find nothing useful} upayukatxvAda yAvudU ilalx. 
\numi{3} 
\banum
\alnum{a} kuSxlalxkavAda -- vasutx, saMgati, mAtu yA vayxkitx: \eng{was nothing to me} adu nanage kuSxlalxkavAda viSayavAgitutx. \eng{the little nothings of life} jiVvanada kuSxlalxka viSayagaLu. \eng{he is nothing without his money} avana baLi haNavilalxde hoVgidadxre avanobabx kelasakekx bArada vayxkitx. 
\alnum{b} (visheVSaNavAgi, \AmA) agaNayx; beleyilalxda: \eng{a nothing sort of day} oMdu riVti agaNayxvAda dina. 
\eanum
\numie
\num{4} aBAva; ilalxme; ilalxdiruvike; ilalxdiruvudu; nAsitx. 
\num{5} (aMkagaNita, lekAkxcAradalilx) sonenx -- aMka, mobalagu, itAyxdi: \eng{multiply 6 by nothing, and the result is nothing} Aranunx sonenxyiMda guNisidare sonenx barutatxde. \eng{a third of nothing is nothing} sonenxya mUraneV oMdu BAga sonenxyeV. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{be nothing}(dhamaRda \vi) 
\banum
\alnum{a} yAva nidiRSaTx paMgaDakUkx seVradiru. 
\alnum{b} nAsitxkanAgiru. 
\alnum{c} ajecnxVyatAvAdiyAgiru. 
\eanum
\numie
\numi{2} \eng{be nothing to} 
\banum
\alnum{a} saMbaMdha beVDadiru; saMbaMdhisiradiru: \eng{it is nothing to do with me} adakUkx nanagU saMbaMdhavilalx. 
\alnum{b} hoVlikeyAgadiru; hoVlike ilalxdiru. 
\eanum
\numie
\numi{3} \eng{be} (\engit{or} \eng{have) nothing to do with} 
\banum
\alnum{a} (vasutx yA vayxkitxya \vi) saMbaMdha ilalxdiru; goDave ilalxdiru; usAbari beVDadiru. 
\alnum{b} (yAvudeV oMdaralilx yA oMdaroDane) saMbaMdha hoMdadiru, seVradiru. 
\eanum
\numie
\num{4} \eng{come to nothing} niSaPxlavAgu; vayxthaRvAgi pariNamisu; EnU parxyoVjanavAgadiru: \eng{hopes that come to nothing} vayxthaRvAgi pariNamisuva BaravasegaLu; niSapxla niriVkeSxgaLu. 
\num{5} \eng{count} (\engit{or} \eng{go) for nothing} gaNanege bAradiru; vayxthaRvAgu. 
\num{6} \eng{feel like nothing on earth}(\AmA) tiVvarxvAda asavxsathxte yA mujugara anuBavisu. 
\numi{7} \eng{for nothing} 
\banum
\alnum{a} pukakxTeyAgi. 
\alnum{b} vayxthaRvAgi; EnU parxyoVjanavilalxde. 
\eanum
\numie
\num{8} \eng{has nothing in him} avanobabx nagaNayx; yaHkashicxtf manuSayx; avanige yAva vayxkitxtavxvU ilalx. 
\hyperdef{N}{nothing(1) pagu(9)}{} 
\numi{9} \eng{have nothing on} 
\banum
\alnum{a} (yAvudoV oMdakikxMta) hecucx anukUla hoMdilalxdiru. 
\alnum{b} (obabxna bagegx) AkeSxVpaNiVyavAda EnanUnx hoMdadiru, gotitxlalxdiru. 
\alnum{c} (vayxMgayx) (yAvudoV oMdakikxMta) tiVrA kiVLAgiru. 
\alnum{d} bari meYyalilxru; betatxleyAgiru. 
\alnum{e} biDuvAgiru; yAvudeV kelasakAyRgaLilalxdiru. 
\eanum
\numie
\numi{10} \eng{is nothing to} 
\banum
\alnum{a} EnU lekakxvilalx; yAvudakUkx ilalx; taqNasamAna: \eng{money is nothing to him} haNa eMdare avanige taqNasamAna. 
\alnum{b} (adara) samAnakekx baralAradu; (adakUkx, idakUkx) hoVlike ilalx. 
\eanum
\numie
\numi{11} \eng{like nothing on earth}(\AmA) (kurUpa \mo vugaLa \vi) atayxMta; vipariVtavAda; ati hecicxna: \eng{look like nothing on earth}(\AmA): 
\banum
\alnum{a} tiVrA avalakaSxNavAgiru; bahaLa kurUpiyAgiru. 
\alnum{b} ati ADaMbaradiMda tuMbiru; kaNiNxge rAcuvaMtiru. 
\eanum
\numie
\numi{12} \eng{make nothing of} 
\banum
\alnum{a} laGuvAgeNisu; alapxveMdu BAvisu. 
\alnum{b} hiMjariyade -- mADu, naDesu. 
\alnum{c} EnU mADalu toVcadiru, athaRvAgadiru. 
\alnum{d} upayoVgisalu, baLasalu sAdhayxvilalxvAgu. 
\alnum{e} vayxvaharisalu sAdhayxvilalxvAgu. 
\eanum
\numie
\numi{13} \eng{mean nothing to} 
\banum
\alnum{a} athaRvilalxvAgu. 
\alnum{b} mecacxdaMtAgu. 
\alnum{c} AshisadaMtAgu. 
\eanum
\numie
\num{14} \eng{neck or nothing} AdadAdxgaleMdu; savaRsavxvanUnx paNavAgi oDiDx. 
\num{15} \eng{no nothing}(\AmA) (ilalxda padAthaRgaLa paTiTxyalilx heVLuva koneya mAtu) yAvudU ilalx; oMdU ilalx: \eng{no bread, no butter, no cheese, no nothing}berxDf ilalx, beNeNx ilalx, ciVsfZ ilalx, yAvudU ilalx. 
\num{16} \eng{nothing but} = \hyperlink{nothingpagu18}{?pagu?\((18)\)}. 
\numi{17} \eng{nothing doing} 
\banum
\alnum{a} (\AmA) muKayxvAdadudx EnU sAdhayxvAgadu; gamanAhaRvAdudeVnU Agutitxlalx: \eng{there was nothing doing in town} Urinalilx gamanAhaRvAdudeVnU naDeyutitxlalx. 
\alnum{b} KaMDita ilalx; eMdeMdigU sAdhayxvilalx. 
\hypertarget{nothingpagu18}{} 
\eanum
\numie
\num{18} \eng{nothing else but} (\engit{or} \eng{than)} sumamxne; keVvala; baride; nisasxMdeVhavAgi; nisasxMshayavAgi: \eng{did nothing but grumble} goNaguvudanunx biTuTx EnanUnx mADalilalx; sumamxne (goNagida). 
\num{19} \eng{nothing for it but to} vidhiyilalx; upAyavilalx; adanunx biTuTx beVre mAgaRvilalx. 
\num{20} \eng{nothing if not} muKayxvAgi; mUlataH: \eng{it is nothing if not critical} tapupx kaMDuhiDiyuvudu biTuTx beVreVnU alalx; tapupx kaMDu hiDiyuvudoMdeV muKayx lakaSxNa. 
\numi{21} \eng{nothing less than} 
\banum
\alnum{a} kaDime Enalalx: \eng{expected nothing less than a riot} kaDeV pakaSx doMbi AgutatxdeMdeV niriVkiSxsida; doMbiyAdarU AgiyeV tiVrutatxdeMdu niriVkiSxsida. 
\alnum{b} (\viparx) idakekx tiVrA virudadhxvAda. 
\alnum{c} (\viparx) eMdeMdigU ilalx; savxlapxvU ilalx. 
\eanum
\numie
\num{22} \eng{nothing like leather} tananx saraku elalx kelasakUkx odagutatxde (eMba sAvxBimAnada aBipArxya). 
\num{23} \eng{nothing venture nothing have}(gelalxbeVkAdare mununxgagxbeVku eMbathaRdalilx) dheYyaRvU ilalx, lABavU ilalx. 
\num{24} \hyperref{kandict_s.pdf}{S}{stop(1) pagu(4)}{$^1$stop} \eng{at nothing.} 
\numi{25} \eng{there is nothing in it} 
\banum
\alnum{a} adu nijavalalx. 
\alnum{b} adeVnU muKayxvalalx. 
\alnum{c} adaralilx EnU toDakilalx. 
\alnum{d} (ibabxra peYki) yArigU adaralilx hecicxna parxyoVjana yA anukUla ilalx. 
\eanum
\numie
\num{26} \eng{there is nothing to it} adu nijavalalx; adeVnU muKayxvalalx; adu tiVrA saraLa, neVra. 
\num{27} \eng{think nothing of it}(kaSxme keVLabeVDa yA kaqtajacnxte toVrisabeVkeMba BAvaneyiTuTxkoLaLxbeVDa eMdu sUcisalu baLasuva mAtAgi) adanunx yoVcisabeVDa; hAge ciMtisabeVDa. 
\num{28} \eng{to nothing} karxmeVNa saMpUNaRvAgi -- horaTuhoVgi, kANadAgi, adaqshayxvAgi. 
\num{29} \eng{to say nothing (of)}(aSuTx muKayxvalalxda aMshavanunx heVLuvAga yA muKayx aMshavanunx AlaMkArikavAgi heVLuvAga) (adanunx) heVLabeVkAdudilalx. 
\numi{30} \eng{with nothing on} 
\banum
\alnum{a} betatxleyAgi; barimeYyalilx. 
\alnum{b} sAkaSuTx (meYmucucxvaSuTx) baTeTx hAkikoLaLxde. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[nothing(2)]{nothing}
\pron{natiMgf}
\gl{\kirxvi}
\bmng
\bnum
\num{1} eMdeMdigU; EnU ilalxvAgi; yAva riVtiyalUlx ilalxdaMte: \eng{helps us nothing} namage avanu EnU sahAyakanAgilalx. \eng{is nothing like enough} EnU, savxlapxvU -- parxyoVjanavilalx. \eng{avails nothing} EnU parxyoVjanavilalx, upayoVgavilalx. 
\num{2} (\ame) (\AmA) eMdigU ilalx; bilfkulf alalx: \eng{Is it gold? Gold nothing, it's pinchbeck} adu cinanxveV? -- eMdigU ilalx, adu KoVTA loVha. 
\enum
\emng
\eentry

\bentry
\word[nothing(3)]{nothing}
\pron{natiMgf}
\gl{\gu}
\bmng
(\AmA) amuKayxvAda; kuSxlalxkavAda; kelasakekx bArada: \eng{a nothing sort of day} kelasakekx bArada, amuKayxvAda dina. 
\emng
\eentry 

\bentry
\word{nothingness}
\pron{natiMgfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} ilalxme; aBAva; shUnayxte. 
\num{2} ilalxdudx; avasutx. 
\num{3} niSapxrXyoVjakate. 
\num{4} alapxte; kuSxlalxkate; kuSxdarxte. 
\num{5} amuKayxte; apArxdhAnayxte. 
\num{6} kuSxlalxka -- vasutxgaLu, viSayagaLu. 
\enum
\emng
\eentry

\bentry
\word[notice(1)]{notice}
\pron{noVTisf}
\gl{\nA}
\bmng
\bnum
\num{1} gamana; daqSiTx; parishiVlane; avagAhane; lakaSxyX: \eng{brought it to his notice} avana gamanakekx taralAyitu. \eng{it escaped my notice} adu nananx gamana tapipxsikoMDitu. 
\num{2} (noVTisf boVDfR meVle aMTisida) parxkaTaNa, sUcana -- patarx; tiLivaLike ciVTi; jAhiVrAtu, isitxhAru, \mo vu. 
\numi{3} 
\banum
\alnum{a} (\kanmu\ sidadhxte mADikoLuLxvudakAkxgi, aupacArikavAgi koDuva) tiLivaLike; sUcane; ecacxrike: \eng{give notice} ecacxrike niVDu. \eng{have notice} sUcane paDe. \eng{at a moment's notice} (sidadhxvAgalu) oMdeV kaSxNada avakAsha koTuTx. 
\hyperdef{N}{notice(1)3b}{} 
\alnum{b} noVTiVsu; ecacxrike; \kanmu\ mAliVkanigU bADigedAranigU, yA yajamAnanigU kelasagArarigU saMbaMdhisidaMte tamamx naDuvaNa opapxMda yAvudoV nigadiyAda kAlakekx mugiyutatxdeMdu yAvudoV oMdu pakaSxvu koDuva tiLivaLike, parxkaTaNe: \eng{to give a week's notice} (sidadhxvirabeVkeMdu mAliVkanu kelasagAranige yA kelasagAranu mAliVkanige) oMdu vAra muMce noVTiVsu koDu. \eng{she is under notice} avaLige sidadhxteya noVTiVsf koDalAgide. \eng{notice to quit} (vAsa \mo vakekx tegedukoMDa mane \mo vanunx biDabeVkeMdu koDuva) sUcane. 
\eanum
\numie
\num{4} (vaqtatxpatirxke yA niyatakAlikadalilx) yAvudAdarU oMdu viSayavanunx kurita parxsAtxpa, paMkitx yA leVKana; (\kanmu) pusatxka vimasheR, nATaka vimasheR, itAyxdi. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{at short} (\engit{or} \eng{a moment's) notice} atayxlapxkAlada (yA kaSxNa mAtarxda) avakAsha koTuTx. 
\numi{2} \eng{take no notice of} 
\banum
\alnum{a} lakaSxyX koDadiru; gamanisadiru; Asakitx toVradiru. 
\alnum{b} yAvudeV oMdara bagegx yAva karxmavanUnx keYgoLaLxdiru. 
\eanum
\numie
\num{3} \eng{take notice} gamana, Asakitx -- toVrisu. 
\numi{4} \eng{take notice of} 
\banum
\alnum{a} arivina, Asakitxya, garxhikeya -- cihenx toVrisu: \eng{take notice of one's surroundings} (tananx) sutatxlina parxpaMcada bagegx Asakitx toVrisu, gamana harisu. 
\alnum{b} anusAravAgi -- kelasa mADu, vatiRsu. 
\eanum
\numie
\num{5} \eng{till further notice} muMde tiLivaLike koDuvavaregU. 
\num{6} \eng{under notice} vidhivatAtxgi noVTisf koTiTxruva. 
\enum
\emng
\eentry

\bentry
\word[notice(2)]{notice}
\pron{noVTisf}
\gl{\sakirx}
\bmng
\bnum
\num{1} garxhisu; gamanisu; lakaSxyXkoDu. 
\num{2} (oMdara viSayavAgi) parxsAtxpisu; mAtanADu. 
\num{3} mayARdeyiMda yA anugarxha toVrisuvaMte -- satakxrisu, kANu. 
\num{4} ecacxrike, tiLivaLike -- koDu; tiLisu: \eng{the men were noticed} A janarige (biTuTx hoVguva) ecacxrike koDalAyitu. 
\enum
\emng
\eentry

\bentry
\word{noticeable}
\pron{noVTisaba(bf)lf}
\gl{\gu}
\bmng
\bnum
\num{1} sulaBavAgi -- kaNiNxge biVLuva, kANuva. 
\num{2} gamanAhaRvAda; gamana -- seLeyabalalx, seLeyuvaMtha. 
\enum
\emng
\eentry

\bentry
\word{noticeably}
\pron{noVTisabilx}
\gl{\kirxvi}
\bmng
gamanAhaRvAgi; gamana seLeyuvaMte. 
\emng
\eentry

\bentry
\word{notice-board}
\pron{noVTisfboVDfR}
\gl{\nA}
\bmng
noVTisf boVDuR; parxkaTaNa Palaka; parxkaTaneya halage; parxkaTaNa patarxgaLu, isitxhAru, \mo vanunxLaLx yA avanunx aMTisuvudakAkxgiruva halage. 
\emng
\eentry

\bentry
\word{notifiable}
\pron{noVTipheYaba(bf)lf}
\gl{\gu}
\bmng
(roVgada \vi) parxkaTaNAhaR; AroVgAyxdhikArigaLige tiLisabeVkAda. 
\emng
\eentry

\bentry
\word{notification}
\pron{noVTiphikeVSa(Sf)nf}
\gl{\nA}
\bmng
parxkaTaNe; isitxhAru; GoVSaNe. 
\emng
\eentry

\bentry
\word{notify}
\pron{noVTipheY}
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{notifies}, \BU\ matutx \BUkaq\ \eng{notified}).}
\bmng
\bnum
\num{1} (vayxkitx \mo varige) tiLisu; tiLiypaDisu; ecacxrike niVDu. 
\num{2} (oMdu viSayavanunx) tiLisu; tiLiyapaDisu; parxkaTisu; tiLivaLike koDu; parxsidadhxpaDisu; isitxhAru koDu; GoVSisu. 
\enum
\emng
\eentry

\bentry
\word{notion}
\pron{noVSa(Sf)nf}
\gl{\nA}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} BAvane; kalapxne; eNike: \eng{the notion of my doing it is absurd} adanunx nAnu mADutetxVneMba eNike hAsAyxsapxda. 
\alnum{b} (Kacitavalalxda) aBipArxya; aBimata; BAvane; aMboVNa: \eng{such is the common notion} sAmAnayx BAvane hAgide. \eng{has the notion that people are honest} janaru pArxmANikaru eMba (asapxSaTx) aBipArxyavide. 
\eanum
\numie
\num{2} parxvaqtitx; olavu; udedxVsha: \eng{he has no notion of obeying} vidheVyanAgiruva udedxVsha avanigilalx. \eng{has no notion of conforming} pAlisuva aBipArxyavilalx. 
\num{3} (\bava dalilx) (\kanmu\ agagxvU upayukatxvU Ada) saNaNxpuTaTx vasutxgaLu; cilalxre sAmAnu (uDupige saMbaMdhisida guMDi, dAra, ribabxnf, \mo\ saNaNxpuTaTx sAmAnugaLu). 
\num{4} (oMdu nidiRSaTx vasutxvanunx yA vayxkitxyanunx yAva vagaRkekx seVrisabahudoV aMtha) sAmAnayx -- BAvane, kalapxne. 
\num{5} arivu; jAcnxna; kalapxne: \eng{has no notion of discipline} shisitxna kalapxneyeV ilalx. \eng{has no notion of the difficulty of this problem} I samaseyxya kaSaTxda ariveV ilalx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{first notion} 
\banum
\alnum{a} (\takaR) BAvane; kalapxne. 
\alnum{b} (vasutxgaLa \vi) parxthama BAvane (\udA\ mara, hasu). 
\eanum
\numie
\num{2} \eng{second notion} divxtiVya BAvane; parxthama BAvaneyiMda huTiTxda divxtiVya BAvane (\udA\ jAti, vagaR, guNa, \mo vu). 
\enum
\emng
\eentry

\bentry
\word{notional}
\pron{noVSanalf}
\gl{\gu}
\bmng
\bnum
\num{1} (vasutx, saMbaMdha, \mo vugaLa \vi) kAlapxnika; keVvala kalapxneyalilx, ciMtaneyalilx -- iruva. 
\num{2} (jAcnxna \mo vugaLa \vi) (parxyoVga, pariVkeSx yA nidashaRnagaLa meVle avalaMbisirade) BAvanAmAtarxvAda; keVvala ciMtaneya; kalapxneya; keVvala seYdAdhxMtika. 
\num{3} (\vAyx) (kirxyApadada \vi) muKayx; parxdhAnavAda; sahAyakavalalxda; savxtaMtarxvAgi tananx athaRvanunx sUcisuva. 
\enum
\emng
\eentry

\bentry
\word{notionalist}
\pron{noVSanalisfTx}
\gl{\nA}
\bmng
kalapxnAshiVla, BAvanAtamxka -- sidAdhxMti. 
\emng
\eentry

\bentry
\word{notionally}
\pron{noVSanali}
\gl{\kirxvi}
\bmng
seYdAdhxMtikavAgi; BAvanAshiVlavAgi; Uheyalilx. 
\emng
\eentry

\bentry
\word{notochord}
\pron{noVTakADfR}
\gl{\nA}
\bmng
noVTakADfR; benunxhuri pUrA rUpugoMDilalxda keLadajeRya pArxNigaLalilxyU kasheVrukagaLa BUrxNagaLalilxyU kANabaruva, benunxhuriya pUvaRrUpa. 
\emng
\eentry

\bentry
\word{notoriety}
\pron{noVTareYaTi}
\gl{\nA}
\bmng
kuparxsididhx; kuKAyxti; keTaTx hesaru paDediruvike. 
\emng
\eentry

\bentry
\word{notorious}
\pron{noV(na)ToVriasf}
\gl{\gu}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a}(viSayagaLa \vi) (niMdAsUcakavAgi) hesaruvAsiyAda; hesarAMta; savaRsAmAnayxvAgi yA cenAnxgi tiLidiruva: \eng{it is notorious that}(eMba viSaya) cenAnxgi tiLidide. 
\alnum{b}(hesaru, naDate, \mo vugaLa \vi) parxkaTavAda; kuKAyxta; elelxlUlx mAtanADikoLuLxva; A hesarige takukxdAda: \eng{a notorious smuggler} kuKAyxta kaLaLxsAgaNedAra. 
\eanum
\numie
\num{2} (oMdu guNakekx, naDatege) kuKAyxta; kuparxsidadhx; keTaTx hesarAMta; apakiVtiR paDediruva: \eng{a ship notorious for ill luck} duradaqSaTxkekx hesarAda haDagu. 
\enum
\emng
\eentry

\bentry
\word{notoriously}
\pron{noV(na)ToVriasfli}
\gl{\kirxvi}
\bmng
kuparxsidadhxvAgi; kuKAyxta riVtiyalilx. 
\emng
\eentry

\bentry
\word{notornis}
\pron{naTAniRsf}
\gl{\nA}
\bmng
naTAniRsf; pAphiRriyoV mAYxMTeli kulakekx seVrida, doDaDx kokUkx, ujavxla vaNaRda garigaLU iruva, hAralArada, nUyxsiZVleMDina oMdu viraLa pakiSx. 
\emng
\eentry

\bentry
\word{Notts.}
\pron{nATfsx}
\gl{\saMkiSx}
\bmng
\eng{Nottinghamshire.} 
\emng
\eentry

\bentry
\word[notwithstanding(1)]{notwithstanding}
\pron{nATfvitf(df)sATxYxMDiMgf}
\bmng
\upa\ AdarU; AdAgUyx: \eng{notwithstanding his objection} avana AkeSxVpaNe idadxrU. 
\emng
\eentry

\bentry
\word[notwithstanding(2)]{notwithstanding}
\pron{nATfvitf(df)sATxYxMDiMgf}
\gl{\kirxvi}
\bmng
AdarU; paraMtu; EneV Agali: \eng{he saw that it would come to pass notwithstanding} EneV Agali, adu naDedeV tiVrutatxdeMbudu avanige gotAtxyitu. 
\emng
\eentry

\bentry
\word[notwithstanding(3)]{notwithstanding}
\pron{nATfvitf(df)sATxYxMDiMgf}
\gl{\saMavayx}
\bmng
(\pArxparx) hAgidadxrU; AdarU saha; heVgAdarU: \eng{notwithstanding that it were once burned by fire} beMki adanunx omemx suTuTx hAkidadxrU saha. 
\emng
\eentry

\bentry
\word{nougat}
\pron{nUgA}
\gl{\nA}
\bmng
(sakakxre, jeVnutupapx, kAyi, koVLimoTeTxya biLiya loVLe, \mo vugaLiMda tayArisida) oMdu sihi tiMDi, miThAyi. 
\emng
\eentry

\bentry
\word{nought}
\pron{nATf}
\gl{\nA}
\bmng
\bnum
\num{1} sonenx; shUnayx. 
\num{2} (\kAparx\ yA \pArxparx) (kelavu \pagu gaLalilx) = \hyperlink{naught(1)}{naught}. 
\enum
\emng
\eentry

\bentry
\word{noumenal}
\pron{nwminalf}
\gl{\gu}
\bmng
shudadhxtatatxvXkekx saMbaMdhisida; mUlasatetxya. 
\emng
\eentry

\bentry
\word{noumenally}
\pron{nwminali}
\gl{\kirxvi}
\bmng
shudadhxtatatxvXda mUlaka; mUlasatetxyiMda. 
\emng
\eentry

\bentry
\word{noumenon}
\pron{nwminanf}
\gl{\nA}
\expl{(\bava\ \eng{noumena}).}
\bmng
shudadhx -- tatatxvX, satatxvX; mUlasatetx; iMdirxyagoVcaravAda bAhayx parxpaMcakekx AdhAravAgi, Adare adara mUlaka tiLiyalu sAdhayxvilalxda, budidhxya sahajashakitxyiMda mAtarx gArxhayxvAguva, yAva vAyxvahArika visheVSaNagaLiMdalU kUDirada shudadhx viSaya, mUla vasutx. 
\emng
\eentry

\bentry
\word{noun}
\pron{nwnf}
\gl{\nA}
\bmng
(\vAyx) nAmavAcaka; nAmapada. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{common noun} sAmAnayx nAma(pada). 
\num{2} \eng{proper noun} aMkita nAma(pada). 
\enum
\emng
\eentry

\bentry
\word{nounal}
\pron{nwnalf}
\gl{\gu}
\bmng
nAmapadadaMtha yA nAmavAcakakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nourish}
\pron{nariSf}
\gl{\sakirx}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (AhAra koTuTx) poVSisu; puSiTxVkarisu; AreYke mADu. 
\alnum{b} (maNuNx \mo vanunx) saMpadaBxritagoLisu; PalavatAtxgisu. 
\alnum{c} (bawdidhxka yA BAvuka) gArxsa niVDu yA poVSaNe odagisu. 
\eanum
\numie
\num{2} (BAvane \mo vanunx) poVSisu; AreYke mADi beLesu; aBivaqdidhxgoLisu. 
\enum
\emng
\eentry

\bentry
\word{nourisher}
\pron{nariSarf}
\gl{\nA}
\bmng
poVSaka; poVSisuvava. 
\emng
\eentry

\bentry
\word{nourishing}
\pron{nariSiMgf}
\gl{\gu}
\bmng
\bnum
\num{1} (\kanmu\ AhArada \vi) pwSiTxka; puSiTx koDuva; poVSisuva; puSiTxVkarisuva. 
\num{2} (Ase, BAvane, \mo vanunx) poVSisuva; Adarisuva. 
\enum
\emng
\eentry

\bentry
\word{nourishingly}
\pron{nariSiMgfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} (\kanmu\ AhArada \vi) poVSisuvaMte; pawSiTxkavAgi; puSiTxkoDuva riVtiyalilx. 
\num{2} (Ase, BAvane, \mo vanunx) poVSisuvaMte. 
\enum
\emng
\eentry

\bentry
\word{nourishment}
\pron{nariSfmaMTf}
\gl{\nA}
\bmng
\bnum
\num{1} poVSaNe; AhAra. 
\num{2} poVSaNe; pAlane; AdaraNe; AreYke. 
\enum
\emng
\eentry

\bentry
\word{nous}
\pron{nwsf}
\gl{\nA}
\bmng
\bnum
\num{1} (\AmA) vayxvahAra jAcnxna; sAmAnayx budidhxshakitx. 
\num{2} (\tashA) manasusx; budidhx. 
\enum
\emng
\eentry

\bentry
\wordf{nouveau riche}
\pron{nUvoV riVSf}
\gl{\nA}
\expl{\F\ (\bava\ \engit{nouveaux riches} \ucAcx\ adeV).}
\bmng
navashirxVmaMtaru; hosadAgi meVleVrida haNavaMtaru; nenenx monenxya (\kanmu\ ADaMbarada) shirxVmaMtaru; diDhiVrf dhanikaru. 
\emng
\eentry

\bentry
\wordf{nouvelle cuisine}
\pron{nUvelf kivxsiZVnf}
\gl{\nA}
\expl{\F}
\bmng
navayxpAka; Adhunika, naviVna -- aDige; hosaruci; hoTeTxge BAravAgade, kaNiNxge suMdaravAgi, nAlagege ruciyAgi iruvaMte tayArisuva hosa pAka padadhxti. 
\emng
\eentry

\bentry
\wordf{nouvelle vague}
\pron{nUvelf vAgf}
\gl{\nA}
\expl{\F}
\bmng
hosa ale; navayx paMtha; \kanmu\ pherxMcf calanacitarx keSxVtarxdalilx {\eng{1960}}ra pArxraMBadalilx baMda hosa sheYli. 
\emng
\eentry

\bentry
\wordnospeech{Nov.}{Nov.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{November}. 
\emng
\eentry

\bentry
\word{nova}
\pron{noVva}
\gl{\nA}
\expl{(\bava\ \eng{novae} \ucAcx-- noVviV yA \eng{novas}).}
\bmng
(\Kavi) noVva; navayxtAre; idadxkikxdadxMte tuMba parxkAshadiMda beLagi matetx modalina sithxtige baruva nakaSxtarx. 
\emng
\eentry

\bentry
\word[novel(1)]{novel}
\pron{nAvalf}
\gl{\nA}
\bmng
\bnum
\num{1} kAdaMbari; oMdu athavA hecucx saMpuTagaLanunx tuMbuvaSuTx doDaDxdAgiyU, aviciCxnanxvAda kathAvasutxvinalilx yathAthaR jiVvanada pAtarxgaLanUnx, kAyaRkalApagaLanUnx citirxsuva kAlapxnika gadayxkathe. 
\hypertarget{novel(1)2}{} 
\num{2} kAdaMbari sAhitayx; kAdaMbari parxkArada sAhitayx. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{the novel} = \hyperlink{novel(1)2}{$^1$novel\((2)\)}. 
\emng
\eentry

\bentry
\word[novel(2)]{novel}
\pron{nAvalf}
\gl{\gu}
\bmng
\bnum
\num{1} hosa -- riVtiya, terana, mAdariya, bageya; naviVna. 
\num{2} apUvaR; vicitarx. 
\enum
\emng
\eentry

\bentry
\word{novelese}
\pron{nAvaliVsfZ}
\gl{\nA}
\bmng
(\hiV) kAdaMbariVya sheYli; kiVLumaTaTxda kAdaMbarigaLa lakaSxNa, sheYli. 
\emng
\eentry

\bentry
\word{novelette}
\pron{nAvaleTf}
\gl{\nA}
\bmng
\bnum
\num{1} kiru, cikakx -- kAdaMbari. 
\num{2} (\birx) (\hiV) oMdu terana laGu romAyxMTikf kathe. 
\num{3} (\saM) aneVka saMgatigaLiMda kUDi, mukatx vinAyxsa hoMdiruva piyAnoV(gAgi racisida) kaqti. 
\enum
\emng
\eentry

\bentry
\word{novelettish}
\pron{nAvaleTiSf}
\gl{\gu}
\bmng
(\hiV) 
\bnum
\num{1} laGu romAyxMTikf katheyaMtha; hagura ramayx katheyaMtha. 
\num{2} atiBAvukateya; BAvAtireVkada. 
\enum
\emng
\eentry

\bentry
\word{novelise}
\pron{nAvaleYsfZ}
\gl{\sakirx}
\bmng
= \hyperlink{novelize}{novelize}. 
\emng
\eentry

\bentry
\word{novelist}
\pron{nAvalisfTx}
\gl{\nA}
\bmng
kAdaMbarikAra; kAdaMbari leVKaka. 
\emng
\eentry

\bentry
\word{novelistic}
\pron{nAvalisiTxkf}
\gl{\gu}
\bmng
kAdaMbarigaLaMtha yA kAdaMbarigaLige saMbaMdhisida; kAdaMbarigaLa guNalakaSxNagaLige -- takakxMtha, sarihoVguva. 
\emng
\eentry

\bentry
\word{novelization}
\pron{nAvaleYseZVSa(Sf)nf}
\gl{\nA}
\bmng
kAdaMbariVkaraNa; kAdaMbari parxkArakekx mApaRDisuvudu, aLavaDisuvudu. 
\emng
\eentry

\bentry
\word{novelize}
\pron{nAvaleYsfZ}
\gl{\sakirx}
\bmng
(nATaka, vAsatxva viSaya, \mo vanunx) kAdaMbariyAgi mADu; kAdaMbari rUpakekx -- mApaRDisu, aLavaDisu. 
\emng
\eentry

\bentry
\word{novella}
\pron{navela}
\gl{\nA}
\expl{(\bava\ \eng{novellas}).}
\bmng
\bnum
\num{1} saNaNx, kiru -- kAdaMbari yA kiru kathe. 
\num{2} kathe. 
\enum
\emng
\eentry

\bentry
\word[novelty(1)]{novelty}
\pron{nAvaliTx}
\gl{\nA}
\expl{(\bava\ \eng{novelties})}
\bmng
\bnum
\num{1} nAviVnayx; naviVnate; hosatana; navatana; navayxte; asAmAnayx yA hosa -- vasutx, viSaya, GaTane. 
\num{2} savxMtike; sovxVpajacnxte. 
\num{3} naviVna mAdariya alaMkArada yA ATikeya vasutx. 
\num{4} naviVna, asAdhAraNa vayxkitx \mo varu. 
\enum
\emng
\eentry

\bentry
\word[novelty(2)]{novelty}
\pron{nAvaliTx}
\gl{\gu}
\bmng
(visheVSaNavAgi) naviVna; hosa, naviVna mAdariya: \eng{novelty toys} hosa bageya ATikegaLu. 
\emng
\eentry

\bentry
\word{November}
\pron{naveMbarf}
\gl{\nA}
\bmng
naveMbarf tiMgaLu; pAshAcxtayx vaSaRda hanonxMdaneya tiMgaLu. 
\emng
\eentry

\bentry
\word{novena}
\pron{naviVna}
\gl{\nA}
\bmng
(\roVkAyx\ cacfR) navAhinxka; satatavAgi oMbatutx dina naDeyuva visheVSa ArAdhane matutx pArxthaRnA vidhi \mo vu. 
\emng
\eentry

\bentry
\word{novice}
\pron{nAvisf}
\gl{\nA}
\bmng
\bnum
\numi{1} navashiSayx: 
\banum
\alnum{a} diVkeSx tegedukoLuLxvudakekx modalu tarabeVtigAgi dhAmiRka saMsethxge seVrisikoMDavanu. 
\alnum{b} hosa anuyAyi; hosadAgi matAMtara hoMdidavanu. 
\eanum
\numie
\num{2} AraMBiga; ananuBavi; nuritilalxdavanu. 
\num{3} paMdayxdalilx muKayx bahumAna paDeyadiruva pArxNi. 
\enum
\emng
\eentry

\bentry
\word{noviciate}
\pron{naviSiaTf}
\gl{\nA}
\bmng
\eng{(novitiate} eMdU \parx). 
\bnum
\num{1} navashiSayxna -- umeVduvAri kAla, pariVkASxkAla. 
\num{2} (\kanmu\ matadhamaR \mo vugaLalilx) navashiSayx; hosadAgi matAMtara hoMdidavanu. 
\num{3} navashiSayxrige gotutx mADiruva vasati. 
\enum
\emng
\eentry

\bentry
\word{novitiate}
\pron{naviSiaTf}
\gl{\nA}
\bmng
= \hyperlink{noviciate}{noviciate}. 
\emng
\eentry

\bentry
\word{Novocaine}
\pron{noVvakeVnf}
\gl{\nA}
\bmng
(\eng{novocaine} eMdU \parx) \P\ noVvakeVnf; deVhaBAgadalilx sathxLiVya arivaLikege baLasuva, benfsez\char'261Vyikf AmalxdiMda tayArisida oMdu padAthaR. 
\emng
\eentry

\bentry
\word[now(1)]{now}
\pron{nw}
\gl{\kirxvi}
\bmng
\bnum
\num{1} Iga; sadayxdalilx; I hotitxnalilx; I hotitxge. 
\num{2} I -- kaSxNa, kUDaleV; tatfkaSxNa: \eng{I must go home now} I kaSxNa nAnu manege hoVgabeVku. 
\num{3} ukatx kAladalilx; heVLida -- hotitxnalilx, samayadalilx. 
\num{4} I yA A -- hotitxge, kAlakekx! \eng{it was now clear} A hotitxge adu sapxSaTxvAgitutx. 
\num{5} Iga; parxkaqta saMdaBaRdalilx: \eng{I cannot now agree} parxkaqta saMdaBaRdalilx nAnu opapxlu sAdhayxvilalx. 
\num{6} I muMdina saninxveVshadalilx, parisithxtiyalilx: \eng{I cannot now ever believe you again} inunx eMdigU nAnu ninanxnunx naMbalAre. 
\num{7} I hiMde; savxlapx hiMde; Iga tAne: \eng{just now} Iga tAne; savxlapx hotitxna hiMde. 
\num{8} (\kanmu\ kathanadalilx yA saMBASaNeyalilx) Aga; taruvAya; AmeVle; naMtara; anaMtara: \eng{Caeser now marched east} Aga siVsaZrf pUvaR dikikxna kaDe muMduvarida. \eng{now let us consider the second point} AmeVle eraDaneV viSayavanunx parishiVlisoVNa. 
\num{9} (kAlavanunx sUcisade AkeSxVpaNe, beVDike, bedarike, vivaraNe, samajAyiSi, \mo\ saMdaBaRgaLalilx takakxMte dhavxnikoTuTx \parx) ililx keVLi; nAnu -- beVDutetxVne, otAtxyisutetxVne, ecacxrike koDutitxdedxVne; AdarU idanunx tiLidirabeVku; idanunx opipxkoLaLxbeVku; KaMDitavAgi (eMba athaRgaLalilx \parx): \eng{now what do you mean by it?} hAgeMdare, Enu aMta niVnu heVLoVdu? \eng{now then don't cry} hoVgali aLabeVDa, sumamxniru. \eng{now, now behave yourself}! noVDu, noVDu, Iga sariyAgi naDeduko! 
\num{10} inunx -- meVle, muMde: \eng{well what do you want now?} sari, inunx muMde ninageVnAgabeVku? 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{but now} = \hyperlink{nowpagu2}{?pagu?\((2)\)}. 
\hypertarget{nowpagu2}{} 
\num{2} \eng{even now} IgaSeTxV. 
\num{3} \eng{every now and again} = \hyperlink{nowpagu5}{?pagu?\((5)\)}. 
\num{4} \eng{how now?}(\pArxparx) Enu viSaya? idarathaRveVnu? 
\hypertarget{nowpagu5}{} 
\num{5} \eng{now and then} AgAga; matetx matetx; padeV padeV; punaH punaH. 
\num{6} \eng{now for it}! idiVga takakx samaya. 
\hyperdef{N}{now(1) pagu(7)}{} 
\hypertarget{now(1) pagu(7)}{} 
\num{7} \eng{now or never} Iga biTaTxre hoVyitu. 
\enum
\emng
\eentry 

\bentry
\word[now(2)]{now}
\pron{nw}
\gl{\saMavayx}
\bmng
AdakAraNadiMda; AdadxriMda; AdudariMda: \eng{now that you mention it, I do remember} niVnu adanunx parxsAtxpisidadxriMda, nanage KaMDita nenapAgutatxde. 
\emng
\eentry

\bentry
\word[now(3)]{now}
\pron{nw}
\gl{\nA}
\bmng
Iga; I saMdaBaR: \eng{till now} iduvarege; ililxyavarege. \eng{by now} sadayxdalilx; I -- hotitxge, veVLege. \eng{should be there by now} I hotitxge alilxrabeVku. 
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{as of now} I hotitxge iruvaMte. 
\num{2} \eng{for now} sadayxkekx; muMdinavaregU; muMde savxlapxkAladavaregU: \eng{goodbye for now} sadayxkekx (muMde punaH nAvu seVruva kAladavarege) namasAkxra; punaH BeVTiyAgoVNa. 
\enum
\emng
\eentry

\bentry
\word[nowadays(1)]{nowadays}
\pron{nwaDeVsfZ}
\gl{\kirxvi}
\bmng
iMdu; Igina kAladalilx; I Adhunika yugadalilx. 
\emng
\eentry

\bentry
\word[nowadays(2)]{nowadays}
\pron{nwaDeVsfZ}
\gl{\nA}
\bmng
iMdina, Igina -- kAla; Adhunika yuga. 
\emng
\eentry

\bentry
\word{noway}
\pron{noVveV}
\gl{\kirxvi}
\bmng
= \hyperlink{nowise}{nowise}. 
\emng
\eentry

\bentry
\word{Nowel}
\pron{noVelf}
\gl{\nA}
\bmng
(\pArxparx) \eng{Noel} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{Nowell}
\pron{noVelf}
\gl{\nA}
\bmng
= \hyperlink{Nowel}{Nowel}. 
\emng
\eentry

\bentry
\word[nowhere(1)]{nowhere}
\pron{noVveVrf}
\gl{\kirxvi}
\bmng
\bnum
\num{1} elUlx (ilalx) 
\num{2} elilxgU (ilalx). 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\hypertarget{nowwherepagu1}{} 
\bnum
\num{1} \eng{be nowhere}(paMdayxdalilx, sapxdheRyalilx) jayagaLisuva saMBavavilalxdiru. 
\hypertarget{nowwherepagu2}{} 
\num{2} \eng{come from nowhere} aniriVkiSxtavAgi kANisiko yA jayagaLisu. 
\num{3} \eng{come} (\engit{or} \eng{come in) nowhere} = \hyperlink{nowwherepagu1}{?pagu?\((1)\)}. 
\numi{4} \eng{get nowhere} 
\banum
\alnum{a} hiMde biVLu; muMduvariyadiru; ELige paDeyadiru. 
\alnum{b} (obabxnige) aBivaqdidhx hoMdalu parxyoVjanavAgadiru, sahAyavAgadiru. 
\eanum
\numie
\num{5} \eng{in the middle of nowhere} (\AmA) Uru, nagara parxdeVsha, \mo vugaLiMda dUravAgiruva. 
\num{6} \eng{nowhere near} hatitxraveVnU ilalx; hatitxradalelxlUlx ilalx. 
\enum
\emng
\eentry

\bentry
\word[nowhere(2)]{nowhere}
\pron{noVveVrf}
\gl{\sanA}
\bmng
ilalxda -- sathxLa, jAga, parxdeVsha; asitxtavxdalilxlalxda nADu. 
\emng
\eentry

\bentry
\word{nowise}
\pron{noVveYsfZ}
\gl{\kirxvi}
\bmng
yAvudeV vidhadalUlx ilalx; KaMDita ilalx. 
\emng
\eentry

\bentry
\word{nowt}
\pron{nwTf}
\gl{\nA}
\bmng
(\AmA\ yA \pArxM) EnU ilalx (\eng{nought} eMbudara rUpAMtara). 
\emng
\eentry

\bentry
\word{noxious}
\pron{nAkfSasf}
\gl{\gu}
\bmng
\bnum
\num{1} hAnikara; aniSaTxkAri; keDukumADuva; apAyakara. 
\num{2} anAroVgayxkara; AroVgayx keDisuva. 
\enum
\emng
\eentry

\bentry
\word{noxiously}
\pron{nAkfSasfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} hAnikaravAgi; keDuku mADuva riVtiyalilx; apAyakaravAgi. 
\num{2} AroVgayx keDisuvaMte. 
\enum
\emng
\eentry

\bentry
\word{noxiousness}
\pron{nAkfSasfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} hAnikAriyAgiruvike. 
\num{2} anAroVgayxkaravAgiruvudu. 
\enum
\emng
\eentry

\bentry
\wordf{noyade}
\pron{nAvxyADf}
\gl{\nA}
\expl{\F} 
\bmng
(\kanmu\ phArXnisxnalilx \eng{$1794$}ralilx naDesutitxdadxMte, janaranunx guMpu guMpAgi) niVrinalilx muLugisi kolulxva maraNadaMDane. 
\emng
\eentry

\bentry
\word{noyau}
\pron{nAvxyoV}
\gl{\nA}
\expl{(\bava\ \eng{noyaux} \ucAcx-- nAvxyoVsfZ).}
\bmng
(sihi suvAsanAdarxvayxgaLiMdalU haNiNxna tiruLiniMdalU rucikaTiTxda) bArxMdi madayx. 
\emng
\eentry

\bentry
\word{nozzle}
\pron{nAsaZ(sfZ)lf}
\gl{\nA}
\bmng
mUti; shuMDa; sUsu bAyi; niVru, anilagaLu, \mo vanunx odagisuva peYpina bAyige tagalisuva kiridAda bAyi uLaLx naLike. 
\emng
\eentry

\bentry
\wordnospeech{NP}{NP}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{Notary Public}. 
\emng
\eentry

\bentry
\wordnospeech{Np}{Np}
\pron{?}
\gl{\saMkeV}
\bmng
(\ravi) \eng{neptunium} dhAtu. 
\emng
\eentry

\bentry
\wordnospeech{n.p.}{n.p.}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{new paragraph}. 
\num{2} \eng{no place of publication}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NPA}{NPA}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{Newspaper Publishers' Association}. 
\emng
\eentry

\bentry
\wordnospeech{NPL}{NPL}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Physical Laboratory}. 
\emng
\eentry

\bentry
\wordnospeech{nr.}{nr.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{near}. 
\emng
\eentry

\bentry
\wordnospeech{NS}{NS}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{new style}. 
\num{2} \eng{new series}. 
\num{3} \eng{Nova Scotia}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NSB}{NSB}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Savings Bank}. 
\emng
\eentry

\bentry
\wordnospeech{NSC}{NSC}
\pron{?}
\gl{\saMkiSx}
\bmng
(amerikadalilx) \eng{National Security Council}. 
\emng
\eentry

\bentry
\wordnospeech{NSF}{NSF}
\pron{?}
\gl{\saMkiSx}
\bmng
(amerikadalilx) \eng{National Science Foundation}. 
\emng
\eentry

\bentry
\wordnospeech{NSPCC}{NSPCC}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Society for the Prevention of Cruelty to Children}. 
\emng
\eentry

\bentry
\wordnospeech{NSW}{NSW}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{New South Wales}. 
\emng
\eentry

\bentry
\wordnospeech{NT}{NT}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{New Testament}. 
\num{2} \eng{Northern Territory (of Australia)}. 
\num{3} \eng{no trumps}. 
\enum
\emng
\eentry

\bentry
\word{n't}
\pron{anfTf}
\gl{\kirxvi}
\bmng
(\saMpa gaLalilx) = \hyperlink{not}{not}(\sA\ \eng{is, are, have, must} joteyalilx hAgU sahAyaka \kirx gaLAda \eng{can, do should, would}gaLa jote \parx: \eng{isn`t, mustn`t}). 
\emng
\eentry

\bentry
\wordnospeech{Nth.}{Nth.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{North}. 
\emng
\eentry

\bentry
\word{nth}
\pron{?}
\bmng
noVDi \hyperlink{N, n}{N}. 
\emng
\eentry

\bentry
\wordnospeech{NTP}{NTP}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{normal temperature and pressure}. 
\emng
\eentry

\bentry
\word{nu}
\pron{nUyx}
\gl{\nA}
\bmng
girxVkf vaNaFmAleyalilx hadimUraneya akaSxra, (\eng{N}, \eng{$\nu$}). 
\emng
\eentry

\bentry
\word[nuance(1)]{nuance}
\pron{nUyx(nuyx)Anfsx}
\gl{\nA}
\bmng
(athaR, BAvane, aBipArxya, baNaNx, \mo vugaLalilx yA avugaLa CAyeyalilx) sUkaSxmX -- vayxtAyxsa, BeVda. 
\emng
\eentry

\bentry
\word[nuance(2)]{nuance}
\pron{nUyx(nuyx)Anfsx}
\gl{\sakirx}
\bmng
(athaR, baNaNx, \mo vugaLalilx) sUkaSxmX vayxtAyxsa mADu. 
\emng
\eentry

\bentry
\word{nub}
\pron{nabf}
\gl{\nA}
\bmng
\bnum
\num{1} (viSaya, kathe, \mo vugaLa) sArAMsha; muKAyxMsha; tiruLu. 
\num{2} (\kanmu\ kalilxdadxlina) gubuTu; uMDe; saNaNx tuMDu. 
\num{3} (katatxrisi) uLida cUru, moVTu, kULe. 
\enum
\emng
\eentry

\bentry
\word{nubbin}
\pron{nabinf}
\gl{\nA}
\bmng
(\ame) kuruTu bidadx, beLavaNigeyilalxda musukina joVLada tene, yA beVre kAyipalayx. 
\emng
\eentry

\bentry
\word{nubble}
\pron{naba(bf)lf}
\gl{\nA}
\bmng
saNaNx tuMDu yA gubuTu. 
\emng
\eentry

\bentry
\word{nubbly}
\pron{nabilx}
\gl{\gu}
\bmng
\bnum
\num{1} saNaNx saNaNx -- gubuTugaLiruva, kULegaLiruva, moVTugaLiruva. 
\num{2} uMDe yA gubuTugaLa AkAradalilxruva. 
\enum
\emng
\eentry

\bentry
\word{nubby}
\pron{nabi}
\gl{\gu}
\bmng
= \hyperlink{nubbly}{nubbly}. 
\emng
\eentry

\bentry
\word{nubile}
\pron{nUyxbeYlf}
\gl{\gu}
\bmng
(heMgasina \vi) 
\banum
\alnum{a} maduveyAgaluLaLx; vivAhayoVgayx; maduvege nereda. 
\alnum{b} leYMgika AkaSaRNeyuLaLx. 
\eanum
\emng
\eentry

\bentry
\word{nubility}
\pron{nUyxbiliTi}
\gl{\nA}
\bmng
(heMgasina \vi) 
\banum
\alnum{a} vivAhayoVgayxvAgiruvike; maduveyAgalu yoVgayxvAgiruva sithxti. 
\alnum{b} leYMgikAkaSaRkate; leYMgikAkaSaRNe hoMdiruvike. 
\eanum
\emng
\eentry

\bentry
\word{nuchal}
\pron{nUyxkalf}
\gl{\gu}
\bmng
heDagatitxna; hekakxtitxna; katitxna hiMBAgada. 
\emng
\eentry

\bentry
\word{nuci-}
\pron{nUyxsi-}
\gl{\sapUpa}
\bmng
kAyi-- (\eng{nuciferous} \mo\ shabadxgaLa racaneyalilx \parx). 
\emng
\eentry

\bentry
\word{nuciferous}
\pron{nUyxsipharasf}
\gl{\gu}
\bmng
(\savi) kAyi biDuva. 
\emng
\eentry

\bentry
\word{nucivorous}
\pron{nUyxsivarasf}
\gl{\gu}
\bmng
kAyi kasaru \mo vanunx tinunxva; kAyitini. 
\emng
\eentry

\bentry
\word{nuclear}
\pron{nUyxkilxyarf}
\gl{\gu}
\bmng
\bnum
\num{1} nABiya; nABisaMbaMdhavAda; nABiyAgiruva; nABiyaMtha. 
\numi{2} beYjika: 
\banum
\alnum{a} paramANu biVjada, adanunx baLasuva, adakekx saMbaMdhisida. 
\alnum{b} beYjika shakitxya, adakekx saMbaMdhisida. 
\alnum{c} beYjika asatxrXda, adakekx saMbaMdhisida. 
\eanum
\numie
\num{3} aNavxsatxrXgaLanunx hoMdiruva. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nuclear bomb}{nuclear bomb}
\pron{?}
\gl{\nA}
\bmng
beYjika bAMbu; nUyxkilxyarf bAMbu. 
\emng
\eentry

\bentry
\wordnospeech{nuclear disarmament}{nuclear disarmament}
\pron{?}
\gl{\nA}
\bmng
beYjika nishayxsitxrXVkaraNa; aNavxsatxrXgaLa vajaRne. 
\emng
\eentry

\bentry
\wordnospeech{nuclear energy}{nuclear energy}
\pron{?}
\gl{\nA}
\bmng
beYjika shakitx; paramANu biVjagaLu pAlogxLuLxva kirxyegaLalilx pAtarx vahisuva, \kanmu\ BAravAda biVjagaLa vidaLanadalilx yA hagura biVjagaLa samimxLanadalilx biDugaDeyAguva shakitx. 
\emng
\eentry

\bentry
\wordnospeech{nuclear family}{nuclear family}
\pron{?}
\gl{\nA}
\bmng
mUla kuTuMba; biDi saMsAra; taMde, tAyi, magu yA makakxLanunx mAtarx oLagoMDa kuTuMba. 
\emng
\eentry

\bentry
\wordnospeech{nuclear fission}{nuclear fission}
\pron{?}
\gl{\nA}
\bmng
biVjavidaLana; beYjika vidaLana; BAravAda kelavu paramANu biVjagaLu (\kanmu\ \eng{U-235}, \eng{Pu-239}), tamage tAveV yA nUyxTArxnanunx oLahogisikoMDudara pariNAmavAgi yA inonxMdu kaNada joteyalilx saMGaTaTxneya pariNAmavAgi ibABxgavAgi oDedu apAra shakitxyanUnx, biDi nUyxTArxnugaLanUnx utapxtitx mADuva beYjika kirxye. 
\emng
\eentry

\bentry
\wordnospeech{nuclear force}{nuclear force}
\pron{?}
\gl{\nA}
\bmng
beYjika bala; paramANu biVjada sithxratege kAraNavAgiruva, paramANu biVjada oLage porxVTAnf-porxVTAnfgaLa naDuve, nUyxTArxnf -- nUyxTArxnfgaLa naDuve iruva parxbala AkaSaRka bala. 
\emng
\eentry

\bentry
\word{nuclear-free}
\pron{nUyxkilxarfphirxV}
\gl{\gu}
\bmng
beYjika -- mukatx, rahita; beYjika asatxrXgaLu, shakitx, \mo vu ilalxda. 
\emng
\eentry

\bentry
\wordnospeech{nuclear fuel}{nuclear fuel}
\pron{?}
\gl{\nA}
\bmng
beYjika iMdhana; nUyxTArxnfniMda vidaLanagoMDu, saraNi kirxyeya mUlaka, shakitxyanunx horageDahabalalx yureVniyM -- \eng{$235$}, yA pUlxToVniyaM -- \eng{$239$}naMtha paramANu biVja. 
\emng
\eentry

\bentry
\wordnospeech{nuclear fusion}{nuclear fusion}
\pron{?}
\gl{\nA}
\bmng
biVjasamimxLana; beYjika samimxLana; haguravAda paramANu biVjagaLu (\kanmu\ heYDorxjanf, DuyxTiVriyamf) oMdugUDi hiVliyamf biVjadaMtha doDaDx biVjavanunx rUpisi, apAravAda shakitxyanunx utapxtitx mADuva beYjika kirxye. 
\emng
\eentry

\bentry
\wordnospeech{nuclear magnetic resonance}{nuclear magnetic resonance}
\pron{?}
\gl{\nA}
\bmng
beYjika kAMtiVya anuraNana; kAMtiVya mahatavxviruva paramANu biVjavu bAhayxkAMtiVya keSxVtarxdalilxruvAga viduyxtAkxMtiVya vikiraNavanunx hiVrikoLuLxva parxkirxye(vishelxVSaNa taMtarxdalilx hAgU roVganidAnakAkxgi deVha samiVkeSxyalilx \kanmu\ upayoVga), \saMkiSx\ \eng{NMR, nmr}. 
\emng
\eentry

\bentry
\wordnospeech{nuclear physics}{nuclear physics}
\pron{?}
\gl{\nA}
\bmng
beYjika BwtavijAcnxna. 
\emng
\eentry

\bentry
\wordnospeech{nuclear power}{nuclear power}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} beYjika viduyxcaCxkitx; beYjika kirxyAkAri \eng{(reactor)}yiMda utapxtitx mADuva viduyxcaCxkitx yA cAlana shakitx. 
\num{2} beYjika rASaTxrX; beYjika shasAtxrXsatxrXgaLanunx saMgarxhisiTuTxkoMDiruva rASaTxrX. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nuclear powered}{nuclear powered}
\pron{?}
\gl{\gu}
\bmng
(\kanmu\ haDagina \vi) beYjika shakitxyanunx baLasuva yA shakitxyiMda calisuva. 
\emng
\eentry

\bentry
\wordnospeech{nuclear reactor}{nuclear reactor}
\pron{?}
\gl{\nA}
\bmng
beYjika kirxyAkAri; beYjika riyAkaTxru; shakitxyanunx utApxdisuva saluvAgi, beYjika vidaLanagaLa saraNiyanunx niraMtaravAgiruvaMte matutx niyaMtarxNadalilxruvaMte iTuTxkoMDu naDesalu nimiRsuva sAdhana. 
\emng
\eentry

\bentry
\wordnospeech{nuclear umbrella}{nuclear umbrella}
\pron{?}
\gl{\nA}
\bmng
beYjika -- Catirx, kavaca, rakeSx; beYjikAsatxrXgaLiruva deVshadoDane meYtirx beLesuvudara pariNAmavAgi dorakuvudenanxlAda rakaSxNe. 
\emng
\eentry

\bentry
\wordnospeech{nuclear warfare}{nuclear warfare}
\pron{?}
\gl{\nA}
\bmng
beYjika yudadhx; aNavxsatxrX samara; beYjika shasAtxrXsatxrXgaLanunx baLasuva yudadhx. 
\emng
\eentry

\bentry
\wordnospeech{nuclear waste}{nuclear waste}
\pron{?}
\gl{\nA}
\bmng
beYjika kasa, nimARlayx; beYjika sAthxvaragaLa nivaRhaNeyalilx utapxtitxyAguva, vikiraNapaTuvAgiruva tAyxjayx padAthaR. 
\emng
\eentry

\bentry
\wordnospeech{nuclear winter}{nuclear winter}
\pron{?}
\gl{\nA}
\bmng
beYjika -- shishira, caLigAla; beYjika samarada pariNAmavAgi sUyaR rashimxgaLige taDeyuMTAguvudariMda BUmiya meVle kANisikoLuLxvudeMdu heVLalAda tiVvarx caLi. 
\emng
\eentry

\bentry
\word{nuclease}
\pron{nUyxkilxEsfZ}
\gl{\nA}
\bmng
nUyxkilxyeVsfZ; nUyxkilxyikf AmalxgaLa jalaviBajanege neravAguva enfseZYmf. 
\emng
\eentry

\bentry
\word[nucleate(1)]{nucleate}
\pron{nUyxkilxETf}
\gl{\gu}
\bmng
nUyxkilxyasf iruva; biVja uLaLx. 
\emng
\eentry

\bentry
\word[nucleate(2)]{nucleate}
\pron{nUyxkilxETf}
\gl{\sakirx}
\bmng
nUkilxVkarisu; biVjiVkarisu. 
\emng

\noindent
\gl{\akirx}
\bmng
nUyxkilxyasf Agi rUpugoLuLx; biVjiVBavisu. 
\emng
\eentry

\bentry
\word{nucleation}
\pron{nUyxkilxESa(Sf)nf}
\gl{\nA}
\bmng
\bnum
\num{1} nUyxkilxVBavana; biVjiVBavana; nUyxkilxyasf Agi rUpugoLuLxvudu. 
\num{2} nUyxkilxVkaraNa; biVjiVkaraNa; nUyxkilxyasAsxgi mADuvudu. 
\enum
\emng
\eentry

\bentry
\word{nuclei}
\pron{nUyxkilxai}
\gl{\nA}
\bmng
\eng{nucleus} padada \bava. 
\emng
\eentry

\bentry
\wordRemoveSpace{nucleic-acid}{nucleic acid}
\pron{nUyxkilxV(kelxV)ikf AYxsiDf}
\gl{\nA}
\bmng
nUyxkelxyikf Amalx; elalx jiVvakoVshagaLalilxyU kaMDubaruva, aneVka nUyxkilxyoTeYDf aNugaLu oMdaroDanoMdu baMdhisikoMDu rUpisida udadxvAda sarapaLiyaMtiruva eraDu saMkiVNaR kAbaRnika aNugaLAda Dienfe athavA Arfenfe gaLalilx yAvudAdaroMdu. 
\emng
\eentry

\bentry
\word{nucleo-}
\pron{nUyxkilxO-}
\gl{\sapUpa}
\bmng
nUyxkilxyasf, nUyxkelxVikf Amalx, eMbathaRgaLa \sapUpa: \eng{nucleo-protein.} 
\emng
\eentry

\bentry
\word{nucleolar}
\pron{nUyxkilxOlarf}
\gl{\gu}
\bmng
nUyxkilxyoVlasfna yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nucleolus}
\pron{nUyxkilxValasf, nUyxkilxOlasf}
\gl{\nA}
\expl{(\bava\ \eng{nucleoli} \ucAcx-- nUyxkilxVOleY).}
\bmng
nUyxkilxyoVlasf; viBajisalAgada nUyxkilxyasfnalilxruva, parxdhAnavAgi porxVTiVniniMda Agiruva, goVLAkArada, oMdu cikakx sAMdarx kAya. 
\emng
\eentry

\bentry
\word{nucleon}
\pron{nUyxkilxAnf}
\gl{\nA}
\bmng
(\Bwvi) nUyxkilxyAnf; paramANu biVjadalilxruva porxVTAnf matutx nUyxTArxnfgaLa peYki oMdu. 
\emng
\eentry

\bentry
\word{nucleonic}
\pron{nUyxkilxAnikf}
\gl{\gu}
\bmng
\bnum
\num{1} nUyxkilxyAnfgaLa yA avakekx saMbaMdhisida. 
\num{2} nUyxkilxyAnikfsxge saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{nucleonics}
\pron{nUyxkilxAnikfsx}
\gl{\nA}
\bmng
(\bava) (\Eva vAgi \parx) nUyxkilxyAnikfsx; \kanmu\ nUyxkilxyarf shakitx yA paramANu biVjashakitxya baLakege saMbaMdhisidaMte paramANu biVjagaLu, avugaLalilxruva nUyxkilxyAnfgaLanunx kurita, vijAcnxna matutx taMtarxvideyxya shAKe. 
\emng
\eentry

\bentry
\word{nucleoprotein}
\pron{nUyxkilxOporxVTiVnf}
\gl{\nA}
\bmng
nUyxkilxyoporxVTiVnf; nUyxkilxyikf Amalx matutx porxVTiVnugaLa saMyoVgadiMda uMTAguva saMkiVNaR saMyukatx. 
\emng
\eentry

\bentry
\word{nucleoside}
\pron{nUyxkilxaseYDf}
\gl{\nA}
\bmng
(\jiVra) puyxriVnf athavA pirimiDiVnf parxtAyxmalxda aNuvu reYboVsf athavA DiVAkisxreYboVsf aNuvinoMdige baMdhisikoMDAga Aguva saMyukatx; nUyxkilxyoseYDf. 
\emng
\eentry

\bentry
\word{nucleotide}
\pron{nUyxkilxaTeYDf}
\gl{\nA}
\bmng
(\jiVra) nUyxkilxyoTeYDf; nUyxkilxyoseYDf matutx phAsAphxrikf AmalxgaLa saMyoVgadiMda dorakuva nUyxkelxyikf Amalxda aNuvina GaTaka. 
\emng
\eentry

\bentry
\word{nucleus}
\pron{nUyxkilxasf}
\gl{\nA}
\expl{(\bava\ \eng{nuclei} \ucAcx-- nUyxkilxai).}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} nABi; keVMdarx; guMba; itara BAgagaLu yA vasutxgaLu sutatxlU kUDikoLuLxva yA saMgarxhavAguva madhayxBAga. 
\alnum{b} (yAvudeV saMGAtada yA darxvayxrAshiya) madhayxBAga; keVMdarxBAga; tiruLu. 
\eanum
\numie
\num{2} biVja; mUlAMsha; hosa seVpaRDegaLanunx seVrisikoLuLxva pArxraMBika aMsha. 
\num{3} (\Kavi) shira; tale; dhUmakeVtuvina taleya GanaBAga. 
\num{4} (\Bwvi) biVja; paramANuvina rAshiya bahuBAgakekx kAraNavAda, paramANuvina tiruLinaMtiruva dhana viduyxdAviSaTx kaNa. 
\num{5} (\jiVvi) biVjakaNa; jiVvakoVshadalilx AnuvaMshika darxvayxvanunx oLagoMDiruva, yUkariyATikf koVshagaLa doDaDx sAMdarx kaNa vayxvasethx \eng{(organelle).} 
\num{6} (\aMrashA) nUyxkilxyasf; keVMdarx nara maMDaladalilxruva parxteyxVka bUdu darxvayxrAshi. 
\num{7} (\Bwvi) biVja; teVvadiMda payARpatxvAgiruva vAyuvinalilx darxva hani rUpugoLaLxlu yA payARpatx dArxvaNadalilx saPxTika rUpugoLaLxlu neravAguva vilaMbita dhULina kaNa itAyxdi. 
\enum
\emng
\eentry

\bentry
\word{nuclide}
\pron{nUyxkelxYDf}
\gl{\nA}
\bmng
(\Bwvi) nUyxkelxYDf; nidiRSaTx saMKeyxya porxVTAnugaLU nidiRSaTx saMKeyxya nUyxTArxnugaLU nidiRSaTx parxmANada shakitxyU iruvaMtha biVja uLaLx paramANu jAti. 
\emng
\eentry

\bentry
\word{nuclidic}
\pron{nUyxkilxDikf}
\gl{\gu}
\bmng
(\Bwvi) nUyxkelxYDfna yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word[nude(1)]{nude}
\pron{nUyxDf}
\gl{\gu}
\bmng
\bnum
\num{1} betatxle(yAda); naganx; nivARNa; nivaRsatxrX; vivasatxrX; vasatxrXrahita; baTeTx toTiTxlalxda; bariya meYyina. 
\num{2} (baNaNxda \vi) mAMsavaNaRda. 
\enum
\emng
\eentry

\bentry
\word[nude(2)]{nude}
\pron{nUyxDf}
\gl{\nA}
\bmng
\bnum
\num{1} naganx citarx yA shilapx; betatxle mAnava vayxkitxya vaNaRcitarx, shilapx, CAyAcitarx, \mo vu. 
\num{2} nagAnxkaqti; vayxkitxya betatxlerUpa. 
\num{3} naganx, betatxle -- vayxkitx. 
\hypertarget{nude(2)4}{} 
\numi{4} 
\banum
\alnum{a} naganxte; nagAnxvasethx; betatxle sithxti. 
\alnum{b} naganx citarxNa; kalAparxkAravAgi, vasatxrX rahita mAnavAkaqtiya rUpaNa, citarxNa. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{the nude} = \hyperlink{nude(2)4}{$^2$nude}\eng{(4 a, b)}. 
\emng
\eentry

\bentry
\wordnospeech{nude contract}{nude contract}
\pron{?}
\gl{\nA}
\bmng
(\nAyxshA) boVLu opapxMda; KAli karAru; parxtiPalavilalxdiruva, AdadxriMda adhikAramuderxyilalxde UjiRtavAgada opapxMda. 
\emng
\eentry

\bentry
\word[nudge(1)]{nudge}
\pron{najf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (itararige tiLiyadaMte gamana seLeyuvudakAkxgi) moLakeYyiMda metatxge tivi. 
\num{2} (nidhAnavAgi) muMdakekx nUku, taLuLx. 
\numi{3} (obabx vayxkitxge) swmayxvAgi, nayavAgi 
\banum
\alnum{a} jAcnxpisu; nenapu koDu. 
\alnum{b} huriduMbisu; utetxVjana koDu; porxVtAsxhisu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[nudge(2)]{nudge}
\pron{najf}
\gl{\nA}
\bmng
\bnum
\num{1} (gamana seLeyuva) moLakeY tivita. 
\num{2} melalxge -- taLuLxvudu, nUkuvudu. 
\num{3} utetxVjane. 
\enum
\emng
\eentry

\bentry
\word{nudger}
\pron{najarf}
\gl{\nA}
\bmng
\bnum
\num{1} moLakeYyiMda, moNakeYyiMda -- metatxge tiviyuvavanu. 
\num{2} hAge tividu jAcnxpisuvavanu yA utetxVjisuvavanu. 
\num{3} metatxge taLuLxvavanu, nUkuvavanu. 
\enum
\emng
\eentry

\bentry
\word{nudi-}
\pron{nU(nUyx)Di-}
\gl{\sapUpa}
\bmng
(\pArxvi dalilx) naganx, betatxleyAda eMbathaRda saMyukatx padagaLa racaneyalilx baLasuva \pUparx: \eng{nudi-brand} cipipxlalxda oMdu jAtiya samudarxda basavanahuLu. 
\emng
\eentry

\bentry
\word{nudism}
\pron{nUyxDisfZmf}
\gl{\nA}
\bmng
naganx -- paMtha, vAda, sidAdhxMta; naganxteya samathaRne. 
\emng
\eentry

\bentry
\word{nudist}
\pron{nUyxDisfTx}
\gl{\nA}
\bmng
naganxpaMthi; naganxvAgiruvudanunx yA naganxteyanunx samathiRsuva vayxkitx. 
\emng
\eentry

\bentry
\word{nudity}
\pron{nUyxDiTi}
\gl{\nA}
\bmng
naganxte; betatxleyAgiruvike. 
\emng
\eentry

\bentry
\word{nugatory}
\pron{nUyxgaTari}
\gl{\gu}
\bmng
\bnum
\num{1} kelasakekx bArada; vayxthaR; niSaTxrXyoVjaka; nirathaRka. 
\num{2} UjiRtavAgada; siMdhuvAgada; kAyaRsAdhakavalalxda; kAyaRkAriyAgada. 
\enum
\emng
\eentry

\bentry
\word{nugget}
\pron{nagiTf}
\gl{\nA}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (sAvxBAvikavAgi BUmiyalilx doreyuva cinanx, pAlxYXTinaM, \mo vugaLa) oraTu gaTiTx. 
\alnum{b} aMtha yAvudeV gaTiTx. 
\eanum
\numie
\num{2} (\rUpa) atayxmUlayx vasutx; gAtarxkAkxgi tuMbA belebALuva vasutx: \eng{a little nugget of information} sudidxya oMdu cikakx gaTiTx. 
\enum
\emng
\eentry

\bentry
\word{nuisance}
\pron{nUyxsa(sf)nfsx}
\gl{\nA}
\bmng
\bnum
\num{1} aniSaTx; piVDe; (toMdare, keVDu, yA kirikiri yA asahayx uMTumADuva) vayxkitx, vasutx yA saMdaBaR. 
\num{2} (nAyxya parihAra paDeyabahudAda) heVsike; aniSaTx; asahayx; galiVju; piVDe; oMdu janavagaRkAkxgali, yAvanigobabxnigAgali keVDuMTumADuva yA aniSaTxkaravAda toMdare. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{nuisance value} asahayx huTiTxsuva, piVDe koDuva, aDiDxyuMTumADuva sAmathayxRda meVle yA mUlaka odaguva anukUla, lABa. 
\numi{2} \eng{what a nuisance}! 
\banum
\alnum{a} eMtha asahayx! 
\alnum{b} eMtha toMdare! 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\wordnospeech{NUJ}{NUJ}
\pron{?}
\gl{\saMkiSx}
\bmng
(\birx) \eng{National Union of Journalists.} 
\emng
\eentry

\bentry
\word[nuke(1)]{nuke}
\pron{nUyxkf}
\gl{\nA}
\bmng
(\AmA) aNubAMbu; paramANu asatxrX. 
\emng
\eentry

\bentry
\word[nuke(2)]{nuke}
\pron{nUyxkf}
\gl{\gu}
\bmng
aNubAMbu dALi mADu; beYjika asatxrXgaLiMda nAshamADu. 
\emng
\eentry

\bentry
\word[null(1)]{null}
\pron{nalf}
\gl{\gu}
\bmng
\bnum
\num{1} (\kanmu\ \eng{null and void} eMdu \parx) baMdhakavalalxda; UjiRtavAgada. 
\num{2} asitxtavxvilalxda; vasutxtavxvilalxda; shUnayxpArxyavAda. 
\num{3} shUnayxmwlayxda; sonenxya. 
\num{4} lakaSxNa rahita yA BAvarahita; yAvudeV guNa lakaSxNa yA BAva ilalxda. 
\num{5} nagaNayx; beleyilalxda. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{null and void} = \hyperlink{null(1)}{$^1$null\((1)\)}. 
\emng
\eentry

\bentry
\word[null(2)]{null}
\pron{nalf}
\gl{\nA}
\bmng
shUnayx; (saMkeVta lipiyalilx) athaRkAkxgi koTiTxlalxda, AkAramAtarxda akaSxra \eng{(O).} 
\emng
\eentry

\bentry
\word{nulla}
\pron{nala}
\gl{\nA}
\bmng
(\AseTxrXV) mUlanivAsigaLu baLasutitxdadx, gaTiTxmarada doNeNx, laTaTx. 
\emng
\eentry

\bentry
\wordf{nulla bona}
\pron{nala boVna}
\gl{\nA}
\expl{\Latin}
\bmng
jaPitx mADalu (kakiSxgArana) sAmAnu yAvudU ilalxveMdu SariVphf kaLuhisuva varadi. 
\emng
\eentry

\bentry
\word{nullah}
\pron{nala}
\gl{\nA}
\bmng
(\AMiM) nAle; tore; korakalu; hariyuva haLaLx. 
\emng
\eentry

\bentry
\word{nulla-nulla}
\pron{nala nala}
\gl{\nA}
\bmng
= \hyperlink{nulla}{nulla}. 
\emng
\eentry

\bentry
\wordnospeech{null character}{null character}
\pron{?}
\gl{\nA}
\bmng
(gaNana) shUnayxcihenx; EnanUnx sUcisada cihenx (\kanmu\ adanunx sonenxyiMda nirUpisalAguvudu). 
\emng
\eentry

\bentry
\wordnospeech{null hypothesis}{null hypothesis}
\pron{?}
\gl{\nA}
\bmng
(\saMshA) shUnayx parxtijecnx; saMKAyxshAsitxrXVya mAdarigaLalilx kaMDubarabahudAda vayxtAyxsadiMda, jana saMdaNigaLalilxyU vayxtAyxsavide eMdu tiVmARnisuvudu sariyalalx eMba Uhana. 
\emng
\eentry

\bentry
\word{nullification}
\pron{naliphikeVSanf}
\gl{\nA}
\bmng
radadxti; radidxyAti; toDedu hAkuvike; vayxthaRgoLisuvudu. 
\emng
\eentry

\bentry
\word[nullifidian(1)]{nullifidian}
\pron{naliphiDianf}
\gl{\gu}
\bmng
nAsitxka(teya); matasaMbaMdhavAda yAvudeV naMbike yA sharxdedhx ilalxda. 
\emng
\eentry

\bentry
\word[nullifidian(2)]{nullifidian}
\pron{naliphiDianf}
\gl{\nA}
\bmng
dhamaRlaMDa; nAsitxka; matasaMbaMdhavAda naMbike yA sharxdedhx ilalxdavanu. 
\emng
\eentry

\bentry
\word{nullify}
\pron{nalipheY}
\gl{\sakirx}
\expl{(vataRmAna kAla \parxpu\ \Eva\ \eng{nullifies}, \BU\ matutx \BUkaq\ \eng{nullified}).}
\bmng
ilalxdAgisu; ilalxdaMte mADu; toDedu hAku; radudx mADu; niSapxrXBAvagoLisu; vayxthaRvAgisu; nirathaRkagoLisu; pariNAma shUnayxvAguvaMte mADu. 
\emng
\eentry


\bentry
\wordnospeech{null instrument}{null instrument}
\pron{?}
\gl{\nA}
\bmng
shUnoyxVpakaraNa; sonenx hatAyxra; sonenx mAhiti baruvaMte aLavaDisuva mUlaka UpayoVgisabahudAda yAvudeV upakaraNa, sAdhana. 
\emng
\eentry

\bentry
\word{nullipara}
\pron{nalipara}
\gl{\nA}
\bmng
aparxsave; oMdu maguvanUnx herada heMgasu. 
\emng
\eentry

\bentry
\word{nulliparous}
\pron{naliparasf}
\gl{\gu}
\bmng
aparxsaveyAda; oMdu maguvanUnx herada. 
\emng
\eentry

\bentry
\word{nullipore}
\pron{nalipoVrf}
\gl{\nA}
\bmng
(\jiVvi) nalipoVrf; havaLadaMte kAyxlisxyaM kAboRneVTanunx osaruva samudarx sasayx. 
\emng
\eentry

\bentry
\word{nullity}
\pron{naliTi}
\gl{\nA}
\expl{(\bava\ \eng{nullities}).}
\bmng
\bnum
\numi{1} (\nAyxshA) 
\banum
\alnum{a} (\kanmu\ maduveya) anUjiRtate; UjiRtavAgadiruvike. 
\alnum{b} anUjiRta -- kAyaR, karxma, dasetxYvaju, \mo vu. 
\eanum
\numie
\num{2} ilalxme; shUnayxte; ilalxdiruvudu; nAsitxtavx; asitxtavxshUnayxte. 
\num{3} shudadhx shUnayxvAdudu. 
\num{4} gaNanege bAradudx; yaHkashicxtAtxda vasutx, vicAra, itAyxdi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{null link}{null link}
\pron{?}
\gl{\nA}
\bmng
(gaNana) shUnayx -- saMpakaR, koMDi; paTiTxyalilx inunx beVre bAbugaLu ilalx eMbudanunx toVrisalu koneya bAbinalilx sUcisuva ulelxVKa. 
\emng
\eentry

\bentry
\wordnospeech{NUM}{NUM}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Union of Mineworkers.} 
\emng
\eentry

\bentry
\wordnospeech{Num.}{Num.}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{Numbers}(haLeya oDaMbaDike). 
\emng
\eentry

\bentry
\word[numb(1)]{numb}
\pron{namf}
\gl{\gu}
\bmng
nishecxVSiTxta; joVvu hiDida; jaDavAda; maragaTiTxda; satxMBiVBUtavAda; saMveVdane yA calanashakitx tapipxhoVda: \eng{numb with cold} caLiyiMda maragaTiTxda. 
\emng
\eentry

\bentry
\word[numb(2)]{numb}
\pron{namf}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} maragaTiTxsu; joVvu hiDisu; saMveVdane ilalxdaMte mADu. 
\num{2} satxbadhxgoLisu; nishecxVSiTxtagoLisu; satxMBiVBUtagoLisu. 
\enum
\emng
\eentry

\bentry
\word{numbat}
\pron{naMbAYxTf}
\gl{\nA}
\bmng
(\AseTxrXV) naMbATf; podarupodarAda bAlavU, beninxna meVle kapupx matutx biLi paTeTxgaLU, hoTeTxyalilx ciVlavU iruva, mamiRkoVbiyasf phAYxSiyeVTasf kulakekx seVrida, oMdu saNaNx sasatxni pArxNi. 
\emng
\eentry

\bentry
\word[number(1)]{number}
\pron{naMbarf}
\gl{\nA}
\bmng
\bnum
\num{1} aMki; saMKeyx: \eng{six is an even number} Aru eMbudu samasaMKeyx. 
\numi{2} aMki; saMKeyx: 
\banum
\alnum{a} aMkiyanunx sUcisuva shabadx, saMkeVta yA citarx. 
\alnum{b} gurutu, parAmashaRne, \mo vugaLigAgi, oMdu gotAtxda motatxda sherxVNiyalilx yAvudAdarU oMdanunx nideRVshisuvAga adakekx [\udA\ vAhanagaLu, dUravANi, \mo vakekx] koDuva saMKeyx, aMki: \eng{registration number} noVMdaNi saMKeyx. 
\eanum
\numie
\num{3} (janara, vasutxgaLa oTuTx motatx sUcisuva) aMki; saMKeyx: \eng{what number of people are you expecting?} niVnu oTuTx eSuTx saMKeyxya janaranunx niriVkiSxsutitxdidxVye? \eng{the number of accidents has decreased} apaGAtagaLa saMKeyx kaDameyAgide. \eng{twenty in number} (saMKeyxyalilx) ipapxtitxve. 
\num{4} (\bava dalilx) gaNita; aMkagaNita: \eng{not good at numbers} gaNitadalilx kushalanalalx. 
\num{5} (\Eva\ yA \bava dalilx) parxmANa; motatx: \eng{a large number of people} BAri motatxda janaru. \eng{only in small numbers} kaDame parxmANadalilx mAtarx. 
\num{6} aMki; saMKeyx; oMdu sherxVNiyalilx iMtiSaTxneyadu eMdu sUcisuva cihenx (\kanmu\ vaqtatx patirxkeya iMtha saMcike, giVtanATakadalilx yA saMgiVta kaceVriyalilx iMtiSaTxneya hADu, \mo vu). 
\num{7} jate; jote; guMpu; vagaR: \eng{he is of our number} avanu namamx guMpinavanu. \eng{among our number} namamx guMpinalilx. 
\num{8} (\vAyx) vacana: \eng{Sanskrit has three numbers} saMsakxqqta BASeyalilx mUru vacanagaLive. 
\num{9} (\AmA) (saligeyiMda yA pirxVtiyiMda vayxkitxyanunx yA vasutxvanunx kareyuvAga) naMbaru; isamu: \eng{an attractive little number} AkaSaRkavAda puTaTx vayxkitx, vasutx. 
\num{10} \eng{(Numbers)}(\beY) saMKAyxkAMDa; haLeya oDaMbaDikeyalilx janagaNatiyiruva nAlakxneya adhAyxya. 
\num{11} oMdu gotAtxda saMKeyxyavaregiruvudaralilx nidiRSaTxvAgi yAvudAdarU oMdu vasutx, vayxkitx, \mo vu: \eng{number 22 wants his boots} ipapxtetxraDaneya isamu, vayxkitx bUTugaLu beVkenunxtAtxne. 
\num{12} (\AmA) vayxkitx, uDupu, kelasa, \mo vu. 
\num{13} (\bava dalilx) saMKAyxbala; saMKeyxya hecacxLa: \eng{they won by force of numbers} avaru keVvala saMKAyxbaladiMda gedadxru. 
\num{14} eNike. 
\num{15} (\pArxparx) tALa; laya. 
\numi{16} (\bava dalilx) 
\banum
\alnum{a} (\saM) savxrataMDa. 
\alnum{b} (\CaM) gaNagaLu. 
\alnum{c} (padayxda) sAlugaLu; caraNagaLu; paMkitxgaLu. 
\alnum{d} padayxgaLu; kAvayx: \eng{I lisped in numbers for the numbers came} nAnu todalu mAtanADutitxruvaMteyeV kAvayx raciside; yAkeMdare kAvayx nananxlilx tAneV tAnAgi suPxrisitu. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\numi{1} \eng{a number of} 
\banum
\alnum{a} kelavu. 
\alnum{b} halavAru; aneVka (\bava da kirxyApadadoDane baLake Iga sAmAnayx): \eng{a number of problems remain} halavAru samaseyxgaLu hAgeyeV uLidive. 
\eanum
\numie
\num{2} \eng{by numbers} saMKAyxnusAra; saMKeyxgaLiMda sUcitavAda saraLa nideRVshagaLanunx anusarisi. 
\num{3} \eng{have a person's number}(\ashi) obabxna nijavAda savxBAva, udedxVsha, \mo vanunx tiLiduko, athaRmADiko. 
\num{4} \eng{have a person's number on it}(bAMbf guMDu, \mo vugaLa \vi) nidiRSaTx vayxkitxyanunx hoDeyuvaMte niyatavAgiru. 
\num{5} \eng{in number} eNikeyalilx; eNisidalilx; saMKeyxyalilx: \eng{one people exceeds another in number} oMdu janAMga inonxMdanunx saMKeyxyalilx miVrisutatxde. 
\num{6} \eng{in numbers}(kathe \mo\ parxkaTaNegaLa \vi) anukarxmavAgi baruva saMcikegaLalilx; saMKAyxnukarxmada BAgagaLalilx. 
\num{7} \eng{in small number} kaDime saMKeyxyalilx; viraLa parxmANadalilx. 
\num{8} \eng{make one's number with}(\ashi) (obabxna jote) saMpakaR hoMdu; obabxnanunx BeVTiyAgu, kANu, saMpakiRsu. 
\num{9} \eng{numbers game} (\hiV) keVvala aMkagaNitada (lekAkxcAra)vanonxLagoMDa kelasa. 
\num{10} \eng{Number Ten} hatutx; DwniMgf siTxrXVTf; iMgelxMDina parxdhAniya adhikaqta laMDanf nivAsa. 
\num{11} \eng{number two} eraDaneya sAthxnadalilxruva; divxtiVya sAthxnada. 
\num{12} \eng{one's number is up}(\AmA) avanu sAyutAtxne; avana kathe mugiyutatxde. 
\num{13} \eng{out of number} = \hyperlink{numberpagu16}{?pagu?\((16)\)}. 
\num{14} \eng{round numbers} (cilalxre saMKeyx biTuTx baruva) pUNARMkagaLu; pUNaR saMKeyxgaLu (\udA\ \eng{10, 20, 100,} \mo vu). 
\num{15} \eng{to the number of}(yAvudoV) aMkiyavarege; saMKeyxyavarege: \eng{to the number of 80} eMbatatxravarege. 
\hypertarget{numberpagu16}{} 
\num{16} \eng{without number} asaMKAyxta; agaNita; lekakxvilalxdaSuTx. 
\enum
\emng
\eentry

\bentry
\word[number(2)]{number}
\pron{naMbarf}
\gl{\sakirx}
\bmng
\bnum
\num{1} (oMdu vagaRdalilx) seVrisu; seVridedxMdu eNisu: \eng{I number you among my friends} nAnu ninanxnunx nananx senxVhitaralilx seVrisidedxVne, niVnu seVrididxVyeMdu eNisutetxVne. 
\num{2} (yAvudakAkxdarU) oMdu saMKeyx yA saMKeyxgaLanunx koDu; oMdu saMKeyxyiMda nideRVshisu. 
\num{3} (oMdu nidiRSaTx saMKeyxyaSuTx) motatxvAgu; (aSuTx) saMKeyxyalilxru: \eng{the crew and passengers numbered thirty-three} haDaginalilx sibabxMdi parxyANikaru seVri mUvatatxmUru maMdi idadxru. 
\num{4} eNisu; lekakx mADu; saMKeyxyanunx gotutxmADu. 
\num{5} oLagoLuLx; oLagoMDiru. 
\num{6} (\kaparx dalilx) saMKeyxyalilx -- parimitavAgiru, siVmitagoLuLx. 
\num{7} (iSuTx vaSaR) tuMbiru; badukiru; jiVvisu: \eng{of as able body as when he numbered thirty} avanu mUvatutx vaSaRdavanAgidAdxga idadxMte daqDhakAya. 
\num{8} (iMtiSuTx saMKeyxya nivAsigaLu, vasutxgaLu, \mo vugaLiMda) kUDiru; tuMbiru: \eng{Mysore numbers eight lakh inhabitants} meYsUru eMTu lakaSx nivAsigaLiMda tuMbide. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{one's days are numbered}(obabxna) dinagaLu mugiyutatx baMdavu; hecucx dina badukuvudilalx. 
\emng
\eentry

\bentry
\wordnospeech{number cruncher}{number cruncher}
\pron{?}
\gl{\nA}
\bmng
(kaMpUyxTarf matutx gaNita) (\ashi) saMkiVNaR gaNaka(yaMtarx); saMkiVNaR lekAkxcAragaLanunx mADuva yaMtarx. 
\emng
\eentry

\bentry
\wordnospeech{number crunching}{number crunching}
\pron{?}
\gl{\nA}
\bmng
(\ashi) saMkiVNaR lekAkxcAragaLanunx mADuvudu yA mADuva kirxye. 
\emng
\eentry

\bentry
\word{numberless}
\pron{naMbarflisf}
\gl{\gu}
\bmng
asaMKAyxta; lekakxvilalxdaSaTxra; eNikege sigada. 
\emng
\eentry

\bentry
\word(number one[1]){number one}
\pron{}
\gl{\nA}
\bmng
(\AmA) tAnu: \eng{always takes care of number one} avanu yAvAgalU tananx yoVgakeSxVma noVDikoLuLxtAtxne; yAvAgalU avanu tananx bagegx ecacxrikeyiMda noVDikoLuLxtAtxne. 
\emng
\eentry

\bentry
\word(number one[2]){number one}
\pron{}
\gl{\gu}
\bmng
(sAthxna yA pArxmuKayxdalilx) modalaneyadAda; atuyxcacx sAthxnada; atuyxtakxqqSaTx; parxthama; Adayx. 
\emng
\eentry

\bentry
\word{number-plate}
\pron{naMbarf pelxVTf}
\gl{\nA}
\bmng
naMbarf pelxVTf; aMkipaTiTx; saMKAyxPalaka; vAhanagaLige hacucxva, rijisaTxrf yA noVMdaNi saMKeyx iruva Palaka. 
\emng
\eentry

\bentry
\word{numb-fish}
\pron{namfphiSf}
\gl{\nA}
\bmng
= \hyperref{kandict_e.pdf}{E}{electric ray}{electric ray}. 
\emng
\eentry

\bentry
\word{numbles}
\pron{naMba(bf)lfsx}
\gl{\nA}
\bmng
(\bava) (\birx) (\pArxparx) (AhAravAgi upayoVgisuva) jiMkeya karuLu, aMtarx. 
\emng
\eentry

\bentry
\word{numbly}
\pron{namfli}
\gl{\kirxvi}
\bmng
nishecxVSaTxvAgi; joVvu hiDidaMte; jaDavAgi; maragaTiTxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{numbness}
\pron{namfnisf}
\gl{\nA}
\bmng
nishecxVSaTxte; joVvu hiMDidaMtiruvike; maragaTiTxdaMtiruvike. 
\emng
\eentry


\bentry
\word{numbskull}
\pron{namfsakxlf}
\gl{\nA}
\bmng
\hyperlink{numskull}{numskull} padada rUpAMtara. 
\emng
\eentry

\bentry
\word{numdah}
\pron{namfDA}
\gl{\nA}
\bmng
(iMDiyA \mo\ kaDegaLiMda baruva) kasUtiya phelfTx kaMbaLi, ragugx. 
\emng
\eentry

\bentry
\word{numen}
\pron{nUyxmenf}
\gl{\nA}
\expl{(\bava\ \eng{numina} \ucAcx-- nUyxmina).}
\bmng
adhideVvate; aBimAni deVvate; sathxLadeVvate yA aBimAni deVvate; yAvudeV sathxLadalilx yA vasutxvinalilx iruva aBimAni deVvate yA ceYtanayx. 
\emng
\eentry

\bentry
\word{numerable}
\pron{nUyxmaraba(bf)lf}
\gl{\gu}
\bmng
eNisalAguva; eNisabalalx; eNisabahudAda. 
\emng
\eentry

\bentry
\word{numerably}
\pron{nUyxmarabilx}
\gl{\kirxvi}
\bmng
eNisalAguvaMte. 
\emng
\eentry

\bentry
\word{numeracy}
\pron{nUyxmarasi}
\gl{\nA}
\bmng
gaNitajacnxte; gaNitada mUla sUtarxgaLanunx tiLidukoMDiruvudu. 
\emng
\eentry

\bentry
\word[numeral(1)]{numeral}
\pron{nUyxmara(rf)lf}
\gl{\nA}
\bmng
aMki; aMka; saMKAyxvAcaka; saMKeyxyanunx sUcisalu baLasuva saMkeVtagaLalolxMdu (shabadx, citarx yA citarxgaLa guMpu, \mo vu). 
\emng
\eentry

\bentry
\word[numeral(2)]{numeral}
\pron{nUyxmara(rf)lf}
\gl{\gu}
\bmng
\bnum
\num{1} saMKAyxvAcaka; saMKeyxyanunx sUcisuva. 
\num{2} saMKeyxya; aMkiya. 
\enum
\emng
\eentry

\bentry
\word[numerate(1)]{numerate}
\pron{nUyxmaraTf}
\gl{\gu}
\bmng
gaNitajacnx(nAda); gaNitada mUlasUtarxgaLanunx arita, tiLidukoMDiruva. 
\emng
\eentry

\bentry
\word[numerate(2)]{numerate}
\pron{nUyxmareVTf}
\gl{\sakirx}
\bmng
\bnum
\num{1} eNisu; gaNisu. 
\num{2} aMkigaLanunx padagaLanunx bare yA padagaLAgi Odu; aMkigaLalilxruvudanunx padarUpadalilx bare yA Odu. 
\enum
\emng
\eentry

\bentry
\word{numeration}
\pron{nUyxmareVSa(Sf)nf}
\gl{\nA}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} eNike; gaNane; eNisuva vidhAna yA parxkirxye. 
\alnum{b} lekAkxcAra; gaNana; eNike (mADuvudu). 
\eanum
\numie
\num{2} (aMkagaNita) aMkaleVKana; saMKAyxleVKana; aMkigaLalilx barediruvudanunx padagaLAgi, padagaLa rUpadalilx Oduvudu yA bareyuvudu. 
\num{3} aMkana; vasutx, vayxkitx, \mo varige saMKeyxgaLanunx koDuvudu yA koDuva vidhAna. 
\enum
\emng
\eentry

\bentry
\wordnospeech{numeration table}{numeration table}
\pron{?}
\gl{\nA}
\bmng
aMkanakoVSaTxka; aMkana padadhxtiyalilx aMkiya sAthxnakekx anuguNavAda adara beleyanunx toVrisuva koVSaTxka. 
\emng
\eentry

\bentry
\word{numerator}
\pron{nUyxmareVTarf}
\gl{\nA}
\bmng
\bnum
\num{1} (\ga) aMsha; BinanxrAshiyalilx aDaDxgereya meVlina saMKeyx (\udA\ \eng{$\frac\bg 2\eg\bg 3\eg$}ralilx \eng{2}). 
\num{2} gaNaka; eNisuvavanu. 
\num{3} aMkanakAra; saMKAyxkAra; saMKeyxyanunx gurutumADuva vayxkitx yA sAdhana. 
\enum
\emng
\eentry

\bentry
\word{numeric}
\pron{nUyxmerikf}
\gl{\gu}
\bmng
= \hyperlink{numerical}{numerical}. 
\emng
\eentry

\bentry
\word{numerical}
\pron{nUyxmerika(kf)lf}
\gl{\gu}
\bmng
saMKAyxtamxka; saMKeyxgaLalilx vayxkatxpaDisida; saMKeyxya yA saMKeyxge, saMKeyxgaLige saMbaMdhisida. 
\emng
\eentry

\bentry
\wordnospeech{numerical analysis}{numerical analysis}
\pron{?}
\gl{\nA}
\bmng
(\ga) saMKAyxtamxka vishelxVSaNe; samaseyxgaLa parihArakekx saMKAyxtamxka vidhAnagaLanunx rUpisuva matutx baLasuva gaNitashAKe. 
\emng
\eentry

\bentry
\word{numerically}
\pron{nUyxmerikali}
\gl{\kirxvi}
\bmng
saMKAyxtamxkavAgi; saMKeyxgaLalilx vayxkatxpaDisuvaMte. 
\emng
\eentry

\bentry
\word{numerological}
\pron{nUyxmaralAjika(kf)lf}
\gl{\gu}
\bmng
\bnum
\num{1} saMKAyxrahasayxshAsatxrXkekx saMbaMdhisida. 
\num{2} saMKAyxBaviSayxda. 
\enum
\emng
\eentry

\bentry
\word{numerologist}
\pron{nUyxmarAlajisfTx}
\gl{\nA}
\bmng
\bnum
\num{1} saMKAyxrahasayxshAsatxrXjacnx. 
\num{2} saMKAyxjoyxVtiSi; saMKAyxBaviSayxvAdi. 
\enum
\emng
\eentry

\bentry
\word{numerology}
\pron{nUyxmarAlaji}
\gl{\nA}
\expl{(\bava\ \eng{numerologies}).}
\bmng
\bnum
\num{1} saMKAyx BaviSayx(jAcnxna); saMKeyxya mUlaka kaNi heVLuvudu. 
\num{2} saMKAyx rahasayx(shAsatxrX); saMKeyxgaLa nigUDha athaRvanunx kurita shAsatxrX. 
\enum
\emng
\eentry

\bentry
\word{numerous}
\pron{nUyxmarasf}
\gl{\gu}
\bmng
\bnum
\num{1} (\bava da jotege) bahaLa; aneVka: \eng{he received numerous gifts} avanige aneVka uDugoregaLu doretavu. 
\num{2} bahusaMKAyxka; aneVka GaTaka yA aMshagaLuLaLx: \eng{a numerous library} bahu pusatxkagaLuLaLx garxMthaBaMDAra. 
\numi{3} 
\banum
\alnum{a} (\pArxparx) aneVkariMda -- baruva, \eng{the numerous voices of the people} bahu kaMThagaLa dhavxni, vANi. 
\alnum{b} kikikxrida; tuMbida; janaBarita: \eng{ numerous gathering} kikikxrida saBe. 
\eanum
\numie
\num{4} (padayx yA gadayxda \vi) layabadadhx; sushArxvayx: \eng{numerous verse} sushArxvayx kAvayx. 
\enum
\emng
\eentry

\bentry
\word{numerously}
\pron{nUyxmarasfli}
\gl{\kirxvi}
\bmng
bahaLavAgi; aneVkavAgi; bahusaMKeyxyiMda kUDi. 
\emng
\eentry

\bentry
\word{numerousness}
\pron{nUyxmarasfnisf}
\gl{\nA}
\bmng
bAhuLayx; bahusaMKeyxyiMda kUDiruvike. 
\emng
\eentry

\bentry
\word{numina}
\pron{nUyxmina}
\gl{\nA}
\bmng
\hyperlink{numen}{numen} eMbudara \bava. 
\emng
\eentry

\bentry
\word{numinous}
\pron{nUyxminasf}
\gl{\gu}
\bmng
\bnum
\num{1} deYvika; deYva perxVrita. 
\num{2} AdhAyxtimxka; pAramAthiRka; ati mAnuSa. 
\num{3} gaMBiVravAda; BayaBakitx huTiTxsuva. 
\num{4} (yAvudeV sathxLada yA vasutxvina) adhideVvateya; aBimAni deVvateya yA aBimAni deVvateyaMtha. 
\num{5} jAgarxta; deVvatA sAninxdhayxviruva. 
\enum
\emng
\eentry

\bentry
\word{numismatic}
\pron{nUyxmisAZmxYxTikf}
\gl{\gu}
\bmng
\bnum
\num{1} nANayxgaLa; nANayx racaneya; TaMka viSayaka. 
\num{2} nANayx saMbaMdhi; nANayxgaLa yA avugaLaMtiruva padakagaLige saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{numismatically}
\pron{nUyxmisAZmxYxTikali}
\gl{\kirxvi}
\bmng
nANayxgaLige saMbaMdhisidaMte; nANayxracanege seVrida hAge. 
\emng
\eentry

\bentry
\word{numismatics}
\pron{nUyxmisAZmxYxTikfsx}
\gl{\nA}
\bmng
(\bava) (\sA\ \Eva vAgi parigaNisalAgide) nANayxvideyx; nANayxshAsatxrX; nANayxgaLa yA padakagaLa adhayxyana. 
\emng
\eentry

\bentry
\word{numismatist}
\pron{nUyxmisaZmxTisfTx}
\gl{\nA}
\bmng
\bnum
\num{1} nANayx pariNata; nANayxshAsatxrXjacnx; nANayxpaMDita. 
\num{2} nANayx saMgArxhaka; nANayx saMgarxhakAra, haLeya nANayxgaLanunx yA padakagaLanunx saMgarxhisuvavanu. 
\enum
\emng
\eentry

\bentry
\word{numismatology}
\pron{nUyxmisazmXTAlaji}
\gl{\nA}
\bmng
= \hyperlink{numismatics}{numismatics}. 
\emng
\eentry

\bentry
\word{nummary}
\pron{namari}
\gl{\gu}
\bmng
nANayxda: 
\banum
\alnum{a} nANayx saMbaMdhi; nANayxkekx (nANayxgaLige) saMbaMdhisida. 
\alnum{b} nANayx vayxvahArada; nANayxdalilx vayxvaharisuva. 
\eanum
\emng
\eentry

\bentry
\word{nummulite}
\pron{namuyxleYTf}
\gl{\nA}
\bmng
namuyxleYTf; TaSiRyari satxradalilx sikukxva, taTeTxyAkArada, porxVToVseZ\char'261VvagaLiMda rUpugoMDa, raMdharxgaLuLaLx phAsilu cipupx. 
\emng
\eentry

\bentry
\word{numnah}
\pron{namanx}
\gl{\nA}
\bmng
jiVnu -- baTeTx, metetx; kudureya jiVnina taLagaDe hAkuva metetx. 
\emng
\eentry

\bentry
\word{numskull}
\pron{namfsakxlf}
\gl{\nA}
\bmng
\bnum
\num{1} daDaDx; gugugx. 
\num{2} daDaDxna tale. 
\enum
\emng
\eentry

\bentry
\word{nun}
\pron{nAnf}
\gl{\nA}
\bmng
\bnum
\num{1} kerxYsatx saMnAyxsini; baDatana, barxhamxcayeR, vidheVyate, \mo vanunx pAlisutatx caciRna maThadalilx vAsisuva saMnAyxsini. 
\numi{2} nanf: 
\banum
\alnum{a} niVli TiTf hakikx. 
\alnum{b} sUmxyX \eng{(smew)}; miVnu hiDiyuva oMdu jAtiya bAtu. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nun's cloth} (\engit{or} \eng{veiling})(uDupige upayoVgisuva) teLaLxneya uNeNxya baTeTx. 
\emng
\eentry

\bentry
\word{nunatak}
\pron{nanaTAYxkf}
\gl{\nA}
\bmng
(\kanmu\ girxVnfleMDf \mo\ kaDegaLalilx) himanelada meVle edudx niMta baMDeya tudi yA koVDugalulx. 
\emng
\eentry

\bentry
\word{nun-buoy}
\pron{nanfbAyf}
\gl{\nA}
\bmng
madhayxvaqtatx pAlxvaka; naDuve vaqtAtxkAravAgidudx eraDu tudigaLa kaDegU cUpAgutAtx hoVguva teVluve; duMDumadhayxda teVluve. 
\emng
\eentry

\bentry
\wordf{nunc dimittis}
\pron{naMkf DimiTisf}
\gl{\nA}
\expl{\Latin}
\bmng
\bnum
\num{1} (beYbalilxna lUyxkf suvAteRya \eng{2:29-32}ralilx baruva) ``naMkf DimiTisf" eMdu pArxraMBavAguva, sotxVtarxdaMte baLasuva, seYmananxna deVvatA pArxthaRne. 
\num{2} nigaRmanAnujecnx; horaDabahudeMba apapxNe. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\engit{sing nunc dimittis} paraloVka yAterxge manaHpUvaRka sidadhxvAgiru. 
\emng
\eentry

\bentry
\word{nunciature}
\pron{nanaSxTuyxarf}
\gl{\nA}
\bmng
\bnum
\num{1} (\roVkAyx, cacfR) poVpf guruvina niyoVgi yA rAyaBAri hudedx yA padavi. 
\num{2} paradeVshadalilx poVpf guruvanunx parxtinidhisuva rAyaBAriya hudedxya avadhi. 
\enum
\emng
\eentry

\bentry
\word{nuncio}
\pron{naniSx(nisx)O}
\gl{\nA}
\expl{(\bava\ \eng{nuncios}).}
\bmng
(beVroMdu deVshadalilx) poVpf guruvina rAyaBAri yA niyoVgi. 
\emng
\eentry

\bentry
\word{nuncupate}
\pron{nanfkUyxpeVTf}
\gl{\sakirx}
\bmng
uyilanunx (baravaNigeyalilxDade) bAyalelxV daqDhiVkarisi heVLu, tiLiyapaDisu. 
\emng
\eentry

\bentry
\word{nuncupation}
\pron{nanfkUyxpeVSanf}
\gl{\nA}
\bmng
(uyilanunx) bAyalelxV daqDhiVkarisi tiLisuvudu. 
\emng
\eentry

\bentry
\word{nuncupative}
\pron{nanfkUyxpaTivf}
\gl{\gu}
\bmng
(uyilanunx) bAyalelxV daqDhiVkarisi heVLida. 
\emng
\eentry

\bentry
\word{nunhood}
\pron{nanfhuDf}
\gl{\nA}
\bmng
(kerxYsatx) saMnAyxsinitana. 
\emng
\eentry

\bentry
\word{nunlike}
\pron{nanfleYkf}
\gl{\gu}
\bmng
(kerxYsatx) saMnAyxsiniyaMtha; saMnAyxsiniya riVtiya. 
\emng
\eentry

\bentry
\word{nunnery}
\pron{nanari}
\gl{\nA}
\expl{(\bava\ \eng{nunneries}).}
\bmng
(kerxYsatx) saMnAyxsiniyara maTha, nivAsa. 
\emng
\eentry

\bentry
\word{nunnish}
\pron{naniSf}
\gl{\gu}
\bmng
= \hyperlink{nunlike}{nunlike}. 
\emng
\eentry


\bentry
\word{nunship}
\pron{nanfSipf}
\gl{\nA}
\bmng
= \hyperlink{nunhood}{nunhood}. 
\emng
\eentry

\bentry
\word{NUPE}
\pron{nUyxpi}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Union of Public Employees}. 
\emng
\eentry

\bentry
\word[nuptial(1)]{nuptial}
\pron{napaSx(pfSx)lf}
\gl{\gu}
\bmng
maduveya; vivAhada; vivAha saMbaMdhada. 
\emng
\eentry

\bentry
\word[nuptial(2)]{nuptial}
\pron{napaSx(pfSx)lf}
\gl{\nA}
\bmng
(\sA\ \bava dalilx \parx) maduve; vivAha. 
\emng
\eentry

\bentry
\wordnospeech{NUR}{NUR}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Union of Railwaymen}. 
\emng
\eentry

\bentry
\word{nurd}
\pron{naDfR}
\bmng
\eng{nerd} padada rUpAMtara. 
\emng
\eentry

\bentry
\word[nurse(1)]{nurse}
\pron{nasfR}
\gl{\nA}
\bmng
\bnum
\num{1} dAdi; Aya; roVgigaLa, ashakatxra upacAradalilx veYdayxrige neravAguva, tarabeVtu paDediruva vayxkitx. 
\num{2} dhAtirx makakxLa dAdi; cikakx makakxLa yoVgakeSxVma mAtarx noVDikoLuLxva dAdi. 
\num{3} (\pArxparx) satxnayx dAdi; moleyUDuva dAdi; beVreyavara maguvige moleyUDisi AreYke mADalu neVmitaLAda heMgasu (= \hyperref{kandict_w.pdf}{W}{wet-nurse(1)}{wetnurse}). 
\num{4} (araNayx vijAcnxna) poVSaka, Asharxya -- vaqkaSx; itara sasayxgaLige Asharxya niVDuvudakAkxgiyeV beLesida mara. 
\num{5} (\pArxvi) poVSaka kiVTa; kelasagAra kiVTa; kiVTada marigaLanunx poVSisuva, leYMgika doVSaviruva jeVnunoNa, kiVTa, itAyxdi. 
\num{6} poVSaka; oMdu vishiSaTx guNa \mo vanunx beLesuva deVsha \mo vu: \eng{the nurd of liberty} sAvxtaMtarxyXda poVSaka. 
\num{7} poVSaNe; AreYke. 
\num{8} moleyUDuvudu yA moleyUDisalapxDuvudu. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{at nurd} dAdiya poVSaNeyalilx, AreYkeyalilx. 
\numi{2} \eng{put out to nurd} 
\banum
\alnum{a} (maguvina \vi) dAdiya vashakekx koDu; dAdiya poVSaNege, AreYkege -- opipxsu. 
\alnum{b} (\rUpa) (Asitx \mo vugaLa \vi) Asitxyanunx pAlisuvavara vashakekx opipxsu, koDu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[nurse(2)]{nurse}
\pron{nasfR}
\gl{\sakirx}
\bmng
\bnum
\num{1} (roVgige) shushUrxSe, AreYke -- mADu. 
\num{2} (kAyile, gAyavAda deVhada aMga, \mo vanunx) vAsimADikoLaLxlu yatinxsu. 
\num{3} (maguvige) moleyuNisu; satxnayx koDu; moleyUDisuva dAdiyAgiru. 
\num{4} (\kaparx dalilx) (oMdu nidiRSaTx sithxtiyalilx) beLasalAgiru: \eng{nursed in pomp and pleasure} suKasaMpatitxnalilx beLasida. \eng{nursed in poverty} baDatanadalilx beLasida. 
\num{5} (ecacxrikeyiMda yA pirxVtiyiMda) hiDiduko yA apipxkoMDu mudAdxDu: \eng{sat nursing my feet} nananx kAlugaLanunx apipxkoMDu kuLita(Lu). 
\num{6} (sasayxgaLu, kalegaLu, \mo vanunx) beLasu; poVSisu; aBivadhiRsu; aBivaqdidhxpaDisu. 
\num{7} (koragu, devxVSa, \mo vanunx) beLasu; poVSisu: \eng{to nurse a grudge} manasisxnalilx jididxTuTxko. 
\num{8} ecacxrikeyiMda noVDiko; gamanaviTuTx noVDiko; visheVSa gamana koDu: \eng{nurse the voters} matakeSxVtarxda vishAvxsavanunx hiDidiTuTxko; matadAraranunx sadA OleYsutAtx, upacarisutAtx (tananxlilx) vishAvxsavuLaLxvaranAnxgi mADiko, iTuTxko. \eng{ to nurse the borough cost him Rs. 50,000/- a year} tananx keSxVtarxda matadAraranunx vishAvxsadalilxTuTxkoLaLxlu avanige parxtivaSaR \eng{50,000/-} rUpAyigaLa KacuR tagulitu. 
\num{9} (bilayxDf\char144R) ATagArana ceMDiniMda oMdAda meVloMdu ceMDu hoDeyalAguvaMte (ceMDugaLanunx) karxmadalilx seVrisi iTiTxru. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} roVgigaLa dAdiyAgiru; roVgigaLa dAdiyAgi kelasa mADu. 
\num{2} moleyUDisu; moleyuNiNxsu. 
\num{3} poVSaNa dAdiyAgiru; makakxLa usutxvAri vahisu. 
\enum
\emng
\eentry

\bentry
\word[nurse(3)]{nurse}
\pron{nasfR}
\gl{\nA}
\bmng
halavu bageya doDaDx doDaDx SAkfR(miVnu) jAti. 
\emng
\eentry

\bentry
\word{nurse-child}
\pron{nasfRceYlfDx}
\gl{\nA}
\bmng
sAkumagu; sAkida magu. 
\emng
\eentry

\bentry
\word{nurse-frog}
\pron{nasfRphArxgf}
\gl{\nA}
\bmng
poVSaka -- kapepx, maMDUka; inUnx mariyAgadiruva moTeTxgaLanunx horuva, oMdu jAtiya gaMDu kapepx. 
\emng
\eentry

\bentry
\word{nurse-hound}
\pron{nasfRhwMDf}
\gl{\nA}
\bmng
cikakx SAkfR miVnu. 
\emng
\eentry

\bentry
\word{nurseling}
\pron{nasfRliMgf}
\gl{\nA}
\bmng
= \hyperlink{nursling}{nursling}. 
\emng
\eentry

\bentry
\word{nursemaid}
\pron{nasfRmeVDf}
\gl{\nA}
\bmng
\bnum
\num{1} dAdi; maguvanunx yA makakxLanunx noVDikoLuLxva heMgasu. 
\num{2} (\rUpa) poVSaka; inonxbabxranunx ecacxrikeyiMda, AdaradiMda noVDikoLuLxva vayxkitx. 
\enum
\emng
\eentry

\bentry
\word{nursery}
\pron{nasaRri}
\gl{\nA}
\expl{(\bava\ \eng{nurseries}).}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} kUsukoVNe; makakxLigU avara dAdiyarigU gotutxmADiruva koVNe. 
\alnum{b} = \hyperref{kandict_d.pdf}{D}{day nursery}{day nursery}. 
\eanum
\numie
\num{2} nasaRri; sasitoVTa; sasoyxVdAyxna; \kanmu\ mArATakAkxgi yA beVre kaDe neDuvudakAkxgi giDagaLanunx yA sasayxgaLanunx beLesi sAkuva sathxLa. 
\num{3} poVSaNasAthxna, maMdira; kelavu vishiSaTx guNagaLanAnxgali, kelavu vagaRda janaranAnxgali poVSisuva, beLesuva -- padadhxti, saMsethx, keSxVtarx, sathxLa. 
\numi{4} (bilayxDf\char144R ATa) 
\banum
\alnum{a} ceMDuguMpu; ATagArana ceMDiniMda oMdAda meVloMdanunx hoDeyalu karxmavAgi seVrisiTuTxkoMDa ceMDugaLa guMpu. 
\hypertarget{nursery(4b)}{} 
\alnum{b} hatitxra hatitxraviruva mUru ceMDugaLa baDita. 
\eanum
\numie
\num{5} miVnu sAkuva koLa. 
\num{6} pArxNivadhaRna keVMdarx; pArxNigaLanunx aBivaqdidhxgoLisuva sathxLa. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nursery cannon} = \hyperlink{nursery(4b)}{nursery}(4b). 
\emng
\eentry

\bentry
\word{nurseryman}
\pron{nasaRrimanf}
\gl{\nA}
\expl{(\bava\ \eng{nurserymen}).}
\bmng
\bnum
\num{1} sasitoVTada yA sasoyxVdAyxnada -- mAli, kelasagAra. 
\num{2} sasi toVTada mAliVka; sasigaLa mArATagAra. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nursery nurse}{nursery nurse}
\pron{?}
\gl{\nA}
\bmng
makakxLa dAdi; shishu dAdi; cikakx makakxLa AreYke mADalu tarabeVti paDeda dAdi. 
\emng
\eentry

\bentry
\wordnospeech{nursery rhyme}{nursery rhyme}
\pron{?}
\gl{\nA}
\bmng
shishu -- pArxsa, giVte; makakxLa hADu; makakxLigoVsakxra pArxsabadadhxvAgi racisida saraLa sAMparxdAyika hADu yA kathana kavana. 
\emng
\eentry

\bentry
\wordnospeech{nursery school}{nursery school}
\pron{?}
\gl{\nA}
\bmng
bAlavana; bAlavADi; shishuvihAra; nasaRri shAle; mUrariMda aidu vaSaRdoLagina makakxLa shAle. 
\emng
\eentry

\bentry
\wordnospeech{nursery slopes}{nursery slopes}
\pron{?}
\gl{\nA}
\bmng
(sikxVyiMgf ATa) tarabeVti iLijAru; hosadAgi kaliyuvavarigAgi sidadhxpaDisida iLijAru. 
\emng
\eentry

\bentry
\wordnospeech{nursery stakes}{nursery stakes}
\pron{?}
\gl{\nA}
\bmng
kudure mari paMdayx; eraDu vaSaR vayasisxna kuduregaLa reVsu, (OTada) paMdayx. 
\emng
\eentry

\bentry
\word{nursing}
\pron{nasiRMgf}
\gl{\nA}
\bmng
\bnum
\num{1} AreYke, poVSaNe -- mADuvudu; roVgigaLanunx dAdiyAgi noVDikoLuLxvudu yA hAge shushUrxSe mADuva vaqtitx. 
\num{2} moleyUDisi AreYke mADuvudu. 
\num{3} yoVgakeSxVma noVDikoLuLxvike. 
\num{4} sAki salahuvike. 
\num{5} beLasi opapx mADuvudu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nursing father}{nursing father}
\pron{?}
\gl{\nA}
\bmng
sAkutaMde. 
\emng
\eentry

\bentry
\wordnospeech{nursing home}{nursing home}
\pron{?}
\gl{\nA}
\bmng
nasiRMgf hoVM; shushUrxSAgaqha; roVgoVpacAra gaqha; shasatxrXcikitesx, roVgigaLa upacAra, \mo vugaLige iTiTxruva KAsagi Asapxterx. 
\emng
\eentry

\bentry
\wordnospeech{nursing mother}{nursing mother}
\pron{?}
\gl{\nA}
\bmng
\bnum
\num{1} sAkutAyi. 
\num{2} poVSaka dAdi; moleyUDisi AreYke mADuva dAdi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nursing officer}{nursing officer}
\pron{?}
\gl{\nA}
\bmng
hiriya dAdi, nasfR. 
\emng
\eentry

\bentry
\word{nursling}
\pron{nasfRliMgf}
\gl{\nA}
\bmng
(\kanmu\ adakekx moleyuNisuva dAdiya saMbaMdhadalilx) kUsu; shishu. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nursling of} (vayxkitx yA vasutxvina \vi\ yAvudoV oMdu saMdaBaR, GaTane, vayxkitx, \mo variMda) beLesalAda; poVSisalAda. 
\emng
\eentry

\bentry
\word[nurture(1)]{nurture}
\pron{nacaRrf}
\gl{\nA}
\bmng
\bnum
\num{1} (\kanmu\ makakxLa) pAlane; AreYke; sAkuvudu, beLasuvudu yA tarabeVtu, shikaSxNa koDuvudu. 
\num{2} AreYke; poVSaNe; puSiTx koDuvaMtha AhAra. 
\num{3} sAmAjika -- parxBAva, pariNAma, vayxkitxtavxda meVle parxBAva biVruva yA vayxkitxtavxvanunx nidhaRrisuva sAmAjika aMshagaLu. 
\enum
\emng
\eentry

\bentry
\word[nurture(2)]{nurture}
\pron{nacaRrf}
\gl{\sakirx}
\bmng
\bnum
\num{1} sAku; beLasu, 
\num{2} poVSisu; pAlane, AreYke -- mADu. 
\num{3} tarabeVtu, shikaSxNa -- koDu. 
\enum
\emng
\eentry

\bentry
\word{nurturer}
\pron{nacaRrarf}
\gl{\nA}
\bmng
poVSaka; sAkuvava; poVSisuvava; AreYke mADuvava. 
\emng
\eentry

\bentry
\wordnospeech{NUS}{NUS}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) 
\banum
\alnum{a} \eng{National Union of Seamen}. 
\alnum{b} \eng{National Union of Students}. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{NUT}{NUT}
\pron{?}
\gl{\saMkiSx}
\bmng
(yuneYTeDf kiMgaDxmfnalilx) \eng{National Union of Teachers}. 
\emng
\eentry

\bentry
\word[nut(1)]{nut}
\pron{naTf}
\gl{\nA}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} karaTakAyi; gaDusAda yA metuvAda cipipxnoLagaDe AhArayoVgayx tiruLiruva kAyi, haNuNx. 
\alnum{b} I karaTa. 
\eanum
\numie
\num{2} gaTiTx biVjaviruva biVjakoVsha. 
\num{3} naTuTx; boVlaTxnunx hAki tirugisuvaMte, raMdharxdalilx tirupu gADigaLuLaLx, \sA\ cacwcxka yA SaDuBxjAkArada tirupu. 
\num{4} (\ashi) tale; talekAyi ; taleburuDe. 
\numi{5} (\ashi) 
\banum
\alnum{a} hucacx; tikakxla(manuSayx). 
\alnum{b} Bakatx; kaTATx aBimAni: perxVmi: \eng{an exercise nut} vAyxyAmada Bakatx, kaTATxBimAni. 
\eanum
\numie
\num{6} (kalilxdadxlu, beNeNx, \mo vugaLa) uMDe; mudedx; tuNuku; saNaNx tuMDu. 
\numi{7} 
\banum
\alnum{a} piTiVlu kamAnu \mo vugaLalilxna kudureya javi \mo vanunx bigisuva yA saDilisuva biraDe, hiDike. 
\alnum{b} (piTiVlu \mo\ taMtivAdayxgaLalilxna) kudure; taMtigaLu hAyuvaMte vAdayxda kaMThada tudiyalilx neleyAgi nililxsiruva kudure. 
\eanum
\numie
\num{8} (\bava dalilx) (ashilxVla \ashi) taraDu; taraDu biVja. 
\num{9} (\pArxparx) (\ashi) SoVkiVlAla; sogasugAra puTaTxsAmi. 
\enum
\emng

\noindent
\gl{\pagu}
\bmng
\bnum
\num{1} \eng{do one's nut}(\ashi) atiyAgi koVpagoMDiru, siDimiDigoMDiru. 
\num{2} \eng{for nuts} (\ashi) sumArAgi kUDa; alapxsavxlapx cenAnxgi saha: \eng{can't shoot for nuts} savxlapx cenAnxgi kUDa hoDeyalAra. 
\num{3} \eng{nuts and bolts}(\rUpa) oMdu kelasada vAyxvahArika vivaragaLu. 
\num{4} \eng{off one's nut} tale keTiTxru; hucucx hiDidiru. 
\enum
\emng
\eentry

\bentry
\word[nut(2)]{nut}
\pron{naTf}
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{nutted}, \vakaq\ \eng{nutting}).}
\bmng
karaTakAyigaLanunx -- huDuku, oTuTxmADu, Ayu, Arisu. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{to go nutting} kAyi Ayalu, arisalu hoVgu. 
\emng
\eentry

\bentry
\word{nutant}
\pron{nUyxTa(Tf)MTf}
\gl{\gu}
\bmng
(\savi) joVtu bididxruva; iLibididxruva; joVlADutitxruva, 
\emng
\eentry

\bentry
\word{nutation}
\pron{nUyxTeVSa(Sf)nf}
\gl{\nA}
\bmng
\bnum
\num{1} taledUguvike; taledUgaNe; taleya tUgATa. 
\num{2} (\Kavi) BUmayxkaSxvalana; BUvalana; BUmiya akaSxvu yA dhurxvagaLu sututxtitxruva buguriyaMte niyatakAlAnusAra OlADuvudu. 
\num{3} tirugutitxruva buguriya tUgATa, OlATa. 
\num{4} beLeyutitxruva sasayx BAga suruLiyAkAradalilx beLeyuvudu. 
\enum
\emng
\eentry

\bentry
\word{nut-brown}
\pron{naTfbwrxnf}
\gl{\gu}
\bmng
(\sA\ huDugiya meYbaNaNx, madayxda baNaNx, \mo vugaLa \vi) balita heVsaZlf kAyi baNaNxda; kaMdukAyi baNaNxda. 
\emng
\eentry

\bentry
\word{nut-butter}
\pron{naTfbaTarf}
\gl{\nA}
\bmng
kAyi beNeNx; kAyi \mo vugaLiMda tayArisida beNeNx \mo\ AhAra vasutxgaLu. 
\emng
\eentry

\bentry
\word{nutcase}
\pron{naTfkeVsf}
\gl{\nA}
\bmng
(\ashi) tikakxla; hucacx. 
\emng
\eentry

\bentry
\word{nutcracker}
\pron{naTfkArxYxkarf}
\gl{\nA}
\bmng
\bnum
\num{1} (\sA\ \bava dalilx) kAyikatatxri; aDike katatxri; aDike \mo vanunx katatxrisuva, siVLuva katatxri. \imglink{nutcracker 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.7cm height 0.5cm {N_Pictures/nutcracker-1.jpg}}} 
\num{2} (sAvxBAvikavAgiyoV, halilxlalxdadxriMdaloV) tudigaLu parasapxra hatitxraviruva, muMcAcida gadadx matutx mUgu. 
\enum
\emng
\eentry

\bentry
\word{nutgall}
\pron{naTfgAlf}
\gl{\nA}
\bmng
raMgugaMTu; raMgAri Okf marada meVle rUpugoLuLxva, vaNaR darxvayxvAgi baLasuva gaMTu. 
\emng
\eentry

\bentry
\word{nuthatch}
\pron{naTfhAYxcf}
\gl{\nA}
\bmng
kAyidini; mara hatitxkAyi, kiVTa, \mo vanunx tinunxva, siTiDeV vaMshakekx seVrida cikakx hakikx. \imglink{nuthatch:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.8cm height 0.5cm {N_Pictures/nuthatch.jpg}}} 
\emng
\eentry

\bentry
\word{nut-house}
\pron{naTfhwsf}
\gl{\nA}
\bmng
(\ashi) hucAcxsapxterx; mAnasika roVgigaLa Asapxterx yA shushUrxSAlaya. 
\emng
\eentry

\bentry
\word{nutlet}
\pron{naTfliTf}
\gl{\nA}
\bmng
saNaNx kAyi yA karaTa kAyiyaMtha haNuNx. 
\emng
\eentry

\bentry
\word{nutlike}
\pron{naTfleYkf}
\gl{\gu}
\bmng
naTfnaMtha; tirupinaMtha. 
\emng
\eentry

\bentry
\word{nut-meat}
\pron{naTfmiVTf}
\gl{\nA}
\bmng
(\ame) kAyi tiruLu; kAyiyoLagaNa tiruLu. 
\emng
\eentry

\bentry
\word{nutmeg}
\pron{naTfmegf}
\gl{\nA}
\bmng
\bnum
\num{1} jAyikAyi mara; jAkAyi mara; mirisiTxka phArxgarxnfsx kulakekx seVrida, gaTiTxyAda, goVLAkArada suvAsaneya biVjavanunx koDuva, iMDiyAda nitayxhasuru mara. 
\num{2} jAyikAyi; jAkAyi; sAMbAra padAthaRvAgiyU auSadhadalilxyU baLasuva idara biVja. 
\enum
\emng
\eentry

\bentry
\wordnospeech{nutmeg apple}{nutmeg apple}
\pron{?}
\gl{\nA}
\bmng
(jApaterxyU jAkAyiyU doreyuva) jAyikAyi haNuNx. 
\emng
\eentry

\bentry
\word{nut-oil}
\pron{naTfAilf}
\gl{\nA}
\bmng
kAyeNeNx; naTf eNeNx; heVsaZlfkAyi, vAlanxTf, akorxVTf, \mo\ cipupxkAyigaLiMda tegeda, raMgugaLu matutx vAniRSfgaLalilx baLasuva, oMdu bageya eNeNx. 
\emng
\eentry 

\bentry
\word{nut-palm}
\pron{naTfpAmf}
\gl{\nA}
\bmng
kAyitALe; AseTxrXVliyadalilx sikukxva, seYkaDeVsiV vaMshakekx seVrida, KAdayxPala koDuva, tALeyaMtha mara. 
\emng
\eentry

\bentry
\word{nut-pine}
\pron{naTfpeYnf}
\gl{\nA}
\bmng
kAyipeYnf(mara); tinanxlu yoVgayxvAda, karaTakAyiyaMtha biVjagaLiruva, \kanu\ amerikada peYnf mara. 
\emng
\eentry

\bentry
\word{nutria}
\pron{nUyxTirxa}
\gl{\nA}
\bmng
nUyxTirxya; \da\ amerikada kAyupx (\eng{coypu}) eMba daMshakada tupupxLu yA camaR. 
\emng
\eentry

\bentry
\word[nutrient(1)]{nutrient}
\pron{nUyxTirxaMTf}
\gl{\nA}
\bmng
pwSiTxka (darxvayx); jiVvavanunx puSiTxgoLisuva, poVSisuva padAthaR. 
\emng
\eentry

\bentry
\word[nutrient(2)]{nutrient}
\pron{nUyxTirxaMTf}
\gl{\gu}
\bmng
pwSiTxka; puSiTxkAri; jiVvavanunx poVSisuva. 
\emng
\eentry

\bentry
\word{nutriment}
\pron{nUyxTirxmaMTf}
\gl{\nA}
\bmng
\bnum
\num{1} pwSiTxka AhAra; puSiTx koDuvaMtha AhAra. 
\num{2} bwdidhxka yA kalAtamxka \mo\ puSiTx, parxcoVdane. 
\enum
\emng
\eentry

\bentry
\word{nutrimental}
\pron{nUyxTirxmeMTa(Tf)lf}
\gl{\gu}
\bmng
\bnum
\num{1} pwSiTxka; puSiTxdAyaka AhArada guNagaLiruva. 
\num{2} puSiTxkaravAda; pwSiTxka; poVSakavAda; puSiTxkoDuvaMtha. 
\enum
\emng
\eentry

\bentry
\word{nutrition}
\pron{nUyxTirxSanf}
\gl{\nA}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} poVSaNe; jiVvavanunx sUkatx AhAradiMda poVSisuvudu. 
\alnum{b} AhAra; poVSaka vasutx. 
\eanum
\numie
\num{2} poVSaNa shAsatxrX; poVSaNeya shAsitxrXVya adhayxyana. 
\enum
\emng
\eentry

\bentry
\word{nutritional}
\pron{nUyxTirxSanalf}
\gl{\gu}
\bmng
poVSaNeya, pwSiTxkateya yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nutritionist}
\pron{nUyxTirxSanisfTx}
\gl{\nA}
\bmng
poVSaNa shAsatxrXjacnx; mAnava poVSaNeyanunx adhayxyana mADuva vayxkitx yA adaralilx tajacnx. 
\emng
\eentry

\bentry
\word{nutritious}
\pron{nUyxTirxSasf}
\gl{\gu}
\bmng
poVSaka; pwSiTxka; utatxma poVSaNe niVDabalalx. 
\emng
\eentry

\bentry
\word{nutritiously}
\pron{nUyxTirxSasfli}
\gl{\kirxvi}
\bmng
pwSiTxkavAgi; puSiTxdAyakavAgi; puSiTxkaravAguvaMte. 
\emng
\eentry

\bentry
\word{nutritiousness}
\pron{nUyxTirxSasfnisf}
\gl{\nA}
\bmng
pwSiTxkate; puSiTxdAyakate; puSiTxkaravAguvaMtiruvudu. 
\emng
\eentry

\bentry
\word[nutritive(1)]{nutritive}
\pron{nUyxTirxTivf}
\gl{\gu}
\bmng
\bnum
\num{1} pwSiTxka; poVSaka -- AhAravAgabalalx, guNavuLaLx. 
\num{2} jiVva poVSaNeya; jiVvapoVSaNege neravAguva. 
\enum
\emng
\eentry

\bentry
\word[nutritive(2)]{nutritive}
\pron{nUyxTirxTivf}
\gl{\nA}
\bmng
pwSiTxka AhAra (padAthaR). 
\emng
\eentry

\bentry
\word[nuts(1)]{nuts}
\pron{naTfsx}
\gl{\gu}
\bmng
(\ashi) hucucxhiDida; tikakxlu hiDida. 
\emng

\noindent
\gl{\pagu}
\bmng
\eng{nuts about} (\engit{or} \eng{on})(\AmA) (oMdu viSayadalilx) 
\banum
\alnum{a} utAsxhiyAda; utusxkanAgiruva. 
\alnum{b} catura; kushala. 
\eanum
\emng
\eentry

\bentry
\word[nuts(2)]{nuts}
\pron{naTfsx}
\gl{\BAavayx}
\bmng
(\ashi) (tirasAkxra yA aNaka sUcisuva udAgxra) thU; thu: \eng{nuts to you} thU ninanx! 
\emng
\eentry

\bentry
\word{nutshell}
\pron{naTfSelf}
\gl{\nA}
\bmng
\bnum
\num{1} kAyi -- karaTa, cipupx. 
\num{2} (\rUpa) (heVLuvudaralilx) atayxMta saMgarxha rUpa; saMkiSxpatx nirUpaNe; bahaLa aDaka riVti: \eng{can give it to you in a nutshell} ninagadanunx aDakavAgi heVLabalelx. 
\enum
\emng
\eentry

\bentry
\word{nutter}
\pron{naTarf}
\gl{\nA}
\bmng
\bnum
\num{1} (\birx) (\ashi) hucacx; tikakxla manuSayx. 
\num{2} kAyi -- saMgArxhaka, oTuTx mADuvavanu. 
\enum
\emng
\eentry

\bentry
\word{nuttiness}
\pron{naTinisf}
\gl{\nA}
\bmng
\bnum
\num{1} karaTakAyi Baritate; karaTakAyi tuMbiruvike. 
\num{2} karaTakAyiya ruciyaMtiruvike. 
\enum
\emng
\eentry

\bentry
\word{nut-tree}
\pron{naTfTirxV}
\gl{\nA}
\bmng
heVsaZlf mara; kAyi biDuva (\kanmu\ heVsaZlf kAyi biDuva) karaTakAyi mara. 
\emng
\eentry

\bentry
\word{nutty}
\pron{naTi}
\gl{\gu}
\expl{(\tara\ \eng{nuttier}, \tama\ \eng{nuttiest}).}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} karaTakAyi -- tuMbida, Barita; heVraLavAgi karaTakAyi biDuva. 
\alnum{b} karaTakAyi ruciya. 
\eanum
\numie
\num{2} (\ashi) = \hyperlink{nuts(1)}{$^1$nuts}. 
\enum
\emng
\eentry

\bentry
\word{nut-weevil}
\pron{naTfviVvilf}
\gl{\nA}
\bmng
kAyi jiVruMDe; hasuru hAgU kaMdu heVsaZlf kAyigaLalilx moTeTxyiDuva, oMdu jAtiya jiVruMDe. 
\emng
\eentry

\bentry
\wordRemoveSpace{nux-vomica}{nux vomica}
\pron{nakfsxvAmika}
\gl{\nA}
\bmng
\bnum
\num{1} kAjavAra; hemumxSiTx; viSamuSiTx; siTxrXkAnxsfnakfsx vAmika kulakekx seVrida, viSa haNuNxgaLanunx biDuva, pUvaR iMDiyAda mara. 
\num{2} siTxrXkinxVnf eMba AlakxlAyfDx iruva I marada biVjagaLu. 
\enum
\emng
\eentry

\bentry
\word{nuzzle}
\pron{nasaZ(sfZ)lf}
\gl{\sakirx}
\bmng
\bnum
\num{1} melalxge mUginiMda ujujx, tivi. 
\num{2} (\AtAmx\ saha) becacxge -- oLage tUriko, mudurikoMDiru. 
\enum
\emng

\noindent
\gl{\akirx}
\bmng
\bnum
\num{1} mUginiMda -- otutx, ujujx; mUganunx(oMdaroLakekx) nUku. 
\num{2} mUginiMda mUsi noVDu. 
\num{3} mUginiMda bila toVDu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NV}{NV}
\pron{?}
\gl{\saMkiSx}
\bmng
(amerika) \eng{Nevada} (adhikaqta aMce baLakeyalilx \parx). 
\emng
\eentry

\bentry
\wordnospeech{NW}{NW}
\pron{?}
\gl{\saMkiSx}
\bmng
\bnum
\num{1} \eng{north-west}. 
\num{2} \eng{northwestern}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{NY}{NY}
\pron{?}
\gl{\saMkiSx}
\bmng
(amerika) \eng{New York} (adhikaqta aMce baLakeyalilx \parx.) 
\emng
\eentry

\bentry
\word{nyala}
\pron{nAyxla}
\gl{\nA}
\bmng
\hyperref{kandict_i.pdf}{I}{inyala}{inyala} padada rUpaMtara. 
\emng
\eentry

\bentry
\wordnospeech{NYC}{NYC}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{New York City.} 
\emng
\eentry

\bentry
\word{nyctalopia}
\pron{nikaTxloVpia}
\gl{\nA}
\bmng
nishAMdhate; mabubxguruDu; rAtirxkuruDu; katatxleyalilx hAgU mabibxnalilx kaNuNx kANadiruvudu \eng{(night-blindness } eMdU \parx). 
\emng
\eentry

\bentry
\word{nyctitropic}
\pron{nikiTxTorxVpikf}
\gl{\gu}
\bmng
(\savi) (giDagaLa calaneya \vi) rAtArxyXvataR; rAtirxya veVLe nishicxta dikikxge tiruguva. 
\emng
\eentry

\bentry
\word{nye}
\pron{neY}
\gl{\nA}
\bmng
\hyperlink{nide}{nide} eMbudara rUpAMtara. 
\emng
\eentry

\bentry
\word{nylghau}
\pron{nilAgx}
\gl{\nA}
\bmng
= \hyperlink{nilgai}{nilgai}. 
\emng
\eentry

\bentry
\word{nylon}
\pron{neYlAnf}
\gl{\nA}
\bmng
\bnum
\num{1} neYlAnf; keYgArikegaLalilx, baTeTxgaLalilx, hagagx tayArikeyalilx, inUnx vividha keSxVtarxgaLalilx baLasutitxruva, daqDhavU haguravU ilAsiTxkf guNagaLanUnx uLaLx, porxVTiVninaMtha racaneyuLaLx halavAru saMshelxVSita kaqtaka pAli ameYDfgaLalilx oMdu. 
\num{2} neYlAnf baTeTx. 
\num{3} (\bava dalilx) neYlAnf(niMda tayArisida) kAluciVlagaLu. 
\enum
\emng
\eentry

\bentry
\word{nymph}
\pron{niMphf}
\gl{\nA}
\bmng
\bnum
\num{1} apasxre; (kaDalu, hoLe, kAraMji, beTaTx, kADu, mara \mo vugaLalilx nelesiruva, yA meVlatxrada deVvategaLige parivAravAgiruva) purANada sitxrXV upadeVvate(gaLalolxbabxLu). 
\num{2} (\kAparx) apasxre; suMdari; suMdara taruNi. 
\num{3} niMphf; vayasakx kiVTavanunx hoVluva, pArxyakekx baMdilalxda kiVTa. 
\num{4} cikakx koDati huLu yA tUkada huLu. 
\enum
\emng
\eentry

\bentry
\word{nymphae}
\pron{niMphiV}
\gl{\nA}
\bmng
(\bava) (\aMrashA) yoVniya, sitxrXV jananAMgada oLa maDikegaLu. 
\emng
\eentry

\bentry
\word{nymphal}
\pron{niMphalf}
\gl{\gu}
\bmng
\bnum
\num{1} apasxreyara; apasxreyariMda tuMbida; apasxreyarige seVrida. 
\num{2} kiVTakoVshadaMtha; kiVTakoVshakekx saMbaMdhisida. 
\num{3} niMphiV eMba jAtiya jalasasayxgaLige, niVrugiDagaLige saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{nymphean}
\pron{niMphiVanf}
\gl{\gu}
\bmng
apasxreyaMtha yA apasxrege saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nymphet}
\pron{niMpheTf}
\gl{\nA}
\bmng
\bnum
\num{1} cikakx vayasisxna apasxre. 
\num{2} (\AmA) apasxreyaMtha yA kAmavanunx keraLisuvaMtha suMdara taruNi. 
\enum
\emng
\eentry

\bentry
\word{nymphlike}
\pron{niMphfleYkf}
\gl{\gu}
\bmng
apasxreyanunx hoVluva; apasxreya hAgiruva. 
\emng
\eentry

\bentry
\word{nympho}
\pron{niMphoV}
\gl{\nA}
\expl{(\bava\ \eng{nymphos}).}
\bmng
(\AmA) = \hyperlink{nymphomaniac(1)}{$^1$nymphomaniac}. 
\emng
\eentry

\bentry
\word{nympholepsy}
\pron{niMphalepisx}
\gl{\nA}
\bmng
BAvoVnAmxda; dulaRBa vasutxvina moVhadiMdAda AveVsha, unAmxda. 
\emng
\eentry

\bentry
\word{nympholept}
\pron{niMphalepfTx}
\gl{\nA}
\bmng
BAvoVnamxtatx; (\kanmu\ oMdu asAdhayx AdashaRkAkxgi) hucucx utAsxhavuLaLxvanu. 
\emng
\eentry

\bentry
\word{nympholeptic}
\pron{niMphalepiTxkf}
\gl{\gu}
\bmng
BAvoVnAmxdada; hucucx, utAsxhadiMda keraLuva. 
\emng
\eentry

\bentry
\word{nymphomania}
\pron{niMphameVnia}
\gl{\nA}
\bmng
(sitxrXVyara) kAmoVnAmxda; atiyAda kAmApeVkeSx. 
\emng
\eentry

\bentry
\word[nymphomaniac(1)]{nymphomaniac}
\pron{niMphameVniAYxkf}
\gl{\nA}
\bmng
kAvoVnamxtetx; atiyAda kAmApeVkeSxyuLaLx heMgasu. 
\emng
\eentry

\bentry
\word[nymphomaniac(2)]{nymphomaniac}
\pron{niMphameVniAYxkf}
\gl{\gu}
\bmng
(heMgasina \vi) kAmoVnAmxdakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nystagmic}
\pron{nisATx Yxgimxkf}
\gl{\gu}
\bmng
kaNaNxdiru roVgada yA adakekx saMbaMdhisida; akiSxdoVlanada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{nystagmus}
\pron{nisATx Yxgamxsf}
\gl{\nA}
\bmng
kaNaNxdiru roVga; akiSxdoVlana; gaNi kelasagArarige sAmAnayxvAgi baruva, kaNuNxguDeDx eDebiDade alugutitxruva, oMdu bageya roVga. 
\emng
\eentry

\bentry
\wordnospeech{NZ}{NZ}
\pron{?}
\gl{\saMkiSx}
\bmng
\eng{New Zealand}. 
\emng
\eentry

%V have 2 put hyperlink to their corresponding TeX files %{\hyperref{kandict_d.pdf}{D}{dressing-gown}{dressing-gown}%\hyperref{kandict_e.pdf}{E}{en}{en}.}%\newpage%\input figs_list
