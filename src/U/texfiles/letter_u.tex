\bentry
\word[U(1)]{U}
\pron{yU}
\gl{\nA}
\expl{(\bava\ \eng{Us} yA \eng{U's} )}
\bmng
(\eng{u} eMdU \parx) 
\bnum
\num{1}iMgilxSf vaNaRmAleya ipapxtotxMdaneya akaSxra. 
\num{2} \eng{U} AkArada vasutx yA vakarx(reVKe) (\kanmu\ \saMpa gaLalilx baLake): \eng{U-bolt.} 
\enum
\emng
\eentry

\bentry
\word[U(2)]{U} 
\pron{U} 
\gl{\nA}
\expl{}
\bmng
(bamiVRyaralilx) obabxna hesarina muMde seVrisuva gwravasUcaka birudu. 
\emng
\eentry

\bentry
\word[U(3)]{U} 
\pron{yU} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ \birx) 
\bnum
\num{1} (\AmA) meVlina vagaRda; meVlavxgaRkekx, shiSaTxvagaRkekx seVrida. 
\num{2} meVluvagaRda vishiSaTx lakaSxNa eMdu naMbalAda. 
\enum
\emng
\eentry

\bentry
\word[U(4)]{U} 
\pron{yU} 
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} (calanacitarx \mo vugaLa \vi) \eng{universal} sAvaRtirxka; yAvudeV nibaRMdhavilalxde elalxrU noVDabahudAda. 
\num{2} \eng{University}. 
\enum
\emng
\eentry

\bentry
\word[U(5)]{U} 
\pron{?}
\gl{\saMkeV}
\expl{(\ravi) }
\bmng
\eng{uranium} dhAtu. 
\emng
\eentry

\bentry
\wordnospeech{u}{u}
\pron{?} 
\gl{\pUparx}
\expl{}
\bmng
= \hyperref{kandict_m.pdf}{M}{mu(2)}{mu(2)}(\eng{$\mu$}). 
\emng
\eentry

\bentry
\wordnospeech{UAE}{UAE}
\pron{?} 
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Arab Emirates.} 
\emng
\eentry

\bentry
\word{ubiety} 
\pron{yUbeYaTi} 
\gl{\nA}
\expl{}
\bmng
sAthxnatavx; oMdu gotAtxda sathxLa yA sAthxnadalilxruvudu. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ubility}{-ubility}
\pron{-yubiliTi} 
\gl{\uparx}
\expl{}
\bmng
\eng{-uble} iMda konegoLuLxva guNavAcakagaLiMda yA avugaLige anuguNavAda nAmapadagaLanunx racisuvalilx baLasuva \uparx: \eng{dissolubility, solubility, volubility.} 
\emng
\eentry

\bentry
\word[ubiquitarian(1)]{ubiquitarian} 
\pron{yUbikivxTeVrianf}
\gl{\gu}
\expl{}
\bmng
(\deVva) yeVsu kirxsatxna savaRtarx asitxtavxkekx saMbaMdhisida yA adanunx naMbuva. 
\emng
\eentry

\bentry
\word[ubiquitarian(2)]{ubiquitarian} 
\pron{yUbikivxTeVrianf}
\gl{\nA}
\expl{}
\bmng
(\deVva) yeVsu kirxsatxna savaRtarx asitxtavxdalilx yA parxtayxkaSxteyalilx vishAvxsi. 
\emng
\eentry

\bentry
\word{ubiquitarianism} 
\pron{yUbikivxTeVrianisaZmf} 
\gl{\nA}
\expl{}
\bmng
(\deVva) yeVsu kirxsatxna savaRtarx parxtayxkaSxteyalilx sharxdedhx, vishAvxsa. 
\emng
\eentry

\bentry
\word{ubiquitous} 
\pron{yUbikivxTasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} savaRga; savaRtarx; elelxDe yA oMdeV kAladalilx vividha eDegaLalilx iruva: \eng{ubiquitous fog} savaRtarx haraDida maMju. 
\num{2} padeVpadeV yA aneVkaveVLe -- siguva, goVcarisuva, saMdhisabahudAda. 
\enum
\emng
\eentry

\bentry
\word{ubiquitously} 
\pron{yUbikivxTasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} savaRtarx sidadhxvAgi; elelxDe yA oMdeV kAladalilx vividheDegaLalilx iruvaMte. 
\num{2} padeVpadeV yA aneVka veVLe siguvaMte, goVcaravAguvaMte. 
\enum
\emng
\eentry

\bentry
\word{ubiquitousness} 
\pron{yUbikivxTasfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{ubiquity}{ubiquity}. 
\emng
\eentry

\bentry
\word{ubiquity} 
\pron{yUbikivxTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} savaRtarx parxtayxkaSxte; savaRtarx asitxtavx; savaRgatavx; oMdeV kAladalilx aneVka sathxLagaLalilxruvike. 
\num{2} (\hA) (vayxkitxya \vi) padeVpadeV siguvike. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{ubiquity of the king} (\birx) (\nAyxshA) rAjana savARMtayARmitavx; rAjanu elAlx nAyxyAlayagaLalUlx nAyxyAdhiVsharugaLalUlx mUtiRmatAtxgiruvudu; nAyxyAdhiVshara rUpadalilx adhikaqtavAgi iruvudu. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-uble}{-uble}
\pron{-yuba(bf)lf} 
\gl{\uparx}
\bmng
sAdhayx yA beVku eMbathaRda guNavAcakagaLa racaneyalilx baLasuva \uparx, \udA\ \eng{soluble} darxvaNasAdhayx yA parihAyaR. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ubly}{-ubly} 
\pron{-yubilx} 
\gl{\uparx}
\expl{}
\bmng
\eng{-uble} iMda konegoLuLxva guNavAcakagaLiMda kirxyAvisheVSaNagaLanunx racisuva \uparx. 
\emng
\eentry

\bentry
\word{U-boat} 
\pron{yUboVTf} 
\gl{\nA}
\bmng
 (\ca) (jamaRnara) jalAMtanwRke; jalAMtagARmi haDagu 
\emng
\eentry

\bentry
\word{U-bolt} 
\pron{yUboVlfTx} 
\gl{\nA}
\expl{}
\bmng
\eng{U} (akaSxrada AkAradalilxruva) tApALu; vaMki tApALu koMDi. 
\emng
\eentry

\bentry
\wordnospeech{UC}{UC}
\pron{?} 
\gl{\saMkiSx}
\expl{}
\bmng
\eng{University College.} 
\emng
\eentry

\bentry
\wordnospeech{u.c.}{u.c.}
\pron{?} 
\gl{\saMkiSx}
\expl{}
\bmng
\eng{upper case.} 
\emng
\eentry

\bentry
\wordnospeech{UCATT}{UCATT}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
(\UK\ yalilx) \eng{Union of Construction, Allied Trades, and Technicians.} 
\emng
\eentry

\bentry
\wordnospeech{UCCA}{UCCA}
\pron{?} 
\gl{\saMkiSx}
\expl{}
\bmng
(\UK\ yalilx) \eng{Universities Central Council on Admissions.} 
\emng
\eentry

\bentry
\wordnospeech{UCW}{UCW}
\pron{?} 
\gl{\saMkiSx}
\expl{}
\bmng
(\UK\ yalilx) \eng{Union of Communication Workers.} 
\emng
\eentry

\bentry
\wordnospeech{UDA}{UDA}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{Ulster Defence Association (a loyalist paramilitary organization).} 
\emng
\eentry

\bentry
\word{udal} 
\pron{yUDa(Df)lf} 
\gl{\nA}
\bmng
niraMtarAnuBoVga sAvxmayx hakukx; yUroVpinalilx ULigamAnayx padadhxtige modalu idadx, IgalU AkinxR matutx SeTelxMDf divxVpagaLalilxruva, aviciPxnanx anuBoVgadiMda baMda, oMdu jamiVnu sAvxmayx padadhxti. 
\emng
\eentry

\bentry
\word{udaller} 
\pron{yUDalarf} 
\gl{\nA}
\expl{}
\bmng
niraMtarAnuBoVga hiDuvaLidAra. 
\emng
\eentry

\bentry
\word{udalman} 
\pron{yUDalfmanf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{udaller}{udaller}. 
\emng
\eentry

\bentry
\wordnospeech{UDC}{UDC}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
(\birx) (\ca) \eng{Urban District Council.} 
\emng
\eentry

\bentry
\word{udder} 
\pron{aDarf} 
\gl{\nA}
\expl{}
\bmng
(hasu \mo vugaLa) kecacxlu. 
\emng
\eentry

\bentry
\word{uddered} 
\pron{aDaDfR} 
\gl{\gu}
\expl{}
\bmng
kecacxliruva (\saMpa gaLalilx saha \parx). 
\emng
\eentry

\bentry
\word{udderless} 
\pron{aDarflisf} 
\gl{\gu}
\expl{}
\bmng
kecacxlilalxda. 
\emng
\eentry

\bentry
\wordnospeech{UDI}{UDI}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{unilateral declaration of independence.} 
\emng
\eentry

\bentry
\word{udometer} 
\pron{yUDAmiTarf} 
\gl{\nA}
\expl{}
\bmng
(\aupa) vaqSiTxmApaka; maLeya motatxvanunx aLeyuva salakaraNe. 
\emng
\eentry

\bentry
\wordnospeech{UDR}{UDR}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{Ulster Defence Regiment.} 
\emng
\eentry

\bentry
\word{UEFA} 
\pron{yUIpha} 
\gl{\saMkiSx}
\bmng
\eng{Union of European Football Associations.} 
\emng
\eentry

\bentry
\word{UFO} 
\pron{yUphoZV} 
\gl{\saMkiSx}
\expl{}
\bmng
(\eng{ufo} eMdU \parx) (\bava\ \eng{UFO s} yA \eng{ufos) Unidentified flying object.} 
\emng
\eentry

\bentry
\word{ufologist} 
\pron{yUphAlajisfTx} 
\gl{\nA}
\expl{}
\bmng
yUphoZV tajacnx; (gurutu sigada) hAruva vasutxgaLanunx adhayxyana mADuvavanu. 
\emng
\eentry

\bentry
\word{ufology} 
\pron{yUphAlaji} 
\gl{\nA}
\expl{}
\bmng
yUphoZVgaLa yA yUephfO \eng{(UFO)} gaLa adhayxyana; (gurutu sigada) hAruva taTeTxgaLa yA vasutxgaLa adhayxyana. 
\emng
\eentry

\bentry
\word{ugh} 
\pron{aKf, agf} 
\gl{\BAavayx}
\bmng
\bnum
\num{1} asahayx, jugupesx yA BiVtiyanunx sUcisuva pada. 
\num{2} kemimxna yA asamAdhAna sUcisuva shabadx. 
\enum
\emng
\eentry

\bentry
\word{Ugli} 
\pron{agilx} 
\gl{\nA}
\expl{(\bava\ \eng{Uglis} yA \eng{Uglies}).}
\bmng
\P\ macecxniMbe; cakokxVtada haNuNx matutx capapxTe kitatxLegaLa misharxtaLiyAda, macecxgaLiruva hasuru matutx haLadi baNaNxda niMbekulada haNuNx. 
\emng
\eentry

\bentry
\word{uglify} 
\pron{agilxpheY} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{uglified}; vataRmAna \parxpu\ \Eva}
\eng{uglifies}) \bmng
vikAragoLisu; aMdageDisu: \eng{they uglified the hall with pews and a gallery} parxteyxVka piVThAvaraNa matutx gAyxlariyanunx joVDisi hajAravanunx aMdageDisidadxru 
\emng
\eentry

\bentry
\word{uglily} 
\pron{agilxli} 
\gl{\kirxvi}
\expl{}
\bmng
vikAravAgi; aMdageVDAda; riVtiyalilx; avalakaSxNavAgi. 
\emng
\eentry

\bentry
\word{ugliness} 
\pron{agilxnisf} 
\gl{\nA}
\expl{}
\bmng
vikArate; aMdageVDu; avalakaSxNa sithxti. 
\emng
\eentry

\bentry
\word{ugly} 
\pron{agilx} 
\gl{\gu}
\expl{(\tara\ \eng{uglier}, \tama\ \eng{ugliest}).}
\bmng
\bnum
\num{1} (noVDalu yA keVLalu) asahayxvAda; avalakaSxNada; kurUpavAda; vikAravAda: \eng{an ugly beast of a bulldog} avalakaSxNada gULinAyi: \eng{an ugly scar} vikAravAda gAyada macecx. 
\num{2} ahitavAda; avamAnakaravAda; agwravada: \eng{ugly rumours are about} avamAnakaravAda vadaMtigaLu edidxve. 
\num{3} bedarikeya; apAyakaravAda: \eng{the cloud has an ugly look} moVDa bedarisuvaMtide. 
\num{4} keTaTx; duSaTx; kutisxta; niVtigeTaTx; heVya; heVsigeya: \eng{ugly vices} duraBAyxsagaLu, durAcAragaLu. 
\num{5} ahita, ashuBa -- sUcaka; nirAshAdAyaka: \eng{an ugly weather} ahita have. 
\enum
\emng
\eentry

\bentry
\wordnospeech{ugly customer}{ugly customer}
\pron{?}
\gl{\nA}
\bmng
 asAdhayx girAki; takarAru girAki; opipxsalu kaSaTxvAda, sulaBanalalxda girAki. 
\emng
\eentry

\bentry
\wordnospeech{ugly duckling}{ugly duckling}
\pron{?}
\gl{\nA}
\expl{}
\bmng
kAgegUDinalilxna koVgile mari; daDaDx yA aSATxvakarxnAgi kANutitxdudx konege elalxra niriVkeSxge miVri parxcaMDanAgi yA celuvanAgi pariNamisuva vayxkitx. 
\emng
\eentry

\bentry
\word[Ugrian(1)]{Ugrian} 
\pron{Ugirxanf} 
\gl{\gu}
\expl{}
\bmng
Ugirxyanf phinikf janAMgadavara pUvaRshAKeya, \kanmu\ magAyxrf yA phinfsx janAMgadavara. 
\emng
\eentry

\bentry
\word[Ugrian(2)]{Ugrian} 
\pron{Ugirxanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} Ugirxyanf janAMgadavanu. 
\num{2} Ugirxyanf BASe. 
\enum
\emng
\eentry

\bentry
\word[Ugric(1)]{Ugric} 
\pron{Ugirxkf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{Ugrian(1)}{$^1$Ugrian}. 
\emng
\eentry

\bentry
\word[Ugric(2)]{Ugric} 
\pron{Ugirxkf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{Ugrian(2)}{$^2$Ugrian}. 
\emng
\eentry

\bentry
\wordnospeech{UHF}{UHF}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{ultra-high frequency.} 
\emng
\eentry

\bentry
\word{uh-huh} 
\pron{aha} 
\bmng
\BAavayx (\AmA) hU(M); opipxgeyanunx sUcisuva shabadx. 
\emng
\eentry

\bentry
\word{uhlan} 
\pron{UlAnf, yUlanf} 
\gl{\nA}
\expl{}
\bmng
(\ca) (pashicxma yUroVpina, \kanmu\ hiMdina jamaRniya kelavu seYnayxgaLalilxna) ITi rAvuta. 
\emng
\eentry

\bentry
\wordnospeech{UHT}{UHT}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
(bahaLa kAla keDadaMtiDalu \kanmu\ hAlina \vi) \eng{ultra heat treated.} 
\emng
\eentry

\bentry
\word{Uigur} 
\pron{viVguarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} viVgUrf BASe; pashicxma ciVNAda TakiRkf janAMgada BASe. 
\num{2} viVgUrf janAMgadavanu. 
\enum
\emng
\eentry

\bentry
\word{Uitlander} 
\pron{ETAlxMDarf} 
\gl{\nA}
\expl{}
\bmng
\da\ Aphirxkadalilx (\kanmu\ boVarf \eng{(Boer)} yudadhxkikxMta muMcina) horanADiga, videVshiVya. 
\emng
\eentry

\bentry
\wordnospeech{UK}{UK}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Kingdom.} 
\emng
\eentry

\bentry
\wordnospeech{UKAEA}{UKAEA}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Kingdom Atomic Energy Authority.} 
\emng
\eentry

\bentry
\word{ukase} 
\pron{yUkeVsfZ} 
\gl{\nA}
\bmng
\bnum
\num{1} (\ca) raSayxda cakarxvatiRya yA sAZrf sakARrada hukuMnAme, Ajecnx. 
\num{2} niraMkusha, sevxVcePxya Ajecnx. 
\enum
\emng
\eentry

\bentry
\word[Ukrainian(1)]{Ukrainian} 
\pron{yUkerxVnianf} 
\gl{\gu}
\expl{}
\bmng
(soVviyetf okUkxTakekx seVrida) yUkerxYnf pArxMta yA adara jana yA BASe ivugaLa yA ivugaLige saMbaMdhisida. 
\emng
\eentry

\bentry
\word[Ukrainian(2)]{Ukrainian} 
\pron{yUkerxVnianf} 
\gl{\nA}
\expl{}
\bmng
\banum
\alnum{a} yUkerxVniyanf: yUkerxVnf parxdeVshada BASe. 
\alnum{b} \eng{USSR} na yUkerxVninalilx huTiTxdava. 
\eanum
\emng
\eentry

\bentry
\word{ukulele} 
\pron{yUkaleVli} 
\gl{\nA}
\expl{}
\bmng
yUkaleVli; havAyf divxVpada, (modalige poVcuRgalfna) nAlukx taMtiya oMdu saNaNx giTArf. \imglink{ukulele:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.4cm height 0.6cm {U_Pictures/ukulele.jpg}}} 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ular}{-ular} 
\pron{-yularf} 
\gl{\uparx}
\expl{}
\bmng
\bnum
\num{1} kelavomemx \eng{-ule} iMda konegoLuLxva nAmapadagaLige saMvAdiyAda alApxthaRka guNavAcakagaLanunx racisuvalilx baLasuva \uparx, \udA\ \eng{pustular} saNaNx guLeLx, bokekx, \mo vugaLa. 
\num{2} aneVkaveVLe alApxthaRka sUcaneyilalxda guNavAcakagaLanunx racisuva \uparx, \udA\ \eng{angular, globular, granular etc.} 
\enum
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ularity}{-ularity}
\pron{-yulAYxriTi} 
\gl{\uparx}
\expl{}
\bmng
nAmapadagaLa racaneyalilx baLasuva \uparx, \udA\ \eng{angularity.} 
\emng
\eentry

\bentry
\word{ulcer} 
\pron{alasxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (deVhada oLa yA horaBAgadalilx uMTAguva, kiVvu sarxvisuva) huNuNx; varxNa. 
\numi{2} 
\banum
\alnum{a} neYtika kaLaMka, kale. 
\alnum{b} keDisuvaMtha parxBAva, niVtiBarxSaTxkAri viSaya, \mo vu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{ulcerable} 
\pron{alasxrabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
huNANxguva; varxNavAgabalalx. 
\emng
\eentry

\bentry
\word{ulcerate} 
\pron{alasxreVTf} 
\gl{\sakirx}
\expl{}
\bmng
huNuNxmADu; varxNavAgisu. 
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} varxNavAgu; huNANxgu 
\num{2} (huNiNxnaMte) tiVvarxyAtane koDu; viSavAgu; naMju hiDi. 
\enum
\emng
\eentry

\bentry
\word{ulceration} 
\pron{alasxreVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
huNANxguvike; varxNamayavAgiruvudu. 
\emng
\eentry

\bentry
\word{ulcerative} 
\pron{alasxraTivf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} varxNavanunx uMTumADuva; huNANxguva. 
\num{2} huNiNxnaMtha; varxNamayavAda. 
\enum
\emng
\eentry

\bentry
\word{ulcered} 
\pron{alasxDfR} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{ulcerous}{ulcerous}. 
\emng
\eentry

\bentry
\word{ulcerous} 
\pron{alasxrasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} varxNavAda; huNANxda; varxNagaLiMda tuMbida. 
\num{2} varxNadaMtha; huNiNxnaMtha. 
\enum
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ule}{-ule}
\pron{-yUlf} 
\gl{\uparx}
\expl{}
\bmng
alApxthaRka nAmapadagaLa racaneyalilx baLasuva \uparx, \udA\ \eng{capsule, granule, etc.} 
\emng
\eentry

\bentry
\word{ulema} 
\pron{Ulima} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} UlemA; musalAmxnf dhamaRshAsatxrXjacnx, deVvatAshAsatxrXjacnx. 
\num{2} musalAmxnf dhamaRshAsatxrXjacnxra yA deVvatAshAsatxrXjacnxra maMDali. 
\enum
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ulence}{-ulence}
\pron{-yulanfsx} 
\gl{\uparx}
\expl{}
\bmng
yAvudoV oMdariMda tuMbiruvike, pUritavAgiruvike eMbathaRda nAmapadagaLanunx racisuva \uparx, \udA\ \eng{turbulence.} 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ulent}{-ulent} 
\pron{-yulaMTf} 
\gl{\uparx}
\expl{}
\bmng
yAvudoV oMdariMda tuMbida, kUDida, Barita, pUrita eMbathaRda guNavAcakagaLa racaneyalilx baLasuva \uparx, \udA\ \eng{fraudulent, turbulent}. 
\emng
\eentry

\bentry
\word{uliginose} 
\pron{yUlijinoVsf} 
\gl{\gu}
\expl{}
\bmng
(\savi) kacaCxja; paMkaja; anUpaja; kesarinalilx, jwginalilx beLeyuva. 
\emng
\eentry

\bentry
\word{uliginous} 
\pron{yUlijinasf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{uliginose}{uliginose}. 
\emng
\eentry

\bentry
\word{ullage} 
\pron{alijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} korate; hArxsa; piVpAyi \mo vu BatiRyAguvudakekx korebidadx motatx. 
\num{2} uLike(yadu); uLidadudx; (ava)sheVSa; oMdu pAterx \mo vugaLalilxTaTx madayx, darxva, \mo vu AviyAgi uLida motatx. 
\enum
\emng
\eentry

\bentry
\word{ulna} 
\pron{alanx} 
\gl{\nA}
\bmng
\bnum
\num{1} (\bava\ \eng{ulnae} \ucAcx\ alinxV). parxkoVSAThxsithx; muMdoVLina, oLa(pAshavxRda ) elubu; hebebxraLige virudadhx dikikxnalilxruva muMdoVLina, udadxvU teLuvU Ada mULe. 
\num{2} (\pArxvi) pArxNiya muMgAlina yA pakiSxya rekekxya adeV riVtiya mULe. 
\enum
\emng
\eentry

\bentry
\word{ulnar} 
\pron{alanxrf} 
\gl{\gu}
\expl{}
\bmng
parxkoVSAThxsithxya; muMdoVLina oLa elubina yA adakekx saMbaMdhisida yA adu iruveDe iruva. 
\emng
\eentry

\bentry
\word[ulotrichan(1)]{ulotrichan} 
\pron{yUlATirxkanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ mAnava janavagaRvanunx sUcisuva) guMguru keVshada, kUdalina. 
\num{2} guMguru keVshi janavagaRda; guMguru kUdaluLaLx janAMgada. 
\enum
\emng
\eentry

\bentry
\word[ulotrichan(2)]{ulotrichan} 
\pron{yUlATirxkanf} 
\gl{\nA}
\expl{}
\bmng
guMguru keVshi, kUdalinavanu; guMguru kUdalina janavagaRkekx seVridavanu. 
\emng
\eentry

\bentry
\word{ulotrichous} 
\pron{yUlATirxkasf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{ulotrichan(1)}{$^1$ulotrichan}. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ulous}{-ulous} 
\pron{-yulasf} 
\gl{\uparx}
\expl{}
\bmng
tuMbida, kUDida eMbathaRda guNavAcakagaLa racaneyalilx baLasuva \uparx, \udA\ \eng{fabulous, populous.} 
\emng
\eentry

\bentry
\word{ulster} 
\pron{alfsaTxrf} 
\gl{\nA}
\expl{}
\bmng
alfsxTarf meVlaMgi; oraTu baTeTxyiMda mADida, naDupaTiTxyiruva, udadxvAda, saDila meVlaMgi. 
\emng
\eentry

\bentry
\wordnospeech{Ulster custom}{Ulster custom} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
alfsxTarf hiDuvaLi padadhxti; aileRMDina alfsxTarf parxdeVshadalilx baLakeyidadx, jamiVnu hiDuvaLi padadhxti. 
\emng
\eentry

\bentry
\word{Ulsterman} 
\pron{alfsxTarfmanf} 
\gl{\nA}
\expl{(\bava\ \eng{Ulstermen}; \sitxrXV\ \eng{Ulsterwoman} }
\bava\ \eng{Ulsterwomen}). \bmng
ailaRMDina alfsxTarfnalilx huTiTxdavanu. 
\emng
\eentry

\bentry
\word{Ulsterwoman} 
\pron{alfsxTarfvumanf} 
\gl{\nA}
\expl{}
\bmng
ailaRMDina alfsxTarfnalilx huTiTxdavaLu. 
\emng
\eentry

\bentry
\wordnospeech{ult.}{ult.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{ultimo.} 
\emng
\eentry

\bentry
\word{ulterior} 
\pron{aliTx(V)ariarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Aceya; dUrada. 
\num{2} sadayxdadxlalxda; BaviSayxda; kAladalilx muMdina, dUrada: \eng{ulterior action} BaviSayxda kirxye, kAyaR. 
\num{3} hinenxleyalilxruva; kANuvudakikxMta, heVLuvudakikxMta miVri iruva; gupatx; agoVcara; avayxkatx: \eng{ulterior motive} gupatx udedxVshagaLu. \eng{ulterior plans} agoVcara haMcikegaLu. 
\enum
\emng
\eentry

\bentry
\word{ulteriorly} 
\pron{aliTx(V)ariarfli} 
\gl{\kirxvi}
\expl{}
\bmng
BaviSayxdalilx; muMdina GaTaTxdalilx; muMdina GaTaTxkekx. 
\emng
\eentry

\bentry
\word{ultima} 
\pron{aliTxma} 
\gl{\nA}
\expl{}
\bmng
oMdu shabadxda koneya, aMtima ucAcxrAMsha. 
\emng
\eentry

\bentry
\word[ultimate(1)]{ultimate} 
\pron{aliTxmaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaTaTxkaDeya; koneya; AtayxMtika; aMtima; AKeYrina; aMtayxda; adara taruvAya EnU irada yA iralu sAdhayxvilalxda. \eng{ultimate result} kaTaTxkaDeya PalitAMsha. 
\num{2} mUlavAda; mUlaBUtavAda: \eng{ultimate principles} mUlatatatxvXgaLu. \eng{ultimate truth} mUlaBUta satayx. 
\num{3} atayxMta hecicxna parxmANada; paramAvadhiya; gariSaThx (mitiya): \eng{ultimate tensile strength} atayxMta hecucx biguvu paDeyuva shakitx. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{the ultimate deterrent} (\kanmu\ AkarxmaNa, bAMbina dALi, \mo vugaLa \vi) aMtima taDe, niroVdhaka. 
\num{2} \eng{ultimate cause} mUlakAraNa. \eng{ultimate facts of nature} (muMde viBajisalasAdhayxvAda) parxkaqtiya mUlasatayxgaLu. 
\enum
\emng
\eentry

\bentry
\word[ultimate(2)]{ultimate} 
\pron{aliTxmaTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} savoRVtakxqqSaTx; paramoVtakxqqSaTx; savaRsherxVSaThx; sAdhaneyalilx yA kalapxneyalilx atuyxcacxvAdudu. 
\num{2} aMtima yA mUlaBUta satayx yA saMgati. 
\enum
\emng
\eentry

\bentry
\word{ultimately} 
\pron{aliTxmaTfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kaTaTxkaDeyadAgi; aMtimavAgi. 
\num{2} koneyalilx; niNARyakavAgi: \eng{I should ultimately succeed} nAnu koneyalilx gelalxleVbeVku. 
\enum
\emng
\eentry

\bentry
\word{ultimateness} 
\pron{aliTxmaTfnisf} 
\gl{\nA}
\expl{}
\bmng
aMtayx; kone. 
\emng
\eentry

\bentry
\wordRemoveSpace{ultima-Thule}{ultima Thule}
\pron{aliTxma tUliV} 
\gl{\nA}
\expl{\Latin}
\bmng
\bnum
\num{1} atayxMta dUrada, ajAcnxta parxdeVsha, sathxLa. 
\num{2} (\rUpa) (yAvudaradeV) gariSaThxmiti; tutatxtudi; parAkASeThx. 
\enum
\emng
\eentry

\bentry
\word{ultimatum} 
\pron{aliTxmeVTamf} 
\gl{\nA}
\expl{}
(\bava\ \eng{ultimatums} yA \eng{ultimata} \ucAcx-- aliTxmeVTa).\bmng
\bnum
\num{1} aMtima beVDike; (yAvudanunx nirAkarisidedxV Adare saMbaMdha vicePxVda, yudadhx GoVSaNe, \mo vakekx AsapxdavAguvudoV A) kaTaTxkaDeya koVrike yA kaTaTxkaDeya SaratutxgaLa GoVSaNe. 
\num{2} koneya nidhARra; kaDeV mAtu; aMtima tiVmARna. 
\enum
\emng
\eentry

\bentry
\word{ultimo} 
\pron{aliTxmoV} 
\gl{\gu}
\expl{}
\bmng
(vANijayx) (vAyxvahArika patarxgaLalilx) hoVda tiMgaLina; hiMdina tiMgaLina; gatamAsada: \eng{the 28th ultimo} hoVda tiMgaLu 28neV tAriVKina. 
\emng
\eentry

\bentry
\word{ultimogeniture} 
\pron{aliTxmoVjenicarf} 
\gl{\nA}
\expl{}
\bmng
kaniSaThx putArxdhikAra padadhxti; koneya maganige mAtarx Asitxya hakikxruva padadhxti. 
\emng
\eentry

\bentry
\word[ultra(1)]{ultra} 
\pron{alfTarx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ dhamaR, rAjakiVyagaLalilx) ugarxvAdi; ugarxpaMthavanunx, ugArxBipArxyagaLanunx, ugarxkAyARcaraNegaLanunx beMbalisuva. 
\emng
\eentry

\bentry
\word[ultra(2)]{ultra} 
\pron{alfTarx} 
\gl{\nA}
\expl{}
\bmng
ugarxvAdi; ugarx paMthadavanu. 
\emng
\eentry

\bentry
\word{ultra-} 
\pron{alfTarx-} 
\gl{\sapUpa}
\expl{}
\bmng
\bnum
\num{1} (yAvudoV oMdara) Aceya; AcekaDeya. 
\num{2} ati; vipariVta; atiyAgi; atiyAda: \eng{ultra-fashionable} ati nAjUkina. \eng{ultra-conservative} ati saMparxdAyaniSaThx. 
\num{3} tiVrA; ati: \eng{ultra-modern} tiVrA Adhunika. 
\enum
\emng
\eentry

\bentry
\word{ultracentrifuge} 
\pron{alfTarxseMTirxphUyxjf} 
\gl{\nA}
\expl{}
\bmng
adhikeVMdArxpavAhi; ati cikakx kaNagaLu matutx baqhadaNugaLu kaliladarxva oMdaralilx gasiyAgi keLakikxLiyuva daragaLa vayxtAyxsavanunx upayoVgisi koMDu avugaLanunx parxteyxVkisalu baLasuva, ativeVgada keVMdArxpavAhi yaMtarx. 
\emng
\eentry

\bentry
\word{ultra-high} 
\pron{alaTxrXheY} 
\gl{\gu}
\bmng
atuyxnanxta; (AvataRda \vi) 300 megahaTfRsxniMda 3000 megahaTfRsx varegina. 
\emng
\eentry

\bentry
\word{ultraism} 
\pron{alfTarxisaZM} 
\gl{\nA}
\expl{}
\bmng
ugarxvAda; ugarxpaMthavAda. 
\emng
\eentry

\bentry
\word{ultraist} 
\pron{alfTarxisfTx} 
\gl{\nA}
\expl{}
\bmng
(rAjakiVya, dhamaR, \mo vugaLalilx) ugarxvAdi; ugarxgAmi. 
\emng
\eentry

\bentry
\word[ultramarine(1)]{ultramarine} 
\pron{alfTarxmariVnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx) samudarxdAceyiruva; sAgaroVtatxrada. 
\num{2} ujajxvXla niVlavaNaRda. 
\enum
\emng
\eentry

\bentry
\word[ultramarine(2)]{ultramarine} 
\pron{alfTarxmariVnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} niVlamaNiyiMda tayArisida ujajxvXla niVlavaNaRdarxvayx. 
\num{2} puDimADi kAyisida jeVDimaNuNx, soVDiyaM kAboRneVTf, gaMdhaka matutx rALadiMda tayArisida, adeV riVtiya vaNaRdarxvayx. 
\num{3} ujajxvXla niVlavaNaR. 
\enum
\emng
\eentry

\bentry
\word{ultramicroscope} 
\pron{alfTarxmeYkarxsokxVpf} 
\gl{\nA}
\expl{}
\bmng
atisUkaSx mxdashaRka; sAmAnayx sUkaSx mxdashaRkada mUlaka noVDalAgada ati saNaNx kaNagaLanunx, avu cedurisuva beLakina mUlaka toVrisabalalx asAmAnayx sUkaSx mxdashaRka. 
\emng
\eentry

\bentry
\word{ultramicroscopic} 
\pron{alfTarxmeYkarxsAkxpikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} atisUkaSx mxdashaRkiVya; atisUkaSx mxdashaRkada mUlaka kANisuva yA kANisabahudAda. 
\num{2} atisUkaSx mxdashaRkada yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word[ultramontane(1)]{ultramontane} 
\pron{alfTarxmAMTeVnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AlfpxsX pavaRtada dakiSxNakekx yA pavaRtadAce iruva. 
\num{2} iTaliya. 
\num{3} (naMbike, shisutx, \mo vugaLalilx) poVpf gurugaLa paramAdhikArakekx anukUlavAda; adanunx samathiRsuva. 
\num{4} (mAtanADuvavana daqSiTxyiMda) AlfpxsX pavaRtadAce iruva. 
\enum
\emng
\eentry

\bentry
\word[ultramontane(2)]{ultramontane} 
\pron{alfTarxmAMTeVnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AlfpxsX pavaRtada dakiSxNadalilx yA Ace pakakxdalilx vAsisuvavanu. 
\num{2} poVpf guruvige paramAdhikAravirabeVkeMdu vAdisuvavanu, samathiRsuvavanu. 
\enum
\emng
\eentry

\bentry
\word{ultramontanism} 
\pron{alfTarxmAMTeVnisaZ} 
\gl{\nA}
\expl{}
\bmng
poVpfge savoRVcacx yA paramAdhikAra irabeVkeMba vAda. 
\emng
\eentry

\bentry
\word{ultramontanist} 
\pron{alfTarxmAMTeVnisTxf} 
\gl{\nA}
\expl{}
\bmng
poVpfge paramAdhikAravirabeVkeMdu vAdisuvavanu. 
\emng
\eentry

\bentry
\word{ultramundane} 
\pron{alfTarxmaMDeVnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pAralwkika; anayxloVka jiVvanada; parxpaMcAtiVta; ihaloVkakekx Aceya. 
\num{2} swravUyxhada Aceya, Ace iruva. 
\enum
\emng
\eentry

\bentry
\word{ultrasonic} 
\pron{alaTxrXsAnikf} 
\gl{\gu}
\expl{}
\bmng
(\Bwvi) (dhavxniya \vi) sharxvaNAtiVta; manuSayxra sharxvaNavAyxpitxyanunx miVrida adhika AvataRda yA aMtha dhavxnitaraMgagaLanunx oLagoMDiruva. 
\emng
\eentry

\bentry
\word{ultrasonically} 
\pron{alaTxrXsAnikali} 
\gl{\kirxvi}
\expl{}
\bmng
(manuSayxra \vi) sharxvaNAtiVtavAgi; sharxvaNavAyxpitxyanunx miVri. 
\emng
\eentry

\bentry
\word{ultrasonics} 
\pron{alaTxrXsAnikfsx} 
\gl{\nA}
\expl{}
\bmng
(\bava) (\sA\ \Eva vAgi baLake) sharxvaNAtiVta vijAcnxna; kivige keVLuva vAyxpitxyanunx miVrida taraMgagaLa adhayxyana matutx avugaLa anavxya yA upayoVga. 
\emng
\eentry

\bentry
\word{ultrasound} 
\pron{alfTarxswMDf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} sharxvaNAtiVta dhavxni. 
\num{2} sharxvaNAtiVta dhavxnitaraMgagaLu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{ultrasound cardiography}{ultrasound cardiography}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\veYshA) sharxvaNAtiVta haqlelxVKana; haqdayada kirxyeyanunx parishiVlisalu sharxvaNAtiVta taraMgagaLanunx baLasikoLuLxvudu. 
\emng
\eentry

\bentry
\word{ultrastructure} 
\pron{alfTarxsaTxrXkacxrf} 
\gl{\nA}
\expl{}
\bmng
(\jiVvi) adhiracane; duyxtisUkaSxmXdashaRkadalilx \eng{(optical microscope)} gurutisalAgada sUkaSxmX racane. 
\emng
\eentry

\bentry
\word{ultraviolet} 
\pron{alfTarxveYalaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (viduyxtAkxMta taraMgagaLa \vi) niVlaloVhitAtiVta; neVraLAtiVta; roVhitada neVraLe tudiyiMda Aceya; neVraLe beLakina kiraNagaLa AvaqtitxgiMta hecicxna AvaqtitxyuLaLx. 
\num{2} neVraLAtiVta vikiraNada yA A vikiraNavanunx upayoVgisuva. 
\enum
\emng
\eentry

\bentry
\wordf(ultra vires[1]){ultra vires} 
\pron{alfTarx veYariVsfZ, ulfTArx viVreVsfZ} 
\gl{\kirxvi}
\expl{\Latin}
\bmng
obabxna (nAyxyavAda, kAnUnubadadhx) adhikArakekx miVri. 
\emng
\eentry

\bentry
\wordf(ultra vires[2]){ultra vires} 
\pron{alfTarx veYariVsfZ, ulfTArx viVreVsfZ} 
\gl{\gu}
\expl{\Latin}
\bmng
\kanu\ obabxna (nAyxyavAda) adhikArakekx miVrida. 
\emng
\eentry

\bentry
\word{ululant} 
\pron{yUlulaMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ULuva, AkarxMdanada; (aLuvaMte) bogaLuva. 
\num{3} saMtoVSadiMda diVGaRvAgi kUguva. 
\num{4} GUtakxrisuva. 
\enum
\emng
\eentry

\bentry
\word{ululate}
\pron{yUluleVTf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} ULu. 
\num{2} (aLuvaMte) bogaLu. 
\num{3} (saMtoVSadiMda) diVGaRvAgi kUgu 
\num{4} (gUbe) GUtAkxra mADu; gugukf anunx. 
\enum
\emng
\eentry

\bentry
\word{ululation} 
\pron{yUluleVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (nAyi, gUbe, \mo vugaLa \vi) ULu; ULuvike. 
\num{2} (nAyiya) diVGaR bogaLuvike. 
\num{3} (gUbeya) GUtAkxra. 
\num{4} saMtoVSadiMda diVGaRvAgi kUguvudu. 
\enum
\emng
\eentry

\bentry
\word{um}
\pron{amf}
\gl{\BAavayx}
\bmng
amf; uM; hiMjarikeyanunx sUcisuva yA mAtanADuvAga yA BASaNadalilx tegedukoLuLxva virAmavanunx sUcisuva shabadx. 
\emng
\eentry

\bentry
\wordnospeech{-um}{-um}
\pron{?}
\gl{\uparx}
\expl{}
\bmng
\eng{-ium(1)} \uparx da rUpAMtara. 
\emng
\eentry

\bentry
\word{umbel} 
\pron{aMbalf} 
\gl{\nA}
\expl{}
\bmng
(\savi) puSapxCatarx; Catirx hUgoMcalu; hUkoDe; keVMdarxvoMdariMda horaTa oMdeV udadxda halavAru kAMDagaLa tudiyalilxruva puSapxgaLu seVrikoMDu capapxTe yA goVLada meVlemxY AkAravanunx rUpisuva puSapxgucaCx, hUgoMcalu, hUgoMDe. 
\emng
\eentry

\bentry
\word{umbellar} 
\pron{aMbalarf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{umbellate}{umbellate}. 
\emng
\eentry

\bentry
\word{umbellate}
\pron{aMbaleVTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} puSapxCatarxgaLanunx yA CatirxhUgoMcalugaLanunx biDuva yA avugaLanunxLaLx yA avugaLaMte vayxvasethxgoLisida. 
\num{2} (racaneyalilx) puSapxCatarx yA hUgoMcalu Catirxyanunx hoVluva. 
\enum
\emng
\eentry

\bentry
\word{umbellifer} 
\pron{aMbelipharf} 
\gl{\gu}
\expl{}
\bmng
koDehUgiDa; puSapxCatarx sasayx; pAsfRli yA acumoVda matutx pAsfRnipf yA mUlaMgiyaMtha sasayxgaLanunx oLagoMDa, puSapxCatarxgaLanunx biDuva, aMbaliphariV vaMshakekx seVrida giDa. 
\emng
\eentry

\bentry
\word{umbelliferous} 
\pron{aMbalipharasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hUkoDegaLanunx, puSapxCatarxgaLanunx biDuva. 
\num{2} koDehU vaMshada yA adakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{umbellule} 
\pron{aMbelUyxlf} 
\gl{\gu}
\expl{}
\bmng
saMyukatx puSapxCatarx maMjariyalilx eraDaneya yA gwNa puSapxCatarxvuLaLx. 
\emng
\eentry

\bentry
\word[umber(1)]{umber} 
\pron{aMbarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaMdukAvi baNaNxda. 
\num{2} kapupxkaMdina; hoge baNaNxda. 
\enum
\emng
\eentry

\bentry
\word[umber(2)]{umber} 
\pron{aMbarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aMbarf raMgu; kaMdukAvi raMgu; kAviyaMtha Adare adakikxMta hecucx kapupxkaMdina, oMdu neYsagiRka vaNaRdarxvayx. 
\num{2} aMbarf yA kaMdukAvi baNaNx. 
\enum
\emng
\eentry

\bentry
\word[umber(3)]{umber} 
\pron{aMbarf} 
\gl{\sakirx}
\expl{}
\bmng
kaMdukAvi baNaNx hAku; aMbarfnaMtha baNaNx leVpisu. 
\emng
\eentry

\bentry
\word{umbilical} 
\pron{aMbilika(kf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hokukxLina; nABiya; hokukxLina hatitxraviruva yA hokukxLige saMbaMdhisida. 
\num{2} keVMdarxsathx; madhayxdalilx, keVMdarxdalilx iruva. 
\enum
\emng
\eentry

\bentry
\wordnospeech{umbilical cord}{umbilical cord}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hokukxLabaLiLx; nABirajujx; BUrxNavanunx mAsige seVrisuva, metuvAgi bAgisabahudAda, hagagxdaMtha racane. 
\num{2} (\vAyA) nABirajujx; kiSxpaNiyanUnx adanunx uDAyisuva sAdhanavanUnx yA AkAshanwkeyanUnx AkAshadalilxruva gagana yAtirxyanUnx kUDisuva, metuvAda horaji yA hagagx. 
\enum
\emng
\eentry

\bentry
\word{umbilicate} 
\pron{aMbilikaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hokukxLinAkArada. 
\num{2} hokukxLuLaLx. 
\enum
\emng
\eentry

\bentry
\word{umbilicus} 
\pron{aMbilikasf, aMbileYkasf} 
\gl{\nA}
\expl{(\bava\ \eng{umbilici} \ucAcx\ aMbiliseY, yA \eng{umbilicuses}).}
\bmng
\bnum
\num{1} (\aMrashA) nABi; hokukxLu. 
\num{2} (\jiVvi) sasayx yA pArxNigaLalilx nABiyaMtha racane. 
\num{3} (\jAyx) (vakarxreVKeya) nABi; tala oMdara meVliruva matutx adara mUlaka hAdu hoVguva elalx aDaDxkoyatxgaLigU samavakarxte iruvaMtha biMdu. 
\enum
\emng
\eentry

\bentry
\word{umbles} 
\pron{aMbf(ba)lf\char144} 
\gl{\nA}
\expl{}
\bmng
(jiMke \mo vugaLa) (tinanxlu baruva jaThara, karuLu \mo vugaLa) mAMsaBAgagaLu. 
\emng
\eentry

\bentry
\word{umbo} 
\pron{aMboV} 
\gl{\nA}
\expl{(\bava\ \eng{umbos} yA \eng{umbones} \ucAcx\ aMboVniVsfZ).}
\bmng
\bnum
\num{1} gurANiya (\kanmu\ madhayxdalilxruva) ububx, gubaTu. 
\num{2} (\savi\ matutx \pArxvi) ububx; gaMti; gubaTu; muMcAcu. 
\enum
\emng
\eentry

\bentry
\word{umbonal} 
\pron{aMboVnalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\savi\ matutx \pArxvi) gubaTina hAgiruva; ubibxdaMtha; garxMthiya hAgiruva: \eng{an umbonal structure} gubaTi naMtha racane. 
\num{2} (gurANiya madhayxBAgada, yA \savi\ matutx \pArxvi\ saha) gubaTina baLiya; gubaTu parxdeVshada; gaMtige seVrida: \eng{the umbonal region} gubaTina parxdeVsha. 
\enum
\emng
\eentry

\bentry
\word{umbonate} 
\pron{aMboVnaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gubaTu yA ububx yA garxMthiyiruva; gaMtiyuLaLx. 
\num{2} gubaTinAkArada: \eng{an umbonate fungus} gubaTinAkArada shiliVMdharx, aNabe. 
\enum
\emng
\eentry

\bentry
\word{umbra} 
\pron{aMbarx} 
\gl{\nA}
\expl{}
\bmng
pUtiR neraLu; pUNaRCAye: 
\banum
\alnum{a} beLakina AkaradiMda barutitxruva beLaku savxlapxvU talapadiruvaMtha neraLina BAga. 
\alnum{b} sUyaRna kaleya madhayxdalilxruva kapu pxBAga. 
\alnum{c} daTaTxneraLu; \sA\ sUyaRgarxhaNada kAladalilx caMdarxniMda BUmiya meVle biVLuva, pUNaR neraLina BAga. 
\eanum
\emng
\eentry

\bentry
\word{umbrage} 
\pron{aMbirxjf} 
\gl{\nA}
\bmng
\bnum
\num{1} asamAdhAna; munisu; apacAra BAvane; tirasAkxra, apacAra, GAsi AyiteMba BAvane. 
\numi{2} (\pArxparx) 
\banum
\alnum{a} neraLu. 
\alnum{b} neraLu koDuva vasutx \mo vu. 
\eanum
\numie
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{give umbrage} apacAravesaguvaMte naDeduko; asamAdhAna uMTumADuvaMte vatiRsu. 
\num{2} \eng{take umbrage (at)} apacAravAyi teMbaMte BAvisu, naDeduko, asamAdhAna paDu: \eng{he took umbrage at what I said about him} avana bagegx nAnu hAge heVLidadxriMda avanu asamAdhAnapaTuTxkoMDa. 
\enum
\emng
\eentry

\bentry
\word{umbrageous} 
\pron{aMberxVjasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} neraLu koDuvaMtha; neraLAda: \eng{an umbrageous tree} neraLu koDuva mara. 
\num{2} asamAdhAnagoLisuvaMtha; apacAravesagatakakx. 
\enum
\emng
\eentry

\bentry
\word{umbrella} 
\pron{aMberxl} 
\gl{\nA}
\bmng
\bnum
\num{1} koDe; Catirx. \imglink{umbrella 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.4cm {U_Pictures/umbrella.jpg}}} 
\num{2} shaturx vimAnagaLa virudadhx rakaSxNe niVDalu niyoVjisida yudadhxvimAnagaLa tere yA beMkiya mare. 
\num{3} (viBinanx aBipArxyadavaru oTATxgalu sahAyakavAda sUtarx, rAji, \mo vugaLa) neraLu; Asharxya; rAjakiVya rakaSxNe: \eng{under the umbrella of the UNO} yuenfO Asharxyada aDiyalilx. 
\num{4} (loVLemiVnu \mo vugaLa) jileTininxnaMtha Ijubilelx. 
\hypertarget{umbrella(5)}{} 
\num{5} koDecipupx pArxNi; koDeyaMtha cipu pxLaLx oMdu udara pAdi. 
\num{6} rakaSxNe; Asharxya; neraLu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{umbrella bird}{umbrella bird}
\pron{?}
\gl{\nA}
\expl{}
\bmng
koDe hakikx; sephalApaTxrasf kulada, koDeyaMte bicicxda kapupxshiKe matutx udadxvAda joVlu mAMsalaBAgagaLu uLaLx, \da\ amerikada halavu bageya hakikxgaLalolxMdu. 
\emng
\eentry

\bentry
\word{umbrellaed} 
\pron{aMberxlaDf} 
\gl{\gu}
\expl{}
\bmng
koDeyAkArAda; Catirxya AkAra tALida. 
\emng
\eentry

\bentry
\word{umbrella-like} 
\pron{aMberxlaleYkf} 
\gl{\gu}
\expl{}
\bmng
CatirxyaMtha; koDeyaMtha. 
\emng
\eentry

\bentry
\wordnospeech{umbrella pine}{umbrella pine}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} = \hyperref{kandict_s.pdf}{S}{stone pine}{stone pine}. 
\num{2} Catirx eleya mara; koMbegaLa tudiyalilx CatirxyaMte suruLi sutitxda elegaLuLaLx, etatxravAda, saDApi vaTiRsilATa kulada, japAnina, nitayxhasuru shaMku mara. 
\enum
\emng
\eentry

\bentry
\wordnospeech{umbrella shell}{umbrella shell}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{umbrella(5)}{umbrella \((5)\)}. 
\emng
\eentry

\bentry
\wordnospeech{umbrella stand}{umbrella stand}
\pron{?}
\gl{\nA}
\expl{}
\bmng
koDe niluvu; Catirx sATxYxMDu; mucicxda koDegaLaninxDuva, \sA\ toTiTxkukxva niVrigAgi keLage taTeTxyiruva, niluvu. 
\emng
\eentry

\bentry
\wordnospeech{umbrella tree}{umbrella tree}
\pron{?}
\gl{\nA}
\expl{}
\bmng
Catirxmara; koMbe tudiyalilx CatirxyaMte suruLi sutitxda ele biDuva, mAyxgonxliya TirxpeTala kulada, saNaNx mAyxgonxVliya mara. 
\emng
\eentry

\bentry
\word[Umbrian(1)]{Umbrian} 
\pron{aMbirxanf}
\gl{\gu}
\expl{}
\bmng
(madhayx iTaliya) aMbirxya pArxMtada, adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word[Umbrian(2)]{Umbrian} 
\pron{aMbirxanf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1}lAYxTinf BASege saMbaMdhaviruva, pArxciVna aMbirxyanf BASe. 
\num{2} pArxciVna aMbirxyanf pArxMtada nivAsi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{Umbrian school}{Umbrian school}
\pron{?}
\gl{\nA}
\expl{}
\bmng
aMbirxyanf paMtha; rAYxpheZVlf, perugino avaranonxLagoMDa, renesAnfsx yugada iTAYxliyanf citarxkaleya oMdu paMtha. 
\emng
\eentry

\bentry
\word{umbriferous} 
\pron{aMbirxpharasf}
\gl{\gu}
\expl{}
\bmng
(\aupa) neraLu koDuva. 
\emng
\eentry

\bentry
\word{umiak} 
\pron{UmiAYxkf} 
\gl{\nA}
\expl{}
\bmng
camaR matutx marada halagegaLiMda mADida, heMgasaru huTuTxhAki naDesuva, oMdu riVtiya esikxmoVgaLa tereda doVNi. \imglink{umiak:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.5cm {U_Pictures/umiak.jpg}}} 
\emng
\eentry

\bentry
\word[umlaut(1)]{umlaut} 
\pron{umflwTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} savxra rUpAMtara; savxravayxtayxya; savxradalilx uMTAguva vayxtAyxsa (\udA\ jamaRnf \eng{Mann, M}\eng{$\ddot\bg\rm a\eg$}\eng{nner;} iMgilxSf \eng{man, men).} 
\num{2} (\kanmu\ jamARYxnikf BASegaLalilx) idanunx sUcisalu savxrAkaSxrada meVle hAkuva eraDu cukekx\eng{$\ddot\bg\phantom\bg a\eg\eg$} gaLa cihenx; savxravisaMdhi cihenx, \udA\ \eng{M}\eng{$\ddot\bg\rm a\eg$}\eng{nner.} 
\enum
\emng
\eentry

\bentry
\word[umlaut(2)]{umlaut} 
\pron{umflwTf} 
\gl{\sakirx}
\expl{}
\bmng
savxrarUpAMtara mADu; savxravayxtayxya mADu; oMdu racane yA dhavxniyanunx savxrarUpAMtaradiMda badalisu, vayxtAyxsamADu. 
\emng
\eentry

\bentry
\word{umpirage} 
\pron{aMpeYarijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aMpeYrfgiri; madhayxsithxke; aMpeYrf yA madhayxsathxgArana adhikAra yA kelasa. 
\num{2} madhayxsathxgArana yA aMpeYrana niNaRya, tiVpuR, tiVmARna. 
\enum
\emng
\eentry

\bentry
\word[umpire(1)]{umpire} 
\pron{aMpeYarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} niNaRyakAra; nAyxyaniNARyaka; vAdigaLalilx BinAnxBipArxya baMdAga madhayxsithxke vahisalu yA nAyxya dorakisalu kareyuva mUraneyavanu. 
\num{2} madhayxsathxgAra; vAyxjayxvimashaRka; vAyxjayx baMdAga niNaRyisalu ArisalAguvavanu. 
\num{3} (kirxkeTf \mo\ ATagaLalilx) aMpeYru; ATada tiVpuRgAra; ATagaLa niyama pAlane hAgU vAyxjayx tiVmARnakekx neVmakavAdavanu. 
\enum
\emng
\eentry

\bentry
\word[umpire(2)]{umpire} 
\pron{aMpeYarf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (ATa \mo vugaLalilx) aMpeYrf Agu 
\num{2} (vAyxjayx \mo vugaLalilx) madhayxsathxgAranAgu; madhayxsithxke naDesu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx \mo vara paravAgi, ATakekx) aMpeYrf Agu; aMpeYrf Agi vatiRsu. 
\num{2} madhayxsathxgAranAgu 
\enum
\emng
\eentry

\bentry
\word{umpireship} 
\pron{aMpeYarfSipf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{umpirage}{umpirage}. 
\emng
\eentry

\bentry
\word[umpteen(1)]{umpteen} 
\pron{aMpiTxVnf} 
\gl{\gu}
\expl{}
\bmng
(\ashi) 
\bnum
\num{1} halavaru; aneVkaru; bahaLa; vipariVta: \eng{he was the umpteenth person to arrive} alilxge baMda aneVkaralilx avanU obabx. 
\num{2} aneVka; bahaLa. 
\enum
\emng
\eentry

\bentry
\word[umpteen(2)]{umpteen} 
\pron{aMpiTxVnf} 
\gl{\sanA}
\expl{}
\bmng
asaMKAyxtaru; anidiRSaTx saMKeyxya halavaru. 
\emng
\eentry

\bentry
\word{umpteenth} 
\pron{aMpiTxVnftx} 
\gl{\gu}
\expl{}
\bmng
halavaralilxna; aneVkara naDuvaNa: \eng{umpteenth person} halavaralilx obabx. 
\emng
\eentry

\bentry
\word{umpty} 
\pron{aMpiTx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{umpteen(1)}{$^1$umpteen}. 
\emng
\eentry

\bentry
\wordnospeech{UN}{UN} 
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Nations.} 
\emng
\eentry

\bentry
\word{'un} 
\pron{anf}  
\gl{\nA}
\expl{}
\bmng
\sA (\AmA) oMdu; obabx: \eng{that's a good un} adu oLeLxyadu. \eng{he's a tough un} avanobabx jagagxda manuSayx. 
\emng
\eentry

\bentry
\word[un(1)]{un-} 
\pron{anf-} 
\gl{\pUparx}
\expl{}
\bmng
\bnum
\numi{1} guNavAcakagaLu matutx kaqdaMtagaLu hAgU avugaLiMda niSapxnanxvAda nAmapadagaLu matutx kirxyAvisheVSaNagaLige seVri keLagina athaRgaLanunx koDuva pUvaRparxtayxya 
\banum
\alnum{a} a--, ni--, nirf-- , anf--, --ilalxda, --alalxda, --Agirada: \eng{unusable, uncalled-for, uneducated, unfailing, unofficially, unhappiness.} 
\alnum{b} virudadhx; vayxtirikatx; viloVma; parxtiloVma: \eng{unselfish, unsociable, unscientific, unadvisable.} 
\eanum
\numie
\num{2} ilalxda; korateyiruva; rahita; aBAva: \eng{unrest, untruth.} 
\enum
\emng
\eentry

\bentry
\word[un(2)]{un-} 
\pron{anf-} 
\gl{\pUparx}
\expl{}
\bmng
kirxyApadagaLige, kelavomemx mAtarx nAmapadagaLige, seVrisuva, keLage sUcisiruva athaRgaLa kirxyApadagaLanunx rUpisalu baLasuva pUvaRparxtayxya. 
\bnum
\num{1} yAvudeV kirxye yA sithxtiyanunx pUvaRsithxtige hiMdirugisuvudu yA radudxmADuvudu: \eng{undress, unbar, unbind, unlock, unsettle.} 
\num{2} tegeduhAkuvudu yA beVpaRDisuvudu: \eng{unmask, unfrock.} 
\num{3} (-iMda) biDisuvudu; biDugaDegoLisuvudu: \eng{unburden, uncage, uncase, unfold.} 
\num{4} muMde aMte iradAgisuvudu, \udA\ \eng{unman}. 
\enum
\emng
\eentry

\bentry
\wordnospeech{UNA}{UNA}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Nations Association.} 
\emng
\eentry

\bentry
\word{unabashed} 
\pron{anabAYxSfDx} 
\gl{\gu}
\expl{}
\bmng
nAcikeyilalxda; nisasxMkoVcavAda; hiMjarike, mujugara -- ilalxda. 
\emng
\eentry

\bentry
\word{unabashedly} 
\pron{anabAYxSiDilx} 
\gl{\kirxvi}
\expl{}
\bmng
nAcikeyilalxde; saMkoVca, mujugara, hiMjarike -- ilalxde. 
\emng
\eentry

\bentry
\word{unabated} 
\pron{anabeVTiDf} 
\gl{\gu}
\expl{}
\bmng
(birugALi \mo vugaLa \vi) kaDameyAgada; nilalxda; oMdeV samanAgi iruva. 
\emng
\eentry

\bentry
\word{unabatedly} 
\pron{anabeVTiDilx} 
\gl{\kirxvi}
\expl{}
\bmng
(birugALi \mo vugaLa \vi) kaDameyAgade; nilalxde; oMdeV samane. 
\emng
\eentry

\bentry
\word{unable} 
\pron{aneVba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
shakatxnalalxda; asamathaR. 
\emng
\eentry

\bentry
\word{unabridged} 
\pron{anabirxjfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (garxMthapATha \mo vugaLa \vi) pUNaR; saMkeSxVpisilalxda; saMgarxha mADilalxda. 
\num{2} (hakukx \mo vugaLa \vi) asaMkucita; mitihAkada. 
\enum
\emng
\eentry

\bentry
\word{unabsorbed} 
\pron{anabfsAZ(sA)bfRDx} 
\gl{\gu}
\expl{}
\bmng
hiVrirada; aMtagaRtavAgada. 
\emng
\eentry

\bentry
\word{unacademic} 
\pron{anAYxkaDemikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pAMDitayxkekx, vidavxtitxge -- saMbaMdhisirada; vidavxtUpxNaRvalalxda. 
\num{2} asheYkaSxNika; vAyxsaMgakekx, adhayxyanakekx -- saMbaMdhisirada. 
\num{3} (vayxkitxya \vi) vAyxsaMgakekx, adhayxyanakekx -- ahaRnalalxda. 
\enum
\emng
\eentry

\bentry
\word{unaccented} 
\pron{anAYxkesxMTiDf} 
\gl{\gu}
\expl{}
\bmng
(ucAcxraNeya \vi) otutxhAkada; GAtavilalxda. 
\emng
\eentry

\bentry
\word{unacceptable} 
\pron{anakesxpaTxba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
opapxlAgada; anaMgiVkAyaR; asivxVkAyaR; aMgiVkArAhaRvalalxda; sivxVkArayoVgayx valalxda. 
\emng
\eentry

\bentry
\word{unacceptableness} 
\pron{anakesxpaTxbalfnisf} 
\gl{\nA}
\expl{}
\bmng
anaMgiVkAyaRte; opapxlAgadiruvike; aMgiVkArAhaRvalalxdiruvike. 
\emng
\eentry

\bentry
\word{unacceptably} 
\pron{anakesxpaTxbilx} 
\gl{\kirxvi}
\expl{}
\bmng
opapxlAgadaMte; anaMgiVkAyaRvAgi. 
\emng
\eentry

\bentry
\word{unacclaimed} 
\pron{anakelxVmfDx} 
\gl{\gu}
\expl{}
\bmng
apurasakxqqta; aparxshaMsita; hogaLike, parxshaMse -- ilalxda. 
\emng
\eentry

\bentry
\word{unaccommodating} 
\pron{anakAmaDeVTiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sahAya mADada; upakAra shiVlanalalxda; nidARkiSxNayx savxBAvada. 
\num{2} hoMdikoMDu hoVgada; hoMdikoLaLxda; sahakarisada. 
\enum
\emng
\eentry

\bentry
\word{unaccompanied} 
\pron{anakaMpaniDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oMTiyAda; EkAkiyAda; joteyalilx yArU ilalxda. 
\num{2} (\saM) pakakxvAdayxgaLilalxda. 
\enum
\emng
\eentry

\bentry
\word{unaccomplished} 
\pron{anaka(kA)MpilxSfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sAdhisilalxda; pUreYsilalxda. 
\num{2} ashikiSxta; akushala; saMsakxqqti, sushikaSxNa, kushalategaLu ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unaccountability} 
\pron{anakwMTabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kAraNa heVLalAgada, kAraNa tiLiyada -- sithxti. 
\num{2} munUnxhisalAgada yA vicitarxnaDevaLike. 
\num{3} beVjavAbAdxri(yiMdiruvudu). 
\enum
\emng
\eentry

\bentry
\word{unaccountable} 
\pron{anakwMTaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kAraNa -- heVLalAgada, tiLiyada; vivarisalAgada. 
\num{2} munUnxhisalAgada yA vicitarx naDateya. 
\num{3} javAbAdxri, hoNegArike -- ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unaccountableness} 
\pron{anakwMTabalfnisf} 
\bmng
= \hyperlink{unaccountability}{unaccountability}. 
\emng
\eentry

\bentry
\word{unaccountably} 
\pron{anakwMTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kAraNa heVLalAgadaMte; kAraNa tiLiyada riVtiyalilx. 
\num{2} vivarisalAgade. 
\num{3} munUnxhisalAgadaMte yA vicitarxvAgi. 
\num{4} javAbAdxri yilalxda hAge; hoNegArike ilalxda riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{unaccounted} 
\pron{anakwMTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} lekakx koDada; lekakxkekx sigada; 
\num{2} (kAraNa) vivarisada. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{unaccounted for} vivarisade hoVda: \eng{four planes came back but two are unaccounted for} nAlukx vimAnagaLu hiMtirugidavu, Adare eraDu lekakxkekx sikakxde hoVdavu. 
\emng
\eentry

\bentry
\word{unaccustomed} 
\pron{anakasaTxmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aBAyxsavilalxda; baLakeyilalxda; rUDhiyilalxda; vADikeyilalxda: \eng{as I am unaccustomed to speaking in public} nanage bahiraMgavAgi mAtanADuva aBAyxsavilalxdadxriMda. 
\num{2} vADikeyalalxda; rUDhiyalalxda; vicitarxvAda; vilakaSxNavAda: \eng{his unaccustomed silence} avana vilakaSxNavAda mwna. 
\enum
\emng
\eentry

\bentry
\word{unaccustomedly} 
\pron{anakasaTxmfDxli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aBAyxsavilalxde; rUDhiyilalxde. 
\num{2} vADikeyalalxde; vicitarxvAgi; vilakaSxNavAgi. 
\enum
\emng
\eentry

\bentry
\word{unacknowledged} 
\pron{anakAnxlijfDx} 
\gl{\gu}
\expl{}
\bmng
opipxkoLaLxda; anaMgiVkaqta. 
\emng
\eentry

\bentry
\word{unacquainted} 
\pron{anakevxVMTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx \mo varoMdige) aparicita; paricayavilalxda. 
\num{2} aBAyxsavilalxda; vADikeyilalxda; rUDhiyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unadaptable} 
\pron{anaDAYxpaTxba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
ogigxsikoLaLxlAgada; hoMdisikoLaLxlAgada. 
\emng
\eentry

\bentry
\word{unadapted} 
\pron{anaDAYxpiTxDf} 
\gl{\gu}
\expl{}
\bmng
ogigxsikoLaLxda; hoMdisikoLaLxda. 
\emng
\eentry

\bentry
\word{unaddressed} 
\pron{anaDerxsfTx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ kAgada \mo vugaLa \vi) viLAsavilalxda; viLAsarahita. 
\emng
\eentry

\bentry
\word{unadjacent} 
\pron{anajeVsa(sf)nfTx} 
\gl{\gu}
\expl{}
\bmng
magugxlalilxlalxda; pakakxdalilxlalxda; AsananxvAgirada. 
\emng
\eentry

\bentry
\word{unadopted} 
\pron{anaDApiTxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ogigxsikoLaLxda; tananxdanAnxgisada. 
\num{2} (\kanmu\ hosa rasetxgaLa \vi) sathxLiVya saMsethx vahisikoMDilalxda. 
\enum
\emng
\eentry

\bentry
\word{unadorned} 
\pron{anaDAnfDxR} 
\gl{\gu}
\expl{}
\bmng
alaMkAravilalxda; analaMkaqta; saraLa. 
\emng
\eentry

\bentry
\word{unadulterated} 
\pron{anaDalaTxreVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kalaberakeyilalxda; apapxTa; parishudadhxvAda. 
\num{2} asali; shuda dhx; pakAkx; tiVrA: \eng{unadulterated nonsense} shudadhx asaMbadadhx. 
\enum
\emng
\eentry

\bentry
\word{unadventurous} 
\pron{anaDevxMcarasf} 
\gl{\gu}
\expl{}
\bmng
sAhasakekx keYhAkada; dheYyaRvilalxda; sAhasa \mo vugaLa goDavege hoVgada. 
\emng
\eentry

\bentry
\word{unadventurously} 
\pron{anaDevxMcarasfli} 
\gl{\kirxvi}
\expl{}
\bmng
sAhasakekx keYhAkade; dheYyaRvilalxde; sAhasike ilalxde. 
\emng
\eentry

\bentry
\word{unadvertised} 
\pron{anAYxDavxTeYRsfZDx} 
\gl{\gu}
\expl{}
\bmng
parxcAra mADada; parxkaTapaDisada; jAhiVrupaDisada. 
\emng
\eentry

\bentry
\word{unadvisable} 
\pron{anaDevxYsaZba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} budidhx heVLalAgada; tidadxlAgada. 
\num{2} (vasutx \mo vugaLa \vi) yukatxvalalxda; sUkatxvalalxda; anucita. 
\enum
\emng
\eentry

\bentry
\word{unadvised} 
\pron{anaDevxYsfZDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} viveVcaneyilalxda; muMdAloVcaneyilalxda; duDukina. 
\num{2} salahe, budidhxvAda -- tegedukoMDilalxda; salahegAraru, budidhx heVLuvavaru -- ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unadvisedly} 
\pron{anaDevxYsiZDilx} 
\gl{\kirxvi}
\expl{}
\bmng
duDukiniMda; viveVcaneyilalxde; muMdAloVcane mADade. 
\emng
\eentry

\bentry
\word{unadvisedness} 
\pron{anaDevxYsiZDfnisf} 
\gl{\nA}
\expl{}
\bmng
viveVcaneyilalxdiruvike; duDuku; muMdAloVcane ilalxdiruvike. 
\emng
\eentry

\bentry
\word{unaffected} 
\pron{anapheZkiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxBAvakokxLapaDada; parxBAvitavAgada; parxBAva taTaTxda. 
\num{2} kaqtakate, naTane, \mo vugaLilalxda; niSakxpaTa; amAyaka; akaqtirxma; pArxmANika. 
\enum
\emng
\eentry

\bentry
\word{unaffectedly} 
\pron{anapheZkiTxDfli} 
\gl{\kirxvi}
\expl{}
\bmng
kaqtakateyilalxde; niSakxpaTavAgi; amAyakavAgi. 
\emng
\eentry

\bentry
\word{unaffectedness} 
\pron{anapheZkiTxDfnisf} 
\gl{\nA}
\expl{}
\bmng
akaqtirxmate; niSakxpaTa savxBAva; amAyakate. 
\emng
\eentry

\bentry
\word{unaffiliated} 
\pron{anaphiliETiDf} 
\gl{\gu}
\expl{}
\bmng
(yAvudeV saMsethx \mo vugaLige) seVrirada; saMbaMdhisirada; aMgavAgilalxda; oLapaTiTxlalxda. 
\emng
\eentry

\bentry
\word{unafraid} 
\pron{anapheZrxVDf} 
\gl{\gu}
\expl{}
\bmng
hedarada; niBaRya; Bayarahita. 
\emng
\eentry

\bentry
\word{unaided} 
\pron{anfEDiDf}
\gl{\gu}
\expl{}
\bmng
sahAyakarilalxda; otAtxse, beMbala -- ilalxda. 
\emng
\eentry

\bentry
\word{unalienable} 
\pron{anfElianabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
(\nAyxshA) = \hyperref{kandict_i.pdf}{I}{inalienable}{inalienable}. 
\emng
\eentry

\bentry
\word{unaligned} 
\pron{analeYnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} = \hyperref{kandict_n.pdf}{N}{non-aligned}{non-aligned}. 
\num{2} jategUDada; sAlugUDisada; sari hoMdisirada. 
\enum
\emng
\eentry

\bentry
\word{unalike} 
\pron{analeYkf} 
\gl{\gu}
\expl{}
\bmng
beVrebeVreyAgiruva; sadaqshavAgilalxda; hoVlikeyilalxda. 
\emng
\eentry

\bentry
\word{unalive} 
\pron{analeYvf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} jiVvarahita; pArxNashakitx, ceYtanayx, satatxvX -- ilalxda, rahita. 
\num{2} jAgarxtavAgilalxda; ecacxtitxlalxda. 
\enum
\emng
\eentry

\bentry
\word{unalleviated} 
\pron{analiVviETiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} upashamanavAgada; pariharisadiruva. 
\num{2} niraMtaravAda; oMdeV samane iruva; viciPxtitx yilalxda; virAmavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unallied} 
\pron{analeYDf} 
\gl{\gu}
\expl{}
\bmng
okUkxTakekx seVrada; oTuTxgUDada; meYtirxrahita; mitarxru yA sahAyakaru ilalxda. 
\emng
\eentry

\bentry
\word{unallowable} 
\pron{analoVaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
avakAshavilalxda; aMgiVkArAhaRvalalxda. 
\emng
\eentry

\bentry
\word{unalloyed} 
\pron{analAyfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} berakeyilalxda; berake mADada; amisharx; beresada. 
\num{2} pUtiR; saMpUNaR; parishudadhx: \eng{unalloyed joy} pUtiR saMtoVSa. 
\enum
\emng
\eentry

\bentry
\word{unalterable} 
\pron{anfAlaTxrabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
badalAyisalAgada; mApaRDisalAgadaMtha. 
\emng
\eentry

\bentry
\word{unalterableness} 
\pron{anfAlaTxrabalfnisf} 
\gl{\nA}
\expl{}
\bmng
badalAyisalAgadiruvike. 
\emng
\eentry

\bentry
\word{unalterably} 
\pron{anfAlaTxrabilx} 
\gl{\kirxvi}
\expl{}
\bmng
badalAyisalAgadaMte. 
\emng
\eentry

\bentry
\word{unaltered} 
\pron{anfAlaTxrfDx} 
\gl{\gu}
\expl{}
\bmng
mApaRDisada; palalxTisada; badalAyisada; vayxtAyxsapaDisada; idadxMteyeV iruva. 
\emng
\eentry

\bentry
\word{unamazed} 
\pron{anameVsfZDx} 
\gl{\gu}
\expl{}
\bmng
cakitagoLaLxda; AshacxyaRgoLaLxda; avisimxta; beragugoLaLxda. 
\emng
\eentry

\bentry
\word{unambiguity} 
\pron{anAYxMbigUyxiTi} 
\gl{\nA}
\expl{}
\bmng
nisasxMdeVha; asaMdigadhxte; susapxSaTxte. 
\emng
\eentry

\bentry
\word{unambiguous} 
\pron{anAYxMbiguyxasf} 
\gl{\gu}
\expl{}
\bmng
(athaRdalilx) asaMdigadhx; saMdeVhavilalxda; davxMdAvxthaRvilalxda; saraLa; neVra; sapxSaTx yA niSakxqqSaTxvAda. 
\emng
\eentry

\bentry
\word{unambiguously} 
\pron{anAYxMbiguyxasfli} 
\gl{\kirxvi}
\expl{}
\bmng
asaMdigadhxvAgi; susapxSaTxvAgi; niSakxqqSaTxvAgi. 
\emng
\eentry

\bentry
\word{unambitious} 
\pron{anAYxMbiSasf} 
\gl{\gu}
\expl{}
\bmng
hebabxyakeyilalxda; mahatAtxvXkAMkeSxyilalxda; mahadAsheyilalxda. 
\emng
\eentry

\bentry
\word{unambitiously} 
\pron{anAYxMbiSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
hebabxyake yilalxde; mahatAtxvXkAMkeSxyilalxde. 
\emng
\eentry

\bentry
\word{unambitiousness} 
\pron{anAYxMbiSasfnisf} 
\gl{\nA}
\expl{}
\bmng
hebabxyakeyilalxdiruvike; mahatAtxvXkAMkeSxrAhitayx. 
\emng
\eentry

\bentry
\word{unambivalent} 
\pron{anAYxMbivalaMTf} 
\gl{\gu}
\expl{}
\bmng
(BAvanegaLu \mo vugaLa \vi) caMcalavAgirada; hoyAdxTavilalxda; sithxra; nishicxtavAda; neVra; Quju. 
\emng
\eentry

\bentry
\word{unambivalently} 
\pron{anAYxMbivalaMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
caMcalavAgirade; hoyAdxTavilalxda; sithxravAgi; neVravAgi. 
\emng
\eentry

\bentry
\word{unamenable} 
\pron{anamiVnabflf} 
\gl{\gu}
\expl{}
\bmng
sagagxda; bagagxda; tidadxlAgada. 
\emng
\eentry

\bentry
\word{un-American} 
\pron{anamerikanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} amerikadadxlalxda; amerikanf padadhxtige, BAvane gaLige, lakaSxNagaLige -- horatAda. 
\num{2} amerika saMyukatxsaMsAthxnagaLa hitAsakitxgaLige viroVdhavAda; amerikanf viroVdhiyAda. 
\num{3} (\ame) rASaTxrXdorxVhada. 
\enum
\emng
\eentry

\bentry
\word{un-Americanism} 
\pron{anamerikanisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} amerikanf padadhxtige, BAvanegaLige, lakaSxNagaLige -- horatAgiruvike, BinanxvAgiruvike. 
\num{2} amerikanf viroVdha. 
\num{3} (\ame) rASaTxrXdorxVha. 
\enum
\emng
\eentry

\bentry
\word{unamiable} 
\pron{anamiabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} senxVhaBAvavilalxda; vishAvxsaparavalalxda; swhAdaRvilalxda. 
\num{2} pirxVtiyilalxda; vishAvxsavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unamplified} 
\pron{anAYxMpilxpheZYDf} 
\gl{\gu}
\expl{}
\bmng
higigxsalAgada; visatxrisalAgada; hecicxsalAgada; avisatxqqta. 
\emng
\eentry

\bentry
\word{unamused} 
\pron{anamUyxsfZDx} 
\gl{\gu}
\expl{}
\bmng
hAsayx garxhisada; vinoVda, tamASe -- rucisada, hiDisada. 
\emng
\eentry

\bentry
\word{unanalysable} 
\pron{anAYxnaleYsaZbflf} 
\gl{\gu}
\expl{}
\bmng
avishelxVSaNiVya; viBajisalAgada; vishelxVSisalAgada. 
\emng
\eentry

\bentry
\word{unanalysed} 
\pron{anAYxnaleYsfZDx}
\gl{\gu}
\expl{}
\bmng
viBajisada; vishelxVSisada. 
\emng
\eentry

\bentry
\word{unanchored} 
\pron{anAYxMkaDfR} 
\gl{\gu}
\expl{}
\bmng
(nwkeya \vi) laMgaru etitxda; laMgaru tegeda. 
\emng
\eentry

\bentry
\word{unaneled} 
\pron{ananiVlfDx} 
\gl{\gu}
\expl{(\roVkAyx) (\pArxparx) }
\bmng
aMtAyxBayxMjanavilalxda; aMtayxkAladalilx (aBayxMjana \mo) matakamaRgaLanunx paDeyada. 
\emng
\eentry

\bentry
\word{unanimity} 
\pron{yUnanimiTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} savARnumata; omamxta. 
\num{2} EkABipArxya. 
\enum
\emng
\eentry

\bentry
\word{unanimous} 
\pron{yUnAYxnimasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} elalxrU oMdeV manasusxLaLx; EkABipArxyada: \eng{the committee was unanimous} saBe EkABipArxya hoMditutx. 
\num{2} (aBipArxya, mata, \mo vugaLa \vi) savARnumatada; omamxtada: \eng{he was elected by a unanimous vote} avananunx savARnumatadiMda ArisalAyitu. 
\enum
\emng
\eentry

\bentry
\word{unanimously} 
\pron{yUnAYxnimasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} savARnumatadiMda; omamxtavAgi. 
\num{2} EkABipArxyadiMda. 
\enum
\emng
\eentry

\bentry
\word{unanimousness} 
\pron{yUnAYxnimasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} savARnumataviruvike; omamxtasithxti. 
\num{2} EkABipArxyaviruvike. 
\enum
\emng
\eentry

\bentry
\word{unannounced} 
\pron{ananwnfsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxkaTisada; GoVSisada; parxsidadhxgoLisada. 
\num{2} (obabxna Agamana \mo vanunx) shurxtapaDisada; tiLisada: \eng{he walked into the room unannounced} avanu tiLisade yA heVLade keVLade rUminoLakekx naDeda. 
\enum
\emng
\eentry

\bentry
\word{unanswerable} 
\pron{anAnasxrabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} utatxra, samAdhAna -- koDalAgada: \eng{an unanswerable question} utatxravilalxda, utatxra koDalAgada -- parxshenx. 
\num{2} parxti heVLalAgada; aparxtAyxKeyxVya; KaMDisalu sAdhayxvilalxda: \eng{has an unanswerable case} KaMDisalAgada parisithxti, parxsaMga. 
\enum
\emng
\eentry

\bentry
\word{unanswerably} 
\pron{anAnasxrabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} utatxra koDalAgada riVtiyalilx. 
\num{2} parxti heVLalAga daMte; aparxtAyxKeyxVyavAgi. 
\enum
\emng
\eentry

\bentry
\word{unanswered} 
\pron{anAnasxDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} utatxra, javAbu -- koTiTxlalxda: \eng{unanswered letters} javAbu koDade hoVda kAgadagaLu. 
\num{2} aparxtidatatx; hiMtirugisada; parxtiyAgi toVrisada: \eng{unanswered love} parxtikirxyeyilalxda perxVma. 
\enum
\emng
\eentry

\bentry
\word{unanticipated} 
\pron{anAYxMTisipeVTiDf} 
\gl{\gu}
\expl{}
\bmng
niriVkiSxsada; aniriVkiSxta. 
\emng
\eentry

\bentry
\word{unapostolic} 
\pron{anfAYxpasATxlikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kerxY) dhamaRparxvataRka padadhxtige virudadhxvAda. 
\num{2} (\kerxY) dhamaRpArxmANayxvilalxda; saMparxdAya bAhiravAda; kerxYsatxdhamaR parxvataRnAcAyaRra aMgiVkAravilalxda. 
\enum
\emng
\eentry

\bentry
\word{unapparent} 
\pron{anapAYxraMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaNiNxge kANuvaMtirada; parxtayxkaSxvalalxda; agoVcara. 
\num{2} asapxSaTx; sapxSaTxvilalxda. 
\enum
\emng
\eentry

\bentry
\word{unappealable} 
\pron{anapiVlabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
(\nAyxshA) (meVlina adhikArigaLige) manavi mADikoLaLxlAgada; meVlamxnavige avakAshavilalxda. 
\emng
\eentry

\bentry
\word{unappealing} 
\pron{anapiVliMgf} 
\gl{\gu}
\expl{}
\bmng
hitavAgada; manasisxge iSaTxvAgada; raMjisada; hiDisada; rucisada. 
\emng
\eentry

\bentry
\word{unappealingly} 
\pron{anapiVliMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
ahitavAgi; hitavAgade; rucisade; hiDisada riVtiyalilx. 
\emng
\eentry

\bentry
\word{unappeasable} 
\pron{anapiVsaZbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
taqpitxpaDisalAgada; taNisalu sAdhayxvilalxda; samAdhAnagoLisalu asAdhayxvAda. 
\emng
\eentry

\bentry
\word{unappeased} 
\pron{anapiVsfZDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} taqpitxpaDisada; taNisada; samAdhAnagoLisada. 
\num{2} ataqpatx; taqpatxvAgada; samAdhAnagoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{unappetizing} 
\pron{anAYxpiTeYsiZMgf} 
\gl{\gu}
\expl{}
\bmng
hasivuMTumADada; ruci huTiTxsada. 
\emng
\eentry

\bentry
\word{unappetizingly} 
\pron{anAYxpiTeYsiZMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
hasivuMTumADade; ruci huTiTxsade. 
\emng
\eentry

\bentry
\word{unapplied} 
\pron{anapelxYDf} 
\gl{\gu}
\expl{}
\bmng
anavxyisada; Anavxyikavalalxda; pArxyoVgikavalalxda; ananivxta. 
\emng
\eentry

\bentry
\word{unappreciated} 
\pron{anapirxVSiETiDf} 
\gl{\gu}
\expl{}
\bmng
sAvxrasayx garxhisada; AsAvxdayxvAgada; shAlxGisada; mecicxke gaLisada. 
\emng
\eentry

\bentry
\word{unappreciative} 
\pron{anapirxVSaTivf} 
\gl{\gu}
\expl{}
\bmng
sAvxrasayxgarxhaNashakitxyilalxda; guNagArxhiyalalxda; mecacxda. 
\emng
\eentry

\bentry
\word{unapproachability} 
\pron{anaporxVcabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} eTukadiruvike; talupalAgadiruvike. 
\num{2} (vayxkitxya \vi) senxVhaparanalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unapproachable} 
\pron{anaporxVcabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eTukada; talupalAgada; hatitxrakekx hoVgalAgadaMtha; sulaBasAdhayxvalalxda. 
\num{2} hoVlikege eTukada; anupama. 
\num{3} (vayxkitxya \vi) sulaBanalalxda; senxVhaparanalalxda. 
\enum
\emng
\eentry

\bentry
\word{unapproachableness} 
\pron{anaporxVcabalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unapproachability}{unapproachability}. 
\emng
\eentry

\bentry
\word{unapproachably} 
\pron{anaporxVcabilx} 
\gl{\kirxvi}
\expl{}
\bmng
samiVpisalu kaSaTxvAgi. 
\emng
\eentry

\bentry
\word{unappropriated} 
\pron{anaporxVpirxETiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (yAradeV) vashadalilx, sAvxdhiVnadalilx -- ilalxda; yAreV vashapaDisikoMDilalxda. 
\num{2} (yArigAgali) gotutxpaDisada; nigadimADada; miVsalAgirisada. 
\enum
\emng
\eentry

\bentry
\word{unapproved} 
\pron{anapUrxvfDx} 
\gl{\gu}
\expl{}
\bmng
anumatisada; opapxda; maMjUru mADada. 
\emng
\eentry

\bentry
\word{unapt} 
\pron{anAYxpfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sarihoMdada; sariyAgada. 
\num{2} (mADalu) ucitavalalxda; yoVgayxvalalxda; takukxdalalxda; yukatxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unaptly} 
\pron{anAYxpfTxli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} anucita riVtiyalilx; yoVgayxvalalxda hAge; takukxdalalxdaMte. 
\num{2} sarihoMdadaMte. 
\enum
\emng
\eentry

\bentry
\word{unaptness} 
\pron{anAYxpfTxnisf} 
\gl{\nA}
\expl{}
\bmng
anwcitayx; ucitavalalxdiruvudu; yukatxvalalxdiruvudu. 
\emng
\eentry

\bentry
\word{unarguable} 
\pron{anAguRyxabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
vAdisalu sAdhayxvAgada; vAdisalAgada. 
\emng
\eentry

\bentry
\word{unarm} 
\pron{anAmfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} nirAyudhagoLisu; nirAyudhananAnxgi mADu. 
\num{2} (matotxbabxna) rakASxkavaca tegeduhAku. 
\enum
\emng
\eentry

\bentry
\word{unarmed} 
\pron{anAmfDxR} 
\gl{\gu}
\expl{}
\bmng
nirAyudha; nirasatxrXnAda; shasatxrXgaLilalxda. 
\emng
\eentry

\bentry
\word{unarresting} 
\pron{anaresiTxMgf} 
\gl{\gu}
\expl{}
\bmng
anAkaSaRkavAda; daqSiTx, gamana, Asakitx, \mo vanunx -- seLeyada. 
\emng
\eentry

\bentry
\word{unarrestingly} 
\pron{anaresiTxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
anAkaSaRkavAgi; gamana \mo vanunx seLeyade. 
\emng
\eentry

\bentry
\word{unarticulated} 
\pron{anATiRkuyxleVTiDf} 
\gl{\gu}
\expl{}
\bmng
(mAtu, daqSiTx, \mo vugaLa \vi) asapxSaTxvAda; asuPxTavAda. 
\emng
\eentry

\bentry
\word{unartistic} 
\pron{anATiRsiTxkf} 
\gl{\gu}
\expl{}
\bmng
kalAtamxkavalalxda; (\kanmu) kalege saMbaMdhisirada. 
\emng
\eentry

\bentry
\word{unartistically} 
\pron{anATiRsiTxkali} 
\gl{\kirxvi}
\expl{}
\bmng
kalArahitavAgi; kalAtamxkavAgilalxde, (\kanmu) kalege saMbaMdhisilalxde. 
\emng
\eentry

\bentry
\word{unascertainable} 
\pron{anAYxsaTeRVnabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
gotutxmADalAgada; kaMDuhiDiyalAgada; nidhaRrisalAgada. 
\emng
\eentry

\bentry
\word{unascertained} 
\pron{anAYxsaTeRVnfDx} 
\gl{\gu}
\expl{}
\bmng
gotutx mADada; kaMDuhiDiyada; anidhARrita. 
\emng
\eentry

\bentry
\word{unashamed} 
\pron{anaSeVmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nAcikeyilalxda; nilaRjajx; lajAjxhiVna. 
\num{2} saMkoVcapaDada; aLukada; mujugara paDada. 
\enum
\emng
\eentry

\bentry
\word{unashamedly} 
\pron{anaSeVmiDilx} 
\gl{\kirxvi}
\expl{}
\bmng
nAcikegeTuTx; nilaRjajxnAgi; nisasxMkoVcadiMda. 
\emng
\eentry

\bentry
\word{unashamedness} 
\pron{anaSeVmiDfnisf} 
\gl{\nA}
\expl{}
\bmng
nilaRjejx; nAcikeyilalxdiruvike; nisasxMkoVca; lajAjxhiVnate. 
\emng
\eentry

\bentry
\word{unasked} 
\pron{anAsfkxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keVLada; apaqSaTx. 
\num{2} apArxthiRta; beVDada; koVrada. 
\num{3} AhAvxnisada; kareyada; anAmaMtirxta; anAhUta. 
\enum
\emng
\eentry

\bentry
\word{unassailability} 
\pron{anaseVlabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anAkarxmaNiVyate; ajeVyate; aBeVdayxte; AkarxmaNa sAdhayxvilalxdiruvike. 
\num{2} AkeSxVpisalAgadiruvike; TiVke mADalAgadiruvike; parxtiheVLalAgadiruvike; aparxtAyxKeyxVyate. 
\enum
\emng
\eentry

\bentry
\word{unassailable} 
\pron{anaseVlabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AkarxmaNa sAdhayxvalalxda; anAkarxmaNiVya. 
\num{2} (heVLike \mo vugaLa \vi) parxti heVLalAgada; TiVkisalAgada; AkeSxVpisalAgada; aparxtAyxKeyxVya. 
\enum
\emng
\eentry

\bentry
\word{unassailableness} 
\pron{anaseVlabalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unassailability}{unassailability}. 
\emng
\eentry

\bentry
\word{unassailably} 
\pron{anaseVlabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anAkarxmaNiVyavAda riVtiyalilx. 
\num{2} parxti heVLalAgadaMte; AkeSxVpisalAgada hAge. 
\enum
\emng
\eentry

\bentry
\word{unassertive} 
\pron{anasaTiRvf} 
\gl{\gu}
\expl{}
\bmng
(vayxkitxya \vi) daqDhavAgi sAdhisada; tananxnunx samathiRsikoLaLxda; paTuTx hiDiyada. 
\emng
\eentry

\bentry
\word{unassertively} 
\pron{anasaTiRvfli} 
\gl{\kirxvi}
\expl{}
\bmng
(vayxkitxya \vi) paTuTx hiDiyade; daqDhavAgi sAdhisade. 
\emng
\eentry

\bentry
\word{unassertiveness} 
\pron{anasaTiRvfnisf} 
\gl{\nA}
\expl{}
\bmng
(vayxkitxya \vi) daqDhavAgilalxdiruvike; paTuTx hiDiyadiruvike. 
\emng
\eentry

\bentry
\word{unassignable} 
\pron{anaseYnabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} haMcikoDalAgada; vahisikoDalAgada; opipxsi koDalAgada. 
\num{2} kAraNa koDalAgada, tiLisalAgada. 
\enum
\emng
\eentry

\bentry
\word{unassigned} 
\pron{anaseYnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} haMcikoDada; vahisikoDada; opipxsikoDadiruva. 
\num{2} kAraNa koDada. 
\enum
\emng
\eentry

\bentry
\word{unassimilable} 
\pron{anasimilabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
aragisikoLaLxlAgada; meYgUDisikoLaLxlAgada. 
\emng
\eentry

\bentry
\word{unassimilated} 
\pron{anasimileVTiDf} 
\gl{\gu}
\expl{}
\bmng
aragisikoLaLxda; meYgUDisikoLaLxda. 
\emng
\eentry

\bentry
\word{unassisted} 
\pron{anasisiTxDf} 
\gl{\gu}
\expl{}
\bmng
neravilalxda; sahAyavilalxda; asahAyaka. 
\emng
\eentry

\bentry
\word{unassuageable} 
\pron{anasevxVjabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
(noVvu \mo vanunx) tagigxsalAgada; kaDame goLisalAgada; shamana mADalAgada. 
\emng
\eentry

\bentry
\word{unassuaged} 
\pron{anasevxVjfDx} 
\gl{\gu}
\expl{}
\bmng
(noVvu \mo vanunx) tagigxsada; kaDame mADada; shamanagoLisada. 
\emng
\eentry

\bentry
\word{unassuming} 
\pron{anasUyxmiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nigaviR; nirahaMkAri; ahaMkAravilalxda. 
\num{2} viniVta; ADaMbaravilalxda. 
\enum
\emng
\eentry

\bentry
\word{unassumingly} 
\pron{anasUyxmiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
nigaviRyAgi; ahaMkAravilalxde; viniVta BAvadiMda. 
\emng
\eentry

\bentry
\word{unassumingness} 
\pron{anasUyxmiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
nigavaRte; viniVtaBAva; ahaMkAravilalxdiruvike. 
\emng
\eentry

\bentry
\word{unatoned} 
\pron{anaToVnfDx} 
\gl{\gu}
\expl{}
\bmng
pArxyashicxtatx mADada, Agada. 
\emng
\eentry

\bentry
\word{unattached} 
\pron{anaTAYxcfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (nidiRSaTx saMGa, saMsethx, \mo vugaLige) saMbaMdhapaTiTxlalxda; seVrilalxda; aMTikoMDilalxda; Asakitx ilalxda; niliRpatx. 
\num{2} yAvudeV saMGa, pakaSx, daLa yA paMthakUkx seVrirada. 
\num{3} oMTiyAda; avivAhita; yAra jateyU maduve opapxMda yA maduve -- mADikoLaLxdiruva. 
\enum
\emng
\eentry

\bentry
\word{unattackable} 
\pron{anaTAYxkaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AkarxmaNiVyavalalxda; dALimADalAgada. 
\num{2} hAnipaDisalAgada. 
\enum
\emng
\eentry

\bentry
\word{unattainable} 
\pron{anaTeVnabflf} 
\gl{\gu}
\expl{}
\bmng
apArxpayx; sAdhisalAgada; paDeyalAgada. 
\emng
\eentry

\bentry
\word{unattainableness} 
\pron{anaTeVnabflfnisf} 
\gl{\nA}
\expl{}
\bmng
apArxpayxte; asAdhayxte; sAdhisalAgadiruvike; paDeyalAgadiruvike. 
\emng
\eentry

\bentry
\word{unattainably} 
\pron{anaTeVnabilx} 
\gl{\kirxvi}
\expl{}
\bmng
sAdhisalAgade; paDeyalAgada riVtiyalilx. 
\emng
\eentry

\bentry
\word{unattempted} 
\pron{anaTeMpiTxDf} 
\gl{\gu}
\expl{}
\bmng
parxyatanx mADada; yatinxsada; aparxyatinxta. 
\emng
\eentry

\bentry
\word{unattended} 
\pron{anaTeMDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (saBe \mo vugaLa \vi) perxVkaSxkarilalxda; shorxVtaqgaLilalxda. 
\num{2} (vayxkitx, vAhana, \mo vugaLa \vi) yArU ilalxda; yAra jateyU ilalxda; oMTiyAda. 
\num{3} noVDi koLuLxvavaru yArU ilalxda; upeVkiSxta: \eng{to leave small children unattended at home} cikakxmakakxLanunx oMTiyAgi maneyalilx biTuTx. 
\enum
\emng
\eentry

\bentry
\word{unattractive} 
\pron{anaTArxYxkiTxvf} 
\gl{\gu}
\expl{}
\bmng
AkaSaRkavalalxda; suMdaravalalxda. 
\emng
\eentry

\bentry
\word{unattractively} 
\pron{anaTArxYxkiTxvfli} 
\gl{\kirxvi}
\expl{}
\bmng
anAkaSaRkavAgi; AkaSaRkavAgilalxde; suMdaravAgilalxde. 
\emng
\eentry

\bentry
\word{unattractiveness} 
\pron{anaTArxYxkiTxvfnisf} 
\gl{\nA}
\expl{}
\bmng
anAkaSaRkate; suMdaravAgilalxdiruvike. 
\emng
\eentry

\bentry
\word{unattributable} 
\pron{anaTirxbuyxTaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ samAcArada \vi) ajAcnxta mUlada; mUla sUcisalAgada. 
\emng
\eentry

\bentry
\word{unattributably} 
\pron{anaTirxbuyxTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
(\kanmu\ samAcArada \vi) mUladedxMdu heVLalAgadaMte. 
\emng
\eentry

\bentry
\word{unau} 
\pron{yUnA, yanw} 
\gl{\nA}
\expl{}
\bmng
(\pArxvi) eraDu kAlebxraLugaLuLaLx, oMdu bageya kADupApa. 
\emng
\eentry

\bentry
\word{unauthentic} 
\pron{anAteMTikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nirAdhAra; parxmANarahita; niSapxrXmANaka. 
\num{2} (\nAyxshA) vidhibadadhxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unauthentically} 
\pron{anAteMTikali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} nirAdhAravAgi; parxmANapUvaRkavAgirade; niSapxrXmANakavAgi. 
\num{2} (\nAyxshA) vidhibadadhxvAgirade. 
\enum
\emng
\eentry

\bentry
\word{unauthenticated} 
\pron{anAteMTikeVTiDf} 
\gl{\gu}
\expl{}
\bmng
nirAdhAravAda; parxmANiVkarisada; daqDhiVkarisada. 
\emng
\eentry

\bentry
\word{unauthorised} 
\pron{anAtareYsfZDx} 
\gl{\gu}
\expl{}
\bmng
(adhikAri \mo vara) apapxNe ilalxda; anadhikaqta. 
\emng
\eentry

\bentry
\word{unauthorized} 
\pron{anAtareYsfZDx} 
\gl{\gu}
\bmng
= \hyperlink{unauthorised}{unauthorised}. 
\emng
\eentry

\bentry
\word{unavailability} 
\pron{anaveVlabiliTi} 
\gl{\nA}
\expl{}
\bmng
sikakxdiruvike; apArxpatxte; alaBayxte. 
\emng
\eentry

\bentry
\word{unavailable} 
\pron{anaveVlabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
sikakxda; doreyada; apArxpatx; alaBayx. 
\emng
\eentry

\bentry
\word{unavailableness} 
\pron{anaveVlabalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unavailability}{unavailability}. 
\emng
\eentry

\bentry
\word{unavailing} 
\pron{anaveVliMgf} 
\gl{\gu}
\expl{}
\bmng
PalakAriyAgada; nirathaRka; niSapxrXyoVjaka; vayxthaR; EnanUnx sAdhisada. 
\emng
\eentry

\bentry
\word{unavailingly} 
\pron{anaveVliMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
nirathaRkavAgi; niSapxrXyoVjakavAgi; vayxthaRvAgi; EnanUnx sAdhisade. 
\emng
\eentry

\bentry
\word{unavoidability} 
\pron{anavAyaDxbiliTi} 
\gl{\nA}
\expl{}
\bmng
anivAyaRte; tapipxsikoLaLxlAgadiruvike. 
\emng
\eentry

\bentry
\word{unavoidable} 
\pron{anavAyaDxbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
anivAyaR; nivArisalAgada; tapipxsikoLaLxlAgada. 
\emng
\eentry

\bentry
\word{unavoidableness} 
\pron{anavAyaDxbalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unavoidability}{unavoidability}. 
\emng
\eentry

\bentry
\word{unavoidably} 
\pron{anavAyaDxbilx} 
\gl{\kirxvi}
\expl{}
\bmng
anivAyaRvAgi; tapipxsikoLaLxlAgadaMte. 
\emng
\eentry

\bentry
\word{unavowed} 
\pron{anavw} 
\gl{\gu}
\expl{}
\bmng
opipxkoLaLxda; aMgiVkarisada; kaMThoVkatxvAgi heVLada; anukatx. 
\emng
\eentry

\bentry
\word[unaware(1)]{unaware} 
\pron{anaveVrf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} arivilalxda; parijAcnxnavilalxda; pariveyilalxda. 
\num{2} (vayxkitxya \vi) garxhisada; saMveVdiyalalxda. 
\enum
\emng
\eentry

\bentry
\word[unaware(2)]{unaware} 
\pron{anaveVrf} 
\gl{\kirxvi}
\expl{}
\bmng
= \hyperlink{unawares}{unawares}. 
\emng
\eentry

\bentry
\word{unawareness} 
\pron{anaveVrfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ajacnxte; arivilalxdiruvike; ajAcnxna. 
\num{2} asaMveVditavx. 
\enum
\emng
\eentry

\bentry
\word{unawares} 
\pron{anaveVsfZR} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aniriVkiSxtavAgi; idadxkikxdadx hAge; haThAtAtxgi: \eng{came upon them unawares} avaranunx haThAtAtxgi saMdhisida. 
\num{2} pariveyilalxde; tiLiyade; parijAcnxnavilalxde; arivilalxde: \eng{must have dropped it unawares} adanunx elolxV arivilalxde udurisirabeVku. 
\enum
\emng
\eentry

\bentry
\word{unbacked} 
\pron{anfbAYxkfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} beMbala koDuvavarilalxda; sahAyakarilalxda. 
\num{2} (jUju, paMdayx, \mo vugaLalilx) paMtha, paMdayx -- kaTuTxvavarilalxda. 
\num{3} (kuciR, citarx, \mo vugaLa \vi) beninxlalxda; benAnxsareyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unbag} 
\pron{anfbAYxgf} 
\gl{\sakirx}
\expl{}
\bmng
ciVladiMda horatege. 
\emng
\eentry

\bentry
\word[unbalance(1)]{unbalance} 
\pron{anfbAYxlanfsx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} samatUka tapipxsu. 
\num{2} (deVha, manasusx, \mo vanunx) hadageDisu; keDisu; ErupeVrAgisu; samatoVla tapipxsu: \eng{unbalanced by the blow} ETiniMda samatoVla tapipxtu. \eng{the shock unbalanced him} AGAta avananunx hadageDisitu. 
\enum
\emng
\eentry

\bentry
\word[unbalance(2)]{unbalance} 
\pron{anfbAYxlanfsx} 
\gl{\nA}
\expl{}
\bmng
(\kanmu\ mAnasika) asamatoVla; asithxrate; asavxsathxte. 
\emng
\eentry

\bentry
\word{unbalanced} 
\pron{anfbAYxlanfsxTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} samatUkavilalxda. 
\num{2} (vayxkitx, manasusx, \mo vugaLa \vi) samatoVla tapipxda; budidhxpalalxTavAda; talekeTaTx. 
\enum
\emng
\eentry

\bentry
\word{unbar} 
\pron{anfbArf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unbarred}; \vakaq\ \eng{unbarring}).}
\bmng
\bnum
\num{1} (kiTaki, bAgilu, \mo vugaLige hAkida) kaMbi, salAkigaLanunx -- tege, kiVLu, muri. 
\num{2} bAgilu tere; tApALu, aguLi -- tege: \eng{unbar a door} bAgilina aguLi tege. 
\enum
\emng
\eentry

\bentry
\word{unbearable} 
\pron{anfbeVrabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
asahaniVya; dusasxhaniVya; sahisalasAdhayxvAda. 
\emng
\eentry

\bentry
\word{unbearableness} 
\pron{anfbeVrabalfnisf} 
\gl{\nA}
\expl{}
\bmng
asahaniVyate; dusasxhaniVyate; sahisalasAdhayxvAgiruvike. 
\emng
\eentry

\bentry
\word{unbearably} 
\pron{anfbeVrabilx} 
\gl{\kirxvi}
\expl{}
\bmng
asahaniVyavAgi; dusasxhaniVyavAgi. 
\emng
\eentry

\bentry
\word{unbeatable} 
\pron{anfbiVTabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
ajeVya; dujaRya; miVrisalasadaLavAda; soVlisalAgada. 
\emng
\eentry

\bentry
\word{unbeaten} 
\pron{anfbiVT(Tf)nf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} soVtilalxda; ajita. 
\num{2} (vikarxma, dAKale, \mo vugaLa \vi) miVrisada; miVrada; atishayada. 
\num{3} (hAdi, jADu, \mo vugaLa \vi) yArU tuLidilalxda, ODADilalxda: \eng{unbeaten paths} yArU tuLiyada hAdigaLu. 
\num{4} (kirxkeTf) (ATagArana \vi) auTAgada; ATadiMda nigaRmisada. 
\enum
\emng
\eentry

\bentry
\word{unbecoming} 
\pron{anfbikamiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mayARdegeTaTx; yoVgayxvalalxda; tagada: \eng{an unbecoming speech} mayARde miVrida BASaNa. 
\num{2} (obabx vayxkitxya yoVgayxtege) takukxdalalxda; anucita; tagada; yoVgayxvalalxda. 
\num{3} (\kanmu\ uDupina \vi) (dharisiruvavanige) hoMdada; opapxda; lakaSxNavenisada: \eng{an unbecoming hat} avalakaSxNavAda ToVpi. 
\enum
\emng
\eentry

\bentry
\word{unbecomingly} 
\pron{anfbikamiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} mayARdegeTuTx; ayoVgayxvAgi; anucitavAgi; takukxdAgiradaMte. 
\num{2} avalakaSxNavAgi; kurUpavAgi; opapxda riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{unbecomingness} 
\pron{anfbikamiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anwcitayx; ayoVgayxte; mayARdegeTaTx naDavaLike \mo vu. 
\num{2} avalakaSxNavAgiruvudu. 
\enum
\emng
\eentry

\bentry
\word{unbefitting} 
\pron{anfbiphiTiMgf} 
\gl{\gu}
\expl{}
\bmng
takukxdalalxda; hoMdada; yoVgayxvalalxda; salalxda; opapxda; sarihoVgada. 
\emng
\eentry

\bentry
\word{unbefittingly} 
\pron{anfbiphiTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
takukxdalalxdAdxgi; anucitavAgi; hoMdade; salalxda, opapxda -- riVtiyalilx. 
\emng
\eentry

\bentry
\word{unbefittingness} 
\pron{anfbiphiTiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
anwcitayx; hoMdadiruvike; opapxda, salalxda -- sithxti. 
\emng
\eentry

\bentry
\word{unbefriended} 
\pron{anfbipherxMDiDf} 
\gl{\gu}
\expl{}
\bmng
senxVhitarirada; neravilalxda; nisasxhAyakanAda. 
\emng
\eentry

\bentry
\word{unbegotten} 
\pron{anfbigAT(Tf)nf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ajAta; inUnx -- huTaTxdiruva, janisadiruva. 
\num{2} huTiTxlalxda; janamxrahita; modalilalxda; anAdi. 
\enum
\emng
\eentry

\bentry
\word{unbeholden} 
\pron{anfbihoVlaDxnf} 
\gl{\gu}
\expl{}
\bmng
upakaqtanAgirada; QuNiyAgirada; kaqtajacnxnAgilalxda. 
\emng
\eentry

\bentry
\word{unbeknown} 
\pron{anfbinoVnf} 
\gl{\gu}
\expl{}
\bmng
(\AmA) tiLiyada; gotAtxgada; ajAcnxta: \eng{was there all the time unbeknown to us} namage gotAtxgadaMte aSuTx kAlavU alilxyeV idadx. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{unbeknown to} tiLiyagoDade; tiLivaLikege bArade, tarade: \eng{stole out unbeknown to us} namage yArigU tiLiyadaMte horakekx nusuLida. 
\emng
\eentry

\bentry
\word{unbeknownst} 
\pron{anfbinoVnfsfTx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unbeknown}{unbeknown}. 
\emng
\eentry

\bentry
\word{unbelief} 
\pron{anfbiliVphf} 
\gl{\nA}
\expl{}
\bmng
(\kanmu\ mata, garxMtha, dhamaR, \mo vugaLalilx) apanaMbike; avishAvxsa; asharxdedhx; naMbikeyilalxdiruvudu. 
\emng
\eentry

\bentry
\word{unbelievability} 
\pron{anfbiliVvabiliTi} 
\gl{\nA}
\expl{}
\bmng
naMbalAgadiruvike; avishavxsaniVyate; vishAvxsAhaRvalalxdiruvike. 
\emng
\eentry

\bentry
\word{unbelievable} 
\pron{anfbiliVvaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
naMbalAgada; avishAvxsaniVya; vishAvxsAhaRvalalxda. 
\emng
\eentry

\bentry
\word{unbelievableness} 
\pron{anfbiliVvabalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unbelievability}{unbelievability}. 
\emng
\eentry

\bentry
\word{unbelievably} 
\pron{anfbiliVvabilx} 
\gl{\kirxvi}
\expl{}
\bmng
naMbalAgadaMte; avishAvxsaniVyavAgi; vishAvxsAhaRvAgirade. 
\emng
\eentry

\bentry
\word{unbeliever} 
\pron{anfbiliVvarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nAsitxka; niriVshavxravAdi; deVvaralilx naMbikeyilalxdavanu. 
\num{2} (matadhamaR \mo vugaLalilx) naMbikeyilalxdavanu; sharxdedhxyilalxdavanu. 
\enum
\emng
\eentry

\bentry
\word{unbelieving} 
\pron{anfbiliVviMgf} 
\gl{\gu}
\expl{}
\bmng
(dhamaR, deVvaru, \mo vugaLanunx) naMbada; (avugaLalilx) naMbikeyilalxda; sharxdedhxyilalxda. 
\emng
\eentry

\bentry
\word{unbelievingly} 
\pron{anfbiliVviMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
(dhamaR, deVvaru, \mo vugaLalilx) naMbikeyilalxde; sharxdedhxyilalxde. 
\emng
\eentry

\bentry
\word{unbelievingness} 
\pron{anfbiliVviMgfnisf} 
\gl{\nA}
\expl{}
\bmng
(dhamaR, deVvaru, \mo vugaLalilx) naMbikeyilalxdiruvike; asharxdedhx. 
\emng
\eentry

\bentry
\word{unbeloved} 
\pron{anfbilavfDx} 
\gl{\gu}
\expl{}
\bmng
pirxVtisalapxDada; perxVmapAtarxnAgirada. 
\emng
\eentry

\bentry
\word{unbelt} 
\pron{anfbelfTx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (uDupina soMTapaTiTx \mo vanunx) tege; kaLacu. 
\num{2} (paTiTx \mo vanunx kaLaci) horatege: \eng{unbelt a sword} katitxyanunx soMTapaTiTxyiMda horatege. 
\enum
\emng
\eentry

\bentry
\word{unbend} 
\pron{anfbeMDf} 
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unbent}). }


\noindent
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} bagigxruvudanunx neVramADu (\akirx\ saha). 
\num{2} (manasusx \mo vugaLa) bigupu kaDamemADu; saDilisu. 
\numi{3} (\nw) 
\banum
\alnum{a} (paTagaLanunx) dimimxgaLiMda hAgU kUve hagagxgaLiMda bicucx. 
\alnum{b} (horajiyanunx) saDilabiDu. 
\alnum{c} (hagagx) bicucx. 
\eanum
\numie
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
ArAmavAgu; parxsananxnAgu; nemamxdigoLuLx; maqduvAgu: \eng{in the classroom the teacher maintains discipline but after the class he unbends} taragatiyalilxruvAga upAdhAyxyaru shisatxnunx pAlisutAtxre, taragatiya naMtara parxsananxcitatxrAgirutAtxre. 
\emng
\eentry

\bentry
\word{unbending} 
\pron{anfbeMDiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} bAgada; neTaTxgiruva; neVravAda. 
\num{2} bagagxda; sagagxda; niSuThxra; daqDha: \eng{unbending rectitude} daqDhavAda niVtiniSeThx. 
\num{3} (otatxDa, kAyaRcaTuvaTike, \mo vugaLa taruvAya) ArAmavAda; nemamxdi paDeda; biguvu kaDameyAda. 
\enum
\emng
\eentry

\bentry
\word{unbendingly} 
\pron{anfbeMDiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sagagxde; bagagxde; talebAgade. 
\num{2} daqDhavAgi; niSuThxravAgi. 
\enum
\emng
\eentry

\bentry
\word{unbendingness} 
\pron{anfbeMDiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} bagagxdiruvudu; sagagxdiruvudu. 
\num{2} daqDhate; niSuThxrate. 
\enum
\emng
\eentry

\bentry
\word{unbent} 
\pron{anfbeMTf} 
\gl{\gu}
\expl{}
\bmng
bagigxrada; bAgirada. 
\emng
\eentry

\bentry
\word{unbeseem} 
\pron{anfbisiVmf} 
\gl{\sakirx}
\expl{}
\bmng
(\pArxparx) (akataRqkavAgi) ayukatxvAgu; anucitavAgu; shoVBisadiru. 
\emng
\eentry

\bentry
\word{unbeseeming} 
\pron{anfbisiVmiMgf} 
\gl{\gu}
\expl{}
\bmng
(\pArxparx) ayukatx; anucita; shoVBisada. 
\emng
\eentry

\bentry
\word{unbesought} 
\pron{anfbisATf} 
\gl{\gu}
\expl{}
\bmng
apArxthiRta; keVLikoLaLxda; beVDikoLaLxda; pArxthiRsada. 
\emng
\eentry

\bentry
\word{unbespoken} 
\pron{anfbisopxVkanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} modaleV -- nigadimADikoLaLxda, niSakxSeR mADikoLaLxda, gotutxmADikoLaLxda; pUvaRnishicxtavalalxda. 
\num{2} (sarakugaLa \vi) koVrada; beVDike salilxsada. 
\num{3} sUcisada. 
\num{4} saMboVdhisada. 
\num{5} anukatx. 
\enum
\emng
\eentry


\bentry
\word{unbiased} 
\pron{anfbeYasfTx}
\gl{\gu}
\expl{}
\bmng
(\eng{unbiassed} eMdU \parx) niSapxkaSxpAtavAda; pUvARgarxhavilalxda: \eng{unbespoken estimate} niSapxkaSxpAtavAda aMdAju, aBipArxya. 
\emng
\eentry

\bentry
\word{unbiassed} 
\pron{anfbeYasfTx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unbiased}{unbiased}. 
\emng
\eentry

\bentry
\word{unbiblical} 
\pron{anfbibilxka(kf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} beYbalinalilxlalxda; beYbalina pArxmANayxvilalxda. 
\num{2} beYbalilxge virudadhxvAda. 
\enum
\emng
\eentry

\bentry
\word{unbiddable} 
\pron{anfbiDaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
aviniVta; avidheVya; heVLida mAtu keVLada. 
\emng
\eentry

\bentry
\word{unbidden} 
\pron{anfbiDanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Ajecnx koTiTxlalxda; apapxNe mADirada. 
\num{2} anAhUta; AhAvxnisilalxda; kareyade, heVLadekeVLade -- baruva: \eng{arrived unbidden} kareyade baMda. 
\enum
\emng
\eentry

\bentry
\word{unbind} 
\pron{anfbeYMDf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unbound}). }
\bmng
baMdhana bicucx; kaTuTx kaLacu. 
\emng
\eentry

\bentry
\word[unbirthday(1)]{unbirthday} 
\pron{anfbatfRDeV} 
\gl{\gu}
\expl{}
\bmng
(uDugore \mo vugaLa \vi\, hAsayxvAgi heVLuvAga) huTuTxhababxdadxlalxda; janamxdineVtarada. 
\emng
\eentry

\bentry
\word[unbirthday(2)]{unbirthday} 
\pron{anfbatfRDeV} 
\gl{\nA}
\expl{}
\bmng
(kelavomemx visheVSaNavAgi) 
\bnum
\num{1} (\hA) (obabxna huTiTxda dinavanunx biTuTx) beVrAvudeV dina; janemxVtara dina. 
\num{2} (huTiTxda dina biTuTx) beVre dina EpaRDisida saMtoVSakUTa. 
\enum
\emng
\eentry

\bentry
\word{unbleached} 
\pron{anfbilxVcfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} baNaNx kaLeyada; biLidAgisada; celuve mADada. 
\num{2} baNaNx hoVgada; biLidAgada. 
\enum
\emng
\eentry

\bentry
\word{unblemished} 
\pron{anfbelxmiSfTx} 
\gl{\gu}
\expl{}
\bmng
kaLaMkavilalxda; malinagoLisada; shudadhx. 
\emng
\eentry

\bentry
\word{unblessed} 
\pron{anfbelxsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AshiVvaRdisada; harasada. 
\num{2} apavitarxvAda; ashuBavAda; pavitarxgoLisada. 
\num{3} keVDina; saMkaTada. 
\enum
\emng
\eentry

\bentry
\word{unblest} 
\pron{anfbelxsfTx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unblessed}{unblessed}. 
\emng
\eentry

\bentry
\word{unblinking} 
\pron{anfbilxMkiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eveyikakxda; animeVSa; kaNuNx miTukisada. 
\num{2} niBARvavAda; (AshacxyaR, galibili, KeVda, \mo) yAvoMdu parxtikirxye, BAva, \mo vanunx parxdashiRsada. 
\num{3} hiMjariyada; niBaRyavAda; daqDhavAda. 
\enum
\emng
\eentry

\bentry
\word{unblinkingly} 
\pron{anfbilxMkiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kaNuNx miTukisade. 
\num{2} niBARvadiMda; parxtikirxye vayxkatxpaDisade. 
\num{3} hiMjariyade; niBaRyavAgi; daqDhavAgi. 
\enum
\emng
\eentry

\bentry
\word{unblock} 
\pron{anfbAlxkf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ koLave, caraMDi, \mo vugaLalilxna) aDetaDegaLanunx tegeduhAku. 
\num{2} (isipxVTu)(jotegArana oMdeV raMgina elegaLa doDaDx saraNiyanunx keDisadiruvudakAkxgi) hecucx beleya ele iLi, hAku (\akirx\ saha). 
\enum
\emng
\eentry

\bentry
\word{unblown} 
\pron{anfbolxVnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Udada; ubibxsada. 
\num{2} (\pArxparx) (hUvina \vi) araLilalxda; avikasitavAda. 
\enum
\emng
\eentry

\bentry
\word{unblushing} 
\pron{anfbalxSiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nilaRjajx; lajejxgeTaTx: \eng{the unblushing corruption of some politicians} kelavu rAjakAraNigaLa nilaRjajx BarxSATxcAra. 
\num{2} muKa keMpeVrada; vivaNaRvAgada. 
\num{3} nAcada; nAcikeyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unbolt} 
\pron{anfboVlfTx} 
\gl{\sakirx}
\expl{}
\bmng
(bAgilina) tApALu \mo vanunx saDilisu; cilaka, agaLi -- tege. 
\emng
\eentry

\bentry
\word{unbolted} 
\pron{anfboVliTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tApALu, cilaka -- hAkada; agaLi hAki baMdhisada. 
\num{2} (hiTuTx \mo vugaLa \vi) jaraDi, oMdari -- ADada; soVsilalxda. 
\enum
\emng
\eentry

\bentry
\word{unbonnet} 
\pron{anfbAniTf} 
\gl{\sakirx}
\expl{}
\bmng
meVlumxcacxLa tege; mucacxLa, bAneTf -- tegedubiDu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
(\pArxparx) \kanmu\ gwrava sUcisalu ToVpi tege. 
\emng
\eentry
 
\bentry
\word{unbookish} 
\pron{anfbukiSf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keVvala gArxMthikavalalxda; pusatxkiVyavalalxda; keVvala `pusatxkada badanekAyi' alalxda; shuSakxpAMDitayxvalalxda. 
\num{2} pusatxkajAcnxnavilalxda; vAyxsaMgapirxyanalalxda; Oduva olavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unboot} 
\pron{anfbUTf} 
\gl{\akirx}
\expl{}
\bmng
(tananx yA inonxbabxna) bUTu kaLacu; bUTu tege (\akirx\ saha). 
\emng
\eentry

\bentry
\word{unborn} 
\pron{anfbAnfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ajAta; inUnx huTaTxdiruva; muMde baraliruva, huTaTxliruva: \eng{unborn generations} muMbaruva piVLigegaLu. 
\num{2} janisada; inUnx gaBaRdalilxruva: \eng{an unborn baby} inUnx janisada kUsu. 
\num{3} huTeTxV ilalxda; eMdeMdigU huTuTx paDeyada: \eng{unborn hopes} huTeTxV paDeyada niriVkeSxgaLu. 
\enum
\emng
\eentry

\bentry
\word{unbosom} 
\pron{anfbusaZmf} 
\gl{\sakirx}
\expl{}
\bmng
(manoVBAva, AloVcane, \mo vanunx) teredu heVLu; vayxkatxpaDisu; parxkaTisu. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{unbosom oneself} (\AtAmx) aMtaraMga, rahasayx, haqdaya -- bicicx heVLiko; haqdaya bicicx mAtanADu. 
\emng
\eentry

\bentry
\word[unbound(1)]{unbound} 
\pron{anfbwMDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} baMdhana kaLeda; kaTuTx harida. 
\num{2} anibaRMdhita. 
\numi{3} (pusatxkada \vi) 
\banum
\alnum{a} pusatxkarUpavAgi kaTaTxda; biDihALegaLiruva; raTuTx hAkilalxda. 
\alnum{b} kAgadada hodikeyiruva. 
\eanum
\numie
\num{4} (darxvayx yA kaNada \vi) biDiyAgiruva; saMyukatxvAgirada yA baMdhitavAgilalxda. 
\enum
\emng
\eentry

\bentry
\word[unbound(2)]{unbound} 
\pron{anfbwMDf} 
\gl{\kirx}
\expl{}
\bmng
\eng{unbind} padada \BU\ matutx \BUkaq 
\emng
\eentry

\bentry
\word{unbounded} 
\pron{anfbwMDiDf} 
\gl{\gu}
\expl{}
\bmng
elelxyilalxda; amita; asiVma; apAra; anaMta: \eng{unbounded optimism} anaMta AshAvAdi. 
\emng
\eentry

\bentry
\word{unboundedly} 
\pron{anfbwMDiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
elelxyilalxde; amitavAgi; anaMtavAgi. 
\emng
\eentry

\bentry
\word{unboundedness} 
\pron{anfbwMDiDfnisf} 
\gl{\nA}
\expl{}
\bmng
amitate; apArate; anaMtate. 
\emng
\eentry

\bentry
\word{unbrace} 
\pron{anfberxVsf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} bigipaTiTxgaLanunx tegeduhAku, kaLacu. 
\num{2} bigupu kaLe, saDilisu. 
\num{3} nirAtaMkagoLisu; manasusx hagura mADiko. 
\num{4} (nara \mo vanunx) saDilisu. 
\enum
\emng
\eentry

\bentry
\word{unbreachable} 
\pron{anfbirxVcaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
(\rUpa\ saha) aBeVdayx; oDeyalAgada. 
\emng
\eentry

\bentry
\word{unbreakable} 
\pron{anfberxVkaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
oDeyalAgada; oDedu cUrAgada. 
\emng
\eentry

\bentry
\word{unbreathable} 
\pron{anfbirxVdaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
usirADalAgada; usirADalu kaSaTxvAda. 
\emng
\eentry

\bentry
\word{unbribable} 
\pron{anfberxYba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
laMca ruSuvatutxgaLige oLagAgada; laMcakoTuTx keDisalAgada; BarxSATxcArakekx oLapaDisalAgada. 
\emng
\eentry

\bentry
\word{unbridgeable} 
\pron{anfbirxjaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} seVtuveyAgada; seVtuve kaTaTxlAgada. 
\num{2} kUDisalAgada; seVrisalAgada; saMpakaR kalipxsalAgada. 
\enum
\emng
\eentry

\bentry
\word{unbridle} 
\pron{anfberxYDa(Df)lf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (kudureya) lagAmu -- tegedubiDu, kaLacu. 
\num{2} (vayxkitx, manuSayxra nAlige, \mo vugaLanunx) saDilisu; sAvxtaMtarxyX koDu; nibaRMdha tege. 
\enum
\emng
\eentry

\bentry
\word{unbridled} 
\pron{anfberxYDa(Df)lfDx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ nAlige, ahaMkAra parxvaqtitx, \mo vugaLa \vi, \rUpa) kaDivANavilalxda; laMgulagAmilalxda; aMkeyilalxda; niyaMtarxNa, saMyama ilalxda: \eng{an unbridled tongue} laMgulagAmilalxda -- nAlige. \eng{unbridled insolence} kaDivANavilalxda sokukx. 
\emng
\eentry

\bentry
\word{unbroken} 
\pron{anfborxVkanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oDeyada; oDedilalxda. 
\num{2} paLagisilalxda; bagagxda: \eng{unbroken horse} paLagisada kudure. 
\num{3} viciPxtitxyilalxda; oMdeV samanAda; aviciPxnanxvAda; niraMtaravAda: \eng{unbroken sleep} niraMtara nidedx. \eng{unbroken peace} aviciPxnanxvAda shAMti. 
\num{4} miVrilalxda; miVrisilalxda; muriyada: \eng{unbroken record} (hiMdeMdU) miVrisada, muriyada -- vikarxma, dAKale. 
\enum
\emng
\eentry

\bentry
\word{unbrokenly} 
\pron{anfborxVkanfli} 
\gl{\kirxvi}
\expl{}
\bmng
aviciPxnanxvAgi; niraMtaravAgi; oMdeV samanAgi. 
\emng
\eentry

\bentry
\word{unbrokenness} 
\pron{anfborxVkanfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aKaMDate; oDeyadiruvudu. 
\num{2} paLagisilalxdiruvike. 
\num{3} aviciPxnanxte; oMdeV samanAda sithxti. 
\enum
\emng
\eentry

\bentry
\word{unbruised} 
\pron{anfbUrxsfZDf} 
\gl{\gu}
\expl{}
\bmng
gAyavAgilalxda; peTuTx bididxlalxda. 
\emng
\eentry

\bentry
\word{unbuckle} 
\pron{anfbaka(kf)lf} 
\gl{\sakirx}
\expl{}
\bmng
(paTiTx, pAdarakeSx, \mo vugaLa) ciluku kaTaTxnunx saDilisu, bicucx; `bakalf' -- tege, saDilisu. 
\emng
\eentry

\bentry
\word{unbuild} 
\pron{anfbilfDx} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unbuilt}). }
\bmng
(kaTaTxDa, sidAdhxMta, vayxvasethx, \mo vanunx) oDeduruLisu; keDahu; nAshapaDisu (\rUpa\ saha). 
\emng
\eentry

\bentry
\word{unbuilt} 
\pron{anfbilfTx} 
\gl{\gu}
\bmng
animiRta; inUnx kaTiTxlalxda, racisilalxda. 
\emng
\eentry

\bentry
\word{unburden} 
\pron{anfbaDaR(DfR)nf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} horeyiLisu; BAraviLisu. 
\num{2} (\AtAmx) (opipxkoLuLxvudu, heVLikoLuLxvudu, \mo vugaLa mUlaka) haqdayada BArayiLisiko; manasusx hagura mADiko: \eng{unburden oneself of a secret} bacicxTaTx guTaTxnunx bayalumADi haqdayada BArayiLisiko. 
\enum
\emng
\eentry

\bentry
\word{unburdened} 
\pron{anfbaDaR(DfR)nfDx} 
\gl{\gu}
\expl{}
\bmng
(manasusx \mo vugaLa) BAraviLisida; haguragoLisida; horeyiLisida. 
\emng
\eentry

\bentry
\word{unburied} 
\pron{anfberiDf} 
\gl{\gu}
\expl{}
\bmng
hULada; samAdhi mADada. 
\emng
\eentry

\bentry
\word{unbury} 
\pron{anfberi} 
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{unburies;} \BU\ matutx \BUkaq\ \eng{unburied}).}
\bmng
\bnum
\num{1} (hUta meVle, hUtiTaTxdadxnunx) agedu tege; BUmiyiMda meVlakekxtutx. 
\num{2} (guTuTx \mo vanunx) beLakige, bayalige taru; horahAku. 
\enum
\emng
\eentry

\bentry
\word{unbusinesslike} 
\pron{anfbisfZnisfleYkf} 
\gl{\gu}
\expl{}
\bmng
vayxvahAra riVtiyalalxda; avAyxvahArika; vayxvahArajAcnxnavilalxda; adakaSx. 
\emng
\eentry

\bentry
\word{unbutton} 
\pron{anfbaT(Tf)nf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (aMgi \mo vugaLa) guMDi -- bicucx, tege; (kAja \mo vugaLiMda) guMDi -- kaLacu, tege. 
\alnum{b} (vayxkitxya) uDupugaLa guMDi -- bicucx, tege. 
\eanum
\numie
\numi{2} (\AmA) 
\banum
\alnum{a} (otatxDa, aupacArikate, \mo vugaLiMda) biDisu; saDila mADiko; kaDime mADiko; horabaru. 
\alnum{b} manasusx bicicx mAtanADu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{unbuttoned} 
\pron{anfbaT(Tf)nfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} guMDi -- hAkada, bicicxda. 
\numi{2} (\rUpa) (\AmA) 
\banum
\alnum{a} anwpacArika; mucucxmareyilalxda. 
\alnum{b} manasusx teredu mAtanADuva. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{uncage} 
\pron{anfkeVjf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} paMjaradiMdAcege, boVniniMdAcege -- biDu. 
\num{2} sereyiMda biDu; biDugaDe mADu; savxtaMtarxvAgisu. 
\enum
\emng
\eentry

\bentry
\word{uncalled} 
\pron{anfkAlfDx} 
\gl{\gu}
\bmng
karedilalxda; kareyada; AhAvxnavilalxda; anAhUta. 
\emng
\eentry

\bentry
\word{uncalled-for} 
\pron{anfkAlfDxphArf} 
\gl{\gu}
\expl{}
\bmng
keVLada; anAvashayxkavAda; adhikaparxsaMgitanada: \eng{such comments are uncalled-for} iMtha TiVkegaLu adhikaparxsaMgitanadavu. \eng{uncalled-for insults} anAvashayxkavAda avamAnakara mAtugaLu. 
\emng
\eentry

\bentry
\word{uncandid} 
\pron{anfkAYxMDiDf} 
\gl{\gu}
\expl{}
\bmng
niSakxpaTavalalxda; neVravAgirada; saraLavAgirada. 
\emng
\eentry

\bentry
\word{uncannily} 
\pron{anfkAYxnili} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} alwkikavenisuvaMte; vicitarxvAgi. 
\num{2} rahasayxmayavAgi. 
\enum
\emng
\eentry

\bentry
\word{uncanniness} 
\pron{anfkAYxninisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} alwkikate; vicitarxte. 
\num{2} rahasayxmayate. 
\enum
\emng
\eentry

\bentry
\word{uncanny} 
\pron{anfkAYxni} 
\gl{\gu}
\expl{(\tara\ \eng{uncannier}, \tama\ \eng{uncanniest}).}
\bmng
\bnum
\num{1} alwkikavenisuva; asAvxBAvikavAda; vicitarx; vilakaSxNa: \eng{an uncanny noise} vilakaSxNavAda sadudx. 
\num{2} rahasayxmaya. 
\enum
\emng
\eentry

\bentry
\word{uncanonical} 
\pron{anfkanAnika(kf)lf} 
\gl{\gu}
\expl{}
\bmng
(\kerxY) aMgiVkaqta kerxYsatxdhamaRdalilx seVrirada; saparxmANavalalxda; aMgiVkaqtavalalxda. 
\emng
\eentry

\bentry
\word{uncanonically} 
\pron{anfkanAnikali} 
\gl{\kirxvi}
\expl{}
\bmng
(\kerxY) dhamARnusAriyAgirade; aMgiVkaqta (kerxYsatx)dhamaRdalilx seVrirade. 
\emng
\eentry

\bentry
\word{uncanonized} 
\pron{anfkAYxnaneYsfZDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kerxY) saMtara paTiTxge seVrisada; saMtananAnxgi parigaNisada. 
\num{2} (\kerxY) parxmANa garxMthaveMdu aMgiVkarisada; kerxYsatx maThAdhikAradiMda maMjUrAgada. 
\enum
\emng
\eentry

\bentry
\word{uncap} 
\pron{anfkAYxpf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{uncapped}; \vakaq\ \eng{uncapping}). }
\bmng
(mucacxLa, biraDe, ToVpi, \mo vanunx) tege; kaLacu. 
\emng
\eentry

\bentry
\word{uncared-for} 
\pron{anfkeVDfRArf} 
\gl{\gu}
\expl{}
\bmng
alakaSxyXkikxVDAda; dikikxlalxda; upeVkiSxta: \eng{uncared-for children} dikikxlalxda makakxLu. \eng{an uncared-for garden} alakaSxyXkikxVDAda toVTa. 
\emng
\eentry

\bentry
\word{uncase} 
\pron{anfkeVsf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (hodike, peTiTxge, \mo vugaLiMda) horatege; Acege tege; hodike tege (\akirx\ saha). 
\num{2} (\pArxparx) baTeTx kaLacu; bayalu mADu; betatxle mADu: \eng{his hypocrisy should be uncased} avana ASADhaBUti tanavanunx bayalu mADaleV beVku. 
\enum
\emng
\eentry

\bentry
\word{uncashed} 
\pron{anfkAYxSfTx} 
\gl{\gu}
\expl{}
\bmng
nagadu mADilalxda; nagadAgilalxda. 
\emng
\eentry

\bentry
\word{uncaught} 
\pron{anfkATf} 
\gl{\gu}
\bmng
sikikxbiVLada; seresikakxda. 
\emng
\eentry

\bentry
\word{uncaused} 
\pron{anfkAsfZDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kAraNa rahitavAda; kAraNasaMbaMdhiyalalxda. 
\num{2} akaqta; uMTu mADidadxlalxda; saqSiTx mADidadxlalxda; savxyaMBU. 
\enum
\emng
\eentry

\bentry
\word{unceasing} 
\pron{anfsiVsiMgf} 
\gl{\gu}
\expl{}
\bmng
nilalxda; niMtu hoVgada; niraMtara; satata: \eng{unceasing effort} satata parxyatanx. 
\emng
\eentry

\bentry
\word{unceasingly} 
\pron{anfsiVsiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
nilalxde; niraMtaravAgi; satatavAgi. 
\emng
\eentry

\bentry
\word{uncensored} 
\pron{anfsenasxDfR} 
\gl{\gu}
\expl{}
\bmng
senasxrf mADirada; katatxri parxyoVga mADirada. 
\emng
\eentry

\bentry
\word{uncensured} 
\pron{anfsenaSxDfR} 
\gl{\gu}
\expl{}
\bmng
aKaMDita; KaMDisilalxda; dUSisilalxda. 
\emng
\eentry

\bentry
\word{unceremonious} 
\pron{anfserimoVniasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anwpacArika; sadarada; saligeya. 
\num{2} oraToraTAda; mayARde tapipxda; shiSATxcAra pAlisada. 
\enum
\emng
\eentry

\bentry
\word{unceremoniously} 
\pron{anfserimoVniasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anwpacArikavAgi; saligeyiMda. 
\num{2} oraToraTAgi; shiSATxcAra pAlisada riVtiyalilx; mayARdegeTuTx. 
\enum
\emng
\eentry

\bentry
\word{unceremoniousness} 
\pron{anfserimoVniasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anwpacArikate; salige; sadara. 
\num{2} amayARde; shiSATxcArarahitate; oraTu vataRne. 
\enum
\emng
\eentry

\bentry
\word{uncertain} 
\pron{anfsaTaR(TfR)nf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} KacitavAgi tiLiyada; sariyAgi gotitxlalxda; anishicxta; saMdigadhx: \eng{the result is uncertain} PalitAMsha anishicxta. 
\num{2} necacxlAgada; naMbalAgada; Baravaseyilalxda: \eng{his aim is uncertain} avana udedxVsha necacxlahaRvalalx. 
\num{3} caMcala; badalAyisuva: \eng{the uncertain weather} caMcala havAmAna. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{in no uncertain terms} suPxTavAgi matutx balavAgi; susapxSaTxvAgi matutx shakitxyutavAgi. 
\emng
\eentry

\bentry
\word{uncertainly} 
\pron{anfsaTaRnfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} KacitavAgi tiLiyade; sariyAgi gotitxlalxde; anishicxtavAgi. 
\num{2} necacxlAgada sithxtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{uncertainty} 
\pron{anfsaTaRnfTi} 
\gl{\nA}
\expl{(\bava\ \eng{uncertainties}).}
\bmng
\bnum
\num{1} anishicxtate; saMdigadhxte; KacitavAgirada sithxti. 
\num{2} anishicxtate; anidhARrita viSaya yA parisithxti. 
\enum
\emng
\eentry

\bentry
\wordnospeech{uncertainty principle}{uncertainty principle}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\Bwvi) = \hyperref{kandict_h.pdf}{H}{Heisenberg's uncertainty principle}{Heisenberg's uncertainty principle}. 
\emng
\eentry

\bentry
\word{uncertificated} 
\pron{anfsaTiRphikeVTiDf} 
\gl{\gu}
\expl{}
\bmng
parxmANapatarx yA samathaRnApatarx ilalxda; pArxmANayx yA samathaRnAdhikAra odagisada. 
\emng
\eentry

\bentry
\word{uncertified} 
\pron{anfsaTiRpheYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} samathaRne ilalxda; daqDhiVkaraNarahita; sAkASxyXdhAravilalxda; parxmANaveMdu daqDhapaDisilalxda. 
\num{2} = \hyperlink{uncertificated}{uncertificated}. 
\num{3} hucecxMdu daqDhapaDisilalxda; budidhxvikalapxveMdu parxmANiVkarisilalxda. 
\enum
\emng
\eentry

\bentry
\word{unchain} 
\pron{anfceVnf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} sarapaNi kaLacu; saMkoVle tege. 
\num{2} baMdhanamukatx mADu; biDugaDegoLisu. 
\enum
\emng
\eentry

\bentry
\word{unchallengeable} 
\pron{anfcAYxliMjabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} savAlu hAkalu sAdhayxvilalxda; paMthAhAvxna koDalAgada. 
\num{2} parxshinxsalasAdhayxvAda. 
\enum
\emng
\eentry

\bentry
\word{unchallengeably} 
\pron{anfcAYxliMjabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} savAlu hAkalAgadaMte. 
\num{2} parxshinxsalasAdhayxvAgi. 
\enum
\emng
\eentry

\bentry
\word{unchallenged} 
\pron{anfcAYxliMjfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} savAlu hAkada; parxshinxsada. 
\num{2} parxshAnxtiVta. 
\enum
\emng
\eentry

\bentry
\word{unchancy} 
\pron{anfcAnisx}
\gl{\gu}
\bmng
\bnum
\num{1} (sAkxTelxMDf) adaqSaTxhiVna; duradaqSaTxda; ashuBasUcaneya. 
\num{2} apAyakara. 
\enum
\emng
\eentry

\bentry
\word{unchangeability} 
\pron{anfceVMjabiliTi} 
\gl{\nA}
\expl{}
\bmng
avayxtayxsaniVyate; apariNAmitavx; badalAyisalAgadiruvike. 
\emng
\eentry

\bentry
\word{unchangeable} 
\pron{anfceVMjabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
badalAyisalAgada; mApaRDisalasAdhayxvAda. 
\emng
\eentry

\bentry
\word{unchangeableness} 
\pron{anfceVMjabalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unchangeability}{unchangeability}. 
\emng
\eentry

\bentry
\word{unchangeably} 
\pron{anfceVMjabilx} 
\gl{\kirxvi}
\expl{}
\bmng
badalAyisalAgadaMte; mApaRDisalAgadaMte; vayxtAyxsamADalAgadaMte. 
\emng
\eentry

\bentry
\word{unchanged} 
\pron{anfceVMjfDx} 
\gl{\gu}
\expl{}
\bmng
badalAyisada; mApARDAgada; vayxtAyxsavAgada; badalAvaNege pakAkxgadiruva. 
\emng
\eentry

\bentry
\word{unchanging} 
\pron{anfceVMjiMgf} 
\gl{\gu}
\expl{}
\bmng
oMdeV samanAda; EkaparxkArada; badalAyisadiruva; mApARDAgadiruva. 
\emng
\eentry

\bentry
\word{unchangingly} 
\pron{anfceVMjiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
vayxtAyxsa vilalxde; mApARDilalxde. 
\emng
\eentry

\bentry
\word{unchangingness} 
\pron{anfceVMjiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
badalAyisadiruvike. 
\emng
\eentry

\bentry
\word{unchaperoned} 
\pron{anfSAYxparoVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (huDugiya \vi) noVDikoLuLxva saMrakaSxki ilalxda; sahacariyilalxda. 
\num{2} (huDugana \vi) noVDikoLuLxva sahacaranilalxda. 
\enum
\emng
\eentry

\bentry
\word{uncharacteristic} 
\pron{anfkAYxrikaTxrisiTxkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} veYlakaSxNayxvanunx sUcisada; vishiSaTxvalalxda; veYshiSaTxyX toVrisada. 
\num{2} vilakaSxNavAda; savxBAvAnurUpavAgirada; savxrUpakekx anuguNavAgilalxda. 
\enum
\emng
\eentry

\bentry
\word{uncharacteristically} 
\pron{anfkAYxrikaTxrisiTxkali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vishiSaTxvAgirade; veYlakaSxNayxvanunx sUcisade. 
\num{2} vilakaSxNavAgi. 
\enum
\emng
\eentry

\bentry
\word{uncharged} 
\pron{anfcAjfRDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} anAviSaTx; viduyxdAveVshavilalxda. 
\alnum{b} (viduyxtfkoVsha \mo vugaLalilx) shakitx saMgarxhisilalxda. 
\eanum
\numie
\num{2} ApAdisirada; nirAroVpada; anAroVpita; AroVpisirada; tapupx horisilalxda. 
\num{3} (saraku, koVvige madudx, \mo vanunx) tuMbilalxda; BatiR mADilalxda. 
\enum
\emng
\eentry

\bentry
\word{uncharitable} 
\pron{anfcAYxriTabflf} 
\gl{\gu}
\expl{}
\bmng
anudAravAda; doVSAroVpaNeya; kaTuvAda; kaThina vimasheRya: \eng{put an uncharitable interpretation on his action} avana naDevaLikeya meVle anudAravAda athaR heVru. 
\emng
\eentry

\bentry
\word{uncharitableness} 
\pron{anfcAYxriTabflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anwdAyaR; dAkiSxNayxvilalxdiruvudu. 
\num{2} kaTuvAda vimasheR mADuvudu; kaThiNavAda TiVke mADuvike. 
\enum
\emng
\eentry

\bentry
\word{uncharitably} 
\pron{anfcAYxriTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} udAra BAvaneyilalxde; dAkiSxNayxvilalxde. 
\num{2} kaTuvAgi, kaThiNavAgi TiVke mADutatx. 
\enum
\emng
\eentry

\bentry
\word{uncharted} 
\pron{anfcATiRDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nakeSxgurutu mADada; taHKetx mADada; gurutu hAkada: \eng{an uncharted island} nakeSxyalilx kANisada divxVpa. 
\num{2} shoVdhisi noVDada; aparicita: \eng{an uncharted ocean} yArU shoVdhisi ariyada sAgara. 
\enum
\emng
\eentry

\bentry
\word{unchartered} 
\pron{anfcATaRDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sananxdu koDada; anadhikaqta; adhikaqta sananxdu paDeyada. 
\num{2} niVtiniyamagaLilalxda; kAnUnu miVrida; hatoVTiyilalxda; savxcaCxMda. 
\enum
\emng
\eentry

\bentry
\word{unchaste} 
\pron{anfceVsfTx} 
\gl{\gu}
\expl{}
\bmng
shiVlavilalxda; viSaya laMpaTa. 
\emng
\eentry

\bentry
\word{unchastely} 
\pron{anfceVsfTxli} 
\gl{\kirxvi}
\expl{}
\bmng
shiVlavilalxde; viSayalaMpaTatanadiMda. 
\emng
\eentry

\bentry
\word{unchasteness} 
\pron{anfceVsfTxnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unchastity}{unchastity}. 
\emng
\eentry

\bentry
\word{unchastity} 
\pron{anfcAYxsiTxTi} 
\gl{\nA}
\expl{}
\bmng
shiVlavilalxdiruvudu; viSayalaMpaTate; aniVtiyuta naDevaLike. 
\emng
\eentry

\bentry
\word{unchecked} 
\pron{anfcekfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aniyaMtirxta; kaTuTxpADilalxda; savxcaCxMda; hatoVTiyilalxda: \eng{unchecked violence} hatoVTiyilalxda hiMsAcAra. 
\num{2} taDegaTiTxlalxda; aDacaNeyilalxda; anibaRMdhita. 
\enum
\emng
\eentry

\bentry
\word{unchivalrous} 
\pron{anfSivalarxsf} 
\gl{\gu}
\expl{}
\bmng
dAkiSxNayxvilalxda; vishAlahaqdayavilalxda; udAriyalalxda. 
\emng
\eentry

\bentry
\word{unchivalrously} 
\pron{anfSivalarxsfli} 
\gl{\kirxvi}
\expl{}
\bmng
anudAriyAgi; anukuMpavilalxde; haqdaya veYshAlayxvilalxde. 
\emng
\eentry

\bentry
\word{unchosen} 
\pron{anfcoVsaZ(sfZ)nf} 
\gl{\gu}
\expl{}
\bmng
(\deVva) deVvarolidilalxda; deVvariMda AyekxyAgada; deYvAnugarxhavilalxda. 
\emng
\eentry

\bentry
\word{unchristian} 
\pron{anfkirxsaTxyXnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} akerxYsatx; kerxYsatxnalalxda. 
\num{2} kerxYsatxdhamaR virudadhxvAda; kerxYsatxyoVgayxvalalxda; kerxYsatxtatatxvXgaLige virudadhxvAda yA alakaSxyX toVruva, \kanmu\ sAvxthiRyAgiruva. 
\num{3} (\AmA) mitimiVrida; GoVra; ugarx; dwjaRnayxkAri; mitimiVri niVtigeTaTx; dUSaNiVya. 
\num{4} (\AmA) ananukUlada; aveVLeya; aswkayaRda; toMdareya: \eng{why do you call on me at this unchristian hour?} I aveVLeyalilx nananxnunx noVDalu yAke baMde? 
\enum
\emng
\eentry

\bentry
\word{unchurch} 
\pron{anfcacfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} caciRniMdAce hAku; bahiSakxrisu. 
\num{2} (kaTaTxDakekx) caciRna aMtasatxnunx tapipxsu; caciRge iruva sAthxnavanunx nirAkarisu. 
\enum
\emng
\eentry

\bentry
\word[uncial(1)]{uncial} 
\pron{anfsialf, anfSa(Sf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\eng{4-8}neya shatamAnagaLa hasatxparxtigaLalilx kaMDubaruva) oMdu bageya doDaDxkaSxragaLige saMbaMdhisida; A bageya doDaDxkaSxradalilx bareda. 
\num{2} aMgulada yA aunisxna yA avugaLige saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word[uncial(2)]{uncial} 
\pron{anfsialf, anfSa(Sf)lf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\eng{4-8}neya shatamAnagaLa hasatxparxtigaLalilx kaMDubaruva) oMdu bageya doDaDxkaSxrada hasatxparxti. 
\num{2} A bageya doDaDxkaSxra yA adara sheYli. 
\enum
\emng
\eentry

\bentry
\word{unciform} 
\pron{anfsiphAmfR} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{uncinate}{uncinate}. 
\emng
\eentry

\bentry
\word{uncinate} 
\pron{anfsinaTf} 
\gl{\gu}
\expl{}
\bmng
(\aMrashA\ \mo vu) kokekxyaMte bAgiruva; kokekxyAkArada; koMkAda; DoMkAgiruva. 
\emng
\eentry

\bentry
\word{uncircumcised} 
\pron{anfsakaRseYsfZDx} 
\gl{\gu}
\bmng
\bnum
\num{1} sunatiyAgilalxda. 
\num{2} (\rUpa) vidhamiRyAda; dhamaRsaMsAkxra rahita. 
\enum
\emng
\eentry

\bentry
\word{uncircumcision}  
\pron{anfsakaRmfsiSaZ(SfZ)nf}  
\gl{\nA} 
\bmng
sunatiyAgilalxdiruvudu. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{the uncircumcision} (hosa oDaMbaDike) sunati yAgilalxdavaru; yehUdeyxVtararu yA musalAmxneVtararu. 
\emng
\eentry

\bentry
\word{uncivil} 
\pron{anfsivilf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anAgarika; asaBayx; oraTuriVtiya. 
\num{2} sAvaRjanikara; nAgarikara hitAsakitxge viroVdhavAda; nAgarika viroVdhi. 
\enum
\emng
\eentry

\bentry
\word{uncivilized} 
\pron{anfsivileYsfZDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anAgarika; nAgarikate -- ilalxda, paDedirada. 
\num{2} oraTu; asaMsakxqqta; pArxkaqta. 
\enum
\emng
\eentry

\bentry
\word{uncivilly} 
\pron{anfsivili} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anAgarikavAgi; asaBayxvAgi; oraTu riVtiyalilx. 
\num{2} nAgarika viroVdhiyAgi. 
\enum
\emng
\eentry

\bentry
\word{unclad} 
\pron{anfkAlxYxDf} 
\gl{\gu}
\expl{}
\bmng
vivasatxrXvAda; betatxle; barimeYya; baTeTx toDada; naganx. 
\emng
\eentry

\bentry
\word{unclaimed} 
\pron{anfkelxVmfDx} 
\gl{\gu}
\expl{}
\bmng
vArasudArarilalxda; hakukxdArarilalxda. 
\emng
\eentry

\bentry
\word{unclasp} 
\pron{anfkAlxsfpx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} cilaka, koMDi, baMdhani, tekekx, \mo vanunx -- saDilisu, tege. 
\num{2} hiDita saDilisu; hiDita tapu px, tapipxsu; keY \mo vugaLa muSiTxyanunx saDilisu, biDu. 
\enum
\emng
\eentry

\bentry
\word{unclassifiable} 
\pron{anfkAlxYxsiZpheYaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
vagiRVkarisalAgada; vagaRgaLAgi viMgaDisalAgada, beVpaRDisalAgada. 
\emng
\eentry

\bentry
\word{unclassified} 
\pron{anfkAlxYxsipheYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vagiRVkarisada; guNalakaSxNagaLiganusAravAgi beVpaRDisada, viBAga mADada. 
\num{2} (aMkiaMshagaLu, dAKale, dasAtxveVju, \mo vugaLa \vi, rASaTxrXda rakaSxNe, suBadarxtegaLigoVsakxra) rahasayxveMdu beVpaRDisada; rahasayxvalalxda; goVpayxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{uncle} 
\pron{aMka(kf)lf} 
\gl{\nA}
\bmng
\bnum
\numi{1} aMkalf: 
\banum
\alnum{a} cikakxpapx yA doDaDxpapx yA soVdaramAva; taMdeya yA tAyiya soVdara. 
\alnum{b} cikakxmamxna yA doDaDxmamxna yA soVdaratetxya -- gaMDa. 
\eanum
\numie
\num{2} (\AmA) aMkalf; (makakxLu, saMbaMdhiyalalxda hiriya vayxkitxyanunx saMboVdhisuvalilx) `mAvayayx' . 
\num{3} (\ashi) (\kanmu\ \ca) giravigAra; giravi aMgaDi mAliVka. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{cry uncle} (\ame) (\AmA) soVlanunx opipxko. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-uncle}{-uncle} 
\pron{-aMka(kf)lf} 
\gl{\uparx}
\expl{}
\bmng
alApxthaRka nAmapadagaLanunx racisuva \uparx, \udA\ \eng{carbuncle} 
\emng
. \eentry

\bentry
\word{unclean} 
\pron{anfkilxVnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} cokakxTavilalxda; ashuciyAda. 
\num{2} holasAda; heVsige tuMbida. 
\num{3} vayxBicArada. 
\num{4} (\beY) (Atamxda \vi) keDukina; duSaTxtanada. 
\num{5} maDiyilalxda; meYligeyAda. 
\num{6} aBakaSxyX; tinanxlu yoVgayxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{uncleanliness} 
\pron{anfkelxnflinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} holasAgiruvike; heVsige. 
\num{2} ashucitavx. 
\num{3} meYlige; maDiyalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word[uncleanly(1)]{uncleanly} 
\pron{anfkelxnfli} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} holasAda; keTaTx; heVsigeya: \eng{uncleanly habits} holasAda cALigaLu. 
\num{2} ashuciya. 
\num{3} meYligeya. 
\enum
\emng
\eentry

\bentry
\word[uncleanly(2)]{uncleanly} 
\pron{anfkilxVnfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} cokakxTavilalxde; ashuciyAgi. 
\num{2} holasAgi; heVsige tuMbi. 
\num{3} maDiyilalxde; meYligeyiMda. 
\enum
\emng
\eentry

\bentry
\word{uncleanness} 
\pron{anfkilxVnfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} cokakxTa vilalxdiruvike; ashuci. 
\num{2} holasAgiruvudu. 
\num{3} maDiyilalxdiruvudu; meYlige. 
\num{4} aBakaSxyXte. 
\enum
\emng
\eentry

\bentry
\word{unclear} 
\pron{anfkilxarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asapxSaTx; saMdigadhx; masukAda; tiLidukoLaLxlu sapxSaTxvAgilalxda yA sulaBavAgilalxda. 
\num{2} (vayxkitxya \vi) anishicxta; saMdeVhAsapxda; saMdigadhx: \eng{I'm unclear as to what you mean} ninanx manasisxnalilx Enide eMbudu nanage saMdigadhxvAgide. 
\enum
\emng
\eentry

\bentry
\word{unclearly} 
\pron{anfkilxarfli} 
\gl{\kirxvi}
\expl{}
\bmng
asapxSaTxvAgi; saMdigadhxvAgi. 
\emng
\eentry

\bentry
\word{unclearness} 
\pron{anfkilxarfnisf} 
\gl{\nA}
\expl{}
\bmng
asapxSaTxte; saMdigadhxte. 
\emng
\eentry

\bentry
\word{unclench} 
\pron{anfkelxnfcx} 
\gl{\sakirx}
\expl{}
\bmng
(muSiTx, muKaBAva, halulx, \mo vugaLa \vi) saDilisu; biDisu; tere. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
bigihiDida muSiTxyanunx saDilisu; bigi saDalisu. 
\emng
\eentry

\bentry
\wordnospeech{Uncle Sam}{Uncle Sam}
\pron{?}
\gl{\nA}
\expl{}
\bmng
sAyxmf mAva: 
\banum
\alnum{a} amerika saMyukatx saMsAthxna sakARrada mUtiRVkarisida pada, sAkArarUpa. \eng{will fight for Uncle Sam} amerikakAkxgi, sAyxmf mAvanigAgi hoVrADuvevu. 
\alnum{b} amerika deVsha yA alilxya parxjegaLu: \eng{a distorted picture of Uncle Sam} amerika deVshada vikaqta citarxNa, rUpa. 
\eanum
\emng
\eentry

\bentry
\wordnospeech{Uncle Tom}{Uncle Tom}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\ame) aMkalfTAmf; biLiyarige gulAmanAgiruva kariya; TAmf kakakx; TAmf cikakxpapx. 
\emng
\eentry

\bentry
\word{unclinch} 
\pron{anfkilxnfcx} 
\gl{\kirx}
\expl{}
\bmng
= \hyperlink{unclench}{unclench}. 
\emng
\eentry

\bentry
\word{uncloak} 
\pron{anfkolxVkf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} niluvaMgi tege, kaLacu. 
\num{2} teredu toVrisu; bayalumADu. 
\enum
\emng

\noindent 
\gl{\akirx}
\bmng
(\sA\ meVlaMgi \mo vanunx) kaLacu; tegedubiDu. 
\emng
\eentry

\bentry
\word{unclog} 
\pron{anfkAlxgf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unclogged}; \vakaq\ \eng{unclogging}).}
\bmng
\bnum
\num{1} taDe, aDacaNe, \mo vanunx tegeduhAku. 
\num{2} (kaTiTxkoMDa caraMDi, moVri, \mo valilx) niVru saliVsAgi kocicx hoVguvaMte, hariyuvaMte mADu. 
\enum
\emng
\eentry

\bentry
\word{uncloister} 
\pron{anfkAlxisaTxrf} 
\gl{\sakirx}
\expl{}
\bmng
(oLage kUDiruvudanunx) baMdhanadiMda biDisu; horatege; biDugaDemADu. 
\emng
\eentry

\bentry
\word{unclose} 
\pron{anfkolxVsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (mucicxTiTxruvudanunx) tere; horatege; terediDu (\akirx\ saha). 
\num{2} horageDavu; bayalu mADu; raTuTxmADu. 
\enum
\emng
\eentry

\bentry
\word{unclothe} 
\pron{anfkolxVdf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} betatxlemADu; baTeTx tege; vivasatxrXgoLisu. 
\num{2} hodike (\mo vanunx) tere, kaLacu; tegeduhAku. 
\num{3} elegaLanunx yA sasayxgaLanunx tegeduhAku, kitutxhAku; kitutx boVLumADu: \eng{trees unclothed by the wind} gALiyiMda boVLAda maragaLu. 
\num{4} horageDavu; bayalumADu. 
\enum
\emng
\eentry

\bentry
\word{unclothed} 
\pron{anfkolxVdfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vivasatxrXnAda; betatxleyAda; baTeTx kaLacida. 
\num{2} bayalumADida; parxkaTisida; horageDahida. 
\enum
\emng
\eentry

\bentry
\word{unclouded} 
\pron{anfkwlxDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} moVDa kaviyada. 
\num{2} mabubx musukada; maMku kaviyada. 
\num{3} (\parx)shAMta; nirAkula; taLamaLavilalxda; koSxVBegoLagAgada; nirudivxganx: \eng{unclouded serenity} nirudevxVgada parxshAMtate. 
\enum
\emng
\eentry

\bentry
\word{uncluttered} 
\pron{anfkalxTaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vasutxgaLu) celAlxpililxyAgi bididxrada; asatxvayxsatxvAgi, aDADxdiDiDxyAgi -- esedirada, aDacirada. 
\num{2} (sheYli \mo vugaLa \vi) saraLa; akilxSaTx; nirADaMbarada; QujuvAda. 
\enum
\emng
\eentry

\bentry
\word[unco(1)]{unco} 
\pron{anfkoV} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vicitarx; asAmAnayx; aparUpada: \eng{an unco sight} vicitarx daqshayx. 
\num{2} gamanAhaRvAda. 
\enum
\emng
\eentry

\bentry
\word[unco(2)]{unco} 
\pron{anfkoV} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AgaMtuka; hosaba; aparicita. 
\num{2} (\bava dalilx) sudidx; samAcAra. 
\enum
\emng
\eentry

\bentry
\word[unco(3)]{unco} 
\pron{anfkoV} 
\gl{\kirxvi}
\expl{}
\bmng
visheVSavAgi; bahaLavAgi; vipariVtavAgi. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{the unco guid} (\kanmu\ \hiV) aMdhamatasharxdedhxya jana; kaTATx mataniSeThxyuLaLxvaru. 
\emng
\eentry

\bentry
\word{uncoil} 
\pron{anfkAilf} 
\gl{\sakirx}
\expl{}
\bmng
suruLi bicucx, saDilisu. 
\emng
\eentry

\bentry
\word{uncolored} 
\pron{anfkalaDfR} 
\gl{\gu}
\expl{(\ame) }
\bmng
= \hyperlink{uncoloured}{uncoloured}. 
\emng
\eentry

\bentry
\word{uncoloured} 
\pron{anfkalaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} baNaNxvilalxda; vaNaRrahita; nivaRNaRvAda. 
\num{2} baNaNx hAkirada; baNaNx hacacxda. 
\num{3} utepxrXVkeSxyilalxda; baNaNx kaTiTxlalxda; atishayoVkitxyilalxda; vasutxniSaThxvAda: \eng{an uncoloured description of events in South Africa} \da\ Aphirxkadalilx naDeda GaTanegaLanunx kurita vasutxniSaThxvAda vaNaRne. 
\num{4} (yAvudeV oMdariMda) parxBAvitavAgada; niSapxkaSxpAtavAda. 
\enum
\emng
\eentry

\bentry
\word{uncombed} 
\pron{anfkoVmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (kUdalina \vi) bAcilalxda; OraNa mADilalxda. 
\num{2} (vayxkitxya \vi) talebAcilalxda; aNiyAgirada. 
\enum
\emng
\eentry

\bentry
\word{uncome-at-able} 
\pron{anfkamAYxTabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\AmA) keYge -- eTukada, sikakxda; muTaTxlAgada; samiVpisalAgada; agamayx. 
\num{2} paDeyalAgada; sikakxda; dusAsxdhayx; alaBayx; apArxpayx; agArxhayx. 
\enum
\emng
\eentry

\bentry
\word{uncomely} 
\pron{anfkamfli} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} yoVgayxvalalxda; ucitavalalxda; takukxdalalxda; sUkatxvalalxda. 
\num{2} aMdavilalxda; suMdaravAgirada; kUrUpi. 
\enum
\emng
\eentry

\bentry
\word{uncomfortable} 
\pron{anfkaMphaTaRbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} toMdareya; aswkayaRda; hitavalalxda; suKavilalxda; nemamxdiyilalxda. 
\num{2} kaLavaLada; mujugarada: \eng{uncomfortable silence} mujugarada mwna. 
\enum
\emng
\eentry

\bentry
\word{uncomfortableness} 
\pron{anfkaMphaTaRbalfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aswkayaR; ahitate; nemamxdiyilalxdiruvike. 
\num{2} mujugara. 
\enum
\emng
\eentry

\bentry
\word{uncomfortably} 
\pron{anfkaMphaTaRbilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aswkayaRvAgi; hitavAgirade; nemamxdiyilalxdaMte. 
\num{2} mujagaradiMda. 
\enum
\emng
\eentry

\bentry
\word{uncommercial}
\pron{anfkamaSaRlf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vAyxpAreVtara; vAyxpAra, vANijayxgaLige saMbaMdhisada; udayxma \mo vugaLalilx toDagirada. 
\num{2} vAyxpAradhamaRvirudadhx(vAda); vAyxpAra niyamagaLige virudadhxvAda. 
\enum
\emng
\eentry

\bentry
\word{uncommited} 
\pron{anfkamiTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (yAvudeV naDavaLike, mAgaR, \mo vugaLige) kaTuTxbiVLada; badadhxnAgada; sikikxkoLaLxda. 
\num{2} savxtaMtarx; alipatx; yAvudeV nidiRSaTx rAjakiVya dheyxVyakekx yA guMpige badadhxnAgirada: \eng{the uncommited countries} (yAvudeV shakitxbaNakUkx seVrada) alipatx rASaTxrXgaLu. 
\enum
\emng
\eentry

\bentry
\word[uncommon(1)]{uncommon} 
\pron{anfkAmanf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asAmAnayx; viraLa; aparUpada; vilakaSxNavAda. 
\num{2} asAdhAraNa mahatitxna; atisherxVSaThxvAda; aduBxtavAda. 
\enum
\emng
\eentry

\bentry
\word[uncommon(2)]{uncommon}
\pron{anfkAmanf} 
\gl{\kirxvi}
\expl{}
\bmng
 (\pArxparx) (\AmA) asAdhAraNavAgi; sAmAnayxvalalxda riVtiyalilx: \eng{he was uncommon fat} avanu asAdhAraNavAgi dhaDUtiyAgidAdxne. 
\emng
\eentry

\bentry
\word{uncommonly} 
\pron{anfkAmanfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} asAmAnayxvAgi; viraLavAgi; aparUpavAgi. 
\num{2} atiyAgi; asAdhAraNavAgi; bahaLa: \eng{an uncommonly intelligent boy} asAdhAraNa budidhxvaMta huDuga. 
\enum
\emng
\eentry

\bentry
\word{uncommonness} 
\pron{anfkAmanfnisf} 
\gl{\nA}
\expl{}
\bmng
 asAmAnayxte; asAdhAraNavAgiruvudu. 
\emng
\eentry

\bentry
\word{uncommunicative} 
\pron{anfkamUyxnikaTi} 
\gl{\gu}
\expl{}
\bmng
 hecucx mAtanADada; manasisxnalilxruvudanunx tiLisada; guTaTxnunx bAyibiTuTx heVLada; gurugumamxneya; mwnada; mitaBASi; gumamxnagusuka. 
\emng
\eentry

\bentry
\word{uncommunicatively} 
\pron{anfkamUyxnikaTili}
\gl{\kirxvi}
\expl{}
\bmng
 hecucx mAtanADade; manasisxnalilxruva guTaTxnunx bAyibiTuTx heVLade; atimwnadiMda; gumamxnagusukanaMte. 
\emng
\eentry

\bentry
\word{uncommunicativeness} 
\pron{anfkamUyxnikaTivfnisf} 
\gl{\nA}
\expl{}
\bmng
hecucx mAtanADadiruvudu; (manasisxnalilxruvudanunx) bAyibiTuTx heVLadiruvudu; atimwna; gumamxnagusukatana. 
\emng
\eentry

\bentry
\word{uncompanionable} 
\pron{anfkaMpAYxnayxnabalf}
\gl{\gu}
\expl{}
\bmng
 sahavAsayoVgayxnalalxda; senxVhaparavalalxda. 
\emng
\eentry

\bentry
\word{uncompensated} 
\pron{anfkAMpenesxVTiDf} 
\gl{\gu}
\expl{}
\bmng
 parihAra paDeyada; naSaTx tuMbada. 
\emng
\eentry

\bentry
\word{uncomplaining} 
\pron{anfkaMpelxVniMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AkeSxVpaNe mADada; asamAdhAna sUcisada; dUrada; doVSAroVpaNe mADada. 
\num{2} goNagade sahisikoLuLxva; dUrade anuBavisuva. 
\enum
\emng
\eentry

\bentry
\word{uncomplainingly} 
\pron{anfkaMpelxVniMgfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} goNagade; kaSaTxnaSaTx heVLikoLaLxde. 
\num{2} dUrade; tALikoMDu; sahisikoMDu. 
\enum
\emng
\eentry

\bentry
\word{uncompleted} 
\pron{anfkaMpilxVTiDf}
\gl{\gu}
\expl{}
\bmng
 pUtiR mugisada; pUtiRmADada; adhaRkekx biTaTx; apUNaR; asaMpUNaR. 
\emng
\eentry

\bentry
\word{uncomplicated} 
\pron{anfkAMpilxkeVTiDf}
\gl{\gu}
\expl{}
\bmng
 asaMkiVNaR; saMkiVNaRgoLisada yA saMkiVNaRvalalxda; gaMTugoVjilalxda; toDakilalxda; jaTilavalalxda; saraLavAda; Quju. 
\emng
\eentry

\bentry
\word{uncomplimentary} 
\pron{anfkAMpilxmeMTari} 
\gl{\gu}
\expl{}
\bmng
 shAlxGayxvalalxda; parxshaMsAtamxkavalalxda; niMdAtamxka. 
\emng
\eentry

\bentry
\word{uncompounded} 
\pron{anfkAMpwMDiDf} 
\gl{\gu}
\expl{}
\bmng
 berasada; misharxNa mADada; amishirxta; saMyukatx vAgisada; asaMyoVjita; asaMyukatx. 
\emng
\eentry

\bentry
\word{uncomprehending} 
\pron{anfkAMpirxheMDiMgf} 
\gl{\gu}
\expl{}
\bmng
 aritukoLaLxda; garxhisada; athaRmADi koLaLxda. 
\emng
\eentry

\bentry
\word{uncomprehendingly} 
\pron{anfkAMpirxheMDiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
 aritukoLaLxde; garxhisade; athaRmADikoLaLxde. 
\emng
\eentry

\bentry
\word{uncomprehension} 
\pron{anfkAMpirxhenaSxnf} 
\gl{\nA}
\expl{}
\bmng
 agarxhaNa; agarxhike; aritukoLaLxdiruvike; athaRmADikoLaLxdiruvike. 
\emng
\eentry

\bentry
\word{uncompromising} 
\pron{anfkAMparxmeYsiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} rAjige barada; saMdhAnakekx sidadhxnalalxda. 
\num{2} paTuTx hiDidiruva; sagagxda; soVlada; KaMDitavAdiyAda; moMDutanada; haTamAriyAda. 
\enum
\emng
\eentry

\bentry
\word{uncompromisingly} 
\pron{anfkAMparxmeYsiZMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} rAjige barade; saMdhAnakekx sidadhxvilalxde. 
\num{2} paTuTxhiDidu; sagagxde; soVlade; haTa{hi}Didu; moMDutanadiMda. 
\enum
\emng
\eentry

\bentry
\word{uncompromisingness} 
\pron{anfkAMparxmeYsiZMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} saMdhAnakekx, rAjige -- sidadhxvilalxdiruvike. 
\num{2} paTuTxhiDidiruvike; KaMDitavAditavx; moMDutana; haTamAritana. 
\enum
\emng
\eentry

\bentry
\word{unconcealed} 
\pron{anfkanisxVlfDx} 
\gl{\gu}
\expl{}
\bmng
 mucicxDada; goVpayxvAgirisada; mucucxmareyilalxda; parxkaTavAda; visapxSaTxvAda. 
\emng
\eentry

\bentry
\word{unconcern} 
\pron{anfkanasxnfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nirAtaMka; nivAyxRkulate; kaLavaLa, ciMte -- ilalxdiruvudu. 
\num{2} audAsiVnayx; nirAsakitx; upeVkeSx; asaDeDx. 
\enum
\emng
\eentry

\bentry
\word{unconcerned} 
\pron{anfkanasxnfDxR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nishicxMta; nirAtaMkavAda; kaLavaLavilalxda; ciMte hacicxkoLaLxda. 
\num{2} udAsiVna; nirAsakatx; asaDeDxya. 
\enum
\emng
\eentry

\bentry
\word{unconcernedly} 
\pron{anfkanasxniRDilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} nirAtaMkavAgi; kaLavaLavilalxde; ciMte hacicxkoLaLxde. 
\num{2} saMbaMdhavilalxdaMte; udAsiVnavAgi; asaDeDxyiMda; upeVkeSxyiMda; nirAsakitxyiMda. 
\enum
\emng
\eentry

\bentry
\word{unconcluded} 
\pron{anfkanfkUlxDiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asamApatx; mugisada; konegANada. 
\num{2} apUNaRvAda; asaMpUNaR.
\num{3} aniNiRVta; tiVmARnisada. 
\enum
\emng
\eentry

\bentry
\word{unconditional} 
\pron{anfkaMDiSana(nf)lf} 
\gl{\gu}
\expl{}
\bmng
 yAva SaratUtx ilalxda; beVSaratAtxda; nirupAdhikavAda: \eng{unconditional surrender} beVSaratAtxda sharaNAgati yA soVlopipxge. 
\emng
\eentry

\bentry
\word{unconditionality} 
\pron{anfkaMDiSanAYxliTi} 
\gl{\nA}
\expl{}
\bmng
 beVSaratutx; yAva SaratUtx ilalxdiruvike; nirupAdhika sithxti. 
\emng
\eentry

\bentry
\word{unconditionally} 
\pron{anfkaMDiSanali} 
\gl{\kirxvi}
\expl{}
\bmng
 beVSaratAtxgi; yAva SaratUtx ilalxde; nirupAdhikavAgi. 
\emng
\eentry

\bentry
\word{unconditioned} 
\pron{anfkaMDiSa(Sf)nfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} niyama, kaTuTxpADugaLige -- oLapaTiTxlalxda; nirupAdhikavAda; beVSaratitxna; yAvoMdu SaratitxgU kaTuTxbiVLada. 
\num{2} (vataRne \mo vugaLa \vi) sAvxBAvika; sahajavAda; pUvaRnishicxtavalalxda; hiMdina anuBavagaLiMda, aBAyxsagaLiMda -- niyaMtirxtavAgirada, rUpitavAgirada. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{unconditioned reflex} (\mavi) (oMdu parxcoVdanege huTuTxguNadiMda parxtikirxye yAgi naDeyuva) sahaja parxtivataRne; aniyaMtirxta parxtikirxye. 
\emng
\eentry

\bentry
\word{unconfined} 
\pron{anfkanfpheYnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hadidxnalilxDada; serehAkada; baMdhisada. 
\num{2} miti kalipxsada; elelxyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unconfirmed} 
\pron{anfkanfphamfDxR} 
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ vadaMtiya \vi) daqDhapaTiTxlalxda; sithxrapaDisada; puSiTxVkarisilalxda. 
\emng
\eentry

\bentry
\word{unconformable} 
\pron{anfkanfphAmaRbf(ba)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anurUpavalalxda; anuguNavalalxda. 
\num{2} anuvatiRsada; opapxda; takukxdalalxda; hoMdikoLaLxlAgada. 
\num{3} (\BUvi) (shilAsatxragaLa \vi) EkABimuKavAgirada; vividhABimuKada; satxragaLelalxvU oMdeV dikikxge tirugirada. 
\num{4} (\ca) iMgelxMDinalilx 1682ralilx kAnUnAgidadx dhAmiRka AcaraNeya EkatA nibaMdhanege anusAra vAgirada. 
\enum
\emng
\eentry

\bentry
\word{unconformableness} 
\pron{anfkanfphamaRbflfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ananurUpate; anurUpavalalxdiruvike. 
\num{2} ananuvataRne; hoMdikoLaLxdiruvike; oLapaTuTx naDeyalAgadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unconformably} 
\pron{anfkanfphamaRbilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anurUpavalalxda hAge. 
\num{2} anuvatiRsade; hoMdikoLaLxlAgade; oLapaTuTx naDeyalAgade. 
\enum
\emng
\eentry

\bentry
\word{unconformity} 
\pron{anfkanfphAmiRTi}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ananurUpate; anuguNavAgilalxdiruvike. 
\num{2} hoMdikeyilalxdiruvike; ananuvataRne. 
\num{3} (\BUvi) EkABimuKarAhitayx; shilAsatxragaLa satxriVkaraNada anukarxmate oMdeV dikikxnalilxlalxdiruvudu. 
\enum
\emng
\eentry

\bentry
\word{uncongenial} 
\pron{anfkanfjiVnialf}
\gl{\gu}
\expl{}
\bmng
 samarasavalalxda; hoMdikeyilalxda; hoMdikoLaLxda; hitavAgirada. 
\emng
\eentry

\bentry
\word{unconjecturable} 
\pron{anfkaMjekacxraba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
UhisalasAdhayxvAda; kalipxsikoLaLxlu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unconnected} 
\pron{anfkanekiTxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parasapxra saMbaMdhavilalxda; saMbaMdhisada. 
\num{2} kUDisirada; seVrisilalxda; asaMyoVjita. 
\num{3} (BASaNa \mo vugaLa \vi) oMdakokxMdu saMbaMdhavilalxda; asaMbadadhx; asaMgatavAda; takaRbadadhxvAgirada: \eng{unconnected ideas} asaMgata aBipArxyagaLu. 
\num{4} saMbaMdhavirada; naMTasatxtanavirada; baMdhutavxvilalxdiruva. 
\enum
\emng
\eentry

\bentry
\word{unconnectedly} 
\pron{anfkanekiTxDilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} saMbaMdha rahitavAgi; saMbaMdhavilalxdaMte. 
\num{2} asaMgatavAgi; asaMbadadhxvAgi; takaRrahitavAgi. 
\enum
\emng
\eentry


\bentry
\word{unconnectedness} 
\pron{anfkanekiTxDfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} saMbaMdharahitate; saMbaMdhavilalxdiruvike. 
\num{2} asaMgatate; asaMbadadhxte; takaRhiVnate. 
\enum
\emng
\eentry

\bentry
\word{unconquerable} 
\pron{anfkAMkarabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 (shaturxvanunx) jayisalAgada; gelalxlAgada; ajeVya. 
\emng
\eentry

\bentry
\word{unconquerableness} 
\pron{anfkAMkarabalfnisf} 
\gl{\nA}
\expl{}
\bmng
 ajeVyate; gelalxlAgadiruvike. 
\emng
\eentry

\bentry
\word{unconquerably} 
\pron{anfkAMkarabilx}
\gl{\kirxvi}
\expl{}
\bmng
 ajeVyavAgi; gelalxlAgadaMte. 
\emng
\eentry

\bentry
\word{unconquered} 
\pron{anfkAMkaDfR}
\gl{\gu}
\expl{}
\bmng
 jayisada; gelalxda; ajita. 
\emng
\eentry

\bentry
\word{unconscionable} 
\pron{anfkAnaSxnabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AtamxsAkiSxyilalxda. 
\num{2} AtamxsAkiSxge -- hedarada,virudadhxvAda. 
\num{3} sariyilalxda; tiVrA anAyxyada; nAyxyavalalxda; takaRsamamxtavalalxda. 
\num{4} (yArU opapxdaSuTx) atiyAda; vipariVtavAda; mitimiVrida: \eng{you take an unconscionable time for dressing} baTeTx hAkikoLaLxlu niVnu mitimiVrida kAla tegedukoLuLxve. 
\enum
\emng
\eentry

\bentry
\word{unconscionableness}
\pron{anfkAnaSxnabalfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AtamxsAkiSxya aBAva. 
\num{2} AtamxsAkiSxge virudadhxvAgiruvudu. 
\num{3} tiVrA anAyxyavAgi naDedukoLuLxvudu. 
\num{4} veYpariVtayx; mitimiVriruvudu. 
\enum
\emng
\eentry

\bentry
\word{unconscionably} 
\pron{anfkAnaSxnabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} AtamxsAkiSxyilalxdaMte. 
\num{2} AtamxsAkiSxge virudadhxvAgi. 
\num{3} tiVrA anAyxyavAgi. 
\num{4} (yArU opapxdaSuTx) atiyAgi; vipariVtavAgi; mitimiVri. 
\enum
\emng
\eentry

\bentry
\word[unconscious(1)]{unconscious} 
\pron{anfkAnaSxsf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxjecnx yilalxda; jAcnxnatapipxda; meYmeVle ecacxravilalxda: \eng{lay unconscious for hours} aneVka gaMTegaLa kAla parxjecnxyilalxde bididxdadx. 
\num{2} aparxjAcnxta; arivilalxda; tiLiyada: \eng{was unconscious of any change} EnoMdu badalAvaNeyanUnx ariyade idadx. \eng{unconscious prejudice} aparxjAcnxta pUvARgarxha. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{unconscious humour} arivilalxda hAsayx. 
\emng
\eentry

\bentry
\word[unconscious(2)]{unconscious} 
\pron{anfkAnaSxsf} 
\gl{\nA}
\expl{}
\bmng
 (\mavi) (manasisxna) aparxjecnx; supatx, ajAgaqta -- sithxti; jAgaqta manasisxge nilukada, Adare naDavaLike, BAvagaLu, \mo vugaLa meVle pariNAma biVruva manasisxna viBAga: \eng{collective unconscious} samaSiTx supatxparxjecnx. 
\emng
\eentry

\bentry
\word{unconsciously} 
\pron{anfkAnaSxsfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} parxjecnxyilalxde; meYmeVle ecacxravilalxde. 
\num{2} arivilalxde; tiLiyade. 
\enum
\emng
\eentry

\bentry
\word{unconsciousness} 
\pron{anfkAnaSxsfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} parxjAcnxhiVnate; meYmeVle ecacxravilalxdiruvike; mUCeR. 
\num{2} ajAcnxna; arivilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unconsecrated} 
\pron{anfkAnisxkerxVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} miVsalAgiDada; muDupAgiDada. 
\num{2} pavitirxVkarisada; pavitarxgoLisada. 
\enum
\emng
\eentry

\bentry
\word{unconsenting} 
\pron{anfkanesxMTiMgf} 
\gl{\gu}
\expl{}
\bmng
 opapxda; samamxtisada; anumoVdisada; oDaMbaDada. 
\emng
\eentry

\bentry
\word{unconsidered} 
\pron{anfkanisxDaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaDegaNisida; upeVkiSxta; parigaNisade biTiTxruva. 
\num{2} (mAtu, kelasa, \mo vugaLa \vi) payARloVcisada; duDukina; vicAra mADada; vicArahiVna. 
\num{3} (utatxra \mo vugaLa \vi) Ashu; tatfkaSxNada; taTakakxneya; A kaSxNada; modaleV yoVcisidadxlalxda. 
\enum
\emng
\eentry

\bentry
\word{unconsolable} 
\pron{anfkanosxVlaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperref{kandict_i.pdf}{I}{inconsolable}{inconsolable}. 
\emng
\eentry

\bentry
\word{unconsolably} 
\pron{anfkanosxVlabilx}
\gl{\kirxvi}
\expl{}
\bmng
= \hyperref{kandict_i.pdf}{I}{inconsolably}{inconsolably}. 
\emng
\eentry

\bentry
\word{unconstitutional} 
\pron{anfkAnfsiTxTUyxSanalf} 
\gl{\gu}
\expl{}
\bmng
 rAjAyxMgakekx virudadhxvAda; saMvidhAna bAhira; deVshada kAnUnige viroVdhavAda. 
\emng
\eentry

\bentry
\word{unconstitutionality} 
\pron{anfkAnfsiTxTUyxSanAYxliTi}
\gl{\nA}
\expl{}
\bmng
 rAjAyxMgakekx virudadhxvAgiruvudu; saMvidhAnabAhirate. 
\emng
\eentry

\bentry
\word{unconstitutionally} 
\pron{anfkAnfsiTxTUyxSanali}
\gl{\kirxvi}
\expl{}
\bmng
 saMvidhAnabAhiravAgi; rAjAyxMgakekx virudadhxvAgi. 
\emng
\eentry

\bentry
\word{unconstrained} 
\pron{anfkanfseTxrXVnfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} balAtakxrisada; nibaRMdhapaDisada; otAtxya mADada; balavaMtakekx sikakxda; nibaRMdhakokxLapaDada; abAdhita. 
\num{2} savxcaCxMdavAda. 
\enum
\emng
\eentry

\bentry
\word{unconstraint} 
\pron{anfkanfseTxrXVMTf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} savxcaCxMdate; sevxVceCx. 
\num{2} anibaRMdhate; otAtxya, balavaMta, nibaRMdha, \mo vugaLilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unconstricted} 
\pron{anfkanfsiTxrXkiTxDf} 
\gl{\gu}
\expl{}
\bmng
 asaMkucita; kiridAgisada; saNaNxdAgisada. 
\emng
\eentry

\bentry
\word{unconsumed} 
\pron{anfkanfsUyxmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nAshavAgade uLidiruva. 
\num{2} vayxyamADada; baLasi mugisada. 
\num{3} inUnx tiMdirada yA kuDidirada. 
\enum
\emng
\eentry

\bentry
\word{unconsummated} 
\pron{anfkAnfsuyxmeVTiDf}
\gl{\gu}
\expl{}
\bmng
 pUNaRgoLisada; sAMgagoLisada; neraveVrisada. 
\emng
\eentry

\bentry
\word{uncontainable} 
\pron{anfkanfTeVnabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oLagoMDiralAgada; hiDisalAgada. 
\num{2} taDeyalAgada. 
\enum
\emng
\eentry

\bentry
\word{uncontaminated} 
\pron{anfkanfTAYxmineVTiDf}
\gl{\gu}
\expl{}
\bmng
 akaluSita; koLeyAgada; malinavAgada; kaLaMkitagoLaLxda; kaluSitavAgada. 
\emng
\eentry

\bentry
\word{uncontested} 
\pron{anfkanfTesiTxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} caciRsada; vAdamADada. 
\num{2} viroVdhisada; takarAru hUDada; parxtivAdisada. 
\num{3} sapxdhiRsada; hoVrADada. 
\enum
\emng
\eentry

\bentry
\word{uncontestedly} 
\pron{anfkanfTesiTxDilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} caciRsade; vAdamADade. 
\num{2} viroVdhisade. 
\num{3} sapxdhiRsade; hoVrADade. 
\enum
\emng
\eentry

\bentry
\word{uncontradicted} 
\pron{anfkAMTarxDikiTxDf}
\gl{\gu}
\expl{}
\bmng
 viroVdhisada; parxti heVLada; eduru nuDiyada; aparxtAyxKAyxta. 
\emng
\eentry

\bentry
\word{uncontrollable} 
\pron{anfkaMTorxVlabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 adhiVnadalilxDalAgada; hiDitadalilxDalAgada; hatoVTiyalilxDalAgada; niyaMtirxsalAgada; nigarxhisalAgada; adamayx; dudaRmayx. 
\emng
\eentry

\bentry
\word{uncontrollableness} 
\pron{anfkaMTorxVlabalfnisf}
\gl{\nA}
\expl{}
\bmng
 dudaRmayxte; adamayxte; aniyaMtarxNa; hiDitadalilxDalAgadiruvike. 
\emng
\eentry

\bentry
\word{uncontrollably} 
\pron{anfkaMTorxVlabilx}
\gl{\kirxvi}
\expl{}
\bmng
 adhiVnadalilxDalAgade; aniyaMtirxtavAgi. 
\emng
\eentry

\bentry
\word{uncontrolled} 
\pron{anfkaMTorxVlfDx}
\gl{\gu}
\expl{}
\bmng
hatoVTiyalilxDada; aniyaMtirxta. 
\emng
\eentry

\bentry
\word{uncontroversial} 
\pron{anfkAMTarxvaSaRlf}
\gl{\gu}
\expl{}
\bmng
 niviRvAdavAgiruva; cacARsapxdavalalxda; vivAdakekx avakAshaveV ilalxda. 
\emng
\eentry

\bentry
\word{uncontroversially} 
\pron{anfkAMTarxvaSaRli}
\gl{\kirxvi}
\expl{}
\bmng
 niviRvAdavAgi; cacARsapxdavAgirade. 
\emng
\eentry

\bentry
\word{uncontroverted} 
\pron{anfkAMTarxvaTiRDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} caciRsada; vAdisada; vAdamADada. 
\num{2} aparxtAyxKAyxta; KaMDisalapxDada; parxti heVLada; viroVdhisalapxDada. 
\enum
\emng
\eentry

\bentry
\word{uncontrovertible} 
\pron{anfkAMTarxvaTiRbflf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vAda mADalAgada; caciRsalAgada. 
\num{2} KaMDisalAgada; parxtiheVLalAgada; viroVdhisalAgada. 
\enum
\emng
\eentry

\bentry
\word{unconventional} 
\pron{anfkanevxnaSxnalf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMparxdAyabadadhxvalalxda; rUDhiyadalalxda. 
\num{2} savxtaMtarxvAda. 
\enum
\emng
\eentry

\bentry
\word{unconventionalism}
\pron{anfkanevxnaSxnalisaZmf}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unconventionality}{unconventionality}. 
\emng
\eentry

\bentry
\word{unconventionality} 
\pron{anfkanevxnaSxnAYxliTi}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} saMparxdAyabadadhxvalalxdiruvudu; rUDhi yaMtilalxdiruvudu. 
\num{2} AcAra, vicAra -- sAvxtaMtarxyX. 
\enum
\emng
\eentry

\bentry
\word{unconventionally} 
\pron{anfkanevxnaSxnali}
\gl{\kirxvi}
\expl{}
\bmng
 saMparxdAyakekx kaTuTxbiVLade. 
\emng
\eentry

\bentry
\word{unconverted} 
\pron{anfkanavxTiRDf} 
\gl{\gu}
\expl{}
\bmng
 badalAyisada; mApaRDisada; parivatiRsada. 
\emng
\eentry

\bentry
\word{unconvinced} 
\pron{anfkanivxnfsfTx} 
\gl{\gu}
\expl{}
\bmng
 manavarikeyAgada; opipxgeyAgada. 
\emng
\eentry

\bentry
\word{unconvincing} 
\pron{anfkanivxnisxMgf}
\gl{\gu}
\expl{}
\bmng
 opapxlAgada; opipxsalAgada; manadaTuTx mADalAgada; taqpitxkaravalalxda. 
\emng
\eentry

\bentry
\word{uncooked} 
\pron{anfkukfTx}
\gl{\gu}
\expl{}
\bmng
 beVyisada; aTiTxlalxda; beMdilalxda; hasiyAda; aDige mADilalxda. 
\emng
\eentry

\bentry
\word{uncool} 
\pron{anfkUlf}
\gl{\gu}
\bmng
\bnum
\num{1} (jAsfZ saMgiVtada \vi) anibaRMdhavAda; savxcaCxMda. 
\num{2} tagagxdiruva; saDilagoLaLxdiruva; bigupina. 
\num{3} hitavalalxda; ahitakara; rucisada. 
\enum
\emng
\eentry

\bentry
\word{uncooperative} 
\pron{anfkoVAparaTivf} 
\gl{\gu}
\expl{}
\bmng
 sahakarisada; ogagxTATxgi kelasa mADada; asahakAri. 
\emng
\eentry

\bentry
\word{uncooperatively} 
\pron{anfkoVAparaTivfli}
\gl{\kirxvi}
\expl{}
\bmng
 sahakarisade. 
\emng
\eentry

\bentry
\word{uncoordinated} 
\pron{anfkoVADiRneVTiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asamanivxta; hoMdANikeyilalxda; susaMGaTitavalalxda; anoyxVnayx saMbaMdhavilalxda. 
\num{2} (vayxkitxya calanavalanagaLu \mo vugaLa \vi) oDoDxDADxda; aDADxdiDiDxyAda. 
\enum
\emng
\eentry

\bentry
\word{uncopiable} 
\pron{anfkApiaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
nakaletatxlAgada; nakalu, (maru)parxti -- mADalAgada. 
\emng
\eentry

\bentry
\word{uncord} 
\pron{anfkADfR}
\gl{\sakirx}
\expl{}
\bmng
 (kaTiTxruva) hagagx, huri, taMti, \mo vanunx -- bicucx, saDilisu. 
\emng
\eentry

\bentry
\word{uncork} 
\pron{anfkAkfR}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (siVse \mo vugaLa) biraDe tege; mucacxLa tere. 
\num{2} (BAvane \mo vanunx) vayxkatxpaDisu; horasUsu; parxkaTapaDisu. 
\enum
\emng
\eentry

\bentry
\word{uncorroborated} 
\pron{anfkarAbareVTiDf}
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ sAkaSxyX \mo vugaLa \vi) sithxrapaDisada; daqDhiVkarisada; rujuvAtAgada; samathiRsadiruva; aparxmANita. 
\emng
\eentry

\bentry
\word{uncorrupted} 
\pron{anfkarapiTxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BarxSATxcArakekx oLagAgada; laMca ruSuvatutxgaLige IDAgada; naDategeDada. 
\num{2} keTiTxrada; ashudadhxgoMDirada. 
\enum
\emng
\eentry

\bentry
\word{uncountability} 
\pron{anfkwMTabiliTi} 
\gl{\nA}
\expl{}
\bmng
 eNisalAgadiruvike; asaMKeyxVyate. 
\emng
\eentry

\bentry
\word{uncountable} 
\pron{anfkwMTaba(bf)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eNisalAgada; asaMKeyxVya. 
\num{2} apAra; BAri; parxBUta: \eng{uncountable wealth} apAra saMpatutx. 
\num{3} (\vAyx) (nAmapadada \vi) niteyxYkavacanada; bahuvacana rUpavilalxda yA anideRVshaka guNavAciyoDane baLasalAgada, \udA\ \eng{uncountable happiness.} 
\enum
\emng
\eentry

\bentry
\word{uncountably} 
\pron{anfkwMTabilx}
\gl{\kirxvi}
\expl{}
\bmng
 eNisalAgadaMte; asaMKAyxtavAgi; agaNitavAgi. 
\emng
\eentry

\bentry
\word{uncounted} 
\pron{anfkwMTiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eNisada; agaNita; asaMKAyxta. 
\num{2} asaMKayx; aneVkAneVka: \eng{uncounted bills} asaMKayx bilulxgaLu. 
\enum
\emng
\eentry

\bentry
\word{uncouple} 
\pron{anfkapa(pf)lf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (reYlevx vAyxganfgaLa \vi) kUDukoMDigaLanunx kaLacu. 
\num{2} (nAyigaLu \mo vugaLa \vi) kUDubAranunx bicucx. 
\enum
\emng
\eentry

\bentry
\word{uncoupled} 
\pron{anfkapa(pf)lfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (reYluvAyxganf \vi) kUDukoMDi kaLacida. 
\num{2} (nAyi \mo vugaLa \vi) kUDubAru bicicxda. 
\enum
\emng
\eentry

\bentry
\word{uncourtly} 
\pron{anfkoVTfRli}
\gl{\gu}
\expl{}
\bmng
 saBayxvalalxda; avinayada; naDenuDigaLalilx nayavilalxda; mayARde pAlisada, oraTAda. 
\emng
\eentry

\bentry
\word{uncouth} 
\pron{anfkUtf} 
\gl{\gu}
\bmng
\bnum
\num{1} (vayxkitx, naDevaLike, \mo vugaLa \vi) oraTu; nayavilalxda; vakarxvAda; eDavaTATxda; nayanAjUkilalxda; asaMsakxqqta: \eng{uncouth voices} oraTu dhavxnigaLu. \eng{uncouth behaviour} asaMsakxqqta naDavaLike. 
\num{2} (BASe) apariSakxqqta; oDoDxDADxda; kakaRsha; kaTu. 
\numi{3} (\pArxparx) 
\banum
\alnum{a} ajAcnxta; tiLiyada; aparicita. 
\alnum{b} (sathxLa \mo vugaLa \vi) yArU tuLidADada; kaMDUkANada; hokukx baLasada; nijaRnavAda. 
\alnum{c} kalulxmoraDigaLiruva; kADu -- vanayx. 
\alnum{d} (jiVvana \mo vugaLa \vi) anAgarika; swkayaRgaLilalxda: \eng{uncouth place} swkayaRgaLilalxda sathxLa. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{uncouthly} 
\pron{anfkUtfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} oraTAgi; nayanAjUkilalxde. 
\num{2} oDoDxDADxgi; vakarxvAgi; eDavaTuTx riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{uncouthness} 
\pron{anfkUtfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vakarxte; vikAravAgiruvudu. 
\num{2} oraTAgiruvudu; nayanAjUkilalxdiruvudu; anAgarikate. 
\enum
\emng
\eentry

\bentry
\word{uncouvenanted} 
\pron{anfkavanaMTiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ deVvara) opapxMdadalilx, oDaMbaDikeyalilx -- vAgAdxna mADilalxda yA AdhAravilalxda. 
\num{2} opapxMdavAgirada; oDaMbaDikeyAgirada. 
\enum
\emng
\eentry

\bentry
\word{uncover} 
\pron{anfkavarf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} hodike \mo vanunx tere; mare, musuku -- tege. 
\num{2} terediDu; teredu toVrisu; bayalu mADu; parxkaTapaDisu: \eng{the police have uncovered a plot against the President} rASaTxrXpatiya virudadhx naDesalAgidadx pitUriyoMdanunx poliVsaru bayaligeLedidAdxre. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
(\pArxparx) hAYxTu, ToVpi, \mo vanunx -- tege, tegediDu. 
\emng
\eentry

\bentry
\word{uncovered} 
\pron{anfkavaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} meYmeVle baTeTxyilalxda; naganx; betatxle meYyina. 
\num{2} taleya meVloMdu sUrilalxda; iralu maneyilalxda. 
\num{3} (tale) ToVpiyilalxda; bari taleya. 
\enum
\emng
\eentry

\bentry
\word{uncreate} 
\pron{anfkirxETf}
\gl{\akirx}
\expl{}
\bmng
 (sAhitayxka) nAshagoLisu; dhavxMsamADu. 
\emng
\eentry

\bentry
\word{uncreated} 
\pron{anfkirxETiDf} 
\gl{\gu}
\expl{}
\bmng
 (\pArxparx) savxyaMBU; (beVrobabxriMda) saqSiTxyAgade tAneV asitxtavxkekx baMda; saqSiTxsada; huTiTxsada. 
\emng
\eentry

\bentry
\word{uncreative} 
\pron{anfkirxETi}
\gl{\gu}
\expl{}
\bmng
 saqjanAtamxkavalalxda; saqSATxyXtamxkavalalxda; saqSiTxsuva shakitxyilalxda. 
\emng
\eentry

\bentry
\word{uncritical} 
\pron{anfkirxTika(kf)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aviveVcita; vimashARrahita; vimasheR mADada; vicAra mADade opipxkoMDubiDuva. 
\num{2} vimashaRnatatatxvXgaLige -- anuguNavAgirada, virudadhxvAda. 
\enum
\emng
\eentry

\bentry
\word{uncritically} 
\pron{anfkirxTikali}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vimasheR mADade; vicArarahitavAgi. 
\num{2} vimashaR tatatxvXgaLige virudadhxvAgi. 
\enum
\emng
\eentry

\bentry
\word{uncropped} 
\pron{anfkArxpfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (beLe \mo valilx) koyayxda; kaTAvu mADada; katatxrisada. 
\num{2} (jamiVnu) beLeyiDada; beLe beLesada. 
\enum
\emng
\eentry

\bentry
\word{uncross} 
\pron{anfkArxsf}
\gl{\akirx}
\expl{}
\bmng
 (katatxriyAkAradalilx yA aDaDxhAyidxruva keYkAlugaLu, cAkugaLu, \mo vanunx) biDisu; aDaDxhAyidxruvudanunx neVragoLisu. 
\emng
\eentry

\bentry
\word{uncrossed} 
\pron{anfkArxsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} shilube dharisada. 
\num{2} aDiDxmADada; aDaDx hAkada; savAlu hAkada. 
\num{3} (\birx) (cekf \mo vugaLa \vi) aDaDxgiVTu hAkada; `kArxsf' mADada; kATu hAkada. 
\enum
\emng
\eentry

\bentry
\word{uncrown} 
\pron{anfkwrxnf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (dore \mo vara) kiriVTAvaroVhaNa mADu; parxButavx tapipxsu. 
\num{2} (vayxkitxge) sAthxnapalalxTa mADu; adhikAradiMda keLagiLisu. 
\enum
\emng
\eentry

\bentry
\word{uncrowned} 
\pron{anfkwrxnfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} inUnx kiriVTadhAraNeyAgilalxda; paTaTx kaTiTxlalxda. 
\num{2} kiriVTavilalxda; anaBiSikatx; rAjaneMba hesarilalxde rAjana adhikAra calAyisutitxruva: \eng{the uncrowned king of boxing} muSiTxkALagada anaBiSikatx dore. 
\enum
\emng
\eentry

\bentry
\word{uncrushable} 
\pron{anfkarxSabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tuLiyalAgada; adamayx; sadebaDiyalAgada; soVlisalAgada; hatitxkakxlAgada; bagugx baDiya lAgada; damana mADalAgada. 
\num{2}(baTeTx) sukAkxgada. 
\enum
\emng
\eentry

\bentry
\word{uncrushed} 
\pron{anfkarxSfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tuLiyada; soVlisada; damanamADada. 
\num{2} hisukada; hiMDada. 
\num{3} jajijx puDimADada. 
\enum
\emng
\eentry

\bentry
\wordnospeech{UNCSTD}{UNCSTD}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{United Nations Conference on Science and Technology for Development. } 
\emng
\eentry

\bentry
\wordnospeech{UNCTAD}{UNCTAD}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{United Nations Conference on Trade and Development. } 
\emng
\eentry

\bentry
\word{unction} 
\pron{aMkfSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (cikitosxVdedxVshadiMda yA matakamaRvAgi eNeNx \mo vugaLiMda mADuva) teYlaleVpana; eNeNxyotutxvudu; aBayxMjana. 
\alnum{b} aBayxMjana, leVpana -- vasutx; teYla \mo vu. 
\eanum
\numie
\num{2} (\rUpa) shAmaka, sAMtavxnada, samAdhAnakAraka -- nuDi, AloVcane, vicAra. 
\numi{3} 
\banum
\alnum{a}(dhAmiRkoVtAsxha, dhAmiRka saMsAkxra, gADhaBAvane, \mo vugaLiMda uMTAguva) mAtina yA daniya -- mAdaRva, nuNupu, naya. 
\alnum{b} iMtahudara toVrike; kaqtaka naya; beNeNxyaMtha nayagArike. 
\eanum
\numie
\num{4}(yAvudoV saninxveVsha yA caTuvaTikeya bagegx) hitavanunx anuBavisuvudu; KuSipaDuvudu; bAyi capapxrisuvudu: \eng{she related the scandal with great unction} avaLu A ashilxVla vadaMtiyanunx bAyi capapxrisutAtx vivarisidaLu. 
\num{5} atiparxshaMse; BaTaTxMgitana; atiyAgi yA apArxmANikavAgi hogaLuvudu. 
\enum
\emng
\eentry

\bentry
\word{unctuous} 
\pron{aMkuTxyXasf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vataRne, mAtu, \mo vugaLa \vi) kaqtakoVtAsxhada; beNeNx hacucxva; nayagArikeya; BaTaTxMgitanada. 
\num{2} (nuDiya, aBipArxyada \vi) sAMtavxnada. 
\num{3} (\kanmu\ KanijagaLa \vi) jiDiDxna; teYlaguNada; sinxgadhx; jiDuDx meYya; sAbUnu sapxshaRda. 
\enum
\emng
\eentry

\bentry
\word{unctuously} 
\pron{aMkuTxyXasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} beNeNx hacucxvaMte; pusalAyisuvaMte; kaqtakoVtAsxhadiMda. 
\num{2} sAMtavxnagoLisuvaMte. 
\num{3} jiDiDxnaMte; sinxgadhxvAgi; sAbUninaMte jArutatx; beNeNxyaMte maqduvAgi. 
\enum
\emng
\eentry

\bentry
\word{unctuousness} 
\pron{aMkuTxyXasfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kaqtakoVtAsxhadiMdiruvike; nayagArike. 
\num{2} sAMtavxnakAritavx. 
\num{3}jiDiDxna hAgiruvudu; sinxgadhxte; beNeNxyaMte nayavAgiruvudu. 
\enum
\emng
\eentry

\bentry
\word{unculled} 
\pron{anfkalfDx}
\gl{\gu}
\expl{}
\bmng
 (hUvu \mo vanunx) biDisada; tiriyada; Ayada; Arisada; kiVLada. 
\emng
\eentry

\bentry
\word{uncultivated} 
\pron{anfkaliTxveVTiDf}
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ BUmiya \vi) sAguvaLi, kaqSi -- mADada. 
\emng
\eentry

\bentry
\word{uncultured} 
\pron{anfkalacxDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMsAkxravilalxda; susaMsakxqqtavalalxda. 
\num{2} (BUmi yA giDagaLa \vi) kaqSi, sAguvaLi -- mADilalxda. 
\enum
\emng
\eentry

\bentry
\word{uncurb} 
\pron{anfkabfR}
\gl{\akirx}
\expl{}
\bmng
 aDiDx, aDetaDe -- tegeduhAku; nibaRMdha tegeduhAku. 
\emng
\eentry

\bentry
\word{uncurbed} 
\pron{anfkabfDxR}
\gl{\gu}
\expl{}
\bmng
 taDehAkada; nibaRMdhisada; aDiDxpaDisada. 
\emng
\eentry

\bentry
\word{uncured} 
\pron{anfkuyxaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} cikitesx mADada; (roVga) vAsimADada yA vAsiyAgada; guNapaDisada yA guNavAgada; pariharisada. 
\num{2} (mAMsa \mo vanunx) saMsakxrisada; upUpxrisada yA hoge hAkada. 
\enum
\emng
\eentry

\bentry
\word{uncurl} 
\pron{anfkalfR}
\gl{\akirx}
\expl{}
\bmng
 suruLi bicucx; guMguru guMgurAgiruvudanunx -- biDisu, bicucx, saDilisu, saDilagoLisu. 
\emng
\eentry

\bentry
\word{uncurtailed} 
\pron{anfkaTeRVlfDx} 
\gl{\gu}
\expl{}
\bmng
 moTaku, kaDame, KoVta -- mADada; katatxrisada; harxsavx mADada (\rUpa\ saha). 
\emng
\eentry

\bentry
\word{uncurtained} 
\pron{anfkaTaRnfDx}
\gl{\gu}
\expl{}
\bmng
 teremucacxda; parade hAkada; tereyilalxda; mare mADada. 
\emng
\eentry

\bentry
\word{uncustomed} 
\pron{anfkasaTxmfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} suMkarahita; suMkavilalxda; suMka vidhisada. 
\num{2} suMka koTiTxlalxda; suMka teradiruva, pAvatisada. 
\enum
\emng
\eentry

\bentry
\word{uncut} 
\pron{anfkaTf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} katatxrisilalxda. 
\num{2} (pusatxkada \vi) hALegaLa aMcu katatxrisade biTiTxruva; hALetudiyanunx katatxrisi sama mADadiruva. 
\num{3} (garxMtha, calanacitarx, \mo vugaLa \vi) (\kanmu\ senAsxrf adhikArigaLu) katatxrisadiruva; moTaku mADadiruva. 
\num{4} (haraLina, \kanmu\ vajarxda \vi) kaMDarisada; sANe hiDiyada. 
\num{5} (baTeTx) juMgu katatxrisada. 
\enum
\emng
\eentry

\bentry
\word{undamaged} 
\pron{anfDAYxmijfDx} 
\gl{\gu}
\expl{}
\bmng
keDisada; hAni -- mADada yA Agada. 
\emng
\eentry

\bentry
\word{undated} 
\pron{anfDeVTiDf}
\gl{\gu}
\bmng
 tAriVKu namUdisada; dinAMka sUcisada; kAla sUcisada. 
\emng
\eentry

\bentry
\word{undaunted} 
\pron{anfDAMTiDf}
\gl{\gu}
\expl{}
\bmng
 edeguMdada; dheYyaRgeDada; niBiRVta. 
\emng
\eentry

\bentry
\word{undauntedly} 
\pron{anfDAMTiDfli}
\gl{\kirxvi}
\expl{}
\bmng
 edeguMdade; niBiRVtanAgi. 
\emng
\eentry

\bentry
\word{undauntedness} 
\pron{anfDAMTiDfnisf}
\gl{\nA}
\expl{}
\bmng
 edeguMdadiruvike; niBiRVti. 
\emng
\eentry

\bentry
\word{undecagon} 
\pron{anfDekaganf}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_h.pdf}{H}{hendecagon}{hendecagon}. 
\emng
\eentry

\bentry
\word{undeceive} 
\pron{anfDisiVvf}
\gl{\sakirx}
\expl{}
\bmng
(obabxna) tapupx tiLivaLikeyanunx kaLe; BArxMtiyanunx kaLe; moVsadiMda pArAgisu. 
\emng
\eentry

\bentry
\word{undecided} 
\pron{anfDiseYDiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nidhARravAgilalxda; tiVmARnavAgilalxda; aniNiRVta: \eng{the point is still undecided} I viSaya inUnx tiVmARnavAgilalx. 
\num{2} anishicxta; hiMdumuMdu noVDuva; tUgADuva, DoVlAyamAna, saMdigadhx -- manasisxna: \eng{he stood undecided} avanu hiMdumuMdu noVDutatx niMtubiTaTx. 
\enum
\emng
\eentry

\bentry
\word{undecidedly} 
\pron{anfDiseYDiDfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anidhARritavAgi; tiVmARnavAgade; aniNiRVtavAgi. 
\num{2} nishacxyavilalxde; hiMdumuMdu noVDutatx; tUgADutatx; DoVlAyamAna manasakxnAgi. 
\enum
\emng
\eentry

\bentry
\word{undecipherable} 
\pron{anfDiseYpharabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 (goVpayxlipi, kAkalipi, athaRvAgada baravaNige, \mo vugaLa \vi) toDaku biDisalAgada; Odalu kaSaTxvAda; gotAtxgada; sAmAnayx lipige tirugisalAgada. 
\emng
\eentry

\bentry
\word{undeclared} 
\pron{anfDikelxVDfR}
\gl{\gu}
\expl{}
\bmng
 tiLiyapaDisada; parxkaTisada; bahiraMgavAgi GoVSisada; aparxkaTita; aGoVSita. 
\emng
\eentry

\bentry
\word{undefeated} 
\pron{anfDiphiVTiDf}
\gl{\gu}
\expl{}
\bmng
 soVtilalxda; aparAjita; parABavagoMDilalxda. 
\emng
\eentry

\bentry
\word{undefended} 
\pron{anfDipheMDiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} arakiSxta; kApilalxda. 
\num{2} (\kanmu\ dAveya \vi) parxtivAda hUDade iruva; parxtivAdisada. 
\enum
\emng
\eentry

\bentry
\word{undefiled} 
\pron{anfDipheYlfDx}
\gl{\gu}
\expl{}
\bmng
 malina mADada; ashudadhxvAgadiruva; meYlige mADadiruva; apavitarxgoLisada; kaluSitavAgada; holegeDada. 
\emng
\eentry

\bentry
\word{undefinable} 
\pron{anfDipheYnaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} lakaSxNa nirUpisalAgada. 
\num{2} (elelx, meVre, \mo vanunx) aniNeRVya; gotutxmADalAgada; niNaRyisalAgada. 
\enum
\emng
\eentry

\bentry
\word{undefinably} 
\pron{anfDipheYnabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} lakaSxNa nirUpisalAgade. 
\num{2} (elelx \mo vanunx) gotutx mADalAgade; niNaRyisalAgade. 
\enum
\emng
\eentry

\bentry
\word{undefined} 
\pron{anfDipheYnfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} niSakxqqSaTxvAgi heVLadiruva; lakaSxNa nirUpisada. 
\num{2} (elelx, meVre, hadudx, \mo vugaLanunx) gotutxmADade biTiTxruva; niNaRyisadiruva; sapxSaTxgoLisadiruva. 
\enum
\emng
\eentry

\bentry
\word{undeify} 
\pron{anfDiipheY} 
\gl{\sakirx}
\expl{}
\bmng
 deVvareMdu gaNisadiru; deVvatavx kaLe; deVvareMdu -- BAvisadiru, ArAdhisadiru. 
\emng
\eentry

\bentry
\word{undelivered} 
\pron{anfDilivaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} udadhxrisada; uLisada; rakiSxsada. 
\num{2} biDugaDe mADada; vimoVcane paDeyada. 
\num{3} (gaBiRNiya \vi inUnx) hetitxrada; haDedilalxda; parxsavisada. 
\num{4} (patarx \mo vugaLa \vi) talapisada; muTiTxsada; baTavADe mADada. 
\num{5} (BASaNa \mo vugaLa \vi) anukatx; paThisada; heVLada; mADada. 
\num{6} (shishuvina \vi) ajAta; inUnx huTiTxlalxda; janisilalxda. 
\enum
\emng
\eentry

\bentry
\word{undemanding} 
\pron{anfDimAMDiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hakukx hiDidu, jarUrAgi -- keVLada, nibaRMdhapaDisi keVLada; tagAde mADadiruva. 
\num{2} apeVkiSxsada; apeVkeSxyilalxda; taqpatx. 
\enum
\emng
\eentry

\bentry
\word{undemandingness} 
\pron{anfDimAMDiMgfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} tagAde mADadiruvike. 
\num{2} anapeVkeSx; taqpitx. 
\enum
\emng
\eentry

\bentry
\word{undemocratic} 
\pron{anfDemakArxYxTikf}
\gl{\gu}
\expl{}
\bmng
 parxjAparxButavxviroVdhiyAda; parxjAtaMtarxkekx anuguNavAgirada. 
\emng
\eentry

\bentry
\word{undemocratically} 
\pron{anfDemakArxYxTikali}
\gl{\kirxvi}
\expl{}
\bmng
 parxjAparxButavxkekx -- anuguNavAgirade yA viroVdhiyAgi. 
\emng
\eentry

\bentry
\word{undemonstrated} 
\pron{anfDemanfseTxrXVTiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxdashiRsada; toVrisada. 
\num{2} parxtayxkaSxmADi yA pArxyoVgikavAgi toVrisada; nidashaRna koDada. 
\num{3} sidAdhxMta mADi toVrisada; sAdhisada. 
\enum
\emng
\eentry

\bentry
\word{undemonstrative} 
\pron{anfDimAnfsaTxrXTivf}
\gl{\gu}
\expl{}
\bmng
 saMyamada; manoVBAvagaLanunx horage toVrisikoLaLxda, parxdashiRsada, parxkaTisada, bahiraMgapaDisada. 
\emng
\eentry

\bentry
\word{undemonstratively} 
\pron{anfDimAnfsaTxrXTivfli}
\gl{\kirxvi}
\expl{}
\bmng
 saMyamadiMda; manoVBAvagaLanunx horage toVrisikoLaLxde, parxdashiRsade. 
\emng
\eentry

\bentry
\word{undemonstrativeness} 
\pron{anfDimAnfsaTxrXTivfnisf}
\gl{\nA}
\expl{}
\bmng
 saMyama; manoVBAvagaLanunx horage toVrisikoLaLxdiruvudu, parxdashiRsadiruvudu. 
\emng
\eentry

\bentry
\word{undeniable} 
\pron{anfDineYabflf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxti heVLalAgada; alalxgaLeyalAgada. 
\num{2} niviRvAdavAda; KacitavAda; tapupx kaMDuhiDiyalAgada; niduRSaTxvAda; nirAkeSxVpaNiVyavAda. 
\num{3} atuyxtatxmavAda; sherxVSaThxvAda; GanavAda: \eng{of undeniable worth} atayxMta sherxVSaThxvAda. 
\enum
\emng
\eentry

\bentry
\word{undeniableness} 
\pron{anfDineYabflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} parxtiheVLalAgadiruvike. 
\num{2} niviRvAdavAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{undeniably} 
\pron{anfDineYabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} nirAkarisalAgadaMte; parxtiheVLalAgada riVtiyalilx. 
\num{2} niviRvAdavAgi; KacitavAgi. 
\enum
\emng
\eentry

\bentry
\word{undenominational} 
\pron{anfDinAmineVSanalf}
\gl{\gu}
\expl{}
\bmng
 jAtayxtiVta; yAvudeV dhamaR, mata, paMgaDagaLigU saMbaMdhisada: \eng{undenominational education} jAtayxtiVta shikaSxNa. 
\emng
\eentry

\bentry
\word[under(1)]{under} 
\pron{aMDarf}
\gl{\upa}
\bmng
\bnum
\num{1} keLage; keLagaDe: \eng{it fell under the table} adu meVjina keLage bitutx. \eng{under the left eye} eDagaNiNxna keLage. 
\num{2} keLage; aDiyalilx; taLagaDe; buDadalilx: \eng{under the high wall} A etatxrada goVDeya buDadalilx. \eng{under the cliff} kaDidAda baMDeya keLage. 
\num{3} hiMdugaDe; hiMBAgadalilx; mareyalilx: \eng{under the island} divxVpada hiMdugaDe. 
\num{4} oLage; oLagaDe; taLagaDe: \eng{inserted a knife under the bark} togaTeya oLage oMdu cUriyanunx cucicxda. \eng{wore a vest under his shirt} SaTiRna oLage vesfTx toTiTxdadx. 
\num{5} keLadajeRya; keLagina: \eng{no one under a bishop} obabx biSapfgiMta keLagina dajeRyavaru yArU ilalx. 
\num{6} kaDame KaciRnalilx, vecacxdalilx: \eng{under Rs. 4000} nAlukx sAvira rUpAyi gaLigiMta kaDame KaciRnalilx. 
\num{7} oLagAgi; sikikx; adhiVnavAgi; oLapaTuTx; IDAgi; anuBavisutatx: \eng{sank under the load} BAradiMda keLage kusiyitu. \eng{groaning under tyranny} dabAbxLikege sikikx naraLutatx. \eng{is under repair} durasitxyalilxde. 
\num{8} peYru -- neTiTxruva, hAkiruva: \eng{50 acres under wheat} aivatutx ekarege goVdhi hAkidudx. 
\num{9} (yAvudoV iMdhanashakitx \mo vugaLiMda) muMdakekx calisutatx; dUDalAgutatx: \eng{under sail} hAyi (paTada) neraviniMda muMdakekx calisutatx. 
\num{10} (\pArxparx) (obabxra adhikaqta ruju, muderx, keYbareha, \mo vugaLiMda) (\kanmu) daqDhiVkarisida; parxmANa mADida: \eng{under one's hand and seal} tAneV rujumADi muderx otitxda. 
\num{11} saradiya parxkAra inonxbabx (isipxVTu) ATagArananunx -- anusarisutatx, hiMbAlisutatx. 
\num{12} keLage; vagaRdalilx; guMpinalilx; vagiRVkaraNagoMDu: \eng{that book goes under biology} A pusatxka jiVvavijAcnxnada keLage barutatxde. \eng{goes under many names} halavAru hesarugaLalilx ide. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\hyperdef{U}{under(1)pagu}{} 
\bnum
\num{1} \eng{take under one's} \hyperref{kandict_w.pdf}{W}{wing(1) pagu(8)}{$^1$wing}. 
\num{2} \eng{under a} \hyperref{kandict_c.pdf}{C}{cloud(1) nuga(2)}{$^1$cloud}. 
\num{3} \eng{under} \hyperref{kandict_a.pdf}{A}{age(1) pagu(5)}{$^1$age}. 
\num{4} \eng{under} \hyperref{kandict_a.pdf}{A}{arm(2) nuga(4)}{$^2$arms}. 
\num{5} \eng{under} \hyperref{kandict_c.pdf}{C}{convas(1) pagu(1)}{$^1$canvas}. 
\num{6} \eng{under} \hyperref{kandict_c.pdf}{C}{control(1) pagu(3)}{$^1$control}. 
\num{7}\eng{under} \hyperref{kandict_c.pdf}{C}{cover(2) pagu(3)}{$^2$cover}. 
\num{8} \eng{under} \hyperref{kandict_f.pdf}{F}{fire(1) pagu(9)}{$^1$fire}. 
\num{9} \eng{under} \hyperref{kandict_f.pdf}{F}{foot(1) nuga(37)}{$^1$foot}. 
\num{10} \eng{under one's} \hyperref{kandict_a.pdf}{A}{arm(1) pagu(2)}{$^1$arm}. 
\num{11} \eng{under one's} \hyperref{kandict_b.pdf}{B}{belt(1) nuga(4)}{$^1$belt}. 
\num{12} \eng{under one's} \hyperref{kandict_b.pdf}{B}{breathpagu(13)}{breath}. 
\num{13} \eng{under one's} \hyperref{kandict_h.pdf}{H}{hat(1) nuga(15)}{$^1$hat}. 
\num{14} \eng{under} \hyperref{kandict_h.pdf}{H}{hatch(1) pagu(2)}{$^1$hatches}. 
\num{15}\eng{under person's} \hyperref{kandict_n.pdf}{N}{nose(1) pagu(29)}{$^1$nose}. 
\num{16} \eng{under separate cover} beVroMdu lakoVTe oLage. 
\num{17}\eng{under the} \hyperref{kandict_c.pdf}{C}{counter(1) nuga(2)}{$^1$counter}. 
\num{18} \eng{under the} \hyperref{kandict_l.pdf}{L}{lee pagu}{lee} \eng{of.} 
\num{19}\eng{under the} \hyperref{kandict_r.pdf}{R}{rose(1) pagu(13)}{$^1$rose}. 
\num{20} \eng{under the sun} parxpaMcadalilx; loVkada yAvudeV kaDe: \eng{there is nothing new under the sun} parxpaMcadalilx hosadeMbudeVnU ilalx. 
\num{21} \eng{under the} \hyperref{kandict_w.pdf}{W}{weather(1) pagu(7)}{$^1$weather}. 
\num{22} \eng{under water} niVroLage; niVrina aDiyalilx. 
\num{23} \eng{under way} calisutAtx; sAgutAtx; muMduvareyutAtx; parxgatiyalilx; aBivaqdidhxyAgutatx. 
\enum
\emng
\eentry

\bentry
\word[under(2)]{under} 
\pron{aMDarf} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (sAthxna yA sithxtiyalilx) keLagaDege; oLagaDe; aDiyalilx; taLakekx: \eng{kept him under} avananunx keLagaDe (keLagina sAthxnadalilx) irisalAgide. 
\num{2} (\AmA) arivaLida sithxtiyalilx; parxjAcnxhiVna avasethxyalilx: \eng{put him under for the operation} shasatxrXcikitesxgAgi avananunx arivaLikeya sithxtiyalilxDu. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \hyperref{kandict_b.pdf}{B}{bring nuga(26)}{bring} \eng{under.} 
\num{2} \hyperref{kandict_k.pdf}{K}{keep(1) nuga(37)}{$^1$keep} \eng{under.} 
\num{3} \hyperref{kandict_k.pdf}{K}{knuckle(2) pagu(1)}{$^2$knuckle} \eng{under.} 
\num{4} \eng{the ship went under} haDagu muLugitu. 
\num{5} \eng{can you stay under (the water) for 2 hours} niVnu niVrinaDiyalilx eraDu gaMTe irabalelxyA? 
\enum
\emng
\eentry

\bentry
\word[under(3)]{under} 
\pron{aMDarf} 
\gl{\gu}
\expl{}
\bmng
 keLa; keLagina; keLagaDeya: \eng{the under jaw} keLadavaDe. \eng{the under servants} keLa ALugaLu. 
\emng
\eentry

\bentry
\word{under-} 
\pron{aMDarf-}
\gl{\pUparx}
\expl{}
\bmng
\bnum
\num{1} keLage; aDiyalilx: \eng{under-ground} BUmiya aDiyalilxna, keLagina. 
\num{2} -- kikxMta kaDameyAgi, keLage: \eng{under-sell} keLagina belege mAru. 
\num{3} kaDameyAgi; apUNaRvAgi: \eng{under-cook} kaDame beVyisu. \eng{under-developed} pUtiR aBivaqdidhx hoMdada; hiMduLida. 
\num{4} -- kekx keLagiruva; oLagaDeyiruva; adhiVnavAda: \eng{under-lip} keLaduTi. \eng{under-soil} keLamaNuNx; taLamaNuNx. \eng{under-secretary} adhiVna kAyaRdashiR. 
\enum
\emng
\eentry

\bentry
\word{underachieve} 
\pron{aMDaraciVvf}
\gl{\akirx}
\expl{}
\bmng
 (\kanmu\ shikaSxNa, vAyxsaMga, \mo valilx) kaDime sAdhisu; hiMde biVLu; niriVkaSxNegiMta kaDame maTaTx sAdhisu. 
\emng
\eentry

\bentry
\word{underachievement} 
\pron{aMDaraciVvfmaMTf}
\gl{\nA}
\expl{}
\bmng
 (\kanmu\ shikaSxNa \mo valilx) kaDime sAdhane; hiMde biVLuvike. 
\emng
\eentry

\bentry
\word{underachiever} 
\pron{aMDaraciVvarf}
\gl{\nA}
\expl{}
\bmng
 kaDame sAdhisuvavanu; hiMdebidadxvanu. 
\emng
\eentry

\bentry
\word{underact} 
\pron{aMDarfAYxkfTx}
\gl{\sakirx}
\expl{}
\bmng
 (pAtarxvanunx) asamapaRkavAgi ADu; asamathaRvAgi ADu, aBinayisu (\akirx\ saha). 
\emng
\eentry

\bentry
\word[underarm(1)]{underarm} 
\pron{aMDarfAmfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaMkuLalilx. 
\hypertarget{underarm(1)2}{} 
\num{2} (kirxVDe, \kanmu\ kirxkeTf) Bujada keLagina; toVLu Bujada maTaTxkikxMta keLagiruva. 
\num{3} toVLina aDiya. 
\enum
\emng
\eentry

\bentry
\word[underarm(2)]{underarm} 
\pron{aMDarfAmfR} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kaMkuLa keLage; kaMkuLalilx. 
\num{2} (kirxkeTf) Bujada keLakekx toVLu sarisi: \eng{underarm bowling} Bujada keLage toVLu taMdu ceMDu eseyuvudu. 
\num{3} toVLina keLage. 
\enum
\emng
\eentry

\bentry
\word{underbelly} 
\pron{aMDarfbeli}
\gl{\nA}
\expl{(\bava\ \eng{underbellies}).}
\bmng
\bnum
\num{1} keLahoTeTx; pArxNi \mo vugaLa deVhada keLaBAga. 
\num{2} dubaRlasAthxna; mamaRsathxLa; sulaBavAgi AkarxmaNa mADabahudAda BAga, parxdeVsha: \eng{the soft underbelly of Europe} yUroVpina dubaRla, mamaRsAthxna. 
\enum
\emng
\eentry

\bentry
\word[underbid(1)]{underbid} 
\pron{aMDarfbiDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kaDame savAlu; (matotxbabxnigiMta) kaDameyAgi kUgida bele. 
\num{2} (mArATadalilx, harAjinalilx) kaDame (beleyalilx) savAlu kUguvudu. 
\enum
\emng
\eentry

\bentry
\word[underbid(2)]{underbid} 
\pron{aMDarfbiDf}
\gl{\sakirx}
\expl{(\vakaq\ \eng{underbidding}, \BU\ matutx \BUkaq\ \eng{underbid}). }
\bmng
\bnum
\num{1} matotxbabxnigiMta -- kaDame savAlukUgu, kaDame bele keVLu. 
\num{2}(birxDfjx ATa) (keYyalilxruva elegaLa belegiMta) kaDame -- ATa, naMbaru heVLu; kaDame savAlu heVLu. 
\enum
\emng
\eentry

\bentry
\word{underbidder} 
\pron{aMDarfbiDarf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kaDame savAludAra; gariSaThx belege keLagina bele keVLuvavanu. 
\num{2} (birxDfjx ATa) (keYyalilxruva elegaLigiMta) kaDime ATa, naMbaru heVLuvavanu. 
\enum
\emng
\eentry

\bentry
\word{underbody} 
\pron{aMDarfbADi}
\gl{\nA}
\expl{}
\bmng
 (\bava\ \eng{underbodies}). (pArxNi, vAhana, \mo vugaLa) oDalina oLameY; oLa meVlemxY. 
\emng
\eentry

\bentry
\word{underbred} 
\pron{aMDarfberxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ashikiSxta; asaBayx; asaMsakxqqta. 
\num{2} tucaCx; kiVLu. 
\num{3} saMkara jAtiya; berake taLiya. 
\enum
\emng
\eentry

\bentry
\word{underbrush} 
\pron{aMDarfbarxSf}
\gl{\nA}
\expl{}
\bmng
 (\ame) keLakurucalu; keLasasayx; kADinalilx doDaDx maragaLa buDadalilx beLeyuva kurucalu sasayx. 
\emng
\eentry

\bentry
\word{undercarriage} 
\pron{aMDarfkAYxrijf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vAhanada -- aDigaTuTx, AdhAra paMjara, cwkaTuTx. 
\hypertarget{undercarriage(2)}{} 
\num{2} vimAnada aDigaTuTx, keLacwkaTuTx yA AdhAra paMjara; beVDavAdAga oLakekx eLedukoLaLxbalalx, vimAna nelada meVle niMtiruvAga yA iLiyuvAga adara AGAtavanunx taDedukoLaLxbalalx, cwkaTuTx. 
\enum
\emng
\eentry

\bentry
\word{undercart} 
\pron{aMDarfkATfR}
\gl{\nA}
\expl{}
\bmng
 (\birx) (\AmA) = \hyperlink{undercarriage(2)}{undercarriage \((2)\)}. 
\emng
\eentry

\bentry
\word{undercharge} 
\pron{aMDarfcAjfR}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vasutx yA vayxkitxge) (nAyxyavAdudakikxMta) kaDame bele, dara -- hAku, tegeduko. 
\num{2} (baMdUku, viduyxtfkoVsha, \mo vugaLa \vi) kaDame cAjfR mADu; beVkAdudakikxMta kaDame viduyxdAveVsha mADu. 
\enum
\emng
\eentry

\bentry
\word{underclass} 
\pron{aMDarfkAlxsf}
\gl{\nA}
\expl{}
\bmng
 keLa (sAmAjika) vagaR. 
\emng
\eentry

\bentry
\word{underclay} 
\pron{aMDarfkelxV}
\gl{\nA}
\expl{}
\bmng
 aDijeVDi padara; kalilxdadxla satxrada keLagina jeVDimaNiNxna padara. 
\emng
\eentry

\bentry
\word{undercliff} 
\pron{aMDarfkilxphf}
\gl{\nA}
\expl{}
\bmng
 keLaparxpAta; BUpAta yA BUjaritadiMduMTAda, parxpAtada keLagina jagali, dibabx, jagati. 
\emng
\eentry

\bentry
\word{underclothes} 
\pron{aMDarfkolxVdf\char144}
\gl{\nA}
\expl{}
\bmng
 (\bava) oLuDupu; itara uDupugaLa oLagaDe toDuva, \kanmu\ camaRkekx taguliruva, uDupu. 
\emng
\eentry

\bentry
\word{underclothing} 
\pron{aMDarfkolxVdiMgf}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{underclothes}{underclothes}. 
\emng
\eentry

\bentry
\word{undercoat} 
\pron{aMDarfkoVTf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oLaMgi; horagina aMgiya oLage dharisida aMgi. 
\num{2} pArxNigaLa deVhada keLapadarada kUdalu, tupupxLa. 
\num{3} hora meYge baLiyuva baNaNxda aMtima leVpanakikxMta muMce baLiyuva -- aDileVpa, oLaleVpa; modala (baNaNxda) toDeta; parxthama leVpa. 
\num{4} hiVge baLida baNaNx \mo vu. 
\enum
\emng
\eentry

\bentry
\word{undercoating} 
\pron{aMDarfkoVTiMgf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (pArxNigaLa) deVhada keLapadarada kUdalu. 
\num{2} oLaleVpa; horagaDe baLiyuva baNaNxda baLitada aDi leVpa. 
\enum
\emng
\eentry

\bentry
\word{undercover} 
\pron{aMDarfkavarf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oLagoLagina; rahasayxvAda; mucucxmareya; gupatx; guTiTxna. 
\num{2} beVhugArikeya; gUDhacAra kaqtayx naDesuva (\kanmu\ janara madheyx ODADutitxdudx avariMda saMgarxhisida mAhitigaLiMdaleV avara meVle gupatxcayeR naDesuva): \eng{an undercover agent} gUDhacAra kAyaRBAri. 
\enum
\emng
\eentry

\bentry
\word{undercroft} 
\pron{aMDarfkArxphfTx} 
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_c.pdf}{C}{crypt(1)}{crypt (1, 2)}. 
\emng
\eentry

\bentry
\word{undercurrent} 
\pron{aMDarfkaraMTf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oLaparxvAha; aMtaHparxvAha; aDiharivu. 
\num{2} aMtaHparxvaqtitx; gupatxvAhini; oLagoLage calisuva, (\kanmu) viroVdhi BAva, parxBAva, caTuvaTike, parxvaqtitx, \mo vu. 
\enum
\emng
\eentry

\bentry
\word[undercut(1)]{undercut} 
\pron{aMDarfkaTf}
\gl{\sakirx}
\expl{}
\bmng
 (\vakaq\ \eng{undercutting}; \BU\ matux \BUkaq\ adeV). 
\bnum
\num{1} (ketatxneyalilx mAdari edudxkANuvaMte, ububxcitarxvAguvaMte) hinenxle padAthaRvanunx ketitx (tegedu)hAku. 
\num{2} (gAlfphx)(etatxrakekx hoVguvaMteyU, nelada meVle hecucx dUra uruLadaMteyU ceMDanunx) madhayxkikxMta keLage hoDe. 
\num{3} (matotxbabxnigiMta) kaDame belege mAru yA kaDame kUlige kelasa mADu. 
\num{4} oMdu vasutxvina -- taLa katatxrisu, keLagina yA aDiya BAgavanunx katatxrisi hAku. 
\num{5} asithxragoLisu; daqDhavAgiradaMte, BadarxvAgiradaMte mADu. 
\enum
\emng
\eentry

\bentry
\word[undercut(2)]{undercut} 
\pron{aMDarfkaTf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\ame) (maravanunx uruLisuvAga adu yAva kaDe uruLabeVkoV A kaDe mADida) kacucx; koyatxgere. 
\num{2} (\birx) danada pakekxmAMsada keLaBAga. 
\num{3} aDi terapu; taLAvakAsha; yAvudeV oMdara keLaBAgadiMda darxvayxvanunx tegeduhAkuvudariMda yA keLaBAgada darxvayx ilalxdiruvudariMda uMTAguva terapu. 
\enum
\emng
\eentry

\bentry
\word{underdeveloped} 
\pron{aMDarfDivelapfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} apakavx; pUtiR aBivaqdidhxyAgada; cenAnxgi vikAsagoLaLxda. 
\num{2} (\CA) sAkaSuTx suPxTagoLisada; masukAda. 
\num{3} (rASaTxrX \mo vugaLa \vi) hiMduLida; anaBivaqdadhx; aMtasathxvAda tananx AthiRka sAmathaRyxda maTaTxkikxMta keLagiruva. 
\enum
\emng
\eentry

\bentry
\word{underdevelopment} 
\pron{aMDarfDivelapfmaMTf}
\gl{\nA}
\expl{}
\bmng
 anaBivaqdadhx sithxti; hiMduLidiruvike. 
\emng
\eentry

\bentry
\word{underdog} 
\pron{aMDarfDAgf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (hoVrATa, jagaLa, \mo vugaLalilx) dubaRla vayxkitx; kiVLunAyi; soVlutitxruva nAyi yA (\sA) vayxkitx. 
\num{2} kiVLALu; parAdhiVna yA kiVLu sithxtiyalilxruva vayxkitx. 
\enum
\emng
\eentry

\bentry
\word{underdone} 
\pron{aMDarfDanf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sariyAgi, cenAnxgi mADada. 
\num{2} (AhArada \vi) kaDame beVyisida; sAkaSuTx beVyisada. 
\enum
\emng
\eentry

\bentry
\word{underdress} 
\pron{aMDarfDerxsf}
\gl{\sakirx}
\expl{}
\bmng
 tiVrA sAdA yA ucitavAdudakikxMta kaDame uDupu hAkiko, hAku(\akirx\ saha). 
\emng
\eentry

\bentry
\word{underemphasis} 
\pron{aMDarfeMphasisf}
\gl{\nA}
\expl{(\bava\ \eng{underemphases} \ucAcx-- aMDarfeMphasiVsfZ). }
\bmng
 kaDame otutxhAkuvudu; sAkaSuTx otitx heVLadiruvudu; arepuSiTx. 
\emng
\eentry

\bentry
\word{underemphasise} 
\pron{aMDarfeMphaseYsfZ}
\gl{\sakirx}
\expl{}
\bmng
= \hyperlink{underemphasize}{underemphasize}. 
\emng
\eentry

\bentry
\word{underemphasize} 
\pron{aMDarfeMphaseYsfZ}
\gl{\akirx}
\expl{}
\bmng
 kaDame otutxhAku; sAkaSuTx otitx heVLadiru. 
\emng
\eentry

\bentry
\word{underemployed} 
\pron{aMDariMpAlxyfDx} 
\gl{\gu}
\expl{}
\bmng
 asaMpUNoRVdoyxVgada; kaDame udoyxVgada; obabxna shakitxsAmathaRyxgaLanunx sariyAgi yA pUtiRyAgi upayoVgisikoLaLxda -- udayxmadalilxruva, udoyxVgadalilxruva. 
\emng
\eentry

\bentry
\word{underemployment} 
\pron{aMDariMpAlxyfmaMTf}
\gl{\nA}
\bmng
 asaMpUNoRVdoyxVga; kaDame udoyxVga; obabxna shakitxsAmathaRyxgaLu upayoVgakekx bAradaMtha udoyxVga. 
\emng
\eentry

\bentry
\word[underestimate(1)]{underestimate} 
\pron{aMDarfesiTxmeVTf}
\gl{\sakirx}
\expl{}
\bmng
 (vAsatxvavAdudakikxMta yA nAyxyavAdudakikxMta) kaDame -- belekaTuTx, aMdAjumADu; kiVLaMdAju mADu; kaDameyAgi eNisu. 
\emng
\eentry

\bentry
\word[underestimate(2)]{underestimate} 
\pron{aMDarfesiTxmeVTf}
\gl{\nA}
\expl{}
\bmng
 kiVLaMdAju; nAyxyavAdudakikxMta yA vAsatxvikavAgiruvudakikxMta kaDame aMdAju yA hAge kaTiTxda kaDame bele. 
\emng
\eentry

\bentry
\word{underestimation} 
\pron{aMDarfesiTxmeVSanf}
\gl{\nA}
\expl{}
\bmng
 kiVLaMdAju mADuvudu; nAyxyavAdudakikxMta yA vAsatxvikavAgiruvudakikxMta kaDame aMdAju mADuvudu yA hAge kaDame aMdAju mADidudx. 
\emng
\eentry

\bentry
\word{underexpose} 
\pron{aMDarikfsxpoVsfZ}
\gl{\sakirx}
\expl{}
\bmng
 (\CA) kaDime oDuDx; philamxnunx tuMba kaDime samaya beLakige oDuDx yA kaDime beLakige oDuDx; sAkaSuTx kAla yA beLakige oDaDxdiru. 
\emng
\eentry

\bentry
\word{underexposure} 
\pron{aMDarikfsxpoVSaZrf}
\gl{\nA}
\expl{}
\bmng
 (\CA) kaDime kAla beLakige oDuDxvike. 
\emng
\eentry

\bentry
\word{underfed} 
\pron{aMDarfpheDf}
\gl{\gu}
\expl{}
\bmng
 arehoTeTxya; sAkaSuTx AhAra koDada. 
\emng
\eentry

\bentry
\word{underfelt} 
\pron{aMDarfphelfTx}
\gl{\nA}
\expl{}
\bmng
 jamaKAnada keLage hAkalu beVkAda aDipheluTx(baTeTx). 
\emng
\eentry

\bentry
\word{underfloor} 
\pron{aMDarfpholxVrf}
\gl{\nA}
\expl{}
\bmng
 oLanela yA keLanela; (\kanmu\ maneya oLagaDe shAKa pUreYsuva vayxvasethxyiruvalilx) nelada keLagina inonxMdu satxra, padara. 
\emng
\eentry

\bentry
\word{underflow} 
\pron{aMDarfpholxV}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{undercurrent}{undercurrent}. 
\emng
\eentry

\bentry
\word{underfoot} 
\pron{aMDarfphuTf}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} pAdadaDiyalilx; kAlina keLagaDe. 
\num{2} nelada meVle; neladalilx. 
\num{3} parAdhiVnateyalilx; (inonxbabxrige) aDiyALAgiruva sithxtiyalilx. 
\num{4} (naDeyuvavarige, aBivaqdidhxge) taDeyAgi; aDacaNeyAgi; ananukUlavAgi. 
\enum
\emng
\eentry

\bentry
\word{undergarment} 
\pron{aMDarfgAmaRMTf}
\gl{\nA}
\expl{}
\bmng
 (itara uDupina oLage toDuva) oLa -- uDupu, baTeTx. 
\emng
\eentry

\bentry
\word{undergird} 
\pron{aMDarfgaDfR}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} keLage BadarxpaDisu; (hagagx \mo vanunx sutatxlU kaTiTx) keLage Badarx mADu. 
\num{2} (\rUpa) balapaDisu; beMbala koDu; AsareyAgu; otAtxse niVDu. 
\enum
\emng
\eentry

\bentry
\word[underglaze(1)]{underglaze} 
\pron{aMDarfgelxVsfZ}
\gl{\gu}
\expl{}
\bmng
 (piMgANi \mo\ vasutxgaLa meVle vaNaRcitarx bareyuvalilx) merugu koDuva modalu baLida yA leVpisida; modala baLitada; parxthama leVpada. 
\emng
\eentry

\bentry
\word[underglaze(2)]{underglaze} 
\pron{aMDarfgelxVsfZ}
\gl{\nA}
\expl{}
\bmng
 (piMgANi \mo\ vasutxgaLa meVle vaNaRcitarx bareyuvalilx) merugu koDuva modalu baLiyuva baNaNx; parxthama leVpada baNaNx. 
\emng
\eentry

\bentry
\word{undergo} 
\pron{aMDarfgoV}
\gl{\sakirx}
\expl{(\parxpu\ \Eva\ vataRmAnakAla \eng{undergoes}; \BU\ \eng{underwent}; \BUkaq\ \eng{undergone}).}
\bmng
(\kanmu\ daqDhamanasisxniMda) anuBavisu; IDAgu; oLagAgu; sahisu; paDu: \eng{has undergone many trials} aneVka kelxVshagaLanunx sahisidAdxne, anuBavisidAdxne. \eng{undergo an operation} shasatxrXcikitesxge oLagAgu. 
\emng
\eentry

\bentry
\word{undergrad} 
\pron{aMDarfgArxYxDf}
\gl{\nA}
\expl{}
\bmng
 (\AmA) = \hyperlink{undergraduate}{undergraduate}. 
\emng
\eentry

\bentry
\word{undergraduate} 
\pron{aMDarfgArxYxDuyxaTf}
\gl{\nA}
\expl{}
\bmng
 padavi shikaSxNada vidAyxthiR; sAnxtakapUvaR vidAyxthiR; vishavxvidAyxnilayadalilx Odutitxdudx inUnx modalaneya padavi paDeyadiruva vidAyxthiR(ni). 
\emng
\eentry

\bentry
\word[underground(1)]{underground} 
\pron{aMDarfgwrxMDf}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} BUmiya keLage; neladaDiyalilx: \eng{miners working underground} BUmiya keLage, neladaDi -- kelasa mADutitxruva gaNi kelasagAraru. 
\num{2} guTATxgi; goVpayxdalilx; rahasayxvAgi: \eng{go underground} bacicxTuTxko; mareyAgu; kANadAgu. 
\enum
\emng
\eentry

\bentry
\word[underground(2)]{underground} 
\pron{aMDarfgwrxMDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BUmiya keLagiruva; neladoLagiruva. 
\num{2} goVpayxvAda; rahasayxda; guTATxda; mareyalilxna; (\kanmu\ ALutitxruva parxButavxvanunx buDameVlu mADalu) rahasayxvAgi kelasa mADuva: \eng{underground movement} rahasayx caLuvaLi. 
\num{3} asAMparxdAyika; pArxyoVgikavAda: \eng{underground press} pArxyoVgika mudarxNAlaya. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
 \eng{underground workers} rahasayx caLuvaLiya sadasayxru. 
\emng
\eentry

\bentry
\word[underground(3)]{underground} 
\pron{aMDarfgwrxMDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} BUgata, neladaDi -- reYlu; BUmiya keLage saMcarisuva reYlu: \eng{travel by underground} neladaDiya reYlinalilx parxyANa mADu. 
\num{2} (\kanmu\ sAthxpita vayxvasethxyanunx buDameVlu mADuva udedxVsha hoMdiruva) gupatx taMDa yA rahasayx caTuvaTike. 
\enum
\emng
\eentry

\bentry
\word[underground(4)]{underground} 
\pron{aMDarfgwrxMDf}
\gl{\sakirx}
\expl{}
\bmng
 BUmiya oLage, neladaDiyalilx (keVbalulxgaLanunx yA horajigaLanunx) hAku, eLe. 
\emng
\eentry

\bentry
\word{undergrowth} 
\pron{aMDarfgorxVtf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{underbrush}{underbrush}. 
\num{2} (\kanmu\ doDaDxmaragaLa aDiyalilx beLeyuva) daTaTx kurucalu, giDagaMTe, \mo vu; aDikurucalu. 
\enum
\emng
\eentry

\bentry
\word[underhand(1)]{underhand} 
\pron{aMDarfhAYxMDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} guTATxda; mareyalilxna; rahasayxda. 
\num{2} moVsada; kaqtirxmada. 
\num{3} oLasaMcina. 
\num{4} (kirxkeTf) =\hyperlink{underarm(1)2}{$^1$underarm \((2)\)}.  
\enum
\emng
\eentry

\bentry
\word[underhand(2)]{underhand} 
\pron{aMDarfhAYxMDf}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} guTATxgi; mareyalilx; rahasayxvAgi. 
\num{2} moVsadiMda; kaqtirxmadiMda. 
\num{3} oLasaMcinaMte. 
\enum
\emng
\eentry

\bentry
\word[underhanded(1)]{underhanded} 
\pron{aMDarfhAYxMDiDf}
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{underhand(1)}{$^1$underhand}. 
\emng
\eentry

\bentry
\word[underhanded(2)]{underhanded} 
\pron{aMDarfhAYxMDiDf}
\gl{\kirxvi}
\expl{}
\bmng
 = \hyperlink{underhand(2)}{$^2$underhand}. 
\emng
\eentry

\bentry
\word{underhung} 
\pron{aMDarfhaMgf}
\gl{\gu}
\expl{}
\bmng
 (keLadavaDeya \vi) 
\bnum
\num{1} meVludavaDegiMta muMcAcikoMDiruva. 
\num{2} muMcAcida keLadavaDeyuLaLx. 
\enum
\emng
\eentry

\bentry
\word[underlay(1)]{underlay} 
\pron{aMDarfleV}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{underlaid}). }
\bmng
 (oMdara) keLage, aDiyalilx -- iDu; (etatxrisalu yA UrekoDalu) oMdara aDiyalilx, keLagaDe EnAdarU irisu. 
\emng
\eentry

\bentry
\word[underlay(2)]{underlay} 
\pron{aMDarfleV}
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (oMdara) aDiyalilxTaTx vasutx \mo vu. 
\alnum{b} (\kanmu) aDikAgada; acicxna moLegaLanunx etatxrisalu keLagiTaTx kAgada. 
\eanum
\numie
\num{2} (jamaKAna, cApe, \mo vugaLa keLage) rakaSxNegAgi yA AdhAravAgi hAsida hALe \mo vu. 
\enum
\emng
\eentry

\bentry
\word[underlay(3)]{underlay} 
\pron{aMDarfleV}
\gl{\kirx}
\expl{}
\bmng
 \eng{underlie} padada BUta rUpa. 
\emng
\eentry

\bentry
\word[underlease(1)]{underlease} 
\pron{aMDarfliVsf}
\gl{\nA}
\expl{}
\bmng
 oLagutitxge; oLaBoVgayx; geVNi tegedukoMDiruvudanunx beVrobabxnige gutitxge koDuvudu. 
\emng
\eentry

\bentry
\word[underlease(2)]{underlease} 
\pron{aMDarfliVsf}
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{underlet}{underlet}. 
\emng
\eentry

\bentry
\word{underlet} 
\pron{aMDarfleTf}
\gl{\sakirx}
\expl{(\vakaq\ \eng{underletting}; \BU\ matutx \BUkaq\ adeV).}
\bmng
\bnum
\num{1} oLagutitxge koDu; oLabADigege koDu. 
\num{2} (nAyxyavAdudakikxMta) kaDame bADigege koDu. 
\enum
\emng
\eentry

\bentry
\word{underlie} 
\pron{aMDarfleY}
\gl{\sakirx}
\expl{(\vakaq\ \eng{underlying}, \BU\ \eng{underlay;} \BUkaq\ \eng{underlain}).}
\bmng
\bnum
\num{1} (satxra \mo vugaLa) keLage iru; aDiyalilx nelasiru (\akirx\ saha). 
\num{2} (tatatxvX, takaR, \mo vugaLa \vi) (sidAdhxMta, kAnUnu, naDevaLike, \mo vugaLige) AdhAravAgiru; mUlAdhAravAgiru. 
\num{3} (yAvudeV bAhayxtoVkeRya, kANuva aMshada) aDiyalilxru; keLagiru; AdhAraBUtavAgiru. 
\enum
\emng
\eentry

\bentry
\word[underline(1)]{underline} 
\pron{aMDarfleYnf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (otitxheVLuva udedxVshadiMda yA OremudArxkaSxragaLirabeVkeMdu sUcisalu) (padada) keLage gere eLe; aDigere hAku. 
\num{2} otitx heVLu; (mAtina meVle) otutx hAku. 
\enum
\emng
\eentry

\bentry
\word[underline(2)]{underline} 
\pron{aMDarfleYnf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aDigere; otitxheVLuva udedxVshadiMda mAtu \mo vugaLa keLage eLeda gere. 
\num{2} (citarx \mo vugaLa keLagaDe vivaraNegAgi koTaTx) aDileVKa; taLabaraha. 
\enum
\emng
\eentry

\bentry
\word{underlinen} 
\pron{aMDarflininf}
\gl{\nA}
\expl{}
\bmng
 linanf baTeTxyiMda mADida oLauDupu, oLabaTeTx. 
\emng
\eentry

\bentry
\word{underling} 
\pron{aMDarfliMgf}
\gl{\nA}
\expl{}
\bmng
 (\sA\ \hiV) keYkeLaginavanu; tAbeVdAra. 
\emng
\eentry

\bentry
\word{underlying} 
\pron{aMDarfleYiMgf}
\gl{\gu}
\expl{}
\bmng
 (tatatxvX, kAraNa, \mo vugaLa \vi) sidAdhxMta, shAsana, naDavaLike, \mo vugaLige -- AdhAravAgiruva, mUlavAgiruva. 
\emng
\eentry

\bentry
\word{undermanned} 
\pron{aMDarfmAYxnfDx}
\gl{\gu}
\expl{}
\bmng
 (haDagu \mo vugaLa \vi) sariyAgi naDesalu takakxSuTx sibabxMdi, kelasagAraru ilalxda; kaDame kelasagArariruva; kelasagArara korateyiruva. 
\emng
\eentry

\bentry
\word{undermentioned} 
\pron{aMDarfmenaSxnfDx}
\gl{\gu}
\expl{}
\bmng
(pusatxka \mo vugaLalilx) (I) keLage sUcisida; muMde heVLuva; keLage kANisida. 
\emng
\eentry

\bentry
\word{undermine} 
\pron{aMDarfmeYnf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} keLagaDe (gaNi yA suraMga) toVDu. 
\num{2} taLaBAgavanunx, aDipAyavanunx -- savesi shithilagoLisu, koreduhAku: \eng{rivers undermine their banks} nadigaLu daMDegaLanunx koredu hAkutatxve. 
\num{3} (vayxkitx, kiVtiR, parxBAva, \mo vanunx) oLagoLage, gupatxvAgi -- hALumADu; buDakekx niVru tidudx. 
\num{4} (AroVgayx \mo vanunx) oLagoLage, agoVcaravAgi -- keDisu, kugigxsu, hALumADu: \eng{his health was undermined by drink} kuDitadiMda avana AroVgayx oLagoLageV hALAyitu. 
\enum
\emng
\eentry

\bentry
\word{underminer} 
\pron{aMDarfmeYnarf}
\gl{\nA}
\expl{}
\bmng
 gupatxvinAshaka; oLagoLageV hALu mADuva vayxkitx yA vasutx. 
\emng
\eentry

\bentry
\word[underneath(1)]{underneath} 
\pron{aMDarfniVtf}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} taLage; aDiyalilx. 
\num{2} oLagaDe; oLage. 
\enum
\emng
\eentry

\bentry
\word[underneath(2)]{underneath} 
\pron{aMDarfniVtf}
\gl{\upa}
\bmng
\bnum
\num{1} keLagaDe; aDiyalilx; keLaBAgadalilx. 
\num{2} oLage; oLagaDe; oLaBAgadalilx. 
\enum
\emng
\eentry

\bentry
\word[underneath(3)]{underneath} 
\pron{aMDarfniVtf}
\gl{\gu}
\expl{}
\bmng
 (meVlemxY BAga \mo vugaLa \vi) keLa; keLagina; aDiya. 
\emng
\eentry

\bentry
\word[underneath(4)]{underneath} 
\pron{aMDarfniVtf}
\gl{\nA}
\expl{}
\bmng
 keLagina meVlemxY; keLagina BAga; aDi. 
\emng
\eentry

\bentry
\word{undernourished} 
\pron{aMDarfnariSfTx}
\gl{\gu}
\expl{}
\bmng
 sariyAgi poVSisada; poVSakAMshagaLa korateyiruva; puSiTxyilalxda. 
\emng
\eentry

\bentry
\word{undernourishment} 
\pron{aMDarfnariSfmaMTf}
\gl{\nA}
\expl{}
\bmng
 poVSaNe korate; poVSakAMshagaLa korateyiruvike; sariyAgi poVSisadiruvike. 
\emng
\eentry

\bentry
\word{underpaid} 
\pron{aMDarfpeVDf}
\gl{\kirx}
\expl{}
\bmng
 \eng{underpay} padada \BU\ matutx \BUkaq\ rUpa. 
\emng
\eentry

\bentry
\word{underpants} 
\pron{aMDarfpAYxMTfsx}
\gl{\nA}
\expl{}
\bmng
 (\bava) (deVhada keLaBAga matutx kAlina savxlapxBAgavanunx mucucxva, \kanmu\ gaMDasara) oLauDupu. 
\emng
\eentry

\bentry
\word{underpart} 
\pron{aMDarfpATfR}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ pArxNiya) deVhada keLagina BAga. 
\num{2} (nATaka \mo vugaLalilx) gwNa, aparxdhAna -- pAtarx. 
\enum
\emng
\eentry

\bentry
\word{underpass} 
\pron{aMDarfpAsf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} keLarasetx; aDihAdi; inonxMdara keLage, aDiyalilx hoVguva rasetx \mo vu. 
\num{2} I riVti hAyuvudu. 
\enum
\emng
\eentry

\bentry
\word{underpay} 
\pron{aMDarfpeV}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{underpaid}).}
\bmng
\bnum
\num{1} (vayxkitxge) kaDime saMbaLa koDu. 
\num{2} (oMdu vasutxvige) kaDime bele koDu. 
\enum
\emng
\eentry

\bentry
\word{underpayment} 
\pron{aMDarfpeVmaMTf}
\gl{\nA}
\expl{}
\bmng
kaDime (saMbaLa yA bele) -- pAvati, koDuvike. 
\emng
\eentry

\bentry
\word{underpin} 
\pron{aMDarfpinf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{underpinned;} \vakaq\ \eng{under-pinning}).  }
\bmng
\bnum
\num{1} (goVDe, cAcukaTeTx, \mo vugaLige) balavAda gAregacicxna aDipAyahAku. 
\num{2} beMbalakoDu; AdhAravAgu; balapaDisu. 
\enum
\emng
\eentry

\bentry
\word{underplay} 
\pron{aMDarfpelxV}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} pArxmuKayxvanunx, mahatatxvXvanunx -- tagigxsu, kaDime mADu. 
\numi{2} (nATaka, raMgaBUmi) 
\banum
\alnum{a} udedxVshapUvaRka saMyamadiMda aBinayisu; vipariVta niyaMtarxNadiMda, hatoVTiyiMda -- pAtarxvahisu. 
\alnum{b} sAkaSuTx humamxsisxrade aBinayisu; asamapaRkavAgi, ataqpitxkaravAgi aBinayisu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{underplot} 
\pron{aMDarfpAlxTf}
\gl{\nA}
\expl{}
\bmng
 (nATaka \mo vugaLalilx) upavasutx; gwNavasutx. 
\emng
\eentry

\bentry
\word{underprice} 
\pron{aMDarfperxYsf}
\gl{\sakirx}
\expl{}
\bmng
 (sAmAnayxvAdudakikxMta yA nAyxyavAdudakikxMta) kaDimebele hAku; bele tagigxsu. 
\emng
\eentry

\bentry
\word{underprivileged} 
\pron{aMDarfpirxvilijfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaDame savalatitxna; beVreyavarigiMta kaDame hakukx, swkayaR, rakaSxNe, parxyoVjana, \mo vugaLanunx hoMdida. 
\num{2} sAmAnayx jiVvanAnukUlagaLilalxda yA hakukxgaLilalxda; oMdu samAjadalilx sAmAnayx jiVvanamaTaTx, hakukx \mo vugaLanunx anuBavisalAgada, hoMdadiruva. 
\enum
\emng
\eentry

\bentry
\word{underproduction} 
\pron{aMDarfparxDakfSa(Sf)nf}
\gl{\nA}
\expl{}
\bmng
 kaDame utApxdane; vADikegiMta, beVDikegiMta kaDame tayArike. 
\emng
\eentry

\bentry
\word{underproof} 
\pron{aMDarfpUrxphf}
\gl{\gu}
\expl{}
\bmng
 parxmANita maTaTxkikxMta keLagaNa; `pUrxphf sipxriTf' giMta kaDime AlakxhAlf uLaLx. 
\emng
\eentry

\bentry
\word{underprop} 
\pron{aMDarfpArxpf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{underpropped;} \vakaq\ \eng{underpropping}).}
\bmng
\bnum
\num{1} (goVDe \mo vugaLige) keLa AdhAra koDu; keLatAMgu, aDi Ure -- koDu. 
\num{2} utetxVjana, sahAya, beMbala, otAtxse -- koDu. 
\enum
\emng
\eentry

\bentry
\word{underquote} 
\pron{aMDarfkovxVTf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (matotxbabxrigiMta) kaDame bele heVLu. 
\num{2} (saraku \mo vugaLige) matotxbabxrigiMta kaDame bele heVLu. 
\enum
\emng
\eentry

\bentry
\word{underrate} 
\pron{aMDareVTf}
\gl{\sakirx}
\expl{}
\bmng
 (obabxna yA oMdara \vi) oLeLxya aBipArxya taLeyadiru; sadaBipArxya hoMdadiru. 
\emng
\eentry

\bentry
\word[underscore(1)]{underscore} 
\pron{aMDarfsokxVrf}
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{underline(1)}{$^1$underline}. 
\emng
\eentry

\bentry
\word[underscore(2)]{underscore} 
\pron{aMDarfsokxVrf}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{underline(2)}{$^2$underline}. 
\emng
\eentry

\bentry
\word{undersea} 
\pron{aMDarfsiV}
\gl{\gu}
\expl{}
\bmng
 samudarxtaLada; sAgaradoLagina; samudarxda keLagiruva. 
\emng
\eentry

\bentry
\word{underseal} 
\pron{aMDarfsiVlf}
\gl{\sakirx}
\expl{}
\bmng
 (\kanmu\ moVTAruvAhana \mo vugaLalilx koLe, dhULu seVradaMte) keLaBAgavanunx mucicx BadarxpaDisu. 
\emng
\eentry

\bentry
\word{under-secretary} 
\pron{aMDarfsekarxTari}
\gl{\nA}
\expl{(\bava\ \eng{under-secretaries}).}
\bmng
 adhiVna kAyaRdashiR; \kanmu\ upa maMtirx yA hiriya sivilf seVveya nwkara. 
\emng
\eentry

\bentry
\word{undersell} 
\pron{aMDarfself}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (inonxbabx mArATagAranigiMta) kaDame belege mAru; agagxvAgi mAru. 
\num{2} (nijavAda belegiMta) kaDame belege mAru. 
\enum
\emng
\eentry

\bentry
\word[underset(1)]{underset} 
\pron{aMDarfseTf}
\gl{\sakirx}
\expl{(\vakaq\ \eng{undersetting;} \BU\ matutx \BUkaq\ adeV).}
\bmng
\bnum
\num{1} Ure, Asare -- koDu; Asare koTuTx balapaDisu. 
\num{2} (oMdara) keLage EnanAnxdarU iDu. 
\enum
\emng
\eentry

\bentry
\word[underset(2)]{underset} 
\pron{aMDarfseTf}
\gl{\nA}
\expl{}
\bmng
 (\nw) eduru oLaparxvAha; gALige yA meVlemxY niVrina parxvAhakekx edurAgi hariyuva oLaparxvAha. 
\emng
\eentry

\bentry
\word{undersexed} 
\pron{aMDarfsekfsxTX}
\gl{\gu}
\expl{}
\bmng
(sAmAnayxvAgi irabeVkAdudakikxMta) kaDame leYMgikAsakitx hoMdiruva; kaDame leYMgika parxvaqtitxyiruva. 
\emng
\eentry

\bentry
\word{under-sheriff} 
\pron{aMDarfSeriphf}
\gl{\nA}
\expl{}
\bmng
 upa Seriphf; Seriphfna keLagina adhikAri. 
\emng
\eentry

\bentry
\word{undershirt} 
\pron{aMDarfSaTfR}
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ \ame) oLaSaTuR; SaTiRna oLage dharisuva (baniVnu \mo) oLauDupu, vesuTx. 
\emng
\eentry

\bentry
\word{undershoot} 
\pron{aMDarfSUTf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vimAnada \vi) `ranfveV' giMta hiMdeyeV iLidubiDu; iLiyabeVkAda hAdi sikukxva modaleV iLidubiDu. 
\num{2} guri(eTukada hAge) tapipx hoDe yA keLage hoDi. 
\enum
\emng
\eentry


\bentry
\word{undershorts} 
\pron{aMDarfSATfsxR}
\gl{\nA}
\expl{}
\bmng
(\ame) giDaDxnAda oLacaDiDx. 
\emng
\eentry

\bentry
\word{undershot} 	
\pron{aMDarfSATf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (cakarxda \vi) keLage niVru taLuLxvudariMda sututxva; taLuLxniVriniMda calisuva.\imglink{undershot wheel:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.5cm {U_Pictures/undershot_wheel.jpg}}} 
\num{2} = \hyperlink{underhung}{underhung}. 
\enum
\emng
\eentry

\bentry
\word{undershrub} 
\pron{aMDarfSarxbf}
\gl{\nA}
\expl{}
\bmng
 saNaNx kurucalu; giDaDx kurucalu. 
\emng
\eentry

\bentry
\word{underside} 
\pron{aMDarfseYDf}
\gl{\nA}
\expl{}
\bmng
 keLa(gina) pakakx; keLameVlemxY; keLatala. 
\emng
\eentry

\bentry
\word{undersigned} 
\pron{aMDarfseYnfDx}
\gl{\gu}
\expl{}
\bmng
 keLage rujumADiruva: \eng{we, the undersigned, wish to state} I keLage, ruju mADiruva nAvu heVLabayasuvudeVneMdare ... ... . 
\emng
\eentry

\bentry
\word{undersized} 
\pron{aMDarfseYsZfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vADikeyalilxruvudakikxMta) kaDame aLateya; (mAmUlAgiruvudakikxMta) kaDame -- AkArada, gAtarxda. 
\num{2} gujujx; giDADxda; kubAjxkArada. 
\enum
\emng
\eentry

\bentry
\word{underskirt} 
\pron{aMDarfsakxTfR}
\gl{\nA}
\expl{}
\bmng
 oLa -- laMga, pAvuDe. 
\emng
\eentry

\bentry
\word{underslung} 
\pron{aMDarfsalxMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} meVle Asare koTiTxruva; (kusiyadaMte) meVlugaDe etitxhiDida; meVliniMda AdhAra koTiTxruva; meVlina AsareyiMda tUgADuva. 
\num{2} (moVTAru vAhana \mo vugaLa AdhAra cwkaTiTxna \vi) cakarxda acucx yA iciRyiMda keLakekx tUgADuva, joVluva. 
\enum
\emng
\eentry

\bentry
\word{undersold} 
\pron{aMDarfsoVlfDx}
\gl{\kirx}
\expl{}
\bmng
 \eng{undersell} kirxyApadada \BU\ matutx \BUkaq\ \rUpa 
\emng
\eentry

\bentry
\word{undersow} 
\pron{aMDarfsoV}
\gl{\sakirx}
\expl{}
\bmng
 (\BUkaq\ \eng{under sown}). 
\bnum
\num{1} (muMde beLeyuva peYranunx) IgAgaleV inonxMdu beLeya biVja bititxruva neladalilx -- bitutx, bitatxnemADu. 
\num{2} IgAgaleV oMdu beLeyanunx bitatxnemADida BUmiyalilx muMde beLeyuva peYranunx bitutx, neDu. 
\enum
\emng
\eentry

\bentry
\word{underspend} 
\pron{aMDarfsepxMDf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{underspent}).}
\bmng
\bnum
\num{1} (nigadiyAgiruvudakikxMta) kaDame KacuR mADu. 
\num{2} (\AtAmx) bahaLa kaDame KacARgu yA tiVrA savxlapx KacuR mADu (\akirx\ saha). 
\enum
\emng
\eentry

\bentry
\word{understaffed} 
\pron{aMDarfsATxphfTx}
\gl{\gu}
\expl{}
\bmng
 sibabxMdi korateya; kaDame sibabxMdi hoMdiruva; kaDame jana kelasagArariruva: \eng{the hospital is understaffed} Asapxterxyalilx bahaLa kaDame sibabxMdi ide. 
\emng
\eentry

\bentry
\word{understand} 
\pron{aMDarfsATxYxMDf}
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{understood} \ucAcx-- aMDarfsuTxDf). }


\noindent
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (mAtugaLu, vayxkitx, BASe, \mo vugaLa) athaR -- garxhisu, hiDi; athaRmADiko; ari(tuko); tiLi(duko): \eng{does not understand what you say} niVnu heVLuvudu(avanige) athaRvAguvudilalx. \eng{understands you perfectly} ninanxnunx pUNaRvAgi aritukoMDidAdxne. \eng{cannot understand French} pherxMcfBASeyanunx athaRmADikoLaLxlAra. 
\num{2} (udedxVsha, vivaraNe yA kAraNavanunx) garxhisu; ari; tiLi; Uhisu; gotutx mADiko; athaR mADiko: \eng{do not understand why he came} avanu yAke baMdanoV nanage tiLiyadu. \eng{could not understand what the noise was about} A galATe yAvudara bagegx eMbudu gotAtxgalilalx. \eng{do not understand the point of his remark} avana TiVkeya aMshavanunx nAnu athaRmADikoLaLxlAre. 
\num{3} (yAvudAdarU sUcane yA mAhitiya AdhArada meVle Uhe, tiVmARna, \mo vugaLige baruvalilx) naMbu; eNisu; Uhisu; BAvisu; takiRsu: \eng{I understand that it begins at noon} adu madhAyxhanx pArxraMBavAgutatxdeMdu nananx Uhe. \eng{I understand that expenses were to be paid} KacuRmADiruvudanunx tuMbikoDalAgutatxdeMdu nAnu eNisutetxVne. 
\num{4} (AshacxyaR, koVpa, saMdeVha, \mo vanunx sUcisuvAga) tiLi; garxhisu; aMduko: \eng{am I to understand that you refuse?} niVnu nirAkarisutitxVyeMdu tiLiyaleV? 
\num{5} (manasisxnalilx) (padavanunx) adhAyxhAra mADiko: \eng{the verb may be either expressed or understood} kirxyApada vAcayxvAgirabahudu yA adhAyxhAravAgirabahudu. 
\num{6} lakaSxNa yA savxBAvavanunx sahAnuBUtiyiMda -- athaRmADiko, tiLi, gamanisu, aritiru: \eng{quite understand your difficulty} ninanx kaSaTx nanage athaRvAgutatxde (nAnu balelx). \eng{cannot understand him at all} avananunx athaRmADikoLuLxvudeV kaSaTx. 
\num{7}(heVge nivaRhisabeVkeMbudanunx, vayxvaharisabeVkeMbudanunx) aritiru; athaRmADikoMDiru: \eng{could never understand algebra} biVjagaNitavanunx eMdU athaRmADikoLaLxlAgiradavanAgidedx. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (sAmAnayxvAgi yA vishiSaTx athaRdalilx) athaRvAgu; arivAgu. 
\num{2} (UheyiMda yA Kacita AdhArada meVle) tiLi; garxhisu; naMbu: \eng{he is coming to -- morrow, I understand} (avanu nALe barutAtxneMdu keVLide, hwde?) avanu nALe barutitxdAdxneMdu tiLididedxVne. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \hyperref{kandict_g.pdf}{G}{give(1) pagu(16)}{$^1$give} \eng{person to understand.} 
\num{2} \eng{tongue not understanded of the people} (\pArxparx) janakekx athaRvAgadidadx paricayavilalxdidadx BASe, anayxBASe, paraBASe. 
\numi{3} \eng{understand each other} 
\banum
\alnum{a} obabxrigobabxru athaRmADikoLiLx; parasapxrara aBipArxya, BAvane, \mo vanunx garxhisikoLiLx. 
\alnum{b} parasapxra omamxtadalilxri yA oLasaMcinalilx BAgigaLAgiri. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{understandable} 
\pron{aMDarfsATxYxMDabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 athaRmADikoLaLxbahudAda; gArxhayx; garxhisabalalx; athaRvAguva. 
\emng
\eentry

\bentry
\word{understandably} 
\pron{aMDarfsATxYxMDabilx}
\gl{\kirxvi}
\expl{}
\bmng
athaRvAguvaMte. 
\emng
\eentry

\bentry
\word{understander} 
\pron{aMDarfsATxYxMDarf}
\gl{\nA}
\expl{}
\bmng
 athaRmADikoLuLxvava; aritukoLaLxbalalxva. 
\emng
\eentry

\bentry
\word[understanding(1)]{understanding} 
\pron{aMDarfsATxYxMDiMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tiLivaLikeyuLaLx; sUkaSxmX budidhxyiruva; viveVcaneyuLaLx: \eng{please be understanding; do not scold the child} dayaviTuTx viveVcaneyuLaLxvanAgiru; maguvanunx bayayxbeVDa. 
\num{2} (inonxbabxra) iMgita garxhisuva; (inonxbabxra BAvane) athaRmADikoLaLxbalalx. 
\num{3} oLeLxya viveVcaneyuLaLx; viveVkavuLaLx. 
\num{4} (inonxbabxra BAvanegaLa bagegx) sahAnuBUtiyuLaLx: \eng{with an understanding smile} sahAnuBUtiya muguLanxge nakukx. 
\enum
\emng
\eentry

\bentry
\word[understanding(2)]{understanding} 
\pron{aMDarfsATxYxMDiMgf}
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} tiLivu; jAcnxna; viveVcane; tiLivaLike; arivu. 
\alnum{b} budidhx(shakitx); budidhxsUkaSxmXte; garxhaNashakitx: \eng{has an excellent understanding} (avanige) utakxqqSaTxvAda budidhxshakitxyide. 
\eanum
\numie
\num{2} (saMveVdane, BAva, \mo vanunx kuritu) garxhaNashakitx; garxhike; athaRmADikoLuLxva shakitx; saMveVdanAshiVlate; BAvagarxhaNashakitx. 
\num{3} (aBipArxya yA BAvaneyalilx) omamxnasusx; hoMdike; sAmarasayx: \eng{disturbed the (good) understanding between them} avaribabxra naDuvaNa sAmarasayxkekx dhakekx baMtu. 
\num{4} (parisithxti, saMdaBaR \mo vugaLa bagegx) vayxkitxya -- garxhike, parijAcnxna, viveVcanA shakitx. 
\num{5} (\kanmu\ aupacArikavAda) opapxMda; gotutxpADu; oDaMbaDike; Saratutx: \eng{had an understanding with the rival company} parxtisapxdhiR kaMpaniyoMdige opapxMda mADikoMDa. \eng{consented only on this understanding} I Saratitxna, oDaMbaDikeya meVle mAtarx opipxkoMDe. 
\num{6} sahAnuBUti yA sahane. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
 \eng{on the distinct understanding} sapxSaTx oDaMbaDikeya meVle; opapxMdada parxkAra. 
\emng
\eentry

\bentry
\word{understate} 
\pron{aMDarfseTxVTf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} iruvudakikxMta, satayxkikxMta kaDameyAgi, koreyAgi -- heVLu; kuMdisi nuDi; tagigxsi heVLu. 
\num{2} vAsatxvadalilxruvudakikxMta kaDameyAgiruvaMte nirUpisu. 
\enum
\emng
\eentry

\bentry
\word{understatement} 
\pron{aMDarfseTxVTfmaMTf}
\gl{\nA}
\expl{}
\bmng
 tagugxnuDi; nUyxnoVkitx; vAsatxvikavAgi iruvudakikxMta kaDame mADi, koreyAgi, kuMdisi heVLuvudu. 
\emng
\eentry

\bentry
\word{understater} 
\pron{aMDarfseTxVTarf}
\gl{\nA}
\expl{}
\bmng
 nUyxnoVkitxkAra; iruvudakikxMta, satayxkikxMta -- koreyAgi heVLuvavanu. 
\emng
\eentry

\bentry
\word[understeer(1)]{understeer} 
\pron{aMDarfsiTxarf}
\gl{\akirx}
\expl{}
\bmng
 (moVTAru vAhanada \vi) (udedxVshisidudakikxMta) nidhAnavAgi, melalxge -- tirugu. 
\emng
\eentry

\bentry
\word[understeer(2)]{understeer} 
\pron{aMDarfsiTxarf}
\gl{\nA}
\expl{}
\bmng
 (vAhana) nidhAnatirugu; melalxge, nidhAnavAgi tiruguvudu. 
\emng
\eentry

\bentry
\word[understood(1)]{understood} 
\pron{aMDarfsuTxDf}
\gl{\kirx}
\expl{}
\bmng
 \eng{understand} dhAtuvina \BU\ matutx \BUkaq\ rUpa. 
\emng
\eentry

\bentry
\word[understood(2)]{understood} 
\pron{aMDarfsuTxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} opipxge koTaTx; samamxtisida. 
\num{2} sUcita; anukatx; sUcisida Adare parxkaTavAgi heVLada. 
\enum
\emng
\eentry

\bentry
\word{understorey} 
\pron{aMDarfsoTxVri}
\gl{\nA}
\expl{(\bava\ \eng{understoreys}).}
\bmng
\bnum
\num{1} aDikADu; kADina hemamxragaLa keLage, neraLinalilx beLeda giDagaMTigaLu, sasayxrAshi. 
\num{2} aDikADinalilx beLeyuva giDagaLu. 
\enum
\emng
\eentry

\bentry
\word{understrapper} 
\pron{aMDarfsATxrXYxparf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{underling}{underling}. 
\num{2} keYkeLagina kelasagAra; tAbeVdAra; kiVLALu; aDiyALu. 
\enum
\emng
\eentry

\bentry
\word[understudy(1)]{understudy} 
\pron{aMDarfsaTxDi}
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{understudies}; \BU\ matutx \BUkaq\ \eng{understudied}).}
\bmng
\bnum
\num{1} badali naTanAgi -- pAtarxda mAtugaLanunx gaTiTxmADu, pAtarxvahisalu sidadhxnAgiru. 
\num{2} badali naTanAgi kelasamADu. 
\enum
\emng
\eentry

\bentry
\word[understudy(2)]{understudy} 
\pron{aMDarfsaTxDi}
\gl{\nA}
\expl{}
\bmng
 (\bava\ \eng{understudies}). (nATakashAle) badali naTa; payARya pAtarxdhAri; niyamita naTanu samayadalilx bAradiruvAga avana badalu, pAtarxkekx sidadhxnAgiruvavanu, pAtarxda mAtu gaTiTxmADikoMDiruvavanu. 
\emng
\eentry

\bentry
\word{undersubscribed} 
\pron{aMDarfsabfsekxrXYbfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sAkaSuTx caMdAdArarilalxda; caMdAdArara korateyiruva. 
\num{2} sAkaSuTx BAgavahisuvavarilalxda. 
\enum
\emng
\eentry

\bentry
\word{undersurface} 
\pron{aMDarfsaphiRsf}
\gl{\nA}
\expl{}
\bmng
 oLameY; meVlemxYya taLaBAga. 
\emng
\eentry

\bentry
\word{undertake} 
\pron{aMDarfTeVkf}
\gl{\sakirx}
\expl{}
\bmng
 (\BU\ \eng{undertook;} \BUkaq\ \eng{undertaken}). 
\bnum
\num{1} (kelasa, udayxma, javAbAdxri, \mo vanunx) keYkoLuLx; hotutxko; vahisu. 
\num{2} (kelasa \mo vakekx) keYhAku; toDagu 
\num{3} (EnanAnxdarU mADalu obabxnige) mAtu koDu; vAgAdxna mADu. 
\num{4} Baravase koDu; AshAvxsane niVDu; KAtaripaDisu; BaravaseyiMda heVLu; daqDhavAgi heVLu: \eng{I will undertake that he has not heard a word} avanige oMdu mAtU keVLisilalxveMdu nAnu daqDhavAgi heVLutetxVne. \eng{I will undertake that you shall not be the loser by it} ninage adariMda yAva naSaTxvU AguvudilalxveMdu nAnu KAtari koDutetxVne. 
\enum
\emng
\eentry

\bentry
\word{undertaker} 
\pron{aMDarfTeVkarf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nivARhaka; EnanAnxdarU -- keYkoLuLxvavanu, hotutxkoLuLxvavanu, nivaRhisuvavanu. 
\num{2} (EnanAnxdarU mADalu) mAtu koDuvavanu; vAgAdxna mADuvavanu. 
\num{3} shavasaMsAkxra nivARhaka. 
\num{4} (\ca) dore kelavu riyAyiti toVrisuvudAdare, avanige beVkAda sarabarAyigaLanunx koDuva shAsanavanunx pAliRmeMTinalilx maMjUru mADisuva javAbAdxri hotutxkoLuLxtitxdadx, \eng{17}neV shatamAnada shirxVmaMta, parxBAviV vayxkitx. 
\enum
\emng
\eentry

\bentry
\word{undertaking} 
\pron{aMDarfTeVkiMgf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (kelasa, javAbAdxri, \mo vanunx) keYkoLuLxvudu; hotutxkoLuLxvudu; vahisikoLuLxvudu. 
\num{2} kAyaR; udayxma; keYkoMDa kelasa: \eng{a serious undertaking} gaMBiVravAda kAyaR. 
\num{3} vAgAdxna, BASe, mAtu -- koDuvudu. 
\num{4} javAbAdxri; hoNe. 
\num{5}shavasaMsAkxrada nivaRhaNe. 
\enum
\emng
\eentry

\bentry
\word{undertenancy} 
\pron{aMDarfTenanisx}
\gl{\nA}
\expl{(\bava\ \eng{undertenancies}).}
\bmng
\bnum
\num{1} oLa -- gutitxge, geVNi. 
\num{2} oLabADige. 
\enum
\emng
\eentry

\bentry
\word{undertenant} 
\pron{aMDarfTenaMTf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oLa -- gutitxgedAra, geVNidAra, okakxlu. 
\num{2} upabADigedAra. 
\enum
\emng
\eentry

\bentry
\word{underthings} 
\pron{aMDarftiMgfsxZ}
\gl{\nA}
\expl{}
\bmng
 (\bava)(\AmA) = \hyperlink{underclothes}{underclothes}. 
\emng
\eentry

\bentry
\word{undertint} 
\pron{aMDarfTiMTf}
\gl{\nA}
\expl{}
\bmng
 teLubaNaNx; tiLibaNaNx. 
\emng
\eentry

\bentry
\word{undertone} 
\pron{aMDarfToVnf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (mAtanADuvalilx) tagugxdani; maMdarxsavxra; meludani: \eng{talk in undertones} tagigxda daniyalilx mAtanADu. 
\num{2} teLubaNaNx. 
\num{3} oLa -- guNa, lakaSxNa; aMtagaRtavAgiruva lakaSxNa, guNa, \mo vu. 
\num{4} parxcaCxnanxvAda BAva; oLadhavxni; AMtaBARva parxvAha. 
\enum
\emng
\eentry

\bentry
\word{undertook} 
\pron{aMDarfTukf}
\gl{\sakirx}
\expl{}
\bmng
 \eng{undertake} dhAtuvina BUtarUpa. 
\emng
\eentry

\bentry
\word{undertow} 
\pron{aMDarfToV}
\gl{\nA}
\expl{}
\bmng
 keLaharivu; adhaHparxvAha; meVlemxYparxvAhakekx virudadhxdikikxnalilx hariyuva, samudarxda keLasatxrada harivu, parxvAha. 
\emng
\eentry

\bentry
\word{undertrick} 
\pron{aMDarfTirxkf}
\gl{\nA}
\expl{}
\bmng
 (birxDfjx ATa) konepaTuTx; yAva oMdu paTuTx sikakxdadxriMda turupu heVLuvavanu vAgAdxna kaLedukoLuLxtAtxnoV A paTuTx. 
\emng
\eentry

\bentry
\word{undervaluation} 
\pron{aMDarfvAYxlUyxESa(Sf)nf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kaDame bele kaTuTxvudu; ava -- mUlayxna, mUlAyxMkana. 
\num{2} kaDegaNane; hiVneYke; kaDameyAgi gaNisuvike, eNisuvike. 
\enum
\emng
\eentry

\bentry
\word{undervalue} 
\pron{aMDarfvAYxlUyx}
\gl{\sakirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{undervalues}; \BU\ matutx}
\BUkaq\ \eng{undervalued;} \vakaq\ \eng{undervalueing}). \bmng
\bnum
\num{1} (nAyxyavAdudakikxMta) tiVrA kaDame bele kaTuTx. 
\num{2} kaDameyAgi eNisu; kaDegaNisu. 
\enum
\emng
\eentry

\bentry
\word{undervest} 
\pron{aMDarfvesfTx}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_v.pdf}{V}{vest(1)1}{$^1$vest\((1)\)}. 
\emng
\eentry

\bentry
\word[underwater(1)]{underwater} 
\pron{aMDarfvATarf}
\gl{\gu}
\expl{}
\bmng
 niVroLagina; aMtajaRliVya. 
\emng
\eentry

\bentry
\word[underwater(2)]{underwater} 
\pron{aMDarfvATarf}
\gl{\kirxvi}
\expl{}
\bmng
 niVroLage; niVrina keLage. 
\emng
\eentry

\bentry
\word{underwear} 
\pron{aMDarfveVrf}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{underclothes}{underclothes}. 
\emng
\eentry

\bentry
\word[underweight(1)]{underweight} 
\pron{aMDarfveVTf}
\gl{\nA}
\expl{}
\bmng
 kaDame -- BAra, tUka; sAkaSiTxlalxda tUka. 
\emng
\eentry

\bentry
\word[underweight(2)]{underweight} 
\pron{aMDarfveVTf}
\gl{\gu}
\expl{}
\bmng
 (agatayxvAgiruvudakikxMta yA apeVkaSxNiVyavAgiruvudakikxMta) kaDame -- BAraviruva, tUkada. 
\emng
\eentry

\bentry
\word{underwent} 
\pron{aMDarfveMTf}
\gl{\kirx}
\expl{}
\bmng
 \eng{undergo} dhAtuvina BUta rUpa. 
\emng
\eentry

\bentry
\word{underwhelm} 
\pron{aMDarfvelfmx}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\hA) pUtiR muLugadiru; thaTaTxne nAshagoLaLxdiru. 
\num{2} BAvaparavasha mADadiru. 
\num{3} (parxshenx \mo vugaLa surimaLeyiMda) galibiliyAgadiru. 
\enum
\emng
\eentry

\bentry
\word{underwing} 
\pron{aMDarfviMgf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} rekekxgaLa hiMBAgadalilx ujajxvXla gurutugaLuLaLx halavu pataMgagaLalolxMdu. 
\num{2} keLarekekx; beVroMdu rekekxya keLagiruva yA aMshataH mareyAda rekekx. 
\enum
\emng
\eentry

\bentry
\word{underwood} 
\pron{aMDarfvuDf}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{undergrowth}{undergrowth \((1)\)}. 
\emng
\eentry

\bentry
\word{underworld} 
\pron{aMDarfvalfDxR}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (purANada) adhoVloVka; pAtALaloVka. 
\num{2} BUgata -- loVka, jagatutx, parxpaMca, samAja; vayxvasithxta aparAdhagaLu, kUrxrakaqtayxgaLu matutx aneYtika vayxvahAragaLiMda jiVvana naDesuva samAjada vagaR, guMpu: \eng{underworld lord don} BUgata dore; BUgata samAjada nAyaka. 
\num{3} = \hyperref{kandict_a.pdf}{A}{antipodes}{antipodes}. 
\enum
\emng
\eentry

\bentry
\word{underwrite} 
\pron{aMDareYTf} 
\gl{\kirx}
\expl{(\BU\ \eng{underwrote}; \BUkaq\ \eng{underwritten}). }
\bmng
\emng

\noindent
\gl{\sakirx}
\bmng
\bnum
\numi{1} (\kanmu\ haDagina vAyxpAra \mo vugaLige) 
\banum
\alnum{a} hoNeyAgi, javAbAdxrivahisi (vimA patarx) baredukoDu. 
\alnum{b} (vimA patarxkekx) rujuhAku. 
\alnum{c} (haDagu \mo vugaLa) kaDala vimA vAyxpAra mADu. 
\eanum
\numie
\num{2} sAvaRjanikaru koLaLxda, oMdu saMsethxya elAlx baMDavALa SeVrugaLanunx koLaLxlu opapxMda mADiko; oTuTx SeVrugaLa koLuLxvikeya opapxMda mADiko. 
\num{3} AthiRka neravu (sAla) yA otAtxse koDalu opapxMda mADiko. 
\num{4} keLage bare: \eng{the underwritten names} keLage barediruva hesarugaLu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
kaDala vimA udoyxVga naDesu. 
\emng
\eentry

\bentry
\word{underwriter} 
\pron{aMDareYTarf}
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (\kanmu\ kaDala) vimA udoyxVgadalilxruvavanu. 
\alnum{b} (vimApatarxda mUlaka) hoNegAranAguvavanu. 
\eanum
\numie
\num{2} (kaMpeniya SeVru \mo vanunx koLaLxlu badadhxnAda) hoNegAra; KAtaridAra. 
\enum
\emng
\eentry

\bentry
\word{undescended} 
\pron{aMDiseMDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keLakekx -barada, iLiyada, sariyada. 
\num{2} (\veYshA) (vaqSaNada \vi) keLakekx iLidirada; sahajavAgi vaqSaNakoVshadoLakikxLiyade hoTeTxya keLaBAgadalelxV uLidubiDuva. 
\enum
\emng
\eentry

\bentry
\word{undeserved} 
\pron{anfDisaZvfDxR}
\gl{\gu}
\expl{}
\bmng
 (shikeSxgAgali, parxshaMsegAgali) anahaRvAda; pAtarxvalalxda; salalxda; takukxdalalxda; ucitavalalxda: \eng{undeserved praise} anucita hogaLike. \eng{undeserved suffering} salalxda veVdane. 
\emng
\eentry

\bentry
\word{undeservedly} 
\pron{anfDisaZviRDilx}
\gl{\kirxvi}
\expl{}
\bmng
 (shikeSxgAgali, parxshaMsegAgali) ahaRvalalxde; pAtarxvAgirade. 
\emng
\eentry

\bentry
\word{undeserving} 
\pron{anfDisaZviRMgf}
\gl{\gu}
\expl{}
\bmng
 pAtarxvAgirada; anahaR; salalxda; tagada; apAtarx; ahaRvAgirada: \eng{undeserving of praise} hogaLikege pAtarxnalalxda; parxshaMsAhaRvalalxda. 
\emng
\eentry

\bentry
\word{undeservingly} 
\pron{anfDisaZviRMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
 apAtarxvAgi; anahaRvAgi; salalxdaMte. 
\emng
\eentry

\bentry
\word{undesigned} 
\pron{anfDiseZYnfDx}
\gl{\gu}
\expl{}
\bmng
 udedxVshisada; udedxVshapUvaRkavalalxda; anudidxSaTx. 
\emng
\eentry

\bentry
\word{undesignedly} 
\pron{anfDiseZYniDfli}
\gl{\kirxvi}
\expl{}
\bmng
udedxVshapUvaRkavAgirade; udedxVshisade. 
\emng
\eentry

\bentry
\word{undesigning} 
\pron{anfDiseZYniMgf}
\gl{\gu}
\expl{}
\bmng
 udedxVsharahita; yAvudeV durudedxVshavilalxda; duraBipArxyavilalxda. 
\emng
\eentry

\bentry
\word{undesirability} 
\pron{anfDiseZYarabiliTi}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AkeSxVpaNiVyate. 
\num{2} ahitakaravAgiruvudu. 
\num{3} anapeVkaSxNiVyate; bayasabAradAgiruvudu. 
\enum
\emng
\eentry

\bentry
\word[undesirable(1)]{undesirable} 
\pron{anfDiseZYarabflf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} apeVkaSxNiVyavalalxda; anapeVkaSxNiVya; bayasatakukxdalalxda; hAreYsatakukxdalalxda; Ashisatakukxdalalxda. 
\num{2} ahitakara; aniSaTxda. 
\num{3} AkeSxVpaNiVya; AkeSxVpaNAhaR. 
\enum
\emng
\eentry

\bentry
\word[undesirable(2)]{undesirable} 
\pron{anfDise ZYarabflf}
\gl{\nA}
\expl{}
\bmng
 anapeVkiSxta yA ahitavAda yA AkeSxVpaNAhaR -- vayxkitx (\kanmu\ kuDuka). 
\emng
\eentry

\bentry
\word{undesirableness} 
\pron{anfDiseYZarabf(ba)lfnisf}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{undesirability}{undesirability}. 
\emng
\eentry

\bentry
\word{undesirably} 
\pron{anfDiseZYarabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} ahitakaravAgi. 
\num{2} anapeVkaSxNiVyavAgi. 
\num{3} AkeSxVpaNiVyavAda riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{undesired} 
\pron{anfDiseZYaDfR}
\gl{\gu}
\expl{}
\bmng
 anapeVkiSxta; beVDavAda; bayasada; keVLada; Ashisada; koVrada; apeVkiSxsidadxlalxda. 
\emng
\eentry

\bentry
\word{undesirous} 
\pron{anfDiseZYarasf} 
\gl{\gu}
\expl{}
\bmng
 iSaTxvilalxda; beVDavAda; bayasada. 
\emng
\eentry

\bentry
\word{undetectable} 
\pron{anfDiTekaTxbf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 kaMDuhiDiyalAgada; gotutxhacacxlAgada; patetx mADalu kaSaTxvAgiruva; amaqgayx. 
\emng
\eentry

\bentry
\word{undetected} 
\pron{anfDiTekiTxDf}
\gl{\gu}
\expl{}
\bmng
 kaMDuhiDiyada; gotutxhacacxdiruva; patetx mADada. 
\emng
\eentry

\bentry
\word{undetermined} 
\pron{anfDiTamiRnfDx}
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{undecided}{undecided}. 
\emng
\eentry

\bentry
\word{undeterred} 
\pron{anfDiTaDfR}
\gl{\gu}
\expl{}
\bmng
 dheYyaRguMdisada; aDiDxmADada; taDeyoDaDxda; himemxTiTxsada; Baya huTiTxsada: \eng{not undeterred by the weather} havAmAnakekx hedari himemxTaTxde. 
\emng
\eentry

\bentry
\word{undeveloped} 
\pron{anfDivelapfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aBivaqdidhxyAgada; hiMduLida: \eng{undeveloped nations} hiMduLida, aBivaqdidhx hoMdada rASaTxrXgaLu. 
\num{2} (BUmiya \vi) kaqSi mADada; sAguvaLi mADada; kaqSige upayoVgisada: \eng{undeveloped land} kaqSi mADada jamiVnu. 
\enum
\emng
\eentry

\bentry
\word{undeviating} 
\pron{anfDiVviETiMgf}
\gl{\gu}
\expl{}
\bmng
 mAgaRcuyxtiyAgada; hAdibiTuTx, dAritapipx hoVgada; aDaDxdAri hiDiyada; neVravAda. 
\emng
\eentry

\bentry
\word{undeviatingly} 
\pron{anfDiVviETiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
 dAritapipx hoVgadaMte; aDaDxdAri hiDiyade; mAgaRcuyxtiyilalxde; neVravAgi. 
\emng
\eentry

\bentry
\word{undiagnosed} 
\pron{anfDeYagonxVsfDx}
\gl{\gu}
\expl{}
\bmng
(roVgada \vi) gotutxhiDiyada; niSakxSiRsada; kaMDuhiDiyada; gotAtxgada. 
\emng
\eentry

\bentry
\word{undid} 
\pron{anfDiDf}
\gl{\kirx}
\bmng
 \eng{undo} dhAtuvina BUtarUpa. 
\emng
\eentry

\bentry
\word{undies} 
\pron{anfDisfZ}
\gl{\nA}
\expl{}
\bmng
 (\bava dalilx)(\AmA) (\kanmu\ heMgasara) oLauDupu. 
\emng
\eentry

\bentry
\word{undifferentiated} 
\pron{anfDiphareniSxETiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vayxtAyxsa mADada; BeVda kalipxsada. 
\num{2} (jiVvi, vaMsha, aMgagaLu, samAnAthaR padagaLu, \mo vugaLa \vi) vayxtAyxsa -- toVrisada, kalipxsada; parxBeVda mADada. 
\enum
\emng
\eentry

\bentry
\word{undigested} 
\pron{anfDi(DeY)jesiTxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (AhAra) jiVNiRsikoMDilalxda. 
\numi{2} 
\banum
\alnum{a} (saMgati \mo vugaLa \vi) athaRmADikoMDilalxda; tiLidukoMDilalxda. 
\alnum{b} (vAsatxvAMshagaLu \mo vugaLa \vi) korxVDiVkarisada; vayxvasethxgoLisada. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{undignified} 
\pron{anfDiginxpheYDf}
\gl{\gu}
\expl{}
\bmng
 Ganate gAMBiVyaRvilalxda; gwravakekx kuMdu mADuva; ayoVgayxvAda. 
\emng
\eentry

\bentry
\word{undiluted} 
\pron{anfDeYlUyxTiDf}
\gl{\gu} 
\bmng
\bnum
\num{1} nisasxtatxvXvAgada; sAraguMdada; bala kugigxsada; dubaRlavAgisada. 
\num{2} teLuvAgisada. 
\enum
\emng
\eentry

\bentry
\word{undiminished} 
\pron{anfDiminiSfTx}
\gl{\gu}
\expl{}
\bmng
 kaDame mADada; kaDameyAgada; kuMdada; kuMdisada; koreyAgada; kugigxsada. 
\emng
\eentry

\bentry
\word{undimmed} 
\pron{anfDimfDx}
\gl{\gu}
\expl{}
\bmng
 mabAbxgada; mabubxgoLisada; kaLeguMdisada; kaLeguMdada; kAMtihiVnavAgada; mAsada; maMkAgada. 
\emng
\eentry

\bentry
\word{undine} 
\pron{anfDiVnf}
\gl{\nA}
\expl{}
\bmng
 jalayakiSx; jalABimAni kaneyx. 
\emng
\eentry

\bentry
\word{undiplomatic} 
\pron{anfDipalxmAYxTikf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (rAyaBAra)kushalateyilalxda; vayxvahAra jAcnxnavilalxda. 
\num{2} aucitayxparxjecnxyilalxda; samayoVpAya ariyada; samayoVcita nayavilalxda: \eng{an undiplomatic answer} samayoVcita nayavilalxda utatxra. 
\enum
\emng
\eentry

\bentry
\word{undiplomatically} 
\pron{anfDipalxmAYxTikali}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vayxvahArajAcnxnavilalxde. 
\num{2} aucitayxparxjecnx ilalxde. 
\enum
\emng
\eentry

\bentry
\word{undischarged} 
\pron{anfDisfcAjfRDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (viduyxtf)AveVsha visajiRsada. 
\num{2} (kelasadiMda) tegeduhAkada; vajA mADada. 
\num{3} (sereyALu, AsapxterxroVgi) vimukatxnAgada; biDugaDeyAgada. 
\num{4} (parxvAha \mo vu) hariyada; suriyada; basiyada. 
\num{5} (\kanmu\ sAlada \vi) tiVruvaLiyAgada; pAvatiyAgada. 
\num{6} (divALiyAdavana \vi) uLike hoNeyiMda - biDugaDemADada, mukatxgoLisada. 
\enum
\emng
\eentry

\bentry
\word{undiscipline} 
\pron{anfDisipilxnf}
\gl{\nA}
\expl{}
\bmng
 ashisutx.\eng{ } 
\emng
\eentry

\bentry
\word{undisciplined} 
\pron{anfDisipilxnfDx}
\gl{\gu}
\expl{}
\bmng
 shisitxlalxda; ashisitxna. 
\emng
\eentry

\bentry
\word{undisclosed} 
\pron{anfDisfkolxVsfDx} 
\gl{\gu}
\expl{}
\bmng
 bayalu mADada; tiLiyapaDisada; parxkaTisada; bahiraMga mADada; horageDahada. 
\emng
\eentry

\bentry
\word{undiscoverable} 
\pron{anfDisakxvarabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 kaMDuhiDiyalAgada; patetxhacacxlu kaSaTxvAda. 
\emng
\eentry

\bentry
\word{undiscovered} 
\pron{anfDisakxvaDfR}
\gl{\gu}
\expl{}
\bmng
 kaMDuhiDiyada; patetxyAgada. 
\emng
\eentry

\bentry
\word{undiscriminating} 
\pron{anfDisikxrXmineVTiMgf}
\gl{\gu}
\expl{}
\bmng
 sUkaSxmXvAgi parishiVlisada; sUkaSxmXviveVcaneyilalxda. 
\emng
\eentry

\bentry
\word{undisguised} 
\pron{anfDisfgeYsfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} veVSa hAkirada; rUpumaresikoLaLxda. 
\num{2} maremAcada; mucicxDada; bacicxDada; parxkaTavAda; mucucxmareyilalxda. 
\enum
\emng
\eentry

\bentry
\word{undisguisedly} 
\pron{anfDisfgeYsiZDilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} veVSa maresikoLaLxde. 
\num{2} sapxSaTxvAgi; parxkaTavAgi; maremAcade; mucucxmareyilalxde. 
\enum
\emng
\eentry

\bentry
\word{undismayed} 
\pron{anfDisfmeVDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nirutAsxhagoLaLxda; edeguMdada. 
\num{2} digilu paDada; BayapaDada; gAbariyAgada; kaLavaLa paDada. 
\enum
\emng
\eentry

\bentry
\word{undisputed} 
\pron{anfDisfpUyxTiDf}
\gl{\gu}
\expl{}
\bmng
 caceR mADada; vAdavilalxda; viroVdhavilalxda; niviRvAdada; nisasxMshayavAda. 
\emng
\eentry

\bentry
\word{undissolved} 
\pron{anfDisAlfvxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} karagisada; karagada. 
\num{2} visajiRsada; konegANisada; samApitxgoLisada. 
\enum
\emng
\eentry

\bentry
\word{undistinguishable} 
\pron{anfDisiTxMgivxSabalf} 
\gl{\gu}
\expl{}
\bmng
 = \hyperref{kandict_i.pdf}{I}{indistinguishable}{indistinguishable}. 
\emng
\eentry

\bentry
\word{undistinguished} 
\pron{anfDisiTxMgivxSfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hecucxgArikeyilalxda; sAmAnayxmaTaTxda; madhayxma dajeRya; veYshiSaTxyXvilalxda. 
\num{2} gamanAhaRvalalxda; edu dxkANisada. 
\enum
\emng
\eentry

\bentry
\word{undistributed} 
\pron{anfDisiTxrXbUyxTiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vitarisada; haMcikoDada; pAlumADada. 
\num{2} viMgaDisada; viBAgisada; vagiRVkarisada. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{undistributed middle} (\takaR) avAyxpitxdoVSa; takaRvAkayxda madhayxmapada A vagaRda elAlx vayxkitxgaLigU anavxyisade hoVguvudariMda uMTAguva doVSa. 
\emng
\eentry

\bentry
\word{undisturbed} 
\pron{anfDisaTxbfRDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} galibiligoLaLxda; parxkuSxbadhxgoLaLxda; kadaDada; koSxVBegoLaLxda. 
\num{2} aDiDxyilalxda; aDetaDeyilalxda; parxtibaMdhakavilalxda; nirAtaMkavAda. 
\enum
\emng
\eentry

\bentry
\word{undivided} 
\pron{anfDiveYDiDf} 
\gl{\gu}
\expl{}
\bmng
 viBajisada; parxteyxVkisada; beVrebeVre viMgaDisada; aviBakatx; aKaMDa. 
\emng
\eentry

\bentry
\word{undo} 
\pron{anfDU}
\gl{\akirx}
\expl{(vataRmAnakAla \parxpu\ \Eva\ \eng{undoes}; \BU\ \eng{undid}; \BUkaq\ \eng{undone}).}
\bmng
\bnum
\num{1} (aMgi, guMDi, BAMgi, \mo vanunx) bicucx; tege; kaLacu: \eng{my shoelace has become undone} nananx pAdarakeSxya dAra bicicxhoVgide. 
\num{2} (vayxkitxya baTeTx \mo vanunx) bicucx; kaLacu; tegeduhAku. 
\num{3} radudxgoLisu; ilalxvAgisu; toDeduhAku: \eng{cannot undo the past} hiMde naDedadadxnunx toDeduhAkalu sAdhayxvilalx. 
\num{4} (hesaru, sananxDate, \mo vanunx) keDisu; hALu mADu; nAshakikxVDu mADu (\kanmu\ \BUkaq dalilx Iga \viparx): \eng{`alas}! \eng{I am undone' she sobbed out} `ayoyxV! nAnu keTeTx' eMdu avaLu bikikxbikikx atatxLu. 
\enum
\emng
\eentry

\bentry
\word{undock} 
\pron{anfDAkf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (haDaganunx) haDagukaTeTxge taradiru (\akirx\ saha). 
\num{2} (haDaganunx) haDagu kaTeTxyiMda Acege, horakekx -- tege, tegedukoMDu hoVgu (\akirx\ saha). 
\num{3} AkAshadalilx oMdu AkAshanwkeyanunx inonxMdariMda - beVpaRDisu, beVremADu (\akirx\ saha). 
\enum
\emng
\eentry

\bentry
\word{undocumented} 
\pron{anfDAkuyxmeMTiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\ame) dAKalerahita; sariyAda yA sUkatx dAKale ilalxda. 
\num{2} dAKalitavAgirada; dAKalegaLalilx, dasAtxveVjugaLalilx - dAKaleyAgilalxda yA avugaLiMda parxmANiVkarisilalxda. 
\enum
\emng
\eentry

\bentry
\word{undoing} 
\pron{anfDUiMgf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (hiMde AgihoVgiruvudanunx, naDedudanunx) radudxgoLisike; toDeduhAkuvudu. 
\num{2} nAshagoLisuvike; hALumADuvike; keDisuvike. 
\num{3} bicucxvudu; saDilagoLisuvudu. 
\num{4} vinAsha yA hAniya mUlakAraNa: \eng{drink was his undoing} kuDita avana vinAshakekx mUlavAyitu. 
\enum
\emng
\eentry

\bentry
\word{undomesticated} 
\pron{anfDamesiTxkeVTiDf}
\gl{\gu}
\expl{}
\bmng
 paLagisilalxda; ogigxsilalxda. 
\emng
\eentry

\bentry
\word{undone} 
\pron{anfDanf}
\gl{\gu}
\bmng
\bnum
\num{1} pUNaRgoLisada; apUNaRvAda; pUtiRyAgada; pUtiR (mADi) mugisada: \eng{left the job undone} kelasavanunx pUNaRgoLisade biTaTxru. 
\num{2} kaTaTxda; bigiyada; baMdhisada; BadarxpaDisada: \eng{left the buttons undone} guMDigaLanunx hAkade yA holiyade hAgeyeV biTaTxnu. 
\num{3} (\pArxparx) nAshavAda; hALAda. 
\enum
\emng
\eentry

\bentry
\word{undoubtable} 
\pron{anfDwTaba(bf)lf}
\gl{\gu}
\expl{}
\bmng
 = \hyperref{kandict_i.pdf}{I}{indubitable}{indubitable}. 
\emng
\eentry

\bentry
\word{undoubted} 
\pron{anfDwTiDf}
\gl{\gu}
\expl{}
\bmng
 saMdeVhapaDada; nisasxMdigadhx; nisasxMdeVhavAda; nisasxMshayavAda. 
\emng
\eentry

\bentry
\word{undoubtedly} 
\pron{anfDwTiDfli}
\gl{\kirxvi}
\expl{}
\bmng
 nisasxMdeVhavAgi; nisasxMshayavAgi. 
\emng
\eentry

\bentry
\word{undrained} 
\pron{anfDerxVnfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} harisada; basiduhoVgada; baridAgada. 
\num{2} (baLasi) mugisada; KAlimADada. 
\enum
\emng
\eentry

\bentry
\word{undraped} 
\pron{anfDerxVpfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (baTeTx, parade, \mo vugaLanunx) hodisada; toDisada; AcACxdisada; aLavaDisada. 
\num{2} betatxleyAda; anAcACxdita. 
\enum
\emng
\eentry

\bentry
\word{undreamed} 
\pron{anfDirxVmfDx, anfDerxmfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kanasinalUlx kANada; eMdU kANada; kAlapxnika: \eng{undreamed of beauties} kanasinalUlx kANada suMdariyaru. 
\num{2} (eMdU) yoVcisada, Uhisada yA kalipxsikoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{undreamt} 
\pron{anfDerxmfTx}
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{undreamed}{undreamed}. 
\emng
\eentry

\bentry
\word[undress(1)]{undress} 
\pron{anfDerxsf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vayxkitxya meYmeVlina) baTeTxbare tege; uDupu kaLacu; baTeTx tegeduhAku. 
\num{2} (gAya, varxNa, \mo vugaLige kaTiTxda) paTiTx tegeduhAku, `bAyxMDeVjf' kaLacu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
(tananx) baTeTx tegeduhAku; vivasatxrXgoLuLx; (tananx) baTeTx kaLaciko. 
\emng
\eentry

\bentry
\word[undress(2)]{undress} 
\pron{anfDerxsf}
\gl{\nA}
\expl{}
\bmng
 (vihitakarxmada, adhikArada uDupu yA samavasatxrX eMbudakekx virudadhxvAgi) sAmAnayx, dinanitayxda -- uDupu; maneyuDupu. 
\emng
\eentry

\bentry
\word{undressed} 
\pron{anfDerxsfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (sariyAgi) uDupu dharisada; baTeTxbare hAkilalxda; sariyAgi sidadhxvAgilalxda; BAgashaH yA pUNaRtaH naganxvAda. 
\num{2} (togalu) nayamADilalxda; oraTAda juMgiruva; saMsakxrisilalxda. 
\num{3} (AhArada \vi) masAle, saMbAra hAkirada. 
\enum
\emng
\eentry

\bentry
\word{undrinkable} 
\pron{anfDirxMkabflf}
\gl{\gu}
\expl{}
\bmng
 kuDiyalAgada; kuDiyalu yoVgayxvalalxda; pAna yoVgayxvalalxda. 
\emng
\eentry

\bentry
\wordnospeech{UNDRO}{UNDRO}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
 \eng{United Nations Disaster Relief Organization.} 
\emng
\eentry

\bentry
\word{undue} 
\pron{anfDUyx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nAyxyavalalxda; yukatxvalalxda; anucita: \eng{exercise an undue influence} anucita parxBAva biVru. 
\num{2} anAvashayxka; agatayxvilalxda: \eng{with undue haste} anAvashayxkavAda AturadiMda. 
\num{3} BAri; vipariVta; mitimiVrida: \eng{with undue consideration} mitimiVrida dAkiSxNayxdiMda. 
\enum
\emng
\eentry

\bentry
\wordnospeech{undue influence}{undue influence}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 (\nAyxshA) anucita parxBAva; anAyxyada vashiVlu; tananx savxMta icePxganuguNavAgi naDedukoLaLxlAgadaMte yA GaTisuva pariNAmagaLa bagegx sAkaSuTx gamanakoDalAgadaMte obabx vayxkitxya meVle hAkuva, heVruva BAri yA anucita otatxDa, parxBAva. 
\emng
\eentry

\bentry
\word{undulant} 
\pron{anfDuyxlaMTf}
\gl{\gu}
\expl{}
\bmng
 taraMgiVya; taraMgadaMtha; caMcala; EriLitada; alegaLaMte calisuva. 
\emng
\eentry

\bentry
\wordnospeech{undulant fever}{undulant fever}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 = \hyperref{kandict_b.pdf}{B}{brucellosis}{brucellosis}. 
\emng
\eentry

\bentry
\word[undulate(1)]{undulate} 
\pron{anfDuyxleVTf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} aleyAkAravAgi calisuvaMte yA kANuvaMte mADu; taraMgadaMte EriLitavanunxMTumADu. 
\num{2} taraMgadaMtha racane yA toVkeR -- uMTumADu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} aleyAkAravAgi calisu yA kANu; taraMgadaMte EriLitaviru. 
\num{2} taraMgadaMtaha racaneyiru. 
\enum
\emng
\eentry

\bentry
\word[undulate(2)]{undulate} 
\pron{anfDuyxlaTf}
\gl{\gu}
\expl{}
\bmng
 taraMgadaMtha; aleyAkArada; taraMgita: \eng{leaves with undulate margins} aleyAkArada aMcuLaLx elegaLu. 
\emng
\eentry

\bentry
\word{undulately} 
\pron{anfDuyxlaTfli}
\gl{\kirxvi}
\expl{}
\bmng
 taraMgadaMte; aleyAkAradalilx; EriLiyutatx. 
\emng
\eentry

\bentry
\word{undulation} 
\pron{anfDuyxleVSa(Sf)nf}
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} taraMga -- rUpa, racane. 
\alnum{b} taraMgacalane; aleyAkArada EriLita. 
\eanum
\numie
\num{2} alegaLaMtha geregaLa oMdu taMDa; taraMga reVKAvaLi. 
\num{3} (oMdu) taraMga; ale; tere; lahari. 
\enum
\emng
\eentry

\bentry
\word{undulatory} 
\pron{anfDuyxlaTari}
\gl{\gu}
\expl{}
\bmng
 taraMga rUpada, racaneya, calaneya yA avugaLige saMbaMdhisida. 
\emng
\eentry

\bentry
\wordnospeech{undulatory theory}{undulatory theory}
\pron{?}
\gl{\nA}
\bmng
 = \hyperref{kandict_w.pdf}{W}{wave theory}{wave theory}. 
\emng
\eentry

\bentry
\word{unduly} 
\pron{anfDUyxli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} nAyxyavalalxda riVtiyalilx; anucitavAgi. 
\num{2} anAvashayxkavAgi; agatayxvilalxde: \eng{unduly pessimistic} anAvashayxkavAda nirAshAvAdadiMda. 
\num{3} vipariVtavAgi; mitimiVri. 
\enum
\emng
\eentry

\bentry
\word{undutiful} 
\pron{anfDUyxTiphulf}
\gl{\gu}
\expl{}
\bmng
 kataRvayxvesagada; kataRvayx pAlisada; kataRvayxBarxSaTx; kataRvayxhiVna. 
\emng
\eentry

\bentry
\word{undutifully} 
\pron{anfDUyxTiphuli}
\gl{\kirxvi}
\expl{}
\bmng
 kataRvayxvanunx pAlisade; kataRvayxBarxSaTxnAgi. 
\emng
\eentry

\bentry
\word{undutifulness} 
\pron{anfDUyxTiphulfnisf}
\gl{\nA}
\expl{}
\bmng
 kataRvayxBarxSaTxte; kataRvayxvesagadiruvike. 
\emng
\eentry

\bentry
\word{undyed} 
\pron{anfDeYDf}
\gl{\gu}
\bmng
 baNaNxhAkilalxda; raMgu hacacxda. 
\emng
\eentry

\bentry
\word{undying} 
\pron{anfDeYiMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} maraNavilalxda; sAvilalxda. 
\num{2} amaravAda; shAshavxta; aMtayxvilalxda: \eng{undying love} amara, ciraMtana -- perxVma. 
\enum
\emng
\eentry

\bentry
\word{undyingly} 
\pron{anfDeYiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sAvilalxdaMte; maraNarahitavAgi. 
\num{2} amaravAgi; shAshavxtavAgi. 
\enum
\emng
\eentry

\bentry
\word{unearned} 
\pron{anfanfDxR}
\gl{\gu}
\bmng
 duDidu saMpAdisidadxlalxda; anajiRta. 
\emng
\eentry

\bentry
\wordnospeech{unearned income}{unearned income}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 anajiRta varamAna; (kUli, saMbaLa, shulakx, \mo vugaLige seVrada) baDiDx \mo vugaLiMda baruva varamAna. 
\emng
\eentry

\bentry
\wordnospeech{unearned increment}{unearned increment}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 anajiRta -- baDitx, hecacxLa; tananx duDime yA hecicxna baMDavALa hUDike \mo\ bAhayx kAraNagaLilalxde Erida jamiVnu, Asitx, \mo vugaLa bele Erike. 
\emng
\eentry

\bentry
\word{unearth} 
\pron{anatfR}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (nari \mo vanunx) ikekxyiMda, biladiMda -- horaDisu. 
\num{2} (BUmiyanunx) agedu tege. 
\num{3} beLakige, bayalige -- taru; horageDavu; horahAku: \eng{unearth new facts about Shakespeare} SeVkfsxpiyarfna bagegx hosa viSayagaLanunx beLakige taru. 
\enum
\emng
\eentry

\bentry
\word{unearthliness} 
\pron{anatfRlinisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} alwkikate; atiBwtikate. 
\num{2} rahasayxmayate. 
\enum
\emng
\eentry

\bentry
\word{unearthly} 
\pron{anatfRli}
\gl{\gu}
\bmng
\bnum
\num{1} alwkika; I loVkadadxlalxda; nisagARtiVta; aBwma. 
\num{2} atirahasayxda; nigUDhavAda. 
\numi{3} (\AmA) 
\banum
\alnum{a} aveVLeya; hotitxlalxda hotitxna: \eng{getting up at this unearthly hour?} I aveVLeyalilx, hotitxlalxda hotitxnalilx ELuvudeV? 
\alnum{b} ananukUlada; aswkayaRda. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{unease} 
\pron{aniVsfZ}
\gl{\nA}
\expl{}
\bmng
 irusumurusu; ahita; kelxVsha; aswKayx; mujugara; kaLavaLa; AtaMka. 
\emng
\eentry

\bentry
\word{uneasily} 
\pron{aniVsiZli}
\gl{\kirxvi}
\expl{}
\bmng
 ahitakaravAgi; nemamxdiyilalxde; AtaMkadiMda; kaLavaLadiMda. 
\emng
\eentry

\bentry
\word{uneasiness} 
\pron{aniVsiZnisf}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{unease}{unease}. 
\emng
\eentry

\bentry
\word{uneasy} 
\pron{aniVsiZ}
\gl{\gu}
\expl{(\tara\ \eng{uneasier}, \tama\ \eng{uneasiest}). }
\bmng
\bnum
\num{1} (deVhadalAlxgali, manasisxnalAlxgali) ahitada; nemamxdiyilalxda; AtaMkada; kaLavaLada; mujugarada: \eng{passed an uneasy night} kaLavaLadiMda rAtirx kaLeda. 
\num{2} kaLavaLagoLisuva; koSxVBegoLisuva; talalxNagoLisuva: \eng{had an uneasy suspicion} talalxNagoLisuvaMtha saMshaya baMtu. 
\enum
\emng
\eentry

\bentry
\word{uneatable} 
\pron{aniVTabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ keTiTxruvudariMda) tinanxlAgada; tinanxluyoVgayxvalalxda; tinanxtakakxdadxlalxda; aBoVjayx. 
\emng
\eentry

\bentry
\word{uneaten} 
\pron{aniVTanf}
\gl{\gu}
\expl{}
\bmng
tinanxda; tinanxde uLida; aBukatx. 
\emng
\eentry

\bentry
\word{uneconomic} 
\pron{aniV(ne)kanAmikf}
\gl{\gu}
\expl{}
\bmng
(AthiRkavAgi) lABakaravalalxda; lABadAyakavAgi naDesalu sAdhayxvAgada. 
\emng
\eentry

\bentry
\word{uneconomical} 
\pron{aniV(ne)kanAmika(kf)lf}
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{uneconomic}{uneconomic}. 
\emng
\eentry

\bentry
\word{uneconomically} 
\pron{aniV(ne)kanAmikali}
\gl{\kirxvi}
\expl{}
\bmng
 (AthiRkavAgi) lABakaravAgilalxde. 
\emng
\eentry

\bentry
\word{unedifying} 
\pron{anfeDipheYiMgf}
\gl{\gu}
\expl{}
\bmng
 niVtikaravalalxda; \kanmu\ boVdhaparxdavalalxda yA neYtika unanxtige oyayxda. 
\emng
\eentry

\bentry
\word{unedifyingly} 
\pron{anfeDipheYiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
 niVtikaravAgirade yA boVdhaparxdavAgirade. 
\emng
\eentry

\bentry
\word{unedited} 
\pron{anfeDiTiDf}
\gl{\gu}
\bmng
 (mudarxNakekx modalu) sarinoVDi tidadxda; pariSakxrisada; saMpAdisada. 
\emng
\eentry

\bentry
\word{uneducable} 
\pron{anfeDuyx(ju)kaba(bf)lf}
\gl{\gu}
\expl{}
\bmng
ashikaSxNiVya; kalisalAgada; shikaSxNa koDalAgada. 
\emng
\eentry

\bentry
\word{uneducated} 
\pron{anfeDuyx(ju)keVTiDf}
\gl{\gu}
\expl{}
\bmng
 ashikiSxta; kalikeyilalxda; shikaSxNakokxLapaDada; \kanmu\ avidAyxvaMta. 
\emng
\eentry

\bentry
\word{unelectable} 
\pron{anilekaTxbalf}
\gl{\gu}
\expl{}
\bmng
 (pakaSx, umeVduvAra, \mo vara \vi) cunAyitanAgade hoVguva; cunAvaNeyalilx soVlu taruvaMtha, tarabahudAda aBipArxyagaLanunx hoMdiruva. 
\emng
\eentry

\bentry
\word{unembellished} 
\pron{aniMbeliSfTx}
\gl{\gu}
\expl{}
\bmng
 analaMkaqta; alaMkarisilalxda; siMgarisilalxda. 
\emng
\eentry

\bentry
\word{unemotional} 
\pron{animoVSanalf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BAvashUnayx; niBARvada; BAvarahita. 
\num{2} BAvAtireVkavilalxda; BAvoVdirxkatxvAgada; anudivxganx; atiBAvukavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unemotionally} 
\pron{animoVSanali}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} BAvashUnayxvAgi; niBARvadiMda. 
\num{2} BAvAtireVkavilalxde; BAvoVdirxkatxvAgade; anudivxganxvAgi. 
\enum
\emng
\eentry

\bentry
\word{unemphatic} 
\pron{aniMphAYxTikf}
\gl{\gu}
\expl{}
\bmng
 hecucx -- otitx heVLada, otutxhAkada; KaDAKaMDitavalalxda. 
\emng
\eentry

\bentry
\word{unemphatically} 
\pron{aniMphAYxTikali}
\gl{\kirxvi}
\expl{}
\bmng
 otitx heVLade; KaDAKaMDitavAgirade. 
\emng
\eentry

\bentry
\word{unemployability} 
\pron{aniMpAlxyabiliTi}
\gl{\nA}
\expl{}
\bmng
 (vayasusx \mo\ kAraNagaLiMda) nwkarige anahaRvAgiruvike; udoyxVgAnahaRte. 
\emng
\eentry

\bentry
\word{unemployable} 
\pron{aniMpAlxyabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 (anahaRte, vayasusx, \mo\ kAraNagaLiMda) nwkarige anahaRnAda; kelasakekx ahaRnalalxda; nwkarige lAyakakxlalxda. 
\emng
\eentry

\bentry
\word{unemployed} 
\pron{aniMpAlxyfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kelasavilalxda; udoyxVgavilalxda; nirudoyxVgiyAda. 
\num{2} upayoVgisilalxda; anupayukatx; toDagisilalxda: \eng{unemployed capital} upayoVgisilalxda baMDavALa. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
 \eng{the unemployed} (sadayxkekx) kelasavilalxda jana; nirudoyxVgigaLu. 
\emng
\eentry

\bentry
\word{unemployment} 
\pron{aniMpAlxyfmaMTf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nirudoyxVga. 
\num{2} (deVsha yA parxdeVshadalilxna) nirudoyxVgada sithxti, parxmANa yA maTaTx: \eng{the rural parts have higher unemployment} gArxmiVNa parxdeVshagaLalilx nirudoyxVgada parxmANa hecAcxgide. 
\enum
\emng
\eentry


\bentry 
\wordnospeech{unemployment benefit}{unemployment benefit}
\pron{?}
\gl{\nA}
\expl{}
\bmng
 nirudoyxVga sahAya; sakARravu nirudoyxVgige niVDuva darxvayx sahAya. 
\emng
\eentry

\bentry
\word{unenclosed} 
\pron{aninfkolxVsfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (goVDe, beVli, \mo vugaLiMda) sututxgaTaTxda; AvaqtavAgada; AvaraNavilalxda; beVli hAkada. 
\num{2} (peTiTxge \mo vugaLalilx) mucicxDada. 
\num{3} (patarx \mo vanunx) lakoVTeyalilxDada. 
\enum
\emng
\eentry

\bentry
\word{unencumbered} 
\pron{aninfkaMbaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (aDiDx, aDacaNe, AtaMkagaLa) horeyilalxda; BAra vilalxda. 
\num{2} (sithxrAsitx \mo vugaLa \vi) pUvARdhiyilalxda; sAla, BoVgayx, \mo vakekx IDAgirada. 
\enum
\emng
\eentry

\bentry
\word{unending} 
\pron{aneMDiMgf}
\gl{\gu}
\bmng
 koneyilalxda; niraMtara; aMtayxvilalxda; eMdeMdigU mugiyada. 
\emng
\eentry

\bentry
\word{unendingly} 
\pron{aneMDiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
 koneyilalxde; aMtayxvilalxde; niraMtaravAgi. 
\emng
\eentry

\bentry
\word{unendingness} 
\pron{aneMDiMgfnisf}
\gl{\nA}
\expl{}
\bmng
 niraMtarate; koneyilalxdiruvike; AnaMtayx. 
\emng
\eentry

\bentry
\word{unendowed} 
\pron{aniMDoVDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (Asitx \mo vugaLa \vi) baredukoDadiruva. 
\num{2} (janamxdatatxvAda yA deYvadatatxvAda) shakitx, kwshala, guNa, \mo vugaLanunx paDedirada. 
\enum
\emng
\eentry

\bentry
\word{unendurable} 
\pron{aniMDuyxarabflf}
\gl{\gu}
\expl{}
\bmng
taDeyalu kaSaTxvAda; asahaniVya; sahisalasAdhayxvAda. 
\emng
\eentry

\bentry
\word{unendurably} 
\pron{aniMDuyxarabilx}
\gl{\kirxvi}
\expl{}
\bmng
asahaniVyavAgi; taDeyalasAdhayxvAgi. 
\emng
\eentry

\bentry
\word{unengaged} 
\pron{aniMgeVjfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} toDagada; parxvaqtatxnAgada. 
\num{2} modaleV gotutxmADi koLaLxda. 
\num{3} (yudadhx \mo vugaLalilx) toDagada; seVrikoLaLxda; seVrisikoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{un-English} 
\pron{aniMgilxSf}
\gl{\gu}
\expl{}
\bmng
 iMgilxSalalxda: 
\banum
\alnum{a} AMgalxra riVti, lakaSxNa, jAyamAna alalxda. 
\alnum{b} rUDhiya iMgilxSf BASeya ucAcxraNe, vAyxkaraNa, parxyoVga alalxda: \eng{an un-English sentence} iMgilxSf rUDhiyadalalxda vAkayx. 
\eanum
\emng
\eentry

\bentry
\word{unenjoyable} 
\pron{aniMjAyabflf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMtoVSapaDalAgada; AnaMdisalAgada; BoVgisalAgada. 
\num{2} (Asitx \mo vugaLa \vi) anuBoVga paDeyalAgada; anuBavisalAgada. 
\enum
\emng
\eentry

\bentry
\word{unenlightened} 
\pron{aninfleYTanfDx}
\gl{\gu}
\expl{}
\bmng
 tiLivaLikeyilalxda; tiLivilalxda; aparxbudadhx. 
\emng
\eentry

\bentry
\word{unenterprising} 
\pron{anfenfTarfperxYsiZMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} udayxmashiVlanalalxda; udayxma, parxyatanx, kAyaR, \mo vugaLalilx toDagada. 
\num{2} sAhasiyalalxda; dheYyaRshAliyalalxda; mununxgagxda. 
\enum
\emng
\eentry

\bentry
\word{unenthusiastic} 
\pron{aninftUyx(tU)siZAYxsiTxkf}
\gl{\gu}
\expl{}
\bmng
 utAsxhavilalxda; utAsxha toVrisada; saMBarxma, saDagaragaLilalxda; nirutAsxhi. 
\emng
\eentry

\bentry
\word{unenthusiastically} 
\pron{aninftUyx(tU)siZAYxsiTxkali}
\gl{\kirxvi}
\expl{}
\bmng
utAsxhavilalxde; nirutAsxhiyAgi. 
\emng
\eentry

\bentry
\word{unenviable} 
\pron{anenivxabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 asUye huTiTxsada; asUyAjanakavalalxda; (inonxbabxru) asUyepaDabeVkAgilalxda. 
\emng
\eentry

\bentry
\word{unenviably} 
\pron{anenivxabilx}
\gl{\kirxvi}
\expl{}
\bmng
(inonxbabxru) asUyepaDadaMte; asUyAjanakavAgirade. 
\emng
\eentry

\bentry
\word{unenvied} 
\pron{anfenivxDf}
\gl{\gu}
\bmng
 (inonxbabxra) ISeRyxge, asUyege, hoTeTxkicicxge -- pAtarxnAgada, IDAgada, oLagAgada. 
\emng
\eentry

\bentry
\word{unequal} 
\pron{anfIkavxlf}
\gl{\gu}
\bmng
\bnum
\num{1} samanalalxda; asama; asamAna. 
\num{2} (\kanmu\ bareha \mo vugaLa \vi) vayxtAyxsavAgutatx hoVguva; EkariVtiyAgirada. 
\num{3} (eraDu pakaSx \mo vugaLige) oMdeV samanAda anukUlavilalxda; sarijoVDiyalalxda; sarisamAnavalalxda: \eng{unequal contest} sarisamAnavalalxda paMdayx. 
\num{4} (sAkaSuTx) shakitx, jANatana, \mo vugaLu ilalxdiruva: \eng{I feel unequal to the task} A kelasakekx beVkAguva shakitx nananxlilalx. 
\enum
\emng
\eentry

\bentry
\word{unequalise} 
\pron{aniVkavxleYsfZ}
\gl{\sakirx}
\expl{}
\bmng
 = \hyperlink{unequalize}{unequalize}. 
\emng
\eentry

\bentry
\word{unequalize} 
\pron{aniVkavxleYsfZ}
\gl{\sakirx}
\expl{}
\bmng
 (oMdanunx inonxMdakekx) asamavAgirisu; sarisamavAgadaMte mADu; samiVkaraNavAgadaMte niyoVjisu. 
\emng
\eentry

\bentry
\word{unequalled} 
\pron{anfIkavxlfDx}
\gl{\gu}
\expl{}
\bmng
 sarigaTaTxda; sarisamAnavilalxda; asamAna; anupama. 
\emng
\eentry

\bentry
\word{unequally} 
\pron{aniVkavxli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} asamavAgi. 
\num{2} vayxtAyxsavAgutatx. 
\enum
\emng
\eentry

\bentry
\word{unequivocal} 
\pron{anikivxvoVkalf}
\gl{\gu}
\expl{}
\bmng
 nisasxMdigadhxvAda; visapxSaTxvAda; tapupxkalapxnege avakAshavilalxda. 
\emng
\eentry

\bentry
\word{unequivocally} 
\pron{anikivxvoVkali}
\gl{\kirxvi}
\expl{}
\bmng
 nisasxMdigadhxvAgi; visapxSaTxvAgi; tapupxgarxhikege avakAshavilalxdaMte. 
\emng
\eentry

\bentry
\word{unequivocalness} 
\pron{anikivxvoVkalfnisf}
\gl{\nA}
\expl{}
\bmng
 nisasxMdigadhxte; visapxSaTxte. 
\emng
\eentry

\bentry
\word{unerring} 
\pron{anferiMgf}
\gl{\gu}
\bmng
\bnum
\num{1} kuriteVTina; guritapapxda. 
\num{2} tapupxmADada; tapipxhoVgada. 
\num{3} sariyAda; KacitavAda; niSakxqqSaTxvAda. 
\enum
\emng
\eentry

\bentry
\word{unerringly} 
\pron{anferiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} guritapapxde. 
\num{2} tapupx mADade; tapipxhoVgada hAge. 
\num{3} niSakxqqSaTxvAgi. 
\enum
\emng
\eentry

\bentry
\word{unerringness} 
\pron{anferiMgfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} guritapapxdiruvike; 
\num{2} tapupx mADadiruvike. 
\num{3} sariyAgiruvudu; niSakxqqSaTxte. 
\enum
\emng
\eentry

\bentry
\word{unescapable} 
\pron{anisekxVpabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 = \hyperref{kandict_i.pdf}{I}{inescapable}{inescapable}. 
\emng
\eentry

\bentry
\word{UNESCO} 
\pron{yUnesokxV}
\gl{\saMkiSx}
\expl{}
\bmng
(\eng{unesco} eMdU \parx). \eng{United Nations Educational, Scientific and Cultural Organization. } 
\emng
\eentry

\bentry
\word{unescorted} 
\pron{anisAkxTiRDf}
\gl{\gu}
\expl{}
\bmng
 beMgAvalilalxda; jotegAvalilalxda. 
\emng
\eentry

\bentry
\word{unespied} 
\pron{anisepxYDf}
\gl{\gu}
\expl{}
\bmng
 kaNiNxge biVLada; noVDada; agoVcara; kaMDuhiDiyada. 
\emng
\eentry

\bentry
\word[unessential(1)]{unessential} 
\pron{anisenaSxlf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anAvashayxkavAda; Avashayxkavalalxda. 
\num{2} parxdhAnavalalxda; muKayxvalalxda; amuKayx; gwNa. 
\enum
\emng
\eentry

\bentry
\word[unessential(2)]{unessential} 
\pron{anisenaSxlf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anAvashayxkavAdudu; Avashayxkavalalxdudu. 
\num{2} amuKayx; parxdhAnavalalxda, gwNa, muKayxvalalxda -- vasutx, vicAra, \mo vu. 
\enum
\emng
\eentry

\bentry
\word{unestablished} 
\pron{anisATxYxbilxSfTx} 
\gl{\gu}
\expl{}
\bmng
 sAthxpitavAgada; sithxravAgi -- nilalxda, nililxsada; sithxragoLisada. 
\emng
\eentry

\bentry
\word{unethical} 
\pron{anfetikalf}
\gl{\gu}
\expl{}
\bmng
 (vAyxpAra, vaqtitx, \mo vugaLalilx) aneYtika; niVtivirudadhx; niVtigeTaTx; niVtiniSeThxgaLilalxda. 
\emng
\eentry

\bentry
\word{unethically} 
\pron{anfetikali}
\gl{\kirxvi}
\expl{}
\bmng
 niVtigeTuTx; niVtiniSeThxgaLilalxdaMte. 
\emng
\eentry

\bentry
\word{unevangelical} 
\pron{aniVvAYxMjilika(kf)lf}
\gl{\gu}
\expl{}
\bmng
 suvAteR yA kerxYsatxmata dhamaRboVdhe \mo vugaLige virudadhxvAda. 
\emng
\eentry

\bentry
\word{uneven} 
\pron{aniVva(vf)nf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oMdeV maTaTxvAgilalxda; ErupeVrina. 
\num{2} samateyilalxda; viSama; vayxtAyxsavAguva; EkariVtiyadalalxda. 
\num{3} (paMdayx \mo vugaLa \vi) sarisamAnavalalxda; joVDiyalalxda. 
\enum
\emng
\eentry

\bentry
\word{unevenly} 
\pron{aniVvanfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} oMdeV maTaTxvAgirade; ErupeVrAgi. 
\num{2} samateyilalxdaMte; vayxtAyxsavAgutatx. 
\num{3} sarisamAnavAgirade. 
\enum
\emng
\eentry

\bentry
\word{unevenness} 
\pron{aniVvanfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oMdeV maTaTxvAgilalxdiruvudu; ErupeVru. 
\num{2} asamate; viSamate; EkariVtiyAgilalxdiruvudu. 
\num{3} (paMdayx \mo vugaLa \vi) asamAnate. 
\enum
\emng
\eentry

\bentry
\word{uneventful} 
\pron{aniveMTfphulf}
\gl{\gu}
\expl{}
\bmng
 visheVSa GaTanegaLilalxda; mahatatxvXda saMgatigaLilalxda. 
\emng
\eentry

\bentry
\word{uneventfully} 
\pron{aniveMTfphuli}
\gl{\kirxvi}
\expl{}
\bmng
 visheVSa GaTanegaLu, mahatatxvXda saMgatigaLu ilalxde. 
\emng
\eentry

\bentry
\word{uneventfulness} 
\pron{aniveMTfphulfnisf}
\gl{\nA}
\expl{}
\bmng
 visheVSa GaTanegaLu, mahatatxvXda saMgatigaLu ilalxdiruvike. 
\emng
\eentry

\bentry
\word{unexamined} 
\pron{anigfsAZYxminfDx}
\gl{\gu}
\expl{}
\bmng
 apariVkiSxta; ashoVdhita; pariVkiSxsada; taniKe mADada. 
\emng
\eentry

\bentry
\word{unexampled} 
\pron{anigfsAZMpa(pf)lfDx}
\gl{\gu}
\expl{}
\bmng
 anAyxdaqshavAda; eNeyilalxda; asadaqsha: \eng{the unexampled heroism of our soldiers} namamx seYnikara anAyxdaqsha shwyaR. 
\emng
\eentry

\bentry
\word{unexcelled} 
\pron{anekesxlfDx}
\gl{\gu}
\expl{}
\bmng
 miVrisada; anatishayita. 
\emng
\eentry

\bentry
\word{unexceptionable} 
\pron{anikesxpaSxnabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tapupx kaMDuhiDiyalAgada; niduRSaTxvAda; nirAkeSxVpaNiVyavAda. 
\num{2} atayxMta, pUtiR -taqpitxkaravAda. 
\enum
\emng
\eentry

\bentry
\word{unexceptionableness} 
\pron{anikesxpaSxnabflfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} tapupx kaMDuhiDiyalAgadiruvike; doVSarAhitayx; niduRSaTxte. 
\num{2} pUtiR taqpitxkaravAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{unexceptionably} 
\pron{anikesxpaSxnabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} tapupx kaMDuhiDiyalAgada riVtiyalilx; niduRSaTxvAgi; nirAkeSxVpaNiVyavAgi. 
\num{2} atayxMta taqpitxdAyakavAgi; pUtiR taqpitxkaravAgi. 
\enum
\emng
\eentry

\bentry
\word{unexceptional} 
\pron{anikesxpaSxna(nf)lf}
\gl{\gu}
\expl{}
\bmng
 asAdhAraNavalalxda; vADikeya; rUDhiya; sAdhAraNavAda; sAmAnayxvAda. 
\emng
\eentry

\bentry
\word{unexceptionally} 
\pron{anikesxpaSxnali}
\gl{\kirxvi}
\expl{}
\bmng
 asAdhAraNavAgirade; sAmAnayx rUDhiyadAdxgi; vADikeyadAdxgi. 
\emng
\eentry

\bentry
\word{unexcitability} 
\pron{anikesxYTabiliTi}
\gl{\nA}
\expl{}
\bmng
 anudevxVgakarate; keraLisalAgadiruvike; uderxVka goLisalAgadiruvike. 
\emng
\eentry

\bentry
\word{unexcitable} 
\pron{anikesxYTabflf}
\gl{\gu}
\expl{}
\bmng
 uderxVkagoLisalAgada; udivxganxgoLisalAgada; keraLisalAgada. 
\emng
\eentry

\bentry
\word{unexciting} 
\pron{anikesxYTiMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (manoVBAva, budidhxshakitx, \mo vugaLanunx) parxcoVdisada; uderxVkisada; utetxVjisada; keraLisada. 
\num{2} niVrasavAda; sapepxyAda. 
\enum
\emng
\eentry

\bentry
\word{unexecuted} 
\pron{anfekisxkUyxTiDf}
\gl{\gu}
\expl{}
\bmng
 \eng{(} Ajecnx, udedxVsha, \mo vanunx) naDesada; neraveVrisada; kAyaRgatagoLisada; jArimADada. 
\emng
\eentry

\bentry
\word{unexemplified} 
\pron{anigfseZMpilxpheYDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} udAharaNe, daqSATxMta -- koDada. 
\num{2} udAharaNeyAgirada; daqSATxMtavAgirada. 
\enum
\emng
\eentry

\bentry
\word{unexhausted} 
\pron{anigfsAZsiTxDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} shakitx kaLeyada; AyAsa paDisada. 
\num{2} (pAterx \mo vugaLalilxruvudanunx) KAli, baridu -mADada; akaSxya. 
\num{3} (oMdu viSayavAgi) tiLiyabeVkAdudanenxlAlx kaMDuhiDiyada; heVLibiDada; buDamaTaTx shoVdhisada. 
\num{4} (shakitx, saMpatutxgaLa \vi) basidubiDada; pUtiR tiVrisibiDada, mugisada. 
\enum
\emng
\eentry

\bentry
\word{unexpected} 
\pron{anikfsepxkiTxDf} 
\gl{\gu}
\expl{}
\bmng
 aniriVkiSxta; AshacxyaRkara. 
\emng
\eentry

\bentry
\word{unexpectedly} 
\pron{anikfsepxkiTxDfli}
\gl{\kirxvi}
\expl{}
\bmng
 aniriVkiSxtavAgi; AshacxyaRkaravAgi. 
\emng
\eentry

\bentry
\word{unexpectedness} 
\pron{anikfsepxkiTxDfnisf}
\gl{\nA}
\expl{}
\bmng
 aniriVkiSxtavAgiruvike. 
\emng
\eentry

\bentry
\word{unexpiated} 
\pron{anfekfsxpiETiDf}
\gl{\gu}
\expl{}
\bmng
 pArxyashicxtatx mADikoLaLxda; parihAra mADikoLaLxda. 
\emng
\eentry

\bentry
\word{unexpired} 
\pron{anikfsxpeYaDfR}
\gl{\gu}
\expl{}
\bmng
 mugiyada; tiVrada; aMtayxvAgada. 
\emng
\eentry

\bentry
\word{unexplainable} 
\pron{anikfsxpelxVnabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 = \hyperref{kandict_i.pdf}{I}{inexplicable}{inexplicable}. 
\emng
\eentry

\bentry
\word{unexplainably} 
\pron{anikfsxpelxVnabilx}
\gl{\kirxvi}
\expl{}
\bmng
 = \hyperref{kandict_i.pdf}{I}{inexplicably}{inexplicably}. 
\emng
\eentry

\bentry
\word{unexplained} 
\pron{anikfsxpelxVnfDx}
\gl{\gu}
\expl{}
\bmng
 vivarisada; vivaraNe niVDada; samajAyiSi koDada. 
\emng
\eentry

\bentry
\word{unexploited} 
\pron{anikfsxpAlxyiTxDf}
\gl{\gu}
\expl{}
\bmng
 (mUlasaMpatutx \mo vugaLa \vi) baLasi koMDirada; upayoVgisikoMDirada. 
\emng
\eentry

\bentry
\word{unexplored} 
\pron{anikfsxpolxVDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ananevxVSita; anevxVSaNe mADada; huDuki noVDada. 
\num{2} pariVkiSxsada; parishoVdhisada. 
\enum
\emng
\eentry

\bentry
\word{unexposed} 
\pron{anikfsxpoVsfZDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (gALi, bisilu, \mo vugaLige) terediDada; oDaDxda. 
\num{2} (apAya \mo vakekx) oLapaDisada; guripaDisada. 
\num{3} horageDahada; bahiraMgagoLisada; parxkAsha mADada; beLakige tarada; bayalu mADada. 
\enum
\emng
\eentry

\bentry
\word{unexpressed} 
\pron{anikfsxperxsfTx}
\gl{\gu}
\expl{}
\bmng
 aparxkaTita; vayxkatxpaDisada; aBivayxkitxgoLisada; horageDavada: \eng{unexpressed fears} aparxkaTita BayagaLu. 
\emng
\eentry

\bentry
\word{unexpurgated} 
\pron{anekfsxpageRVTiDf}
\gl{\gu}
\expl{}
\bmng
 (garxMtha \mo vugaLa \vi) 
\banum
\alnum{a} AkeSxVpaNiVya BAgagaLanunx tegeduhAkada; doVSagaLanunx tidadxda; vajaRyxBAgagaLanunx tegeduhAki shudidhxVkarisada. 
\alnum{b} saMpUNaRvAda. 
\eanum
\emng
\eentry

\bentry
\word{unfaceable} 
\pron{anfpheVsaba(bf)lf}
\gl{\gu}
\expl{}
\bmng
 edurisalAgada; muKAmuKiyAgi nilalxlAgada. 
\emng
\eentry

\bentry
\word{unfading} 
\pron{anfpheVDiMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mAsada; maMkAgada; baNaNxhoVgada. 
\num{2} asapxSaTxvAgada. 
\enum
\emng
\eentry

\bentry
\word{unfailing} 
\pron{anfpheVliMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} viPalavAgada; tapapxda. 
\num{2} korateyAgada; kaDame biVLada. 
\num{3} niraMtaravAda; sadA kAlavU iruva; badalAyisada: \eng{his unfailing good humour} avana niraMtara hAsayxparxvaqtitx. 
\num{4} necicxkeya; vishavxsaniVya; daqDhaniSeThxya: \eng{an unfailing friend} necicxkeya senxVhita. 
\enum
\emng
\eentry

\bentry
\word{unfailingly} 
\pron{anfpheVliMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} viPalavAgadaMte; tapapxde. 
\num{2} korate biVLade. 
\num{3} sadA; niraMtaravAgi. 
\num{4} daqDhaniSeThxyiMda. 
\enum
\emng
\eentry

\bentry
\word{unfailingness} 
\pron{anfpheVliMgfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} viPalavAgadiruvike. 
\num{2} korate biVLadiruvike. 
\num{3} vishAvxsaniVyate. 
\num{4} daqDhaniSeThx. 
\enum
\emng
\eentry

\bentry
\word{unfair} 
\pron{anfpheVrf}
\gl{\gu}
\bmng
\bnum
\num{1} taravalalxda; anAyxyada; yukatxvAgilalxda; apArxmANikavAda: \eng{unfair advantage} anAyxyavAgi paDeda lABa, upayoVga. 
\num{2} pakaSxpAtada; niyamagaLige anusAravAgilalxda: \eng{unfair play} niyamAnusAravAgirada ATa. 
\enum
\emng
\eentry

\bentry
\word{unfairly} 
\pron{anfpheVrfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} ayukatxvAgi; anAyxyavAgi. 
\num{2} pakaSxpAtadiMda; niyamAnusAravAgirade. 
\enum
\emng
\eentry

\bentry
\word{unfairness} 
\pron{anfpheVrfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anAyxyavAda naDavaLike; apArxmANikate. 
\num{2} pakaSxpAta manoVBAva. 
\enum
\emng
\eentry

\bentry
\word{unfaithful} 
\pron{anfpheVtfphulf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} niSeThxyilalxda. 
\num{2} dorxVhabudidhxya; vaMcakatanada. 
\num{3} vayxBicArada; hAdarada. 
\enum
\emng
\eentry

\bentry
\word{unfaithfully} 
\pron{anfpheVtfphuli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} niSeThxyilalxde. 
\num{2} dorxVhiyAgi; vaMcakatanadiMda. 
\num{3} vayxBicAriyAgi; hAdara mADi. 
\enum
\emng
\eentry

\bentry
\word{unfaithfulness} 
\pron{anfpheVtfphulfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} niSeThxyilalxda naDate. 
\num{2} dorxVha; vaMcakatana. 
\num{3} vayxBicAra; hAdara. 
\enum
\emng
\eentry

\bentry
\word{unfaltering} 
\pron{anfphAlaTxriMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eDavada; mugagxrisada; tapapxDiyiDada; tatatxrisada; taDavarisada. 
\num{2} daqDhavAda; anumAnisada; sithxrasaMkalapxda. 
\enum
\emng
\eentry

\bentry
\word{unfalteringly} 
\pron{anfphAlaTxriMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} eDavade; mugagxrisade. 
\num{2} daqDhavAgi. 
\enum
\emng
\eentry

\bentry
\word{unfamiliar} 
\pron{anfphamilayxrf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aparicita; paricayavilalxda; gurutilalxda: \eng{that face is not unfamiliar to me} nanage A muKa aparacitaveVnU alalx; A muKavanunx elolxV noVDida hAgide. 
\num{2} paricayavilalxda; tiLiyada: \eng{he is still unfamiliar with this district} avaniginUnx I jilelxya paricayavilalx. 
\enum
\emng
\eentry

\bentry
\word{unfamiliarity} 
\pron{anfphamiliAYxriTi}
\gl{\nA}
\expl{}
\bmng
 aparicaya; paricayavilalxdiruvike; gotitxlalxdiruvike. 
\emng
\eentry

\bentry
\word{unfashionable} 
\pron{anfphAYxSanabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
phAYxSanf Agirada. 
\emng
\eentry

\bentry
\word{unfashionableness} 
\pron{anfphAYxSanabalfnisf}
\gl{\nA}
\expl{}
\bmng
phAyxSanf alalxdiruvike. 
\emng
\eentry

\bentry
\word{unfashionably} 
\pron{anfphAYxSanabilx}
\gl{\kirxvi}
\expl{}
\bmng
phAyxSanf Agirade. 
\emng
\eentry

\bentry
\word{unfashioned} 
\pron{anfphAYxSa(Sf)nfDx}
\gl{\gu}
\expl{}
\bmng
 rUpisirada; sUkatx rUpa koDada; takakx AkArakekx taMdilalxda. 
\emng
\eentry

\bentry
\word{unfasten} 
\pron{anfphAsa(sf)nf}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (aguLi, kokekx, \mo vanunx) kaLacu; tege. 
\num{2} aLaLxka mADu; saDilamADu. 
\num{3} (kaTuTx \mo vanunx) bicucx; tege. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
saDilavAgu; aLaLxkavAgu. 
\emng
\eentry

\bentry
\word{unfastened} 
\pron{anfphAsa(sf)nfDx}
\gl{\gu}
\expl{}
\bmng
 (kaTuTx, aguLi, \mo vanunx) kaLacida; bicicxda; saDilisida; aLaLxka mADida. 
\emng
\eentry

\bentry
\word{unfathered} 
\pron{anfphAdaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ajAcnxtapitaqka; taMde gotitxrada; jAraja; hAdarada. 
\num{2} mUla tiLiyada; ajAcnxta mUlada: \eng{unfathered rumours} ajAcnxtamUlada UhApoVhagaLu. 
\enum
\emng
\eentry

\bentry
\word{unfatherly} 
\pron{anfphAdarfli}
\gl{\gu}
\expl{}
\bmng
 apeYtaqka; pitaqsahajavalalxda; taMdege takukxdalalxda. 
\emng
\eentry

\bentry
\word{unfathomable} 
\pron{anfphAYxdamabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ALavariyalAgada; taLamuTaTxlAgada; ALa kANalAgada; agAdha; (ALa) aLeyalu sAdhayxvilalxda. 
\num{2} ALa tiLiyada; ariyalu sAdhayxvilalxdaSuTx vicitarxvU kilxSaTxvU Ada. 
\enum
\emng
\eentry

\bentry
\word{unfathomably} 
\pron{anfphAYxdamabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} ALavariyalAgadaSuTx; agAdhavAgi. 
\num{2} ALa tiLiyade; athaRmADikoLaLxlu sAdhayxvAgade. 
\enum
\emng
\eentry

\bentry
\word{unfathomed} 
\pron{anfphAYxdamfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (manuSayxna savxBAva \mo vugaLa \vi) ariyada; ajAcnxta; kaMDuhiDiyalAgada. 
\num{2} (aparAdha \mo vugaLa \vi) patetx hacacxlAgada; rahasayx BeVdisalAgada. 
\num{3} (samudarx \mo vugaLa \vi) ALavariyada; agAdha. 
\enum
\emng
\eentry

\bentry
\word{unfavorable} 
\pron{anfpheVvarabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 (\ame) = \hyperlink{unfavourable}{unfavourable}. 
\emng
\eentry

\bentry
\word{unfavorite} 
\pron{anfpheVvariTf}
\gl{\gu}
\expl{}
\bmng
 (\ame) = \hyperlink{unfavourite}{unfavourite}. 
\emng
\eentry

\bentry
\word{unfavourable} 
\pron{anfpheVvarabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anukUla manoVBAvavilalxda; pirxVtivishAvxsagaLilalxda; aparxsananx. 
\num{2} opipxge koDada; samamxti sUcisada. 
\num{3} ashuBa sUcaneyuLaLx; ashuBakara; amaMgaLakara; Baravase toVrisada. 
\num{4} sAdhakavalalxda; parxtikUlavAda. 
\enum
\emng
\eentry

\bentry
\word{unfavourableness} 
\pron{anfpheVvarabalfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ananukUla manoVBAva; aparxsananxte. 
\num{2} parxtikUlate; viroVdha. 
\enum
\emng
\eentry

\bentry
\word{unfavourably} 
\pron{anfpheVvarabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anukUla manoVBAvavilalxde; aparxsananxnAgi. 
\num{2} virudadhxvAgi; parxtikUlavAgi. 
\enum
\emng
\eentry

\bentry
\word{unfavourite} 
\pron{anfpheVvariTf}
\gl{\gu}
\expl{}
\bmng
 (\AmA) apirxyanAda; kaMDarAgada; (yArU) iSaTxpaDadiruva; (yArigU) Agada. 
\emng
\eentry

\bentry
\word{unfazed} 
\pron{anfpheVsfDx}
\gl{\gu}
\expl{}
\bmng
(\AmA) 
\bnum
\num{1} keDisada; hALumADada. 
\num{2} goMdalakekx biVLada; galibiligoLaLxda; shAMta. 
\enum
\emng
\eentry

\bentry
\word{unfeasibility} 
\pron{anfphiVsiZbiliTi} 
\gl{\nA}
\expl{}
\bmng
kAyaRsAdhayxvalalxdiruvike; naDesalu ashakayxte. 
\emng
\eentry

\bentry
\word{unfeasible} 
\pron{anfphiVsiZbflf}
\gl{\gu}
\expl{}
\bmng
 kAyaRrUpakekx taralAgada; kAyaRsAdhayxvalalxda; shakayxvalalxda. 
\emng
\eentry

\bentry
\word{unfeasibly} 
\pron{anfphiVsiZbilx}
\gl{\kirxvi}
\expl{}
\bmng
 kAyaRsAdhayxvalalxde; naDesalu Agade. 
\emng
\eentry

\bentry
\word{unfeather} 
\pron{anfphedarf}
\gl{\sakirx}
\expl{}
\bmng
 (hakikxya \vi) gari -- katatxrisu, udurisu. 
\emng
\eentry

\bentry
\word{unfeathered} 
\pron{anfphedarfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} (hakikxya \vi) 
\banum
\alnum{a} gariyilalxda; garigaLiMda -- mucacxda, Avarisada. 
\alnum{b} gariyilalxda jAtige seVrida. 
\eanum
\numie
\num{2} (bANada \vi) gariyiMda alaMkarisada; gari joVDisada, kaTaTxda: \eng{unfeathered arrow} gari kaTaTxda aMbu. 
\enum
\emng
\eentry

\bentry
\word{unfed} 
\pron{anfpheDf}
\gl{\gu}
\bmng
 AhAra koDada; hoTeTxge hAkada; AhAravilalxda. 
\emng
\eentry

\bentry
\word{unfeeling} 
\pron{anfphiVliMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMveVdaneyilalxda; saMveVdanA -- shUnayx, rahita; jaDa. 
\num{2} nidaRya; kUrxra; kaThiNa; kalelxdeya; manasusx karagada. 
\enum
\emng
\eentry

\bentry
\word{unfeelingly} 
\pron{anfphiVliMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} saMveVdaneyilalxde; saMveVdanAshUnayxvAgi; jaDateyiMda. 
\num{2} nidaRyavAgi; kUrxravAgi; kaThiNavAgi; kalelxdeyiMda; manasusx karagade. 
\enum
\emng
\eentry

\bentry
\word{unfeelingness} 
\pron{anfphiVliMgfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} saMveVdanArAhitayx; jaDatavx. 
\num{2} nidaRyate; kalelxde; kAThinayx; kUrxrate. 
\enum
\emng
\eentry

\bentry
\word{unfeigned} 
\pron{anfpheVnfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} naTaneyalalxda; soVgu hAkada; veVSa kaTaTxda. 
\num{2} mucucxmare ilalxda; pArxmANikavAda; nijavAda; sAcA; niSakxpaTa; akaqtirxma: \eng{he showed unfeigned satisfaction at the boy's success} bAlaka jayashAliyAdudara bagegx avanu nijavAda taqpitx sUcisida. 
\enum
\emng
\eentry

\bentry
\word{unfelt} 
\pron{anfphelfTx}
\gl{\gu}
\expl{}
\bmng
 manataTaTxda; anuBavisada. 
\emng
\eentry

\bentry
\word{unfeminine} 
\pron{anfphemininf}
\gl{\gu}
\expl{}
\bmng
 heNuNxtana, sitxrXVtavx -- ilalxda; heNiNxna (mAdaRva \mo) savxBAvavilalxda, lakaSxNagaLilalxda. 
\emng
\eentry

\bentry
\word{unfemininity} 
\pron{anfphemininiTi}
\gl{\nA}
\expl{}
\bmng
 sitxrXVtavxrAhitayx; heNuNxtana ilalxdiruvike; maqdutavx, mAdaRvate, \mo\ heNiNxna lakaSxNagaLilalxdiruvike. 
\emng
\eentry

\bentry
\word{unfenced} 
\pron{anfphenfsfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (beVli \mo) AvaraNavilalxda; beVliyilalxda; beVli hAkada. 
\num{2} arakiSxta; kApADada; rakaSxNeyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unfermented} 
\pron{anfphameRMTiDf}
\gl{\gu}
\expl{}
\bmng
 hudugeVLada; hudugu barada; kiNavxnagoLisada. 
\emng
\eentry

\bentry
\word{unfertilised} 
\pron{anfphaTiRleYsfZDx}
\gl{\gu}
\expl{}
\bmng
 = \hyperlink{unfertilized}{unfertilized}. 
\emng
\eentry

\bentry
\word{unfertilized} 
\pron{anfphaTiRleYsfZDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} PalavatAtxgisada. 
\num{2} gababxvAgisada; gaBaR mUDisada. 
\enum
\emng
\eentry

\bentry
\word{unfetter} 
\pron{anfpheTarf}
\gl{\sakirx}
\expl{}
\bmng
 keYbeVDi (\mo vanunx) tegeduhAku; saMkoVle kaLacu; vimukatxgoLisu. 
\emng
\eentry

\bentry
\word{unfettered} 
\pron{anfpheTaDfR}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} beVDi kaLacida; saMkoVle tegeda; vimukatxgoLisida. 
\num{2} savxtaMtarx; aniyaMtirxta; nibaRMdhagaLilalxda. 
\enum
\emng
\eentry

\bentry
\word{unfilial} 
\pron{anfphilialf}
\gl{\gu}
\expl{}
\bmng
 (taMdetAyaMdira bagegx) pirxVtAyxdaravilalxda; aputirxVya; putarx dhamaRvilalxda. 
\emng
\eentry

\bentry
\word{unfilled} 
\pron{anfphilfDx}
\gl{\gu}
\expl{}
\bmng
 tuMbada; BatiRyAgada; KAli biTiTxruva. 
\emng
\eentry

\bentry
\word{unfiltered} 
\pron{anfphilaTxDfR}
\gl{\gu}
\expl{}
\bmng
 shoVdhisada; soVsada; basidu tegeyada. 
\emng
\eentry

\bentry
\word{unfinished} 
\pron{anfphiniSfTx}
\gl{\gu}
\expl{}
\bmng
mugisada; mugiyada; apUNaR; pUtiR mADada; pUNaRgoLisada; pUNaRvAgirada. 
\emng
\eentry

\bentry
\word[unfit(1)]{unfit} 
\pron{anfphiTf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sarihoMdada; tagada; yukatxvalalxda; anucita; yoVgayxvalalxda. 
\num{2} anahaR; asamathaR: \eng{he is unfit for business} avanu vAyxpArada udayxmakekx ahaRnalalx. 
\enum
\emng
\eentry

\bentry
\word[unfit(2)]{unfit} 
\pron{anfphiTf}
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unfitted}; \vakaq\ \eng{unfitting}).}
\bmng
anahaRvAgisu; asaMgatavAgisu; hoMdadaMte mADu. 
\emng
\eentry

\bentry
\word{unfitly} 
\pron{anfphiTfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} hoMdadaMte; tagadaMte. 
\num{2} anucitavAgi; yoVgayxvalalxda hAge. 
\enum
\emng
\eentry

\bentry
\word{unfitness} 
\pron{anfphiTfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hoMdadiruvudu; takukxdalalxdiruvudu. 
\num{2} anahaRte; asAmathayxR. 
\enum
\emng
\eentry

\bentry
\word{unfitted} 
\pron{anfphiTiDf}
\gl{\gu} 
\bmng
\bnum
\num{1} = \hyperlink{unfit(1)}{$^1$unfit}. 
\num{2} joVDisilalxda; aLavaDisilalxda. 
\num{3} sajujx goLisilalxda; sajijxna vasutxgaLanunx odagisada, aLavaDisada. 
\enum
\emng
\eentry

\bentry
\word{unfitting} 
\pron{anfphiTiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sariyAgada; hoMdada. 
\num{2} anucitavAda. 
\enum
\emng
\eentry

\bentry
\word{unfittingly} 
\pron{anfphiTiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sarihoMdade; hoMdikeyAgade. 
\num{2} anucitavAgi. 
\enum
\emng
\eentry

\bentry
\word{unfix} 
\pron{anfphikfsx}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} aBadarx mADu. 
\num{2} (nATiruvudanunx) saDilisu; kaLacu; beVpaRDisu. 
\enum
\emng
\eentry

\bentry
\word{unfixed} 
\pron{anfphikfsxTf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nATisilalxda; bigisilalxda. 
\num{2} (manasusx \mo vugaLa \vi) vicalitagoMDa; adhiVravAda; neTiTxlalxda. 
\enum
\emng
\eentry

\bentry
\word{unflagging} 
\pron{anfphAlxYxgiMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} joVlu biVLada; saDilagoLaLxda. 
\num{2} shakitxguMdada; soragada; soVtu biVLada. 
\num{3} utAsxhahiVnavAgada; KinanxvAgada. 
\enum
\emng
\eentry

\bentry
\word{unflaggingly} 
\pron{anfphAlxYxgiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} shakitxguMdade. 
\num{2} utAsxha kaLedukoLaLxde; soVlade. 
\enum
\emng
\eentry

\bentry
\word{unflappability} 
\pron{anfphAlxYxpabiliTi}
\gl{\nA}
\expl{}
\bmng
 shAMtate; shAMtacitatxte; sethxYyaR; nishacxlate; niviRkAra sithxti. 
\emng
\eentry

\bentry
\word{unflappable} 
\pron{anfphAlxYxpabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
(\AmA) 
\bnum
\num{1} galibiligoLaLxda; goMdalagoLaLxda; koSxVBegoLaLxda. 
\num{2} (utakxTa sithxtiyalilx) nishacxlavAgiruva; sethxYyaRdiMdiruva; vicalitanAgada. 
\enum
\emng
\eentry

\bentry
\word{unflappably} 
\pron{anfphAlxYxpabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} galibiligoLaLxde; vicalitanAgade. 
\num{2} sethxYyaRdiMda; nishacxlavAgi. 
\enum
\emng
\eentry

\bentry
\word{unflattering} 
\pron{anfphAlxYxTariMgf}
\gl{\gu}
\expl{}
\bmng
 shAlxGayxvalalxda; hogaLikeyAgirada; parxshaMseyAgirada. 
\emng
\eentry

\bentry
\word{unflatteringly} 
\pron{anfphAlxYxTariMgfli}
\gl{\kirxvi}
\expl{}
\bmng
 hogaLade; shAlxGayxvAgirade; parxshaMsAtamxkavAgirade. 
\emng
\eentry

\bentry
\word{unflavoured} 
\pron{anfphelxVvaDfR}
\gl{\gu}
\expl{}
\bmng
suvAsane yA ruci ilalxda; (su)vAsane, ruci -- kaTaTxda. 
\emng
\eentry

\bentry
\word{unfledged} 
\pron{anfphelxjfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (hakikxya \vi) garimUDada; rekekx baliyada. 
\num{2} (vayxkitxya \vi) ananuBaviyAda; anuBavavilalxda; tiVrA eLasAda. 
\enum
\emng
\eentry

\bentry
\word{unfleshed} 
\pron{anfphelxSfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mAMsa tuMbilalxda; mAMsavilalxda. 
\num{2} mAMsa tegeduhAkida. 
\enum
\emng
\eentry

\bentry
\word{unflinching} 
\pron{anfphilxMciMgf}
\gl{\gu}
\expl{}
\bmng
 hiMdegeyada; hiMjariyada; himemxTaTxda; aLukada. 
\emng
\eentry

\bentry
\word{unflinchingly} 
\pron{anfphilxMciMgfli}
\gl{\kirxvi}
\expl{}
\bmng
 hiMdegeyade; hiMjariyade; aLukade. 
\emng
\eentry

\bentry
\word{unfocused} 
\pron{anfphoVkasfTx}
\gl{\gu}
\expl{}
\bmng
 \eng{(unfocussed} saha\eng{).}
\bnum
\num{1} (kaNuNx, masUra, \mo vugaLa \vi) nABiVkarisilalxda; hoMdike mADada. 
\num{2} (manasusx \mo vugaLa \vi) keVMdirxVkarisada; EkAgarxteyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unfold} 
\pron{anfphoVlfDx}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} maDikegaLanunx bicucx; padaragaLanunx tege; maDike teredu haravu. 
\num{2} vikAsagoLisu: \eng{the story unfolds itself} kathe vikAsagoLuLxtAtx hoVgutatxde. 
\num{3} (AloVcanegaLu, haMcike, \mo vanunx) bayalumADu; parxkaTagoLisu; horageDavu: \eng{she unfolded her plans} avaLu tananx yoVjanegaLanunx horageDavidaLu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
araLu; vikAsavAgu; bicicxko; tereduko. 
\emng
\eentry

\bentry
\word{unfoldment} 
\pron{anfphoVlfDxmaMTf}
\gl{\nA}
\expl{}
\bmng
 (\ame) vikasana; araLike; parxkAshana; horageDahuvike; teredu toVrisuvike; bicicxkoLuLxvike. 
\emng
\eentry

\bentry
\word{unforced} 
\pron{anfphoVsfRTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} balAtAkxra mADada; otAtxyapaDisada; balavaMtamADada; balaparxyoVga mADada; nibaRMdhisada; kaDADxya mADada. 
\num{2} sarAgavAda; saliVsAda. 
\num{3} sahajavAda; sAvxBAvikavAda. 
\num{4} aparxyatinxta; parxyatanxpaDade, parxyatanxvilalxde -mADida. 
\enum
\emng
\eentry

\bentry
\word{unforcedly} 
\pron{anfphoVsiRDfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} balAtAkxra mADade; otAtxya mADade; balavaMtapaDisade. 
\num{2} sarAgavAgi; saliVsAgi. 
\num{3} sahajavAgi; sAvxBAvikavAgi. 
\num{4} parxyatanxvilalxde; aparxyatanx(pUvaRka)vAgi. 
\enum
\emng
\eentry

\bentry
\word{unfordable} 
\pron{anfphoVDaRbf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 (nadi \mo vugaLa \vi) naDageyiMda yA vAhanadalilx dATihoVgalAgada; hAyalu kaSaTxvAda. 
\emng
\eentry

\bentry
\word{unforeseeable} 
\pron{anfphoVsiRVaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
 muMgANalAgada; niriVkiSxsalAgada; UhisalAgada. 
\emng
\eentry

\bentry
\word{unforeseen} 
\pron{anfphoVsiRVnf}
\gl{\gu}
\expl{}
\bmng
 muMgANalAgada; aniriVkiSxta; pUvaRBAviyAgi UhisalAgada. 
\emng
\eentry

\bentry
\word{unforetold} 
\pron{anfphoVToRVlfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} modaleV heVLalAgirada. 
\num{2} munfsUcaneyilalxda; aniriVkiSxtavAda; modaleV Uhisadidadx. 
\enum
\emng
\eentry

\bentry
\word{unforgettable} 
\pron{anfphageRTabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 mareyalAgada; avisamxraNiVya. 
\emng
\eentry

\bentry
\word{unforgettably} 
\pron{anfphageRTabilx}
\gl{\kirxvi}
\expl{}
\bmng
 avisamxraNiVyavAgi; mareyalAgadaMte. 
\emng
\eentry

\bentry
\word{unforgivable} 
\pron{anfphagiRvabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 kaSxmisalAgada; akaSxmayx; kaSxmAhaRvalalxda. 
\emng
\eentry

\bentry
\word{unforgivably} 
\pron{anfphagiRvabilx}
\gl{\kirxvi} 
\bmng
kaSxmisalAgadaMte; akaSxmayxvAgi. 
\emng
\eentry

\bentry
\word{unforgiven} 
\pron{anfphagiRva(vf)nf}
\gl{\gu} 
\bmng
 kaSxmisada. 
\emng
\eentry

\bentry
\word{unforgiving} 
\pron{anfphagiRviMgf}
\gl{\gu} 
\bmng
kaSxmisada. 
\emng
\eentry

\bentry
\word{unforgivingly} 
\pron{anfphagiRviMgfli}
\gl{\kirxvi} 
\bmng
kaSxmisade. 
\emng
\eentry

\bentry
\word{unforgivingness} 
\pron{anfphagiRviMgfnisf}
\gl{\nA} 
\bmng
 kaSxmisadiruvike. 
\emng
\eentry

\bentry
\word{unforgotten} 
\pron{anfphagARTanf} 
\gl{\gu}
\expl{}
\bmng
mareyada; avisamxqqta. 
\emng
\eentry

\bentry
\word{unformed} 
\pron{anfphAmfRDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asidadhx; aracita; rUpugoMDilalxda. 
\num{2} beLedilalxda; vikAsagoMDilalxda; aBivaqdidhxyAgada. 
\num{3} sariyAda yA suMdaravAda AkAravilalxda. 
\enum
\emng
\eentry

\bentry
\word{unformulated} 
\pron{anfphAmuyxRleVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sUtirxVkarisada; sUtarxrUpakekx iLisada, tarada. 
\num{2} nidiRSaTxvAgi -- heVLada, nirUpisada. 
\enum
\emng
\eentry

\bentry
\word{unforthcoming} 
\pron{anfphoVtfRkamiMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} odagibaradiruva; saMBavisadiruva. 
\numi{2} (vayxkitxya \vi) 
\banum
\alnum{a} bAyibiDada; mAhiti odagisada. 
\alnum{b} sahakarisada. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{unfortified} 
\pron{anfphATiRpheYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} koVTeyiMdAvaqtavAgada; sutatxlU koVTe ilalxda. 
\num{2} arakiSxta. 
\enum
\emng
\eentry

\bentry
\word[unfortunate(1)]{unfortunate} 
\pron{anfphATuyx(ca)RnaTf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} adaqSaTxhiVna; dwBARgayxda; duradaqSaTxda; dudeYRvi; natadaqSaTxda. 
\num{2} dudeYRvada; anathaRkAri; garxhacArada: \eng{an unfortunate turn of events} dudeYRvada GaTanAvaLigaLu. 
\num{3} shoVcaniVya; viSAdakara: \eng{an unfortunate remark} viSAdaniVyavAda mAtu. 
\enum
\emng
\eentry

\bentry
\word[unfortunate(2)]{unfortunate} 
\pron{anfphATuyx(ca)RnaTf}
\gl{\nA}
\expl{}
\bmng
 niBARgayx; dudeYRvi (vayxkitx); natadaqSaTx vayxkitx. 
\emng
\eentry

\bentry
\word{unfortunately} 
\pron{anfphATuyx(ca)RnaTfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} viSAdakaravAgi; shoVcaniVyavAgi. 
\num{2} duradaqSaTxvashAtf; dwBARgayxdiMda; durAdaqSaTxdiMda. 
\enum
\emng
\eentry

\bentry
\word{unfounded} 
\pron{anfphwMDiDf}
\gl{\gu}
\expl{}
\bmng
AdhAravilalxda; taLabuDavilalxda; aDipAyavilalxda; nirAdhAravAda: \eng{unfounded rumours} taLabuDavilalxda UhApoVhagaLu. \eng{unfounded hopes} nirAdhAravAda BaravasegaLu. 
\emng
\eentry

\bentry
\word{unfoundedly} 
\pron{anfphwMDiDfli}
\gl{\kirxvi}
\expl{}
\bmng
 AdhAravilalxde; nirAdhAravAgi.\eng{} 
\emng
\eentry

\bentry
\word{unfoundedness} 
\pron{anfphwMDiDfnisf}
\gl{\nA}
\expl{}
\bmng
nirAdhArate; apArxmANayx; AdhAravilalxdiruvike. 
\emng
\eentry

\bentry
\word{unframed} 
\pron{anfpherxVmfDx}
\gl{\gu}
\expl{}
\bmng
 (\kanmu\ citarxda \vi) cwkaTiTxlalxda; kaTuTxhAkilalxda.\eng{ } 
\emng
\eentry

\bentry
\word{unfreeze} 
\pron{anfphirxVsfZ}
\gl{\kirx}
\expl{(\BU\ \eng{unfroze,} \BUkaq\ \eng{unfrozen}). }


\noindent
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (hepupxgaTiTxruvudanunx) karagisu; niVrAgisu; darxviVkarisu. 
\num{2} nibaRMdhagaLanunx saDilisu yA tegeduhAku. 
\num{3} (Asitx, TheVvaNi haNa, \mo vanunx) nagadAgisu; baLasikoLuLxvaMthAgisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
karagu; niVrAgu; darxviVBavisu. 
\emng
\eentry

\bentry
\word{unfrequented} 
\pron{anfphirxkevxMTiDf}
\gl{\gu}
\expl{}
\bmng
padeVpadeV hoVgada; vADikeyAgi (AgAga) BeVTi mADada yA koDada. 
\emng
\eentry

\bentry
\word{unfriended} 
\pron{anfpherxMDiDf}
\gl{\gu}
\expl{}
\bmng
 (sAhitayxka) senxVhitarilalxda; keLeyilalxda. 
\emng
\eentry

\bentry
\word{unfriendliness} 
\pron{anfpherxMDilxnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} senxVhABAva; senxVharAhitayx; devxVSa; viroVdha. 
\num{2} ananukUlate; parxtikUlate. 
\enum
\emng
\eentry

\bentry
\word{unfriendly} 
\pron{anfpherxMDfli}
\gl{\gu}
\expl{(\tara\ \eng{unfriendlier}, \tama\ \eng{unfriendliest}).}
\bmng
\bnum
\num{1} senxVhaBAvavilalxda; sahaqdayateyilalxda. 
\num{2} viroVdhi; veYrada; devxVSaBAvada. 
\num{3} (saninxveVsha \mo vugaLa \vi) parxtikUlavAda; anukUlavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unfrock} 
\pron{anfphArxkf}
\gl{\sakirx}
\expl{}
\bmng
 = \hyperref{kandict_d.pdf}{D}{defrock}{defrock}. 
\emng
\eentry

\bentry
\word{unfroze} 
\pron{anfphorxVsfZ}
\gl{\kirx}
\expl{}
\bmng
 \eng{unfreeze} dhAtuvina \BUkaq\ rUpa. 
\emng
\eentry

\bentry
\word{unfrozen} 
\pron{anfphorxVsfZnf}
\gl{\kirx}
\expl{}
\bmng
 \eng{ufreeze} 
\emng
\eentry

\bentry
\word{unfruitful} 
\pron{anfphUrxTfphulf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} lABadAyakavalalxda; vayxthaR; niSaPxlavAda; udedxVshisida Pala, PalitAMsha, taqpitx, \mo vugaLanunx koDada. 
\num{2} goDuDx; baMjeyAda. 
\num{3} goDuDx; PalabiDada; haNuNx biDada. 
\enum
\emng
\eentry

\bentry
\word{unfruitfully} 
\pron{anfphUrxTfphuli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} niSaPxlavAgi; lABadAyakavalalxde; udedxVshisida Pala \mo vanunx koDade. 
\num{2} haNuNx biDade. 
\enum
\emng
\eentry

\bentry
\word{unfruitfulness} 
\pron{anfphUrxTfphulfnisf}
\gl{\nA}
\expl{}
\bmng
 niSaPxlate: 
\banum
\alnum{a} lABadAyakavalalxdiruvike. 
\alnum{b} haNuNx biDadiruvike; goDuDx. 
\eanum
\emng
\eentry

\bentry
\word{unfulfillable} 
\pron{anfphulfphilabflf}
\gl{\gu} 
\bmng
\bnum
\num{1} sAdhisalAgada; keYgUDalAgada. 
\num{2} pUreYsalAgada. 
\enum
\emng
\eentry

\bentry
\word{unfulfilled} 
\pron{anfphulfphilfDx}
\gl{\gu}
\bmng
\bnum
\num{1} ataqpatx; keYgUDada; taNiyada; taNisalAgada. 
\num{2} IDeVrada; pUreYsada; saPalavAgada. 
\enum
\emng
\eentry

\bentry
\word{unfunded} 
\pron{anfphaMDiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (sAlada \vi) (cAlitx sAlavanunx) kAyaM sAlavanAnxgi parivatiRsada. 
\num{2} (yoVjane \mo vugaLa \vi) haNa, baMDavALa -- odagisada. 
\enum
\emng
\eentry

\bentry
\word{unfunnily} 
\pron{anfphanili}
\gl{\kirxvi}
\expl{}
\bmng
 tamASeyAgirade; vinoVdakaravAgilalxde. 
\emng
\eentry

\bentry
\word{unfunniness} 
\pron{anfphaninisf}
\gl{\nA}
\expl{}
\bmng
 hAsayxkaravAgilalxdiruvike. 
\emng
\eentry

\bentry
\word{unfunny} 
\pron{anfphani}
\gl{\gu}
\expl{}
\bmng
 (udedxVshakekx virudadhxvAgi) tamASeyAgada; vinoVdakaravalalxda; hAsAyxtamxkavalalxda. 
\emng
\eentry

\bentry
\word{unfurl} 
\pron{anfphalfR}
\gl{\sakirx}
\expl{}
\bmng
 (haDagina paTa, Catirx, bAvuTa, \mo vanunx) bicicx haraDu; bicucx. 
\emng

\noindent
\gl{\akirx} 
\bmng
bicicx haraDiko; bicicxko. 
\emng
\eentry

\bentry
\word{unfurnished} 
\pron{anfphaniRSfTx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} odagisilalxda; sarabarAyi mADilalxda. 
\num{2} (piVThoVpasakxragaLiMda) sajujxgoLisilalxda; alaMkarisilalxda. 
\enum
\emng
\eentry

\bentry
\word{ungainliness} 
\pron{anfgeVnflinisf}
\gl{\nA}
\expl{}
\bmng
 (vayxkitx, pArxNi, \mo vugaLa calanavalanagaLa \vi) oDoDxDADxgiruvike; nayanAjUkilalxdiruvudu. 
\emng
\eentry

\bentry
\word{ungainly} 
\pron{anfgeVnfli}
\gl{\gu}
\expl{}
\bmng
 (vayxkitx, pArxNi, \mo vugaLa calane \mo vugaLa \vi) oDoDxDADxda; vikAravAda; nAjUkilalxda. 
\emng
\eentry

\bentry
\word{ungallant} 
\pron{anfgAYxlaMTf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} viVroVcitavalalxda. 
\num{2} swjanayxvilalxda; sajajxnikeyilalxda. 
\num{3} sitxrXVparAyaNanAgirada. 
\enum
\emng
\eentry

\bentry
\word{ungallantly} 
\pron{anfgAYxlaMTfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} viVroVcitavAgirade. 
\num{2} sajajxnikeyilalxde. 
\num{3} sitxrXVyaralilx Asakitx, swjanayx toVrade. 
\enum
\emng
\eentry

\bentry
\word{ungarbled} 
\pron{anfgAbfRlfDx}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (heVLike, saMgati, \mo vugaLa \vi) tiricada; virUpagoLisada; ABAsamADi nirUpisada. 
\num{2} (heVLike, saMgati, \mo vugaLiMda) (\kanmu\ apAthaR baruvaMte) Arisada; Ayekx mADi tegeyada. 
\enum
\emng
\eentry

\bentry
\word{ungenerous} 
\pron{anfjenarasf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} audAyaRvilalxda; jipuNatanada; udAravalalxda; dhArALavalalxda. 
\num{2} haqdayaveYshAlayxvilalxda; alapxtanada; saMkucita manasisxna. 
\enum
\emng
\eentry

\bentry
\word{ungenerously} 
\pron{anfjenarasfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} audAyaRvilalxde; jipuNatanadiMda. 
\num{2} saMkucita manasisxniMda; alapxtanadiMda. 
\enum
\emng
\eentry

\bentry
\word{ungenerousness} 
\pron{anfjenarasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} loVBa; jipuNatana. 
\num{2} anwdArayx; alapxtana; saMkucita manasusx. 
\enum
\emng
\eentry

\bentry
\word{ungenial} 
\pron{anfjiVnialf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hitakaravalalxda; lavalavikeyilalxda; utAsxhakaravalalxda; ulAlxsavilalxda; gelavilalxda. 
\num{2} swjanayxvilalxda; senxVhaparavalalxda. 
\num{3}ogagxda; anukUlavAgirada. 
\enum
\emng
\eentry

\bentry
\word{ungentle} 
\pron{anfjeMTf(Ta)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oraTAda; meduvalalxda; swmayxvalalxda; sAdhuvalalxda. 
\num{2} asaBayx; asaMsakxqqta. 
\enum
\emng
\eentry

\bentry
\word{ungentlemanliness} 
\pron{anfjeMTalfmanflinisf}
\gl{\nA}
\expl{}
\bmng
 asaMBAvitate; asaBayxte; sadagxqqhasathxnige salalxda riVti. 
\emng
\eentry

\bentry
\word{ungentlemanly} 
\pron{anfjeMTalfmanfli}
\gl{\gu}
\expl{}
\bmng
 asaMBAvita vataRneya; doDaDx manuSayxnige takukxdalalxda; sadagxqqhasathxnige salalxda riVtiya. 
\emng
\eentry

\bentry
\word{ungentleness} 
\pron{anfjeMTf(Ta)lfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oraTutana; nayavilalxdiruvike. 
\num{2} asaBayxte; asaMsakxqqtate. 
\enum
\emng
\eentry

\bentry
\word{ungently} 
\pron{anfjeMTilx}
\gl{\kirxvi}
\bmng
\bnum
\num{1} oraTAgi. 
\num{2} asaBayxvAgi. 
\enum
\emng
\eentry

\bentry
\word{unget-at-able} 
\pron{anfgeTAYxTabf(ba)lf}
\gl{\gu}
\expl{}
\bmng
 (\AmA) agamayx; apArxpayx; paDeyalAgada; sigada; eTukalAgada; (hoVgi) muTaTxlAgada; doreyada. 
\emng
\eentry

\bentry
\word{ungifted} 
\pron{anfgiphi TxDf}
\gl{\gu}
\expl{}
\bmng
 (sahajavAda) parxtiBeyilalxda; kwshalavilalxda. 
\emng
\eentry

\bentry
\word{ungild} 
\pron{anfgilfDx}
\gl{\akirx}
\expl{}
\bmng
 giliVTilalxdaMte mADu; thaLukilalxdaMte mADu. 
\emng
\eentry

\bentry
\word{ungimmicky} 
\pron{anfgimiki}
\gl{\gu}
\expl{}
\bmng
 thaLuku paLukilalxda; giliVTilalxda; ATa kaTaTxda; soVgu \mo vugaLilalxda. 
\emng
\eentry

\bentry
\word{ungird} 
\pron{anfgaDfR}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} naDupaTiTx \mo vanunx saDilisu, bicucx. 
\num{2} paTiTx \mo vanunx tegedu saDilamADu, aLaLxkamADu. 
\enum
\emng
\eentry

\bentry
\word{unglazed} 
\pron{anfgelxVsfZDx}
\gl{\gu}
\expl{}
\bmng
gAjumeY ilalxdaMte mADida; mirugilalxda; hoLapu, naya, \mo vugaLilalxda. 
\emng
\eentry

\bentry
\word{unglove} 
\pron{anfgalxvf}
\gl{\sakirx}
\expl{}
\bmng
 (keYyina) gavasu tege. 
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
keYgavasu tege. 
\emng
\eentry

\bentry
\word{ungloved} 
\pron{anfgalxvfDx}
\gl{\gu}
\expl{}
\bmng
 keYgavasilalxda; keYgavasu tegeduhAkida. 
\emng
\eentry

\bentry
\word{ungodliness} 
\pron{anfgADflinisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anAcAra; adhAmiRkate. 
\num{2} deYvaniSeThxyilalxdiruvike; nAsitxkate. 
\enum
\emng
\eentry

\bentry
\word{ungodly} 
\pron{anfgADfli}
\gl{\gu}
\bmng
\bnum
\num{1} deYvaniSeThxyilalxda; dhamaRsharxdedhxyilalxda. 
\num{2} anAcArada; adhAmiRka. 
\num{3} duSaTx; niVca; BarxSaTx. 
\num{4} (\AmA) GoVra; ati; mitimiVrida; ugarx; tiVvarx; asahaniVya: \eng{an ungodly hour} vipariVta aveVLe hotutx. 
\enum
\emng
\eentry

\bentry
\word{ungovernability} 
\pron{anfgavanaRbiliTi}
\gl{\nA}
\expl{}
\bmng
 aMkeyalilxDalAgadiruvike; aniyaMtarxNiVyate. 
\emng
\eentry

\bentry
\word{ungovernable} 
\pron{anfgavanaRbf(ba)lf}
\gl{\gu}
\expl{}
\bmng
aMkeyalilxDalAgada; hatoVTi miVrida; sevxVcACxcArada; savxtaMtarx naDavaLikeya: \eng{ungovernable passions} hatoVTi miVrida rAgadevxVSagaLu. 
\emng
\eentry

\bentry
\word{ungovernably} 
\pron{anfgavanaRbilx}
\gl{\kirxvi}
\expl{}
\bmng
aMkeyalilxDalAgade; hatoVTi miVri; sevxVcACxcAradiMda. 
\emng
\eentry

\bentry
\word{ungraceful} 
\pron{anfgerxVsfphulf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (naDavaLike, mAtu, \mo vugaLalilx) aucitayxvilalxda; gAMBiVyaRvilalxda; mayARde miVrida; oDoDxDADxda; oraTAda. 
\num{2} sogasilalxda; nayanAjUkilalxda. 
\num{3} avalakaSxNavAda; kurUpada. 
\enum
\emng
\eentry

\bentry
\word{ungracefully} 
\pron{anfgerxVsfphuli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aucitayx, mayARde -- miVri; oraTAgi. 
\num{2} avalakaSxNavAgi. 
\enum
\emng
\eentry

\bentry
\word{ungracefulness} 
\pron{anfgerxVsfphulfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anwcitayx; gAMBiVyaRvilalxdiruvike. 
\num{2} avalakaSxNa(vAgiruvike). 
\enum
\emng
\eentry

\bentry
\word{ungracious} 
\pron{anfgerxVSasf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anAdaraNeya; swjanayxvilalxda. 
\num{2} kanikaravilalxda; nidaRya.
\num{3} suMdaravalalxda; AkaSiRsada. 
\enum
\emng
\eentry

\bentry
\word{ungraciously} 
\pron{anfgerxVSasfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anAdaraNeyiMda; swjanayxvilalxde. 
\num{2} kanikaravilalxde; nidaRyavAgi. 
\enum
\emng
\eentry

\bentry
\word{ungraciousness} 
\pron{anfgerxVSasfnisf}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anAdaraNeyiMdiruvike; aswjanayx. 
\num{2} kanikaravilalxdiruvudu; nidaRya vataRne. 
\enum
\emng
\eentry

\bentry
\word{ungrammatical} 
\pron{anfgarxmAYxTika(kf)lf}
\gl{\gu}
\expl{}
\bmng
 vAyxkaraNavirudadhx; vAyxkaraNasamamxtavalalxda. 
\emng
\eentry

\bentry
\word{ungrammaticality} 
\pron{anfgarxmAYxTikAYxliTi}
\gl{\nA}
\expl{}
\bmng
 vAyxkaraNavirudadhxte. 
\emng
\eentry

\bentry
\word{ungrammatically} 
\pron{anfgarxmAYxTikali} 
\gl{\kirxvi}
\expl{}
\bmng
 vAyxkaraNavirudadhxvAgi; vAyxkaraNasamamxtavalalxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{ungrammaticalness} 
\pron{anfgarxmAYxTikalfnisf}
\gl{\nA}
\expl{}
\bmng
 = \hyperlink{ungrammaticality}{ungrammaticality}. 
\emng
\eentry

\bentry
\word{ungrateful} 
\pron{anfgerxVTf{phu}lf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaqtaGanx; kaqtajacnxteyilalxda. 
\num{2} hitavalalxda; ahitakara; hiDisada; rucisada: \eng{ungrateful task} ahitakaravAda kAyaR. 
\enum
\emng
\eentry

\bentry
\word{ungratefully} 
\pron{anfgerxVTfphuli}
\gl{\kirxvi}
\expl{}
\bmng
 kaqtaGanxnAgi; kaqtajacnxteyilalxde. 
\emng
\eentry

\bentry
\word{ungratefulness} 
\pron{anfgerxVTfphulfnisf}
\gl{\nA}
\expl{}
\bmng
 kaqtaGanxte; kaqtajacnxteyilalxdiruvike. 
\emng
\eentry

\bentry
\word{ungrounded} 
\pron{anfgwrxMDiDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AdhAra, nele -- ilalxda; nirAdhAravAda yA samathaRneyilalxda; taLabuDavilalxda. 
\num{2} (\viduyx) BUsaMbaMdhavilalxda. 
\num{3} (viSaya \mo vugaLalilx) sariyAgi shikaSxNa koDada; ashikiSxta. 
\num{4} (vimAna, haDagu, \mo vugaLa \vi) nela kacacxda; nelakekx iLiyada; nelada meVlirada. 
\enum
\emng
\eentry

\bentry
\word{ungrudging} 
\pron{anfgarxjiMgf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asamAdhAna paDada; asamAdhAna toVrisada; asahaneyilalxda; asUyepaDada; haqtUpxvaRkavAda. 
\num{2} kApaRNayxvilalxda; keYhiDitamADada; koDugeYna; dhArALada. 
\enum
\emng
\eentry

\bentry
\word{ungrudgingly} 
\pron{anfgarxjiMgfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} asamAdhAnapaDade; asahaneyilalxde. 
\num{2} keYhiDitamADade; dhArALavAgi. 
\enum
\emng
\eentry

\bentry
\word{ungual} 
\pron{aMgavxlf}
\gl{\gu}
\bmng
\bnum
\num{1} uguru, paMja, gorasu, \mo vugaLige saMbaMdhisida. 
\num{2} uguru, paMja, gorasu -- uLaLx. 
\num{3} ugurina AkArada; paMjadaMtha; gorasinAkArada. 
\enum
\emng
\eentry

\bentry
\word{unguard} 
\pron{anfgADfR}
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} rakaSxNe tapipxsu; kAvalupaDe \mo vanunx tegeduhAku. 
\num{2} (isipxVTu) hecucx beleyiruva eleyanunx rakiSxsalu, uLisikoLaLxlu kaDame beleya eleyanunx -- tayxjisu, biTuTxkoDu, iLisu. 
\enum
\emng
\eentry

\bentry
\word{unguarded} 
\pron{anfgADiRDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} arakiSxtavAda; kAvalilalxda. 
\num{2} ecacxrike tapipxda; beVhuSArina; jAgarUkate vahisada: \eng{in an unguarded moment, he gave away most important secrets} yAvudoV beVhuSArina gaLigeyalilx avanu atayxMta muKayxvAda rahasayxgaLanunx biTuTxkoTaTxnu. 
\enum
\emng
\eentry

\bentry
\word{unguardedly} 
\pron{anfgADiRDfli}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} rakaSxNeyilalxde; kAvalilalxde. 
\num{2} ecacxrike tapipx; beVhuSArAgi; jAgarUkavAgirade. 
\enum
\emng
\eentry

\bentry
\word{unguardedness} 
\pron{anfgADiRDfnisf} 
\gl{\nA}
\expl{}
\bmng
 beVhuSAru; ecacxrageVDitana. 
\emng
\eentry

\bentry
\word{unguent} 
\pron{aMgavxMTf}
\gl{\nA}
\expl{}
\bmng
 aMjana(sAmagirx); leVpana; mulAmu; mulAmAgi yA GaSaRNe kaDime mADalu baLasuva maqduvasutx. 
\emng
\eentry

\bentry
\word{unguessable} 
\pron{anfgesaba(bf)lf}
\gl{\gu}
\expl{}
\bmng
 anUhayx; UhisalAgada; kalipxsikoLaLxlAgada. 
\emng
\eentry



\bentry
\word{unguiculate} 
\pron{aMgivxkuyxlaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx) uguruLaLx; naKavuLaLx; paMjAgaLuLaLx. 
\num{2} (\savi) (daLagaLa \vi) cikakx yA kiridAda buDavuLaLx. 
\enum
\emng
\eentry

\bentry
\word{unguided} 
\pron{anfgeYDiDf} 
\gl{\gu}
\expl{}
\bmng
anideRVshita; mAgaRdashaRnavilalxda; nidiRSaTx mAgaR yA dikukx vidhisada; savxMta dArige biTiTxruva; savxtaMtarxvAgi sAguva. 
\emng
\eentry

\bentry
\word{unguis} 
\pron{aMgivxsf} 
\gl{\nA}
\expl{(\bava\ \eng{ungues} \ucAcx-- aMgivxVsfZ) }
\bmng
naKa: 
\banum
\alnum{a} (\savi) puSapxdaLada kiridAda buDa. 
\alnum{b} (\pArxvi) uguru yA paMja. 
\eanum
\emng
\eentry

\bentry
\word{ungula} 
\pron{aMguyxl} 
\gl{\nA}
\expl{(\bava\ \eng{ungulae} \ucAcx-- aMguyxliV).}
\bmng
\bnum
\num{1} gorasu; shaPa. 
\num{2} moneyuguru; naKa; paMja. 
\enum
\emng
\eentry

\bentry
\word[ungulate(1)]{ungulate} 
\pron{aMguyxl(leV)Tf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gorasuLaLx; shaPavuLaLx. 
\num{2} gorasinAkArada. 
\enum
\emng
\eentry

\bentry
\word[ungulate(2)]{ungulate} 
\pron{aMguyxl(leV)Tf} 
\gl{\nA}
\expl{}
\bmng
shaPi; gorasuLaLx pArxNi. 
\emng
\eentry

\bentry
\word{ungum} 
\pron{anfgamf} 
\gl{\akirx}
\expl{}
\bmng
aMTisiruvudanunx biDisu; aMTisalu baLasida goVMdu \mo vanunx toLedubiDu, tegeduhAku. 
\emng
\eentry

\bentry
\word{unhackneyed} 
\pron{anfhAYxkinxDf} 
\gl{\gu}
\expl{}
\bmng
caviRta cavaRNavAgada; haLasalAgada; teVdu sameyada. 
\emng
\eentry

\bentry
\word{unhair} 
\pron{anfheVrf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} kUdalu tegeduhAku, katatxrisi hAku. 
\num{2} (maqgada camaR \mo vugaLiMda) kUdalanunx tege, here. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
kUdalilalxvAgu; boVLAgu. 
\emng
\eentry

\bentry
\word{unhallowed} 
\pron{anfhAYxloVDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pavitarxgoLisada; muDupAgiDada; apiRsada. 
\num{2} apavitarxvAda; kaluSitagoMDa. 
\num{3} pApapUrita; keVDina; niVca; duSaTx: \eng{with unhallowed joy} niVca saMtoVSadiMda. 
\enum
\emng
\eentry

\bentry
\word{unhampered} 
\pron{anfhAYxMpaDfR} 
\gl{\gu}
\expl{}
\bmng
aparxtibadadhxvAda; aDetaDeyilalxda; parxtibaMdhisada; aDacaNeyilalxda. 
\emng
\eentry

\bentry
\word{unhand} 
\pron{anfhAYxMDf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (\kAparx) keYtege: \eng{`unhand me, villain}!' \eng{she cried} nananxnunx biTuTxbiDu, naninxMda keYtege, niVca! eMdu avaLu kiricidaLu. 
\num{2} hiDita biDu; (hiDitadalilxruvudanunx) biDu, biTuTxbiDu. 
\enum
\emng
\eentry

\bentry
\word{unhandily} 
\pron{anfhAYxMDili} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (etatxlu, hiDiyalu) anuvAgiradaMte; sulaBa vAgilalxde. 
\num{2} oDoDxDADxgi; nAjUkilalxde. 
\enum
\emng
\eentry

\bentry
\word{unhandiness} 
\pron{anfhAYxMDinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (etatxlu, hiDiyalu) ananukUlate; anuvAgadiruvike; aswlaBayx; sulaBavAgilalxdiruvike. 
\num{2} oDoDxDuDx; nAjUkilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unhandsome} 
\pron{anfhAYxnasxmf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} rUpavilalxda; aMdaceMdavilalxda; avalakaSxNada; kurUpi. 
\num{2} anAdaraNeya; vinayavilalxda. 
\num{3} audAyaRvilalxda; jipuNatanada. 
\enum
\emng
\eentry

\bentry
\word{unhandy} 
\pron{anfhAYxMDi} 
\gl{\gu}
\bmng
\bnum
\num{1} (etatxlu, hiDiyalu) anuvAgilalxda; aDakavAgilalxda; sulaBavAgilalxda. 
\num{2} oDoDxDADxda; nAjUkilalxda. 
\num{3} keYkushalateyilalxda; keYcaLakavilalxda; keYgaLanunx baLasuvudaralilx neYpuNayxvilalxda. 
\enum
\emng
\eentry

\bentry
\word{unhang} 
\pron{anfhAYxMgf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unhung}). }
\bmng
neVtADuvudanunx, joVlADuvudanunx -- keLagiLisu. 
\emng
\eentry

\bentry
\word{unhanged} 
\pron{anfhAYxMgfDx} 
\gl{\gu}
\expl{}
\bmng
galulx tapipxsikoMDiruva; galAlxgadiruva; neVNu hAkadiruva: \eng{the greatest rascal unhanged} galulx tapipxsikoMDiruva BAri dagAkoVra. 
\emng
\eentry

\bentry
\word{unhappily} 
\pron{anfhAYxpili} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} suKavilalxde; saMtoVSavilalxde; nemamxdi kANade. 
\num{2} duradaqSaTxkaravAgi. 
\num{3} ashuBakaravAgi; anathaRkAriyAgi. 
\enum
\emng
\eentry

\bentry
\word{unhappiness} 
\pron{anfhAYxpinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} suKasaMtoVSarAhitayx; duHKasithxti. 
\num{2} adaqSaTxhiVnate; duradaqSaTxkara sithxti. 
\enum
\emng
\eentry

\bentry
\word{unhappy} 
\pron{anfhAYxpi} 
\gl{\gu}
\expl{(\tara\ \eng{unhappier}, \tama\ \eng{unhappiest}).}
\bmng
\bnum
\num{1} suKavilalxda; saMtoVSavilalxda; nemamxdi kANada. 
\num{2} duHKaBarita; duHKakara; goVLina; daridarx. 
\num{3} duradaqSaTxda; dudeYRvi; adaqSaTxhiVna: \eng{some unhappy mortal shut up in the box} saMdUka doLage sikikxbidadx yAvanoV dudeYRvi. 
\num{4} hiVna adaqSaTxvanunxMTumADuva; duradaqSaTxkara; ashuBakara: \eng{an unhappy propensity to drinking} kuDiyuva duradaqSaTxda caTa. 
\num{5} keVDina; keTaTx; vipatitxna; anathaRkAri; Apatitxna: \eng{born under an unhappy star} keTaTx nakaSxtarxdalilx huTiTx. 
\num{6} keLa; kiVLAda; hiVna: \eng{an unhappy social position} kiVLAda sAmAjika sAthxna. 
\num{7} ayashasivx; viPalavAda: \eng{unhappy attempts} viPala parxyatanxgaLu. 
\enum
\emng
\eentry

\bentry
\word{unharbour} 
\pron{anfhAbaRrf} 
\gl{\sakirx}
\expl{}
\bmng
(\birx) (jiMke \mo vanunx) podeyiMdAcege horaDisu, ODisu. 
\emng
\eentry

\bentry
\word{unharmed} 
\pron{anfhAmfDxR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} naSaTxkikxVDAgada; keVDige sikakxda; apAyakekx sikakxda. 
\num{2} surakiSxta; apAyakikxVDAgada. 
\enum
\emng
\eentry

\bentry
\word{unharmful} 
\pron{anfhAmfRPuZlf} 
\gl{\gu}
\expl{}
\bmng
hAni mADada; nirapAyakAri. 
\emng
\eentry

\bentry
\word{unharmonious} 
\pron{anfhAmoRVniasf} 
\gl{\gu}
\expl{}
\bmng
samarasavAgirada; susaMgatavAgilalxda; hoMdikeyAgada. 
\emng
\eentry

\bentry
\word{unharness} 
\pron{anfhAniRsf} 
\gl{\sakirx}
\expl{}
\bmng
(kudureya) sajujx, saraMjAmu -- kaLacu, tegeduhAku. 
\emng
\eentry

\bentry
\word{unhasp} 
\pron{anfhAsfpx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} cilaka, koMDi, \mo vanunx -- tege, kaLacu. 
\num{2} (dAra \mo vanunx) biDisu; bicucx. 
\enum
\emng
\eentry

\bentry
\word{unhatched} 
\pron{anfhAYxcfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (moTeTx \mo vugaLa \vi) mariyAgada; marimADada; moTeTxyiMda horage barada. 
\num{2} (haMcike, pitUri, \mo vugaLa \vi) yoVjisada; sidadhxgoLisada. 
\enum
\emng
\eentry

\bentry
\word{unhealthful} 
\pron{anfhelftxphulf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AroVgayxkaravalalxda; anAroVgayxda. 
\num{2} hitakaravalalxda. 
\enum
\emng
\eentry

\bentry
\word{unhealthfulness} 
\pron{anfhelftxphulfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anAroVgayx; AroVgayx sariyilalxdiruvike. 
\num{2} ahitakaravAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{unhealthily} 
\pron{anfhelitxli} 
\gl{\kirxvi}
\expl{}
\bmng
anAroVgayxkaravAgi; ahitavAgi (\rUpa\ saha). 
\emng
\eentry

\bentry
\word{unhealthiness} 
\pron{anfhelitxnisf} 
\gl{\nA}
\expl{}
\bmng
anAroVgayx sithxti; ahitakara sithxti. 
\emng
\eentry

\bentry
\word{unhealthy} 
\pron{anfhelitx} 
\gl{\gu}
\expl{(\tara\ \eng{unhealthier}, \tama\ \eng{unhealthiest}).}
\bmng
\bnum
\num{1} AroVgayx sithxtiyalilxlalxda; anAroVgayx sithxtiya. 
\numi{2} 
\banum
\alnum{a} AroVgayxkaravalalxda. 
\alnum{b} hitakaravalalxda. 
\eanum
\numie
\num{3} (sathxLa \mo vugaLa \vi) anAroVgayxkara; anAroVgayxvanunxMTumADuva. 
\num{4} keTaTx; kaluSitagoLisuva. 
\num{5} (\ashi) (jiVvakekx) apAyakaravAda. 
\enum
\emng
\eentry

\bentry
\word{unheard} 
\pron{anfhaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keVLilalxda; ashurxta; keVLi ariyada. 
\num{2} ajAcnxta; gotitxlalxda; tiLidilalxda: \eng{an unheard of artist} ajAcnxta kalAvida. 
\enum
\emng
\eentry

\bentry
\word{unheard-of} 
\pron{anfhaDfRAphf} 
\gl{\gu}
\expl{}
\bmng
\banum
\alnum{a} ashurxtapUvaR; hiMdeMdU keVLilalxdaMtha. 
\alnum{b} apUvaRvAda. 
\eanum
\emng
\eentry

\bentry
\word{unheated} 
\pron{anfhiVTiDf} 
\gl{\gu}
\bmng
bisimADada; atapatx; kAyisada; taNaNxneya. 
\emng
\eentry

\bentry
\word{unheeded} 
\pron{anfhiVDiDf} 
\gl{\gu}
\bmng
gamana koDada; lakaSxyX koDada; alakiSxta; upeVkiSxta. 
\emng
\eentry

\bentry
\word{unheedful} 
\pron{anfhiVDfphulf} 
\gl{\gu}
\expl{}
\bmng
gamana koDada; alakaSxyXda; nilaRkaSxyXda. 
\emng
\eentry

\bentry
\word{unheeding} 
\pron{anfhiVDiMgf} 
\gl{\gu}
\expl{}
\bmng
gamanisada; alakaSxyXdiMda kUDida: \eng{unheeding of his proffered aid} odagida neravina bagegx lakaSxyXvilalxda, gamana koDada. 
\emng
\eentry

\bentry
\word{unheedingly} 
\pron{anfhiVDiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
gamana koDade; alakaSxyXdiMda; upeVkeSxyiMda. 
\emng
\eentry

\bentry
\word{unhelpful} 
\pron{anfhelfpxphulf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sahAyakavalalxda; neravAgada; anukUlavAgada. 
\num{2} niSapxrXyoVjaka; upayoVgavAgada. 
\enum
\emng
\eentry

\bentry
\word{unhelpfully} 
\pron{anfhelfpxphuli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sahAyakavAgirade. 
\num{2} niSapxrXyoVjakavAgi; upayukatxvAgirade. 
\enum
\emng
\eentry

\bentry
\word{unhelpfulness} 
\pron{anfhelfpxphulfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} niSapxrXyoVjakate; anupayukatxte; upayoVgavAgadiruvike. 
\num{2} sahAyakavalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unheralded} 
\pron{anfheraliDxDf} 
\gl{\gu}
\expl{}
\bmng
(baravanunx) sArada; GoVSisada; sUcisada. 
\emng
\eentry

\bentry
\word{unheroic} 
\pron{anfhiVroVikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} viVroVcitavalalxda; viVranige takukxdalalxda. 
\num{2} viVralakaSxNa gaLilalxda. 
\num{3} (BASeya \vi) udAtatxvalalxda; pwrxDhavalalxda. 
\num{4} sAhasamayavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unheroically} 
\pron{anfhiVroVikali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} viVroVcitavalalxde. 
\num{2} viVralakaSxNagaLilalxde. 
\num{3} sAhasamayavAgirade. 
\enum
\emng
\eentry

\bentry
\word{unhesitating} 
\pron{anfhesiZTeVTiMgf} 
\gl{\gu}
\expl{}
\bmng
anishicxtavalalxda; shaMkisada; hiMjariyada; hiMdumuMdu noVDada. 
\emng
\eentry

\bentry
\word{unhesitatingly} 
\pron{anfhesiZTeVTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
nishicxtavAgi; hiMjariyade; hiMdumuMdu noVDade. 
\emng
\eentry

\bentry
\word{unhesitatingness} 
\pron{anfhesiZTeVTiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
nishicxtate; nishashxMke; hiMjariyadiruvike; hiMdumuMdu noVDadiruvike. 
\emng
\eentry

\bentry
\word{unhindered} 
\pron{anfhiMDaDfR} 
\gl{\gu}
\expl{}
\bmng
aparxtibadadhx; aDiDxpaDisada; taDeyirada; aDacaNeyilalxda. 
\emng
\eentry

\bentry
\word{unhinge} 
\pron{anfhiMjf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (kada \mo vanunx) kiVlukaLacu; kiVlu kaLaciDu; tiruguNiyiMda tegediDu. 
\num{2} (\rUpa, \kanmu\ \BUkaq dalilx) (budidhxyanunx) avayxvasethxgoLisu; BarxmegoLisu; vikalapxgoLisu: \eng{his mind is unhinged} avanige budidhxBarxmaNeyAgide. 
\enum
\emng
\eentry

\bentry
\word{unhistoric} 
\pron{anfhisATxrikf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unhistorical}{unhistorical}. 
\emng
\eentry

\bentry
\word{unhistorical} 
\pron{anfhisATxrikalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} cAritirxkavalalxda; aitihAsikavalalxda; itihAsakekx virudadhxvAda. 
\num{2} cariterxyalilx dAKalilalxda; cariterxyalilx naDeyada. 
\enum
\emng
\eentry

\bentry
\word{unhistorically} 
\pron{anfhisATxrikali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} cAritirxkavAgirade; aneYtihAsikavAgi. 
\num{2} cariterxyalilx dAKalAgirade. 
\enum
\emng
\eentry

\bentry
\word{unhitch} 
\pron{anfhicf} 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (kudure \mo vanunx) gADiyiMda bicucx, kaLacu. 
\num{2} (vasutx \mo vanunx kaTiTxniMda) kaLacu; bicucx; (koMDiyiMda) tege. 
\enum
\emng
\eentry

\bentry
\word{unholiness} 
\pron{anfhoVlinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} apavitarxte. 
\num{2} duSaTxtana; pApiSaThxte. 
\enum
\emng
\eentry

\bentry
\word{unholy} 
\pron{anfhoVli} 
\gl{\gu}
\expl{(\tara\ \eng{unholier}, \tama\ \eng{unholiest}).}
\bmng
\bnum
\num{1} pavitarxvalalxda; apavitarx. 
\num{2} duSaTx; pApakara. 
\num{3} (\AmA) BAri; tiVvarx; ugarx; GoVra; atiyAda; vipariVta: \eng{made an unholy row about it} adara bagegx atiyAda galATe, gadadxla mADida. 
\enum
\emng
\eentry

\bentry
\word{unhonoured} 
\pron{anfAnaDfR} 
\gl{\gu}
\expl{}
\bmng
gwravisada; mayARde paDeyada; agwravita. 
\emng
\eentry

\bentry
\word{unhook} 
\pron{anfhukf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (tagulisiruva) kokekxyiMda, horakekx tege; kokekx biDisi tege. 
\num{2} (uDupu \mo vanunx) kokekx kaLaci bicucx, haraDu. 
\enum
\emng
\eentry

\bentry
\word{unhoped} 
\pron{anfhoVpfTx} 
\gl{\gu}
\expl{}
\bmng
niriVkiSxsada; (EnAdarU Agutatxde eMba) Baravase, naMbike -- ilalxda. 
\emng
\eentry

\bentry
\word{unhorse} 
\pron{anfhAsfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} kudureyiMda keLakekx keDavu, keDavisu. 
\num{2} (kudureya \vi) savArananunx -- etitxhAku, keLagiLiyuvaMte mADu, keDavu. 
\num{3} (\rUpa) (padavi, adhikAra, \mo vugaLiMda) iLisu; uruLisu; padacuyxtigoLisu. 
\enum
\emng
\eentry

\bentry
\word{unhouse} 
\pron{anfhwsfZ} 
\gl{\akirx}
\expl{}
\bmng
Asharxya tapipxsu; manebiTuTx horaDisu; maneyiMda hora hAku. 
\emng
\eentry

\bentry
\word{unhouseled} 
\pron{anfhwsfZlfDx} 
\gl{\gu}
\expl{}
\bmng
(\kerxY) parxBuBoVjana saMsAkxra paDeyada; dhamaR saMsAkxra paDeyada. 
\emng
\eentry

\bentry
\word{unhuman} 
\pron{anfhUyxmanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} = \hyperref{kandict_i.pdf}{I}{inhuman}{inhuman}. 
\num{2} manuSAyxtiVta; atimAnuSa; divayx; manuSayxshakitxge miVrida. 
\num{3} manuSeyxVtara; manuSayxkulakekx seVrirada. 
\enum
\emng
\eentry

\bentry
\word[unhung(1)]{unhung} 
\pron{anfhaMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (kiTakiya parade \mo vanunx) iLibiDada; odagisada (Iga \viparx). 
\num{2} inUnx galilxgeVrisada. 
\num{3} (citarx \mo vugaLa \vi) inUnx neVtuhAkada; inUnx parxdashaRnakekx iDada. 
\enum
\emng
\eentry

\bentry
\word[unhung(2)]{unhung} 
\pron{anfhaMgf} 
\gl{\kirx}
\expl{}
\bmng
\eng{unhang} padada \BU\ matutx \BUkaq\ rUpa. 
\emng
\eentry

\bentry
\word{unhurried} 
\pron{anfhariDf} 
\gl{\gu}
\expl{}
\bmng
avasaravilalxda; Aturavilalxda; nidhAnavAda; tarAturi ilalxda. 
\emng
\eentry

\bentry
\word{unhurriedly} 
\pron{anfhariDilx} 
\gl{\kirxvi}
\expl{}
\bmng
avasaravilalxde; Aturavilalxde; nidhAnavAgi. 
\emng
\eentry

\bentry
\word{unhurt} 
\pron{anfhaTfR} 
\gl{\gu}
\bmng
\bnum
\num{1} gAyavAgada; peTuTx tagalada. 
\num{2} hAnigiVDAgada; toMdareyAgada. 
\num{3} vayxthe, noVvu, \mo vugaLige oLagAgada. 
\enum
\emng
\eentry

\bentry
\word{unhusk} 
\pron{anfhasfkx} 
\gl{\akirx}
\expl{}
\bmng
sipepx suli; hoTuTx tege. 
\emng
\eentry

\bentry
\word{unhygienic} 
\pron{anfheYjiVnikf} 
\gl{\gu}
\expl{}
\bmng
anAroVgayxkaravAda; AroVgayxkekx hAniyAguva. 
\emng
\eentry

\bentry
\word{unhygienically} 
\pron{anfheYjiVnikali} 
\gl{\kirxvi}
\expl{}
\bmng
anAroVgayxkaravAgi; AroVgayxkekx hAniyAguvaMte. 
\emng
\eentry

\bentry
\word{unhyphenated} 
\pron{anfheYphaneVTiDf} 
\gl{\gu}
\expl{}
\bmng
(padagaLanunx) kUDugereyiMda kUDisada. 
\emng
\eentry

\bentry
\word{uni} 
\pron{yUni} 
\gl{\nA}
\expl{(\bava\ \eng{unis}). }
\bmng
(\kanmu\ \AseTxrXV\ matutx nUsiZVlaMDf) vishavxvidAyxnilaya. 
\emng
\eentry

\bentry
\word{uni-} 
\pron{yUni-} 
\gl{\sapUpa}
\expl{}
\bmng
oMdu--, Eka-- eMba athaRda \sapUpa/ 
\emng
\eentry

\bentry
\word[Uniat(1)]{Uniat} 
\pron{yUniAYx(a)Tf} 
\gl{\gu}
\expl{}
\bmng
poVpfna sherxVSaThxteyanunx opipxkoMDu tananxdeV Ada ArAdhanA karxma \mo vanunx uLisikoMDiruva pUvaR yUroVpina samiVpa pArxcayxda yAvudeV oMdu kerxYsatx janAMgada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word[Uniat(2)]{Uniat} 
\pron{yUniAYx(a)Tf} 
\gl{\nA}
\expl{}
\bmng
yUniAYxTf kerxYsatx saMGadavanu. 
\emng
\eentry

\bentry
\word[Uniate(1)]{Uniate} 
\pron{yUniETf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{Uniat(1)}{$^1$Uniat}. 
\emng
\eentry

\bentry
\word[Uniate(2)]{Uniate} 
\pron{yUniETf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{Uniat(2)}{$^2$Uniat}. 
\emng
\eentry

\bentry
\word{uniaxial} 
\pron{yUniAYxkisxalf} 
\gl{\gu}
\expl{}
\bmng
oMdeV akaSxvuLaLx; EkAkiSxVya. 
\emng
\eentry

\bentry
\word{uniaxially} 
\pron{yUniAYxkisxali} 
\gl{\kirxvi}
\expl{}
\bmng
oMdeV akaSxvuLaLxMte; EkAkiSxVyavAgi. 
\emng
\eentry

\bentry
\word{unicameral} 
\pron{yUnikAYxmara(rf)lf} 
\gl{\gu}
\expl{}
\bmng
oMdeV saBeyuLaLx: \eng{a unicameral legislature} EkasaBA shAsanasaBe. 
\emng
\eentry

\bentry
\word{UNICEF} 
\pron{yUniseZphf} 
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Nations Children's (}hiMde \eng{Inter- national Children's Emergency) Fund.} 
\emng
\eentry

\bentry
\word{unicellular} 
\pron{yUniseluyxlarf} 
\gl{\gu}
\expl{}
\bmng
(\jiVvi) (aMga, Utaka, \mo vugaLa \vi) EkakoVshiVya; EkakaNada; oMdeV koVshavuLaLx yA oMdeV jiVvakaNaviruva. 
\emng
\eentry

\bentry
\word{unicolour} 
\pron{yUnikalarf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unicoloured}{unicoloured}. 
\emng
\eentry

\bentry 
\word{unicoloured} 
\pron{yUnikalaDfR} 
\gl{\gu}
\expl{}
\bmng
oMdeV baNaNxda; EkavaNiR. 
\emng
\eentry

\bentry
\word{unicorn} 
\pron{yUnikAnfR} 
\gl{\nA}
\bmng
\bnum
\numi{1} EkashaqMgi; okokxMbi: 
\banum
\alnum{a} kudureya meY hAgU haNeya meVle oMdeV neVra koMbu uLaLxdeMdu (girxVkf) purANaparxsidadhxvAda pArxNi. \imglink{unicorn 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.5cm {U_Pictures/unicorn.jpg}}} 
\alnum{b} (haLeya oDaMbaDike) eraDu koMbina pArxNi, pArxyashaH kADetutx. 
\alnum{c} tirucida koMbu, jiMkeya pAdagaLu, hoVtana gaDaDx hAgU siMhada bAlaviruva, birxTiSf rAjavaMsha lAMCanadalilx nirUpisalAgiruva, EkashaqMgiya citarx. 
\eanum
\numie
\numi{2} 
\banum
\alnum{a} muMdugaDe mUraneya kudure hUDiruva joVDi kudure. 
\alnum{b} iMtha kuduregaLa sajujx, gADi. 
\hypertarget{unicorn(3)}{} 
\eanum
\numie
\num{3} = \hyperref{kandict_n.pdf}{N}{narwhal}{narwhal}. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{sea-unicorn} = \hyperlink{unicorn(3)}{unicorn \((3)\)}. 
\emng
\eentry

\bentry
\wordnospeech{unicorn shell}{unicorn shell}
\pron{?} 
\gl{\nA}
\expl{}
\bmng
EkashaqMgi cipu px; cipipxna tuTiyalilx koMbinaMtha cAcuLaLx, samudarx udarapAdi. 
\emng
\eentry

\bentry
\wordnospeech{unicorn whale}{unicorn whale}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unicorn(3)}{unicorn \((3)\)}. 
\emng
\eentry

\bentry 
\word[unicuspid(1)]{unicuspid} 
\pron{yUnikasipxDf} 
\gl{\gu}
\expl{}
\bmng
oMdeV moneyuLaLx; EkAgarxda. 
\emng
\eentry

\bentry 
\word[unicuspid(2)]{unicuspid} 
\pron{yUnikasipxDf} 
\gl{\nA}
\expl{}
\bmng
EkAgarxdaMta; oMdeV moneyuLaLx halulx. 
\emng
\eentry

\bentry
\word{unicycle} 
\pron{yUniseYkalf} 
\gl{\nA}
\expl{}
\bmng
(sakaRsisxna doMbarATadavaru baLasuva) oMdu cakarxda seYkalulx; Ekacakirx.\imglink{unicycle:fig}{\raisebox{-0.15cm}[0pt][0pt] {\pdfimage width 0.4cm height 0.6cm {U_Pictures/unicycle.jpg}}} 
\emng
\eentry

\bentry
\word{unicyclist} 
\pron{yUniseYkilxsfTx} 
\gl{\nA}
\expl{}
\bmng
Ekacakarx seYkalf savAra. 
\emng
\eentry

\bentry
\word{unidea'd} 
\pron{aneYDiaDf} 
\gl{\gu}
\expl{}
\bmng
kalapxnAshUnayx; BAvanAshUnayx; yAvoMdu BAvane yA vicAra yA kalapxnegaLilalxda. 
\emng
\eentry

\bentry
\word{unideal} 
\pron{anfaiDialf} 
\gl{\gu}
\bmng
\bnum
\num{1} AdashaRpArxyavalalxda. 
\num{2} yAvoMdu AdashaRvanUnx anusarisada. 
\enum
\emng
\eentry

\bentry
\word{unidentifiable} 
\pron{aneYDeMTipheYaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
gurutisalAgada; kuruhu sikakxda. 
\emng
\eentry

\bentry
\word{unidentified} 
\pron{aneYDeMTipheYDf} 
\gl{\gu}
\expl{}
\bmng
gurutisada; gurutige sikakxda. 
\emng
\eentry

\bentry
\wordnospeech{unidentified flying object}{unidentified flying object}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperref{kandict_f.pdf}{F}{flying saucer}{flying saucer}. 
\emng
\eentry

\bentry
\word{unidimensional} 
\pron{yUniDeYmenaSxna(nf)lf} 
\gl{\gu}
\expl{}
\bmng
oMdeV AyAma uLaLx; EkAyAmada; Ekavimitiya. 
\emng
\eentry

\bentry
\word{unidirectional} 
\pron{yUniDi(DeY)rekaSxna(nf)lf} 
\gl{\gu}
\expl{}
\bmng
Ekadikikxna; EkamAgiR; oMdeV dikikxna calane, kirxyAgati, \mo vugaLanunxLaLx. 
\emng
\eentry

\bentry
\word{UNIDO} 
\pron{yUniDoV} 
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Nations Industrial Development Organization.} 
\emng
\eentry

\bentry
\word{unification} 
\pron{yUniphikeVSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
EkiVkaraNa; saMGaTane; oMdugUDisuvudu; aikayxgoLisuvudu; oTiTxge seVrisuvudu. 
\emng
\eentry

\bentry
\word{unificatory} 
\pron{yUniphikeVTari} 
\gl{\gu}
\expl{}
\bmng
oMdugUDisuva; EkiVkarisuva; oTiTxge seVrisuva; saMGaTanAtamxka. 
\emng
\eentry

\bentry
\wordnospeech{unified field theory}{unified field theory}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\Bwvi) EkiVkaqta keSxVtarx sidAdhxMta; sAmAnayxvAgi beVrebeVre sidAdhxMtagaLa neraviniMda vivarisalAguva nAlukx bageya aMtarakirxyegaLalilx (gurutavx, viduyxtAkxMtiVya, dubaRla matutx parxbala) eraDu athavA hecucx aMtarakirxyegaLanunx vivarisabalalx oMdeV oMdu sidAdhxMta. 
\emng
\eentry

\bentry
\word{unifier} 
\pron{yUnipheYarf} 
\gl{\nA}
\expl{}
\bmng
EkiVkaraNa mADuvavanu; saMGaTane mADuvavanu. 
\emng
\eentry

\bentry
\word{uniflow} 
\pron{yUnipholxV} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ hogeya yA upayoVgisida anilagaLa) harivu, parxvAha oMdeV dikikxnalilxruva; EkamuKada yA EkamuKiVya parxvAhada. 
\emng
\eentry

\bentry
\word[uniform(1)]{uniform} 
\pron{yUniphAmfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (rUpadalAlxgali, savxBAvadalAlxgali) EkariVtiya; EkaparxkArada; EkarUpada; badalAvaNeyilalxda: \eng{present a uniform appearance} EkaparxkAravAgi kANisutatx. 
\num{2} kAlakarxmadalilx badalAgada; oMdeV samanAda; EkaparxkArada: \eng{of uniform temperature} oMdeV samanAda shAKa. \eng{uniform acceleration} EkaparxkArada gativaqdidhx, veVgoVtakxSaR. 
\num{3} (terige, kAnUnu, \mo vugaLa \vi) kAladeVshagaLananxnusarisi badalAyisada; EkarUpada. 
\num{4} EkarUpada; oMdeV bageya; oMdeV maTaTx, niyama yA racanAvinAyxsavanunx anusarisuva. 
\enum
\emng
\eentry

\bentry
\word[uniform(2)]{uniform} 
\pron{yUniphAmfR} 
\gl{\nA}
\expl{}
\bmng
(sipAyigaLu, nAvikaru, poliVsinavaru, dAdiyaru, shAlAmakakxLu, \mo\ oMdeV daLadavarelalx hAkikoLuLxva) samavasatxrX; EkarUpada uDupu. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{in uniform} samavasatxrXdalilx \eng{: he looks handsome in uniform} avanu samavasatxrXdalilx cenAnxgi kANutAtxne. 
\emng
\eentry

\bentry
\word[uniform(3)]{uniform} 
\pron{yUniphAmfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} EkariVtiyAgisu; EkaparxkAravAgisu. 
\num{2} samavasatxrX hAku: \eng{a uniformed officer} samavasatxrX dharisida adhikAri. 
\enum
\emng
\eentry

\bentry
\word[uniformitarian(1)]{uniformitarian} 
\pron{yUniphAmiRTeVrianf} 
\gl{\gu}
\expl{}
\bmng
(\BUvi) EkarUpa parxkirxyeya; BUveYjAcnxnika parxkirxyegaLu satatavAgi matutx aviratavAgi parxyoVgavAgutitxruva balagaLa pariNAma eMba sidAdhxMtada, adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word[uniformitarian(2)]{uniformitarian} 
\pron{yUniphAmiRTeVrianf} 
\gl{\nA}
\expl{}
\bmng
(\BUvi) EkarUpaparxkirxyAvAdi; EkarUpa parxkirxyA sidAdhxMtada anuyAyi. 
\emng
\eentry

\bentry
\word{uniformitarianism} 
\pron{yUniphAmiRTeVrianisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
(\BUvi) EkarUpa parxkirxyA -- sidAdhxMta, vAda; BUveYjAcnxnika parxkirxyegaLu satatavAgi matutx aviratavAgi parxyoVgavAgutitxruva balagaLa pariNAma eMba sidAdhxMta yA vAda. 
\emng
\eentry

\bentry
\word{uniformity} 
\pron{yUniphAmiRTi} 
\gl{\nA}
\expl{(\bava\ \eng{uniformities}). }
\bmng
oMdeV samanAgiruvudu; EkariVtiyAgiruvudu; EkaparxkArate; EkarUpate; EkakarxmAnusaraNe. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{Act of Uniformity} (\ca) cacuRgaLa pArxthaRnAvidhigaLalilx EkarUpate taralu, sAmAnayx pArxthaRneya nidiRSaTx pusatxkavanunx baLasalu iMgelxMDinalilx 1662ralilx mADida shAsana, masUde. 
\emng
\eentry

\bentry
\word{uniformly} 
\pron{yUniphAmfRli} 
\gl{\kirxvi}
\expl{}
\bmng
EkariVtiyalilx; EkaparxkAravAgi; EkarUpadalilx; oMdeV samanAgi; yAvoMdu badalAvaNeyilalxde. 
\emng
\eentry

\bentry
\word{unify} 
\pron{yUnipheY} 
\gl{\akirx}
\expl{(vataRmAna \parxpu\ \Eva\ \eng{unifies}; \BU\ matutx \BUkaq}
\eng{unified}).  \bmng
\bnum
\num{1} oMdugUDisu; ogUgxDisu; oTiTxge seVrisu; oMdumADu; aikayxmADu. 
\num{2} EkaparxkAravAgisu; EkarUpakekx taru. 
\enum
\emng
\eentry

\bentry 
\word{unilateral} 
\pron{yUnilAYxTara(rf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} EkapakiSxVya; oMdeV pakaSxda; omamxgugxlina. 
\num{2} oMdeV pakaSx yA vayxkitxge saMbaMdhisida; oMdeV pakaSx yA vayxkitx keYkoMDa: \eng{unilateral declaration of independence} EkapakiSxVyavAda sAvxtaMtarxyX GoVSaNe. 
\num{3} (rasetxya badiyalilx vAhanAdigaLanunx nililxsuva \vi) rasetxya omamxgagxlige mAtarx niyamitavAda; oMdeV pakakxda. 
\num{4} (sasayxgaLa \vi) EkapAshaRvxka; EkapAshivxRVya; tALina oMdeV pakakxdiMda horaTiruva, oMdeV pakakxdalilxruva. 
\num{5} (saMtatiya vaMshAnukarxmada \vi) EkaliMgada; oMdeV liMgada pUviRkariMda baMda. 
\enum
\emng
\eentry

\bentry
\word{unilateralism} 
\pron{yUnilAYxTaralisaZ(sfZ)mf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} EkapakiSxVya nishashxsitxrXVkaraNa; oMdu pakaSx mAtarx shasatxrXgaLanunx tayxjisuvudu yA kaDime mADuvudu. 
\num{2} (\ame) EkapakiSxVya videVshAMga niVti; mitarxra sahAyavilalxde videVshAMga niVtiyanunx anusarisuvudu. 
\enum
\emng
\eentry

\bentry
\word[unilateralist(1)]{unilateralist} 
\pron{yUnilAYxTaralisfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} EkapakiSxVya nishashxsitxrXVkaraNavAdi. 
\num{2} (\ame) EkapakiSxVya videVshAMga niVtiya anuyAyi yA samathaRka. 
\enum
\emng
\eentry

\bentry
\word[unilateralist(2)]{unilateralist} 
\pron{yUnilAYxTaralisfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} EkapakiSxVya nishashxsitxrXVkaraNa -- vAdada yA vAdiya. 
\num{2} (\ame) EkapakiSxVya videVshAMga niVtiya yA niVtiya anuyAyiya. 
\enum
\emng
\eentry

\bentry
\word{unilaterally} 
\pron{yUnilAYxTarali} 
\gl{\kirxvi}
\expl{}
\bmng
EkapakiSxVyavAgi; oMdeV pakaSx yA vayxkitxge saMbaMdhisidaMte; omamxgugxlAgi. 
\emng
\eentry

\bentry
\word{unilingual} 
\pron{yUniliMgavxlf} 
\gl{\gu}
\expl{}
\bmng
oMdeV BASeya(lilxna); EkaBASika; oMdeV BASeya yA BASege saMbaMdhisida. 
\emng
\eentry

\bentry
\word{unilingually} 
\pron{yUniliMgavxli} 
\gl{\kirxvi}
\expl{}
\bmng
EkaBASikavAgi; oMdeV BASeyalilx yA oMdeV BASege saMbaMdhisidaMte. 
\emng
\eentry

\bentry
\word{uniliteral} 
\pron{yUniliTara(rf)lf} 
\gl{\gu}
\expl{}
\bmng
oMdeV akaSxra uLaLx; EkAkaSxrada. 
\emng
\eentry

\bentry
\word{unilluminated} 
\pron{anfilU(lUyx)mineVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} beLagisada; parxkAshisuvaMte mADirada. 
\num{2} jAcnxnoVdayavAgada; AtamxjAcnxnavAgada. 
\num{3} (viSayada \vi) vivarisada; vishadiVkarisada; sapxSaTxgoLisada. 
\enum
\emng
\eentry

\bentry
\word{unillustrated} 
\pron{anfilaseTxrXVTiDf} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ pusatxkada \vi) citarxgaLilalxda; nidashaRnakAkxgi citarxgaLanunx oLagoMDirada. 
\emng
\eentry

\bentry
\word{unilocular} 
\pron{yUnilAkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
(\savi\ matutx \pArxvi) oMdeV gUDuLaLx; EkakoVSaThxvuLaLx. 
\emng
\eentry

\bentry
\word{unimaginable} 
\pron{animAYxjinaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
Uhisalu kaSaTxvAda; UhisalAgada; UhAtiVta; kalapxnege miVrida; kalapxnAtiVta. 
\emng
\eentry

\bentry
\word{unimaginably} 
\pron{animAYxjinabilx} 
\gl{\kirxvi}
\expl{}
\bmng
kalapxnAtiVtavAgi; anUhayxvAgi. 
\emng
\eentry

\bentry
\word{unimaginative} 
\pron{animAYxjinaTivf} 
\gl{\gu}
\expl{}
\bmng
kalapxnAshakitxrahita; kalapxnAshUnayx; kalapxnAshakitxyilalxda yA adanunx upayoVgisada, toVrisada. 
\emng
\eentry

\bentry
\word{unimaginatively} 
\pron{animAYxjinaTivfli} 
\gl{\kirxvi}
\expl{}
\bmng
kalapxnAshakitxyilalxde; kalapxnAshUnayxvAgi. 
\emng
\eentry

\bentry
\word{unimaginativeness} 
\pron{animAYxjinaTivfnisf} 
\gl{\nA}
\expl{}
\bmng
kalapxnAshUnayxte; UhanAshakitx ilalxdiruvike. 
\emng
\eentry

\bentry
\word{unimpaired} 
\pron{aniMpeVDfR} 
\gl{\gu}
\expl{}
\bmng
kugagxda; tagagxda; naSaTxvAgada; dubaRlavAgada; shithilavAgada; hAniyAgada. 
\emng
\eentry

\bentry
\word{unimpassioned} 
\pron{aniMpAYxSanfDx} 
\gl{\gu}
\expl{}
\bmng
BAvAveVsharahita; rAgoVderxVkakekx sikakxda; uderxVkagoLaLxda. 
\emng
\eentry

\bentry
\word{unimpeachable} 
\pron{aniMpiVcaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AkeSxVpisalAgada; AroVpaNege avakAshavilalxda; nirAkeSxVpaNiVya; ApAdanege Asapxdavilalxda. 
\num{2} niduRSaTx; nisasxMdigadhx; Kacita; parxshinxsalu sAdhayxvilalxda; parxshAnxtiVta: \eng{unimpeachable honesty} niduRSaTxvAda pArxmANikate. \eng{news from an unimpeachable source} nisasxMdeVhavAda, Kacita -- mUladiMda baMda sudidx. 
\enum
\emng
\eentry

\bentry
\word{unimpeachably} 
\pron{aniMpiVcabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} AkeSxVpisalAgadaMte; ApAdanege AsapxdavilalxdaMte; alalxgaLeyalAgada riVtiyalilx. 
\num{2} parxshAnxtiVtavAgi; nisasxMdeVhavAgi. 
\enum
\emng
\eentry

\bentry
\word{unimpeded} 
\pron{aniMpiVDiDf} 
\gl{\gu}
\expl{}
\bmng
aparxtibadadhx; taDeyilalxda; aDiDxyilalxda; niroVdhisada; aDacaNeyilalxda; niSakxMTakavAda. 
\emng
\eentry

\bentry
\word{unimpededly} 
\pron{aniMpiVDiDilx} 
\gl{\kirxvi}
\expl{}
\bmng
taDeyilalxde; aDiDxyilalxde; niSakxMTakavAgi; aparxtibadadhxvAgi. 
\emng
\eentry

\bentry
\word{unimportance} 
\pron{aniMpAT(Tf)Rnfsx} 
\gl{\nA}
\expl{}
\bmng
amuKayxte; apArxdhAnayx; muKayxvAgilalxdiruvudu. 
\emng
\eentry

\bentry
\word{unimportant} 
\pron{aniMpATaRMTf} 
\gl{\gu}
\expl{}
\bmng
muKayxvalalxda; amuKayx; parxdhAnavalalxda. 
\emng
\eentry

\bentry
\word{unimposing} 
\pron{aniMpoVsiZMgf} 
\gl{\gu}
\expl{}
\bmng
parxBAva biVrada; parxBAvakAriyAgirada; pariNAmakArakavalalxda; aparxBAvi. 
\emng
\eentry

\bentry
\word{unimposingly} 
\pron{aniMpoVsiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
parxBAva biVrade; pariNAmakAriyAgirade. 
\emng
\eentry

\bentry
\word{unimpressed} 
\pron{aniMperxsfTx} 
\gl{\gu}
\expl{}
\bmng
parxBAvitavAgada; manasisxge taTaTxda. 
\emng
\eentry

\bentry
\word{unimpressionable} 
\pron{aniMperxSanabflf} 
\gl{\gu}
\expl{}
\bmng
parxBAvavashayxvalalxda; sulaBavAgi BAvanegaLu \mo vugaLa pariNAmakokxLagAgada. 
\emng
\eentry

\bentry
\word{unimpressive} 
\pron{aniMperxsivf} 
\gl{\gu}
\expl{}
\bmng
(BASe, vayxkitx, daqshayx, \mo vugaLa \vi) parxBAvakAriyalalxda; manasisxna meVle parxBAva biVrada; manataTaTxda; pariNAmakAriyalalxda. 
\emng
\eentry

\bentry
\word{unimpressively} 
\pron{aniMperxsivfli} 
\gl{\kirxvi}
\expl{}
\bmng
parxBAvashUnayxvAgi; manataTaTxde. 
\emng
\eentry

\bentry
\word{unimpressiveness} 
\pron{aniMperxsivfnisf} 
\gl{\nA}
\expl{}
\bmng
parxBAvashUnayxte; manataTaTxdiruvike; manasisxna meVle parxBAva biVradiruvike. 
\emng
\eentry

\bentry
\word{unimproved} 
\pron{aniMpUrxvfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} utatxmapaDisada; aBivaqdidhxgoLisada. 
\num{2} baLasada; upayoVgisada. 
\num{3} (BUmiya \vi) aBivaqdidhxpaDisilalxda; kaqSigAgali, kaTaTxDakAkxgali baLasada. 
\enum
\emng
\eentry

\bentry
\word{unimpugned} 
\pron{aniMpUyxnfDx} 
\gl{\gu}
\expl{}
\bmng
parxtiBaTisada; AkeSxVpisada; KaMDisada. 
\emng
\eentry

\bentry
\word{unincorporated} 
\pron{aninfkApaRreVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} EkiVBavisada; oMdAgada; oMdugUDada. 
\num{2} sAMsithxkavAgirada; saMGavAgi oMdugUDada; saMGaTitavAgada. 
\enum
\emng
\eentry

\bentry
\word{unindexed} 
\pron{aninfDekfsxTX} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (pusatxka) akArAdi -- sUciyilalxda, odagisada. 
\num{2} niSidadhx BAgagaLanunx tegedu hAkada. 
\enum
\emng
\eentry

\bentry
\word{uninfected} 
\pron{aninfphekiTxDf} 
\gl{\gu}
\expl{}
\bmng
soVMku -- tagulada, aMTada; roVgANugaLiMda kUDilalxda. 
\emng
\eentry

\bentry
\word{uninflamed} 
\pron{aninfphelxVmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (beMkiya \vi) uri hotatxda; uriyada. 
\num{2} keraLada; uderxVkagoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{uninflammable} 
\pron{aninfphAlxYxmaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sulaBavAgi hotitxkoLaLxda yA hotitxsalAgada. 
\num{2} sulaBavAgi keraLada yA uderxVkagoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{uninflected} 
\pron{aninfphelxkiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oLakekx -- bAgisada, bagigxsada, koMkisada. 
\num{2} (\vAyx) (BASeya \vi) viBakitx parxtayxyagaLanunx hacacxda. 
\num{3} (\saM) savxra -- Erisada yA iLisada. 
\num{4} badalAyisada; mApARDAgada. 
\enum
\emng
\eentry

\bentry
\word{uninfluenced} 
\pron{anfinfphulxanfsfTx} 
\gl{\gu}
\expl{}
\bmng
aparxBAvita; yAva parxBAvakUkx oLagAgada; yAra pArxbalayx, mahime, \mo vakUkx silukada. 
\emng
\eentry

\bentry
\word{uninfluential} 
\pron{aninfphulxenfSalf} 
\gl{\gu}
\expl{}
\bmng
parxBAvarahitavAda; parxBAva biVrada. 
\emng
\eentry

\bentry
\word{uninformative} 
\pron{aninfphAmaRTivf} 
\gl{\gu}
\expl{}
\bmng
(viSaya, samAcAra, \mo vanunx) tiLisada; heVLada; boVdhaparxdavalalxda. 
\emng
\eentry

\bentry
\word{uninformed} 
\pron{aninfphAmfRDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ajacnx; tiLiyada; anaBijacnx; arivilalxda: \eng{uninformed criticism} anaBijacnx -- vimasheR, TiVke. 
\num{2} tiLivaLikeyilalxda; ajAcnxni; ashikiSxta. 
\enum
\emng
\eentry

\bentry
\word{uninhabitable} 
\pron{aninfhAYxbiTabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
vAsAhaRvalalxda; vAsayoVgayxvalalxda; (janaru) vAsisalu kaSaTxvAda. 
\emng
\eentry

\bentry
\word{uninhabitableness} 
\pron{aninfhAYxbiTabalfnisf} 
\gl{\nA}
\expl{}
\bmng
vAsAnahaRte; vAsayoVgayxvalalxdiruvike. 
\emng
\eentry

\bentry
\word{uninhabited} 
\pron{aninfhAYxbiTiDf} 
\gl{\gu}
\expl{}
\bmng
nijaRna; janarilalxda; janavAsavilalxda. 
\emng
\eentry

\bentry
\word{uninhibited} 
\pron{aninfhibiTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (mADadaMte) nibaRMdhisada; niSeVdhisada; parxtibaMdhisada. 
\num{2} hiMjarikeyilalxda. 
\enum
\emng
\eentry

\bentry
\word{uninhibitedly} 
\pron{aninfhibiTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (mADadaMte) nibaRMdhisade; niSeVdhisade; parxtibaMdhisade. 
\num{2} hiMjarikeyilalxde. 
\enum
\emng
\eentry

\bentry
\word{uninhibitedness} 
\pron{aninfhibiTiDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (mADadaMte) aparxtibaMdha; aniSeVdha. 
\num{2} hiMjarikeyilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{uninitiated} 
\pron{aniniSiETiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AraMBisada; modalu mADada. 
\num{2} (saMGa, shAsatxrX, \mo vakekx vidhivatAtxgi saMsAkxra naDasi) seVrisada; parxveVshagoLisada; diVkeSx koDada; adiVkiSxta. 
\enum
\emng
\eentry

\bentry
\word{uninjured} 
\pron{anfinfjaDfR} 
\gl{\gu}
\expl{}
\bmng
GAsi hoMdada; hAnigoLagAgada; noVvu, gAya, bAdhe, \mo vugaLige oLagAgada. 
\emng
\eentry

\bentry
\word{uninspired} 
\pron{aninfsepxYaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sUPxtiRyilalxda; BAvAveVsha tuMbada. 
\num{2} (BASaNa \mo vu) sUPxtiRrahita; BAvoVdevxVgashUnayx. 
\num{3} niVrasa; sapepxyAda. 
\enum
\emng
\eentry

\bentry
\word{uninspiring} 
\pron{aninfsepxYariMgf} 
\gl{\gu}
\expl{}
\bmng
sUPxtiRdAyakavalalxda; niVrasavAda. 
\emng
\eentry

\bentry
\word{uninspiringly} 
\pron{aninfsepxYariMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
sUPxtiRdAyakavAgirade; niVrasavAgi. 
\emng
\eentry

\bentry
\word{uninstructed} 
\pron{anfinfsaTxrXkiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} boVdhaneyilalxda; shikaSxNavilalxda; kalikeyilalxda. 
\num{2} tiLivaLikeyilalxda; ajAcnxni. 
\num{3} mAhiti paDeyada, hoMdirada. 
\enum
\emng
\eentry

\bentry
\word{uninsurable} 
\pron{aninfSuarabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
vimAyoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{uninsured} 
\pron{aninfSuaDfR} 
\gl{\gu}
\expl{}
\bmng
vime mADirada; vimAshUnayx. 
\emng
\eentry

\bentry
\word{unintelligent} 
\pron{aninfTelijaMTf} 
\gl{\gu}
\expl{}
\bmng
budidhxshakitxyilalxda; hecucx arivanunx toVrada; curuku budidhxyilalxda. 
\emng
\eentry

\bentry
\word{unintelligently} 
\pron{aninfTelijaMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
budidhxshakitxyilalxde; curuku budidhxyilalxde. 
\emng
\eentry

\bentry
\word{unintelligibility} 
\pron{aninfTelijibiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} agArxhayxte; tiLiyalu kaSaTxvAgiruvike. 
\num{2} (\tashA) keVvala budidhxgArxhayxvalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unintelligible} 
\pron{aninfTelijibalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tiLiyalu kaSaTxvAda; garxhisuvudakAkxgada. 
\num{2} (\tashA) keVvala budidhxgArxhayxvalalxda; ajecnxVya. 
\enum
\emng
\eentry

\bentry
\word{unintelligibleness} 
\pron{aninfTelijibalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unintelligibility}{unintelligibility}. 
\emng
\eentry

\bentry
\word{unintelligibly} 
\pron{aninfTelijibilx} 
\gl{\kirxvi}
\expl{}
\bmng
agArxhayxvAgi; tiLiyalu kaSaTxvAgi. 
\emng
\eentry

\bentry
\word{unintended} 
\pron{aninfTeMDiDf} 
\gl{\gu}
\expl{}
\bmng
anudidxSaTx; asaMkalipxta; udedxVshisirada; saMkalipxsirada. 
\emng
\eentry

\bentry
\word{unintentional} 
\pron{aninfTenaSxna(nf)lf} 
\gl{\gu}
\expl{}
\bmng
udedxVshapUvaRkavalalxda; budidhxpUvaRkavalalxda. 
\emng
\eentry

\bentry
\word{unintentionally} 
\pron{aninfTenaSxnali} 
\gl{\kirxvi}
\expl{}
\bmng
udedxVshapUvaRkavalalxde; udedxVsharahitavAgi. 
\emng
\eentry

\bentry
\word{uninterested} 
\pron{aninfTarx(Tirx)siTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Asakitx toVrisada; nirAsakitxya. 
\num{2} tAtAsxraBAvada; audAsiVnayxda; asaDeDxya; upeVkeSxya. 
\enum
\emng
\eentry

\bentry
\word{uninterestedly} 
\pron{aninfTarx(Tirx)siTxDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} Asakitx toVrisade. 
\num{2} tAtAsxradiMda; audAsiVnayxdiMda; asaDeDxyiMda. 
\enum
\emng
\eentry

\bentry
\word{uninterestedness} 
\pron{aninfTarx(Tirx)siTxDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nirAsakitx; Asakitx toVrisadiruvudu. 
\num{2} tAtAsxra; audAsiVnayx; asaDeDx. 
\enum
\emng
\eentry

\bentry
\word{uninteresting} 
\pron{anfiMTarx(Tirx)siTxMgf} 
\gl{\gu}
\expl{}
\bmng
Asakitx huTiTxsada; sAvxrasayxvilalxda; kutUhala keraLisada; niVrasavAda. 
\emng
\eentry

\bentry
\word{uninterestingly} 
\pron{anfiMTarx(Tirx)siTxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
AsakitxrahitavAgi; sAvxrasayxvilalxde. 
\emng
\eentry

\bentry
\word{uninterestingness} 
\pron{anfiMTarx(Tirx)siTxMgfnisf} 
\gl{\nA}
\expl{}
\bmng
AsakitxrAhitayx; asAvxrasayx; sAvxrasayxvilalxdiruvike. 
\emng
\eentry

\bentry
\word{uninterpretable} 
\pron{aninfTapirxRTaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
avAyxKeyxVya; athaRvivaraNe mADalAgada; vivarisalAgada. 
\emng
\eentry

\bentry
\word{uninterruptable} 
\pron{aninfTarapaTxba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aDiDxpaDisalAgada; aDetaDeyuMTu mADalAgada. 
\num{2} avicePxVdayx; vicePxVda mADalAgada. 
\enum
\emng
\eentry

\bentry
\word{uninterrupted} 
\pron{aninfTarapiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aDetaDeyilalxda; aDiDxyilalxda. 
\num{2} aviciPxnanxvAda. 
\enum
\emng
\eentry

\bentry
\word{uninterruptedly} 
\pron{aninfTarapiTxDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aDetaDeyilalxde. 
\num{2} aviciPxnanxvAgi; niraMtaravAgi. 
\enum
\emng
\eentry

\bentry
\word{uninterruptedness} 
\pron{aninfTarapiTxDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aDetaDeyilalxdiruvike. 
\num{2} aviciPxnanxte. 
\enum
\emng
\eentry

\bentry
\word{uninucleate} 
\pron{yUninUyxkilxE(a)Tf} 
\gl{\gu}
\expl{}
\bmng
(\jiVvi) EkabiVjiVya; EkabiVjavuLaLx; oMdeV biVja yA nUyxkilxyasf iruva. 
\emng
\eentry

\bentry
\word{uninventive} 
\pron{aninfveMTivf} 
\gl{\gu}
\expl{}
\bmng
nimARNashiVlavalalxda; saqjanacaturateyilalxda. 
\emng
\eentry

\bentry
\word{uninventively} 
\pron{aninfveMTivfli} 
\gl{\kirxvi}
\expl{}
\bmng
nimARNa yA saqjanashakitxyilalxde. 
\emng
\eentry

\bentry
\word{uninventiveness} 
\pron{aninfveMTivfnisf} 
\gl{\nA}
\expl{}
\bmng
nimARNa yA saqjanashakitx ilalxdiruvike. 
\emng
\eentry

\bentry
\word{uninvestigated} 
\pron{aninfvesiTxgeVTiDf} 
\gl{\gu}
\expl{}
\bmng
pariVkiSxsada; vicAraNe naDesada; taniKe mADada. 
\emng
\eentry

\bentry
\word{uninvited} 
\pron{aninfveYTiDf} 
\gl{\gu}
\expl{}
\bmng
AhAvxnavilalxda; kareyade baruva; AmaMtarxNavilalxda; anAhUta. 
\emng
\eentry

\bentry
\word{uninvitedly} 
\pron{aninfveYTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
AhAvxnavilalxde; anAhUtanAgi; kareyade baruvaMte. 
\emng
\eentry

\bentry
\word{uninviting} 
\pron{aninfveYTiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} AkaSaRkavalalxda; anAkaSaRka. 
\num{2} asahayxvAda; jugupesx huTiTxsuva. 
\enum
\emng
\eentry

\bentry
\word{uninvitingly} 
\pron{aninfveYTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} AkaSaRkavalalxda riVtiyalilx; anAkaSaRkavAgi. 
\num{2} asahayxvAgi; jugupesx huTiTxsuvaMte. 
\enum
\emng
\eentry

\bentry
\word{uninvoked} 
\pron{aninfvoVkfTx} 
\gl{\gu}
\expl{}
\bmng
pArxthiRsada; AhAvxna mADada; (maMtArxdigaLiMda) AhAvxnisada; moreyiTuTx kareyada; beVDada. 
\emng
\eentry

\bentry
\word{uninvolved} 
\pron{aninfvAlfvxDf} 
\gl{\gu}
\expl{}
\bmng
toDagirada; BAgiyAgirada; alipatx; taTasathx. 
\emng
\eentry

\bentry
\word{union} 
\pron{yUnayxnf, yUnianf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} okakxTuTx; okUkxTa; aikayx; seVrike; saMyoVga: \eng{the union of the parts was imperfect} BAgagaLa seVrike asamapaRkavAgitutx. 
\num{2} maduve; vivAha (saMbaMdha): \eng{a happy union} suKavAda vivAha saMbaMdha. 
\num{3} hoMdike; sAmarasayx; sAMgatayx; meYtirx; EkiVBAva: \eng{lived together in perfect union} saMpUNaRvAda meYtirxyiMda oMdAgi jiVvana naDesidaru. 
\numi{4} (vividha BAgagaLu seVri racitavAgiruva) rAjakiVya okUkxTa: 
\banum
\alnum{a} (\kanmu) amerikada saMyukatx saMsAthxnagaLu. 
\alnum{b} gerxVTf birxTanf matutx \kanu\ aileRMDu. 
\alnum{c} dakiSxNa Aphirxkada okUkxTa. 
\alnum{d} soVviyeTf okUkxTa. 
\eanum
\numie
\num{5} (\ga) kUTa; eraDu athavA hecucx gaNa \eng{(set)} gaLige seVrida elalx dhAtugaLanUnx oLagoMDa motatx. 
\num{6} saMGa; saMGaTane; aMgaBAgagaLu yA sadasayxru parasapxra kUDi racitavAda okUkxTada vayxvasethx. 
\numi{7} 
\banum
\alnum{a} (\ca) pAyxriSf kUTa; saMyukatx pAyxriSf; nigaRtikara jiVvanAdhAra kAnUnugaLa nivARhakAkxgi oMdugUDisida halavu pAyxriSfgaLu. 
\hypertarget{union(7)b}{} 
\alnum{b} iMtha saMyukatx pAyxriSf sAthxpisida UT-vasati odagisuva, nigaRtikara Alaya; anAthAlaya. 
\eanum
\numie
\num{8} (\birx) (sahakArakAkxgi) oMdugUDisida cacuRgaLa kUTa. 
\numi{9} 
\banum
\alnum{a} (vishavxvidAyxnilayada yA kAleVjina) vidAyxthiR saMGa yA cacARsaMGa: \eng{the debate at the Oxford union} AkfsxphaDfR vidAyxthiRsaMGadalilx naDeda cacARkUTa. 
\alnum{b} iMtha saMGada vasati kaTaTxDagaLu. 
\eanum
\numie
\num{10} gerxVTf birxTaninxna dhavxjadalilx (rUDhiyAgi dhavxjasatxMBada magugxlina meVlu mUleyalilx kANuva) okUkxTa cihenx. 
\num{11} (koLave \mo vugaLanunx sulaBavAgi kUDisalu yA beVpaRDisalu baLasuva) joVDaNe salakaraNe; saMyoVjaka. \imglink{union 11.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.5cm {U_Pictures/union.jpg}}} 
\num{12} (hatitx, linanf, reVSemx yA seNabu, \mo vugaLa) misharx neyegx baTeTx. 
\numi{13} 
\banum
\alnum{a} ogUgxDike; saMyoVjane; saMGaTane; oMdugUDuvudu. 
\alnum{b} ogUgxDida sithxti; saMyukatx sithxti; saMGaTita sithxti. 
\eanum
\numie
\num{14} vaqtitx saMGa; vaqtitxnirataru, kAmiRkaru tamamx aBivaqdithx matutx hitarakaSxNegAgi EpaRDisikoMDa okUkxTa. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\numi{1} \eng{the Union} 
\banum
\alnum{a} saMyukatx rAjayx, rASaTxrX; 1603ralilx iMgelxMDu matutx sAkxTelxMDugaLa okUkxTa. 
\alnum{b} 1801ralilx gerxVTf birxTanf matutx airelxMDugaLa okUkxTa. 
\alnum{c} 1707ralilx iMgelxMDina hAgU sAkxTelxMDina pAliRmeMTugaLa okUkxTa. 
\eanum
\numie
\num{2} \eng{union down} (vipatitxna sUcaneyAgi) okUkxTa cihenx keLage baruvaMte hArisida bAvuTa; talekeLagAgi hArisida bAvuTa. 
\enum
\emng
\eentry

\bentry
\word{union-bashing} 
\pron{yUnianfbAYxSiMgf} 
\gl{\nA}
\expl{}
\bmng
(\birx) (\AmA) okUkxTamadaRna; kAmiRka saMGagaLige matutx avugaLa hakukxgaLige toVrisuva sakirxya parxtiBaTane. 
\emng
\eentry

\bentry
\wordnospeech{union catalogue}{union catalogue}
\pron{?}
\gl{\nA}
\expl{}
\bmng
saMyukatx garxMthasUci; beVrebeVre pusatxka BaMDAragaLalilx iruva garxMthagaLanunx sUcisuva saMyukatx paTiTx. 
\emng
\eentry

\bentry
\wordnospeech{Union flag}{Union flag}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{Union Jack(1)}{$^1$Union Jack}. 
\emng
\eentry

\bentry
\word{unionise} 
\pron{yUnayxneYsfZ, yUnianeYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
= \hyperlink{unionize}{unionize}. 
\emng
\eentry

\bentry
\word{unionism} 
\pron{yUnayxnisaZmf, yUnianisaZmf}
\gl{\nA}
\bmng
\bnum
\num{1} vaqtitxsaMGa yA kAmiRka saMGa -- tatatxvX, vAda. 
\num{2} okUkxTa padadhxti, vayxvasethx. 
\num{3} vaqtitxsaMGa, kAmiRka saMGa -- padadhxti. 
\num{4} (yAvudAdarU) saMGakekx seVriruvudu. 
\num{5} (\ame) EkiVkaraNavAda; okUkxTa tatatxvX. 
\enum
\emng
\eentry

\bentry
\word{unionist} 
\pron{yUnayxnisfTx, yUnianisfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vaqtitxsaMGa yA TerxVDf yUniyanfna sadasayx. 
\num{2} vaqtitxsaMGavAdi; kAmiRkasaMGavAdi. 
\num{3} (\rAshA) EkiVkaraNavAdi; okUkxTavAdi; \kanmu\ gerxVTf birxTanf hAgU utatxra aileRMDugaLa pAliRmeMTugaLa viBajane yoVjanege viroVdhiyAdavanu. 
\num{4} (\pArxparx) kanasxveRTivf (pakaSxdavanu). 
\num{5} (\ame, \ca) EkiVkaraNavAdi; okUkxTavAdi; amerikada oLayudadhx naDedAga dakiSxNa saMsAthxnagaLu okUkxTadiMda beVreyAguvudanunx viroVdhisidavanu. 
\num{6} cacuRgaLa okUkxTavanunx samathiRsuvavanu. 
\enum
\emng
\eentry

\bentry
\word{unionistic} 
\pron{yUnayxnisiTxkf, yUnianisiTxkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vaqtitxsaMGa tatatxvXda; kAmiRkasaMGavAda. 
\num{2} EkiVkaraNavAdakekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{unionization} 
\pron{yUnayxneYseVZSa(Sf)nf, yUnianeYseVZSa(Sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vaqtitxsaMGa yA kAmiRkasaMGa -- racane, sAthxpane, okUkxTa yA saMyoVjane. 
\num{2} vaqtitxsaMGakekx seVrisuvike yA oLapaDisuvike. 
\num{3} vaqtitxsaMGada niyamagaLige oLapaDisuvike. 
\enum
\emng
\eentry

\bentry
\word{unionize} 
\pron{yUnayxneYsfZ, yUnianeYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (kelasagAraranunx) vaqtitxsaMGakekx seVrisu. 
\num{2} (kelasagAraranunx) vaqtitxsaMGa vayxvasethxge yA niyamagaLige oLapaDisu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} vaqtitxsaMGakekx seVru. 
\num{2} vaqtitxsaMGada niyamagaLige oLapaDu, badadhxnAgu. 
\enum
\emng
\eentry

\bentry
\word{un-ionized} 
\pron{anfaianeYsfZDx} 
\gl{\gu}
\expl{}
\bmng
ayAniVkarisida yA ayAniVBavisada; ayAnu Agada. 
\emng
\eentry

\bentry
\wordspecial{Union Jack}{1}{1}{\hyperlink{union jack(2)}{\textcolor{superscript}{$^2$}\eng{union jack}}} 
\pron{}
\gl{\nA}
\expl{}
\bmng
yUniyanf jAYxkf; saMta jAjfR, saMta AYxMDUrx, saMta pAYxTirxkf I mUrU shilube gurutugaLu seVriruva, gerxVTf birxTanf matutx aileRMDugaLa (yuneYTeDf kiMgaDxmfna) rASaTxrXdhavxja. 
\emng
\eentry

\bentry
\wordspecial{union jack}{2}{2}{\hyperlink{Union Jack(1)}{\textcolor{superscript}{$^1$}\eng{Union Jack}}} 
\pron{}
\gl{\nA}
\expl{}
\bmng
(amerikadalilx dalilx) haDagina patAke; haDagina muMtudiyalilx Erisiruva, rASaTxrXvanunx saMkeVtisuva kirubAvuTa. 
\emng
\eentry

\bentry
\wordRemoveSpace{union-shop}{union shop}
\pron{?}
\gl{\nA}
\expl{}
\bmng
okUkxTa maLige; nwkararu oMdu vaqtitxsaMGakekx seVrirabeVkeMba yA nigadita samayadoLage seVrabeVkeMba Saratitxruva aMgaDi, kAKARne, vaqtitx, \mo vu. 
\emng
\eentry

\bentry
\wordnospeech{union suit}{union suit}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\ame) kUDuDupu; keYkAlu seVridaMte iDiV deVhavanunx mucucxva, aKaMDavAda oLa uDupu. \imglink{union suit:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.6cm {U_Pictures/union_suit.jpg}}} 
\emng
\eentry

\bentry
\wordnospeech{union workhouse}{union workhouse} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{union(7)b}{union (7b)}. 
\emng
\eentry

\bentry
\word{uniparous} 
\pron{yUniparasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Ekaparxsavada; EkaparxsUtiya; oMdu salakekx oMdanunx mAtarx heruva. 
\num{2} (\savi) oMdeV akaSx yA shAKeyuLaLx; EkAkaSxda; EkashAKeya. 
\enum
\emng
\eentry

\bentry
\word[uniped(1)]{uniped} 
\pron{yUnipeDf} 
\gl{\gu}
\expl{}
\bmng
oMdeV pAdavuLaLx; EkapAdi. 
\emng
\eentry

\bentry
\word[uniped(2)]{uniped} 
\pron{yUnipeDf} 
\gl{\nA}
\expl{}
\bmng
EkapAdi; oMdeV pAdaviruva yA kAliruva jiVvi. 
\emng
\eentry

\bentry
\word{unipersonal} 
\pron{yUnipasaRna(nf)lf} 
\gl{\gu}
\expl{}
\bmng
(deVvateya \vi) EkapuruSiVya; obabxneV vayxkitxyAgiruva. 
\emng
\eentry

\bentry
\word{uniplanar} 
\pron{yUnipelxVnarf} 
\gl{\gu}
\expl{}
\bmng
oMdeV (sama)taladalilxruva; EkataliVya. 
\emng
\eentry

\bentry
\word{unipod} 
\pron{yUnipADf} 
\gl{\nA}
\expl{}
\bmng
(kAyxmara \mo vugaLa) okAkxlina Asare; oMdeV kAliruva Asare, AdhAra. 
\emng
\eentry

\bentry
\word{unipolar} 
\pron{yUnipoVlarf} 
\gl{\gu}
\expl{}
\bmng
EkadhurxviVya: 
\banum
\alnum{a} (viduyxtf matutx kAMtiVya upakaraNada \vi) oMdeV dhurxvaviruva. 
\alnum{b} (\jiVvi) (narakoVshada \vi) oMdeV oMdu nUyxkilxyasf uLaLx. 
\eanum
\emng
\eentry

\bentry
\word{unipolarity} 
\pron{yUnipoVlAYxriTi} 
\gl{\nA}
\expl{}
\bmng
(viduyxtitxna \vi) EkadhurxviVyate; EkadhurxviVyavAgiruvike. 
\emng
\eentry

\bentry
\word[unique(1)]{unique} 
\pron{yu(yU)niVkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eNeyilalxda; ananayx; sATiyilalxda; asadaqsha; EkamAtarx: \eng{his position was unique} avanadu ananayx sAthxna. 
\num{2} asAdhAraNa; apUvaR; vilakaSxNa; asAmAnayx: \eng{the most unique man I ever met} nAnu noVDidavaralelxlAlx vilakaSxNa vayxkitx. 
\enum
\emng
\eentry

\bentry
\word[unique(2)]{unique} 
\pron{yu(yU)niVkf} 
\gl{\nA}
\expl{}
\bmng
asAdhAraNa vayxkitx; ananayx vasutx; asAmAnayxvAdudu. 
\emng
\eentry

\bentry
\word{uniquely} 
\pron{yu(yU)niVkfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} eNeyilalxdaMte; sATiyilalxdaMte; asadaqshavAgi. 
\num{2} asAdhAraNa riVtiyalilx; apUvaRvAgi; asAmAnayxvAgi. 
\enum
\emng
\eentry

\bentry
\word{uniqueness} 
\pron{yu(yU)niVkfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} eNeyilalxdiruvike; sATiyilalxdiruvike; asadaqshate; ananayxte. 
\num{2} asAdhAraNatavx; vilakaSxNate; apUvaRvAgiruvike; asAmAnayxvAgiruvudu. 
\enum
\emng
\eentry

\bentry
\word{unironed} 
\pron{anfaianfRDx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ uDupu, linanf, \mo vugaLa \vi) isitxrX mADilalxda. 
\emng
\eentry

\bentry
\word{uniserial} 
\pron{yUnisiarialf} 
\gl{\gu}
\expl{}
\bmng
(\savi\ matutx \pArxvi) oMdeV sAlina; EkapaMkitxya. 
\emng
\eentry

\bentry
\word[unisex(1)]{unisex} 
\pron{yUnisekfsx} 
\gl{\nA}
\expl{}
\bmng
EkaliMga parxvaqtitx; keVshavinAyxsa, uDupu, \mo vugaLalilx sitxrXVpuruSaru liMgaBeVdavilalxdaMte kANisikoLuLxva parxvaqtitx. 
\emng
\eentry

\bentry
\word[unisex(2)]{unisex} 
\pron{yUnisekfsx} 
\gl{\gu}
\expl{}
\bmng
(uDupina \vi) oMdeV liMgada; sitxrXV puruSaralilx yAru beVkAdarU dharisuvaMte sidadhxpaDisida, yoVjisida. 
\emng
\eentry

\bentry
\word{unisexual} 
\pron{yUnisekfsuyx(Su)alf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oMdeV liMgada; EkaliMgi. 
\num{2} (\savi) keVsara yA shalAke oMdu mAtarx uLaLx; avu eraDU oTiTxge ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unisexuality} 
\pron{yUnisekfsuyx(Su)AYxliTi} 
\gl{\nA}
\expl{}
\bmng
EkaliMgatavx. 
\emng
\eentry

\bentry
\word{unisexually} 
\pron{yUnisekfsuyx(Su)ali} 
\gl{\kirxvi}
\expl{}
\bmng
oMdeV liMgavidudx; EkaliMgiyAgi. 
\emng
\eentry

\bentry
\word[unison(1)]{unison} 
\pron{yUnisa(sf)nf} 
\gl{\gu}
\expl{}
\bmng
(\saM) oMdeV -sAthxyiya, shurxtiya. 
\emng
\eentry

\bentry
\word[unison(2)]{unison} 
\pron{yUnisa(sf)nf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} sAthxyi meVLa; shurxtimeVLa. 
\alnum{b} sAthxyiya aMtaravAgi parigaNisiruva I meVLa. 
\eanum
\numie
\num{2} (\saM) kUDugAyana yA kUDuvAdana; savxrameVLa yA vAdayxmeVLa; oMdeV shurxtiyalilxruva yA aSaTxmasavxradiMda badalAguva oMdu yA hecicxna shurxtigaLalilxruva kaMThagaLu yA vAdayxgaLu parasapxra hoMdikeyAguvaMte saMyoVgavAguvudu, meVLeYsuvudu: \eng{sang in unison} kUDugAyanadalilx hADidaru. 
\num{3} sAmarasayx; hoMdike; sAMgatayx; meVLa: \eng{acted in perfect unison} (elalxroDane) saMpUNaR sAmarasayxdiMda naDedukoMDa. 
\enum
\emng
\eentry

\bentry
\word{unisonal} 
\pron{yUnisanalf} 
\gl{\gu}
\expl{}
\bmng
(\saM) oMdeV sAthxyiya; oMdeV shurxtiya. 
\emng
\eentry

\bentry
\word{unisonant} 
\pron{yUnisanaMTf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unisonal}{unisonal}. 
\emng
\eentry

\bentry
\word{unisonous} 
\pron{yUnisanasf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unisonal}{unisonal}. 
\emng
\eentry

\bentry
\wordnospeech{unison string}{unison string}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(matotxMdaroDane nuDisalu hoMdisida matutx adaroDane miVTabeVkAda) shurxtitaMti. 
\emng
\eentry

\bentry
\word{unissued} 
\pron{anfiSUDf, anfisUyxDf} 
\gl{\gu}
\expl{}
\bmng
horahoraDisada; parxkaTavAgada; horabiVLada; horakaLuhada. 
\emng
\eentry

\bentry
\word{unit} 
\pron{yUniTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} oMdu; Eka; EkAMsha; GaTaka; \kanmu\ lekAkxcAradalilx `oMdu' eMbudAgi gaNisuva biDi matutx pUNaR vasutx, vayxkitx, athavA avugaLa yA avara taMDa. 
\alnum{b} EkAMsha; oMdu saMkiVNaR aKaMDadalilx seVriruva ati cikakxdAda biDi vayxkitxgaLalilx, vasutxgaLalilx yA guMpugaLalilx parxti oMdu: \eng{the family as the unit of society} samAjada GaTaka, (EkAMsha)veMdu parigaNisida kuTuMba. 
\eanum
\numie
\num{2} `oMdu' eMba saMKeyx. 
\num{3} (itara motatxgaLanunx nideRVshisalu parxmANavAgi tegedukoLuLxva) EkamAna: \eng{unit of heat} tApada EkamAna. \eng{mass per unit volume} oMdu Gana aLate EkAMshakekx, EkamAnakekx iruva darxvayxrAshi. 
\num{4} (viduyxcaCxkitx \mo vugaLige dara vidhisuvAga gaNisuva) EkamAna. 
\num{5} yUniTf TarxsfTxna atayxMta kaniSaThxvAda SeVru. 
\num{6} (saMkiVNaRvAda oMdu yaMtarxracaneyalilx vishiSaTxvAda kelasakAkxgi niyoVjisida, Adare oTuTx yaMtarxda BAgavAda) biDi salakaraNe. 
\num{7} (adaraMteyeV iruva, uLidavugaLoDane hoMdikoLaLxleMdu niyoVjisalAda yA oMdara pUraka BAgagaLalilx oMdAgi racisida) piVThoVpakaraNa. 
\num{8} (oMdu saMsethxyalilx) GaTaka; visheVSa taMDa; vishiSaTxvAda kelasakekxMdu udedxVshisalAda guMpu yA taMDa. 
\enum
\emng
\eentry

\bentry
\word[Unitarian(1)]{Unitarian} 
\pron{yUniTeVrianf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} EkamUtiRvAdi; (kerxYsatxra tirxmUtiR tatatxvXkekx virudadhxvAgi) deVvaru obabxneV vayxkitx eMdu vAdisuvavanu. 
\num{2} EkamUtiR vAdavanunx parxtipAdisuva dhAmiRka saMGakekx seVrida matutx saMparxdAyabadadhx matasharxdedhxyalilx sAvxtaMtarxyXvirabeVkeMba tatatxvXvanunx samathiRsuvavanu. 
\num{3} (rAjakiVya \mo vugaLalilx) keVMdirxVkaraNa vAdi; EkiVkaraNa vAdi. 
\enum
\emng
\eentry

\bentry
\word[Unitarian(2)]{Unitarian} 
\pron{yUniTeVrianf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kerxY) EkamUtiRvAdada; EkamUtiRtatatxvXkekx saMbaMdhisida. 
\num{2} (rAjakiVya \mo vugaLalilx) EkiVkaraNa yA keVMdirxVkaraNa tatatxvXkekx saMbaMdhisida. 
\enum
\emng
\eentry

\bentry
\word{Unitarianism} 
\pron{yUniTeVrianisaZmf} 
\gl{\nA}
\expl{}
\bmng
(\kerxY) EkamUtiRvAda. 
\emng
\eentry

\bentry
\word{Unitarianize} 
\pron{yUniTeVrianeYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
(\kerxY) EkamUtiR vAdakekx -- parivatiRsu, tirugisu. 
\emng
\eentry

\bentry
\word{unitarily} 
\pron{yUniTarili} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} EkavAgi; oMdaraMte. 
\num{2} EkamAnadAdxgi. 
\num{3} (rAjakiVya) EkiVkaqtavAgi; EkAtamxkavAgi. 
\enum
\emng
\eentry

\bentry
\word{unitarity} 
\pron{yUniTAYxriTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} Ekatavx. 
\num{2} EkamAnatavx. 
\num{3} EkAtamxkate; EkarUpadalilxruvike. 
\enum
\emng
\eentry

\bentry
\word{unitary} 
\pron{yUniTari} 
\gl{\gu}
\bmng
\bnum
\num{1} Ekada. 
\num{2} EkamAnada. 
\num{3} (rAjakiVya) EkiVkaqta; EkiVBUta; EkAtamxka; Ekateya. 
\enum
\emng
\eentry

\bentry
\wordnospeech{unit cell}{unit cell}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\saPxvi) Eka -- koVsha, kaNa; haraLugaLalilxruva, matetxmatetx punarAvatiRsuva paramANugaLa, ayAnugaLa yA aNugaLa atayxMta cikakx yA kaniSaThx taMDa. 
\emng
\eentry

\bentry
\wordnospeech{unit cost}{unit cost}
\pron{?}
\gl{\nA}
\expl{}
\bmng
EkAMsha bele; utapxnanxda oMdoMdu GaTakAMshavanunx tayArisalu tagaluva bele. 
\emng
\eentry

\bentry
\word{unite} 
\pron{yu(yU)neYTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} seVrisu; kUDisu; saMyoVjisu; oMdugUDisu; oMdumADu; oTuTxgUDisu; EkiVkarisu: \eng{unite the parts with cement} simeMTiniMda biDiBAgagaLanunx seVrisu. 
\num{2} maduveyiMda saMbaMdha kalipxsu, oMdugUDisu (\akirx\ saha). 
\num{3} (guNagaLu, lakaSxNagaLu, \mo vanunx) oTiTxge hoMdiru; seVrisikoMDiru; kUDisikoMDiru: \eng{united anger with mercy} dayeyoMdige koVpavanunx seVrisikoMDidadx. 
\num{4} (BwtikavAgi yA rAsAyanikavAgi) oMdugUDisu; seVrisu; saMyoVjisu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} oMdAgu; seVru; kUDu; oTATxgu; EkiVBavisu; saMyoVgavAgu: \eng{oil will not unite with water} eNeNx niVrinoDane seVrikoLuLxvudilalx. 
\num{2} (oMdeV udedxVshakAkxgi) kUDiko; hoMdiko; ogagxTATxgu; aikamatayxdiMdiru: \eng{unite in sentiment} BAveYkayxvAgu. \eng{united in their struggle against injustice} anAyxyada virudadhx hoVrATakekx avaru ogUgxDidaru. 
\enum
\emng
\eentry

\bentry
\wordnospeech{United Brethren}{United Brethren}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperref{kandict_m.pdf}{M}{Moravian(2)}{$^2$Moravian}. 
\emng
\eentry

\bentry 
\wordnospeech{United Kingdom}{United Kingdom}
\pron{?}
\gl{\nA}
\expl{}
\bmng
gerxVTf birxTanf matutx utatxra aileRMDf (1922ra varege, gerxVTf birxTanf matutx aileRMDf). 
\emng
\eentry

\bentry
\wordnospeech{United Nations}{United Nations}
\pron{?}
\gl{\nA}
\expl{}
\bmng
saMyukatx rASaTxrX saMsethx; vishavxsaMsethx; (parxparxthamavAgi 1942ralilx) 1939-45ra mahAyudadhxda kAladalilx shaturxrASaTxrXgaLa virudadhx oMdAgi seVrida mitarxrASaTxrXgaLa okUkxTa, AmeVle uLida rASaTxrXgaLU saMGaTisi vishavxshAMtiyananxrasi racisida saMsethx. 
\emng
\eentry

\bentry
\wordnospeech{United Provinces}{United Provinces}
\pron{?}
\gl{\nA}
\expl{(\ca)}
\bmng
\bnum
\num{1} 1579ralilx oMdugUDi AmeVle nedaleRMDfsx ripabilxkf sAthxpanege AdhAravAda ELu pArxMtagaLa okUkxTa. 
\num{2} (\ca) sAvxtaMtarxyXpUvaR BAratadalilx AgArx matutx audhfgaLu seVri oMdAgidadx, 1950riMda utatxraparxdeVshaveMdu kareyuva oMdu parxmuKa ADaLita pArxMta. 
\enum
\emng
\eentry

\bentry
\wordnospeech{United Reformed Church}{United Reformed Church}
\pron{?}
\gl{\nA}
\expl{}
\bmng
iMgelxMDina perxsibxTeVriyanf matutx kAMgirxgeVSanalf cacuRgaLu 1972ralilx oMdAgi seVrida cacuR. 
\emng
\eentry

\bentry
\wordnospeech{United States of America}{United States of America}
\pron{?}
\gl{\nA}
\expl{}
\bmng
amerikada saMyukatx saMsAthxnagaLu; bahupAlu \kanu\ amerikadalilxdadx \eng{50} rAjayxgaLu matutx alAsakx hAgU havAyfgaLanUnx oLagoMDa saMyukatx gaNarAjayx. 
\emng
\eentry

\bentry
\word{unit-holder} 
\pron{yUniTfhoVlaDxrf} 
\gl{\nA}
\expl{}
\bmng
yUniTf TarxsfTxnalilx hUDikeyuLaLx sadasayx. 
\emng
\eentry

\bentry
\word{unitive} 
\pron{yUniTivf} 
\gl{\gu}
\bmng
saMyoVjanada; saMyoVgada; EkiVkaraNada; EkiVBAvada. 
\emng
\eentry

\bentry
\word{unitively} 
\pron{yUniTivfli} 
\gl{\kirxvi}
\expl{}
\bmng
saMyoVgavAgi; EkiVkaraNadiMda. 
\emng
\eentry

\bentry
\wordnospeech{unit price}{unit price}
\pron{?}
\gl{\nA}
\expl{}
\bmng
oMdara bele; biDibele; sarabarAju mADida sAmAnina meVle EkAMshakekx iMtiSeTxMdu vidhisuva bele. 
\emng
\eentry

\bentry
\wordnospeech{unit trust}{unit trust}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\birx) yUniTf TarxsuTx; tananx beVrebeVre sekUyxriTigaLalilx halavAru janaru toDagisida oTuTx haNavanunx tegedukoMDu beVrebeVre kaDe toDagisuva matutx SeVrudArarige avaravara TheVvaNigaLiganusAravAgi lABAMshagaLanunx koDuva baMDavALa hUDike saMsethx. 
\emng
\eentry

\bentry
\word{unity} 
\pron{yUniTi} 
\gl{\nA}
\expl{(\bava\ \eng{unities}).}
\bmng
\bnum
\num{1} Ekate; aKaMDate. 
\num{2} aikayx; samagarxte; aMgaBAgagaLa meVLanadiMda oMdAgi racitavAgiruvudu: \eng{disturb the unity of the idea} BAvada samagarxtege dhakekxyuMTumADu. 
\num{3} aikayxte; sAMgatayx; sAmaMjasayx; aMgaBAgagaLa ucitavAda parasapxra saMbaMdha yA hoMdike: \eng{the picture lacks unity} I citarxdalilx aikayxte, sAMgatayx ilalx. 
\num{4} vayxSiTx: aKaMDa samagarxvAgiruva saMkiVNaR vasutx: \eng{a person regarded as a unity} aKaMDa samagarxvAgi pariBAvitanAgiruva vayxkitx. 
\num{5} (\ga) oMdu; Eka. 
\num{6} (vayxkitxgaLu \mo vara naDuvaNa) hoMdike; sAmarasayx; omamxnasusx: \eng{dwell together in unity} oMdAgi bALu. 
\num{7} ogagxTuTx; aikamatayx. 
\num{8} (raMgaBUmi) = \hyperlink{unity pagu(3)}{unity ?pagu? \((3)\)}. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{at unity with} hoMdikoMDu, aikamatayxdiMda. 
\num{2} \eng{national unity} rASeTxrXYkayx(te); rASiTxrXVya Ekate, aikamatayx. 
\hypertarget{unity pagu(3)}{} 
\num{3} \eng{the dramatic unities (the unities of time, place and action)} nATakiVya aikayxgaLu (kAleYkayx, sathxLeYkayx, kirxyeYkayx); (nATakavanunx ADalu hiDiyuva kAlavU nATakada kathe nijavAgi naDeyalu beVkAda kAlavU sarisumAru oMdeV parimiti uLaLxdAdxgiruvudu kAleYkayx, daqshayx badalAvaNe ilalxde, oTuTx kathe oMdeV sathxLadalAlxdaMte toVrisuvudu sathxLeYkayx, kathAvasutxvige neVra saMbaMdhavilalxdadxnenxlAlx biTuTx katheyanunx omumxKavAgi naDesuvudu kirxyeYkayx). 
\enum
\emng
\eentry

\bentry
\wordnospeech{Univ.}{Univ.}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{University.} 
\emng
\eentry

\bentry
\word[univalent(1)]{univalent} 
\pron{yUniveVlaMTf} 
\gl{\gu}
\expl{}
\bmng
Eka veVlenisxVya: 
\banum
\alnum{a} (\ravi) veVlenisx oMdu Agiruva. 
\alnum{b} (\jiVvi) (korxVmosoVmf \vi) meYyoVsisf \eng{(meiosis)} nalilx jotegUDadiruva. 
\eanum
\emng
\eentry

\bentry
\word[univalent(2)]{univalent} 
\pron{yUniveVlaMTf} 
\gl{\nA}
\expl{}
\bmng
(\jiVvi) EkaveVlenisxVya korxVmosoVmu. 
\emng
\eentry

\bentry
\word[univalve(1)]{univalve} 
\pron{yUnivAYxlfvx} 
\gl{\gu}
\expl{}
\bmng
(\pArxvi) EkakavATada; oMdeV oMdu kavATaviruva, cipupxLaLx. 
\emng
\eentry

\bentry
\word[univalve(2)]{univalve} 
\pron{yUnivAYxlfvx} 
\gl{\nA}
\expl{}
\bmng
(\pArxvi) Eka kavATi; oMdeV oMdu kavATaviruva, cipupxLaLx maqdavxMgi. 
\emng
\eentry

\bentry
\word[universal(1)]{universal} 
\pron{yUnivasaRlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sAvaRtirxka; sAvaRlwkika; savaRsAmAnayx; savaRvAyxpi; vishavxvAyxpi; (loVkada yA nidiRSaTx(vagaRda)) elalx manuSayxra yA elalx vasutxgaLa yA elalx manuSayxrige yA vasutxgaLige saMbaMdhisida yA elalxrU mADida yA elalx saMdaBaRgaLigU anavxyisuva: \eng{the terror was universal} Baya sAvaRtirxkavAgitutx, elelxDe Avarisitutx. 
\num{2} (\takaR) (anumitiya \vi) savARnavxyi; A vagaRda elalxkUkx anavxyisuva. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{a universal rule} sAvaRtirxka niyama. 
\num{2} \eng{have universal sanction} savaRmAnayxte hoMdiru. 
\num{3} \eng{meet with universal applause} elalxra mecicxge paDe, parxshaMsegaLisu. 
\enum
\emng
\eentry

\bentry
\word[universal(2)]{universal} 
\pron{yUnivasaRlf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\takaR) = \hyperlink{universal proposition}{universal proposition}. 
\numi{2} (\tashA) 
\banum
\alnum{a} sAvaRtirxka, sAmAnayx -- kalapxne, BAvane yA pada; elalxkUkx anavxyisuva kalapxne yA shabadx. 
\alnum{b} sAmAnayx lakaSxNa; sAmAnayx pada sUcisuva -- sAmAnayx savxBAva yA guNa. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\wordnospeech{universal agent}{universal agent}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(parxtinidhiya elalx kelasagaLanunx mADalu adhikAra paDeda) savaRparxtinidhi; savaRniyoVgi. 
\emng
\eentry

\bentry
\wordnospeech{universal compass}{universal compass}
\pron{?}
\gl{\nA}
\expl{}
\bmng
vAyxpaka keYvAra; doDaDx vaqtatx eLeyabeVkAdAga kAlu higagxlisalu sAdhayxviruva doDaDx keYvAra. 
\emng
\eentry

\bentry
\wordnospeech{universal coupling}{universal coupling}
\pron{?}
\gl{\nA}
\expl{}
\bmng
savaR saMyoVjaka; beVkAda yAvudeV koVnadalilx cAlakadaMDadiMda shakitxyanunx vagARyisalu anukUlisuva saMyoVjaka. 
\emng
\eentry

\bentry
\word{universalise} 
\pron{yUnivasaRleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
= \hyperlink{universalize}{universalize}. 
\emng
\eentry

\bentry
\word{universalism} 
\pron{yUnivasaRlisaZmf} 
\gl{\nA}
\expl{}
\bmng
savaRmukitx tatatxvX; savaRmoVkaSxvAda. 
\emng
\eentry

\bentry
\word{universalist} 
\pron{yUnivasaRlisfTx} 
\gl{\nA}
\expl{}
\bmng
savaRmoVkaSxvAdi; savaRmukitxvAdi: elalx mAnavarU konege moVkaSx paDeyutAtxre eMba aBipArxyavuLaLx kerxYsatx paMgaDakekx seVrida, vayxkitx. 
\emng
\eentry

\bentry
\word{universalistic} 
\pron{yUnivasaRlisiTxkf} 
\gl{\gu}
\expl{}
\bmng
savaRmoVkaSx vAdada; savaRmukitx tatatxvXkekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{universality} 
\pron{yUnivasAYxRliTi} 
\gl{\nA}
\expl{}
\bmng
sAvaRtirxkatavx; sAvaRlwkikatavx; savaRvAyxpakate; vishavxvAyxpakate. 
\emng
\eentry

\bentry
\word{universalization} 
\pron{yUnivasaRleYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
sAvaRtirxkiVkaraNa; vishavxvAyxpakagoLisuvike. 
\emng
\eentry

\bentry
\word{universalize} 
\pron{yUnivasaRleYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
sAvaRtirxkagoLisu; vishavxvAyxpakagoLisu. 
\emng
\eentry

\bentry
\wordnospeech{universal joint}{universal joint}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{universal coupling}{universal coupling}. 
\emng
\eentry

\bentry
\wordnospeech{universal language}{universal language}
\pron{?}
\gl{\nA}
\expl{}
\bmng
sAvaRtirxka BASe; elalx rASaTxrXgaLu baLasaleMdu udedxVshisi racisida kaqtaka BASe. 
\emng
\eentry

\bentry
\word{universally} 
\pron{yUnivasaRli} 
\gl{\kirxvi}
\expl{}
\bmng
sAvaRtirxkavAgi; sAvaRlwkikavAgi; vishavxvAyxpakavAgi; savaRvAyxpiyAgi; savaRsAmAnayxvAgi; elalxrigU, elalxdakUkx anavxyisuvaMte. 
\emng
\eentry

\bentry
\wordnospeech{universal proposition}{universal proposition}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\takaR) sAvaRtirxka parxtijecnx; vidheVya yA visheVSaNavu A vagaRda elAlx biDi GaTakagaLigU anavxyavAgutatxdeyeMdu heVLuva parxtijAcnxvAkayx. 
\emng
\eentry

\bentry
\wordnospeech{universal suffrage}{universal suffrage}
\pron{?}
\gl{\nA}
\expl{}
\bmng
sAvaRtirxka matAdhikAra; saNaNxpuTaTx vinAyitigaLa vinA pArxpatx vayasakxrelalxrigU visatxrisiruva, matacalAvaNeya adhikAra. 
\emng
\eentry

\bentry
\wordnospeech{universal time}{universal time}
\pron{?}
\gl{\nA}
\expl{}
\bmng
sAvaRtirxka kAla; KagoVLa vijAcnxnada lekAkxcArakAkxgi elalx kaDe baLasuva kAla (= \hyperref{kandict_g.pdf}{G}{Greenwich mean time}{Greenwich Mean Time}). 
\emng
\eentry

\bentry
\word{universe} 
\pron{yUnivasfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vishavx; barxhAmxMDa; asitxtavxdalilxruva elalx vasutx, jiVvi, \mo vugaLu. 
\num{2} suvayxvasithxta vishavx; savaRloVka; samasatx parxpaMca. 
\num{3} savaRsaqSiTx; samasatx saqSiTx. 
\num{4} manuSayxkoVTi; manuSayx loVka. 
\num{5} (\saMshA\ matutx \takaR) savaR(savx); savaR vicArakekx oLapaTaTx samasatx viSaya; parigaNaneyalilxruva elAlx vasutxgaLu. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{universe of discourse} (\takaR) vicArakekx oLapaTaTx samasatx viSaya. 
\emng
\eentry

\bentry
\word{university} 
\pron{yUnivasiRTi} 
\gl{\nA}
\expl{(\bava\ \eng{universities}). }
\bmng
\bnum
\num{1} vishavxvidAyxnilaya. 
\num{2} vishavxvidAyxnilayada sadasayxru; vishavxvidAyxnilayadavaru. 
\num{3} vishavxvidAyxnilayada daLa, kirxVDA taMDa, \mo vu. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{at university} vishavxvidAyxnilayadalilx (vAyxsaMga mADutAtx). 
\emng
\eentry

\bentry
\word[univocal(1)]{univocal} 
\pron{yUnivoVkalf} 
\gl{\gu}
\expl{}
\bmng
(padada \vi) oMdeV oMdu sariyAda athaRvuLaLx; EkAthaRka. 
\emng
\eentry

\bentry
\word[univocal(2)]{univocal} 
\pron{yUnivoVkalf} 
\gl{\nA}
\expl{}
\bmng
EkAthaRka(pada). 
\emng
\eentry

\bentry
\word{univocality} 
\pron{yUnivoVkAYxliTi} 
\gl{\nA}
\expl{}
\bmng
EkAthaRkate; padakekx oMdeV oMdu athaRviruvike. 
\emng
\eentry

\bentry
\word{univocally} 
\pron{yUnivoVkali} 
\gl{\kirxvi}
\expl{}
\bmng
(padada \vi) EkAthaRkavAgi; oMdeV oMdu athaRviruvaMte. 
\emng
\eentry

\bentry
\word{unjaundiced} 
\pron{anfjAMDisfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kAmAle roVgavilalxda. 
\num{2} ISeRyx yA matasxradiMda daqSiTxgeDada; matasxragarxsatxvalalxda; asUyAgarxsatxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unjoin} 
\pron{anfjAinf} 
\gl{\sakirx}
\expl{}
\bmng
(joVDisiruvudanunx, oMdAgiruvudanunx) beVpaRDisu; viyoVjisu; parxteyxVkisu; biDisu; bicucx. 
\emng
\eentry

\bentry
\word{unjoined} 
\pron{anfjAinfDx} 
\gl{\gu}
\expl{}
\bmng
beVpaRDisida; viyoVjisida; parxteyxVkagoLisida; biDisida. 
\emng
\eentry

\bentry
\word{unjoint} 
\pron{anfjAiMTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} kiVlugaLanunx kaLacu; saMdhigaLanunx parxteyxVkisu. 
\num{2} beVpaRDu; parxteyxVkavAgu; beVrebeVreyAgu. 
\enum
\emng
\eentry

\bentry
\word{unjust} 
\pron{anfjasfTx} 
\gl{\gu}
\expl{}
\bmng
nAyxyavalalxda; anAyxyada; taravalalxda; sariyalalxda. 
\emng
\eentry

\bentry
\word{unjustifiable} 
\pron{anfjasiTxpheYabflf} 
\gl{\gu}
\expl{}
\bmng
asamathaRniVya; sariyalalxda; (nAyxyaveMdu) samathiRsalu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unjustifiably} 
\pron{anfjasiTxpheYabilx} 
\gl{\kirxvi}
\expl{}
\bmng
asamathaRniVyavAgi; nAyxyaveMdu samathiRsalAgade. 
\emng
\eentry

\bentry
\word{unjustified} 
\pron{anfjasiTxpheYDf} 
\gl{\gu}
\expl{}
\bmng
asamathaRniVyavAda; sariyalalxda. 
\emng
\eentry

\bentry
\word{unjustly} 
\pron{anfjasfTxli} 
\gl{\kirxvi}
\expl{}
\bmng
anAyxyavAgi; nAyxyasamamxtavalalxdaMte; sariyalalxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{unjustness} 
\pron{anfjasfTxnisf} 
\gl{\nA}
\expl{}
\bmng
anAyxya; sariyalalxdiruvike. 
\emng
\eentry

\bentry
\word{unkempt} 
\pron{anfkeMpfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} talebAcikoLaLxda; (tale) kedarikoMDiruva; tale kedarida. 
\num{2} asatxvayxsatxvAda; opapxOraNavilalxda; avayxvasithxtavAda. 
\enum
\emng
\eentry

\bentry
\word{unkemptly} 
\pron{anfkeMpfTxli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (tale) kedarikoMDiruvaMte; talebAcikoLaLxde. 
\num{2} asatxvayxsatxvAgi. 
\enum
\emng
\eentry

\bentry
\word{unkemptness} 
\pron{anfkeMpfTxnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (tale) kedarikoMDiruvike; talebAcikoLaLxdiruvike. 
\num{2} asatxvayxsatxte; opapxOraNagaLilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unkennel} 
\pron{anfkenalf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (nariyanunx) (bila, pode, \mo vugaLiMda) Acege, hora -- horaDisu. 
\num{2} beLakige taru; bayalu mADu. 
\enum
\emng
\eentry

\bentry
\word{unkept} 
\pron{anfkepfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vAgAdxna, kAnUnu, \mo vugaLa \vi) pAlisada; anusarisada; upeVkiSxsida. 
\num{2} noVDikoLaLxda; upeVkiSxsida; kaDegaNisida. 
\enum
\emng
\eentry

\bentry
\word{unkillable} 
\pron{anfkilabflf} 
\gl{\gu}
\expl{}
\bmng
kolalxlAgada; vadhisalAgada; avadhayx. 
\emng
\eentry

\bentry
\word{unkind} 
\pron{anfkeYMDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} daye, karuNe -ilalxda. 
\num{2} nidaRya; kaThiNa; kUrxra. 
\enum
\emng
\eentry

\bentry
\word{unkindly} 
\pron{anfkeYMDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} daye ilalxde; karuNe ilalxde. 
\num{2} kUrxravAgi; nidaRyeyiMda; kaThiNa riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{unkindness} 
\pron{anfkeYMDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nidaRyate; niSakxruNe. 
\num{2} kwrxyaR; kAThinayx. 
\enum
\emng
\eentry

\bentry
\word{unking} 
\pron{anfkiMgf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} doretanavilalxdaMte mADu; siMhAsanadiMda keLagiLisu; rAjana adhikAra tapipxsu: \eng{they design to unking the Queen} rANiyanunx siMhAsanadiMda keLakikxLisalu avaru hoMcuhAkutitxdAdxre. 
\num{2} (\pArxparx)(deVshakekx) rAjana adhikAra tapipxsu; rAjanilalxdaMte mADu. 
\enum
\emng
\eentry

\bentry
\word{unkingly} 
\pron{anfkiMgfli} 
\gl{\gu}
\expl{}
\bmng
rAjanige takukxdalalxda; rAjalakaSxNakekx salalxda; rAjayoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{unkink} 
\pron{anfkiMkf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} neVragoLisu; neVpARgisu; aDetaDe, aMkuDoMku, toMdare, \mo vanunx -- toDeduhAku, tegedubiDu. 
\num{2} (manasisxna) tiricumurucugaLanunx toDe; vicitarx BAvane, vipariVta kalapxne, \mo vugaLanunx nivArisu, pariharisu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
neVravAgu; aDetaDe, toMdare, \mo vugaLu kaLe, ilalxvAgu. 
\emng
\eentry

\bentry
\word{unknightly} 
\pron{anfneYTfli} 
\gl{\gu}
\expl{}
\bmng
viVroVcitavalalxda; dhiVranige takukxdalalxda. 
\emng
\eentry

\bentry
\word{unknit} 
\pron{anfniTf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unknitted}; \vakaq\ \eng{unknitting}). }
\bmng
(seVrisida, gaMTu hAkida vasutxgaLanunx) biDisu; saDilisu; bicucx. 
\emng
\eentry

\bentry
\word{unknot} 
\pron{anfnATf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unknotted}; \vakaq\ \eng{unknotting}). }
\bmng
gaMTubicucx; gaMTu saDilisu. 
\emng
\eentry

\bentry
\word[unknowable(1)]{unknowable} 
\pron{anfnoVabflf} 
\gl{\gu}
\expl{}
\bmng
tiLiyalAgada; ajecnxVya. 
\emng
\eentry

\bentry
\word[unknowable(2)]{unknowable} 
\pron{anfnoVabflf} 
\gl{\nA}
\expl{}
\bmng
tiLiyalAgadudu; ajecnxVya. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{the Unknowable} ajecnxVya parasatayx. 
\emng
\eentry

\bentry
\word[unknowing(1)]{unknowing} 
\pron{anfnoViMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tiLiyada; ariyada. 
\num{2} ajAcnxnada; arivilalxda. 
\enum
\emng
\eentry

\bentry
\word[unknowing(2)]{unknowing} 
\pron{anfnoViMgf} 
\gl{\nA}
\expl{}
\bmng
ajAcnxna: \eng{cloud of unknowing} ajAcnxnada moVDa. 
\emng
\eentry

\bentry
\word{unknowingly} 
\pron{anfnoViMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
tiLiyade; ajAcnxnadiMda. 
\emng
\eentry

\bentry
\word{unknowingness} 
\pron{anfnoViMgfnisf} 
\gl{\nA}
\expl{}
\bmng
ajAcnxna; arivilalxdiruvudu; tiLiyade iruvudu. 
\emng
\eentry

\bentry
\word[unknown(1)]{unknown} 
\pron{anfnoVnf} 
\gl{\gu}
\expl{}
\bmng
tiLiyada; gotitxrada; tiLidilalxda; ajAcnxta: \eng{he was unknown to me} avanu nanage gotitxralilalx. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{unknown} \hyperref{kandict_c.pdf}{C}{country(1) nuga(4)}{$^1$country}. 
\num{2} \eng{unknown} \hyperref{kandict_q.pdf}{Q}{quantity pagu(2)}{quantity}. 
\enum
\emng
\eentry

\bentry
\word[unknown(2)]{unknown} 
\pron{anfnoVnf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ajAcnxta -- vasutx, vayxkitx. 
\num{2} ajAcnxta pada: \eng{equation in two unknowns} eraDu ajAcnxta padagaLa samiVkaraNa. 
\enum
\emng
\eentry

\bentry
\word[unknown(3)]{unknown} 
\pron{anfnoVnf} 
\gl{\kirxvi}
\expl{}
\bmng
tiLiyade; arivilalxde. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{unknown to} tiLiyade; gotitxrade: \eng{did it unknown to me} nanage tiLiyade mADida. 
\emng
\eentry

\bentry
\word{unknownness} 
\pron{anfnoVnfnisf} 
\gl{\nA}
\expl{}
\bmng
ajAcnxtate; tiLiyadiruvike. 
\emng
\eentry

\bentry
\wordnospeech{Unknown Soldier}{Unknown Soldier}
\pron{?}
\gl{\nA}
\expl{}
\bmng
ajAcnxta yoVdha; ajAcnxta seYnika; yudadhxdalilx maDida seYnikaralilx rASaTxrXkAkxgi baliyAdavara saMkeVtavAgi Arisi, sAvaRjanikavAgi visheVSa gwravagaLoMdige saMsAkxra naDasalAguva ajAcnxta seYnika, yoVdha. 
\emng
\eentry

\bentry
\wordnospeech{Unknown Warrior}{Unknown Warrior}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{Unknown Soldier}{Unknown Soldier}. 
\emng
\eentry

\bentry
\word{unlabelled} 
\pron{anfleVbalfDx} 
\gl{\gu}
\expl{}
\bmng
(\ame\ \eng{unlabeled}). taleciVTi hacacxda; gurutina ciVTi aMTisada. 
\emng
\eentry

\bentry
\word{unlaboured} 
\pron{anfleVbaDfR} 
\gl{\gu}
\expl{}
\bmng
(\ame\ \eng{unlabored}). (sheYli \mo vugaLa \vi) kaSaTxpaTiTxlalxda; tiNikada; akilxSaTx; sahaja; akaqtaka. 
\emng
\eentry

\bentry
\word{unlace} 
\pron{anfleVsf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (bUTu \mo vugaLa) dAra bicucx; kaTaTxnunx bicucx. 
\num{2} dAra bicicx -- saDilisu, tege. 
\enum
\emng
\eentry

\bentry
\word{unlade} 
\pron{anfleVDf} 
\gl{\sakirx}
\expl{}
\bmng
(haDaginiMda) heVru, hore, saraku, \mo vanunx -- iLisu; horeyiLisu. 
\emng
\eentry

\bentry
\word{unladen} 
\pron{anfleVDanf} 
\gl{\gu}
\expl{}
\bmng
BAra heVrada; BAra horisada; (sAmAnu \mo vanunx) tuMbada. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{unladen weight} (saraku, BAra, \mo vanunx heVrirada vAhanada \vi) keVvala, KAli vAhanada tUka. 
\emng
\eentry

\bentry
\word{unladylike} 
\pron{anfleVDileYkf} 
\gl{\gu}
\expl{}
\bmng
kulasitxrXVyaMtirada; gwravasathx -- mahiLeyaMtirada, mahiLege tagada; kulasitxrXVya naDavaLikeyilalxda; kulasitxrXVsahaja gAMBiVyaRvilalxda. 
\emng
\eentry

\bentry
\word[unlaid(1)]{unlaid} 
\pron{anfleVDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} iDada; iTiTxrada. 
\num{2} (shavada \vi) hULuvudakekx -- sidadhxmADada, aNigoLisada, sajujxgoLisada. 
\num{3} hagagxvAgi heNeyada, hoseyada. 
\enum
\emng
\eentry

\bentry
\word[unlaid(2)]{unlaid} 
\pron{anfleVDf} 
\gl{\kirx}
\expl{}
\bmng
\eng{unlay} padada \BU\ matutx \BUkaq\ rUpa. 
\emng
\eentry

\bentry
\word{unlamented} 
\pron{anflameMTiDf} 
\gl{\gu}
\expl{}
\bmng
(satatxvara \vi) ashoVcita; yArU duHKisada; yArU parxlApisada; yArU shoVkisada (\rUpa\ saha). 
\emng
\eentry

\bentry
\word{unlash} 
\pron{anflAYxSf} 
\gl{\akirx}
\expl{}
\bmng
(kaTiTxdadxnunx) saDilisu; bicucx. 
\emng
\eentry

\bentry
\word{unlatch} 
\pron{anflAYxcf} 
\gl{\akirx}
\expl{}
\bmng
(bAgilina) aguLi tege (\akirx\ saha). 
\emng
\eentry

\bentry
\word{unlawful} 
\pron{anflAphulf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kAnUnige virudadhxvAda; shAsanavirudadhxvAda; nAyxya samamxtavalalxda; kAnUnubAhira(vAda). 
\num{2} = \hyperref{kandict_i.pdf}{I}{illegitimate(1)}{illegitimate}. 
\enum
\emng
\eentry

\bentry
\word{unlawfully} 
\pron{anflAphuli} 
\gl{\kirxvi}
\expl{}
\bmng
kAnUnubAhiravAgi. 
\emng
\eentry

\bentry
\word{unlawfulness} 
\pron{anflAphulfnisf} 
\gl{\nA}
\expl{}
\bmng
kAnUnubAhirate; nAyxyasamamxtavalalxdiruvike. 
\emng
\eentry

\bentry
\word{unlay} 
\pron{anfleV} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unlaid}). }
\bmng
(\nw) (hagagxda \vi) tirucanunx tege; hosediruvudanunx bicucx. 
\emng
\eentry

\bentry 
\word{unleaded} 
\pron{anfleDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (peTorxVlina \vi) siVsa seVrisada. 
\num{2} (meVlAPxvaNi, kaTaTxDa, kiTaki, \mo vanunx) siVsada tagaDugaLiMda mucacxda; siVsada cwkaTuTx hAkada. 
\num{3} (mudarxNasAlu \mo vanunx) teLutagaDiniMda parxteyxVkisada, beVpaRDisada. 
\enum
\emng
\eentry

\bentry
\word{unlearn} 
\pron{anflanfR} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unlearned} yA \eng{unlearnt}).}
\bmng
\bnum
\num{1} maretubiDu; nenapiniMda aLisibiDu. 
\num{2} (duraBAyxsa, tapupx tiLivaLike, \mo vanunx) tayxjisu; tore. 
\enum
\emng
\eentry

\bentry
\word[unlearned(1)]{unlearned} 
\pron{anflaniRDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pAMDitayxvilalxda; paMDitanalalxda. 
\num{2} Odirada; ashikiSxta; avidAyxvaMta; videyx ilalxda. 
\enum
\emng
\eentry

\bentry
\word[unlearned(2)]{unlearned} 
\pron{anflaniRDf} 
\gl{\gu}
\expl{}
\bmng
(\eng{unlearnt} eMdU \parx) kalitilalxda; kaliyada; kalitukoLaLxda. 
\emng
\eentry

\bentry
\word{unlearnedly} 
\pron{anflaniRDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} pAMDitayxvilalxde. 
\num{2} ajAcnxniyaMte; avidAyxvaMtana hAge. 
\enum
\emng
\eentry

\bentry
\word{unleash} 
\pron{anfliVSf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (nAyi \mo vugaLa) togala bAru bicucx; baMdhana kaLacu; `CU' biDu. 
\num{2} sevxVcePxyAgi biTuTxbiDu; hiDita saDilisu. 
\enum
\emng
\eentry

\bentry
\word{unleavened} 
\pron{anflevanfDx} 
\gl{\gu}
\expl{}
\bmng
(berxDf \mo vugaLa \vi) hudugilalxde tayArisida. 
\emng
\eentry

\bentry
\word{unless} 
\pron{anflisf} 
\gl{\saMavayx}
\expl{}
\bmng
-de idadxre; horatu; vinA: \eng{I shall go unless I hear from him} avaniMda vataRmAna baradidadxre nAnu hoVgutetxVne. 
\emng
\eentry

\bentry
\word{unlettered} 
\pron{anfleTaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pusatxkajAcnxnavilalxda; Odirada; pusatxka pAMDitayxvilalxda. 
\num{2} anakaSxrasathx; akaSxrajAcnxnavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unliberated} 
\pron{anflibareVTiDf} 
\gl{\gu}
\expl{}
\bmng
amukatx; vimoVcaneyAgada; biDugaDe hoMdirada. 
\emng
\eentry

\bentry
\word{unlicensed} 
\pron{anfleYsanfsfTx} 
\gl{\gu}
\expl{}
\bmng
sananxdu, riyAyiti, \mo vanunx paDeyada; paravAnagi ilalxda. 
\emng
\eentry

\bentry
\word{unlicked} 
\pron{anflikfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} rUpakekx taMdilalxda; vayxvasethxge taMdirada. 
\num{2} naDategeTaTx; asaBayx: \eng{an unlicked cub} asaBayx naDavaLikeya yuvaka. 
\enum
\emng
\eentry

\bentry
\word{unlighted} 
\pron{anfleYTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (diVpada \vi) hotitxsada; urisada; hotitx uriyada. 
\num{2} beLakilalxda; diVpavilalxda; diVpa odagisada. 
\enum
\emng
\eentry

\bentry
\word{unlikable} 
\pron{anfleYkabflf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unlikeable}{unlikeable}. 
\emng
\eentry

\bentry
\word[unlike(1)]{unlike} 
\pron{anfleYkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (obabxra, oMdara) hAgilalxda; hoVlikeyilalxda; asadaqsha; sAdaqshayxvilalxda: \eng{is unlike both his parents} taMdetAyigaLalilx obabxranUnx hoVluvudilalx. \eng{the portrait is utterly unlike} citarxvu savxlapxvU mUladaMte ilalx. 
\num{2} vilakaSxNavAda; sahajavalalxda; savxBAvakakxnuguNavAgirada: \eng{such behaviour is unlike him} aMtha naDavaLike avana savxBAvakekx virudadhxvAdudu. 
\num{3} visadaqsha; BinanxvAda; beVreyAda. 
\enum
\emng
\eentry

\bentry
\word[unlike(2)]{unlike} 
\pron{anfleYkf} 
\gl{\upa}
\expl{}
\bmng
BinanxvAgi; beVre riVtiyalilx: \eng{acts quite unlike anyone else} beVre elalxrigiMtalU BinanxvAgi vatiRsutAtxne. 
\emng
\eentry

\bentry
\word{unlikeable} 
\pron{anfleYkabflf} 
\gl{\gu}
\expl{}
\bmng
(\eng{unlikable} eMdU \parx) ahitakara; iSaTxvAgada. 
\emng
\eentry

\bentry
\word{unlikelihood} 
\pron{anfleYkflihuDf} 
\gl{\nA}
\expl{}
\bmng
asaMBavate; saMBavisalAgadiruvudu. 
\emng
\eentry

\bentry
\word{unlikeliness} 
\pron{anfleYkflinisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unlikelihood}{unlikelihood}. 
\emng
\eentry

\bentry
\word{unlikely} 
\pron{anfleYkfli} 
\gl{\gu}
\expl{(\tara\ \eng{unlikelier,} \tama\ \eng{unlikeliest}). }
\bmng
\bnum
\num{1} asaMBava: \eng{unlikely tale} asaMBavavAda kathe. 
\num{2} naDesuva niriVkeSxyilalxda; naDesalu asAdhayxvenisabahudAda. 
\num{3} viPalavAguva; jayada Baravaseyilalxda: \eng{an unlikely errand} jayada Baravaseyilalxda kelasa. 
\enum
\emng
\eentry

\bentry
\word{unlikeness} 
\pron{anfleYkfnisf} 
\gl{\nA}
\expl{}
\bmng
asadaqshate; BinanxvAgiruvike. 
\emng
\eentry

\bentry
\wordnospeech{unlike signs}{unlike signs}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\ga) visadaqsha cihenxgaLu \eng{[+} (saMkalana) matutx -- (vayxvakalana) cihenxgaLu\eng{]}. 
\emng
\eentry

\bentry
\word{unlimber} 
\pron{anfliMbarf} 
\gl{\akirx}
\expl{}
\bmng
(kAyARcaraNegAgi PiraMgi gADiyiMda tupAki \mo vanunx) horatege; tegedu sidadhxpaDisu. 
\emng
\eentry

\bentry
\word{unlimited} 
\pron{anflimiTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} elelx ilalxda; mitiyilalxda; yatheVSaTx; anaMta; apAra; a(pari)mita: \eng{has unlimited scope} apAra avakAshavide. \eng{unlimited possibilities} anaMta, yatheVSaTx sAdhayxtegaLu. 
\num{2} beVSaratitxna; Saratitxlalxda: \eng{unlimited surrender} beVSaratAtxda sharaNAgati. 
\enum
\emng
\eentry

\bentry
\word{unlimitedly} 
\pron{anflimiTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
elelx ilalxde; a(pari)mitavAgi; yatheVSaTxvAgi; apAravAgi. 
\emng
\eentry

\bentry
\word{unlimitedness} 
\pron{anflimiTiDfnisf} 
\gl{\nA}
\expl{}
\bmng
elelx ilalxde iruvudu; mitiyilalxda sithxti; anaMtate; a(pari)mitatavx. 
\emng
\eentry

\bentry
\word[unlined(1)]{unlined} 
\pron{anfleYnfDx} 
\gl{\gu}
\expl{}
\bmng
(uDupina \vi) asatxri hAkilalxda. 
\emng
\eentry

\bentry
\word[unlined(2)]{unlined} 
\pron{anfleYnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (kAgada \mo vugaLa \vi) geregaLilalxda. 
\num{2} (muKada \vi) sukikxlalxda. 
\enum
\emng
\eentry

\bentry
\word{unlink} 
\pron{anfliMkf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} koMDigaLanunx -- biDisu, parxteyxVkisu, bicucx, kaLacu. 
\num{2} saMbaMdha -- tapipxsu, saDilisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
parxteyxVkavAgu; beVpaRDu. 
\emng
\eentry

\bentry
\word{unliquidated} 
\pron{anflikivxDeVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (sAla \mo vugaLa \vi) salilxsada; tiVrisada; pheYsalAgada; tiVruvaLiyAgada. 
\num{2} (saMGa \mo vugaLa \vi) mucicxbiDada; divALiyAgada. 
\enum
\emng
\eentry

\bentry
\word{unlisted} 
\pron{anflisiTxDf} 
\gl{\gu}
\bmng
\bnum
\num{1} (SeVrupeVTe) daragaLa paTiTxyalilx parxkaTisada, namUdisada. 
\num{2} (dUravANi) keYpiDiyalilx seVrisada. 
\enum
\emng
\eentry

\bentry
\word{unlit} 
\pron{anfliTf} 
\gl{\gu}
\bmng
= \hyperlink{unlighted}{unlighted}. 
\emng
\eentry

\bentry
\word{unlivable} 
\pron{anflivabflf} 
\gl{\gu}
\expl{}
\bmng
vAsayoVgayxvalalxda; vAsavAgiralu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unlived-in} 
\pron{anflivfDxinf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vAsaviradaMte kANuva, toVruva. 
\num{2} nivAsigaLu -- baLasada, baLasirada. 
\enum
\emng
\eentry

\bentry
\word{unload} 
\pron{anfloVDf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ haDagu, baMDi, \mo vugaLiMda) hore iLisu; saraku keLagiLisu (\akirx\ saha). 
\num{2} (manasusx, haqdaya, \mo vugaLa) BAraviLisu; haguragoLisiko; ciMte \mo vanunx pariharisiko. 
\num{3} (baMdUkada) madudx, toVTa, \mo vanunx tegedubiDu; KAlimADu. 
\num{4} (\AmA) tolagisu; tapipxsu. 
\num{5} (\AmA) (viSayavanunx) horageDahu; bayalumADu; parxkaTisu. 
\enum
\emng
\eentry

\bentry
\word{unlock} 
\pron{anflAkf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ biVgada keYyiMda peTiTxge, bAgilu, \mo vugaLa) biVga tege. 
\num{2} manasusx bicicx heVLu; (guTuTx \mo vanunx) horageDavu, bahiraMgapaDisu. 
\enum
\emng
\eentry

\bentry
\word{unlocked} 
\pron{anflAkfTx} 
\gl{\gu}
\expl{}
\bmng
biVga hAkilalxda; biVga tereda. 
\emng
\eentry

\bentry
\word{unlooked-for} 
\pron{anflukfTxphArf} 
\gl{\gu}
\expl{}
\bmng
aniriVkiSxta; niriVkiSxsilalxda. 
\emng
\eentry

\bentry
\word{unloose} 
\pron{anflUsf} 
\gl{\sakirx}
\expl{}
\bmng
saDilisu; bicucx; kaLacu. 
\emng
\eentry

\bentry
\word{unloosen} 
\pron{anflUsanf} 
\gl{\sakirx}
\expl{}
\bmng
= \hyperlink{unloose}{unloose}. 
\emng
\eentry

\bentry
\word{unlovable} 
\pron{anflavabflf} 
\gl{\gu}
\expl{}
\bmng
pirxVtisalu kaSaTxvAda, sAdhayxvilalxda; pirxVtiyoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{unloved} 
\pron{anflavfDx} 
\gl{\gu}
\bmng
pirxVtipAtarxvAgirada. 
\emng
\eentry

\bentry
\word{unloveliness} 
\pron{anflavflinisf} 
\gl{\nA}
\expl{}
\bmng
anAkaSaRkate; asuMdarate; ramaNiVyavalalxvAgiruvudu. 
\emng
\eentry

\bentry
\word{unlovely} 
\pron{anflavfli} 
\gl{\gu}
\bmng
AkaSaRkavalalxda; ramaNiVyavalalxda; ceMdavilalxda; asuMdara. 
\emng
\eentry

\bentry
\word{unloving} 
\pron{anflaviMgf} 
\gl{\gu}
\bmng
(yAranUnx) pirxVtisada. 
\emng
\eentry

\bentry
\word{unlovingly} 
\pron{anflaviMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
(yAranUnx) pirxVtisade. 
\emng
\eentry

\bentry
\word{unlovingness} 
\pron{anflaviMgfnisf} 
\gl{\nA}
\expl{}
\bmng
(yAranUnx) pirxVtisadiruvike; apirxVti. 
\emng
\eentry

\bentry
\word{unluckily} 
\pron{anflakili} 
\gl{\kirxvi}
\expl{}
\bmng
duradaqSaTxvashAtf; dwBARgayxdiMda. 
\emng
\eentry

\bentry
\word{unluckiness} 
\pron{anflakinisf} 
\gl{\nA}
\expl{}
\bmng
duradaqSaTx; dwBARgayx. 
\emng
\eentry

\bentry
\word{unlucky} 
\pron{anflaki} 
\gl{\gu}
\expl{(\tara\ \eng{unluckier}, \tama\ \eng{unluckiest}). }
\bmng
\bnum
\num{1} (vayxkitxya \vi) adaqSaTxhiVna; duradaqSaTxda; niBARgayx. 
\num{2} niSaPxlavAda; viPala. 
\num{3} hALu; daridarx; keTaTx; aniSaTxda: \eng{some unlucky barber notched my hair} yAvanoV daridarx hajAma nananx kUdalanunx ketitxda. 
\num{4} dushashxkunada; keTaTx gaLigeya; keTaTxdadxnunx taruva yA ashuBa sUcisuva. 
\enum
\emng
\eentry

\bentry
\word{unmade} 
\pron{anfmeVDf} 
\gl{\gu}
\bmng
\bnum
\num{1} mADada; tayArisada; rUpisada. 
\num{2} (DeVge sAkaNe) (sere hiDida DeVgeya \vi) beVTege paLagisada, tarapeVti ilalxda. 
\num{3} nAshamADida. 
\num{4} radudxpaDisida. 
\enum
\emng
\eentry

\bentry
\word{unmaidenly} 
\pron{anfmeVDanfli} 
\gl{\gu}
\expl{}
\bmng
kanAyxhaRvalalxda; kaneyxge -- sahajavalalxda, yoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{unmake} 
\pron{anfmeVkf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unmade}).}
\bmng
\bnum
\num{1} hALumADu; nAshapaDisu. 
\num{2} aLisu; radudxmADu. 
\enum
\emng
\eentry

\bentry
\word{unmalleable} 
\pron{anfmAYxliabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} akuTayx; baDidu tagaDAgisalArada yA reVkugaLAgi mADalAgada. 
\num{2} anamayx; bagigxsalAgada; hoMdikoLuLxvaMte mADalAgada; tidadxlAgada. 
\enum
\emng
\eentry

\bentry
\word{unman} 
\pron{anfmAYxnf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unmanned;} \vakaq\ \eng{unmanning}).}
\bmng
\bnum
\num{1} (haDagu \mo vugaLalilxruva) sibabxMdiyanunx tegeduhAku; sibabxMdiyilalxdaMte mADu. 
\num{2} pwruSa kaLe; dheYyaR uDugisu; edegeDisu; niviRVyaRgoLisu; nisasxtatxvXgoLisu: \eng{the news unmanned him for a while} A sudidx avananunx savxlapxhotutx edegeDisitu. 
\enum
\emng
\eentry

\bentry
\word{unmanageable} 
\pron{anfmAYxnijabflf} 
\gl{\gu}
\expl{}
\bmng
anivaRhaNiVya; (sulaBavAgi) nivaRhisalAgada; saMyoVjisalAgada yA niyaMtirxsalAgada yA hatoVTiyalilxDalAgada. 
\emng
\eentry

\bentry
\word{unmanageableness} 
\pron{anfmAYxnijabflfnisf} 
\gl{\nA}
\expl{}
\bmng
anivaRhaNiVyate; sulaBavAgi nivaRhisalAgadiruvudu. 
\emng
\eentry

\bentry
\word{unmanageably} 
\pron{anfmAYxnijabilx} 
\gl{\kirxvi}
\expl{}
\bmng
nivaRhisalAgadaMte; niyaMtirxsalAgadaMte yA hatoVTiyalilxDalu kaSaTxvAgi. 
\emng
\eentry

\bentry
\word{unmanliness} 
\pron{anfmAYxnflinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} pwruSarahitate; heVDitana. 
\num{2} heNiNxgatana. 
\enum
\emng
\eentry

\bentry
\word{unmanly} 
\pron{anfmAYxnfli} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gaMDasutanavilalxda; pwruSahiVna; aMjikeya; heVDitanada; dubaRlateya. 
\num{2} heNiNxgatanada. 
\enum
\emng
\eentry

\bentry
\word{unmanned} 
\pron{anfmAYxnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (neVravAgi niyaMtirxsuva) cAlakanilalxda: \eng{an unmanned spacecraft} cAlakanilalxda aMtarikaSx nwke. 
\num{2} BAvakokxLagAda; BAvavashanAda. 
\enum
\emng
\eentry

\bentry
\word{unmannerliness} 
\pron{anfmAYxnarflinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} mayARdegeTaTx naDavaLike; saBayxtegeTiTxruvudu. 
\num{2} (naDenuDigaLa \vi) oraTutana; nayavinayagaLilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unmannerly} 
\pron{anfmAYxnarfli} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mayARdegeTaTx; saBayxvalalxda; naDategeTaTx. 
\num{2} (naDenuDigaLa \vi) oraTa; asaMsakxqqta; nayavinayavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unmarked} 
\pron{anfmAkfRTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gurutu hAkilalxda; gurutisilalxda. 
\num{2} gamanisada. 
\num{3} gAyada kale, gurutu, \mo vugaLilalxda: \eng{face unmarked by the accident} apaGAtada gurutugaLoMdU kANisada muKa. 
\num{4} tidudxpaDi, cihenxgaLu, \mo vugaLilalxda: \eng{an unmarked copy of the report} tidudxpaDi movugaLilalxda varadiya parxti. 
\num{5} (pariVkeSxya utatxrapatirxke \mo vugaLa \vi) inUnx mwlayxmApana mADada. 
\num{6} (samAdhiya \vi) yAradedxMba gurutilalxda. 
\enum
\emng
\eentry

\bentry
\word{unmarketable} 
\pron{anfmAkiRTabflf} 
\gl{\gu}
\expl{}
\bmng
avikerxVya; mArATa mADalAgada; mAralu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unmarriageable} 
\pron{anfmAYxrijabflf} 
\gl{\gu}
\expl{}
\bmng
vivAhayoVgayxvalalxda; vivAhasAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unmarried} 
\pron{anfmAYxriDf} 
\gl{\gu}
\expl{}
\bmng
maduveyAgada; maduveyilalxda; avivAhita. 
\emng
\eentry

\bentry
\word{unmasculine} 
\pron{anfmAYxsukxyXlinf} 
\gl{\gu}
\expl{}
\bmng
gaMDasutanavilalxda; puMsatxvXvilalxda. 
\emng
\eentry

\bentry
\word{unmask} 
\pron{anfmAsfkx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} muKavADa kaLacu; musuku tege. 
\num{2} nijavAda savxBAvavanunx -- bayalu mADu, raTuTxmADu, bahiraMga paDisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
(tananx nijasavxBAva \mo vanunx) parxkaTisu; toVpaRDisiko. 
\emng
\eentry

\bentry
\word{unmasker} 
\pron{anfmAsakxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} muKavADa kaLacuvavanu. 
\num{2} (nijavAda savxBAvavanunx) bayalu mADuvavanu. 
\enum
\emng
\eentry

\bentry
\word{unmatchable} 
\pron{anfmAYxcabflf} 
\gl{\gu}
\expl{}
\bmng
sarijoVDi hoMdisalAgada; sarisamagoLisalu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unmatchably} 
\pron{anfmAYxcabilx} 
\gl{\kirxvi}
\expl{}
\bmng
sarijoVDi hoMdisalAgade. 
\emng
\eentry

\bentry
\word{unmatched} 
\pron{anfmAYxcfTx} 
\gl{\gu}
\expl{}
\bmng
eNeyilalxda; IDilalxda; (sari)sATiyilalxda; asamAna. 
\emng
\eentry

\bentry
\word{unmatured} 
\pron{anfmaTuyx(ca)aDfR} 
\gl{\gu}
\expl{}
\bmng
apakavx; pUtiRyAgi beLeyada, baliyada. 
\emng
\eentry

\bentry
\word{unmeaning} 
\pron{anfmiVniMgf} 
\gl{\gu}
\expl{}
\bmng
athaRvilalxda; asaMbadadhx; athaRshUnayx. 
\emng
\eentry

\bentry
\word{unmeaningly} 
\pron{anfmiVniMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
athaRvilalxda riVtiyalilx; asaMbadadhxvAgi. 
\emng
\eentry

\bentry
\word{unmeaningness} 
\pron{anfmiVniMgfnisf} 
\gl{\nA}
\expl{}
\bmng
athaRshUnayxte; asaMbadadhxte. 
\emng
\eentry

\bentry
\word{unmeant} 
\pron{anfmeMTf} 
\gl{\gu}
\bmng
udedxVshisilalxda; anudidxSaTx. 
\emng
\eentry

\bentry
\word{unmeasurable} 
\pron{anfmeSaZrabflf} 
\gl{\gu}
\expl{}
\bmng
aLeyalAgada; aparimeVya; ameVya. 
\emng
\eentry

\bentry
\word{unmeasured} 
\pron{anfmeSaZDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aLeyada. 
\num{2} mitiyilalxda; asiVma; vipariVtada: \eng{unmeasured abuse} mitiyilalxda beYguLa. 
\num{3} (saMgiVta, kavanagaLa \vi) CaMdoVbadadhxvalalxda; layabadadhxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unmeet} 
\pron{anfmiVTf} 
\gl{\gu}
\bmng
(\pArxparx) takukxdalalxda; anucita; ayukatx. 
\emng
\eentry

\bentry
\word{unmeetly} 
\pron{anfmiVTfli} 
\gl{\kirxvi}
\expl{}
\bmng
(\pArxparx) takukxdalalxda riVtiyalilx; anucitavAgi; yukatxvAgirade. 
\emng
\eentry

\bentry
\word{unmeetness} 
\pron{anfmiVTfnisf} 
\gl{\nA}
\expl{}
\bmng
anwcitayx; takukxdalalxvAgiruvudu; yukatxvalalxdiruvike. 
\emng
\eentry

\bentry
\word{unmelodious} 
\pron{anfmiloVDiasf} 
\gl{\gu}
\expl{}
\bmng
iMpAgilalxda; shurxtimadhuravalalxda; kivige iMpAgada; sushArxvayxvAgirada; apasavxradiMda kUDida. 
\emng
\eentry

\bentry
\word{unmelodiously} 
\pron{anfmiloVDiasfli} 
\gl{\kirxvi}
\expl{}
\bmng
iMpAgilalxde; sushArxvayxvAgirade; apasavxravAgi; kakaRshavAgi. 
\emng
\eentry

\bentry
\word{unmelted} 
\pron{anfmeliTxDf} 
\gl{\gu}
\expl{}
\bmng
karagada. 
\emng
\eentry

\bentry
\word{unmemorable} 
\pron{anfmemarabflf} 
\gl{\gu}
\expl{}
\bmng
samxraNiVyavalalxda; samxraNayoVgayxvalalxda; nenapiDalu takukxdalalxda. 
\emng
\eentry

\bentry
\word{unmemorably} 
\pron{anfmemarabilx} 
\gl{\kirxvi}
\expl{}
\bmng
samxraNiVyavAgirade; samxraNayoVgayxvalalxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{unmentionability} 
\pron{anfmenaSxnabiliTi} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unmentionableness}{unmentionableness}. 
\emng
\eentry

\bentry
\word[unmentionable(1)]{unmentionable} 
\pron{anfmenaSxnabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (sariyAgi) ulelxVKisalAgada; heVLalAgada. 
\num{2} heVLalu takukxdalalxda; avAcayx; anulelxVKaniVya; ulelxVKanAhaRvalalxda; ulelxVKisalu yoVgayxvalalxda. 
\enum
\emng
\eentry

\bentry
\word[unmentionable(2)]{unmentionable} 
\pron{anfmenaSxnabflf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} (\bava dalilx) (\pArxparx) ijAra; SarAyi. 
\alnum{b} (\hA) oLauDupu. 
\eanum
\numie
\num{2} avacaniVya; hesaru heVLabArada vasutx yA vayxkitx. 
\enum
\emng
\eentry

\bentry
\word{unmentionableness} 
\pron{anfmenaSxnabflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (sariyAgi) heVLalAgadaMtiruvike. 
\num{2} heVLalu takukxdAgilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unmentionably} 
\pron{anfmenaSxnabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (sariyAgi) heVLalAgadaMte. 
\num{2} heVLalu takukxdAgilalxdaMte. 
\enum
\emng
\eentry

\bentry
\word{unmentioned} 
\pron{anfmenaSxnfDx} 
\gl{\gu}
\expl{}
\bmng
heVLirada; ulelxVKisada; anukatx. 
\emng
\eentry

\bentry
\word{unmerchantable} 
\pron{anfmacaRMTabflf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unmarketable}{unmarketable}. 
\emng
\eentry

\bentry
\word{unmerciful} 
\pron{anfmasiRphulf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nidaRyavAda; niSakxruNeya; dayedAkiSxNayxgaLilalxda. 
\num{2} atayxMta hecicxna; vipariVtavAda: \eng{an unmerciful length of time} atayxMta diVGaR avadhiya kAla. 
\enum
\emng
\eentry

\bentry
\word{unmercifully} 
\pron{anfmasiRphuli} 
\gl{\kirxvi}
\expl{}
\bmng
nidaRyavAgi; niSakxruNeyiMda. 
\emng
\eentry

\bentry
\word{unmercifulness} 
\pron{anfmasiRphulfnisf} 
\gl{\nA}
\expl{}
\bmng
nidaRyate; niSAkxruNayx. 
\emng
\eentry

\bentry
\word{unmerited} 
\pron{anfmeriTiDf} 
\gl{\gu}
\expl{}
\bmng
takukxdalalxda; ahaRvalalxda; anahaRvAda. 
\emng
\eentry

\bentry
\word{unmet} 
\pron{anfmeTf} 
\gl{\gu}
\bmng
(beVDike, guri, \mo vugaLa \vi) sAdhisada; pUreYsada; viPalavAda. 
\emng
\eentry

\bentry
\word{unmetalled} 
\pron{anfmeTalfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} loVha hodisada. 
\num{2} (\birx) (rasetxya \vi) jalilxhAki durasutx mADada. 
\enum
\emng
\eentry

\bentry
\word{unmethodical} 
\pron{anfmetADikalf} 
\gl{\gu}
\expl{}
\bmng
vayxvasethxyilalxda; riVtirivAjilalxda; karxmavilalxda. 
\emng
\eentry

\bentry
\word{unmethodically} 
\pron{anfmetADikali} 
\gl{\kirxvi}
\expl{}
\bmng
avayxvasithxtavAgi; riVtirivAjilalxde. 
\emng
\eentry

\bentry
\word{unmetrical} 
\pron{anfmeTirxkalf} 
\gl{\gu}
\expl{}
\bmng
CaMdasisxlalxda; CaMdoVbadadhxvalalxda; layabadadhxvalalxda. 
\emng
\eentry

\bentry
\word{unmilitary} 
\pron{anfmiliTari} 
\gl{\gu}
\expl{}
\bmng
aseYnika: 
\banum
\alnum{a} seYnikarige saMbaMdhisada. 
\alnum{b} seYnikarige takukxdalalxda. 
\eanum
\emng
\eentry

\bentry
\word{unmindful} 
\pron{anfmeYMDf{phu}lf} 
\gl{\gu}
\expl{}
\bmng
lekikxsada; lakiSxsada; gamanisada; nilaRkaSxyXda; upeVkeSxya. 
\emng
\eentry

\bentry
\word{unmindfully} 
\pron{anfmeYMDf{phu}li} 
\gl{\kirxvi}
\expl{}
\bmng
lekikxsade; gamanisade; nilaRkaSxyXdiMda; upeVkeSxyiMda. 
\emng
\eentry

\bentry
\word{unmindfulness} 
\pron{anfmeYMDf{phu}lfnisf} 
\gl{\nA}
\expl{}
\bmng
gamanisadiruvike; nilaRkaSxyX; upeVkeSx. 
\emng
\eentry

\bentry
\word{unmissable} 
\pron{anfmisabflf} 
\gl{\gu}
\expl{}
\bmng
(guri, avakAsha, daqshayx, vayxkitxyoDane BeVTi, \mo vugaLa \vi) tapipxsikoLaLxlAgada yA tapipxsikoLaLxkUDada. 
\emng
\eentry

\bentry
\word{unmistakability} 
\pron{anfmiseTxVkabiliTi} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unmistakableness}{unmistakableness}. 
\emng
\eentry

\bentry
\word{unmistakable} 
\pron{anfmiseTxVkabflf} 
\gl{\gu}
\expl{}
\bmng
tapupx BAvisalu avakAshaveV ilalxda; saMdeVhAsapxdavalalxda; susapxSaTx; nicacxLavAda; nisasxMdigadhx: \eng{are black clouds an unmistakable sign of rain?} kapupx moVDagaLu maLe baruva sapxSaTx sUcanegaLeV? 
\emng
\eentry

\bentry
\word{unmistakableness} 
\pron{anfmiseTxVkabflfnisf} 
\gl{\nA}
\expl{}
\bmng
tapupx BAvisalu avakAshaveV ilalxdaMtiruvike; susapxSaTxte; asaMdigadhxte. 
\emng
\eentry

\bentry
\word{unmistakably} 
\pron{anfmiseTxVkabilx} 
\gl{\kirxvi}
\expl{}
\bmng
saMdeVhAsapxdavalalxdaMte; tapApxgi tiLiyalu sAdhayxvilalxda hAge; susapxSaTxvAgi; nicacxLavAgi. 
\emng
\eentry

\bentry
\word{unmistaken} 
\pron{anfmiseTxVkanf} 
\gl{\gu}
\expl{}
\bmng
tapApxgi BAvisada, tiLiyada; sariyAda. 
\emng
\eentry

\bentry
\word{unmitigated} 
\pron{anfmiTigeVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} savxlapxvU kaDameyAgada; swmayxvAgada. 
\num{2} shudAdhxMgavAda; pakAkx; apapxTa; pUrA: \eng{an unmitigated scoundrel} shudAdhxMgavAda paTiMga; pakAkx dagAkoVra. 
\enum
\emng
\eentry

\bentry
\word{unmixed} 
\pron{anfmikfsxTf} 
\gl{\gu}
\expl{}
\bmng
beresada; berakeyilalxda; amishirxta; shudadhx; apapxTa. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{unmixed blessing} shudAdhxnugarxha; keVvalAnugarxha; anukUlagaLu mAtarx idudx ananukUlagaLeV ilalxda vasutx, vara. 
\emng
\eentry

\bentry
\word{unmodified} 
\pron{anfmADipheYDf} 
\gl{\gu}
\expl{}
\bmng
(savxlapxvU) vayxtAyxsagoLisada; avikaqta; mApaRDisada; badalAyisada. 
\emng
\eentry

\bentry
\word{unmodulated} 
\pron{anfmADuyxleVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aLavaDisada; hoMdisada. 
\num{2} (\dhavxni) shurxtigUDisada. 
\num{3} (\saM) sAthxyigaLu badalAyisada; EkatAnada. 
\enum
\emng
\eentry

\bentry
\word{unmolested} 
\pron{anfmalesiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} piVDege, halelxge -- oLagAgada. 
\num{2} atAyxcArakokxLagAgada. 
\enum
\emng
\eentry

\bentry
\word{unmoor} 
\pron{anfmuarf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (haDagina) kaTuTxgaLanunx kaLacu, tegeduhAku (\rUpa\ saha). 
\num{2} (eraDu yA hecicxna laMgarugaLalilx) oMdu laMgaru etutx. 
\enum
\emng
\eentry

\bentry
\word{unmoral} 
\pron{anfmAralf} 
\gl{\gu}
\expl{}
\bmng
niVtige saMbaMdhisilalxda; niVtivivakeSxyilalxda. 
\emng
\eentry

\bentry
\word{unmorality} 
\pron{anfmarAYxliTi} 
\gl{\nA}
\expl{}
\bmng
nineRYtikate; niVtinirapeVkaSxte; niVtige saMbaMdhisilalxdiruvike. 
\emng
\eentry

\bentry
\word{unmorally} 
\pron{anfmArali} 
\gl{\kirxvi}
\expl{}
\bmng
niVtivivakeSxyilalxde; niVtinirapeVkaSxvAgi; niVtige -- saMbaMdhisirade, saMbaMdhapaTiTxrade. 
\emng
\eentry

\bentry
\word{unmortise} 
\pron{anfmATiRsf} 
\gl{\sakirx}
\expl{}
\bmng
(kUlu kUrisi) BadarxvAgi joVDisiruvudanunx -biDisu, parxteyxVkisu; kUliniMda tegeduhAku; vejajxdiMda kaLacu. 
\emng
\eentry

\bentry
\word{unmotherly} 
\pron{anfmadarfli} 
\gl{\gu}
\expl{}
\bmng
mAtaqsahajavalalxda; tAyiyAdavaLige takukxdalalxda; mAtaqBAva yA vAtasxlayx ilalxda. 
\emng
\eentry

\bentry
\word{unmotivated} 
\pron{anfmoVTiveVTiDf} 
\gl{\gu}
\expl{}
\bmng
anudidxSaTx yA aperxVrita; perxVraNe yA udedxVshavilalxda. 
\emng
\eentry

\bentry
\word{unmounted} 
\pron{anfmwMTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (poliVsinavana \vi) kudureyeVrilalxda. 
\num{2} (citarx) raTiTxge hacicxlalxda; benunx raTiTxlalxda. 
\num{3} (ratanx \mo vu) gUDinalilx kUDisilalxda, keVvaNisilalxda, KacitamADilalxda. 
\enum
\emng
\eentry

\bentry
\word{unmourned} 
\pron{anfmwnfRDx} 
\gl{\gu}
\expl{}
\bmng
(maqta vayxkitx, naSaTx, duradaqSaTx, kaLeduhoVda saMgati, \mo vugaLa \vi) shoVkisada; duHKisada; marugada; goVLADada; vayxthepaDada. 
\emng
\eentry

\bentry
\word{unmovable} 
\pron{anfmUvabflf} 
\gl{\gu}
\expl{}
\bmng
(\eng{unmoveable} eMdU \parx) calisalAgada; acala. 
\emng
\eentry

\bentry
\word{unmoved} 
\pron{anfmUvfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} calisada; acala. 
\num{2} daqDhasaMkalapxda; daqDha udedxVshada. 
\num{3} (rAgaBAvagaLiMda) manoVvikAra hoMdada; BAvavikAragaLilalxda. 
\num{4} (manasusx) caMcalavAgada; akuSxbadhx. 
\enum
\emng
\eentry

\bentry
\word{unmown} 
\pron{anfmoVnf} 
\gl{\gu}
\bmng
\bnum
\num{1} (peYru \mo vugaLa \vi) koyayxda; koyulx mADada; kaTAvu mADada. 
\num{2} katatxrisi hAkada; taridu hAkada. 
\enum
\emng
\eentry

\bentry
\word{unmuffle} 
\pron{anfmaphalf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} (DoVlu, gaMTe, \mo vugaLiMda) shabadxmaMdakAriyanunx athavA nishashxbadxkavanunx tege: 
\banum
\alnum{a} (gaMTe) shabadxvAguvaMte mADu. 
\alnum{b} (DoVlu) shabadx maMdavAgadaMte mADu. 
\eanum
\numie
\num{2} (muKa \mo vugaLiMda) musuku tege. 
\enum
\emng
\eentry

\bentry
\word{unmurmuring} 
\pron{anfmamaRriMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} goNaguTaTxda; cakAravetatxda; asamAdhAna sUcisada, heVLikoLaLxda. 
\num{2} (kaSaTxniSuThxragaLanunx) mwnavAgi sahisikoLuLxva. 
\enum
\emng
\eentry

\bentry
\word{unmurmuringly} 
\pron{anfmamaRriMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} goNaguTaTxde; asamAdhAna sUcisade; cakAravetatxde. 
\num{2} (kaSaTx \mo vanunx) mwnavAgi sahisikoMDu. 
\enum
\emng
\eentry

\bentry
\word{unmusical} 
\pron{anfmUyxsiZkalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kivige hitavalalxda; shArxvayxvalalxda; iMpAgilalxda. 
\num{2} saMgiVtadalilx kwshalavilalxda, aBiruciyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unmusicality} 
\pron{anfmUyxsiZkAYxliTi} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unmusicalness}{unmusicalness}. 
\emng
\eentry

\bentry
\word{unmusically} 
\pron{anfmUyxsiZkali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kivige hitavalalxdaMte; sushArxvayxvAgirade. 
\num{2} saMgiVtadalilx -- kwshalavilalxde, aBiruciyilalxde. 
\enum
\emng
\eentry

\bentry
\word{unmusicalness} 
\pron{anfmUyxsiZkalfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kivige hitavalalxdiruvike; sushArxvayxvAgiradiruvike. 
\num{2} saMgiVtadalilx -- kwshalavilalxdiruvudu, aBiruciyilalxdiruvudu. 
\enum
\emng
\eentry

\bentry
\word{unmutilated} 
\pron{anfmUyxTileVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (pArxNi \mo vugaLa \vi) aMgahiVna mADada. 
\num{2} (garxMtha \mo vugaLa \vi) UnavAgada; katatxrisi hAkada; apUNaRvAgirada. 
\enum
\emng
\eentry

\bentry
\word{unmuzzle} 
\pron{anfmasaZlf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} bAyi -- kalilx, kukekx tege. 
\num{2} (\kanmu\ \rUpa) mAtanADalu avakAsha, sAvxtaMtarxyX -- koDu; mwnavAgirabeVkeMba baMdhanavanunx tege. 
\enum
\emng
\eentry 

\bentry
\word{unnail} 
\pron{anfneVlf} 
\gl{\sakirx}
\expl{}
\bmng
moLe hoDedu Badarx paDisiruvudanunx -- kaLacu, bicucx, kitutxhAku. 
\emng
\eentry

\bentry
\word{unnameable} 
\pron{anfneVmabflf} 
\gl{\gu}
\expl{}
\bmng
hesaru heVLalAgada; avAcayx; (\kanmu) hesarisalAgadaSuTx GoVravAda. 
\emng
\eentry

\bentry
\word{unnamed} 
\pron{anfneVmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anAmi(ka); hesariDada; hesarilalxda. 
\num{2} hesariTuTx kareyada; hesaru heVLi sUcisada. 
\enum
\emng
\eentry

\bentry
\word{unnational} 
\pron{anfnAYxSanalf} 
\gl{\gu}
\expl{}
\bmng
arASiTxrXVya; rASiTxrXVyavalalxda; rASaTxrXkekx vishiSaTxvalalxda; deVshiVya lakaSxNavilalxda. 
\emng
\eentry

\bentry
\word{unnatural} 
\pron{anfnAYxcaralf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asAvxBAvika; asahaja; parxkaqtivirudadhx. 
\num{2} GoVra; kUrxra; duSaTx. 
\num{3} asahaja; sAvxBAvika BAvaBAvanegaLilalxda: \eng{a mother who is cruel to her children is unnatural} tAyi tananx makakxLa viSayadalilx kUrxravAgiruvudu asAvxBAvika. 
\num{4} kaqtaka; soVgina; naTaneya. 
\enum
\emng
\eentry

\bentry
\word{unnaturally} 
\pron{anfnAYxcarali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} asAvxBAvikavAgi; asahajavAgi; parxkaqtige virudadhxvAgi. 
\num{2} GoVrariVtiyalilx. 
\num{3} kaqtakavAgi; soVgu hAkutAtx. 
\enum
\emng
\eentry

\bentry
\word{unnaturalness} 
\pron{anfnAYxcuralfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} asAvxBAvikate; asahajate; parxkaqtige virudadhxvAgiruvudu. 
\num{2} kaqtakate; soVgu. 
\enum
\emng
\eentry

\bentry
\word{unnavigability} 
\pron{anfnAYxvigabiliTi} 
\gl{\nA}
\expl{}
\bmng
nwkAsaMcArakekx yoVgayxvAgilalxdiruvike. 
\emng
\eentry

\bentry
\word{unnavigable} 
\pron{anfnAYxvigabflf} 
\gl{\gu}
\expl{}
\bmng
nwkAsaMcArakekx yoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{unnecessarily} 
\pron{anfnesasarili} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anAvashayxkavAgi; anagatayxvAgi. 
\num{2} agatayx miVri; AvashayxkategiMta hecAcxgi. 
\enum
\emng
\eentry

\bentry
\word{unnecessariness} 
\pron{anfnesasarinisf} 
\gl{\nA}
\expl{}
\bmng
anAvashayxkate; anagatayxvAgiruvike. 
\emng
\eentry

\bentry
\word[unnecessary(1)]{unnecessary} 
\pron{anfnesasari} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Avashayxkavalalxda; agatayxvalalxda. 
\num{2} beVkAdadxkikxMta hecicxna; anagatayxvAda; agatayx miVrida; anAvashayxkavAda: \eng{with unnecessary caution} anagatayxvAda ecacxrikeyiMda. 
\enum
\emng
\eentry

\bentry
\word[unnecessary(2)]{unnecessary} 
\pron{anfnesasari} 
\gl{\nA}
\expl{(\bava\ \eng{unnecessaries}). }
\bmng
(\sA\ \bava dalilx) anagatayx vasutx. 
\emng
\eentry

\bentry
\word{unneeded} 
\pron{anfniVDiDf} 
\gl{\gu}
\bmng
anAvashayxka; agatayx ilalxda; beVkilalxda. 
\emng
\eentry

\bentry
\word{unneighbourliness} 
\pron{anfneVbaliRnisf} 
\gl{\nA}
\expl{}
\bmng
vishAvxsABAva; senxVhaparavalalxdiruvike. 
\emng
\eentry

\bentry
\word{unneighbourly} 
\pron{anfneVbarfli} 
\gl{\gu}
\expl{}
\bmng
nerehoreyavara bagegx pirxVtivishAvxsagaLilalxda; senxVhaparavalalxda. 
\emng
\eentry

\bentry
\word{unnerve} 
\pron{anfnavfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} dheYyaRgeDisu; edeguMdisu; citatxsethxYyaRvilalxda hAge mADu. 
\num{2} (naragaLa) shakitxyuDugisu. 
\enum
\emng
\eentry

\bentry
\word{unnervingly} 
\pron{anfnaviRMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
dheYyaRgeDisuvaMte; edeguMdisuvaMte; shakitxyuDugisuvaMte; niviRVyaRgoLisuvaMte. 
\emng
\eentry

\bentry
\word{unnoticeable} 
\pron{anfnoVTisabalf} 
\gl{\gu}
\expl{}
\bmng
sulaBavAgi kANada yA gamanisalAgada. 
\emng
\eentry

\bentry
\word{unnoticeably} 
\pron{anfnoVTisabilx} 
\gl{\kirxvi}
\expl{}
\bmng
sulaBavAgi kANade yA gamanisalAgade. 
\emng
\eentry

\bentry
\word{unnoticed} 
\pron{anfnoVTisfTx} 
\gl{\gu}
\expl{}
\bmng
gamanisada; lakiSxsada; gamanakekx bArada; daqSiTxgoVcaravAgada: \eng{the event passed unnoticed} A GaTane naDedadudx yAra gamanakUkx barade hoVyitu. \eng{shall you let the insult pass unnoticed ?} ninage Ada avamAnada bagegx ninage savxlapxvU lakaSxyXvilalxve? 
\emng
\eentry

\bentry
\word{unnumbered} 
\pron{anfnaMbaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMKeyx hAkilalxda; saMKeyxgurutu hAkada. 
\num{2} eNisada. 
\num{3} eNike mADalAgada; asaMKAyxta; agaNita. 
\enum
\emng
\eentry

\bentry
\word{UNO} 
\pron{yUnoV} 
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Nations Organization.} 
\emng
\eentry

\bentry
\word{unobjectionable} 
\pron{anabejxkfSanabflf} 
\gl{\gu}
\expl{}
\bmng
nirAkeSxVpavAda; AkeSxVpaNAhaRvalalxda; AkeSxVpisalAgada; samamxtisabahudAda. 
\emng
\eentry

\bentry
\word{unobjectionableness} 
\pron{anabejxkaSxnabalfnisf} 
\gl{\nA}
\expl{}
\bmng
nirAkeSxVpate; samamxtisabahudAgiruvike. 
\emng
\eentry

\bentry
\word{unobjectionably} 
\pron{anabejxkaSxnabilx} 
\gl{\kirxvi}
\expl{}
\bmng
nirAkeSxVpavAgi; samamxtisuva hAge. 
\emng
\eentry

\bentry
\word{unobliging} 
\pron{anabelxYjiMgf} 
\gl{\gu}
\expl{}
\bmng
neravu, sahakAra -- niVDada. 
\emng
\eentry

\bentry
\word{unobscured} 
\pron{anabfsukxyXaDfR} 
\gl{\gu}
\expl{}
\bmng
daqSiTxge maremADada; sapxSaTxgoVcaravAda. 
\emng
\eentry

\bentry
\word{unobservable} 
\pron{anabasxvaRbalf} 
\gl{\gu}
\expl{}
\bmng
dugArxRhayx; aviVkaSxyX; agoVcara; kaNiNxge kANada; daqSiTxgoVcaravalalxda. 
\emng
\eentry

\bentry
\word{unobservant} 
\pron{anabfsaZvaRMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gamanaviTuTx -- Acarisada, naDesada, pAlisada. 
\num{2} lakaSxyXviTuTx, sharxdedhxyiMda -- gamanisada; ajAgarUka. 
\enum
\emng
\eentry

\bentry
\word{unobservantly} 
\pron{anabfsaZvaRMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} gamanaviTuTx naDesade, pAlisade. 
\num{2} lakaSxyXviTuTx gamanisade. 
\enum
\emng
\eentry

\bentry
\word{unobserved} 
\pron{anabfsaZvfRDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Acarisada; anusarisada. 
\num{2} lakaSxyXviTuTx noVDada; aviVkiSxta; gamanisada. 
\enum
\emng
\eentry

\bentry
\word{unobservedly} 
\pron{anabfsaZviRDfli} 
\gl{\kirxvi}
\expl{}
\bmng
gamanisadaMte; daqSiTxge biVLadaMte; daqSiTx goVcaravAgade. 
\emng
\eentry

\bentry
\word{unobstructed} 
\pron{anabfsaTxrXkiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} taDeyada; taDeyoDaDxda; taDegaTaTxda. 
\num{2} aDiDx ilalxda; aDacaNe ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unobtainable} 
\pron{anabeTxVnabflf} 
\gl{\gu}
\expl{}
\bmng
apArxpayx; paDeyalAgada; dorakisikoLaLxlu sAdhayxvilalxda; saMpAdisalAgada. 
\emng
\eentry

\bentry
\word{unobtrusive} 
\pron{anabfTUrxsivf} 
\gl{\gu}
\expl{}
\bmng
daqSiTxge, kaNiNxge -- biVLada; gamanakekx bArada; mareya; guTiTxna. 
\emng
\eentry

\bentry
\word{unobtrusively} 
\pron{anabfTUrxsivfli} 
\gl{\kirxvi}
\expl{}
\bmng
daqSiTxge, kaNiNxge -- biVLadaMte; gamanakekx bAradaMte; guTiTxnalilx; mareyalilx. 
\emng
\eentry

\bentry
\word{unobtrusiveness} 
\pron{anabfTUrxsivfnisf} 
\gl{\nA}
\expl{}
\bmng
daqSiTxge biVLadiruvike; gamanakekx baradiruvike. 
\emng
\eentry

\bentry
\word{unoccupied} 
\pron{anfAkuyxpeYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (maneya \vi) vAsa mADuvavaru ilalxda; okakxlilalxda; vAsisuvavarilalxde KAli iruva. 
\num{2} kelasavilalxda; kelasa mADutitxlalxda; virAmada; vishArxMti anuBavisutitxruva. 
\enum
\emng
\eentry

\bentry
\word{unoffended} 
\pron{anapheMDiDf} 
\gl{\gu}
\expl{}
\bmng
manasisxge noVvAgada. 
\emng
\eentry

\bentry
\word{unoffending} 
\pron{anapheMDiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} manasusx noVyisada. 
\num{2} nidoRVSi; apacAravesagada; niyama ulalxMGisada; aparAdhavesagada. 
\num{3} hAni mADada; apAyavilalxda; nirapAyada. 
\num{4} saraLa; niSakxpaTa. 
\enum
\emng
\eentry

\bentry
\word{unofficial} 
\pron{anaphiSalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anadhikaqta; adhikArayutavalalxda. 
\num{2} adhikArigaLa lakaSxNavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unofficially} 
\pron{anaphiSali} 
\gl{\kirxvi}
\expl{}
\bmng
anadhikaqtavAgi; adhikArayutavalalxda riVtiyalilx. 
\emng
\eentry

\bentry
\wordnospeech{unofficial strike}{unofficial strike}
\pron{?}
\gl{\nA}
\expl{}
\bmng
kAnUnubAhiravAda saMpu; anadhikaqta muSakxra; muSakxra naDesuvavaru tamamx vaqtitxsaMGa yA TerxVDf yUniyaninxna opipxge ilalxde naDesuva saMpu yA muSakxra. 
\emng
\eentry

\bentry
\word{unoiled} 
\pron{anfAilfDx} 
\gl{\gu}
\bmng
eNeNx -- hacacxda, baLiyada, hAkada. 
\emng
\eentry

\bentry
\word{unopened} 
\pron{anfOpanfDx} 
\gl{\gu}
\expl{}
\bmng
tereyada; bicacxda. 
\emng
\eentry

\bentry
\word{unopposed} 
\pron{anapoVsfDx} 
\gl{\gu}
\expl{}
\bmng
aviroVdha(vAda); parxtiBaTaneilalxda; taDeyoDiDxrada; aDacaNeilalxda. 
\emng
\eentry

\bentry
\word{unordained} 
\pron{anfADeRVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kerxY) vidhipUvaRkavAgi (pAdirx) diVkeSx niVDirada. 
\num{2} vidhipUvaRkavAgi, kAnUnubadadhxvAgi -- vidhisada, kaTaTxLe mADada, niyamisada. 
\enum
\emng
\eentry

\bentry
\word{unordinary} 
\pron{anfADiRnari} 
\gl{\gu}
\expl{}
\bmng
sAmAnayx riVtiyadalalxda; visheVSavAda; asAdhAraNavAda. 
\emng
\eentry

\bentry
\word{unorganised} 
\pron{anfAgaRneYsfDx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unorganized}{unorganized}. 
\emng
\eentry

\bentry
\word{unorganized} 
\pron{anfAgaRneYsfZDx} 
\gl{\gu}
\expl{}
\bmng
vayxvasethx ilalxda; saMGaTane ilalxda; asaMGaTita; sariyAda EpARDu ilalxda. 
\emng
\eentry

\bentry
\word{unoriginal} 
\pron{anarijinalf} 
\gl{\gu}
\expl{}
\bmng
sovxVpajacnxteyilalxda; savxMtikeyilalxda; savxMta parxtiBe, kalapxnAshakitx -- ilalxda; matotxbabxriMda tegedukoMDa, paDeda. 
\emng
\eentry

\bentry
\word{unoriginality} 
\pron{anarijinAYxliTi} 
\gl{\nA}
\expl{}
\bmng
sovxVpajacnxteya aBAva; savxMtikeya rAhitayx; savxMta parxtiBe, kalapxnAshakitx ilalxdiruvike. 
\emng
\eentry

\bentry
\word{unoriginally} 
\pron{anarijinali} 
\gl{\kirxvi}
\expl{}
\bmng
savxMtikeyilalxde; savxMta parxtiBe ilalxde; tananxdeV Ada kalapxnAshakitx ilalxde; sovxVpajacnxteyilalxde. 
\emng
\eentry

\bentry
\word{unornamental} 
\pron{anAnaRmeMTalf} 
\gl{\gu}
\expl{}
\bmng
alaMkAravAgirada; suMdaravAgirada. 
\emng
\eentry

\bentry
\word{unornamented} 
\pron{anAnaRmeMTiDf} 
\gl{\gu}
\expl{}
\bmng
niralaMkaqta; analaMkaqta; alaMkAra -- ilalxda, mADirada; alaMkarisada. 
\emng
\eentry

\bentry
\word{unorthodox} 
\pron{anAtaRDAkfsx} 
\gl{\gu}
\expl{}
\bmng
saMparxdAyabadadhxvalalxda; shAsatxrXsamamxtavalalxda; AcArasamamxtavalalxda; asAMparxdAyikavAda. 
\emng
\eentry

\bentry
\word{unorthodoxly} 
\pron{anAtaRDAkfsxli} 
\gl{\kirxvi}
\expl{}
\bmng
asAMparxdAyikavAgi; saMparxdAyakekx virudadhxvAgi. 
\emng
\eentry

\bentry
\word{unorthodoxy} 
\pron{anAtaRDAkisx} 
\gl{\nA}
\expl{}
\bmng
asAMparxdAyikate; saMparxdAya viroVdha. 
\emng
\eentry

\bentry
\word{unostentatious} 
\pron{anAseTxMTeVSasf} 
\gl{\nA}
\expl{}
\bmng
nirADaMbarada; DAMBikavalalxda; ADaMbaravilalxda; saraLa. 
\emng
\eentry

\bentry
\word{unostentatiously} 
\pron{anAseTxMTeVSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
DAMBikateyilalxdaMte; ADaMbaravilalxde; nirADaMbaravAgi; saraLavAgi. 
\emng
\eentry

\bentry
\word{unostentatiousness} 
\pron{anAseTxMTeVSasfnisf} 
\gl{\nA}
\expl{}
\bmng
nirADaMbara(te); DAMBikateyilalxdiruvike; ADaMbaravilalxdiruvike; saraLate. 
\emng
\eentry

\bentry
\word{unowned} 
\pron{anoVnfDx} 
\gl{\gu}
\bmng
\bnum
\num{1} sAvxmayx paDeyada; sAvxmayxrahita; sAvxmAyxBAvada; oDeyarilalxda. 
\num{2} (sAvxmayx \mo vanunx) tananxdeMdu -- opipxkoLaLxda, aMgiVkarisada, sivxVkarisada. 
\enum
\emng
\eentry

\bentry
\word{unpack} 
\pron{anfpAYxkf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} gaMTumUTe -- tere, bicucx. 
\numi{2}
\banum
\alnum{a} (vasutx \mo vanunx) mUTeyiMdAcege tege. 
\alnum{b} (mUTe \mo vugaLalilx) aDakidadxnunx Acege, horakekx tege. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{unpacker} 
\pron{anfpAYxkarf} 
\gl{\nA}
\expl{}
\bmng
gaMTumUTe -- tereyuvavanu, bicucxvavanu. 
\emng
\eentry

\bentry
\word{unpaged} 
\pron{anfpeVjfDx} 
\gl{\gu}
\bmng
puTasaMKeyx hAkilalxda. 
\emng
\eentry

\bentry
\word{unpaid} 
\pron{anfpeVDf} 
\gl{\gu}
\bmng
\bnum
\num{1} (sAlada \vi) tiVrisilalxda; salilxsilalxda; pAvati mADadiruva yA Agadirada. 
\num{2} (vayxkitxya kelasada yA rajeya \vi) veVtanarahita; saMbaLavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unpaired} 
\pron{anfpeVDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} joVDiyAgirada; joVDi mADirada; oMTiyAgiruva. 
\num{2} (maduveyiMda) daMpatigaLAgada; oTuTxgUDada. 
\num{3} (pArxNigaLa \vi) kUTa kUDada. 
\enum
\emng
\eentry

\bentry
\word{unpalatability} 
\pron{anfpAYxlaTabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (bAyige) rucisadiruvike; aruci. 
\num{2} (BAvane, salahe, \mo vu) manasisxge hitavAgadiruvike; hiDisadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unpalatable} 
\pron{anfpAYxlaTabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (bAyige) rucisada. 
\num{2} (BAvane, salahe, \mo vu) manasisxge hitavAgada; ahitakaravAda. 
\enum
\emng
\eentry

\bentry
\word{unpalatableness} 
\pron{anfpAYxlaTabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unpalatability}{unpalatability}. 
\emng
\eentry

\bentry
\word{unparalleled} 
\pron{anfpAYxralelfDx} 
\gl{\gu}
\expl{}
\bmng
eNe ilalxda; (sari)sATiyilalxda; samanilalxda. 
\emng
\eentry

\bentry
\word{unpardonable} 
\pron{anfpADaRnabflf} 
\gl{\gu}
\expl{}
\bmng
kaSxmisalahaRvalalxda; akaSxmayx. 
\emng
\eentry

\bentry
\word{unpardonableness} 
\pron{anfpADaRnabflfnisf} 
\gl{\nA}
\expl{}
\bmng
akaSxmayxte; kaSxmisalanahaRte. 
\emng
\eentry

\bentry
\word{unpardonably} 
\pron{anfpADaRnabilx} 
\gl{\kirxvi}
\expl{}
\bmng
kaSxmisalahaRvalalxde; akaSxmayxvAgi. 
\emng
\eentry

\bentry
\word{unparliamentary} 
\pron{anfpAlaRmeMTari} 
\gl{\gu}
\expl{}
\bmng
shAsanasaBege yoVgayxvalalxda; asaMsadiVya; pAliRmeMTina mayARdege ucitavalalxda, virudadhxvAda. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{unparliamentary language} beYguLa; asaBayxvAda BASe. 
\emng
\eentry

\bentry
\word{unpasteurized} 
\pron{anfpAYxsaTxyX(sacx)reYsfZDx} 
\gl{\gu}
\expl{}
\bmng
pAsacxriVkarisada. 
\emng
\eentry

\bentry
\word{unpatented} 
\pron{anfpeV(pAYx)TaMTiDf} 
\gl{\gu}
\expl{}
\bmng
(sAvxmayxda) sananxdu paDeyada; sananxdurahita. 
\emng
\eentry

\bentry
\word{unpatriotic} 
\pron{anfpAYx(peV)TirxATikf} 
\gl{\gu}
\expl{}
\bmng
deVshaperxVmavilalxda; savxdeVshABimAnavilalxda. 
\emng
\eentry

\bentry
\word{unpatriotically} 
\pron{anfpAYx(peV)TirxATikali} 
\gl{\kirxvi}
\expl{}
\bmng
deVshaperxVmarahitavAgi; savxdeVshABimAna ilalxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{unpaved} 
\pron{anfpeVvfDx} 
\gl{\gu}
\bmng
(biVdi, nela, \mo vugaLige) kalulx hadidxsada yA iTiTxge hAsada; (kalulx, iTiTxge, gAre, \mo vugaLiMda) nelagaTuTx mADada. 
\emng
\eentry

\bentry
\word{unpeeled} 
\pron{anfpiVlfDx} 
\gl{\gu}
\bmng
sipepx suliyada. 
\emng
\eentry

\bentry
\word{unpeg} 
\pron{anfpegf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unpegged}; \vakaq\ \eng{unpegging}).}
\bmng
\bnum
\num{1} gUTa, koMDi, \mo vanunx -- tege, kiVLu. 
\num{2} gUTa, beNe, \mo vanunx kitutx (EnanAnxdarU) tere, biDisu. 
\num{3} (belegaLu \mo vanunx) sithxragoLisadiru; calavAgiTiTxru. 
\enum
\emng
\eentry

\bentry
\word[unpeople(1)]{unpeople} 
\pron{anfpiVpa(pf)lf} 
\gl{\sakirx}
\expl{}
\bmng
janarahitagoLisu; nijaRnavAgisu; janashUnayx mADu; janarilalxdaMte mADu. 
\emng
\eentry

\bentry
\word[unpeople(2)]{unpeople} 
\pron{anfpiVpa(pf)lf} 
\gl{\nA}
\expl{}
\bmng
(\bava) = \hyperlink{unperson}{unpersons}. 
\emng
\eentry

\bentry
\word{unperceived} 
\pron{anfpasiRVvfDx} 
\gl{\gu}
\expl{}
\bmng
(iMdirxyagaLiMda, \kanmu\ kaNiNxniMda) noVDada; gamanisada; garxhisada; agaqhiVta. 
\emng
\eentry

\bentry
\word{unperceptive} 
\pron{anfpaseRpiTxvf} 
\gl{\gu}
\expl{}
\bmng
(iMdirxyagaLiMda) garxhisada; garxhaNashakitx ilalxda; asaMveVdaka. 
\emng
\eentry

\bentry
\word{unperceptively} 
\pron{anfpaseRpiTxvfli} 
\gl{\kirxvi}
\expl{}
\bmng
(\kanmu\ iMdirxyagaLa mUlaka) garxhisalAradaMte; agArxhayxvAgi; asaMveVdayxvAgi. 
\emng
\eentry

\bentry
\word{unperceptiveness} 
\pron{anfpaseRpiTxvfnisf} 
\gl{\nA}
\expl{}
\bmng
agarxhaNa; agArxhayxte; (iMdirxya, budidhxgaLiMda) garxhisalAradiruvike. 
\emng
\eentry

\bentry
\word{unperfected} 
\pron{anfpapheRkiTxDf} 
\gl{\gu}
\expl{}
\bmng
aparipUNaRvAda; paripUNaRgoLisada. 
\emng
\eentry

\bentry
\word{unperforated} 
\pron{anfpaphaRreVTiDf} 
\gl{\gu}
\expl{}
\bmng
niVraMdharx; raMdharxrahita; tUtugaLanunx mADada; raMdharx mADada. 
\emng
\eentry

\bentry
\word{unperformed} 
\pron{anfpaphARmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} akaqta; naDesada; neraveVrisada; nivaRhisada. 
\num{2} (nATaka) aBinayisada; ADada; parxdashiRsada. 
\num{3} (\saM) bAjisada yA hADada. 
\enum
\emng
\eentry

\bentry
\word{unperfumed} 
\pron{anfpaphUyxRmfDx} 
\gl{\gu}
\expl{}
\bmng
asuvAsita; asugaMdhita; sugaMdhadarxvayx seVrisada, hAkada. 
\emng
\eentry

\bentry
\word{unperson} 
\pron{anfpasaRnf} 
\gl{\nA}
\expl{}
\bmng
anAmadheVya; avajAcnxta; yAra hesaru, asitxtavx, \mo vanunx nilaRkiSxsalAgideyoV yA nirAkarisalAgideyoV aMthavanu. 
\emng
\eentry

\bentry
\word{unpersuadable} 
\pron{anfpasevxRVDabalf} 
\gl{\gu}
\expl{}
\bmng
manavopipxsalu asAdhayxvAda. 
\emng
\eentry

\bentry
\word{unpersuaded} 
\pron{anfpasevxRVDiDf} 
\gl{\gu}
\expl{}
\bmng
manavopipxsalAgada; ananuneVya; olisalAgada; oDaMbaDisalu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unpersuasive} 
\pron{anfpasevxRVsivf} 
\gl{\gu}
\expl{}
\bmng
anunayisalAgada; olisuva shakitx ilalxda; oDaMbaDisuva shakitx ilalxda. 
\emng
\eentry

\bentry
\word{unperturbed} 
\pron{anfpaTaRbfDxR} 
\gl{\gu}
\expl{}
\bmng
galibiligoLaLxda; avicalita; akuSxbadhx; keraLada. 
\emng
\eentry

\bentry
\word{unperturbedly} 
\pron{anfpaTaRbiRDilx} 
\gl{\kirxvi}
\expl{}
\bmng
galibiligoLaLxde; akuSxbadhxvAgi; avicalitavAgi. 
\emng
\eentry

\bentry
\word{unphilosophic} 
\pron{anfphilasAphikf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unphilosophical}{unphilosophical}. 
\emng
\eentry

\bentry
\word{unphilosophical} 
\pron{anfphilasAphikalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tatatxvXshAsatxrXsamamxtavalalxda; tatatxvXshAsatxrXvirudadhx. 
\num{2} tatatxvXjAcnxnavilalxda. 
\num{3} saMyama, viveVka -- ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unphilosophically} 
\pron{anfphilasAphikali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} tatatxvXshAsatxrX -- samamxtavalalxde, virudadhxvAgi. 
\num{2} saMyama kaLedukoMDu; viveVkarahitavAgi. 
\enum
\emng
\eentry

\bentry
\word{unphysiologic} 
\pron{anfphisiZalAjikf} 
\gl{\gu}
\expl{}
\bmng
shariVravAyxpArakakxnuguNavalalxda; deYhikakirxyege anuguNavAgirada. 
\emng
\eentry

\bentry
\word{unphysiological} 
\pron{anfphisiZalAjikalf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unphysiologic}{unphysiologic}. 
\emng
\eentry

\bentry
\word{unphysiologically} 
\pron{anfphisiZalAjikali} 
\gl{\kirxvi}
\expl{}
\bmng
shariVravAyxpArakakxnuguNavAgilalxde. 
\emng
\eentry

\bentry
\word{unpick} 
\pron{anfpikf} 
\gl{\akirx}
\expl{}
\bmng
(baTeTx \mo vugaLa) holige kitutxhAku, bicucx. 
\emng
\eentry

\bentry
\word{unpicked} 
\pron{anfpikfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Arisada; Ayekx mADada. 
\num{2} (hUvu) kititxrada; koyidxrada. 
\enum
\emng
\eentry

\bentry
\word{unpicturesque} 
\pron{anfpikacxresfkx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} citarxyukatxvalalxda; citarxyoVgayxvalalxda; citarxdaMte AkaSaRkavalalxda. 
\num{2} (kaNiNxge kaTiTxdaMte) susapxSaTxvAgilalxda; suPxTavAgirada. 
\enum
\emng
\eentry

\bentry
\word{unpin} 
\pron{anfpinf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unpinned}; \vakaq\ \eng{unpinning}).}
\bmng
\bnum
\num{1} sUji(gaLanunx) tegedu kaLacibiDu, bicicxbiDu. 
\num{2} (caduraMga) kaTiTxhAkalAgidadx kAyanunx baMdhanadiMda biDisu, biDugaDe mADu. 
\enum
\emng
\eentry

\bentry
\word{unpitied} 
\pron{anfpiTiDf} 
\gl{\gu}
\expl{}
\bmng
yAra kanikarakUkx pAtarxvAgada; yArU -- marukapaDada, karuNe toVrada. 
\emng
\eentry

\bentry
\word{unpitying} 
\pron{anfpiTiiMgf} 
\gl{\gu}
\expl{}
\bmng
marugada; kanikarapaDada; karuNe toVrada; nidaRya; niSakxruNa. 
\emng
\eentry

\bentry
\word{unpityingly} 
\pron{anfpiTiiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
marugade; kanikara toVrade; nidaRyavAgi. 
\emng
\eentry

\bentry
\word{unplaceable} 
\pron{anfpelxVsabflf} 
\gl{\gu}
\expl{}
\bmng
nidhaRrisalAgada yA vagiRVkarisalAgada: \eng{his accent is unplaceable} avana ucAcxraNA riVtiyanunx nidhaRrisalAguvudilalx. 
\emng
\eentry

\bentry
\word{unplaced} 
\pron{anfpelxVsfTx} 
\gl{\gu}
\expl{}
\bmng
(paMdayx, sapxdheR, \mo vugaLalilx) sAthxnagaLisada, paDeyada; \kanmu\ modalaneya mUru sAthxnagaLalilx oMdanunx paDeyada. 
\emng
\eentry

\bentry
\word{unplanned} 
\pron{anfpAlxYxnfDx} 
\gl{\gu}
\expl{}
\bmng
vayxvasethxgoLisada; yoVjisada; ayoVjita; yoVjanArahita. 
\emng
\eentry

\bentry
\word{unplanted} 
\pron{anfpAlxniTxDf} 
\gl{\gu}
\expl{}
\bmng
neDada; nililxsada; neTuTx beLasada. 
\emng
\eentry

\bentry
\word{unplastered} 
\pron{anfpAlxsaTxDfR} 
\gl{\gu}
\expl{}
\bmng
gilAvu mADada; gAre hacacxda. 
\emng
\eentry

\bentry
\word{unplausible} 
\pron{anfpAlxsiZbflf} 
\gl{\gu}
\expl{}
\bmng
asaMBavaniVya; saMBavaniVyavAgirada; opupxvaMtirada; nAyxyasamamxtaveMdu kANisada. 
\emng
\eentry

\bentry
\word{unplayable} 
\pron{anfpelxVabflf} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ ceMDATa \mo vugaLalilx ceMDina \vi) ADalu kaSaTxvAda; ADalu asAdhayxvAda; hiMdirugisi hoDeyalu ati kaSaTxvAda. 
\emng
\eentry

\bentry
\word{unplayably} 
\pron{anfpelxVabilx} 
\gl{\kirxvi}
\expl{}
\bmng
(ceMDina \vi) ADalu, hiMdirugisi hoDeyalu kaSaTxvAgi. 
\emng
\eentry

\bentry
\word{unpleasant} 
\pron{anfpelxsaZMTf} 
\gl{\gu}
\expl{}
\bmng
hitavalalxda; ahitakara; rucisada. 
\emng
\eentry

\bentry
\word{unpleasantly} 
\pron{anfpelxsaZMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
hitavalalxda hAge; ahitakaravAgi; rucisadaMte. 
\emng
\eentry

\bentry
\word{unpleasantness} 
\pron{anfpelxsaZMTfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hitavalalxdiruvudu; ahitakara sithxti. 
\num{2} manasAtxpa; veYmanasayx. 
\enum
\emng
\eentry

\bentry
\word{unpleasing} 
\pron{anfpilxVsiZMgf} 
\gl{\gu}
\expl{}
\bmng
ahita; hitavAgada; saMtoVSapaDisada; taqpitxpaDisada; mecicxsada; rucisada. 
\emng
\eentry

\bentry
\word{unpleasingly} 
\pron{anfpilxVsiZMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
hitavAgadaMte; rucisade. 
\emng
\eentry

\bentry
\word{unpledged} 
\pron{anfpelxjfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aDavu iDada; otetxyiDada; giraviyiDada. 
\num{2} mAtukoDada; vAgAdxna mADirada; parxtijecnx mADirada. 
\enum
\emng
\eentry

\bentry
\word{unploughed} 
\pron{anfpwlxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} neVgilu hoDeyada; uLada; ututx neVgila sAlu mADada. 
\num{2} sAguvaLi mADada. 
\enum
\emng
\eentry

\bentry
\word{unplucked} 
\pron{anfpalxkfTx} 
\gl{\gu}
\expl{}
\bmng
(hUvu \mo vugaLa \vi) giDadiMda kiVLada, biDisada, koyayxda. 
\emng
\eentry

\bentry
\word{unplug} 
\pron{anfpalxgf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unplugged}; \vakaq\ \eng{unplugging}).}
\bmng
\bnum
\num{1} beNe tege; (viduyxtf sAdhanavoMdanunx) `sAkeTf' niMda palxgf tegedu viduyxtf saMpakaR kaDiduhAku. 
\num{2} aDacaNe tege. 
\enum
\emng
\eentry

\bentry
\word{unplumbed} 
\pron{anfpalxmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ALa kaMDuhiDiyada; ALa tiLiyada. 
\num{2} pUtiRyAgi shoVdhisada yA ariyada. 
\enum
\emng
\eentry

\bentry
\word{unpoetic} 
\pron{anfpoVeTikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kAvayxguNavilalxda; guNalakaSxNagaLilalxda. 
\num{2} kavisahajavalalxda. 
\num{3} kavitAyoVgayxvalalxda. 
\num{4} padAyxtamxkavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unpoetical} 
\pron{anfpoVeTikalf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unpoetic}{unpoetic}. 
\emng
\eentry

\bentry
\word{unpointed} 
\pron{anfpAiMTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} monacilalxda; tudi cUpu mADada; moMDutudiya. 
\num{2} virAmacihenx hAkada. 
\num{3} (hiVbUrx BASe) savxra cukekx hAkilalxda. 
\num{4} (kalulx kaTaTxDa yA iTiTxge kelasa) saMdugAre mADilalxda. 
\enum
\emng
\eentry

\bentry 
\word{unpolished} 
\pron{anfpAliSfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nayagoLisada; merugu koDada; hoLapu koDada. 
\num{2} (\rUpa) nayanAjUku ilalxda; asaMsakxqqta; apariSakxqqta; oraTu. 
\enum
\emng
\eentry

\bentry
\word{unpolitic} 
\pron{anfpAliTikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aviveVkada; acAtuyaRda. 
\num{2} samayoVcitavalalxda; anucitavAda; anwcitayxda. 
\enum
\emng
\eentry

\bentry
\word{unpolitical} 
\pron{anfpaliTikalf} 
\gl{\gu}
\expl{}
\bmng
rAjakiVyavalalxda; rAjakiVya saMbaMdhavilalxda; rAjakiVyeVtara. 
\emng
\eentry

\bentry
\word{unpolitically} 
\pron{anfpaliTikali} 
\gl{\kirxvi}
\expl{}
\bmng
rAjakiVyavAgirade; rAjakiVya saMbaMdhavilalxdaMte; rAjakiVyeVtaravAgi. 
\emng
\eentry

\bentry
\word{unpolled} 
\pron{anfpoVlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (cunAvaNeyalilx) matagaLanunx hAkada yA paDeyada. 
\num{2} aBipArxya matasaMgarxhaNeyalilx seVrada. 
\enum
\emng
\eentry

\bentry
\word{unpolluted} 
\pron{anfpalUTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} apavitarx mADada; aparishudadhxgoLisada; holegeDisada; kulageDisada. 
\num{2} (niVru \mo vugaLa \vi) keDisada; koLe mADada; malinagoLisada. 
\enum
\emng
\eentry

\bentry
\word{unpopular} 
\pron{anfpApuyxlarf} 
\gl{\gu}
\expl{}
\bmng
janapirxyavalalxda; janAnurAgavilalxda; janara mecucxge paDeyada. 
\emng
\eentry

\bentry
\word{unpopularity} 
\pron{anfpApuyxlAYxriTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} janapirxyavalalxdiruvike; janakekx iSaTxvilalxde iruvudu. 
\num{2} apaKAyxti; keTaTx hesaru. 
\enum
\emng
\eentry

\bentry
\word{unpopularly} 
\pron{anfpApuyxlarfli} 
\gl{\kirxvi}
\expl{}
\bmng
janapirxyavalalxda riVtiyalilx; janakekx iSaTxvilalxda hAge; janakekx mecicxgeyAgadaMte. 
\emng
\eentry

\bentry
\word{unpopulated} 
\pron{anfpApuyxleVTiDf} 
\gl{\gu}
\expl{}
\bmng
janaru nelesilalxda; janavasatiyilalxda. 
\emng
\eentry

\bentry
\word{unpossessed} 
\pron{anfpaseZsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (yAvudoV oMdara) sAvxmayxvilalxda; oDetanavilalxda. 
\num{2} (yAradeV) sAvxdhiVnadalilxlalxda; vashadalilxlalxda. 
\enum
\emng
\eentry

\bentry
\word{unposted} 
\pron{anfpoVsiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} samAcAra tiLidilalxda; viSaya tiLiyada. 
\num{2} (kAgada) aMcege hAkilalxda. 
\enum
\emng
\eentry

\bentry
\word{unpractical} 
\pron{anfpArxYxkiTxkalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (haMcike \mo vu) pArxyoVgikavalalxda; kAyaRtaH naDesalAgada; parxyoVgashiVlavalalxda; kAyaRkAriyalalxda. 
\num{2} (vicAra \mo vu) pArxyoVgikavalalxda; vAyxvahArikavalalxda; kAyaRrUpadalilx kaMDubarada; upayoVgakekx yA parxyoVjanakekx bArada; naDesalu kaSaTxvAda. 
\num{3} (vayxkitx) kAyaRniratanalalxda; vAyxvahArika daqSiTxyilalxda; kAyaRnipuNanalalxda. 
\enum
\emng
\eentry

\bentry
\word{unpracticality} 
\pron{anfpArxYxkiTxkAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} apArxyoVgikate; kAyaRtaH naDesalAgadiruvudu; parxyoVgAtamxkavalalxdiruvike. 
\num{2} kAyaRrUpadalilx kaMDubaradiruvudu. 
\num{3} kAyaRkwshala ilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unpractically} 
\pron{anfpArxYxkiTxkali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} pArxyoVgikavAgiradaMte; kAyaRtaH naDesalAgade. 
\num{2} kAyaRrUpadalilx kaMDubarade; parxyoVjanavAgada riVtiyalilx; naDesalu kaSaTxvAgi. 
\num{3} kAyaRkwshalavilalxde; vayxvahAra daqSiTx ilalxde. 
\enum
\emng
\eentry

\bentry
\word{unpractised} 
\pron{anfpArxYxkiTxsfTx} 
\gl{\gu}
\expl{}
(\ame\ \eng{unpracticed}). \bmng
\bnum
\num{1} nuritilalxda; ananuBaviyAda; kwshalavilalxda. 
\num{2} aBAyxsakekx taMdilalxda; kAyaRrUpakekx tarade iruva. 
\enum
\emng
\eentry

\bentry
\word{unprecedented} 
\pron{anfperxsiDeMTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aBUtapUvaR; hiMdeMdU kaMDilalxda; pUvaRnidashaRnavilalxda. 
\num{2} hosadAda; nUtanavAda. 
\num{3} eNeyilalxda; sATiyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unprecedentedly} 
\pron{anfperxsiDeMTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
aBUtapUvaRvAgi; hiMdeMdU kANada riVtiyalilx; pUvaRnidashaRnavilalxde. 
\emng
\eentry

\bentry
\word{unpredictable} 
\pron{anfpirxDikaTxbflf} 
\gl{\gu}
\expl{}
\bmng
BaviSayx nuDiyalAgada; (muMde Agaliruvudanunx) Uhisi heVLalAgada; muMgANalAgada. 
\emng
\eentry

\bentry
\word{unpredictably} 
\pron{anfpirxDikaTxbilx} 
\gl{\kirxvi}
\expl{}
\bmng
(muMde Agaliruvudanunx) Uhisi heVLalAgadaMte; muMBAvisalAgadaMte. 
\emng
\eentry

\bentry
\word{unpredicted} 
\pron{anfpirxDikiTxDf} 
\gl{\gu}
\expl{}
\bmng
ajAcnxta BaviSayxda; muMdAguvudanunx heVLirada. 
\emng
\eentry

\bentry
\word{unprejudiced} 
\pron{anfperxjuDisfTx} 
\gl{\gu}
\expl{}
\bmng
pUvARgarxhagaLilalxda; munonxlavu, pakaSxpAta -ilalxda; niSapxkaSxpAtavAda. 
\emng
\eentry

\bentry
\word{unpremeditated} 
\pron{anfpirxmeDiTeVTiDf} 
\gl{\gu}
\expl{}
\bmng
pUvARloVcitavalalxda; hiMdeyeV, modaleV -- AloVcisada, yoVcane mADada yA mADirada. 
\emng
\eentry

\bentry
\word{unpremeditatedly} 
\pron{anfpirxmeDiTeVTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
pUvARloVcitavAgiradaMte; hiMdeyeV AloVcisiradaMte. 
\emng
\eentry

\bentry
\word{unprepared} 
\pron{anfpirxpeVDfR} 
\gl{\gu}
\expl{}
\bmng
sidadhxvAgilalxda; pUvaRsidadhxteyilalxda. 
\emng
\eentry

\bentry
\word{unpreparedly} 
\pron{anfpirxpeVriDfli} 
\gl{\kirxvi}
\expl{}
\bmng
pUvaRsidadhxteyilalxde. 
\emng
\eentry

\bentry
\word{unpreparedness} 
\pron{anfpirxpeVriDfnisf} 
\gl{\nA}
\expl{}
\bmng
sidadhxvAgilalxdiruvike; pUvaRsidadhxteyilalxdiruvudu. 
\emng
\eentry

\bentry
\word{unprepossessing} 
\pron{anfpirxVpaseZsiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (BAvane \mo vugaLa \vi) pUvARgarxhagaLiMda kUDirada; munonxlavugaLilalxda. 
\num{2} visheVSavilalxda; sAmAnayxvAda. 
\enum
\emng
\eentry

\bentry
\word{unprescribed} 
\pron{anfpirxsekxrXYbfDx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ auSadhigaLa \vi) vidhisada. 
\emng
\eentry

\bentry
\word{unpresentable} 
\pron{anfpirxseZMTabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} noVTakekx cenAnxgirada; noVDalu aMdavirada. 
\num{2} goVSiThxge parxveVshisalu yA seVralu yoVgayxte ilalxda. 
\num{3} bahumAnayoVgayxvalalxda; uDugoreyAgi koDalu takukxdalalxda. 
\enum
\emng
\eentry

\bentry
\word{unpressed} 
\pron{anfperxsfTx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ baTeTxbaregaLa \vi) isitxrXmADada. 
\emng
\eentry

\bentry
\word{unpresuming} 
\pron{anfpirxsUyxZmiMgf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unpresumptuous}{unpresumptuous}. 
\emng
\eentry

\bentry
\word{unpresumptuous} 
\pron{anfpirxsaZMpuTxyXasf} 
\gl{\gu}
\expl{}
\bmng
durahaMkAravilalxda; mitimiVrida AtamxvishAvxsavilalxda. 
\emng
\eentry

\bentry
\word{unpretending} 
\pron{anfpirxTeMDiMgf} 
\gl{\gu}
\expl{}
\bmng
ADaMbaravilalxda; soVgilalxda; parxdashaRnashiVlavalalxda; nirADaMbarada; saraLavAda. 
\emng
\eentry

\bentry
\word{unpretendingly} 
\pron{anfpirxTeMDiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
ADaMbaravilalxde; parxdashaRnashiVlavalalxda hAge; nirADaMbaravAgi; saraLavAgi; soVgilalxde. 
\emng
\eentry

\bentry
\word{unpretendingness} 
\pron{anfpirxTeMDiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
nirADaMbaravAgiruvudu; parxdashaRnashiVlavalalxdiruvike; saraLate. 
\emng
\eentry

\bentry
\word{unpretentious} 
\pron{anfpirxTenaSxsf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unpretending}{unpretending}. 
\emng
\eentry

\bentry
\word{unpretentiously} 
\pron{anfpirxTenaSxsfli} 
\gl{\kirxvi}
\expl{}
\bmng
= \hyperlink{unpretendingly}{unpretendingly}. 
\emng
\eentry

\bentry
\word{unpretentiousness} 
\pron{anfpirxTenaSxsfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unpretendingness}{unpretendingness}. 
\emng
\eentry

\bentry
\word{unpriced} 
\pron{anfperxYsfTx} 
\gl{\gu}
\expl{}
\bmng
bele nidhaRrisilalxda, gotutxmADilalxda; bele -- hAkilalxda, namUdisilalxda. 
\emng
\eentry

\bentry
\word{unprimed} 
\pron{anfperxYmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\ca) (baMdUka) koVvimadadxnunx tuMbada. 
\num{2} (obabxnige sudidx \mo vanunx havaNisi) tayAru mADada. 
\enum
\emng
\eentry

\bentry
\word{unprincipled} 
\pron{anfpirxnisxpflfDx} 
\gl{\gu}
\expl{}
\bmng
tatatxvXniSeThxyilalxda; niVtiniyamagaLilalxda; utatxma -- AdashaRvilalxda, dheyxVyagaLilalxda. 
\emng
\eentry

\bentry
\word{unprincipledness} 
\pron{anfpirxnisxpflfDxnisf} 
\gl{\nA}
\expl{}
\bmng
tatatxvXniSeThxyilalxdiruvike; niVtiniyamagaLilalxdiruvike. 
\emng
\eentry

\bentry
\word{unprintable} 
\pron{anfpirxMTabflf} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ tiVra dUSaNe, ashilxVla, \mo\ kAraNagaLiMda) mudarxNAhaRvalalxda; mudarxNayoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{unprintably} 
\pron{anfpirxMTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
(\kanmu\ tiVra dUSaNe, ashilxVlate, \mo\ kAraNagaLiMda) mudarxNAhaRvalalxdaMte; mudarxNakekx yoVgayxvalalxdaMte. 
\emng
\eentry

\bentry
\word{unprinted} 
\pron{anfpirxMTiDf} 
\gl{\gu}
\expl{}
\bmng
mudarxNavAgada; amudirxta. 
\emng
\eentry

\bentry
\word{unprivileged} 
\pron{anfpirxvilijfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vayxkitx, paMgaDa, adhikAra, \mo vakekx seVrida) visheVSa hakukxgaLilalxda; savalatutxgaLilalxda. 
\num{2} (\ame) dwBARgayxvaMta; kaDubaDava; diVnadalita. 
\enum
\emng
\eentry

\bentry
\word{unproblematic} 
\pron{anfpArxbalxmAYxTikf} 
\gl{\gu}
\expl{}
\bmng
samasAyxtamxkavalalxda; samaseyxyAgirada yA kaSaTxkoDada. 
\emng
\eentry

\bentry
\word{unproblematically} 
\pron{anfpArxbalxmAYxTikali} 
\gl{\kirxvi}
\expl{}
\bmng
samasAyxtamxkavalalxdaMte; kaSaTx koDade. 
\emng
\eentry

\bentry
\word{unproclaimed} 
\pron{anfporxVkelxVmfDx} 
\gl{\gu}
\expl{}
\bmng
aGoVSita; GoVSaNe mADada. 
\emng
\eentry

\bentry
\word{unprocurable} 
\pron{anfparxkuyxarabflf} 
\gl{\gu}
\expl{}
\bmng
apArxpayx; paDeyalAgada; gaLisalAgada; saMpAdisalAgada; saMgarxhisalAgada. 
\emng
\eentry

\bentry
\word{unproductive} 
\pron{anfparxDakiTxvf} 
\gl{\gu}
\expl{}
\bmng
anutApxdaka; utapxtitx mADada. 
\emng
\eentry

\bentry
\word{unproductively} 
\pron{anfparxDakiTxvfli} 
\gl{\kirxvi}
\expl{}
\bmng
anutApxdakavAgi; utApxdisade. 
\emng
\eentry

\bentry
\word{unproductiveness} 
\pron{anfparxDakiTxvfnisf} 
\gl{\nA}
\expl{}
\bmng
anutApxdakate; utApxdane mADadiruvike. 
\emng
\eentry

\bentry
\word{unprofessional} 
\pron{anfparxpheSanalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (tananx) kasubige, vaqtitxge -- saMbaMdhisada, seVrirada. 
\num{2} vaqtitxmayARdege ucitavalalxda; kasubina gwravakekx salalxda; vaqtitxyoVgayxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unprofessionally} 
\pron{anfparxpheSanali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vaqtitxmayARdege, riVtiniVtigaLige ucitavalalxda riVtiyalilx. 
\num{2} (tananx) kasubige, vaqtitxge -- tagadaMte. 
\enum
\emng
\eentry

\bentry
\word{unprofitable} 
\pron{anfpArxphiTabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} lABakaravalalxda; AdAyavilalxda. 
\num{2} Palaparxdavalalxda; parxyoVjanakaravalalxda; niSapxrXyoVjaka. 
\enum
\emng
\eentry

\bentry
\word{unprofitableness} 
\pron{anfpArxphiTabflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} lABakaravalalxdiruvike. 
\num{2} niSapxrXyoVjakate; Palaparxdavalalxdiruvike; niSaPxlate. 
\enum
\emng
\eentry

\bentry
\word{unprofitably} 
\pron{anfpArxphiTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} lABakaravalalxdaMte. 
\num{2} niSapxrXyoVjakavAgi; niSapxlavAgi. 
\enum
\emng
\eentry

\bentry
\word{unprogressive} 
\pron{anfparxgerxsivf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} munanxDeyada; muMdakekx calisada. 
\num{2} muMduvariyada; parxgatiparavalalxda; parxgatigAmiyalalxda. 
\enum
\emng
\eentry

\bentry
\word{unpromising} 
\pron{anfpArxmisiMgf} 
\gl{\gu}
\expl{}
\bmng
(oLeLxyadAguvudeMba) Baravase koDada; AshAjanakavalalxda; nirAshAdAyaka. 
\emng
\eentry

\bentry
\word{unpromisingly} 
\pron{anfpArxmisiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
AshAjanakavAgirade; Baravase huTiTxsadaMte. 
\emng
\eentry

\bentry
\word{unprompted} 
\pron{anfpArxMpiTxDf} 
\gl{\gu}
\expl{}
\bmng
(utatxra, parxtikirxye, \mo vugaLa \vi) (perxVraNe ilalxde) tAneV tAnAgi baMda; aparxcoVdita; aperxVrita; aparxyatanxpUvaRkavAda. 
\emng
\eentry

\bentry
\word{unpronounceable} 
\pron{anfparxnwnasxbflf} 
\gl{\gu}
\expl{}
\bmng
anucacxraNiVya; ucacxrisalAgada. 
\emng
\eentry

\bentry
\word{unpronounceably} 
\pron{anfparxnwnasxbilx} 
\gl{\kirxvi}
\expl{}
\bmng
anucacxraNiVyavAgi; ucacxrisalAgadaMte. 
\emng
\eentry

\bentry
\word{unprophetic} 
\pron{anfparxpheTikf} 
\gl{\gu}
\expl{}
\bmng
BaviSayxjAcnxnavirada. 
\emng
\eentry

\bentry
\word{unpropitious} 
\pron{anfparxpiSasf} 
\gl{\gu}
\expl{}
\bmng
sumuKavalalxda; anukUlavalalxda; shuBasUcakavalalxda. 
\emng
\eentry

\bentry
\word{unpropitiously} 
\pron{anfparxpiSasfli} 
\gl{\kirxvi}
\expl{}
\bmng
anukUlavAgirade; shuBasUcakavAgirade. 
\emng
\eentry

\bentry
\word{unprosperous} 
\pron{anfpArxsapxrasf} 
\gl{\gu}
\expl{}
\bmng
ucACxrXya sithxtiyalilxlalxda; jaya paDeyada; ELigeyAgada; aBivaqdidhx hoMdada. 
\emng
\eentry

\bentry
\word{unprosperously} 
\pron{anfpArxsapxrasfli} 
\gl{\kirxvi}
\expl{}
\bmng
ucACxrXya sithxtiyalilxlalxdaMte. 
\emng
\eentry

\bentry
\word{unprotected} 
\pron{anfparxTekiTxDf} 
\gl{\gu}
\expl{}
\bmng
rakaSxNe ilalxda; arakiSxta: 
\banum
\alnum{a} (apAya, keVDu, \mo vugaLiMda) rakiSxsada; kApADada; salahada. 
\alnum{b} Asharxya, poVSaNegaLilalxda. 
\eanum
\emng
\eentry

\bentry
\word{unprotectedness} 
\pron{anfparxTekiTxDfnisf} 
\gl{\nA}
\expl{}
\bmng
arakiSxtate; (apAya, keVDu, \mo vugaLiMda) rakaSxNeyilalxdiruvike. 
\emng
\eentry

\bentry
\word{unprotesting} 
\pron{anfparxTesiTxMgf} 
\gl{\gu}
\expl{}
\bmng
parxtiBaTisada; viroVdhisada. 
\emng
\eentry

\bentry
\word{unprotestingly} 
\pron{anfparxTesiTxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
parxtiBaTisade; viroVdhisade. 
\emng
\eentry

\bentry
\word{unprovability} 
\pron{anfpUrxvabiliTi} 
\gl{\nA}
\expl{}
\bmng
(satayxte, saMgati, \mo vugaLa \vi) parxmANiVkarisalAgadiruvike; daqDhapaDisalAgadiruvike; rujuvAtu mADi toVrisalAgadiruvike. 
\emng
\eentry

\bentry
\word{unprovable} 
\pron{anfpUrxvabflf} 
\gl{\gu}
\expl{}
\bmng
(satayxte, saMgati, \mo vugaLa \vi) daqDhapaDisalAgada; rujuvAtu mADalAgada; nidashaRna mADikoDalAgada; sidadhxpaDisi toVrisalAgada; satayxveMdu samathiRsalAgada. 
\emng
\eentry

\bentry
\word{unprovableness} 
\pron{anfpUrxvabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unprovability}{unprovability}. 
\emng
\eentry

\bentry
\word{unproved} 
\pron{anfpUrxvfDx} 
\gl{\gu}
\expl{}
\bmng
daqDhapaDisada; rujuvAtu mADada; parxmANiVkarisada; sidadhxpaDisi toVrisada. 
\emng
\eentry

\bentry
\word{unproven} 
\pron{anfpUrxvanf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unproved}{unproved}. 
\emng
\eentry

\bentry
\word{unprovided} 
\pron{anfparxveYDiDf} 
\gl{\gu}
\expl{}
\bmng
(jiVvanakekx haNa \mo vanunx) odagisilalxda; havaNisada; jiVvanoVpAya kalipxsada: \eng{the widow was left unprovided for} A vidhavege yAvudoMdu jiVvanoVpAya kalipxsiralilalx. 
\emng
\eentry

\bentry
\word{unprovoked} 
\pron{anfparxvoVkfTx} 
\gl{\gu}
\expl{}
\bmng
(vayxkitx yA naDavaLikeya \vi) aparxcoVdita; parxcoVdane ilalxda. 
\emng
\eentry

\bentry
\word{unpublishable} 
\pron{anfpabilxSabflf} 
\gl{\gu}
\expl{}
\bmng
parxkaTanAhaRvalalxda; parxkaTisalAgada. 
\emng
\eentry

\bentry
\word{unpublished} 
\pron{anfpabilxSfTx} 
\gl{\gu}
\expl{}
\bmng
aparxkaTita; parxkaTavAgada; parxkAshitavAgada. 
\emng
\eentry

\bentry
\word{unpunctual} 
\pron{anfpaMkuTxyXalf} 
\gl{\gu}
\expl{}
\bmng
kAlaniSeThx ilalxda; sakAladalilxrada; hotutxmiVrida; gotAtxda hotatxnunx sariyAgi pAlisada. 
\emng
\eentry

\bentry
\word{unpunctuality} 
\pron{anfpaMkuTxyXAYxliTi} 
\gl{\nA}
\expl{}
\bmng
kAlaniSeThxyilalxdiruvike; samayapAlaneyilalxdiruvike. 
\emng
\eentry

\bentry
\word{unpunctuated} 
\pron{anfpaMkuTxyXETiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} virAmacihenx hAkada; virAmacihenx hAki gurutisada. 
\num{2} nilalxda; avirata; virAma, vishArxMti{i}lalxda. 
\enum
\emng
\eentry

\bentry
\word{unpunishable} 
\pron{anfpaniSabflf} 
\gl{\gu}
\expl{}
\bmng
shikiSxsalAgada; daMDisalu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unpunished} 
\pron{anfpaniSfTx} 
\gl{\gu}
\expl{}
\bmng
daMDisada; shikiSxsada; shikeSx vidhisada. 
\emng
\eentry

\bentry
\word{unpurified} 
\pron{anfpuyxaripheYDf} 
\gl{\gu}
\expl{}
\bmng
shudidhxgoLisada; shudidhxVkarisada. 
\emng
\eentry

\bentry
\word{unputdownable} 
\pron{anfpuTfDwnabflf} 
\gl{\gu}
\expl{}
\bmng
(\AmA) (pusatxkada \vi) keLagiDalAgada; keLagiDalAgadaSuTx sAvxrasayxvAgidudx OdutAtx hoVgabeVkAda. 
\emng
\eentry

\bentry
\word{unqualified} 
\pron{anfkAvxlipheYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anahaR; ahaRte, yoVgayxte -- ilalxda; ahaRnalalxda: \eng{I am unqualified to serve} seVve salilxsalu nAnu ahaRnalalx. 
\num{2} kAnUnu parxkAra yA adhikaqtavAgi sananxdu paDeyada: \eng{an unqualified medical practitioner} sananxdilalxda veYdayx. 
\num{3} mitihAkada; nirupAdhikavAda; shudAdhxMga; saMpUNaRvAda: \eng{unqualified assent} saMpUNaR samamxti. 
\enum
\emng
\eentry

\bentry
\word{unquenchable} 
\pron{anfkevxnacxbflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (niVraDikeya \vi) taNisalAgada; shamanagoLisalAgada. 
\num{2} (beMkiya \vi) ArisalAgada. 
\enum
\emng
\eentry

\bentry
\word{unquenched} 
\pron{anfkevxnfcxTf} 
\gl{\gu}
\bmng
\bnum
\num{1} taNisada; shamanagoLisada. 
\num{2} (beMkiya \vi) Arisada. 
\enum
\emng
\eentry

\bentry
\word{unquestionability} 
\pron{anfkevxsacxnabiliTi} 
\gl{\nA}
\expl{}
\bmng
parxshAnxtiVtate; niviRvAdate. 
\emng
\eentry

\bentry
\word{unquestionable} 
\pron{anfkevxsacxnabflf} 
\gl{\gu}
\expl{}
\bmng
niviRvAdavAda; parxshAnxtiVta; nisasxMdeVhavAda; anumAnavilalxda. 
\emng
\eentry

\bentry
\word{unquestionableness} 
\pron{anfkevxsacxnabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unquestionability}{unquestionability}. 
\emng
\eentry

\bentry
\word{unquestionably} 
\pron{anfkevxsacxnabilx} 
\gl{\kirxvi\ \nA}
\expl{}
\bmng
parxshAnxtiVtavAgi; niviRvAdavAgi; nisasxMdeVhavAgi; yAva anumAnavU ilalxde. 
\emng
\eentry

\bentry
\word{unquestioned} 
\pron{anfkevxsacxnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vAdagarxsatxvalalxda; niviRvAdavAda; KacitavAda; nisasxMdigadhxvAda. 
\num{2} taniKege oLapaDisada; parxshenxge guripaDisada; parxshinxsada: \eng{I cannot let your statement pass unquestioned} nAnu ninanx heVLikeyanunx parxshinxsadiralAre. 
\enum
\emng
\eentry

\bentry
\word{unquestioning} 
\pron{anfkevxsacxniMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxshinxsada; parxshenx etatxda; parxti heVLada: \eng{unquestioning obedience} parxshenx etatxda vidheVyate. \eng{unquestioning loyalty} parxti heVLada niSeThx. 
\num{2} parxshinxsade, parxshenx keVLade -- mADida, neraveVrisida. 
\enum
\emng
\eentry

\bentry
\word{unquestioningly} 
\pron{anfkevxsacxniMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} parxshinxsade; yAva parxshenxyanUnx etatxde, keVLade. 
\num{2} parxti heVLade; pUNaR niSeThxyiMda. 
\enum
\emng
\eentry

\bentry
\word[unquiet(1)]{unquiet} 
\pron{anfkevxYaTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (parisithxti \mo vugaLa \vi) ashAMta; parxkuSxbadhx. 
\num{2} (vayxkitx) taLamaLagoMDa; kaLavaLada; AtaMkada; talalxNagoMDa; vAyxkulada. 
\enum
\emng
\eentry

\bentry
\word[unquiet(2)]{unquiet} 
\pron{anfkevxYaTf} 
\gl{\nA}
\expl{}
\bmng
ashAMtate; taLamaLa; kaLavaLa; parxkuSxbadhxte. 
\emng
\eentry

\bentry
\word{unquietly} 
\pron{anfkevxYaTfli} 
\gl{\kirxvi}
\expl{}
\bmng
ashAMtavAgi; kaLavaLadiMda. 
\emng
\eentry

\bentry
\word{unquietness} 
\pron{anfkevxYaTfnisf} 
\gl{\nA}
\expl{}
\bmng
ashAMtate; parxkuSxbadhxte. 
\emng
\eentry

\bentry
\word{unquotable} 
\pron{anfkovxVTabflf} 
\gl{\gu}
\expl{}
\bmng
ulelxVKisalAgada; anulelxVKaniVya; ulelxVKanAhaRvalalxda. 
\emng
\eentry

\bentry
\word{unquote} 
\pron{anfkovxVTf} 
\gl{\sakirx}
\expl{}
\bmng
(ukatxleVKana koDuvudu, gaTiTxyAgi Oduvudu, \mo vugaLalilx) udadhxqqta (BAga) mugiyitenunxvudanunx sUcisu; udadhxraNada samApitxyanunx, aMtayxvanunx tiLisu. 
\emng
\eentry

\bentry
\word{unquoted} 
\pron{anfkovxVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} udadhxqqta BAgakekx seVrada; (ulelxVKana) udadhxraNa cihenxyiMdAce, horage iruva. 
\num{2} \kanmu\ `sATxkf ekfsxceVMjf' nalilx sATxkina -- dara sUcisada, bele tiLisada. 
\enum
\emng
\eentry

\bentry
\word{unransomed} 
\pron{anfrAYxnasxmfDx} 
\gl{\gu}
\expl{}
\bmng
(yudadhxKeYdi) daMDa terada; biDugaDe haNa koTuTx biDisikoLaLxda; haNakoTuTx biDugaDe mADada. 
\emng
\eentry

\bentry
\word{unravel} 
\pron{anfrAYxvalf} 
\gl{\sakirx}
\expl{[\BU\ matutx \BUkaq\ \eng{unravelled} (\ame\ }
\eng{unraveled}); \vakaq\ \eng{unravelling} (\ame\ \eng{unraveling})].\bmng
\bnum
\num{1} goVju biDisu; goVjilalxda hAge mADu (\rUpa\ saha): \eng{unravel a mystery} rahasayx biDisu. 
\num{2} (heNige, kasUti, \mo vugaLa) heNige biDisu; dAravAgi bicicxbiDu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
goVjilalxdaMtAgu; heNige bicicxko. 
\emng
\eentry

\bentry
\word{unravished} 
\pron{anfrAYxviSfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keDisada; kaLaMkitavAgada. 
\num{2} atAyxcArakekx oLagAgada. 
\enum
\emng
\eentry

\bentry
\word{unreachable} 
\pron{anfriVcabflf} 
\gl{\gu}
\expl{}
\bmng
apArxpayx; alaBayx; agamayx; eTukada; hoVgi talupalAgada; muTaTxlAgada. 
\emng
\eentry

\bentry
\word{unreachableness} 
\pron{anfriVcabflfnisf} 
\gl{\nA}
\expl{}
\bmng
apArxpayxte; agamayxte; (hoVgi) talapalAgadiruvike. 
\emng
\eentry

\bentry
\word{unreachably} 
\pron{anfriVcabilx} 
\gl{\kirxvi}
\expl{}
\bmng
apArxpayxvAgi; agamayxvAgi; (hoVgi) talapalAgadaMte. 
\emng
\eentry

\bentry
\word{unread} 
\pron{anfreDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (pusatxkada \vi) Odada; apaThita; inUnx Odade iruva. 
\num{2} (vayxkitxya \vi) cenAnxgi Odu bArada; avidAyxvaMta; aSuTx cenAnxgi Odirada. 
\enum
\emng
\eentry

\bentry
\word{unreadability} 
\pron{anfriVDabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} apaThaniVyate; OdalAgadiruvike. 
\num{2} OdalanahaRvAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{unreadable} 
\pron{anfriVDabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} OdalAgada; Odalu kaSaTxvAda; apaThaniVya. 
\num{2} Odalu ahaRvalalxda; OdabArada. 
\enum
\emng
\eentry

\bentry
\word{unreadably} 
\pron{anfriVDabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} apaThaniVyavAgi; OdalAgadaMte. 
\num{2} OdalanahaRnAgi. 
\enum
\emng
\eentry

\bentry
\word{unreadily} 
\pron{anfreDili} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sidadhxvAgade; sajAjxgilalxde. 
\num{2} curukAgirade. 
\enum
\emng
\eentry

\bentry
\word{unreadiness} 
\pron{anfreDinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} asidadhxte; sajAjxgilalxdiruvike. 
\num{2} curukilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word[unready(1)]{unready} 
\pron{anfreDi} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sidadhxvAgilalxda; sajAjxgilalxda. 
\num{2} curukAgilalxda: \eng{awkward situations often found him unready} eDavaTATxda saninxveVshagaLalilx halavu veVLe avanu curukAgirutitxralilalx. 
\enum
\emng
\eentry

\bentry
\word[unready(2)]{unready} 
\pron{anfreDi} 
\gl{\gu}
\expl{}
\bmng
(\pArxparx) duDukina; avicAradiMda (AturadiMda) mununxgugxva; takakxSuTx AloVcane ilalxde vatiRsuva: \eng{Rama the Unready} duDuku budidhxya rAma. 
\emng
\eentry

\bentry
\word{unreal} 
\pron{anfrialf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asatayxvAda; avAsatxvikavAda; mitheyxya. 
\num{2} kAlapxnika; kalipxta; nijavalalxda; toVkeRya. 
\num{3} (\ame\ matutx \AseTxrXV) (\ashi) naMbalAgada; soVjigada; AshacxyaRda. 
\enum
\emng
\eentry

\bentry
\word{unrealistic} 
\pron{anfrialisiTxkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vasutxsithxtiya arivilalxda; kAyaRsAdhayxvalalxda; avAyxvahArika. 
\num{2} yathAthaRvalalxda; avAsatxvika; vasutxsithxtige saMbaMdhisada. 
\enum
\emng
\eentry

\bentry
\word{unrealistically} 
\pron{anfrialisiTxkali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vasutxsithxtiya arivilalxdaMte; avAyxvahArikavAgi. 
\num{2} yathAthaRvAgirade; avAsatxvikavAgi. 
\enum
\emng
\eentry

\bentry
\word{unreality} 
\pron{anfriAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} asatayx; mitheyx. 
\num{2} kAlapxnikate; toVkeR. 
\enum
\emng
\eentry

\bentry
\word{unrealizable} 
\pron{anfrialeYsaZbflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keYgUDisikoLaLxlAgada; sAdhisalAgada. 
\num{2} athaRvAgada; garxhisalu kaSaTxvAda; AloVcanege sikakxda. 
\enum
\emng
\eentry

\bentry
\word{unrealized} 
\pron{anfrialeYsZfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keYgUDada; sididhxsada; sAdhisada. 
\num{2} tiLiyade hoVda; ajAcnxta; avayxkatx; supatxvAda; yArigU gotAtxgada: \eng{unrealized talent} supatxvAda parxtiBe. 
\enum
\emng
\eentry

\bentry
\word{unreally} 
\pron{anfriali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (\ame, \ashi) naMbalAgadaMte; avishavxsaniVyavAgi. 
\num{2} AshacxyaRjanakavAgi. 
\enum
\emng
\eentry

\bentry
\word{unreaped} 
\pron{anfriVpfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (beLe) kaTAvu mADada; koyulx mADada; koyudx saMgarxha mADada. 
\num{2} (\rUpa) (Pala) paDeyada; anuBavisada; anuBoVgisada; uNaNxda. 
\enum
\emng
\eentry

\bentry
\word{unreason} 
\pron{anfriVsaZnf} 
\gl{\nA}
\expl{}
\bmng
budidhx, viveVcane ilalxdiruvudu; aviveVka; mUKaRtana; mwDhayx. 
\emng
\eentry

\bentry
\word{unreasonable} 
\pron{anfriVsaZnabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vicArahiVna; aviveVkada. 
\num{2} ati dubAriya; nAyxyavalalxda; anAyxyada: \eng{unreasonable demands} nAyxyavalalxda beVDikegaLu. 
\enum
\emng
\eentry

\bentry
\word{unreasonableness} 
\pron{anfriVsaZnabflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vicArashUnayxte; aviveVka; mUKaRtana; mwDhayx. 
\num{2} nAyxyavalalxda vataRne. 
\enum
\emng
\eentry

\bentry
\word{unreasonably} 
\pron{anfriVsaZnabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} viveVcanAshUnayxvAgi; aviveVkadiMda; mUKaRtanadiMda. 
\num{2} nAyxyasamamxtavalalxda riVtiyalilx; anAyxyavAgi. 
\enum
\emng
\eentry

\bentry
\word{unreasoned} 
\pron{anfriVsaZnfDx} 
\gl{\gu}
\expl{}
\bmng
viveVcaneyilalxda; aviveVkada. 
\emng
\eentry

\bentry
\word{unreasoning} 
\pron{anfriVsaZniMgf} 
\gl{\gu}
\expl{}
\bmng
viveVcane mADada; aviveVkada. 
\emng
\eentry

\bentry
\word{unreasoningly} 
\pron{anfriVsaZniMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
viveVcaneyilalxde; aviveVkadiMda. 
\emng
\eentry

\bentry
\word{unreceptive} 
\pron{anfrisepiTxvf} 
\gl{\gu}
\expl{}
\bmng
(BAvanegaLu \mo vanunx) garxhisuva sAmathaRyxvilalxda; garxhaNashakitx ilalxda. 
\emng
\eentry

\bentry
\word{unreciprocated} 
\pron{anfresiparxkeVTiDf} 
\gl{\gu}
\expl{}
\bmng
(pirxVti, gwrava, \mo vugaLa \vi) parasapxravalalxda; hiMtirugisada; parxtiyAgi toVrisada. 
\emng
\eentry

\bentry
\word{unreckoned} 
\pron{anfrekanfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} lekakxdalilx yA lekakxkekx seVrisada; eNikege seVrada. 
\num{2} BAvisada; parigaNisada. 
\enum
\emng
\eentry

\bentry
\word{unreclaimed} 
\pron{anfrikelxVmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (BUmiya \vi) sAguvaLige tarada; kaqSi yoVgayxvAgisada. 
\num{2} (vayxkitxgaLa \vi) sanAmxgaRkekx tarada; udAdhxra mADada. 
\num{3} sudhArisada; paLagisada; tidadxda; sAdhu mADada. 
\num{4} punavaRshamADikoLaLxda; punaHsAvxdhiVnapaDisikoLaLxda; maruvasha paDeyada. 
\enum
\emng
\eentry

\bentry
\word{unrecognised} 
\pron{anfrekagenxYsfDx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unrecognized}{unrecognized}. 
\emng
\eentry

\bentry 
\word{unrecognizable} 
\pron{anfrekagenxYsaZbflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gurutisalAgada. 
\num{2} mAnayxte niVDalAgada; aMgiVkArAhaRvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unrecognizableness} 
\pron{anfrekagenxYsaZbflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} gurutisalAgadiruvike. 
\num{2} mAnayxte niVDalAgadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unrecognizably} 
\pron{anfrekagenxYsaZbilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} gurutisalAgadaMte. 
\num{2} mAnayxte niVDalAgada riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{unrecognized} 
\pron{anfrekagenxYsZfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gurutisada. 
\num{2} mAnayxte niVDada; aMgiVkarisada. 
\enum
\emng
\eentry

\bentry
\word{unrecompensed} 
\pron{anfrekaMpenfsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (kAyaRkekx takakx) parxtiPala niVDada; takakx purasAkxravilalxda. 
\num{2} parihAra niVDada; naSaTx kaTiTx koDada. 
\enum
\emng
\eentry

\bentry
\word{unreconciled} 
\pron{anfrekanfseYlfDx} 
\gl{\gu}
\expl{}
\bmng
manasAtxpa bagehariyada; rAjiyAgada; samAdhAnakekx barada. 
\emng
\eentry

\bentry
\word{unreconstructed} 
\pron{anfriVkanfsaTxrXkiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} punaniRmARNa mADada; punAracisada; pUvaRrUpakekx yA hosarUpakekx yA pUNaRrUpakekx tarada. 
\num{2} parxcalita rAjakiVya saMparxdAyakekx -- hoMdAvaNe mADikoLaLxda yA parivatiRtavAgada. 
\enum
\emng
\eentry

\bentry
\word{unrecordable} 
\pron{anfrikADaRbflf} 
\gl{\gu}
\expl{}
\bmng
dAKalisalAgada. 
\emng
\eentry

\bentry
\word{unrecorded} 
\pron{anfrikADiRDf} 
\gl{\gu}
\expl{}
\bmng
dAKalu mADada; namUdisada; gurutu hAkiDada. 
\emng
\eentry

\bentry
\word{unrectified} 
\pron{anfrekiTxpheYDf} 
\gl{\gu}
\expl{}
\bmng
tapupx tidadxda; doVSa pariharisada; neVpaRDisada; QujugoLisada. 
\emng
\eentry

\bentry
\word{unredeemable} 
\pron{anfriDiVmabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} punaHsAvxdhiVnapaDisikoLaLxlAgada; haNa koTuTx matetx vashapaDisikoLaLxlu sAdhayxvAgada. 
\num{2} udadhxrisalAgada; udAdhxra mADalu sAdhayxvAgada. 
\enum
\emng
\eentry

\bentry
\word{unredeemably} 
\pron{anfriDiVmabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} punaHsAvxdhiVnapaDisikoLaLxlAgadaMte; haNa koTuTx hiMtegedukoLaLxlu sAdhayxvAgadaMte. 
\num{2} udAdhxramADalu sAdhayxvilalxdaMte; udadhxrisalAgadaMte. 
\enum
\emng
\eentry

\bentry
\word{unredeemed} 
\pron{anfriDiVmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} QuNamukatxvAgada; haNa koTuTx hiMtegedukoLaLxda. 
\num{2} udAdhxra mADirada. 
\enum
\emng
\eentry

\bentry
\word{unredressed} 
\pron{anfriDerxsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nivArisada; neVpaRDisada. 
\num{2} (naSaTxkekx) parihAra koDada; parxtiPala koDada. 
\enum
\emng
\eentry

\bentry
\word{unreel} 
\pron{anfriVlf} 
\gl{\sakirx}
\expl{}
\bmng
(dAra \mo vanunx) rATeyiMda bicucx; sutitxruvudanunx bicucx (\akirx\ saha). 
\emng
\eentry

\bentry
\word{unreeve} 
\pron{anfriVvf} 
\gl{\sakirx}
\expl{(\BU\ \eng{unrove}). }
\bmng
(raMdharx \mo vugaLalilx tUrisiruva hagagx \mo vugaLanunx) horageLe; horakekx tege. 
\emng
\eentry

\bentry
\word{unrefined} 
\pron{anfripheYnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parishudadhxgoLisada; pariSakxrisada; saMsakxrisada: \eng{unrefined metals} parishudadhx goLisada loVhagaLu. 
\num{2} (BASe, naDate, \mo vugaLalilx) asaMsakxqqta; nayanAjUkilalxda. 
\enum
\emng
\eentry

\bentry
\word{unreflecting} 
\pron{anfriphelxkiTxMgf} 
\gl{\gu}
\expl{}
\bmng
ciMtane mADada; (tananxlelxV) AloVcane mADada; AloVcanA rahita: \eng{an unreflecting, self-satisfied man} AloVcane mADada, AtamxsaMtaqpatx vayxkitx. 
\emng
\eentry

\bentry
\word{unreflectingly} 
\pron{anfriphelxkiTxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
AloVcisade; viveVcisade. 
\emng
\eentry

\bentry
\word{unreflectingness} 
\pron{anfriphelxkiTxMgfnisf} 
\gl{\nA}
\expl{}
\bmng
AloVcane mADadiruvike. 
\emng
\eentry

\bentry
\word{unreformed} 
\pron{anfriphAZmfRDx} 
\gl{\gu}
\expl{}
\bmng
pariSakxrisada; sudhArisada; sudhAraNe mADada yA Agada. 
\emng
\eentry

\bentry
\word{unregal} 
\pron{anfriVgalf} 
\gl{\gu}
\expl{}
\bmng
rAjayoVgayxvalalxda; rAjaveYBavavilalxda. 
\emng
\eentry

\bentry
\word{unregarded} 
\pron{anfrigADiRDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} alakiSxta; lakaSxyXkoDada; gaNanege tegedukoLaLxda. 
\num{2} pariBAvisada; parigaNisada; upeVkiSxta. 
\enum
\emng
\eentry

\bentry
\word{unregeneracy} 
\pron{anfrijenarasi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} pashAcxtAtxpa paDadiruvike. 
\num{2} moMDutana; haTamAritana. 
\enum
\emng
\eentry

\bentry
\word{unregenerate} 
\pron{anfrijenaraTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pashAcxtAtxpavilalxda: \eng{an unregenerate sinner} pashAcxtAtxpavilalxda pApi. 
\num{2} AtomxVdAdhxra mADikoLaLxda; AtomxVnanxti sAdhisada. 
\num{3} moMDutanada; haTamAritanada: \eng{an unregenerate reactionary} haTamAritanada parxtigAmi. 
\enum
\emng
\eentry

\bentry
\word{unregenerately} 
\pron{anfrijenaraTfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} pashAcxtAtxpa paDade. 
\num{2} moMDutanadiMda; haTamAritanadiMda. 
\enum
\emng
\eentry

\bentry
\word{unregistered} 
\pron{anfrejisaTxDfR} 
\gl{\gu}
\expl{}
\bmng
(gotAtxda dAKale pusatxkadalilx) dAKalu mADada; noVMdAyisada; dAKalAti ilalxda. 
\emng
\eentry

\bentry
\word{unregretted} 
\pron{anfrigerxTiDf} 
\gl{\gu}
\expl{}
\bmng
viSAdisada; vayxthepaDada; pashAcxtAtxpa paDada; marugada. 
\emng
\eentry

\bentry
\word{unregulated} 
\pron{anfreguyxleVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaTiTxgoLapaDisada; niyamagaLigoLapaDada. 
\num{2} mitigoLisada; keVvala Avashayxkatege takakxMte hoMdisada. 
\num{3} niyaMtirxsada; aniyaMtirxta. 
\num{4} sarihoMdisada; karxmapaDisada. 
\enum
\emng
\eentry

\bentry
\word{unrehearsed} 
\pron{anfrihasfRTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaMThapATha opipxsada. 
\num{2} (nATaka \mo vugaLa \vi) pUvARBinaya mADada; parxdashaRnABAyxsavilalxda; pUvaRparxyoVgavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unrein} 
\pron{anfreVnf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (kudure) lagAmu saDilisu. 
\num{2} (\rUpa) hiDita saDilisu; kaTuTx saDilisu. 
\enum
\emng
\eentry

\bentry
\word{unrelated} 
\pron{anfrileVTiDf} 
\gl{\gu}
\expl{}
\bmng
saMbaMdhapaDada; saMbaMdharahita. 
\emng
\eentry

\bentry
\word{unrelatedness} 
\pron{anfrileVTiDfnisf} 
\gl{\nA}
\expl{}
\bmng
saMbaMdharAhitayx; saMbaMdhapaDadiruvike. 
\emng
\eentry

\bentry
\word{unrelaxed} 
\pron{anfrilAYxkfsxTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saDila biDada; haguragoLisada; bigupina. 
\num{2} (manasusx) maqduvAgada; karagada. 
\enum
\emng
\eentry

\bentry
\word{unrelenting} 
\pron{anfrileMTiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} maruka toVrada; dayedAkiSxNayxvilalxda; manasusx karagada; maqduvAgada. 
\num{2} (kwrxyaR, kAThinayx, tiVvarxte, veVga, \mo vugaLa \vi) kaDame mADada; tagigxsada; EkaparxkAravAda: \eng{unrelenting progress} EkaparxkAravAda aBivaqdidhx. 
\num{3} maNiyada; bagagxda; paTuTx -- biDada, saDilisada. 
\enum
\emng
\eentry

\bentry
\word{unrelentingly} 
\pron{anfrileMTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} maruka toVradaMte; manasusx karagadaMte. 
\num{2} (kwrxyaR, kAThinayx, tiVvarxte, \mo vugaLa \vi) kaDame mADadaMte; EkaparxkAravAgiruvaMte; tagagxdaMte. 
\num{3} maNiyade; bagagxde; sagagxde. 
\enum
\emng
\eentry

\bentry
\word{unrelentingness} 
\pron{anfrileMTiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} maruka toVradiruvike; manasusx karagadiruvike. 
\num{2} (kwrxyaR, kAThinayx, tiVvarxte, \mo vugaLa \vi) kaDame mADadiruvike; EkaparxkAravAgi muMduvariyuvike. 
\num{3} maNiyadiruvike; bagagxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unreliability} 
\pron{anfrileYabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} avishavxsaniVyate; necacxlAgadiruvike; naMbikege ahaRvalalxdiruvike; vishAvxsAhaRvalalxdiruvike. 
\num{2} KAtari, Baravase -- ilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unreliable} 
\pron{anfrileYabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} necacxlAgada; avalaMbisalAgada; naMbikege ahaRvalalxda; vishAvxsakekx yoVgayxvalalxda. 
\num{2} KAtari ilalxda; Baravase iDalAgada. 
\enum
\emng
\eentry

\bentry
\word{unreliableness} 
\pron{anfrileYabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unreliability}{unreliability}. 
\emng
\eentry

\bentry
\word{unreliably} 
\pron{anfrileYabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} necacxlAgadaMte; vishAvxsAhaRvalalxdaMte. 
\num{2} KAtari ilalxdaMte. 
\enum
\emng
\eentry

\bentry
\word{unrelieved} 
\pron{anfriliVvfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} upashamana, nemamxdi, parihAra -- ilalxda. 
\num{2} veYvidhayxvilalxda; beVsara huTiTxsuva; EkarUpateya; oMdeV samanAda: \eng{a plain black dress unrelieved by any trimming} oMdiSUTx holige alaMkAravilalxda, veYvidhayxrahita, sAdA kapupx uDupu. 
\num{3} sahAya, neravu, beMbala -- ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unrelievedly} 
\pron{anfriliVvfDxli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} upashamana, parihAra, nemamxdi -- ilalxdaMte. 
\num{2} veYvidhayxvilalxde; oMdeV samanAgi; beVsara huTiTxsuvaMte. 
\enum
\emng
\eentry

\bentry
\word{unreligious} 
\pron{anfrilijasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} = \hyperref{kandict_i.pdf}{I}{irreligious}{irreligious}. 
\num{2} matadhameRVtara; matadhamaRkekx saMbaMdhapaDada. 
\enum
\emng
\eentry

\bentry
\word{unremarkable} 
\pron{anfrimAkaRbflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gamanAhaRvalalxda; gamana seLeyada. 
\num{2} AkaSaRNeyilalxda; sAdhAraNavAda. 
\enum
\emng
\eentry

\bentry
\word{unremarkably} 
\pron{anfrimAkaRbilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} gamanAhaRvalalxdaMte. 
\num{2} AkaSaRNeyilalxde; sAdhAraNavAda riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{unremembered} 
\pron{anfrimeMbaDfR} 
\gl{\gu}
\expl{}
\bmng
maretuhoVda; visamxqqta; samxrisada; nenapu mADikoLaLxda. 
\emng
\eentry

\bentry
\word{unremitting} 
\pron{anfrimiTiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaDameyAgada; akuMThita; kuMdada. 
\num{2} eDebiDada; aviciPxnanx; satata: \eng{unremitting noise} satatavAda sadudx. 
\enum
\emng
\eentry

\bentry
\word{unremittingly} 
\pron{anfrimiTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kuMdade; kaDameyAgade. 
\num{2} satatavAgi; eDebiDade; avishArxMtavAgi; aviciPxnanxvAgi. 
\enum
\emng
\eentry

\bentry
\word{unremittingness} 
\pron{anfrimiTiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
sAtatayx; aviciPxnanxte. 
\emng
\eentry

\bentry
\word{unremorseful} 
\pron{anfrimAsfRphulf} 
\gl{\gu}
\expl{}
\bmng
pashAcxtAtxpa -- paDada, ilalxda. 
\emng
\eentry

\bentry
\word{unremorsefully} 
\pron{anfrimAsfRphuli} 
\gl{\kirxvi}
\expl{}
\bmng
pashAcxtAtxpavilalxdaMte; pashAcxtAtxparahitavAgi. 
\emng
\eentry

\bentry
\word{unremovable} 
\pron{anfrimUvabflf} 
\gl{\gu}
\expl{}
\bmng
tegedu hAkalAgada; tegedu hAkalu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{unremunerative} 
\pron{anfrimUyxnaraTivf} 
\gl{\gu}
\expl{}
\bmng
lABakaravalalxda; lABadAyakavalalxda; lABarahita; savxlapxvU yA sAkaSuTx lABa yA varamAna niVDada. 
\emng
\eentry

\bentry
\word{unremuneratively} 
\pron{anfrimUyxnaraTivfli} 
\gl{\kirxvi}
\expl{}
\bmng
lABadAyakavalalxda riVtiyalilx; lABakaravAgirade. 
\emng
\eentry

\bentry
\word{unremunerativeness} 
\pron{anfrimUyxnaraTivfnisf} 
\gl{\nA}
\expl{}
\bmng
lABadAyakavAgilalxdiruvike. 
\emng
\eentry

\bentry
\word{unrenewable} 
\pron{anfrinUyxabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} punanaRviVkarisalAgada; matetx yA hosadAgi mADalAgada. 
\num{2} pUvaRsithxtige taralAgada. 
\num{3} muMduvaresalAgada. 
\enum
\emng
\eentry

\bentry
\word{unrenewed} 
\pron{anfrinUyxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} punanaRviVkarisada. 
\num{2} pUvaRsithxtige tarada. 
\num{3} muMduvaresada. 
\enum
\emng
\eentry

\bentry
\word{unrenounced} 
\pron{anfrinwnfsxTf} 
\gl{\gu}
\expl{}
\bmng
tayxjisada; tirasakxrisada; vajiRsada; nirAkarisada; toreyada. 
\emng
\eentry

\bentry
\word{unrepealed} 
\pron{anfripiVlfDx} 
\gl{\gu}
\expl{}
\bmng
radudxmADada; tegeduhAkada; toDeduhAkada; vApasu tegedukoLaLxda; hiMtegedukoLaLxda. 
\emng
\eentry

\bentry
\word{unrepeatability} 
\pron{anfripiVTabiliTi} 
\gl{\nA}
\expl{}
\bmng
apunarAvataRniVyate; punarAvaqtitxrAhitayx: 
\banum
\alnum{a} punarAvaqtitx mADalAgadiruvike; matetx mADalAgadiruvike; heVLalAgadiruvike. 
\alnum{b} punarucacxrisalAgadiruvike. 
\alnum{c} (matotxmemx heVLalAgadaSuTx) holasAgiruvike; ashilxVlavAgiruvike. 
\eanum
\emng
\eentry

\bentry
\word{unrepeatable} 
\pron{anfripiVTabflf} 
\gl{\gu}
\expl{}
\bmng
punarAvatiRsalAgada: 
\banum
\alnum{a} punarAvaqtitx mADalAgada; matetx mADalAgada; heVLalAgada. 
\alnum{b} tirugi, matotxmemx -- heVLalAgada; punarucacxrisalAgada. 
\alnum{c} (matotxmemx heVLalAgadaSuTx) holasAda; ashilxVlavAda. 
\eanum
\emng
\eentry

\bentry
\word{unrepentent} 
\pron{anfripeMTaMTf} 
\gl{\gu}
\expl{}
\bmng
pashAcxtAtxpa paDada. 
\emng
\eentry

\bentry
\word{unrepentently} 
\pron{anfripeMTaMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
pashAcxtAtxpa paDade. 
\emng
\eentry

\bentry
\word{unreported} 
\pron{anfripoVTiRDf} 
\gl{\gu}
\expl{}
\bmng
varadi -mADada, opipxsada; varadiyAgada. 
\emng
\eentry

\bentry
\word{unrepresentative} 
\pron{anfripirxseZMTaTivf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} apArxtinidhika; pArxtinidhikavalalxda; parxtinidhisada. 
\num{2} (oMdu vagaRda yA vagaRgaLa) nidashaRnavAgirada; mAdariyAgirada. 
\enum
\emng
\eentry

\bentry
\word{unrepresentativeness} 
\pron{anfripirxseZMTaTivfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} apArxtinidhikate; parxtinidhisadiruvike. 
\num{2} (oMdu vagaRda yA vagaRgaLa) nidashaRnavAgadiruvike; mAdariyalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unrepresented} 
\pron{anfripirxseZMTiDf} 
\gl{\gu}
\expl{}
\bmng
parxtinidhiyilalxda; pArxtinidhayxvilalxda; pArxtinidhayx rahitavAda. 
\emng
\eentry

\bentry
\word{unreproved} 
\pron{anfripUrxvfDx} 
\gl{\gu}
\expl{}
\bmng
AkeSxVpaNegoLagAgada; dUSaNege silukada. 
\emng
\eentry

\bentry
\word{unrequested} 
\pron{anfrikevxsiTxDf} 
\gl{\gu}
\expl{}
\bmng
ayAcita; keVLada yA koVrada. 
\emng
\eentry

\bentry
\word{unrequited} 
\pron{anfrikevxYTiDf} 
\gl{\gu}
\expl{}
\bmng
(perxVma \mo vugaLa \vi) parxtiyAgi hiMtirugisada; parxtiyAgi koDada; aparxtidatatx. 
\emng
\eentry

\bentry
\word{unrequitedly} 
\pron{anfrikevxYTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
(perxVma \mo vugaLa \vi) aparxtidatatxvAgi; parxtiyAgi -- hiMtirugisadaMte, koDadaMte. 
\emng
\eentry

\bentry
\word{unrequitedness} 
\pron{anfrikevxYTiDfnisf} 
\gl{\nA}
\expl{}
\bmng
aparxtidAna; (perxVma \mo vugaLa \vi) parxtiyAgi -- hiMtirugisadiruvike, koDadiruvike. 
\emng
\eentry

\bentry
\word{unreserve} 
\pron{anfrisaZvfR} 
\gl{\nA}
\expl{}
\bmng
nisasxMkoVcate; mucucxmare ilalxda naDavaLike. 
\emng
\eentry

\bentry
\word{unreserved} 
\pron{anfrisaZRvfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} miVsalalalxda; kAyidxrisada; miVsalAgiDada: \eng{unreserved seats} (modaleV haNakoTuTx) miVsalAgiDada piVThagaLu yA sAthxnagaLu. 
\num{2} itimitigaLilalxda; kaTuTxpADilalxda; saMpUNaRvAda: \eng{unreserved confidence} saMpUNaRvAda naMbike. 
\num{3} mucucxmareyilalxda; tereda manasisxna: \eng{an unreserved nature} mucucxmareyilalxda savxBAva. 
\enum
\emng
\eentry

\bentry
\word{unreservedly} 
\pron{anfrisaviRDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} saMkoVca, bigumAna, mucucxmare -- ilalxde; nisasxMkoVcavAgi; sapxSaTxvAgi; saraLavAgi. 
\num{2} saMpUNaRvAgi. 
\enum
\emng
\eentry

\bentry
\word{unreservedness} 
\pron{anfrisaviRDfnisf} 
\gl{\nA}
\expl{}
\bmng
saMkoVca, bigumAna, mucucxmare -ilalxdiruvudu; nisasxMkoVcavAgiruvike; niSakxpaTate. 
\emng
\eentry

\bentry
\word{unresisted} 
\pron{anfrisiZsiTxDf} 
\gl{\gu}
\expl{}
\bmng
viroVdhisada; parxtiBaTisada; edurisi hoVrADada. 
\emng
\eentry

\bentry
\word{unresistedly} 
\pron{anfrisiZsiTxDfli} 
\gl{\kirxvi}
\expl{}
\bmng
niroVdhisade; parxtiBaTisade; edurisade. 
\emng
\eentry

\bentry
\word{unresisting} 
\pron{anfrisiZsiTxMgf} 
\gl{\gu}
\expl{}
\bmng
viroVdhisadiruva. 
\emng
\eentry

\bentry
\word{unresistingly} 
\pron{anfrisiZsiTxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
= \hyperlink{unresistedly}{unresistedly}. 
\emng
\eentry

\bentry 
\word{unresistingness} 
\pron{anfrisiZsiTxMgfnisf} 
\gl{\nA}
\expl{}
\bmng
aparxtiBaTane; aviroVdha; viroVdhisadiruvike; parxtiBaTisadiruvike. 
\emng
\eentry

\bentry
\word{unresolvable} 
\pron{anfrisAZlavxbalf} 
\gl{\gu}
\expl{}
\bmng
(samaseyx, tikAkxTa, \mo vugaLa \vi) pariharisalAgada; biDisalAgada. 
\emng
\eentry

\bentry
\word{unresolved} 
\pron{anfrisAZlfvxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pariharisada; bageharisada; biDisada. 
\num{2} nidhaRrisada; manasusx mADada; saMkalipxsada. 
\enum
\emng
\eentry

\bentry
\word{unresolvedly} 
\pron{anfrisAZlivxDilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} pariharisalAgadaMte. 
\num{2} nidhaRrisalAgade. 
\enum
\emng
\eentry

\bentry
\word{unresolvedness} 
\pron{anfrisAZlivxDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aparihAyaR; bageharisalAgadiruvike. 
\num{2} nidhaRrisalAgadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unresponsive} 
\pron{anfrisApxnisxvf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (parxBAva \mo vugaLige) sulaBavAgi parxtikirxye toVrisada. 
\num{2} sulaBavAgi -- perxVrisalAgada, parxBAvisalAgada. 
\num{3} sahAnuBUtiyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unresponsively} 
\pron{anfrisApxnisxvfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (parxBAva \mo vugaLige) parxtikirxye toVrisadaMte. 
\num{2} sahAnuBUti toVrisade. 
\enum
\emng
\eentry

\bentry
\word{unresponsiveness} 
\pron{anfrisApxnisxvfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (parxBAva \mo vugaLige) parxtikirxye toVrisadiruvike; parxtikirxyAshUnayxte; parxtikirxyArAhitayx. 
\num{2} sahAnuBUtiya aBAva. 
\enum
\emng
\eentry

\bentry
\word{unrest} 
\pron{anfresfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vishArxMtiyilalxdiruvudu; 
\num{2} (manasisxna) taLamaLa; goMdala. 
\num{3} ashAMti; koSxVBe: \eng{political unrest} rAjakiVya ashAMti. 
\enum
\emng
\eentry

\bentry
\word{unrestful} 
\pron{anfresfTxPuZlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vishArxMtiyilalxda. 
\num{2} taLamaLadiMda kUDida. 
\num{3} ashAMtavAda; parxkuSxbadhx. 
\enum
\emng
\eentry

\bentry
\word{unrestfully} 
\pron{anfresfTxPuZli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} virAmavilalxde; vishArxMtiyilalxde. 
\num{2} ashAMtavAgi; parxkuSxbadhxvAgi; taLamaLadiMda kUDi. 
\enum
\emng
\eentry

\bentry
\word{unresting} 
\pron{anfresiTxMgf} 
\gl{\gu}
\expl{}
\bmng
vishArxMti -- ilalxda yA paDeyada. 
\emng
\eentry

\bentry
\word{unrestingly} 
\pron{anfresiTxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
viramisade; vishArxMti -- ilalxde yA paDeyade. 
\emng
\eentry

\bentry
\word{unrestored} 
\pron{anfrisoTxVDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hiMdakekx koDada; maruvashavAgada; punaH sAvxdhiVnapaDisada. 
\num{2} (haLeya kaTaTxDa \mo vugaLa \vi) jiVNoRVdAdhxra mADada; pUvaRsithxtige tarada. 
\num{3} (modalina gwrava, padavi, \mo vugaLanunx) matetx koDada. 
\num{4} pUvaRsAthxnadalilx yA pUvaRsithxtiyalilx punaH sAthxpisada. 
\num{5} (garxMtha \mo vugaLalilx biTuTxhoVda yA naSaTxvAda pada \mo vugaLanunx) Uhisi matetx seVrisada; punaH seVrisi modalina yA mUlapAThavanunx sAthxpisada. 
\enum
\emng
\eentry

\bentry
\word{unrestrained} 
\pron{anfriseTxrXVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} taDeyada; bigi hiDiyada; niroVdhisada. 
\num{2} aMkeyalilx iDada; hatoVTiyalilxTuTxkoLaLxda; hadidxnalilxDada; nigarxhisada; nibaRMdhadalilxDada. 
\num{3} asaMyamita; saMyama mADada. 
\enum
\emng
\eentry

\bentry
\word{unrestrainedly} 
\pron{anfriseTxrXVniDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sevxVcePxyAgi; nibaRMdhavilalxde. 
\num{2} saMyama vilalxde; aMkeyalilxlalxde; niyaMtarxNarahitavAgi. 
\enum
\emng
\eentry

\bentry
\word{unrestrainedness} 
\pron{anfriseTxrXVniDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} taDeyilalxdiruvike; anibaRMdhate. 
\num{2} sevxVcePxyAgiruvike; asaMyama. 
\enum
\emng
\eentry

\bentry
\word{unrestricted} 
\pron{anfrisiTxrXkiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mitagoLisada; parimita mADada. 
\num{2} kaTuTxpaDisada; nibaRMdhisada; anibaRMdhita. 
\num{3} (\kanmu\ vAhana saMcAra mAgaR \mo vugaLa \vi) veVgada nibaRMdhavilalxda; veVgada miti ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unrestrictedly} 
\pron{anfrisiTxrXkiTxDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} mitagoLisade. 
\num{2} anibaRMdhitavAgi. 
\num{3} (\kanmu\ vAhanasaMcAra mAgaR \mo vugaLa \vi) veVgamiti ilalxdaMte. 
\enum
\emng
\eentry

\bentry
\word{unrestrictedness} 
\pron{anfrisiTxrXkiTxDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} mitagoLisadiruvike. 
\num{2} anibaRMdhitate. 
\num{3} (\kanmu\ vAhanasaMcAra mAgaR \mo vugaLa \vi) veVgamiti ilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unreturned} 
\pron{anfriTanfRDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hiMdirugisada; parxtiPalavAgi yA parxtiyAgi salilxsada. 
\num{2} hiMdakekx koDada; vApasusx mADada. 
\enum
\emng
\eentry

\bentry
\word{unrevealed} 
\pron{anfriviVlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} horageDahada; bayalu mADada; parxkaTisada; bahiraMgapaDisada; guTATxda. 
\num{2} teredu toVrada; toVrisada; kANisada. 
\enum
\emng
\eentry

\bentry
\word{unreversed} 
\pron{anfrivasfRTx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ nidhARra \mo vugaLa \vi) hiMdumuMdu mADada; talekeLagu mADada; avipayaRsatx; vipayaRya mADada; virudadhxvAgi tirugisada. 
\emng
\eentry

\bentry
\word{unrevised} 
\pron{anfriveYsfZDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} punaHparishiVlisada; matetx Odi tidadxda, saripaDisada; punaHpariSakxrisada; sudhArisada. 
\num{2} mUlarUpada. 
\enum
\emng
\eentry

\bentry
\word{unrevoked} 
\pron{anfrivoVkfTx} 
\gl{\gu}
\expl{}
\bmng
(tiVpuR, shAsana, anumati, \mo vugaLa \vi) hiMdakekx tegedukoLaLxda; radudx mADada. 
\emng
\eentry

\bentry
\word{unrewarded} 
\pron{anfrivADiRDf} 
\gl{\gu}
\expl{}
\bmng
parxtiPalavilalxda; mecucxge paDeyada. 
\emng
\eentry

\bentry
\word{unrewarding} 
\pron{anfrivADiRMgf} 
\gl{\gu} 
\bmng
\bnum
\num{1} parxtiPala niVDada; vayxthaR parxyAsada. 
\num{2} taqpitx koDada; saMtoVSadAyakavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unrhetorical} 
\pron{anfriTArikalf} 
\gl{\gu}
\expl{}
\bmng
kaqtaka yA ADaMbara BASeyalilx heVLada; AlaMkArikavalalxda; vAgADaMbaravilalxda. 
\emng
\eentry

\bentry
\word{unrhymed} 
\pron{anfreYmfDx} 
\gl{\gu}
\expl{}
\bmng
pArxsavilalxda; pArxsabadadhxvAgi bareyada. 
\emng
\eentry

\bentry
\word{unrhythmical} 
\pron{anfridimxkalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} CaMdoVgatiyilalxda; CaMdoVbadadhxvalalxda; tALabadadhxvalalxda. 
\num{2} (kale \mo vugaLa \vi) sAvayava samanavxya ilalxda; sAmarasayxvilalxda. 
\enum
\emng
\eentry

\bentry
\word{unrhythmically} 
\pron{anfridimxkali} 
\gl{\kirxvi}
\expl{}
\bmng
CaMdoVgatiyilalxdaMte; tALabadadhxvAgirade. 
\emng
\eentry

\bentry
\word{unridable} 
\pron{anfreYDabflf} 
\gl{\gu}
\expl{}
\bmng
savArige yoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{unridden} 
\pron{anfriDanf} 
\gl{\gu}
\expl{}
\bmng
(yAvudoV oMdariMda) tapipxsada; biDisada; biDugaDe mADada; vimoVcisada. 
\emng
\eentry

\bentry
\word{unriddle} 
\pron{anfriDflf} 
\gl{\akirx}
\expl{}
\bmng
rahasayx, samaseyx, \mo vanunx -- biDisu, pariharisu, bageharisu. 
\emng
\eentry

\bentry
\word{unriddler} 
\pron{anfriDalxrf} 
\gl{\nA}
\expl{}
\bmng
rahasayx, samaseyx, \mo vanunx -- biDisuvava, pariharisuvava. 
\emng
\eentry

\bentry
\word{unrig} 
\pron{anfrigf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unrigged}; \vakaq\ \eng{unrigging}).}
\bmng
\bnum
\num{1} (\nw) hagagxda kaTuTxgaLanunx bicucx. 
\num{2} (\pArxM) vasatxrX kaLacu; baTeTx tege. 
\enum
\emng
\eentry

\bentry
\word{unright} 
\pron{anfreYTf} 
\gl{\nA}
\expl{}
\bmng
(\pArxparx) anAyxya; tapupx; sariyilalxdudx. 
\emng
\eentry

\bentry
\word{unrighteous} 
\pron{anfreYcasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Qujuvalalxda; anAyxyada. 
\num{2} duSaTx; niVca. 
\num{3} apArxmANika. 
\enum
\emng
\eentry

\bentry
\word{unrighteously} 
\pron{anfreYcasfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} Qujuvalalxda riVtiyalilx; anAyxyavAgi. 
\num{2} duSaTxtanadiMda; niVcatanadiMda. 
\num{3} apArxmANikavAgi. 
\enum
\emng
\eentry

\bentry
\word{unrighteousness} 
\pron{anfreYcasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} Qujute ilalxdiruvudu; anAyxyada naDevaLike. 
\num{2} duSaTxtana; niVcatana. 
\num{3} apArxmANikate. 
\enum
\emng
\eentry

\bentry
\word{unrip} 
\pron{anfripf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unripped}; \vakaq\ \eng{unripping}). }
\bmng
haridu tege; siVLi tege; siVLihAku. 
\emng
\eentry

\bentry
\word{unripe} 
\pron{anfreYpf} 
\gl{\gu}
\expl{}
\bmng
baliyada; mAgada; pakavxvalalxda; hadavalalxda; haNANxgilalxda; apakavx. 
\emng
\eentry

\bentry
\word{unripeness} 
\pron{anfreYpfnisf} 
\gl{\nA}
\expl{}
\bmng
apakavxte; baliyadiruvudu. 
\emng
\eentry

\bentry
\word{unrisen} 
\pron{anfrisaZnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ELada; meVlakekxVLada; anutithxta; edudxnilalxda. 
\num{2} huTaTxda; udayisada; mUDada. 
\enum
\emng
\eentry

\bentry
\word{unrivalled} 
\pron{anfreYvalfDx} 
\gl{\gu}
\expl{}
\bmng
(\ame\ \eng{unrivaled}). eNeyilalxda; sATiyilalxda; aparxtima; anupama. 
\emng
\eentry

\bentry
\word{unrivet} 
\pron{anfriviTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\BU\ matutx \BUkaq\ \eng{unriveted}; \vakaq\ \eng{unriveting}). (kaTimoLe, `riveTf' \mo vanunx hAki BadarxpaDisiruvudanunx) tege; saDilisu. 
\num{2} saDilisu; tege; biDu; kaLacu; bicucx. 
\enum
\emng
\eentry

\bentry
\word{unrobe} 
\pron{anfroVbf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} = \hyperref{kandict_d.pdf}{D}{disrobe}{disrobe}. 
\num{2} = \hyperlink{undress(1)}{$^1$undress} (\akirx\ saha). 
\enum
\emng
\eentry

\bentry
\word{unroll} 
\pron{anfroVlf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} suruLi bicucx; suruLi bicicx tege. 
\num{2} suruLi bicicx toVrisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
suruLi bicicxko. 
\emng
\eentry

\bentry
\word{unromantic} 
\pron{anframAYxMTikf} 
\gl{\gu}
\expl{}
\bmng
`romAyxMTikf' alalxda; ati ramayxvalalxda. 
\emng
\eentry

\bentry
\word{unromantically} 
\pron{anframAYxMTikali} 
\gl{\kirxvi}
\expl{}
\bmng
`romAyxMTikf' AgiradaMte; ramayxvalalxdaMte. 
\emng
\eentry

\bentry
\word{unroof} 
\pron{anfrUphf} 
\gl{\akirx}
\expl{}
\bmng
meVlAPxvaNi, mucicxge, \mo vanunx -- tege, kaLacu. 
\emng
\eentry

\bentry
\word{unroofed} 
\pron{anfrUphfTx} 
\gl{\gu}
\expl{}
\bmng
meVlAPxvaNi tegeda; mucicxge ilalxda. 
\emng
\eentry

\bentry
\word{unroot} 
\pron{anfrUTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} beVru yA buDa(sahita) -- kitutxhAku. 
\num{2} nimURla mADu. 
\enum
\emng
\eentry

\bentry
\word{unrope} 
\pron{anfroVpf} 
\gl{\akirx}
\expl{}
\bmng
hagagx bicucx; (hagagxdiMda kaTiTxruvudanunx) bicucx, kaLacu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
(pavaRtAroVhaNa) hagagxdiMda tananxnunx bicicxko, biDisiko. 
\emng
\eentry

\bentry
\word{unrounded} 
\pron{anfrwMDiDf} 
\gl{\gu}
\expl{}
\bmng
duMDAgirada yA duMDage mADada. 
\emng
\eentry

\bentry
\word{unrove} 
\pron{anfroVvf} 
\gl{\kirx}
\expl{}
\bmng
\eng{unreeve} padada BUtarUpa. 
\emng
\eentry

\bentry
\word{unroyal} 
\pron{anfrAyalf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unregal}{unregal}. 
\emng
\eentry

\bentry
\word{unruffled} 
\pron{anfraphalfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kadaDada; kalakada; koSxVBegoLaLxda; akuSxbadhx. 
\num{2} avicalita; sethxYyaRgeDada; galibiligoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{unruled} 
\pron{anfrUlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ADaLitavilalxda; ALike ilalxda. 
\num{2} (kAgadada hALeya \vi) gere hAkada. 
\enum
\emng
\eentry

\bentry
\word{unruliness} 
\pron{anfrUlinisf} 
\gl{\nA}
\expl{}
\bmng
ashisutx; hatoVTi ilalxdiruvudu; aMke tapipxruvike. 
\emng
\eentry

\bentry
\word{unruly} 
\pron{anfrUli} 
\gl{\gu}
\expl{(\tara\ \eng{unrulier}, \tama\ \eng{unruliest}). }
\bmng
niyaMtarxNadalilxDalAgada; hatoVTiyalilxDalAgada; shisitxge bagagxda: \eng{an unruly child} ashisitxna magu. 
\emng
\eentry

\bentry
\word{UNaRWA} 
\pron{anfrA} 
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United Nations Relief and Works Agency.} 
\emng
\eentry

\bentry
\word{unsaddle} 
\pron{anfsAYxDflf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (kudure \mo vugaLiMda) taDi tege; jiVnu tege. 
\num{2} (obabxna) kataRvayx, hoNe, BAra, \mo vanunx iLikisu; BAra iLisu. 
\enum
\emng
\eentry

\bentry
\word{unsafe} 
\pron{anfseVphf} 
\gl{\gu}
\expl{}
\bmng
surakiSxtavalalxda; apAyakara. 
\emng
\eentry

\bentry
\word{unsafely} 
\pron{anfseVphfli} 
\gl{\kirxvi}
\expl{}
\bmng
surakiSxtavalalxdaMte; apAyakaravAgi. 
\emng
\eentry

\bentry
\word{unsafeness} 
\pron{anfseVphfnisf} 
\gl{\nA}
\expl{}
\bmng
surakiSxtavalalxdiruvike; apAyakaravAgiruvike. 
\emng
\eentry

\bentry
\word[unsaid(1)]{unsaid} 
\pron{anfseDf} 
\gl{\gu}
\expl{}
\bmng
anukatx; (bAyi biTuTx) heVLada; vayxkatxpaDisada: \eng{some things are better left unsaid} kelavu viSayagaLanunx (bAyi biTuTx) heVLadiruvudeV cenanx. 
\emng
\eentry

\bentry
\word[unsaid(2)]{unsaid} 
\pron{anfseDf} 
\gl{\sakirx}
\expl{}
\bmng
\eng{unsay} eMbudara \BU\ matutx \BUkaq. 
\emng
\eentry

\bentry
\word{unsalaried} 
\pron{anfsAYxlariDf} 
\gl{\gu}
\expl{}
\bmng
saMbaLavilalxda; pagAravilalxda; veVtanarahita. 
\emng
\eentry

\bentry
\word{unsaleability} 
\pron{anfseVlabiliTi} 
\gl{\nA}
\expl{}
\bmng
avikerxVyate; mAralAgadiruvike; mArATa yoVgayxvalalxdiruvike. 
\emng
\eentry

\bentry
\word{unsaleable} 
\pron{anfseVlabflf} 
\gl{\gu}
\expl{}
\bmng
mAralAgada; mArATayoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{unsalted} 
\pron{anfsAliTxDf} 
\gl{\gu}
\expl{}
\bmng
upipxlalxda; upupx -beresada, hacacxda, hAkada. 
\emng
\eentry

\bentry
\word{unsanctified} 
\pron{anfsAYxMkiTxpheYDf} 
\gl{\gu}
\expl{}
\bmng
pavitirxVkarisada; shudidhxmADada; saMsakxrisada; dhamaRsamamxtavalalxda. 
\emng
\eentry

\bentry
\word{unsanctioned} 
\pron{anfsAYxMkfSanfDx} 
\gl{\gu}
\expl{}
\bmng
maMjUrAtiyalalxda; maMjUru mADada; opipxge koDada. 
\emng
\eentry

\bentry
\word{unsanitary} 
\pron{anfsAYxniTari} 
\gl{\gu}
\expl{}
\bmng
= \hyperref{kandict_i.pdf}{I}{insanitary}{insanitary}. 
\emng
\eentry

\bentry
\word{unsatisfactorily} 
\pron{anfsAYxTisfphAYxkaTxrili} 
\gl{\kirxvi}
\expl{}
\bmng
ataqpitxkaravAgi. 
\emng
\eentry

\bentry
\word{unsatisfactoriness} 
\pron{anfsAYxTisfphAYxkaTxrinisf} 
\gl{\nA}
\expl{}
\bmng
ataqpitxkaravAgiruvike. 
\emng
\eentry

\bentry
\word{unsatisfactory} 
\pron{anfsAYxTisfphAYxkaTxri} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ataqpitxkara. 
\num{2} kaLape dajeRya; sivxVkArAhaRvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unsatisfied} 
\pron{anfsAYxTisfpheYDf} 
\gl{\gu}
\expl{}
\bmng
taNisada; taNiyada; ataqpatx; taqpitxyAgada; taqpitx ilalxda. 
\emng
\eentry

\bentry
\word{unsatisfiedness} 
\pron{anfsAYxTisfpheYDfnisf} 
\gl{\nA}
\expl{}
\bmng
ataqpitx; taNisadiruvike; taqpitxyAgadiruvike; taqpitx ilalxdiruvike. 
\emng
\eentry

\bentry
\word{unsatisfying} 
\pron{anfsAYxTisfpheYiMgf} 
\gl{\gu}
\expl{}
\bmng
taqpitxkaravalalxda; taqpitxyAgada; taqpitx koDada. 
\emng
\eentry

\bentry
\word{unsatisfyingly} 
\pron{anfsAYxTisfpheYiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
taqpitxyAgirade; taqpitx koDadaMte. 
\emng
\eentry

\bentry
\word{unsaturated} 
\pron{anfsAYxca(Tuyx)reVTiDf} 
\gl{\gu}
\expl{}
\bmng
(\ravi) apayARpatx: 
\banum
\alnum{a} (dArxvaNada \vi) inUnx hecucx dArxvayxvanunx viliVna mADikoLaLxbalalx. 
\alnum{b} (kAbaRnika saMyukatxda \vi, \kanmu\ kobubx yA eNeNxya \vi) aNuracaneyalilx divxbaMdha yA tirxbaMdhagaLiruva, AdadxriMda saMkalana kirxyegaLalilx pAlogxLaLxbalalx. 
\eanum
\emng
\eentry

\bentry
\word{unsaturation} 
\pron{anfsAYxca(Tuyx)reVSanf} 
\gl{\nA}
\expl{}
\bmng
(\ravi) apayARpatxte: 
\banum
\alnum{a} (dArxvaNada \vi) inUnx hecucx dArxvayxvanunx viliVna mADikoLaLxbalilxke. 
\alnum{b} (kAbaRnika saMyukatxda \vi) racaneyalilx divxbaMdha yA tirxbaMdhagaLiruva, AdadxriMdaleV saMkalana irxyegaLalilx pAlogxLaLxbalilxke. 
\eanum
\emng
\eentry

\bentry
\word{unsaved} 
\pron{anfseVvfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} uLisada; uLitAya mADada. 
\num{2} rakiSxtanAgada. 
\num{3} udAdhxra paDeyada. 
\enum
\emng
\eentry

\bentry
\word{unsavourily} 
\pron{anfseVvarili} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} rucisade; hiDisade; hitavilalxde. 
\num{2} asahayxkaravAgi; jugupesx barisuvaMte. 
\enum
\emng
\eentry

\bentry
\word{unsavouriness} 
\pron{anfseVvarinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (ruci, vAsane, BAvanegaLige) hiDisadiruvudu; hitavilalxdiruvike; ahitavAgiruvike. 
\num{2} asahayxte; jugupesx huTiTxsuvaMtAguvudu. 
\enum
\emng
\eentry

\bentry
\word{unsavoury} 
\pron{anfseVvari} 
\gl{\gu}
\expl{}
\bmng
(\ame\ \eng{unsavory}). 
\bnum
\num{1} (ruci, vAsane, BAvanegaLige) hiDisada; hitavilalxda; ahitavAda. 
\num{2} jugupesx huTiTxsuva: \eng{unsavoury stories} asahayxkara kathegaLu. 
\num{3} niVtigeTaTx; aneYtika: \eng{a man with an unsavoury reputation} neYtika apaKAyxtiya vayxkitx. 
\enum
\emng
\eentry

\bentry
\word{unsay} 
\pron{anfseV} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unsaid}). }
\bmng
(heVLike \mo vanunx) nirAkarisu; (heVLidadxnunx) hiMtegeduko, ilalxvenunx. 
\emng
\eentry

\bentry
\word{unsayable} 
\pron{anfseVabflf} 
\gl{\gu}
\expl{}
\bmng
heVLalAgada; heVLalu ahaRvalalxda; avAcayx; avacaniVya. 
\emng
\eentry

\bentry
\word{unscalable} 
\pron{anfsekxVlabflf} 
\gl{\gu}
\expl{}
\bmng
hatatxlAgada; EralAgada; anAroVhayx. 
\emng
\eentry

\bentry
\word{unscale} 
\pron{anfsekxVlf} 
\gl{\sakirx}
\expl{}
\bmng
(miVnu, bAdAmi, \mo vugaLiMda) pore, hoTuTx, sipepx, \mo vanunx tegeyadiru. 
\emng
\eentry

\bentry
\word{unscarred} 
\pron{anfsAkxDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gAyada gurutu, kale -- Agada, mADada; kalegaTaTxda. 
\num{2} kaLaMkitagoLisada. 
\enum
\emng
\eentry

\bentry
\word{unscathed} 
\pron{anfsekxVdfDx} 
\gl{\gu}
\expl{}
\bmng
gAyavAgade tapipxsikoMDa; peTuTx tAgada; apAyadiMda pArAda. 
\emng
\eentry

\bentry
\word{unscented} 
\pron{anfseMTiDf} 
\gl{\gu}
\expl{}
\bmng
parimaLavilalxda; suvAsane ilalxda; suvAsane kaTaTxda. 
\emng
\eentry

\bentry
\word{unscheduled} 
\pron{anfSeDUyxlfDx} 
\gl{\gu}
\expl{}
\bmng
anigadita: 
\banum
\alnum{a} tapisxVlu paTiTx tayArisada. 
\alnum{b} tapisxVlu paTiTxyalilx seVrisada; nigadita vivarapaTiTxyalilx namUdisada. 
\eanum
\emng
\eentry

\bentry
\word{unscholarliness} 
\pron{anfsAkxlarflinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} paMDitoVcitavalalxdiruvike; vidAvxMsarige takukxdalalxdiruvike. 
\num{2} pAMDitayxpUNaRvalalxdiruvike; vidavxtUpxNaRvalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unscholarly} 
\pron{anfsAkxlarfli} 
\gl{\gu} 
\bmng
\bnum
\num{1} paMDitoVcitavalalxda; vidAvxMsarige takukxdalalxda. 
\num{2} pwrxDhavalalxda; pAMDitayxpUNaRvalalxda; vidavxtUpxNaRvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unschooled} 
\pron{anfsUkxlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} shAlege hoVgada; videyx kaliyada. 
\num{2} shisitxge, hatoVTige -- oLapaDada. 
\num{3} nuriyada; anuBavavilalxda. 
\num{4} sahajavAda; kaliyada; shikaSxNada mUlaka paDeyada. 
\enum
\emng
\eentry

\bentry
\word{unscientific} 
\pron{anfseYaMTiphikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vijAcnxnada arivilalxda; vijAcnxnada paricayavilalxda. 
\num{2} aveYjAcnxnika; ashAsitxrXVya; veYjAcnxnika sUtarxgaLige yA tatatxvXgaLige -- anuguNavAgirada, anusAravAgirada. 
\enum
\emng
\eentry

\bentry
\word{unscientifically} 
\pron{anfseYaMTiphikali} 
\gl{\kirxvi}
\expl{}
\bmng
aveYjAcnxnikavAgi; ashAsitxrXVyavAgi; vijAcnxnAnusAriyAgilalxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{unscramble} 
\pron{anfsAkxrXYxMbflf} 
\gl{\sakirx}
\expl{}
\bmng
(AkAshavANi yA dUravANiyalilx beVkeMdeV taraMgAMtara badalAyisi kaLuhisida gupatx sudidx, goMdalada mAtu, \mo vanunx) sariyAgi vAyxKAyxna mADu; atheRYsu. 
\emng
\eentry

\bentry
\word{unscrambler} 
\pron{anfsAkxrXYxMbalxrf} 
\gl{\nA}
\expl{}
\bmng
(taraMgAMtara badalisi kaLuhisida gupatx sudidx \mo vanunx) sariyAgi vAyxKAyxna mADuvava, atheRYsuvava. 
\emng
\eentry

\bentry
\word{unscreened} 
\pron{anfsikxrXVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parade ilalxda; parade aLavaDisada. 
\num{2} (\caci) paradeya meVle toVrisada; parxdashiRtavAgada. 
\num{3} (\kanmu\ kalilxdadxlina \vi) (jalalxDi hiDidu) shoVdhisada, keVrada, tUrada. 
\num{4} (\rUpa) (janaru, vasutxgaLu, vicAragaLu, \mo vugaLa \vi) joVpAnavAgi Arisada; Arisi tegeyada yA tegedu hAkada; parigaNisi shoVdhisi Ayekx mADada. 
\num{5} (\kanmu\ Badarxte yA veYdayxkiVya daqSiTxyiMda) shoVdhane yA pariVkeSxge oLagAgada. 
\enum
\emng
\eentry

\bentry
\word{unscrew} 
\pron{anfsUkxrX} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} tirupu moLe kaLaci (EnanAnxdarU) tege(\akirx\ saha). 
\alnum{b} tirupumoLeyaMte (EnanAnxdarU) tirugisi tege(\akirx\ saha). 
\eanum
\numie
\num{2} (tirupu moLe) kaLacu; kaLaci tege. 
\enum
\emng
\eentry

\bentry
\word{unscripted} 
\pron{anfsikxrXpiTxDf} 
\gl{\gu}
\expl{}
\bmng
(BASaNa \mo vugaLa \vi) baredu sidadhxpaDisade mADida; hasatxparxtiya neravilalxde mADida. 
\emng
\eentry

\bentry
\word{unscriptural} 
\pron{anfsikxrXpacxralf, anfsikxrXpucxaralf} 
\gl{\gu}
\expl{}
\bmng
beYbalilxganusAravAgilalxda; shurxtisamamxtavalalxda; dhamaRgarxMthadalilxlalxda. 
\emng
\eentry

\bentry
\word{unscripturally} 
\pron{anfsikxrXpacxrali} 
\gl{\kirxvi}
\expl{}
\bmng
beYbalilxge virudadhxvAgi; shurxtisamamxtavalalxda riVtiyalilx. 
\emng
\eentry

\bentry
\word{unscrupulous} 
\pron{anfsUkxrXpuyxlasf} 
\gl{\gu}
\expl{}
\bmng
yAvudakUkx heVsada; nilaRjejxya; niVti niSeThxgaLilalxda. 
\emng
\eentry

\bentry
\word{unscrupulously} 
\pron{anfsUkxrXpuyxlasfli} 
\gl{\kirxvi}
\expl{}
\bmng
yAvudakUkx heVsade; nilaRjejxyiMda; niVti niSeThxgaLilalxde. 
\emng
\eentry

\bentry
\word{unscrupulousness} 
\pron{anfsUkxrXpuyxlasfnisf} 
\gl{\nA}
\expl{}
\bmng
yAvudakUkx heVsadiruvudu; nilaRjejxyiMda vatiRsuvike; niVtiniSeThxgaLilalxdiruvudu. 
\emng
\eentry

\bentry
\word{unseal} 
\pron{anfsiVlf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} muderxyoDe. 
\num{2} lakoVTe oDe. 
\enum
\emng
\eentry

\bentry
\word{unsealed} 
\pron{anfsiVlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} muderxyoDeda. 
\num{2} (patarx) lakoVTe oDeda. 
\num{3} muderx hAkada; muderx otatxda. 
\enum
\emng
\eentry

\bentry
\word{unseam} 
\pron{anfsiVmf} 
\gl{\akirx}
\expl{}
\bmng
(uDupu) holige kiVLu; holige kitutxkoLuLxvaMte hari. 
\emng
\eentry

\bentry
\word{unsearchable} 
\pron{anfsacaRbflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ananevxVSaNiVya; huDukalAgada. 
\num{2} (huDukidare sikakxdaSuTx) rahasayxvAda; duranevxVSaNiVya; nigUDhavAda: \eng{the unsearchable ways of the universe} I vishavxda nigUDha riVtigaLu. 
\enum
\emng
\eentry

\bentry
\word{unsearchableness} 
\pron{anfsacaRbflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ananevxVSaNiVyate; huDukalAgadiruvike. 
\num{2} (huDukidare sikakxdaSuTx) rahasayxvAgiruvike; duranevxVSaNiVyate. 
\enum
\emng
\eentry

\bentry
\word{unsearchably} 
\pron{anfsacaRbilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} huDukalAgadaMte. 
\num{2} (huDukidare sikakxdaSuTx) rahasayxvAda riVtiyalilx; nigUDhavAgi. 
\enum
\emng
\eentry

\bentry
\word{unsearched} 
\pron{anfsacfRTx} 
\gl{\gu}
\expl{}
\bmng
huDukada; shoVdhisada; anevxVSisada. 
\emng
\eentry

\bentry
\word{unseasonable} 
\pron{anfsiVsaZnabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kAloVcitavalalxda; akAlika; AyA kAlakekx, Qutuvige sahajavalalxda: \eng{unseasonable weather} kAloVcitavalalxda have (\udA\ caLigAladalilx seke). 
\num{2} saMdaBoRVcitavalalxda; akAlika; samayoVcitavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unseasoned} 
\pron{anfsiVsaZnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vasutx) hadamADada; saMsakxrisada; mAgisada; pakavx mADada. 
\num{2} (vayxkitx ) paLagada; parisharxmavilalxda; anuBavavilalxda. 
\num{3} (havAmAna \mo vugaLige) ogagxda. 
\num{4} (AhAra) rucigoLisada; saviyAgisada; ogagxraNe yA masAle hAki rucigoLisada. 
\num{5} (\kanmu\ dAruvina \vi) baliyada; hadavAgirada. 
\num{6} rUDhiyAgirada; ogigxrada. 
\enum
\emng
\eentry

\bentry
\word{unseat} 
\pron{anfsiVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ pAliRmeMTf \mo vugaLa sadasayxnanunx) cunAvaNeyalilx soVlisu. 
\num{2} sAthxnacuyxtagoLisu; piVThadiMda taLiLx hAku. 
\num{3} (kudure) beninxMda, jiVniniMda -- etitxhAku, keLaguruLisu. 
\enum
\emng
\eentry

\bentry
\word{unseaworthy} 
\pron{anfsiVvadiR} 
\gl{\gu}
\expl{}
\bmng
(haDagina \vi) 
\bnum
\num{1} samudarxsaMcArakekx yoVgayxvalalxda; yAnayoVgayxvalalxda. 
\num{2}yAnakekx sajAjxgada, sidadhxvAgilalxda. 
\enum
\emng
\eentry

\bentry
\word{unseconded} 
\pron{anfsekaMDiDf} 
\gl{\gu}
\expl{}
\bmng
(caceRyalilx sUcane, sUcaka, \mo vara \vi) anumoVdisada; beMbalisada; puSiTxVkarisada; beMbala -- ilalxda yA paDeyada. 
\emng
\eentry

\bentry
\word{unsecured} 
\pron{anfsekuyxaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ sAla \mo vugaLa \vi) AdhAravilalxda; IDilalxda: \eng{an unsecured loan} AdhAravilalxda sAla. 
\num{2} (bAgilu) biVga hAkada; BadarxpaDisada. 
\num{3} (kUdalu) BadarxvAgi kaTaTxda. 
\enum
\emng
\eentry

\bentry
\word{unseeable} 
\pron{anfsiVabflf} 
\gl{\gu}
\expl{}
\bmng
noVDalAgada; adashaRniVya. 
\emng
\eentry

\bentry
\word{unseeded} 
\pron{anfsiVDiDf} 
\gl{\gu}
\expl{}
\bmng
(Tenisf \mo\ ATada sapxdhARLugaLa \vi) karxmAMkarahita; Ayadx sherxVNiyalilx seVrirada; karxmAMka paDeyada. 
\emng
\eentry

\bentry
\word{unseeing} 
\pron{anfsiViMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kuruDAda; kaNiNxlalxda. 
\num{2} lakaSxyXviTuTx gamanisada; ajAgarUka. 
\enum
\emng
\eentry

\bentry
\word{unseeingly} 
\pron{anfsiViMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kuruDAgi; kaNiNxlalxde. 
\num{2} lakaSxyXviTuTx gamanisade; ajAgarUkateyiMda. 
\enum
\emng
\eentry

\bentry
\word{unseemliness} 
\pron{anfsiVmflinisf} 
\gl{\nA}
\expl{}
\bmng
(mAtu, naDate, \mo vugaLa \vi) asaBayxte; yoVgayxvalalxdiruvike. 
\emng
\eentry

\bentry
\word[unseemly(1)]{unseemly} 
\pron{anfsiVmfli} 
\gl{\gu}
\expl{(\tara\ \eng{unseemlier}, \tama\ \eng{unseemliest}). }
\bmng
(mAtu, naDate, \mo vugaLa \vi) 
\banum
\alnum{a} tagada; yoVgayxvalalxda; ayukatx. 
\alnum{b} asaBayx; agwravada; mayARde miVrida. 
\eanum
\emng
\eentry

\bentry
\word[unseemly(2)]{unseemly} 
\pron{anfsiVmfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} tagada riVtiyalilx; yoVgayxvalalxdaMte. 
\num{2} ayukatxvAgi; asaBayxvAgi; agwravadiMda. 
\enum
\emng
\eentry

\bentry
\word[unseen(1)]{unseen} 
\pron{anfsiVnf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kANada; noVDada. 
\num{2} adaqshayx. 
\num{3} (BASAMtarada \vi) pUvaRparicayavilalxda; paricitavalalxda. 
\enum
\emng
\eentry

\bentry
\word[unseen(2)]{unseen} 
\pron{anfsiVnf} 
\gl{\nA}
\expl{}
\bmng
(\birx) pUvaRparicayavilalxda, modalu noVDirada BAgagaLa kaqtigaLa BASAMtara, anuvAda: \eng{Latin unseens} (pUvaR paricayavilalxda) lAyxTinf kaqtigaLa anuvAdagaLu. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{The Unseen} paraloVka. 
\emng
\eentry

\bentry
\word{unsegregated} 
\pron{anfsegirxgeVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ janAMgaviBeVda yA vaNaRBeVdaniVtige anusAravAgi) parxteyxVkisada. 
\num{2} janAMgaviBeVda niVtiyananxnusarisada; vaNaRBeVdaniVtige virudadhxvAda. 
\enum
\emng
\eentry

\bentry
\word{unselect} 
\pron{anfsilekfTx} 
\gl{\gu}
\expl{}
\bmng
Arisada; Ayekx mADada. 
\emng
\eentry

\bentry
\word{unselective} 
\pron{anfsilekiTxvf} 
\gl{\gu}
\expl{}
\bmng
Ayekxyilalxda; Ayudx tegeyada; Ayekx mADada. 
\emng
\eentry

\bentry
\word{unselfconscious} 
\pron{anfselfPxkAnaSxsf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} biMkavilalxda; (tAnu elalxra gamanakUkx pAtarx neMba) sAvxBimAnavilalxda; savxparxtiSeThx ilalxdiruvike. 
\num{2} savxciMtane mADada; yAvAgalU tananx bagegxyeV yoVcisutatx kAla kaLeyada. 
\enum
\emng
\eentry

\bentry
\word{unselfconsciously} 
\pron{anfselfPxkAnaSxsfli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} biMkavilalxde; savxparxtiSeThxyilalxdaMte. 
\num{2} savxciMtaneyilalxde; yAvAgalU tananx bagegxyeV yoVcisade. 
\enum
\emng
\eentry

\bentry
\word{unselfconsciousness} 
\pron{anfselfPxkAnaSxsfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} biMkavilalxdiruvike; savxparxtiSeThxyilalxdiruvike. 
\num{2} savxciMtaneyilalxdiruvike; yAvAgalU tananx bagegxyeV yoVcisadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unselfish} 
\pron{anfseliPxSf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nisAsxvXthaR; sAvxthaRparanalalxda; savxparxyoVjanAsakatxnalalxda. 
\num{2} paroVpakAriyAda; parahitavanunx lakiSxsuva; vishAla haqdayada. 
\enum
\emng
\eentry

\bentry
\word{unselfishly} 
\pron{anfseliPxZSfli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} nisAsxvXthaRvAgi; sAvxthaRparanalalxde. 
\num{2} paroVpakAriyAgi; parahitavanunx lakiSxsuvaMte. 
\enum
\emng
\eentry

\bentry
\word{unselfishness} 
\pron{anfseliZPxSfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nisAsxvXthaRte. 
\num{2} paroVpakArite. 
\enum
\emng
\eentry

\bentry
\word{unsensational} 
\pron{anfsenesxVSanalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BAvoVderxVkakAriyalalxda; koVlAhala ebibxsada. 
\num{2} kwtukakAriyalalxda; AshacxyaRkaravalalxda. 
\enum
\emng
\eentry

\bentry
\word{unsentimental} 
\pron{anfseMTimeMTalf} 
\gl{\gu}
\expl{}
\bmng
atiBAvukavalalxda; BAvAtireVkavilalxda. 
\emng
\eentry

\bentry
\word{unsentimentality} 
\pron{anfseMTimeMTAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} BAvoVdevxVgakAriyalalxdiruvike. 
\num{2} AshacxyaRkaravalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsentimentally} 
\pron{anfseMTimeMTali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} BAvoVdevxVgakAriyalalxde. 
\num{2} AshacxyaRkaravalalxde. 
\enum
\emng
\eentry

\bentry
\word{unseparated} 
\pron{anfsepareVTiDf} 
\gl{\gu}
\expl{}
\bmng
beVpaRDisada; parxteyxVkisada. 
\emng
\eentry

\bentry
\word{unserviceability} 
\pron{anfsaviRsabiliTi} 
\gl{\nA}
\expl{}
\bmng
anupayukatxte; kelasakekx bAradiruvike. 
\emng
\eentry

\bentry
\word{unserviceable} 
\pron{anfsaviRsabflf} 
\gl{\gu}
\expl{}
\bmng
parxyoVjanakaravalalxda; kelasakekx bArada; anupayukatx; niSapxrXyoVjaka. 
\emng
\eentry

\bentry
\word{unset} 
\pron{anfseTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sithxravalalxda. 
\num{2} neleyAgirada; neleniMtilalxda. 
\enum
\emng
\eentry

\bentry
\word{unsettle} 
\pron{anfseTflf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (vayxvasithxtavAgiruvudanunx) avayxvasethxgoLisu; galibili mADu; kadaDu; kalaku; asatxvayxsatxgoLisu; aNigeDisu. 
\num{2} citatxBarxmaNeyuMTumADu; tale keDisu; hucucxhiDisu. 
\enum
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} asatxvayxsatxgoLuLx; avayxvasethxgoLagAgu. 
\num{2} talekeDisiko; hucucxhiDisiko. 
\enum
\emng
\eentry

\bentry
\word{unsettled} 
\pron{anfseTflfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sithxravAgirada; nilalxda; nelegoLaLxda; kAlUri, taLavUri nilalxda. 
\num{2} sitxmitakekx barada; tiLiyAgada; shAMtasithxtige barada; samAdhAna sithxtige barada. 
\num{3} (inUnx) itayxthaRvAgada; tiVmARnavAgada; bage hariyada. 
\num{4} (lekakx \mo vugaLa \vi) bageharisada; pheYsalu mADada; tiVruvaLiyAgada; cukAtx Agada. 
\enum
\emng
\eentry

\bentry
\word{unsettledness} 
\pron{anfseTflfDxnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} neleyAgi nilalxdiruvike. 
\num{2} itayxthaRvAgadiruvike; bagehariyadiruvike. 
\num{3} (lekakx) cukAtx Agadiruvike; pheYsalu mADadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsettlement} 
\pron{anfseTflfmaMTf} 
\gl{\nA}
\expl{}
\bmng
avayxvasethxgoLisuvike; asatxvayxsatxgoLisuvike. 
\emng
\eentry

\bentry
\word{unsewn} 
\pron{anfsoVnf} 
\gl{\gu}
\expl{}
\bmng
holiyada. 
\emng
\eentry

\bentry
\wordnospeech{unsewn binding}{unsewn binding}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\hyperref{kandict_p.pdf}{P}{perfect(1)}{$^1$perfect} (\pagu\ \eng{1}) noVDi. 
\emng
\eentry

\bentry
\word{unsex} 
\pron{anfsekfsx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (heMgasina) sitxrXVtavx kaLe; heNuNxtana yA sitxrXVlakaSxNagaLanunx kaLe. 
\num{2}(gaMDasina) puMsatxvX kaLe; puruSatavx, puruSana guNalakaSxNagaLu -- ilalxdaMte mADu. 
\num{3} (vayxkitxya) leYMgika shakitxharaNa mADu; leYMgikashakitx ilalxdaMte mADu. 
\enum
\emng
\eentry

\bentry
\word{unsexed} 
\pron{anfsekfsxTf} 
\gl{\gu}
\expl{}
\bmng
(gaMDasina yA heMgasina \vi) (savx)leYMgika lakaSxNagaLilalxda. 
\emng
\eentry

\bentry
\word{unshackle} 
\pron{anfSAYxkflf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} saMkoVle kaLacu; (baMdhanadiMda) vimukatxgoLisu. 
\num{2} (\kanmu\ saMBASaNe \mo vugaLa \vi) mukatxvAgisu; nibaRMdhavilalxvAgisu. 
\enum
\emng
\eentry

\bentry
\word{unshaded} 
\pron{anfSeVDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} maremADada; mareyiMda mucacxda, kavisada. 
\num{2} maMku mADada; maMku kaviyada. 
\num{3} neraLu mADada; beLakiniMda maremADada. 
\num{4} neraLilalxda. 
\enum
\emng
\eentry

\bentry
\word{unshakeability} 
\pron{anfSeVkabiliTi} 
\gl{\nA}
\expl{}
\bmng
alAlxDisalAgadiruvike; daqDhavAgiruvike; niSakxMpate. 
\emng
\eentry

\bentry
\word{unshakeable} 
\pron{anfSeVkabflf} 
\gl{\gu}
\expl{}
\bmng
alAlxDisalAgada; niSakxMpavAda; daqDhavAda; sithxravAda. 
\emng
\eentry

\bentry
\word{unshakeably} 
\pron{anfSeVkabilx} 
\gl{\kirxvi}
\expl{}
\bmng
alAlxDisalAgadaMte; daqDhavAgi. 
\emng
\eentry

\bentry
\word{unshaken} 
\pron{anfSeVkanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} alAlxDisada; daqDha; sithxra. 
\num{2} adhiVranAgada; gAbariyAgada; galibiligoLaLxda; sethxYyaR kaLedukoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{unshakenly} 
\pron{anfSeVkanilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sithxravAgi; alAlxDisalAgadaMte. 
\num{2} adhiVranAgade. 
\enum
\emng
\eentry

\bentry
\word{unshapeliness} 
\pron{anfSeVpflinisf} 
\gl{\nA}
\expl{}
\bmng
kurUpa; aMdavAda meYkaTiTxlalxdiruvike; rUpavilalxdiruvike. 
\emng
\eentry

\bentry
\word{unshapely} 
\pron{anfSeVpfli} 
\gl{\gu}
\expl{}
\bmng
aMdavAda meYkaTiTxlalxda; rUpavilalxda; AkAra swSaThxvavilalxda. 
\emng
\eentry

\bentry
\word{unshared} 
\pron{anfSeVDfR} 
\gl{\gu}
\expl{}
\bmng
(inonxbabxroDane) haMcikoLaLxda; viBAga mADikoLaLxda; pAlugoLaLxda; BAgiyAgada. 
\emng
\eentry

\bentry
\word{unsharp} 
\pron{anfSApfR} 
\gl{\gu}
\expl{}
\bmng
(\CA) asapxSaTx; asuPxTa; Akaqti -- sapxSaTxvilalxda, suPxTavAgirada. 
\emng
\eentry

\bentry
\word{unsharpness} 
\pron{anfSApfRnisf} 
\gl{\nA}
\expl{}
\bmng
(\CA) asapxSaTxte; asuPxTate. 
\emng
\eentry

\bentry
\word{unshaved} 
\pron{anfSeVvfDx} 
\gl{\gu}
\expl{}
\bmng
kwSxravilalxda; kwSxra mADikoLaLxda. 
\emng
\eentry

\bentry
\word{unshaven} 
\pron{anfSeVvanf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unshaved}{unshaved}. 
\emng
\eentry

\bentry
\word{unsheathe} 
\pron{anfSiVdf} 
\gl{\sakirx}
\expl{}
\bmng
(katitx \mo vanunx) oreyiMda horakekx tege; oreyiMdAcege eLe. 
\emng
\eentry

\bentry
\word{unshed} 
\pron{anfSeDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} biVLisada; udurisada. 
\num{2} toreyada; tayxjisada. 
\num{3} harisada; surisada; basiyada. 
\num{4} (beLaku \mo vanunx) biVrada; haraDada; parxsarisada. 
\num{5} (kaNiNxVru) surisada; toTiTxkikxsada. 
\enum
\emng
\eentry

\bentry
\word{unshell} 
\pron{anfSelf} 
\gl{\sakirx}
\expl{}
\bmng
(cipupx, koVsha, hoTuTx, \mo vugaLanunx) tegeyadiru; suliyade hAgeV biDu. 
\emng
\eentry

\bentry
\word{unshelled} 
\pron{anfSelfDx} 
\gl{\gu}
\expl{}
\bmng
(sipepx \mo vanunx) tegedirada; sulidirada. 
\emng
\eentry

\bentry
\word{unsheltered} 
\pron{anfSelaTxDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} Asharxya, rakaSxNe, \mo vu ilalxda; nirAshirxta. 
\num{2} (doVSAroVpadiMda) rakiSxsada; kApADada. 
\enum
\emng
\eentry

\bentry
\word{unshielded} 
\pron{anfSiVliDxDf} 
\gl{\gu}
\expl{}
\bmng
rakiSxtavAgilalxda; kApADilalxda. 
\emng
\eentry

\bentry
\word{unship} 
\pron{anfSipf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unshipped}; \vakaq\ \eng{unshipping}). }
\bmng
\bnum
\num{1} (saraku, parxyANikaru, \mo vanunx) haDaginiMda -- iLisu yA tege. 
\num{2} (\kanmu\ nAve, kUvekaMba, huTuTxgoVlu, \mo vanunx) tege; kaLacu. 
\enum
\emng
\eentry

\bentry
\word{unshockable} 
\pron{anfSAkabflf} 
\gl{\gu}
\expl{}
\bmng
(koVpa, Baya, \mo vugaLiMda) AGAta mADalAgada; talalxNagoLisalAgada; asahayx, asamAdhAna huTiTxsalAgada. 
\emng
\eentry

\bentry
\word{unshod} 
\pron{anfSADf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} joVDu hAkikoLaLxda. 
\num{2} (kudure \mo vugaLa \vi) lALa hoDeyada. 
\num{3} (kaMba \mo vugaLa \vi) taLarakeSx hAkada; baLe, aNasu, \mo vanunx odagisada. 
\enum
\emng
\eentry

\bentry
\word{unshoe} 
\pron{anfSU} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} joVDu kaLacu, tege. 
\num{2} (pAdada yA buDada taLarakeSxgAgi hAkida) lALa, baLe, aNasu, \mo vanunx kaLacu. 
\enum
\emng
\eentry

\bentry
\word{unshorn} 
\pron{anfSAnfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaDiyada; katatxrisada; koyayxda. 
\num{2} kaLedukoLaLxda. 
\num{3} (tale gaDaDxgaLa kUdalu, kuriya tupapxTa, \mo vanunx) katatxrisada; tegeyada. 
\enum
\emng
\eentry

\bentry
\word{unshrinkability} 
\pron{anfSirxMkabiliTi} 
\gl{\nA}
\expl{}
\bmng
kugigxsalAgadiruvike; asaMkoVcayxte. 
\emng
\eentry

\bentry
\word{unshrinkable} 
\pron{anfSirxMkabflf} 
\gl{\gu}
\expl{}
\bmng
(baTeTx \mo vugaLa \vi) kugigxsalAgada; asaMkoVcayx; sukikxsalAgada; mudurisalAgada; suruTisalAgada. 
\emng
\eentry

\bentry
\word{unshrinking} 
\pron{anfSirxMkiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kugagxda; kaDameyAgada; saMkoVcagoLaLxda. 
\num{2} sukakxda; mudurada; suruTada. 
\num{3} himemxTaTxda; hiMjariyada. 
\num{4} aLukada; heVsada. 
\enum
\emng
\eentry

\bentry
\word{unshrinkingly} 
\pron{anfSirxMkiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kugagxdaMte; kaDameyAgadaMte. 
\num{2} sukakxdaMte; muduradaMte. 
\num{3} himemxTaTxde; hiMjariyade. 
\enum
\emng
\eentry

\bentry
\word{unsighted} 
\pron{anfseYTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kANada; noVDada; kaNiNxge biVLada; daqSiTxge biVLada; goVcarisada. 
\num{2} (\kanmu\ taDeyoMdanunx nimiRsi) kANisadaMte mADida; daqSiTxge biVLuvudanunx tapipxsida. 
\enum
\emng
\eentry

\bentry
\word{unsightliness} 
\pron{anfseYTflinisf} 
\gl{\nA}
\expl{}
\bmng
noVDalu aMdavilalxdAgiruvudu; kurUpa, avalakaSxNa, vikAra -- Agiruvudu. 
\emng
\eentry

\bentry
\word{unsightly} 
\pron{anfseYTfli} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} noVDalu aMdavAgirada, asahayxvAda: \eng{unsightly advertisements on the roadside} rasetxbadiyalilxna asahayxvAda jAhiVrAtugaLu. 
\num{2} kurUpavAda; avalakaSxNada; vikArada: \eng{an unsightly wound} vikAravAda gAya. 
\enum
\emng
\eentry

\bentry
\word{unsigned} 
\pron{anfseYnfDx} 
\gl{\gu}
\expl{}
\bmng
rujumADada; dasakxtitxlalxda; sahi mADada. 
\emng
\eentry

\bentry
\word{unsinkability} 
\pron{anfsiMkabiliTi} 
\gl{\nA}
\expl{}
\bmng
muLugisalAgadiruvike. 
\emng
\eentry

\bentry
\word{unsinkable} 
\pron{anfsiMkabflf} 
\gl{\gu}
\expl{}
\bmng
muLugisalAgada. 
\emng
\eentry

\bentry
\word[unsized(1)]{unsized} 
\pron{anfseYsZfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} aLatege sariyAgilalxda; aLatege sariyAgi mADada. 
\num{2} aLatege sariyAgi joVDisada; aLateya parxkAra viMgaDisada. 
\enum
\emng
\eentry

\bentry
\word[unsized(2)]{unsized} 
\pron{anfseYsZfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (kAgadakekx hoLapu koDuva) `jileTinf' aMTu, gaMji, vajarxsari, \mo vanunx baLidu saMsakxrisada. 
\num{2} (baTeTxya \vi) gaMji hAki gari mADada, gAru mADada. 
\enum
\emng
\eentry

\bentry
\word{unskilful} 
\pron{anfsikxlfPuZlf} 
\gl{\gu}
\expl{}
\bmng
(\ame\ \eng{unskillful}). jANemxyilalxda; keYcaLakavilalxda; cAtuyaRvilalxda; kwshala, neYpuNayx -- rahita. 
\emng
\eentry

\bentry
\word{unskilfully} 
\pron{anfsikxlfPuZli} 
\gl{\kirxvi}
\expl{}
\bmng
cAtuyaRvilalxde; jANemxyilalxde. 
\emng
\eentry

\bentry
\word{unskilfulness} 
\pron{anfsikxlfPuZlfnisf} 
\gl{\nA}
\expl{}
\bmng
jANemxyilalxdiruvike. 
\emng
\eentry

\bentry
\word{unskilled} 
\pron{anfsikxlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (oMdu kelasakekx beVkAda) jANatanavilalxda; jANemxyilalxda; budidhxvaMtikeyilalxda; kwshalavilalxda; nipuNate ilalxda. 
\num{2} tarapeVtu ilalxda; anuBavavilalxda; nuritilalxda; vishiSaTx parisharxmavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unskimmed} 
\pron{anfsikxmfDx} 
\gl{\gu}
\expl{}
\bmng
(hAlu \mo vugaLa \vi) kene tegeyada. 
\emng
\eentry

\bentry
\word{unslakeable} 
\pron{anfselxVkabflf} 
\gl{\gu}
\expl{}
\bmng
(\eng{unslakable} eMdU \parx) (bAyArike \mo vanunx) aDagisalAgada; pariharisalAgada. 
\emng
\eentry

\bentry
\word{unsleeping} 
\pron{anfsilxVpiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nidirxsada; niderxbarada. 
\num{2} nidedx mADada; sadA jAgarxtanAgiruva; ecacxraviruva. 
\enum
\emng
\eentry

\bentry
\word{unsleepingly} 
\pron{anfsilxVpiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
nidirxsade; niderx mADadaMte. 
\emng
\eentry

\bentry
\word{unsliced} 
\pron{anfselxYsfTx} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ berxDiDxna gaTiTxya \vi, koMDAga) tuMDu mADirada; tuMDugaLAgi katatxrisada. 
\emng
\eentry

\bentry
\word{unsling} 
\pron{anfsilxMgf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unslung}). }
\bmng
\bnum
\num{1} (neVtuhAkiruvudanunx) keLakikxLisu; iLiyabiDu: \eng{unsling a rifle from one's shoulder} hegala meVle neVtuhAkikoMDa baMdUka keLakikxLisu. 
\num{2} (\nw) tUgAsareyiMda iLisu; (doVNi, sAgisabeVkAda sAmAnu, \mo vanunx tUguhAkida) sarapaNi, hagagx, \mo vugaLiMda iLisu. 
\enum
\emng
\eentry

\bentry
\word{unsmiling} 
\pron{anfsemxYliMgf} 
\gl{\gu}
\expl{}
\bmng
nagada; nage ilalxda; nagemuKavilalxda; aparxsananx. 
\emng
\eentry

\bentry
\word{unsmilingly} 
\pron{anfsemxYliMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
nagade; nage ilalxde. 
\emng
\eentry

\bentry
\word{unsmilingness} 
\pron{anfsemxYliMgfnisf} 
\gl{\nA}
\expl{}
\bmng
nagadiruvike; nage ilalxdiruvike. 
\emng
\eentry

\bentry
\word{unsmoked} 
\pron{anfsomxVkfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (AhAra \mo vu keDadaMte) hoge hiDisada; hogeyADisada; hogeyADisi saMsakxrisada. 
\num{2} hogeyalilxDada; hogeyADisi kapupx tirugisada. 
\num{3} seVdi mugisirada, kuDiyada: \eng{an unsmoked cigar} seVdi mugisirada cuTATx. 
\enum
\emng
\eentry

\bentry
\word{unsnarl} 
\pron{anfsAnxlfR} 
\gl{\sakirx}
\expl{}
\bmng
(sikikxkoMDiruvudanunx) biDisu; goVju biDisi tege. 
\emng
\eentry

\bentry
\word{unsociability} 
\pron{anfsoVSabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (obabxra jate) seVradiruvike; bereyadiruvike; saMgadevxVSa. 
\num{2} senxVha swhAdaRgaLilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsociable} 
\pron{anfsoVSabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (obabxra jateyalilx) seVrada; bereyada; sahavAsashiVlanalalxda; saMgapirxyanalalxda. 
\num{2} senxVhaswhAdaRvilalxda; meYtirxBAvavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unsociableness} 
\pron{anfsoVSabflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} janaroDane bereyadiruvike. 
\num{2} senxVhaswhAdaRgaLilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsociably} 
\pron{anfsoVSabilx}
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} janaroDane bereyadaMte. 
\num{2} senxVha swhAdaRgaLilalxde. 
\enum
\emng
\eentry

\bentry
\word{unsocial} 
\pron{anfsoVSa(Sf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oTATxgi, guMpAgi vAsamADada; saMGajiVviyalalxda. 
\num{2} saMGajiVvana bayasada; saMGajiVvanakekx -hoMdada, ogagxda; samudAya jiVvanavanunx anusarisada. 
\num{3} samAjakaMTaka; samAja viroVdhi; samAjaGAtuka. 
\num{4} sAmAnayx kelasada dinadiMda horagina. 
\enum
\emng
\eentry

\bentry
\word{unsocially} 
\pron{anfsoVSali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} saMGajiVviyalalxdaMte. 
\num{2} saMGajiVvana bayasadaMte. 
\num{3} samAjaGAtukavAgi. 
\enum
\emng
\eentry

\bentry
\word{unsoiled} 
\pron{anfsAilfDx} 
\gl{\gu}
\expl{}
\bmng
koLeyAgada; malinavAgada; kaLaMkavilalxda. 
\emng
\eentry

\bentry
\word{unsold} 
\pron{anfsoVlfDx} 
\gl{\gu}
\expl{}
\bmng
mArATavAgada; vAyxpAravAgada. 
\emng
\eentry

\bentry
\word{unsolder} 
\pron{anfsoVlaDxrf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} besuge hAkiruvudanunx biDisu, beVpaRDisu. 
\num{2} beVpaRDisu; parxteyxVkisu. 
\enum
\emng
\eentry

\bentry
\word{unsoldierly} 
\pron{anfsoVlajxliR} 
\gl{\gu}
\expl{}
\bmng
yoVdhanige -- sahajavalalxda, takukxdalalxda. 
\emng
\eentry

\bentry
\word{unsolicited} 
\pron{anfsalisiTiDf} 
\gl{\gu}
\expl{}
\bmng
koVrada; bayasada; beVDada; apeVkiSxsada; apArxthiRta; AyAcita. 
\emng
\eentry

\bentry
\word{unsolicitedly} 
\pron{anfsalisiTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
koVrade; beVDade; ayAcitavAgi; apArxthiRtavAgi. 
\emng
\eentry

\bentry
\word{unsolvability} 
\pron{anfsAlavxbiliTi} 
\gl{\nA}
\expl{}
\bmng
biDisalAgadiruvike; pariharisalAgadiruvike; aparihAyaRte. 
\emng
\eentry

\bentry
\word{unsolvable} 
\pron{anfsAlavxba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
biDisalAgada; pariharisalAgada; bageharisalAgada; aparihAyaR. 
\emng
\eentry

\bentry
\word{unsolvableness} 
\pron{anfsAlavxbflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unsolvability}{unsolvability}. 
\emng
\eentry

\bentry
\word{unsolved} 
\pron{anfsAlfvxDf} 
\gl{\gu}
\expl{}
\bmng
biDisada; pariharisada; bageharisada; aparihaqta. 
\emng
\eentry

\bentry
\word{unsophisticated} 
\pron{anfsaphisiTxkeVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saraLateyanunx hALumADada; neYjate keDisada. 
\num{2} saraLa; neYja; akaqtaka; niSakxpaTa; QujuvAda; amAyakavAda. 
\num{3} apapxTa; shudadhx; berakeyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unsophisticatedly} 
\pron{anfsaphisiTxkeVTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} saraLavAgi; neYjateyiMda. 
\num{2} niSakxpaTa riVtiyalilx; amAyakavAgi. 
\enum
\emng
\eentry

\bentry
\word{unsophisticatedness} 
\pron{anfsaphisiTxkeVTiDfnisf} 
\gl{\nA}
\expl{}
\bmng
saraLate(yiMdiruvudu); neYjate; niSAkxpaTayx; amAyakate; Qujute. 
\emng
\eentry

\bentry
\word{unsophistication} 
\pron{anfsaphisiTxkeVSanf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unsophisticatedness}{unsophisticatedness}. 
\emng
\eentry

\bentry
\word{unsordid} 
\pron{anfsADiRDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kuSxdarxvalalxda; tucaCxvalalxda. 
\num{2} koLakilalxda; holasilalxda. 
\enum
\emng
\eentry

\bentry
\word{unsorted} 
\pron{anfsATiRDf} 
\gl{\gu}
\expl{}
\bmng
Ayudx, vagiRVkarisi -- joVDisada. 
\emng
\eentry

\bentry
\word{unsought} 
\pron{anfsATf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} huDukada; arasada; kANalu, paDeyalu -- parxyatinxsada. 
\num{2} koVrada; apeVkiSxsada. 
\enum
\emng
\eentry

\bentry
\word{unsound} 
\pron{anfswMDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} susithxtiyalilxrada; sariyAgilalxda. 
\num{2} asavxsathx; AroVgayxkara sithxtiyalilxlalxda. 
\num{3} keTuTxhoVgiruva; koLetu hoVda. 
\num{4} buDa Badarxvilalxda; daqDhavAda taLahadiyilalxda. 
\num{5} takaRbadadhxvalalxda; asamaMjasavAda; doVSadiMda kUDida: \eng{an unsound argument} takaRbadadhxvalalxda vAda. 
\num{6} haguravAda; gADhavalalxda; joVralalxda: \eng{unsound slumber} haguravAda niderx. 
\num{7} (AthiRkavAgi) gaTiTxyAgirada; buDaBadarxvalalxda; avishAvxsaniVya; necacxlAgada: \eng{an unsound corporation} necacxlahaRvalalxda saMsethx. 
\num{8} duSaTx; niVca; keTaTx. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{of unsound mind} mAnasika asavxsathxteyalilxruva; manoVroVgakekx tutAtxgiruva; budidhxBarxmaNe yAgiruva. 
\emng
\eentry

\bentry
\word{unsounded} 
\pron{anfswMDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vAdayxda \vi) Udada; bArisada; nuDisada; moLagada. 
\num{2} (ALada guMDina mUlaka) ALa kaMDuhiDiyada; taLa pariVkiSxsada. 
\num{3} manoVgatavariyada; iMgita tiLiyada; BAvavanunx pariVkiSxsada; miDidu noVDada. 
\num{4} (akaSxrada \vi) ucacxrisada. 
\enum
\emng
\eentry

\bentry
\word{unsoundly} 
\pron{anfswMDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} oLeLxya sithxtiyalilxlalxde. 
\num{2} asavxsathx sithxtiyalilx. 
\num{3} keTuTx hoVgi; koLetu. 
\num{4} buDa Badarxvilalxde; daqDhavAda taLahadi ilalxde. 
\num{5} atAkiRkavAgi; doVSapUritavAgi; asamaMjasavAgi. 
\num{6} haguravAgi; gADhavalalxda riVtiyalilx. 
\num{7} necacxlAgadaMte. 
\enum
\emng
\eentry

\bentry
\word{unsoundness} 
\pron{anfswMDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oLeLxya sithxtiyalilxlalxdiruvudu. 
\num{2} asavxsathxte; anAroVgayx sithxti. 
\num{3} keTuTxhoVgiruvudu; koLetu hoVgiruvike. 
\num{4} BadarxvAda bunAdi ilalxdiruvudu. 
\num{5} (AthiRkavAgi) necacxlAgada sithxti; AthiRka duravasethx. 
\enum
\emng
\eentry

\bentry
\word{unsoured} 
\pron{anfswaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} huLiyAgada; huLi hiDiyada. 
\num{2} (\rUpa) manasusx huLiyAgada. 
\enum
\emng
\eentry

\bentry
\word{unsown} 
\pron{anfsoVnf} 
\gl{\gu}
\expl{}
\bmng
bitatxda; biVja celalxda; biVja eracada; bitatxne mADada. 
\emng
\eentry

\bentry
\word{unsparing} 
\pron{anfsepxVriMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vipula; puSakxla; taDeyilalxde koDuva; dhArALavAgi utapxtitx mADuva; koDugeYyina: \eng{unsparing of praise} dhArALavAda parxshaMseya. 
\num{2} niSakxruNeya; daye dAkiSxNayx toVrada. 
\enum
\emng
\eentry

\bentry
\word{unsparingly} 
\pron{anfsepxVriMgfli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} vipulavAgi; puSakxlavAgi. 
\num{2} niSakxruNeyiMda; daye dAkiSxNayx toVrade. 
\enum
\emng
\eentry

\bentry
\word{unsparingness} 
\pron{anfsepxVriMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vipulate; puSakxlate. 
\num{2} niSakxruNe; daye dAkiSxNayx ilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unspeakable} 
\pron{anfsipxVkaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mAtinalilx vaNiRsalAgada; anivaRcaniVya; avaNaRniVya: \eng{unspeakable joy} anivaRcaniVyavAda AnaMda. 
\num{2} heVLalAgadaSuTx keTaTx, holasAda, AkeSxVpaNiVyavAda: \eng{unspeakable wickedness} vaNaRnAtiVtavAda keTaTxtana. 
\enum
\emng
\eentry

\bentry
\word{unspeakableness} 
\pron{anfsipxVkabflfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} mAtinalilx baNiNxsalAgadiruvike; anivaRcaniVyate. 
\num{2} heVLalAgadaSuTx keTaTxdAgiruvike, holasAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{unspeakably} 
\pron{anfsipxVkabilx} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} mAtinalilx baNiNxsalAgadaMte; anivaRcaniVyavAgi. 
\num{2} heVLalAgadaSuTx keTaTxdAgi, holasAgi. 
\enum
\emng
\eentry

\bentry
\word{unspecialized} 
\pron{anfsepxSaleYsfZDx} 
\gl{\gu}
\expl{}
\bmng
visheVSa adhayxyana mADada; tajacnxteyilalxda. 
\emng
\eentry

\bentry
\word{unspecified} 
\pron{anfsepxsipheYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sapxSaTxvAgi, nidiRSaTxvAgi heVLada. 
\num{2} vivara, tapasiVlu -- namUdisada; tapashiVlu paTiTxyalilx seVrisada, kANisada. 
\enum
\emng
\eentry

\bentry
\word{unspectacular} 
\pron{anfsepxkATxYxkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
perxVkaSxNiVyavalalxda; AkaSaRkavalalxda; niVrasavAda. 
\emng
\eentry

\bentry
\word{unspectacularly} 
\pron{anfsepxkATxYxkuyxlarfli} 
\gl{\kirxvi}
\expl{}
\bmng
perxVkaSxNiVyavAgilalxdaMte; AkaSaRNiVyavAgiradaMte; niVrasavAgi. 
\emng
\eentry

\bentry
\word{unspell} 
\pron{anfsepxlf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} shApavanunx kaLe, pariharisu; maMtarxshakitxyanunx nivArisu. 
\num{2} shApadiMda biDisu; shApada parxBAvadiMda mukatxgoLisu. 
\enum
\emng
\eentry

\bentry
\word{unspent} 
\pron{anfsepxMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} KacuRmADada, upayoVgisada; baLasada; KacARgada. 
\num{2} baLasi mugisada; upayoVgisi KAli mADada: \eng{unspent energy} baLasada shakitx. 
\enum
\emng
\eentry

\bentry
\word{unspilled} 
\pron{anfsipxlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} soVrada; celalxda. 
\num{2} poVlAgada; vayxthaRmADada. 
\enum
\emng
\eentry

\bentry
\word{unspilt} 
\pron{anfsipxlfTx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unspilled}{unspilled}. 
\emng
\eentry

\bentry
\word{unspiritual} 
\pron{anfsipxriTuyxalf} 
\gl{\gu}
\expl{}
\bmng
AdhAyxtimxkavalalxda; anAdhAyxtimxka. 
\emng
\eentry

\bentry
\word{unspirituality} 
\pron{anfsipxriTuyxAYxliTi} 
\gl{\nA}
\expl{}
\bmng
AdhAyxtimxkavalalxdiruvike. 
\emng
\eentry

\bentry
\word{unspiritually} 
\pron{anfsipxriTuyxali} 
\gl{\kirxvi}
\expl{}
\bmng
anAdhAyxtimxkateyiMda; AdhAyxtimxkavAgiradaMte. 
\emng
\eentry

\bentry
\word{unspiritualness} 
\pron{anfsipxriTuyxalfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unspirituality}{unspirituality}. 
\emng
\eentry

\bentry
\word{unspliced} 
\pron{anfsepxlXYsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (hagagxda \vi) hosedu kUDisada. 
\num{2} (marada tuMDu \mo vu) paDa kUDisada; malaku hAkada. 
\enum
\emng
\eentry

\bentry
\word{unsplinterable} 
\pron{anfsipxlXnaTxraba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
sibiru, cUru -- mADalAgada; cakekx -ebibxsalAgada, siVLalAgada, mADalAgada. 
\emng
\eentry

\bentry
\word{unspoiled} 
\pron{anfsApxilfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keDisada; hALu mADada. 
\num{2} keDada; hALAgada. 
\enum
\emng
\eentry

\bentry
\word{unspoilt} 
\pron{anfsApxilfTx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unspoiled}{unspoiled}. 
\emng
\eentry

\bentry
\word{unspoken} 
\pron{anfsopxVkanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sUcayxvAda; (mAtanADade) iMgita sUcisida. 
\num{2} (obabxnanunx) kuritu heVLada; saMboVdhisi heVLada; anukatx. 
\num{3} mAtanADada; mwnada. 
\enum
\emng
\eentry

\bentry
\word{unsporting} 
\pron{anfsopxVTiRMgf} 
\gl{\gu}
\expl{}
\bmng
ATagAranige takukxdalalxda; kirxVDAmanoVBAvakekx sahajavalalxda; udAravalalxda. 
\emng
\eentry

\bentry
\word{unsportingly} 
\pron{anfsopxVTiRMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
kirxVDAmanoVBAvavilalxdaMte; udAravAgilalxde. 
\emng
\eentry

\bentry
\word{unsportingness} 
\pron{anfsopxVTiRMgfnisf} 
\gl{\nA}
\expl{}
\bmng
kirxVDAmanoVBAvavilalxdiruvike; anudArate. 
\emng
\eentry

\bentry
\word{unsportsmanlike} 
\pron{anfsopxVTfRsxmanfleYkf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unsporting}{unsporting}. 
\emng
\eentry

\bentry
\word{unspotted} 
\pron{anfsApxTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} cukekxgaLilalxda; cukekx gurutilalxda; cukekx gurutu mADada. 
\num{2} niSakxLaMka; doVSarahita; niduRSaTx. 
\num{3} gurutisada; gamana seLeyada; yAra gamanakUkx bArada. 
\enum
\emng
\eentry

\bentry
\word{unsprung} 
\pron{anfsapxrXMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sipxrXMgf odagisada, hAkada; cimamxda; hArada; puTiyada. 
\num{2} udaBxvisada; huTaTxda; moLeyada; kANisikoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{unstable} 
\pron{anfseTxVba(bf)lf} 
\gl{\gu}
\expl{(\tara\ \eng{unstabler}, \tama\ \eng{unstablest}).}
\bmng
\bnum
\num{1} alugADuva; Badarxvilalxda; asithxra; daqDhavalalxda. 
\num{2} anishicxta; badalAyisuva; caMcalavAda. 
\num{3} manoVnishacxyavilalxda; manoVdADhaRyxvilalxda. 
\num{4} calaneyalilx karxmavilalxda; avayxvasithxta calaneya. 
\num{5} (\ravi) asithxra; sithxravalalxda. 
\enum
\emng
\eentry

\bentry
\wordnospeech{unstable equilibrium}{unstable equilibrium} 
\pron{?}
\gl{\nA}
\expl{}
\bmng
asithxra samatoVlana; kadalikeya PalavAgi kAyavu samatoVladiMda dUra sariyuvaMtiruva samatoVla sithxti. 
\emng
\eentry

\bentry
\word{unstableness} 
\pron{anfseTxVbalfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} alugATa; asithxrate. 
\num{2} anishicxtate. 
\num{3} manoVnishacxyavilalxdiruvike. 
\num{4} avayxvasethx. 
\enum
\emng
\eentry

\bentry
\word{unstably} 
\pron{anfseTxVbilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} alugADutatx; balavAgi nilalxde; aBadarx sithxtiyalilx; asithxravAgi; daqDhavAgirade. 
\num{2} anishicxtavAgi; caMcalavAgi. 
\num{3}manoVnishacxyavilalxde; manoVdADhaRyxvilalxde. 
\num{4} karxmavilalxda riVtiyalilx; avayxvasithxtavAgi. 
\enum
\emng
\eentry

\bentry
\word{unstained} 
\pron{anfseTxVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaleyAgada; cukekxyilalxda. 
\num{2} (\rUpa) niSakxLaMka; doVSarahita; niduRSaTx. 
\enum
\emng
\eentry

\bentry
\word{unstamped} 
\pron{anfsATxYxMpfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (loVha, kAgada, \mo vugaLa meVle hesaru, gurutu \mo vanunx) muderxyotatxda; Thasesx hAkada; acocxtatxda; CApisada. 
\num{2} (lakoVTe, dasetxYvaju) aMceciVTi hacacxda; taleciVTi aMTisada; sATxMpu hAkada. 
\enum
\emng
\eentry

\bentry
\word{unstarched} 
\pron{anfsATxcfRTx} 
\gl{\gu}
\expl{}
\bmng
gaMji hAkada; gaMji hAki gaDasu mADada, garimuri mADada. 
\emng
\eentry

\bentry
\word{unstated} 
\pron{anfseTxVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (mAtinalilx yA barahadalilx pUtiRyAgi, sapxSaTxvAgi) heVLada; tiLisada; anukatx; vayxkatxpaDisada; namUdisada. 
\num{2} (tAriVKu \mo vanunx) gotutxpaDisada; nideRVshisada. 
\enum
\emng
\eentry

\bentry
\word{unstatesmanlike} 
\pron{anfseTxVTfsxmanfleYkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} rAjaniVtijacnxnige -- takukxdalalxda, yoVgayxvalalxda, sahajavalalxda. 
\num{2} rAjaniVtijacnxna -- riVtiyadalalxda, dUradaqSiTx ilalxda, kAyaRshiVlate ilalxda. 
\enum
\emng
\eentry

\bentry
\word{unstatutable} 
\pron{anfsATxYxTuyx(cu)Taba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
shAsanabadadhxvalalxda; kAyide anusarisada. 
\emng
\eentry

\bentry
\word{unstatutably} 
\pron{anfsATxYxTuyx(cu)Tabilx} 
\gl{\kirxvi}
\expl{}
\bmng
shAsanabadadhxvalalxda riVtiyalilx; kAyidege anusAravAgirade. 
\emng
\eentry

\bentry
\word{unsteadfast} 
\pron{anfseTxDfphAsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} asithxravAda; daqDhavilalxda; anishicxtavAda. 
\num{2} daqDhaniSeThxyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unsteadily} 
\pron{anfseTxDili} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} BadarxvAgi nilalxde; alAlxDutatx; tatatxrisutatx. 
\num{2} asithxravAgi; caMcalavAgi. 
\num{3} (idadxkikxdadxMte) manasusx caMcalagoMDu, badalAyisi; sithxra manasisxlalxde. 
\num{4} oMdeV riVtiyAgirade; EkaparxkAravAgirade. 
\enum
\emng
\eentry

\bentry
\word{unsteadiness} 
\pron{anfseTxDinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} BadarxvAgi nilalxda sithxti; tatatxrisuvudu. 
\num{2} asithxrate; caMcalate. 
\num{3} (manasisxna) caMcala sithxti; cAMcalayx; sithxra manasisxlalxdiruvudu. 
\num{4} EkaparxkAravAgi, oMdeV samanAgi -- ilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsteady} 
\pron{anfseTxDi} 
\gl{\gu}
\expl{(\tara\ \eng{unsteadier}, \tama\ \eng{unsteadiest}).}
\bmng
\bnum
\num{1} BadarxvAgi nilalxda; alAlxDuva; tatatxrisuva. 
\num{2} asithxravAda; caMcalavAda. 
\num{3} (idadxkikxdadxMte) manasusx caMcalavAguva; badalAyisuva; sithxra manasisxlalxda. 
\num{4} EkaparxkAravAgirada; oMdeV samanAgilalxda: \eng{an unsteady development} EkaparxkAravAgilalxda beLavaNige. 
\enum
\emng
\eentry

\bentry
\word[unstick(1)]{unstick} 
\pron{anfsiTxkf} 
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unstuck}). }

\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (oMdakokxMdu aMTiruvudanunx) biDisu; parxteyxVkisu; beVpaRDisu; aMTu biDisu. 
\num{2} (\vAyA) (\AmA) (vimAnavanunx) nela biDuvaMte mADu; meVlakekx hAruvaMte mADu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
(vimAna) hAru; nelabiTuTx Eru. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{come unstuck} (\AmA) 
\banum
\alnum{a} kaSaTxkekx sikukx; duHKakekx oLagAgu; vipatitxge guriyAgu. 
\alnum{b} (parxyatanx \mo vugaLalilx) soVtu hoVgu; keLakekx biVLu. 
\eanum
\emng
\eentry

\bentry
\word[unstick(2)]{unstick} 
\pron{anfsiTxkf} 
\gl{\nA}
\expl{}
\bmng
(\vAyA) (\AmA) (vimAnavu nelabiTuTx) meVlakekxVruva kaSxNa; uDaDxyana kaSxNa. 
\emng
\eentry

\bentry
\word{unstinted} 
\pron{anfsiTxMTiDf} 
\gl{\gu}
\expl{}
\bmng
jipuNatana mADada; kApaRNayxvilalxda; keYhiDita mADada. 
\emng
\eentry

\bentry
\word{unstintedly} 
\pron{anfsiTxMTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
jipuNatana mADade; kApaRNayxvilalxde; dhArALavAgi; bicucxgeYyiMda. 
\emng
\eentry

\bentry
\word{unstinting} 
\pron{anfsiTxMTiMgf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unstinted}{unstinted}. 
\emng
\eentry

\bentry
\word{unstintingly} 
\pron{anfsiTxMTiMgfli} 
\gl{\kirxvi}
\bmng
jipuNatana mADade; keYhiDiyade; dhArALavAgi. 
\emng
\eentry

\bentry
\word{unstirred} 
\pron{anfsaTxDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} alugada; kadalada; calisada. 
\num{2} alugisada; kadalisada. 
\num{3} toLasada; kadaDada; kalakada; kedakada. 
\num{4} keraLada; perxVraNegoLaLxda; parxcoVditavAgada. 
\enum
\emng
\eentry

\bentry
\word{unstitch} 
\pron{anfsiTxcf} 
\gl{\akirx}
\expl{}
\bmng
holige bicucx. 
\emng
\eentry

\bentry
\word{unstop} 
\pron{anfsATxpf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unstopped}; \vakaq\ \eng{unstopping}).}
\bmng
\bnum
\num{1} aDiDxyAgiruvudanunx, kaTiTxruvudanunx -- biDisu, saDilagoLisu, terapu mADu, tege, tere: \eng{unstop a drain} (kaTiTxkoMDiruva) moVriyanunx terapugoLisi cokakxTa mADu. 
\num{2} biraDe, mucacxLa -- tege, tere. 
\enum
\emng
\eentry

\bentry
\word{unstoppability} 
\pron{anfsATxpabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} biraDe hAki mucacxlAgadiruvike. 
\num{2} aDiDxpaDisalAgadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unstoppable} 
\pron{anfsATxpaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} biraDe, mucacxLa hAki mucacxlAgada. 
\num{2} aDiDxpaDisalAgada; niroVdhisalAgada; taDe hAkalu sAdhayxvilalxda; taDedu nililxsalAgada. 
\enum
\emng
\eentry

\bentry
\word{unstoppably} 
\pron{anfsATxpabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} biraDe hAki mucacxlAgade. 
\num{2} aDiDxpaDisalAgade. 
\enum
\emng
\eentry

\bentry
\word{unstopper} 
\pron{anfsATxparf} 
\gl{\sakirx}
\expl{}
\bmng
(Dababx \mo vugaLiMda) mucacxLa, biraDe, \mo vanunx tege. 
\emng
\eentry

\bentry
\word{unstrained} 
\pron{anfseTxrXVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eLeta, tuyatx, bigita, \mo vugaLige -- sikakxda; otatxDakekx guriyAgada. 
\num{2} parxyAsa, sharxma, duDita -- anuBavisada. 
\num{3} soVsada; basiyada; jaraDi hiDiyada; sAraNigeyalilx soVsada. 
\num{4} bahaLa baLasidudariMda yA vipariVta beVDikeyiMda apAyavAgada, naSaTxvAgada. 
\enum
\emng
\eentry

\bentry
\word{unstrap} 
\pron{anfsATxrXYxpf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unstrapped}; \vakaq\ \eng{unstrapping}). }
\bmng
(Dababx \mo vugaLa) paTiTxyanunx bicucx. 
\emng
\eentry

\bentry
\word{unstratified} 
\pron{anfsATxrXTipheYDf} 
\gl{\gu}
\expl{}
\bmng
satxragaLilalxda; padaragaLilalxda; satxragaLAgirada; padarapadaravAgirada; satxrarUpadalilxrada. 
\emng
\eentry

\bentry
\word{unstreamed} 
\pron{anfsiTxrXVmfDx} 
\gl{\gu}
\expl{}
\bmng
(\birx) (shAlAmakakxLa \vi) vagiRVkarisada; pATha heVLalu oMdu vayasisxna makakxLanunx taMDagaLAgi viMgaDisada, viBAga mADilalxda. 
\emng
\eentry

\bentry
\word{unstressed} 
\pron{anfseTxrXsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} otatxDakekx, balAtAkxrakekx, otAtxyakekx -- sikakxda. 
\num{2} (akaSxra, ucAcxrAMsha, \mo vugaLa \vi) savxraBAra, otutx -- hAkada; otutx, savxraBAra -- hAki ucacxrisada. 
\enum
\emng
\eentry

\bentry
\word{unstring} 
\pron{anfsiTxrXMgf} 
\gl{\akirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unstrung}).}
\bmng
\bnum
\num{1} (bilulx, taMtivAdayx, \mo vugaLa) dAra -- bicucx, saDilisu; hede bicucx; taMti -- bicucx, saDilisu: \eng{unstring a bow} bililxna hede bicucx. 
\num{2} (poVNisiruva, kaTiTxruva) dAradiMda horakekx tege, bicucx: \eng{unstring beads} maNigaLanunx dAradiMda bicucx. 
\enum
\emng
\eentry

\bentry
\word[unstuck(1)]{unstuck} 
\pron{anfsaTxkf} 
\gl{\kirx}
\expl{}
\bmng
\eng{unstick} dhAtuvina \BU\ matutx \BUkaq\ rUpa. 
\emng
\eentry

\bentry
\word[unstuck(2)]{unstuck} 
\pron{anfsaTxkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (yAvudakekxV aMTisiruvudanunx, kaTiTxhAkiruvudanunx) biDisida; mukatxvAda; beVpaRDisida. 
\num{2} savxtaMtarxvAda; karxma, niyaMtarxNa, hoMdANike, \mo vugaLiMda -- kaLacida, biDisida. 
\enum
\emng
\eentry

\bentry
\word{unstructured} 
\pron{anfsaTxrXkacxrfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} racanAvinAyxsavilalxda; racanAkarxmavilalxda. 
\num{2} vidhuyxkatxvalalxda; anibaRMdha riVtiya; anwpacArikavAda. 
\enum
\emng
\eentry

\bentry
\word{unstrung} 
\pron{anfsaTxrXMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} dubaRlavAda; shakitxyuDugida; balahiVnavAda. 
\num{2} manasisxna yA naragaLa meVle hiDita yA niyaMtarxNa ilalxda yA kaLedukoMDa. 
\enum
\emng
\eentry

\bentry
\word{unstudied} 
\pron{anfsaTxDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} budidhxpUvaRkavalalxda. 
\num{2} ayatanxpUvaRkavAda; saraLavAda; sahajavAda; neYsagiRkavAda. 
\num{3} vAyxsaMgamADada; adhayxyana mADada; (oMdu vishiSaTx keSxVtarxdalilx) tiLivaLike ilalxda, arivilalxda. 
\enum
\emng
\eentry

\bentry
\word{unstudiedly} 
\pron{anfsaTxDiDfli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} budidhxpUvaRkavalalxde; ayatanxpUvaRkavAgi. 
\num{2} vAyxsaMgamADade; adhayxyana mADade. 
\enum
\emng
\eentry

\bentry
\word{unstuffed} 
\pron{anfsaTxphfTx} 
\gl{\gu}
\expl{}
\bmng
aDakirada; oLatuMbirada. 
\emng
\eentry

\bentry
\word{unstuffy} 
\pron{anfsaTxphi} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gALiyADuva; shudadhxgALiya korate ilalxdiruva. 
\num{2} sAdhAraNavAda; anwpacArikavAda; sAMdaBiRka. 
\enum
\emng
\eentry

\bentry
\word{unsubdued} 
\pron{anfsabfDUyxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} jayisada; soVlisada; nigarxhisada; bagigxsada; aDagisada; damana mADada; aMkege tarada; sAvxdhiVnakekx tarada. 
\num{2} tagigxsada; tiVkaSxNXte kaDime mADada. 
\enum
\emng
\eentry

\bentry
\word{unsubjugated} 
\pron{anfsabujxgeVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} soVlisada; nigarxhisada; aDagisada; damana mADada; aMkege tarada; sAvxdhiVnakekx oLapaDisada. 
\num{2} tagigxsada; tiVkaSx Nxte kaDime mADada. 
\enum
\emng
\eentry

\bentry
\word{unsubstantial} 
\pron{anfsabfsATxYxnfSa(Sf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} vasutxBUtavAgirada; vAsatxvavalalxda; nijavAgi irada; avAsatxvika. 
\num{2} tiruLilalxda; satatxvXvilalxda; sAraBUtavalalxda. 
\num{3} amuKayxvAda; gaNaniVyavalalxda. 
\num{4} daDUtiyAgilalxda; daSaTxpuSaTxvAgirada. 
\enum
\emng
\eentry

\bentry
\word{unsubstantiality} 
\pron{anfsabfsATxYxniSxAYxliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vAsatxvavalalxdiruvudu; vasutxBUtavAgiradiruvudu; avAsatxvikate. 
\num{2} tiruLilalxdAgiruvudu; satatxvXrahita sithxti. 
\num{3} amuKayxte. 
\enum
\emng
\eentry

\bentry
\word{unsubstantially} 
\pron{anfsabfsATxYxnaSxli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} vasutxBUtavAgirade; avAsatxvikavAgi. 
\num{2} tiruLilalxde; satatxvXvilalxde. 
\enum
\emng
\eentry

\bentry
\word{unsubstantiated} 
\pron{anfsabfsATxYxniSxETiDf} 
\gl{\gu}
\expl{}
\bmng
(ApAdane, heVLike, hakukx, \mo vugaLa \vi) AdhAra koDada; rujuvAtu mADada; samathiRsada; parxmANa koTuTx -- daqDhiVkarisada, balapaDisada, BadarxpaDisada. 
\emng
\eentry

\bentry
\word{unsuccess} 
\pron{anfsakasxsf} 
\gl{\nA}
\expl{}
\bmng
soVlu; apajaya: 
\banum
\alnum{a} jayashAliyAgadiruvudu. 
\alnum{b} soVtiruva parxsaMga. 
\eanum
\emng
\eentry

\bentry
\word{unsuccessful} 
\pron{anfsakasxsfPuZlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} soVta; apajayada; jayashAliyAgada. 
\num{2} niSaPxlavAda; neraveVrada; sididhxsada; ELigeyAgada. 
\num{3} teVgaRDe hoMdada; napAsAda. 
\enum
\emng
\eentry

\bentry
\word{unsuccessfully} 
\pron{anfsakasxsfPuZli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} jayashAliyAgadaMte. 
\num{2} niSaPxlavAdaMte; sididhxsadaMte. 
\num{3} teVgaRDe hoMdadaMte. 
\enum
\emng
\eentry

\bentry
\word{unsuccessfulness} 
\pron{anfsakasxsfPuZlfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} apajaya. 
\num{2} niSaPxlate; asididhx. 
\num{3} napAsu; teVgaRDe hoMdadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsugared} 
\pron{anfSugaDfR} 
\gl{\gu}
\expl{}
\bmng
sihi mADada; savigoLisada; sakakxre hAkada, beresada. 
\emng
\eentry

\bentry
\word{unsuggestive} 
\pron{anfsajesiTxvf} 
\gl{\gu}
\expl{}
\bmng
asUcaka; sUcisada; sUcane koDada. 
\emng
\eentry

\bentry
\word{unsuitability} 
\pron{anfsUTabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} hoMdANikeyilalxdiruvike; sarihoMdadiruvike. 
\num{2} takukxdAgilalxdiruvike; anucitate; ananukUlate. 
\enum
\emng
\eentry

\bentry
\word{unsuitable} 
\pron{anfsUTaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sarihoMdada; saribiVLada. 
\num{2} takukxdalalxda; anucitavAda; anukUlavAgada. 
\enum
\emng
\eentry

\bentry
\word{unsuitableness} 
\pron{anfsUTabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unsuitability}{unsuitability}. 
\emng
\eentry

\bentry
\word{unsuitably} 
\pron{anfsUTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sarihoMdadaMte. 
\num{2} takukxdAgiradaMte. 
\enum
\emng
\eentry

\bentry
\word{unsuited} 
\pron{anfsU(sUyx)TiDf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unsuitable}{unsuitable}. 
\emng
\eentry

\bentry
\word{unsullied} 
\pron{anfsaliDf} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ \kAparx) 
\bnum
\num{1} koLeyAgada; malinavAgada. 
\num{2} kaLaMkitavAgada; dUSitavAgada; keTiTxrada. 
\enum
\emng
\eentry

\bentry
\word{unsummoned} 
\pron{anfsamanfDx} 
\gl{\gu}
\expl{}
\bmng
hAjarAgabeVkeMdu Ajecnx mADada, karekoDada; baraheVLada. 
\emng
\eentry

\bentry
\word{unsung} 
\pron{anfsaMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kavite kaTiTx hADada; kavitArUpadalilx hADada. 
\num{2} hADi hogaLada; guNagAna mADada; koMDADada; kiVtaRne mADada: \eng{an unsung hero} (yArU) hADi hogaLada, koMDADada viVra. 
\enum
\emng
\eentry

\bentry
\word{unsunned} 
\pron{anfsanfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} bisilu biVLada; bisilu tAkada, tagulada. 
\num{2} bisilu kAyisada; bisilige oDaDxda, hiDiyada; bisilinalilx iDada, oNagisada. 
\enum
\emng
\eentry

\bentry
\word{unsupervised} 
\pron{anfsU(sUyx)paveRYsfDx} 
\gl{\gu}
\expl{}
\bmng
meVlusutxvAri mADada; usutxvAriyilalxda. 
\emng
\eentry

\bentry
\word{unsupplied} 
\pron{anfsapelxYDf} 
\gl{\gu}
\expl{}
\bmng
odagisada; sarabarAju mADada. 
\emng
\eentry

\bentry
\word{unsupportable} 
\pron{anfsapoVTaRba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} etitxhiDiyalAgada; samathiRsalAgada; anumoVdisalAgada. 
\num{2} tALalAgada; sahisalAgada; seYrisalAgada. 
\enum
\emng
\eentry

\bentry
\word{unsupportably} 
\pron{anfsapoVTaRbilx} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} samathiRsalAgadaMte. 
\num{2} tALalAgadaMte; sahisalAgadaMte; asahaniVyavAgi. 
\enum
\emng
\eentry

\bentry
\word{unsupported} 
\pron{anfsapoVTiRDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} etitxhiDiyada; beMbalisada; samathiRsada; anumoVdisada. 
\num{2} tALada; sahisada; seYrisada. 
\enum
\emng
\eentry

\bentry
\word{unsupportedly} 
\pron{anfsapoVTiRDfli} 
\gl{\kirxvi}
\expl{}
\bmng
beMbalavilalxde; samathaRne ilalxdaMte. 
\emng
\eentry

\bentry
\word{unsure} 
\pron{anfSuarf, anfSoVrf} 
\gl{\gu}
\expl{}
\bmng
KAtari ilalxda; nishacxyavilalxda; anishicxta; daqDhavalalxda. 
\emng
\eentry

\bentry
\word{unsurely} 
\pron{anfSualiR, anfSoVliR} 
\gl{\kirxvi}
\expl{}
\bmng
KAtari ilalxde; anishicxtavAgi. 
\emng
\eentry

\bentry
\word{unsureness} 
\pron{anfSuarfnisf, anfSorfnisf} 
\gl{\nA}
\expl{}
\bmng
KAtari ilalxdiruvike; anishicxtate. 
\emng
\eentry

\bentry
\word{unsurpassable} 
\pron{anfsapARsaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
miVralAgada; miVrisalAgada; atishayisalAgada. 
\emng
\eentry

\bentry
\word{unsurpassably} 
\pron{anfsapARsabilx} 
\gl{\kirxvi}
\expl{}
\bmng
miVralAgadaMte yA miVrisalAgadaMte. 
\emng
\eentry

\bentry
\word{unsurpassed} 
\pron{anfsapARsfTx} 
\gl{\gu}
\expl{}
\bmng
miVrada; miVrisada; atishayisada; meVlAgirada. 
\emng
\eentry

\bentry
\word{unsurprising} 
\pron{anfsarfperxYsiMgf} 
\gl{\gu}
\expl{}
\bmng
AshacxyaRkaravalalxda; visamxyakAriyalalxda. 
\emng
\eentry

\bentry
\word{unsurprisingly} 
\pron{anfsarfperxYsiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
AshacxyaRkaravalalxdaMte. 
\emng
\eentry

\bentry
\word{unsurveyed} 
\pron{anfsaveRVDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} daqSiTx harisada; avaloVkisada; parishiVlisada; samiVkiSxsada. 
\num{2} (jamiVnu) aLate mADada; moVjaNi mADada; saveR mADada. 
\enum
\emng
\eentry

\bentry
\word{unsusceptibility} 
\pron{anfsasepiTxbiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} niravakAshate; eDegoDadiruvike. 
\num{2} (parxBAva \mo vakekx) oLagAgadiruvike; pakAkxgadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsusceptible} 
\pron{anfsasepiTxba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} eDegoDada; avakAshavirada. 
\num{2} (parxBAva \mo vakekx) oLapaDada; IDAgada; pakAkxgada; manasoVlada; vashavAgada. 
\enum
\emng
\eentry

\bentry
\word{unsuspected} 
\pron{anfsasepxkiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMdeVhakokxLagAgada; shaMkege guriyAgada; anumAna, apanaMbike, gumAnige -- oLagAgada. 
\num{2} (ideyeMdu) Uhisada; kalipxsada; BAvisada. 
\enum
\emng
\eentry

\bentry
\word{unsuspecting} 
\pron{anfsasepxkiTxMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMdeVha paDada; saMshaya savxBAvavilalxda; shaMkepaDada. 
\num{2} Uhisada; BAvisada. 
\enum
\emng
\eentry

\bentry
\word{unsuspectingly} 
\pron{anfsasepxkiTxMgfli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} saMdeVhapaDade. 
\num{2} Uhisade. 
\enum
\emng
\eentry

\bentry
\word{unsuspectingness} 
\pron{anfsasepxkiTxMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} saMdeVhapaDadiruvike; shaMkisadiruvike. 
\num{2} Uhisadiruvike; BAvisadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsuspicious} 
\pron{anfsasipxSasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} saMdeVha paDada; anumAna savxBAvavilalxda; shaMkepaDada. 
\num{2} saMdeVhAsapxdavalalxda; shaMkege Asapxdavilalxda; gumAnige avakAshavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unsuspiciously} 
\pron{anfsasipxSasfli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} saMdeVhapaDadaMte; shaMkisadaMte. 
\num{2} saMdeVhAsapxdavAgiradaMte; shaMkege avakAsha vilalxdaMte. 
\enum
\emng
\eentry

\bentry
\word{unsuspiciousness} 
\pron{anfsasipxSasfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} saMdeVhapaDadiruvike; shaMkisadiruvike. 
\num{2} saMdeVhAsapxdavAgiradiruvike; shaMkege Asapxdavilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unsustained} 
\pron{anfsaseTxVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} poVSitavAgada; puSiTx paDeyada. 
\num{2} samathiRtavAgada. 
\enum
\emng
\eentry

\bentry
\word{unswathe} 
\pron{anfsevxVdf} 
\gl{\sakirx}
\expl{}
\bmng
(kaTiTxda, sutitxda) paTiTxgaLanunx bicucx, saDilisu. 
\emng
\eentry

\bentry
\word{unswayed} 
\pron{anfsevxVDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} avicalita; oledADada; tonedADada. 
\num{2} parxBAvakekx oLagAgada. 
\enum
\emng
\eentry

\bentry
\word{unswear} 
\pron{anfsevxVrf} 
\gl{\sakirx}
\expl{}
\bmng
(ANeyiTuTx, parxmANa mADiruvudanunx) ilalxvenunx; nirAkarisu; parxmANa, parxtijecnx, shApa -hiMtegeduko. 
\emng
\eentry

\bentry
\word{unsweetened} 
\pron{anfsivxVT(Tf)nfDx} 
\gl{\gu}
\expl{}
\bmng
sihigoLisada; sihi, sakakxre -- beresada. 
\emng
\eentry

\bentry
\word{unswept} 
\pron{anfsevxpfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} guDisada. 
\num{2} biVsi (kocicx)koMDu hoVgada; raBasadiMda teVlihoVgada. 
\enum
\emng
\eentry

\bentry
\word{unswerving} 
\pron{anfsavxviRMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} dAribiTuTx hoVgada; dAribiTuTx tirugada; atitxtatx horaLada; pathacuyxtavAgada. 
\num{2} (\kanmu\ udedxVsha, guri, \mo vu) avicalita; neVravAda; oMdeV samanAda; sithxravAda; daqDhavAda: \eng{pursue an unswerving course} neVravAda mAgaRvanunx anusarisu. 
\enum
\emng
\eentry

\bentry
\word{unswervingly} 
\pron{anfsavxviRMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} dAribiTuTx tirugade; atitxtatx horaLade; pathacuyxti ilalxde. 
\num{2} avicalitavAgi; neVravAgi; oMdeV samanAda riVtiyalilx; sithxravAgi. 
\enum
\emng
\eentry

\bentry
\word{unsworn} 
\pron{anfsovxVnfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vayxkitxya \vi) badadhxvalalxda; ANe iDada; parxmANamADi heVLada; parxtijAcnxpUvaRkavalalxda. 
\num{2} parxtijecnxyiMda, parxmANamADi -- daqDhapaDisilalxda. 
\enum
\emng
\eentry

\bentry
\word{unsymmetrical} 
\pron{anfsimeTirxka(kf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (BAgagaLa parasapxra) parimANa aucitayxvilalxda; hoMdANikeilalxda. 
\num{2} samasUtarxte, samarUpate -- ilalxda; asamimxta. 
\enum
\emng
\eentry

\bentry
\word{unsymmetrically} 
\pron{anfsimeTirxkali} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (BAgagaLa parasapxra) parimANwcitayxvilalxdaMte. 
\num{2} asamimxtavAgi; samarUpate ilalxdaMte. 
\enum
\emng
\eentry

\bentry
\word{unsympathetic} 
\pron{anfsiMpateTikf} 
\gl{\gu}
\expl{}
\bmng
sahAnuBUtiyilalxda; anukaMpavilalxda; maruka, kanikara -- ilalxda. 
\emng
\eentry

\bentry
\word{unsympathetically} 
\pron{anfsiMpateTikali} 
\gl{\kirxvi}
\expl{}
\bmng
sahAnuBUtiyilalxde; anukaMpavilalxdaMte; maruka, kanikara -- ilalxdaMte. 
\emng
\eentry

\bentry
\word{unsystematic} 
\pron{anfsisaTxmAYxTikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} karxmabadadhxvalalxda; avayxvasithxta; vayxvasithxtavalalxda; kaTuTxpADilalxda; yAva padadhxtiyanUnx anusarisada. 
\num{2} ayoVjita; yoVjane ilalxda; udedxVsha pUvaRkavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unsystematically} 
\pron{anfsisaTxmAYxTikali} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} karxmabadadhxvalalxde; kaTuTxpADilalxde; vayxvasethxyilalxde; avayxvasithxtavAgi. 
\num{2} ayoVjitavAgi; yoVjane ilalxde; udedxVshapUvaRkavalalxdaMte. 
\enum
\emng
\eentry

\bentry
\word{untack} 
\pron{anfTAYxkf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} moLe hoDedu seVrisiruvudanunx kaLacu. 
\num{2} kUDisi holidiruvudanunx biDisu, bicucx. 
\num{3} jotege seVrisiruvudanunx parxteyxVkisu. 
\enum
\emng
\eentry

\bentry
\word{untainted} 
\pron{anfTeVniTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaLaMkarahita; doVSarahita. 
\num{2} roVga tagulada; soVMku tagulada. 
\enum
\emng
\eentry

\bentry
\word{untalented} 
\pron{anfTAYxlaniTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} parxtiBe ilalxda; meVdhAviyalalxda. 
\num{2} kwshalavilalxda. 
\enum
\emng
\eentry

\bentry
\word{untameable} 
\pron{anfTeVmaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (pArxNi) paLagisalAgada; sAdhu mADalAgada. 
\num{2} (kecucx \mo vanunx) kugigxsalAgada; iLisalAgada; adamayx; damana mADalAgada. 
\enum
\emng
\eentry

\bentry
\word{untamed} 
\pron{anfTeVmfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} paLagisada; sAdhu mADada. 
\num{2} kugigxsada; iLisada; damana mADada. 
\enum
\emng
\eentry

\bentry
\word{untangle} 
\pron{anfTAYxMga(gf)lf} 
\gl{\sakirx}
\expl{}
\bmng
goVju, toDaku, heNige -- biDisu. 
\emng
\eentry

\bentry
\word{untanned} 
\pron{anfTAYxnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (camaR) hadamADada. 
\num{2} (bisilige oDiDx) kaMdu tirugada; kaMdu tirugisada; kaMdAgada. 
\enum
\emng
\eentry

\bentry
\word{untapped} 
\pron{anfTAYxpfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1}
\banum
\alnum{a} (madayxda piVpAyi) tUtumADi horage tegeyada. 
\alnum{b} (mara) rasa tegeyada. 
\eanum
\numie
\num{2} (oMdu parxdeVsha \mo vugaLalilx) vayxvahAra beLesada; vAyxpAra hUDada; saMpakaR sAthxpisada. 
\num{3} (yAvudariMdaleV) mAhiti yA sAmagirx yA saMpanUmxlagaLanunx seLeyada, paDeyada: \eng{untapped resources} horaseLeyada, paDedukoLaLxda saMpanUmxlagaLu. 
\num{4} (keVLalu yA kare mADalu, dUravANi yA TeligArxM taMtige) sharxvaNasAdhanavanunx joVDisada, lagatitxsada. 
\enum
\emng
\eentry

\bentry
\word{untarnished} 
\pron{anfTAniRSfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaLeguMdada; kAMtihiVnavAgada; mAsada; masukAgada; mabAbxgada. 
\num{2} malinavAgisada; kaLaMka hacacxda. 
\enum
\emng
\eentry

\bentry
\word{untasted} 
\pron{anfTeVsiTxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ruci noVDada; saviyada; AsAvxdisada. 
\num{2} anuBavisada. 
\enum
\emng
\eentry

\bentry
\word{untaught} 
\pron{anfTATf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (viSaya \mo vu) kalisilalxda; heVLikoDada. 
\num{2} (vayxkitx) aritavanalalxda; ajAcnxniyAda; avidAyxvaMtanAda. 
\num{3} savx(yaM)shikiSxta; boVdhaneya mUlaka gaLisidadxlalxda; sahajavAgi kalita; tananxSaTxkekx, savxMtavAgi kalita. 
\enum
\emng
\eentry

\bentry
\word{untaxed} 
\pron{anfTAYxkfsxTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} terige hAkada; terige vidhisada. 
\num{2} terigeyanunx koVrada, hAkabeVkAgilalxda. 
\num{3} terige koDabeVkAgilalxda. 
\enum
\emng
\eentry

\bentry
\word{unteach} 
\pron{anfTiVcf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{untaught}).}
\bmng
\bnum
\num{1} IgAgaleV kalitiruva vicAragaLanunx (avugaLige virudadhxvAda boVdhaneya mUlaka) mareyuvaMte mADu. 
\num{2} modaleV kalitiruvudanunx mareyuvaMte yA tayxjisuvaMte mADu. 
\enum
\emng
\eentry

\bentry
\word{unteachable} 
\pron{anfTiVcaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (viSayada \vi) boVdhisalu kaSaTxvAda yA sAdhayxvilalxda; aboVdhaniVya. 
\num{2} (vayxkitxya \vi) shikaSxNAnahaR; kaliyuva yoVgayxte, sAmathaRyx ilalxda. 
\enum
\emng
\eentry

\bentry
\word{untearable} 
\pron{anfTeVraba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
hariyalAgada; haridu hAkalu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{untechnical} 
\pron{anfTekinxka(kf)lf} 
\gl{\gu}
\expl{}
\bmng
tAMtirxkavalalxda; shAsatxrXkekx saMbaMdhisada; atAMtirxka. 
\emng
\eentry

\bentry
\word{untechnically} 
\pron{anfTekinxkali} 
\gl{\kirxvi}
\expl{}
\bmng
tAMtirxkavalalxdaMte; atAMtirxkavAgi. 
\emng
\eentry

\bentry
\word{untempered} 
\pron{anfTeMpaDfR} 
\gl{\gu}
\expl{}
\bmng
(loVhada \vi) hada -- mADada, koDada; apeVkiSxta kAThinayxkekx yA sAMdarxtege tarada. 
\emng
\eentry

\bentry
\word{untenability} 
\pron{anfTenabiliTi} 
\gl{\nA}
\expl{}
\bmng
(vAdadiMda) samathiRsalAgadiruvike; asamathaRniVyate. 
\emng
\eentry

\bentry
\word{untenable} 
\pron{anfTenaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
(vAdadiMda) samathiRsalAgada; samathaRniVyavalalxda; opapxlAgada; takaRsaMgatavalalxda. 
\emng
\eentry

\bentry
\word{untenableness} 
\pron{anfTenabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{untenability}{untenability}. 
\emng
\eentry

\bentry
\word{untenably} 
\pron{anfTenabilx} 
\gl{\kirxvi}
\expl{}
\bmng
(vAdadiMda) samathiRsalAgadaMte. 
\emng
\eentry

\bentry
\word{untenanted} 
\pron{anfTenaMTiDf} 
\gl{\gu}
\expl{}
\bmng
vAsavilalxda; vAsisuvavarilalxda. 
\emng
\eentry

\bentry
\word{untended} 
\pron{anfTeMDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} rakaSxNe odagisada; kAvalilalxda; usutxvAri ilalxda. 
\num{2} upacarisada; AreYke mADada. 
\num{3} sariyAgi naDesada, noVDikoLaLxda; upeVkiSxta. 
\enum
\emng
\eentry

\bentry
\word{untested} 
\pron{anfTesiTxDf} 
\gl{\gu}
\expl{}
\bmng
apariVkiSxta; pariVkiSxsada; pariVkiSxsi, orehacicx noVDada. 
\emng
\eentry

\bentry
\word{untether} 
\pron{anfTedarf} 
\gl{\sakirx}
\expl{}
\bmng
(meVyalu kaTiTx hAkiruva pArxNi \mo vanunx) hagagx saDilisu; gUTadiMda bicucx. 
\emng
\eentry

\bentry
\word{untethered} 
\pron{anfTedaDfR} 
\gl{\gu}
\expl{}
\bmng
(meVyalu kaTiTxhAkiruva pArxNi \mo vugaLa \vi) hagagx bicicxda; gUTadiMda bicicxbiTaTx. 
\emng
\eentry

\bentry
\word{unthanked} 
\pron{anftAYxMkfTx} 
\gl{\gu}
\expl{}
\bmng
vaMdane, dhanayxvAda -- apiRsada; ABAra mananxNe mADada; kaqtajacnxte salilxsada. 
\emng
\eentry

\bentry
\word{unthankful} 
\pron{anftAYxMkfPuZlf} 
\gl{\gu}
\expl{}
\bmng
kaqtajacnxte ilalxda; ABAriyAgirada; upakAra samxrisada. 
\emng
\eentry

\bentry
\word{unthankfully} 
\pron{anftAYxMkfPuZli} 
\gl{\kirxvi}
\expl{}
\bmng
kaqtajacnxte ilalxdaMte; upakAra samxrisadaMte. 
\emng
\eentry

\bentry
\word{unthankfulness} 
\pron{anftAYxMkfPuZlfnisf} 
\gl{\nA}
\expl{}
\bmng
kaqtajacnxteya aBAva; upakAra samxraNeyilalxdiruvudu. 
\emng
\eentry

\bentry
\word{unthatched} 
\pron{anftAYxcfTx} 
\gl{\gu}
\expl{}
\bmng
(cAvaNi, mane, \mo vugaLige) hululx hodisada; gari hocacxda; gariya sUru hAkada. 
\emng
\eentry

\bentry
\word{unthinkability} 
\pron{anftiMkabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aciMtayxte. 
\num{2} (\AmA) asaMBavaniVyate. 
\enum
\emng
\eentry

\bentry
\word{unthinkable} 
\pron{anftiMkaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} yoVcisalAgada; aciMtayx. 
\numi{2} (\AmA) 
\banum
\alnum{a} tiVrA asaMBavaniVya. 
\alnum{b} apeVkaSxNiVyavalalxda; ahitakara; ananukUlada; aniSaTxda. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{unthinkableness} 
\pron{anftiMkabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unthinkability}{unthinkability}. 
\emng
\eentry

\bentry
\word{unthinkably} 
\pron{anftiMkabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} yoVcisalAgadaSuTx. 
\num{2} asaMBavaniVyavAgi. 
\num{3} apeVkaSxNiVyavalalxda riVtiyalilx. 
\enum
\emng
\eentry

\bentry
\word{unthinking} 
\pron{anftiMkiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} yoVcane mADada; avicArada; dUrAloVcane ilalxda. 
\num{2} nilaRkaSxyXda; ecacxrike ilalxda; duDukina: \eng{in an unthinking moment} yAvudoV ecacxrike ilalxda GaLigeyalilx. 
\num{3} udedxVshapUvaRkavalalxda; ajAgarUkateyiMda, upeVkeSxyiMda -- Ada, mADida. 
\enum
\emng
\eentry

\bentry
\word{unthinkingly} 
\pron{anftiMkiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} yoVcane mADade; dUrAloVcaneyilalxde. 
\num{2} nilaRkaSxyXvAgi; ajAgarUkateyiMda; duDuki. 
\enum
\emng
\eentry

\bentry
\word{unthinkingness} 
\pron{anftiMkiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AloVcanArAhitayx; yoVcanemADadiruvike; dUrAloVcaneyilalxdiruvike. 
\num{2} ajAgarUkate; nilaRkaSxyX; duDuku. 
\enum
\emng
\eentry

\bentry
\word{unthought} 
\pron{anftATf} 
\gl{\gu}
\expl{}
\bmng
yoVcisada; eNisirada; anAloVcita. 
\emng
\eentry

\bentry
\word{unthoughtful} 
\pron{anftATfPuZlf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unthinking}{unthinking}. 
\emng
\eentry

\bentry
\word{unthoughtfully} 
\pron{anftATfPuZli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} yoVcanemADade; AloVcaneyilalxde. 
\num{2} lekikxsade; duDukiniMda. 
\enum
\emng
\eentry

\bentry
\word{unthoughtfulness} 
\pron{anftATfPuZlfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AloVcanAshUnayxte; yoVcane mADadiruvike. 
\num{2} lekikxsadiruvike; duDuku. 
\enum
\emng
\eentry

\bentry
\word{unthread} 
\pron{anfterxDf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} poVNisidadx dAra(vanunx sUjiyiMda) tegeduhAku. 
\num{2} (toLasu baLasiniMda) horakekx bA; sutitx baLasi horakekx dAri mADikoMDu bA. 
\enum
\emng
\eentry

\bentry
\word{unthreshed} 
\pron{anfterxSfTx} 
\gl{\gu}
\expl{}
\bmng
(dhAnayx) okakxda; tene baDiyada. 
\emng
\eentry

\bentry
\word{unthriftily} 
\pron{anftirxphiTxli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} duMdugArikeyiMda; duMduvecacx mADuvaMte. 
\num{2} ELige hoMdadaMte; aBivaqdidhx paDeyadaMte. 
\enum
\emng
\eentry

\bentry
\word{unthriftiness} 
\pron{anftirxphiTxnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} duMdugArike; duMduvecacx mADuvudu. 
\num{2} ELige hoMdadiruvudu; aBivaqdidhxyAgadiruvudu. 
\enum
\emng
\eentry

\bentry
\word{unthrifty} 
\pron{anftirxphiTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} duMdugArikeya; duMduvecacx mADuva. 
\num{2} ELige hoMdada; aBivaqdidhx, parxgati, ucACxrXya -- hoMdada. 
\enum
\emng
\eentry

\bentry
\word{unthrone} 
\pron{anftorxVnf} 
\gl{\sakirx}
\expl{}
\bmng
= \hyperref{kandict_d.pdf}{D}{dethrone}{dethrone}. 
\emng
\eentry

\bentry
\word{untidily} 
\pron{anfTeYDili} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} opapxOraNavilalxde; acucxkaTATxgilalxde. 
\num{2} koLakAgi. 
\num{3} avayxvasethxyiMda. 
\enum
\emng
\eentry

\bentry
\word{untidiness} 
\pron{anfTeYDinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} opapxOraNavilalxdiruvike; acucxkaTATxgilalxdiruvike. 
\num{2} koLakAgiruvike. 
\num{3} avayxvasethx. 
\enum
\emng
\eentry

\bentry
\word{untidy} 
\pron{anfTeYDi} 
\gl{\gu}
\expl{(\tara\ \eng{untidier}, \tama\ \eng{untidiest}).}
\bmng
\bnum
\num{1} opapxOraNavilalxda; acucxkaTATxgilalxda; paDapoVsiyAda. 
\num{2} koLakAda; holasAda. 
\num{3} vayxvasethxyilalxda; avayxvasethxya: \eng{an untidy plan} vayxvasethxyilalxda yoVjane. 
\enum
\emng
\eentry

\bentry
\word{untie} 
\pron{anfTeY} 
\gl{\akirx}
\expl{(\vakaq\ \eng{untying}).}
\bmng
\bnum
\num{1} (gaMTu \mo vanunx) bicucx. 
\num{2} (mUTe \mo vugaLa) kaTuTx bicucx. 
\num{3} (\rUpa) (opapxMda, saMbaMdha, \mo vugaLiMda) biDugaDe mADu; vimukitxgoLisu; vimoVcisu. 
\enum
\emng
\eentry

\bentry
\word{untied} 
\pron{anfTeYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaTiTxlalxda; bicicxda. 
\num{2} (\rUpa) (opapxMda \mo vugaLiMda) biDugaDe mADida; vimukitxgoLisida. 
\enum
\emng
\eentry

\bentry
\word[until(1)]{until} 
\pron{anfTilf} 
\gl{\upa}
\expl{}
\bmng
\bnum
\num{1} (\kanmu\ vAkayx AraMBisuvAga, matutx aupacArika sheYliyalilx \parx) -varegU; -varege; -tanaka: \eng{until you told me I had no idea of it} niVnu heVLuvavaregU nanage adara kalapxneyeV iralilalx. 
\num{2} (niSeVdhAthaRka racaneyalilx): \eng{he did not go until night} rAtirxyavaregU avanu hoVgalilalx. 
\num{3} (vidhuyxkatxvAda baLakeyalilx): \eng{resided there until his death} avanu tiVrihoVguvavaregU alilx vAsavAgidadx. 
\enum
\emng
\eentry

\bentry
\word[until(2)]{until} 
\pron{anfTilf} 
\gl{\saMavayx}
\expl{}
\bmng
= \hyperref{kandict_t.pdf}{T}{till(1)}{$^1$till}. (\sA\ niSeVdhAthaRka vAkayxgaLalilx \parx): \eng{he did not come until the meeting was half over} saBe adhaR mugiyuva tanaka avanu baralilalx. 
\emng
\eentry

\bentry
\word{untillable} 
\pron{anfTilaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
uLalu sAdhayxvilalxda; beVsAya mADalAgada; kaqSi sAdhayxvalalxda; sAguvaLige yoVgayxvalalxda. 
\emng
\eentry

\bentry
\word{untilled} 
\pron{anfTilfDx} 
\gl{\gu}
\expl{}
\bmng
uLada; beVsAya, kaqSi, sAguvaLi -- mADada. 
\emng
\eentry

\bentry
\word{untimeliness} 
\pron{anfTeYmflinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} kAloVcitavalalxdiruvike; samayoVcitavalalxdiruvike. 
\num{2} (sAvina \vi) akAlikate; kAlakekx muMceyeV saMBavisuvike. 
\enum
\emng
\eentry

\bentry
\word[untimely(1)]{untimely} 
\pron{anfTeYmfli} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sariyAda kAla odaguvudakekx muMcina; kAloVcitavalalxda; samayoVcitavalalxda; akAlada: \eng{an untimely remark} samayoVcitavalalxda TiVke. 
\num{2} (maraNada \vi) akAlika; kAlakekx muMceyeV odagida: \eng{he came to an untimely end} avanu akAlamaraNakekx tutAtxda. 
\enum
\emng
\eentry

\bentry
\word[untimely(2)]{untimely} 
\pron{anfTeYmfli} 
\gl{\kirxvi}
\expl{}
\bmng
(\pArxparx) 
\bnum
\num{1} kAloVcitavalalxda riVtiyalilx; samayoVcita valalxda hAge. 
\num{2} akAlikavAgi. 
\enum
\emng
\eentry

\bentry
\word{untinged} 
\pron{anfTiMjfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (baNaNx \mo vugaLa) CAyeyilalxda; beresada; seVrisada; mishirxtavalalxda. 
\num{2} pUvARgarxhavilalxda; pakaSxpAtavilalxda; vasutxniSaThx. 
\enum
\emng
\eentry

\bentry
\word{untiring} 
\pron{anfTeYariMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} daNivilalxda; susAtxgada; AyAsagoLaLxda; daNivilalxde duDiyuva: \eng{she seems to be untiring} avaLu daNivilalxde duDiyuvaMte toVrutatxde. 
\num{2} avAyxhata; aviciPxnanx; niraMtaravAgi mADuva, muMduvarisuva: \eng{his untiring efforts} avana avAyxhata parxyatanxgaLu. 
\enum
\emng
\eentry

\bentry
\word{untitled} 
\pron{anfTeYT(Tf)lfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} birudu bAvaligaLilalxda: \eng{an untitled nobleman} birudAMkitagaLilalxda shirxVmaMta. 
\num{2} hakukxbAdhayxtegaLilalxda. 
\num{3} hesariDada; shiroVnAmeyilalxda; anAmika. 
\enum
\emng
\eentry

\bentry
\word{unto} 
\pron{anfTu(Ta)} 
\gl{\upa}
\expl{}
\bmng
(\pArxparx) = \hyperref{kandict_t.pdf}{T}{to(1)}{$^1$to}. (keVvala catuthiRV \parx) tumaMta (\eng{infinitive} ) \parx\ --vinA: \eng{do unto others} inonxbabxra viSayadalilx oMdu riVtiyalilx naDeduko. \eng{sick} (\engit{or} \eng{faithful) unto death} koneya gaLigeyavaregU roVgagarxsatxnAgi (yA niSeThxyiMda kUDi). 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{be a} \hyperref{kandict_l.pdf}{L}{law(1) nuga(1)}{$^1$law} \eng{unto oneself.} 
\emng
\eentry

\bentry
\word{untold} 
\pron{anfToVlfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anukatx; heVLilalxda; heVLalAgada. 
\num{2} aLeyalAgada; apAra; aparimita; lekakxvilalxda; agaNita: \eng{untold wealth} apAra saMpatutx. \eng{untold misery} heVLatiVrada kaSaTx. 
\enum
\emng
\eentry

\bentry
\word{untouchability} 
\pron{anfTacabiliTi} 
\gl{\nA}
\expl{}
\bmng
asapx qshayxte. 
\emng
\eentry

\bentry
\word[untouchable(1)]{untouchable} 
\pron{anfTacaba(bf)lf} 
\gl{\gu}
\expl{}
\bmng
asapxqqshayx: 
\banum
\alnum{a} muTaTxlAgada; muTaTxlu sAdhayxvilalxda. 
\alnum{b} muTaTxbArada; muTaTxlu yoVgayxvalalxda. 
\eanum
\emng
\eentry

\bentry
\word[untouchable(2)]{untouchable} 
\pron{anfTacaba(bf)lf} 
\gl{\nA}
\expl{}
\bmng
asapx qshayx; (hiMdU samAjada) nimanx vagaRdavanu. 
\emng
\eentry

\bentry
\word{untouchableness} 
\pron{anfTacabflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{untouchability}{untouchability}. 
\emng
\eentry

\bentry
\word{untouched} 
\pron{anfTacfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (keYyiMda) muTaTxda; keYsoVkada; asapxqqSaTx. 
\num{2} sutitx noVDada; ananevxVSita; pariVkiSxsada; hokukx baLasada: \eng{untouched lands} hokukx baLasada deVshagaLu. 
\num{3} apiVta; aBukatx; kuDiyade, tinanxde (hAgeyeV) -- biTaTx, uLisida. 
\num{4} (hiMdina) parishudadhx sithxtiya; Adisithxtiya. 
\num{5} hAnige IDAgada. 
\num{6} (yAvudariMdalU) parxBAvitavAgada; pariNAmagoLaLxda; mApARDAgada: \eng{she was untouched by the life around her} avaLa sutatxlina baduku avaLa meVle yAva pariNAmavanUnx biVralilalx. 
\num{7} (BAvukavAgi) yAva olavU ilalxda; udAsiVnada; anAdarada; tAtAsxrada. 
\num{8} (garxMtha, mAtukate, \mo vugaLalilx) ulelxVKisada; parxsAtxpisada; caciRsada. 
\enum
\emng
\eentry

\bentry
\word{untoward} 
\pron{anfTavaDfR, anfToVaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx) vakarx; mUKaR; aDaDxmAgaRda; aMkegoLapaDada. 
\num{2} (\gaparx) aDADxdiDiDxya; eDavaTATxda. 
\num{3} duradaqSaTxda; niBARgayx; duSapxriNAmada: \eng{untoward circumstances forced him into bankruptcy} duradaqSaTxkara saninxveVshagaLu avananunx divALitanakekx taLiLxdavu. 
\num{4} anucitavAda; yukatxvalalxda; yoVgayxvalalxda: \eng{untoward social behaviour} anucitavAda sAmAjika naDavaLike. 
\enum
\emng
\eentry

\bentry
\word{untowardly} 
\pron{anfTavaDfRli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx) vakarxvAgi; mUKaRtanadiMda; aDaDxmAgaRdalilx; aMke ilalxda hAge. 
\num{2} (\gaparx) aDADxdiDiDxyAgi; eDavaTATxgi. 
\num{3} duradaqSaTxvashAtf; dwBARgayxdiMda. 
\num{4} anucitavAgi. 
\enum
\emng
\eentry

\bentry
\word{untowardness} 
\pron{anfTavaDfRnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (\pArxparx) vakarxte; mUKaRtana; aMke miVrida naDate. 
\num{2} duradaqSaTxkara sithxtiyalilx sikukxvudu; duSapxriNAmakekx tutAtxgiruvudu. 
\num{3} anucita naDavaLike. 
\enum
\emng
\eentry

\bentry
\word{untraceable} 
\pron{anfTerxVsabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} reVKisalAgada; citirxsalAgada. 
\num{2} nele kaMDuhiDiyalAgada; suLivu kANalu kaSaTxvAda; patetx hacacxlu sAdhayxvilalxda; cihenx kANalAgada; amaqgayx. 
\enum
\emng
\eentry

\bentry
\word{untraceably} 
\pron{anfTerxVsabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} reVKisalAgade; citirxsalAgade. 
\num{2} nele kaMDuhiDiyalAgade. 
\enum
\emng
\eentry

\bentry
\word{untraced} 
\pron{anfTerxVsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} reVKisada; citirxsada. 
\num{2} nele kaMDuhiDiyada; suLivu kANada; patetx hacacxda; jADu gurutisada. 
\enum
\emng
\eentry

\bentry
\word{untrained} 
\pron{anfTerxVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tarapeVtu ilalxda yA mADada. 
\num{2} paLagada yA paLagisada. 
\num{3} shikaSxNa koDada; ashikiSxta. 
\num{4} anuBavavilalxda; nuritilalxda. 
\enum
\emng
\eentry

\bentry
\word{untrammelled} 
\pron{anfTArxYxmalfDx} 
\gl{\gu}
\expl{}
\bmng
nibaRMdhavilalxda; abAdhita; taDeyilalxda; aDacaNeyuMTumADada; kaTiTx hAkada (\sA\ \rUpa). 
\emng
\eentry

\bentry
\word{untransferable} 
\pron{anfTArxYxnfsxpharabflf} 
\gl{\gu}
\expl{}
\bmng
vagARvaNe mADalAgada. 
\emng
\eentry

\bentry
\word{untranslatability} 
\pron{anfTArxYx(TArx)nfsxleVTabiliTi} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anuvAdisalAgadiruvike. 
\num{2} aparivataRniVyate. 
\enum
\emng
\eentry

\bentry
\word{untranslatable} 
\pron{anfTArxYx(TArx)nfsxleVTabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anuvAdisalAgada; BASAMtarisalAgada; BASAMtara sAdhayxvalalxda. 
\num{2} parivatiRsalAgada; badalAyisalAgada; rUpAMtarisalAgada; aparivataRniVya. 
\enum
\emng
\eentry

\bentry
\word{untranslatably} 
\pron{anfTArxYx(TArx)nfsxleVTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} anuvAdisalAgade. 
\num{2} parivatiRsalAgade. 
\enum
\emng
\eentry

\bentry
\word{untransportable} 
\pron{anfTArxYx(TArx)nfsxpoV(pA)TaRbflf} 
\gl{\gu}
\expl{}
\bmng
sAgisalAgada; oyayxlu sAdhayxvAgada; avahaniVya; avAhayx. 
\emng
\eentry

\bentry
\word{untravelled} 
\pron{anfTArxYxvalfDx} 
\gl{\gu}
\expl{}
\bmng
(\ame\ \eng{untraveled}). 
\bnum
\num{1} (deVsha) saMcAra mADada; parxyANa mADilalxda; deVshapayaRTane mADada. 
\num{2} (deVsha, pArxMtada \vi) yArU saMcAra mADilalxda; hokukx noVDilalxda; aparicita. 
\enum
\emng
\eentry

\bentry
\word{untreatable} 
\pron{anfTirxVTabf(ba)lf} 
\gl{\gu}
\expl{}
\bmng
 (kAyile \mo vugaLa \vi) acikitasxyX; cikitesx mADalu sAdhayxvilalxda. 
\emng
\eentry

\bentry
\word{untreated} 
\pron{anfTirxVTiDf} 
\gl{\gu}
\expl{}
\bmng
(kAyileya \vi) cikitesx mADada. 
\emng
\eentry

\bentry
\word{untried} 
\pron{anfTerxYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anuBavavilalxda; parisharxmavilalxda. 
\num{2} pariVkiSxsidadxlalxda; anuBavisi noVDada. 
\num{3} (nAyxyAlayadalilx) vicAraNeyAgilalxda. 
\enum
\emng
\eentry

\bentry
\word{untrodden} 
\pron{anfTArxDanf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} hejejx hAkada; kAliDada; naDedADada; tuLidADada. 
\num{2} aparicita; ajAcnxta. 
\enum
\emng
\eentry

\bentry
\word{untroubled} 
\pron{anfTarxbalfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} toMdare paDada. 
\num{2} samAdhAna sithxtiya; taLamaLavilalxda; parxshAMta; akuSxbadhx. 
\enum
\emng
\eentry

\bentry
\word{untrue} 
\pron{anfTUrx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nijavalalxda; asatayx; suLiLxna. 
\num{2} vAsatxvAMshakekx virudadhxvAda; avAsatxvika. 
\num{3} niSeThxyilalxda; daqDhabadadhxnAgilalxda; dorxVhiyAda. 
\num{4} sariyalalxda; Qujuvalalxda; tapupxdAriya. 
\enum
\emng
\eentry

\bentry
\word{untruly} 
\pron{anfTUrxli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} nijavalalxda riVtiyalilx; asatayxvAgi. 
\num{2} avAsatxvikavAgi; vAsatxvAMshakekx virudadhxvAgi. 
\num{3} niSeThxyilalxde; dorxVhiyaMte. 
\enum
\emng
\eentry

\bentry
\word{untruss} 
\pron{anfTarxsf} 
\gl{\sakirx}
\expl{}
\bmng
(kAlAvadi, Asare cwkaTuTx, \mo vanunx) bicucx; kaLacu; saDilisu. 
\emng
\eentry

\bentry
\word{untrussed} 
\pron{anfTarxsfTx} 
\gl{\gu}
\expl{}
\bmng
(Asare cwkaTuTx \mo vanunx) bicicxda; kaLacida; saDilisida. 
\emng
\eentry

\bentry
\word{untrustworthiness} 
\pron{anfTarxsfTxvadiRnisf} 
\gl{\nA}
\expl{}
\bmng
avishavxsaniVyate; naMbikege anahaRvAgiruvike; vishAvxsakekx yoVgayxvalalxdiruvike. 
\emng
\eentry

\bentry
\word{untrustworthy} 
\pron{anfTarxsfTxvadiR} 
\gl{\gu}
\expl{}
\bmng
avishavxsaniVya; naMbikege yoVgayxvalalxda; vishAvxsAhaRvalalxda; necicxkege pAtarxvalalxda. 
\emng
\eentry

\bentry
\word{untruth} 
\pron{anfTUrxtf} 
\gl{\nA}
\expl{(\bava\ \eng{untruths} \ucAcx-- anfTUrxtfsx(dfsx)).}
\bmng
\bnum
\num{1} satayxvalalxdiruvudu. 
\num{2} suLuLx; saTe; asatayx; anaqta: \eng{told me an untruth} nanage avanoMdu suLuLx heVLida. 
\enum
\emng
\eentry

\bentry
\word{untruthful} 
\pron{anfTUrxtfPuZlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} satayxvalalxda; suLiLxna. 
\num{2} niSeThx ilalxda. 
\enum
\emng
\eentry

\bentry
\word{untruthfully} 
\pron{anfTUrxtfPuZli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} asatayxvAgi; nijavAgirade; suLiLxnaMte. 
\num{2} niSeThx ilalxdaMte. 
\enum
\emng
\eentry

\bentry
\word{untruthfulness} 
\pron{anfTUrxtfPuZlfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} nijavalalxdiruvike; asatayxte. 
\num{2} niSeThx ilalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{untuck} 
\pron{anfTakf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} nirige -- bicucx, biDisu; nirige hiDidu holidiruvudanunx bicucx. 
\num{2} meVlakekx, oLakekx -- sikikxsiruvudanunx bicucx. 
\num{3} musuku tere; hodisiruvudanunx tege. 
\num{4} turukiruvudanunx tege; aDakiruvudanunx bicucx. 
\enum
\emng
\eentry

\bentry
\word{untunable} 
\pron{anfTUyxnabflf} 
\gl{\gu}
\expl{}
\bmng
(piyAnoV \mo vugaLa \vi) shurxti mADalAgada. 
\emng
\eentry

\bentry
\word{untuned} 
\pron{anfTUyxnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} (vAdayx) 
\banum
\alnum{a} shurxtigUDisada. 
\alnum{b} apasavxrada; iMpAgirada. 
\eanum
\numie
\num{2} (\rUpa) hoMdikeyilalxda; samarasavilalxda; asaMgatavAda. 
\num{3} (reVDiyoV gArxhaka \mo vugaLa \vi) yAvudeV oMdu taraMgAMtarakekx tirugisada. 
\enum
\emng
\eentry

\bentry
\word{untuneful} 
\pron{anfTUyxnfPuZlf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} apashurxtiya; shurxti sariyilalxda. 
\num{2} apasavxrada; iMpilalxda; sushArxvayxvalalxda. 
\enum
\emng
\eentry

\bentry 
\word{untunefully} 
\pron{anfTUyxnfPuZli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} apashurxtiyiMda. 
\num{2} apasavxravAgi; iMpilalxdaMte; shurxtikakaRshavAgi. 
\enum
\emng
\eentry

\bentry
\word{untunefulness} 
\pron{anfTUyxnfPuZlfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} apashurxti. 
\num{2} apasavxra; iMpilalxdiruvike; shurxtikakaRshate. 
\enum
\emng
\eentry

\bentry
\word{unturned} 
\pron{anfTanfRDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tirugada; sutatxda. 
\num{2} tirugisada; sutitxsada; sutatx tirugisada. 
\num{3} talekeLagu mADada; maguci hAkada. 
\num{4} sututx tirugisi AkAra koDada, rUpisada. 
\enum
\emng
\eentry

\bentry
\word{untutored} 
\pron{anfTUyxTaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} heVLikoTiTxlalxda; boVdhaneyAgilalxda. 
\num{2} ashikiSxta; tiLivaLike ilalxda. 
\enum
\emng
\eentry

\bentry
\word{untwine} 
\pron{anfTevxYnf} 
\gl{\sakirx}
\expl{}
\bmng
hosediruvudanunx bicucx; huri bicucx; tiruciruvudanunx biDisu; suruLi bicucx. 
\emng
\eentry

\bentry
\word{unused} 
\pron{anfyUsfZDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} upayoVgisada; baLasada. 
\num{2} (\ucAcx\ anfyUsfTx) rUDhi ilalxda; vADikeyilalxda. 
\enum
\emng
\eentry

\bentry
\word{unusual} 
\pron{anfyUSuZalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} rUDhiyalalxda. 
\num{2} asAmAnayx; asAdhAraNa; visheVSavAda; gamanAhaRvAda. 
\enum
\emng
\eentry

\bentry
\word{unusually} 
\pron{anfyUSuZali} 
\gl{\kirxvi}
\expl{}
\bmng
asAmAnayxvAgi; asAdhAraNavAgi; visheVSa riVtiyalilx. 
\emng
\eentry

\bentry
\word{unusualness} 
\pron{anfyUSuZalfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} rUDhiyalilxlalxdiruvike. 
\num{2} asAmAnayxte; asAdhAraNatavx; vishiSaTxte. 
\enum
\emng
\eentry

\bentry
\word{unutterable} 
\pron{anfaTarabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mAtiniMda heVLalAgada; avaNaRniVya; anivaRcaniVya; vaNiRsalAgada: \eng{unutterable torment} heVLalAgadaSuTx hiMse. \eng{unutterable joy} anivaRcaniVyavAda AnaMda. \eng{unutterable fool} heVLalAgadaSuTx mUKaR. 
\num{2} heVLalu ahaRvalalxda; avAcayx. 
\enum
\emng
\eentry

\bentry
\word{unutterableness} 
\pron{anfaTarabalfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} mAtiniMda heVLalAgadiruvike; avaNiRVyate; anivaRcaniVyate. 
\num{2} avAcayxte; heVLalu ahaRvalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unutterably} 
\pron{anfaTarabilx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} mAtiniMda heVLalAgade; avaNaRniVyavAgi; anivaRcaniVyavAgi. 
\num{2} heVLalAgada riVtiyalilx; vaNiRsalAgadaSuTx. 
\enum
\emng
\eentry

\bentry
\word{unuttered} 
\pron{anfaTaDfR} 
\gl{\gu}
\expl{}
\bmng
mAtiniMda heVLada; anukatx. 
\emng
\eentry

\bentry
\word{unvaccinated} 
\pron{anfvAYxkisxneVTiDf} 
\gl{\gu}
\expl{}
\bmng
siDubu hAkada. 
\emng
\eentry

\bentry
\word{unvalued} 
\pron{anfvAYxlUyxDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} beleyaLaLxdedxMdu eNisada; gaNane ilalxda; bele ilalxda. 
\num{2} bele-- kaTiTxlalxda, hAkada, namUdisada. 
\enum
\emng
\eentry

\bentry
\word{unvaried} 
\pron{anfveVriDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} badalAyisada; mApaRDisada; vayxtAyxsavilalxda. 
\num{2} veYvidhayxvilalxda; EkaparxkAravAda; oMdeV samanAda. 
\enum
\emng
\eentry

\bentry
\word{unvarnished} 
\pron{anfvAniRSfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vaNaRne \mo vugaLalilx) horanayavilalxda; baNaNx kaTaTxda; thaLukilalxda; saraLa; neVra: \eng{give an unvarnished account of what happened} EneVnu AyiteMbudanunx baNaNx kaTaTxde tiLisu: \eng{unvarnished truth} thaLuku paLukilalxda nijAMsha; shudadhx satAyxMsha. 
\num{2} merugeNeNx, vAniRSu, \mo vugaLanunx hacacxda; hora nuNupu, hoLapugaLilalxda; merugu koDada. 
\enum
\emng
\eentry

\bentry
\word{unvarying} 
\pron{anfveV{ri}iMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} badalAvaNe hoMdada; mApARDAgada; vayxtAyxsavAgada. 
\num{2} veYvidhayxvilalxda; EkaparxkAravAda; oMdeV samanAda. 
\enum
\emng
\eentry

\bentry
\word{unvaryingly} 
\pron{anfveV{ri}iMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} badalAvaNe hoMdadaMte; mApARDAgada riVtiyalilx. 
\num{2} veYvidhayxvilalxdaMte; EkaparxkAravAgi. 
\enum
\emng
\eentry

\bentry
\word{unvaryingness} 
\pron{anfveV{ri}iMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} avayxtayxsaniVyate; badalAvaNe hoMdadiruvike. 
\num{2} veYvidhayxvilalxdaMtiruvike; EkaparxkArate. 
\enum
\emng
\eentry

\bentry
\word{unveil} 
\pron{anfveVlf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} musuku sarisu; buruki tege. 
\num{2} (parxtime, BAvacitarx, \mo vugaLa) anAvaraNa mADu. 
\num{3} bayalu mADu; tiLiyapaDisu; parxkaTisu; bahiraMga mADu; horageDahu; raTuTx mADu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
musuku sarisu, tere; buruki kaLaciko. 
\emng
\eentry

\bentry
\word{unventilated} 
\pron{anfveMTileVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gALiyADada; vAyusaMcAravilalxda; gALige terediDada. 
\num{2} (parxshenx, ahavAlu, \mo vugaLa \vi) bahiraMgapaDisada; parxkaTamADada. 
\enum
\emng
\eentry

\bentry
\word{unverifiable} 
\pron{anfveripheYabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (nijaveMdu, sariyeMdu) daqDhapaDisalAgada; samathiRsalAgada. 
\num{2} saritapupx noVDalAgada; tALe hAki noVDalAgada. 
\num{3} pariVkiSxsalAgada; shoVdhisalAgada. 
\enum
\emng
\eentry

\bentry
\word{unverified} 
\pron{anfveripheYDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (nijaveMdu, sariyeMdu) daqDhapaDisada; samathiRsada. 
\num{2} sari tapupx noVDada; tALehAki noVDada. 
\num{3} pariVkiSxsada; shoVdhisada. 
\enum
\emng
\eentry

\bentry
\word{unversed} 
\pron{anfvasfRTx} 
\gl{\gu}
\expl{}
\bmng
(viSaya, kelasa, \mo vugaLalilx) nuriyada; paLagada; niSANxtanalalxda; nipuNanalalxda. 
\emng
\eentry

\bentry
\word{unviability} 
\pron{anfveYabiliTi} 
\gl{\nA}
\expl{}
\bmng
avayxvahAyaRte; kAyaRsAdhayxvalalxdiruvike; kAyaRrUpakekx taralu sAdhayxvAgadiruvike. 
\emng
\eentry

\bentry
\word{unviable} 
\pron{anfveYabflf} 
\gl{\gu}
\expl{}
\bmng
avayxvahAyaR; kAyaRsAdhayxvalalxda. 
\emng
\eentry

\bentry
\word{unviolated} 
\pron{anfveYaleVTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anulalxMGita; ulalxMGane mADada. 
\num{2} keDisada; ashudadhxgoLisada; apavitarxgoLisada. 
\num{3} atikarxmisada; atikarxmaparxveVsha mADada. 
\num{4} (heMgasina \vi) keDisada; shiVlaBaMga mADada; atAyxcArakekx oLagAgada. 
\enum
\emng
\eentry

\bentry
\word{unvisited} 
\pron{anfvisiZTiDf} 
\gl{\gu}
\expl{}
\bmng
BeVTimADirada. 
\emng
\eentry

\bentry
\word{unvitiated} 
\pron{anfviSiETiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} keDisada; kaluSitagoLisada. 
\num{2} anUjiRtavAgisada; nirathaRkagoLisada. 
\enum
\emng
\eentry

\bentry
\word{unvoiced} 
\pron{anfvAisfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} heVLada; vayxkatxpaDisada; tiLisada; nuDiyada; ucacxrisilalxda; anukatx; avayxkatx; mwnada: \eng{unvoiced complaints} bahiraMgavAgi heVLada dUrugaLu. 
\num{2} (\BAshA) aGoVSa; dhavxnitaMtuvanunx kaMpisade ucacxrisuva. 
\enum
\emng
\eentry

\bentry
\word{unwaged} 
\pron{anfveVjfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} veVtanarahita; veVtana paDeyada. 
\num{2} nwkariyilalxda; kelasavilalxda; kelasa kaLedukoMDa. 
\enum
\emng
\eentry

\bentry
\word{unwanted} 
\pron{anfvAMTiDf} 
\gl{\gu}
\expl{}
\bmng
beVDavAda; agatayxvilalxda; Avashayxkavilalxda; bayasada; anapeVkiSxta. 
\emng
\eentry

\bentry
\word{unwarily} 
\pron{anfveVrili} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} ecacxrike tapipx; ajAgarUkateyiMda. 
\num{2} asaDeDxyiMda; nilaRkaSxyXvAgi. 
\enum
\emng
\eentry

\bentry
\word{unwariness} 
\pron{anfveVrinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} ecacxrike tapipxruvike; ajAgarUkate; huSArAgi iradiruvike; beVhuSAru. 
\num{2} (apAyada eduru) asaDeDx; nilaRkaSxyX; alakaSxyX. 
\enum
\emng
\eentry

\bentry
\word{unwarlike} 
\pron{anfvArfleYkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} yudodhxVtAsxhavilalxda. 
\num{2} yudadhxyoVgayxvalalxda. 
\num{3} yoVdha lakaSxNavilalxda. 
\num{4} yudadhxkekx saMbaMdhisada; yudadhxsUcakavalalxda; yudadhxda bedarike hAkada. 
\enum
\emng
\eentry

\bentry
\word{unwarmed} 
\pron{anfvAmfRDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} atapatx; becacxge mADada; kAyisada; kAveVrada. 
\num{2} uderxVkagoLaLxda; utAsxhagoLaLxda; huriduMbisada. 
\enum
\emng
\eentry

\bentry
\word{unwarned} 
\pron{anfvAnfRDx} 
\gl{\gu}
\expl{}
\bmng
(modaleV) ecacxrike koDada. 
\emng
\eentry

\bentry
\word{unwarrantable} 
\pron{anfvAraMTabflf} 
\gl{\gu}
\expl{}
\bmng
samathiRsalAgada; asamathaRniVya. 
\emng
\eentry

\bentry
\word{unwarrantableness} 
\pron{anfvAraMTabflfnisf} 
\gl{\nA}
\expl{}
\bmng
samathiRsalAgadiruvudu; asamathaRniVyate. 
\emng
\eentry

\bentry
\word{unwarrantably} 
\pron{anfvAraMTabilx} 
\gl{\kirxvi}
\expl{}
\bmng
samathiRsalAgadaMte. 
\emng
\eentry

\bentry
\word{unwarranted} 
\pron{anfvAraMTiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anadhikaqta; adhikArayukatxvalalxda. 
\num{2} asamathaRniVyavAda; nirAdhAravAda. 
\enum
\emng
\eentry

\bentry
\word{unwary} 
\pron{anfveVri} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ecacxrike tapipxda; ajAgarUka; jAgarUkanalalxda. 
\num{2} (apAyadeduru) asaDeDxya; huSArAgirada; beVhuSArina; ajAgarxteya. 
\enum
\emng
\eentry

\bentry
\word{unwashed} 
\pron{anfvASfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\numi{1} 
\banum
\alnum{a} toLeyada; ogeyada; amAjiRta. 
\alnum{b} asAnxta; sAnxna mADirada. 
\eanum
\numie
\num{2} sadA koLakAgiruva. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{the great unwashed} (\AmA) janajaMguLi; janasAmAnayxru; asAnxtaloVka. 
\emng
\eentry

\bentry 
\word{unwatched} 
\pron{anfvAcfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pahare ilalxda; kAvalilalxda. 
\num{2} gamanisi noVDada; nigA iDada; kaNiNxTiTxrada. 
\enum
\emng
\eentry

\bentry
\word{unwatchful} 
\pron{anfvAcfPuZlf} 
\gl{\gu}
\expl{}
\bmng
ecacxravAgilalxda; jAgarUkanalalxda; ecacxradiMda gamanisada. 
\emng
\eentry

\bentry
\word{unwatered} 
\pron{anfvATaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} niVru -- hAkada, kaTaTxda, hAyisada, ereyada. 
\num{2} niVru berasada. 
\num{3} (danakarugaLa \vi) niVru -- kuDisada, toVrisada. 
\num{4} (haDagu, eMjinu) tamamx kaDAyige niVru tuMbikoLaLxda. 
\enum
\emng
\eentry

\bentry
\word{unwavering} 
\pron{anfveVvariMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oledADada; tUgADada; alAlxDada; niSakxMpa. 
\num{2} (seYnayxdaLa) tatatxrisada; sethxYyaRgeDada; hiMdeVTu hAkada. 
\num{3} (manasisxna \vi) (hiVgoV hAgoV eMdu) hiMdumuMdu noVDada; AMdoVlavAgada; hoyAdxDada. 
\num{4} (naMbike, dheYyaR) kadalada; sithxravAda; daqDhavAda. 
\enum
\emng
\eentry

\bentry
\word{unwaveringly} 
\pron{anfveVvariMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} kaMpisade; nishacxlavAgi; acaMcalavAgi; oledADade. 
\num{2} (seYnayx, daLa) tatatxrisade; hiMjariyade; dhaqtigeDade. 
\num{3} (manasisxna \vi) hiMdumuMdu noVDade; hoyAdxDade. 
\num{4} (naMbike, dheYyaR) kadalade; niSakxMpavAgi; acalavAgi. 
\enum
\emng
\eentry

\bentry
\word{unweaned} 
\pron{anfviVnfDx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} molehAlu biDisada, tapipxsada. 
\num{2} (beVre AkaSaRNe toVrisi aBAyxsa \mo vugaLiMda) biDisada; tapipxsada; beVre kaDege tirugisada. 
\enum
\emng
\eentry

\bentry
\word{unwearable} 
\pron{anfveVrabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} uDalAgada; toDalAgada. 
\num{2} samesalAgada; teVyisalAgada; jiVNaR mADalAgada. 
\enum
\emng
\eentry

\bentry
\word{unwearied} 
\pron{anfviariDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} baLalada; daNiyada; AyAsapaDada. 
\num{2} utAsxhaguMdada; beVsaragoLaLxda. 
\num{3} avishArxMtavAda; aviratavAda. 
\enum
\emng
\eentry

\bentry
\word{unweariedly} 
\pron{anfviariDfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} baLalade; AyAsapaDade. 
\num{2} utAsxhaguMdade. 
\num{3} avishArxMtavAgi; aviratavAgi. 
\enum
\emng
\eentry

\bentry
\word{unweariedness} 
\pron{anfviariDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} anAyAsa; baLaladiruvike; AyAsa paDadiruvike. 
\num{2} utAsxhaguMdadiruvike. 
\num{3} avishArxMtate. 
\enum
\emng
\eentry

\bentry
\word{unweary} 
\pron{anfviari} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{unwearied}{unwearied}. 
\emng
\eentry

\bentry
\word{unwearying} 
\pron{anfvia{ri}iMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} avishArxMtavAda; niraMtaravAda. 
\num{2} AyAsavuMTu mADada. 
\enum
\emng
\eentry

\bentry
\word{unwearyingly} 
\pron{anfvia{ri}iMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} avishArxMtavAgi; niraMtaravAgi. 
\num{2} AyAsavuMTumADadaMte. 
\enum
\emng
\eentry

\bentry
\word{unweave} 
\pron{anfviVvf} 
\gl{\sakirx}
\expl{}
\bmng
(neVyidxruvudara) heNige bicucx; neyegx bicucx; nUlunUlAgi biDisu. 
\emng
\eentry

\bentry
\word{unwed} 
\pron{anfveDf} 
\gl{\gu}
\expl{}
\bmng
maduveyAgada; avivAhita. 
\emng
\eentry

\bentry
\word{unwedded} 
\pron{anfveDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} maduve ilalxda; maduveyAgada. 
\num{2} (aBipArxya, tatatxvX, \mo vugaLige) niSeThx ilalxda; Asakatxnalalxda; kaTuTx biVLada; gaMTu biVLada. 
\enum
\emng
\eentry

\bentry
\word{unweddedness} 
\pron{anfveDiDfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} avivAhita sithxti; maduveyAgadiruvike. 
\num{2} (aBipArxya, tatatxvX, \mo vugaLige) niSeThx ilalxdiruvike; kaTuTx biVLadiruvike; gaMTu biVLadiruvike. 
\enum
\emng
\eentry

\bentry
\word{unweeded} 
\pron{anfviVDiDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kaLe tegeyada; kaLe kiVLada. 
\num{2} (\rUpa) kaLapeyAgiruvudanunx tegedu hAkada; shoVdhisada. 
\enum
\emng
\eentry

\bentry
\word{unweighed} 
\pron{anfveVDf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vasutxgaLa \vi) tUka mADada; tUgi noVDada. 
\num{2} sariyAgi parishiVlisada; avasarada. 
\enum
\emng
\eentry

\bentry
\word{unwelcome} 
\pron{anfvelfkamf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} sAvxgatisada; AdaradiMda baramADikoLaLxda; saMtoVSadiMda edurugoLaLxda. 
\num{2} iSaTxvilalxda; pirxyavAgirada. 
\enum
\emng
\eentry

\bentry
\word{unwelcomely} 
\pron{anfvelfkamfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} sAvxgata koVrade; sAvxgatisade. 
\num{2} iSaTxvilalxdaMte; pirxyavAgirade. 
\enum
\emng
\eentry

\bentry
\word{unwelcomeness} 
\pron{anfvelfkamfnisf} 
\gl{\nA}
\expl{}
\bmng
iSaTxvilalxdiruvike; pirxyavAgadiruvike. 
\emng
\eentry

\bentry
\word{unwell} 
\pron{anfvelf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} savxsathxvilalxda; asavxsathx; AroVgayxvilalxda. 
\num{2} (savxlapx) kAyile Agiruva. 
\enum
\emng
\eentry

\bentry
\word{unwept} 
\pron{anfvepfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} yArU -shoVkisada, duHKisada; ashoVcita: \eng{an unwept loss} yArU shoVkisada naSaTx. 
\num{2} (kaNiNxVrina \vi) aLada; kaNiNxVru surisada. 
\enum
\emng
\eentry

\bentry
\word{unwetted} 
\pron{anfveTiDf}
\gl{\gu}
\expl{}
\bmng
odedx mADada; akelxVdayx; toVyisada; nenesada. 
\emng
\eentry

\bentry
\word{unwhipped} 
\pron{anfvipfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} cAvaTiyiMda hoDeyada; tiVDada. 
\num{2} (\birx) pakaSxda (pAliRmeMTinalilx hAjaridudx mata calAyisabeVkeMba) nideRVshanakekx oLapaDada. 
\enum
\emng
\eentry

\bentry
\word{unwholesome} 
\pron{anfhoVlfsamf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} anAroVgayxkaravAda; deYhika yA neYtika AroVgayxkekx oLeLxyadalalxda yA hAnikaravAda. 
\num{2} anAroVgayxviruvaMte kANuva; anAroVgayx lakaSxNada. 
\enum
\emng
\eentry

\bentry
\word{unwholesomely} 
\pron{anfhoVlfsamfli} 
\gl{\kirxvi}
\expl{}
\bmng
anAroVgayxkaravAda; (deYhika yA neYtika) AroVgayxkekx -- oLeLxyadalalxdaMte yA hAnikaravAgi. 
\emng
\eentry

\bentry
\word{unwholesomeness} 
\pron{anfhoVlfsamfnisf} 
\gl{\nA}
\expl{}
\bmng
anAroVgayxkarate; (deYhika yA neYtika) AroVgayxkekx -- oLeLxyadalalxdiruvike yA hAnikaravAgiruvike. 
\emng
\eentry

\bentry
\word{unwieldily} 
\pron{anfviVliDxli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} oDoDxDADxgi; nAjUkilalxde; aMdageVDAgi. 
\num{2} (gAtarx, AkAra yA tUkada kAraNa) nivaRhisalu kaSaTxvAgi; nivaRhisalu sulaBavAgirade; anukUlavAgirade. 
\enum
\emng
\eentry

\bentry
\word{unwieldiness} 
\pron{anfviVliDxnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} oDoDxDADxgiruvudu; aMdageTiTxruvudu. 
\num{2} anivARhayxte; baLakege, hiDitakekx -- anukUlavAgilalxdiruvudu, nivaRhisalAgadaMtiruvudu. 
\enum
\emng
\eentry

\bentry
\word{unwieldy} 
\pron{anfviVliDx} 
\gl{\gu}
\expl{(\tara\ \eng{unwieldier}, \tama\ \eng{unwieldiest}). }
\bmng
\bnum
\num{1} oDoDxDADxda; nAjUkilalxda; aMdageVDAda. 
\num{2} nivaRhisalu anukUlavalalxdaSuTx -- doDaDxdAda, vikAravAda yA tUkavAda. 
\enum
\emng
\eentry

\bentry
\word{unwilling} 
\pron{anfviliMgf} 
\gl{\gu}
\expl{}
\bmng
iSaTxvilalxda; manasisxlalxda; olalxda. 
\emng
\eentry

\bentry
\word{unwillingly} 
\pron{anfviliMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
iSaTxvilalxde; manasisxlalxde; olalxda manasisxniMda. 
\emng
\eentry

\bentry
\word{unwillingness} 
\pron{anfviliMgfnisf} 
\gl{\nA}
\expl{}
\bmng
iSaTxvilalxdiruvike; manasisxlalxdiruvudu; olalxda manasisxniMdiruvudu. 
\emng
\eentry

\bentry
\word{unwind} 
\pron{anfveYnfDx} 
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unwound}). }


\noindent
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (suruLi \mo vanunx) bicucx. 
\num{2} manasasxnunx haguragoLisu, manasisxna otatxDa kaDame mADu; nemamxdigoLisu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
\bnum
\num{1} (suruLi \mo vu) bicicxko. 
\num{2} (manasisxna \vi) haguravAgu; otatxDa kaDame mADiko; ArAmavAgu; nemamxdi paDeduko. 
\enum
\emng
\eentry

\bentry
\word{unwinking} 
\pron{anfviMkiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} repepx hAkada; eveyikakxda. 
\num{2} ecacxradiMdiruva; jAgarUka. 
\enum
\emng
\eentry

\bentry
\word{unwinkingly} 
\pron{anfviMkiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} repepx baDiyade; eveyikakxde. 
\num{2} ecacxradiMda; jAgarUkanAgi. 
\enum
\emng
\eentry

\bentry
\word{unwinnable} 
\pron{anfvinabflf} 
\gl{\gu}
\expl{}
\bmng
gelalxlAgada; ajeVya. 
\emng
\eentry

\bentry
\word{unwisdom} 
\pron{anfvisfZDamf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} aviveVka; vicArahiVnate. 
\num{2} mwDhayx; mUKaRtana; muTAThxLatana. 
\enum
\emng
\eentry

\bentry
\word{unwise} 
\pron{anfveYsfZ} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} budidhxhiVna. 
\num{2} aviveVkada; mUKaRtanada. 
\enum
\emng
\eentry

\bentry 
\word{unwisely} 
\pron{anfveYsfZli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} aviveVkadiMda; vicArashUnayxvAgi. 
\num{2} mUKaRtanadiMda. 
\enum
\emng
\eentry

\bentry
\word{unwished} 
\pron{anfviSfTx} 
\gl{\gu}
\expl{}
\bmng
aniciPxta; koVridadxlalxda; hAreYsada; bayasada; Ase paDada. 
\emng
\eentry

\bentry
\word{unwithered} 
\pron{anfvidaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oNagada; bADihoVgada. 
\num{2} inUnx utAsxhapUNaRvAgiruva. 
\enum
\emng
\eentry

\bentry
\word{unwitnessed} 
\pron{anfviTinxsfTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} noVDada; viVkiSxsada; (iMdirxyagaLa mUlaka) garxhisada. 
\num{2} sAkiSxya rujuvilalxda. 
\enum
\emng
\eentry

\bentry
\word{unwitting} 
\pron{anfviTiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ajacnx; tiLiyada; ariyada: \eng{an unwitting offender} ariyada aparAdhi. 
\num{2} budidhxpUvaRkavalalxda; udedxVshapUvaRkavalalxda. 
\enum
\emng
\eentry

\bentry
\word{unwittingly} 
\pron{anfviTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} tiLiyade; ariyade. 
\num{2} budidhxpUvaRkavalalxdaMte; udedxVshavilalxde: \eng{If I hurt your feelings, it was unwittingly} nAnu ninanx manasusx noVyisidadxre, adu keVvala anudidxSaTxvAdudu. 
\enum
\emng
\eentry

\bentry
\word{unwittingness} 
\pron{anfviTiMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} tiLiyadiruvike. 
\num{2} budidhxpUvaRkavalalxdiruvike. 
\enum
\emng
\eentry

\bentry
\word{unwomanliness} 
\pron{anfvumanflinisf} 
\gl{\nA}
\expl{}
\bmng
sitxrXVsahajavalalxdiruvike; heMgasige takukxdalalxdiruvike. 
\emng
\eentry

\bentry
\word{unwomanly} 
\pron{anfvumanfli} 
\gl{\gu}
\expl{}
\bmng
heMgasige takukxdalalxda; sitxrXVsahajavalalxda. 
\emng
\eentry

\bentry
\word{unwonted} 
\pron{anfvoVMTiDf} 
\gl{\gu}
\expl{}
\bmng
rUDhiyalalxda; vADikeyalalxda; mAmUlanunx anusarisada; sAmAnayx padadhxtiyalilxlalxda. 
\emng
\eentry

\bentry
\word{unwontedly} 
\pron{anfvoVMTiDfli} 
\gl{\kirxvi}
\expl{}
\bmng
rUDhiyaMtalalxde; mAmUlanunx anusarisade. 
\emng
\eentry

\bentry
\word{unwontedness} 
\pron{anfvoVMTiDfnisf} 
\gl{\nA}
\expl{}
\bmng
rUDhiyaMtalalxdiruvike; mAmUlanunx anusarisadiruvike. 
\emng
\eentry

\bentry
\word{unwooded} 
\pron{anfvuDiDf} 
\gl{\gu}
\expl{}
\bmng
giDamaragaLilalxda; kADu beLeyada. 
\emng
\eentry

\bentry
\word{unwooed} 
\pron{anfvUDf}
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pirxVtisada; pirxVti toVrisada; moVhisada. 
\num{2} (\kanmu\ maduveya udedxVshadiMda) anusaraNe mADada; anunayisada; OleYsada; pusalAyisada. 
\enum
\emng
\eentry

\bentry
\word{unworkability} 
\pron{anfvakaRbiliTi} 
\gl{\nA}
\expl{}
\bmng
kAyARtamxkavalalxdiruvike; kAyaRrUpakekx taralAgadiruvike. 
\emng
\eentry

\bentry
\word{unworkable} 
\pron{anfvakaRbflf} 
\gl{\gu}
\expl{}
\bmng
kAyARtamxkavalalxda; kAyaRrUpakekx taralAgada. 
\emng
\eentry

\bentry
\word{unworkableness} 
\pron{anfvakaRbflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{unworkability}{unworkability}. 
\emng
\eentry

\bentry
\word{unworkably} 
\pron{anfvakaRbilx} 
\gl{\kirxvi}
\expl{}
\bmng
kAyaRrUpakekx taralAgadaMte. 
\emng
\eentry

\bentry
\word{unworked} 
\pron{anfvakfRTx} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} inUnx AkAra, rUpa -- koDada; rUpisada. 
\num{2} kelasakekx toDagisada; baLasada; upayoVga mADada. 
\num{3} kAyaRrUpakekx tarada; kAyaRgoLisada. 
\enum
\emng
\eentry

\bentry
\word{unworkmanlike} 
\pron{anfvakfRmanfleYkf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} oLeLxya kelasagAranige takukxdalalxda; kasubudAranadalalxda; sahajavalalxda; kelasagArana lakaSxNavilalxda. 
\num{2} keTaTxdAgi mADida; aDaDxkasabiya. 
\enum
\emng
\eentry

\bentry
\word{unworldliness} 
\pron{anfvalfDxRlinisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} alwkikate. 
\num{2} AdhAyxtimxkate. 
\num{3} pArxpaMcika jAcnxnavilalxdiruvike; vayxvahArajAcnxnashUnayxte. 
\enum
\emng
\eentry

\bentry
\word{unworldly} 
\pron{anfvalfDxRli} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pArxpaMcikavalalxda; alwkika. 
\num{2} AdhAyxtimxka. 
\num{3} pArxpaMcika, vayxvahAra -- jAcnxnavilalxda. 
\enum
\emng
\eentry

\bentry
\word{unworn} 
\pron{anfvoVnfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (vasutxgaLa \vi) samedilalxda; jiVNaRvAgilalxda. 
\num{2} (uDupina, ABaraNada \vi) toTuTxkoMDilalxda; toTiTxlalxda; dharisilalxda. 
\enum
\emng
\eentry

\bentry
\word{unworried} 
\pron{anfvariDf} 
\gl{\gu}
\expl{}
\bmng
nishicxMta; nirAtaMkanAda; tale keDisikoLaLxda; ciMtAgarxsatxnalalxda. 
\emng
\eentry

\bentry
\word{unworshipped} 
\pron{anfvaSiRpfTx} 
\gl{\gu}
\expl{}
\bmng
pUjisada; ArAdhisada. 
\emng
\eentry

\bentry
\word{unworthily} 
\pron{anfvadiRli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (obabxna yoVgayxtege) salalxda riVtiyalilx; ayoVgayxvAgi. 
\num{2} tirasAkxrAhaRvAgi; tucaCxvAgi; heVyavAgi. 
\enum
\emng
\eentry

\bentry
\word{unworthiness} 
\pron{anfvadiRnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} yoVgayxvalalxdAgiruvudu; ayoVgayxvAgiruvudu. 
\num{2} tirasAkxrAhaRte; tucaCxte; alapxtana; kuSxdarxte. 
\enum
\emng
\eentry

\bentry
\word{unworthy} 
\pron{anfvadiR} 
\gl{\gu}
\expl{(\tara\ \eng{unworthier}, \tama\ \eng{unworthiest}). }
\bmng
\bnum
\num{1} (vayxkitxya yoVgayxtege) tagada; yoVgayxvalalxda; ucitavalalxda. 
\num{2} ayoVgayx; anahaR. 
\num{3} tirasAkxrAhaRvAda; tucaCx; alapx; heVya; kuSxdarx; nikaqSaTx. 
\enum
\emng
\eentry

\bentry
\word[unwound(1)]{unwound} 
\pron{anfvwMDf} 
\gl{\gu}
\expl{}
\bmng
suruLi bicicxda. 
\emng
\eentry

\bentry
\word[unwound(2)]{unwound} 
\pron{anfvwMDf} 
\gl{\kirx}
\expl{}
\bmng
\eng{unwind} eMbudara \BU\ matutx \BUkaq\ rUpa. 
\emng
\eentry

\bentry
\word{unwounded} 
\pron{anfvUMDiDf} 
\gl{\gu}
\expl{}
\bmng
noVvAgada; gAyavAgada; akaSxta. 
\emng
\eentry

\bentry
\word{unwoven} 
\pron{anfvoVvanf} 
\gl{\gu}
\expl{}
\bmng
heNige bicicxda; neyegx bicicxda. 
\emng
\eentry

\bentry
\word{unwrap} 
\pron{anfrAYxpf} 
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unwrapped}; \vakaq\ \eng{unwrapping}). }


\noindent
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} kaTiTxruvudanunx bicucx; sutitxruvudanunx bicucx. 
\num{2} tere; bicucx; musuku tege. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
bicicxko; tereduko. 
\emng
\eentry

\bentry
\word{unwrinkled} 
\pron{anfriMkflfDx} 
\gl{\gu}
\expl{}
\bmng
nerigeyAgada; sukakxda; sukikxna geregaLilalxda. 
\emng
\eentry

\bentry
\word{unwritable} 
\pron{anfreYTabflf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} bareyalu sAdhayxvAgada. 
\num{2} leVKanAhaRvalalxda; leVKana yoVgayxvalalxda. 
\enum
\emng
\eentry

\bentry
\word{unwritten} 
\pron{anfriT(Tf)nf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} bareyada; aliKita; barediDada: \eng{the unwritten songs of the countryfolk} aliKita (barehadalilxrada) jAnapada giVtegaLu. 
\num{2} (kAnUnu \mo vugaLa \vi) aliKita nAyxya; saMparxdAya nAyxya; liKitashAsanagaLa AdhAravilalxdidadxrU nAyxyAlayada tiVpuRgaLanonxV samAjada saMparxdAyavanonxV AdhAravAguLaLx nAyxya. 
\enum
\emng
\eentry

\bentry
\word{unwrought} 
\pron{anfrATf} 
\gl{\gu}
\expl{}
\bmng
(loVhagaLa \vi) 
\banum
\alnum{a} AkAra tALuvaMte kuTaTxda, baDiyada. 
\alnum{b} aMtima -- rUpa koDada, sithxtige tarada. 
\eanum
\emng
\eentry

\bentry
\word{unwrung} 
\pron{anfraMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} tirucada; hiMDada; hisukada; hiMDi tegeyada. 
\num{2} balAtAkxradiMda -- kiVLada, seLeyada, horaDisada. 
\enum
\emng
\eentry

\bentry
\word{unyielding} 
\pron{anfyiVliDxMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (otatxDa \mo vugaLige) maNiyada; bagagxda; sagagxda. 
\num{2} paTuTx hiDidiruva; moMDutanada. 
\enum
\emng
\eentry

\bentry
\word{unyieldingly} 
\pron{anfyiVliDxMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} (otatxDa \mo vugaLige) maNiyade; bagagxde; sagagxde. 
\num{2} paTuTx hiDidu; moMDutanadiMda; sulaBavAgi biTuTxkoDade; baDapeTiTxge bagagxde. 
\enum
\emng
\eentry

\bentry
\word{unyieldingness} 
\pron{anfyiVliDxMgfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (otatxDa \mo vugaLige) maNiyadiruvudu; bagagxdiruvudu; sagagxdiruvudu. 
\num{2} paTuTx hiDidiruvike; moMDutanadiMdiruvudu. 
\enum
\emng
\eentry

\bentry
\word{unyoke} 
\pron{anfyoVkf} 
\gl{\sakirx}
\expl{}
\bmng
noga -- bicucx, tege, kaLacu. 
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
(mADutitxruva) kelasa nililxsu; kelasadiMda viramisu. 
\emng
\eentry

\bentry
\word{unzip} 
\pron{anfsiZpf} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{unzipped}; \vakaq\ \eng{unzipping}).}
\bmng
\bnum
\num{1} jipf bicucx. 
\num{2} (kaMpUyxTarf) kugigxsida `pheYl' nunx higigxsu; `anfsiZpf' mADu. 
\enum
\emng
\eentry

\bentry
\word[up(1)]{up} 
\pron{apf} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} meVle; meVlakekx; meVlugaDe (yalilx); meVlugaDege: \eng{high up in the air} (meVle) AkAshadalilx. \eng{what are they doing up there?} alilx meVle avaru Enu mADutitxdAdxre? \eng{lives four floors up} meVle nAlakxneya aMtasitxnalilx vAsisutAtxne. \eng{went up three places in class} avanu taragatiyalilx mUru sAthxna(daSuTx) meVlakekxVrida. 
\num{2} (\AmA) alilx muMdugaDe; alilx meVlugaDe: \eng{went up front} muMdina sAlige hoVda. 
\num{3} (hiMde) yAvudoV kAladavarege. 
\num{4} nadiya meVlakekx, mUladavarege: \eng{follow a stream up to its source} nadiya meVlakekx, mUladavarege (jADu hiDidu) hoVgu. 
\num{5} oLanADinalilx. 
\num{6} (jUjukudureya savArana \vi) kudure Eri, hatitx. 
\num{7} oMdu samathaR yA kAyARtamxkavAda yA parxgatiya sithxtiyalilx yA sithxtige: \eng{stirred up trouble} samaseyx huTuTx hAkida. \eng{the house is up for sale} mane mArATakikxde. \eng{the hunt is up} beVTe yA shoVdhane shuru Agide. 
\num{8} kalitu yA tiLidu: \eng{is well up in French} pherxMcanunx cenAnxgi kalitukoMDidAdxne. 
\num{9} (rasetx \mo vugaLa \vi) ripeVriyalilx; durasitxnalilx. 
\numi{10} 
\banum
\alnum{a} (ATada gelalxMka yA sokxVrina \vi) sokxVrf boVDiRnalilx dAKale mADi, noVMdaNisi; yAvudoV oMdu samayadalilx oTuTx motatxvAgi; oMdu taMDa yA obabx ATagArana inonxMdu taMDa yA ATagArana motatxkikxMta miVrisida motatxvAgi: \eng{our team is 3 goals up} namamx taMDa mUru goVlu gaLiside yA mUru goVlugaLiMda muMdide. 
\alnum{b} (ATagArana \vi) (hAge inonxMdu taMDa yA ATagAranigiMta) hecucx gelalxMka gaLisi. 
\eanum
\numie
\num{11} nAyxyAlaya yA nAyxyAdhiVshana muMde hAjarupaDisi: \eng{was brought up for speeding} ati veVgavAgi ODisida kAraNakAkxgi nAyxyAlayada muMde hAjarupaDisalAyitu. 
\num{12} (\birx) (yAvudoV oMdu) bele utatxma sithxtiyalilx Eri: \eng{Rs.5 up on the transaction} (vAyxpArada) vahivATinalilx 5 rU.gaLaSuTx muMdAgi, hecAcxgi. 
\num{13} (padAthaR \mo vugaLa \vi) bele Eri: \eng{meat is up this week} I vAra mAMsada bele Eride. 
\num{14} (nATakashAleya parade) (raMgasathxLa kANisuvaMte) parade -- meVlakekx Eri, meVledudx. 
\num{15} (mAtanADuvavanu niMtiruva yA sUcisida) sathxLada yA kAladavarege baMdu, samiVpisi: \eng{the child came up and asked me the time} magu nananx baLi baMdu `eSuTx gaMTe' eMdu keVLitu. \eng{went straight up to the door} neVravAgi bAgilavarege hoVda. \eng{has been up till now} iduvarege cenAnxgi, AroVgayxvAgidAdxne. 
\numi{16} 
\banum
\alnum{a} (hAsige \mo vugaLiMda) meVlakekxdudx. 
\alnum{b} (piVThadalilx kuLitavanu, maMDiyUri kuLitavanu) meVledudx niMtu: \eng{sprang up from his seat} avanu piVThadiMda Pakakxne meVledudx niMta. 
\alnum{c} (caTuvaTikeya sUcaneyAgi) meVledudx; meVlakekx ebibxsi (modalAdAdxgi): \eng{put your umbrella up} ninanx Catirx bicicx meVletutx. 
\alnum{d} nililxsi; niMtiruva sithxtiyalilx: \eng{stood it up} adanunx nililxsida. 
\eanum
\numie
\num{17} (\BAavayx dalilx) edudxniMtu; meVlakekx niMtu. 
\num{18} parisharxma paDedu; kalitu; shikaSxNahoMdi: \eng{he is well up in mathematics} avanige gaNitadalilx oLeLxya parisharxmavide. 
\num{19} muMduvaridu; ELigeyAgi: \eng{he is up with the leaders} avanu muMduvaridu muKaMDara jate idAdxne. 
\num{20} (\nw) (cukAkxNiya \vi) gALigedurAgi. 
\num{21} daMgeyedudx. 
\num{22} (beVTe \mo vu) muMduvariyutatx; naDeyutatx. 
\num{23} (sUyaR, caMdarx, \mo vu) udayisi: \eng{the moon is up} caMdarx mUDidAdxne. 
\num{24} pUtiRyAgi yA pariNAmakAriyAgi: \eng{eat up} tiMdubiDu. \eng{drink up} kuDidubiDu. \eng{sawn up into logs} dimimxgaLAgi koyudx biTaTx. 
\num{25} aDakavAda, sheVKarisida yA suBadarxvAda sithxtige taMdu: \eng{pack up} poTaTxNa kaTuTx. \eng{bind up} kaTuTxgaLanAnxgi mADu. \eng{store up} sheVKara mADu. \eng{lock up} surakiSxtavAgi, BadarxvAgi -- biVga hAki BadarxpaDisu. 
\numi{26} meVlinadu eMdu BAvitavAda sathxLakekx yA sathxLadalilx, \kanmu\ 
\banum
\alnum{a} utatxrakekx, utatxradalilx: \eng{is up in Kashmir} utatxrada kAshimxVradalilx. 
\alnum{b} (\birx) parxmuKa nagarakekx yA vishavxvidAyxnilayakekx: \eng{went up to London} laMDanf nagarakekx hoVda. \eng{went up to Oxford} AkfsxphaDfR vishavxvidAyxnilayakekx seVrida. 
\eanum
\numie
\num{27} sidadhxpaDisida yA apeVkiSxta sithxtige: \eng{wound up the watch} gaDiyArakekx kiVlukoTaTx. 
\num{28} (gaNakayaMtarxda \vi) naDeyutatx; baLakege sidadhxvAda. 
\num{29} hecucx gaTiTxyAgi yA sapxSaTxvAgi: \eng{speak up} gaTiTxyAgi yA sapxSaTxvAgi heVLu, mAtanADu. 
\num{30} mugidu hoVgi; pUtiRyAgi; sigadaMtAgi: \eng{time is up} kAla mugiduhoVyitu, hotAtxyitu; veVLe miVritu. 
\num{31} hAsigeyiMda edudx: \eng{are you up yet?} niVnu inUnx edidxdidxVyA? 
\num{32} (\kanmu\ asAdhAraNavAgi yA aniriVkiSxtavAgi) naDeyatatx; Agutatx; saMBavisutatx: \eng{something is up} EnoV Agutitxde, naDeyutitxde. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{be all up with} (vayxkitxge) vinAshakaravAgi; pUtiR hAnikaravAgi; Baravase, niriVkeSx ilalxdaMte: \eng{it is all up with him} (\AmA) (nirAsheya sUcaneyAgi) avana kathe pUreYsitu. avana kathe pUtiR mugidaMteyeV. 
\num{2} \eng{something is up} (\AmA) EnoV kAdide; yAvudoV asAdhAraNavAdudu yA anapeVkiSxtavAdudu naDeyuvudaralilxde, saMBavisuvudaralilxde. 
\numi{3} \eng{up against} 
\banum
\alnum{a} hatitxravAgi; samiVpadalilx. 
\alnum{b} saMpakaRdalilx yA saMpakiRsi. 
\alnum{c} (\AmA) muKAmuKiyAgi; edurAgi; edurinalilx: \eng{up against a problem} samaseyxyanunx edurisi. 
\eanum
\numie
\num{4} \eng{up against it.} (\AmA) vipariVta kaSaTxkekx sikikx. 
\num{5} \eng{up and} \hyperref{kandict_a.pdf}{A}{about(1) nuga(6)}{$^1$about} (\engit{or} \eng{doing).} 
\numi{6} \eng{up and down} 
\banum
\alnum{a} atitxtatx; alilxMdililxge; keLagiMda meVlakekx: \eng{marching up and down} atitxtatx naDedADutatx. 
\alnum{b} elelxlUlx; elalx kaDe: \eng{I have looked for it up and down} adakAkxgi nAnu elalx kaDe huDukidedxVne. 
\alnum{c} (\AmA) AroVgayx yA utAsxha ErupeVrAgutatx. 
\eanum
\numie
\numi{7} \eng{up for} 
\banum
\alnum{a} (nideRVshisuva belege) siguva; dorakuva. 
\alnum{b} (hudedx \mo vugaLige) parigaNaneyalilxruva. 
\eanum
\numie
\num{8} \eng{up in} \hyperref{kandict_a.pdf}{A}{arms(2) nuga(5)}{$^2$arms}. 
\numi{9}\eng{up to} 
\banum
\alnum{a} varege; tanaka: \eng{up to the present} iduvarege; ililxyatanaka. 
\alnum{b} iSuTx saMKeyxya tanaka, saMKeyxya oLage: \eng{can take up to six guests} Arakekx miVradaSuTx atithigaLanunx nAnu baramADikoLaLxbalelx. 
\alnum{c} (iSuTx) saMKeyx yA motatxkekx miVradaMte: \eng{sums up to Rs. 5} aidu rUpAyigaLige miVradaSuTx motatx. 
\alnum{d} (yAvudoV kelasa) mADuva sithxtiyalilx, shakatxnAgi: \eng{am not up to walking 20 miles} nAnu ipapxtutx meYli naDeyuva sithxtiyalilxlalx. 
\alnum{e} maTaTxkekx sariyAgi: \eng{not up to your usual standard} nimamx mAmUli maTaTxkekx EralAradu, baralAradu. 
\alnum{f} oMdu kelasadalilx toDagiru, uduyxkatxnAgiru: \eng{what have you been up to?} niVnu Enu mADutAtx ididxVye? 
\eanum
\numie
\num{10} \eng{up to} \hyperref{kandict_d.pdf}{D}{date(2) nuga(3)}{$^2$date}. 
\num{11} \eng{up to one's} \hyperref{kandict_t.pdf}{T}{trick pagu(7)}{tricks}. 
\num{12} \eng{up to} \hyperref{kandict_s.pdf}{S}{snuff(3) pagu(3)}{$^3$snuff}. 
\num{13} \eng{up to the} \hyperref{kandict_k.pdf}{K}{knocker nuga(2)}{knocker}. 
\num{14} \eng{up to the} \hyperref{kandict_m.pdf}{M}{mark(1) pagu(16)}{$^1$mark}. 
\num{15} \eng{up to the} \hyperref{kandict_m.pdf}{M}{minute(1) pagu(3)}{$^1$minute}. 
\num{16} \eng{up to us} namamx kelasa; namage seVrida kelasa: \eng{it is up to us to do something} Iga nAvu EnAdarU mADaleVbeVku (Iga EnAdarU mADuvudu namage seVridudx). 
\num{17} \eng{up with (it)} (\BAavayx) (nidiRSaTx vayxkitxge yA vasutxvige beMbala sUcisuvalilx) etutx! ELu! meVlakekxtutx! jaya \mo vu Agali! adanenxtutx; adanunx meVlakekx tege: \eng{up with the Revolution}! kArxMti cirAyuvAgali! \eng{up with you} meVleVLu; edudx nilulx. 
\numi{18} \eng{what's up?} (\AmA) 
\banum
\alnum{a} Enu Agutitxde? 
\alnum{b} Enu samAcAra? 
\eanum
\numie
\enum
\emng
\eentry

\bentry 
\word[up(2)]{up} 
\pron{apf} 
\gl{\upa}
\bmng
\bnum
\num{1} meVle; meVlugaDege; meVlakekx; keLaginiMda meVlinavarege: \eng{climbed up the ladder} ENiya meVle hatitxda. \eng{up the hill} beTaTxda meVlakekx. \eng{smoke goes up the chimney} hogekoLaviyalilx hoge meVleVri sAgutatxde. 
\num{2} (nadiya) mUladavarege. 
\hyperdef{U}{up(2)3}{} 
\num{3} (rasetx \mo vugaLa) udadxkUkx: \eng{walk up the street} rasetxyudadxkUkx naDe. 
\num{4} meVluBAgadalilx: \eng{lives farther up the road} rasetxyalilx inUnx savxlapx muMde, meVlugaDe vAsisutAtxne. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{sitting up a tree} marada meVleVri, tudiyalilx kuLitu. 
\num{2} \eng{up and down} (\rUpa) (yAvudoV) sathxLakekx matutx alilxMda hiMdakekx. 
\numi{3} \eng{up hill and down dale} 
\banum
\alnum{a} (parxyAsada parxyANadalilx) haLaLx tiTuTx elAlx; beTaTx hatitx kaNive iLidu. 
\alnum{b} (\rUpa) (yAvudeV saninxveVsha \mo vugaLanunx) edurisutAtx; heVgeMdare hAge naDedu koLuLxtAtx. 
\eanum
\numie
\num{4} \eng{up stage} raMgasathxLada -- hiMBAgadalilx, hiMbadiyalilx, meVluBAgadalilx. 
\num{5} \eng{up the} \hyperref{kandict_p.pdf}{P}{pole(1) pagu(2)}{$^1$pole}. 
\num{6} \eng{up the} \hyperref{kandict_s.pdf}{S}{spout(2) pagu}{$^2$spout}. 
\enum
\emng
\eentry

\bentry
\word[up(3)]{up} 
\pron{apf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} meVlumxKavAgi hoVguva; meVlugaDe hoVguva; UdhavxRgAmi: \eng{up stroke} meVlumxKada hoDeta. 
\num{2} (\birx) rAjadhAniyatatx yA keVMdarxdatatx parxyANa mADuva: \eng{the up train} rAjadhAniyatatx hoVguva reYlu. 
\num{3} (biyarf \mo vugaLa \vi) ukekxVLuva; burugeVLuva. 
\enum
\emng
\eentry

\bentry
\word[up(4)]{up} 
\pron{apf} 
\gl{\nA}
\expl{}
\bmng
adaqSaTxda -- hoDeta, avadhi, dese. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\numi{1} \eng{on the up and up} (\AmA) 
\banum
\alnum{a} (\birx) EkaparxkAra utatxmagoLuLxtitxruva. 
\alnum{b} (\kanmu\ \ame) satayxvAgiruva; pArxmANikavAgiruva. 
\eanum
\numie
\num{2} \eng{up and over} (bAgilu \mo vugaLa \vi) meVlakekxtitx maTaTxvAgi hiMtaLaLxbahudAda. 
\numi{3} \eng{ups and downs} 
\banum
\alnum{a} haLaLxtiTuTxgaLu; nelada EriLita. 
\alnum{b} (\rUpa) adaqSaTxda EriLita, ELubiVLugaLu. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[up(5)]{up} 
\pron{apf} 
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{upped}; \vakaq\ \eng{upping}). }


\noindent
\gl{\sakirx}
\expl{}
\bmng
(\AmA) Erisu; hecicxsu: \eng{upped all their prices} avugaLa belegaLanenxlAlx ErisibiTiTxru. 
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
ededxVLu; Pakakxnedudx EnAdarU heVLu, mADu: \eng{he ups and says} avanu Pakakxnedudx heVLutAtxne. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{up with} etutx; ebibxsu; meVlakekxtutx: \eng{he upped with his stick} koVliniMda meVlakekxtitxda. 
\num{2} \eng{up} \hyperref{kandict_s.pdf}{S}{stick(2) pagu(6)}{$^2$sticks}. 
\enum
\emng
\eentry

\bentry
\word{up-} 
\pron{apf-} 
\gl{\pUparx}
\expl{}
\bmng
meVlf; meVlakekx; meVle: 
\bnum
\num{1} \kirxvi gaLanunx racisuva \pUparx: \eng{upcurved; update.} 
\num{2} \kirxvi gaLanunx hAgU \gu gaLanunx racisalu nAmapadagaLige seVrisuva \upa vAgi: \eng{upcountry, uphill.} 
\num{3} nAmapadagaLanunx visheVSisuva \gu vAgi: \eng{upland, upstroke.} 
\enum
\emng
\eentry

\bentry
\word{up-anchor} 
\pron{apfAYxMkarf} 
\gl{\sakirx}
\expl{}
\bmng
(\nw) laMgaru etutx. 
\emng
\eentry

\bentry
\word{up-and-coming} 
\pron{apf aMDf kamiMgf} 
\gl{\gu}
\expl{}
\bmng
(\AmA) yashasivxyAguva BaravaseyuLaLx; parxgati sAdhisi ELigege baruvaMtiruva. 
\emng
\eentry

\bentry
\word{Upanishad} 
\pron{UpAYxniSAYxDf} 
\gl{\nA}
\expl{}
\bmng
upaniSatutx; veVdAMta. 
\emng
\eentry

\bentry
\word{upas} 
\pron{yUpasf} 
\gl{\nA}
\bmng
\hypertarget{upas(1)}{} 
\bnum
\numi{1} yUpasf: 
\banum
\alnum{a} (\savi) bANagaLige baLiyuva viSavAgi baLasuva biLi hAlu koDuva, AMTiyArisf TAkisxkeVriya kulada, jAvA divxVpada mara. 
\alnum{b} (\pu) tananx baLige baMdadedxlalxkUkx mArakavAgutatxdeMdu BAvisalAda, jAvAdivxVpada mara. 
\eanum
\numie
\num{2} vinAshaka; nAshakara parxBAva, padadhxti, \mo vu. 
\num{3} yUpasf marada yA itara maragaLa viSamaya rasa. 
\enum
\emng
\eentry

\bentry
\word{upas-tree} 
\pron{yUpasfTirxV} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{upas(1)}{upas \((1)\)}. 
\emng
\eentry

\bentry
\word[upbeat(1)]{upbeat} 
\pron{apfbiVTf} 
\gl{\nA}
\expl{}
\bmng
(\saM) 
\bnum
\num{1} laGutALa; otitxlalxda baDita; tALada baDita sUcisalu, otutx hAkida baDitakikxMta muMce baruva, otitxlalxda baDita. 
\num{2} (AkeRsATxrX nivARhakanu) otitxlalxda tALabaDita sUcisalu baLasuva tALadaMDada saMjecnx. 
\enum
\emng
\eentry

\bentry
\word[upbeat(2)]{upbeat} 
\pron{apfbiVTf} 
\gl{\gu}
\expl{}
\bmng
(\AmA) AshABAvavuLaLx; lavalavikeya; ulAlxsaBarita. 
\emng
\eentry

\bentry
\word{upbraid} 
\pron{apfberxVDf} 
\gl{\sakirx}
\expl{}
\bmng
bayiyx; CiVmAri mADu. 
\emng
\eentry

\bentry
\word{upbraiding} 
\pron{apfberxVDiMgf} 
\gl{\nA}
\expl{}
\bmng
bayugxLa; CiVmAri (hAkuvudu). 
\emng
\eentry

\bentry
\word{upbringing} 
\pron{apfbirxMgiMgf} 
\gl{\nA}
\expl{}
\bmng
sAkuvudu; (makakxLa) pAlane; shikaSxNa: \eng{the proper upbringing of young people} yuvakarige sariyAda shikaSxNa niVDuvudu. 
\emng
\eentry

\bentry
\word[upcast(1)]{upcast} 
\pron{apfkAsfTx} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ adeV). }
\bmng
meVlakekxse; etetxse. 
\emng
\eentry

\bentry
\word[upcast(2)]{upcast} 
\pron{apfkAsfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} meVleseta; meVlakekxseyuvudu; utfkeSxVpaNa. 
\num{2} (\gaNi) vAtAyana; vAyumAgaR; gaNiyiMda gALi hora hoVguva mAgaR. 
\num{3} (\BUvi) = \hyperlink{upthrow}{upthrow}. 
\enum
\emng
\eentry

\bentry
\word[upcast(3)]{upcast} 
\pron{apfkAsfTx} 
\gl{\gu}
\expl{}
\bmng
meVletitxda; meVle tirugisida: \eng{with upcast eyes} meVle tirugisida kaNuNxgaLu. 
\emng
\eentry

\bentry
\word{upcoming} 
\pron{apfkamiMgf} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ \ame) BAvi; muMbaruva; Agaliruva; baraliruva; saMBavisaliruva. 
\emng
\eentry

\bentry
\word[up-country(1)]{up-country} 
\pron{apfkaMTirx} 
\gl{\kirxvi}
\expl{}
\bmng
oLanADina kaDege; oLaparxdeVshadalilx. 
\emng
\eentry

\bentry
\word[up-country(2)]{up-country} 
\pron{apfkaMTirx} 
\gl{\gu}
\expl{}
\bmng
oLanADina(lilxruva); oLadeVshada. 
\emng
\eentry

\bentry
\word[update(1)]{update} 
\pron{apfDeVTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (naviVkarisi) iMdinadanAnxgi mADu; AdhunikiVkarisu; iMdinadu enisu. 
\num{2} (garxMtha, aMkiaMshagaLu, \mo vugaLa \vi) pariSakxrisi hosadanAnxgi mADu. 
\enum
\emng
\eentry

\bentry
\word[update(2)]{update} 
\pron{apfDeVTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} Adhuni(ki)VkaraNa; iMdinadanAnxgi mADuvudu yA mADidudx. 
\num{2} AdhunikiVkarisida -- Avaqtitx yA mAhiti. 
\enum
\emng
\eentry

\bentry
\word{updater} 
\pron{apfDeVTarf} 
\gl{\nA}
\expl{}
\bmng
Adhuni(ki)VkaraNagAra; iMdinadanAnxgi mADuvava. 
\emng
\eentry

\bentry
\wordnospeech{up draught}{up draught}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\kanmu\ koThaDiya hoge koLaviya mUlaka) meVleVruva otatxDada gALi. 
\emng
\eentry

\bentry
\word{up-end} 
\pron{apfeMDf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} tudiya; buDada, taLada -- meVle nililxsu. 
\num{2} (\AmA) (aBiruci, aBipArxya, parxsididhx yA vayxvasethxgaLa \vi) saMpUNaR yA AmUlAgarx badalAvaNe taru, uMTumADu. 
\num{3} (\AmA) sapxdheR, vAyxpAra, \mo vugaLalilx -- soVlisu, jayagaLisu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
tudiya yA buDada meVle nilulx. 
\emng
\eentry

\bentry
\word{upfield} 
\pron{apfphiVlfDx} 
\gl{\kirxvi}
\expl{}
\bmng
(kAlecxMDATa \mo vugaLalilx) edurALiya meYdAnadalilx yA adara baLi. 
\emng
\eentry

\bentry
\word{upfold} 
\pron{apfphoVlfDx} 
\gl{\nA}
\expl{}
\bmng
(\BUvi) = \hyperref{kandict_a.pdf}{A}{anticline}{anticline}. 
\emng
\eentry

\bentry
\word[upfront(1)]{upfront} 
\pron{apfpharxMTf} 
\gl{\kirxvi}
\expl{}
\bmng
(\sA\ \eng{up front} ) 
\bnum
\num{1} muMdugaDe; muMBAgadalilx. 
\num{2} (pAvatigaLa \vi) muMgaDavAgi. 
\enum
\emng
\eentry

\bentry
\word[upfront(2)]{upfront} 
\pron{apfpharxMTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} pArxmANika; mucucxmareyilalxda; bicucxmanasisxna. 
\num{2} muMdugaDeya; muMBAgada yA atayxMta parxdhAnavAda. 
\num{3} (pAvatigaLa \vi) muMpAvatiya; muMgaDavAgi niVDida. 
\enum
\emng
\eentry

\bentry
\word[upgrade(1)]{upgrade} 
\pron{apfgerxVDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} rasetx \mo vugaLa caDAvu, Eru. 
\num{2} ELige; hecacxLa; munanxDe. 
\num{3} sudhAraNe mADida, utatxmagoLisida -- salakaraNe \mo vu. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{on the upgrade} 
\banum
\alnum{a} AroVgayx \mo vugaLalilx sudhArisutAtx. 
\alnum{b} muMduvariyutAtx; parxgati hoMdutAtx. 
\eanum
\emng
\eentry

\bentry
\word[upgrade(2)]{upgrade} 
\pron{apfgerxVDf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} muMbaDitx koDu; meVlina padavigeVrisu: \eng{upgrade labourers to managerial positions} kelasagAraranunx meVlivxcAraka padavigaLigeVrisu. 
\num{2} (sAdhana salakaraNe, yaMtarx, \mo vanunx) \kanmu\ aMgaBAgagaLanunx badalAyisi -- sudhArisu; utatxmapaDisu. 
\enum
\emng
\eentry

\bentry
\word{upgrader} 
\pron{apfgerxVDarf} 
\gl{\nA}
\expl{}
\bmng
sudhAraka; (\kanmu\ aMgaBAgagaLanunx badalAyisuva mUlaka) sAdhana salakaraNe, yaMtarx, \mo vanunx utatxma goLisuvavanu. 
\emng
\eentry

\bentry
\word{upgrowth} 
\pron{apfgorxVtf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} beLavaNige; aBivadhaRne. 
\num{2} beLedadudx; aBivaqdidhx. 
\enum
\emng
\eentry

\bentry
\word{upheaped} 
\pron{apfhiVpfTx} 
\gl{\gu}
\expl{}
\bmng
etitx guDeDxhAkida; rAshimADida; oTiTxlu mADida. 
\emng
\eentry

\bentry
\word{upheaval} 
\pron{apfhiVva(vf)lf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} vipalxva; kArxMti; haThAtAtxda yA birusina sAmAjika \mo\ badalAvaNe. 
\num{2} (\BUvi) porxVtAthxna; BUmiya cipipxna BAga meVleVLuvudu yA ububxvudu. 
\num{3} meVleVLuvudu; utAthxna; Erike. 
\enum
\emng
\eentry

\bentry
\word{upheave} 
\pron{apfhiVvf} 
\gl{\akirx}
\expl{}
\bmng
balavaMtavAgi, parxyAsadiMda -- etutx. 
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
meVleVLu; ububx. 
\emng
\eentry

\bentry
\word[uphill(1)]{uphill} 
\pron{apfhilf} 
\gl{\nA}
\expl{}
\bmng
rasetx, beTaTx, \mo vugaLa Eru, caDAvu. 
\emng
\eentry

\bentry
\word[uphill(2)]{uphill} 
\pron{apfhilf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} meVleVrAda; meVleVruva. 
\num{2} (\rUpa) duSakxra; kaSaTxvAda; sharxmada: \eng{uphill task} duSakxra kelasa. 
\enum
\emng
\eentry

\bentry
\word[uphill(3)]{uphill} 
\pron{apfhilf} 
\gl{\kirxvi}
\expl{}
\bmng
(rasetx, beTaTx, \mo vugaLa \vi) Erutatx; hatutxtatx: \eng{the soldiers marched uphill} seYnikaru guDaDx Erutatx naDedaru. 
\emng
\eentry

\bentry
\word{uphold} 
\pron{apfhoVlfDx} 
\gl{\sakirx}
\expl{(\BU\ matutx \BUkaq\ \eng{upheld}). }
\bmng
\bnum
\num{1} etitx hiDi; neTaTxge hiDi. 
\num{2} (vayxkitx, padadhxti, \mo vugaLige) beMbala koDu; Asharxya niVDu; avalaMbana koDu. 
\num{3} (niNaRya, tiVmARna, \mo vanunx) etitx hiDi; sithxriVkarisu; samathiRsu. 
\enum
\emng
\eentry

\bentry
\word{upholder} 
\pron{apfhoVlaDxrf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} etitx hiDiyuvavanu. 
\num{2} beMbaliga; AsharxyadAta. 
\num{3} samathaRnakAra. 
\enum
\emng
\eentry

\bentry
\word{upholster} 
\pron{apfhoVlfsaTxrf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (koVNe \mo vanunx tere, jamaKAna, piVThoVpakaraNagaLu, \mo vugaLiMda) sajujxgoLisu. 
\num{2} (kuciR \mo vakekx) metetx, baTeTxya gavasu, \mo vanunx havaNisu. 
\num{3} (kuciR \mo vakekx) gavasu hAku; alaMkArada baTeTx kavisu. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\hyperdef{U}{upholsterpagu}{} \eng{well-upholstered} (\hA) (vayxkitxya \vi) bojujx beLeda; ToNapanAda. 
\emng
\eentry

\bentry
\word{upholsterer} 
\pron{apfhoVlfsaTxrarf} 
\gl{\nA}
\expl{}
\bmng
(piVThoVpasakxra \mo vugaLa) sajujxgAra; metetxgAra. 
\emng
\eentry

\bentry
\word{upholsterer-bee} 
\pron{apfhoVlfsaTxrarfbiV} 
\gl{\nA}
\expl{}
\bmng
sajujxgAra duMbi; tananx gUDanunx ele \mo vugaLiMda sajujxgoLisuva oMdu bageya duMbi. 
\emng
\eentry

\bentry
\word{upholstery} 
\pron{apfhoVlfsaTxri} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (piVThoVpasakxragaLige hAkuva) sajujx; metetx, diMbu, baTeTxya gavusu, \mo vu. 
\num{2} (piVThoVpakaraNa \mo vugaLa) sajujxgArana kelasa. 
\enum
\emng
\eentry

\bentry
\word{uphroe} 
\pron{apfhorxV} 
\gl{\nA}
\expl{}
\bmng
(\nw) (baTeTxya cAvaNi aLavaDisuva hurigaLanunx tUrisalu) tUtu koreda udadxvAda marada tuMDu. 
\emng
\eentry

\bentry
\word{upkeep} 
\pron{apfkiVpf} 
\gl{\nA}
\bmng
\bnum
\num{1} durasutx; saMrakaSxNe; susithxtiyalilxDuvudu. 
\numi{2} durasitxna, saMrakaSxNeya 
\banum
\alnum{a} KacuR; vecacx. 
\alnum{b} riVti; vidhAna. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[upland(1)]{upland} 
\pron{apflaMDf} 
\gl{\nA}
\expl{}
\bmng
(deVshada) oLanADina yA malenADina parxdeVsha. 
\emng
\eentry

\bentry
\word[upland(2)]{upland} 
\pron{apflaMDf} 
\gl{\gu}
\expl{}
\bmng
oLaparxdeVshada; malenADina. 
\emng
\eentry

\bentry
\word[uplift(1)]{uplift} 
\pron{apfliphfTx} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (padavi, neYtika maTaTx, \mo vugaLanunx) meVlakekxtutx. 
\num{2} (neYtikavAgi yA AdhAyxtimxkavAgi) meVletutx; udAdhxra mADu. 
\enum
\emng
\eentry

\bentry
\word[uplift(2)]{uplift} 
\pron{apfliphfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} udAdhxra; unanxti; udAdhxra mADuvudu yA Aguvudu. 
\num{2} (\BUvi) BUmiya meVlemxYyalilx ubibxda BAga. 
\num{3} (\AmA) neYtikavAgi yA AdhAyxtimxkavAgi udAdhxragoLisuva yA unanxtigeVrisuva parxBAva. 
\num{4} (\kanmu\ heMgasara uDupinalilx) ede \mo vugaLige AsareyAgi koDuva metetx. 
\enum
\emng
\eentry

\bentry
\word{uplifter} 
\pron{apfliphaTxrf} 
\gl{\nA}
\expl{}
\bmng
udAdhxraka, \kanmu\ samAjoVdAdhxraka. 
\emng
\eentry

\bentry
\word{uplifting} 
\pron{apfliphiTxMgf} 
\gl{\gu}
\expl{}
\bmng
(neYtikavAgi yA AdhAyxtimxkavAgi) udadhxrisuva; unanxtigeVrisuva. 
\emng
\eentry

\bentry
\word[up-market(1)]{up-market} 
\pron{apfmAkiRTf} 
\gl{\gu}
\expl{}
\bmng
(\birx) dubAri mArukaTeTxya; mArukaTeTxya hecucx dubAri yA shirxVmaMta viBAgakekx saMbaMdhisida yA adara kaDege Oluva. 
\emng
\eentry

\bentry
\word[up-market(2)]{up-market} 
\pron{apfmAkiRTf} 
\gl{\kirxvi}
\expl{}
\bmng
(\birx) dubAriyAgi; hecicxna beleyadAgi. 
\emng
\eentry

\bentry
\word[upmost(1)]{upmost} 
\pron{apfmoVsfTx} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{uppermost(1)}{$^1$uppermost}. 
\emng
\eentry

\bentry
\word[upmost(2)]{upmost} 
\pron{apfmoVsfTx} 
\gl{\kirxvi}
\expl{}
\bmng
= \hyperlink{uppermost(2)}{$^2$uppermost}. 
\emng
\eentry

\bentry
\word{upon} 
\pron{apAnf} 
\gl{\upa}
\expl{}
\bmng
= \hyperref{kandict_o.pdf}{O}{on(1)}{$^1$on}. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{Christmas is almost upon us} inenxVnu kirxsfmasf baMdeVbiTiTxtu. 
\num{2} \eng{fell upon him unawares} idadxkikxdadxMte avana meVle bidadx. 
\num{3} \eng{had him upon toast} avananunx tananx hiDitakekx tegedukoMDa. 
\num{4} \eng{once upon a time} oMdAnoMdu kAladalilx. 
\num{5} \eng{take it upon trust} naMbikeya AdhArada meVle sivxVkarisu; pariVkeSx mADade naMbu. 
\num{6} \eng{tier upon tier of seats} sAlusAlAgi Erisida, peVrisida piVThagaLu. 
\num{7} \eng{upon my word} nananx ANegU. 
\num{8} \eng{upon receiving the message} sudidx baMda kUDaleV. 
\num{9} \eng{upon the whole} oTATxre; oTiTxnalilx. 
\enum
\emng
\eentry

\bentry
\word[upper(1)]{upper} 
\pron{aparf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} meVlina; meVlugaDeya: \eng{upper lip} meVludxTi. \eng{upper arm} toVLina meVluBAga. 
\num{2} meVluBAgadalilxruva; etatxradalilxruva: \eng{Upper Egypt} IjipiTxna oLanADu; meVlu BAgada samudarxdiMda dUraviruva IjipfTx. 
\num{3} meVlaMtasitxna; meVludajeRya; unanxta adhikAradalilxruva: \eng{upper class} meVlina vagaRda, parxtiSiThxta vagaRda. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{the upper bench} (\ca) iMgelxMDinalilx kAmanfvelftx ALivxkeya kAladalilxdadx rAjana daMDAdhikArigaLu. 
\num{2} \eng{the upper crust} (\AmA) (samAjada) atuyxnanxta, variSaThx, meVlu -- vagaR; \kanmu\ shirxVmaMta vagaR. 
\numi{3} \eng{the upper house} (\birx) 
\banum
\alnum{a} (pAliRmeMTina) meVlamxne; hwsf Aphf lADfRsx. 
\alnum{b} biSapfgaLa maMDali. 
\eanum
\numie
\numi{4} \eng{the upper regions} 
\banum
\alnum{a} AkAsha; bAnu. 
\alnum{b} savxgaR.
\eanum
\numie
\num{5} \eng{upper} \hyperref{kandict_c.pdf}{C}{case(2) pagu(2)}{$^2$case}. 
\enum
\emng
\eentry

\bentry
\word[upper(2)]{upper} 
\pron{aparf} 
\gl{\nA}
\expl{}
\bmng
pAdarakeSxya bUTina -- meVluBAga. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{on one's uppers} (\AmA) kaSaTxdalilx; baDatanadalilx; tiVrA duDiDxlalxde. 
\emng
\eentry

\bentry
\word[upper(3)]{upper} 
\pron{aparf} 
\gl{\nA}
\expl{}
\bmng
(\ashi) utetxVjaka madudx (\kanmu\ AYxMphiTamiVnf). 
\emng
\eentry

\bentry
\word(upper class[1]){upper class} 
\pron{}
\gl{\nA}
\expl{}
\bmng
(samAjada) meVlu, variSaThx -- vagaR, \kanmu\ shirxVmaMta vagaR. 
\emng
\eentry

\bentry
\word(upper class[2]){upper class} 
\pron{}
\gl{\gu}
\expl{}
\bmng
(samAjada) meVluvagaRda(vara); shirxVmaMtavagaRda(vara). 
\emng
\eentry

\bentry
\word[upper-cut(1)]{upper-cut} 
\pron{aparfkaTf} 
\gl{\nA}
\expl{}
\bmng
toVLanunx bagigxsi meVluBAgakekx hoDeda ETu; meVleVTu; UdhavxRparxhAra. 
\emng
\eentry

\bentry
\word[upper-cut(2)]{upper-cut} 
\pron{aparfkaTf} 
\gl{\sakirx}
\expl{}
\bmng
meVleVTu hoDe; toVLanunx bagigxsi meVluBAgakekx ETu hoDi. 
\emng
\eentry

\bentry
\wordnospeech{Upper House}{Upper House}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(shAsanasaBeya) meVlamxne: 
\banum
\alnum{a} gerxVTf birxTaninxna hwsf Aphf lADfRsx saBe. 
\alnum{b} BAratada rAjayxsaBe. 
\eanum
\emng
\eentry

\bentry
\word[uppermost(1)]{uppermost} 
\pron{aparfmoVsfTx} 
\gl{\gu}
\expl{(\eng{upmost} eMdU \parx).}
\bmng
\bnum
\num{1} (padadhxti, sAthxna, \mo vugaLalilx) atayxMta meVlina; atuyxnanxta. 
\num{2} parxmuKa; parxdhAna: \eng{a subject of uppermost importance} atayxMta pArxmuKayxda viSaya. 
\enum
\emng
\eentry

\bentry
\word[uppermost(2)]{uppermost} 
\pron{aparfmoVsfTx} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} elalxkikxMta meVle yA meVlugaDege; moTaTxmodalige: \eng{it is not always wise to say whatever comes uppermost} yAvAgalU manasisxge modalu baMdadadxnunx heVLibiDuvudu viveVkavalalx. 
\num{2} atayxMta parxmuKa yA parxdhAna sAthxnadalilx yA sAthxnakekx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{upper works}{upper works}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(saraku BatiRmADiruvAga) niVrina meVlugaDe iruva haDagina BAga. 
\emng
\eentry

\bentry
\word{uppish} 
\pron{apiSf} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ \birx, \AmA) 
\bnum
\num{1} biMkada; sokikxna; ahaMkArada. 
\num{2} ilalxda doDaDxsitxke toVrisuva; savxparxtiSeThxya; oNajaMbada. 
\enum
\emng
\eentry

\bentry
\word{uppishly}
\pron{apiSfli} 
\gl{\kirxvi}
\bmng
\bnum
\num{1} biMkadiMda; sokikxniMda; ahaMkAradalilx. 
\num{2} ilalxda doDaDxsitxke toVrisutatx; savxparxtiSeThxyiMda. 
\enum
\emng
\eentry

\bentry
\word{uppishness} 
\pron{apiSfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} biMka; sokukx; ahaMkAra. 
\num{2} ilalxda doDaDxsitxke, parxtiSeThx. 
\enum
\emng
\eentry

\bentry
\word{uppity} 
\pron{apiTi} 
\gl{\gu}
\expl{}
\bmng
(\AmA) = \hyperlink{uppish}{uppish}. 
\emng
\eentry

\bentry
\word{upraise} 
\pron{apfreVsfZ} 
\gl{\sakirx}
\expl{}
\bmng
etatxrakekx Erisu; meVlakekxtutx; unanxtamaTaTxkekx Erisu. 
\emng
\eentry

\bentry
\word[upright(1)]{upright} 
\pron{apfreYTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} neTaTxneya; laMba; neVra: \eng{an upright post} neTaTxneya kaMba. \eng{stood upright} neTaTxge niMta. 
\num{2} (\kanmu\ pusatxka, citarx, \mo vugaLa \vi) agalakikxMta hecucx etatxrada. 
\num{3} (vayxkitx yA naDateya \vi) satayxvaMtanAda; pArxmANikanAda; Quju; neVravAda: \eng{an upright person} satayxvaMta. 
\num{4} (piyAnoV \vi) laMbataMtiya; UdhaRvx taMtiya. 
\enum
\emng
\eentry

\bentry
\word[upright(2)]{upright} 
\pron{apfreYTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (UreyAgi) neTaTxge nililxsida kaMba; neTaTxneya kaMba yA dasi. 
\num{2} = \hyperlink{upright piano}{upright piano}. 
\enum
\emng
\eentry

\bentry
\word[upright(3)]{upright} 
\pron{apfreYTf} 
\gl{\kirxvi}
\expl{}
\bmng
= \hyperlink{uprightly}{uprightly}. 
\emng
\eentry

\bentry
\word[upright(4)]{upright} 
\pron{apfreYTf} 
\gl{\sakirx}
\expl{}
\bmng
neTaTxge nililxsu; neVra nililxsu; laMbavAgi neDu. 
\emng
\eentry

\bentry
\word{uprightly} 
\pron{apfreYTfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} neTaTxge; neTaTxne; neVravAgi. 
\num{2} satayxvaMtanAgi; pArxmANikateyiMda; QujuvAgi; neVravAgi. 
\enum
\emng
\eentry

\bentry
\word{uprightness} 
\pron{apfreYTfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} laMbate; neTaTxgiruvudu; neVravAgiruvudu. 
\num{2} satayxsaMdhate; pArxmANikate; Qujute; neVravAda naDevaLike. 
\enum
\emng
\eentry

\bentry
\wordnospeech{upright piano}{upright piano}
\pron{?}
\gl{\nA}
\expl{}
\bmng
laMba piyAno; laMbavAgi nililxsuva diVGaR caturasArxkArada oDaluLaLx, piyAnoVvAdayx. 
\emng
\eentry

\bentry
\word{uprise} 
\pron{apfreYsfZ} 
\gl{\akirx}
\expl{(\BU\ \eng{uprose}; \BUkaq\ \eng{uprisen}). }
\bmng
edudxnilulx. 
\emng
\eentry

\bentry
\word{uprising} 
\pron{apfreYsiZMgf} 
\gl{\nA}
\expl{}
\bmng
daMge; baMDAya. 
\emng
\eentry

\bentry
\word{uproar} 
\pron{apfroVrf} 
\gl{\nA}
\expl{}
\bmng
goMdala; galATe; huyilu; koVlAhala; doMbi; dAMdhale: \eng{the meeting ended in an uproar} saBe koVlAhaladalilx mukAtxyavAyitu. 
\emng
\eentry

\bentry
\word{uproarious} 
\pron{apfroVriasf} 
\gl{\nA}
\expl{}
\bmng
(\kanmu, nage, ulAlxsa, humamxsusx, \mo vugaLa \vi) galATeya; huyilebibxsuva; koVlAhalada; hucucxhucAcxda: \eng{they burst into uproarious laughter} avaru Pakakxne nageya huyilebibxsidaru. 
\emng
\eentry

\bentry
\word{uproariously} 
\pron{apfroVriasfli} 
\gl{\kirxvi}
\expl{}
\bmng
(\kanmu\ nage, ulAlxsa, humamxsusx, \mo vugaLa \vi) galATeyebibxsuvaMte; huyilebibxsuvaMte. 
\emng
\eentry

\bentry
\word{uproariousness} 
\pron{apfroVriasfnisf} 
\gl{\nA}
\expl{}
\bmng
(\kanmu\ nage, ulAlxsa, humamxsusx, \mo vugaLa \vi) galATe; koVlAhala; huyilu. 
\emng
\eentry

\bentry
\word{uproot} 
\pron{apfrUTf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (giDa \mo vanunx) beVrusahita kiVLu; buDasahita kitutxhAku. 
\num{2} (obabxnanunx avana sathxLadiMda) ODisu; sAthxnapalalxTa mADu: \eng{the industrial revolution uprooted large rural populations} keYgArikA kArxMtiyiMdAgi hecucx saMKeyxyalilx haLiLxya janaru sAthxnapalalxTagoMDaru. 
\num{3} unUmxlana mADu; nAsha mADu; vinAshagoLisu; mUloVtApxTana mADu; buDamuTaTx kitutxhAku; beVrusahita nAshamADu: \eng{the war uprooted the entire nation} yudadhxdiMda iDiV deVshaveV vinAshavAyitu. 
\enum
\emng
\eentry

\bentry
\word{uprush} 
\pron{apfraSf} 
\gl{\nA}
\expl{}
\bmng
udevxVga; (\kanmu\ \mavi) nigUDha parxjecnxyoLaginiMda meVlakekx nugugxvudu. 
\emng
\eentry

\bentry
\word{ups-a-daisy} 
\pron{apisxDeVsiZ} 
\gl{\BAavayx}
\expl{}
\bmng
= \hyperlink{upsy-daisy}{upsy-daisy}. 
\emng
\eentry

\bentry
\word[upset(1)]{upset} 
\pron{apfseTf} 
\gl{\kirx}
\expl{(\BU\ matutx \BUkaq\ \eng{upset}, \vakaq\ \eng{upsetting}). }


\noindent
\gl{\sakirx}
\bmng
\bnum
\num{1} talekeLagu mADu; buDameVlu mADu; uruLisu: \eng{the carriage was upset} gADi talekeLagAyitu. 
\num{2} (shAMti, jiVNaRshakitx, \mo vanunx) keDisu; palalxTagoLisu: \eng{the news quite upset him} A sudidx avana manasasxnunx pUrA palalxTagoLisitu. \eng{ate something that upset him} avana deVhasithxti keDisida EnanonxV tiMdidAdxne. 
\num{3} avayxvasethxyanunxMTumADu; goMdalavebibxsu; asatxvayxsatxgoLisu: \eng{upset a system} oMdu vayxvasethxyanunx asatxvayxsatxgoLisu. 
\num{4} (loVhada kelasa) moVTu mADu; gujujx mADu; (loVhavanunx, \kanmu\ paTiTxyanunx) eraDu kaDegaLalUlx baDidu yA otitx giDaDxnAgi, dapapxnAgi AguvaMte mADu. 
\enum
\emng

\noindent
\gl{\akirx}
\expl{}
\bmng
talekeLagAgu; buDameVlAgu; uruLu. 
\emng
\eentry

\bentry
\word[upset(2)]{upset}
\pron{apfseTf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} talekeLakAguvudu; talekeLagu mADuvudu. 
\num{2} asatxvayxsatx sithxti; keTaTx sithxti: \eng{stomach upset} hoTeTx keTiTxruvudu. 
\num{3} (paMdayx \mo vugaLalilx) AshacxyaRkara, aniriVkiSxta -- PalitAMsha; talekeLagAda lekAkxcAra. 
\enum
\emng
\eentry

\bentry
\word[upset(3)]{upset} 
\pron{apfseTf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} uruLisida; talekeLagu mADida: \eng{upset milk pail} uruLisida hAlina pAterx. 
\num{2} avayxvasethxgoLisida; asatxvayxsatxgoLisida: \eng{the house is upset} maneyelAlx avayxvasethxyAgide. 
\enum
\emng
\eentry

\bentry
\wordnospeech{upset price}{upset price}
\pron{?}
\gl{\nA}
\expl{}
\bmng
kaniSa Thxbele; harAju \mo vugaLalilx AsitxyoMdara atayxMta kaDimeya bele. 
\emng
\eentry

\bentry
\word{upsetter} 
\pron{apfseTarf} 
\gl{\nA}
\bmng
\bnum
\num{1} talekeLagu mADuvavanu. 
\num{2} avayxvasethx uMTumADuvavanu. 
\enum
\emng
\eentry

\bentry
\word{upsettingly} 
\pron{apfseTiMgfli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} talekeLagu mADuvaMte. 
\num{2} (manasisxna \vi) goMdalavAguvaMte. 
\enum
\emng
\eentry

\bentry
\word{upshot} 
\pron{apfSATf} 
\gl{\nA}
\expl{}
\bmng
pariNAma; Palashurxti; payaRvasAna; itayxthaR; tiVmARna: \eng{what will be its upshot ?} adara pariNAma EnAgutatxde? 
\emng
\eentry

\bentry
\word[upside-down(1)]{upside-down} 
\pron{apfseYDfDwnf} 
\gl{\gu}
\expl{}
\bmng
(\eng{upside down} eMdU \parx) talekeLagAda; buDameVlAda; ulATx. 
\emng
\eentry

\bentry
\word[upside-down(2)]{upside-down} 
\pron{apfseYDfDwnf} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} talekeLakAgi; buDameVlAgi. 
\num{2} asatxvayxsatxvAgi; celAlxpililxyAgi; avayxvasethxyAgi: \eng{the house was turned upside-down by the burglars} kaLaLxru maneyanunx celAlxpililx kedari hoVgidadxru. \eng{everything was turned upside-down} elalxvU asatxvayxsatxvAgitutx. 
\enum
\emng
\eentry

\bentry
\wordnospeech{upside-down cake}{upside-down cake}
\pron{?}
\gl{\nA}
\expl{}
\bmng
ulATx keVku; haNiNxna padarada meVle haraDi, beVyisi, talekeLagu mADidAga haNiNxna padara meVliruvaMte mADiruva oMdu sihitiMDi. 
\emng
\eentry

\bentry
\word{upsides} 
\pron{apfseYDfsfZ} 
\gl{\kirxvi}
\expl{}
\bmng
(\birx) (\AmA) (seVDu, parxtiVkAra, muyiyx, \mo vugaLa mUlaka obabxnige) samanAgu; sATiyAgu. 
\emng
\eentry

\bentry
\word{upsilon} 
\pron{yUpisxlAnf, apfseYlanf} 
\gl{\nA}
\expl{}
\bmng
girxVkf vaNaRmAleya 20neya akaSxra. (\eng{g,} \eng{n}). 
\emng
\eentry

\bentry
\word[upstage(1)]{upstage} 
\pron{apfseTxVjf} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} raMgasathxLada hiMBAgadalilx; hiMraMgadalilx. 
\num{2} jaMbadiMda; doDaDxsitxkeyiMda; savxparxtiSeThxyiMda. 
\enum
\emng
\eentry

\bentry
\word[upstage(2)]{upstage} 
\pron{apfseTxVjf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} raMgasathxLada hiMBAgada, hiMdugaDeya. 
\num{2} jaMbada; doDaDxsitxkeya; savxparxtiSeThxya. 
\enum
\emng
\eentry

\bentry
\word[upstage(3)]{upstage} 
\pron{apfseTxVjf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (naTana \vi) (inonxbabx naTananunx perxVkaSxkarige vimuKanAguvaMte, benunx tirugisuvaMte mADalu tAnu) raMgada hiMBAgakekx, hiMdugaDeya BAgakekx -- sari, hoVgu, calisu. 
\num{2} elalxra daqSiTxyanunx (beVrobabxna kaDeyiMda) tananx kaDege -- seLeduko; tiruguvaMte mADiko. 
\num{3} (obabxnanunx) miVrisu; atishayisi beLagu; miVri parxkAshisu. 
\enum
\emng
\eentry

\bentry
\word{upstair} 
\pron{apfseTxVrf} 
\gl{\gu}
\bmng
= \hyperlink{upstairs(3)}{$^3$upstairs}. 
\emng
\eentry

\bentry
\word[upstairs(1)]{upstairs} 
\pron{apfseTxVsfZR} 
\gl{\nA}
\expl{}
\bmng
(\sA\ \Eva dalilx) mahaDi; meVlaMtasutx; upapxrige. 
\emng
\eentry

\bentry
\word[upstairs(2)]{upstairs} 
\pron{apfseTxVsfZR} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} mahaDiya meVle; meVlaMtasitxnalilx; upapxrigeyalilx. 
\num{2} mahaDiya meVlugaDege, meVlakekx. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\hyperref{kandict_k.pdf}{K}{kick(2) nuga(3)}{$^2$kick} \eng{a (person) upstairs.} 
\emng
\eentry

\bentry
\word[upstairs(3)]{upstairs} 
\pron{apfseTxVsfZR} 
\gl{\gu}
\expl{}
\bmng
mahaDiya; meVlaMtasitxnalilxruva; upapxrigege seVrida: \eng{an upstairs window} mahaDiya kiTaki. 
\emng
\eentry

\bentry
\word{upstanding} 
\pron{apfsATxYxMDiMgf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} neTaTxneya; neTaTxge niMtiruva. 
\num{2} neVravAda; Quju; pArxmANikavAda. 
\num{3} gaTiTxmuTATxgi AroVgayxvaMtavAgiruva. 
\enum
\emng
\eentry

\bentry
\word[upstart(1)]{upstart} 
\pron{apfsATxTfR} 
\gl{\nA}
\expl{}
\bmng
(\kanmu\ durahaMkAradiMda vatiRsuva) nava -- kubeVra, shirxVmaMta; diDiVrf -- shirxVmaMta, doDaDxmanuSayx; sAmAnayx sithxtiyiMda idadxkikxdadxMte aishavxyaR yA unanxta adhikAra paDeda vayxkitx. 
\emng
\eentry

\bentry
\word[upstart(2)]{upstart} 
\pron{apfsATxTfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} nava shirxVmaMtanAda; idadxkikxdadxMte shirxVmaMtanAda yA meVlina adhikArakekx baMda: \eng{upstart officials in government offices} sakARri kaceVrigaLalilxna navoVnanxta adhikArigaLu. 
\numi{2} 
\banum
\alnum{a} nava shirxVmaMtana yA shirxVmaMtikeya. 
\alnum{b} nava shirxVmaMtana parxtiSeThxya yA durahaMkArada. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word[upstate(1)]{upstate} 
\pron{apfseTxVTf} 
\gl{\gu}
\expl{}
\bmng
(\ame) (rAjayxda) \kanmu\ baDagunADina; rAjayxda doDaDxnagaragaLiMda dUraviruva BAgakekx, parxdeVshakekx (\kanmu\ utatxra BAgakekx) seVrida. 
\emng
\eentry

\bentry
\word[upstate(2)]{upstate} 
\pron{apfseTxVTf} 
\gl{\kirxvi}
\expl{}
\bmng
(\ame) rAjayxda \kanmu\ utatxra BAgadalilx, parxdeVshadalilx. 
\emng
\eentry

\bentry
\word[upstate(3)]{upstate} 
\pron{apfseTxVTf} 
\gl{\nA}
\expl{}
\bmng
(\ame) doDaDxnagaragaLiMda dUravAgiruva rAjayxda \kanmu\ utatxra BAga, baDagunADu. 
\emng
\eentry

\bentry
\word{upstater} 
\pron{apfseTxVTarf} 
\gl{\nA}
\expl{}
\bmng
(\ame) rAjayxda utatxra BAgadavanu, utatxra BAgada nivAsi; autatxreVya. 
\emng
\eentry

\bentry
\word[upstream(1)]{upstream} 
\pron{apfsiTxrXVmf} 
\gl{\gu}
\expl{}
\bmng
(nadi yA hoLeya) parxvAhakekx edurAgi hoVguva; parxvAhakekx virudadhx dikikxnalilx sAguva. 
\emng
\eentry

\bentry
\word[upstream(2)]{upstream} 
\pron{apfsiTxrXVmf} 
\gl{\kirxvi}
\expl{}
\bmng
(nadi yA hoLeya) parxvAhakekx edurAgi; parxvAhakekx virudadhx dikikxnalilx. 
\emng
\eentry

\bentry
\word{up-stroke} 
\pron{apfsoTxrXVkf} 
\gl{\nA}
\expl{}
\bmng
(akaSxragaLanunx bareyuvAga hAkuva) meVlugiVTu. 
\emng
\eentry

\bentry
\word{upsurge} 
\pron{apfsajfR} 
\gl{\nA}
\expl{}
\bmng
hecacxLa; Erike; aBivaqdidhx; Adhikayx; hecucxvudu: \eng{an upsurge in sales} mArATadalilx hecacxLa. 
\emng
\eentry

\bentry
\word{upswept} 
\pron{apfsevxpfTx} 
\gl{\gu}
\bmng
\bnum
\num{1} (kUdalina \vi) hiMdakekx bAcida; hiMdakekx bAci taleya meVle OraNagoLisida.
\num{2} meVloVre mADida; meVlakekx bAgisida; meVlakekx vAlisida, Ore mADida: \eng{upswept automobile fenders} meVloVre mADida moVTAruvAhanagaLa muMgApugaLu. 
\enum
\emng
\eentry

\bentry
\word{upswing} 
\pron{apfsivxMgf} 
\gl{\nA}
\bmng
\bnum
\num{1} UdhaRvxgamana; UdhaRvxgAmitavx; meVlakekx calisuvudu; meVlakekx calisuva parxvaqtitx. 
\num{2} gamanAhaRvAda hecucxvari, Erike; Adhikayx; meVlApxDu: \eng{an upswing in prices} belegaLalilx Erike. 
\enum
\emng
\eentry

\bentry
\word{upsy-daisy} 
\pron{apisxDeVsiZ} 
\gl{\BAavayx}
\expl{}
\bmng
(magu jAribidAdxga yA adanunx meVlakekxtitx saMtayisuvAga) jANamari ededxVLu! (EnU Agilalx, ELu!). 
\emng
\eentry

\bentry
\word{uptake} 
\pron{apfTeVkf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (meVlakekx) etutxvudu. 
\num{2} (\AmA) garxhisuvudu; garxhaNashakitx; garxhike; athaRmADikoLuLxvike: \eng{quick} (\engit{or} \eng{slow) on the uptake} garxhisuvudaralilx curuku (yA nidhAna). 
\num{3} (hoge, gALi, \mo vanunx meVlakekx sAgisuva) koLave \mo vu. 
\enum
\emng
\eentry

\bentry
\word{upthrow} 
\pron{apftorxV} 
\gl{\nA}
\bmng
\bnum
\num{1} meVleseta; UdhaRvxkeSxVpa; meVlakekxseyuvudu. 
\num{2} (BUmi) meVlakekxdudxkoLuLxvudu; BUmiya ububx. 
\num{3} (\BUvi) (\kanmu\ \gaNi) (BUsatxrada oMdu pakakxkekx) baMDe edudxkoLuLxvudu. 
\enum
\emng
\eentry

\bentry
\word{upthrust} 
\pron{apftarxsfTx} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} meVlumxKa -- otatxDa, nUkalu, taLiLxke (\udA\ muLugisida kAyada meVlina darxvada meVlumxKanAda otatxDa). 
\num{2} (\BUvi) BUmiya meVlemxYna UdhaRvx calane. 
\enum
\emng
\eentry

\bentry
\word{uptight} 
\pron{apfTeYTf} 
\gl{\gu}
\expl{}
\bmng
(\AmA) 
\bnum
\num{1} bigidukoMDa; biguvina; udevxVgada; sulaBavAgi koSxVBegoLuLxva. 
\num{2} reVguva; thaTaTxne keraLuva. 
\num{3} (\ame) kaTATx saMparxdAyaniSaThx; tiVvarx saMparxdAyasharaNa. 
\enum
\emng
\eentry

\bentry
\word[uptown(1)]{uptown} 
\pron{apfTwnf} 
\gl{\gu}
\expl{}
\bmng
paTaTxNa yA nagarada -- vasatiBAgada, meVluBAgada; vAsada manegaLiruva parxdeVshada: \eng{uptown train} nagarada meVluBAgakekx hoVguva reYlu. 
\emng
\eentry

\bentry
\word[uptown(2)]{uptown} 
\pron{apfTwnf} 
\gl{\kirxvi}
\expl{}
\bmng
paTaTxNa yA nagarada -- meVluBAgadalilx, vAsada manegaLiruva kaDeyalilx, kaDege, dikikxnalilx: \eng{he rode uptown on the bus} avanu basisxnalilx nagarada meVluBAgakekx hoVda. 
\emng
\eentry

\bentry
\word[uptown(3)]{uptown} 
\pron{apfTwnf} 
\gl{\nA}
\expl{}
\bmng
paTaTxNa yA nagarada -- meVluBAga, vAsaBAga, vasatigaLiruva parxdeVsha. 
\emng
\eentry

\bentry
\word{uptowner} 
\pron{apfTwnarf} 
\gl{\nA}
\expl{}
\bmng
paTaTxNa yA nagarada meVluBAgadalilx yA janavasati BAgadalilx vAsisuvava. 
\emng
\eentry

\bentry
\word{uptrain} 
\pron{apfTerxVnf} 
\gl{\nA}
\expl{}
\bmng
doDaDxnagarada kaDege hoVguva reYlu. 
\emng
\eentry

\bentry
\word[upturn(1)]{upturn} 
\pron{apfTanfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} meVlakekx tirugisu; meVlumxKavAgisu. 
\num{2} (maNuNx, nela, \mo vanunx neVgilu \mo vugaLiMda) meVlakekxtitx hAku; tiruvi hAku. 
\enum
\emng
\eentry

\bentry
\word[upturn(2)]{upturn} 
\pron{apfTanfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} = \hyperlink{upheaval}{upheaval}. 
\num{2} aBivaqdidhx; hecacxLa; utakxSaR; meVleVrike. 
\enum
\emng
\eentry

\bentry
\word{upturned} 
\pron{apfTanfRDx} 
\gl{\gu}
\expl{}
\bmng
talekeLagAda; maguci bidadx; muKavaDiyAda. 
\emng
\eentry

\bentry
\wordnospeech{UPU}{UPU}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{Universal Postal Union.} 
\emng
\eentry

\bentry
\word[upward(1)]{upward} 
\pron{apfvaDfR} 
\gl{\kirxvi}
\expl{}
\bmng
(\eng{upwards} eMdU \parx). 
\bnum
\num{1} meVle; meVlakekx; meVlugaDe; UdhaRvxvAgi. 
\num{2} meVlina -- sithxtige, sAthxnakekx, padavige. 
\num{3} hecAcxgi; meVlapxTuTx; miVri: \eng{fourscore and upward} eMbatatxkUkx miVri, meVlapxTuTx. \eng{it cost him upward of a Rs. 1000} adakekx avanige oMdu sAvira rUpAyigU meVlapxTuTx KacARyitu. 
\enum
\emng
\eentry

\bentry
\word[upward(2)]{upward} 
\pron{apfvaDfR} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} meVlina; meVliruva; meVlugaDeya. 
\num{2} meVlakekx hoVguva; UdhaRvxgAmi; meVlakekx cAciruva. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{upward (s) of} miVri; meVlapxTuTx; hecAcxgi: \eng{found upward of 40 specimens} nalavatatxkikxMta hecucx mAdarigaLu sikikxdadxvu. 
\emng
\eentry

\bentry
\word{upwardly} 
\pron{apfvaDfRli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} meVlakekx; meVlugaDege. 
\num{2} UdhaRvxmuKiyAgi. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{upwardly mobile} (sAmAjikavAgi yA vaqtitxyalilx) meVlakekxVrabayasuva yA meVleVrabalalx; unanxtipara gatiya. 
\emng
\eentry

\bentry
\word{upwarp} 
\pron{apfvApfR} 
\gl{\nA}
\expl{(\BUvi) }
\bmng
= \hyperref{kandict_a.pdf}{A}{anticline}{anticline}. 
\emng
\eentry

\bentry
\word[upwind(1)]{upwind} 
\pron{apfviMDf} 
\gl{\kirxvi}
\expl{}
\bmng
gALige edurAgi; gALi biVsuva kaDege virudadhxvAgi; gALige parxtimuKavAgi. 
\emng
\eentry

\bentry
\word[upwind(2)]{upwind} 
\pron{apfviMDf} 
\gl{\gu}
\expl{}
\bmng
gALige edurAgiruva; gALige edurAgi calisuva. 
\emng
\eentry

\bentry
\word[upwind(3)]{upwind} 
\pron{apfviMDf} 
\gl{\nA}
\expl{}
\bmng
edurugALi; (iLijAru rasetx \mo\ kaDe) edurugaDeyiMda biVsuva gALi. 
\emng
\eentry

\bentry
\word{ur-} 
\pron{Urf-} 
\gl{\sapUpa}
\expl{}
\bmng
atayxMta pArxciVna, pArxkatxna, purAtana, Adiyugada eMba athaRgaLalilx baLasuva \sapUpa. 
\emng
\eentry

\bentry
\word{uracil} 
\pron{yuarasilf} 
\gl{\nA}
\expl{}
\bmng
(\jiVra) yurasilf; Arfenfe GaTakavAgi jiVvaMta UtakagaLalilx kaMDubaruva, pirimiDiVnfjanayx parxtAyxmalx. 
\emng
\eentry

\bentry
\word{uraemia} 
\pron{yuriVmia} 
\gl{\nA}
\expl{}
\bmng
(\roVshA) yuriVmiya; mUtarxpiMDagaLu mUtarxda mUlaka horahAkuva, mUtarxdalilxrabeVkAda rAsAyanikagaLu rakatxdalilxruva asavxsathxteya sithxti. 
\emng
\eentry

\bentry
\word{uraemic} 
\pron{yuriVmikf} 
\gl{\gu}
\bmng
(\roVshA) yuriVmiya uLaLx. 
\emng
\eentry

%%%%%%%%%%%\bentry
\word{uraeus} 
\pron{yuriVasf} 
\gl{\nA}
\expl{}
\bmng
(IjipiTxna deVvategaLa matutx doregaLa taleyuDugeyalilxya) sapaRlAMCana; nAgABaraNa. \imglink{uraeus:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.6cm height 0.6cm {U_Pictures/uraeus.jpg}}} 
\emng
\eentry

%%%%%%%%%%
\bentry
\word[Ural-Altaic(1)]{Ural-Altaic} 
\pron{yuaralAYxleTxVikf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} soVviyatf raSAyx matutx madhayx ESAyxgaLalilxna yUralf matutx AleTxVyikf pavaRta parxdeVshagaLa. 
\num{2} (\BAshA) yUralAYxleTxVyikf BASA vagaRda. 
\enum
\emng
\eentry

\bentry
\word[Ural-Altaic(2)]{Ural-Altaic} 
\pron{yuaralAYxleTxVikf} 
\gl{\nA}
\expl{}
\bmng
(\BAshA) yUralAYxleTxVikf BASA vagaR; utatxra yUroVpu matutx ESAyxKaMDagaLa phinoVyugirxkf, TakiRkf, maMgoVliyanf, matitxtara yUroVpf matutx ESAyxgaLa saMshilxSaTx BASegaLa kuTuMba. 
\emng
\eentry

\bentry
\word{uranium} 
\pron{yureVniamf} 
\gl{\nA}
\expl{}
\bmng
(\ravi) yureVniyamf; neYsagiRkavAgi picfbelxMDf Kanijadalilx doreyuva, dhAtugaLalilx atayxMta hecucx paramANu saMKeyxya (92), atayxdhika paramANu tUkada (238) vidaLaniVya paramANu biVjavuLaLx, AdudariMda beYjikashakitxya AkaravAgiruva, bUdu baNaNxda, BAravAda, vikiraNapaTu loVhadhAtu (\saMkeV\ \eng{U}). 
\emng
\eentry

\bentry
\word[urano-(1)]{urano-} 
\pron{yuaranoV-} 
\gl{\sapUpa}
\expl{}
\bmng
AkAsha-, KagoVLa- eMbathaRda \sapUpa. 
\emng
\eentry

\bentry
\word[urano-(2)]{urano-} 
\pron{yuaranoV-} 
\gl{\sapUpa}
\expl{}
\bmng
yUreVniyaMna eMbathaRda \sapUpa. 
\emng
\eentry

\bentry
\word{uranographer} 
\pron{yuaranAgarxpharf} 
\gl{\nA}
\expl{}
\bmng
vaNaRnAtamxka KagoVLa vijAcnxni. 
\emng
\eentry

\bentry
\word{uranographic} 
\pron{yuaranagArxYxphikf} 
\gl{\gu}
\expl{}
\bmng
vaNaRnAtamxka KagoVLa vijAcnxnada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{uranography} 
\pron{yuaranAgarxphi} 
\gl{\nA}
\expl{}
\bmng
vaNaRnAtamxka KagoVLa vijAcnxna; nakaSxtarxgaLu, garxhagaLu, \mo vanunx vivarisi nakeSxyalilx toVrisuva, KagoVLa vijAcnxnada shAKe. 
\emng
\eentry

\bentry
\word{uranus} 
\pron{yuaranasf, yureVnasf} 
\gl{\nA}
\expl{}
\bmng
yureVnasf (garxha); \kirxsha\ 1781ralilx haSaRlf eMbuvanu kaMDuhiDida, swravUyxhadalilx nepUTxyXnf matutx shanigarxhagaLa naDuve iruva garxha. 
\emng
\eentry

\bentry
\word{urban} 
\pron{abaRnf} 
\gl{\gu}
\expl{}
\bmng
paTaTxNada; paTaTxNadalilxya; nagarada; nagaradalilxya; nAgara; pwra: \eng{urban population} nagara nivAsigaLu. 
\emng
\eentry

\bentry
\wordnospeech{urban district}{urban district}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\birx) (\ca) cunAyita saBeya ADaLitadalilxruva pwra samudAyagaLu, nagara samAjagaLu. 
\emng
\eentry

\bentry
\word{urbane} 
\pron{abeRVnf} 
\gl{\gu}
\bmng
nayavuLaLx; saBayx; susaMsakxqqta; nAgarika: \eng{urbane manners} nAgarika naDavaLike. 
\emng
\eentry

\bentry
\word{urbanely} 
\pron{abeRVnfli} 
\gl{\kirxvi}
\expl{}
\bmng
nayavAgi; saBayxvAgi; susaMsakxqqta, nAgarika riVtiyalilx. 
\emng
\eentry

\bentry
\wordnospeech{urban guerrilla}{urban guerrilla}
\pron{?}
\gl{\nA}
\expl{}
\bmng
nagara gorilAlxgaLu; balAtAkxra haraNa \mo vugaLa mUlaka nagaragaLalilx hoVrADuva gupatxdaLa. 
\emng
\eentry

\bentry
\word{urbanise} 
\pron{abaRneYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
= \hyperlink{urbanize}{urbanize}. 
\emng
\eentry

\bentry
\word{urbanism} 
\pron{abaRnisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\numi{1} nAgarate: 
\banum
\alnum{a} nagarada guNalakaSxNagaLu. 
\alnum{b} nagarajiVvanada riVtiniVtigaLu. 
\eanum
\numie
\num{2} nagarAdhayxyana; purAdhayxyana; nagara jiVvanada adhayxyana. 
\enum
\emng
\eentry

\bentry
\word{urbanist} 
\pron{abaRnisfTx} 
\gl{\nA}
\expl{}
\bmng
nagarashAsatxrXjacnx; nagara yA nagarajiVvana kuritu adhayxyana mADuvava. 
\emng
\eentry

\bentry
\word{urbanite} 
\pron{abaRneYTf} 
\gl{\nA}
\expl{}
\bmng
paTaTxNavAsi; nagaravAsi; puravAsi. 
\emng
\eentry

\bentry
\word{urbanity} 
\pron{abARYxniTi} 
\gl{\nA}
\bmng
\bnum
\num{1} naya; nAjUku; saBayxte; susaMsakxqqti; nAgarika naDavaLike. 
\num{2} nagara jiVvana. 
\enum
\emng
\eentry

\bentry
\word{urbanization} 
\pron{abaRneYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
nagariVkaraNa. 
\emng
\eentry

\bentry
\word{urbanize} 
\pron{abaRneYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} paTaTxNavAgisu; nagariVkarisu. 
\num{2} (jilelx \mo vugaLa) gArxmiVNa lakaSxNa kaLe. 
\enum
\emng
\eentry

\bentry
\wordnospeech{urban renewal}{urban renewal}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(koLace parxdeVsha nimURlana mADi) nagarABivaqdidhx(goLisuvudu). 
\emng
\eentry

\bentry
\wordnospeech{urban sprawl}{urban sprawl}
\pron{?}
\gl{\nA}
\expl{}
\bmng
nagara -- vAyxpane; niyaMtarxNavilalxde, sevxVcePxyAgi beLeyuva (baDAyisuva) nagara parxdeVshagaLu; nagarada aniyaMtirxta haraDike, habibxke. 
\emng
\eentry

\bentry
\word{urceolate} 
\pron{asiRalaTf} 
\gl{\gu}
\bmng
(\savi) kuMBAkArada; doDaDx kAya matutx cikakx bAyiya; gaDigeyaMtha. 
\emng
\eentry

\bentry
\word{urchin} 
\pron{aciRnf} 
\gl{\nA}
\bmng
\bnum
\num{1} tuMTa huDuga; ceVSeTx huDuga. 
\num{2} poVra; ciLeLx huDuga; saNaNx huDuga; piLeLx; cilATxri. 
\num{3} = \hyperref{kandict_s.pdf}{S}{sea urchin}{sea urchin}. 
\numi{4} (\pArxparx) 
\banum
\alnum{a} muLuLxhaMdi. 
\alnum{b} tuMTa pishAci. 
\eanum
\numie
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{street urchin} biVdi huDuga, poVra; manemaThavilalxda baDa huDuga. 
\num{2}\eng{urchin cut} (heMgasara) gutatxnAgi katatxrisida keVshavinAyxsa. 
\enum
\emng
\eentry

\bentry
\word{Urdu} 
\pron{uDUR, aDUR} 
\gl{\nA}
\expl{}
\bmng
uduR (BASe). 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ure}{-ure}
\pron{-yarf} 
\gl{\uparx}
\expl{}
\bmng
\banum
\alnum{a} yAvudoV kirxyeya, parxkirxyeya eMbathaRda \uparx, \udA\ \eng{censure, composure, enclosure}. 
\alnum{b} (yAvudoV) adhikAra, padavi, kelasa, eMbathaRda \uparx, \udA\ \eng{judicature, ligature}. 
\alnum{c} (yAvudoV) samudAya eMbathaRda \uparx: \eng{legislature}. 
\alnum{d} yAvudoV kirxyeya Pala, pariNAma eMbathaRda \uparx, \udA\ \eng{creature, scripture.} 
\eanum
\emng
\eentry

\bentry
\word{urea} 
\pron{yuari(V)a} 
\gl{\nA}
\expl{}
\bmng
(\jiVra) yUriya; sasatxnigaLa deVhadalilx neYTorxjanf upAvacayada, aMtima utapxnanxvAgidudx, sasatxnigaLa mUtarxdalilx kaMDubaruva, vaNaRrahita saPxTikiVya, dArxvayxsaMyukatx, \eng{$\bg\rm CN\eg_2\bg\rm H\eg_4\bg\rm O\eg$.} 
\emng
\eentry

\bentry
\word{ureal} 
\pron{yuari(V)alf} 
\gl{\gu}
\expl{}
\bmng
yUriyada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{uremia} 
\pron{yuriVmia} 
\gl{\nA}
\expl{}
\bmng
(\ame) \eng{uraemia} eMbudara rUpAMtara. 
\emng
\eentry

\bentry
\word{ureter} 
\pron{yuariVTarf} 
\gl{\nA}
\expl{}
\bmng
mUtarxnALa; mUtarxpiMDagaLiMda mUtarxvanunx mUtarxkoVshakekx sAgisuva nALa. 
\emng
\eentry

\bentry
\word{ureteral} 
\pron{yuariVTaralf} 
\gl{\gu}
\expl{}
\bmng
mUtarxnALada; mUtarxnALakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{ureteric} 
\pron{yuariTerikf} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{ureteral}{ureteral}. 
\emng
\eentry

\bentry
\word{ureteritis} 
\pron{yuariTareYTisf} 
\gl{\nA}
\expl{}
\bmng
mUtarxnALoVderxVka; mUtarxnALada uriyUta. 
\emng
\eentry

\bentry
\word{urethane} 
\pron{yu(a)riVteVnf} 
\gl{\nA}
\expl{}
\bmng
(\ravi) yUritheVnf; pAlxsiTxkfgaLu matutx vaNaRdarxvayxgaLa tayArikeyalilx baLasuva ItheYlf kAbaRmeVTf. 
\emng
\eentry

\bentry
\word{urethra} 
\pron{yuariVtarx} 
\gl{\nA}
\expl{(\bava\ \eng{urethrae}, \ucAcx-- yuariVtirxV). }
\bmng
mUtarxvisajaRna nALa; mUtarxkoVshadiMda mUtarxvanunx visajiRsuva nALa. 
\emng
\eentry

\bentry
\word{urethral} 
\pron{yuariVtarxlf} 
\gl{\gu}
\expl{}
\bmng
mUtarxvisajaRna nALada yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{urethritis} 
\pron{yuariterxYTisf} 
\gl{\nA}
\expl{}
\bmng
mUtarx visajaRna nALoVderxVka. 
\emng
\eentry

\bentry
\word[urge(1)]{urge} 
\pron{ajfR} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} joVrAgi, balAtAkxradiMda -- naDasu, ODisu, dUDu: \eng{urged his horse forward} avana kudureyanunx balavaMtavAgi muMdakekx ODisida. 
\numi{2} (sharxdedhxyiMda, satatavAgi) 
\banum
\alnum{a} pArxthiRsu; moreyiDu; koVru. 
\alnum{b} parxcoVdisu; perxVreVpisu; porxVtAsxhisu: \eng{urged him to action} kAyaRtatapxranAguvaMte parxcoVdisidevu. 
\eanum
\numie
\num{3} (oMdu kAyaRkarxma \mo vanunx keYkoLuLxvaMte) meVliMda meVle otAtxyapaDisu, otitxheVLu, vAdisu: \eng{he urged upon his pupils the importance of hard work} kaSaTxpaTuTx kelasa mADuvudara agatayxvanunx avanu tananx vidAyxthiRgaLige otAtxyapUvaRkavAgi tiLiyaheVLidanu. 
\num{4} (obabxnanunx) beVDi, vAdisi -- otAtxyapaDisu: \eng{the argument was urged in vain} eSuTx otAtxyisi vAda mADidarU adu niSaPxlavAyitu. \eng{the shopkeeper urged me to buy a coat} aMgiyanunx koLuLxvaMte aMgaDiyavanu nananxnunx otAtxyapaDisida. 
\num{5} (kAraNa yA samathaRneyAgi) muMdiDu; muMdoDuDx; vAdisu: \eng{he urged the difficulty of getting supplies} sarabarAyi tarisikoLuLxvudu eSuTx dusatxravenunxvudanunx muMdoDiDxda. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{urge (one) on} muMde hoVguvaMte perxVreVpisu yA otAtxyisu. 
\emng
\eentry

\bentry
\word[urge(2)]{urge} 
\pron{ajfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} coVdane; parxcoVdane; perxVraNe; perxVreVpaNe. 
\num{2} parxbalavAda Ase; haMbala; kaTATxse; utakxTeVcePx: \eng{an urge to travel} parxvAsa mADalu kaTATxse. 
\enum
\emng
\eentry

\bentry
\word{urgency} 
\pron{ajaRnisx} 
\gl{\nA}
\expl{}
\bmng
avasara; tutuR; jarUru. 
\emng
\eentry

\bentry
\word{urgent} 
\pron{ajaRMTf} 
\gl{\gu}
\bmng
\bnum
\num{1} avasarada; tutiRna; jarUrAda; oDane -- gamanisabeVkAda, mADabeVkAda: \eng{am in urgent need} nanage jarUrAgi adu beVkAgide. 
\num{2} ototxtitx meVliMda meVle keVLuva; otAtxyadiMda pArxthiRsuva; (pArxthaRnegaLiMda) beVDuva, koVruva, kADuva. 
\enum
\emng
\eentry

\bentry
\word{urgently} 
\pron{ajaRMTfli} 
\gl{\kirxvi}
\expl{}
\bmng
avasaravAgi; tutARgi; jarUrAgi. 
\emng
\eentry

\bentry
\word{urger} 
\pron{ajaRrf} 
\gl{\nA}
\expl{}
\bmng
parxcoVdisuvavanu; parxcoVcaka; perxVraka. 
\emng
\eentry

\bentry
\wordnospeech{URI}{URI}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{upper respiratory infection.} 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-uria}{-uria}
\pron{-yuaria} 
\gl{\saupa}
\expl{}
\bmng
(yAvudoV aMsha) mUtarxdalilx hecAcxgide eMbathaRda \upa, \udA\ \eng{albuminuria.} 
\emng
\eentry

\bentry
\word{uric} 
\pron{yuarikf} 
\gl{\gu}
\expl{}
\bmng
mUtarxda yA mUtarxkekx saMbaMdhisida. 
\emng
\eentry

\bentry
\wordnospeech{uric acid}{uric acid}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\jiVra) yUrikf Amalx; sasatxnigaLa mUtarxdalilx alapxparxmANadalilxruva, bahupAlu sasatxnigaLa maladalilx lavaNa rUpadalilxruva, \eng{$\bg\rm C\eg_5\bg\rm H\eg_4\bg\rm N\eg_4\bg\rm O\eg_3$} sUtarxda, saPxTikiVya biLi adArxvayx GanapadAthaR. 
\emng
\eentry

\bentry
\word{urinal} 
\pron{yuareY(ri)nalf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} mUtarxpAterx; mUtarxdAni (\sA\ roVgigaLu hAsigeyalilx baLasuvaMthadu). 
\num{2} mUtirx; mUtarx mADuva sathxLa. 
\enum
\emng
\eentry

\bentry
\word{urinalysis} 
\pron{yuarinAYxlisisf} 
\gl{\nA}
\expl{(\bava\ \eng{urinalyses} \ucAcx-- yuarinAYxlisiVsfZ). }
\bmng
mUtarxpariVkeSx. 
\emng
\eentry

\bentry
\word{urinary} 
\pron{yuarinari} 
\gl{\gu}
\expl{}
\bmng
mUtarxda; mUtirxVya; mUtarxkekx saMbaMdhisida: \eng{urinary diseases} mUtarxroVgagaLu. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{urinary organs} mUtArxMgagaLu. 
\emng
\eentry

\bentry
\word{urinate} 
\pron{yuarineVTf} 
\gl{\akirx}
\expl{}
\bmng
ucecx hoyiyx; mUtarx visajiRsu. 
\emng
\eentry

\bentry
\word{urination} 
\pron{yuarineVSanf} 
\gl{\nA}
\expl{}
\bmng
mUtarx visajaRne. 
\emng
\eentry

\bentry
\word{urine} 
\pron{yuarinf} 
\gl{\nA}
\expl{}
\bmng
ucecx; mUtarx; gaMjala. 
\emng
\eentry

\bentry
\word{urinose} 
\pron{yuarinoVsfZ} 
\gl{\gu}
\expl{}
\bmng
= \hyperlink{urinous}{urinous}. 
\emng
\eentry

\bentry
\word{urinous} 
\pron{yuarinasf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} ucecxya. 
\num{2} ucecxya hAge iruva. 
\num{3}mUtarxvuLaLx. 
\enum
\emng
\eentry

%%%%%%%%%%%%%\bentry
\word[urn(1)]{urn} 
\pron{anfR} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (hiMde) (citABasamxvanunx sheVKarisiDutitxdadx) bUdigaDige; Basamx karaMDa. \imglink{\(^1\)urn 1.:fig}{\raisebox{-0.15cm}[0pt][0pt]{\pdfimage width 0.5cm height 0.6cm {U_Pictures/urn.jpg}}} 
\num{2} (\kAparx) goVri; samAdhi; sAmxraka; kaLeVbara, asithx, citABasamx, \mo vugaLaninxTiTxruva sathxLa. 
\num{3} (nalilx iruva) kAphi yA TiV pAterx; kAphi, TiV tayArisuva yA bisiyAgiDuva pAterx. 
\enum
\emng
\eentry

%%%%%%%%%%%%%%
\bentry
\word[urn(2)]{urn} 
\pron{anfR} 
\gl{\sakirx}
\expl{}
\bmng
(\pArxparx) BasamxkaraMDadalilxDu. 
\emng
\eentry

\bentry
\word{urnful} 
\pron{anfRPuZlf} 
\gl{\nA}
\expl{(\bava\ \eng{urnfuls}). }
\bmng
gaDige, kaLasha, karaMDa tuMbuvaSuTx parxmANa; gaDige BatiR; kaLashaBatiR. 
\emng
\eentry

\bentry
\word{urning} 
\pron{aniRMgf} 
\gl{\nA}
\expl{}
\bmng
saliMgakAmi (\sA\ gaMDasu). 
\emng
\eentry

\bentry
\word[uro-(1)]{uro-} 
\pron{yuaroV-} 
\gl{\sapUpa}
\expl{}
\bmng
`mUtarxda', `mUtarxkekx saMbaMdhisida' eMbathaRda \sapUpa. 
\emng
\eentry

\bentry
\word[uro-(2)]{uro-} 
\pron{yuaroV-} 
\gl{\sapUpa}
\expl{}
\bmng
(\pArxvi) bAlada, pucaCxda eMbathaRda \sapUpa. 
\emng
\eentry

\bentry
\word{urochord} 
\pron{yuaroVkADfR} 
\gl{\nA}
\expl{}
\bmng
(\bava) (\pArxvi) yUrokADeRVTA; kaMcukitagaLanonxLagoLuLxva, kADeRVTA viBAgada oMdu upaviBAga. 
\emng
\eentry

\bentry
\word[urochordate(1)]{urochordate} 
\pron{yuaroVkADeRVTf} 
\gl{\nA}
\expl{}
\bmng
(\pArxvi) yUrokADeRVTf viBAgakekx seVrida yAvudeV pArxNi. 
\emng
\eentry

\bentry
\word[urochordate(2)]{urochordate} 
\pron{yuaroVkADeRVTf} 
\gl{\gu}
\expl{}
\bmng
(\pArxvi) yUrokADeRVTf viBAgakekx seVrida. 
\emng
\eentry

\bentry
\word{urodela} 
\pron{yuaroVDiVl} 
\gl{\nA}
\expl{}
\bmng
(\bava) (\pArxvi) yUroVDiVla; nUyxTf\eng{(newt)} gaLanUnx sAyxlamAMDarf \eng{(salamander)} gaLanUnx oLagoLuLxva, uBayajiVvigaLa oMdu gaNa. 
\emng
\eentry

\bentry
\word{urodele} 
\pron{yuaroVDiVlf} 
\gl{\nA}
\expl{}
\bmng
yUroVDiVlf(pArxNi); yUroVDiVla gaNakekx seVrida, nUyxTf (\eng{newt} )gaLu matutx beMkimosaLe (\eng{salamander} )gaLanunx oLagoMDa, beLeda meVle bAla baruva, uBayacara jiVvi. 
\emng
\eentry

\bentry
\word{urogenital} 
\pron{yuarajeniTalf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} mUtArxMga matutx jananeVMdirxyagaLige saMbaMdhisida. 
\num{2} mUtArxMga matutx jananeVMdirxyagaLiMda utapxtitxyAda. 
\enum
\emng
\eentry

\bentry
\word{urologic} 
\pron{yuaralAjikf} 
\gl{\gu}
\expl{}
\bmng
mUtarxshAsatxrXda yA adakekx saMbaMdhisida. 
\emng
\eentry

\bentry
\word{urologist} 
\pron{yuarAlajisfTx} 
\gl{\nA}
\expl{}
\bmng
mUtarxshAsatxrXjacnx. 
\emng
\eentry

\bentry
\word{urology} 
\pron{yuarAlaji} 
\gl{\nA}
\expl{}
\bmng
mUtarxshAsatxrX; mUtarxvayxvasethxya veYjAcnxnika adhayxyana. 
\emng
\eentry

\bentry
\word{uropygium} 
\pron{yuaroVpijiamf} 
\gl{\nA}
\expl{}
\bmng
hakikxkuMDe; pakiSxpaqSaThx. 
\emng
\eentry

\bentry
\word{uroscopy} 
\pron{yuarAsakxpi} 
\gl{\nA}
\expl{}
\bmng
(\veYshA) (\ca) (\kanmu\ roVganidAnadalilx) mUtarxpariVkeSx. 
\emng
\eentry

\bentry
\wordRemoveSpace{Ursa-Major}{Ursa Major}
\pron{asaR meVjarf} 
\gl{\nA}
\expl{}
\bmng
= \hyperref{kandict_b.pdf}{B}{bear(1) pagu(1)}{Great $^1$bear}. 
\emng
\eentry

\bentry
\wordRemoveSpace{Ursa-Minor}{Ursa Minor}
\pron{asaR meYnarf} 
\gl{\nA}
\expl{}
\bmng
= \hyperref{kandict_b.pdf}{B}{bear(1) pagu(2)}{Little $^1$bear}. 
\emng
\eentry

\bentry 
\word{ursine} 
\pron{aseRYnf} 
\gl{\gu}
\bmng
karaDiya yA karaDiyaMtha. 
\emng
\eentry

\bentry
\word[Ursuline(1)]{Ursuline} 
\pron{asuyxRleY(li)nf} 
\gl{\gu}
\expl{}
\bmng
asaRleYnf paMthada. 
\emng
\eentry

\bentry
\word[Ursuline(2)]{Ursuline} 
\pron{asuyxRleY(li)nf} 
\gl{\nA}
\expl{}
\bmng
asaRleYnf paMthada saMnAyxsini; heNuNxmakakxLa shikaSxNa hAgU roVgigaLa shushUrxSegaLigAgi 1535ralilx saMta EMjelA eMbAke sAthxpisida, dhAmiRka paMtha. 
\emng
\eentry

\bentry
\word{urticaria} 
\pron{aTiRkeVria} 
\gl{\nA}
\expl{}
\bmng
(\veYshA) turuci dadudx; alajiRya pariNAmavAgi uMTAguva naveyiMda kUDida, turuci muLiLxniMda AguvaMtha bokekxgaLu. 
\emng
\eentry

\bentry
\word{urticate} 
\pron{aTiRkeVTf} 
\gl{\sakirx}
\expl{}
\bmng
turuci yA curucurike giDada muLuLxgaLaMte cucucx. 
\emng
\eentry

\bentry
\word{urtication} 
\pron{aTiRkeVSanf} 
\gl{\nA}
\expl{}
\bmng
turucike; curucurike giDada muLiLxnaMte cucucxvudu. 
\emng
\eentry

\bentry
\word{urus} 
\pron{yuarasf} 
\gl{\nA}
\expl{}
\bmng
= \hyperref{kandict_a.pdf}{A}{aurochs}{aurochs}. 
\emng
\eentry

\bentry
\wordnospeech{US}{US}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{Under Secretary.} 
\num{2} \eng{United States (of America).} 
\num{3} \eng{unserviceable.} 
\enum
\emng
\eentry

\bentry
\word{us} 
\pron{asf} 
\gl{\sanA}
\expl{}
\bmng
\bnum
\num{1} \eng{we} eMbudara divxtiVyA viBakitx rUpa: \eng{they saw us} avaru namamxnunx noVDidaru. 
\num{2} (\AmA) = \hyperref{kandict_w.pdf}{W}{we}{we}: \eng{it's us again} matetx, punaH nAveV. 
\num{3} (\AmA) = \hyperref{kandict_m.pdf}{M}{me(1)}{me}: \eng{give us a kiss} nanagoMdu mututx koDu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{USA}{USA} 
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{United States of America.} 
\num{2} (\ame) \eng{United States Army.} 
\enum
\emng
\eentry

\bentry
\word{usability} 
\pron{yUsaZbiliTi} 
\gl{\nA}
\expl{}
\bmng
baLasabalilxke; parxyoVjanakekx baruvike; upayukatxte. 
\emng
\eentry

\bentry
\word{usable} 
\pron{yUsaZbflf} 
\gl{\gu}
\expl{}
\bmng
baLasabalalx; upayujayx; parxyoVjanakekx baruva. 
\emng
\eentry

\bentry
\word{usableness} 
\pron{yUsaZbflfnisf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{usability}{usability}. 
\emng
\eentry

\bentry
\wordnospeech{USAF}{USAF}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United States Air Force.} 
\emng
\eentry

\bentry
\word{usage} 
\pron{yUsijf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} baLake; upayoVga; upayoVgisuva riVti; naDesikoLuLxva vidhAna: \eng{met with harsh usage} (avananunx) kaThiNavAgi naDesikoLaLxlAyitu; avana bagegx kUrxravAgi naDedukoMDaru. \eng{damaged by rough usage} oraTAgi upayoVgisidadxriMda hALAda. 
\num{2} baLake; upayoVga; parxyoVga; vayxvahAra; vahivATu: \eng{sanctified by usage} baLakeyalilx aMgiVkAra paDeda. 
\num{3} (\kanmu\ hakukx, kataRvayx, javAbAdxri, mAnadaMDa, \mo vanunx nidhaRrisuva) padadhxti; saMparxdAya; vADike: \eng{industrialization and urbanization influence social usages} audoyxVgiVkaraNa hAgU nagariVkaraNa sAmAjika padadhxtigaLa meVle parxBAva birutatxve. 
\num{4} (\nAyxshA) (anAdi kAladiMdalalxdidadxrU rUDhiyAgi naDeyutatx baMdiruva) mAmUlu; saMparxdAya. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{an ancient usage} pArxciVna saMparxdAya. 
\num{2} \eng{modern English usage} Adhunika iMgilxSf (BASA) parxyoVga. 
\enum
\emng
\eentry

\bentry
\word{usance} 
\pron{yUsaZnfsx} 
\gl{\nA}
\bmng
\bnum
\num{1} (\pArxparx) baLake; upayoVga. 
\num{2} videVshiV vinimayapatarxgaLa haNavanunx pAvati mADalu, vAyxpAriV vADikeyu niVDuva kAlAvakAsha; (videVshi huMDi haNada) pAvativAyide. 
\enum
\emng
\eentry

\bentry
\wordnospeech{USB}{USB}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
(kaMpUyxTarf) \eng{universal serial bus.} 
\emng
\eentry

\bentry
\wordnospeech{USD}{USD}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United States dollar(s).} 
\emng
\eentry

\bentry
\wordnospeech{USDAW}{USDAW}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
(\UK\ yalilx) \eng{Union of Shop, Distributive, and Allied Workers.} 
\emng
\eentry

\bentry
\word[use(1)]{use} 
\pron{yUsfZ} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} baLasuvudu; upayoVgisuvudu; baLake; upayoVga; parxyoVga; viniyoVga: \eng{put it to good use} adanunx cenAnxgi upayoVgisu. \eng{has passed out of use} adu Iga baLakeyalilxlalx. \eng{is in daily use} dinabaLakeyalilxde. 
\num{2} upayoVgisuva hakukx yA shakitx yA adhikAra: \eng{lost the use of his left arm} eDatoVLanunx baLasalu avanige sAdhayxvilalx (eDatoVLanunx baLasuva shakitx kaLedukoMDidAdxne). 
\num{3} (baLasuvudara) Pala; parxyoVjana: \eng{a blunt knife is of no use} moMDa cAkuviniMda yAva parxyoVjanavU ilalx. \eng{it is no use talking} mAtanADuvudariMda, mAtanADi -- parxyoVjanavilalx. 
\num{4} padadhxti; aBAyxsa; vADike: \eng{long use has reconciled me to it} diVGaRkAlada aBAyxsadiMda nAnu adanunx sarihoMdisikoMDidedxVne. 
\num{5} (caciRna, ArAdhanA maMdirada) pUjA padadhxti; kamaRpadadhxti; dhAmiRka parxkirxyegaLu: \eng{Anglican use} iMgilxSf caciRna padadhxti. 
\num{6} (\nAyxshA) (\ca) (beVrobabx PalAnuBaviya anuBoVgakAkxgi mAtarx obabxna vashadalilxruva Asitxya) PalAnuBava; anuBoVga. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\numi{1} \eng{have no use for} 
\banum
\alnum{a} parxyoVjanavAgadiru; agatayxvilalxdiru. 
\alnum{b} (\rUpa) iSaTxvilalxdiru: kaMDare Agadiru: \eng{he had no use for laggards} avanige soVmArigaLanunx kaMDare Agadu. 
\eanum
\numie
\num{2} \eng{make use of} baLasu; upayoVgisu; parxyoVjana paDe. 
\num{3} \eng{meant for use, not for ornament} adara udedxVsha -- baLake, alaMkAravalalx. 
\num{4} \eng{use and wont} rUDhi padadhxti; sAthxpitavAda vADike. 
\enum
\emng
\eentry

\bentry
\word[use(2)]{use} 
\pron{yUsfZ} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} baLasu; upayoVgisu; parxyoVgisu; viniyoVgisu: \eng{seldom uses a knife} cAkuvanunx baLasuvudeV ilalx. \eng{use your discretion} ninanx viveVcaneyanunx upayoVgisu. 
\num{2} (hesaru \mo vanunx) savxMtakAkxgi baLasu, upayoVgisu. \eng{may I use your telephone?} nimamx dUravANiyanunx upayoVgisaleV? 
\num{3} vayxvaharisu; kANu; vatiRsu; naDeduko; naDesiko: \eng{has used me shamefully} avanu nananx bagegx avamAnakaravAgi vatiRsidAdxne. 
\num{4} (Iga \sA\ \BU\ mAtarx, \ucAcx\ yUsfTx). padadhxtiyAgiru; rUDhiyAgiru; vADikeyAgiru; aBAyxsavAgiru: \eng{I used to take the bus} nAnu basisxnalilx hoVguva rUDhi itutx. \eng{there used to be a law against it} adara virudadhxvAgi oMdu kAnUnu vADikeyalilxtutx. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{could use} (\AmA) saMtoVSadiMda iTuTxko; parxyoVjana paDeduko. 
\num{2} \eng{use force} balaparxyoVga mADu. 
\num{3} \eng{use a person's name} obabxna hesaranunx baLasiko, ulelxVKisu; obabxna hesarina parxyoVjana paDeduko.
\numi{4} \eng{use up} 
\banum
\alnum{a} (elalxvanUnx) KacuR mADibiDu; pUreYsibiDu. 
\alnum{b} (uLidiruvudanunx) upayoVgisibiDu; baLasibiDu. 
\alnum{c} (ati sharxma \mo vugaLiMda) susutx mADibiDu: \eng{he has used up all his strength} avanu tanenxlalx shakitxyanUnx kaLedukoMDu susAtxgidAdxne. 
\eanum
\numie
\enum
\emng
\eentry

\bentry
\word{used} 
\pron{yUsfDx} 
\gl{\gu}
\bmng
\bnum
\num{1} baLasida; upayoVgisida. 
\num{2} KacuRmADida; viniyoVgisida. 
\num{3} (baTeTxbare, vAhana, \mo vugaLa \vi) haLeya; inonxbabxru upayoVgisi biTaTx; haLatAda; sameda; eraDaneya mAliVkana keYge baMda. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{used to} padadhxtiyAda; vADikeyAda; aBAyxsavAda: \eng{I am not used to being called a liar} suLuLxgAra enisikoLuLxvudu nanage aBAyxsavilalx (nananxnunx Ivarege yArU `suLaLx' eMdilalx). \eng{not used to hardwork} kaSaTxpaTuTx kelasa mADi rUDhiyilalx. 
\emng
\eentry

\bentry
\word{useful} 
\pron{yUsfphulf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} upayoVgakaravAda; parxyoVjanakAriyAda. 
\num{2} PaladAyakavAda; lABakaravAda; upayukatx: \eng{gave me some useful hints} nanage kelavu upayukatx sUcanegaLanunx koTaTx. 
\num{3} (\AmA) atuyxtatxma; BajaRri; samathaR: \eng{a useful performance} BajaRri ATa; sogasAda parxdashaRna. 
\enum
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\bnum
\num{1} \eng{make oneself useful} seVve salilxsu; parxyoVjanavAgu. 
\num{2} \eng{useful load} baLake (sAmAnugaLa) hore; vimAnavu tananx tUkada jatege koMDoyuyxva itara sAmAnugaLu. 
\enum
\emng
\eentry

\bentry
\word{usefully} 
\pron{yUsfphuli} 
\gl{\kirxvi}
\expl{}
\bmng
\bnum
\num{1} upayukatxvAgi; parxyoVjanakAriyAgi. 
\num{2} PalaparxdavAgi; lABakaravAgi. 
\enum
\emng
\eentry

\bentry
\word{usefulness} 
\pron{yUsfphulfnisf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} upayukatxte; parxyoVjanakAriyAgiruvudu. 
\num{2} lABadAyakate; PaladAyakavAgiruvike. 
\enum
\emng
\eentry

\bentry
\word{useless} 
\pron{yUsflisf} 
\gl{\gu}
\bmng
\bnum
\num{1} niSapxrXyoVjaka; vayxthaR; niSaPxla; kelasakekx bArada: \eng{useless erudition} vayxthaR pAMDitayx. 
\num{2} (\AmA) asamathaR; dubaRla: \eng{useless in swimming} Ijinalilx dubaRla. 
\enum
\emng
\eentry

\bentry
\word{uselessly} 
\pron{yUsflisfli} 
\gl{\kirxvi}
\expl{}
\bmng
niSapxrXyoVjakavAgi; vayxthaRvAgi; niSaPxlavAgi; kelasakekx bArada hAge. 
\emng
\eentry

\bentry
\word{uselessness} 
\pron{yUsflisfnisf} 
\gl{\nA}
\expl{}
\bmng
niSapxrXyoVjakate; nirupayukatxte; vayxthaRvAguvudu; niSaPxlate. 
\emng
\eentry

\bentry
\word{user} 
\pron{yUsaZrf} 
\gl{\nA}
\bmng
\bnum
\num{1} baLakedAra; upayoVgisuvavanu; baLasuvavanu. 
\num{2} (\AmA) (apiVmu, gAMjA, \mo) mAdakavasutxgaLanunx baLasuvavanu; apiVmu -koVra, vayxsani. 
\num{3} (\nAyxshA) (hakukx \mo vugaLa) niraMtara anuBoVga; mAmUlu. 
\enum
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\eng{right of user} 
\banum
\alnum{a} upayoVgada hakukx; anuBoVgada hakukx. 
\alnum{b} mAmUlina hakukx. 
\eanum
\emng
\eentry

\bentry
\word{user-friendly} 
\pron{yUsaZrfpherxMDfli} 
\gl{\gu}
\expl{}
\bmng
(\kanmu\ kaMpUyxTarina \vi) baLakedAra mitarxnAgiruva; baLasalu anukUlavAgiruvaMte, sulaBavAgiruvaMte rUpisida. 
\emng
\eentry

\bentry
\word[usher(1)]{usher} 
\pron{aSarf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} dAvxrAdhikAri; (nAyxyAlaya, AsAthxna, \mo\ kaDe) dAvxrapAlakana kelasa mADuva yA janaranunx avaravara sAthxnagaLige koMDoyuyxva vayxkitx. 
\num{2} (\birx) parxtiVhAri; dwvArika; meVlupadaviyavana muMdugaDe gwravAthaRvAgi naDeduhoVguva adhikAri yA seVvaka (= \hyperref{kandict_b.pdf}{B}{Black Rod}{Black Rod}.) 
\num{3} (\pArxparx\ yA \hA) adhiVna upAdhAyxya, sahoVpAdhAyxya. 
\enum
\emng
\eentry

\bentry
\word[usher(2)]{usher} 
\pron{aSarf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} dAvxrapAlakanAgu; (dAvxrapAlakanAgi janaranunx) sAvxgatisu, avaravara piVThagaLige koMDoyiyx; dAri toVrisu; nideRVshisu. 
\num{2} (\rUpa) Agamanavanunx -- sUcisu, sAru, GoVSisu: \eng{the change of government ushered in a period of prosperity} hosa sakARravu ADaLitakekx baMdu shuBoVdaya kAlavanunx sAritu. 
\enum
\emng
\eentry

\bentry
\word{usherette} 
\pron{aSareTf} 
\gl{\nA}
\expl{}
\bmng
(\kanmu\ sinimA maMdiragaLalilx) kiriya dAvxraseVvaki; dAvxrapAlaki. 
\emng
\eentry

\bentry
\word{ushership} 
\pron{aSarfSipf} 
\gl{\nA}
\expl{}
\bmng
dAvxrAdhikAri kelasa; parxtiVhAri padavi; dAvxrapAlakatavx. 
\emng
\eentry

\bentry
\wordnospeech{USM}{USM}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
(sATxkf ekfsxceVMjf) \eng{Unlisted Securities Market.} 
\emng
\eentry

\bentry
\wordnospeech{USN}{USN}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United States Navy.} 
\emng
\eentry

\bentry
\word{usquebaugh} 
\pron{asfkivxboV} 
\gl{\nA}
\expl{}
\bmng
(ailaRMDf matutx sAkxTalxMDf) visikx. 
\emng
\eentry

\bentry
\wordnospeech{USS}{USS}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{United States Ship.} 
\emng
\eentry

\bentry
\wordnospeech{USSR}{USSR}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{Union of Soviet Socialist Republics.} 
\emng
\eentry

\bentry
\word{usual} 
\pron{yUSuZalf} 
\gl{\gu}
\bmng
vADikeya; mAmUlina; eMdina hAgiruva; rUDhiya; yathAparxkArada; sAmAnayxvAda: \eng{asked the usual questions} mAmUlina parxshenxgaLanunx keVLida. 
\emng

\noindent
\gl{\pagu}
\expl{}
\bmng
\hypertarget{usual pagu1}{} 
\bnum
\num{1} \eng{as usual} yathAparxkAra; eMdina hAge. 
\num{2} \eng{as per usual} (\gArx\ matutx \hA) = \hyperlink{usual pagu1}{?pagu? \((1)\)}. 
\num{3} \eng{the} (\engit{or} \eng{my) usual} yathAparxkArada: \eng{the usual drink} mAmUlina pAniVya. 
\enum
\emng
\eentry

\bentry
\word{usually} 
\pron{yUSuZali} 
\gl{\kirxvi}
\expl{}
\bmng
vADikeyaMte; mAmUlinaMte; eMdinaMte; rUDhiyaMte; yathAparxkAra. 
\emng
\eentry

\bentry
\word{usualness} 
\pron{yUSuZalfnisf} 
\gl{\nA}
\expl{}
\bmng
vADikeyaMtiruvike; mAmUlinaMtiruvudu; rUDhiya hAgiruvike; yathA parxkArada sithxti. 
\emng
\eentry

\bentry
\word{usucapion} 
\pron{yUsUZyxkeVpianf} 
\gl{\nA}
\expl{}
\bmng
= \hyperlink{usucaption}{usucaption}. 
\emng
\eentry

\bentry
\word{usucaption} 
\pron{yUsUZyxkAYxpaSxnf} 
\gl{\nA}
\expl{}
\bmng
(\nAyxshA) anuBavada hakukx; niraMtaravAda anuBoVgadiMda laBisuva Asitxya hakukx. 
\emng
\eentry

\bentry
\word[usufruct(1)]{usufruct} 
\pron{yUsUZyxpharxkfTx} 
\gl{\nA}
\expl{}
\bmng
PalAnuBavada hakukx; obabxna Asitxyanunx savxlapxvU hALumADade yA karagisade anuBoVgisuva hakukx. 
\emng
\eentry

\bentry
\word[usufruct(2)]{usufruct} 
\pron{yUsUZyxParxkfTx} 
\gl{\sakirx}
\expl{}
\bmng
PalAnuBavada hakukx paDe. 
\emng
\eentry

\bentry
\word[usufructuary(1)]{usufructuary} 
\pron{yUsUZyxpharxkuTxyXari} 
\gl{\gu}
\expl{}
\bmng
PalAnuBavada hakikxge saMbaMdhisida; PalAnuBavada hakikxna. 
\emng
\eentry

\bentry
\word[usufructuary(2)]{usufructuary} 
\pron{yUsUZyxpharxkuTxyXari} 
\gl{\nA}
\expl{}
\bmng
PalAnuBavada hakukxdAra. 
\emng
\eentry

\bentry
\word{usurer} 
\pron{yUSaZrarf} 
\gl{\nA}
\expl{}
\bmng
kusiVdaka; dubAri baDiDx (hAkuva) sAladAta. 
\emng
\eentry

\bentry
\word{usurious} 
\pron{yusuZariasf} 
\gl{\gu}
\expl{}
\bmng
dubAri baDiDxya; dubAri baDiDxyanunx hAkuva; dubAri baDiDxge sAla koDuva. 
\emng
\eentry

\bentry
\word{usuriously} 
\pron{yusuZariasfli} 
\gl{\kirxvi}
\expl{}
\bmng
dubAri baDiDxyanunx hAki. 
\emng
\eentry

\bentry
\word{usurp} 
\pron{yusaZpfR} 
\gl{\akirx}
\expl{}
\bmng
(padavi, siMhAsana, adhikAra, Asitx, \mo vanunx hakikxlalxde, anAyxyavAgi) kitutxko; Akarxmisiko; durAkarxmaNa mADu. 
\emng

\noindent 
\gl{\akirx}
\expl{}
\bmng
atikarxmisu. 
\emng
\eentry

\bentry
\word{usurpation} 
\pron{yusaZpeRVSanf} 
\gl{\nA}
\expl{}
\bmng
durAkarxmaNa. 
\emng
\eentry

\bentry
\word{usurper} 
\pron{yusaZpaRrf} 
\gl{\nA}
\expl{}
\bmng
apahAraka; durAkarxmaNa mADuvavanu. 
\emng
\eentry

\bentry
\word{usury} 
\pron{yUSaZri} 
\gl{\nA}
\bmng
\bnum
\num{1} kusiVda padadhxti; (kAnUninalilx avakAshaviruvudakikxMta) dubAri baDiDxge sAla koDuvudu. 
\num{2} dubAri baDiDx. 
\num{3} (\sA\ \rUpa) cuMgaDi; baDiDx: \eng{the service was repaid with usury} seVveya parxtiPalavanunx baDiDxsameVta hiMtirugisalAyitu. 
\enum
\emng
\eentry


\bentry
\wordnospeech{UT}{UT}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\bnum
\num{1} \eng{universal time.} 
\num{2} (\ame) \eng{Utah} (aMceyalilx baLake). 
\enum
\emng
\eentry

\bentry
\word{ute} 
\pron{yUTf} 
\gl{\nA}
\expl{}
\bmng
(\AseTxrXV\ matutx nUyxsiZVlaMDf) (\ashi) savoRVpayoVgi vAhana; nAnA kelasagaLige odaguva vAhana. 
\emng
\eentry

\bentry
\word{utensil} 
\pron{yUTinfsalf} 
\gl{\nA}
\expl{}
\bmng
(mane kelasada) pAterx paraDi; upakaraNa: \eng{cooking utensils} aDigeya pAterxparaDi. 
\emng
\eentry

\bentry
\word{uterine} 
\pron{yUTareY(ri)nf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} gaBARshayada; gaBARshayakekx saMbaMdhisida. 
\num{2} EkamAtaqka; samAnamAtaqka; taMde beVreyAgidudx, oMdeV tAyalilx huTiTxda: \eng{brother uterine} samAna mAtaqka soVdara. 
\enum
\emng
\eentry

\bentry
\word{uteritis} 
\pron{yUTareYTisf} 
\gl{\nA}
\expl{}
\bmng
gaBaRkoVshada uriyUta. 
\emng
\eentry

\bentry
\word{uterus} 
\pron{yUTarasf} 
\gl{\nA}
\expl{(\bava\ \eng{uteri} \ucAcx-- yUTareY). }
\bmng
gaBaRkoVsha; gaBARshaya. 
\emng
\eentry

\bentry
\word{utile} 
\pron{yUTeYlf} 
\gl{\gu}
\bmng
parxyoVjanakekx baruva; upayukatx; upayoVgisabahudAda. 
\emng
\eentry

\bentry
\word{utilise} 
\pron{yUTileYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
= \hyperlink{utilize}{utilize}. 
\emng
\eentry

\bentry
\word[utilitarian(1)]{utilitarian} 
\pron{yUTiliTeVrianf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (AkaSaRkavAgiruvudakikxMtalU) udedxVsha sAdhanege anukUlavAda; upayukatxvAda; parxyoVjanAtamxka. 
\num{2} parxyoVjana tatatxvXda. 
\num{3} parxyoVjakatA vAdada. 
\enum
\emng
\eentry

\bentry
\word[utilitarian(2)]{utilitarian} 
\pron{yUTiliTeVrianf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} parxyoVjana vAdi. 
\num{2} parxyoVjakatA vAdi. 
\enum
\emng
\eentry

\bentry
\word{utilitarianism} 
\pron{yUTiliTeVrianisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} parxyoVjana tatatxvX; kirxyegaLu parxyoVjanakaravAgidadxre yA bahujanara hitAthaRvAgidadxre mAtarx sari eMba tatatxvX; bahujana suKaveV neYtika jiVvanada guriyAgirabeVkeMba vAda. 
\num{2} parxyoVjakatAvAda; adhikasaMKeyxya janara adhika hitasAdhaneyeV dhamaR eMba vAda. 
\enum
\emng
\eentry

\bentry
\word[utility(1)]{utility} 
\pron{yUTiliTi} 
\gl{\nA}
\expl{(\bava\ \eng{utilities}).}
\bmng
\bnum
\num{1} parxyoVjanakarate; upayukatxte. 
\num{2} parxyoVjana; upayukatxvAdudu; parxyoVjanakAriyAdudu. 
\num{3} sAvaRjanika hita; sAvaRjanika upayoVga. 
\enum
\emng
\eentry

\bentry
\word[utility(2)]{utility} 
\pron{yUTiliTi} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} upayoVgakekx baruva; kelasakekx upayoVgavAguva. 
\num{2} jiVvanoVpayoVgi; nitayxjiVvanakekx atayxMta agatayxvAda, upayukatxvAda.
\num{3} nitoyxVpayukatx matutx parxmANAnusAra tayArisida: \eng{utility cloth} sATxYxMDaDfR baTeTx. \eng{utility furniture} nitoyxVpayukatx piVThoVpasakxragaLu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{utility man}{utility man}
\pron{?}
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} (nATaka) cilalxre naTa; saNaNxpAtarxgaLa naTa. 
\num{2} (kAlecxMDATa) vividha sAthxnagaLalilx niMtu ADabalalx ATagAra. 
\num{3} cilalxre kelasagAra; saNaNxpuTaTx kelasa mADuvavanu. 
\enum
\emng
\eentry

\bentry
\wordnospeech{utility room}{utility room}
\pron{?}
\gl{\nA}
\expl{}
\bmng
gaqhoVpayoVgi koVNe; baTeTx ogeyuva yaMtarx \mo\ manegelasada sAdhana salakaraNegaLanunx sajujxgoLisida koVNe. 
\emng
\eentry

\bentry
\wordnospeech{utility truck}{utility truck}
\pron{?}
\gl{\nA}
\expl{}
\bmng
= \hyperlink{utility vehicle}{utility vehicle}. 
\emng
\eentry

\bentry
\wordnospeech{utility vehicle}{utility vehicle}
\pron{?}
\gl{\nA}
\expl{}
\bmng
savoRVpayoVgi vAhana; nAnA kelasagaLige odaguva vAhana. 
\emng
\eentry

\bentry
\word{utilizable} 
\pron{yUTileYsaZbflf} 
\gl{\gu}
\expl{}
\bmng
upayoVgisikoLaLxbalalx; parxyoVjanakekx baruva; baLasabalalx. 
\emng
\eentry

\bentry
\word{utilization} 
\pron{yUTileYseZVSanf} 
\gl{\nA}
\expl{}
\bmng
baLake; upayoVga. 
\emng
\eentry

\bentry
\word{utilize} 
\pron{yUTileYsfZ} 
\gl{\sakirx}
\expl{}
\bmng
baLasu; upayoVgisu; parxyoVjanakekx taru. 
\emng
\eentry

\bentry
\word{utilizer} 
\pron{yUTileYsaZrf} 
\gl{\nA}
\expl{}
\bmng
baLakedAra; parxyoVjanakekx taMdukoLuLxvava; upayoVgisikoLuLxvava. 
\emng
\eentry

\bentry
\wordwithhyphen{hyp-ution}{-ution} 
\pron{-yUSanf, -USanf} 
\gl{\uparx}
\expl{}
\bmng
nAmapadagaLanunx racisuvalilx baLasuva \uparx, \udA\ \eng{solution}. 
\emng
\eentry

\bentry 
\wordRemoveSpace{uti-possidetis}{uti possidetis}
\pron{UTi pAsiDiVTisf} 
\gl{\nA}
\expl{\Latin }
\bmng
yudadhxrASaTxrXgaLu yudadhxdalilx gedidxruva deVsha, Asitx, \mo vugaLanunx tamamx sAvxdhiVnadalelxV uLisikoLuLxva tatatxvX. 
\emng
\eentry

\bentry
\word[utmost(1)]{utmost} 
\pron{aTfmoVsfTx} 
\gl{\gu}
\expl{}
\bmng
atayxMta; kaTaTxkaDeya; atayxdhika; paramAvadhiya: \eng{the utmost limits} kaTaTxkaDeya miti. \eng{utmost reluctance} savxlapxvU iSaTxvilalxdiruvike. 
\emng
\eentry

\bentry
\word[utmost(2)]{utmost} 
\pron{aTfmoVsfTx} 
\gl{\nA}
\expl{}
\bmng
parAkASeThx; gariSaThx miti; paramAvadhi; atayxMta hecicxna parxmANa. 
\emng

\noindent 
\gl{\pagu}
\expl{}
\bmng
\eng{do one's utmost} keYlAdadedxlAlx mADu; mitimiVri sharxmisu. 
\emng
\eentry

\bentry
\word{Utopia} 
\pron{yUToVpia} 
\gl{\nA}
\bmng
\bnum
\num{1} AdashaR rAjayx; rAmarAjayx. 
\num{2} AdashaR sithxti. 
\enum
\emng
\eentry

\bentry
\word[Utopian(1)]{Utopian} 
\pron{yUToVpianf} 
\gl{\gu}
\expl{}
\bmng
(\eng{utopian} saha). 
\bnum
\num{1} AdashaR rAjayxda yA rAjayxkekx seVrida. 
\num{2} AdashaR sithxtiya. 
\enum
\emng
\eentry

\bentry
\word[Utopian(2)]{Utopian} 
\pron{yUToVpianf} 
\gl{\nA}
\expl{}
\bmng
(\eng{utopian} saha).
\bnum
\num{1} AdashaR rAjayxvAdi. 
\num{2} AdashaRvAdi sudhAraka; AdashaRdaqSiTxya sudhAraNAvAdi. 
\enum
\emng
\eentry

\bentry
\word{Utopianism} 
\pron{yUToVpianisaZmf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} AdashaRrAjayx kalapxne; rAmarAjayx kalapxne. 
\num{2} AdashaR rAjayxdalilx sharxdedhx, naMbike. 
\enum
\emng
\eentry

\bentry 
\word{utricle} 
\pron{yUTirxkflf} 
\gl{\nA}
\bmng
\bnum
\num{1} (pArxNiya yA sasayxda) jiVvakoVsha yA jiVvakaNa. 
\num{2} (shariVradalilxna, \kanmu\ oLakiviyalilxya) saNaNx ciVla; kirukoVsha. 
\enum
\emng
\eentry

\bentry
\word{utricular} 
\pron{yUTirxkuyxlarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} (pArxNiya yA sasayxda) jiVvakoVshada; jiVvakaNakekx saMbaMdhisida. 
\num{2} (oLakiviya) saNaNx ciVlada; kirukoVshada. 
\enum
\emng
\eentry

\bentry
\word[utter(1)]{utter} 
\pron{aTarf} 
\gl{\gu}
\expl{}
\bmng
(visheVSaNavAgi) saMpUNaR; tiVrA; atayxMta; kaDu; pUtiR: \eng{utter misery} daTaTxdAridarxyX; kaDukaSaTx. \eng{an utter denial} saMpUNaR nirAkaraNe. 
\emng
\eentry

\bentry
\word[utter(2)]{utter} 
\pron{aTarf} 
\gl{\sakirx}
\expl{}
\bmng
\bnum
\num{1} (suKa, duHKa, \mo vanunx) dhavxniya mUlaka -- sUcisu, vayxkatxpaDisu: \eng{utter a sigh} niTuTxsiru biDu. 
\num{2} (mAtinalilx, baravaNigeyalilx) heVLu; usuru; nuDi; ucacxrisu: \eng{the last words he uttered} avanu heVLida koneya mAtugaLu. 
\num{3} dhavxni mADu; kiricu; kUgu; moLagu: \eng{the engine uttered a shriek} eMjinunx kiVcalu dhavxnimADitu (gaTiTxyAgi kiricitu). 
\num{4} (\nAyxshA) KoVTA nANayx yA noVTu calAvaNe mADu. 
\enum
\emng
\eentry

\bentry
\word{utterance} 
\pron{aTaranfsx} 
\gl{\nA}
\bmng
\bnum
\num{1} heVLike; mAtinalilx vayxkatxpaDisuvudu, heVLuvudu. 
\num{2} vAkfshakitx; ucAcxraNe; mAtADuva shakitx. 
\num{3} nuDi; vacana; ukitx; BASaNa: \eng{his pulpit utterances} veVdikeya meVlina (avana) mAtugaLu, BASaNa. 
\num{4} mAtina riVti: \eng{his very utterance was spell- binding} avana mAtina riVtiyeV moVDihAkitutx. 
\num{5} (\BAshA) shabadxsaMtAna yA padasaMtAna; vAyxkaraNabadadhxvAda yAvudeV oMdu AMshika yA pUNaR GaTakavAgirabeVkAgirada, ucacxrita yA liKita padagaLa aviciCxnanx saraNi. 
\enum
\emng
\eentry

\bentry
\wordnospeech{utter barrister}{utter barrister}
\pron{?}
\gl{\nA}
\expl{}
\bmng
(\birx) (nAyxyasAthxnadalilx rAjasananxdu paDedavara piVThapaMkitxyiMda horagidudx vAdisuva) kiriya vakiVla. 
\emng
\eentry

\bentry
\word{utterly} 
\pron{aTarfli} 
\gl{\kirxvi}
\expl{}
\bmng
saMpUNaRvAgi; atayxMta; tiVrA; pUtAR; koneyadAgi. 
\emng
\eentry

\bentry
\word{uttermost} 
\pron{aTarfmoVsfTx} 
\gl{\gu}
\expl{}
\bmng
atayxMta dUrada yA dUradalilxya: \eng{the uttermost stars of the galaxy} kiSxVrapathadalilxna atayxMta dUrada nakaSxtarxgaLu. 
\emng
\eentry

\bentry
\word{utterness} 
\pron{aTarfnisf} 
\gl{\nA}
\expl{}
\bmng
pUtiRyAgiruvike; atayxMtavAgiruvike. 
\emng
\eentry

\bentry
\word{U-tube} 
\pron{yUTUyxbf} 
\gl{\nA}
\expl{}
\bmng
\eng{U} AkArada koLavi; `yU' koLave. 
\emng
\eentry

\bentry
\word{U-turn} 
\pron{yUTanfR} 
\gl{\nA}
\expl{}
\bmng
\banum
\alnum{a} `yU' tiruvu; (vAhana itAyxdigaLu) virudadhx dikikxge tiruguvaMte \eng{`U'} pathadalilx tiruguvudu. 
\alnum{b} (niVti, tatatxvX, \mo vugaLa \vi) saMpUNaR -- badalAvaNe, tirugumurugu yA radidxyAti. 
\eanum
\emng
\eentry

\bentry 
\wordnospeech{UV}{UV}
\pron{?}
\gl{\saMkiSx}
\expl{}
\bmng
\eng{ultraviolet.} 
\emng
\eentry

\bentry
\word{uvea} 
\pron{yUvia} 
\gl{\nA}
\expl{}
\bmng
(\aMrashA) yUviya; kaNuNxpApeya muMBAgadalilxruva vaNaRdarxvayx pUrita satxra. 
\emng
\eentry

\bentry
\word{uvula} 
\pron{yUvuyxl} 
\gl{\nA}
\expl{(\bava\ \eng{uvulae}, \ucAcx-- yUvuyxliV).}
\bmng
\bnum
\num{1} kirunAlage; alijihevx. 
\num{2} mUtarxkoVshadalilxya yA upamasitxSakxdalilxya toMgalu. 
\enum
\emng
\eentry

\bentry
\word[uvular(1)]{uvular} 
\pron{yUvuyxlarf} 
\gl{\gu}
\expl{}
\bmng
\bnum
\num{1} kirunAlageya; alijihevxya. 
\num{2} nAlageya hiMBAgadiMda matutx kirunAlageyiMda ucacxrisida, \udA\ pherxMcfna \engit{r}. 
\enum
\emng
\eentry

\bentry
\word[uvular(2)]{uvular} 
\pron{yUvuyxlarf} 
\gl{\nA}
\expl{}
\bmng
alijihivxVya vayxMjana. 
\emng
\eentry

\bentry
\word{uxorial} 
\pron{akosxVrialf} 
\gl{\gu}
\expl{}
\bmng
heMDatiya; heMDatige sahajavAda; patinxVyoVgayxvAda; heMDatige opupxvaMtha; maDadige takakx. 
\emng
\eentry

\bentry
\word{uxoricidal} 
\pron{akosxVriseYDalf} 
\gl{\gu}
\expl{}
\bmng
heMDatiyanunx kolulxva parxvaqtitxya yA kolulxvudakekx saMbaMdhisida; heMDati haMtaka; patinxVhaMtaka. 
\emng
\eentry

\bentry
\word{uxoricide} 
\pron{akosxVriseYDf} 
\gl{\nA}
\expl{}
\bmng
\bnum
\num{1} heMDatiyanunx kolulxvudu; heMDati kole; patinxV hateyx. 
\num{2} patinxVhaMtaka; heMDatiya kolegAra; heMDatiyanunx kolulxvavanu. 
\enum
\emng
\eentry

\bentry
\word{uxorious}
\pron{akosxVriasf}
\gl{\gu}
\bmng
\bnum
\num{1} BAyARsakatx; heMDati vAyxmoVhada; tananx heMDatiyalilx atayxMta vAyxmoVhavuLaLx. 
\num{2} BAyARsakitx toVrisuva. 
\num{3} patinxVdAsanAda; heMDatiya gulAmanAda. 
\enum
\emng
\eentry

\bentry
\word{uxoriously}
\pron{akosxVriasfli}
\gl{\kirxvi}
\bmng
\bnum
\num{1} BAyARsakatxnAgi; heMDati vAyxmoVhadiMda. 
\num{2} heMDatiya gulAmanAgi; patinxVdAsanAgi. 
\enum
\emng
\eentry

\bentry
\word{uxoriousness}
\pron{akosxVriasfnisf}
\gl{\nA}
\bmng
\bnum
\num{1} BAyARsakitx; heMDati vAyxmoVha. 
\num{2} heMDatiya gulAmagiri; patinxVsharaNayxte. 
\enum
\emng
\eentry

\bentry
\word{Uzbeg}
\pron{a(u)sfZbegf}
\gl{\nA}
\bmng
= \hyperlink{Uzbek}{Uzbek}. 
\emng
\eentry

\bentry
\word{Uzbek}
\pron{a(u)asfZbekf}
\gl{\nA}
\bmng
usfZbekf: 
\banum
\alnum{a} soVviyatf raSayxda, usfZbekisAtxnada oMdu TakiRSf jAtiyavanu. 
\alnum{b} I janara BASe. 
\eanum
\emng
\eentry


%\newpage
%\input figs_list
